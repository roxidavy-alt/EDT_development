...
// Данные заполнения разборки на основании сборки.
// 
// Параметры:
...
Функция ДанныеЗаполненияРазборкиНаОснованииСборки(Источник) Экспорт
	
	Результат = Новый Структура("ТипПроизводственногоПроцесса", Перечисления.ТипыПроизводственныхПроцессов.Разборка);
	
	Если ЗначениеЗаполнено(Источник) Тогда
		
		Объект = Источник.ПолучитьОбъект();
		
		Результат.Вставить("Наименование", СтрШаблон(НСтр("ru = '%1 (Разборка)';
															|en = '%1 (Disassembly)'"), Объект.Наименование));
		Результат.Вставить("Родитель", Объект.Родитель);
		...
		Для каждого КлючИЗначение Из Результат Цикл
			Если ТипЗнч(КлючИЗначение.Значение) = Тип("ТаблицаЗначений") Тогда
				Результат.Вставить(КлючИЗначение.Ключ, ПоместитьВоВременноеХранилище(КлючИЗначение.Значение));
			КонецЕсли;
		КонецЦикла;
...
Функция ЗаполнитьЭтапыПоРесурснойСпецификации(Приемник, Источник, СсылкаПриемника)
	...
	|	*
	|ИЗ
	|	Справочник.РесурсныеСпецификации.ВыполняемыеРаботы КАК ТабличнаяЧасть
	|ГДЕ
	|	ТабличнаяЧасть.Ссылка = &Источник
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Приемник.ВыполняемыеРаботы.Загрузить(Результат[7].Выгрузить());
	//-- НЕ УТКА
	...
	Для ИндексЭтап = 0 По ТаблицаЭтапов.Количество() - 1 Цикл
		...
		ВыполнитьЗаменуСсылокВКоллекции(Приемник.ВыполняемыеРаботы, "Этап", СтрокаЭтап.Ссылка, НовыйЭтапСсылка);
