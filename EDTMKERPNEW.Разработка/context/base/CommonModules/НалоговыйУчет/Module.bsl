...
//  ЭтоПерерасчет - Булево - Это перерасчет
// 
// Возвращаемое значение:
//  Число - Выручка от реализации
Функция ВыручкаОтРеализации(Период, Организация, МенеджерВременныхТаблиц = Неопределено, ЭтоПерерасчет = Ложь) Экспорт
	...
	Если МенеджерВременныхТаблиц.Таблицы.Найти("ПорядокОтраженияНаСчетахУчетаПоПериодуДействия") = Неопределено Тогда
		МассивВидовСчетов = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("ВыручкаОтПродаж");
		ТекстЗапроса = РегистрыСведений.ПорядокОтраженияНаСчетахУчета.ТекстВременнойТаблицыПорядкаОтраженияПоПериодуДействия(МассивВидовСчетов);
		МассивТекстовЗапроса.Добавить(ТекстЗапроса);
	КонецЕсли;
	
	Если МенеджерВременныхТаблиц.Таблицы.Найти("Организации") = Неопределено Тогда
	...
	Если МенеджерВременныхТаблиц.Таблицы.Найти("СтатьиПрочихДоходов") = Неопределено Тогда 
		Запрос.Текст = "ВЫБРАТЬ
		...
		|	И НЕ Хозрасчетный.Ссылка = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ВыручкаЕНВД)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Счет
		|;
...
Функция ПолучитьТаблицуВидовАктивовИОбязательств() Экспорт
	...
	НоваяСтрока = ТаблицаВидовАктивовИОбязательств.Добавить();
	НоваяСтрока.ВидАктивовОбязательств = Перечисления.ВидыАктивовИОбязательств.НеотфактурованныеПоставки;
	НоваяСтрока.Счета.Добавить(ПланыСчетов.Хозрасчетный.ОбязательстваПоНеотфактурованнымПоставкам);
	НоваяСтрока.Субконто.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты);
	НоваяСтрока.Субконто.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры);
	
	// Права на возмещение НДС
	НоваяСтрока = ТаблицаВидовАктивовИОбязательств.Добавить();
	НоваяСтрока.ВидАктивовОбязательств = Перечисления.ВидыАктивовИОбязательств.ПраваНаВозмещениеНДС;
	НоваяСтрока.Счета.Добавить(ПланыСчетов.Хозрасчетный.НДСпоНеотфактурованнымПоставкам);
	НоваяСтрока.Субконто.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты);
...
//		Организация - СправочникСсылка.Организации
//		ПротоколНормируемыеРасходы - ТаблицаЗначений - пустая, содержит колонки регистра сведений "РасчетНормированияРасходов".
//		ТранспортныеРасходы - Булево - учитывать или нет транспортные расходы при определении долей.
//		МенеджерВременныхТаблиц - МенеджерВременныхТаблиц .
//		ЭтоПерерасчет - Булево - учитывать или нет транспортные расходы при определении долей.
//
//	Возвращаемое значение:
//		Структура - инициализируется в начале функции.
//
Функция ДолиСписанияРасходов(Период, Организация, ПротоколНормируемыеРасходы, ТранспортныеРасходы, МенеджерВременныхТаблиц = Неопределено, ЭтоПерерасчет = Ложь) Экспорт
	...
	Иначе
		...
		Ограничения = ДолиСписанияНормируемыхРасходов(Период, Организация, ПротоколНормируемыеРасходы, МенеджерВременныхТаблиц, ЭтоПерерасчет);
		Для Каждого ОписаниеПоля Из РегистрыСведений.ДолиСписанияКосвенныхРасходов.ИменаВидовНормируемыхРасходов() Цикл
			Доля = Ограничения[Перечисления.ВидыРасходовНУ[ОписаниеПоля.Значение]];
			Если ЗначениеЗаполнено(Доля) Тогда
				Результат[ОписаниеПоля.Ключ] = Доля;
			КонецЕсли;
		КонецЦикла;
...
// Счета учета расходов с детализацией
// 
// Возвращаемое значение:
//  Массив из ПланСчетовСсылка.Хозрасчетный - Счета учета расходов с детализацией
Функция СчетаУчетаРасходовСДетализацией() Экспорт
	
	СчетаЗатрат = Новый Массив;
	
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаЗатрат, 
		РеглУчетВыборкиСерверПовтИсп.СчетаРасходыОсновногоПроизводстваКРаспределению());
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаЗатрат, 
		РеглУчетВыборкиСерверПовтИсп.СчетаПроизводственныеЗатратыОсновногоПроизводства());
	
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаЗатрат, 
		РеглУчетВыборкиСерверПовтИсп.СчетаРасходыВспомогательныхПроизводствКРаспределению());
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаЗатрат, 
		РеглУчетВыборкиСерверПовтИсп.СчетаПроизводственныеЗатратыВспомогательныхПроизводств());

	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаЗатрат, 
		РеглУчетВыборкиСерверПовтИсп.СчетаРасходыОбслуживающихПроизводствКРаспределению());
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаЗатрат, 
		РеглУчетВыборкиСерверПовтИсп.СчетаПроизводственныеЗатратыОбслуживающихПроизводств());
...
Процедура ДобавитьЭтап_ОсвобождениеОтУплатыНалоговВСвязиСКоронавирусом(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ОсвобождениеОтУплатыНалоговВСвязиСКоронавирусом,
		Ложь, Истина, Ложь);
		
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Освобождение от уплаты налогов в связи с коронавирусом';
											|en = 'Tax exemption due to coronavirus'"; // @НСтр-1
...
Процедура ДобавитьЭтап_ПереходНаУчетКурсовыхРазницНУПо67ФЗ(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ПереходНаУчетКурсовыхРазницНУПо67ФЗ, Ложь, Истина, Ложь);
		
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Переход на учет курсовых разниц НУ по 67-ФЗ';
											|en = 'Transfer to TA exchange difference accounting under 67-FZ'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
...
// 	Отказ - Булево
// 	МенеджерВременныхТаблиц - МенеджерВременныхТаблиц
// 	ДолиСписанияКосвенныхРасходов - см. ДолиСписанияРасходов()
// 	
// Возвращаемое значение:
...
// * СуммаПР - Число -
// * СуммаВР - Число -
Функция ПолучитьРасчетСписанияКосвенныхРасходов(КонтекстРасчета, Отказ, МенеджерВременныхТаблиц = Неопределено, ДолиСписанияКосвенныхРасходов = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("НачалоПериода",           КонтекстРасчета.НачДата);
	Запрос.УстановитьПараметр("КонецПредыдущегоМесяца",  КонтекстРасчета.НачГраница);
	Запрос.УстановитьПараметр("Организация",             КонтекстРасчета.Организация);
	Запрос.УстановитьПараметр("НачалоОтчетногоПериода",  НачалоНалоговогоПериода(КонтекстРасчета.КонДата, КонтекстРасчета.Организация));
	...
	Если МенеджерВременныхТаблиц.Таблицы.Найти("Организации") = Неопределено Тогда
		...
		|	Справочник.Организации КАК Организации
		|ГДЕ
		|	&НеЭтоПерерасчет И Организации.ГоловнаяОрганизация = &Организация
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
	...
	СоздатьВременнуюТаблицуДолиСписанияКосвенныхРасходов(
		...
		КонтекстРасчета.НаборКорректировок,
	...
	|	ВложенныйЗапрос.СчетНормируемыхРасходов    КАК СчетНормируемыхРасходов,
	|	ВложенныйЗапрос.Организация                КАК Организация,
	|	ВложенныйЗапрос.Подразделение              КАК Подразделение,
	|	ВложенныйЗапрос.НаправлениеДеятельности    КАК НаправлениеДеятельности,
	|	СУММА(ВложенныйЗапрос.СуммаНУОстаток)      КАК СуммаНУОстаток,
	|	СУММА(ВложенныйЗапрос.СуммаВРОстаток)      КАК СуммаВРОстаток,
	|	СУММА(ВложенныйЗапрос.СуммаРанееПринятая)  КАК СуммаРанееПринятая,
	|	СУММА(ВложенныйЗапрос.СуммаЗакрытияНМР)    КАК СуммаЗакрытияНМР
	|ПОМЕСТИТЬ ТаблицаДвиженийПредварительная
	|ИЗ
	|	(ВЫБРАТЬ
	|		&КонецПериода КАК Период,
	|		Остатки.Субконто1 КАК СтатьяРасходов,
	...
	|		0 КАК СуммаРанееПринятая,
	|		0 КАК СуммаЗакрытияНМР
	|	ИЗ
	|		РегистрБухгалтерии.Хозрасчетный.Остатки(
	...
	|						Организации.Организация
	|					ИЗ
	|						Организации)) КАК Остатки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		&КонецПериода КАК Период,
	|		ДанныеПроводок.СтатьяРасходов КАК СтатьяРасходов,
	|		ДанныеПроводок.СчетКт КАК СчетНормируемыхРасходов,
	|		ДанныеПроводок.Организация КАК Организация,
	|		ДанныеПроводок.Подразделение КАК Подразделение,
	|		ДанныеПроводок.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|		- ДанныеПроводок.СуммаНУ КАК СуммаНУОстаток,
	...
	|	ГДЕ
	|		ДанныеПроводок.СчетКт = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НормируемыеРасходы)
	|		И ДанныеПроводок.Период МЕЖДУ &НачалоОтчетногоПериода И &КонецПериода
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		&КонецПериода КАК Период,
	|		ДанныеОборотов.Субконто1 КАК СтатьяРасходов,
	|		ДанныеОборотов.Счет КАК СчетНормируемыхРасходов,
	|		ДанныеОборотов.Организация КАК Организация,
	|		ДанныеОборотов.Подразделение КАК Подразделение,
	|		ДанныеОборотов.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|		ДанныеОборотов.СуммаНУОборотДт КАК СуммаНУОстаток,
	|		ДанныеОборотов.СуммаВРОборотДт КАК СуммаВРОстаток,
	|		ВЫБОР
	|			КОГДА ДанныеОборотов.КорСчет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.УбытокПодлежащийПокрытию)
	|			ТОГДА 0
	|			ИНАЧЕ ДанныеОборотов.СуммаНУОборотКт
	|		КОНЕЦ КАК СуммаРанееПринятая,
	|		ВЫБОР
	|			КОГДА ДанныеОборотов.КорСчет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.УбытокПодлежащийПокрытию)
	|			ТОГДА ДанныеОборотов.СуммаНУОборотКт
	|			ИНАЧЕ 0
	|		КОНЕЦ КАК СуммаЗакрытияНМР
	|	ИЗ
	...
	|						Организации.Организация
	|					ИЗ
	|						Организации), , ) КАК ДанныеОборотов
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		КОНЕЦ КАК СуммаЗакрытияНМР
	|	ИЗ
	|		РегистрБухгалтерии.КорректировкиНалоговойБазы.Обороты(&НачалоПериода, &КонецПериода, ,
	|												Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НормируемыеРасходы),
	|												ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат),
	|				Организация В
	|					(ВЫБРАТЬ
	...
	|					ИЗ
	|						Организации)
	|				И (&НеЭтоПерерасчет ИЛИ НаборКорректировок В (ВЫБРАТЬ Т.НаборКорректировок ИЗ ВТНаборыКорректировок КАК Т)), , ) КАК ДанныеОборотов
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|						ИЗ
	|							Организации)
	|					И (&НеЭтоПерерасчет ИЛИ НаборКорректировок В (ВЫБРАТЬ Т.НаборКорректировок ИЗ ВТНаборыКорректировок КАК Т))) КАК Остатки) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.Подразделение,
	|	ВложенныйЗапрос.СтатьяРасходов,
	|	ВложенныйЗапрос.НаправлениеДеятельности,
	|	ВложенныйЗапрос.СчетНормируемыхРасходов,
	|	ВложенныйЗапрос.Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ДанныеПроводок
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Остатки.СчетНормируемыхРасходов КАК СчетНормируемыхРасходов,
	|	Остатки.Организация             КАК Организация,
	|	Остатки.Подразделение           КАК Подразделение,
	|	Остатки.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ДолиСписания.ДоляСписания       КАК ДоляСписания,
	|	Остатки.СуммаНУОстаток          КАК СуммаНУОстаток,
	|	Остатки.СуммаВРОстаток          КАК СуммаВРОстаток,
	|	Остатки.СуммаРанееПринятая      КАК СуммаРанееПринятая,
	|	Остатки.СуммаЗакрытияНМР        КАК СуммаЗакрытияНМР,
	|	ПорядокОтраженияНаСчетахУчетаПереопределяемый.СчетУчета КАК СчетСписанияОСНО
	|ПОМЕСТИТЬ ТаблицаДвижений
	|ИЗ
	|	ТаблицаДвиженийПредварительная КАК Остатки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтДолиСписанияКосвенныхРасходов КАК ДолиСписания
	|		ПО Остатки.СтатьяРасходов = ДолиСписания.СтатьяРасходов
	...
	|		ПО ПорядокОтраженияНаСчетахУчетаПереопределяемый.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.РасходыТекущегоПериода)
	...
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВтДолиСписанияКосвенныхРасходов
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ТаблицаДвиженийПредварительная
	|";
	
	РегистрыСведений.ПорядокОтраженияНаСчетахУчета.ПереопределитьВТекстеЗапросаПорядокОтраженияСчетаУчета(ТекстЗапроса, "КонецПериодаГраница");
	...
	ТекстЗапроса = 
		...
		|	ТаблицаДвижений.СуммаРанееПринятая  КАК СуммаРанееПринятая,
		|	ТаблицаДвижений.СуммаЗакрытияНМР    КАК СуммаЗакрытияНМР,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.СуммаНУОстаток = 0 И Выборка.СуммаРанееПринятая = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		СписаниеПоНУ = Окр(Выборка.СуммаНУОстаток * Выборка.ДоляСписания, 2) - Выборка.СуммаРанееПринятая;
		...
		Если СписаниеПоНУ <> 0 Тогда
			...
			Если КонтекстРасчета.ЭтоПерерасчет Тогда
				ОписаниеРасходов.СуммаПР = - СписаниеПоНУ;
			Иначе
		...
		ОстатокПоНУНаКонец = Выборка.СуммаНУОстаток - СписаниеПоНУ - Выборка.СуммаЗакрытияНМР - Выборка.СуммаРанееПринятая;
		...
		Если ЭтоКонецГода И (ОстатокПоНУНаКонец > 0 Или КонтекстРасчета.ЭтоПерерасчет) Тогда
...
// 	МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - 
// 	НаборКорректировок - СправочникСсылка.КорректировкиПрошлыхПериодовНУ - Набор корректировок, по которому производится перерасчет налога на прибыль
// 	ДолиСписанияКосвенныхРасходов - см. ДолиСписанияРасходов()
//
Процедура СоздатьВременнуюТаблицуДолиСписанияКосвенныхРасходов(Период, Организации, МенеджерВременныхТаблиц, НаборКорректировок = Неопределено, ДолиСписанияКосвенныхРасходов = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("НаборКорректировок",      ?(ЗначениеЗаполнено(НаборКорректировок), НаборКорректировок, Справочники.КорректировкиПрошлыхПериодовНУ.ПустаяСсылка())); 
	
	Если ДолиСписанияКосвенныхРасходов = Неопределено Тогда
		Запрос.УстановитьПараметр("ДоляРасходовНаРекламу", 0);
		Запрос.УстановитьПараметр("ДоляПредставительскихРасходов", 0);
		Запрос.УстановитьПараметр("ДоляРасходовНаДобровольноеМедицинскоеСтрахование", 0);
		Запрос.УстановитьПараметр("ДоляРасходовНаДобровольноеСтрахованиеЖизни", 0);
		Запрос.УстановитьПараметр("ДоляРасходовНаДобровольноеСтрахованиеОтНесчастныхСлучаев", 0);
		Запрос.УстановитьПараметр("ДоляРасходовНаВозмещениеПроцентовРаботникам", 0);
	Иначе
		Запрос.УстановитьПараметр("ДоляРасходовНаРекламу", ДолиСписанияКосвенныхРасходов.ДоляРасходовНаРекламу);
		Запрос.УстановитьПараметр("ДоляПредставительскихРасходов", ДолиСписанияКосвенныхРасходов.ДоляПредставительскихРасходов);
		Запрос.УстановитьПараметр("ДоляРасходовНаДобровольноеМедицинскоеСтрахование", ДолиСписанияКосвенныхРасходов.ДоляРасходовНаДобровольноеМедицинскоеСтрахование);
		Запрос.УстановитьПараметр("ДоляРасходовНаДобровольноеСтрахованиеЖизни", ДолиСписанияКосвенныхРасходов.ДоляРасходовНаДобровольноеСтрахованиеЖизни);
		Запрос.УстановитьПараметр("ДоляРасходовНаДобровольноеСтрахованиеОтНесчастныхСлучаев", ДолиСписанияКосвенныхРасходов.ДоляРасходовНаДобровольноеСтрахованиеОтНесчастныхСлучаев);
		Запрос.УстановитьПараметр("ДоляРасходовНаВозмещениеПроцентовРаботникам", ДолиСписанияКосвенныхРасходов.ДоляРасходовНаВозмещениеПроцентовРаботникам);
	КонецЕсли; 
	
	ЭтоПерерасчет = ЗначениеЗаполнено(НаборКорректировок);
	
	Если МенеджерВременныхТаблиц.Таблицы.Найти("втОтборОрганизации") = Неопределено Тогда
	...
	Запрос.Текст =
		...
		|			ТОГДА ЕСТЬNULL(ДолиСписания.ДоляРасходовНаРекламу, &ДоляРасходовНаРекламу)
		|		КОГДА СтатьиРасходов.ВидРасходов = ЗНАЧЕНИЕ(Перечисление.ВидыРасходовНУ.ПредставительскиеРасходы)
		|			ТОГДА ЕСТЬNULL(ДолиСписания.ДоляПредставительскихРасходов, &ДоляПредставительскихРасходов)
		|		КОГДА СтатьиРасходов.ВидРасходов = ЗНАЧЕНИЕ(Перечисление.ВидыРасходовНУ.ДобровольноеЛичноеСтрахование)
		|			ТОГДА ЕСТЬNULL(ДолиСписания.ДоляРасходовНаДобровольноеМедицинскоеСтрахование, &ДоляРасходовНаДобровольноеМедицинскоеСтрахование)
		...
		|			ТОГДА ЕСТЬNULL(ДолиСписания.ДоляРасходовНаДобровольноеСтрахованиеЖизни, &ДоляРасходовНаДобровольноеСтрахованиеЖизни)
		|		КОГДА СтатьиРасходов.ВидРасходов
		|			= ЗНАЧЕНИЕ(Перечисление.ВидыРасходовНУ.ДобровольноеЛичноеСтрахованиеНаСлучайСмертиИлиУтратыРаботоспособности)
		|			ТОГДА ЕСТЬNULL(ДолиСписания.ДоляРасходовНаДобровольноеСтрахованиеОтНесчастныхСлучаев, &ДоляРасходовНаДобровольноеСтрахованиеОтНесчастныхСлучаев)
		|		КОГДА СтатьиРасходов.ВидРасходов
		|			= ЗНАЧЕНИЕ(Перечисление.ВидыРасходовНУ.РасходыНаВозмещениеЗатратРаботниковПоУплатеПроцентов)
		|			ТОГДА ЕСТЬNULL(ДолиСписания.ДоляРасходовНаВозмещениеПроцентовРаботникам, &ДоляРасходовНаВозмещениеПроцентовРаботникам)
		|		ИНАЧЕ
		|			1
		...
		|			И ДолиСписания.НаборКорректировок = &НаборКорректировок
		|
...
Функция ДоляТранспортныхРасходовТекущегоМесяца(Период, Организация, МенеджерВременныхТаблиц = Неопределено) Экспорт
	...
	МассивТекстовЗапроса = Новый Массив;
	МассивВидовСчетов = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("НаСкладе");
	ТекстЗапроса = РегистрыСведений.ПорядокОтраженияНаСчетахУчета.ТекстВременнойТаблицыПорядкаОтраженияПоПериодуДействия(МассивВидовСчетов);
	МассивТекстовЗапроса.Добавить(ТекстЗапроса);
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПорядокОтраженияНаСчетахУчетаПоПериодуДействия КАК НастройкиПоМестуУчета
	...
	|			И Продажи.Период >= НастройкиПоМестуУчета.НачалоДействия
	|			И Продажи.Период < НастройкиПоМестуУчета.ОкончаниеДействия
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПорядокОтраженияНаСчетахУчетаПоПериодуДействия КАК НастройкиПоОрганизации
	...
	|			И Продажи.Период >= НастройкиПоОрганизации.НачалоДействия
	|			И Продажи.Период < НастройкиПоОрганизации.ОкончаниеДействия
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПорядокОтраженияНаСчетахУчетаПоПериодуДействия КАК НастройкиПоАналитике
	...
	|			И Продажи.Период >= НастройкиПоАналитике.НачалоДействия
	|			И Продажи.Период < НастройкиПоАналитике.ОкончаниеДействия
	|	ГДЕ
	...
	|			И Товары.Период >= НастройкиПоМестуУчета.НачалоДействия
	|			И Товары.Период < НастройкиПоМестуУчета.ОкончаниеДействия
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПорядокОтраженияНаСчетахУчетаПоПериодуДействия КАК НастройкиПоОрганизации
	...
	|			И НастройкиПоОрганизации.МестоУчета = НЕОПРЕДЕЛЕНО
	|			И Товары.Период >= НастройкиПоОрганизации.НачалоДействия
	|			И Товары.Период < НастройкиПоОрганизации.ОкончаниеДействия
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПорядокОтраженияНаСчетахУчетаПоПериодуДействия КАК НастройкиПоАналитике
	|		ПО НастройкиПоАналитике.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|			И НастройкиПоАналитике.АналитикаУчета = Товары.ГруппаФинансовогоУчета
	|			И НастройкиПоАналитике.МестоУчета = НЕОПРЕДЕЛЕНО
	|			И Товары.Период >= НастройкиПоАналитике.НачалоДействия
	|			И Товары.Период < НастройкиПоАналитике.ОкончаниеДействия
	|	ГДЕ
	|		ЕСТЬNULL(НастройкиПоМестуУчета.СчетУчета, ЕСТЬNULL(НастройкиПоОрганизации.СчетУчета, ЕСТЬNULL(НастройкиПоАналитике.СчетУчета,
	|			ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)))) В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Товары))
	|	) КАК ИсходныеДанные
	...
	Если ТипЗнч(Организация) = Тип("Массив") И МенеджерВременныхТаблиц <> Неопределено Тогда
		Запрос.Текст = Запрос.Текст + "
			...
			|;
			|////////////////////////////////////////////////////////////////////////////////
			|УНИЧТОЖИТЬ ПорядокОтраженияНаСчетахУчетаПоПериодуДействия
			|;
			|////////////////////////////////////////////////////////////////////////////////
...
Функция ДолиСписанияНормируемыхРасходов(Период, Организация, Протокол = Неопределено, МенеджерВременныхТаблиц = Неопределено, ЭтоПерерасчет = Ложь)
	
	// Речь идет о тех расходах, которые уже отражены на счетах затрат (20, 25, 44 и т.п.).
	
	Результат = Новый Соответствие;
	
	ОграниченияРасходов      = ОграниченияРасходов(Период, Организация, МенеджерВременныхТаблиц, ЭтоПерерасчет);
	ВидыНормируемыхРасходов  = ОграниченияРасходов.ВыгрузитьКолонку("ВидРасходов");
	СуммыНормируемыхРасходов = СуммыНормируемыхРасходов(Период, Организация, ВидыНормируемыхРасходов, МенеджерВременныхТаблиц, ЭтоПерерасчет);
...
Функция ОграниченияРасходов(Период, Организация, МенеджерВременныхТаблиц, ЭтоПерерасчет)
	...
	РасходыНаОплатуТруда = РасходыНаОплатуТруда(Период, Организация, МенеджерВременныхТаблиц, ЭтоПерерасчет);
	ВыручкаОтРеализации  = ВыручкаОтРеализации(Период, Организация, МенеджерВременныхТаблиц, ЭтоПерерасчет);
...
Функция РасходыНаОплатуТруда(Период, Организация, МенеджерВременныхТаблиц, ЭтоПерерасчет = Ложь)
	...
	// Исключаем "внутренние" обороты по распределению затрат (Дт счетов затрат Кт счетов затрат).
	...
	Запрос.УстановитьПараметр("НеПерерасчетНалога",      ЭтоПерерасчет);
	...
	Если МенеджерВременныхТаблиц.Таблицы.Найти("СтатьиРасходов") = Неопределено Тогда
		Запрос.Текст ="ВЫБРАТЬ
		|	Хозрасчетный.Ссылка КАК Счет
	...
	|	СУММА(Вознаграждение.СуммаПринимаемые) КАК Сумма
	|ПОМЕСТИТЬ Вознаграждение
	|ИЗ
	|	(ВЫБРАТЬ
	|		ОблагаетсяНалогомНаПрибыль.Субконто1 КАК СтатьяРасходов,
	...
	|	
	|	ВЫБРАТЬ
	|		КорректировкиТекущийМесяц.Субконто1,
	|		КорректировкиТекущийМесяц.СуммаНУОборотДт
	|	ИЗ
	|		РегистрБухгалтерии.КорректировкиНалоговойБазы.Обороты(
	|				&НачалоОтчетногоПериода,
	|				&КонецОтчетногоПериода,
	...
	|							СтатьиРасходов),
	|				,
	|				) КАК КорректировкиТекущийМесяц
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		КорректировкиТекущийМесяцЛишнее.Субконто1,
	|		-КорректировкиТекущийМесяцЛишнее.СуммаНУОборотДт
	|	ИЗ
	|		РегистрБухгалтерии.КорректировкиНалоговойБазы.Обороты(
	|				&НачалоОтчетногоПериода,
	|				&КонецОтчетногоПериода,
	...
	|							СтатьиРасходов),
	|				КорСчет В (&СчетаРасходов),
	|				) КАК КорректировкиТекущийМесяцЛишнее) КАК Вознаграждение
	|
	|СГРУППИРОВАТЬ ПО
	...
	|				КОГДА ЕСТЬNULL(СтатьиРасходов.ОплатаТруда, ИСТИНА)
	|					ТОГДА Вознаграждение.Сумма
	|				ИНАЧЕ 0
...
Функция СуммыНормируемыхРасходов(Период, Организация, ВидыНормируемыхРасходов, МенеджерВременныхТаблиц, ЭтоПерерасчет)
	...
	Если МенеджерВременныхТаблиц.Таблицы.Найти("ДанныеПроводок") = Неопределено Тогда
		РасчетНалогаНаПрибыль.СоздатьВТ_ТаблицаПроводок(МенеджерВременныхТаблиц);
	КонецЕсли;
	...
	|						Организации.Организация
	|					ИЗ
	|						Организации)) КАК ТекущийМесяц
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ПредыдущиеМесяцы.Субконто1,
	|		ПредыдущиеМесяцы.СуммаНУОборот,
	|		ПредыдущиеМесяцы.СуммаНУОборот,
	|		0
	|	ИЗ
	|		РегистрБухгалтерии.КорректировкиНалоговойБазы.Обороты(
	...
	|	
	|	ВЫБРАТЬ
	|		ПредыдущиеМесяцы.СтатьяРасходов,
	|		-ПредыдущиеМесяцы.СуммаНУ,
	|		0,
	|		0
	|	ИЗ
	|		ДанныеПроводок КАК ПредыдущиеМесяцы
	|	ГДЕ
	|		ПредыдущиеМесяцы.СчетКт = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НормируемыеРасходы)
	|		И ПредыдущиеМесяцы.Период МЕЖДУ &НачалоОтчетногоПериода И &КонецПредыдущегоМесяца
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|						ИЗ
	|							Организации)
	|					И (&НеЭтоПерерасчет ИЛИ НаборКорректировок В (ВЫБРАТЬ Т.НаборКорректировок ИЗ ВТНаборыКорректировок КАК Т))) КАК ТекущийМесяц) КАК Расходы
	|
	|СГРУППИРОВАТЬ ПО
