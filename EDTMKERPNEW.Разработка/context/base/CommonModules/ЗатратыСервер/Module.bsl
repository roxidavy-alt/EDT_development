...
Функция ПартииПроизводстваДляРаспределенияЗатрат(Отборы) Экспорт
	...
	// Документы партий производства без заказа.
	ТекстыЗапроса.Добавить(
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ДокументыПартий.Ссылка                              КАК Ссылка,
	|	ДокументыПартий.Организация                         КАК Организация,
	|	ДокументыПартий.Подразделение                       КАК Подразделение,
	//++ НЕ УТКА
	|	ДокументыПартий.ВнутренняяПереработка               КАК ВнутренняяПереработка,
	|	ДокументыПартий.ОрганизацияДавалец                  КАК ОрганизацияДавалец,
	//-- НЕ УТКА
	|	ДокументыПартий.Дата                                КАК Дата,
	...
	|	СпрПартииПроизводства.Серия                         КАК Серия,
	|	СпрПартииПроизводства.НаправлениеДеятельности       КАК НаправлениеДеятельности
	|ПОМЕСТИТЬ ВтДокументыПартийПроизводстваБезЗаказа
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|	ГДЕ
	|		(&ПоВсемОрганизациям
	|			ИЛИ РеквизитыДокумента.Организация В(&Организация))
	|		И РеквизитыДокумента.Проведен
	|		И РеквизитыДокумента.Дата МЕЖДУ &НачалоПериода И КОНЕЦПЕРИОДА(ДобавитьКДате(&ОкончаниеПериода, МЕСЯЦ, 1), МЕСЯЦ)
	//++ НЕ УТКА
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	|		И (&ПоВсемОрганизациям
	|			ИЛИ ТаблицаТоваров.ОрганизацияДавалец В(&Организация))
	|		И РеквизитыДокумента.Проведен
	|		И РеквизитыДокумента.Дата МЕЖДУ &НачалоПериода И КОНЕЦПЕРИОДА(ДобавитьКДате(&ОкончаниеПериода, МЕСЯЦ, 1), МЕСЯЦ)
	//-- НЕ УТКА
	|	) КАК ДокументыПартий
	...
	|	ПО РеестрДокументов.СторнируемыйДокумент = ДокументыПартий.Ссылка
	|	И РеестрДокументов.ДатаДокументаИБ МЕЖДУ НАЧАЛОПЕРИОДА(ДокументыПартий.Дата, МЕСЯЦ) И КОНЕЦПЕРИОДА(ДокументыПартий.Дата, МЕСЯЦ)
	...
	|	И (&ВсеПодразделения ИЛИ ДокументыПартий.Подразделение = &Подразделение)
	|	И (&ЭтапыЗаВесьПериод ИЛИ ДокументыПартий.Дата МЕЖДУ &НачалоПериода И &ОкончаниеПериода)
	...
	//++ НЕ УТКА
	|	И (НЕ &ВнутренняяПереработка ИЛИ ДокументыПартий.ВнутренняяПереработка)
	|	И (&ВсеДавальцы ИЛИ ДокументыПартий.ОрганизацияДавалец = &Давалец)
	//-- НЕ УТКА
	|
	|СГРУППИРОВАТЬ ПО
	|	ДокументыПартий.Ссылка,
	|	ДокументыПартий.Организация,
	|	ДокументыПартий.Подразделение,
	//++ НЕ УТКА
	|	ДокументыПартий.ВнутренняяПереработка,
	|	ДокументыПартий.ОрганизацияДавалец,
	//-- НЕ УТКА
	|	ДокументыПартий.Дата,
	|	СпрПартииПроизводства.Ссылка,
	|	СпрПартииПроизводства.ОсновноеИзделиеНоменклатура,
	|	СпрПартииПроизводства.ОсновноеИзделиеХарактеристика,
	|	СпрПартииПроизводства.Спецификация,
	|	СпрПартииПроизводства.Назначение,
	|	СпрПартииПроизводства.Серия,
	...
	|	ДанныеДокумента.ПартияПроизводства      КАК ПартияПроизводства,
	...
	//++ НЕ УТКА
	|	ДанныеДокумента.ВнутренняяПереработка   КАК ВнутренняяПереработка,
	|	ДанныеДокумента.ОрганизацияДавалец      КАК Давалец,
	//-- НЕ УТКА
	|	ЛОЖЬ                                    КАК ПроизводствоНаСтороне
	|ПОМЕСТИТЬ ВтЭтапы
	|ИЗ
	|	ВтДокументыПартийПроизводстваБезЗаказа КАК ДанныеДокумента
	//++ НЕ УТКА
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Процедура ПроверкаКорректностиУказанияПартийПроизводства(ПараметрыПроверки) Экспорт
	...
	|			И НЕ ДокументИсправления.ИсправлениеСохраняетПартииПроизводства)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Процедура ПроверкаАктуальностиУказанныхПартийПроизводства(ПараметрыПроверки) Экспорт
	...
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	РеквизитыОтчетаДавальцу.Организация     КАК Организация,
	|	РеквизитыОтчетаДавальцу.Ссылка          КАК ДокументРаспределения,
	|	ПартииОтчетаДавальцу.ПартияПроизводства КАК ПартияПроизводства
	|ИЗ
	|	Документ.ОтчетДавальцуМеждуОрганизациями КАК РеквизитыОтчетаДавальцу
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетДавальцуМеждуОрганизациями.Товары КАК ПартииОтчетаДавальцу
	|	ПО РеквизитыОтчетаДавальцу.Ссылка = ПартииОтчетаДавальцу.Ссылка
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПартииПроизводства КАК СпрПартииПроизводства
	|	ПО СпрПартииПроизводства.Ссылка = ПартииОтчетаДавальцу.ПартияПроизводства
	|	И ТИПЗНАЧЕНИЯ(СпрПартииПроизводства.Документ) = ТИП(Документ.ПроизводствоБезЗаказа)
	|
	|ГДЕ
	|	РеквизитыОтчетаДавальцу.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	|	И РеквизитыОтчетаДавальцу.Проведен
	|	И РеквизитыОтчетаДавальцу.Организация В (&МассивОрганизаций)
	// Отчет давальцу может оформляться по партии производства без заказа, если в текущем периоде по ней было движение затрат.
	|	И НЕ ИСТИНА В(
	|		ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ РегистрНакопления.СебестоимостьТоваров КАК ПриходыНЗП
	|		ГДЕ
	|			ПриходыНЗП.Партия = ПартииОтчетаДавальцу.ПартияПроизводства
	|			И ПриходыНЗП.Организация = РеквизитыОтчетаДавальцу.Организация
	|			И ПриходыНЗП.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|			И НЕ ПриходыНЗП.РасчетПартий
	|			И НЕ ПриходыНЗП.РасчетСебестоимости
	|		)
	|	И НЕ ИСТИНА В(
	|		
	|		ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ РегистрНакопления.ПрочиеРасходыНезавершенногоПроизводства КАК ПриходыНЗП
	|		ГДЕ
	|			ПриходыНЗП.ПартияПроизводства = ПартииОтчетаДавальцу.ПартияПроизводства
	|			И ПриходыНЗП.Организация = РеквизитыОтчетаДавальцу.Организация
	|			И ПриходыНЗП.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|			И НЕ ПриходыНЗП.РасчетПартий
	|			И НЕ ПриходыНЗП.РасчетСебестоимости
	...
	|		ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ РегистрНакопления.ТрудозатратыНезавершенногоПроизводства КАК ПриходыНЗП
	|		ГДЕ
	|			ПриходыНЗП.ПартияПроизводства = ПартииОтчетаДавальцу.ПартияПроизводства
	|			И ПриходыНЗП.Организация = РеквизитыОтчетаДавальцу.Организация
	|			И ПриходыНЗП.Период МЕЖДУ &НачалоПериода И &КонецПериода
	...
	|		ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ РегистрНакопления.ПрочиеРасходы КАК ПриходыНЗП
	|		ГДЕ
	|			ПриходыНЗП.АналитикаРасходов = ПартииОтчетаДавальцу.ПартияПроизводства
	|			И ПриходыНЗП.Организация = РеквизитыОтчетаДавальцу.Организация
	|			И ПриходыНЗП.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|			И НЕ ПриходыНЗП.РасчетПартий
	|			И НЕ ПриходыНЗП.РасчетСебестоимости
...
Процедура ПроверитьСоответствиеОтчетаПереработчикаИКорректировкиОтчетаПереработчика(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить(ИмяПоляОбъекта, НСтр("ru = 'Корректировка отчета переработчика';
												|en = 'Adjust subcontracting services notification received'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ДобавитьЭтап_НастройкаРаспределенияРасходов(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Настройка распределения расходов';
											|en = 'Configure expense allocation'"; // @НСтр-1
...
Процедура ДобавитьЭтап_НастройкаРаспределенияРасходовВручную(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Настройка распределения расходов, требующих ручного ввода';
											|en = 'Configure expenses allocation that require manual input'"; // @НСтр-1
	...
	Если РазмерыВременныхТаблиц.СостояниеРаспределенияРасходов = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Все производственные расходы полностью распределены по статьям калькуляции.';
				|en = 'All production costs are completely allocated to product cost elements.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ДобавитьЭтап_РаспределениеМатериаловИРаботНаСебестоимостьПродукции22(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.РаспределениеМатериаловИРаботНаСебестоимостьПродукции22);
	
	НоваяСтрока.МультиязычноеНаименование =
		"ru = 'Распределение материалов и работ на себестоимость продукции (версия 2.2)';
		|en = 'Material and labor allocation to product cost (version 2.2)'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
...
Процедура ОписаниеПроверок_РаспределениеМатериаловИРаботНаСебестоимостьПродукции22(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		...
		НСтр("ru = 'Должно быть оформлено распределение всех материалов и работ на этапы производства документами ""Распределение материалов и работ"".';
			|en = 'Allocation of all materials and works to production stages should be registered with the ""Material and labor allocation"" documents.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ДобавитьЭтап_НастройкаПоказателейДляРаспределенияРасходовНаСебестоимостьПродукции(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.НастройкаПоказателейДляРаспределенияРасходовНаСебестоимостьПродукции);
	
	НоваяСтрока.МультиязычноеНаименование =
		"ru = 'Настройка показателей для распределения расходов на себестоимость продукции';
		|en = 'Configure indicators for expense allocation to product cost'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
...
Процедура ДобавитьЭтап_НастройкаПоказателейДляРаспределенияРасходовМеждуОВЗ(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.НастройкаПоказателейДляРаспределенияРасходовМеждуОВЗ);
	
	НоваяСтрока.МультиязычноеНаименование =
		"ru = 'Настройка показателей для распределения расходов между ОВЗ';
		|en = 'Configure indicators for expenses allocation between cost centers'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
	НоваяСтрока.ТекстВыполнить = НСтр("ru = 'Настроить';
										|en = 'Customize'");
	НоваяСтрока.ДействиеИспользование = ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
...
Процедура ПроверкаПриЗаписиПрочиеРасходы(Регистратор, Отказ) Экспорт
	...
	|ГДЕ
	|	ДД.Регистратор = &Регистратор И ДД.Активность
	|	И НАЧАЛОПЕРИОДА(ДД.Период, МЕСЯЦ) > НАЧАЛОПЕРИОДА(Док.Дата, МЕСЯЦ)
	|	И НЕ ДД.РасчетСебестоимости
	|	И НЕ ДД.РасчетПартий
	// Расходы по партии производства прошлого периода нельзя отразить, если период отклонения 
	// не соответствует периоду документа производства или период отклонения не заполнен.
	|	И (НАЧАЛОПЕРИОДА(ДД.ПериодОтклонения, МЕСЯЦ) <> НАЧАЛОПЕРИОДА(Док.Дата, МЕСЯЦ)
	|		ИЛИ ДД.ПериодОтклонения = ДАТАВРЕМЯ(1,1,1))
	|";
	#КонецОбласти
