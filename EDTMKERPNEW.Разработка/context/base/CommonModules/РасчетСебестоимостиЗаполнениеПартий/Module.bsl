...
Функция ТекстИсправленныеДокументы() Экспорт // вт ИсправленныеДокументы
	Возврат "
		...
		|	ПО РеквизитыПБЗ.Ссылка = Реестр.Ссылка
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК РеквизитыСторнируемогоПбЗ
		|	ПО РеквизитыСторнируемогоПбЗ.Ссылка = Реестр.СторнируемыйДокумент
		//-- НЕ УТ
		|
		|ГДЕ
...
Функция ТекстТекущиеОборотыПартийТоваров() // использует ВозвратныеОтходы
	ТекстЗапроса =
		...
		|		КОГДА НЕ КорПартияПроизводства.ГруппаПродукции ЕСТЬ NULL
		|		 И &АналитическийУчетПоГруппамПродукции
		...
		|		 И КорПартияПроизводства.Ссылка ЕСТЬ NULL
		//-- НЕ УТ
		|			ТОГДА Т.АналитикаФинансовогоУчетаДокумента
...
Функция ТекстСторноДвиженияИсправленныхДокументовПрошлогоПериода() // использует ИсправленныеДокументы
	...
	Возврат " 
		...
		//++ НЕ УТ
		|			ИЛИ ТИПЗНАЧЕНИЯ(ДанныеРегистра.Партия) = ТИП(Справочник.ПартииПроизводства)
		//-- НЕ УТ
...
Функция ТекстЗапросаДляСторноДвиженийРасчетаПартийСебестоимостьТоваров(ПараметрыРасчета = Неопределено) Экспорт
	...
	ТекстЗапроса = ""
		...
		//++ НЕ УТ
		+ ОбщегоНазначения.РазделительПакетаЗапросов() + ТекстИсправленныеПартииПроизводства()
		//-- НЕ УТ
		+ ОбщегоНазначения.РазделительПакетаЗапросов()
		// выборка данных
		+ ТекстОписаниеДанныхДляПартийТоваров() // вт Данные
...
// 	Строка - текст запроса
//
Функция ТекстИсправленныеПартииПроизводства() // вт ИсправленныеПартииПроизводства
	
	Возврат "	
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ДД.СторнируемыйДокумент КАК ДокументИсточник,
		...
		|		ПО ПартииПроизводства.Документ = ДД.СторнируемыйДокумент
		|";
КонецФункции
