...
Процедура РаспределитьВсеРасчетыСКлиентами(ОкончаниеПериодаРасчета, АналитикиРасчета = Неопределено, ПроверятьНачалоПереходаНаНовуюАрхитектуруВзаиморасчетов = Истина) Экспорт
	...
	Если ПроверятьНачалоПереходаНаНовуюАрхитектуруВзаиморасчетов
		...
		ВызватьИсключение НСтр("ru = 'Начат переход на новую архитектуру взаиморасчетов. 
						...
						|en = 'Migration to online AR/AP accounting has started.
						|Allocation of customer AR/AP cannot be performed before the end of migration.'");
	КонецЕсли;
...
Процедура РаспределитьВсеРасчетыСПоставщиками(ОкончаниеПериодаРасчета, АналитикиРасчета = Неопределено, ПроверятьНачалоПереходаНаНовуюАрхитектуруВзаиморасчетов = Истина) Экспорт
	...
	Если ПроверятьНачалоПереходаНаНовуюАрхитектуруВзаиморасчетов 
		...
		ВызватьИсключение НСтр("ru = 'Начат переход на новую архитектуру взаиморасчетов. 
						...
						|en = 'Migration to new online AR/AP accounting has started.
						|Allocation of vendor AR/AP cannot be performed before the end of migration.'");
	КонецЕсли;
	...
	Если ДатыЗапретаИзменения.НайденЗапретИзмененияДанных(ШаблонЗапретаДанных) Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось запустить распределение взаиморасчетов за %1 из-за установленной даты запрета.
				           ...
				           |en = 'Cannot start AR/AP allocation for %1 due to the set period-end closing date.
