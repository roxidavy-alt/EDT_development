...
Процедура ОтразитьОперацииСПрослеживаемымиТоварами(Источник, Отказ, Замещение) Экспорт
	...
	Если ЭтоПроведениеДокумента ИЛИ ЭтоОтменаРучногоРедактирования Тогда
		...
		ТоварыОрганизаций = ТипЗнч(Источник) = Тип("РегистрНакопленияНаборЗаписей.ТоварыОрганизаций");
		ПараметрыИсточника = Новый Структура("ПрослеживаемыеТоварыВСоставеОС, ТоварыОрганизаций", ПрослеживаемыеТоварыВСоставеОС, ТоварыОрганизаций);
		ОперацииПрослеживаемости = ОперацииПрослеживаемости(Регистратор, Источник.Выгрузить(), ПараметрыИсточника);
...
Процедура ПолучитьДанныеРНПТ(СтруктураПараметров) Экспорт
	...
	ПолучитьДанныеРНПТИсправленныеСФНаОтгрузку(СтруктураПараметров);
...
Функция ТекстЗапросаДвиженияПрослеживаемогоТовара(ЕстьКолонкаРНПТ = Ложь, ЕстьТоварыОрганизаций = Ложь)
	...
	|		ПО ДвижениеТоваров.ДокументРеализации = ПриобретениеТоваровУслуг.Ссылка
	|			И ПриобретениеТоваровУслуг.ХозяйственнаяОперация В (&ХозОперацииЗакупкаВСтранахЕАЭС)
	|
	|СГРУППИРОВАТЬ ПО
	...
	Если Не ЕстьТоварыОрганизаций Тогда 
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Движения.ДокументРеализации КАК ДокументРеализации", "ЗНАЧЕНИЕ(Документ.ПриобретениеТоваровУслуг.ПустаяСсылка) КАК ДокументРеализации");
	КонецЕсли;
...
Функция ОперацииПрослеживаемости(Регистратор, ТаблицаДвижений, ПараметрыИсточникаДвижений)
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ТаблицаДвижений", ТаблицаДвижений);
	Запрос.УстановитьПараметр("ПрослеживаемыеТоварыВСоставеОС", ПараметрыИсточникаДвижений.ПрослеживаемыеТоварыВСоставеОС);
	Запрос.УстановитьПараметр("ХозОперацииЗакупкаВСтранахЕАЭС", ЗакупкиВызовСервера.ХозяйственныеОперацииПоОсновной(
	...
	Запрос.Текст = ТекстЗапросаДвиженияПрослеживаемогоТовара(ТаблицаДвижений.Колонки.Найти("РНПТ") <> Неопределено, ПараметрыИсточникаДвижений.ТоварыОрганизаций);
	Выборка = Запрос.Выполнить().Выбрать(); 
...
Функция НоваяСтруктураСвойствОперации()
	...
	Свойства.Вставить("ЭтоВвозИзЕАЭС", Ложь);
...
Функция ПолучитьСвойстваРегистратора(Регистратор, ТаблицаДвижений)
	...
	Если Свойства.ТипРегистратора = Тип("ДокументСсылка.РеализацияТоваровУслуг")      
		...
		|			И НЕ ТаблицаДокумента.Ссылка.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ОблагаетсяНДССМежценовойРазницы)
...
Функция ОтчетПрослеживаемойОперации(СвойстваОперации)
	...
	ИначеЕсли СвойстваОперации.ЭтоВвозИзЕАЭС
		И (СвойстваОперации.ТипРегистратора = Тип("ДокументСсылка.ВозвратТоваровПоставщику")
			ИЛИ СвойстваОперации.ТипРегистратора = Тип("ДокументСсылка.КорректировкаПриобретения")) Тогда 
...
Функция КодПрослеживаемойОперации(СвойстваОперации)
	...
	Если СвойстваОперации.ТипРегистратора = Тип("ДокументСсылка.ВнутреннееПотребление")
		...
		ИЛИ СвойстваОперации.ТипРегистратора = Тип("ДокументСсылка.ЭтапПроизводства2_2")
		//-- НЕ УТКА
		ИЛИ СвойстваОперации.ТипРегистратора = Тип("ДокументСсылка.СписаниеОтгруженныхТоваров") Тогда
...
Процедура ПолучитьОсновныеДанныеРНПТ(СтруктураПараметров)
	...
	|	Операции.КодОперации,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(СписокДокументов.Документ) = ТИП(Документ.ОтгрузкаТоваровКлиенту)
	|			ТОГДА 0
	|		ИНАЧЕ СУММА(Операции.СтоимостьПоРНПТ)
	|	КОНЕЦ
	|ИЗ
	|	РегистрСведений.ОперацииСПрослеживаемымиТоварамиРасширенный КАК Операции
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_СписокДокументов КАК СписокДокументов
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СчетФактураНаОтгрузку.ДокументОснование КАК Документ,
	|	СчетФактураНаОтгрузкуТовары.Номенклатура КАК Номенклатура,
	|	СчетФактураНаОтгрузкуТовары.НомерГТД КАК РегНомПросл,
	|	СУММА(ЕСТЬNULL(Суммы.СуммаБезНДСРегл, СчетФактураНаОтгрузкуТовары.Сумма - СчетФактураНаОтгрузкуТовары.СуммаНДС)) КАК Сумма
	|ПОМЕСТИТЬ ВТ_ТоварыОтгрузки
	|ИЗ
	|	ВТ_ПрослеживаемыеОперации КАК ПрослеживаемыеОперации
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный КАК СчетФактураНаОтгрузку
	|		ПО ПрослеживаемыеОперации.Документ = СчетФактураНаОтгрузку.ДокументОснование
	|			И СчетФактураНаОтгрузку.НачислениеНДСПриОтгрузке
	|			И СчетФактураНаОтгрузку.Проведен
	|			И НЕ (СчетФактураНаОтгрузку.Исправление ИЛИ СчетФактураНаОтгрузку.Корректировочный)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.Товары КАК СчетФактураНаОтгрузкуТовары
	|		ПО СчетФактураНаОтгрузку.Ссылка = СчетФактураНаОтгрузкуТовары.Ссылка
	|			И ПрослеживаемыеОперации.Номенклатура = СчетФактураНаОтгрузкуТовары.Номенклатура
	|			И ПрослеживаемыеОперации.РегНомПросл = СчетФактураНаОтгрузкуТовары.НомерГТД
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СуммыДокументовВВалютахУчета КАК Суммы
	|		ПО Суммы.Регистратор = СчетФактураНаОтгрузкуТовары.Ссылка
	|			И Суммы.ИдентификаторСтроки = СчетФактураНаОтгрузкуТовары.ИдентификаторСтроки
	|
	|СГРУППИРОВАТЬ ПО
	|	СчетФактураНаОтгрузку.ДокументОснование,
	|	СчетФактураНаОтгрузкуТовары.Номенклатура,
	|	СчетФактураНаОтгрузкуТовары.НомерГТД
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Документ
	|;
	...
	|	И НЕ ТИПЗНАЧЕНИЯ(ПрослеживаемыеОперации.Документ) = ТИП(Документ.ОтгрузкаТоваровКлиенту)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|		ИНАЧЕ -ПрослеживаемыеОперации.Количество
	|	КОНЕЦ,
	|	ЕСТЬNULL(ТоварыОтгрузки.Сумма, 0),
	|	ПрослеживаемыеОперации.КодОперации КАК КодОперации
	|ИЗ
	|	ВТ_ПрослеживаемыеОперации КАК ПрослеживаемыеОперации
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ТоварыОтгрузки КАК ТоварыОтгрузки
	|		ПО ПрослеживаемыеОперации.Документ = ТоварыОтгрузки.Документ
	|			И ПрослеживаемыеОперации.Номенклатура = ТоварыОтгрузки.Номенклатура
	|			И ПрослеживаемыеОперации.РегНомПросл = ТоварыОтгрузки.РегНомПросл
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НомераГТД КАК НомераГТД
	|		ПО ПрослеживаемыеОперации.РегНомПросл = НомераГТД.Ссылка
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(ПрослеживаемыеОперации.Документ) = ТИП(Документ.ОтгрузкаТоваровКлиенту)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура ПолучитьДанныеРНПТИсправленныеСФНаОтгрузку(СтруктураПараметров)
	
	Если НЕ СтруктураПараметров.ЕстьОтгрузки Тогда
		Возврат;
	КонецЕсли;
	...
	Если СтруктураПараметров.МенеджерВременныхТаблиц.Таблицы.Найти("ВТ_СопоставлениеСФОснований") <> Неопределено Тогда
		...
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВЫБОР
		|		КОГДА ВТ_СопоставлениеСФОснований.Корректировочный
		|			ТОГДА ВТ_СопоставлениеСФОснований.СчетФактура
		|		ИНАЧЕ
		|			ВТ_СопоставлениеСФОснований.ДокументДляСвязи
		|	КОНЕЦ КАК Документ,
		|	ВТ_СопоставлениеСФОснований.СчетФактура КАК СчетФактура,
		|	ВТ_СопоставлениеСФОснований.Исправление КАК Исправление,
		|	ВТ_СопоставлениеСФОснований.Корректировочный КАК Корректировочный,
		...
		|	Операции.АналитикаУчетаНоменклатуры.Номенклатура КАК Номенклатура,
		|	Операции.КодОперации КАК КодОперации
		|ПОМЕСТИТЬ ВТ_Отгрузки_Предварительная
		|ИЗ
		|	РегистрСведений.ОперацииСПрослеживаемымиТоварамиРасширенный КАК Операции
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_СопоставлениеСФОснований КАК ВТ_СопоставлениеСФОснований
		|		ПО Операции.Регистратор = ВТ_СопоставлениеСФОснований.Документ
		|			И (ВТ_СопоставлениеСФОснований.Документ ССЫЛКА Документ.ОтгрузкаТоваровКлиенту)
		|ГДЕ
		|	Операции.Организация В(&Организация)
		|	И Операции.Активность
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	СчетФактура,
		...
		|ИЗ
		|	ВТ_Отгрузки_Предварительная КАК Операции
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТ_Отгрузки_Предварительная";
...
Процедура ПолучитьИтоговыеДанныеРНПТ(СтруктураПараметров)
	...
	Если СтруктураПараметров.ЕстьОтгрузки Тогда
		Запрос.Текст = Запрос.Текст + "
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ВТ_Отгрузки.Документ,
		|	ВТ_Отгрузки.ТипЗапасов,
		|	ВТ_Отгрузки.ДатаОперации,
		|	ВТ_Отгрузки.ТипДокумента,
		|	ВТ_Отгрузки.Контрагент,
		|	ВТ_Отгрузки.Организация,
		|	ВТ_Отгрузки.Номенклатура.НаименованиеПолное КАК НаименованиеТовара,
		|	ВТ_Отгрузки.РегНомПросл,
		|	ВТ_Отгрузки.КолТовПросл,
		|	ВЫБОР 
		|		КОГДА ВТ_Отгрузки.СтоимТовПросл < 0
		|			ТОГДА  ВЫРАЗИТЬ(-ВТ_Отгрузки.СтоимТовПросл КАК ЧИСЛО(15, 2)) 
		|		ИНАЧЕ ВЫРАЗИТЬ(ВТ_Отгрузки.СтоимТовПросл КАК ЧИСЛО(15, 2))	   
		|	КОНЕЦ,
		|	ВТ_Отгрузки.КодОперации
		|ИЗ
		|	ВТ_Отгрузки КАК ВТ_Отгрузки
		|ГДЕ
		|	ВТ_Отгрузки.КолТовПросл <> 0 ИЛИ ВЫРАЗИТЬ(ВТ_Отгрузки.СтоимТовПросл КАК ЧИСЛО(15, 2)) <> 0";
	КонецЕсли;
...
Процедура ЗаполнитьДополнительныеПараметрыЗапросаРНПТ(СтруктураПараметров)
	...
	|			ПО ВТ_СписокДокументов.Документ = Операции.Регистратор
	|	ГДЕ
	|		ТИПЗНАЧЕНИЯ(ВТ_СписокДокументов.Документ) = ТИП(Документ.ОтгрузкаТоваровКлиенту)
	|		И Операции.Организация В(&Организация)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	СтруктураПараметров.Вставить("ЕстьОтгрузки",					НЕ МассивТипов.Найти(Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту"))		= Неопределено);
