...
Функция РассчитатьОграниченияГрафикаПроизводстваПоМатериалам(Ссылка) Экспорт
	...
	ТекстЗапроса =
		...
		|ГДЕ
		|	Таблица.СостояниеЗапаса = ЗНАЧЕНИЕ(Перечисление.СостоянияЗапаса.ВНаличии)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	Таблица.Назначение КАК Назначение,
		|	2 КАК Состояние,
		|	Таблица.ДатаДоступности КАК Дата,
		|	Таблица.Количество КАК Количество
		|ИЗ
		|	ВтРаспределение КАК Таблица
		|ГДЕ
...
Функция ОграниченияГрафикаПроизводстваПоМатериалам(Ссылка) Экспорт
	...
	ТекстЗапроса = 
		...
		|	РаспределениеЗапасов.Назначение КАК Назначение,
		|	2 КАК Состояние,
		|	РаспределениеЗапасов.ДатаДоступности КАК Дата,
		|	РаспределениеЗапасов.Количество КАК Количество
		|ИЗ
		|	РегистрСведений.РаспределениеЗапасов КАК РаспределениеЗапасов
		|ГДЕ
...
Функция ДатаСУчетомБуфера(Дата, Буфер, ГрафикРаботыИлиПодразделение, ЕдиницаИзмеренияБуфера) Экспорт
	...
	Если ЕдиницаИзмеренияБуфера = Перечисления.ЕдиницыИзмеренияВремени.День Тогда
		...
		Если Буфер > 0 Тогда
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВЫБРАТЬ ПЕРВЫЕ 99999", "ВЫБРАТЬ ПЕРВЫЕ " + Формат(?(Цел(Буфер / 86400) = Буфер / 86400, Буфер / 86400, Цел(Буфер / 86400) + 1), "ЧН=0; ЧГ="));//@Query-part-1 //@Query-part-2
