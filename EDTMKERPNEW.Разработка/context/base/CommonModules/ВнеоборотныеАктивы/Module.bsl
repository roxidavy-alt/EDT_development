...
// Определяет наличие узлов.
// 
// Параметры:
//  СписокОС - Массив из СправочникСсылка.ОбъектыЭксплуатации - Список ОС.
//  Ссылка - ДокументСсылка - Документ.
//  Период - Дата - Дата.
// 
// Возвращаемое значение:
//  Булево - Истина, если есть узлы
Функция ЕстьУзлы(СписокОС, Ссылка, Период) Экспорт

	ТекстЗапроса =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ДанныеРегистра.УзелКомпонент КАК ОсновноеСредство
	|ИЗ
	|	РегистрСведений.УзлыКомпонентыАмортизации.СрезПоследних(&Период, Регистратор <> &Ссылка) КАК ДанныеРегистра
	|ГДЕ
	|	ДанныеРегистра.ОсновноеСредство В(&СписокОС)
	|	И ДанныеРегистра.УзелКомпонент.ТипОС = ЗНАЧЕНИЕ(Перечисление.ТипыОС.Узел)";
...
Процедура ПриОпределенииОбъектовСКомандамиСозданияНаОсновании(Объекты) Экспорт
	...
	Объекты.Добавить(Метаданные.Документы.ОтнесениеОтклоненийВСтоимостьОС);
...
Процедура ПриОпределенииСправочниковХраненияФайлов(ТипВладелецФайла, ИменаСправочников) Экспорт
	...
	Если ТипВладелецФайла = Тип("ДокументСсылка.АмортизацияНМА2_4")
		...
		ИЛИ ТипВладелецФайла = Тип("ДокументСсылка.ОтнесениеОтклоненийВСтоимостьОС")
...
Процедура ПриОпределенииНастроекПечати(Настройки) Экспорт
	...
	СписокОбъектов.Добавить(Документы.ОтнесениеОтклоненийВСтоимостьОС);
...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		...
		|en = 'Asset activity records'", КодыЯзыков); // @НСтр-2
...
Процедура ДобавитьЭтап_ЗаполнениеФактическойСтоимостиВНАПриОтключенномУчетеСебестоимости(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ЗаполнениеФактическойСтоимостиВНАПриОтключенномУчетеСебестоимости);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Заполнение фактической стоимости внеоборотных активов при отключенном учете себестоимости';
											|en = 'Fill in actual cost of fixed assets with disabled cost accounting'"; // @НСтр-1
...
Функция СтоимостьИАмортизацияОС(ОсновноеСредство) Экспорт
	...
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ЕСТЬNULL(СтоимостьОСОстатки.СтоимостьОстаток, 0) КАК Стоимость,
	|	ЕСТЬNULL(СтоимостьОСОстатки.СтоимостьРеглОстаток, 0) КАК СтоимостьРегл,
	|	ЕСТЬNULL(СтоимостьОСОстатки.СтоимостьНУОстаток, 0) КАК СтоимостьНУ,
	|	ЕСТЬNULL(СтоимостьОСОстатки.СтоимостьПРОстаток, 0) КАК СтоимостьПР,
	|	ЕСТЬNULL(СтоимостьОСОстатки.СтоимостьВРОстаток, 0) КАК СтоимостьВР,
	|	ЕСТЬNULL(СтоимостьОСОстатки.СтоимостьЦФОстаток, 0) КАК СтоимостьЦФ,
	|	ЕСТЬNULL(СтоимостьОСОстатки.СтоимостьНУЦФОстаток, 0) КАК СтоимостьНУЦФ,
	|	ЕСТЬNULL(СтоимостьОСОстатки.СтоимостьПРЦФОстаток, 0) КАК СтоимостьПРЦФ,
	|	ЕСТЬNULL(СтоимостьОСОстатки.СтоимостьВРЦФОстаток, 0) КАК СтоимостьВРЦФ,
	|
	|	ЕСТЬNULL(-АмортизацияОСОстатки.АмортизацияОстаток, 0) КАК Амортизация,
	|	ЕСТЬNULL(-АмортизацияОСОстатки.АмортизацияРеглОстаток, 0) КАК АмортизацияРегл,
	|	ЕСТЬNULL(-АмортизацияОСОстатки.АмортизацияНУОстаток, 0) КАК АмортизацияНУ,
	|	ЕСТЬNULL(-АмортизацияОСОстатки.АмортизацияПРОстаток, 0) КАК АмортизацияПР,
	|	ЕСТЬNULL(-АмортизацияОСОстатки.АмортизацияВРОстаток, 0) КАК АмортизацияВР,
	|	ЕСТЬNULL(-АмортизацияОСОстатки.АмортизацияЦФОстаток, 0) КАК АмортизацияЦФ,
	|	ЕСТЬNULL(-АмортизацияОСОстатки.АмортизацияНУЦФОстаток, 0) КАК АмортизацияНУЦФ,
	|	ЕСТЬNULL(-АмортизацияОСОстатки.АмортизацияПРЦФОстаток, 0) КАК АмортизацияПРЦФ,
	|	ЕСТЬNULL(-АмортизацияОСОстатки.АмортизацияВРЦФОстаток, 0) КАК АмортизацияВРЦФ,
	|
	|	ЕСТЬNULL(СтоимостьОСОстатки.ЗалоговаяСтоимостьОстаток, 0) КАК ЗалоговаяСтоимость,
	|
	|	ЕСТЬNULL(-ОбесценениеВНАОстатки.ОбесценениеУпрОстаток, 0) КАК ОбесценениеУпр,
	|	ЕСТЬNULL(-ОбесценениеВНАОстатки.ОбесценениеРеглОстаток, 0) КАК ОбесценениеРегл
	|
	|ИЗ
	|	Справочник.ОбъектыЭксплуатации КАК ОбъектыЭксплуатации
	|
...
// 
// Возвращаемое значение:
//  Структура - Сведения о стоимости и амортизации.
//
Функция СтоимостьИАмортизацияНМА(НематериальныйАктив) Экспорт
	...
	|		КОГДА НематериальныеАктивы.ВидОбъектаУчета = ЗНАЧЕНИЕ(Перечисление.ВидыОбъектовУчетаНМА.РасходыНаНИОКР)
	|			ТОГДА ЕСТЬNULL(ПервоначальныеСведенияНМА.ПервоначальнаяСтоимостьБУ, 0)
	|					- ЕСТЬNULL(ПервоначальныеСведенияНМА.ПервоначальнаяСтоимостьНУ, 0)
	|					- ЕСТЬNULL(ПервоначальныеСведенияНМА.ПервоначальнаяСтоимостьПР, 0)
	...
	|
	|	ЕСТЬNULL(-ОбесценениеВНА.ОбесценениеРеглОстаток, 0) КАК ОбесценениеРегл,
	|   ЕСТЬNULL(-ОбесценениеВНА.ОбесценениеУпрОстаток, 0) КАК ОбесценениеУпр
	|
	|ИЗ
...
Процедура ПроверитьСоответствиеДатыВерсииУчета(Объект, ЭтоДокументВерсии2_4, Отказ) Экспорт
	...
	Если ИспользуетсяУправлениеВНА_2_4 И НЕ ЭтоДокументВерсии2_4 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Дата документа должна быть до %1 (начало ведения учета внеоборотных активов версии 2.4).
                               ...
                               |After this date, you must use documents from the ""Assets management"" section.'");
...
Процедура ПроверитьПрименениеФСБУ14(Объект, Отказ) Экспорт
	...
	ПрименяетсяФСБУ14 = НастройкиНалоговУчетныхПолитикПовтИсп.ПрименяетсяФСБУ14(
		Объект.Организация, КонецМесяца(?(Объект.Дата <> '00010101', Объект.Дата, ТекущаяДатаСеанса())));
	
	Если НЕ ПрименяетсяФСБУ14 Тогда
...
// Проверяет, что документ проводится при учете ОС и НМА по стандартам международного учета.
//
// Параметры:
//  Объект - ДокументОбъект- Проводимый документ.
//  Отказ - Булево - Отказ от проведения.
//
Процедура ПроверитьПрименениеСтандартаМУ(Объект, Отказ) Экспорт
	...
	Если ПорядокУчетаВНА <> Перечисления.ПорядокУчетаВНА.ПоСтандартамМУ Тогда
		ТекстСообщения = НСтр("ru = 'Оформление документа поддерживается только при учете ОС и НМА в упр. учете по стандартам международного учета.';
								|en = 'Document creation is supported only when accounting for fixed assets and intangible assets in management accounting according to the financial accounting standards.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Объект, "Дата",, Отказ); 
	КонецЕсли;
...
Функция ГруппыОрганизацийОСиНМА(Период) Экспорт
	...
	|			ИЛИ НЕ ПорядокУчета.СпособОтраженияАмортизацииБУ В (&ПустыеСпособыОтраженияАмортизации))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	РегистрСведений.ПорядокУчетаОСБУ.СрезПоследних(&КонецПредыдущегоМесяца, ДатаИсправления = ДАТАВРЕМЯ(1,1,1)) КАК ПорядокУчета
	|ГДЕ
	|	НЕ ПорядокУчета.СпособОтраженияАмортизацииНУ В (&ПустыеСпособыОтраженияАмортизации)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	(ПорядокУчета.ПередаватьРасходыВДругуюОрганизацию
	|			ИЛИ НЕ ПорядокУчета.СпособОтраженияАмортизации В (&ПустыеСпособыОтраженияАмортизации))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Процедура ДобавитьРолиДляДоступаКПодсистемеВНАНаИзменение(ОписаниеПрофиля) Экспорт
	...
	ОписаниеПрофиля.Роли.Добавить("ДобавлениеИзменениеВключенияОтклоненийВРасходахВСтоимостьОС");
	...
	ОписаниеПрофиля.Роли.Добавить("ДобавлениеИзменениеИнвентаризацииОС");
	...
	ОписаниеПрофиля.Роли.Добавить("ИспользованиеОбработкиПринятиеКУчетуИВключениеЗатратВСтоимостьОС");
...
Процедура ДобавитьРолиДляДоступаКПодсистемеВНАНаЧтение(ОписаниеПрофиля) Экспорт
	...
	ОписаниеПрофиля.Роли.Добавить("ЧтениеВключенияОтклоненийВРасходахВСтоимостьОС");
	...
	ОписаниеПрофиля.Роли.Добавить("ЧтениеИнвентаризацииОС");
...
Процедура ТекстЗапросаСрезПоследнихРегистра(ИмяРегистра, ИмяТаблицыСрезПоследних, ИмяОсновнойТаблицы, ТекстыЗапроса, СоответствиеИзмерений = Неопределено, УчестьИсправления = Ложь) Экспорт
	...
	|	&ВыборкаИзмерений,
	|	МАКСИМУМ(ТаблицаРегистра.Период) КАК ПериодРегистра
	|
	...
	|		ПО (ТаблицаРегистра.Период <= ОсновнаяТаблица.ДатаДокумента)
	...
	Если СоответствиеИзмерений.Свойство("Период") Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ОсновнаяТаблица.ДатаДокумента", "ОсновнаяТаблица" + "." + СоответствиеИзмерений.Период);
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ОсновнаяТаблица.ДатаДокумента", "ОсновнаяТаблица.Период");
	КонецЕсли;
	...
	Если ПараметрыРегистра.ЭтоНезависимыйРегистр Тогда
		...
		|
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ПервоначальныеСведенияОС КАК ТаблицаРегистра
		|		ПО ТаблицаРегистра.Период = ОсновнаяТаблица.ПериодРегистра
	...
	Иначе
		...
		|	ОсновнаяТаблица.Ссылка КАК Ссылка,
		|	ОсновнаяТаблица.Период КАК Период,
		|	ТаблицаРегистра.Период КАК ПериодРегистра,
		|	&ВыборкаИзмерений,
...
Функция ТекстЗапросаСрезПоследнихРегистраСУчетомИсправлений(ИмяРегистра, ИмяВременнойТаблицы = "", ДополнительныеПоля = Неопределено, УсловияЗапроса = "") Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(ЕСТЬNULL(ДанныеРегистраИсправление.ДатаИсправления, ДАТАВРЕМЯ(1,1,1))) КАК ДатаИсправления,
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Документы.ИнвентаризацияНМА.ПриДобавленииОбработчиковОбновления(Обработчики);
	...
	Документы.ОтнесениеОтклоненийВСтоимостьОС.ПриДобавленииОбработчиковОбновления(Обработчики);
	...
	Документы.ПринятиеКУчетуУзловКомпонентовАмортизации.ПриДобавленииОбработчиковОбновления(Обработчики);
...
Функция СписокДокументовВНА() Экспорт
	...
	СписокДокументовВНА.Добавить(Метаданные.Документы.ИнвентаризацияНМА.Имя);
	СписокДокументовВНА.Добавить(Метаданные.Документы.ИнвентаризацияОС.Имя);
	...
	СписокДокументовВНА.Добавить(Метаданные.Документы.ОтнесениеОтклоненийВСтоимостьОС.Имя);
...
//-- НЕ УТКА
	...
	ОписаниеПрофиля.Описание = 
		...
		|3. Receive reports on local accounting and tax accounting data.'");
...
Функция ПроверитьНаличиеЗаданийКЗакрытиюМесяца(ПараметрыОбработчика, ИмяРегистраЗаданий) Экспорт
	...
	ДанныеЗаданий = ДанныеЗаданийКЗакрытиюМесяца(
...
// Формирует данные заданий в регистре сведений.
//
// Параметры:
...
//
Функция ДанныеЗаданийКЗакрытиюМесяца(Период, СписокОрганизаций, ИмяРегистраЗаданий) Экспорт
	
	ДанныеЗаданий = Новый Структура;
	ДанныеЗаданий.Вставить("КоличествоОбъектов", 0);
	ДанныеЗаданий.Вставить("ДатаНачалаРасчета", '00010101');
	...
	Если НЕ ЗначениеЗаполнено(Выборка.Месяц) Тогда
		Возврат ДанныеЗаданий;
	КонецЕсли;
	
	ДанныеЗаданий.ДатаНачалаРасчета = Выборка.Месяц;

	Выборка = Результат[Результат.ВГраница()-1].Выбрать();
	Выборка.Следующий();
	
	ДанныеЗаданий.КоличествоОбъектов = Выборка.КоличествоОбъектов;
