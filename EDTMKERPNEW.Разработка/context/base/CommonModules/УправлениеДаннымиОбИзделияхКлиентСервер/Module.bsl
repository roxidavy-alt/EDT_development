...
Функция ПараметрыВыбораСпецификаций() Экспорт
	...
	ПараметрыВыбора.Вставить("РеквизитыСпецификации", ""); // Для получения реквизитов подбираемой спецификации
...
Функция ПараметрыДействияПроверитьЗаполнитьСпецификацию(Объект,
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура")
			И ДополнительныеПараметры.Свойство("ИмяРеквизитаСпецификация") Тогда
...
// Возвращает доступные типы номенклатуры по типу производственного процесса
//
// Параметры:
//  ТипПроизводственногоПроцесса - ПеречислениеСсылка.ТипыПроизводственныхПроцессов	 - тип производственного процесса
// 
// Возвращаемое значение:
//  Массив из ПеречислениеСсылка.ТипыПроизводственныхПроцессов- доступные типы номенклатуры
//
Функция ТипыНоменклатурыПоТипуПроизводственногоПроцесса(ТипПроизводственногоПроцесса) Экспорт

	МассивТипов = Новый Массив;
	
	Если ТипПроизводственногоПроцесса = ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот") Тогда
		
		МассивТипов.Добавить(ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Работа"));
		
	ИначеЕсли ТипПроизводственногоПроцесса = ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.Разборка")
		Или ТипПроизводственногоПроцесса = ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.Ремонт") Тогда
		
		МассивТипов.Добавить(ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Товар"));
	
	Иначе // производство (Сборка, БезСпецификаций)
...
//  ДоступныеТипы	 - Массив			 - доступные типы номенклатуры
//
Процедура УстановитьОтборПоТипуНоменклатуры(Элемент, ДоступныеТипы) Экспорт
	
	МассивПараметровВыбора = Новый Массив;
	МассивПараметровВыбора.Добавить(Новый ПараметрВыбора("Отбор.ТипНоменклатуры", Новый ФиксированныйМассив(ДоступныеТипы)));
	
	Если ТипЗнч(Элемент) = Тип("ПолеФормы") Тогда
		Элемент.ПараметрыВыбора = Новый ФиксированныйМассив(МассивПараметровВыбора);
	Иначе
		Для Индекс = 0 По Элемент.ВГраница() Цикл
			Элемент[Индекс].ПараметрыВыбора = Новый ФиксированныйМассив(МассивПараметровВыбора);
		КонецЦикла;
	КонецЕсли;
...
Функция ПоляСтруктурыДанныхОбИзделииДляВыбораСпецификации() Экспорт
	...
	Результат.Добавить("НачалоПроизводства");
...
Функция СтруктураДанныхОбИзделииДляВыбораСпецификации() Экспорт
	...
	Результат.РемонтируемоеИзделиеНоменклатура   = ПредопределенноеЗначение("Справочник.Номенклатура.ПустаяСсылка");
	Результат.РемонтируемоеИзделиеХарактеристика = ПредопределенноеЗначение("Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка");
...
// Вариант получения промежуточных полуфабрикатов в данных спецификации - не получать.
// 
// Возвращаемое значение:
//  Строка
//
Функция ПромежуточныеПолуфабрикатыНеПолучать() Экспорт
	
	Возврат "НеПолучать";
	
КонецФункции

// Вариант получения промежуточных полуфабрикатов в данных спецификации - получить.
// 
// Возвращаемое значение:
//  Строка
//
Функция ПромежуточныеПолуфабрикатыПолучить() Экспорт
...
//++ НЕ УТКА
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеСтроки, "СпособПолученияМатериала") Тогда
		Результат = (ДанныеСтроки.СпособПолученияМатериала = ПредопределенноеЗначение("Перечисление.СпособыПолученияМатериаловВСпецификации.ПроизводитсяНаЭтапе")
	...
	ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеСтроки, "ПроизводитсяВПроцессе") Тогда
		Результат = ДанныеСтроки.ПроизводитсяВПроцессе;
	КонецЕсли;
//-- НЕ УТКА
...
Функция ДанныеОсновногоИзделияСпецификации(Объект) Экспорт
	...
	ЭтоРемонт   = (Объект.ТипПроизводственногоПроцесса = ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот")
...
// * ВидСтроки - ПеречислениеСсылка.ВидыСтрокСпецификации - вид строки
...
// * РемонтируемоеИзделиеНоменклатура - СправочникСсылка.Номенклатура,Строка - номенклатура ремонтируемого изделия
// * РемонтируемоеИзделиеХарактеристика - СправочникСсылка.ХарактеристикиНоменклатуры - характеристика ремонтируемого изделия
...
// По переданной строке дерева возвращает ключ параметров выбора спецификации.
// 
// Параметры:
...
// Возвращаемое значение:
//  Строка, Неопределено -
Функция КлючПараметровВыбораСпецификацийДерева(СтрокаДерева) Экспорт
	
	Результат = "МатериалыИУслуги";
	
	Если СтрокаДерева <> Неопределено Тогда 
	
		Если СтрокаДерева.ВидСтроки = ПредопределенноеЗначение("Перечисление.ВидыСтрокСпецификации.ВыходноеИзделие") Тогда
			Результат = "ВыходныеИзделия";
		ИначеЕсли СтрокаДерева.ВидСтроки = ПредопределенноеЗначение("Перечисление.ВидыСтрокСпецификации.Номенклатура") Тогда
			Результат = "МатериалыИУслуги";
		ИначеЕсли СтрокаДерева.ВидСтроки = ПредопределенноеЗначение("Перечисление.ВидыСтрокСпецификации.Материал")
			И ТипПроизводственногоПроцессаСтрокиДерева(СтрокаДерева) = ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот") Тогда
			Результат = "МатериалыИУслуги_ВыполнениеРемонтныхРабот";
		ИначеЕсли СтрокаДерева.ВидСтроки = ПредопределенноеЗначение("Перечисление.ВидыСтрокСпецификации.Материал") Тогда
			Результат = "МатериалыИУслуги_СборкаРемонт";
		КонецЕсли;
...
// По переданной строке дерева возвращает тип процесса верхнеуровневой спецификации.
// 
// Параметры:
//  СтрокаДерева - ДанныеФормыЭлементДерева
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.ТипыПроизводственныхПроцессов, Неопределено - Тип производственного процесса строки дерева
Функция ТипПроизводственногоПроцессаСтрокиДерева(СтрокаДерева) Экспорт
	
	СтрокаРодитель = СтрокаДерева.ПолучитьРодителя();
	
	Если СтрокаРодитель = Неопределено Тогда
		Возврат Неопределено;
	ИначеЕсли ЗначениеЗаполнено(СтрокаРодитель.Спецификация) Тогда
		Возврат СтрокаРодитель.ТипПроизводственногоПроцесса;
	Иначе
		Возврат ТипПроизводственногоПроцессаСтрокиДерева(СтрокаРодитель);
	КонецЕсли;
...
// Параметры выбора элемента спецификации.
// 
// Параметры:
//  ПараметрыВыбораСпецификаций - см. ПараметрыВыбораСпецификаций
// 
// Возвращаемое значение:
//  Массив из ПараметрВыбора
Функция ПараметрыВыбораЭлементаСпецификации(ПараметрыВыбораСпецификаций) Экспорт
...
// Конструктор параметров выбора спецификаций для типа процесса - Сборка
// 
// Возвращаемое значение:
// 	Структура
//
...
// Конструктор параметров выбора спецификаций для типа процесса - Выполнение ремонтных работ
// 
// Возвращаемое значение:
// 	Структура
//
Функция ПараметрыВыбораСпецификацийНаВыполнениеРемонтныхРабот() Экспорт
	
	ПараметрыВыбора = ПараметрыВыбораСпецификаций();
	
	ПараметрыВыбора.ДоступныеТипы.Добавить(ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот"));
	ПараметрыВыбора.ДоступныеСтатусы.Добавить(ПредопределенноеЗначение("Перечисление.СтатусыСпецификаций.Действует"));
...
// Конструктор параметров выбора спецификаций для типа процесса - Разборка
// 
// Возвращаемое значение:
// 	Структура
//
Функция ПараметрыВыбораСпецификацийНаРазборку() Экспорт
	
	ПараметрыВыбора = ПараметрыВыбораСпецификаций();
	
	ПараметрыВыбора.ДоступныеТипы.Добавить(ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.Разборка"));
	ПараметрыВыбора.ДоступныеСтатусы.Добавить(ПредопределенноеЗначение("Перечисление.СтатусыСпецификаций.Действует"));
