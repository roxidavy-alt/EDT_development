...
Процедура ДополнитьЗначенияКонстантКонтролируемыхВБазовойВерсии(Результат) Экспорт 
	...
	Результат.Вставить(Метаданные.Константы.ИспользоватьСтатусыАктовВыполненныхВнутреннихРабот.Имя, Ложь);
	...
	Результат.Вставить(Метаданные.Константы.ИспользоватьСогласованиеРазрешенийНаЗаменуМатериалов.Имя, Ложь);
...
#Область БЗКР
	//++ НЕ УТКА
	НастройкиСистемы.ДобавитьСтрокуТаблицыЗависимостиКонстант(Результат,
		Метаданные.Константы.ИспользоватьНачислениеЗарплаты.Имя, Истина,
		Метаданные.Константы.НормированиеИРасценкаРаботПоТарифнымСеткам.Имя, Ложь);
	
	НастройкиСистемы.ДобавитьСтрокуТаблицыЗависимостиКонстант(Результат,
		Метаданные.Константы.НормированиеИРасценкаРаботПоТарифнымСеткам.Имя, Ложь,
		Метаданные.Константы.ДополнительныйРазрезТарифнойСетки.Имя, Перечисления.ДополнительныеРазрезыТарифнойСетки.БезДетализации);
...
Процедура ПриСозданииНаСервере_Продажи(Форма) Экспорт
	...
	Элементы.ПояснениеИспользованиеЭтаповОплатыВПродажах.Заголовок = НСтр(
		...
		|en = 'The setting determines possible payment planning options in sales orders, consignment issue notifications, and sales return requests.'");
	//++ НЕ УТ
	Элементы.ПояснениеИспользованиеЭтаповОплатыВПродажах.Заголовок = НСтр(
		"ru = 'Настройка определяет возможные варианты планирования оплаты в документах ""Заказ клиента"", ""Заявка на возврат товаров от клиента"", ""Заказ давальца"".';
		|en = 'The setting determines possible payment planning options in sales orders, sales return requests, and subcontracting sales orders.'");
	//-- НЕ УТ
...
Процедура ПриСозданииНаСервере_Производство(Форма) Экспорт
	...
	//++ НЕ УТКА
	Форма.ИспользоватьДополнительныйРазрезТарифнойСетки = НаборКонстант.ДополнительныйРазрезТарифнойСетки <> Перечисления.ДополнительныеРазрезыТарифнойСетки.БезДетализации;
	//-- НЕ УТКА
	...
	Если Элементы.ЛокализацияГруппаИспользоватьПроизводствоНаСтороне.Видимость Тогда
		Элементы.ИспользоватьПроизводствоНаСтороне2_5.Заголовок = НСтр("ru = 'Производство на стороне (версия 2.5)';
																		|en = 'Production by third party (version 2.5)'");
	КонецЕсли;
	//-- Устарело_Переработка24
	//-- Локализация
	Возврат
...
Процедура УстановитьДоступность_Производство(РеквизитПутьКДанным, Форма) Экспорт
	...
	Если РеквизитПутьКДанным = "" Тогда
		Элементы.ГруппаНормированиеИРасценкаРаботПоТарифнымСеткам.Видимость = Ложь;
	КонецЕсли;
	...
	Если РеквизитПутьКДанным = ""
		ИЛИ СтрНайти(РеквизитПутьКДанным, "НормированиеИРасценкаРаботПоТарифнымСеткам") > 0 Тогда
		ИспользоватьКадровыйУчет = НаборКонстант.ИспользоватьКадровыйУчет;
		...
		Элементы.ГруппаНормированиеИРасценкаРаботПоТарифнымСеткам.Видимость = НаборКонстант.ИспользоватьУправлениеПроизводством2_2
			И НЕ НаборКонстант.ИспользоватьУправлениеПроизводством;
		
		Элементы.ГруппаДетализацияТарифныхСтавок.Видимость = НЕ ИспользоватьКадровыйУчет;
		
		Элементы.НормированиеИРасценкаРаботПоТарифнымСеткам.Доступность = НЕ ИспользоватьКадровыйУчет
			ИЛИ ИспользоватьКадровыйУчет И ИспользоватьШтатноеРасписание И (ИспользоватьТарифныеСеткиПриРасчетеЗарплаты ИЛИ НаборКонстант.НормированиеИРасценкаРаботПоТарифнымСеткам);
		Элементы.ГруппаКомментарийНормированиеИРасценкаРаботПоТарифнымСеткам.Видимость = ИспользоватьКадровыйУчет
			И НЕ (ИспользоватьШтатноеРасписание И ИспользоватьТарифныеСеткиПриРасчетеЗарплаты);
		
		ЗначениеКонстанты = НаборКонстант.НормированиеИРасценкаРаботПоТарифнымСеткам;
		
		ОбщегоНазначенияУТКлиентСервер.ОтображениеПредупрежденияПриРедактировании(
			Элементы.НормированиеИРасценкаРаботПоТарифнымСеткам, ЗначениеКонстанты);
	КонецЕсли;
	
	Если РеквизитПутьКДанным = "" 
		ИЛИ СтрНайти(РеквизитПутьКДанным, "НормированиеИРасценкаРаботПоТарифнымСеткам") > 0 Тогда
		Элементы.ГруппаДетализацияТарифныхСтавок.Доступность = НаборКонстант.НормированиеИРасценкаРаботПоТарифнымСеткам;
		Элементы.ДополнительныйРазрезТарифнойСетки.Доступность = Форма.ИспользоватьДополнительныйРазрезТарифнойСетки;
	КонецЕсли;
	
	Если РеквизитПутьКДанным = "" 
		ИЛИ СтрНайти(РеквизитПутьКДанным, "ДополнительныйРазрезТарифнойСетки") > 0 Тогда
		Элементы.ДополнительныйРазрезТарифнойСетки.Доступность = НаборКонстант.ДополнительныйРазрезТарифнойСетки <> Перечисления.ДополнительныеРазрезыТарифнойСетки.БезДетализации;
		
		Если РеквизитПутьКДанным = "" И НаборКонстант.ДополнительныйРазрезТарифнойСетки <> Перечисления.ДополнительныеРазрезыТарифнойСетки.БезДетализации Тогда
			ОбщегоНазначенияУТКлиентСервер.ОтображениеПредупрежденияПриРедактировании(
				Элементы.ДополнительныйРазрезТарифнойСетки, Истина);
		КонецЕсли;
...
Процедура СохранитьЗначениеРеквизита_Производство(КонстантаИмя, КонстантаЗначение, Форма) Экспорт
	...
	Если КонстантаИмя = "НормированиеИРасценкаРаботПоТарифнымСеткам" 
		И НЕ Форма.НаборКонстант[КонстантаИмя] Тогда
		Форма.ИспользоватьДополнительныйРазрезТарифнойСетки = Ложь;
	КонецЕсли;
	//-- Локализация
	Возврат
КонецПроцедуры
