...
Процедура ЗаполнитьНастройкиПоУмолчаниюОФР(Процессор)
	...
	ДобавитьПоказатель(Процессор, НСтр("ru = 'Результаты инвентаризации';
										|en = 'Physical inventory count results'", КодЯзыка));
...
Процедура ЗаполнитьНастройкиПоУмолчаниюОДДС(Процессор)
	...
	ДобавитьПоказательДенежногоПотокаПоСчету(
		...
		НСтр("ru = 'Поступления от продаж по платежным картам';
			|en = 'Receipts from sales by payment cards'", КодЯзыка),
	...
	Если Не РаспределятьЕдиныйНалоговыйПлатеж Тогда
		...
		ДобавитьЗначениеСвернутогоДенежногоПотока(
			...
			НСтр("ru = 'Налог на добавленную стоимость в составе расчетов с контрагентами';
				|en = 'Value added tax as part of counterparty AR/AP'", КодЯзыка));
	...
	ДобавитьГруппуОтрицательныхЗначенийЕдиныйНалоговыйПлатеж(
			...
			НСтр("ru = 'Налоги (сборы, взносы) в составе единого налогового платежа';
				|en = 'Taxes (charges, contributions) as part of unified tax payment'", КодЯзыка),
		...
		УстановитьНаименованиеГруппыИзОбразца(НСтр("ru = 'налогов (сборов, взносов) в составе единого налогового платежа';
													|en = 'taxes (charges, contributions) as part of unified tax payment'", КодЯзыка), Процессор);
...
Функция ОбязательныеСчетаПродажи(Период)
	
	ОбязательныеСчета = Новый Массив;
	
	Если РеглУчетСервер.ИспользуетсяРеглУчетПродажПоПартиямРеализованнойНоменклатуры() Тогда
		ПериодыДетализацииУчета = РеглУчетСервер.ПериодыДетализацииУчета(Период.Начало, Период.КонецДата, "РеглУчетПродажПоПартиямРеализованнойНоменклатуры");
		Если ПериодыДетализацииУчета.ДанныеСДетализацией Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ОбязательныеСчета, РеглУчетВыборкиСерверПовтИсп.СчетаВыручка());
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ОбязательныеСчета, РеглУчетВыборкиСерверПовтИсп.СчетаСебестоимостьПродаж());
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ОбязательныеСчета, РеглУчетВыборкиСерверПовтИсп.СчетаПродажи_НДС());
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ОбязательныеСчета, РеглУчетВыборкиСерверПовтИсп.СчетаРасходыНаПродажу());
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ОбязательныеСчета, РеглУчетВыборкиСерверПовтИсп.СчетаУправленческиеРасходы());
		КонецЕсли;
		Если ПериодыДетализацииУчета.ДанныеБезДетализации Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ОбязательныеСчета, РеглУчетВыборкиСерверПовтИсп.УстаревшиеСчетаВыручка());
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ОбязательныеСчета, РеглУчетВыборкиСерверПовтИсп.УстаревшиеСчетаСебестоимостьПродаж());
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ОбязательныеСчета, РеглУчетВыборкиСерверПовтИсп.УстаревшиеСчетаПродажи_НДС());
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ОбязательныеСчета, РеглУчетВыборкиСерверПовтИсп.УстаревшиеСчетаРасходыНаПродажу());
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ОбязательныеСчета, РеглУчетВыборкиСерверПовтИсп.УстаревшиеСчетаУправленческиеРасходы());
		КонецЕсли;
	Иначе
...
Процедура ПолучитьДанныеУчетаПродажи(Процессор)
	...
	ОписаниеСчетов = ОписаниеСчетовУчета(Процессор.ДанныеАлгоритма.Счета, ОбязательныеСчетаПродажи(Процессор.Контекст.Период));
	...
	Если РеглУчетСервер.ИспользуетсяРеглУчетПродажПоПартиямРеализованнойНоменклатуры() Тогда     
		...
		Запрос.Текст = Запрос.Текст + "
		|ВЫБРАТЬ
		|	ЕстьNull(ВЫРАЗИТЬ(ОборотыДетально.Субконто1 КАК Справочник.ПартииРеализованнойНоменклатуры).Номенклатура.ГруппаФинансовогоУчета, 
		|		ЗНАЧЕНИЕ(Справочник.ГруппыФинансовогоУчетаНоменклатуры.ПустаяСсылка))КАК Объект,
		|	ОборотыДетально.Счет КАК Счет,
		|	ВЫБОР
		|		КОГДА ОборотыДетально.Счет.Вид = ЗНАЧЕНИЕ(ВидСчета.Активный)
		|			ТОГДА -ОборотыДетально.СуммаОборотДт
		...
		|	ОборотыДетально.Организация КАК Организация
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, , Счет В (&СчетаДетальноПартии), &АналитикаПартии, Организация В (&Организации), , ) КАК ОборотыДетально";
		АналитикаПартии = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПартииРеализованнойНоменклатуры;
		Процессор.ДанныеАлгоритма.Вставить("СчетаДетальноПартии", СчетаПрименяетсяАналитика(СчетаСводно, АналитикаПартии));
		СчетаСводно = ОбщегоНазначенияКлиентСервер.РазностьМассивов(СчетаСводно, Процессор.ДанныеАлгоритма.СчетаДетальноПартии);
		Запрос.УстановитьПараметр("СчетаСводно",   СчетаСводно);
		Запрос.УстановитьПараметр("СчетаДетальноПартии", Процессор.ДанныеАлгоритма.СчетаДетальноПартии);
		Запрос.УстановитьПараметр("АналитикаПартии",     АналитикаПартии);		
	КонецЕсли;
...
Процедура СоздатьДенежныеПотокиКонтрагентов(Процессор)
	...
	|	Потоки.Организация КАК Организация,
	|	Потоки.Счет КАК Счет,
	|	ВЫБОР КОГДА Потоки.Счет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПродажиПоПлатежнымКартам))
...
Функция ПредставлениеПоказателяКодыСтрок(Показатель, КодыСтрок, КодЯзыка)
	...
	Возврат СтрШаблон(НСтр("ru = '%1 (см. %2)';
							|en = '%1 (see %2)'", КодЯзыка), Показатель.Наименование, ПредставлениеКодовСтрок);
...
Процедура ОписатьПорядокЗаполненияПоказателяДенежныеПотоки(Описание, Показатель, ДанныеАлгоритма, КодЯзыка)
	...
	Если СодержитДвижениеДенежныхСредств Тогда
		...
		Если Показатель.Заполнение.Отбор.Свойство("Счет") Тогда
			...
			ТекстРазъяснения.Добавить(НСтр(
				...
				|en = 'There is no cash item accounting on this account. Its entire turnover belongs to one report indicator'",
				КодЯзыка));
		...
		Иначе
			...
			ШаблонТекста = НСтр("ru = 'Оборот по счетам денежных средств и денежных эквивалентов %1.';
								|en = 'The turnover on cash accounts and cash equivalent accounts %1.'", КодЯзыка);
			ОписаниеСтатей = "";
			...
			Если Не КомбинированныйПоказатель Тогда
				ТекстОписания.Добавить(НСтр("ru = 'Список счетов приведен ниже.';
											|en = 'The list of accounts is given below.'", КодЯзыка));
			КонецЕсли;
	...
	Если Показатель.Заполнение.Отбор.Свойство("Свернутый") Тогда
		Если Показатель.Заполнение.Отбор.Свернутый = "АгентПродавца" Тогда
			ТекстРазъяснения.Добавить(НСтр(
				...
				|en = 'Conditionally consider that the agent received the amounts paid to consignors in full from customers'",
				КодЯзыка));
		ИначеЕсли Показатель.Заполнение.Отбор.Свернутый = "АгентПокупателя" Тогда
			ТекстРазъяснения.Добавить(НСтр(
				"ru = 'Условно считаем, что сумму, полученную от комитента для приобретения чего-либо, агент в полном объеме заплатил поставщикам.
                 ...
                 |en = 'Conditionally consider that the agent paid the amount received from the consignor to purchase something in full to vendors.
                 |It is assumed that the amounts received for payments to vendors are recorded in accounts of other settlements, and the commission is recorded in accounts of settlements with customers.'",
				КодЯзыка));
			...
			Если Не СодержитДвижениеДенежныхСредств Тогда
				ТекстОписания.Добавить(НСтр(
					"ru = 'Сальдирует денежные потоки по налогу на добавленную стоимость
                     ...
                     |en = 'Balances cash flows from value added tax
                     |- Excluded from counterparty AR/AP.
                     | - Transferred from the unified tax payment.'", 
					КодЯзыка));
			Иначе
				ТекстОписания.Добавить(НСтр(
					"ru = 'Также сальдирует денежные потоки по налогу на добавленную стоимость, исключенные из расчетов с контрагентами.';
					|en = 'Also balances cash flows from value added tax excluded from counterparty AR/AP.'",
					КодЯзыка));
			КонецЕсли;
		Иначе
	...
	Если БухгалтерскаяОтчетностьБРО.ИдентификаторПоказателя(Показатель) = ДанныеАлгоритма.ПоказательЕдиныйНалоговыйПлатежБезРаспределения Тогда
		ТекстРазъяснения.Добавить(НСтр(
			...
			|en = 'Unified tax payment is presented in the report as a total amount
			|(cl. 2 of the AMC Recommendation R-175/2025-KpR ""Unified tax account"")'",
			КодЯзыка));
	КонецЕсли;
	...
	Если ЗаписьРасчет.ТипЗаписи = "ЭтоДвижениеДенежныхСредств" Тогда
		Если СоставШагаРасчета.ЭтоДвижениеДенежныхСредств Тогда
			ЗаписьРасчет.Описание = НСтр("ru = 'Оборот по счетам учета денежных средств';
										|en = 'Turnover on cash accounts'", КодЯзыка);
		Иначе
			ЗаписьРасчет.Описание = НСтр("ru = 'Корректировки других показателей отчета';
										|en = 'Adjustments to other report indicators'", КодЯзыка);
		КонецЕсли;
		...
		Иначе
			...
			ИначеЕсли СоставШагаРасчета.ВидКорректировки = "ВременныйУчетЕдиногоНалоговогоПлатежа" Тогда
				ЗаписьРасчет.Описание = НСтр("ru = 'Суммы единого налогового платежа перенесены в одноименный показатель';
											|en = 'Amounts of a unified tax payment are transferred to the same-named indicator'", КодЯзыка);
			КонецЕсли;
		КонецЕсли;
...
Функция ПредставлениеПредметаКорректировки(ВидКорректировки, ПредставленияСворачиваемыхДенежныхПотоков, КодЯзыка)
	...
	Если ВидКорректировки = "ВременныйУчетЕдиногоНалоговогоПлатежа" Тогда
		Возврат НСтр("ru = 'единый налоговый платеж';
					|en = 'unified tax payment'", КодЯзыка);
	Иначе
