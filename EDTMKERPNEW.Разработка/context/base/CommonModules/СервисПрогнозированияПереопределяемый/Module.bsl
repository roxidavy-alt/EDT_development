...
Функция ЗаключениеПоПоказателямКачестваДанныхИнформационнойБазы(ПоказателиКачества, БазаПодключена) Экспорт
	...
	Если Категория = 0
		...
		ТекстПояснения = НСтр("ru = 'В информационной базе недостаточно подходящих данных для работы с сервисом прогнозирования продаж.
			...
			|en = 'The infobase does not have enough data to use the sales forecasting service.
			|The infobase is connected but cannot operate with the available data, re-export is prohibited.'");
	...
	ИначеЕсли Категория = 1
		...
		ТекстПояснения = НСтр("ru = 'Данные в информационной базе подходят для работы с сервисом прогнозирования продаж, но качество прогноза по ним ограничено.
			...
			|en = 'Data in the infobase is appropriate to use the sales forecasting service, but the quality of forecasting based on it is limited.
			|The infobase is connected, you can continue and re-export the data for forecasting.'");
	ИначеЕсли Категория = 1
		И Не БазаПодключена Тогда
		ТекстПояснения = НСтр("ru = 'Данные в информационной базе подходят для работы с сервисом прогнозирования продаж, но качество прогноза по ним ограничено.
			|Рекомендуется продолжить подключение и получить первый прогноз.';
			|en = 'Data in the infobase is appropriate to use the sales forecasting service, but the quality of forecasting based on it is limited.
			|We recommend that you continue the connection and get the first forecast.'");
	ИначеЕсли Категория = 2
		...
		ТекстПояснения = НСтр("ru = 'Данные в информационной базе подходят для работы с сервисом прогнозирования продаж.
			|База подключена, можно продолжить работу и перевыгрузить данные для прогнозирования.';
			|en = 'Data in the infobase is appropriate to use the sales forecasting service.
			|The infobase is connected, you can continue and re-export the data for forecasting.'");
	ИначеЕсли Категория = 2
		И Не БазаПодключена Тогда
		ТекстПояснения = НСтр("ru = 'Данные в информационной базе подходят для работы с сервисом прогнозирования продаж.
...
Функция РегистраторыНевыполненныхЗаказов()
	...
	//++ НЕ УТКА
	Регистраторы.Добавить(Тип("ДокументСсылка.ЗаказДавальца2_5"));
	//++ Устарело_Переработка24
	Регистраторы.Добавить(Тип("ДокументСсылка.ЗаказДавальца"));
	Регистраторы.Добавить(Тип("ДокументСсылка.ПередачаДавальцу"));
	//-- Устарело_Переработка24
	Регистраторы.Добавить(Тип("ДокументСсылка.ОтчетДавальцу2_5"));
	Регистраторы.Добавить(Тип("ДокументСсылка.ЭтапПроизводства2_2"));
	//-- НЕ УТКА
	Регистраторы.Добавить(Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента"));
	//++ НЕ УТ
	Регистраторы.Добавить(Тип("ДокументСсылка.ЗаказПереработчику2_5"));
	//++ Устарело_Переработка24
	Регистраторы.Добавить(Тип("ДокументСсылка.ЗаказПереработчику"));
	Регистраторы.Добавить(Тип("ДокументСсылка.ПередачаСырьяПереработчику"));
	//-- Устарело_Переработка24
	...
	Если Категория = 0 Тогда
		ТекстПояснения = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<b>Длина истории продаж</b>: недостаточно продаж для работы сервиса прогнозирования; в расширенном режиме имеется возможность учесть продажи из другой информационной базы.';
				|en = '<b>Sale history length</b>: there are not enough sales to use the forecasting service. In advanced mode, you can account for sales from another infobase.'"));
	ИначеЕсли Категория = 1 Тогда
		ТекстПояснения = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<b>Длина истории продаж</b>: продажи ведутся менее 2 лет, из-за чего невозможно вычислить сезонность; в расширенном режиме имеется возможность учесть продажи из другой информационной базы.';
				|en = '<b>Sale history length</b>: there are sales for less than two years, so it is impossible to calculate the seasonality. In advanced mode, you can account for sales from another infobase.'"));
	ИначеЕсли Категория = 2 Тогда
		ТекстПояснения = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<b>Длина истории продаж</b>: данных достаточно для построения прогноза и вычисления сезонных коэффициентов.';
				|en = '<b>Sale history length</b>: there is not enough data to make a forecast and calculate seasonal indexes.'"));
	КонецЕсли;
	...
	Если Категория = 0 Тогда
		ТекстПояснения = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<b>Количество дней продаж потоварно</b>: нет ни одного товара, по которому есть хотя бы 7 дней продаж.';
				|en = '<b>Number of sales days by items</b>: there are no items with at least seven days of sales.'"));
	ИначеЕсли Категория = 2 Тогда
		ТекстПояснения = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<b>Количество дней продаж потоварно</b>: по самому популярному товару есть 7 или более дней продаж.';
				|en = '<b>Number of sales days by items</b>: there are seven or more sales days for the most popular item.'"));
	КонецЕсли;
	...
	Если Категория = 0 Тогда
		ТекстПояснения = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<b>Количество продаж в год</b>: текущей регулярности продаж (менее 5 в год) недостаточно для работы сервиса прогнозирования.';
				|en = '<b>Number of sales per year</b>: the current sales frequency (less than five sales per year) is not enough for the sales forecasting service.'"));
	ИначеЕсли Категория = 1 Тогда
		ТекстПояснения = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<b>Количество продаж в год</b>: при текущей регулярности продаж (менее 50 в год) не рекомендуется построение прогноза в разрезе дней. Доступно построение прогноза в разрезе недель и месяцев.';
				|en = '<b>Number of sales per year</b>: with the current sales frequency (less than 50 sales a year) it is not recommended that you make forecasts broken down by days. You can  make forecasts broken down by weeks and months.'"));
	ИначеЕсли Категория = 2 Тогда
		ТекстПояснения = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<b>Количество продаж в год</b>: высокое количество продаж в год позволяет выбрать любую из доступных детализаций (день, неделя, месяц).';
				|en = '<b>Number of sales per year</b>: with a great number of sales per year, you can select any of the available drilldowns (day, week, and month).'"));
	КонецЕсли;
	...
	Если Категория = 1 Тогда
		ТекстПояснения = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<b>Активность продаж</b>: недостаточно товаров, которые продавались в течение последнего года.';
				|en = '<b>Sales performance</b>: there are not enough goods that were sold over the past year.'"));
	ИначеЕсли Категория = 2 Тогда
		ТекстПояснения = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<b>Активность продаж</b>: имеется большое количество товаров, которые продавались в течение последнего года.';
				|en = '<b>Sales performance</b>: there is a great number of goods that were sold over the past year.'"));
	КонецЕсли;
	...
	Если Категория = 1 Тогда
		ТекстПояснения = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<b>Отрицательные продажи</b>: есть дни, когда за день вернулось больше товара, чем было продано; отрицательные продажи не будут учитываться сервисом прогнозирования.';
				|en = '<b>Negative sales</b>: there are days when more goods were returned than sold within a day. The forecasting service does not consider negative sales.'"));
	ИначеЕсли Категория = 2 Тогда
		ТекстПояснения = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<b>Отрицательные продажи</b>: не обнаружены, либо их количество не велико; отрицательные продажи не будут учитываться сервисом прогнозирования.';
				|en = '<b>Negative sales</b>: not found, or there are few of them. The forecasting service does not consider negative sales.'"));
	КонецЕсли;
	...
	Если Категория = 1 Тогда
		ТекстПояснения = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<b>Новинки</b>: имеется большое количество товаров, которые продаются менее полугода; качество прогнозирования по ним ограничено.';
				|en = '<b>New items</b>: there are many goods that are on sale for less than six months. Their forecasting quality is limited.'"));
	ИначеЕсли Категория = 2 Тогда
		ТекстПояснения = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<b>Новинки</b>: не обнаружены, либо их количество небольшое. Это положительно влияет на качество прогнозирования.';
				|en = '<b>New items</b>: not found, or there are few of them. It improves the forecasting quality.'"));
	КонецЕсли;
	...
	Если Категория = 1 Тогда
		ТекстПояснения = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<b>Частота продаж</b>: часто продаваемых товаров менее половины, что может снизить общее качество прогноза продаж.';
				|en = '<b>Sales frequency</b>: less than half of goods are frequently sold. It can decrease the overall sales forecast quality.'"));
	ИначеЕсли Категория = 2 Тогда
		ТекстПояснения = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<b>Частота продаж</b>: имеется большое количество часто продаваемых товаров, что положительно влияет на качество прогнозирования.';
				|en = '<b>Sales frequency</b>: there are many frequently sold goods. It improves the forecasting quality.'"));
	КонецЕсли;
...
Функция ТекстФильтрыЗагрузкиИсключаютВсеДанныеПрогноза(ВидФильтра)
	...
	Если ВидФильтра = "Общий" Тогда
		ТекстОшибки = НСтр("ru = 'Общий фильтр на загрузку исключил все данные прогноза.
			...
			|en = 'The common import filter excluded all forecast data.
			|Adjust the filter and try to get the forecast again.
			|The filter is configured in the ""4. Filters"" section of the connection wizard.'");
	ИначеЕсли ВидФильтра = "ВидаПлана" Тогда
		ТекстОшибки = НСтр("ru = 'Фильтр на загрузку у вида плана исключил все данные прогноза.
			...
			|en = 'The import filter of the plan profile excluded all forecast data.
			|Adjust the filter and try to get the forecast again.
			|The filter is configured in the ""Population settings"" section of the plan profile.'");
	Иначе // Комбинация фильтров.
		ТекстОшибки = НСтр("ru = 'Фильтры на загрузку (общий и у вида плана) исключили все данные прогноза.
			...
			|en = 'The import filters (common and the plan profile''s one) excluded all forecast data.
			|Adjust the filters and try to get the forecast again.
			|Filters are configured:
			|1) Common filter: in the ""4. Filters"" section of the connection wizard.
			|2) Plan profiles filter: in the ""Population settings"" section.'");
	КонецЕсли;
