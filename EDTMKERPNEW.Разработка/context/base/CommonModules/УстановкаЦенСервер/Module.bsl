			...
			Если ВидЦены.Выбрана
				...
				Если Не ЭтоРегистрацияЦенПоставщика
					...
					Если УказыватьОбщуюСтавкуНДС ИЛИ УказыватьСтавкиНДСПострочно Тогда
						...
						НоваяГруппаСтавкиНДС = ДобавитьГруппуФормы(Форма, "ГруппаСтавкаНДС" + ИмяКолонки, НСтр("ru = 'Ставка НДС';
																												|en = 'VAT rate'"), Ложь, Неопределено, ГруппировкаКолонок.ВЯчейке, НоваяГруппа);
						...
						Если УказыватьСтавкиНДСПострочно Тогда
							...
							СтруктураПараметров.Вставить("Подсказка",              НСтр("ru = 'Ставка НДС изменена вручную';
																						|en = 'VAT rate is changed manually'"));
	...
	Если ТаблицаНедоступныхТоваров.Количество() Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Установка цен недоступна для товаров:
				|%1';
				|en = 'Price setup is unavailable for goods:
				|%1'"),
		...
		Исключение
			...
			Если ЗначениеЗаполнено(СтрокаТаблицыЦен.Характеристика) Тогда
				ТекстСообщения = НСтр("ru = 'Для номенклатуры ""%Номенклатура%"" с характеристикой ""%Характеристика%"" не рассчитана цена по виду цен ""%ВидЦены%"" по причине: %ОписаниеОшибки%. Проверьте корректность формулы.';
										|en = 'The price is not calculated for the ""%Характеристика%"" variant of the ""%Номенклатура%"" item by the ""%ВидЦены%"" price type. Reason: %ОписаниеОшибки%. Check whether the formula is correct.'");
			Иначе
				ТекстСообщения = НСтр("ru = 'Для номенклатуры ""%Номенклатура%"" не рассчитана цена по виду цен ""%ВидЦены%"" по причине: %ОписаниеОшибки%. Проверьте корректность формулы.';
										|en = 'The price is not calculated for the ""%Номенклатура%"" item by the ""%ВидЦены%"" price type. Reason: %ОписаниеОшибки%. Check whether the formula is correct.'");
			КонецЕсли;
...
Функция ТоварыНедоступныеДляУстановкиЦен(ТаблицаТовары)
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрока.Причина = НСтр("ru = 'набор, цена складывается из цен комплектующих';
									|en = 'set, the price is a sum of component prices'");
