...
Процедура ДобавитьПредопределенныеСчета(СчетаСУсловнымиКодами) Экспорт
	...
	СчетаДополнительныхАналитик = СчетаДополнительныхАналитик();
	Для Каждого Счет Из СчетаДополнительныхАналитик Цикл
		СтрокаСчета = СчетаСУсловнымиКодами.Добавить();
		СтрокаСчета.УсловныйКод = Счет.Код;
...
Процедура ДополнитьАлгоритмыБО2025Кв1Баланс(Алгоритмы, ПараметрыЗаполнения) Экспорт
	
	ДополнитьАлгоритмыБО2025Кв1БалансERP(Алгоритмы, ПараметрыЗаполнения);
...
Процедура ПереопределитьЗапросСтруктураНДСвДДС(Запрос) Экспорт
	...
	МассивТекстовЗапроса = Новый Массив;
	МассивВидовСчетов = СтрРазделить("ДенежныеСредства,ПродажиПоЭквайрингу", ",");
	...
	|
	|ИЗ
	|	РегистрНакопления.ДвиженияДенежныеСредстваКонтрагент.Обороты(&НачПериода, &КонПериода, Месяц, Организация = &Организация) КАК ДанныеРегистра
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПорядокОтраженияНаСчетахУчетаПоПериодуДействия КАК ПорядокОтраженияДенежныхСредств
	...
	|				И ДанныеРегистра.Период >= ПорядокОтраженияДенежныхСредств.НачалоДействия
	|				И ДанныеРегистра.Период < ПорядокОтраженияДенежныхСредств.ОкончаниеДействия
	|	
	|ГДЕ
	...
	|
	|ИЗ
	|	РегистрНакопления.ДвиженияДенежныеСредстваДоходыРасходы.Обороты(&НачПериода, &КонПериода, Месяц, Организация = &Организация) КАК ДанныеРегистра
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПорядокОтраженияНаСчетахУчетаПоПериодуДействия КАК ПорядокОтраженияДенежныхСредств
	|			ПО ПорядокОтраженияДенежныхСредств.ВидСчета В (ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.ДенежныеСредства), ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.ПродажиПоЭквайрингу))
	|				И ПорядокОтраженияДенежныхСредств.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|				И ПорядокОтраженияДенежныхСредств.АналитикаУчета = ДанныеРегистра.ДенежныеСредства
...
Процедура ДополнитьАлгоритмыБО2025Кв1БалансERP(Алгоритмы, ПараметрыЗаполнения)
	
	ИспользуетсяРеглУчетНЗППоПартиямПроизводства = РеглУчетСервер.ИспользуетсяРеглУчетНЗППоПартиямПроизводства();
#Область Строка1210
	Если ИспользуетсяРеглУчетНЗППоПартиямПроизводства Тогда
		СчетаДополнительныхАналитик = Новый Массив;
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДополнительныхАналитик,РеглУчетВыборкиСерверПовтИсп.УстаревшиеСчетаОсновноеПроизводство());
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДополнительныхАналитик,РеглУчетВыборкиСерверПовтИсп.УстаревшиеСчетаВспомогательныеПроизводства());
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДополнительныхАналитик,РеглУчетВыборкиСерверПовтИсп.СчетаМатериалыПодрядныеРаботыОсновногоПроизводстваКИспользованию());
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДополнительныхАналитик,РеглУчетВыборкиСерверПовтИсп.СчетаРасходыОсновногоПроизводстваКРаспределению());
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДополнительныхАналитик,РеглУчетВыборкиСерверПовтИсп.СчетаПроизводственныеЗатратыОсновногоПроизводства());
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДополнительныхАналитик,РеглУчетВыборкиСерверПовтИсп.СчетаИзготовленнаяПродукцияИВыполненныеРаботыОсновногоПроизводства()); 
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДополнительныхАналитик,РеглУчетВыборкиСерверПовтИсп.СчетаМатериалыПодрядныеРаботыВспомогательныхПроизводствКИспользованию());
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДополнительныхАналитик,РеглУчетВыборкиСерверПовтИсп.СчетаРасходыВспомогательныхПроизводствКРаспределению());
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДополнительныхАналитик,РеглУчетВыборкиСерверПовтИсп.СчетаПроизводственныеЗатратыВспомогательныхПроизводств());
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДополнительныхАналитик,РеглУчетВыборкиСерверПовтИсп.СчетаИзготовленнаяПродукцияИВыполненныеРаботыВспомогательныхПроизводств());
		УдалитьСоставляющую(Алгоритмы,Составляющая("+ СКД 20"), "1210");
		УдалитьСоставляющую(Алгоритмы,Составляющая("+ СКД 23"), "1210");
		Для Каждого Счет Из СчетаДополнительныхАналитик Цикл
			ДополнитьАлгоритмы(Алгоритмы, Составляющая("+ СКД " + Счет.Код), "1210");
		КонецЦикла;
		
		УдалитьСоставляющую(Алгоритмы,Составляющая("+ СКД 20"), "12105", "1210");
		УдалитьСоставляющую(Алгоритмы,Составляющая("+ СКД 23"), "12105", "1210");
		Для Каждого Счет Из СчетаДополнительныхАналитик Цикл
			ДополнитьАлгоритмы(Алгоритмы, Составляющая("+ СКД " + Счет.Код), "12105", "1210");
		КонецЦикла;
...
Функция СчетаДополнительныхАналитик()
	СчетаДополнительныхАналитик = Новый Массив;
	...
	Если РеглУчетСервер.ИспользуетсяРеглУчетПродажПоПартиямРеализованнойНоменклатуры() Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДополнительныхАналитик,РеглУчетВыборкиСерверПовтИсп.УстаревшиеСчетаПрибыльУбытокОтПродаж());
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДополнительныхАналитик,РеглУчетВыборкиСерверПовтИсп.СчетаВыручка());
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДополнительныхАналитик,РеглУчетВыборкиСерверПовтИсп.СчетаСебестоимостьПродаж());
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДополнительныхАналитик,РеглУчетВыборкиСерверПовтИсп.СчетаРасходыНаПродажу());
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДополнительныхАналитик,РеглУчетВыборкиСерверПовтИсп.СчетаУправленческиеРасходы());
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДополнительныхАналитик,РеглУчетВыборкиСерверПовтИсп.СчетаПрибыльУбытокОтПродаж());
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДополнительныхАналитик,РеглУчетВыборкиСерверПовтИсп.СчетаФинансовогоРезультатаСДетализацией());
	КонецЕсли;
	Возврат СчетаДополнительныхАналитик;
КонецФункции
...
Процедура УдалитьСоставляющую(Алгоритмы, Составляющая, НомерСтроки, НомерОсновнойСтроки = Неопределено)
	...
	Если Строка <> Неопределено Тогда
		Если НомерОсновнойСтроки <> Неопределено Тогда
			ДопСтрока = АлгоритмыСтроки(Строка.ДопСтроки, НомерСтроки); 
			Если ДопСтрока <> Неопределено Тогда  
				Инд = ДопСтрока.Составляющие.Количество()-1;
				Пока Инд > 0 Цикл
					Если ДопСтрока.Составляющие[Инд][0] = Составляющая[0] 
						И ДопСтрока.Составляющие[Инд][1] = Составляющая[1] Тогда
						ДопСтрока.Составляющие.Удалить(Инд);
					КонецЕсли;
					Инд = Инд - 1;
				КонецЦикла;
			КонецЕсли;
		Иначе   
			Инд = Строка.Составляющие.Количество()-1;
			Пока Инд > 0 Цикл
				Если Строка.Составляющие[Инд][0] = Составляющая[0]
					И Строка.Составляющие[Инд][1] = Составляющая[1] Тогда
					Строка.Составляющие.Удалить(Инд);
				КонецЕсли;
				Инд = Инд - 1;
			КонецЦикла;
		КонецЕсли;
