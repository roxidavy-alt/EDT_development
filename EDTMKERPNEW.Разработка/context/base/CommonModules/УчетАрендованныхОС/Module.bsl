...
Процедура ДобавитьЭтап_НачислениеПроцентовПоАренде(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Начисление процентов по аренде';
											|en = 'Accrual of rental interest'"; // @НСтр-1
	НоваяСтрока.ПредшествующиеЭтапы.Добавить(Справочники.ОперацииЗакрытияМесяца.НачислениеАмортизацииОС);
	НоваяСтрока.ПредшествующиеЭтапы.Добавить(Справочники.ОперацииЗакрытияМесяца.НачислениеАмортизацииНМА);
...
Процедура ДобавитьЭтап_НачислениеПроцентовПоДоходнойАренде(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.НачислениеПроцентовПоДоходнойАренде);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Начисление процентов по доходной аренде';
											|en = 'Accrue profitable rental interest'"; // @НСтр-1
	НоваяСтрока.ТекстВыполнить = НСтр("ru = 'Начислить';
										|en = 'Charge'");
...
Процедура СформироватьЗаданияКНачислениюПроцентовПоАренде(ДанныеТаблиц)
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.НачислениеПроцентовПоАренде) КАК Операция,
...
Процедура СформироватьЗаданияКНачислениюПроцентовПоДоходнойАренде(Документ, ДанныеТаблиц)
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.НачислениеПроцентовПоДоходнойАренде) КАК Операция,
	|	ИзмененныеДанные.Организация КАК Организация,
	|	ИзмененныеДанные.Документ КАК Документ,
	|	МИНИМУМ(ВЫБОР 
	|				КОГДА ИзмененныеДанные.Период < &ДатаНачалаУчета24
	...
	Иначе
		...
		Если ПараметрыДокумента.Балансодержатель = Перечисления.БалансодержательПредметовАренды.Арендодатель Тогда
			...
			ОССтоимость_Подсказка = НСтр("ru = 'Залоговая стоимость предмета аренды';
										|en = 'Collateral value of rental object'");
			...
			Если НЕ СтрокиОплатПоОС.Количество() Тогда
				...
				Если ВыводитьСообщения Тогда
					...
					ЗарегистрироватьПроблемуВыполненияРасчета(
						Справочники.ОперацииЗакрытияМесяца.РасчетСтоимостиИнвестицииВАренду, 
...
Процедура ПроверитьВозвратИзСубаренды(Объект, Отказ, ОтборПоДействиям = Ложь) Экспорт
	...
	Если Не Результат.Пустой() Тогда
		...
		ШаблонСообщения = НСтр("ru = 'Основное средство ""%1"" передано в субаренду по договору %2, документ передачи в аренду %3.
								...
								|Before you terminate the fixed asset lease, return it from sublease.'"); 
