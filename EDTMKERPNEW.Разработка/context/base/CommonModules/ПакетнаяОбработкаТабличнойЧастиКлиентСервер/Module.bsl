...
Функция ПолучитьСтруктуруЗаполненияСодержанияУслугиВСтрокеТЧ(Объект, ЗаполнятьДляВсехУслуг) Экспорт
	...
	Если Объект.Свойство("ХозяйственнаяОперация")
		 И (Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПроизводствоИзДавальческогоСырья2_5")
			Или Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПроизводствоПродукцииИзДавальческогоСырья2_5")
			Или Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.РемонтПродукцииИзДавальческогоСырья2_5")
			...
			//-- Устарело_Переработка24
			Или Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВыполнениеРаботСИспользованиемДавальческогоСырья2_5")
			Или Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВыполнениеРемонтныхРаботСИспользованиемДавальческогоСырья2_5")) Тогда
...
Функция СуммаНДСССуффиксом(Суффикс) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Неизвестный суффикс для суммы НДС:';
								|en = 'Unknown suffix for VAT amount:'") + " " + Суффикс;
	КонецЕсли;
				...
				Если СтруктураПараметров.ИспользоватьРеглУчет Тогда
					...
					Иначе
						Представление = НСтр("ru = 'Упр.: %1% (%2%), Регл.: %3% (%4%)';
											|en = 'Mgmt: %1% (%2%), Local: %3% (%4%)'");
					КонецЕсли;
				Иначе
					Если СтруктураПараметров.НалогообложениеНДС = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС") Тогда
						Представление = НСтр("ru = 'Упр. с НДС: %1% (%2%)';
											|en = 'Mgmt including VAT: %1% (%2%)'");
					Иначе
