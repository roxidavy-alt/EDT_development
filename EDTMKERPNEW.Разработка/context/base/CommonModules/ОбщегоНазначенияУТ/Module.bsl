...
Процедура ПриОпределенииПодчиненныхОбъектов(ПодчиненныеОбъекты) Экспорт
	...
	//++ НЕ УТ
	ПодчиненныйОбъект = ПодчиненныеОбъекты.Добавить();
	ПодчиненныйОбъект.ПодчиненныйОбъект = Метаданные.Справочники.ПартииРеализованнойНоменклатуры;
	ПодчиненныйОбъект.ПоляСвязей = Справочники.ПартииРеализованнойНоменклатуры.КлючевыеРеквизиты();
	ПодчиненныйОбъект.ПриПоискеЗаменыСсылок = "Справочники.ПартииРеализованнойНоменклатуры";
	ПодчиненныйОбъект.ВыполнятьАвтоматическийПоискЗаменСсылок = Истина;
	//-- НЕ УТ
		...
		Пока Выборка.Следующий() Цикл
			...
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Партнер %1 связан с внешним пользователем. Перед заменой партнера необходимо удалить внешнего пользователя.';
					|en = 'The %1 partner is connected to an external user. Before replacing the partner, delete the external user.'"),
