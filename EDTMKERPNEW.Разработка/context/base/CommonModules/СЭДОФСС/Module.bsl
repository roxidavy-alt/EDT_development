...
Функция ВыплатаПособияПереведенаНаЕЦП(Документ, ВидПособия) Экспорт
	...
	Если ВидПособия = Перечисления.ПособияНазначаемыеФСС.ЕдиновременноеПособиеПриРожденииРебенка Тогда
		...
		Возврат ТекущаяДатаФонда() >= '20260505000000';
	...
	ИначеЕсли ВидПособия = Перечисления.ПособияНазначаемыеФСС.ПособиеПоБеременностиИРодам
		...
		Возврат ТекущаяДатаФонда() >= '20260513000000';
...
Функция СписокВыбораКатегорийЗастрахованныхЛиц(СписокВыбора = Неопределено) Экспорт
	...
	СписокВыбора.Добавить("ENDOFMILITARY", НСтр("ru = 'Лицо, в период приостановления ТД после окончания военной службы';
												|en = 'A person during employment contract suspension after completing military service'"));
