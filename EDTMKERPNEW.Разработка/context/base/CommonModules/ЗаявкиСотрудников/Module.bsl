...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий 		= НСтр("ru = 'Обновление типа дополнительной адресации роли Обращения сотрудников.';
											|en = 'Обновление типа дополнительной адресации роли Обращения сотрудников.'");
	...
	Обработчик.Комментарий 		= НСтр("ru = 'Обновление назначений ролей исполнителей задач.';
											|en = 'Обновление назначений ролей исполнителей задач.'");
...
Процедура ПриНачальномЗаполненииРолейИсполнителей(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	Элемент = ОбновлениеБЗК.ОписаниеПредопределенного(КодыЯзыков, Элементы, "ОбработкаЗаявленийВПроизвольнойФорме",
		"ru = 'Обработка заявлений в произвольной форме';
		|en = 'Обработка заявлений в произвольной форме'", Реквизиты);
	Назначение = ТабличныеЧасти.Назначение.Скопировать();
	ЭлементТЧ = Назначение.Добавить();
	ЭлементТЧ.ТипПользователей = Справочники.Пользователи.ПустаяСсылка();
	Элемент.Назначение = Назначение;
	...
	Элемент = ОбновлениеБЗК.ОписаниеПредопределенного(КодыЯзыков, Элементы, "ОтветственныйЗаСпособыВыплатыЗарплаты",
		"ru = 'Способы выплаты зарплаты';
		|en = 'Способы выплаты зарплаты'", Реквизиты);
	Назначение = ТабличныеЧасти.Назначение.Скопировать();
	ЭлементТЧ = Назначение.Добавить();
	ЭлементТЧ.ТипПользователей = Справочники.Пользователи.ПустаяСсылка();
	Элемент.Назначение = Назначение;
...
Процедура АннулироватьНеОбработанныеЗаявки() Экспорт
	...
	Пока Выборка.СледующийПоЗначениюПоля("Заявка") Цикл
		...
		ЗаявкаОбъект.Комментарий = НСтр("ru = 'Аннулирована при отключении сервиса.';
										|en = 'Аннулирована при отключении сервиса.'");
		Попытка
			ЗаявкаОбъект.Записать();
		Исключение
		...
		Пока Выборка.Следующий() Цикл
			...
			ЗадачаОбъект.РезультатВыполнения 	= НСтр("ru = 'Задача выполнена при отключении сервиса.';
														|en = 'Задача выполнена при отключении сервиса.'");
			Попытка
