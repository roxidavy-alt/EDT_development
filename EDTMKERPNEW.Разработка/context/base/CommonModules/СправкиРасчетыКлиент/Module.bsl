...
Процедура УстановитьТекущийПериод(Форма, СтандартнаяОбработка, ОбработчикНастроитьПериодНаСервере) Экспорт
	...
	ИзменениеПериодаМесяц(Форма, ТекущаяДата(), ОбработчикНастроитьПериодНаСервере);
...
Функция НастроитьПериодИнтервал(Форма, ЦелыйИнтервал = Неопределено) Экспорт
	...
	Если Форма.Элементы.Найти("ВариантНалогообложенияПрибыли") <> Неопределено Тогда
		ТребуетсяВызовСервера.Вставить("НастроитьЭлементВариантНалогообложенияПрибыли");
	КонецЕсли;
