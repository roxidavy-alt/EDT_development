...
Процедура ИсправитьПроблемыВДвиженияхИДокументах(ПараметрыРасчета) Экспорт
	...
	РасшифровкаКодовОшибок = Новый Соответствие;
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru = 'Лишние движения по регистру ""Партии прочих расходов"" при УСН.';
											|en = 'Лишние движения по регистру ""Партии прочих расходов"" при УСН.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//++ НЕ УТ
	РасшифровкаКодовОшибок.Вставить(2, НСтр("ru = 'Не заполнено направление деятельности в движениях документа ""Резервы отпусков"" по регистру ""Партии прочих расходов"".';
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ПерепровестиДокументыПоОтдельнымРегистрамНакопления(
		...
		Запрос,
		РасшифровкаКодовОшибок,
		НСтр("ru = 'ошибки в движениях по регистру ""Партии прочих расходов""';
			|en = 'ошибки в движениях по регистру ""Партии прочих расходов""'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		Исключение
			...
			ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось создать ключ аналитики без назначения для аналитики учета номенклатуры ""%1"":
					|%2';
					|en = 'Cannot generate the dimension key without the assignment for the ""%1"" inventory location attributes:
		...
		Исключение
			...
			ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось создать ключ аналитики для номенклатуры ""%1"":
					|%2';
					|en = 'Cannot generate the dimension key for the ""%1"" item:
					|%2'", ОбщегоНазначения.КодОсновногоЯзыка()),
