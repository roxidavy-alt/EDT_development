...
#Область ТекстКомитентПродажаКомиссионногоТовара // (Дт  :: Кт 004.02)
	...
	|	""ОтчетПоКомиссииМеждуОрганизациямиЛокализация_737"" КАК НомерФрагмента
	|
	|ИЗ
...
#Область ТекстКомитентВзаиморасчетыПоКомиссионномуТовару // (Дт 62.01 :: Кт 76.ОК)
	...
	|	""ОтчетПоКомиссииМеждуОрганизациямиЛокализация_738"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстКомитентЗатратыНаКомиссионноеВознаграждениеБезСтатьиРасходов // (Дт 44.01 :: Кт 60.01)
	ТекстКомитентЗатратыНаКомиссионноеВознаграждениеБезСтатьиРасходов = "
	...
	|	""ОтчетПоКомиссииМеждуОрганизациямиЛокализация_742"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстКомитентРаспределениеЗатратКомиссионногоВознагражденияБезСтатьиРасходов // (Дт 90.07.1 :: Кт 44.01)
	ТекстКомитентРаспределениеЗатратКомиссионногоВознагражденияБезСтатьиРасходов = "
	|ВЫБРАТЬ //// Распределение затрат на комиссионное вознаграждение (Дт 90.07.1 :: Кт 44.01)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.РасходыТекущегоПериодаПСН)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.РасходыТекущегоПериода)
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Распределение затрат на комиссионное вознаграждение"" КАК Содержание,
	|	""ОтчетПоКомиссииМеждуОрганизациямиЛокализация_743"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстКомитентНДСНаКомиссионноеВознаграждение // (Дт 19.04 :: Кт 60.01)
	...
	|	""ОтчетПоКомиссииМеждуОрганизациямиЛокализация_744"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстКомитентУдержатьВознаграждениеАванс // (Дт 60.01 :: Кт 62.02)
	...
	|	""ОтчетПоКомиссииМеждуОрганизациямиЛокализация_745"" КАК НомерФрагмента
	|
	|ИЗ 
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстКомитентУдержатьВознаграждениеОплатаДолга // (Дт 60.01 :: Кт 62.01)
	...
	|	""ОтчетПоКомиссииМеждуОрганизациямиЛокализация_746"" КАК НомерФрагмента
	|
	|ИЗ 
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстКомиссионерЗадолженностьПередКомитентом // (Дт 76.ОК :: Кт 60.01)
	...
	|	ЕСТЬNULL(ВидыЗапасовДокумента.Договор.ГруппаФинансовогоУчетаПолучателя, ЗНАЧЕНИЕ(Справочник.ГруппыФинансовогоУчетаРасчетов.ПустаяСсылка)) КАК ГруппаФинансовогоУчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаДт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаДт,
	|	ЕСТЬNULL(ВидыЗапасовДокумента.Договор.Подразделение, ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)) КАК ПодразделениеДт,
	|	ЕСТЬNULL(ВидыЗапасовДокумента.Договор.НаправлениеДеятельности, ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)) КАК НаправлениеДеятельностиДт,
	|
	|	НЕОПРЕДЕЛЕНО КАК СчетДт,
	...
	|	""ОтчетПоКомиссииМеждуОрганизациямиЛокализация_749"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстАгентЗадолженностьПередПринципалом // (Дт 76.ОК :: Кт 76.09)
	...
	|	""ОтчетПоКомиссииМеждуОрганизациямиЛокализация_750"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстКомиссионерУдержатьВознаграждениеАвансКомитенту // (Дт 60.02 :: Кт 62.01)
	...
	|	""ОтчетПоКомиссииМеждуОрганизациямиЛокализация_751"" КАК НомерФрагмента
	|
	|ИЗ 
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстКомиссионерУдержатьВознаграждениеОплатаДолгаКомитенту // (Дт 60.01 :: Кт 62.01)
	...
	|	""ОтчетПоКомиссииМеждуОрганизациямиЛокализация_752"" КАК НомерФрагмента
	|
	|ИЗ 
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстВключениеНДСВСтоимость // (Дт 44.01 :: Кт 19.04)
	...
	|	НДСПредъявленный.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|
	|	НДСПредъявленный.НДС    КАК Сумма,
	|	НДСПредъявленный.НДСУпр КАК СуммаУУ,
	|
	...
	|	""ОтчетПоКомиссииМеждуОрганизациямиЛокализация_755"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстВосстановлениеАвансовКлиентов // (Дт 62.02 :: Кт 62.01)
	...
	|	""ОтчетПоКомиссииМеждуОрганизациямиЛокализация_756"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстВосстановлениеАвансаПоставщику // (Дт 60.01 :: Кт 60.02)
	...
	|	""ОтчетПоКомиссииМеждуОрганизациямиЛокализация_757"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	ТекстыОтражения.Добавить(РеглУчетВыборкиСерверПовтИсп.ТекстВыручкаОтРеализации()); // Выручка от реализации собственного товара (Дт 62.01 :: Кт 90.01)
	ТекстыОтражения.Добавить(РеглУчетВыборкиСерверПовтИсп.ТекстНачислениеНДССВыручки()); // НДС с реализации (Дт 90.03 :: Кт 68.02)
	...
	ТекстыОтражения.Добавить(ТекстКомитентЗатратыНаКомиссионноеВознаграждениеБезСтатьиРасходов);
	ТекстыОтражения.Добавить(ТекстКомитентЗатратыНаКомиссионноеВознаграждение());
	ТекстыОтражения.Добавить(ТекстКомитентРаспределениеЗатратКомиссионногоВознагражденияБезСтатьиРасходов);
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	|	ЕСТЬNULL(РасчетыСПоставщиками.ИдентификаторФинЗаписи, РасчетыСКлиентами.ИдентификаторФинЗаписи) КАК ИдентификаторФинЗаписи,
	...
	|	ЕСТЬNULL(РасчетыСПоставщиками.ИдентификаторФинЗаписи, РасчетыСКлиентами.ИдентификаторФинЗаписи),
	...
	|	ЕСТЬNULL(РасчетыСКлиентами.ИдентификаторФинЗаписи, РасчетыСПоставщиками.ИдентификаторФинЗаписи) КАК ИдентификаторФинЗаписи,
	...
	|	ЕСТЬNULL(РасчетыСКлиентами.ИдентификаторФинЗаписи, РасчетыСПоставщиками.ИдентификаторФинЗаписи),
...
Функция ТекстКомитентЗатратыНаКомиссионноеВознаграждение()
	
	ТекстЗапроса = "
	|////////////////////////////////////////////////////////////////////////////////////////////////////
	|// Затраты на комиссионное вознаграждение (Дт 44.01 :: Кт 60.01)
	|ВЫБРАТЬ
	...
	|
	|	Строки.РасходыНаПродажуРегл КАК Сумма,
	|	Строки.РасходыНаПродажуУпр КАК СуммаУУ,
	|
	|	// Дт ///////////////////////////////////////////////////////////////////////////////////////////
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.Расходы) КАК ВидСчетаДт,
	...
	|	0 КАК КоличествоДт,
	|
	|	Строки.РасходыНаПродажуРегл - Строки.СуммаПР - Строки.СуммаВР КАК СуммаНУДт,
	...
	|
	|	ВЫБОР
	|		КОГДА Строки.НалогообложениеНДС В (&ТипыНеОблагаемыеНДС)
	|				И НЕ ЕСТЬNULL(УчетнаяПолитикаОрганизаций.НДСПредъявленныйПриВключенииВСтоимость, ЛОЖЬ)
	|			ТОГДА Строки.РасходыНаПродажу
	|		ИНАЧЕ Строки.РасходыНаПродажуБезНДС
	|	КОНЕЦ КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	|
	|	0 КАК СуммаНУДКт,
	...
	|	0 КАК СуммаВРКт,
	|	""Затраты на комиссионное вознаграждение"" КАК Содержание,
	|	""ОтчетПоКомиссииМеждуОрганизациямиЛокализация_1990"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		ВтВыручкаИСебестоимостьПродаж КАК Строки
	|	ПО
	|		ДокументыКОтражению.Ссылка = Строки.Ссылка
	|		И (Строки.РасходыНаПродажуРегл <> 0 ИЛИ Строки.РасходыНаПродажуУпр <> 0)
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РасчетыПоВознаграждению КАК СтрокиКт
	|	ПО
	|		Строки.Ссылка = СтрокиКт.Ссылка
	|		И Строки.Организация = СтрокиКт.Организация
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.ОтчетПоКомиссииМеждуОрганизациями КАК Операция
	|	ПО
...
Функция ТекстКомитентРаспределениеЗатратКомиссионногоВознаграждения()
	
	ТекстЗапроса = "
	|////////////////////////////////////////////////////////////////////////////////////////////////////
	|// Распределение затрат на комиссионное вознаграждение (Дт 90.07.1 :: Кт 44.01)
	|ВЫБРАТЬ
	...
	|
	|	СтрокиДт.РасходыНаПродажуРегл КАК Сумма,
	|	СтрокиДт.РасходыНаПродажуУпр КАК СуммаУУ,
	|
	|	// Дт ///////////////////////////////////////////////////////////////////////////////////////////
	|
	|	ВЫБОР
	|		КОГДА СтрокиДт.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.РасходыТекущегоПериодаПСН)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.РасходыТекущегоПериода)
	|	КОНЕЦ КАК ВидСчетаДт,
	|	Операция.СтатьяРасходов КАК АналитикаУчетаДт,
	...
	|	СтрокиДт.ГруппаФинансовогоУчета КАК СубконтоДт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|
	|	0 КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	|
	|	СтрокиДт.РасходыНаПродажуРегл - СтрокиДт.СуммаПР - СтрокиДт.СуммаВР КАК СуммаНУДт,
	...
	|	0 КАК КоличествоКт,
	|
	|	СтрокиДт.РасходыНаПродажуРегл - СтрокиДт.СуммаПР - СтрокиДт.СуммаВР КАК СуммаНУКт,
	|	СтрокиДт.СуммаПР КАК СуммаПРКт,
	|	СтрокиДт.СуммаВР КАК СуммаВРКт,
	|	""Распределение затрат на комиссионное вознаграждение"" КАК Содержание,
	|	""ОтчетПоКомиссииМеждуОрганизациямиЛокализация_1991"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|		И (СтрокиДт.РасходыНаПродажуРегл <> 0 ИЛИ СтрокиДт.РасходыНаПродажуУпр <> 0)
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РасчетыПоВознаграждению КАК Строки
	|	ПО
