...
Функция ПараметрыДляПроведенияДокумента(Документ, Свойства) Экспорт
	...
	Если Свойства.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если ТипЗнч(Документ) = Тип("ДокументОбъект.ЗаявкаНаРасходованиеДенежныхСредств")
			...
			Или ТипЗнч(Документ) = Тип("ДокументОбъект.ЗаявкаНаЗакупку")
			Или ТипЗнч(Документ) = Тип("ДокументОбъект.ЗаявкаНаОбеспечение")
...
Процедура ПередЗаписьюДвиженийДокумента(Документ, МенеджерВременныхТаблиц, Отказ) Экспорт
	...
	Если ИспользуетсяРегистрЛимитовБюджетирования()
...
Процедура ПослеЗаписиДвиженийДокумента(Документ, МенеджерВременныхТаблиц, Отказ) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьЛимитыРасходаДенежныхСредствБюджетирования")
		Или ИспользоватьЛимитыБДР() Тогда
...
Функция СтатьяБюджетовИспользуетсяВНастройкахЛимитов(СтатьяБюджетов, ПроверятьПравилаЛимитов = Истина) Экспорт
	...
	Если Не ИспользоватьЛимитыБДР()
		И Не ПолучитьФункциональнуюОпцию("ИспользоватьЛимитыРасходаДенежныхСредствБюджетирования") Тогда
		Возврат Ложь;
	КонецЕсли;
...
// Выполняется при проведении документов Заявка на командировку и Заявка на внутреннее потребление.
// Проверяется актуальность правил лимитов по статьям бюджетов на даты планируемых расходов.
//
// Параметры:
//  ДокументОбъект - ДокументОбъект.ЗаявкаНаКомандировку
//                   ДокументОбъект.ЗаявкаНаОбеспечение - контролируемый документ
//  Отказ - Булево - признак отказа от проведения документа.
//
Процедура ПроверитьАктуальностьПравилЛимитовПоСтатьямБюджетов(ДокументОбъект, Отказ) Экспорт
	
	Если Не ИспользоватьЛимитыБДР() Тогда
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЗаявкаНаКомандировку") Тогда
		ТаблицаРасходов = ДокументОбъект.Расходы.Выгрузить(, "ПланируемаяДатаРасходов, СтатьяБюджетов");
	Иначе
		ТаблицаРасходов = ДокументОбъект.Товары.Выгрузить(, "СтатьяБюджетов"); // -ТаблицаЗначений
		ТаблицаРасходов.Колонки.Добавить("ПланируемаяДатаРасходов", ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.Дата));
		ТаблицаРасходов.ЗаполнитьЗначения(ДокументОбъект.Дата, "ПланируемаяДатаРасходов");
	КонецЕсли;
	ТаблицаРасходов.Свернуть("ПланируемаяДатаРасходов, СтатьяБюджетов");
	
	Если Не ТаблицаРасходов.Количество() Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ВЫРАЗИТЬ(ТаблицаРасходов.ПланируемаяДатаРасходов КАК ДАТА) КАК ДатаРасходов,
	|	ВЫРАЗИТЬ(ТаблицаРасходов.СтатьяБюджетов КАК Справочник.СтатьиБюджетов) КАК СтатьяБюджетов
	...
	|ПОМЕСТИТЬ АктуальныеПравилаЛимитов
	|ИЗ
	...
	|			И (ПравилаЛимитовПоДаннымБюджетирования.ЛимитируемаяОперация = ЗНАЧЕНИЕ(Перечисление.ЛимитируемыеОперации.Расходы))
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаРасходов КАК ТаблицаРасходов
	|		ПО (ПравилаЛимитовПоДаннымБюджетирования.СтатьяБюджета = ТаблицаРасходов.СтатьяБюджетов)
	|ГДЕ
	|	ВЫБОР
	|			КОГДА МоделиБюджетирования.КонецДействия = ДАТАВРЕМЯ(1, 1, 1)
	|				ТОГДА МоделиБюджетирования.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыМоделейБюджетирования.Действует)
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|	И НЕ ПравилаЛимитовПоДаннымБюджетирования.ПометкаУдаления
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаРасходов.СтатьяБюджетов КАК СтатьяБюджетов,
	|	ТаблицаРасходов.ДатаРасходов КАК ДатаРасходов,
	|	МАКСИМУМ(АктуальныеПравилаЛимитов.ПравилоЛимитов) КАК ПравилоЛимитов
	|ИЗ
	|	ТаблицаРасходов КАК ТаблицаРасходов
	|		ЛЕВОЕ СОЕДИНЕНИЕ АктуальныеПравилаЛимитов КАК АктуальныеПравилаЛимитов
	|		ПО ТаблицаРасходов.СтатьяБюджетов = АктуальныеПравилаЛимитов.СтатьяБюджетов
	|			И ТаблицаРасходов.ДатаРасходов >= АктуальныеПравилаЛимитов.НачалоДействия
	|			И ТаблицаРасходов.ДатаРасходов <= АктуальныеПравилаЛимитов.КонецДействия
	|ГДЕ
	|	ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	...
	|				Справочник.ПравилаЛимитовПоДаннымБюджетирования КАК ПравилаЛимитовПоДаннымБюджетирования
	|			ГДЕ
	|				ТаблицаРасходов.СтатьяБюджетов = ПравилаЛимитовПоДаннымБюджетирования.СтатьяБюджета
	|				И НЕ ПравилаЛимитовПоДаннымБюджетирования.ПометкаУдаления)
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаРасходов.СтатьяБюджетов,
	|	ТаблицаРасходов.ДатаРасходов";
	
	ШаблонТекстаОшибки = НСтр("ru = 'Для статьи бюджетов %1 не найдено актуальных правил лимитов на %2';
								|en = 'No relevant limit rules are found for the %1 budget flow item as of %2'");
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Если Не ЗначениеЗаполнено(Выборка.ПравилоЛимитов) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонТекстаОшибки,
				Выборка.СтатьяБюджетов, Формат(Выборка.ДатаРасходов, "ДЛФ=D;"));
...
Функция ПланФактЛимитов(Параметры, ОтборРасшифровки = Неопределено) Экспорт
	...
	Если ПродолжитьРасчет Тогда
		...
		Если ДействующиеЛимиты.ПравилаЛимитов.Количество() = 0 Тогда
	...
	Если ПродолжитьРасчет Тогда
		Если ВозможенКонтрольПоДокументу Тогда
			...
			Если СработавшиеЛимиты.ПравилаЛимитов.Количество() = 0 Тогда
				ПродолжитьРасчет = Ложь;
			КонецЕсли;
		Иначе
	...
	Если ПродолжитьРасчет Тогда
		...
		Если ПоТаблицеКорреспонденций Тогда
			...
			ПравилаПоСработавшимЛимитам = СработавшиеЛимиты.ПравилаПолученияФакта.Скопировать(
			...
			Если ПравилаПоСработавшимЛимитам.Количество() > 0 Тогда
				...
				ВидыДокументов = ОбщегоНазначенияУТКлиентСервер.ПреобразоватьСоответствиеИлиСтруктуруВМассив(
					ИсточникиДанныхПовтИсп.ВидыДокументовДляКонтроляЛимитов(ПравилаПоСработавшимЛимитам[0].ТипДанныхУчетаРасход));
				Для Каждого СтрокаЛимит Из СработавшиеЛимиты.ПравилаЛимитов Цикл
					Если РегистрыСведений.ЗаданияКОтражениюВБюджетировании.ТребуетсяОтражениеВБюджетированииДляОтчетаЗаПериод(
						СтрокаЛимит.НачалоПериода, СтрокаЛимит.КонецПериода, , Истина, ВидыДокументов) Тогда
						ФактическиеДанныеБюджетированияСервер.ОтразитьДокументыВБюджетировании(СтрокаЛимит.НачалоПериода,
						СтрокаЛимит.КонецПериода, ВидыДокументов);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		...
		Если ПоТаблицеКорреспонденций И ПравилаПоСработавшимЛимитам.Количество() > 0 Тогда
			...
			ПараметрыПолученияХранимогоФакта.Вставить("ДействующиеЛимиты",
				СработавшиеЛимиты.ПравилаЛимитов.Скопировать( , "СтатьяБюджетов,НачалоПериода,КонецПериода"));
	...
	Если ПродолжитьРасчет Тогда
		...
		Если ПланПриОтсутствииФакта И КоличествоФормальноДействующихЛимитов <> КоличествоСработавшихЛимитов Тогда
			...
			ОбщегоНазначенияУТКлиентСервер.ДополнитьТаблицу(
				НеСработавшиеЛимиты.ПравилаЛимитов,
				ФактПоДокументам);
		КонецЕсли;
...
// Возвращает признак использования регистра "Лимиты по данным бюджетирования"
//
// Возвращаемое значение:
//  Булево - Истина, если регистр используется.
//
Функция ИспользуетсяРегистрЛимитовБюджетирования() Экспорт
	
	Возврат ПолучитьФункциональнуюОпцию("ИспользоватьЛимитыБДРПриЗакупках")
		Или ПолучитьФункциональнуюОпцию("ИспользоватьЛимитыБДРПриПотреблении")
		Или ПолучитьФункциональнуюОпцию("ИспользоватьЛимитыРасходаДенежныхСредствБюджетирования")
		И Не ПолучитьФункциональнуюОпцию("КонтрольЛимитовПоДаннымБюджетирования2_4");
	
КонецФункции

// Возвращает признак использования лимитов БДР
//
// Возвращаемое значение:
//  Булево - Истина, если лимиты БДР используются.
//
Функция ИспользоватьЛимитыБДР() Экспорт
	
	Возврат ИспользоватьЛимитыБДРПриЗакупках()
		Или ПолучитьФункциональнуюОпцию("ИспользоватьЛимитыБДРПриПотреблении");

КонецФункции

// Возвращает признак использования лимитов БДР при закупках
//
// Возвращаемое значение:
//  Булево - Истина, если лимиты БДР используются.
//
Функция ИспользоватьЛимитыБДРПриЗакупках() Экспорт
	
	Возврат ПолучитьФункциональнуюОпцию("ИспользоватьЛимитыБДРПриЗакупках");

КонецФункции
	
// Возвращает лимитируемые операции, используемые в соответствии с настройками программы.
//
// Возвращаемое значение:
//  Массив из ПеречислениеСсылка.ЛимитируемыеОперации
//
Функция ИспользуемыеЛимитируемыеОперации() Экспорт
	
	ЛимитируемыеОперации = Новый Массив;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьЛимитыРасходаДенежныхСредствБюджетирования") Тогда
		ЛимитируемыеОперации.Добавить(Перечисления.ЛимитируемыеОперации.ДвижениеДенежныхСредств);
	КонецЕсли;
	Если ИспользоватьЛимитыБДР() Тогда
		ЛимитируемыеОперации.Добавить(Перечисления.ЛимитируемыеОперации.Расходы);
	КонецЕсли;
	
	Возврат ЛимитируемыеОперации;
...
Процедура УстановитьВариантКонтроляЛимитовПоДаннымБюджетирования(Параметры, АдресХранилища) Экспорт
	...
	ПоРегиструЛимитов = Параметры.Свойство("КонтрольЛимитовПоДаннымБюджетирования2_5");
...
Функция ТребуетсяКонтрольПоЛимитамБюджетирования(ДокументОбъект)
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЗаявкаНаЗакупку") Тогда
		ИспользоватьЛимитыБДР = Документы.ЗаявкаНаЗакупку.КонтролироватьЛимитыБДР(ДокументОбъект);
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЗаявкаНаОбеспечение") Тогда
		ИспользоватьЛимитыБДР = ПолучитьФункциональнуюОпцию("ИспользоватьЛимитыБДРПриПотреблении")
			Или ПолучитьФункциональнуюОпцию("ИспользоватьЛимитыБДРПриЗакупках") И ДокументОбъект.ОбеспечиваетсяЗакупкой;
	Иначе
		ИспользоватьЛимитыБДР = ИспользоватьЛимитыБДР();
	КонецЕсли;
	ИспользоватьЛимитыБДДС = ПолучитьФункциональнуюОпцию("ИспользоватьЛимитыРасходаДенежныхСредствБюджетирования");
	ИспользоватьЛимитыБДДС2_4 = ПолучитьФункциональнуюОпцию("КонтрольЛимитовПоДаннымБюджетирования2_4");
	
	Если (ИспользоватьЛимитыБДР Или ИспользоватьЛимитыБДДС И Не ИспользоватьЛимитыБДДС2_4)
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЗаявкаНаРасходованиеДенежныхСредств") Тогда
	
		ТребуетсяКонтроль = (ДокументОбъект.Статус <> Перечисления.СтатусыЗаявокНаРасходованиеДенежныхСредств.Отклонена
			И ИспользоватьЛимитыБДДС И (Не ИспользоватьЛимитыБДДС2_4 Или Не ДокументОбъект.СверхЛимита));
	
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЗаявкаНаКомандировку") Тогда
	
		ТребуетсяКонтроль = (ДокументОбъект.Статус <> Перечисления.СтатусыЗаявокСотрудников.Отклонена
			И ДокументОбъект.Статус <> Перечисления.СтатусыЗаявокСотрудников.Отозвана
			И (ИспользоватьЛимитыБДДС И ДокументОбъект.ВыдачаПодОтчет.Количество()
			И (Не ИспользоватьЛимитыБДДС2_4 Или Не ДокументОбъект.СверхЛимита)
			Или ИспользоватьЛимитыБДР И ДокументОбъект.Расходы.Количество()));
		
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЗаявкаНаЗакупку") Тогда
		
		ТребуетсяКонтроль = ИспользоватьЛимитыБДР И ДокументОбъект.Товары.Количество();
		
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЗаявкаНаОбеспечение") Тогда
		
		ТребуетсяКонтроль = (ДокументОбъект.Статус <> Перечисления.СтатусыЗаявокНаОбеспечение.Отменена
			И ДокументОбъект.Статус <> Перечисления.СтатусыЗаявокНаОбеспечение.Черновик
			И ИспользоватьЛимитыБДР И ДокументОбъект.Товары.Количество());
		
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.СписаниеБезналичныхДенежныхСредств") Тогда
	
		ТребуетсяКонтроль = (ИспользоватьЛимитыБДДС И Не ИспользоватьЛимитыБДДС2_4 И ДокументОбъект.ПроведеноБанком);
	
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.РасходныйКассовыйОрдер") Тогда
	
		ТребуетсяКонтроль = (ИспользоватьЛимитыБДДС И Не ИспользоватьЛимитыБДДС2_4);
...
Функция ПараметрыКонтроляПоЛимитамБюджетирования(Документ) Экспорт
	...
	ПараметрыКонтроля.Вставить("ПоРегиструЛимитов", ИспользуетсяРегистрЛимитовБюджетирования());
	...
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств") Тогда
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Документ,
			"Валюта, ДатаПлатежа, СверхЛимита, СуммаДокумента, ХозяйственнаяОперация, ХозяйственнаяОперацияПоЗарплате");
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаявкаНаКомандировку") Тогда
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Документ,
			"Валюта, ЖелательнаяДатаПлатежа, СверхЛимита, СуммаДокумента, ПредполагаемаяСуммаРасходов");
		...
		КонецЕсли;
		ЛимитируемыеОперации = ИспользуемыеЛимитируемыеОперации();
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаявкаНаЗакупку")
		Или ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаявкаНаОбеспечение") Тогда
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Документ, "Валюта, Дата, СуммаДокумента");
		Если Не ЗначениеЗаполнено(РеквизитыДокумента.Валюта) Тогда
			РеквизитыДокумента.Вставить("Валюта", ЗначениеНастроекПовтИсп.ВалютаУправленческогоУчета());
		КонецЕсли;
		РеквизитыДокумента.Вставить("ПредполагаемаяСуммаРасходов", РеквизитыДокумента.СуммаДокумента);
		РеквизитыДокумента.СуммаДокумента = 0;
		РеквизитыДокумента.Вставить("ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.СписаниеТоваровПоТребованию);
		ЛимитируемыеОперации.Добавить(Перечисления.ЛимитируемыеОперации.Расходы);
	Иначе
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Документ, "Валюта, Дата, СуммаДокумента, ХозяйственнаяОперация");
	...
	Если ЗначениеЗаполнено(ИмяРеквизитаДатаПроверки) Тогда
		ТекстыЗапросов.Добавить("
			|ВЫБРАТЬ
			|	&ДатаПроверки КАК ДатаЛимитов,
			|	ЗНАЧЕНИЕ(Перечисление.ЛимитируемыеОперации.ДвижениеДенежныхСредств) КАК ЛимитируемаяОперация,
			|	ЗНАЧЕНИЕ(Справочник.СтатьиБюджетов.ПустаяСсылка) КАК СтатьяБюджетов");
		Запрос.УстановитьПараметр("ДатаПроверки", РеквизитыДокумента[ИмяРеквизитаДатаПроверки]);
	КонецЕсли;
	Если ЛимитируемыеОперации.Найти(Перечисления.ЛимитируемыеОперации.Расходы) <> Неопределено Тогда
		ТекстЗапроса = "
			|ВЫБРАТЬ
			|	&ДатаПроверки КАК ДатаЛимитов,
			|	ЗНАЧЕНИЕ(Перечисление.ЛимитируемыеОперации.Расходы) КАК ЛимитируемаяОперация,
			|	ТаблицаРасходов.СтатьяБюджетов КАК СтатьяБюджетов
			|ИЗ
			|	&ТабличнаяЧастьДокумента КАК ТаблицаРасходов
			|ГДЕ
			|	ТаблицаРасходов.Ссылка = &Документ";
		Если ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаявкаНаКомандировку") Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ДатаПроверки", "ТаблицаРасходов.ПланируемаяДатаРасходов");
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТабличнаяЧастьДокумента", "Документ.ЗаявкаНаКомандировку.Расходы");
		Иначе
			ИмяДокумента = Метаданные.НайтиПоТипу(ТипЗнч(Документ)).ПолноеИмя();
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ДатаПроверки", "ТаблицаРасходов.Ссылка.Дата");
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТабличнаяЧастьДокумента",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.Товары", ИмяДокумента));
		КонецЕсли;
		ТекстыЗапросов.Добавить(ТекстЗапроса);
		Запрос.УстановитьПараметр("Документ", Документ);
	КонецЕсли;
	Запрос.Текст = СтрСоединить(ТекстыЗапросов, ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении()) + Символы.ПС
		+ "УПОРЯДОЧИТЬ ПО ДатаЛимитов"; // @query-part
	РеквизитыДокумента.Вставить("ДатыПроверки", Запрос.Выполнить().Выгрузить());
	...
	РеквизитыДокумента.Вставить("ЛимитируемыеОперации", ЛимитируемыеОперации);
...
Функция РассчитанФактПоДокументуПоНастройкамПолученияФакта(РеквизитыДокумента, МенеджерВременныхТаблиц,
	...
	Запрос.УстановитьПараметр("НачалоПериода", РеквизитыДокумента.ДатыПроверки[0].ДатаЛимитов);
	Запрос.УстановитьПараметр("КонецПериода",
		РеквизитыДокумента.ДатыПроверки[РеквизитыДокумента.ДатыПроверки.Количество() - 1].ДатаЛимитов);
...
Процедура ВыполнитьКонтрольПоЛимитамБюджетирования(ПараметрыКонтроля)
	...
	ПоискРазрешающегоЛимита = Новый Структура("ТипЛимита, ЛимитируемаяОперация",
		Перечисления.ТипыЛимитовРасходованияДС.РазрешающийВПределахЛимита,
		Перечисления.ЛимитируемыеОперации.ДвижениеДенежныхСредств);
	ПоискИнформационногоЛимита = Новый Структура("ТипЛимита, ЛимитируемаяОперация",
		Перечисления.ТипыЛимитовРасходованияДС.Информационный,
		Перечисления.ЛимитируемыеОперации.ДвижениеДенежныхСредств);
	...
	Если ПродолжитьРасчет Тогда
		...
		Если ДействующиеЛимиты.ПравилаЛимитов = Неопределено Или ДействующиеЛимиты.ПравилаЛимитов.Количество() = 0 Тогда
	...
	Если ПродолжитьРасчет Тогда
		Если ПараметрыКонтроля.ПоРегиструЛимитов Тогда
			...
			ИначеЕсли Не РассчитанФактПоДокументуПоНастройкамПолученияФакта(РеквизитыДокумента,
				...
				ОбщегоНазначенияУТ.УничтожитьВременныеТаблицы(ПараметрыКонтроля.МенеджерВременныхТаблиц,
					"ФактическиеДанныеПоДокументу");
				ПродолжитьРасчет = Ложь;
			КонецЕсли;
		Иначе
	...
	Если ПродолжитьРасчет Тогда
		...
		РазрешающийЛимитНайден    = ПланФактПоПравилам.НайтиСтроки(ПоискРазрешающегоЛимита).Количество() > 0;
		ИнформационныйЛимитНайден = ПланФактПоПравилам.НайтиСтроки(ПоискИнформационногоЛимита).Количество() > 0;
	КонецЕсли;
	
	Если РеквизитыДокумента.СуммаДокумента > 0 И НЕ РазрешающийЛимитНайден И РазрешающийЛимитОбязателен Тогда
		// Анализ, что заявка попала в один из "разрешающих лимитов".
		НарушеныТолькоИнформационныеЛимиты = Ложь;
		ТекстСообщения = НСтр("ru = 'Заявка не попадает ни в одно ""Разрешающее"" правило лимитирования по движению денежных средств. 
									|Заявка может быть только сверх-лимитной.';
									|en = 'Request is not included in any Allowing cash flow limiting rule.
									|Request can be only above the limit.'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "",ТекстСообщения, "");
	ИначеЕсли РеквизитыДокумента.СуммаДокумента > 0 И НЕ РазрешающийЛимитНайден И НЕ ИнформационныйЛимитНайден Тогда
		...
		НарушеныТолькоИнформационныеЛимиты = Ложь;
		ТекстСообщения = НСтр("ru = 'Заявка не попадает ни в одно правило лимитирования по движению денежных средств.
									|Заявка может быть только сверх-лимитной.';
									|en = 'Request is not included in any cash flow limiting rule.
									|Request can be only above the limit.'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "",ТекстСообщения, "");
	КонецЕсли;
...
Функция ТекстСообщенияПроверкиПревышенияЛимитовПоДаннымБюджетирования(ИерархияАналитик, СтрокаПланФакта, ДатаПроверки, КэшПредставлений)
	...
	Если ИспользуетсяРегистрЛимитовБюджетирования() Тогда
...
Функция ПланФактПоПравиламЛимитов(ПараметрыПолученияФакта, ПараметрыКонтроля, ФактическиеДанные, ПравилаЛимитов, ВРазрезеГруппировокФакта = Ложь, ДополнительныйОтборРасшифровкиОтчета = Неопределено)
	...
	ДатыПроверки = ПараметрыКонтроля.РеквизитыДокумента.ДатыПроверки;
	ЛимитируемыеОперации = ПараметрыКонтроля.РеквизитыДокумента.ЛимитируемыеОперации;
	МаксимальноеКоличествоАналитик = БюджетированиеКлиентСервер.МаксимальноеКоличествоАналитик();
	...
	ПравилаЛимитовСДопДанными = ПравилаЛимитов.ПравилаЛимитов;
	...
	ДополнительныеПараметры.Вставить("ДатыПроверки", ДатыПроверки);
	...
	Иначе
		...
		ФинансоваяОтчетностьСервер.УстановитьПараметрКомпоновки(КомпоновщикНастроек, "ДатаПроверкиКурса",
			ДатыПроверки[0].ДатаЛимитов);
...
Функция ПлановыеДанныеДляПланФакта(ПравилаЛимитовСДопДанными, ДополнительныеПараметры)
	...
	ДатаПроверки = ДополнительныеПараметры.ДатыПроверки[0].ДатаЛимитов;
...
Функция ДанныеДополнительныхПолейПравилЛимитов(ПравилаЛимитов, ИерархияАналитикПоПравилам)
	...
	ДанныеДополнительныхПолей.Колонки.Добавить("ХешСхемыКомпоновкиДанных");
...
//  ПравилаПолученияФакта - Неопределено,
//                          ТаблицаЗначений - правила получения факта.
//  ПравилаЛимитовПоВидамАналитик - Неопределено,
//                          ТаблицаЗначений - правила заполнения видов аналитик правил лимитов.
//
// Возвращаемое значение:
...
//
Функция НастройкаПолученияФактаПоДокументуНаДату(ВидДокумента, ПравилаПолученияФакта, ПравилаЛимитовПоВидамАналитик)
	
	НастройкаПолученияФакта = Новый Структура("ТекстЗапроса, ПараметрыЗапроса, ИменаВременныхТаблиц", "",
		Новый Структура, Новый Массив);
	
	КоличествоСтрок = 0;
	Если ТипЗнч(ПравилаПолученияФакта) = Тип("ТаблицаЗначений") Тогда
		КоличествоСтрок = КоличествоСтрок + ПравилаПолученияФакта.Количество();
	КонецЕсли;
	Если ТипЗнч(ПравилаЛимитовПоВидамАналитик) = Тип("ТаблицаЗначений") Тогда
		КоличествоСтрок = КоличествоСтрок + ПравилаЛимитовПоВидамАналитик.Количество();
	КонецЕсли;
	...
	Если ТипЗнч(ПравилаПолученияФакта) = Тип("ТаблицаЗначений") Тогда
		...
		Если ТипЗнч(ПравилаЛимитовПоВидамАналитик) = Тип("ТаблицаЗначений")
			И ПравилаЛимитовПоВидамАналитик.Количество() > 0 Тогда
			ЗаполнитьЗначенияСвойств(ТипыДанныхУчетаПравил.Добавить(), ПравилаЛимитовПоВидамАналитик[0],
				КолонкиТипыДанныхУчетаПравил);
		КонецЕсли;
	Иначе
		ТипыДанныхУчетаПравил = ПравилаЛимитовПоВидамАналитик.Скопировать(, КолонкиТипыДанныхУчетаПравил);
	КонецЕсли;
	...
	ПолноеИмяДокумента = ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору(ВидДокумента).ПолноеИмя();
	...
	Для Каждого ПриходРасход Из ТипыДанныхУчетаПравил Цикл
		Если ПустаяСтрока(ПриходРасход.ХешСхемыКомпоновкиДанных) Тогда
			ИмяВременнойТаблицы = "Движения" + МониторингЦелевыхПоказателей.ПолучитьИмяЗначенияПеречисления(ПриходРасход.ТипДанныхУчетаРасход)
				+ МониторингЦелевыхПоказателей.ПолучитьИмяЗначенияПеречисления(ПриходРасход.ТипДанныхУчетаПриход);
	...
	ШаблонЗапросаКИсточникуФактаПоПравилу =
		...
		|	&ПравилоЛимитовGUID КАК ПравилоЛимитов,
		|	&ПериодЛимитирования КАК ПериодЛимитирования,
		|	ПсевдонимТаблицыИсточника.Период КАК Период,
	...
	Если ТипЗнч(ПравилаПолученияФакта) = Тип("ТаблицаЗначений") Тогда
		...
		ИспользоватьСуммуФакт = (ПолноеИмяДокумента = Метаданные.Документы.СписаниеБезналичныхДенежныхСредств.ПолноеИмя()
			Или ПолноеИмяДокумента = Метаданные.Документы.РасходныйКассовыйОрдер.ПолноеИмя());
		...
		Для Каждого СтрокаПравило Из ПравилаПолученияФакта Цикл
			...
			КонецЕсли;
			Если ИспользоватьСуммуФакт Тогда
				ТекстЗапроса = СтрЗаменить(ШаблонЗапросаКИсточникуФактаПоПравилу, "&ТекстСуммаФакт", "ПсевдонимТаблицыИсточника." + ИмяПоказателяСуммы);
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстСумма", ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(0));
			Иначе
				ТекстЗапроса = СтрЗаменить(ШаблонЗапросаКИсточникуФактаПоПравилу, "&ТекстСуммаФакт", ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(0));
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстСумма", "ПсевдонимТаблицыИсточника." + ИмяПоказателяСуммы);
			КонецЕсли;
			...
			КонецЕсли;
			Если СтрокаПравило.РазделИсточникаДанных = Перечисления.РазделыИсточниковДанныхБюджетирования.ОперативныйУчет Тогда
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеИсточникаДанных", "ПсевдонимТаблицыИсточника.ИсточникДанных = "
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПравилоЛимитовGUID",
				ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(Неопределено));
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПериодЛимитирования",
				ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(Неопределено));
	...
	Если ТипЗнч(ПравилаЛимитовПоВидамАналитик) = Тип("ТаблицаЗначений") Тогда
		...
		ПериодичностиПравилЛимитов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(
			ПравилаЛимитовПоВидамАналитик.ВыгрузитьКолонку("Правило"), "Периодичность");
		
		Для Каждого СтрокаПравило Из ПравилаЛимитовПоВидамАналитик Цикл
			...
			ДанныеДляЗапросаПоПравилу = ИсточникиДанныхСервер.ДанныеДляЗапросаКИсточникуФактаПоПравилу(СтрокаПравило,
				ТипыДанныхУчетаПравила.НастройкиСКД);
			
			ИмяПоказателяСуммы = "Сумма";
			ИмяПоказателяСуммыФакт = "СуммаФакт";
			ТекстЗапроса = СтрЗаменить(ШаблонЗапросаКИсточникуФактаПоПравилу, "&ТекстСуммаФакт", "ПсевдонимТаблицыИсточника."
				+ ИмяПоказателяСуммыФакт);
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстСумма", "ПсевдонимТаблицыИсточника." + ИмяПоказателяСуммы);
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстПолейАналитик",
				ДанныеДляЗапросаПоПравилу.ФрагментыТекстаЗапроса.ТекстПолейАналитик);
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстСоединений",
			...
			КонецЕсли;
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстУсловия",
				ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(Истина));
			ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(НастройкаПолученияФакта.ПараметрыЗапроса,
				ДанныеДляЗапросаПоПравилу.ПараметрыЗапроса);
			ИмяПараметраСтатьиБюджетов = "СтатьяБюджетов" + МониторингЦелевыхПоказателей.УникальныйИдентификаторВСтроку(
				СтрокаПравило.СтатьяБюджетов.УникальныйИдентификатор());
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеИсточникаДанных",
				"ПсевдонимТаблицыИсточника.СтатьяБюджетов = &СтатьяБюджетовGUID");
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "СтатьяБюджетовGUID", ИмяПараметраСтатьиБюджетов);
			НастройкаПолученияФакта.ПараметрыЗапроса.Вставить(ИмяПараметраСтатьиБюджетов, СтрокаПравило.СтатьяБюджетов);
			ИмяПараметраПравилаЛимитов = "ПравилоЛимитов" + МониторингЦелевыхПоказателей.УникальныйИдентификаторВСтроку(
				СтрокаПравило.Правило.УникальныйИдентификатор());
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПравилоЛимитовGUID", ИмяПараметраПравилаЛимитов);
			НастройкаПолученияФакта.ПараметрыЗапроса.Вставить(ИмяПараметраПравилаЛимитов, СтрокаПравило.Правило);
			ТекстПериодЛимитирования = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"НАЧАЛОПЕРИОДА(ПсевдонимТаблицыИсточника.Период, %1)", 
				ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(ПериодичностиПравилЛимитов.Получить(СтрокаПравило.Правило))); 
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПериодЛимитирования", ТекстПериодЛимитирования);
	...
	НастройкаПолученияФакта.ИменаВременныхТаблиц = ТипыДанныхУчетаПравил.ВыгрузитьКолонку("ИмяВременнойТаблицы");
...
Функция ПараметрыПолученияФактаДляКонтроляЛимитов(РеквизитыДокумента, ДействующиеЛимиты, ОтборПоЗаявке, ДополнительныйОтборРасшифровкиОтчета = Неопределено)
	...
	ПравилаЛимитов = ДействующиеЛимиты.ПравилаЛимитов;
	
	Периодичности = ПравилаЛимитов.Скопировать(, "Периодичность");
	...
	ДатаПроверки = РеквизитыДокумента.ДатыПроверки[0].ДатаЛимитов;
	...
	ПараметрыПолученияФакта.Вставить("ГраницаФактДанных", ДатаПроверки);
	ПараметрыПолученияФакта.Вставить("ДатаАктуальности",  ДатаПроверки);
	ПараметрыПолученияФакта.Вставить("НачалоПериода",     ДатаПроверки); // Уточняется выражениями источников данных
	ПараметрыПолученияФакта.Вставить("КонецПериода",      ДатаПроверки); // Уточняется выражениями источников данных
	ПараметрыПолученияФакта.Вставить("ПериодКурса",       ДатаПроверки); // Переопределение даты пересчета курсов валют на дату проверки.
	ПараметрыПолученияФакта.Вставить("Сценарий",          Справочники.Сценарии.ИсполнениеБюджета);
	ПараметрыПолученияФакта.Вставить("Период", Новый СтандартныйПериод(ДатаПроверки, ДатаПроверки));
	...
	Для Каждого ПризнакУникальности Из Периодичности Цикл 
		...
		Если Периодичности.Количество() = 1 Тогда
			УникальныеПараметрыИсточникаДанных.Объекты = ПравилаЛимитов.ВыгрузитьКолонку("СтатьяБюджетов");
		Иначе
			ОтборПравилЛимитов = Новый Структура("Периодичность");
			ЗаполнитьЗначенияСвойств(ОтборПравилЛимитов, ПризнакУникальности);
			
			УникальныеПараметрыИсточникаДанных.Объекты = ПравилаЛимитов.Скопировать(ОтборПравилЛимитов, "СтатьяБюджетов").ВыгрузитьКолонку("СтатьяБюджетов");
		КонецЕсли;
...
// Возвращает действующие на указанные даты проверки настройки лимитов по данным бюджетирования.
// 
// Параметры:
...
// 
Функция НастройкиЛимитовНаДатыПроверки(Параметры) Экспорт
	
	ПериодыДействияПоДатамЛимитов = Параметры.ДатыПроверки.Скопировать(, "ДатаЛимитов"); // - ТаблицаЗначений -
	ПериодыДействияПоДатамЛимитов.Свернуть("ДатаЛимитов");
	ПериодыДействияПоДатамЛимитов.Колонки.Добавить("НачалоДействия", Новый ОписаниеТипов("Дата"));
	ПериодыДействияПоДатамЛимитов.Колонки.Добавить("КонецДействия", Новый ОписаниеТипов("Дата"));
	
	НастройкиЛимитовПоПериодамДействия = Новый Массив;
	ДатыЛимитовКРасчету = Параметры.ДатыПроверки.СкопироватьКолонки("ДатаЛимитов");
	
	ИменаКолонокПериодыДействия = "НачалоДействия,КонецДействия";
	СтруктураПериодДействия = Новый Структура(ИменаКолонокПериодыДействия);
	
	Счетчик = 0;
	Пока Счетчик < ПериодыДействияПоДатамЛимитов.Количество() Цикл
		ДатаЛимитов = ПериодыДействияПоДатамЛимитов[Счетчик];
		ЗначениеКэша = РегистрыСведений.КэшНастроекЛимитовПоДаннымБюджетирования.ПолучитьЗначениеКэша(
			ДатаЛимитов.ДатаЛимитов, Параметры.ВидДокумента, Параметры.ХозяйственнаяОперация);
		Если ЗначениеКэша <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(СтруктураПериодДействия, ЗначениеКэша);
			Если ПериодыДействияПоДатамЛимитов.НайтиСтроки(СтруктураПериодДействия).Количество() = 0 Тогда
				НастройкиЛимитовПоПериодамДействия.Добавить(ЗначениеКэша);
			КонецЕсли;
			ЗаполнитьЗначенияСвойств(ДатаЛимитов, ЗначениеКэша);
			Счетчик = Счетчик + 1;
		Иначе
			ЗаполнитьЗначенияСвойств(ДатыЛимитовКРасчету.Добавить(), ДатаЛимитов);
			ПериодыДействияПоДатамЛимитов.Удалить(Счетчик);
		КонецЕсли;
	КонецЦикла;
	
	НастройкиЛимитов = Новый Структура("НастройкиЛимитовПоПериодамДействия, ПериодыДействияПоДатамЛимитов",
		НастройкиЛимитовПоПериодамДействия, ПериодыДействияПоДатамЛимитов);
	
	Если ДатыЛимитовКРасчету.Количество() = 0 Тогда
		Возврат НастройкиЛимитов;
	КонецЕсли;
	
	ПоРегиструЛимитов = ИспользуетсяРегистрЛимитовБюджетирования();
	
	ИспользоватьПравилаПолученияФакта = (Параметры.ЛимитируемыеОперации.Найти(
		Перечисления.ЛимитируемыеОперации.ДвижениеДенежныхСредств) <> Неопределено);
	ИспользоватьПравилаЛимитовБДР = (Параметры.ЛимитируемыеОперации.Найти(Перечисления.ЛимитируемыеОперации.Расходы)
		<> Неопределено);
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("ДатыЛимитовКРасчету", ДатыЛимитовКРасчету);
	Если ИспользоватьПравилаПолученияФакта Тогда
		СтрокаДатаЛимитов = Параметры.ДатыПроверки.Найти(Перечисления.ЛимитируемыеОперации.ДвижениеДенежныхСредств);
		Запрос.УстановитьПараметр("ДатаАктуальностиПравилФакта", ?(СтрокаДатаЛимитов = Неопределено,
			Параметры.ДатыПроверки[0].ДатаЛимитов, СтрокаДатаЛимитов.ДатаЛимитов));
	КонецЕсли;
	Запрос.УстановитьПараметр("ХозяйственнаяОперация", Параметры.ХозяйственнаяОперация);
	Запрос.УстановитьПараметр("ЛимитируемыеОперации", Параметры.ЛимитируемыеОперации);
	...
	ТекстыЗапросов.Добавить("
		|ВЫБРАТЬ
		|	ВЫРАЗИТЬ(ДатыЛимитов.ДатаЛимитов КАК ДАТА) КАК ДатаЛимитов
		|ПОМЕСТИТЬ ДатыЛимитов
		|ИЗ
		|	&ДатыЛимитовКРасчету КАК ДатыЛимитов
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ДатаЛимитов");
	...
	ТекстыЗапросов.Добавить("
		|ВЫБРАТЬ
		|	МоделиБюджетирования.Ссылка КАК Модель,
		|	МоделиБюджетирования.НачалоДействия КАК НачалоДействия,
		|	МоделиБюджетирования.КонецДействия КАК КонецДействия,
		|	ДатыЛимитов.ДатаЛимитов КАК ДатаЛимитов
		|ПОМЕСТИТЬ АктуальныеМоделиБюджетированияПоДатамЛимитов
		|ИЗ
		|	Справочник.МоделиБюджетирования КАК МоделиБюджетирования
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДатыЛимитов КАК ДатыЛимитов
		|		ПО МоделиБюджетирования.НачалоДействия <= ДатыЛимитов.ДатаЛимитов
		|			И МоделиБюджетирования.КонецДействия >= ДатыЛимитов.ДатаЛимитов
		|ГДЕ
		|	ИСТИНА В
		|			(ВЫБРАТЬ ПЕРВЫЕ 1
		|				ИСТИНА
		...
		|				И ПравилаЛимитовПоДаннымБюджетирования.ЛимитируемаяОперация В (&ЛимитируемыеОперации))
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	МоделиБюджетирования.Ссылка,
		|	МоделиБюджетирования.НачалоДействия,
		|	ДАТАВРЕМЯ(3999, 12, 31),
		|	ДатыЛимитов.ДатаЛимитов
		|ИЗ
		|	Справочник.МоделиБюджетирования КАК МоделиБюджетирования
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДатыЛимитов КАК ДатыЛимитов
		|		ПО МоделиБюджетирования.НачалоДействия <= ДатыЛимитов.ДатаЛимитов
		|			И МоделиБюджетирования.КонецДействия = ДАТАВРЕМЯ(1, 1, 1)
	...
	Если Не ФинансовыйУчетПоДаннымБалансовыхРегистров.ЕстьЗаписиВоВременнойТаблице(Запрос, ТекстыЗапросов,
		"АктуальныеМоделиБюджетированияПоДатамЛимитов") Тогда
		Возврат НастройкиЛимитов;
	КонецЕсли;
	
	ТекстыЗапросов.Добавить("
		|ВЫБРАТЬ
		|	МАКСИМУМ(АктуальныеМоделиБюджетированияПоДатамЛимитов.НачалоДействия) КАК НачалоДействия,
		|	МИНИМУМ(АктуальныеМоделиБюджетированияПоДатамЛимитов.КонецДействия) КАК КонецДействия,
		|	АктуальныеМоделиБюджетированияПоДатамЛимитов.ДатаЛимитов КАК ДатаЛимитов
		|ПОМЕСТИТЬ ПериодыДействияПоДатамЛимитов
		|ИЗ
		|	АктуальныеМоделиБюджетированияПоДатамЛимитов КАК АктуальныеМоделиБюджетированияПоДатамЛимитов
		|
		|СГРУППИРОВАТЬ ПО
		|	АктуальныеМоделиБюджетированияПоДатамЛимитов.ДатаЛимитов
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ДатаЛимитов");
	...
	ТекстыЗапросов.Добавить("
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	АктуальныеМоделиБюджетированияПоДатамЛимитов.Модель КАК Модель,
		|	ПериодыДействияПоДатамЛимитов.НачалоДействия КАК НачалоДействия,
		|	ПериодыДействияПоДатамЛимитов.КонецДействия КАК КонецДействия
		|ПОМЕСТИТЬ АктуальныеМоделиБюджетирования
		|ИЗ
		|	АктуальныеМоделиБюджетированияПоДатамЛимитов КАК АктуальныеМоделиБюджетированияПоДатамЛимитов
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПериодыДействияПоДатамЛимитов КАК ПериодыДействияПоДатамЛимитов
		|		ПО АктуальныеМоделиБюджетированияПоДатамЛимитов.ДатаЛимитов = ПериодыДействияПоДатамЛимитов.ДатаЛимитов
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Модель");
	
	ФинансовыйУчетПоДаннымБалансовыхРегистров.ДобавитьЗапросУдаленияВременнойТаблицы(ТекстыЗапросов,
		"АктуальныеМоделиБюджетированияПоДатамЛимитов");
	...
	ТекстыЗапросов.Добавить("
		...
		|	ПравилаЛимитовПоДаннымБюджетирования.ЛимитируемаяОперация КАК ЛимитируемаяОперация,
		|	АктуальныеМоделиБюджетирования.НачалоДействия КАК НачалоДействия,
		|	АктуальныеМоделиБюджетирования.КонецДействия КАК КонецДействия
		|ПОМЕСТИТЬ ДействующиеПравилаЛимитов
		|ИЗ
		|	Справочник.ПравилаЛимитовПоДаннымБюджетирования КАК ПравилаЛимитовПоДаннымБюджетирования
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ АктуальныеМоделиБюджетирования КАК АктуальныеМоделиБюджетирования
		|		ПО ПравилаЛимитовПоДаннымБюджетирования.Владелец = АктуальныеМоделиБюджетирования.Модель
		|		
		|ГДЕ
	...
	Если ИспользоватьПравилаПолученияФакта Тогда
		...
		Иначе
			...
			Если ПоРегиструЛимитов Тогда
				МассивИменДокументов = ОбщегоНазначенияУТКлиентСервер.ПреобразоватьСоответствиеИлиСтруктуруВМассив(
					ИсточникиДанныхПовтИсп.ВидыДокументовДляКонтроляЛимитов(Перечисления.ТипыДанныхУчета.ДенежныеСредства), Ложь);
			Иначе
			...
			Для Каждого ИмяДокумента Из МассивИменДокументов Цикл
				...
				Если ТекстыПодзапросов.Количество() > 0 Тогда
					ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПОМЕСТИТЬ ИсточникиДанныхПравилПолученияФакта", ""); // @query-part
				КонецЕсли;
		...
		ТекстыЗапросов.Добавить("
			...
			|	И НЕ ПравилаПолученияФактаПоСтатьямБюджетов.ПометкаУдаления
			|	И ПравилаПолученияФактаПоСтатьямБюджетов.ДействуетС <= &ДатаАктуальностиПравилФакта
			|	И (ПравилаПолученияФактаПоСтатьямБюджетов.ДействуетПо >= &ДатаАктуальностиПравилФакта
	...
	Иначе
		...
		Если Не ФинансовыйУчетПоДаннымБалансовыхРегистров.ЕстьЗаписиВоВременнойТаблице(Запрос, ТекстыЗапросов,
			"ДействующиеПравилаЛимитов") Тогда
			Возврат НастройкиЛимитов;
		КонецЕсли;
	...
	ТекстыЗапросов.Добавить("
		...
		|	ДействующиеПравилаЛимитов КАК ДействующиеПравилаЛимитов
		|ГДЕ
		|	ДействующиеПравилаЛимитов.ТипЛимита = ЗНАЧЕНИЕ(Перечисление.ТипыЛимитовРасходованияДС.РазрешающийВПределахЛимита)
		|	И ДействующиеПравилаЛимитов.ЛимитируемаяОперация = ЗНАЧЕНИЕ(Перечисление.ЛимитируемыеОперации.ДвижениеДенежныхСредств)");
	
	ТекстыЗапросов.Добавить("
		|ВЫБРАТЬ
		|	ПериодыДействияПоДатамЛимитов.ДатаЛимитов КАК ДатаЛимитов,
		|	ПериодыДействияПоДатамЛимитов.НачалоДействия КАК НачалоДействия,
		|	ПериодыДействияПоДатамЛимитов.КонецДействия КАК КонецДействия
		|ИЗ
		|	ПериодыДействияПоДатамЛимитов КАК ПериодыДействияПоДатамЛимитов");
	...
	Если ИспользоватьПравилаПолученияФакта Тогда
		...
		ТекстыЗапросов.Добавить("
			...
			|ИЗ
			|	Справочник.ПравилаПолученияФактаПоСтатьямБюджетов КАК ПравилаПолученияФактаПоСтатьямБюджетов
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДействующиеПравилаПолученияФакта КАК ДействующиеПравилаПолученияФакта
			|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПериодыДействияПоДатамЛимитов КАК ПериодыДействияПоДатамЛимитов
			|			ПО (ПериодыДействияПоДатамЛимитов.ДатаЛимитов = &ДатаАктуальностиПравилФакта)
			|		ПО ПравилаПолученияФактаПоСтатьямБюджетов.СтатьяБюджетов = ДействующиеПравилаПолученияФакта.СтатьяБюджетов
		...
		ТекстыПодзапросов.Добавить("
			...
			|	ДействующиеПравилаЛимитов.ЛимитируемаяОперация КАК ЛимитируемаяОперация,
			|	ДействующиеПравилаЛимитов.НачалоДействия КАК НачалоДействия,
			|	ДействующиеПравилаЛимитов.КонецДействия КАК КонецДействия
			|ИЗ
			|	ДействующиеПравилаЛимитов КАК ДействующиеПравилаЛимитов
			|ГДЕ
			|	ДействующиеПравилаЛимитов.ЛимитируемаяОперация = ЗНАЧЕНИЕ(Перечисление.ЛимитируемыеОперации.ДвижениеДенежныхСредств)
			|	И ИСТИНА В
			|			(ВЫБРАТЬ ПЕРВЫЕ 1
			|				ИСТИНА
			|			ИЗ
			...
			|				И (ДействующиеПравилаПолученияФакта.ПрименятьПравило))");
		Если ИспользоватьПравилаЛимитовБДР Тогда
			ТекстыПодзапросов.Добавить("
				|ВЫБРАТЬ
				|	ДействующиеПравилаЛимитов.ПравилоЛимитов,
				|	ДействующиеПравилаЛимитов.Наименование,
				|	ДействующиеПравилаЛимитов.СтатьяБюджетов,
				|	ДействующиеПравилаЛимитов.Сценарий,
				|	ДействующиеПравилаЛимитов.Периодичность,
				|	ДействующиеПравилаЛимитов.ТипЛимита,
				|	ДействующиеПравилаЛимитов.ЛимитируемаяОперация,
				|	ДействующиеПравилаЛимитов.НачалоДействия,
				|	ДействующиеПравилаЛимитов.КонецДействия
				|ИЗ
				|	ДействующиеПравилаЛимитов КАК ДействующиеПравилаЛимитов
				|ГДЕ
				|	ДействующиеПравилаЛимитов.ЛимитируемаяОперация = ЗНАЧЕНИЕ(Перечисление.ЛимитируемыеОперации.Расходы)");
		КонецЕсли;
	...
	ИначеЕсли ИспользоватьПравилаЛимитовБДР Тогда
		
		ТекстЗапроса = "
			|ВЫБРАТЬ
			|	ДействующиеПравилаЛимитов.ПравилоЛимитов КАК Правило,
			|	ДействующиеПравилаЛимитов.СтатьяБюджетов КАК СтатьяБюджетов,
			|	&ТекстВидовАналитик,
			|	&ТекстВыраженийЗаполненияАналитик,
			|	&ТекстЗаполненияУказаннымЗначениемАналитик,
			|	&ТекстЗначенийАналитик,
			|	ЗНАЧЕНИЕ(Перечисление.ЛимитируемыеОперации.Расходы) КАК ЛимитируемаяОперация,
			|	ЗНАЧЕНИЕ(Перечисление.ТипыДанныхУчета.ДоходыРасходы) КАК ТипДанныхУчетаПриход,
			|	ЗНАЧЕНИЕ(Перечисление.ТипыДанныхУчета.Номенклатура) КАК ТипДанныхУчетаРасход,
			|	НЕОПРЕДЕЛЕНО КАК ХешСхемыКомпоновкиДанных
			|ИЗ
			|	ДействующиеПравилаЛимитов КАК ДействующиеПравилаЛимитов
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПравилаЛимитовПоДаннымБюджетирования.ИерархияАналитик КАК ПравилаЛимитовПоДаннымБюджетированияИерархияАналитик
			|		ПО ДействующиеПравилаЛимитов.ПравилоЛимитов = ПравилаЛимитовПоДаннымБюджетированияИерархияАналитик.Ссылка
			|			И (ПравилаЛимитовПоДаннымБюджетированияИерархияАналитик.ВидАналитики <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.АналитикиСтатейБюджетов.ПустаяСсылка))
			|			И (ПравилаЛимитовПоДаннымБюджетированияИерархияАналитик.Используется)
			|ГДЕ
		...
		Для НомерАналитики = 1 По МаксимальноеКоличествоАналитик Цикл
			ТекстВидовАналитик = ТекстВидовАналитик + ?(ТекстВидовАналитик = "", "", "," + Символы.ПС + Символы.Таб)
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("МАКСИМУМ(ВЫБОР
				|	КОГДА ПравилаЛимитовПоДаннымБюджетированияИерархияАналитик.ВидАналитики = ДействующиеПравилаЛимитов.СтатьяБюджетов.ВидАналитики%1
				|		ТОГДА ПравилаЛимитовПоДаннымБюджетированияИерархияАналитик.ВидАналитики
				|	ИНАЧЕ НЕОПРЕДЕЛЕНО
				|КОНЕЦ) КАК ВидАналитики%1", НомерАналитики);
			ТекстВыраженийЗаполненияАналитик = ТекстВыраженийЗаполненияАналитик + ?(ТекстВыраженийЗаполненияАналитик
				= "", "", "," + Символы.ПС + Символы.Таб) + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("МАКСИМУМ(ВЫБОР
				|	КОГДА ПравилаЛимитовПоДаннымБюджетированияИерархияАналитик.ВидАналитики = ДействующиеПравилаЛимитов.СтатьяБюджетов.ВидАналитики%1
				|		ТОГДА ВЫРАЗИТЬ(ПравилаЛимитовПоДаннымБюджетированияИерархияАналитик.ВыражениеЗаполненияАналитики КАК СТРОКА(1000))
				|	ИНАЧЕ """"
				|КОНЕЦ) КАК ВыражениеЗаполненияАналитики%1", НомерАналитики);
			ТекстЗаполненияУказаннымЗначениемАналитик = ТекстЗаполненияУказаннымЗначениемАналитик + ?(
				ТекстЗаполненияУказаннымЗначениемАналитик = "", "", "," + Символы.ПС + Символы.Таб)
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("МАКСИМУМ(ВЫБОР
				|	КОГДА ПравилаЛимитовПоДаннымБюджетированияИерархияАналитик.ВидАналитики = ДействующиеПравилаЛимитов.СтатьяБюджетов.ВидАналитики%1
				|		ТОГДА ПравилаЛимитовПоДаннымБюджетированияИерархияАналитик.ЗаполнятьУказаннымЗначениемАналитику
				|	ИНАЧЕ ЛОЖЬ
				|КОНЕЦ) КАК ЗаполнятьУказаннымЗначениемАналитику%1", НомерАналитики);
			ТекстЗначенийАналитик = ТекстЗначенийАналитик + ?(ТекстЗначенийАналитик = "", "", "," + Символы.ПС
				+ Символы.Таб) + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("МАКСИМУМ(ВЫБОР
				|	КОГДА ПравилаЛимитовПоДаннымБюджетированияИерархияАналитик.ВидАналитики = ДействующиеПравилаЛимитов.СтатьяБюджетов.ВидАналитики%1
				|		ТОГДА ПравилаЛимитовПоДаннымБюджетированияИерархияАналитик.ЗначениеАналитики
				|	ИНАЧЕ НЕОПРЕДЕЛЕНО
				|КОНЕЦ) КАК ЗначениеАналитики%1", НомерАналитики);
		КонецЦикла;
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстВидовАналитик", ТекстВидовАналитик);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстВыраженийЗаполненияАналитик", ТекстВыраженийЗаполненияАналитик);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстЗаполненияУказаннымЗначениемАналитик",
			ТекстЗаполненияУказаннымЗначениемАналитик);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстЗначенийАналитик", ТекстЗначенийАналитик);
		ТекстыЗапросов.Добавить(ТекстЗапроса);
	...
	ПериодыДействияПоДатамЛимитовКРасчету = РезультатЗапроса[1].Выгрузить();
	ОбщегоНазначенияУТ.ДобавитьСтрокиВТаблицу(ПериодыДействияПоДатамЛимитов, ПериодыДействияПоДатамЛимитовКРасчету);
	ПериодыДействия = ПериодыДействияПоДатамЛимитовКРасчету.Скопировать(, ИменаКолонокПериодыДействия);
	ПериодыДействия.Свернуть(ИменаКолонокПериодыДействия);
	НесколькоПериодовДействия = (ПериодыДействия.Количество() > 1);
	
	ПравилаЛимитов = РезультатЗапроса[3-СмещениеИндекса].Выгрузить();
	Если НесколькоПериодовДействия Тогда
		ПравилаЛимитов.Индексы.Добавить(ИменаКолонокПериодыДействия);
	КонецЕсли;
	
	ПравилаПолученияФакта = Неопределено;
	Если ИспользоватьПравилаПолученияФакта Тогда
		ПравилаПолученияФакта = РезультатЗапроса[2].Выгрузить();
		...
		Если НесколькоПериодовДействия Тогда
			ПравилаПолученияФакта.Индексы.Добавить(ИменаКолонокПериодыДействия);
		КонецЕсли;
	КонецЕсли;
	
	РазрешающийЛимитОбязателен = Не РезультатЗапроса[0].Пустой();
	Если Не ПоРегиструЛимитов Тогда
		ВидыАналитик = РезультатЗапроса[4].Выгрузить();
		Измерения = РезультатЗапроса[5].Выгрузить();
	ИначеЕсли ИспользоватьПравилаЛимитовБДР Тогда
		ВидыАналитик = РезультатЗапроса[4-СмещениеИндекса].Выгрузить();
	КонецЕсли;
	
	Для Каждого ПериодДействия Из ПериодыДействия Цикл
	
		СценарииПравил = Новый Соответствие;
		
		Если НесколькоПериодовДействия Тогда
			ЗаполнитьЗначенияСвойств(СтруктураПериодДействия, ПериодДействия);
			ПравилаЛимитовПоПериодуДействия = ПравилаЛимитов.Скопировать(СтруктураПериодДействия);
		Иначе
			ПравилаЛимитовПоПериодуДействия = ПравилаЛимитов;
		КонецЕсли;
		...
		ИерархияАналитикПоПравилам = Справочники.ПравилаЛимитовПоДаннымБюджетирования.ИерархияАналитикПоПравиламСУказаниемКолонокИсточника(
			ПравилаЛимитовПоПериодуДействия.ВыгрузитьКолонку("ПравилоЛимитов"));
		Для каждого СтрокаПравила Из ПравилаЛимитовПоПериодуДействия Цикл
		...
		ПравилаЛимитовСДопДанными = ДанныеДополнительныхПолейПравилЛимитов(ПравилаЛимитовПоПериодуДействия, ИерархияАналитикПоПравилам);
		
		ПравилаЛимитовНаПериодДействия = ОбщегоНазначения.СкопироватьРекурсивно(ПравилаЛимитовНаДату);
		Если ИспользоватьПравилаПолученияФакта И НесколькоПериодовДействия Тогда
			ПравилаЛимитовНаПериодДействия.ПравилаПолученияФакта = ПравилаПолученияФакта.Скопировать(СтруктураПериодДействия);
		Иначе
			ПравилаЛимитовНаПериодДействия.ПравилаПолученияФакта = ПравилаПолученияФакта;
		КонецЕсли;
		ПравилаЛимитовНаПериодДействия.ПравилаЛимитов = ПравилаЛимитовСДопДанными;
		ПравилаЛимитовНаПериодДействия.ИерархияАналитикПоПравилам = ИерархияАналитикПоПравилам;
		ПравилаЛимитовНаПериодДействия.РазрешающийЛимитОбязателен = РазрешающийЛимитОбязателен;
		ПравилаЛимитовНаПериодДействия.СценарииПравил = СценарииПравил;
		Если Не ПоРегиструЛимитов Тогда
			ПравилаЛимитовНаПериодДействия.ВидыАналитик = ВидыАналитик;
			ПравилаЛимитовНаПериодДействия.Измерения = Измерения;
		ИначеЕсли ИспользоватьПравилаЛимитовБДР Тогда
			Если НесколькоПериодовДействия Тогда
				ПравилаЛимитовНаПериодДействия.ВидыАналитик = ВидыАналитик.СкопироватьКолонки(); // - ТаблицаЗначений -
				Для Каждого СтрокаТаблицы Из ВидыАналитик Цикл
					Если СценарииПравил.Получить(СтрокаТаблицы.Правило) <> Неопределено Тогда
						НоваяСтрока = ПравилаЛимитовНаПериодДействия.ВидыАналитик.Добавить();
		...
		Если ПоРегиструЛимитов И ЗначениеЗаполнено(Параметры.ВидДокумента) Тогда
			НастройкиЛимитовНаДату.НастройкаПолученияФакта = НастройкаПолученияФактаПоДокументуНаДату(
				Параметры.ВидДокумента, ПравилаЛимитовНаПериодДействия.ПравилаПолученияФакта,
				ПравилаЛимитовНаПериодДействия.ВидыАналитик);
		КонецЕсли;
		НастройкиЛимитовНаДату.НастройкаПравилЛимитов = ПравилаЛимитовНаПериодДействия;
		РегистрыСведений.КэшНастроекЛимитовПоДаннымБюджетирования.УстановитьЗначениеКэша(
			ПериодДействия.НачалоДействия, ПериодДействия.КонецДействия, Параметры.ВидДокумента,
			Параметры.ХозяйственнаяОперация, НастройкиЛимитовНаДату);
		
		НастройкиЛимитовНаДату.Вставить("НачалоДействия", ПериодДействия.НачалоДействия);
		НастройкиЛимитовНаДату.Вставить("КонецДействия", ПериодДействия.КонецДействия);
		НастройкиЛимитовПоПериодамДействия.Добавить(НастройкиЛимитовНаДату);
	КонецЦикла;
...
Процедура ДобавитьЗапросПолученияФактаПоПериодуДействия(ТекстыЗапросов, НастройкаПолученияФакта, НачалоДействия,
	КонецДействия, ДатыЛимитов)
	
	СтруктураПериодДействия = Новый Структура("НачалоДействия, КонецДействия", НачалоДействия, КонецДействия);
	
	ХешПериодаДействия = ОбщегоНазначения.КонтрольнаяСуммаСтрокой(СтруктураПериодДействия);
	Для Каждого ИмяВременнойТаблицы Из НастройкаПолученияФакта.ИменаВременныхТаблиц Цикл
		НастройкаПолученияФакта.ТекстЗапроса = СтрЗаменить(НастройкаПолученияФакта.ТекстЗапроса, ИмяВременнойТаблицы,
			ИмяВременнойТаблицы + ХешПериодаДействия);
	КонецЦикла;
	ИмяВременнойТаблицыФакта = "ФактическиеДанныеПоДокументу" + ХешПериодаДействия;
	НастройкаПолученияФакта.ТекстЗапроса = СтрЗаменить(НастройкаПолученияФакта.ТекстЗапроса,
		"ФактическиеДанныеПоДокументу", ИмяВременнойТаблицыФакта);
	
	ДатыЛимитовПоПериодуДействия = ДатыЛимитов.НайтиСтроки(СтруктураПериодДействия);
	МинимальнаяДатаЛимитов = ДатыЛимитовПоПериодуДействия[0].ДатаЛимитов;
	МаксимальнаяДатаЛимитов = ДатыЛимитовПоПериодуДействия[ДатыЛимитовПоПериодуДействия.ВГраница()].ДатаЛимитов;
	НастройкаПолученияФакта.ТекстЗапроса = СтрЗаменить(НастройкаПолученияФакта.ТекстЗапроса, "&НачалоПериода",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(МинимальнаяДатаЛимитов));
	НастройкаПолученияФакта.ТекстЗапроса = СтрЗаменить(НастройкаПолученияФакта.ТекстЗапроса, "&КонецПериода",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(МаксимальнаяДатаЛимитов));
	...
	Для Каждого ИмяВременнойТаблицы Из НастройкаПолученияФакта.ИменаВременныхТаблиц Цикл
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ДобавитьЗапросУдаленияВременнойТаблицы(ТекстыЗапросов,
			ИмяВременнойТаблицы + ХешПериодаДействия);
	КонецЦикла;
	НастройкаПолученияФакта.ИменаВременныхТаблиц.Очистить();
	НастройкаПолученияФакта.ИменаВременныхТаблиц.Добавить(ИмяВременнойТаблицыФакта);
...
//		*ПравилаЛимитов - см. ЛимитыПоДаннымБюджетированияСервер.НастройкиЛимитовНаДатыПроверки
//		*ПравилаПолученияФакта - ТаблицаЗначений - см. в Справочники.ПравилаПолученияФактаПоСтатьямБюджетов.Макеты.ПравилаПолученияФакта
...
Функция ДействующиеЛимиты(Параметры, ОтборПоСтатьямБюджетов = Неопределено, ИнвертироватьОтбор = Ложь, ОтборПоПравиламЛимитов = Неопределено)
	...
	ДействующиеЛимиты.Вставить("ПравилаЛимитов");
	ДействующиеЛимиты.Вставить("ПравилаПолученияФакта");
	ДействующиеЛимиты.Вставить("ИерархияАналитикПоПравилам");
	...
	НастройкиЛимитовНаДаты = НастройкиЛимитовНаДатыПроверки(Параметры);
	Если НастройкиЛимитовНаДаты.НастройкиЛимитовПоПериодамДействия.Количество() = 0 Тогда
		Возврат ДействующиеЛимиты;
	КонецЕсли;
	
	ТекстыЗапросов = Новый Массив;
	
	Для Каждого НастройкаЛимитовПоПериодуДействия Из НастройкиЛимитовНаДаты.НастройкиЛимитовПоПериодамДействия Цикл
		Если ДействующиеЛимиты.ПравилаЛимитов = Неопределено Тогда
			ЗаполнитьЗначенияСвойств(ДействующиеЛимиты, НастройкаЛимитовПоПериодуДействия);
			ЗаполнитьЗначенияСвойств(ДействующиеЛимиты, НастройкаЛимитовПоПериодуДействия.НастройкаПравилЛимитов);
			Если НастройкиЛимитовНаДаты.НастройкиЛимитовПоПериодамДействия.Количество() > 1 Тогда
				ДобавитьЗапросПолученияФактаПоПериодуДействия(ТекстыЗапросов,
					НастройкаЛимитовПоПериодуДействия.НастройкаПолученияФакта,
					НастройкаЛимитовПоПериодуДействия.НачалоДействия, НастройкаЛимитовПоПериодуДействия.КонецДействия,
					НастройкиЛимитовНаДаты.ПериодыДействияПоДатамЛимитов);
			КонецЕсли;
		Иначе
			// Настройка получения факта
			ДобавитьЗапросПолученияФактаПоПериодуДействия(ТекстыЗапросов,
				НастройкаЛимитовПоПериодуДействия.НастройкаПолученияФакта,
				НастройкаЛимитовПоПериодуДействия.НачалоДействия, НастройкаЛимитовПоПериодуДействия.КонецДействия,
				НастройкиЛимитовНаДаты.ПериодыДействияПоДатамЛимитов);
			ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ДействующиеЛимиты.НастройкаПолученияФакта.ПараметрыЗапроса,
				НастройкаЛимитовПоПериодуДействия.НастройкаПолученияФакта.ПараметрыЗапроса, Ложь);
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
				ДействующиеЛимиты.НастройкаПолученияФакта.ИменаВременныхТаблиц,
				НастройкаЛимитовПоПериодуДействия.НастройкаПолученияФакта.ИменаВременныхТаблиц);
			
			// Настройка правил лимитов
			НастройкаПравилЛимитов = НастройкаЛимитовПоПериодуДействия.НастройкаПравилЛимитов;
			ОбщегоНазначенияУТ.ДобавитьСтрокиВТаблицу(ДействующиеЛимиты.ПравилаЛимитов,
				НастройкаПравилЛимитов.ПравилаЛимитов);
			Если ТипЗнч(НастройкаПравилЛимитов.ПравилаПолученияФакта) = Тип("ТаблицаЗначений") Тогда
				Если ДействующиеЛимиты.ПравилаПолученияФакта = Неопределено Тогда
					ДействующиеЛимиты.ПравилаПолученияФакта = НастройкаПравилЛимитов.ПравилаПолученияФакта.Скопировать();
				Иначе
					ОбщегоНазначенияУТ.ДобавитьСтрокиВТаблицу(ДействующиеЛимиты.ПравилаПолученияФакта,
						НастройкаПравилЛимитов.ПравилаПолученияФакта);
				КонецЕсли;
			КонецЕсли;
			ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(ДействующиеЛимиты.ИерархияАналитикПоПравилам,
				НастройкаПравилЛимитов.ИерархияАналитикПоПравилам, Ложь);
			Если Не ДействующиеЛимиты.РазрешающийЛимитОбязателен Тогда
				ДействующиеЛимиты.РазрешающийЛимитОбязателен = НастройкаПравилЛимитов.РазрешающийЛимитОбязателен;
			КонецЕсли;
			Если ТипЗнч(НастройкаПравилЛимитов.ВидыАналитик) = Тип("ТаблицаЗначений") Тогда
				Если ДействующиеЛимиты.ВидыАналитик = Неопределено Тогда
					ДействующиеЛимиты.ВидыАналитик = НастройкаПравилЛимитов.ВидыАналитик.Скопировать();
				Иначе 
					Для Каждого СтрокаТаблицы Из НастройкаПравилЛимитов.ВидыАналитик Цикл
						Если ДействующиеЛимиты.СценарииПравил.Получить(СтрокаТаблицы.Правило) = Неопределено Тогда
							НоваяСтрока = ДействующиеЛимиты.ВидыАналитик.Добавить();
							ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
			ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(ДействующиеЛимиты.СценарииПравил,
				НастройкаПравилЛимитов.СценарииПравил, Ложь);
		КонецЕсли;
	КонецЦикла;
	
	Если ТекстыЗапросов.Количество() > 0 Тогда
		ИменаВременныхТаблиц = ДействующиеЛимиты.НастройкаПолученияФакта.ИменаВременныхТаблиц;
		ШаблонТекстаПодзапроса = "
			|ВЫБРАТЬ
			|	*
			|ПОМЕСТИТЬ ФактическиеДанныеПоДокументу
			|ИЗ
			|	ПсевдонимТаблицыФакта КАК ПсевдонимТаблицыФакта";
		ТекстыПодзапросов = Новый Массив;
		Для Каждого ИмяВременнойТаблицы Из ИменаВременныхТаблиц Цикл
			ТекстПодзапроса = ?(ИменаВременныхТаблиц.Найти(ИмяВременнойТаблицы) = 0, ШаблонТекстаПодзапроса,
				СтрЗаменить(ШаблонТекстаПодзапроса, "ПОМЕСТИТЬ ФактическиеДанныеПоДокументу", "")); // @query-part
			ТекстыПодзапросов.Добавить(СтрЗаменить(ТекстПодзапроса, "ПсевдонимТаблицыФакта", ИмяВременнойТаблицы));
		КонецЦикла;
		ТекстыЗапросов.Добавить(СтрСоединить(ТекстыПодзапросов, ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении())
			+ Символы.ПС + "ИНДЕКСИРОВАТЬ ПО СтатьяБюджетов, ПравилоЛимитов, ПериодЛимитирования"); // @query-part
		Для Каждого ИмяВременнойТаблицы Из ИменаВременныхТаблиц Цикл
			ФинансовыйУчетПоДаннымБалансовыхРегистров.ДобавитьЗапросУдаленияВременнойТаблицы(ТекстыЗапросов,
				ИмяВременнойТаблицы);
		КонецЦикла;
		ИменаВременныхТаблиц.Очистить();
		ДействующиеЛимиты.НастройкаПолученияФакта.ТекстЗапроса = СтрСоединить(ТекстыЗапросов,
			ОбщегоНазначения.РазделительПакетаЗапросов());
	КонецЕсли;
	
	СтруктураПериодДействия = Новый Структура("НачалоДействия, КонецДействия");
	
	ИменаКолонокПравилЛимитов = Новый Массив;
	Для Каждого Колонка Из ДействующиеЛимиты.ПравилаЛимитов.Колонки Цикл // - КолонкаТаблицыЗначений -
		Если Не СтруктураПериодДействия.Свойство(Колонка.Имя) Тогда
			ИменаКолонокПравилЛимитов.Добавить(Колонка.Имя);
		КонецЕсли;
	КонецЦикла;
	ИменаКолонокПравилЛимитов = СтрСоединить(ИменаКолонокПравилЛимитов, ",");
	ПравилаЛимитовСПериодамиЛимитирования = ДействующиеЛимиты.ПравилаЛимитов.СкопироватьКолонки(ИменаКолонокПравилЛимитов);
	ПериодыЛимитированияПоПравилу = ПравилаЛимитовСПериодамиЛимитирования.СкопироватьКолонки("НачалоПериода, КонецПериода");
	Для Каждого СтрокаПравило Из ДействующиеЛимиты.ПравилаЛимитов Цикл
		ЗаполнитьЗначенияСвойств(СтруктураПериодДействия, СтрокаПравило);
		ПериодыЛимитированияПоПравилу.Очистить();
		Для Каждого ДатаЛимитов Из НастройкиЛимитовНаДаты.ПериодыДействияПоДатамЛимитов.НайтиСтроки(
			СтруктураПериодДействия) Цикл
			ПериодЛимитирования = ПериодыЛимитированияПоПравилу.Добавить();
			ПериодЛимитирования.НачалоПериода = РаботаСПериодамиКлиентСервер.ДатаНачалаПериода(ДатаЛимитов.ДатаЛимитов,
				СтрокаПравило.Периодичность);
			ПериодЛимитирования.КонецПериода = РаботаСПериодамиКлиентСервер.ДатаКонцаПериода(ДатаЛимитов.ДатаЛимитов,
				СтрокаПравило.Периодичность);
		КонецЦикла;
		ПериодыЛимитированияПоПравилу.Свернуть("НачалоПериода, КонецПериода");
		Для Каждого ПериодЛимитирования Из ПериодыЛимитированияПоПравилу Цикл
			НоваяСтрока = ПравилаЛимитовСПериодамиЛимитирования.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаПравило);
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ПериодЛимитирования);
		КонецЦикла;
	КонецЦикла;
	ДействующиеЛимиты.Вставить("ПравилаЛимитов", ПравилаЛимитовСПериодамиЛимитирования);
	ПравилаЛимитовСПериодамиЛимитирования.Свернуть(ИменаКолонокПравилЛимитов);
	
	ЛимитыДействующиеНаДаты = ДействующиеЛимиты.ПравилаЛимитов; // ТаблицаЗначений -
	ЛимитыДействующиеНаДаты.Индексы.Добавить("ПравилоЛимитов"); // Ссылка на СправочникСсылка.ПравилаЛимитовПоДаннымБюджетирования
	ЛимитыДействующиеНаДаты.Индексы.Добавить("Сценарий");
	ЛимитыДействующиеНаДаты.Индексы.Добавить("Периодичность");
	...
	Если ТипЗнч(ОтборПоСтатьямБюджетов) = Тип("Массив") Тогда
		...
		СтатьиБюджетовПравилЛимитов = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ЛимитыДействующиеНаДаты.ВыгрузитьКолонку("СтатьяБюджетов"));
	...
	Если ТребуетсяОтборПоСтатьямБюджетов Тогда
		Для Каждого КлючИЗначение Из ДействующиеЛимиты Цикл
	...
	Если ТипЗнч(ОтборПоПравиламЛимитов) = Тип("Массив") Тогда
		ПравилаЛимитов = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ЛимитыДействующиеНаДаты.ВыгрузитьКолонку("ПравилоЛимитов"));
...
Функция МенеджерВТ_ПланФактПоРегиструЛимитов(ПравилаЛимитовСДопДанными, ФактическиеДанные, ДополнительныеПараметры,
	...
	Запрос.УстановитьПараметр("ДатаПроверки", ДополнительныеПараметры.ДатыПроверки[0].ДатаЛимитов);
	...
	|	ВЫРАЗИТЬ(ПравилаЛимитовСДопДанными.ЛимитируемаяОперация КАК Перечисление.ЛимитируемыеОперации) КАК ЛимитируемаяОперация,
	...
	|	СУММА(ФактическиеДанные.СуммаФакт) КАК СуммаФакт
	...
	|
	|ГДЕ
	|	ПравилаЛимитовСДопДанными.ЛимитируемаяОперация = ЗНАЧЕНИЕ(Перечисление.ЛимитируемыеОперации.ДвижениеДенежныхСредств)
	|
	|СГРУППИРОВАТЬ ПО
	|	ПравилаЛимитовСДопДанными.ПравилоЛимитов,
	|	ПравилаЛимитовСДопДанными.НачалоПериода,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ФактическиеДанныеПоДокументу КАК ФактическиеДанные
	|		ПО ПравилаЛимитовСДопДанными.СтатьяБюджетов = ФактическиеДанные.СтатьяБюджетов
	|			И ПравилаЛимитовСДопДанными.ПравилоЛимитов = ФактическиеДанные.ПравилоЛимитов
	|			И ПравилаЛимитовСДопДанными.НачалоПериода = ФактическиеДанные.ПериодЛимитирования
	|
	|ГДЕ
	|	ПравилаЛимитовСДопДанными.ЛимитируемаяОперация = ЗНАЧЕНИЕ(Перечисление.ЛимитируемыеОперации.Расходы)
	|
	|СГРУППИРОВАТЬ ПО
	|	ПравилаЛимитовСДопДанными.ПравилоЛимитов,
	|	ПравилаЛимитовСДопДанными.НачалоПериода,
	...
	Для НомерАналитики = 1 По МаксимальноеКоличествоАналитик Цикл
		ТекстПолейАналитик = ТекстПолейАналитик + ?(ТекстПолейАналитик = "", "", "," + Символы.ПС + Символы.Таб)
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				НомерАналитики);
		ТекстПолейАналитикФакт = ТекстПолейАналитикФакт + ?(ТекстПолейАналитикФакт = "", "", "," + Символы.ПС
			+ Символы.Таб) + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"ФактическиеДанные.Аналитика%1 КАК Аналитика%1Факт",
				НомерАналитики);
		ТекстГруппАналитик = ТекстГруппАналитик + ?(ТекстГруппАналитик = "", "", "," + Символы.ПС + Символы.Таб)
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|КОНЕЦ",
				НомерАналитики);
		ТекстГруппАналитикФакт = ТекстГруппАналитикФакт + ?(ТекстГруппАналитикФакт = "", "", "," + Символы.ПС
			+ Символы.Таб) + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"ФактическиеДанные.Аналитика%1",
				НомерАналитики);
	КонецЦикла;
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстПолейАналитикФакт", ТекстПолейАналитикФакт);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстПолейАналитик", ТекстПолейАналитик);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстГруппАналитикФакт", ТекстГруппАналитикФакт);
	...
	|	ФактическиеДанные.Валюта КАК Валюта,
	|	ФактическиеДанные.Сумма КАК Сумма,
	|	ФактическиеДанные.СуммаФакт КАК СуммаФакт
...
Процедура ДополнитьВременнуюТаблицуФактаАналитикамиПлана(МенеджерВременныхТаблиц, ТаблицаФакта)
	...
	|	ВЫРАЗИТЬ(ФактическиеДанные.Период КАК ДАТА) КАК Период,
	...
	|	&ТекстАналитикФакт,
	|	&ТекстАналитикПлан,
	|	ВЫРАЗИТЬ(ФактическиеДанные.Сумма КАК ЧИСЛО(31, 2)) КАК Сумма,
	|	ВЫРАЗИТЬ(ФактическиеДанные.СуммаФакт КАК ЧИСЛО(31, 2)) КАК СуммаФакт
	|ПОМЕСТИТЬ ФактическиеДанныеПоПравилам
	|ИЗ
	|	&ФактическиеДанные КАК ФактическиеДанные
	|
...
Функция РассчитатьПланФактИСформироватьТаблицуДвижений(МенеджерВременныхТаблиц, ПараметрыКонтроля)
	...
	|	ПравилаЛимитовСДопДанными.ПравилоЛимитов КАК ПравилоЛимитов,
	|	ПравилаЛимитовСДопДанными.ТипЛимита КАК ТипЛимита,
	|	ПравилаЛимитовСДопДанными.ЛимитируемаяОперация КАК ЛимитируемаяОперация,
	|	ПравилаЛимитовСДопДанными.НачалоПериода КАК НачалоПериода,
	|	ПравилаЛимитовСДопДанными.КонецПериода КАК КонецПериода,
	...
	|ГДЕ
	|	&РезервироватьСуммуЛимита
	|	И ФактическиеДанные.СуммаФакт <> 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	0,
	|	0,
	|	ВЫРАЗИТЬ(ФактическиеДанные.СуммаФакт / ЕСТЬNULL(ПлановыеДанные.КоэффициентПересчетаВЦелевуюВалюту, 1) КАК ЧИСЛО(31,2))
	|ИЗ
	|	ФактическиеДанныеПоПравилам КАК ФактическиеДанные
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПравилаЛимитовСДопДанными КАК ПравилаЛимитовСДопДанными
	|		ПО ФактическиеДанные.НачалоПериода = ПравилаЛимитовСДопДанными.НачалоПериода
	...
	Для Каждого СтрокаЗадание Из Задания Цикл
		...
		Попытка
			...
			Иначе
				...
				Запрос.УстановитьПараметр("ДатаПроверки", Параметры.РеквизитыДокумента.ДатыПроверки[0].ДатаЛимитов);
...
Процедура ОпределитьНеобходимостьКонтроляЛимитовПередЗаписьюДокумента(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Если Не ИспользуетсяРегистрЛимитовБюджетирования() Тогда
		Возврат;
	КонецЕсли;
	...
	Если Источник.Проведен И РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если ТипЗнч(Источник) = Тип("ДокументОбъект.ЗаявкаНаРасходованиеДенежныхСредств")
			Или ТипЗнч(Источник) = Тип("ДокументОбъект.ЗаявкаНаКомандировку") Тогда
			НепроверяемыеРеквизиты.Вставить("КтоРешил");
			ТекущийСтатус = Источник.Статус;
			ИсходныйСтатус = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Источник.Ссылка, "Статус");
			ПроверяемыеСтатусы = Новый Массив;
			ПроверяемыеСтатусы.Добавить(Перечисления.СтатусыЗаявокНаРасходованиеДенежныхСредств.Отклонена);
			ПроверяемыеСтатусы.Добавить(Перечисления.СтатусыЗаявокСотрудников.Отклонена);
			ПроверяемыеСтатусы.Добавить(Перечисления.СтатусыЗаявокСотрудников.Отозвана);
			Если ПроверяемыеСтатусы.Найти(ТекущийСтатус) = Неопределено И ПроверяемыеСтатусы.Найти(ИсходныйСтатус)
				= Неопределено Тогда
				НепроверяемыеРеквизиты.Вставить("Статус");
			КонецЕсли;
		КонецЕсли;
