...
// Выполнить подключаемую команду интеграции ССППР.
// 
// Параметры:
//  Ссылка - ЛюбаяСсылка
//  Параметры - Структура - Структура с параметрами команды.
//  Форма - ФормаКлиентскогоПриложения - Форма в которой расположена команда.
//
Процедура ВыполнитьПодключаемуюКомандуИнтеграцииССППР(Ссылка, Параметры) Экспорт
	Форма = Параметры.Форма;
	Если Найти(Форма.ИмяФормы, "Отчет.") <> 0 Тогда
		ПараметрыФормы = Новый Структура("ДополнительныеПараметры");
		ПараметрыФормы.ДополнительныеПараметры.Вставить("Отчет", Форма.ИмяФормы);
	Иначе
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ИмяФормы", Форма.ИмяФормы);
		ПараметрыФормы.Вставить("Заголовок", Форма.Заголовок);
	КонецЕсли;
	ВыполнитьКомандуИнтеграцииССППР(Форма,Новый Структура("Имя", "КомандаСППР"), ПараметрыФормы);
