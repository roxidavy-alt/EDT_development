...
Функция ЗаголовокСостоянияОшибка(ДатаОшибки) Экспорт
	...
	Если Пользователи.РолиДоступны("ПросмотрЖурналаРегистрации") Тогда
		ТекстЗаголовка = НСтр("ru = '%1 при синхронизации с банком произошла ошибка. Подробнее в <a href = ""ЖурналРегистрации"">Журнале регистрации</a>.';
								|en = '%1 при синхронизации с банком произошла ошибка. Подробнее в <a href = ""ЖурналРегистрации"">Журнале регистрации</a>.'");
	Иначе
		ТекстЗаголовка = НСтр("ru = '%1 при синхронизации с банком произошла ошибка. Подробнее в журнале регистрации, обратитесь к администратору.';
								|en = '%1 при синхронизации с банком произошла ошибка. Подробнее в журнале регистрации, обратитесь к администратору.'");
	КонецЕсли;
...
Функция ТекстПисьмаЗапросОПодключенииАвтоматическойВыписки(Организация, Банк) Экспорт
	...
	МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'ИНН: %1';
										|en = 'TIN: %1'"), РеквизитыОрганизации.ИНН));
	МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'КПП: %1';
										|en = 'KPP: %1'"), РеквизитыОрганизации.КПП));
	...
	Если Счета.Количество() > 0 Тогда
		...
		МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Номер счета: %1';
											|en = 'Номер счета: %1'"), Счета[0]));
		МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Банк: %1';
											|en = 'Банк: %1'"), РеквизитыБанка.Наименование));
		МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'БИК: %1';
											|en = 'BIC: %1'"), РеквизитыБанка.Код));
	КонецЕсли;
...
Функция ЗапускЗаданияСменыПароляСбербанк(
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Смена пароля на сервере Сбербанка.';
															|en = 'Change the password on Sberbank server.'");
...
Функция ЗапускЗаданияОжиданиеЗавершенияОбработкиДокументовБанком(
	...
	Если Не ЗначениеЗаполнено(ДанныеСессии.ИмяЗадания) Тогда
		...
		ДанныеСессии.ИмяЗадания = СтрШаблон(НСтр("ru = '1С:ДиректБанк. Проверка статусов документов в банке: %1';
												|en = '1С:ДиректБанк. Проверка статусов документов в банке: %1'"),
			Банк);
