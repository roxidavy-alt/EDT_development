...
// 
// Параметры:
// 	Период - Дата - начало месяца, на дату которого выполняется расчет.
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Параметры учета отложенного налога в добавочном капитале:
// * Счет - ПланСчетовСсылка.Хозрасчетный - исходный счет;
// * СчетОтложенногоНалога - ПланСчетовСсылка.Хозрасчетный - счет отложенного налога;
// * КоррСчетОтложенногоНалога - ПланСчетовСсылка.Хозрасчетный - корреспондирующий счет отложенного налога;
// * ВидАктивовОбязательств - ПеречислениеСсылка.ВидыАктивовИОбязательств - используемый вид активов и обязательств;
// * ВидСубконтоАктивовОбязательств - ПланВидовХарактеристикСсылка.ВидыСубконтоХозрасчетные - вид субконто актива/обязательства.
Функция ПараметрыУчетаОтложенногоНалогаВДобавочномКапитале(Период) Экспорт
...
// Дополнить правила группировки видов активов/обязательств основного производства.
// 
// Параметры:
//  Правила - ТаблицаЗначений - Правила:
//   * ОсновнойСчет - ПланСчетовСсылка.Хозрасчетный - 
//   * ВидАктивовОбязательств - ПеречислениеСсылка.ВидыАктивовИОбязательств - 
//   * ДополнительныеСчета - Массив из ПланСчетовСсылка.Хозрасчетный - 
//   * ИсключенныеСубконто - Массив из ПланВидовХарактеристикСсылка.ВидыСубконтоХозрасчетные - 
Процедура ДополнитьПравилаОсновногоПроизводства(Правила) Экспорт

	СчетаМатериалы = РеглУчетВыборкиСерверПовтИсп.СчетаМатериалыПодрядныеРаботыОсновногоПроизводстваКИспользованию();
	СчетаРезервы = РеглУчетВыборкиСерверПовтИсп.СчетаРезервыПодСнижениеСтоимостиМатериаловВПроизводственныхПодразделениях();
	
	Для Каждого СчетМатериалы Из СчетаМатериалы Цикл
		Правило = Правила.Добавить();
		Правило.ОсновнойСчет           = СчетМатериалы;
		Правило.ВидАктивовОбязательств = Перечисления.ВидыАктивовИОбязательств.НезавершенноеПроизводство;
		
		Для Каждого СчетРезервов Из СчетаРезервы Цикл
			Правило.ДополнительныеСчета.Добавить(СчетРезервов);		
		КонецЦикла;			
	КонецЦикла;
	
	СчетаПроизводственныеЗатраты = РеглУчетВыборкиСерверПовтИсп.СчетаПроизводственныеЗатратыОсновногоПроизводства();
	СчетаРезервы = РеглУчетВыборкиСерверПовтИсп.СчетаРезервыПодСнижениеСтоимостиНезавершенногоПроизводства();

	Для Каждого СчетПроизводственныеЗатраты Из СчетаПроизводственныеЗатраты Цикл
		Правило = Правила.Добавить();
		Правило.ОсновнойСчет           = СчетПроизводственныеЗатраты;
		Правило.ВидАктивовОбязательств = Перечисления.ВидыАктивовИОбязательств.НезавершенноеПроизводство;
		...
		Правило.ИсключенныеСубконто.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат);
		Правило.ИсключенныеСубконто.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиКалькуляции);
	КонецЦикла;
	
КонецПроцедуры

// Дополнить правила группировки видов активов/обязательств вспомогательных производства.
// 
// Параметры:
//  Правила - ТаблицаЗначений - Правила:
//   * ОсновнойСчет - ПланСчетовСсылка.Хозрасчетный - 
...
//   * ДополнительныеСчета - Массив из ПланСчетовСсылка.Хозрасчетный - 
//   * ИсключенныеСубконто - Массив из ПланВидовХарактеристикСсылка.ВидыСубконтоХозрасчетные - 
Процедура ДополнитьПравилаВспомогательныхПроизводств(Правила) Экспорт

	СчетаМатериалы = РеглУчетВыборкиСерверПовтИсп.СчетаМатериалыПодрядныеРаботыВспомогательныхПроизводствКИспользованию();
	...
	СчетаПроизводственныеЗатраты = РеглУчетВыборкиСерверПовтИсп.СчетаПроизводственныеЗатратыВспомогательныхПроизводств();
...
Процедура ЗаполнитьПравилаГруппировкиВидовАктивовИОбязательств(Правила) Экспорт
	...
	ИспользуетсяРеглУчетНЗППоПартиямПроизводства = ИспользуетсяРеглУчетНЗППоПартиямПроизводства();
	...
	Если ИспользуетсяРеглУчетНЗППоПартиямПроизводства Тогда		
		УстаревшиеСчетаОсновноеПроизводство = РеглУчетВыборкиСерверПовтИсп.УстаревшиеСчетаОсновноеПроизводство();
		
		Для Каждого Счет Из УстаревшиеСчетаОсновноеПроизводство Цикл
			Правило = Правила.Добавить();
		...
		ДополнитьПравилаОсновногоПроизводства(Правила);
		
		УстаревшиеСчетаВспомогательныеПроизводства = РеглУчетВыборкиСерверПовтИсп.УстаревшиеСчетаВспомогательныеПроизводства();
		
		Для Каждого Счет Из УстаревшиеСчетаВспомогательныеПроизводства Цикл
			Правило = Правила.Добавить();
			Правило.ОсновнойСчет           = Счет;
			Правило.ВидАктивовОбязательств = Перечисления.ВидыАктивовИОбязательств.НезавершенноеПроизводство;
		КонецЦикла;

		ДополнитьПравилаВспомогательныхПроизводств(Правила);
	Иначе
		Правило = Правила.Добавить();
	...
	Правило = Правила.Добавить();
	Правило.ОсновнойСчет           = ПланыСчетов.Хозрасчетный.ОбязательстваПоНеотфактурованнымПоставкам;
	Правило.ВидАктивовОбязательств = Перечисления.ВидыАктивовИОбязательств.НеотфактурованныеПоставки;
	
	Правило = Правила.Добавить();
	Правило.ОсновнойСчет           = ПланыСчетов.Хозрасчетный.НДСпоНеотфактурованнымПоставкам;
	Правило.ВидАктивовОбязательств = Перечисления.ВидыАктивовИОбязательств.ПраваНаВозмещениеНДС;
...
// * ПозицияСубконтоНалогообложенияРеализаций - Число -
//
Функция ПланГруппировкиАктивовОбязательств(ПравилаГруппировки, ВключитьСчетаДоходовРасходов = Ложь) Экспорт
	...
	ПланГруппировки.Колонки.Добавить("ПозицияСубконтоНалогообложенияРеализаций", ОбщегоНазначения.ОписаниеТипаЧисло(1));
	
	ПланГруппировки.Индексы.Добавить("Счет");// для каждого счета должна быть ровно одна запись
	
	ОписаниеСчетов = БалансовыеСчетаВременныхРазниц(ВключитьСчетаДоходовРасходов);
...
Функция ПараметрыДляПроведенияДокумента(Документ, Свойства) Экспорт
	...
	Если Свойства.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Параметры.ПодчиненныеРегистры.Добавить(Метаданные.РегистрыСведений.РасчетНалогаНаПрибыль);
...
Процедура ОтразитьДвижения(ТаблицыДляДвижений, Движения, Отказ) Экспорт
	...
	ПроведениеДокументов.ОтразитьДвижения(ТаблицыДляДвижений, Движения , Метаданные.РегистрыСведений.РасчетНалогаНаПрибыль.Имя);
...
Функция ПользователюДоступныСуммыРазниц() Экспорт
	...
	Возврат Не Запрос.Выполнить().Пустой() Или ПолучитьФункциональнуюОпцию("ВедетсяУчетПостоянныхИВременныхРазницОбщая");
...
Функция ТекстЗапроса_ПараметрыУчетаОтложенногоНалогаВДобавочномКапитале()
	...
	|ИЗ
	|	РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПоследних(&Период,
	|		ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НалогНаПрибыльОтОперацийПереоценки)) КАК ПорядокОтраженияНаСчетахУчета
	|ГДЕ
	...
	|ИЗ
	|	РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПоследних(&Период,
	|		ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.РезервДоОценкиВНА)) КАК ПорядокОтраженияНаСчетахУчета
	|ГДЕ
	|	ПорядокОтраженияНаСчетахУчета.АналитикаУчета В
	|		(ВЫБРАТЬ
	|			ВТ_ГФУ.АналитикаУчета
...
Функция СчетаДоходовРасходов(ВключитьСчетаДоходовРасходов = Ложь)
	
	// Временные разницы определяются на балансовых счетах, а не на счетах формирования прибыли.
	
	СчетаДоходовРасходов = Новый Массив;

	Если ВключитьСчетаДоходовРасходов Тогда
		СчетаДоходовРасходов.Добавить(ПланыСчетов.Хозрасчетный.ПрибыльУбытокОтПродаж);
		СчетаДоходовРасходов.Добавить(ПланыСчетов.Хозрасчетный.СальдоПрочихДоходовИРасходов);
	Иначе
...
Функция СчетаСПредопределеннымВариантомНалогообложения(ВключитьСчетаДоходовРасходов)
	...
	Если ВключитьСчетаДоходовРасходов Тогда
		СчетаСВариантамиНалогообложения.Добавить(ПланыСчетов.Хозрасчетный.Продажи);
		СчетаСВариантамиНалогообложения.Добавить(ПланыСчетов.Хозрасчетный.ПрочиеДоходыИРасходы);
	КонецЕсли;
...
Функция ГлобальныеИсключенияДетализацииАктивовОбязательств()
	...
	Исключения.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПартииНоменклатуры);
...
Функция ИспользуетсяРеглУчетНЗППоПартиямПроизводства()
	Возврат РеглУчетСервер.ИспользуетсяРеглУчетНЗППоПартиямПроизводства();	
КонецФункции
...
Процедура ЗаполнитьДанныеФинРезультатаИНалогов(ТаблицаРасчета, КонтекстРасчета)
	...
	СтруктураПараметров.Вставить("Организация",   КонтекстРасчета.Организация);
	СтруктураПараметров.Вставить("НачалоПериода", КонтекстРасчета.НачалоНалоговогоПериода);
	СтруктураПараметров.Вставить("КонецПериода",  КонтекстРасчета.КонецПериода);
...
Процедура СоздатьТаблицуПравилГруппировки(ПланГруппировки, МенеджерВременныхТаблиц)
	...
	|	ПланГруппировки.ПозицияСубконтоНалогообложенияРеализаций КАК ПозицияСубконтоНалогообложенияРеализаций
...
Функция ПланГруппировки(КонтекстРасчета, ВключитьСчетаДоходовРасходов = Ложь)
	...
	ПланГруппировки = ПланГруппировкиАктивовОбязательств(ПравилаГруппировки, ВключитьСчетаДоходовРасходов);
...
Процедура ЗаписатьОтражениеИнвестиционногоВычета(Проводки, КонтекстРасчета)
	...
	Пока Выборка.Следующий() Цикл
		...
		СодержаниеПризнание  = НСтр("ru = 'Признание отложенного налогового актива';
									|en = 'Deferred tax asset recognition'", ОбщегоНазначения.КодОсновногоЯзыка());
		ВидДвиженияПризнание = ВидДвиженияБухгалтерии.Дебет;
		СодержаниеПогашение  = НСтр("ru = 'Погашение отложенного налогового актива';
									|en = 'Repayment of deferred tax asset'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция БалансовыеСчетаВременныхРазниц(ВключитьСчетаДоходовРасходов = Ложь)
	...
	ВидыСубконтоСВариантомНО = Новый Массив;
	ВидыСубконтоСВариантомНО.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВариантыНалогообложенияПрибыли);     
	ВидыСубконтоСВариантомНО.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НалогообложенияРеализаций);
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("СчетаДоходовРасходов",       СчетаДоходовРасходов(ВключитьСчетаДоходовРасходов));
	Запрос.УстановитьПараметр("СчетаБезНалоговойСтоимости", СчетаБезНалоговойСтоимости());
	Запрос.УстановитьПараметр("ВключитьСчетаДоходовРасходов", ВключитьСчетаДоходовРасходов);
	Запрос.УстановитьПараметр("ВидыСубконтоСВариантомНО", ВидыСубконтоСВариантомНО);
	Запрос.УстановитьПараметр("СчетаСПредопределеннымВариантомНалогообложения",
		СчетаСПредопределеннымВариантомНалогообложения(ВключитьСчетаДоходовРасходов));
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА Счета.Ссылка В ИЕРАРХИИ (&СчетаСПредопределеннымВариантомНалогообложения)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ
	|			ЛОЖЬ
	...
	|			ИЛИ ВЫБОР
	|				КОГДА &ВключитьСчетаДоходовРасходов
	|					ТОГДА ВидыСубконто1.ВидСубконто В (&ВидыСубконтоСВариантомНО)
	|				ИНАЧЕ ЛОЖЬ
	|			КОНЕЦ)
	...
	|			ИЛИ ВЫБОР
	|				КОГДА &ВключитьСчетаДоходовРасходов
	|					ТОГДА ВидыСубконто2.ВидСубконто В (&ВидыСубконтоСВариантомНО)
	|				ИНАЧЕ ЛОЖЬ
	|			КОНЕЦ)
	...
	|			ИЛИ ВЫБОР
	|				КОГДА &ВключитьСчетаДоходовРасходов
	|					ТОГДА ВидыСубконто3.ВидСубконто В (&ВидыСубконтоСВариантомНО)
	|				ИНАЧЕ ЛОЖЬ
	|			КОНЕЦ)
...
Процедура ДобавитьЗаписьПланаВыполненияПравила(ПланГруппировки, Правило, ОписаниеСчетов, ОписаниеСчета, СубконтоСчетаГруппировки)
	...
	Для НомерСубконто = 1 По 3 Цикл
		...
		ИначеЕсли ОписаниеСчета.СчетСПредопределеннымВариантомНалогообложения
			И ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НалогообложенияРеализаций Тогда
			
			Запись.ПозицияСубконтоНалогообложенияРеализаций = НомерСубконто;
		КонецЕсли;
		...
		Если ЕстьДанныеИНВ Тогда
			...
			ПорядокРасчета = СтрШаблон(
				НСтр("ru = 'Оборот по дебету счета %1 в корреспонденции со счетом %2';
					|en = '%1 debit turnover in correspondence with account %2'", Расчет.КодЯзыка),
		...
		Если ЕстьДанныеИНВ Тогда
			ПорядокРасчета = СтрШаблон(
				НСтр("ru = 'Оборот по кредиту счета %1 в корреспонденции со счетом %2 (со знаком ""минус"") + (%3)';
					|en = 'Account %1 credit turnover in correspondence with account %2 (with negative sign) + (%3)'", Расчет.КодЯзыка),
				КодСчета(ПланыСчетов.Хозрасчетный.РасчетыСБюджетом),
				КодСчета(КоррСчетДляРасчетыСБюджетом),
...
Процедура ОбработатьИзменениеВариантаНалогообложенияПрибыли(ТаблицаРасчета, КонтекстРасчета)
	...
	ПланГруппировки = ПланГруппировки(КонтекстРасчета, Истина);
	СоздатьТаблицуПравилГруппировки(ПланГруппировки, МенеджерВременныхТаблиц);
...
Функция ТекстЗапроса_ДанныеПроводокПриИзмененииВариантаНалогообложенияПрибыли(КонтекстРасчета, 
	...
	Текст = "ВЫБРАТЬ
	        |	ВЫБОР
	        |		КОГДА ПравилаДт.ПозицияСубконтоВариантНалогообложения = 0 И ПравилаДт.ПозицияСубконтоНалогообложенияРеализаций = 0
	        |			ТОГДА ЕСТЬNULL(НастройкиНалогообложенияДт.ВариантНалогообложенияПрибыли, &ПолеВариантНалогообложенияПоУмолчанию)
	        |		КОГДА ПравилаДт.ПозицияСубконтоВариантНалогообложения = 1
	        |				И ДанныеПроводок.СубконтоДт1 <> НЕОПРЕДЕЛЕНО
	        |			ТОГДА ДанныеПроводок.СубконтоДт1
	        |		КОГДА ПравилаДт.ПозицияСубконтоВариантНалогообложения = 2
	        |				И ДанныеПроводок.СубконтоДт2 <> НЕОПРЕДЕЛЕНО
	        |			ТОГДА ДанныеПроводок.СубконтоДт2
	        |		КОГДА ПравилаДт.ПозицияСубконтоВариантНалогообложения = 3
	        |				И ДанныеПроводок.СубконтоДт3 <> НЕОПРЕДЕЛЕНО
	        |			ТОГДА ДанныеПроводок.СубконтоДт3
	        |		КОГДА ПравилаДт.ПозицияСубконтоНалогообложенияРеализаций = 1
	        |				И ДанныеПроводок.СубконтоДт1 <> НЕОПРЕДЕЛЕНО
	        |			ТОГДА ВЫРАЗИТЬ(ДанныеПроводок.СубконтоДт1 КАК Справочник.ВариантыНалогообложенияРеализации).ВариантНалогообложенияПрибыли
	        |		КОГДА ПравилаДт.ПозицияСубконтоНалогообложенияРеализаций = 2
	        |				И ДанныеПроводок.СубконтоДт2 <> НЕОПРЕДЕЛЕНО
	        |			ТОГДА ВЫРАЗИТЬ(ДанныеПроводок.СубконтоДт2 КАК Справочник.ВариантыНалогообложенияРеализации).ВариантНалогообложенияПрибыли
	        |		КОГДА ПравилаДт.ПозицияСубконтоНалогообложенияРеализаций = 3
	        |				И ДанныеПроводок.СубконтоДт3 <> НЕОПРЕДЕЛЕНО
	        |			ТОГДА ВЫРАЗИТЬ(ДанныеПроводок.СубконтоДт3 КАК Справочник.ВариантыНалогообложенияРеализации).ВариантНалогообложенияПрибыли
	        |		ИНАЧЕ &ПолеВариантНалогообложенияПоУмолчанию
	        |	КОНЕЦ КАК ВариантНалогообложенияПрибылиДт,
	        |	ВЫБОР
	        |		КОГДА ПравилаКт.ПозицияСубконтоВариантНалогообложения = 0 И ПравилаКт.ПозицияСубконтоНалогообложенияРеализаций = 0
	        |			ТОГДА ЕСТЬNULL(НастройкиНалогообложенияКт.ВариантНалогообложенияПрибыли, &ПолеВариантНалогообложенияПоУмолчанию)
	        |		КОГДА ПравилаКт.ПозицияСубконтоВариантНалогообложения = 1
	        |				И ДанныеПроводок.СубконтоКт1 <> НЕОПРЕДЕЛЕНО
	        |			ТОГДА ДанныеПроводок.СубконтоКт1
	        |		КОГДА ПравилаКт.ПозицияСубконтоВариантНалогообложения = 2
	        |				И ДанныеПроводок.СубконтоКт2 <> НЕОПРЕДЕЛЕНО
	        |			ТОГДА ДанныеПроводок.СубконтоКт2
	        |		КОГДА ПравилаКт.ПозицияСубконтоВариантНалогообложения = 3
	        |				И ДанныеПроводок.СубконтоКт3 <> НЕОПРЕДЕЛЕНО
	        |			ТОГДА ДанныеПроводок.СубконтоКт3
	        |		КОГДА ПравилаКт.ПозицияСубконтоНалогообложенияРеализаций = 1
	        |				И ДанныеПроводок.СубконтоКт1 <> НЕОПРЕДЕЛЕНО
	        |			ТОГДА ВЫРАЗИТЬ(ДанныеПроводок.СубконтоКт1 КАК Справочник.ВариантыНалогообложенияРеализации).ВариантНалогообложенияПрибыли
	        |		КОГДА ПравилаКт.ПозицияСубконтоНалогообложенияРеализаций = 2
	        |				И ДанныеПроводок.СубконтоКт2 <> НЕОПРЕДЕЛЕНО
	        |			ТОГДА ВЫРАЗИТЬ(ДанныеПроводок.СубконтоКт2 КАК Справочник.ВариантыНалогообложенияРеализации).ВариантНалогообложенияПрибыли
	        |		КОГДА ПравилаКт.ПозицияСубконтоНалогообложенияРеализаций = 3
	        |				И ДанныеПроводок.СубконтоКт3 <> НЕОПРЕДЕЛЕНО
	        |			ТОГДА ВЫРАЗИТЬ(ДанныеПроводок.СубконтоКт3 КАК Справочник.ВариантыНалогообложенияРеализации).ВариантНалогообложенияПрибыли
	        |		ИНАЧЕ &ПолеВариантНалогообложенияПоУмолчанию
	        |	КОНЕЦ КАК ВариантНалогообложенияПрибылиКт,
	        |	ПравилаДт.СчетГруппировки КАК ПравилаСчетДт,
	        |	ПравилаКт.СчетГруппировки КАК ПравилаСчетКт,
	        |	ДанныеПроводок.СчетДт КАК СчетДт,
	        |	ДанныеПроводок.СчетКт КАК СчетКт,
	        ...
	        |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПравилаГруппировки КАК ПравилаДт
	        |		ПО ДанныеПроводок.СчетДт = ПравилаДт.Счет
	        |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПравилаГруппировки КАК ПравилаКт
	        |		ПО ДанныеПроводок.СчетКт = ПравилаКт.Счет
	        ...
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        |	ВТ_ДанныеПроводокПоВариантамНалогообложения.ВариантНалогообложенияПрибылиДт КАК ВариантНалогообложенияПрибылиДт,
	        |	ВТ_ДанныеПроводокПоВариантамНалогообложения.ВариантНалогообложенияПрибылиКт КАК ВариантНалогообложенияПрибылиКт,
	        ...
	        |	СУММА(ВТ_ДанныеПроводокПоВариантамНалогообложения.СуммаВР) КАК СуммаВР
	        |ИЗ
	        ...
	        |	ВТ_ДанныеПроводокПоВариантамНалогообложения.ВариантНалогообложенияПрибылиДт,
	        |	ВТ_ДанныеПроводокПоВариантамНалогообложения.ВариантНалогообложенияПрибылиКт";
