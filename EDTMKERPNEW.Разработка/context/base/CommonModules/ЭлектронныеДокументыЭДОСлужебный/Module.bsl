	...
	Попытка
		...
		Если ОписаниеСообщения.Данные.ЕстьОшибки Тогда
			...
			ТекстОшибки = НСтр("ru = 'Не удалось сформировать дополнительную информацию';
								|en = 'Не удалось сформировать дополнительную информацию'");
			ДобавитьОшибкуВыполненияДействия(ТекстОшибки, КонтекстВыполненияДействия, КонтекстДиагностики);
...
Процедура ДобавитьОшибкуДокументВУстаревшемФормате(КонтекстДиагностики, ДанныеСообщения, 
	...
	ШаблонСообщения = Нстр("ru = 'Документ ""%1"" сформирован в формате ""%2"", срок действия которого истек %3';
							|en = 'Документ ""%1"" сформирован в формате ""%2"", срок действия которого истек %3'")
		+ Символы.ПС + НСтр("ru = 'Для подписания и отправки необходимо переформировать электронный документ.';
							|en = 'Для подписания и отправки необходимо переформировать электронный документ.'");
...
Функция ВидОшибкиДокументыВУстаревшемФормате(ИспользоватьОбработчик = Ложь)
	...
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Документы сформированы в устаревшем формате';
										|en = 'Документы сформированы в устаревшем формате'");
	ВидОшибки.ОписаниеПроблемы = НСтр("ru = 'Формат документов больше не действует';
										|en = 'Формат документов больше не действует'");
	Если ИспользоватьОбработчик Тогда
		ВидОшибки.ВыполнятьОбработчикАвтоматически = Истина;
		ВидОшибки.АвтоматическиВыполняемыйОбработчик =
			"ЭлектронныеДокументыЭДОКлиент.ВывестиПредупреждениеОДокументеВНеПоддерживаемомФормате";
...
Процедура ЗагрузитьДанныеОбъектовПакета(ДанныеОбъектовПакета, ПроверенныеПодписи, КонтекстДиагностики, РезультатЗагрузки)
	...
	Исключение
		...
		Если ЗначениеЗаполнено(ДанныеОбъекта) Тогда
			ДанныеОбъекта.Отказ = Истина;
			ТекстОшибки = НСтр("ru = 'Не удалось загрузить электронный документ по причине:';
								|en = 'Cannot load the electronic document as:'") + Символы.ПС
...
Процедура ЗапуститьПолнуюПроверкуПодписейПоМЧД()
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось запустить полную проверку подписей по МЧД по причине:';
								|en = 'Не удалось запустить полную проверку подписей по МЧД по причине:'")
			+ Символы.ПС + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура ПроверитьПодписиФайловПоМЧДПослеЗагрузкиДанных(СведенияОПодписяхПоМЧД, КонтекстДиагностики)
	...
	Для Каждого СведенияОПодписиПоМЧД Из СведенияОПодписяхПоМЧД Цикл
		...
		Исключение     
			ВидОперации = НСтр("ru = 'Проверка подписей по МЧД';
								|en = 'Check machine-readable authorization letter signatures'");
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось проверить подпись документа по МЧД. %1';
										|en = 'Не удалось проверить подпись документа по МЧД. %1'"), ФайлСообщения);			
			ПодробныйТекстОшибки = ТекстОшибки + Символы.ПС + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			Ошибка = ОбработкаНеисправностейБЭД.НоваяОшибка(ВидОперации,
				ОбработкаНеисправностейБЭДКлиентСервер.ВидОшибкиНеизвестнаяОшибка(),
				ПодробныйТекстОшибки, ТекстОшибки);
