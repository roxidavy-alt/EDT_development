...
Процедура ПересчитатьСтатусУказанияСерийПриОбработке(ПараметрыУказанияСерий, ТекущийСтатус, СерииУказаныПолностью, КоличествоСерий, ВариантОбеспечения = Неопределено, ТекущиеДанные = Неопределено) Экспорт
	...
	ЭтоМатериалыМЛИлиПроизвОперацииИлиРемонт = 
		ПараметрыУказанияСерий.СкладскиеОперации.Найти(ПредопределенноеЗначение("Перечисление.СкладскиеОперации.РемонтПродукции")) <> Неопределено
		ИЛИ ПараметрыУказанияСерий.СкладскиеОперации.Найти(ПредопределенноеЗначение("Перечисление.СкладскиеОперации.ПотреблениеМатериаловПриПроизводстве")) <> Неопределено
	...
	ПрименятьСтатусМожноУказать = (ПараметрыУказанияСерий.ПодготовкаОрдера
									...
									Или ЭтоМатериалыМЛИлиПроизвОперацииИлиРемонт
										И (ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ТекущиеДанные, "Отменено", Ложь)
											ИЛИ НЕ (ПараметрыУказанияСерий.ФактОтбора
													ИЛИ ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ТекущиеДанные, "Статус", Неопределено)
														= ПредопределенноеЗначение("Перечисление.СтатусыЭтаповПроизводства2_2.Начат")
													ИЛИ ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ТекущиеДанные, "Статус", Неопределено)
