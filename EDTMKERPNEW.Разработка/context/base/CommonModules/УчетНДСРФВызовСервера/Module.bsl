...
Функция ПолучитьДОПППоОснованию(Ссылка) Экспорт
	...
	МассивДОПП = РегистрыСведений.ДокументыОПредстоящейПоставке.НайтиДОПППоОснованию(Ссылка).МассивДОПП;
...
// Создание регламентированного отчета ДОПП
// 
// Параметры:
//  Параметры - Структура:
...
// 
// Возвращаемое значение:
//  Неопределено, ДокументСсылка.УведомлениеОСпецрежимахНалогообложения -Созданный документ
Функция СоздатьРеглОтчетДОПП(Параметры) Экспорт
	
	ИмяФормы = РегистрыСведений.ДокументыОПредстоящейПоставке.ИмяФормыДОПП(Параметры.Дата);
	Если ЗначениеЗаполнено(ИмяФормы) Тогда
		КонтейнерДОПП = Отчеты.РегламентированноеУведомлениеДокументОПредстоящейПоставке.КонтейнерДляЗаполнения(ИмяФормы);
		ПараметрыСоздания = ДанныеДляЗаполненияРеглОтчетаДОПП(КонтейнерДОПП, Параметры, ИмяФормы);
		СсылкаНаДОПП = Отчеты.РегламентированноеУведомлениеДокументОПредстоящейПоставке.НовыйДОПП(ПараметрыСоздания);
		Возврат СсылкаНаДОПП;
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ОчиститьДокументПриобретенияВСчетеФактуреПолученномНалоговогоАгента(СчетФактура) Экспорт
	...
	Попытка
		...
		КорректировкиНалоговойБазы = РегистрыБухгалтерии.КорректировкиНалоговойБазы.СоздатьНаборЗаписей();
		КорректировкиНалоговойБазы.Отбор.Регистратор.Установить(СчетФактура);
		Движения.Вставить("КорректировкиНалоговойБазы", КорректировкиНалоговойБазы);
		//-- НЕ УТ
...
//	УникальныйИдентификаторЗаявлениеОВвозе - УникальныйИдентификатор- Новый уникальный идентификатор.
//	УникальныйИдентификаторУчетПеремещенияТоваров - УникальныйИдентификатор- Новый уникальный идентификатор.
//
// Возвращаемое значение:
//	- Структура - структура с данными файлов выгрузки, содержит:
//		* ЗаявлениеОВвозеТоваров - Массив Из Структура - структуры с данными файлов выгрузки, содержит:
...
//		* УчетПеремещенияТоваров - Массив Из Структура - структуры с данными файлов выгрузки, содержит:
//									* АдресФайлаВыгрузки - Строка - адрес двоичных данных файла выгрузки во временном хранилище.
//									* ИмяФайлаВыгрузки - Строка - короткое имя файла выгрузки (с расширением).
//
Функция ПолучитьВыгружаемыеДанныеЗаявлениеОВвозеТоваров(Ссылка, УникальныйИдентификаторЗаявлениеОВвозе, УникальныйИдентификаторУчетПеремещенияТоваров) Экспорт
...
Функция ДанныеДляЗаполненияРеглОтчетаДОПП(КонтейнерДОПП, Параметры, ИмяФормы)
	
	МассивОснований = Новый Массив();
	МассивОснований.Добавить(Параметры.Основание);
	Параметры.Вставить("МассивДокументов", МассивОснований);
	Параметры.Вставить("ДатаПодписи", ТекущаяДатаСеанса());
	РегламентированнаяОтчетностьУП.ЗаполнитьУведомлениеОПредстоящейПоставке(Параметры, КонтейнерДОПП);
	
	СсылкаНаДОПП = Документы.УведомлениеОСпецрежимахНалогообложения.ПустаяСсылка();
	
	ПараметрыСоздания = Новый Структура();
	ПараметрыСоздания.Вставить("Организация",       Параметры.Организация);
	ПараметрыСоздания.Вставить("РегистрацияВИФНС",  Справочники.Организации.РегистрацияВНалоговомОргане(Параметры.Организация, ТекущаяДатаСеанса()));
	ПараметрыСоздания.Вставить("Контейнер",         КонтейнерДОПП);
	ПараметрыСоздания.Вставить("ДокументОснование", Параметры.Основание);
	ПараметрыСоздания.Вставить("ИмяФормы",          ИмяФормы);
