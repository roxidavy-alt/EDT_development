...
Функция СпецификацииИзделия(ДанныеОбИзделии, ПараметрыВыбораСпецификаций) Экспорт
	...
	|	&РемонтируемоеИзделиеНоменклатура                                   КАК РемонтируемоеИзделиеНоменклатура,
	|	&РемонтируемоеИзделиеХарактеристика                                 КАК РемонтируемоеИзделиеХарактеристика
	...
	Запрос.УстановитьПараметр("РемонтируемоеИзделиеНоменклатура",   ДанныеОбИзделии.РемонтируемоеИзделиеНоменклатура);
	Запрос.УстановитьПараметр("РемонтируемоеИзделиеХарактеристика", ДанныеОбИзделии.РемонтируемоеИзделиеХарактеристика);
...
Функция СпецификацииИзделий(ДанныеОбИзделиях, ПараметрыВыбораСпецификаций) Экспорт
	...
	|	Т.РемонтируемоеИзделиеНоменклатура        КАК РемонтируемоеИзделиеНоменклатура,
	|	Т.РемонтируемоеИзделиеХарактеристика      КАК РемонтируемоеИзделиеХарактеристика
...
Функция ПараметрыТекстаЗапросаСпецификацийИзделий() Экспорт
	...
	// Дополнить выходную таблицу (она уже существует)
	Результат.Вставить("ДополнитьВыходнуюТаблицу", Ложь);
...
Функция ТекстЗапросаСпецификацийИзделий(ПараметрыЗапроса, ПараметрыВыбораСпецификаций) Экспорт
	...
	Иначе
		...
		|
		|ВЫБРАТЬ
		|	ВтСписокНоменклатуры.ИндексДанных            КАК ИндексДанных,
		|	ВтСписокНоменклатуры.Номенклатура            КАК Номенклатура,
		|	ВтСписокНоменклатуры.Характеристика          КАК Характеристика,
		|	ВтСписокНоменклатуры.ПодразделениеДиспетчер  КАК ПодразделениеДиспетчер,
		|	ВтСписокНоменклатуры.НаправлениеДеятельности КАК НаправлениеДеятельности,
		...
		|
		|	СпецификацииИзделий.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыСпецификаций.Действует)
		|		И НЕ СпецификацииИзделий.Спецификация.ИсключитьАвтоматическийВыбор
		|		И &ТекстУсловиеАвтовыбораРемонтируемоеИзделие
		...
		|
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтСписокНоменклатуры КАК ВтСписокНоменклатуры
		|		 ПО СпецификацииИзделий.ВидНоменклатуры = ВтСписокНоменклатуры.ВидНоменклатуры
		|			И (СпецификацииИзделий.Номенклатура = ВтСписокНоменклатуры.Номенклатура
		|				ИЛИ СпецификацииИзделий.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
		|			И (СпецификацииИзделий.Характеристика = ВтСписокНоменклатуры.Характеристика
		|				ИЛИ СпецификацииИзделий.Характеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка))
		|			И &ТекстУсловиеОтбораРемонтируемоеИзделие
		|			И &ТекстУсловиеОтбораВходящихДанных
		|
		|ГДЕ
		|
		|	(ВтСписокНоменклатуры.НачалоПроизводства = ДАТАВРЕМЯ(1, 1, 1) 
		|		ИЛИ СпецификацииИзделий.НачалоДействия <= ВтСписокНоменклатуры.НачалоПроизводства)
		|	И (СпецификацииИзделий.КонецДействия = ДАТАВРЕМЯ(1, 1, 1)
		|		ИЛИ СпецификацииИзделий.КонецДействия >= ВтСписокНоменклатуры.НачалоПроизводства)
		|
		...
		ТекстПоляНоменклатура                    = "ВтСписокНоменклатуры.Номенклатура";
	...
	ТекстУсловия           = "ИСТИНА";
	ТекстУсловияАвтовыбора = "ИСТИНА";
	//++ НЕ УТКА
	Если (ПараметрыВыбораСпецификаций.ДоступныеТипы.Количество() = 0
			ИЛИ ПараметрыВыбораСпецификаций.ДоступныеТипы.Найти(Перечисления.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот) <> Неопределено) Тогда
		
		ТекстУсловия = "
		|ВЫБОР
		|	КОГДА ВтСписокНоменклатуры.РемонтируемоеИзделиеНоменклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|		ТОГДА ИСТИНА
		|	ИНАЧЕ
		|		(ВтСписокНоменклатуры.РемонтируемоеИзделиеНоменклатура.ВидНоменклатуры = СпецификацииИзделий.Спецификация.РемонтируемоеИзделиеВидНоменклатуры
		|			ИЛИ СпецификацииИзделий.Спецификация.РемонтируемоеИзделиеВидНоменклатуры = ЗНАЧЕНИЕ(Справочник.ВидыНоменклатуры.ПустаяСсылка))
		|		И (ВтСписокНоменклатуры.РемонтируемоеИзделиеНоменклатура = СпецификацииИзделий.Спецификация.РемонтируемоеИзделиеНоменклатура
		|			ИЛИ СпецификацииИзделий.Спецификация.РемонтируемоеИзделиеНоменклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
		|		И (СпецификацииИзделий.Спецификация.РемонтируемоеИзделиеХарактеристика = ВтСписокНоменклатуры.РемонтируемоеИзделиеХарактеристика
		|			ИЛИ СпецификацииИзделий.Спецификация.РемонтируемоеИзделиеХарактеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка))
		|КОНЕЦ
		|";
		
		ТекстУсловияАвтовыбора = "
		|ВЫБОР
		|	КОГДА ВтСписокНоменклатуры.РемонтируемоеИзделиеНоменклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|		ТОГДА СпецификацииИзделий.Спецификация.РемонтируемоеИзделиеВидНоменклатуры = ЗНАЧЕНИЕ(Справочник.ВидыНоменклатуры.ПустаяСсылка)
		|	ИНАЧЕ
		|		ИСТИНА
		|КОНЕЦ
		|";
	...
	КонецЕсли;
	//-- НЕ УТКА
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстУсловиеОтбораРемонтируемоеИзделие",     ТекстУсловия);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстУсловиеАвтовыбораРемонтируемоеИзделие", ТекстУсловияАвтовыбора);
	
	ТекстУсловия = "ИСТИНА";
	Если ЗначениеЗаполнено(ПараметрыЗапроса.УсловияОтбораВходящихДанных) Тогда
		ТекстУсловия = СтрСоединить(ПараметрыЗапроса.УсловияОтбораВходящихДанных, " И ");
	КонецЕсли;
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстУсловиеОтбораВходящихДанных", ТекстУсловия);
	...
	Если НЕ ПустаяСтрока(ПараметрыЗапроса.ИмяВыходнойТаблицы) Тогда
		...
		Если ПараметрыЗапроса.ДополнитьВыходнуюТаблицу Тогда
			
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПОМЕСТИТЬ ИмяВыходнойТаблицы", "ДОБАВИТЬ "+ПараметрыЗапроса.ИмяВыходнойТаблицы);
...
//
// Параметры:
//  Элемент						 - ПолеФормы, Массив из ПолеФормы - поле ввода спецификации или список полей
...
Процедура УстановитьПараметрыВыбораСпецификаций(Элемент, ПараметрыВыбораСпецификаций) Экспорт
	...
	ПараметрыВыбора = УправлениеДаннымиОбИзделияхКлиентСервер.ПараметрыВыбораЭлементаСпецификации(ПараметрыВыбораСпецификаций);
	...
	Для каждого ТекущийЭлемент Из МассивЭлементов Цикл // ПолеФормы
		
		ТекущийЭлемент.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрыВыбора);
		ТекущийЭлемент.ФормаВыбора     = "Справочник.РесурсныеСпецификации.Форма.ФормаВыбораПоНоменклатуре";
...
// Доступны параметры производственного процесса.
// 
// Параметры:
...
// Возвращаемое значение:
//  Булево
Функция ДоступныПараметрыПроизводственногоПроцесса(Объект) Экспорт
	
	Результат = Ложь;
	//++ НЕ УТКА
	Результат = НЕ (ЭтоТехнологическийНабор(Объект)
		ИЛИ Объект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот
...
//
// Параметры:
//  ТекущаяСтрока		 - СтрокаДереваЗначений - данные текущей строки
	...
	Если СтруктураДерева <> Неопределено Тогда
		...
		Если СтрокаДерева <> Неопределено Тогда
			...
			Если Не СтрокаДерева.СпецификацияПрочитана Тогда
				...
				ТекущаяСтрока.ТипПроизводственногоПроцесса = СтрокаДерева.ТипПроизводственногоПроцесса;
...
Процедура ИзменитьСпецификациюВСтрокеДерева(Форма, Параметры, ТекущиеДанные) Экспорт
	...
	Если Параметры.ДинамическоеСчитывание Тогда
		...
		КоллекцияЭлементов = ДеревоСпецификацийПрочитатьЭлементы(ТекущиеДанные, АдресВХранилище, ОбновитьДанныеСтроки);
...
Процедура ЗаполнитьСтрокиДерева(Параметры, Дерево, СтрокаДерева, ИспользованныеСпецификации = Неопределено, КлючСвязи = Неопределено) Экспорт
	...
	Если Не ЭтоАдресВременногоХранилища(Параметры.АдресВХранилище) Тогда
		...
		Если СтрокаДерева = Неопределено ИЛИ СтрокаДерева.СтруктураДанных = Неопределено Тогда
			...
			ДобавитьДанныеОбИзделииДерева(ДанныеПоНоменклатуре, СтрокаДерева);
		...
		Справочники.РесурсныеСпецификации.ОтметитьПромежуточныйВыпуск(СтруктураДанных, Истина);
	...
	Если Не ВыводитьЭтапы Тогда
		...
		МатериалыИУслуги = ОбщегоНазначенияУТ.СвернутаяКопияТаблицы(
			...
			|РемонтируемоеИзделиеНоменклатура,
			|РемонтируемоеИзделиеХарактеристика,
			|КлючСвязи",
	...
	Если МатериалыИУслуги.Количество() > 0 Тогда
		...
		Для каждого Строка Из МатериалыИУслуги Цикл
			...
			Если СтруктураДанных.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот
				И НЕ Строка.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Работа Тогда
				Продолжить;
			КонецЕсли;
			
			ДанныеОбИзделиях = УправлениеДаннымиОбИзделияхКлиентСервер.СтруктураДанныхОбИзделииДляВыбораСпецификации();
			ЗаполнитьЗначенияСвойств(ДанныеОбИзделиях, Строка);
		...
		Если СтруктураДанных.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот Тогда
			ПараметрыВыбораСпецификаций.ДоступныеТипы.Очистить();
			ПараметрыВыбораСпецификаций.ДоступныеТипы.Добавить(СтруктураДанных.ТипПроизводственногоПроцесса);
		КонецЕсли;
	...
	Если ВыводитьЭтапы Тогда
		НаправлениеУбыв = (СтруктураДанных.ТипПроизводственногоПроцесса <> Перечисления.ТипыПроизводственныхПроцессов.Разборка
							И СтруктураДанных.ТипПроизводственногоПроцесса <> Перечисления.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот);
	...
	Пока УсловиеЦикла Цикл
		...
		Если ВыводитьЭтапы Тогда
			...
			СтрокаРодитель = ДобавитьСтрокуДерева(
				?(СтрокаДерева = Неопределено, Дерево, СтрокаДерева), Перечисления.ВидыСтрокСпецификации.Этап, ДанныеЭтапа);
		...
		Если (Параметры.ВыводитьВыходныеИзделия ИЛИ КоличествоВыходныхИзделий > 0)
			...
			Если КоличествоВыходныхИзделий > 0 И ВыводитьЗаголовкиГруппировок Тогда
				СтрокаРодитель = ДобавитьСтрокуДерева(СтрокаЭтап, Перечисления.ВидыСтрокСпецификации.ВыходныеИзделия);
				СтрокаРодитель.Номенклатура = "" + Перечисления.ВидыСтрокСпецификации.ВыходныеИзделия +
			...
			Для каждого Строка Из МассивСтрок Цикл
				...
				НоваяСтрока = ДобавитьСтрокуДерева(СтрокаРодитель, Строка.ВидСтроки, Строка);
				...
				НоваяСтрока.ИндексКартинки = ПолучитьИндексКартинки(НоваяСтрока, Строка);
		...
		Если (Параметры.ВыводитьМатериалы ИЛИ КоличествоМатериалов > 0)
			...
			// Вывод строки "Материалы и работы".
			Если КоличествоМатериалов > 0 И ВыводитьЗаголовкиГруппировок Тогда
				СтрокаРодитель = ДобавитьСтрокуДерева(СтрокаЭтап, Перечисления.ВидыСтрокСпецификации.МатериалыИУслуги);
				СтрокаРодитель.Номенклатура = "" + Перечисления.ВидыСтрокСпецификации.МатериалыИУслуги +
		...
		Если Параметры.ВыводитьТрудозатраты И ЕстьСпецификация И КоличествоТрудозатрат > 0 Тогда
			...
			Если КоличествоТрудозатрат > 0 И ВыводитьЗаголовкиГруппировок Тогда
				СтрокаРодитель = ДобавитьСтрокуДерева(СтрокаЭтап, Перечисления.ВидыСтрокСпецификации.Трудозатраты);
				СтрокаРодитель.Номенклатура = "" + Перечисления.ВидыСтрокСпецификации.Трудозатраты + " (" + КоличествоТрудозатрат + ")";
			КонецЕсли;
			
			Для Каждого Строка Из МассивСтрок Цикл
				НоваяСтрока = ДобавитьСтрокуДерева(СтрокаРодитель, Перечисления.ВидыСтрокСпецификации.Трудозатрата, Строка);
		...
		Если ВыводитьОперации И КоличествоОпераций > 0 Тогда
			...
			Если ВыводитьЗаголовкиГруппировок Тогда
				СтрокаРодитель = ДобавитьСтрокуДерева(СтрокаЭтап, Перечисления.ВидыСтрокСпецификации.Операции);
				СтрокаРодитель.Номенклатура = "" + Перечисления.ВидыСтрокСпецификации.Операции + " (" + КоличествоОпераций + ")";
			КонецЕсли;
			
			Для Каждого Строка Из МассивСтрок Цикл
				НоваяСтрока = ДобавитьСтрокуДерева(СтрокаРодитель, Перечисления.ВидыСтрокСпецификации.Операция, Строка);
	...
	Если СтрокаДерева <> Неопределено И СтрокаДерева.ЕстьСпецификация Тогда
		СтрокаДерева.СпецификацияПрочитана = Истина;
		СтрокаДерева.СтруктураДанных = Неопределено;
		СтрокаДерева.ТипПроизводственногоПроцесса = СтруктураДанных.ТипПроизводственногоПроцесса;
	КонецЕсли;
...
Процедура НастроитьУсловноеОформлениеДереваСпецификаций(УсловноеОформление) Экспорт
	...
	ЭлементОтбора.ПравоеЗначение = Перечисления.ВидыСтрокСпецификации.Этап;
	...
	ВидыСтрок.Добавить(Перечисления.ВидыСтрокСпецификации.Этап);
	ВидыСтрок.Добавить(Перечисления.ВидыСтрокСпецификации.Трудозатраты);
	ВидыСтрок.Добавить(Перечисления.ВидыСтрокСпецификации.Трудозатрата);
	ВидыСтрок.Добавить(Перечисления.ВидыСтрокСпецификации.МатериалыИУслуги);
	ВидыСтрок.Добавить(Перечисления.ВидыСтрокСпецификации.ВыходныеИзделия);
	ВидыСтрок.Добавить(Перечисления.ВидыСтрокСпецификации.Операции);
	ВидыСтрок.Добавить(Перечисления.ВидыСтрокСпецификации.Операция);
	...
	ЭлементОтбора.ПравоеЗначение = Перечисления.ВидыСтрокСпецификации.Номенклатура;
	...
	ВидыСтрок = Новый СписокЗначений;
	ВидыСтрок.Добавить(Перечисления.ВидыСтрокСпецификации.Номенклатура);
	ВидыСтрок.Добавить(Перечисления.ВидыСтрокСпецификации.ВыходноеИзделие);
	ВидыСтрок.Добавить(Перечисления.ВидыСтрокСпецификации.ВходящееИзделие);
	ВидыСтрок.Добавить(Перечисления.ВидыСтрокСпецификации.ПобочныйВыход);
	ВидыСтрок.Добавить(Перечисления.ВидыСтрокСпецификации.Материал);
	ВидыСтрок.Добавить(Перечисления.ВидыСтрокСпецификации.ПромежуточныйПолуфабрикат);
	ВидыСтрок.Добавить(Перечисления.ВидыСтрокСпецификации.ВыполняемаяРабота);
	...
	// Цветовое оформление уровней
	ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
	
	ОформляемоеПоле = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
	ОформляемоеПоле.Поле = Новый ПолеКомпоновкиДанных("ДеревоСпецификаций");

	ЭлементОтбора = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ДеревоСпецификаций.УровеньОформления");
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	...
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ЦветФона", ЦветаСтиля.ЦветФонаГруппировкиОтчета2);
...
Функция ТаблицаДанныхОбИзделииДляВыбораСпецификации() Экспорт
	...
	Результат.Колонки.Добавить("РемонтируемоеИзделиеНоменклатура",   Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	Результат.Колонки.Добавить("РемонтируемоеИзделиеХарактеристика", Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
...
// Возвращаемое значение:
//  Булево
Функция ЭтоТехнологическийНабор(Объект) Экспорт
	
	Результат = Ложь;
	
	//++ НЕ УТКА
	Если НЕ Объект.ДополнительныеСвойства.Свойство("ЭтоТехнологическийНабор") Тогда
		
		ТипНоменклатуры = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ОсновноеИзделиеВидНоменклатуры, "ТипНоменклатуры");
		
		Объект.ДополнительныеСвойства.Вставить("ЭтоТехнологическийНабор", ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Набор);
		
	КонецЕсли;
	Результат = Объект.ДополнительныеСвойства.ЭтоТехнологическийНабор; // Булево
...
Функция СоставНабораВПроизводствеКонструктор()
	...
	Таблица.Колонки.Добавить("ЕдиницаИзмеренияШтучная",    Новый ОписаниеТипов("Булево"));
	...
	Таблица.Колонки.Добавить("ДанныеУпаковки");
...
Функция ПараметрыПолученияСоставаНабораВПроизводстве() Экспорт
	...
	Параметры.Вставить("ИсточникиСостава",         Новый Массив);
	
	Параметры.Вставить("Изделие",                  Справочники.Номенклатура.ПустаяСсылка());
	Параметры.Вставить("ХарактеристикаИзделия",    Справочники.ХарактеристикиНоменклатуры.ПустаяСсылка());
	Параметры.Вставить("СпецификацияИзделия",      Справочники.РесурсныеСпецификации.ПустаяСсылка());
	Параметры.Вставить("ЭтапСпецификацииИзделия",  Справочники.ЭтапыПроизводства.ПустаяСсылка());
...
Функция СоставНабораВПроизводстве(Параметры) Экспорт
	...
	Если УправлениеДаннымиОбИзделияхПовтИсп.ДоступноХранениеСоставаНабораВСпецификациях()
		И (НЕ ЗначениеЗаполнено(Параметры.ИсточникиСостава)
			ИЛИ Параметры.ИсточникиСостава.Найти(Перечисления.ИсточникиСоставаНаборовВПроизводстве.Спецификации) <> Неопределено) Тогда
		...
		Если ЗначениеЗаполнено(СпецификацияСостава) Тогда
			...
			ДанныеПоНоменклатуре.Вставить("Спецификация",             СпецификацияСостава);
			ДанныеПоНоменклатуре.Вставить("ПодразделениеИсполнитель", Параметры.ПодразделениеИсполнитель);
			ДанныеПоНоменклатуре.Вставить("Изделие",                  Параметры.Изделие);
			ДанныеПоНоменклатуре.Вставить("ХарактеристикаИзделия",    Параметры.ХарактеристикаИзделия);
			ДанныеПоНоменклатуре.Вставить("СпецификацияИзделия",      Параметры.СпецификацияИзделия);
			ДанныеПоНоменклатуре.Вставить("ЭтапСпецификацииИзделия",  Параметры.ЭтапСпецификацииИзделия);
	...
	Если УправлениеДаннымиОбИзделияхПовтИсп.ДоступноХранениеСоставаНабораВВариантахКомплектации()
		И Состав = Неопределено
		И (НЕ ЗначениеЗаполнено(Параметры.ИсточникиСостава)
			ИЛИ Параметры.ИсточникиСостава.Найти(Перечисления.ИсточникиСоставаНаборовВПроизводстве.ВариантыКомплектации) <> Неопределено) Тогда 
		...
		Если ЗначениеЗаполнено(ДанныеВариантаКомплектации) Тогда
			...
			ВыполнитьДирективныеЗаменыВНабореДанных(Состав, Параметры);
...
Процедура ДобавитьСоставНаборовВРезультатПодбораВФоне(Параметры, АдресХранилища) Экспорт
	...
	ДобавитьСоставНаборовВТаблицу(Параметры.ТаблицаТоваров, Параметры.ПараметрыПолученияСостава);
	
	Результат = Новый Структура;
	Результат.Вставить("ВариантПодбора", Параметры.ВариантПодбора);
	Результат.Вставить("ТаблицаТоваров", Параметры.ТаблицаТоваров);
...
Процедура ДобавитьСоставНаборовВТаблицу(Таблица, ПараметрыПолученияСостава) Экспорт
	
	Если Таблица.Колонки.Найти("ДанныеСостава") = Неопределено Тогда
		Таблица.Колонки.Добавить("ДанныеСостава");
	КонецЕсли;
	...
	Для каждого Строка Из Таблица Цикл
		...
		Параметры = ПараметрыПолученияСоставаНабораВПроизводстве();
		ЗаполнитьЗначенияСвойств(Параметры, ПараметрыПолученияСостава,,"КлючСвязиНабор,КлючСвязи");
		ЗаполнитьЗначенияСвойств(Параметры, Строка, РеквизитыСтроки);
		
		Строка.ДанныеСостава = СоставНабораВПроизводстве(Параметры);
		
	КонецЦикла;
	
КонецПроцедуры

Процедура РаспаковатьСоставНаборовТехпроцесса(НаборДанных, ПараметрыПолученияСостава) Экспорт
	...
	ДобавитьСоставНаборовВТаблицу(НаборДанных, ПараметрыПолученияСостава);
	...
	Пока ИндексСтроки < КоличествоСтрокТаблицы Цикл
		...
		Если СтрокаНабораДанных.ТипНоменклатуры = ТипНоменклатурыНабор Тогда
			...
			ДанныеСостава = СтрокаНабораДанных.ДанныеСостава;
			Если ЗначениеЗаполнено(ДанныеСостава) Тогда
				
				Для каждого СтрокаКомплектующие Из ДанныеСостава.МатериалыИУслуги Цикл
					...
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаНабораДанных);
			...
			Иначе
				...
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось получить состав набора: %1';
					...
					НоменклатураКлиентСервер.ПредставлениеНоменклатуры(СтрокаНабораДанных.Номенклатура, СтрокаНабораДанных.Характеристика));
					
				ОбщегоНазначения.СообщитьПользователю(
					ТекстСообщения);
					
				СтруктураОчистки = Новый Структура;
				СтруктураОчистки.Вставить("Номенклатура", Справочники.Номенклатура.ПустаяСсылка());
				СтруктураОчистки.Вставить("Характеристика", Справочники.ХарактеристикиНоменклатуры.ПустаяСсылка());
				СтруктураОчистки.Вставить("СпособАвтовыбораНоменклатуры", Перечисления.СпособыАвтовыбораНоменклатуры.УточняетсяПриПроизводстве);
				СтруктураОчистки.Вставить("СпособАвтовыбораХарактеристики", Перечисления.СпособыАвтовыбораХарактеристики.УказываетсяВНСИ);
				
				ЗаполнитьЗначенияСвойств(СтрокаНабораДанных, СтруктураОчистки);
...
Функция ТабличныеЧастиСПараметризациейКонструктор() Экспорт
	
	Результат = Новый ТаблицаЗначений();
	
	Результат.Колонки.Добавить("ИмяТЧ",                Новый ОписаниеТипов("Строка"));
	Результат.Колонки.Добавить("ЕстьАвтовыбор",        Новый ОписаниеТипов("Булево"));
	Результат.Колонки.Добавить("ЕстьОтборПоСвойствам", Новый ОписаниеТипов("Булево"));
	Результат.Колонки.Добавить("ЕстьРасчетПоФормуле",  Новый ОписаниеТипов("Булево"));
	Результат.Колонки.Добавить("ЕстьУпаковки",         Новый ОписаниеТипов("Булево"));
...
//
Функция ПараметрыАвтовыбораКонструктор() Экспорт
...
// Создает параметры автовыбора для объекта и переданных данных по номенклатуре
// 
// Параметры:
//  Спецификация - СправочникСсылка.РесурсныеСпецификации
//  ДанныеПоНоменклатуре - Структура
// 
// Возвращаемое значение:
//  см. ПараметрыАвтовыбораКонструктор
Функция ПолучитьПараметрыАвтовыбора(Спецификация, ДанныеПоНоменклатуре) Экспорт
	
	ПараметрыАвтовыбора = ПараметрыАвтовыбораКонструктор();
	ПараметрыАвтовыбора.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц();
	
	СоздатьВТДанныеПоНоменклатуре(ДанныеПоНоменклатуре, ПараметрыАвтовыбора.МенеджерВременныхТаблиц);
	
	ПараметрыАвтовыбора.ТаблицаДанныхПоНоменклатуре = ПараметрыАвтовыбора.МенеджерВременныхТаблиц.Таблицы[0].ПолучитьДанные().Выгрузить();
	ПараметрыАвтовыбора.ЗначенияСвойствИзделий      = ЗначенияСвойствИзделия(ДанныеПоНоменклатуре);
	ПараметрыАвтовыбора.СоответствиеСвойств         = НастройкиСоответствияСвойств(Спецификация);
	
	Возврат ПараметрыАвтовыбора;
	
КонецФункции

Процедура СоздатьВТДанныеПоНоменклатуре(ДанныеПоНоменклатуре, МенеджерВременныхТаблиц) Экспорт
	...
	Для каждого КлючИЗначение Из ДанныеПоНоменклатуре Цикл
		ДополнительныеПоля.Добавить(СтрШаблон("&%1 КАК %1", КлючИЗначение.Ключ));
		Запрос.УстановитьПараметр(КлючИЗначение.Ключ, КлючИЗначение.Значение);
	КонецЦикла;
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ДополнительныеПоля", СтрСоединить(ДополнительныеПоля,","+Символы.ПС));
...
// 	ПараметрыАвтовыбора - см. ПараметрыАвтовыбораКонструктор
//
...
Функция НастройкиСоответствияСвойств(Спецификация) Экспорт
	...
	ИначеЕсли ТипЗнч(Спецификация) = Тип("СправочникСсылка.ТехнологическиеПроцессы") Тогда
...
// Есть автовыбор номенклатуры в таблице.
// 
// Параметры:
...
// Возвращаемое значение:
//  Булево
Функция ЕстьАвтовыборНоменклатурыВТаблице(Таблица) Экспорт
...
Функция СвойствоИмеетДопустимыйДляОперандаТип(Свойство)
	...
	Возврат ЗначениеЗаполнено(Свойство.ТипЗначения)
		И (Свойство.ТипЗначения.СодержитТип(Тип("Число"))
...
// Добавить таблицу для расчета по формулам в массив таблиц
// 
// Параметры:
//  МассивТаблиц - Массив из ТаблицаЗначений
//  СтруктураДанных - Структура
//  ИмяТЧ - Строка
Процедура ДобавитьТаблицуДляРасчетаПоФормулам(МассивТаблиц, СтруктураДанных, ИмяТЧ) Экспорт
	
	Отбор = Новый Структура("АлгоритмРасчетаКоличества", "");
	
	Если СтруктураДанных.Свойство(ИмяТЧ)
		И СтруктураДанных[ИмяТЧ].НайтиСтроки(Отбор).Количество() <> СтруктураДанных[ИмяТЧ].Количество() Тогда
		
		Если СтруктураДанных[ИмяТЧ].Колонки.Найти("Формула") = Неопределено Тогда
			СтруктураДанных[ИмяТЧ].Колонки.Добавить("Формула", Новый ОписаниеТипов("Строка"));
			СтруктураДанных[ИмяТЧ].Колонки.Добавить("ФормулаОшибкаРасчета", Новый ОписаниеТипов("Строка"));
		КонецЕсли;
		
		МассивТаблиц.Добавить(СтруктураДанных[ИмяТЧ]);
		
	КонецЕсли;
	
КонецПроцедуры

Процедура РассчитатьКоличествоВСтроках(СтруктураДанных, ИмяТЧ, ЕстьУпаковки, ДанныеДляРасчета) Экспорт
	
	Если СтруктураДанных.Свойство(ИмяТЧ) Тогда
		
		Для каждого Строка Из СтруктураДанных[ИмяТЧ] Цикл
			
			Если НЕ ПустаяСтрока(Строка.АлгоритмРасчетаКоличества) Тогда
				
				РассчитатьКоличествоПоФормуле(Строка, ДанныеДляРасчета);
...
Процедура СоздатьВТОтборПоСвойствам(СсылкаНаОбъект, МенеджерВременныхТаблиц, ДанныеПоНоменклатуре = Неопределено) Экспорт
	...
	Если ДанныеПоНоменклатуре = Неопределено Тогда
		...
		|";
		Запрос.УстановитьПараметр("ПустойКлючСвязи", ОбщегоНазначенияКлиентСервер.ПустойУникальныйИдентификатор());
	Иначе
		Запрос.Текст = ТекстЗапросаВТОтборПоСвойствамНастройки(СсылкаНаОбъект)
			+ ОбщегоНазначения.РазделительПакетаЗапросов()
			+ ТекстЗапросаВТОтборПоСвойствам("ВТОтборПоСвойствамНастройки");
...
// Текст запроса временной таблицы настроек отбора по свойствам
// 
// Параметры:
//  СсылкаНаОбъект - СправочникСсылка.РесурсныеСпецификации - ссылка на объект, для которого формируется запрос
// 
// Возвращаемое значение:
//  Строка - текст запроса
Функция ТекстЗапросаВТОтборПоСвойствамНастройки(СсылкаНаОбъект) Экспорт
	...
	|	ОтборПоСвойствам.Значение  КАК ЗначениеОтбор,
	|
	|	ВЫБОР
	...
	|ИЗ
	|	Справочник.РесурсныеСпецификации.ОтборПоСвойствам КАК ОтборПоСвойствам
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура.ДополнительныеРеквизиты КАК ЗначенияДопРеквизитовНоменклатуры
	|		ПО ОтборПоСвойствам.Свойство = ЗначенияДопРеквизитовНоменклатуры.Свойство
	...
	|		ПО ОтборПоСвойствам.Свойство = ЗначенияДопРеквизитовХарактеристик.Свойство
	...
	|		ПО ОтборПоСвойствам.Свойство = ЗначенияДопРеквизитовНаправленийДеятельности.Свойство
	|			И (ЗначенияДопРеквизитовНаправленийДеятельности.Ссылка = &НаправлениеДеятельности)
	|
	|ГДЕ
	|	ОтборПоСвойствам.Ссылка = &Ссылка
	|	И &ИспользоватьПараметризацию
	|";
	
	Если ТипЗнч(СсылкаНаОбъект) = Тип("СправочникСсылка.МаршрутныеКарты") Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РесурсныеСпецификации", "МаршрутныеКарты");
	ИначеЕсли ТипЗнч(СсылкаНаОбъект) = Тип("СправочникСсылка.ТехнологическиеПроцессы") Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РесурсныеСпецификации", "ТехнологическиеПроцессы");
	КонецЕсли;
...
// Текст запроса временной таблицы отбора по свойствам
// 
// Параметры:
//  ИмяТаблицыНастроек - Строка - имя таблицы настроек
// 
// Возвращаемое значение:
//  Строка - текст запроса
Функция ТекстЗапросаВТОтборПоСвойствам(ИмяТаблицыНастроек) Экспорт
...
Процедура ЗаполнитьДеревоСпецификаций(Параметры, Дерево)
	...
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		...
		НоваяСтрокаДерева.ВидСтроки      = Перечисления.ВидыСтрокСпецификации.Номенклатура;
		НоваяСтрокаДерева.ИндексКартинки = ПолучитьИндексКартинки(НоваяСтрокаДерева);
		...
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Номенклатура, "ТипНоменклатуры,ЕдиницаИзмерения");
		ЗаполнитьЗначенияСвойств(НоваяСтрокаДерева, ЗначенияРеквизитов);
	...
	ИначеЕсли ЗначениеЗаполнено(Параметры.Спецификация) Тогда
		...
		ЗаполнитьЗначенияСвойств(
			НоваяСтрокаДерева,
			ОсновноеИзделие,
			"Количество,КоличествоУпаковок,Упаковка,ЕдиницаИзмерения,ХарактеристикиИспользуются,ТипНоменклатуры");
...
Процедура ЗаполнитьДанныеСпецификацииСтрокДерева(Параметры, МассивСтрок, ПараметрыВыборки = Неопределено)
	...
	ДанныеСпецификаций = Справочники.РесурсныеСпецификации.ДанныеСпецификацииПоСпискуНоменклатуры(
		СписокНоменклатуры, ПараметрыВыборки);
...
Функция ПолучитьИндексКартинки(ДанныеСтроки, СтрокаОснование = Неопределено)
	
	ВидСтроки = ДанныеСтроки.ВидСтроки;
	
	РеквизитыСтрокиОснования = Новый Структура;
	РеквизитыСтрокиОснования.Вставить("ВидСтроки");
	РеквизитыСтрокиОснования.Вставить("ТипНоменклатуры");
	РеквизитыСтрокиОснования.Вставить("ПодлежитРемонту");
	РеквизитыСтрокиОснования.Вставить("ПланированиеРабот");
	
	Если СтрокаОснование <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(РеквизитыСтрокиОснования, СтрокаОснование);
	КонецЕсли;
	
	Если ВидСтроки = Перечисления.ВидыСтрокСпецификации.Этап
			И РеквизитыСтрокиОснования.ПланированиеРабот = Истина Тогда
		Результат = 2;
	ИначеЕсли ВидСтроки = Перечисления.ВидыСтрокСпецификации.Этап Тогда
		Результат = 0;
	ИначеЕсли ВидСтроки = Перечисления.ВидыСтрокСпецификации.Операция Тогда
		Результат = 4;
	ИначеЕсли РеквизитыСтрокиОснования.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Работа Тогда
		Результат = 10;
	ИначеЕсли ВидСтроки = Перечисления.ВидыСтрокСпецификации.ПромежуточныйПолуфабрикат
			И РеквизитыСтрокиОснования.ПодлежитРемонту <> Истина Тогда
		Результат = 13;
	ИначеЕсли ВидСтроки = Перечисления.ВидыСтрокСпецификации.Материал
			И ДанныеСтроки.ПроизводитсяВПроцессе Тогда
		Результат = 6;
	ИначеЕсли ВидСтроки = Перечисления.ВидыСтрокСпецификации.Материал Тогда
		Результат = 7;
	ИначеЕсли ВидСтроки = Перечисления.ВидыСтрокСпецификации.Трудозатрата Тогда
		Результат = 15;
	ИначеЕсли ВидСтроки = Перечисления.ВидыСтрокСпецификации.ВыходноеИзделие Тогда
		Результат = 12;
	ИначеЕсли ВидСтроки = Перечисления.ВидыСтрокСпецификации.ПобочныйВыход Тогда
		Результат = 12;
	ИначеЕсли РеквизитыСтрокиОснования.ПодлежитРемонту = Истина Тогда
		Результат = 12;
	Иначе
		Результат = 100;
	КонецЕсли;
...
Функция ПолучитьДанныеСпецификацииЗаказа(АдресВХранилище, КлючСвязи)
	...
	СтруктураДанных.Вставить("Операции", Новый Массив);
...
Процедура СтрокаДереваСпецификацийВМассивСтруктур(СтрокаДерева, МассивСтруктур)
...
//  ВидСтроки - ПеречислениеСсылка.ВидыСтрокСпецификации
...
Функция ДобавитьСтрокуДерева(СтрокаРодитель, ВидСтроки, СтрокаОснование = Неопределено)
	...
	НоваяСтрока.ИндексКартинки = ПолучитьИндексКартинки(НоваяСтрока, СтрокаОснование);
	НоваяСтрока.Уровень        = НоваяСтрока.Уровень();
	
	НоваяСтрока.УровеньОформления = СтрокаРодитель.УровеньОформления;
	Если НоваяСтрока.ВидСтроки = Перечисления.ВидыСтрокСпецификации.ВыходноеИзделие
		ИЛИ НоваяСтрока.ВидСтроки = Перечисления.ВидыСтрокСпецификации.ВходящееИзделие
		ИЛИ НоваяСтрока.ВидСтроки = Перечисления.ВидыСтрокСпецификации.ПобочныйВыход
		ИЛИ НоваяСтрока.ВидСтроки = Перечисления.ВидыСтрокСпецификации.Материал
		ИЛИ НоваяСтрока.ВидСтроки = Перечисления.ВидыСтрокСпецификации.ПромежуточныйПолуфабрикат
		ИЛИ НоваяСтрока.ВидСтроки = Перечисления.ВидыСтрокСпецификации.ВыполняемаяРабота
		ИЛИ НоваяСтрока.ВидСтроки = Перечисления.ВидыСтрокСпецификации.Трудозатрата
		ИЛИ НоваяСтрока.ВидСтроки = Перечисления.ВидыСтрокСпецификации.Операция Тогда
		НоваяСтрока.УровеньОформления = НоваяСтрока.УровеньОформления + 1;
	КонецЕсли;
...
Процедура ДобавитьДанныеОбИзделииДерева(ДанныеПоНоменклатуре, СтрокаДерева)
	
	ДанныеОбИзделии = Новый Структура;
	
	ДанныеСтроки = СтрокаДерева;
	Пока ДанныеСтроки <> Неопределено Цикл
		Если ДанныеСтроки.ВидСтроки = Перечисления.ВидыСтрокСпецификации.Номенклатура Тогда
			ДанныеОбИзделии.Вставить("Изделие",                 ДанныеСтроки.Номенклатура);
			ДанныеОбИзделии.Вставить("ХарактеристикаИзделия",   ДанныеСтроки.Характеристика);
			ДанныеОбИзделии.Вставить("СпецификацияИзделия",     ДанныеСтроки.Спецификация);
		ИначеЕсли ДанныеСтроки.ВидСтроки = Перечисления.ВидыСтрокСпецификации.Этап Тогда
			ДанныеОбИзделии.Вставить("Изделие",                 Справочники.Номенклатура.ПустаяСсылка());
			ДанныеОбИзделии.Вставить("ХарактеристикаИзделия",   Справочники.ХарактеристикиНоменклатуры.ПустаяСсылка());
			ДанныеОбИзделии.Вставить("СпецификацияИзделия",     Справочники.РесурсныеСпецификации.ПустаяСсылка());
			ДанныеОбИзделии.Вставить("ЭтапСпецификацииИзделия", ДанныеСтроки.Номенклатура);
		КонецЕсли;
		ДанныеСтроки = ДанныеСтроки.Родитель;
	КонецЦикла;
	
	Если ДанныеОбИзделии.Свойство("Изделие")
		И ЗначениеЗаполнено(ДанныеОбИзделии.Изделие) Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ДанныеПоНоменклатуре, ДанныеОбИзделии);
	КонецЕсли;
...
Функция ТекущиеОстаткиПоСпискуНоменклатуры(ТаблицаМатериалов)
	...
	|			ИЗ
	|				ВТ_Материалы КАК Фильтр)) КАК Запасы
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Расчет.КоличествоВНаличии КАК ВНаличии,
	|	Расчет.КоличествоДляДействияОтгрузить КАК Доступно
	|ИЗ
	|	ВТ_Материалы КАК ВТ_Материалы
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ РасчетОстатковДляУстановкиДействий КАК Расчет
	|		ПО Расчет.Номенклатура = ВТ_Материалы.Номенклатура
	|			И Расчет.Характеристика = ВТ_Материалы.Характеристика
	|			И Расчет.Склад = ВТ_Материалы.Склад";
...
Функция ОписаниеТаблицыТекстаЗапросаДанныеУпаковок() Экспорт
	...
	Результат.Вставить("ТекстУсловия",         "ИСТИНА");
...
Функция ТекстЗапросаДанныеУпаковок(ОписанияТаблиц) Экспорт
	...
	Для Индекс = 0 По ОписанияТаблиц.ВГраница() Цикл
		...
		ТекстЗапросаПоШаблону = СтрЗаменить(ТекстЗапросаПоШаблону, "&ТекстУсловия",    ОписаниеТаблицы.ТекстУсловия);
...
Функция ИсключаемыеРеквизитыЭлементаЗагружаемыхДанных(ИмяОбъектаМетаданных, ИмяТЧ = "")
	...
	ИначеЕсли ИмяОбъектаМетаданных = "РесурсныеСпецификации" Тогда
		...
		Результат.Добавить("РемонтируемоеИзделиеВидНоменклатуры");
		Результат.Добавить("РемонтируемоеИзделиеНоменклатура");
		Результат.Добавить("РемонтируемоеИзделиеХарактеристика");
	...
	ИначеЕсли ИмяОбъектаМетаданных = "ЭтапыПроизводства" Тогда
		...
		Результат.Добавить("ПланированиеРабот");
...
#Область ДирективныеЗамены

// Текст подстановки полей подбора замен.
// 
// Параметры:
//  ПараметрыВыборки - см. Справочники.РесурсныеСпецификации.ПараметрыВыборкиДанных
//  ИмяТаблицыМатериалы - Строка - 
//  Отбор - Строка - дополнительный отбор
// 
// Возвращаемое значение:
//  Строка
Функция ТекстЗапросаПоляДирективнойЗамены(ПараметрыВыборки = Неопределено, ИмяТаблицыМатериалы = "", Отбор = "") Экспорт
	
	ТекстЗапроса = "
	//++ НЕ УТКА
	|	ЗНАЧЕНИЕ(Документ.РазрешениеНаЗаменуМатериалов.ПустаяСсылка) КАК РазрешениеНаЗамену,
	//-- НЕ УТКА
	|	ЛОЖЬ                                                         КАК ПодбиратьЗамену
	|"; //@Query-part
...
Процедура ВыполнитьДирективныеЗаменыВНабореДанных(НаборДанных, Параметры) Экспорт
	
	Если НЕ АналогиМатериалов.ИспользоватьДирективныеЗамены() Тогда
		Возврат;
	КонецЕсли;
	...
	|	НаборДанных.Упаковка             КАК Упаковка
	|ПОМЕСТИТЬ ВтСписокНоменклатуры
	|ИЗ
	...
	ТаблицаПараметров = АналогиМатериалов.ТаблицаПараметровАналогов(Новый ОписаниеТипов("Число"));
	СтрокаПараметров = ТаблицаПараметров.Добавить();
	
	Если НЕ Параметры.Свойство("НачалоПроизводства", СтрокаПараметров.ДатаДействияРазрешений)
		ИЛИ НЕ ЗначениеЗаполнено(СтрокаПараметров.ДатаДействияРазрешений) Тогда
		СтрокаПараметров.ДатаДействияРазрешений = ТекущаяДатаСеанса();
	КонецЕсли;
	Если Параметры.Свойство("Изделие", СтрокаПараметров.Изделие)
		И ЗначениеЗаполнено(СтрокаПараметров.Изделие) Тогда
		Параметры.Свойство("ХарактеристикаИзделия", СтрокаПараметров.ХарактеристикаИзделия);
		Параметры.Свойство("СпецификацияИзделия", СтрокаПараметров.Спецификация);
		Параметры.Свойство("ЭтапСпецификацииИзделия", СтрокаПараметров.Этап);
	Иначе
		Параметры.Свойство("Номенклатура", СтрокаПараметров.Изделие);
		Параметры.Свойство("Характеристика", СтрокаПараметров.ХарактеристикаИзделия);
		Параметры.Свойство("Спецификация", СтрокаПараметров.Спецификация);
		Параметры.Свойство("ЭтапСпецификации", СтрокаПараметров.Этап);
	КонецЕсли;
	Параметры.Свойство("Распоряжение", СтрокаПараметров.ЗаказНаПроизводство);
	Параметры.Свойство("ПодразделениеДиспетчер", СтрокаПараметров.Подразделение);
	Параметры.Свойство("НаправлениеДеятельности", СтрокаПараметров.НаправлениеДеятельности);
	
	ТекстыЗапросов.Добавить(ТекстЗапроса);
	
	СоответствиеПолейПодбораАналогов = Новый Соответствие;
	СоответствиеПолейПодбораАналогов.Вставить("ПриоритетЗамены", "УказанияКЗамене");
	
	ПараметрыПодбораАналогов = Новый Структура;
	ПараметрыПодбораАналогов.Вставить("ТоварыВместеСПараметрами",         Ложь);
	ПараметрыПодбораАналогов.Вставить("УчестьМногиеКоМногим",             Ложь);
	ПараметрыПодбораАналогов.Вставить("ПодбиратьВзаимозаменяемыеАналоги", Ложь);
	ПараметрыПодбораАналогов.Вставить("ОтборРазрешений",                  СоответствиеПолейПодбораАналогов);
	ПараметрыПодбораАналогов.Вставить("ИмяВременнойТаблицаТовары",        "ВтСписокНоменклатуры");
	ПараметрыПодбораАналогов.Вставить("ИмяВременнойТаблицы",              "ВтДирективныеЗамены");
	
	ТекстыЗапросов.Добавить(АналогиМатериалов.ТекстЗапросаДирективныеЗамены(ПараметрыПодбораАналогов));
	
	// Данные упаковок
	ТекстыЗапросов.Добавить(ТекстЗапросаДанныеУпаковок(СтрРазделить("ВтСписокНоменклатуры,ВтДирективныеЗамены", ",")));
	
	ТекстЗапроса = "
	|ВЫБРАТЬ
	|	Замены.Номенклатура.ТипНоменклатуры              КАК ТипНоменклатуры,
	|	Замены.Номенклатура.ЕдиницаИзмерения             КАК ЕдиницаИзмерения,
	|	НЕОПРЕДЕЛЕНО                                     КАК ДанныеУпаковки,
	|	Замены.Номенклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.КоличествоШтук)   КАК ЕдиницаИзмеренияШтучная,
	|	ЕСТЬNULL(&ТекстЗапросаТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.КоличествоШтук), ЛОЖЬ)           КАК УпаковкаШтучная,
 	|	ВЫБОР 
	|		КОГДА Замены.Номенклатура.ИспользованиеХарактеристик В (
	|				ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.ОбщиеДляВидаНоменклатуры),
	|				ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.ОбщиеСДругимВидомНоменклатуры),
	|				ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.ИндивидуальныеДляНоменклатуры))
	|			ТОГДА ИСТИНА
	...
	|		И Замены.Номенклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.КоличествоШтук)
	|		И Замены.Упаковка <> ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	...
	|	Замены.*
	|ИЗ
	|	ВтДирективныеЗамены КАК Замены
	|
	|УПОРЯДОЧИТЬ ПО
	|	Замены.МаксимальныйПриоритетОбластиДействия УБЫВ,
	|	Замены.Период УБЫВ,
	|	Замены.Разрешение УБЫВ";
	...
	Округлять = ?(Параметры.Свойство("ОкруглятьКоличествоШтучныхТоваров"), Параметры.ОкруглятьКоличествоШтучныхТоваров, Истина);
	Запрос.УстановитьПараметр("ОкруглятьКоличествоШтучныхТоваров", Округлять);
	...
	ДанныеУпаковок = РезультатыЗапроса[РезультатыЗапроса.Количество()-2].Выгрузить();
	ДанныеЗамен = РезультатыЗапроса[РезультатыЗапроса.Количество()-1].Выгрузить();
	
	ЗаполнитьДанныеУпаковокВНабореДанных(ДанныеЗамен, ДанныеУпаковок);
	ЗаполнитьДанныеУпаковокВНабореДанных(НаборДанных, ДанныеУпаковок);
	
	Для каждого ДанныеСтроки Из НаборДанных Цикл
		
		ДанныеЗамены = ДанныеЗаменыДляСтрокиНабораДанных(ДанныеСтроки, ДанныеЗамен);
		Если ДанныеЗамены <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(ДанныеСтроки, ДанныеЗамены);
		КонецЕсли;
...
// Выполнить директивную замену в строке набора данных.
// 
// Параметры:
//  ДанныеСтроки - СтрокаТаблицыЗначений
//  ДанныеЗамен - ТаблицаЗначений
//  ДополнительныеПоляПоиска - Строка
// Возвращаемое значение:
//  Структура,Неопределено - данные результата замены
Функция ДанныеЗаменыДляСтрокиНабораДанных(ДанныеСтроки, ДанныеЗамен, ДополнительныеПоляПоиска = "") Экспорт
	
	Если ДанныеЗамен.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ПоляПоиска1 = "РольНоменклатуры,Номенклатура,Характеристика";
	ПоляПоиска2 = "РольНоменклатуры,Разрешение";
	
	Если НЕ ПустаяСтрока(ДополнительныеПоляПоиска) Тогда
		ПоляПоиска1 = ПоляПоиска1 + "," + ДополнительныеПоляПоиска;
		ПоляПоиска2 = ПоляПоиска2 + "," + ДополнительныеПоляПоиска;
	КонецЕсли;
	
	Если ДанныеЗамен.Индексы.Количество() = 0 Тогда
		ДанныеЗамен.Индексы.Добавить(ПоляПоиска1);
		ДанныеЗамен.Индексы.Добавить(ПоляПоиска2);
	КонецЕсли;
	
	СтруктураПоиска1 = Новый Структура(ПоляПоиска1);
	ЗаполнитьЗначенияСвойств(СтруктураПоиска1, ДанныеСтроки);
	СтруктураПоиска1.РольНоменклатуры = Перечисления.РолиАналогов.Материал;
	
	НайденныеСтроки = ДанныеЗамен.НайтиСтроки(СтруктураПоиска1);
	Если НайденныеСтроки.ВГраница() = -1 Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	СтруктураПоиска2 = Новый Структура(ПоляПоиска2);
	ЗаполнитьЗначенияСвойств(СтруктураПоиска2, СтрокаМатериал);
	СтруктураПоиска2.РольНоменклатуры = Перечисления.РолиАналогов.Аналог;
	
	НайденныеСтроки = ДанныеЗамен.НайтиСтроки(СтруктураПоиска2);
	Если НайденныеСтроки.ВГраница() = -1 Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	СтрокаАналог = НайденныеСтроки[0];
	
	КоэффициентМатериала = СтрокаМатериал.КоличествоУпаковок
		* СтрокаМатериал.ДанныеУпаковки.Числитель / СтрокаМатериал.ДанныеУпаковки.Знаменатель;
		
	КоэффициентАналога = СтрокаАналог.КоличествоУпаковок
		* СтрокаАналог.ДанныеУпаковки.Числитель / СтрокаАналог.ДанныеУпаковки.Знаменатель;
		
	Если ДанныеСтроки.ЕдиницаИзмеренияШтучная
		И (КоэффициентМатериала <> 1 ИЛИ КоэффициентАналога <> 1) Тогда
		Возврат Неопределено; // некорректное директивное разрешение на замену
	КонецЕсли;
	
	РезультатЗамены = Новый Структура(ОбрабатываемыеРеквизитыПриДирективнойЗамене());
	
	Если ДанныеСтроки.КоличествоУпаковок <> 0 Тогда
		
		КоличествоАналога = КоэффициентАналога * ПроизводствоКлиентСервер.КоэффициентПоНормативу(Ложь,
			ДанныеСтроки.КоличествоУпаковок * ДанныеСтроки.ДанныеУпаковки.Числитель / ДанныеСтроки.ДанныеУпаковки.Знаменатель,
			СтрокаМатериал.КоличествоУпаковок,
			СтрокаМатериал.ДанныеУпаковки);
			
		РезультатЗамены.Вставить("КоличествоУпаковок", КоличествоАналога);
		РезультатЗамены.Вставить("КоличествоУпаковокНаЕдиницуПартииВыпуска", КоличествоАналога);
		
	КонецЕсли;
	
	РезультатЗамены.РазрешениеНаЗамену               = СтрокаАналог.Разрешение;
	
	РезультатЗамены.Номенклатура                     = СтрокаАналог.Номенклатура;
	РезультатЗамены.Характеристика                   = СтрокаАналог.Характеристика;
	РезультатЗамены.ХарактеристикиИспользуются       = СтрокаАналог.ХарактеристикиИспользуются;

	РезультатЗамены.ЕдиницаИзмерения                 = СтрокаАналог.ЕдиницаИзмерения;
	РезультатЗамены.Упаковка                         = Неопределено;
	РезультатЗамены.КоэффициентУпаковки              = 1;
	РезультатЗамены.ДанныеУпаковки                   = УправлениеДаннымиОбИзделияхПовтИсп.СтруктураДанныхУпаковки();
	РезультатЗамены.Округлить                        = СтрокаАналог.Округлить;
	
	РезультатЗамены.СпособПолученияМатериала         = Перечисления.СпособыПолученияМатериаловВСпецификации.Обеспечивать;
	РезультатЗамены.Спецификация                     = Справочники.РесурсныеСпецификации.ПустаяСсылка();
	РезультатЗамены.ИсточникПолученияПолуфабриката   = Справочники.РесурсныеСпецификации.ПустаяСсылка();
	РезультатЗамены.Производится                     = Ложь;
	РезультатЗамены.ПроизводитсяВПроцессе            = Ложь;
	
	РезультатЗамены.ИспользуетсяАвтовыбор            = Ложь;
	РезультатЗамены.СпособАвтовыбораНоменклатуры     = Перечисления.СпособыАвтовыбораНоменклатуры.УказываетсяВНСИ;
	РезультатЗамены.СпособАвтовыбораХарактеристики   = Перечисления.СпособыАвтовыбораХарактеристики.УказываетсяВНСИ;
	РезультатЗамены.СведенияАвтовыбора               = "";
	РезультатЗамены.АлгоритмАвтовыбораХарактеристики = "";
	
	Возврат РезультатЗамены;
	
КонецФункции

Функция ОбрабатываемыеРеквизитыПриДирективнойЗамене(ВМассив = Ложь)
	
	Результат = Новый Массив;
	
	Результат.Добавить("РазрешениеНаЗамену");
	...
	Результат.Добавить("КоэффициентУпаковки");
	Результат.Добавить("ДанныеУпаковки");
	Результат.Добавить("Округлить");
	
	Результат.Добавить("СпособПолученияМатериала");
	Результат.Добавить("Спецификация");
	Результат.Добавить("ИсточникПолученияПолуфабриката");
	Результат.Добавить("Производится");
	Результат.Добавить("ПроизводитсяВПроцессе");
	
	Результат.Добавить("ИспользуетсяАвтовыбор");
	Результат.Добавить("СпособАвтовыбораНоменклатуры");
	Результат.Добавить("СпособАвтовыбораХарактеристики");
	Результат.Добавить("СведенияАвтовыбора");
	Результат.Добавить("АлгоритмАвтовыбораХарактеристики");
...
Процедура НастроитьЭлементОтбораПоПараметрам(СтруктураПараметров, ИмяОтбора, ИмяРеквизитаФормы) Экспорт
	
	ЗначениеОтбора = Неопределено;
	
	Если НЕ СтруктураПараметров.Свойство(ИмяОтбора, ЗначениеОтбора) Тогда
		Если СтруктураПараметров.Свойство("Отбор") Тогда
			//
		КонецЕсли;
	КонецЕсли;
