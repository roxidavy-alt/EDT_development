...
Функция НеобходимоЗаполнениеТоваровПоОтгрузке(ДокументПроведен) Экспорт
	...
	Если Не ДокументПроведен Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Документ не проведен, заполнение невозможно.';
									|en = 'Document is not posted, cannot fill in the attributes.'"));
		Возврат Ложь;
	Иначе
		Возврат Истина;
	КонецЕсли;
...
Процедура ОповеститьОбОтменеВыделенныхСтрок(ПричинаОтмены) Экспорт
	...
	ШаблонТекстаОповещения = НСтр("ru = 'Выделенные строки отменены%ТекстПричинаОтмены%.';
									|en = 'Selected lines are canceled%ТекстПричинаОтмены%.'");
	
	Если ЗначениеЗаполнено(ПричинаОтмены) Тогда
		ТекстПричинаОтмены =
			СтрЗаменить(НСтр("ru = ' по причине ""%ПричинаОтмены%""';
							|en = ' due to ""%ПричинаОтмены%""'"), "%ПричинаОтмены%", ПричинаОтмены);
	Иначе
		ТекстПричинаОтмены = "";
	КонецЕсли;
		...
		Иначе
			...
			Если ПроверятьОстатки Тогда
				ШаблонТекстаОповещения = НСтр("ru = 'Все непоставленные строки отменены%ТекстПричинаОтмены%.';
												|en = 'All undelivered items are canceled%ТекстПричинаОтмены%.'");
			Иначе
				ШаблонТекстаОповещения = НСтр("ru = 'Все строки отменены%ТекстПричинаОтмены%.';
												|en = 'All items are canceled%ТекстПричинаОтмены%.'");
			КонецЕсли;
	...
	Если РезультатВыполнения.Результат Тогда
		...
		ПодключаемоеОборудованиеУТКлиент.ВыполнитьДействиеПослеЗаписиФискальнойОперацииВЖурнал(
			...
			НСтр("ru = 'Не удалось записать данные в журнал фискальных операций.';
				|en = 'Cannot save data to the fiscal transactions log.'"));
