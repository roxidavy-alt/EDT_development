	...
	Попытка
		Если Не ДанныеОперации.Свойство("ПараметрыXML") Тогда
			ВызватьИсключение НСтр("ru = 'Не сформированы параметры запроса XML';
									|en = 'Не сформированы параметры запроса XML'");
		КонецЕсли;
...
Асинх Функция ПодготовитьДанныеАутентификацииПиот(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);
	...
	Если ПустаяСтрока(ДанныеОперации.ВходящиеДанные) Тогда
		ОписаниеОшибки = НСтр("ru = 'Параметры вызова операции не заполнены.';
								|en = 'The operation call parameters are not filled in.'");
		ЗаписатьОшибкуВЛог(ПараметрыПодключения, "PiotSetChallenge", ОписаниеОшибки);
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки); 
		Возврат РезультатВыполнения;
	КонецЕсли;
...
Асинх Функция НачатьРаботуПИоТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);
	...
	Если НЕ ИспользоватьТСПИоТ Тогда
		ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Данная команда доступна только при использовании ""%1""';
										|en = 'This command is only available when using %1'"), Наименование1СТСПиОТ());
		ЗаписатьОшибкуВЛог(ПараметрыПодключения, "PiotStart", ОписаниеОшибки);
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		Возврат РезультатВыполнения;
	КонецЕсли;
	...
	Если НЕ ПроверкаАутентификацияПИоТ Тогда 
		...
		Иначе                                                       
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Аутентификация %1 не пройдена.';
										|en = 'Authentication of %1 failed.'"), НаименованиеТСПиОТ());
...
Асинх Функция ПроверкаКМПИоТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);
	...
	Если НЕ ИспользоватьТСПИоТ Тогда
		ОписаниеОшибки = НСтр("ru = 'Данная команда доступна только при использовании ""1С:ПИОТ""';
								|en = 'This command is only available when using 1C:PIoT'");
		ЗаписатьОшибкуВЛог(ПараметрыПодключения, "PiotCheckKM", ОписаниеОшибки);
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		Возврат РезультатВыполнения;
	КонецЕсли;
	...
	Если ПустаяСтрока(ДанныеОперации.ЗапросJSON) Тогда
		ОписаниеОшибки = НСтр("ru = 'Параметры запроса JSON не заполнен.';
								|en = 'JSON request parameters are not filled in.'");
		ЗаписатьОшибкуВЛог(ПараметрыПодключения, "PiotCheckKM", ОписаниеОшибки);
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки); 
		Возврат РезультатВыполнения;
	КонецЕсли;
	...
	Иначе
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Проверка КМ в %1 не пройдена.';
									|en = 'MC check in %1 failed.'"), НаименованиеТСПиОТ());
		РезультатВыполнения = Ждать ПолучитьОшибкуПИоТ(ОбъектДрайвера, ЗаписьЛога, ТекстОшибки);
	КонецЕсли;                                  
...
Асинх Функция СтатусПИоТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);
	...
	Если НЕ ПроверкаАутентификацияПИоТ Тогда      
		ОписаниеОшибки = НСтр("ru = 'Процедура аутентификация ""1С:ПИОТ"" ранее не была запущена.';
								|en = '1C:TS PIoT authentication procedure was not launched before.'");
		ЗаписатьОшибкуВЛог(ПараметрыПодключения, "PiotStatus", ОписаниеОшибки);
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		Возврат РезультатВыполнения;
	КонецЕсли;
	...
	Иначе
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка определения состояния %1';
									|en = 'Error defining %1 state'"), НаименованиеТСПиОТ());
		РезультатВыполнения = Ждать ПолучитьОшибкуПИоТ(ОбъектДрайвера, ЗаписьЛога, ТекстОшибки);
	КонецЕсли;    
...
Асинх Функция СлужебнаяИнформацияПИоТ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации) Экспорт
	...
	Иначе
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Служебная информация %1 не получена.';
									|en = '%1 service information is not received.'"), НаименованиеТСПиОТ());
		РезультатВыполнения = Ждать ПолучитьОшибкуПИоТ(ОбъектДрайвера, ЗаписьЛога, ТекстОшибки);
	КонецЕсли;    
...
Асинх Функция ПодключитьТСПИоТАсинх(ПараметрыПодключения) 
		...
		Если Не РезультатПодключения.Подключено Тогда
			...
			Если Не РезультатУстановки.Установлено Тогда
				...
				Результат.ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Ошибка установки ""%1""';
															|en = '%1 installation error'"), Наименование1СТСПиОТ());
				Возврат Результат;
			КонецЕсли;
			...
			Если Не РезультатПодключения.Подключено Тогда
				...
				Результат.ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Ошибка подключения ""%1""';
															|en = '%1 connection error'"), Наименование1СТСПиОТ());
				Возврат Результат;
			КонецЕсли;
		КонецЕсли;                                     
	...
	Исключение             
		...
		ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Ошибка инцицилизации %1';
										|en = '%1 initialization error'"), НаименованиеТСПиОТ()) + Символы.ПС + ТекстОшибки;
