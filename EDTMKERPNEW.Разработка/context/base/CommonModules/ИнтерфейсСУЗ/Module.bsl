				...
				Если Статус = Перечисления.СтатусыПулаКодовМаркировкиСУЗ.Отклонен Тогда
					...
					Иначе
						ВозвращаемоеЗначение.ТекстОшибки = НСтр("ru = 'Заказ на эмиссию отклонен. Проверьте баланс счета в СУЗ.';
																|en = 'Заказ на эмиссию отклонен. Проверьте баланс счета в СУЗ.'");
					КонецЕсли;
	...
	Иначе
		...
		Если ТоварнаяГруппа = Неопределено Тогда
			ВозвращаемоеЗначение.ТекстОшибки = НСтр(
				...
				|en = 'Не найдено ни одной товарной группы по которой ведется учет маркируемой продукции'");
		КонецЕсли;
		Если КонструкторИСМПОбщегоНазначенияКлиентСервер.ЭтоПродукцияМЕХ(ТоварнаяГруппа) Тогда
			ЗаголовокHTTP.Вставить("Content-Type", "application/json");
			URLЗапроса = СтрШаблон(
	...
	Если Результат = Неопределено Тогда
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Не удалось определить имя товарной группы: %1';
				|en = 'Не удалось определить имя товарной группы: %1'"),
			ТоварнаяГруппа);
	КонецЕсли;
