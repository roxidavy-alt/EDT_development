...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Справочники.ПоказателиБюджетов.ПриДобавленииОбработчиковОбновления(Обработчики);
	Справочники.ПравилаЛимитовПоДаннымБюджетирования.ПриДобавленииОбработчиковОбновления(Обработчики);
	Справочники.ПравилаРасчетаСвязанныхОстатковИОборотовБюджетов.ПриДобавленииОбработчиковОбновления(Обработчики);
	Справочники.СтатьиБюджетов.ПриДобавленииОбработчиковОбновления(Обработчики);
	РегистрыСведений.КэшИменДокументовДляОбработкиПоПравилу.ПриДобавленииОбработчиковОбновления(Обработчики);
	РегистрыСведений.УдалитьСвязиПоказателейБюджетов.ПриДобавленииОбработчиковОбновления(Обработчики);
	...
	Справочники.ВидыРасчетовССотрудниками.ПриДобавленииОбработчиковОбновления(Обработчики);
	Справочники.ГруппыФинансовогоУчетаВнеоборотныхАктивов.ПриДобавленииОбработчиковОбновления(Обработчики);
	Справочники.ВидыНачисленийУдержаний.ПриДобавленииОбработчиковОбновления(Обработчики);
...
#Область УстановитьКонстантуИспользоватьЗаказыМатериаловВПроизводство
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыКА.УстановитьКонстантуИспользоватьЗаказыМатериаловВПроизводство";
	Обработчик.Версия = "2.5.25.12";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("25ff4063-a129-4519-82fd-d5ca8ab806e2");
	...
	Обработчик.Комментарий = НСтр("ru = 'Начальное заполнение константы ""Использовать заказы материалов в производство"".';
									|en = 'Initial filling of the ""Use material orders for shop floor stockroom"" constant.'");
	
	ЧитаемыеОбъекты = Новый Массив;
	ЧитаемыеОбъекты.Добавить(Метаданные.Константы.ИспользоватьПроизводство.ПолноеИмя());
	ЧитаемыеОбъекты.Добавить(Метаданные.Константы.ИспользоватьНесколькоСкладов.ПолноеИмя());
	ЧитаемыеОбъекты.Добавить(Метаданные.Константы.ИспользоватьЗаказыМатериаловВПроизводство.ПолноеИмя());
	
	Обработчик.ЧитаемыеОбъекты = СтрСоединить(ЧитаемыеОбъекты, ",");
	
	ИзменяемыеОбъекты = Новый Массив;
	ИзменяемыеОбъекты.Добавить(Метаданные.Константы.ИспользоватьЗаказыМатериаловВПроизводство.ПолноеИмя());
...
#Область УстановкаКонстанты_ИспользоватьЗаказыМатериаловВПроизводство

Процедура УстановитьКонстантуИспользоватьЗаказыМатериаловВПроизводство(ПараметрыОбновления) Экспорт
	...
	ЗначениеКонстанты = ИспользоватьПроизводство И ИспользоватьНесколькоСкладов;
	
	Если Константы.ИспользоватьЗаказыМатериаловВПроизводство.Получить() <> ЗначениеКонстанты Тогда
		Константы.ИспользоватьЗаказыМатериаловВПроизводство.Установить(ЗначениеКонстанты);
	КонецЕсли; 	
