...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	ТекстЗапроса =
	"////////////////////////////////////////////////////////////////////////////////////////////////////
	|// Списание остаточной стоимости НМА \ Частичная ликвидация остаточной стоимости НМА (Дт СчетРасходов:: Кт СчетУчета). Списание стоимости на расходы.
	|ВЫБРАТЬ
	|	
	...
	|	""СписаниеНМАЛокализация_1874"" КАК НомерФрагмента
	|
	|ИЗ
	...
	ТекстЗапроса =
	"////////////////////////////////////////////////////////////////////////////////////////////////////
	|// Списание остаточной стоимости НМА \ Частичная ликвидация остаточной стоимости НМА (Дт СчетРасходов:: Кт СчетУчета). Списание стоимости на статьи активов/пассивов.
	|ВЫБРАТЬ
	|	
	...
	|		ИНАЧЕ ""Списание остаточной стоимости НМА""
	|	КОНЕЦ КАК Содержание,
	|	""СписаниеНМАЛокализация_1875"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеНМА2_4 КАК Операция
	...
	ТекстЗапроса =
	"////////////////////////////////////////////////////////////////////////////////////////////////////
	|// Списание остаточной стоимости НМА \ Частичная ликвидация остаточной стоимости НМА (Дт СчетРасходов:: Кт СчетУчета). Списание стоимости ЦФ на расходы.
	|ВЫБРАТЬ
	|	
	|	СтрокиДт.Регистратор КАК Ссылка,
	|	СтрокиДт.Период КАК Период,
	...
	|	КОНЕЦ КАК Содержание,
	|	""СписаниеНМАЛокализация_1876"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	ТекстЗапроса =
	"////////////////////////////////////////////////////////////////////////////////////////////////////
	|// Списание остаточной стоимости НМА \ Частичная ликвидация остаточной стоимости НМА (Дт СчетРасходов:: Кт СчетУчета). Списание стоимости ЦФ на статьи активов/пассивов.
	|ВЫБРАТЬ
	|	
	|	Строки.Регистратор КАК Ссылка,
	|	Строки.Период КАК Период,
	...
	|		ИНАЧЕ ""Списание целевых средств в остаточной стоимости НМА на расходы""
	|	КОНЕЦ КАК Содержание,
	|	""СписаниеНМАЛокализация_1877"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеНМА2_4 КАК Операция
	...
	|	КОНЕЦ КАК Содержание,
	|	""СписаниеНМАЛокализация_1878"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеНМА2_4 КАК Операция
	...
	|		ИНАЧЕ ""Списание начисленной амортизации""
	|	КОНЕЦ КАК Содержание,
	|	""СписаниеНМАЛокализация_1879"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеНМА2_4 КАК Операция
	...
	|	""СписаниеНМАЛокализация_1294"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеНМА2_4 КАК Операция
	...
	|	""СписаниеНМАЛокализация_1295"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|	""СписаниеНМАЛокализация_1296"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьНМА КАК Строки
...
Функция ТекстЗапросаТаблицаДвиженияДоходыРасходыПрочиеАктивыПассивы(ТекстыЗапроса) Экспорт
	...
	|
	|	ВЫБОР 
	|		КОГДА ВложенныйЗапрос.ОтражатьВБУ 
	...
	|		ИЛИ ДанныеДокумента.ОтражатьВБУ И ТаблицаНМА.РезервПереоценкиАмортизацииРегл <> 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|		ИЛИ ДанныеДокумента.ОтражатьВБУ И ТаблицаНМА.РезервПереоценкиСтоимостиРегл <> 0";
	...
	Если СтруктураИзмененныхРеквизитов.Свойство("Дата")
		...
		Если Форма.СлужебныеПараметрыФормы.ВариантУчетаОтложенногоНалога = ПредопределенноеЗначение("Перечисление.ВариантыУчетаОтложенногоНалога.БалансовыйМетод") Тогда
			...
			Для Каждого ДанныеСтроки Из Объект.НМА Цикл
				...
				Если ДанныеСтроки.СуммаСписанияПР <> 0 Тогда
