...
Процедура ПроверитьДоговорНаВозможностьКорреспонденцииСоСчетомЦифровогоРубля(ДоговорОбъект, Отказ) Экспорт
	...
	Если Не ПараметрыКорреспонденции.ДопустимаКорреспонденцияСоСчетомЦифровогоРубля Тогда
		...
		ТекстОшибки = НСтр("ru = 'Расчетный счет не может корреспондировать со счетом цифрового рубля.
		                         |Укажите другой счет';
		                         |en = 'The current account cannot correspond with the digital ruble account.
...
Процедура ПроверитьЗаполнениеНалоговыхРеквизитов(Объект, Отказ, НепроверяемыеРеквизиты, ФлагОбменСБанками = Ложь, ОшибкиЗаполнения = "") Экспорт
	...
	Если ПеречислениеВБюджет И МассивОпераций.Найти(ХозяйственнаяОперация) <> Неопределено Тогда
		...
		Если ПараметрыПроверкиЗаполнения.ПрименениеПриказа58н
			...
			ТекстОшибки =
				СтрШаблон(НСтр("ru = 'В поле ""Статус плательщика"" следует указать одно из доступных значений: %1';
								|en = 'В поле ""Статус плательщика"" следует указать одно из доступных значений: %1'"), СтрокаДоступныхСтатусов);
			Если ФлагОбменСБанками Тогда
				ДобавитьОшибкуЗаполнения(ОшибкиЗаполнения, ТекстОшибки);
			Иначе
				...
				Если Не СсылкаКорректная Тогда
					...
					ТекстСообщения = СтрШаблон(
						НСтр("ru = 'Реквизиты получателя платежа, указанные в платежной ссылке %1, не соответствуют данным получателя в документе.
									...
									|en = 'The payment recipient details specified in the %1 payment link do not match the recipient details in the document.
									|FPS payment recipient: TIN - %2 , account - %3'"),
	...
	Исключение
		...
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Не удалось выполнить проверку платежных ссылок СБП по причине: %1';
				|en = 'Cannot check FPS payment links due to: %1'"),
...
Функция ОшибкиЗаполненияРеквизитовПлательщикаПолучателя(ЭтоПлательщик, ДокументОбъект, РеквизитыСтороны, ВыводитьСообщение, Отказ = Ложь) Экспорт
	...
	ПолеИНН = ?(ЭтоПлательщик, НСтр("ru = 'ИНН плательщика';
									|en = 'Payer TIN'"), НСтр("ru = 'ИНН получателя';
																	|en = 'Recipient TIN'"));
	ПолеКПП = ?(ЭтоПлательщик, НСтр("ru = 'КПП плательщика';
									|en = 'Payer KPP'"), НСтр("ru = 'КПП получателя';
																	|en = 'KPP of recipient'"));
		...
		Иначе
			...
			Иначе
				ТекстыСообщений.Добавить(НСтр("ru = 'В строке №%1 необходимо скорректировать объект, созданный при включенной настройке ""Выплаты самозанятым"" или создать новый:';
												|en = 'In line %1, adjust the object created when the ""Payments to self-employed persons"" setting was enabled or create a new one:'"));
			КонецЕсли;
...
Функция ДанныеШапкиДокументаДДСПоДоговоруЗаймаСотруднику(Договор, ЭтоПоступление = Ложь)
	...
	СтруктураПараметров.Организация = ДанныеШапки.Организация;
	СтруктураПараметров.Валюта = ДанныеШапки.Валюта;
	...
	ИначеЕсли Объект.ТипНалога = Справочники.ВидыНалоговВзносов.ОбеспечительныйПлатежСПОТ Тогда
		...
		Если Не ПустаяСтрока(Объект.КодБК) И СокрЛП(Объект.КодБК) <> КодБК Тогда
			ТекстОшибки = НСтр("ru = 'КБК не соответствует реквизитам обеспечительного платежа';
								|en = 'КБК не соответствует реквизитам обеспечительного платежа'");
			Если ФлагОбменСБанками Тогда
				ДобавитьОшибкуЗаполнения(ОшибкиЗаполнения, ТекстОшибки);
			Иначе
...
Процедура ПроверитьЗаполнениеКБК(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения, ПараметрыПроверки)
	...
	ИначеЕсли Не СтроковыеФункцииКлиентСервер.ТолькоЛатиницаВСтроке(ВРег(КБК), Истина, "0123456789АБВГДЕЖИКЛМНПРСТУФЦЧШЩЭЮЯ")
		...
		ТекстОшибки = НСтр("ru = 'В КБК допустимо вводить только цифры и буквы (8 - 17 разряды, кроме ""О"" и ""Х"")';
							|en = 'Only numbers and letters (8-17 digits, except ""O"" and ""X"") are allowed in the Budget Classification Code'");
