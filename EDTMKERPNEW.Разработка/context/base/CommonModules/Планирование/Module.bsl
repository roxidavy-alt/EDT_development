...
Функция ОсновнойОперандПлана(Форма, СОтбором = Ложь) Экспорт
	...
	//++ НЕ УТ
	ИначеЕсли ТипЗнч(Форма.Объект.Ссылка) = Тип("ДокументСсылка.ПланПроизводства") Тогда
		Если Форма.Объект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот
			ИЛИ Форма.Объект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Ремонт Тогда
...
Функция ПоместитьДополнительныеПоляВХранилище(Форма, Смещение) Экспорт
	...
	ИмяГруппыПредставление.Добавить(Новый Структура("Идентификатор, Представление", "ДополнительныеОперандыФакты",  НСтр("ru = 'Дополнительные операнды (факты)';
																														|en = 'Additional operands (actual data)'")));
...
Процедура ПроверитьНаличиеБюджетнойЗадачи(Отказ, Ссылка, ЭтотОбъект) Экспорт
	...
	Если Не ЗначениеЗаполнено(ЭтотОбъект.СценарийБюджетирования) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Синхронизирует значения новых констант использования обработок формирования документов по планам с их ведущими константами: использование документов и соответствующих планов.';
									|en = 'Synchronizes values of new constants for using data processors for converting plans into documents with their leading constants: using documents and corresponding plans.'");
...
Процедура ВыполнитьСхемуПреобразования(СКД_Преобразование, Параметры, ТаблицаРезультата, НаборыДанныхОбъекты = Неопределено)
	...
	//++ НЕ УТКА
	Если Параметры.Свойство("ТипПроизводственногоПроцесса") И Параметры.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот Тогда
		КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(
			КомпоновщикНастроек,
