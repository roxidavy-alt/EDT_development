...
Функция ВыраженияПоказателейСуммы(ИмяСправочникаИсточника, ИдентификаторИсточникаДанных, РазделИсточникаДанных, ТипИтога = Неопределено, ИсточникСуммыОперации = Неопределено, ИсточникВалютный = Ложь) Экспорт
	...
	Если РазделИсточникаДанных = РазделыИсточниковДанных.ОперативныйУчет
		...
		Если ПолучитьФункциональнуюОпцию("НастройкаПравилПолученияФактаПоСтатьямАктивовПассивов") Тогда
			ПравилоПоСтатьеАктивовПассивов = ИсточникиДанныхСервер.ЭтоСтатьяАктивовПассивов(ИдентификаторИсточникаДанных);
		Иначе
			ПравилоПоСтатьеАктивовПассивов = Ложь;
		КонецЕсли;
		
		Если ПравилоПоСтатьеАктивовПассивов Тогда
			ИсточникДанных = ПланыВидовХарактеристик.СтатьиАктивовПассивов[ИдентификаторИсточникаДанных];
		ИначеЕсли ИдентификаторИсточникаДанных <> "ДвиженияДенежныхСредствКВыплате" Тогда
		...
		Если ИмяСправочникаИсточника = "ПравилаПолученияФактаПоПоказателямБюджетов" Тогда
			...
			Если ПравилоПоСтатьеАктивовПассивов Тогда
				ПоказателиРегистра = ПоказателиСуммыСтатьиАктивовПассивов(ИсточникДанных);
		...
		ИначеЕсли ИмяСправочникаИсточника = "ПравилаПолученияФактаПоСтатьямБюджетов" Тогда
			...
			ИначеЕсли ПравилоПоСтатьеАктивовПассивов Тогда
				ПоказателиРегистра = ПоказателиСуммыСтатьиАктивовПассивов(ИсточникДанных);
			Иначе
...
//		*Значение - Строка - имя поля в схеме-источнике данных.
// 
Функция ВыражениеПоказателяКоличества(ИмяСправочникаИсточника, ИдентификаторИсточникаДанных, РазделИсточникаДанных, ТипИтога, ИсточникСуммыОперации = Неопределено) Экспорт
	...
	Если РазделИсточникаДанных = РазделыИсточниковДанных.ОперативныйУчет
		...
		Если ПравилоПоСтатьеАктивовПассивов Тогда
			ИсточникДанных = ПланыВидовХарактеристик.СтатьиАктивовПассивов[ИдентификаторИсточникаДанных];
			ПоказателиРегистра = ПоказателиКоличестваСтатьиАктивовПассивов(ИсточникДанных);
			РесурсыИсточникаКоличества = ПоказателиРегистра.Получить(ИсточникСуммыОперации);
			
			Если РесурсыИсточникаКоличества.Количество() > 0 Тогда
...
Функция СлужебныеПоляМакетовРасчета() Экспорт
	...
	СлужебныеПоля.Добавить("КорИсточникДанных");
...
Функция ПоказателиСуммыПоИсточникуДанных(ИсточникДанных, СписокВыбора = Ложь) Экспорт
	...
	Если ТипыДанныхУчета.Найти(Перечисления.ТипыДанныхУчета.ДенежныеСредства) <> Неопределено Тогда
		...
		Если Не ЛимитыПоДаннымБюджетированияСервер.ИспользуетсяРегистрЛимитовБюджетирования() Или Не СписокВыбора Тогда
...
// Возвращает соответствие показателей аналитических регистров суммовым ресурсам источника данных
// переданной статьи активов и пассивов для расчета фактических данных статей и показателей бюджетов.
//
// Параметры:
//  ИсточникДанных - ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов
//
// Возвращаемое значение:
//  Соответствие из КлючИЗначение:
//  * Ключ - ПеречислениеСсылка.ПоказателиАналитическихРегистров
//  * Значение - Массив из Структура - Описание:
//   ** Имя - Строка - имя ресурса, связанное с показателем
//   ** ИсточникВалюты - Строка - строковое представление валюты ресурса
//
Функция ПоказателиСуммыСтатьиАктивовПассивов(ИсточникДанных) Экспорт
	
	Если ТипЗнч(ИсточникДанных) <> Тип("ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов") Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ПоказателиСуммы = Новый Соответствие();
	СвойстваПоказателей = "Ресурсы";
	СвойстваРесурсов = "Имя, ИсточникВалюты";
	
	МенеджерСтатьиАктивовПассивов = ПланыВидовХарактеристик.СтатьиАктивовПассивов;
	ПоказателиИсточника = МенеджерСтатьиАктивовПассивов.ПоказателиСтатьиАктивовПассивов(ИсточникДанных);
	
	Для Каждого КлючИЗначение Из ПоказателиИсточника Цикл
		...
		Для Каждого КлючИЗначение Из ОписаниеПоказателя Цикл
			...
			Если ГруппаРесурсов = МенеджерСтатьиАктивовПассивов.ГруппаРесурсовУправленческогоУчета() Тогда
				ИмяРесурса = "СуммаУпр";
				ИсточникВалюты = "ВалютаУпр";
			ИначеЕсли ГруппаРесурсов = МенеджерСтатьиАктивовПассивов.ГруппаРесурсовРегламентированногоУчета() Тогда
				ИмяРесурса = "СуммаРегл";
				ИсточникВалюты = "ВалютаРегл";
			ИначеЕсли ГруппаРесурсов = МенеджерСтатьиАктивовПассивов.ГруппаРесурсовВалютногоУчета() Тогда
				ИмяРесурса = "СуммаВВалюте";
				ИсточникВалюты = "Валюта";
			Иначе
				Продолжить;
			КонецЕсли;
			
			Если ТаблицаРесурсов.Количество() > 0 Тогда
				МассивРесурсов.Добавить(Новый Структура(СвойстваРесурсов, ИмяРесурса, ИсточникВалюты));
			КонецЕсли;;
		КонецЦикла;
		
		ПоказателиСуммы.Вставить(Показатель, Новый Структура(СвойстваПоказателей, МассивРесурсов));
	КонецЦикла;
	
	Возврат ПоказателиСуммы;
...
// Возвращает соответствие показателей аналитических регистров количественным ресурсам источника данных
// переданной статьи активов и пассивов для расчета фактических данных статей и показателей бюджетов.
//
// Параметры:
//  ИсточникДанных - ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов
...
//  Соответствие из КлючИЗначение:
//  * Ключ - ПеречислениеСсылка.ПоказателиАналитическихРегистров
//  * Значение - Массив из Строка - имя ресурса, связанное с показателем
//
Функция ПоказателиКоличестваСтатьиАктивовПассивов(ИсточникДанных) Экспорт
	...
	Для Каждого КлючИЗначение Из ПоказателиИсточника Цикл
		...
		Для Каждого КлючИЗначение Из ОписаниеПоказателя Цикл
			...
			Если ГруппаРесурсов = МенеджерСтатьиАктивовПассивов.ГруппаРесурсовКоличественногоУчета() Тогда
			...
			Если ТаблицаРесурсов.Количество() > 0 Тогда
				МассивРесурсов.Добавить(ИмяРесурса);
			КонецЕсли;
		КонецЦикла;
		
		ПоказателиКоличества.Вставить(Показатель, МассивРесурсов);
	КонецЦикла;
...
Функция СтруктураАналитикиПоТипуДанныхУчета(ТипДанныхУчета) Экспорт
	...
	ИначеЕсли ТипДанныхУчета = Перечисления.ТипыДанныхУчета.ДенежныеСредства Тогда
		...
		ПараметрыПоляАналитики.ПутьКДанным = "";
		ПараметрыПоляАналитики.Тип = Новый ОписаниеТипов("СправочникСсылка.СтруктураПредприятия");
		ПараметрыПоляАналитики.Заголовок = НСтр("ru = 'Подразделение платежа';
												|en = 'Payment business unit'");
		ПараметрыПоляАналитики.ИмяКор = "КорПодразделениеПлатежа";
		ПараметрыПоляАналитики.ЗаголовокКор = НСтр("ru = 'Кор. подразделение платежа';
													|en = 'Cor. payment business unit'");
		СтруктураАналитики.Вставить("ПодразделениеПлатежа", ПараметрыПоляАналитики);
...
// Возвращает массив видов документов по указанному типу данных учета хозяйственной операции
//
// Параметры:
//  ТипДанныхУчета - ПеречислениеСсылка.ТипыДанныхУчета - тип данных учета
// Возвращаемое значение:
//  Массив из СправочникСсылка.ИдентификаторыОбъектовМетаданных - массив видов документов
//
Функция МассивВидовДокументовПоТипуДанныхУчета(ТипДанныхУчета) Экспорт
...
// Возвращает соответствие видов документов, по которым осуществляется контроль лимита расхода денежных средств.
//
// Параметры:
...
// Возвращаемое значение:
//  Соответствие из КлючИЗначение:
//   *Ключ - СправочникСсылка.ИдентификаторыОбъектовМетаданных - вид документа
//   *Значение - Строка - полное имя документа
//
Функция ВидыДокументовДляКонтроляЛимитов(ТипДанныхУчета) Экспорт
	...
	Если Не ПустаяСтрока(ИмяРегистра) Тогда
		...
		Для Каждого ТипРегистратора Из ТипыРегистраторов Цикл
			...
			Если ИменаНеиспользуемыхДокументов.Найти(МетаданныеТипа.Имя) = Неопределено Тогда
				ВидыДокументов.Вставить(ОбщегоНазначения.ИдентификаторОбъектаМетаданных(МетаданныеТипа),
...
Функция ИспользуетсяСКД_ИсполнениеБюджета(ИсточникСуммыПравила, Расход) Экспорт
	...
	Если (ПолучитьФункциональнуюОпцию("НастройкаПравилПолученияФактаПоКорреспондирующимРегистрам")
		Или ЛимитыПоДаннымБюджетированияСервер.ИспользуетсяРегистрЛимитовБюджетирования()) Тогда
...
Функция СКД_ИсполнениеБюджета(Расход, Приход) Экспорт
	...
	ВидыДокументов = ВидыДокументовДляКонтроляЛимитов(Расход);
