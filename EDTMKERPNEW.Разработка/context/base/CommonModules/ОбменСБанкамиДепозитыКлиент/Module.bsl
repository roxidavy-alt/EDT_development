...
Процедура ПослеОбновленияДанныхДепозитныхПродуктов(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не ТипЗнч(Результат) = Тип("Структура") Тогда // задание было отменено
		ТекстСообщения =  НСтр("ru = 'Не удалось отправить запрос в банк.';
								|en = 'Не удалось отправить запрос в банк.'");
...
Процедура УстановитьПараметрыИПредложенияПослеПолучения(Форма, ПараметрыПредложений,
	...
	Если ДанныеПредложенийБанков.ОдинБанк Тогда
		ТекстЗаголовка = СтрШаблон(НСтр("ru = 'Предложения банка %1';
										|en = 'Предложения банка %1'"), ДанныеВыбранногоУсловия.НаименованиеБанка);
	КонецЕсли;
...
Процедура УстановитьПредложениеБанка(Форма, ВыбранноеПредложение, ВыбранноеУсловие, ПараметрыПредложений)
	...
	Иначе
		...
		ЭлементДоходность.Заголовок = НСтр("ru = 'Не выполнены условия продукта';
											|en = 'Не выполнены условия продукта'");
		Если ОдноПредложение Тогда
			ЭлементОписание.Заголовок = 
				НСтр("ru = 'Укажите другие параметры депозита или подберите подходящие условия';
					|en = 'Укажите другие параметры депозита или подберите подходящие условия'");
		Иначе
			ЭлементОписание.Заголовок = 
				НСтр("ru = 'Выберите другой продукт из списка предложений или подберите подходящие условия';
					|en = 'Выберите другой продукт из списка предложений или подберите подходящие условия'");
		КонецЕсли;
