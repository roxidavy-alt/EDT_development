		...
		И ОписаниеОшибок.Ошибки[0].Описание = ЭлектроннаяПодписьСлужебныйКлиент.ТекстОшибкиУстройствоНеПоддерживается() Тогда
			Если ОписаниеОшибок.Ошибки.Количество() = 1 Тогда
				ОписаниеОшибок.Ошибки[0].Описание = НСтр("ru = 'Рутокен не найден. Подробности см. в журнале регистрации.';
														|en = 'Rutoken is not found. See the Event log for details.'");
			Иначе
	...
	Если ОбъектКомпоненты = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(Результат.Ошибка) Тогда
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
				НСтр("ru = 'Электронная подпись.Получение сертификатов на токене';
					|en = 'Digital signature.Retrieving certificates on token'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
	...
	Если ЗначениеЗаполнено(ПараметрыОперации.АлгоритмШифрования) Тогда
		...
		Если АлгоритмШифрования = АлгоритмНеПоддерживается() Тогда
			...
			НСтр("ru = 'На мобильном устройстве шифрование алгоритмом %1 будет реализовано позднее.';
				|en = 'Encryption using the %1 algorithm on mobile devices will be available in a future version.'"),
