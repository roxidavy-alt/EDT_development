...
Процедура ПриИзмененииСостоянияСогласования(ПредметСогласования, Состояние, ВызовИзФормыОбъекта, Установил,
	...
	Если ТипОбъекта <> Тип("ДокументСсылка.КоммерческоеПредложениеКлиенту")
			...
			//++ НЕ УТКА
			И ТипОбъекта <> Тип("ДокументСсылка.РазрешениеНаЗаменуМатериалов")
			//-- НЕ УТКА
...
Процедура ПриОпределенииОбъектовПоддерживающихАвтонастройку(Объекты) Экспорт
	...
	Строка.ОписаниеВыполняемыхДействий =
		НСтр("ru = 'Будут созданы правила интеграции и вид внутренних документов для справочника ""Соглашения об условиях продаж"".';
			|en = 'Integration rules and a kind of internal transfer documents for the ""Terms of sale"" catalog will be created.'");
	...
	Строка.ОписаниеВыполняемыхДействий =
		НСтр("ru = 'Будут созданы правила интеграции и вид внутренних документов для справочника ""Договоры с контрагентами"".';
			|en = 'Integration rules and a kind of internal transfer documents for the ""Contracts"" catalog will be created.'");
...
Процедура ПриОпределенииТиповОбъектовПоддерживающихИнтеграцию(Типы) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьАналогиМатериалов") Тогда
		ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(Типы, Тип("ДокументСсылка.РазрешениеНаЗаменуМатериалов"));
	КонецЕсли;
	//-- НЕ УТКА
...
Процедура ПриПроверкеСоответствияПравилаФункциональнымОпциям(Правило, ТипОбъектаДО, ТипОбъектаИС, Отказ,
	...
	//++ НЕ УТКА
	МассивТиповДокументов = Новый Массив; // Массив Из Строка
	МассивТиповДокументов.Добавить("Документ.РазрешениеНаЗаменуМатериалов");
	
	Если МассивТиповДокументов.Найти(ТипОбъектаИС) <> Неопределено
			И Не ПолучитьФункциональнуюОпцию("ИспользоватьАналогиМатериалов") Тогда
		Отказ = Истина;
	КонецЕсли;
	//-- НЕ УТКА
