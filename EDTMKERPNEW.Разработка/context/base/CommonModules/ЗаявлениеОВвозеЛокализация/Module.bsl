...
// Формирует структуру таблиц для выгрузки данных отчета "Статистическая форма учета перемещения товаров"
// на основании документа ЗаявлениеОВвозеТоваров.
//
// Параметры:
...
// 		* РеквизитыШапка - Структура - данные шапки документа ЗаявлениеОВвозеТоваров.
...
Функция ПодготовитьТаблицыСтатистическойФормыУчетаПеремещенияТоваровДляВыгрузкиПечати(МассивДокументов) Экспорт
	...
	УчетНДСРФ.ЗаполнитьСписокВыбораВидовТранспорта(СписокВидовТранспорта);
	...
	Для каждого СтрокаШапки Из Шапка Цикл
		...
		Если ЗначениеЗаполнено(РеквизитыШапка.Организация) И ЗначениеЗаполнено(РеквизитыШапка.Контрагент) Тогда
			...
			ОтправительАдрес = УчетНДСРФ.ПолучитьАдресВВидеСоответствия(
			...
			ПолучательАдрес = УчетНДСРФ.ПолучитьАдресВВидеСоответствия(
		...
		Если НайденныеСтрокиТовары.Количество() > 0 Тогда
			...
			Для Каждого СтрокаТовары Из НайденныеСтрокиТовары Цикл
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрокаВыгрузки,
					РегламентированнаяОтчетностьУП.РеквизитыТаможеннойДекларации(СтрокаТовары.НомерТаможеннойДекларации),
					"КодТО, ДатаРегистрацииСтр, НомерДок, ДекларацияНом");
...
Процедура СформироватьДвиженияВЖурналУчетаСчетовФактур(МассивРегистраторов, ПризнакОтменаПроведения = Ложь) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			КорректировкиНалоговойБазы = РегистрыБухгалтерии.КорректировкиНалоговойБазы.СоздатьНаборЗаписей();
			КорректировкиНалоговойБазы.Отбор.Регистратор.Установить(Выборка.СчетФактура);
			Движения.Вставить("КорректировкиНалоговойБазы", КорректировкиНалоговойБазы);
			//-- НЕ УТ
			...
			//++ НЕ УТКА
			ТаблицаВыборочнойРегистрацииКОтражениюМеждународныйУчет = МеждународныйУчетПроведениеСервер.ТаблицаВыборочнойРегистрацииКОтражению();
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(
				ТаблицаВыборочнойРегистрации, ТаблицаВыборочнойРегистрацииКОтражениюМеждународныйУчет);
			Если ЗначениеЗаполнено(ТаблицаВыборочнойРегистрацииКОтражениюМеждународныйУчет) Тогда
				ТаблицыДляДвижений.Вставить("ТаблицаВыборочнойРегистрацииКОтражениюМеждународныйУчет", ТаблицаВыборочнойРегистрацииКОтражениюМеждународныйУчет);
			КонецЕсли;
...
Процедура СформироватьТекстОтраженияВРеглУчете(ТекстЗапроса) Экспорт
	...
	|	""ЗаявлениеОВвозеЛокализация_375"" КАК НомерФрагмента
	|ИЗ
	...
	|	""ЗаявлениеОВвозеЛокализация_376"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|	0 КАК СуммаВРКт,
	|	""Включение НДС в стоимость"" КАК Содержание,
	|	""ЗаявлениеОВвозеЛокализация_377"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|	0 КАК СуммаВРКт,
	|	""Включение НДС в стоимость"" КАК Содержание,
	|	""ЗаявлениеОВвозеЛокализация_378"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	#Область НачислениеАкцизов
	ТекстНачислениеАкцизов = "
	|////////////////////////////////////////////////////////////////////////////////////////////////////
	|// Начисление акцизов (Дт 19.06 :: Кт 68.03)
	|ВЫБРАТЬ
	...
	|	""ЗаявлениеОВвозеЛокализация_1936"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	ТекстыОтражения.Добавить(ТаможеннаяДекларацияИмпортЛокализация.ТекстВключениеАкцизовВСтоимость());
	ТекстыОтражения.Добавить(ТаможеннаяДекларацияИмпортЛокализация.ТекстВключениеАкцизовВСтоимостьОтклоненияВСтоимости());
...
// Формирует текст запроса дополнительных временных таблиц, необходимых для отражения в регламентированном учете.
// 
// Параметры:
...
//
// Параметры:
// УникальныйИдентификатор - УникальныйИдентификатор - Уникальный идентификатор формы выгружаемого заявления. 
...
//
// Параметры:
// УникальныйИдентификатор - УникальныйИдентификатор - идентификатор формы выгружаемого заявления. 
//	Объект - ДокументСсылка.ЗаявлениеОВвозеТоваров - ссылка на выгружаемый документ.
//	ТолькоПроверка - Булево -  признак выгрузки (= Ложь) / проверки возможности выгрузки (= Истина).
//
// Возвращаемое значение:
...
// 	ФормаЗаявления			- ФормаКлиентскогоПриложения - форма, из обработчика события которой происходит вызов процедуры.
//
Процедура ИнициализацироватьФормуРегламентированногоОтчета(ФормаЗаявления) Экспорт
...
Функция ТекстЗапросаТаблицаОтражениеДокументовВМеждународномУчете(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса =
		...
		|	&Период                        КАК Период,
		|	&Организация                   КАК Организация,
		|	НАЧАЛОПЕРИОДА(ДанныеОплатыВБюджет.ДатаПодтвержденияОплаты, ДЕНЬ)   КАК ДатаОтражения
		|ИЗ
		|	РегистрСведений.ПодтверждениеОплатыНДСВБюджет КАК ДанныеОплатыВБюджет
		|ГДЕ
		|	ДанныеОплатыВБюджет.СчетФактура В(&Ссылка)
		|	И ДанныеОплатыВБюджет.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияОплатыНДСВБюджет.ПолученоПодтверждение)";
...
Процедура ПроверитьВозможностьВыгрузки_Товары(СтатусПроверки, ДанныеДокумента)
	...
	Для Каждого СтрТабл Из ТаблицаРаздел1 Цикл
		...
		Если ЗаполнятьЕдИзмТовНБАкц И ПустаяСтрока(СтрТабл.П000010000123) Тогда
			ТекстСообщения = НСтр("ru = 'Не заполнена единица измерения акциза.';
									|en = 'Excise unit of measure is required.'");
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель, ДанныеДокумента)
	...
	Для Каждого СтрТабл Из ТаблицаРаздел1 Цикл
		...
		Если ЗначениеЗаполнено(СтрТабл.П000010000102) Тогда
			...
			ЕдИзмТовНБАкц = "";
			КоличТов = СтрТабл.П000010000105;
			НБАкциз = СтрТабл.П000010000114;
			Если (КоличТов <> НБАкциз) И (КоличТов <> 0) И (НБАкциз <> 0) И Не (ПрОсвАкциз = 3 ИЛИ ПрОсвАкциз = 4) Тогда
				ЕдИзмТовНБАкц = СтрТабл.П000010000123;
			КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть2015Кв1(УзелРодитель, ДанныеДокумента)
	...
	Для Каждого СтрТабл Из ТаблицаРаздел1 Цикл
		...
		Если ЗначениеЗаполнено(СтрТабл.П000010000102) Тогда
			...
			КоличТов = СтрТабл.П000010000105;
			НБАкциз = СтрТабл.П000010000114;
			Если (КоличТов <> НБАкциз) И (КоличТов <> 0) И Не (ПрОсвАкциз = 3 ИЛИ ПрОсвАкциз = 4) Тогда
				ЕдИзмТовНБАкц = СтрТабл.П000010000123;
			КонецЕсли;
...
Процедура ОпределитьСсылкуНаСуществующийОтчетВФТС(Форма)
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ДатаНачала", НачалоМесяца(Форма.Объект.Дата));
...
Функция ТекстОтклонениеПриФактуровкеТоваровРабот()
	...
	|	""ЗаявлениеОВвозеЛокализация_2010"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстОтклонениеПриФактуровкеУслуг()
	...
	|	""ЗаявлениеОВвозеЛокализация_2011"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СебестоимостьТоваров КАК Строки
