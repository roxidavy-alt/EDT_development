...
Функция ДоступныеПользователюНаборыСуммовыхПоказателей(ИмяОтчета, НачалоПериода, КонецПериода, Организация) Экспорт
	...
	Если Не НалоговыйУчет.ТребуетсяНалоговыйУчет() Тогда
		...
		Иначе
			...
			ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(
				ДоступныеНаборы,
				СправкиРасчетыКлиентСервер.НомерНабораСуммовыхПоказателейНалоговыйУчетСКорректировкой());
...
Функция КонтекстФормированияОтчета(Форма)
	...
	Если ПоддерживаетПерерасчет(ИдентификаторОтчета) И БазовыеРеквизиты.СУчетомКорректировокНалоговойБазы Тогда
		Контекст.Вставить("ОграничитьНаборомКорректировок", Истина);
		Контекст.Вставить("ДатаЗакрытияНабора", ?(ЗначениеЗаполнено(БазовыеРеквизиты.ДатаНачалаПодготовкиДекларации),
			КонецДня(БазовыеРеквизиты.ДатаНачалаПодготовкиДекларации), КонецДня(ТекущаяДатаСеанса())));
	Иначе
		Контекст.Вставить("ОграничитьНаборомКорректировок", Ложь);
		Контекст.Вставить("ДатаЗакрытияНабора", Дата(1,1,1));
	КонецЕсли;
	...
	Если ДоступныеНаборыСуммовыхПоказателей <> Неопределено Тогда
		...
		Контекст.Вставить("ПоказательНУсКорректировкой", Ложь);
	...
	Если ДоступныеНаборыСуммовыхПоказателей <> Неопределено Тогда
		...
		Если СправкиРасчетыКлиентСервер.ЭтоНаборСуммовыхПоказателейНалоговыйУчет(Контекст.НомерНабораСуммовыхПоказателей)
			ИЛИ СправкиРасчетыКлиентСервер.ЭтоНаборСуммовыхПоказателейНалоговыйУчетСКорректировкой(Контекст.НомерНабораСуммовыхПоказателей) Тогда
...
// Определяет поддерживает ли отчет вывод данных с учетом Перерасчета
// 
// Параметры:
//  ИдентификаторОтчета - Строка, Неопределено - Идентификатор отчета
// 
// Возвращаемое значение:
//  Булево - Поддерживает перерасчет
Функция ПоддерживаетПерерасчет(ИдентификаторОтчета) Экспорт
	
	Если ИдентификаторОтчета = "СправкаРасчетУбытковПрошлыхЛет"
		 Или ИдентификаторОтчета = "СправкаРасчетНормированияРасходов" 
		 Или ИдентификаторОтчета = "СправкаРасчетДолиНалоговойБазы"
		 Или ИдентификаторОтчета = "СправкаРасчетНалогаНаПрибыль" Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
