...
Функция ПараметрыДляПроведенияДокумента(Документ, Свойства) Экспорт
	...
	Если Свойства.РежимЗаписи <> РежимЗаписиДокумента.Запись Тогда
		...
		Если ТипДокумента = Тип("ДокументОбъект.ПроизводствоБезЗаказа")
			//++ НЕ УТКА
			...
			//-- НЕ УТКА
			Или Свойства.РежимЗаписи = РежимЗаписиДокумента.Проведение
				И ТипДокумента = Тип("ДокументОбъект.ВыработкаСотрудников") Тогда
...
Функция ТекстыЗапросовСторнирования(МетаданныеДокумента) Экспорт
	...
	Если ДвиженияДокумента.Содержит(МетаданныеРегистра) Тогда
		...
		Если ИмяДокумента = "Документ.ПроизводствоБезЗаказа" Тогда
			
			ТекстЗапроса = "
			// Если в исправляющем документе не поддерживается сохранение партий производства,
			// то движения по регистру сторнируем только в случае, если не оформлена выработка.
			// Если в исправляющем документе поддерживается сохранение партий производства,
			// то исправляющий документ формирует движения на дельту.
			|ВЫБРАТЬ
			|	СторнирующиеДокументы.Ссылка КАК Регистратор,
			|	СторнирующиеДокументы.Дата КАК Период,
			...
			|
			|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ &ИмяДокумента КАК СторнирующиеДокументы
			|	ПО ДанныеРегистра.Регистратор = СторнирующиеДокументы.СторнируемыйДокумент
			|	И СторнирующиеДокументы.Ссылка В(&Ссылка)
			|	
			|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТрудозатратыКОформлению КАК ОформлениеТрудозатрат
			|	ПО ДанныеРегистра.Распоряжение = ОформлениеТрудозатрат.Распоряжение
			|	И ОформлениеТрудозатрат.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
			|	
			|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК РеквизитыСторнирующегоПроизводстваБезЗаказа
			|	ПО РеквизитыСторнирующегоПроизводстваБезЗаказа.Ссылка = СторнирующиеДокументы.Ссылка
			|
			|ГДЕ
			...
			// В производстве без заказа с ИсправлениеСохраняетПартииПроизводства поддерживается сохранение партий производства - исключаем.
			|	И НЕ ЕСТЬNULL(РеквизитыСторнирующегоПроизводстваБезЗаказа.ИсправлениеСохраняетПартииПроизводства, ЛОЖЬ)
			|";
		...
		ИначеЕсли ИмяДокумента = "Документ.Сторно" Тогда
			
			ТекстЗапроса = "
			// Если оформляется Сторно производства без заказа с сохранением партий производства,
			// то сторнируются все ранее не отсторнированные по партии производства движения.
			|ВЫБРАТЬ
			...
			|	
			|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК РеквизитыПроизводстваБезЗаказа
			|	ПО РеквизитыПроизводстваБезЗаказа.Ссылка = ДокументыСторно.СторнируемыйДокумент
			|	И РеквизитыПроизводстваБезЗаказа.ИсправлениеСохраняетПартииПроизводства
			|	
			|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПартииПроизводства КАК СпрПартииПроизводства
			|	ПО ВЫБОР
			|		КОГДА РеквизитыПроизводстваБезЗаказа.Исправление
			|			ТОГДА РеквизитыПроизводстваБезЗаказа.ИсправляемыйДокумент
			|		ИНАЧЕ РеквизитыПроизводстваБезЗаказа.Ссылка
			|	КОНЕЦ = СпрПартииПроизводства.Документ
			|	И НЕ СпрПартииПроизводства.ПометкаУдаления
			|	
			|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ТрудозатратыКОформлению КАК ДанныеРегистра
			|	ПО ДанныеРегистра.ПартияПроизводства = СпрПартииПроизводства.Ссылка
			|	
			|ГДЕ
			|	ДокументыСторно.Ссылка В(&Ссылка)
			|	И ТИПЗНАЧЕНИЯ(ДанныеРегистра.Регистратор) = ТИП(Документ.ПроизводствоБезЗаказа)
			|
			|СГРУППИРОВАТЬ ПО
			|	ДокументыСторно.Ссылка,
			|	ДокументыСторно.Дата,
			|	ДанныеРегистра.ВидДвижения,
			|	ДанныеРегистра.Организация,
			|	ДанныеРегистра.ПартияПроизводства,
			|	ДанныеРегистра.Распоряжение,
			|	ДанныеРегистра.ВидРабот,
			|	ДанныеРегистра.Исполнитель,
			|	ДанныеРегистра.КодСтрокиРаспоряжения,
			|	ДанныеРегистра.Подразделение
			|ИМЕЮЩИЕ
			|	НЕ СУММА(ДанныеРегистра.Количество) = 0
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			// Сторнирование остальных документов выполняется по общим правилам.
			|ВЫБРАТЬ
			|	ДокументыСторно.Ссылка               КАК Регистратор,
			|	ДокументыСторно.Дата                 КАК Период,
			|	ДанныеРегистра.ВидДвижения           КАК ВидДвижения,
			...
			|	ПО ДокументыСторно.СторнируемыйДокумент = ДанныеРегистра.Регистратор
			...
			|ГДЕ
			|	НЕ ДанныеРегистра.Сторно
			|	И (НЕ ТИПЗНАЧЕНИЯ(ДанныеРегистра.Регистратор) = ТИП(Документ.ПроизводствоБезЗаказа)
			|		ИЛИ НЕ ЕСТЬNULL(РеквизитыПроизводстваБезЗаказа.ИсправлениеСохраняетПартииПроизводства, ЛОЖЬ))
			|";
...
Процедура ИнициализироватьДанныеКонтроляИзменений(Запрос, ТекстыЗапроса, Документ) Экспорт
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияТрудозатратыКОформлениюИзменение") Тогда
		...
		ТекстЗапроса =
			...
			|ИЗ
			|	ДвиженияТрудозатратыКОформлениюИзменение КАК ДвиженияТрудозатратыКОформлениюИзменение
			|	
			|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ТрудозатратыКОформлению КАК ТаблицаОстатков
			|	ПО ТаблицаОстатков.Организация = ДвиженияТрудозатратыКОформлениюИзменение.Организация
			|	И ТаблицаОстатков.ПартияПроизводства = ДвиженияТрудозатратыКОформлениюИзменение.ПартияПроизводства
			|	И ТаблицаОстатков.Распоряжение = ДвиженияТрудозатратыКОформлениюИзменение.Распоряжение
			|	И ТаблицаОстатков.ВидРабот = ДвиженияТрудозатратыКОформлениюИзменение.ВидРабот
			|	И ТаблицаОстатков.Исполнитель = ДвиженияТрудозатратыКОформлениюИзменение.Исполнитель
			|	И ТаблицаОстатков.Подразделение = ДвиженияТрудозатратыКОформлениюИзменение.Подразделение
			|	И ТаблицаОстатков.КодСтрокиРаспоряжения = ДвиженияТрудозатратыКОформлениюИзменение.КодСтрокиРаспоряжения
			|
			|ГДЕ
			|	НАЧАЛОПЕРИОДА(ТаблицаОстатков.Период, МЕСЯЦ) = ДвиженияТрудозатратыКОформлениюИзменение.Месяц
			|
			|СГРУППИРОВАТЬ ПО
			|	ТаблицаОстатков.Организация,
			|	ТаблицаОстатков.ПартияПроизводства,
			|	ТаблицаОстатков.Распоряжение,
			|	ТаблицаОстатков.ВидРабот,
			|	ТаблицаОстатков.Исполнитель,
			|	ТаблицаОстатков.Подразделение,
			|	ТаблицаОстатков.КодСтрокиРаспоряжения
			|
			|ИМЕЮЩИЕ
			|	СУММА(ВЫБОР
			|		КОГДА ТаблицаОстатков.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияТрудозатратыКОформлениюИзменение") Тогда
		...
		Если ТипЗнч(Документ) = Тип("ДокументОбъект.ВыработкаСотрудников") Тогда
			ШаблонСообщения = НСтр("ru = 'Количество по виду работ ""%1""
				| превышает или не соответствует данным распоряжения ""%2"" (на %3 %4)';
				|en = 'The quantity according to the ""%1"" activity kind
				| exceeds or does not correspond to the ""%2"" reference data (by %3 %4)'");
		Иначе
			ШаблонСообщения = НСтр("ru = 'Выработка сотрудников по виду работ ""%1""
				| по распоряжению ""%2"" уже оформлена на %3 %4';
				|en = 'Timesheets charge by the ""%1"" activity kind
				| under the ""%2"" reference is already generated for %3 %4'");
		КонецЕсли;
...
// Определяет использование участков в подразделениях с пооперационным управлением
// 
// Возвращаемое значение:
//  Булево
Функция ИспользоватьУчасткиПриПооперационномУправлении() Экспорт
	...
	Возврат ИспользоватьУчастки;
	
КонецФункции

// Определяет использование участков в подразделениях с пооперационным планированием
// 
// Возвращаемое значение:
//  Булево
Функция ИспользоватьУчасткиПриПооперационномПланировании() Экспорт
...
Процедура УстановитьУсловноеОформлениеПоляДоляСтоимостиПроцент(Форма, ИмяОбъекта, ИмяТЧ, ПутьКЭлементам = "", ПутиКДанным = Неопределено) Экспорт
	...
	ОтборЭлемента = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных(ИмяОбъекта + "." + ИмяТЧ + ".ДоляСтоимости");
	ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.Заполнено;
	...
	ОтборЭлемента = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных(ИмяОбъекта + "." + ИмяТЧ + ".ЕстьОшибкиЗаполненияДолиСтоимости");
	ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Ложь;
	...
	// доля стоимости, % менее 0.01 (представление)
	Элемент = УсловноеОформление.Элементы.Добавить();
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<0,01%';
																|en = '<0,01%'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("Видимость", Истина);
	
КонецПроцедуры

// Устанавливает условное оформление полей группы "Доля стоимости" для отмененных строк
//
// Параметры:
//  Форма            - ФормаКлиентскогоПриложения - форма.
//  ИмяОбъекта       - Строка - имя объекта.
...
//  ДинамическаяСтруктура	 - Булево	 - значение опции (контекстное)
//
Процедура ЗаполнитьСписокТиповПроизводственногоПроцесса(Элемент, ИсключаемыеТипы = Неопределено, ДинамическаяСтруктура = Неопределено) Экспорт
	
	МассивТипов = Перечисления.ТипыПроизводственныхПроцессов.ДоступныеТипы(ДинамическаяСтруктура);
	
	СписокВыбора = Элемент.СписокВыбора;
	СписокВыбора.Очистить();
	
	Для Каждого ТипПроцесса Из МассивТипов Цикл
...
Процедура ПроверкаЗаполненияИсполнителей(Объект, Дата = Неопределено, Отказ) Экспорт
	...
	Если ВездеУказаныСотрудники
		И ЗначениеЗаполнено(НачалоУчетаТрудозатратПоСотрудникам)
		// Исправления исключаются из проверки
		И (Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "Исправление")
			Или Не Объект.Исправление) Тогда
		
		КолонкиТрудозатрат = "Исполнитель";
		Если НЕ ЗначениеЗаполнено(Дата) Тогда
			КолонкиТрудозатрат = КолонкиТрудозатрат + ", ДатаВыполнения";
		КонецЕсли;
		
		ПроверяемыеСотрудники = Объект.Трудозатраты.Выгрузить(,КолонкиТрудозатрат);
		
		Если ЗначениеЗаполнено(Дата) Тогда
			ПроверяемыеСотрудники.Колонки.Добавить("ДатаВыполнения", Новый ОписаниеТипов("Дата"));
			ПроверяемыеСотрудники.ЗаполнитьЗначения(Дата, "ДатаВыполнения");
		КонецЕсли;
		
		РаботающиеСотрудники = ПроизводствоСервер.ОтобратьРаботающихСотрудников(ПроверяемыеСотрудники, Объект.Организация);
		...
		Для Каждого Строка Из Объект.Трудозатраты Цикл
			...
			Если РаботающийСотрудник = Неопределено Тогда
					...
					ТекстСообщения = СтрШаблон(ШаблонОшибкаУволенный, Строка(Строка.НомерСтроки), Объект.Организация);
...
// 
// Параметры:
//  Сотрудники - ТаблицаЗначений - Исполнители:
...
Функция ОтобратьРаботающихСотрудников(Сотрудники, Организация) Экспорт
	
	Запрос = Новый Запрос("
	|ВЫБРАТЬ
	|	Сотрудники.Исполнитель КАК Сотрудник,
	|	Сотрудники.ДатаВыполнения КАК ДатаВыполнения
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеДляПодбораСотрудников КАК ДанныеДляПодбора
	|	ПО Сотрудники.Сотрудник = ДанныеДляПодбора.Сотрудник
	|	И ДанныеДляПодбора.Организация = &Организация
	|	И ДанныеДляПодбора.ВидСобытия <> ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.Увольнение)
	|	И Сотрудники.ДатаВыполнения >= ДанныеДляПодбора.Начало
	|	И Сотрудники.ДатаВыполнения < ДанныеДляПодбора.Окончание");
...
Процедура ДобавитьЭтап_ОформлениеПроизводстваБезЗаказов(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока =  ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ОформлениеПроизводстваБезЗаказов);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Оформление производства без заказов (2.2)';
											|en = 'Register backflush production (2.2)'"; // @НСтр-1
...
Процедура ДобавитьЭтап_ОформлениеВыработкиСотрудников(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ОформлениеВыработкиСотрудников);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Оформление выработки сотрудников';
											|en = 'Register timesheets charge'"; // @НСтр-1
...
Процедура Использование_ВыработкаСотрудников(ПараметрыОбработчика) Экспорт
	...
	|	СУММА(ДД.КоличествоПриход - ДД.КоличествоРасход) КАК КоличествоОборот
	|ПОМЕСТИТЬ Обороты
	|ИЗ
	|	РегистрНакопления.ТрудозатратыКОформлению.ОстаткиИОбороты(
	|		&НачалоПериода,
	|		&КонецПериода,
	|		Месяц,,
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	// Получим распоряжения, по которым были оформлены исправления.
	// Если исправлений не было, то количество к оформлению получается по актуальным остаткам,
	// если было, то по оборотам за период.
	|ВЫБРАТЬ
	|	Обороты.Организация           КАК Организация,
	|	Обороты.ПартияПроизводства    КАК ПартияПроизводства,
	|	Обороты.Распоряжение          КАК Распоряжение,
	|	Обороты.КодСтрокиРаспоряжения КАК КодСтрокиРаспоряжения
	|ПОМЕСТИТЬ РаспоряженияСИсправлением
	|ИЗ
	|	Обороты КАК Обороты
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК РеквизитыПбЗ
	|	ПО РеквизитыПбЗ.Ссылка = Обороты.Распоряжение
	|	И РеквизитыПбЗ.ИсправлениеСохраняетПартииПроизводства
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ТрудозатратыКОформлению КАК ТрудозатратыКОформлению
	|	ПО ТрудозатратыКОформлению.Организация = Обороты.Организация
	|	И ТрудозатратыКОформлению.ПартияПроизводства = Обороты.ПартияПроизводства
	|	И ТрудозатратыКОформлению.Распоряжение = Обороты.Распоряжение
	|	И ТрудозатратыКОформлению.КодСтрокиРаспоряжения = Обороты.КодСтрокиРаспоряжения
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК ДокументыСторно
	|	ПО ДокументыСторно.Ссылка = ТрудозатратыКОформлению.Регистратор
	|	И ДокументыСторно.СторноИсправление
	|	И ДокументыСторно.Проведен
	|	И НЕ ДокументыСторно.ДополнительнаяЗапись
	|
	|ГДЕ
	|	НЕ ТрудозатратыКОформлению.Количество = 0
	|
	|СГРУППИРОВАТЬ ПО
	|	Обороты.Организация,
	|	Обороты.ПартияПроизводства,
	|	Обороты.Распоряжение,
	|	Обороты.КодСтрокиРаспоряжения
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	ПартияПроизводства,
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	// Выбираем остатки по распоряжениям текущего месяца.
	|ВЫБРАТЬ
	...
	|			,
	|			(Организация, ПартияПроизводства, Распоряжение, КодСтрокиРаспоряжения) В
	|				(ВЫБРАТЬ
	|					Т.Организация,
	...
	|	
	// Исключаем распоряжения с исправлениями.
	|	ЛЕВОЕ СОЕДИНЕНИЕ РаспоряженияСИсправлением КАК РаспоряженияСИсправлением
	|	ПО РаспоряженияСИсправлением.Организация = Остатки.Организация
	|	И РаспоряженияСИсправлением.ПартияПроизводства = Остатки.ПартияПроизводства
	|	И РаспоряженияСИсправлением.Распоряжение = Остатки.Распоряжение
	|	И РаспоряженияСИсправлением.КодСтрокиРаспоряжения = Остатки.КодСтрокиРаспоряжения
	|
	|ГДЕ
	// Исключаем распоряжения с исправлениями.
	|	РаспоряженияСИсправлением.ПартияПроизводства ЕСТЬ NULL
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Распоряжения с исправлениями.
	|ВЫБРАТЬ
	|	Обороты.Организация КАК Организация,
	|	Обороты.КоличествоОборот КАК КоличествоОстаток
	|ИЗ
	|	Обороты КАК Обороты
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РаспоряженияСИсправлением КАК РаспоряженияСИсправлением
	|	ПО РаспоряженияСИсправлением.Организация = Обороты.Организация
	|	И РаспоряженияСИсправлением.ПартияПроизводства = Обороты.ПартияПроизводства
	|	И РаспоряженияСИсправлением.Распоряжение = Обороты.Распоряжение
	|	И РаспоряженияСИсправлением.КодСтрокиРаспоряжения = Обороты.КодСтрокиРаспоряжения
	|ГДЕ
	|	НЕ Обороты.КоличествоОборот = 0
	|";
...
#Область ПриемкаВнутреннихРабот
...
// 	ТекущийРодитель - Строка - идентификатор группы.
//
Процедура ДобавитьЭтап_ПриемкаВнутреннихРабот(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	
	НоваяСтрока =  ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(
		ТаблицаЭтапов,
		ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ПриемкаВнутреннихРабот);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Приемка внутренних работ';
											|en = 'Internal work acceptance'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
	НоваяСтрока.ДействиеОформление =  ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
		"ПроизводствоСервер.Оформление_ПриемкаВнутреннихРабот");
	НоваяСтрока.ДействиеИспользование =  ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
		"ПроизводствоСервер.Использование_ПриемкаВнутреннихРабот");
	
	НоваяСтрока.ДействиеВыполнить =
		ЗакрытиеМесяцаСервер.ОписаниеДействия_ОткрытьФорму(
			Метаданные.Обработки.ЖурналДокументовПроизводства.Формы.СписокДокументов.ПолноеИмя(),
			Истина);
	
	НоваяСтрока.ДействиеВыполнить.ПараметрыФормы.Вставить("КлючНазначенияФормы", "ЗакрытиеМесяца");
	
	ОтборТипыДокументов = Новый СписокЗначений;
	ОтборТипыДокументов.Добавить(ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Тип("ДокументСсылка.АктВыполненныхВнутреннихРабот")));
	НоваяСтрока.ДействиеВыполнить.ПараметрыФормы.Отбор.Вставить("ОтборТипыДокументов", ОтборТипыДокументов);
	
	ОтборХозяйственныеОперации = Новый СписокЗначений;
	ОтборХозяйственныеОперации.Добавить(Перечисления.ХозяйственныеОперации.ПриемПередачаРаботМеждуПодразделениями);
	ОтборХозяйственныеОперации.Добавить(Перечисления.ХозяйственныеОперации.ПриемПередачаРаботМеждуФилиалами);
	НоваяСтрока.ДействиеВыполнить.ПараметрыФормы.Отбор.Вставить("ОтборХозяйственныеОперации", ОтборХозяйственныеОперации);
	
	НоваяСтрока.ДействиеВыполнить.ПараметрыФормы.Отбор.Вставить("Статус", Перечисления.СтатусыАктовВыполненныхВнутреннихРабот.Выставлен);
...
// 	ПараметрыОбработчика - см. ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыОбработчикаЭтапаЗакрытияМесяцаДляПроверки
//
Процедура Использование_ПриемкаВнутреннихРабот(ПараметрыОбработчика) Экспорт
	...
	|	РеквизитыДокумента.Документ    КАК Документ
	|ПОМЕСТИТЬ ПриемкаВнутреннихРабот
	|ИЗ
	|	(ВЫБРАТЬ
	|		РеквизитыДокумента.Организация КАК Организация,
	|		РеквизитыДокумента.Ссылка      КАК Документ
	|	ИЗ
	|		Документ.АктВыполненныхВнутреннихРабот КАК РеквизитыДокумента
	|	ГДЕ
	|		РеквизитыДокумента.Проведен
	|		И РеквизитыДокумента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыАктовВыполненныхВнутреннихРабот.Выставлен)
	...
	|	
	|	ВЫБРАТЬ
	|		РеквизитыДокумента.ОрганизацияПолучатель КАК Организация,
	|		РеквизитыДокумента.Ссылка                КАК Документ
	|	ИЗ
	|		Документ.АктВыполненныхВнутреннихРабот КАК РеквизитыДокумента
	|	ГДЕ
	|		РеквизитыДокумента.Проведен
	|		И РеквизитыДокумента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыАктовВыполненныхВнутреннихРабот.Выставлен)
	|		И РеквизитыДокумента.ОрганизацияПолучатель В (&МассивОрганизаций)
	|		И РеквизитыДокумента.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	|	) КАК РеквизитыДокумента
	|
	|СГРУППИРОВАТЬ ПО
	...
	Если РазмерыВременныхТаблиц.ПриемкаВнутреннихРабот = 0 Тогда
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru = 'Нет актов внутренних работ, требующих приемки.';
				|en = 'There are no internal works acceptance to take over.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	КонецЕсли;
...
// 	ПараметрыОбработчика - см. ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыОбработчикаЭтапаЗакрытияМесяцаДляПроверки
//
Процедура Оформление_ПриемкаВнутреннихРабот(ПараметрыОбработчика) Экспорт
	
	ПараметрыОбработчика.ДанныеЭтапа.ТекстВыполнить = НСтр("ru = 'Принять';
															|en = 'Accept'");
...
//
Процедура ОписаниеПроверок_ПриемкаВнутреннихРабот(ТаблицаПроверок) Экспорт
	
	ОписаниеПроверки = ЗакрытиеМесяцаСервер.ДобавитьОписаниеНовойПроверки(
		ТаблицаПроверок,
		"ПриемкаВнутреннихРабот",
		Справочники.ОперацииЗакрытияМесяца.ПриемкаВнутреннихРабот,
		Перечисления.МоментЗапускаПроверкиОперацииЗакрытияМесяца.ДоРасчета,
		"ПроизводствоСервер.ПроверкаПриемкаВнутреннихРабот");
	
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Не приняты акты внутренних работ.';
			|en = 'Internal works acceptance is not performed.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Все выставленные акты выполненных внутренних работ должны быть приняты, либо отменены.';
			|en = 'All issued internal works acceptance must be performed or cancelled.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
//
Процедура ПроверкаПриемкаВнутреннихРабот(ПараметрыПроверки) Экспорт
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		"ПриемкаВнутреннихРабот",
		НСтр("ru = 'Обнаружены акты внутренних работ по организации ""%1"" за %2, требующие приемки.';
			|en = 'The internal work acceptance documents of the ""%1"" company for %2 that require acceptance are found.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
	...
	Для каждого Строка Из ТабличнаяЧасть Цикл
		...
		Если Итоги = Неопределено Тогда
			...
			Итоги.Вставить("ДоляСтоимостиОбязательна", ДоляСтоимостиРассчитывается
				И Не ПараметрыРаспределенияЗатрат.УстановитьДоляСтоимостиОбязательнаПоУмолчанию);
...
#Область ИнформированиеФункциональностьУстарела

//++ Устарело_Производство21
	
// Предупреждение о скором ограничении функциональности подсистемы 2.1
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения -
//  ГруппаРодитель - ГруппаФормы -
//  
// Возвращаемое значение:
//  ГруппаФормы -
Функция ДобавитьПредупреждениеПодсистема21Устарела(Форма, ГруппаРодитель = Неопределено) Экспорт
	
	ТекстНадписи = НСтр("ru = 'Внимание! Функциональность ""Управление производством 2.1"" устарела, и будет удалена в версии 2.6.3.';
						|en = 'Important. Production Management 2.1. is outdated and will be removed in version 2.6.3.'");
	ТекстНадписи = ТекстНадписи + " " + НСтр("ru = 'Пожалуйста, перейдите на ""Управление производством 2.2"".';
											|en = 'Please switch to Production Management 2.2.'");
	
	Возврат ДобавитьПредупреждениеФункциональностьУстарела(Форма, ТекстНадписи, ГруппаРодитель);
	
КонецФункции

//-- Устарело_Производство21

// Предупреждение о скором ограничении функциональности маршрутных карт
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения -
//  ГруппаРодитель - ГруппаФормы -
...
// Возвращаемое значение:
//  ГруппаФормы -
Функция ДобавитьПредупреждениеМаршрутныеКартыУстарели(Форма, ГруппаРодитель = Неопределено) Экспорт
	
	//++ Устарело_Производство21
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользуетсяТолькоУправлениеПроизводством22") Тогда
		Возврат ДобавитьПредупреждениеПодсистема21Устарела(Форма, ГруппаРодитель);
	КонецЕсли;
	//-- Устарело_Производство21
	
	ТекстНадписи = НСтр("ru = 'Внимание! Функциональность ""Маршрутных карт"" устарела и будет удалена в версии 2.6.3.';
						|en = 'Important. Route sheets are outdated and will be removed in version 2.6.3.'");
	ТекстНадписи = ТекстНадписи + " "
	+ НСтр("ru = 'Для сохранения данных, воспользуйтесь инструментом автоматического переноса операций в справочник ""Ресурсные спецификации"", в котором теперь предусмотрено пооперационное описание производственного процесса.';
			|en = 'To save the data, use the automatic operation transfer tool in the ""Bills of materials"" catalog, which now describes the production process by operations.'");
	
	Возврат ДобавитьПредупреждениеФункциональностьУстарела(Форма, ТекстНадписи, ГруппаРодитель);
	
КонецФункции

Функция ДобавитьПредупреждениеФункциональностьУстарела(Форма, ТекстНадписи, ГруппаРодитель = Неопределено)
	
	ПрефиксЭлементов = "ФункциональностьУстарела";
	
	Если Форма.Элементы.Найти("Группа"+ПрефиксЭлементов) <> Неопределено Тогда
		
		Возврат Неопределено;
	
	ИначеЕсли ГруппаРодитель = Неопределено Тогда
		
		Если НЕ Форма.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная Тогда
			Возврат Неопределено; // если не указана группа размещения, автоматически размещаем только в вертикальных формах 
		КонецЕсли;
		
		Для каждого ТекущийЭлемент Из Форма.Элементы Цикл
			Если ТекущийЭлемент <> Форма.КоманднаяПанель Тогда
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		Если ТекущийЭлемент = Неопределено Тогда
			НоваяГруппа = Форма.Элементы.Добавить("Группа"+ПрефиксЭлементов, Тип("ГруппаФормы"));
		Иначе
			НоваяГруппа = Форма.Элементы.Вставить("Группа"+ПрефиксЭлементов, Тип("ГруппаФормы"),,ТекущийЭлемент);
		КонецЕсли;
		
	Иначе
		
		НоваяГруппа = Форма.Элементы.Добавить("Группа"+ПрефиксЭлементов, Тип("ГруппаФормы"), ГруппаРодитель);
	...
	НовыйЭлемент = Форма.Элементы.Добавить(ПрефиксЭлементов+"Декорация1", Тип("ДекорацияФормы"), НоваяГруппа);
	...
	НоваяГруппа = Форма.Элементы.Добавить("Группа"+ПрефиксЭлементов+"Предупреждение", Тип("ГруппаФормы"), НоваяГруппа);
	НоваяГруппа.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	...
	НовыйЭлемент = Форма.Элементы.Добавить(ПрефиксЭлементов+"Декорация2", Тип("ДекорацияФормы"), НоваяГруппа);
	НовыйЭлемент.Вид = ВидДекорацииФормы.Надпись;
	НовыйЭлемент.Заголовок = ТекстНадписи;
	НовыйЭлемент.АвтоМаксимальнаяШирина = Ложь;
	НовыйЭлемент.ЦветТекста = WebЦвета.Кирпичный;
	
	Возврат НоваяГруппа; // Для добавления дополнительных элементов
