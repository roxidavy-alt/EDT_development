...
Процедура УстановитьВидимостьСтранаРеализации(Форма) Экспорт
	...
	//++ НЕ УТ
	УчетНДСРФ.УстановитьВидимостьСтранаРеализации(Форма);
	//-- НЕ УТ
...
Процедура ДобавитьЭтап_РаспределениеНДС(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	//++ НЕ УТКА
	ПредшествующийЭтап = Справочники.ОперацииЗакрытияМесяца.РасчетДетализацииПродукции;
	//-- НЕ УТКА
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Распределение НДС';
											|en = 'VAT allocation'"; // @НСтр-1
...
Процедура ДобавитьЭтап_ФормированиеДвиженийПоНДС(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Формирование движений по НДС';
											|en = 'Generate VAT records'"; // @НСтр-1
...
Процедура УстановитьСвойстваФормыВыбораДляСтатформыВФТС(Форма) Экспорт
	...
	Если Параметры.Статформа Тогда
		...
		Если ИмяФормы = "Документ.РеализацияТоваровУслуг.Форма.ФормаВыбора" Тогда
			Элементы[ИмяСписка + "КлиентКонтрагент"].Видимость = Истина;
		КонецЕсли;
		
		Элементы[ИмяСписка + "СтранаРеализации"].Видимость = Истина;
		Элементы[ИмяСписка + "НалогообложениеНДС"].Видимость = Истина;
		
		Форма.СтранаРеализации = Параметры.СтранаРеализации;
		Элементы.СтранаРеализации.РежимВыбораИзСписка = Истина;
		УчетНДСРФ.ЗаполнитьСписокСтранУчастниковЕАЭС(Элементы.СтранаРеализации.СписокВыбора);
		
		Элементы.Налогообложение.РежимВыбораИзСписка = Истина;
		УчетНДСРФ.ЗаполнитьСписокНалогообложенийНДСДляСтатформыВФТС(Элементы.Налогообложение.СписокВыбора);
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Форма[ИмяСписка],
			"НалогообложениеНДС",
			Элементы.Налогообложение.СписокВыбора,
			ВидСравненияКомпоновкиДанных.ВСписке,
			,
			Истина);
		
		Период = Параметры.НачалоПериода;
...
Функция ОписаниеПараметровОперацииЗакрытияМесяца() Экспорт
	...
	ОписаниеПараметра.Описание =
		НСтр("ru = 'При расчете партий НДС могут возникать погрешности округления, из-за которых в регистре “НДС предъявленный” могут оставаться копеечные остатки.
              ...
              |If you specify a non-zero threshold for writing off a rounding error, then all balances within this threshold will be written off as expenses to the ""Goods cost variance"" item.
