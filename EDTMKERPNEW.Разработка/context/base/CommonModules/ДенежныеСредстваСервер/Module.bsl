	...
	Пока Выборка.Следующий() Цикл
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Организация в %1 в строке %2 списка ""Расшифровка платежа"" не соответствует организации документа %3';
				|en = 'Company in %1 in line %2 of the ""Payment details"" list does not match the company mentioned in document %3 '"),
			ТекстДокумент,
			Выборка.НомерСтроки,
			ДокументОбъект.Организация);
		Если ФлагОбменСБанками Тогда
				...
				Если СтрокаПроверки.ЕстьОтличияВОрганизации Тогда
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Организация документа %1 не соответствует организации в %2';
							|en = 'Company mentioned in the document %1 does not match the company in %2'"),
						ДокументОбъект.Организация,
		...
		Если Реквизиты.Организация <> ДокументОбъект.Организация Тогда
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Организация в распоряжении не соответствует организации документа %1';
					|en = 'Company in the reference does not correspond to the company mentioned in document %1'"),
				ДокументОбъект.Организация);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				Текст,
				ДокументОбъект,
			...
			Если ДокументОбъект.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзДругойОрганизации
				...
				И ДокументОбъект.Организация <> РеквизитыКассы.Владелец Тогда
					...
					Текст = НСтр("ru = 'Организация кассы отправителя не соответствует организации документа';
								|en = 'The company mentioned in the withdrawing cash account does not correspond to the company mentioned in the document'");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						Текст,
						ДокументОбъект,
						"КассаОтправитель",
			...
			ИначеЕсли (ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзДругойОрганизации
				...
				И ДокументОбъект.Организация = РеквизитыКассы.Владелец Тогда
					...
					Текст = НСтр("ru = 'Организация кассы отправителя должна отличаться от организации документа';
								|en = 'The company mentioned in the withdrawing cash account should differ from the company mentioned in the document'");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						Текст,
						ДокументОбъект,
						"КассаОтправитель",
	...
	Тогда
		...
		Если ДокументОбъект.Организация <> РеквизитыКассыККМ.Организация Тогда
			Текст = НСтр("ru = 'Организация кассы ККМ не соответствует организации документа';
						|en = 'Cash register company does not match the company mentioned in the document'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				Текст,
				ДокументОбъект,
				"КассаККМ",
		...
		Иначе
			...
			Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВнутренняяПередачаДенежныхСредств
				...
				Текст = НСтр("ru = 'Организация счета получателя должна быть взаимосвязана с организацией счета отправителя по организационной структуре.';
							|en = 'Companies mentioned in the beneficiary account and in the payer account must be interrelated in terms of company structure.'");
				Если ФлагОбменСБанками Тогда
					ДобавитьОшибкуЗаполнения(ОшибкиЗаполнения, Текст);
				Иначе
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			...
			ИначеЕсли ДокументОбъект.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ВнутренняяПередачаДенежныхСредств
				...
				Текст = НСтр("ru = 'Организация счета получателя не должна быть взаимосвязана с организацией счета отправителя по организационной структуре.';
							|en = 'Companies mentioned in the beneficiary account and in the payer account must not be interrelated in terms of company structure.'");
				Если ФлагОбменСБанками Тогда
					ДобавитьОшибкуЗаполнения(ОшибкиЗаполнения, Текст);
				Иначе
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
...
Процедура ДобавитьЭтап_ПроверкаКурсовЗачетаВАвансовыхОтчетах(ТаблицаЭтапов, ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Проверка курсов зачета в авансовых отчетах';
											|en = 'Check offset rates in expense reports'"; // @НСтр-1
...
Функция ДанныеДокумента(ДокументСсылка, ТипРасчетов, ВыбранныеРеквизиты, ПолучитьСуммыВзаиморасчетов = Ложь) Экспорт
	...
	Если СтруктураРеквизитов = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не определены параметры взаиморасчетов документа.';
								|en = 'The document AR/AP accounting parameters are not identified.'");
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Не определены данные документа.';
								|en = 'The document data is not identified.'");
	КонецЕсли;
...
Функция НазначениеПлатежаДокументаОперации(ДокументСсылка, ДанныеДокумента, ТаблицаСумм) Экспорт
	...
	Если ДанныеДокумента.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов
		...
		Если ЗначениеЗаполнено(ДатаОснования)
			...
			ПредставлениеОснования = СтрШаблон(НСтр("ru = 'Оплата по договору №%1 от %2';
													|en = 'Payment under contract No. %1 dated %2'"), НомерОснования, ДатаОснования);
		Иначе
			ПредставлениеОснования = НСтр("ru = 'Оплата по договору';
											|en = 'Payment under contract'");
		КонецЕсли;
	...
	Иначе
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПредставленияДокументов, ИмяДокумента) Тогда
			ПредставлениеОснования = СтрШаблон(НСтр("ru = 'Оплата по %1 №%2 от %3';
													|en = 'Payment under %1 No. %2 dated %3'"), ПредставленияДокументов[ИмяДокумента], НомерОснования, ДатаОснования);
		Иначе
			ПредставлениеОснования = СтрШаблон(НСтр("ru = 'Оплата по накладной №%1 от %2';
													|en = 'Payment under invoice No. %1 dated %2'"), НомерОснования, ДатаОснования);
		КонецЕсли;
...
Функция НадписьВалютыИКурсПлатежа(Форма) Экспорт
	...
	Возврат СтрШаблон("%1: %2", НСтр("ru = 'Расчеты';
									|en = 'AR/AP accounting'"), СтрСоединить(СтрокиНадписи, "; "));
...
Процедура ПолучитьДоступныеХозяйственныеОперацииПоОснованию(ДокументОснование, ФормаОплаты, СписокОпераций = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументОснование) Тогда
		...
		ИначеЕсли ОперацияОснования = Перечисления.ХозяйственныеОперации.ПроизводствоИзДавальческогоСырья2_5
			Или ОперацияОснования = Перечисления.ХозяйственныеОперации.ПроизводствоПродукцииИзДавальческогоСырья2_5
			Или ОперацияОснования = Перечисления.ХозяйственныеОперации.РемонтПродукцииИзДавальческогоСырья2_5
			Или ОперацияОснования = Перечисления.ХозяйственныеОперации.ВыполнениеРаботСИспользованиемДавальческогоСырья2_5
			Или ОперацияОснования = Перечисления.ХозяйственныеОперации.ВыполнениеРемонтныхРаботСИспользованиемДавальческогоСырья2_5 Тогда
			СписокОпераций.Добавить(Перечисления.ХозяйственныеОперации.ОплатаПоставщику);
			СписокОпераций.Добавить(Перечисления.ХозяйственныеОперации.ПоступлениеОплатыОтКлиента);
		//-- НЕ УТКА
...
Функция НаименованиеПлательщикаПолучателяПлатежа(Наименование, ПрямыеРасчеты, НомерСчета, Банк, Город) Экспорт
	...
	Иначе
		...
		ИначеЕсли ПустаяСтрока(Банк) Тогда
			Результат = СтрШаблон(
				НСтр("ru = '%1 р/с %2';
					|en = '%1 account %2'"), Наименование, СокрЛП(НомерСчета));
		ИначеЕсли ПустаяСтрока(Город) Тогда
			Результат = СтрШаблон(
				НСтр("ru = '%1 р/с %2 в %3';
					|en = '%1 account %2 in %3'"), Наименование, СокрЛП(НомерСчета), СокрЛП(Банк));
		Иначе
			Результат = СтрШаблон(
				НСтр("ru = '%1 р/с %2 в %3 %4';
					|en = '%1 account %2 in %3 %4'"), Наименование, СокрЛП(НомерСчета), СокрЛП(Банк), СокрЛП(Город));
		КонецЕсли;
...
Функция ПолучитьПредставленияДокументов()
	...
	ВидыДокументовКонтрагента.Вставить("ЗаказДавальца2_5",                     НСтр("ru = 'акту';
																					|en = 'certificate'"));
	ВидыДокументовКонтрагента.Вставить("ОтчетДавальцу2_5",                     НСтр("ru = 'акту';
																					|en = 'certificate'"));
	//++ Устарело_Переработка24
	ВидыДокументовКонтрагента.Вставить("ЗаказДавальца",                        НСтр("ru = 'акту';
																					|en = 'certificate'"));
	ВидыДокументовКонтрагента.Вставить("ОтчетДавальцу",                        НСтр("ru = 'акту';
																					|en = 'certificate'"));
	//-- Устарело_Переработка24
	...
	Иначе
		...
		Форма.ГиперссылкаРеквизитыОбъектаРасчетов =
			СтрШаблон(НСтр("ru = 'Без указания объекта расчетов: %1 %2';
							|en = 'Без указания объекта расчетов: %1 %2'"), Формат(СуммаНеРаспределено, "ЧДЦ=2"), Объект.Валюта);
...
Функция ТекстЗапросаСуммПолнойОплаты(Документ)
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаказДавальца") Тогда
		ТекстЗапроса = 
			...
			|ПОМЕСТИТЬ СуммыПолнойОплаты
			|ИЗ
			|	Документ.ЗаказДавальца.Продукция КАК ДанныеТабличнойЧасти
			|ГДЕ
			|	ДанныеТабличнойЧасти.Ссылка = &ДокументОперации
			|	И НЕ ДанныеТабличнойЧасти.Отменено
			|";
		
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ОтчетДавальцу") Тогда
		ТекстЗапроса = 
			"ВЫБРАТЬ
			|	ЛОЖЬ КАК ЭтоЗалогЗаТару,
			|	ДанныеТабличнойЧасти.СтавкаНДС КАК СтавкаНДС,
			...
			|ПОМЕСТИТЬ СуммыПолнойОплаты
			|ИЗ
			|	Документ.ОтчетДавальцу.Продукция КАК ДанныеТабличнойЧасти
			|ГДЕ
			|	ДанныеТабличнойЧасти.Ссылка = &ДокументОперации
			|СГРУППИРОВАТЬ ПО
			|	ДанныеТабличнойЧасти.СтавкаНДС
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаказДавальца2_5") Тогда
		ТекстЗапроса = 
			...
			|ПОМЕСТИТЬ СуммыПолнойОплаты
			|ИЗ
			|	Документ.ЗаказДавальца2_5.Продукция КАК ДанныеТабличнойЧасти
			|ГДЕ
			|	ДанныеТабличнойЧасти.Ссылка = &ДокументОперации
			|	И НЕ ДанныеТабличнойЧасти.Отменено
			|";
		
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ОтчетДавальцу2_5") Тогда
		ТекстЗапроса = 
			"ВЫБРАТЬ
			|	ЛОЖЬ КАК ЭтоЗалогЗаТару,
			|	НЕОПРЕДЕЛЕНО КАК СтавкаНДС,
			...
			|ПОМЕСТИТЬ СуммыПолнойОплаты
			|ИЗ
			|	Документ.ОтчетДавальцу2_5.Продукция КАК ДанныеТабличнойЧасти
			|ГДЕ
			|	ДанныеТабличнойЧасти.Ссылка = &ДокументОперации
			|";
		
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ОтчетДавальцуМеждуОрганизациями") Тогда
		ТекстЗапроса = 
			"ВЫБРАТЬ
			|	ЛОЖЬ КАК ЭтоЗалогЗаТару,
			|	ДанныеТабличнойЧасти.СтавкаНДС КАК СтавкаНДС,
			...
			|ПОМЕСТИТЬ СуммыПолнойОплаты
			|ИЗ
			|	Документ.ОтчетДавальцуМеждуОрганизациями.Товары КАК ДанныеТабличнойЧасти
			|ГДЕ
			|	ДанныеТабличнойЧасти.Ссылка = &ДокументОперации
			|СГРУППИРОВАТЬ ПО
			|	ДанныеТабличнойЧасти.СтавкаНДС
