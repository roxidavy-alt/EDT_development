...
Процедура ЗаполнитьСтатусыУказанияСерий(Объект,
	...
	Если ПараметрыУказанияСерий.ТоварВШапке Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			//++ НЕ УТКА
			Если ПараметрыУказанияСерий.СкладскиеОперации.Найти(Перечисления.СкладскиеОперации.РемонтПродукции) <> Неопределено Тогда
				СтатусыПоля.Добавить(2);
				СтатусыПоля.Добавить(21);
			КонецЕсли;
			//-- НЕ УТКА
					...
					Если Выборка.ОшибкаКоличества Тогда
						Если Выборка.ЭтоТоварноеМесто Тогда
							ШаблонСообщения = НСтр("ru = 'Политика учета серий товара ""%1"" предусматривает, что количество товарных мест будет соответствовать количеству товара равному 1, т.е. %2';
													|en = 'According to the batch accounting policy of the ""%1"" item, the number of cargo items matches the number of items equal to 1, that is, %2'");
...
Процедура УстановитьУсловноеОформлениеНоменклатурыДругогоКачества(Форма, ПараметрыУсловногоОформления = Неопределено) Экспорт
	...
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<укажите товар исходного качества>';
																					|en = '<specify the goods of original quality>'"));
	...
	Иначе
		...
		Если ПараметрыПроверки.ЭтоПроверкаТекущейСтроки Тогда 
			...
			ШаблонОшибкаКоличества = НСтр("ru = 'Не заполнено поле ""%ПредставлениеКолонки%""';
											|en = 'The ""%ПредставлениеКолонки%"" field is not filled in'");
	...
	Иначе
		...
		Иначе 
			ШаблонОшибкаОкругления = НСтр("ru = 'В строке %НомерСтроки% списка ""%ПредставлениеТЧ%"" количество в ""%Упаковка%"" некратно количеству в ""%БазоваяЕдиницаИзмерения%"". Отклонение - %КоличествоОтклонение% %Упаковка%.';
											|en = 'Quantity in ""%Упаковка%"" is not a multiple of quantity  in ""%БазоваяЕдиницаИзмерения%"" in line %НомерСтроки% of list ""%ПредставлениеТЧ%"". Variance - %КоличествоОтклонение% %Упаковка%.'");
		КонецЕсли;
	...
	Пока ВыборкаНезаполненные.Следующий() Цикл
		...
		Если ВыборкаНезаполненные.ЭтоТоварноеМесто Тогда
			...
			ТекстСообщения = НСтр("ru = 'В строке %НомерСтроки% для товара ""%ПредставлениеТовара%"" серии не заполнены, т.к. указана упаковка - товарное место. Заполнение серий FEFO по товарным местам не поддерживается.';
									|en = 'Batches are not filled in for the ""%ПредставлениеТовара%"" goods item in line %НомерСтроки%. Cargo item is specified as packaging unit. Cannot fill in FEFO batch according to the cargo items.'");
...
//  ВидСтрокиСпецификации	- ПеречислениеСсылка.ВидыСтрокСпецификации - Проверка для указанного вида строки спецификации (если не указано, то для всех видов строк).
// 
// Возвращаемое значение:
...
Функция ИспользуетсяСправочноеУказаниеСерийВМаршрутныхЛистах(Спецификация = Неопределено, ВидСтрокиСпецификации = Неопределено) Экспорт
	...
	|			ИЛИ &ВидСтрокиСпецификации = ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСпецификации.ПустаяСсылка))";
...
Функция ДанныеДляЗаполненияСерийПоFEFOВЭтапе(Объект, ПараметрыУказанияСерий, Запрос)
	...
	|		РегистрНакопления.ТоварыНаСкладах КАК ТоварыНаСкладах
	|	ГДЕ
	|		(ТоварыНаСкладах.Регистратор, ТоварыНаСкладах.Склад, ТоварыНаСкладах.Номенклатура, ТоварыНаСкладах.Характеристика, ТоварыНаСкладах.Назначение, ТоварыНаСкладах.КонтролируемаяПартия) В
	|				(ВЫБРАТЬ
