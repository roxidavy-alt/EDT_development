...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	ТекстыОтражения.Добавить(ТекстОтклоненияПрошлыхПериодовСписаниеНаРасходы());
	ТекстыОтражения.Добавить(ТекстОтклоненияПрошлыхПериодовНормируемыеРасходы());
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	ТекстыЗапроса.Добавить(РеглУчетВыборкиСерверПовтИсп.ТекстКосвенныеРасходы());
...
Функция ТекстВыпускСобственнойПродукцииФактическаяСтоимостьМатериальныеЗатраты() Экспорт
	...
	|	""РегистраторРасчетаСебестоимостиЛокализация_1199"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстВыпускСобственнойПродукцииФактическаяСтоимостьПостатейныеРасходы() Экспорт
	...
	|
	|	""Выпуск собственной продукции (фактическая стоимость)"" КАК Содержание,
	|	""РегистраторРасчетаСебестоимостиЛокализация_1200"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстВыпускСобственнойПродукцииФактическаяСтоимостьТрудозатраты() Экспорт
	...
	|
	|	""Выпуск собственной продукции (фактическая стоимость)"" КАК Содержание,
	|	""РегистраторРасчетаСебестоимостиЛокализация_1201"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстВыпускСобственнойПродукцииОтклонениеНаСебестоимостьПродаж() Экспорт
	...
	|	""РегистраторРасчетаСебестоимостиЛокализация_1202"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстВыпускСобственнойПродукцииНаСкладОтклонение() Экспорт
	...
	|	""РегистраторРасчетаСебестоимостиЛокализация_1203"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстВыпускПродукцииСоСписаниемНаРасходыОтклонение() Экспорт
	...
	|	""РегистраторРасчетаСебестоимостиЛокализация_1204"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстВыпускПродукцииСоСписаниемНаАктивыПассивыОтклонение() Экспорт
	...
	|	""РегистраторРасчетаСебестоимостиЛокализация_1205"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстОтклоненияВСтоимостиФактуровкаПоставки() Экспорт
	...
	|	""РегистраторРасчетаСебестоимостиЛокализация_1206"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
Функция ТекстОтклоненияВСтоимостиТовараНаСебестоимостьПродаж() Экспорт
	...
	|	""РегистраторРасчетаСебестоимостиЛокализация_1207"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстОтклоненияВСтоимостиТовараНаПрочиеРасходы() Экспорт
	...
	|	""РегистраторРасчетаСебестоимостиЛокализация_1208"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстОтклоненияВСтоимостиТовараНаПрочиеДоходы() Экспорт
	...
	|	""РегистраторРасчетаСебестоимостиЛокализация_1209"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстОтклоненияВСтоимостиТовараНаАктивыПассивы() Экспорт
	...
	|	""РегистраторРасчетаСебестоимостиЛокализация_1210"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстПеремещениеОтклоненийМеждуАналитиками() Экспорт
	...
	|	""РегистраторРасчетаСебестоимостиЛокализация_1211"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстПеремещениеОтклоненийСОбособленногоПодразделения() Экспорт
	...
	|	""РегистраторРасчетаСебестоимостиЛокализация_1212"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстПеремещениеОтклоненийНаОбособленноеПодразделение() Экспорт
	...
	|	""РегистраторРасчетаСебестоимостиЛокализация_1213"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстСписаниеПостатейныхРасходовНаВыпускПродукцииОтклонение() Экспорт
	...
	|	""РегистраторРасчетаСебестоимостиЛокализация_1214"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
// Функция возвращает текст фрагмента отражения отклонения прошлых периодов списание на расходы.
//
// Возвращаемое значение:
//	Строка - сформированный текст запроса.
//
Функция ТекстОтклоненияПрошлыхПериодовСписаниеНаРасходы() Экспорт
	...
	|	КосвенныеРасходы.Организация КАК Организация,
	|	КосвенныеРасходы.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|
	|	СУММА(КосвенныеРасходы.СуммаРеглОСНО) КАК Сумма,
	|	СУММА(КосвенныеРасходы.СуммаУпрОСНО) КАК СуммаУУ,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.РасходыТекущегоПериода) КАК ВидСчетаДт,
	...
	|	""РаспределениеПрочихЗатратЛокализация_1957"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|	ПО 
	|		ТаблицаНалогообложенияПрибыли.Период = ДокументыКОтражению.Период
	...
	|	КосвенныеРасходы.ИдентификаторФинЗаписи,
...
// Функция возвращает текст фрагмента отражения отклонения прошлых периодов на нормируемые расходы.
//
// Возвращаемое значение:
//	Строка - сформированный текст запроса.
//
Функция ТекстОтклоненияПрошлыхПериодовНормируемыеРасходы() Экспорт
	...
	|	""РаспределениеПрочихЗатратЛокализация_1958"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
// Функция возвращает текст запроса для получения временной таблицы ОтклоненияВСтоимостиТоваров.
//
// Возвращаемое значение:
//	Строка - сформированный текст запроса.
//
...
// Функция возвращает текст запроса для получения временной таблицы ВыручкаИСебестоимостьПродаж.
//
// Возвращаемое значение:
//	Строка - сформированный текст запроса.
//
Функция ТекстЗапросаВыручкаИСебестоимостьПродаж() Экспорт
	...
	|	Строки.ИдентификаторФинЗаписи КАК ИдентификаторФинЗаписи,
	|	ВЫБОР
	|		КОГДА Строки.НалогообложениеНДС В (ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД),
