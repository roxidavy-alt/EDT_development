	...
	ИначеЕсли МетаданныеДокументаОснования = Метаданные.Документы.ПроизводственнаяОперацияВЕТИС Тогда
		...
		Если МетаданныеДокументаИСМП = Метаданные.Документы.МаркировкаТоваровИСМП Тогда
			...
			ТекстЗапроса = ТекстЗапросаТоварыДокументаОснования(МетаданныеДокументаОснования, ТабличныеЧастиДокумента,
				"РегистрНоменклатура.КлючВидовПродукции.ВидПродукцииГИСМТ <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)
				|И РегистрНоменклатура.КлючВидовПродукции.ВидПродукцииВЕТИС <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)");
	...
	ИначеЕсли МетаданныеДокументаОснования = Метаданные.Документы.ЭтапПроизводства2_2 Тогда
		...
		Если МетаданныеДокументаЗЕРНО = Метаданные.Документы.СписаниеПартийЗЕРНО Тогда
			...
			МатериалыИРаботыРасход.ДопОтборы_Общие = 
				"И РегистрНоменклатура.КлючВидовПродукции.ВидПродукцииЗЕРНО <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)"
...
Функция ВДокументеДругиеВидыПродукцииИНетЗерна(ИмяДокумента, ДопОтборТовары = "")
	...
	|	МАКСИМУМ(ДанныеПродукции.ВидПродукцииЗЕРНО = ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)) КАК ДругиеОсобенностиУчета,
	|	МАКСИМУМ(ДанныеПродукции.ВидПродукцииЗЕРНО <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)) КАК ЕстьЗерно
	|ИЗ
	|	(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		ТаблицаТовары.Ссылка КАК Ссылка,
	|		ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииЗЕРНО КАК ВидПродукцииЗЕРНО
	|	ИЗ
