...
Процедура ДобавитьЭтап_РаспределениеРасходовБудущихПериодов(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Распределение расходов будущих периодов';
											|en = 'Deferred expense allocation'"; // @НСтр-1
	...
	Если РазмерыВременныхТаблиц.ВТНераспределенныеРБП = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Нет незакрытых оборотов по статьям расходов с вариантом распределения ""На расходы будущих периодов""';
				|en = 'No unclosed turnovers for expense item with the ""To deferred expenses"" allocation option'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаНераспределенныеРасходыБудущихПериодов(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("СуммаРегл",		 		НСтр("ru = 'Сумма (БУ)';
															|en = 'Amount (LA)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//++ НЕ УТ
	...
	СписокПолей.Добавить("БезСуммРегл",		 		НСтр("ru = 'Автосумма (БУ)';
															|en = 'Autosum (LA)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//++ НЕ УТ
