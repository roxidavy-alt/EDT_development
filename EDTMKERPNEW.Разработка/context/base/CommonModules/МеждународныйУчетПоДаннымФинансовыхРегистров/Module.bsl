...
//
// Параметры:
//  ПараметрыОтражения - см. ПараметрыОтраженияДвиженийВМеждународномУчете
//
...
//  МетаданныеРегистра - ОбъектМетаданныхРегистрНакопления - метаданные регистра накопления - источника данных
...
//
Функция ПараметрыОтраженияДвиженийВМеждународномУчете(ИмяРегистра, МетаданныеРегистра = Неопределено, РегистрацияКОтражению = Ложь) Экспорт
	
	МенеджерРегистра = РегистрыНакопления[ИмяРегистра]; // РегистрНакопленияМенеджер -
	ПараметрыОтраженияВУчете = МенеджерРегистра.ПараметрыОтраженияДвиженийВФинансовомУчете(МетаданныеРегистра, РегистрацияКОтражению); // см. ФинансовыйУчетПоДаннымБалансовыхРегистров.ПараметрыОтраженияДвиженийВФинансовомУчете
	...
	Для Каждого ОписаниеРеквизита Из ПараметрыОтраженияВУчете.ОписаниеРеквизитов Цикл
		...
		Для Каждого ОборотноеСубконто Из ОборотныеСубконто Цикл
			...
			Иначе
				...
				Для Каждого ТипСубконто Из ТипыСубконто Цикл
					Если ОписаниеПоля.Тип.СодержитТип(ТипСубконто) Тогда
						ДобавитьРеквизитВИсточникиАналитики = Истина;
						Прервать;
					КонецЕсли;
				КонецЦикла;
		...
		Если ДобавитьРеквизитВИсточникиАналитики Тогда
			ПараметрыОтраженияВУчете.ОписаниеИзмерений.Вставить(ИмяПоля, ОбщегоНазначения.СкопироватьРекурсивно(ОписаниеПоля));
			РеквизитыИсточникиАналитики.Добавить(ИмяПоля);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого ИмяПоля Из РеквизитыИсточникиАналитики Цикл
		ПараметрыОтраженияВУчете.ОписаниеРеквизитов.Удалить(ИмяПоля);
	КонецЦикла;
...
// Добавляет описание отбора по типу регистратора в параметры отражения движений регистра накопления
// в международном учете.
// 
// Параметры:
//  ПараметрыОтраженияВУчете - См. ПараметрыОтраженияДвиженийВМеждународномУчете
//
Процедура ДобавитьОтборПоТипуРегистратора(ПараметрыОтраженияВУчете) Экспорт
	
	ОписаниеПоляРегистратор = ПараметрыОтраженияВУчете.ОписаниеРеквизитов.Регистратор;
	
	ОписаниеПоляРегистра = ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных();
	...
	ПараметрыОтраженияВУчете.ОписаниеРеквизитов.Вставить("ТипРегистратора", ОписаниеПоляРегистра);
...
Функция ДанныеРеклассификацииДолгосрочныхАктивовОбязательств(Организации, Период) Экспорт
	...
	Для Каждого ИмяРегистра Из БалансовыеФинансовыеРегистры Цикл
		...
		ПараметрыОтраженияВУчете = ПараметрыОтраженияДвиженийВМеждународномУчете(ИмяРегистра, МетаданныеРегистра);
...
Функция ОрганизацииРеклассификацииДолгосрочныхАктивовОбязательств(Организации, Период) Экспорт
	...
	Для Каждого ВременнаяТаблица Из МенеджерВременныхТаблиц.Таблицы Цикл
		ИмяРегистра = ВременнаяТаблица.ПолноеИмя;
		МетаданныеРегистра = МетаданныеРегистрыНакопления[ИмяРегистра];
		ПараметрыОтраженияВУчете = ПараметрыОтраженияДвиженийВМеждународномУчете(ИмяРегистра, МетаданныеРегистра);
...
// Возвращает описание полей источника данных объекта учета, которые могут быть использованы для заполнения субконто.
// Состав полей заполняется всеми измерениями источника данных и реквизитами, соответствующими оборотным субконто.
// 
// Параметры:
...
//
Функция ИсточникиАналитикиФинансовогоРегистра(ИмяРегистра, ПараметрыОтраженияВУчете = Неопределено) Экспорт
	
	Если ПараметрыОтраженияВУчете = Неопределено Тогда
		ПараметрыОтраженияВУчете = ПараметрыОтраженияДвиженийВМеждународномУчете(ИмяРегистра);
	КонецЕсли;
	
	ИсточникиАналитики = ОбщегоНазначения.СкопироватьРекурсивно(ПараметрыОтраженияВУчете.ОписаниеИзмерений);
	
	Возврат ИсточникиАналитики;
	
КонецФункции

// Возвращает описание всех доступных полей источника данных объекта учета.
// 
// Параметры:
//  ИмяРегистра - Строка - Имя регистра - источника данных
//  ПараметрыОтраженияВУчете - См. ПараметрыОтраженияДвиженийВМеждународномУчете
...
Функция ОписаниеПолейФинансовогоРегистра(ИмяРегистра, ПараметрыОтраженияВУчете = Неопределено) Экспорт
	...
	ОписаниеПолей = ИсточникиАналитикиФинансовогоРегистра(ИмяРегистра, ПараметрыОтраженияВУчете);
	
	ДопустимыеРеквизиты = Новый Массив();
	ДопустимыеРеквизиты.Добавить("Регистратор");
	ДопустимыеРеквизиты.Добавить("ТипРегистратора");
	ДопустимыеРеквизиты.Добавить("НастройкаХозяйственнойОперации");
	
	Для Каждого ИмяПоля Из ДопустимыеРеквизиты Цикл
		ОписаниеПоля = Неопределено;
		Если ПараметрыОтраженияВУчете.ОписаниеРеквизитов.Свойство(ИмяПоля, ОписаниеПоля) Тогда
			ОписаниеПолей.Вставить(ИмяПоля, ОбщегоНазначения.СкопироватьРекурсивно(ОписаниеПоля));
		КонецЕсли;
	КонецЦикла;
...
Функция ПараметрыОтраженияДвиженийФинансовыхРегистров(Документ, РегистрацияКОтражению = Ложь)
	...
	Для Каждого ИмяРегистра Из БалансовыеФинансовыеРегистры Цикл
		...
		МетаданныеРегистра = МетаданныеРегистрыНакопления[ИмяРегистра];
		ПараметрыОтраженияВУчете = ПараметрыОтраженияДвиженийВМеждународномУчете(ИмяРегистра, МетаданныеРегистра, РегистрацияКОтражению);
...
Функция БалансовыеФинансовыеРегистры(ОбъектыФинансовогоУчета = Неопределено, НастройкиУчета = Неопределено)
	
	Если ОбъектыФинансовогоУчета = Неопределено Тогда
		Если НастройкиУчета = Неопределено Тогда
			НастройкиУчета = НастройкиУчетаПоНастройкамФормированияПроводок(
				Справочники.НастройкиФормированияПроводокМеждународногоУчета.НастройкиФормированияПроводок());
		КонецЕсли;
...
Функция ПоляРегистраПоТипуСубконто(ТипЗначенияСубконто, ИмяРегистра)
	...
	ИсточникиАналитики = ИсточникиАналитикиФинансовогоРегистра(ИмяРегистра);
...
Функция ЗапросДанныхФинансовыхРегистров(ПараметрыЗапроса, МенеджерВременныхТаблиц)
	...
	Иначе
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеОрганизация", ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаУсловияОрганизация(Запрос.Параметры));
...
Функция ТекстыЗапросовЗаписейРегистров(Запрос)
	...
	Для Каждого ИмяИсточникаДанных Из БалансовыеФинансовыеРегистры Цикл
		...
		ПараметрыОтраженияДвижений = ПараметрыОтраженияДвиженийВМеждународномУчете(ИмяИсточникаДанных, МетаданныеРегистра);
		...
		Для Каждого СтрокаОбъектаУчета Из СтрокиОбъектовУчета Цикл
			...
			ФинансовыйУчетПоДаннымБалансовыхРегистров.ЗаполнитьИсточникиСоСрокамиОбращения(Запрос.Параметры, СтрокаОбъектаУчета, ПараметрыОтраженияДвижений);
			ФинансовыйУчетПоДаннымБалансовыхРегистров.ЗаполнитьПризнакиУчетаКорреспонденций(Запрос.Параметры, СтрокаОбъектаУчета);
			ФинансовыйУчетПоДаннымБалансовыхРегистров.ЗаполнитьОбъектыНесобственныхЦенностей(Запрос.Параметры, СтрокаОбъектаУчета);
...
Процедура СформироватьКорреспонденцииЗаписейРегистров(Запрос, ТекстыЗапросов)
	...
	ВосстановитьРасчетыПоСторноПриНеобходимости(Запрос, ТекстыЗапросов);
...
Процедура ОпределитьСчетаУчетаОтдельныхКорреспонденций(Запрос, ТекстыЗапросов, ТекстыЗапросовРегистров)
	...
	Пока ВыборкаИмяРегистра.Следующий() Цикл
		...
		Если ИмяТаблицыРегистра <> ФинансовыйУчетПоДаннымБалансовыхРегистров.ИмяРегистраСлужебныхОбъектовУчета() Тогда
			...
			ФинансовыйУчетПоДаннымБалансовыхРегистров.ДобавитьВыборкуАналитикиВТекстЗапроса(
				СтрокиПолейАналитики, СтрокиГруппАналитики, ТекстЗапросаРегистра);
			...
			Если Запрос.Параметры.ВосстановитьРасчетыПоСторно И РегистрыВосстановленияРасчетовПоСторно.Найти(ИмяТаблицыРегистра) <> Неопределено Тогда
	...
	Пока ВыборкаИмяРегистраДт.Следующий() Цикл
		...
		Пока ВыборкаИмяРегистраКт.Следующий() Цикл
			...
			СхемаКомпоновкиДанных = КомпоновкаДанныхСервер.СхемаКомпоновкиДанныхПоТекстуЗапроса(ТекстЗапроса);
			...
			Пока ВыборкаОтборы.Следующий() Цикл
				...
				Если ВыборкаОтборы.ЕстьДополнительныйОтборДт Тогда
					...
					ФинансовыйУчетПоДаннымБалансовыхРегистров.СкопироватьОтборКомпоновкиДанных(ОтборОборота, КомпоновщикНастроекДт.Настройки.Отбор,, "Дт");
				КонецЕсли;
				...
				Если ВыборкаОтборы.ЕстьДополнительныйОтборКт Тогда
					...
					ФинансовыйУчетПоДаннымБалансовыхРегистров.СкопироватьОтборКомпоновкиДанных(ОтборОборота, КомпоновщикНастроекКт.Настройки.Отбор,, "Кт");
				КонецЕсли;
			КонецЦикла;
...
Процедура ВосстановитьРасчетыПоСторноПриНеобходимости(Запрос, ТекстыЗапросов)
	
	Если НЕ Запрос.Параметры.ВосстановитьРасчетыПоСторно Тогда
		Возврат;
	КонецЕсли;
	...
	|			ТаблицаКорреспонденцийДвиженийРегистров КАК ДвиженияДебетКредит
	|		ГДЕ
	|			ДвиженияДебетКредит.Период = РеестрДокументов.ДатаДокументаИБ
	|			И ДвиженияДебетКредит.Регистратор = РеестрДокументов.Ссылка
	|			И ДвиженияДебетКредит.ИмяРегистраДт В (&РегистрыВосстановленияРасчетовПоСторно)
	|			И ДвиженияДебетКредит.ИмяРегистраКт В (&РегистрыВосстановленияРасчетовПоСторно)
	...
	Если ЕстьПорцияДокументов Тогда
		...
		УсловиеОтбораПоОрганизации = "ДвиженияДебетКредит.Организация = ПорцияДокументов.Организация";
	Иначе
		...
		УсловиеОтбораПоОрганизации = "ДвиженияДебетКредит.Организация = РеестрДокументов.Организация";
	КонецЕсли;
	
	СтрокиУсловийОтбораДокументов = Новый Массив();
	
	Если Запрос.Параметры.ДатаНачала > '00010101' Тогда
		СтрокиУсловийОтбораДокументов.Добавить("РеестрДокументов.ДатаДокументаИБ >= &ДатаНачала");
	КонецЕсли;
	
	Если Запрос.Параметры.ДатаОкончания > '00010101' Тогда
		СтрокиУсловийОтбораДокументов.Добавить("РеестрДокументов.ДатаДокументаИБ <= &ДатаОкончания");
	КонецЕсли;
	
	Если ТипЗнч(Запрос.Параметры.Регистратор) = Тип("Массив") Тогда
		СтрокиУсловийОтбораДокументов.Добавить("РеестрДокументов.Ссылка В (&Регистратор)");
	ИначеЕсли ЗначениеЗаполнено(Запрос.Параметры.Регистратор) Тогда
		СтрокиУсловийОтбораДокументов.Добавить("РеестрДокументов.Ссылка = &Регистратор");
	КонецЕсли;
	
	Если СтрокиУсловийОтбораДокументов.Количество() > 0 Тогда
		РазделительСтрок = Символы.ПС + Символы.Таб + "И ";
		УсловияОтбораДокументов = СтрСоединить(СтрокиУсловийОтбораДокументов, РазделительСтрок);
	Иначе
		УсловияОтбораДокументов = "ИСТИНА";
	КонецЕсли;
		
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПолеОрганизация", ПолеВыборкиОрганизации);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловияОтбораДокументов", УсловияОтбораДокументов);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеОтбораПоОрганизации", УсловиеОтбораПоОрганизации);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТаблицаКорреспонденцийДвиженийРегистров", Запрос.Параметры.ТаблицаКорреспонденцийДвиженийРегистров);
	
	ТекстыЗапросов.Добавить(ТекстЗапроса);
	
	Если НЕ ФинансовыйУчетПоДаннымБалансовыхРегистров.ЕстьЗаписиВоВременнойТаблице(Запрос, ТекстыЗапросов, "втРеестрДокументовСторно") Тогда
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ДобавитьЗапросУдаленияВременнойТаблицы(ТекстыЗапросов, "втРеестрДокументовСторно");
		Запрос.Параметры.ВосстановитьРасчетыПоСторно = Ложь;
		Возврат;
	КонецЕсли;
	...
	ОписаниеПолей = ОписаниеПолейФинансовогоРегистра(Метаданные.РегистрыНакопления.РасчетыСКлиентамиПоСрокам.Имя);
	
	Для Каждого КлючИЗначение Из ОписаниеПолей Цикл
	...
	Если ФинансовыйУчетПоДаннымБалансовыхРегистров.ЕстьЗаписиВоВременнойТаблице(Запрос, ТекстыЗапросов, "втРасчетыСКлиентамиПоСторно") Тогда
	...
	СтрокиВыборкиПрочихПолей = Новый Массив();
	РазделительСтрок = "," + Символы.ПС + Символы.Таб;
	ОписаниеПолей = ОписаниеПолейФинансовогоРегистра(Метаданные.РегистрыНакопления.РасчетыСПоставщикамиПоСрокам.Имя);
	...
	Если ФинансовыйУчетПоДаннымБалансовыхРегистров.ЕстьЗаписиВоВременнойТаблице(Запрос, ТекстыЗапросов, "втРасчетыСПоставщикамиПоСторно") Тогда
		ОбщегоНазначенияУТ.ДобавитьИдентификаторыВоВременнуюТаблицу(
...
Процедура ПодготовитьЗапросАнализаКорреспонденцийДляНастройкиСОтбором(Запрос, ТекстыЗапросов, ТекстыЗапросовРегистров, ДополнительныйОтбор)
	...
	ФинансовыйУчетПоДаннымБалансовыхРегистров.ДобавитьПоляЭлементовОтбора(ПоляОтбораДт, ДополнительныйОтбор, "Дт");
	ФинансовыйУчетПоДаннымБалансовыхРегистров.ДобавитьПоляЭлементовОтбора(ПоляОтбораКт, ДополнительныйОтбор, "Кт");
	...
	Если ТекстыЗапросов.Количество() = 0 Тогда
		...
		Для Каждого КлючИЗначение Из ТекстыЗапросовРегистров Цикл
			ДоступныеПоляОтбора = ОписаниеПолейФинансовогоРегистра(КлючИЗначение.Ключ);
...
Процедура ИсключитьРегистрыПоКоллекциямДвиженийРегистраторов(ПараметрыЗапроса, БалансовыеФинансовыеРегистры)
	...
	Для Каждого ИмяРегистра Из БалансовыеФинансовыеРегистры Цикл
		...
		ПараметрыОтраженияДвижений = ПараметрыОтраженияДвиженийВМеждународномУчете(ИмяРегистра, МетаданныеРегистра);
...
Функция СтрокиВыборкиПолейПоОписаниюОбъектаУчета(СтрокиОбъектовУчета, ПараметрыОтраженияДвижений, ПараметрыЗапроса)
	...
	ПоляВыборкиОбъектаУчета.ОбъектНастройки = ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиПоляОбъектаУчета(
		"ПутьКДаннымОбъектНастройки", СтрокиОбъектовУчета, ПараметрыОтраженияДвижений, ПараметрыЗапроса, Неопределено);
	
	ПоляВыборкиОбъектаУчета.ДатаПогашения = ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиПоляСПризнакомУчетаВОбъекте(
		"ПутьКДаннымДатаПогашения", "ВыделениеДолгосрочныхАктивовОбязательств", СтрокиОбъектовУчета, ПараметрыОтраженияДвижений, ПараметрыЗапроса, '00010101');
...
Процедура ДобавитьДоступныеПоляРегистра(СтрокиПолейАналитики, СтрокиГруппАналитики, ДоступныеПоляРегистра, ИмяРегистра)
	...
	СуществующиеПоляВыборки.Добавить("Регистратор");
	СуществующиеПоляВыборки.Добавить("НастройкаХозяйственнойОперации");
	СуществующиеПоляВыборки.Добавить("ДатаПогашения");
	...
	Если ПустаяСтрока(ИмяРегистра)
	 ИЛИ ИмяРегистра = ФинансовыйУчетПоДаннымБалансовыхРегистров.ИмяРегистраСлужебныхОбъектовУчета() Тогда
		...
		КонецЦикла;
	Иначе
		ОписаниеПолей = ОписаниеПолейФинансовогоРегистра(ИмяРегистра);
		Для Каждого КлючИЗначение Из ОписаниеПолей Цикл
			ЧастиПутиКДанным = СтрРазделить(КлючИЗначение.Значение.ПутьКДанным, ".");
			ИмяПоляРегистра = ЧастиПутиКДанным[0];
