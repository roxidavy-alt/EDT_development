			...
			Если ЭтоМодельБюджетирования Тогда
				...
				Иначе
					...
					Если Параметры.ЗагружаемыеУИН.Найти(ИскомаяСтрокаXML) = Неопределено Тогда
						Записывать = Ложь;
					КонецЕсли;
					Если Записывать И Не ПустаяСтрока(ЗаписанноеЗначение.Код) И Не Параметры.ИспользоватьИсходныеКодыЗагружаемыхДанных Тогда
						Если ЗаписанноеЗначение.ЭтоНовый() Тогда
							ЗаписанноеЗначение.УстановитьНовыйКод();
						Иначе
							ЗаписанноеЗначение.Код = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗаписанноеЗначение.Ссылка, "Код");
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
