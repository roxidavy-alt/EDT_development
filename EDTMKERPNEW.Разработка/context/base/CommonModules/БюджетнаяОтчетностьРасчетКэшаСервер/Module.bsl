...
Функция ПолучитьОписаниеИсточниковДанных(ВидБюджета, СтруктураКолонок, РежимФормирования, РекурсивноеПолучениеИсточниковПоСтатьям = Ложь) Экспорт
	...
	"ВЫБРАТЬ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПравилаРасчетаСвязанныхОстатковИОборотовБюджетов КАК ПравилаРасчета
		|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПравилаПолученияФактаПоСтатьямБюджетов КАК ПравилаПолученияФактаПоСвязаннымСтатьям
		|			ПО ПравилаРасчета.Источник = ПравилаПолученияФактаПоСвязаннымСтатьям.СтатьяБюджетов
		|				И (НЕ ПравилаПолученияФактаПоСвязаннымСтатьям.ПометкаУдаления)
		|		ПО ТаблицаПроверки.СтатьяПоказатель = ПравилаРасчета.Приемник
...
Функция ТипКолонкиСтруктурыЯчеек(Имя, Аналитика = Ложь) Экспорт
	...
	ИначеЕсли СтрНайти(Имя, "Период") Тогда
	...
	ИначеЕсли СтрНайти(Имя, "Аналитика") Тогда
...
// 		* ВыводитьЗаголовкиПовертикали - Булево -
...
Функция НовыйИсточник()
	...
	СтруктураИсточника.Вставить("ПравилаРасчета", Новый Массив);
...
Процедура ДополнитьИсточник(СтруктураКэша, НайденныйИсточник, ОтобраннаяСтрока)
	...
	ПравилаРасчета = Новый Массив;
	Если ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.СтатьяБюджетов Тогда
		ПравилаРасчета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
			ЭлементОтчета, "ПравилаРасчета.Правило").Выгрузить().ВыгрузитьКолонку("Правило");
	ИначеЕсли ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.ПоказательБюджетов Тогда
		ПравилаРасчета = Справочники.ПравилаРасчетаСвязанныхОстатковИОборотовБюджетов.ПравилаРасчетаСтатьиПоказателяБюджетов(
			ОбъектУчета);
	КонецЕсли;
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(НайденныйИсточник.ПравилаРасчета, ПравилаРасчета, Истина);
...
Процедура ДобавитьИсточник(СтруктураКэша, Источники, ОтобраннаяСтрока, СтруктураКолонок, ДополнительныеПараметрыИсточника)
	...
	Если ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.СтатьяБюджетов Тогда
		ОбъектУчета = ФинансоваяОтчетностьСервер.ЗначениеДополнительногоРеквизита(
			...
			ДопРеквизиты.СтатьяБюджетов,
			КэшДопРеквизитов);
		Массив = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ОбъектУчета);
		ПравилаРасчета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
			ЭлементОтчета, "ПравилаРасчета.Правило").Выгрузить().ВыгрузитьКолонку("Правило");
	ИначеЕсли ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.ПоказательБюджетов Тогда
		ОбъектУчета = ФинансоваяОтчетностьСервер.ЗначениеДополнительногоРеквизита(
	...
	СтруктураИсточника.Вставить("ПравилаРасчета", ПравилаРасчета);
...
Функция СравнитьИсточники(СтруктураКэша, Источник, ОтобраннаяСтрока, СтруктураКолонок, ДополнительныеПараметрыИсточника)
	...
	Если ВидЭлементаСравнения = Перечисления.ВидыЭлементовФинансовогоОтчета.СтатьяБюджетов Тогда
		...
		ПравилаРасчета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭлементОтчета,
			"ПравилаРасчета.Правило").Выгрузить().ВыгрузитьКолонку("Правило");
		Если ПравилаРасчета.Количество() = 0 И Источник.ПравилаРасчета.Количество() > 0
			Или ПравилаРасчета.Количество() > 0 И Источник.ПравилаРасчета.Количество() = 0 Тогда
			Возврат Ложь;
		КонецЕсли;
...
Процедура ЗаполнитьСведенияОбИсточникеДанных(НоваяСтрока, ИсточникиДанных, ИсточникиПериодовГруппировок, Элемент, ПодчиненныеСтроки, Параметры)
	...
	ИначеЕсли ВидЭлементаСравнения = Перечисления.ВидыЭлементовФинансовогоОтчета.СтатьяБюджетов Тогда
		...
		ИначеЕсли РасчетЗаполнение = 3 Тогда
			НоваяСтрока.РасчетЗаполнение = "расчет";
		КонецЕсли;
...
Процедура ДобавитьПодчиненныйНаборПоБюджетномуРегистру(ПараметрыПриемника,
	...
	ПравилаРасчета = Новый Массив;
	Если Не ХранимыеФактическиДанные И РежимФормирования = Перечисления.РежимыФормированияБюджетныхОтчетов.Заполнение
		Или ДляПоказателя Тогда
		ПравилаРасчета = Источник.ПравилаРасчета;
	КонецЕсли;
	ЗапросПоПравиламРасчета = БюджетированиеСервер.СформироватьЗапросКИсточникуДанныхПоПравиламРасчета(ПравилаРасчета,
		?(ХранимыеФактическиДанные, "ХранимыйФакт", "План"));
	...
	Если ДляПоказателя Тогда
		ТекстЗапросаПоляВыборки.Вставить("КоличествоИспользуемыхАналитик", "Данные.ПоказательБюджетов.КоличествоИспользуемыхАналитик");
		
		Для Сч = 1 По МаксимальноеКоличествоАналитик Цикл
			ТекстЗапросаПоляВыборки.Вставить("ВидАналитики" + Сч, "Данные.ПоказательБюджетов.ВидАналитики" + Сч);
		КонецЦикла;
		
		Если ПустаяСтрока(ЗапросПоПравиламРасчета.ТекстЗапроса) Тогда
		
			Если Источник.Объекты <> Неопределено Тогда
			
				ИмяПараметра = "ВыбранныеПоказателиБюджетов" + Источник.Идентификатор;
				УсловияНСИ = СтрШаблон("ПравилаРасчета.Приемник В (&%1)", ИмяПараметра);
			...
			ТекстЗапросаНСИ = СтрЗаменить(БюджетированиеСервер.ПолучитьТекстЗапросаКИсточникуДанныхПоказателейБезПравилРасчета(),
				"&ТекстУсловия", УсловияНСИ);
			ТекстЗапросаНСИ = СхемыЗапросов.УстановитьПомещениеВоВременнуюТаблицу(ТекстЗапросаНСИ, "Данные");
	...
	Если ДляПоказателя Тогда
		ТекстЗапросаПоляВыборки.Вставить("ПоказательБюджетов", "Данные.ПоказательБюджетов");
		ТекстЗапросаПоляВыборки.Вставить("УчитыватьПоВалюте", "Данные.ПоказательБюджетов.УчитыватьПоВалюте");
		ТекстЗапросаПоляВыборки.Вставить("ВалютаОпределяетсяАналитикой", "Данные.ПоказательБюджетов.ВалютаОпределяетсяАналитикой");
		ТекстЗапросаПоляВыборки.Вставить("АдресАналитикиВалюта", "Данные.ПоказательБюджетов.АдресАналитикиВалюта");
		ТекстЗапросаПоляВыборки.Вставить("ИсточникЗаполненияВалюты", "Данные.ПоказательБюджетов.АналитикаВалюты.ЗаполнениеВалюты");
	Иначе
	...
	ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "ИД_Источника", "ИД_Источника");
	ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "ПоказательБюджетов", "ПоказательБюджетов", Истина);
	ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "СтатьяБюджетов", "СтатьяБюджетов", Истина);
	ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "УчитыватьПоВалюте", "УчитыватьПоВалюте");
	ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "ВалютаОпределяетсяАналитикой", "ВалютаОпределяетсяАналитикой");
	ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "АдресАналитикиВалюта", "АдресАналитикиВалюта");
	ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "ИсточникЗаполненияВалюты", "ИсточникЗаполненияВалюты");
	...
	Если СтруктураКолонокТаблицыДанных.Измерения.Найти("ЕдиницаИзмерения") <> Неопределено Тогда
		...
		ТекстЗапросаПоляВыборки.Вставить("ЕдиницаИзмеренияОпределяетсяАналитикой",
			"Данные.СтатьяБюджетов.ЕдиницаИзмеренияОпределяетсяАналитикой");
		ТекстЗапросаПоляВыборки.Вставить("АдресАналитикиКоличество", "Данные.СтатьяБюджетов.АдресАналитикиКоличество");
		ТекстЗапросаПоляВыборки.Вставить("ИсточникЗаполнениеЕдиницыИзмерения",
			"Данные.СтатьяБюджетов.АналитикаЕдиницыИзмерения.ЗаполнениеЕдиницыИзмерения");
		ТекстЗапросаПоляВыборки.Вставить("ЕдиницаИзмерения", "Данные.СтатьяБюджетов.ЕдиницаИзмерения");
		
		ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "УчитыватьПоКоличеству", "УчитыватьПоКоличеству");
		ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "ЕдиницаИзмеренияОпределяетсяАналитикой",
			"ЕдиницаИзмеренияОпределяетсяАналитикой");
		ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "АдресАналитикиКоличество", "АдресАналитикиКоличество");
		ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "ИсточникЗаполнениеЕдиницыИзмерения",
			"ИсточникЗаполнениеЕдиницыИзмерения");
		ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "ЕдиницаИзмерения", "ЕдиницаИзмерения");
	КонецЕсли;
	
	Если ХранимыеФактическиДанные Тогда
		Если Не ДляПоказателя И ПолучитьФункциональнуюОпцию(
			"НастройкаПравилПолученияФактаПоКорреспондирующимРегистрам") Тогда
	...
	Если СтруктураКолонокТаблицыДанных.Измерения.Найти("Сценарий") = Неопределено Тогда
		...
		КонецЕсли;
		ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "Сценарий", "Сценарий", Истина);
	КонецЕсли;
	...
	Для Каждого Измерение Из СтруктураКолонокТаблицыДанных.Измерения Цикл
		Если Не СтрНайти(Измерение, "Аналитика") Тогда
			...
			ИначеЕсли Измерение = "Валюта" Тогда
				...
				КонецЕсли;
				ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "ВалютаХранения", "ВалютаХранения");
			Иначе
				ТекстЗапросаПоляВыборки.Вставить(Измерение, "Данные." + Измерение);
				ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, Измерение, Измерение, Истина);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Для Сч = 1 По МаксимальноеКоличествоАналитик Цикл
		ТекстЗапросаПоляВыборки.Вставить("Аналитика" + Сч, "Данные.Аналитика" + Сч);
		ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "Аналитика" + Сч, "Аналитика" + Сч, Истина);
	КонецЦикла;
	...
	Если СтруктураКолонокТаблицыДанных.Периодичности.Количество() Тогда
		...
		ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "Период", "Период");
	...
	Для Каждого Аналитика Из СтруктураКолонокТаблицыДанных.ВидыАналитик Цикл
		...
		Если ДляПоказателя Тогда
			ПараметрыВыражения.ВыражениеКоличествоИспользуемыхАналитик = "Данные.ПоказательБюджетов.КоличествоИспользуемыхАналитик";
			ПараметрыВыражения.ВыражениеВидАналитикиБезИндекса = "Данные.ПоказательБюджетов.ВидАналитики";
		Иначе
		...
		ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, ИмяПараметра, ИмяПараметра);
	...
	Иначе
		...
		ТекстЗапросаПоляВыборки.Вставить("СуммаПриход", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"ВЫБОР КОГДА НЕ Данные.Расход ТОГДА %1 ИНАЧЕ 0 КОНЕЦ", ТекстПоказателя));
		ТекстЗапросаПоляВыборки.Вставить("СуммаРасход", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"ВЫБОР КОГДА Данные.Расход ТОГДА %1 ИНАЧЕ 0 КОНЕЦ", ТекстПоказателя));
		ТекстЗапросаПоляВыборки.Вставить("СуммаВалХраненияПриход",
			"ВЫБОР КОГДА НЕ Данные.Расход ТОГДА СуммаВВалютеОборот ИНАЧЕ 0 КОНЕЦ");
		ТекстЗапросаПоляВыборки.Вставить("СуммаВалХраненияРасход",
			"ВЫБОР КОГДА Данные.Расход ТОГДА СуммаВВалютеОборот ИНАЧЕ 0 КОНЕЦ");
		ТекстЗапросаПоляВыборки.Вставить("СуммаНачальныйОстаток", "NULL");
		ТекстЗапросаПолявыборки.Вставить("СуммаВалХраненияНачальныйОстаток", "NULL");
		ТекстЗапросаПоляВыборки.Вставить("КоличествоПриход",
			"ВЫБОР КОГДА НЕ Данные.Расход ТОГДА КоличествоОборот ИНАЧЕ 0 КОНЕЦ");
		ТекстЗапросаПоляВыборки.Вставить("КоличествоРасход",
			"ВЫБОР КОГДА Данные.Расход ТОГДА КоличествоОборот ИНАЧЕ 0 КОНЕЦ");
		ТекстЗапросаПоляВыборки.Вставить("КоличествоНачальныйОстаток", "NULL");
		
	КонецЕсли;
	
	ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "СуммаПриход", "СуммаПриход");
	ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "СуммаРасход", "СуммаРасход");
	ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "СуммаВалХраненияПриход", "СуммаВалХраненияПриход");
	ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "СуммаВалХраненияРасход", "СуммаВалХраненияРасход");
	ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "КоличествоПриход", "КоличествоПриход");
	ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "КоличествоРасход", "КоличествоРасход");
	Если ДляПоказателя Тогда
		ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "СуммаНачальныйОстаток", "СуммаНачальныйОстаток");
		ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "СуммаВалХраненияНачальныйОстаток", "СуммаВалХраненияНачальныйОстаток");
		ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "КоличествоНачальныйОстаток", "КоличествоНачальныйОстаток");
	КонецЕсли;
	...
	Если Не ПустаяСтрока(ЗапросПоПравиламРасчета.ТекстЗапроса) Или Не ПустаяСтрока(ТекстЗапросаНСИ) Тогда
		ТекстИсточникаДанных = "(
			...
			|	Данные КАК Данные
			|ГДЕ
			|	&УсловияПоТаблицеПриемникаИПоляКомпановки)";
		Для Каждого ПараметрЗапроса Из ЗапросПоПравиламРасчета.ПараметрыЗапроса Цикл
			КомпоновкаДанныхСервер.ДобавитьПараметр(СКДПлановыеДанные, ПараметрЗапроса.Ключ, , ПараметрЗапроса.Значение, Истина,
				Истина);
		КонецЦикла;
		Если Не ПустаяСтрока(ЗапросПоПравиламРасчета.ТекстЗапросаВТ) Тогда
			Если ДляПоказателя Тогда
				ЗапросПоПравиламРасчета.ТекстЗапросаВТ = СтрЗаменить(ЗапросПоПравиламРасчета.ТекстЗапросаВТ, "&ВыражениеНачалоПериода",
					Источник.ВыражениеНачалоПериода);
				ЗапросПоПравиламРасчета.ТекстЗапросаВТ = СтрЗаменить(ЗапросПоПравиламРасчета.ТекстЗапросаВТ, "&ВыражениеКонецПериода",
					Источник.ВыражениеКонецПериода);
			КонецЕсли;
			ЗапросПоПравиламРасчета.ТекстЗапросаВТ = ЗапросПоПравиламРасчета.ТекстЗапросаВТ
				+ ОбщегоНазначения.РазделительПакетаЗапросов();
		КонецЕсли;
		Если Не ПустаяСтрока(ЗапросПоПравиламРасчета.ТекстЗапроса) Тогда
			Если ДляПоказателя Тогда
				ЗапросПоПравиламРасчета.ТекстЗапроса = СтрЗаменить(ЗапросПоПравиламРасчета.ТекстЗапроса, "&ВыражениеНачалоПериода",
					Источник.ВыражениеНачалоПериода);
				ЗапросПоПравиламРасчета.ТекстЗапроса = СтрЗаменить(ЗапросПоПравиламРасчета.ТекстЗапроса, "&ВыражениеКонецПериода",
					Источник.ВыражениеКонецПериода);
			КонецЕсли;
			ЗапросПоПравиламРасчета.ТекстЗапроса = ЗапросПоПравиламРасчета.ТекстЗапроса
				+ ОбщегоНазначения.РазделительПакетаЗапросов();
		КонецЕсли;
	ИначеЕсли ХранимыеФактическиДанные Тогда
	...
	УсловияПоТаблицеИсточника = Новый Соответствие;
	УсловияКомпоновкиВиртуальнойТаблицыИсточника = Новый Соответствие;
	Если ЗапросПоПравиламРасчета.ТипыИсточников.Количество() = 0 Тогда
		ТипыИсточников = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("ОборотыБюджетов");
	Иначе
		ТипыИсточников = ЗапросПоПравиламРасчета.ТипыИсточников;
	КонецЕсли;
	Для Каждого ТипИсточника Из ТипыИсточников Цикл
		УсловияПоТаблицеИсточника.Вставить(ТипИсточника, Новый Массив);
		УсловияКомпоновкиВиртуальнойТаблицыИсточника.Вставить(ТипИсточника, Новый Массив);
	КонецЦикла;
	УсловияПоТаблицеПриемника = Новый Массив;
	УсловияКомпоновкиВиртуальнойТаблицыПриемника = Новый Массив;
	
	ИзмеренияПоТипамИсточников = Новый Соответствие;
	ДобавитьУсловиеПоОрганизации = Истина;
	Для Каждого ТипИсточника Из ТипыИсточников Цикл
		ТипИсточникаЗначение = Перечисления.ТипыИсточниковПолученияДанныхДляБюджетирования[ТипИсточника];
		ИзмеренияПоТипамИсточников.Вставить(ТипИсточника,
			БюджетированиеПовтИсп.СоответствиеИзмеренийБюджетированияПоТипуИсточника(ТипИсточникаЗначение));
		Если ДобавитьУсловиеПоОрганизации
			И Перечисления.ТипыИсточниковПолученияДанныхДляБюджетирования.ПоТипуИсточникаВедетсяУчетПоОрганизации(ТипИсточникаЗначение) Тогда
			ДобавитьУсловиеПоОрганизации = Ложь;
		КонецЕсли;
	КонецЦикла;
	
	Если ДобавитьУсловиеПоОрганизации Тогда
		...
		|{ГДЕ
		|	Организации.Ссылка.* КАК Организация_Идентификатор}";
		ТекстЗапросаПоОрганизациям = СтрЗаменить(ТекстЗапросаПоОрганизациям, "Идентификатор", Источник.Идентификатор);
		ЗапросПоПравиламРасчета.ТекстЗапросаВТ = ЗапросПоПравиламРасчета.ТекстЗапросаВТ + 
			ТекстЗапросаПоОрганизациям + ОбщегоНазначения.РазделительПакетаЗапросов();
	КонецЕсли;
	
	Если Источник.Объекты <> Неопределено
		// В запросе, сформированном по правилам расчета, уже есть отбор по статьям бюджетов
		И ПустаяСтрока(ЗапросПоПравиламРасчета.ТекстЗапроса) Тогда
		
		Если Не ДляПоказателя Тогда
			ИмяПараметра = "ВыбранныеСтатьиБюджетов" + Источник.Идентификатор;
			Выражение = "СтатьяБюджетов В (&" + ИмяПараметра + ")";
			УсловияПоТаблицеИсточника["ОборотыБюджетов"].Добавить(Выражение);
	...
	Для Каждого Измерение Из СтруктураКолонокТаблицыДанных.Измерения Цикл
		...
		Если Источник.Свойство(Измерение, ОтборПоИзмерениям)
			...
			Для Каждого ТипИсточника Из ТипыИсточников Цикл
				ИзмерениеПоТипуИсточника = ИзмеренияПоТипамИсточников.Получить(ТипИсточника)[Измерение];
				Если ИзмерениеПоТипуИсточника <> Неопределено Тогда
					Выражение = "" + СтрЗаменить(ИзмерениеПоТипуИсточника, "ИмяТаблицы.", "") + " " + "В (&"
						+ ИмяПараметра + ")";
					УсловияПоТаблицеИсточника[ТипИсточника].Добавить(Выражение);
				КонецЕсли;
			КонецЦикла;
		...
		Для Каждого Измерение Из СтруктураКолонокТаблицыДанных.ВидыАналитик Цикл
			...
			Если Источник.Свойство(ИмяАналитики, ОтборПоИзмерениям)
				...
				Если ПустаяСтрока(ЗапросПоПравиламРасчета.ТекстЗапроса) Тогда
					УсловияПоТаблицеИсточника["ОборотыБюджетов"].Добавить(Выражение);
				Иначе
					УсловияПоТаблицеПриемника.Добавить(Выражение);
				КонецЕсли;
	...
	Если Не ХранимыеФактическиДанные Тогда
		Для Каждого ТипИсточника Из ТипыИсточников Цикл
			УсловияКомпоновкиВиртуальнойТаблицыИсточника[ТипИсточника].Добавить("(Статус).* КАК Статус");
		КонецЦикла;
	КонецЕсли;
		...
		Если НайтиОтборПоИмени(Источник.ДополнительныйОтбор, КлючИЗначение.Ключ,, Истина) Тогда
			Если ХранимыеФактическиДанные Тогда
				Если КлючИЗначение.Ключ = "Сценарий" Тогда
					...
					Если Не ПустаяСтрока(ТекстЗапросаНСИ) Тогда
						УсловияКомпоновкиВиртуальнойТаблицыПриемника.Добавить(Выражение);
					Иначе
						УсловияКомпоновкиВиртуальнойТаблицыИсточника["ОборотыБюджетов"].Добавить(Выражение);
					КонецЕсли;
					Продолжить;
				КонецЕсли;
			КонецЕсли;
			...
			Если Не ПустаяСтрока(ЗапросПоПравиламРасчета.ТекстЗапроса) И СтрНайти(КлючИЗначение.Ключ, "Аналитика") > 0
				Или Не ПустаяСтрока(ТекстЗапросаНСИ) Тогда
			...
			Иначе
				Для Каждого ТипИсточника Из ТипыИсточников Цикл
					ИзмерениеПоТипуИсточника = ИзмеренияПоТипамИсточников.Получить(ТипИсточника)[КлючИЗначение.Ключ];
					Если ИзмерениеПоТипуИсточника <> Неопределено Тогда
						ВыражениеПоТипуИсточника = СтрЗаменить(Выражение, "(" + КлючИЗначение.Ключ + ").*", "(" + СтрЗаменить(
							ИзмерениеПоТипуИсточника, "ИмяТаблицы.", "") + ").*");
						УсловияКомпоновкиВиртуальнойТаблицыИсточника[ТипИсточника].Добавить(ВыражениеПоТипуИсточника);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
	...
	УсловияПоТаблицеИсточникаИПоляКомпановки = Новый Соответствие;
	Для Каждого ТипИсточника Из ТипыИсточников Цикл
		Если УсловияПоТаблицеИсточника[ТипИсточника].Количество() = 0 Тогда
			УсловияПоТаблицеИсточникаИПоляКомпановки.Вставить(ТипИсточника, "ИСТИНА");
		Иначе
			РазделительИ = Символы.ПС + Символы.Таб + Символы.Таб + Символы.Таб + " И ";
			УсловияПоТаблицеИсточникаИПоляКомпановки.Вставить(ТипИсточника, СтрСоединить(
				УсловияПоТаблицеИсточника[ТипИсточника], РазделительИ));
		КонецЕсли;
	КонецЦикла;
	Если УсловияПоТаблицеПриемника.Количество() = 0 Тогда
		УсловияПоТаблицеПриемникаИПоляКомпановки = "ИСТИНА";
	Иначе
		РазделительИ = Символы.ПС + Символы.Таб + Символы.Таб + Символы.Таб + " И ";
		УсловияПоТаблицеПриемникаИПоляКомпановки = СтрСоединить(УсловияПоТаблицеПриемника, РазделительИ);
	КонецЕсли;
	Для Каждого ТипИсточника Из ТипыИсточников Цикл
		Если УсловияКомпоновкиВиртуальнойТаблицыИсточника[ТипИсточника].Количество() > 0 Тогда
			Выражение = СтрСоединить(УсловияКомпоновкиВиртуальнойТаблицыИсточника[ТипИсточника], ", ");
			УсловияПоТаблицеИсточникаИПоляКомпановки[ТипИсточника] = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"%1 {%2}",
				УсловияПоТаблицеИсточникаИПоляКомпановки[ТипИсточника],
				Выражение);
		КонецЕсли;
	КонецЦикла;
	Если УсловияКомпоновкиВиртуальнойТаблицыПриемника.Количество() > 0 Тогда
		Выражение = СтрСоединить(УсловияКомпоновкиВиртуальнойТаблицыПриемника, ", ");
		УсловияПоТаблицеПриемникаИПоляКомпановки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"%1
			|{ГДЕ %2}",
			УсловияПоТаблицеПриемникаИПоляКомпановки,
			Выражение);
	...
	Если Не ПустаяСтрока(ЗапросПоПравиламРасчета.ТекстЗапроса) Тогда
		Для Каждого ТипИсточника Из ТипыИсточников Цикл
			ИмяПараметраУсловияПоТаблице = "&УсловияПоТаблицеИсточникаИПоляКомпановки" + ?(ТипИсточника = "ОборотыБюджетов", "",
				"_" + ТипИсточника);
			ЗапросПоПравиламРасчета.ТекстЗапроса = СтрЗаменить(ЗапросПоПравиламРасчета.ТекстЗапроса,
				ИмяПараметраУсловияПоТаблице, УсловияПоТаблицеИсточникаИПоляКомпановки[ТипИсточника]);
			Если Не ПустаяСтрока(ЗапросПоПравиламРасчета.ТекстЗапросаВТ) Тогда
				ЗапросПоПравиламРасчета.ТекстЗапросаВТ = СтрЗаменить(ЗапросПоПравиламРасчета.ТекстЗапросаВТ,
					ИмяПараметраУсловияПоТаблице, УсловияПоТаблицеИсточникаИПоляКомпановки[ТипИсточника]);
			КонецЕсли;
		КонецЦикла;
		ТекстИсточникаДанных = СтрЗаменить(ТекстИсточникаДанных, "&УсловияПоТаблицеПриемникаИПоляКомпановки",
			УсловияПоТаблицеПриемникаИПоляКомпановки);
	ИначеЕсли Не ПустаяСтрока(ТекстЗапросаНСИ) Тогда
		ТекстИсточникаДанных = СтрЗаменить(ТекстИсточникаДанных, "&УсловияПоТаблицеПриемникаИПоляКомпановки",
			УсловияПоТаблицеПриемникаИПоляКомпановки);
	Иначе
		ТекстИсточникаДанных = СтрЗаменить(ТекстИсточникаДанных, "&УсловияПоТаблицеИсточникаИПоляКомпановки",
			УсловияПоТаблицеИсточникаИПоляКомпановки["ОборотыБюджетов"]);
	КонецЕсли;
	...
	Если НЕ ХранимыеФактическиДанные И 
		...
		Если ПустаяСтрока(ЗапросПоПравиламРасчета.ТекстЗапроса) Тогда
			...
			ТекстЗапросаИсключения = ТекстЗапросаИсключения + Символы.ПС + "ГДЕ %1"; // @query-part
		Иначе
			ПозицияИндексироватьПо = СтрНайти(ЗапросПоПравиламРасчета.ТекстЗапроса, "ИНДЕКСИРОВАТЬ ПО"); // @query-part
			ТекстЗапросаИсключения = СтрЗаменить(Лев(ЗапросПоПравиламРасчета.ТекстЗапроса, ПозицияИндексироватьПо - 1),
				"ПОМЕСТИТЬ Данные", ""); // @query-part
			ТекстыЗапросовИсключения = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ТекстЗапросаИсключения,
				ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении());
			Для Счетчик = 0 По ТекстыЗапросовИсключения.ВГраница() Цикл
				ТекстЗапросаИсключения = ТекстыЗапросовИсключения[Счетчик];
				ТекстЗапросаИсключения = СтрЗаменить(ТекстЗапросаИсключения, "ПсевдонимТаблицыИсточника.СуммаУпрОборот",
					"-ПсевдонимТаблицыИсточника.СуммаУпрОборот");
				ТекстЗапросаИсключения = СтрЗаменить(ТекстЗапросаИсключения, "ПсевдонимТаблицыИсточника.СуммаРеглОборот",
					"-ПсевдонимТаблицыИсточника.СуммаРеглОборот");
				ТекстЗапросаИсключения = СтрЗаменить(ТекстЗапросаИсключения, "ПсевдонимТаблицыИсточника.СуммаСценарияОборот",
					"-ПсевдонимТаблицыИсточника.СуммаСценарияОборот");
				ТекстЗапросаИсключения = СтрЗаменить(ТекстЗапросаИсключения, "ПсевдонимТаблицыИсточника.СуммаВВалютеОборот",
					"-ПсевдонимТаблицыИсточника.СуммаВВалютеОборот");
				ТекстЗапросаИсключения = СтрЗаменить(ТекстЗапросаИсключения, "ПсевдонимТаблицыИсточника.КоличествоОборот",
					"-ПсевдонимТаблицыИсточника.КоличествоОборот");
				ТекстЗапросаИсключения = ТекстЗапросаИсключения + Символы.ПС + "И %1"; // @query-part
				ТекстыЗапросовИсключения[Счетчик] = ТекстЗапросаИсключения;
			КонецЦикла;
			ТекстЗапросаИсключения = СтрСоединить(ТекстыЗапросовИсключения, ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении());
		КонецЕсли;
		ТекстЗапросаИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстЗапросаИсключения,
			"(Регистратор = &Документ ИЛИ &Действие = ЗНАЧЕНИЕ(Перечисление.ТипыДействийЭкземпляровБюджетов.ИсправлениеЭкземпляраБюджета))"); // @query-part
		Если Не ПустаяСтрока(ЗапросПоПравиламРасчета.ТекстЗапроса) Тогда
			ЗапросПоПравиламРасчета.ТекстЗапроса = Лев(ЗапросПоПравиламРасчета.ТекстЗапроса, ПозицияИндексироватьПо
				- 1) + ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении() + ТекстЗапросаИсключения + Символы.ПС
				+ Символы.Таб + Сред(ЗапросПоПравиламРасчета.ТекстЗапроса, ПозицияИндексироватьПо);
		Иначе
			ТекстЗапроса = ТекстЗапроса + ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении() + ТекстЗапросаИсключения;
		КонецЕсли;
	...
	ТекстЗапроса = ?(ПустаяСтрока(ТекстЗапросаНСИ), "", ТекстЗапросаНСИ + ОбщегоНазначения.РазделительПакетаЗапросов())
		+ ЗапросПоПравиламРасчета.ТекстЗапросаВТ + ЗапросПоПравиламРасчета.ТекстЗапроса +
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	* 
		|ИЗ
	...
	Если ТаблицаПолейКомпоновки.Количество() > 0
		...
		ИндексПакета = ?(ТекстЗапросаНСИ = "", 0, 1);
		Если Не ПустаяСтрока(ЗапросПоПравиламРасчета.ТекстЗапросаВТ) Тогда
			ИндексПакета = ИндексПакета + СтрЧислоВхождений(ЗапросПоПравиламРасчета.ТекстЗапросаВТ, ";");
		КонецЕсли;
		Если Не ПустаяСтрока(ЗапросПоПравиламРасчета.ТекстЗапроса) Тогда
...
Процедура ДобавитьПодчиненныйНаборПоРегиструНачальныхОстатков(ПараметрыПриемника, ПараметрыИсточника,
	РежимФормирования, РежимПолученияДанных)
	...
	ТекстЗапросаВременныеТаблицы = СтрСоединить(
		РегистрыСведений.НачальныеОстаткиРасчетныхПоказателей.ТекстыЗапросовАктуальныеДатыОстатковПоСценариям(Ложь, Ложь,
			Истина), ОбщегоНазначения.РазделительПакетаЗапросов());
	
	ТекстЗапросаВременныеТаблицы = СтрЗаменить(ТекстЗапросаВременныеТаблицы, "&НачальнаяДатаОстатков",
		Источник.ВыражениеНачалоПериода);
	...
	|	&ПоляВыборки1
	|ИЗ
	|	РегистрСведений.НачальныеОстаткиРасчетныхПоказателей КАК Данные
	|ГДЕ
	|	Данные.ДатаОстатков = &НачальнаяДатаОстатков
	|	И ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	...
	|			ГДЕ
	|				АктуальныеДатыОстатковПоСценариям.ДатаОстатков = Данные.ДатаОстатков
	|				И АктуальныеДатыОстатковПоСценариям.ПоказательБюджетов = Данные.ПоказательБюджетов
	|				И АктуальныеДатыОстатковПоСценариям.Сценарий = Данные.Сценарий)
	|	И &УсловияПоТаблицеИсточникаИПоляКомпоновки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|			ГДЕ
	|				АктуальныеДатыОстатков.ДатаОстатков = Данные.ДатаОстатков
	|				И АктуальныеДатыОстатков.ПоказательБюджетов = Данные.ПоказательБюджетов)
	|	И НЕ ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	...
	|	И Данные.Сценарий = ЗНАЧЕНИЕ(Справочник.Сценарии.ПустаяСсылка)
	|	И &УсловияПоТаблицеИсточникаИПоляКомпоновки";
	ТекстЗапросаПоляВыборки.Вставить("ИД_Источника", """" + Источник.Идентификатор + """");
	ТекстЗапросаПоляВыборки.Вставить("ПоказательБюджетов", "Данные.ПоказательБюджетов");
	ТекстЗапросаПоляВыборки.Вставить("УчитыватьПоВалюте", "Данные.ПоказательБюджетов.УчитыватьПоВалюте");
	...
	Если СтруктураКолонокТаблицыДанных.Измерения.Найти("ЕдиницаИзмерения") <> Неопределено Тогда
		ТекстЗапросаПоляВыборки.Вставить("УчитыватьПоКоличеству", "Данные.ПоказательБюджетов.УчитыватьПоКоличеству");
		ТекстЗапросаПоляВыборки.Вставить("ЕдиницаИзмеренияОпределяетсяАналитикой",
			"Данные.ПоказательБюджетов.ЕдиницаИзмеренияОпределяетсяАналитикой");
		ТекстЗапросаПоляВыборки.Вставить("АдресАналитикиКоличество", "Данные.ПоказательБюджетов.АдресАналитикиКоличество");
		ТекстЗапросаПоляВыборки.Вставить("ИсточникЗаполнениеЕдиницыИзмерения",
			"Данные.ПоказательБюджетов.АналитикаЕдиницыИзмерения.ЗаполнениеЕдиницыИзмерения");
		ТекстЗапросаПоляВыборки.Вставить("ЕдиницаИзмерения", "Данные.ПоказательБюджетов.ЕдиницаИзмерения");
	...
	Если СтруктураКолонокТаблицыДанных.Периодичности.Количество() Тогда
		...
		ВыражениеПериодичности = СтрЗаменить(Источник.ВыражениеПериодаГруппировки, "ПериодПланирования", "ДатаОстатков");
		ТекстЗапросаПоляВыборки.Вставить("Период", ВыражениеПериодичности);
		ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "Период", "Период");
	...
	ТекстЗапросаПоляВыборки.Вставить("СуммаНачальныйОстаток", ТекстПоказателя);
	ТекстЗапросаПоляВыборки.Вставить("СуммаВалХраненияНачальныйОстаток", "СуммаВВалюте");
	ТекстЗапросаПоляВыборки.Вставить("КоличествоНачальныйОстаток", "Количество");
	
	ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "СуммаНачальныйОстаток", "СуммаНачальныйОстаток");
	ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "СуммаВалХраненияНачальныйОстаток",
		"СуммаВалХраненияНачальныйОстаток");
	ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "КоличествоНачальныйОстаток", "КоличествоНачальныйОстаток");
	
	ДобавитьСтрокуПолейКомпоновки(ТаблицаПолейКомпоновки, "ПериодКурса", "ПериодКурса");
	ТекстЗапросаПоляВыборки.Вставить("ПериодКурса", "Данные.ДатаОстатков");
	
	КлючевыеПоляОтбора = "ПоказательБюджетов,Сценарий";
	УсловияПоТаблицеИсточника = Новый Структура("Прочее," + КлючевыеПоляОтбора, Новый Массив);
	УсловияКомпоновкиТаблицы = Новый Структура("Прочее," + КлючевыеПоляОтбора, Новый Массив);
	
	Если Источник.Объекты <> Неопределено Тогда
		
		ИмяПараметра = "ВыбранныеПоказателиБюджетов" + Источник.Идентификатор;
		Выражение = "НачальныеОстаткиРасчетныхПоказателей.ПоказательБюджетов В (&" + ИмяПараметра + ")";
		УсловияПоТаблицеИсточника.ПоказательБюджетов = Выражение;
	...
	Для Каждого Измерение Из СтруктураКолонокТаблицыДанных.Измерения Цикл
		...
		Если Источник.Свойство(Измерение, ОтборПоИзмерениям)
			...
			Если Измерение = "Сценарий" Тогда
				Выражение = "Сценарии.Ссылка В (&" + ИмяПараметра + ")";
				УсловияПоТаблицеИсточника.Сценарий = Выражение;
			Иначе
				Выражение = "Данные." + Измерение + " " + "В (&" + ИмяПараметра + ")";
				УсловияПоТаблицеИсточника.Прочее.Добавить(Выражение);
			КонецЕсли;
	...
	Для Каждого Измерение Из СтруктураКолонокТаблицыДанных.ВидыАналитик Цикл
		...
		Если Источник.Свойство(ИмяАналитики, ОтборПоИзмерениям)
			...
			ПараметрыВыражения = БюджетированиеСервер.ШаблонПараметровДляПолученияТекстаПоляСУсловиемПоВидуАналитики();
			ПараметрыВыражения.ВыражениеКоличествоИспользуемыхАналитик = "ПоказательБюджетов.КоличествоИспользуемыхАналитик";
			ПараметрыВыражения.ВыражениеВидАналитикиБезИндекса = "ПоказательБюджетов.ВидАналитики";
			ПараметрыВыражения.ВыражениеЗначенияАналитикиБезИндекса = "Аналитика";
			ПараметрыВыражения.ИмяПараметраВидАналитики = ИмяПараметраВидаАналитики;
			...
			Выражение = "Данные." + ТекстАналитики + " " + "В (&" + ИмяПараметра + ")";
			УсловияПоТаблицеИсточника.Прочее.Добавить(Выражение);
	...
	Для Каждого КлючИЗначение Из СтруктураОтборов Цикл
		...
		Если НайтиОтборПоИмени(Источник.ДополнительныйОтбор, КлючИЗначение.Ключ,, Истина) Тогда
			Если КлючИЗначение.Ключ = "ВалютаХранения" Тогда
				Выражение = "(Данные.Валюта).* КАК" + " " + КлючИЗначение.Ключ + "_" + Источник.Идентификатор;
				УсловияКомпоновкиТаблицы.Прочее.Добавить(Выражение);
				Продолжить;
			КонецЕсли;
			
			Если КлючИЗначение.Ключ = "Сценарий" Тогда
				Выражение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("(Сценарии.Ссылка).* КАК %1_%2",
					КлючИЗначение.Ключ, Источник.Идентификатор);
				УсловияКомпоновкиТаблицы.Сценарий = Выражение;
			Иначе
				Выражение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("(Данные.%1).* КАК %1_%2", КлючИЗначение.Ключ,
					Источник.Идентификатор);
				УсловияКомпоновкиТаблицы.Прочее.Добавить(Выражение);
			КонецЕсли;
		КонецЕсли;
	...
	Для Каждого КлючевоеПолеОтбора Из СтрРазделить(КлючевыеПоляОтбора, ",") Цикл
		Если Не ПустаяСтрока(УсловияПоТаблицеИсточника[КлючевоеПолеОтбора]) Тогда
			ТекстУсловия = УсловияПоТаблицеИсточника[КлючевоеПолеОтбора];
		Иначе
			ТекстУсловия = "ИСТИНА";
		КонецЕсли;
		Если Не ПустаяСтрока(УсловияКомпоновкиТаблицы[КлючевоеПолеОтбора]) Тогда
			ТекстУсловия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"%1
				|{ГДЕ %2}",
				ТекстУсловия,
				УсловияКомпоновкиТаблицы[КлючевоеПолеОтбора]);
		КонецЕсли;
		ТекстЗапросаВременныеТаблицы = СтрЗаменить(ТекстЗапросаВременныеТаблицы, "&Условие" + КлючевоеПолеОтбора,
			ТекстУсловия);
	КонецЦикла;
	Если УсловияПоТаблицеИсточника.Прочее.Количество() = 0 Тогда
		УсловияПоТаблицеИсточникаИПоляКомпоновки = "ИСТИНА";
	Иначе
		РазделительИ = Символы.ПС + Символы.Таб + Символы.Таб + Символы.Таб + " И ";
		УсловияПоТаблицеИсточникаИПоляКомпоновки = СтрСоединить(УсловияПоТаблицеИсточника.Прочее, РазделительИ);
	КонецЕсли;
	Если УсловияКомпоновкиТаблицы.Прочее.Количество() > 0 Тогда
		Выражение = СтрСоединить(УсловияКомпоновкиТаблицы.Прочее, ", ");
		УсловияПоТаблицеИсточникаИПоляКомпоновки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"%1
			|{ГДЕ %2}",
			УсловияПоТаблицеИсточникаИПоляКомпоновки,
			Выражение);
	КонецЕсли;
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&НачальнаяДатаОстатков", Источник.ВыражениеНачалоПериода);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловияПоТаблицеИсточникаИПоляКомпоновки", УсловияПоТаблицеИсточникаИПоляКомпоновки);
	
	ПоляВыборки1 = Новый Массив;
	ПоляВыборки2 = Новый Массив;
	Для Каждого СинонимИВыражение Из ТекстЗапросаПоляВыборки Цикл
		ПоляВыборки1.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 КАК %2",
			СинонимИВыражение.Значение, СинонимИВыражение.Ключ));
		ПоляВыборки2.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 КАК %2",
			?(СинонимИВыражение.Ключ = "Сценарий", "Сценарии.Сценарий", СинонимИВыражение.Значение),
			СинонимИВыражение.Ключ));
		Если СинонимИВыражение.Ключ = "СуммаНачальныйОстаток"
			Или СинонимИВыражение.Ключ = "СуммаВалХраненияНачальныйОстаток"
			Или СинонимИВыражение.Ключ = "КоличествоНачальныйОстаток" Тогда
			Продолжить;
		КонецЕсли;
	КонецЦикла;
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПоляВыборки1", СтрСоединить(ПоляВыборки1, "," + Символы.ПС + Символы.Таб));
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПоляВыборки2", СтрСоединить(ПоляВыборки2, "," + Символы.ПС + Символы.Таб));
	...
	ЗапросИсточник = ТекстЗапроса;
	ТекстЗапроса = ТекстЗапросаВременныеТаблицы + ОбщегоНазначения.РазделительПакетаЗапросов() + 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	* 
		|ИЗ
		|	&ЗапросИсточник КАК ПредварительныйРасчет
	...
	Если НЕ РежимПолученияДанных = Перечисления.РежимПолученияДанныхБюджетирования.Разрешенные Тогда
		ТекстЗапроса = СхемыЗапросов.УстановитьВыборкуРазрешенныхЗаписейВоВсемЗапросе(ТекстЗапроса, Ложь);
	КонецЕсли;
	Если ТаблицаПолейКомпоновки.Количество() > 0 Тогда
		
		СхемаЗапроса = Новый СхемаЗапроса();
		СхемаЗапроса.УстановитьТекстЗапроса(ТекстЗапроса);
		ИзменяемыйЗапрос = СхемаЗапроса.ПакетЗапросов.Получить(3);
		Если ТипЗнч(ИзменяемыйЗапрос) = Тип("ЗапросУничтоженияТаблицыСхемыЗапроса") Тогда
			ВызватьИсключение НСтр("ru = 'Невозможно добавить поля в запрос уничтожения таблицы.';
									|en = 'Cannot add fields to the table removal query.'"); 
		КонецЕсли;
	...
	ТипИсточника = Тип("НаборДанныхЗапросСхемыКомпоновкиДанных");
	Имя = "Набор_План_НачальныеОстатки_" + Источник.Идентификатор;
	НаборСКД = КомпоновкаДанныхСервер.ДобавитьПустойНаборДанных(БазовыйОбъединенныйНабор, ТипИсточника, Имя);
	НаборСКД.Запрос = ТекстЗапроса;
...
Функция ПолучитьСКДПлановыеДанные(ИсточникиДанных, ПоляИсточников, СтруктураКолонокТаблицыДанных, Аналитики, ПараметрыКУстановке, РежимыФормирования)
	...
	ФинОтчеты = ФинансоваяОтчетностьСервер;
	СКДПлановыеДанные = КомпоновкаДанныхСервер.ПустаяСхема();
	...
	ИспользуютсяНачальныеОстатки = Ложь;
	...
	Для Каждого Источник Из ИсточникиДанных Цикл
		...
		Если Источник.ТипИсточника = Перечисления.ТипыИсточниковДанныхБюджетирования.Остатки Тогда
			ДобавитьПодчиненныйНаборПоРегиструНачальныхОстатков(ПараметрыПриемника, ПараметрыИсточника, РежимФормирования,
				РежимПолученияДанных);
			ИспользуютсяНачальныеОстатки = Истина;
		КонецЕсли;
	...
	ПоляРесурсовКоличество = Новый Массив;
	ПоляРесурсовКоличество.Добавить("КоличествоПриход");
	ПоляРесурсовКоличество.Добавить("КоличествоРасход");
	Если ИспользуютсяНачальныеОстатки Тогда
		ПоляРесурсовКоличество.Добавить("КоличествоНачальныйОстаток");
	КонецЕсли;
	
	Для Каждого ПолеРесурсаКоличество Из ПоляРесурсовКоличество Цикл
		
		Если ПоляРесурсов.Свойство(ПолеРесурсаКоличество) Тогда
		
			ВыражениеВычисляемогоПоля = СтрЗаменить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"ВЫБОР КОГДА (НЕ УчитыватьПоКоличеству ИЛИ НЕ ЕдиницаИзмеренияОпределяетсяАналитикой) ТОГДА %1
				|	ИНАЧЕ &ТекстИсточникЗаполненияЕдиницыИзмерения
				|	КОНЕЦ", ПолеРесурсаКоличество), "&ТекстИсточникЗаполненияЕдиницыИзмерения",
				ТекстУсловияИсточникаЗаполненияЕдиницыИзмерения(ПолеРесурсаКоличество));
			ДополнитьВыражениеВычисляемогоПоляТрансляциейПоНомеруАналитики(
				ВыражениеВычисляемогоПоля,
				"АдресАналитикиКоличество",
				"Аналитика",
	...
	ПоляРесурсовВалюта = Новый Соответствие;
	ПоляРесурсовВалюта.Вставить("СуммаВалПриход", "СуммаВалХраненияПриход");
	ПоляРесурсовВалюта.Вставить("СуммаВалРасход", "СуммаВалХраненияРасход");
	Если ИспользуютсяНачальныеОстатки Тогда
		ПоляРесурсовВалюта.Вставить("СуммаВалНачальныйОстаток", "СуммаВалХраненияНачальныйОстаток");
	КонецЕсли;
	
	Для Каждого ПолеРесурсаВалюта Из ПоляРесурсовВалюта Цикл
	
		Если ПоляРесурсов.Свойство(ПолеРесурсаВалюта.Ключ) Тогда
			...
			ВыражениеВычисляемогоПоля =
			"ВЫБОР КОГДА ПоказательБюджетов = НЕОПРЕДЕЛЕНО
			|	ИЛИ (НЕ УчитыватьПоВалюте ИЛИ НЕ ВалютаОпределяетсяАналитикой) ТОГДА ЕСТЬNULL(%ИмяРесурса%, 0)
			|	ИНАЧЕ
			|		ВЫРАЗИТЬ(БюджетнаяОтчетностьВыводСервер.Подключаемый_ПересчитатьПоКурсу(
			|								ЕСТЬNULL(%ИмяРесурса%, 0),
			|								%ИмяПериода%,
			|								ВалютаХранения,
			|								Сценарий,
			|								&КэшПлановыхКурсов,
			...
			ВыражениеВычисляемогоПоля = СтрЗаменить(ВыражениеВычисляемогоПоля, "%ИмяРесурса%", ПолеРесурсаВалюта.Значение);
...
Функция ПолучитьСКДФактическиеДанные(ИсточникиДанных, ПоляИсточников, СтруктураКолонокТаблицыДанных, Аналитики, ПараметрыКУстановке, РежимыФормирования)
	...
	ФинОтчеты = ФинансоваяОтчетностьСервер;
	СКДФактическиеДанные = КомпоновкаДанныхСервер.ПустаяСхема();
...
Функция ПолучитьСКДИтоговыеДанныеОстатки(ИсточникиДанных, ПоляИсточников, СтруктураКолонокТаблицыДанных)
	...
	ФинОтчеты = ФинансоваяОтчетностьСервер;
	СКДИтоговыеДанные = КомпоновкаДанныхСервер.ПустаяСхема();
...
Функция ПолучитьСКДИтоговыеДанныеОбороты(ИсточникиДанных, ПоляИсточников, СтруктураКолонокТаблицыДанных, ЕстьХранимыеПравила, ЕстьПривилегированныйРежим)
	...
	Для НомерИтерации = 1 По 2 Цикл
		
		КлючДанных = ?(НомерИтерации = 1, "ПлановыеДанные", "ПлановыеДанныеПривилегированно");
		Если НомерИтерации = 2 И Не ЕстьПривилегированныйРежим Тогда
			Продолжить;
		КонецЕсли;
		...
		ПлановыеДанные.ИмяОбъекта = КлючДанных;
	...
	Если ЕстьХранимыеПравила Тогда
		...
		Для НомерИтерации = 1 По 2 Цикл
			...
			КлючДанных = ?(НомерИтерации = 1, "ХранимыеФактическиеДанные", "ХранимыеФактическиеДанныеПривилегированно");
			Если НомерИтерации = 2 И Не ЕстьПривилегированныйРежим Тогда
				Продолжить;
			КонецЕсли;
			...
			ХранимыеФактическиеДанные.ИмяОбъекта = КлючДанных;
...
Функция ПолучитьСКДДанныхДокумента(Источник, СтруктураКолонокТаблицыДанных, Аналитики, АналитикиШапки, ПараметрыКУстановке)
	...
	НаборДанныхДокумента = КомпоновкаДанныхСервер.ДобавитьПустойНаборДанных(СКД,
		Тип("НаборДанныхЗапросСхемыКомпоновкиДанных"), "ЭкземплярБюджета_Данные");
	НаборДанныхДокумента.Запрос = 
	"ВЫБРАТЬ
	|	АналитикаСтатейБюджетов.СтатьяБюджетов КАК СтатьяБюджетов,
	|	&Документ_Сценарий КАК Сценарий,
	|	&Статус КАК Статус,
	|	&Документ_Организация КАК Организация,
	|	&Документ_Подразделение КАК Подразделение,
	|	&ТекстПолейАналитик,
	|	АналитикаСтатейБюджетов.Валюта КАК Валюта,
	|	&ТекстПолейНастроекСвязей,
	|	&ТекстПолейПараметровСтатей,
	|	НАЧАЛОПЕРИОДА(&ДатаАктуальности, ДЕНЬ) КАК ПериодДень,
	|	ОборотыПоСтатьямБюджетов.ПериодПланирования КАК ПериодПланирования,
	|	ОборотыПоСтатьямБюджетов.НомерПоПорядку КАК НомерПоПорядку,
	|	ОборотыПоСтатьямБюджетов.Примечание КАК Примечание,
	|	ОборотыПоСтатьямБюджетов.Сумма КАК Сумма,
	|	ОборотыПоСтатьямБюджетов.СуммаВВалюте КАК СуммаВВалюте,
	|	ОборотыПоСтатьямБюджетов.Количество КАК Количество
	|ИЗ
	|	АналитикаСтатейБюджетов КАК АналитикаСтатейБюджетов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОборотыПоСтатьямБюджетов КАК ОборотыПоСтатьямБюджетов
	...
	Для Сч = 1 По МаксимальноеКоличествоАналитик Цикл
		МассивПолейАналитик.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"АналитикаСтатейБюджетов.Аналитика%1 КАК Аналитика%1", Сч)); //@ query-part
	КонецЦикла;
	ТекстПолейАналитик = СтрСоединить(МассивПолейАналитик, "," + Символы.ПС + Символы.Таб);
	НаборДанныхДокумента.Запрос = СтрЗаменить(НаборДанныхДокумента.Запрос, "&ТекстПолейАналитик", ТекстПолейАналитик);
	...
	Если ЕстьВидыНоменклатуры Тогда
		...
		НаборДанныхДокумента.Запрос = НаборДанныхДокумента.Запрос + "
		|	ЛЕВОЕ СОЕДИНЕНИЕ КэшПараметровСтатей КАК КэшПараметровСтатей
		|	ПО АналитикаСтатейБюджетов.СтатьяБюджетов = КэшПараметровСтатей.СтатьяБюджетов"; // @query-part
		
		ПоляПараметровСтатей = Новый Массив;
		ПоляПараметровСтатей.Добавить("КэшПараметровСтатей.КоличествоИспользуемыхАналитик");
		Для НомерАналитики = 1 По МаксимальноеКоличествоАналитик Цикл
			ПоляПараметровСтатей.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"КэшПараметровСтатей.ВидАналитики%1", НомерАналитики));
		КонецЦикла;
		ПоляПараметровСтатей.Добавить("КэшПараметровСтатей.СтатьяУчитыватьПоКоличеству");
		ПоляПараметровСтатей.Добавить("КэшПараметровСтатей.СтатьяЕдиницаИзмеренияОпределяетсяАналитикой");
		ПоляПараметровСтатей.Добавить("КэшПараметровСтатей.СтатьяАдресАналитикиКоличество");
		ПоляПараметровСтатей.Добавить("КэшПараметровСтатей.СтатьяИсточникЗаполнениеЕдиницыИзмерения");
		ПоляПараметровСтатей.Добавить("КэшПараметровСтатей.СтатьяЕдиницаИзмерения");
		ТекстПолейПараметровСтатей = СтрСоединить(ПоляПараметровСтатей, "," + Символы.ПС + Символы.Таб);
		НаборДанныхДокумента.Запрос = СтрЗаменить(НаборДанныхДокумента.Запрос, "&ТекстПолейПараметровСтатей",
			ТекстПолейПараметровСтатей);
		
		Для Каждого Поле Из ПоляПараметровСтатей Цикл
			ФинОтчеты.НовоеПолеНабора(НаборДанныхДокумента, Сред(Поле, СтрНайти(Поле, ".") + 1));
		КонецЦикла;
	...
	Если Источник.ТипИсточника = Перечисления.ТипыИсточниковДанныхБюджетирования.Остатки Тогда
		...
		НаборДанныхДокумента.Запрос = НаборДанныхДокумента.Запрос + "
		|	ЛЕВОЕ СОЕДИНЕНИЕ НастройкиСвязей КАК НастройкиСвязей
		|	ПО АналитикаСтатейБюджетов.СтатьяБюджетов = НастройкиСвязей.СтатьяБюджетов"; // @query-part
		
		ПоляНастроекСвязей = Новый Массив;
		ПоляНастроекСвязей.Добавить("НастройкиСвязей.ПоказательБюджетов");
		ПоляНастроекСвязей.Добавить("НастройкиСвязей.ПоказательКоличествоИспользуемыхАналитик");
		Для НомерАналитики = 1 По МаксимальноеКоличествоАналитик Цикл
			ПоляНастроекСвязей.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"НастройкиСвязей.ПоказательВидАналитики%1", НомерАналитики));
		КонецЦикла;
		ПоляНастроекСвязей.Добавить("НастройкиСвязей.УчитыватьПоВалюте");
		ПоляНастроекСвязей.Добавить("НастройкиСвязей.АдресАналитикиВалюта");
		ПоляНастроекСвязей.Добавить("НастройкиСвязей.ИсточникЗаполненияВалюты");
		ПоляНастроекСвязей.Добавить("НастройкиСвязей.ВалютаОпределяетсяАналитикой");
		ПоляНастроекСвязей.Добавить("НастройкиСвязей.ПоказательУчитыватьПоКоличеству");
		ПоляНастроекСвязей.Добавить("НастройкиСвязей.ПоказательЕдиницаИзмеренияОпределяетсяАналитикой");
		ПоляНастроекСвязей.Добавить("НастройкиСвязей.ПоказательАдресАналитикиКоличество");
		ПоляНастроекСвязей.Добавить("НастройкиСвязей.ПоказательИсточникЗаполнениеЕдиницыИзмерения");
		ПоляНастроекСвязей.Добавить("НастройкиСвязей.ПоказательЕдиницаИзмерения");
		ТекстПолейНастроекСвязей = СтрСоединить(ПоляНастроекСвязей, "," + Символы.ПС + Символы.Таб);
		НаборДанныхДокумента.Запрос = СтрЗаменить(НаборДанныхДокумента.Запрос, "&ТекстПолейНастроекСвязей",
			ТекстПолейНастроекСвязей);
		
		Для Каждого Поле Из ПоляНастроекСвязей Цикл
			ФинОтчеты.НовоеПолеНабора(НаборДанныхДокумента, Сред(Поле, СтрНайти(Поле, ".") + 1));
		КонецЦикла;
		
		НаборДанныхДокумента.Запрос = СхемыЗапросов.УстановитьПомещениеВоВременнуюТаблицу(НаборДанныхДокумента.Запрос,
			"ДанныеДокумента");
		НаборДанныхДокумента.Запрос = СхемыЗапросов.УстановитьИндексациюПолейЗапроса(НаборДанныхДокумента.Запрос,
		...
		Если ПустаяСтрока(ТекстЗапроса) Тогда
			ТекстЗапроса = СтрЗаменить(БюджетированиеСервер.ПолучитьТекстЗапросаКИсточникуДанныхПоказателейБезПравилРасчета(),
				"&ТекстУсловия", "ИСТИНА");
			ОписаниеТаблицы = СхемыЗапросов.ОписаниеТаблицы(Тип("ОписаниеВременнойТаблицыСхемыЗапроса"), "ДанныеДокумента",
				"ПсевдонимТаблицыИсточника", "СтатьяБюджетов,ПоказательБюджетов", ТекстЗапроса);
			ОписаниеСоединения = СхемыЗапросов.ОписаниеСоединения(ОписаниеТаблицы,
				...
				|	И ПравилаРасчета.Приемник = ПсевдонимТаблицыИсточника.ПоказательБюджетов",
		...
		Если Не ПустаяСтрока(ТекстПолейПараметровСтатей) Тогда
			ТекстПолейПараметровСтатей = СтрЗаменить(ТекстПолейПараметровСтатей, "КэшПараметровСтатей", "ПсевдонимТаблицыИсточника");
		КонецЕсли;
		ТекстПолейНастроекСвязей = СтрЗаменить(ТекстПолейНастроекСвязей, "НастройкиСвязей", "ПсевдонимТаблицыИсточника");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, Символы.ПС + "ИЗ", ?(ПустаяСтрока(ТекстПолейПараметровСтатей), "", "," // @query-part
			+ Символы.ПС + Символы.Таб + ТекстПолейПараметровСтатей) + "," + Символы.ПС + Символы.Таб + ТекстПолейНастроекСвязей
			+ Символы.ПС + "ИЗ"); // @query-part
		
		ПоляКУдалению = Новый Массив;
		ПоляКУдалению.Добавить("СуммаУпрОборот");
		ПоляКУдалению.Добавить("СуммаРеглОборот");
		Для НомерАналитики = 1 По МаксимальноеКоличествоАналитик Цикл
			ПоляКУдалению.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Аналитика%1Источника",
				НомерАналитики));
		КонецЦикла;
		ТекстЗапроса = СхемыЗапросов.УдалитьПоляИзЗапроса(ТекстЗапроса, ПоляКУдалению);
		
		НаборДанныхДокумента.Запрос = НаборДанныхДокумента.Запрос + ОбщегоНазначения.РазделительПакетаЗапросов()
			+ ТекстЗапроса;
		
		Для Каждого ПараметрЗапроса Из ЗапросПоПравиламРасчета.ПараметрыЗапроса Цикл
			КомпоновкаДанныхСервер.ДобавитьПараметр(СКД, ПараметрЗапроса.Ключ, , ПараметрЗапроса.Значение, Истина, Истина);
		КонецЦикла;
		
		ФинОтчеты.УстановитьОтбор(СКД.НастройкиПоУмолчанию.Отбор, "ПоказательБюджетов", , ВидСравненияКомпоновкиДанных.Заполнено);
		
		ФинОтчеты.НовоеПолеНабора(НаборДанныхДокумента, "Расход");
		ФинОтчеты.НовоеПолеНабора(НаборДанныхДокумента, "Валюта", "ВалютаХранения");
		
	Иначе
		
		НаборДанныхДокумента.Запрос = СтрЗаменить(НаборДанныхДокумента.Запрос, "&ТекстПолейНастроекСвязей,", "");
		
		ФинОтчеты.НовоеПолеНабора(НаборДанныхДокумента, "Валюта");
		
	КонецЕсли;
	
	ФинОтчеты.НовоеПолеНабора(НаборДанныхДокумента, "ИдентификаторСтроки");
	ФинОтчеты.НовоеПолеНабора(НаборДанныхДокумента, "СтатьяБюджетов", , , ТипКолонкиСтруктурыЯчеек("СтатьяБюджетов"));
	ФинОтчеты.НовоеПолеНабора(НаборДанныхДокумента, "Статус", , , Новый ОписаниеТипов("ПеречислениеСсылка.СтатусыПланов"));
	ФинОтчеты.НовоеПолеНабора(НаборДанныхДокумента, "Сценарий", , , ТипКолонкиСтруктурыЯчеек("Сценарий"));
	ФинОтчеты.НовоеПолеНабора(НаборДанныхДокумента, "Организация", , , ТипКолонкиСтруктурыЯчеек("Организация"));
	ФинОтчеты.НовоеПолеНабора(НаборДанныхДокумента, "Подразделение", , , ТипКолонкиСтруктурыЯчеек("Подразделение"));
	Для Сч = 1 По МаксимальноеКоличествоАналитик Цикл
		ФинОтчеты.НовоеПолеНабора(НаборДанныхДокумента, "Аналитика" + Сч, , , ТипКолонкиСтруктурыЯчеек("Аналитика"));
	КонецЦикла;
	ФинОтчеты.НовоеПолеНабора(НаборДанныхДокумента, "ПериодПланирования", , , Новый ОписаниеТипов("Дата"));
	ФинОтчеты.НовоеПолеНабора(НаборДанныхДокумента, "ПериодДень", , , Новый ОписаниеТипов("Дата"));
	ФинОтчеты.НовоеПолеНабора(НаборДанныхДокумента, "НомерПоПорядку");
	ФинОтчеты.НовоеПолеНабора(НаборДанныхДокумента, "Примечание");
	ФинОтчеты.НовоеПолеНабора(НаборДанныхДокумента, "Количество");
	ФинОтчеты.НовоеПолеНабора(НаборДанныхДокумента, "Сумма");
	ФинОтчеты.НовоеПолеНабора(НаборДанныхДокумента, "СуммаВВалюте");
	...
	Если Источник.ТипИсточника = Перечисления.ТипыИсточниковДанныхБюджетирования.Остатки Тогда
		...
		ФинОтчеты.НовоеВычисляемоеПоле(СКД, "СуммаПриход", "ВЫБОР КОГДА НЕ Расход ТОГДА Сумма ИНАЧЕ 0 КОНЕЦ");
		ФинОтчеты.НовоеВычисляемоеПоле(СКД, "СуммаРасход", "ВЫБОР КОГДА Расход ТОГДА Сумма ИНАЧЕ 0 КОНЕЦ");
		
		ТекстыАналитик = Новый Соответствие;
		
		Для Сч = 1 По МаксимальноеКоличествоАналитик Цикл
			ТекстАналитики = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Аналитика%1", Сч);
			ФинОтчеты.НовоеВычисляемоеПоле(СКД, ТекстАналитики, ТекстАналитики);
...
Функция ПолучитьСКДЗначенийДокумента(Источник, СтруктураКолонокТаблицыДанных, Аналитики, АналитикиШапки, ПараметрыКУстановке)
	...
	НаборЗначенийДокумента = КомпоновкаДанныхСервер.ДобавитьПустойНаборДанных(СКД,
		Тип("НаборДанныхЗапросСхемыКомпоновкиДанных"), "ЭкземплярБюджета_Значения");
	НаборЗначенийДокумента.Запрос = 
	"ВЫБРАТЬ
	|	АналитикаЗначений.РедактируемоеЗначение КАК РедактируемоеЗначение,
	|	&ТекстПолейВидовАналитик,
	|	&ТекстПолейАналитик,
	|	АналитикаЗначений.КоличествоИспользуемыхАналитик КАК КоличествоИспользуемыхАналитик,
	|	ОборотыПоСтатьямБюджетов.ПериодПланирования КАК ПериодПланирования,
	|	ОборотыПоСтатьямБюджетов.НомерПоПорядку КАК НомерПоПорядку,
	|	ОборотыПоСтатьямБюджетов.Примечание КАК Примечание,
	|	ОборотыПоСтатьямБюджетов.Значение КАК Значение
	|ИЗ
	|	АналитикаЗначений КАК АналитикаЗначений
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОборотыПоСтатьямБюджетов КАК ОборотыПоСтатьямБюджетов
	|		ПО АналитикаЗначений.ИдентификаторСтроки = ОборотыПоСтатьямБюджетов.ИдентификаторСтроки";
	
	МаксимальноеКоличествоАналитик = БюджетированиеКлиентСервер.МаксимальноеКоличествоАналитик();
	ВсеТипыАналитик = БюджетированиеСервер.ВсеТипыАналитик();
	
	МассивПолейАналитик = Новый Массив;
	МассивПолейВидовАналитик = Новый Массив;
	ФинОтчеты.НовоеПолеНабора(НаборЗначенийДокумента, "ИдентификаторСтроки");
	Для Сч = 1 По МаксимальноеКоличествоАналитик Цикл
		МассивПолейАналитик.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"АналитикаЗначений.Аналитика%1 КАК Аналитика%1", Сч)); //@ query-part
		МассивПолейВидовАналитик.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"АналитикаЗначений.ВидАналитики%1 КАК ВидАналитики%1", Сч)); //@ query-part
		ФинОтчеты.НовоеПолеНабора(НаборЗначенийДокумента, "Аналитика" + Сч,,,ВсеТипыАналитик);
		ФинОтчеты.НовоеПолеНабора(НаборЗначенийДокумента, "ВидАналитики" + Сч);
	КонецЦикла;
	ТекстПолейАналитик = СтрСоединить(МассивПолейАналитик, "," + Символы.ПС + Символы.Таб);
	ТекстПолейВидовАналитик = СтрСоединить(МассивПолейВидовАналитик, "," + Символы.ПС + Символы.Таб);
	НаборЗначенийДокумента.Запрос = СтрЗаменить(НаборЗначенийДокумента.Запрос, "&ТекстПолейАналитик", ТекстПолейАналитик);
	НаборЗначенийДокумента.Запрос = СтрЗаменить(НаборЗначенийДокумента.Запрос, "&ТекстПолейВидовАналитик",
		ТекстПолейВидовАналитик);
	
	ФинОтчеты.НовоеПолеНабора(НаборЗначенийДокумента, "КоличествоИспользуемыхАналитик");
	ФинОтчеты.НовоеПолеНабора(НаборЗначенийДокумента, "РедактируемоеЗначение", , ,
		ТипКолонкиСтруктурыЯчеек("РедактируемоеЗначение"));
	
	ФинОтчеты.НовоеПолеНабора(НаборЗначенийДокумента, "ПериодПланирования", , , Новый ОписаниеТипов("Дата"));
	ФинОтчеты.НовоеПолеНабора(НаборЗначенийДокумента, "НомерПоПорядку");
	ФинОтчеты.НовоеПолеНабора(НаборЗначенийДокумента, "Примечание");
	ФинОтчеты.НовоеПолеНабора(НаборЗначенийДокумента, "ИдентификаторСтроки");
	ФинОтчеты.НовоеПолеНабора(НаборЗначенийДокумента, "Значение", "ЗначениеПоказателя");
...
Процедура СоздатьРекурсивноГруппировкиДереваВывода(ВидБюджета, КэшДопРеквизиты, Знач МестоДобавления, Элементы)
	...
	Для Каждого Элемент Из Элементы.Строки Цикл
		...
		ИначеЕсли Элемент.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.ТаблицаПоказателиВКолонках
			...
			ВыводитьЗаголовкиПоВертикали = ФинОтчеты.ЗначениеДополнительногоРеквизита(Элемент.ЭлементОтчета, "ВыводитьЗаголовкиПоВертикали", КэшДопРеквизиты);
			...
			Таблица.ПараметрыТаблицы.Вставить("ВыводитьЗаголовкиПоВертикали", ВыводитьЗаголовкиПоВертикали = Истина);
