...
Функция ПолучитьТокенАвторизации(ИдентификаторУчетнойЗаписи, ДанныеМаркера, КонтекстДиагностики) Экспорт
	...
	ПараметрыВыполнения.ПредставлениеОперации = НСтр("ru = 'Получение токена доступа к сервису ЭДО.';
													|en = 'Get the access token to the EDI service.'");
...
Функция ДанныеРегистрационнойИнформации(ПараметрыРегистрации, ДанныеОператора, Ошибки)
	...
	Если ЭтоПодписаниеСертификатомРуководителя И Не ЭтоПодписаниеПоМЧД
		...
		ТекстОшибки = НСтр("ru = 'ОГРН в выбранном сертификате ""%1"" не соответствует ОГРН в организации ""%2"".
			...
			|en = 'The primary state registration number in the selected ""%1"" certificate does not match the primary state registration number in the ""%2"" company.
...
Функция АктуализироватьТокенПриВыполненииОперации(ПараметрыОперации, КонтекстДиагностики, КлючСинхронизации = Неопределено)
	...
	Если УниверсальнаяДатаПроверки > ТокенАвторизации.СрокДействияТокенаОбновления Тогда
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Истек срок действия токена доступа для учетной записи ЭДО %1';
										|en = 'The access token for the %1 EDI account expired'"),
			ИдентификаторУчетнойЗаписи);
...
Функция ОбновитьТокенАвторизации(ИдентификаторУчетнойЗаписи, ТокенОбновления, КонтекстДиагностики)
	...
	ПараметрыВыполнения.ПредставлениеОперации = НСтр("ru = 'Обновление токена доступа к сервису ЭДО.';
													|en = 'Update the access token to the EDI service.'");
