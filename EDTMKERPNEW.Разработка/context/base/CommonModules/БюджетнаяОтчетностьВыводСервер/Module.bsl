...
//	  * Колонка - СписокЗначений из СправочникСсылка.ЭлементыФинансовыхОтчетов, Неопределено - список колонок к выводу
//	  * Строка - СписокЗначений из СправочникСсылка.ЭлементыФинансовыхОтчетов, Неопределено - список строк к выводу
...
//	  *ВыводитьИерархиюЭлементов - Булево - признак вывода в иерархии
//	  *ВыражениеПредставления - Строка - выражение представления аналитики
//	  *ВыражениеСортировки - Строка - выражение для сортировки аналитики
//
...
//
// Возвращаемое значение:
//	Массив из СтрокаТаблицыЗначений - массив строк, найденных в таблице ДанныеЯчеек.
//
Функция ПолучитьВыборкуЯчеек(ДанныеЯчеек, НакопленныйОтбор, НеобязательныеПараметры = Неопределено)
...
Функция ПолучитьВыборкуЯчеекИерархически(ДанныеЯчеек, Измерения, НакопленныйОтбор,
	...
	Для Каждого КолонкаДанныеЯчеек Из ДанныеЯчеек.Колонки Цикл
		Если СтрНайти(КолонкаДанныеЯчеек.Имя, "Сумма") > 0 Или СтрНайти(КолонкаДанныеЯчеек.Имя, "Количество") > 0 Тогда
			ИменаКолонокРесурсов.Добавить(КолонкаДанныеЯчеек.Имя);
		КонецЕсли;
	КонецЦикла;
	Если ИменаКолонокРесурсов.Количество() = 0 Тогда
		ИменаКолонокРесурсов.Добавить("ЗначениеПоказателя");
	КонецЕсли;
	...
	Если ТипЗнч(Измерения) = Тип("Массив") Тогда
		...
		Для Каждого ИмяКолонки Из ИменаКолонокРесурсов Цикл
	...
	Если СписокИзмерений.Количество() > 1
		...
		ТекстВыраженияНет = " И (НЕ ЗначениеЗаполнено (" + СтрСоединить(СписокИзмерений, ") ИЛИ НЕ ЗначениеЗаполнено(")
			+ "))";
		ТекстУсловия = СтрСоединить(ИменаКолонокРесурсов, " = 0 ИЛИ ") + " = 0";
...
Функция ДанныеЭкземпляраБюджета(Параметры) Экспорт
	...
	Если РежимФормирования = Перечисления.РежимыФормированияБюджетныхОтчетов.Документ
		...
		ИначеЕсли Результат.Свойство("СтрокаСортировкиДанныхЯчеек") Тогда
...
Процедура ПоместитьДанныеОборотовБюджетовВХранилище(Параметры, АдресХранилища) Экспорт
	...
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(Настройки, "ДатаАктуальностиДанных", Параметры.ДатаАктуальности);
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(Настройки, "Валюта", Параметры.ВалютаОтчета);
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(Настройки, "Период", Новый СтандартныйПериод(Параметры.НачалоПериода, Параметры.КонецПериода));
	КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(Настройки, "СтатьяБюджетов", Параметры.ВидБюджета);
	КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(Настройки, "Сценарий", Параметры.Сценарий);
	Если Параметры.Свойство("Организация") Тогда
		КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(Настройки, "Организация", Параметры.Организация, ВидСравненияКомпоновкиДанных.ВСписке);
	КонецЕсли;
	Если Параметры.Свойство("Подразделение") Тогда
		КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(Настройки, "Подразделение", Параметры.Подразделение, ВидСравненияКомпоновкиДанных.ВСписке);
	КонецЕсли;
...
// Возвращаемое значение:
// 	Структура - Описание:
// 	*Результат - Соответствие из КлючИЗначение - содержит:
...
Процедура ДополнитьНастройкиСКДПередВыводом(Настройки, ПараметрыДанных) Экспорт
	...
	Структура = ОбщегоНазначения.СкопироватьРекурсивно(ПараметрыДанных.ДополнительныеФильтрыПоАналитикам);
...
Функция ПолучитьДанныеПоИсточникам(ВидБюджета, СКДДанных, ПараметрыДанных, ИсточникиДанных, СтруктураЯчеек, КэшДанных)
	...
	Если Не ПустаяСтрока(ПараметрыДанных.ТекстЗапросаТаблицыДокумента) Тогда
		МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
		Запрос = Новый Запрос(ПараметрыДанных.ТекстЗапросаТаблицыДокумента);
		Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
		Запрос.УстановитьПараметр("АналитикаСтатейБюджетов", ПараметрыДанных.АналитикаСтатейБюджетов);
		Запрос.УстановитьПараметр("ОборотыПоСтатьямБюджетов", ПараметрыДанных.ОборотыПоСтатьямБюджетов);
		Запрос.УстановитьПараметр("АналитикаЗначений", ПараметрыДанных.АналитикаЗначений);
		Запрос.Выполнить();
	КонецЕсли;
	...
	ЗапросыПоНаличиюПлановыхОстатков = Новый Соответствие;
	ШаблонЗапросаПоНаличиюПлановыхОстатков =
		...
		|ГДЕ
		|	НачальныеОстаткиРасчетныхПоказателей.ДатаОстатков = &НачалоПериода
		|	И НачальныеОстаткиРасчетныхПоказателей.ПоказательБюджетов В(&ВыбранныеПоказателиБюджетовНомерИД)";
	
	Для Каждого ИсточникДанных Из ИсточникиДанных Цикл
		Если ИсточникДанных.ТипИсточника = Перечисления.ТипыИсточниковДанныхБюджетирования.Остатки Тогда
			ТекстЗапросаПоНаличиюПлановыхОстатков = СтрЗаменить(ШаблонЗапросаПоНаличиюПлановыхОстатков, "НомерИД", ИсточникДанных.Идентификатор);
			ТекстЗапросаПоНаличиюПлановыхОстатков = СтрЗаменить(ТекстЗапросаПоНаличиюПлановыхОстатков, "&Идентификатор", """" + ИсточникДанных.Идентификатор + """");
			ЗапросыПоНаличиюПлановыхОстатков.Вставить(ИсточникДанных.Идентификатор, ТекстЗапросаПоНаличиюПлановыхОстатков);
		КонецЕсли;
	КонецЦикла;
	
	// 1 - Плановые данные
	// 2 - Плановые данные привилегированно
	Для НомерИтерации = 1 По 2 Цикл
		
		КлючДанных = ?(НомерИтерации = 1, "ПлановыеДанные", "ПлановыеДанныеПривилегированно");
		ИмяСКДДанных = ?(НомерИтерации = 1, "СКДПлановыеДанные", "СКДПлановыеДанныеПривилегированно");
		
		СКДПлановыеДанные = Неопределено;
		
		Если КэшДанных.Свойство(КлючДанных) Тогда
			
			ПлановыеДанные = КэшДанных[КлючДанных];
			
		ИначеЕсли СКДДанных.Свойство(ИмяСКДДанных, СКДПлановыеДанные)
			...
			Если ЗапросыПоНаличиюПлановыхОстатков.Количество() Тогда
				ЗапросПоНаличиюПлановыхОстатков = Новый Запрос;
				ЗапросПоНаличиюПлановыхОстатков.УстановитьПараметр("НачалоПериода", НачалоПериода);
				Для Каждого КлючИЗначение Из ЗапросыПоНаличиюПлановыхОстатков Цикл
					Если ПустаяСтрока(ЗапросПоНаличиюПлановыхОстатков.Текст) Тогда
						ЗапросПоНаличиюПлановыхОстатков.Текст = КлючИЗначение.Значение;
					Иначе
						ЗапросПоНаличиюПлановыхОстатков.Текст = ЗапросПоНаличиюПлановыхОстатков.Текст +
						ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении() + СтрЗаменить(КлючИЗначение.Значение, "РАЗРЕШЕННЫЕ", "");
					КонецЕсли;
					ИмяПараметраПоказателиБюджетов = "ВыбранныеПоказателиБюджетов" + КлючИЗначение.Ключ;
					ЗапросПоНаличиюПлановыхОстатков.УстановитьПараметр(ИмяПараметраПоказателиБюджетов,
						СКДДанных.Параметры[ИмяПараметраПоказателиБюджетов]);
				КонецЦикла;
				//@skip-check query-in-loop
				Результат = ЗапросПоНаличиюПлановыхОстатков.Выполнить();
				Если Результат.Пустой() Тогда
					ИдентификаторыСПлановымиОстатками = Новый Массив;
				Иначе
					ИдентификаторыСПлановымиОстатками = Результат.Выгрузить().ВыгрузитьКолонку("Идентификатор");
				КонецЕсли;
				Для Каждого КлючИЗначение Из ЗапросыПоНаличиюПлановыхОстатков Цикл
					Если ИдентификаторыСПлановымиОстатками.Найти(КлючИЗначение.Ключ) = Неопределено Тогда
						НаборДанныхНачальныеОстатки = СКДПлановыеДанные.НаборыДанных[0].Элементы.Найти(
							"Набор_План_НачальныеОстатки_" + КлючИЗначение.Ключ);
						Если НаборДанныхНачальныеОстатки <> Неопределено Тогда
							СКДПлановыеДанные.НаборыДанных[0].Элементы.Удалить(НаборДанныхНачальныеОстатки);
						КонецЕсли;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
			...
			КонецЕсли;
			Если ПлановыеДанные.Колонки.Найти("СуммаВалНачальныйОстаток") <> Неопределено Тогда
				ПоляПересчета.Добавить(Новый Структура("ПолеПересчета, ПолеФункциональнойВалюты, ПолеВыходныхДанных, ПолеВыходныхДанныхВВалюте",
					"СуммаВалНачальныйОстаток", "СуммаНачальныйОстаток", "СуммаНачальныйОстаток", "СуммаВалНачальныйОстаток"));
			КонецЕсли;
			
			Если ПоляПересчета.Количество() Тогда
				ПараметрыТаблицыДанных = БюджетированиеСервер.ПараметрыПересчетаТаблицыДанныхВВалюту(ПлановыеДанные);
				БюджетированиеСервер.ПересчитатьТаблицуДанныхВВалюту(ПараметрыТаблицыДанных, ПараметрыДанных.ВалютаОтчета,
					ФункциональнаяВалюта, ПоляПересчета);
				ПлановыеДанные = ПараметрыТаблицыДанных.ТаблицаДанных;
			КонецЕсли;
			
			КэшДанных.Вставить(КлючДанных, ПлановыеДанные);
		...
		ВнешниеНаборы.Вставить(КлючДанных, ПлановыеДанные);
	...
	Если СКДДанных.Свойство("СКДДанныеДокумента", СКДДанныеДокумента) И СКДДанныеДокумента <> Неопределено Тогда
		...
		Если МенеджерВременныхТаблиц.Таблицы.Найти("НастройкиСвязей") = Неопределено Тогда
			
			ПравилаРасчета = Новый Массив;
			Для Счетчик = 0 По ИсточникиДанных.ВГраница() Цикл
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ПравилаРасчета, ИсточникиДанных[Счетчик].ПравилаРасчета, Истина);
			КонецЦикла;
			МаксимальноеКоличествоАналитик = БюджетированиеКлиентСервер.МаксимальноеКоличествоАналитик();
			ТекстЗапроса =
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			|	ВЫРАЗИТЬ(ПравилаРасчета.Приемник КАК Справочник.ПоказателиБюджетов).КоличествоИспользуемыхАналитик КАК ПоказательКоличествоИспользуемыхАналитик,
			|	ВЫРАЗИТЬ(ПравилаРасчета.Приемник КАК Справочник.ПоказателиБюджетов).УчитыватьПоВалюте КАК УчитыватьПоВалюте,
			|	ВЫРАЗИТЬ(ПравилаРасчета.Приемник КАК Справочник.ПоказателиБюджетов).ВалютаОпределяетсяАналитикой КАК ВалютаОпределяетсяАналитикой,
			|	ВЫРАЗИТЬ(ПравилаРасчета.Приемник КАК Справочник.ПоказателиБюджетов).АдресАналитикиВалюта КАК АдресАналитикиВалюта,
			|	ВЫРАЗИТЬ(ПравилаРасчета.Приемник КАК Справочник.ПоказателиБюджетов).АналитикаВалюты.ЗаполнениеВалюты КАК ИсточникЗаполненияВалюты,
			|	ВЫРАЗИТЬ(ПравилаРасчета.Приемник КАК Справочник.ПоказателиБюджетов).УчитыватьПоКоличеству КАК ПоказательУчитыватьПоКоличеству,
			|	ВЫРАЗИТЬ(ПравилаРасчета.Приемник КАК Справочник.ПоказателиБюджетов).ЕдиницаИзмеренияОпределяетсяАналитикой КАК ПоказательЕдиницаИзмеренияОпределяетсяАналитикой,
			|	ВЫРАЗИТЬ(ПравилаРасчета.Приемник КАК Справочник.ПоказателиБюджетов).АдресАналитикиКоличество КАК ПоказательАдресАналитикиКоличество,
			|	ВЫРАЗИТЬ(ПравилаРасчета.Приемник КАК Справочник.ПоказателиБюджетов).АналитикаЕдиницыИзмерения.ЗаполнениеЕдиницыИзмерения КАК ПоказательИсточникЗаполнениеЕдиницыИзмерения,
			|	ВЫРАЗИТЬ(ПравилаРасчета.Приемник КАК Справочник.ПоказателиБюджетов).ЕдиницаИзмерения КАК ПоказательЕдиницаИзмерения
			...
			Для НомерАналитики = 1 По МаксимальноеКоличествоАналитик Цикл
				...
				ТекстВидыАналитикПоказателей = ТекстВидыАналитикПоказателей + ?(ТекстВидыАналитикПоказателей = "", "", "," + Символы.ПС + Символы.Таб)
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						"ВЫРАЗИТЬ(ПравилаРасчета.Приемник КАК Справочник.ПоказателиБюджетов).ВидАналитики%1 КАК ПоказательВидАналитики%1",
						НомерАналитики);
			...
			Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
			Запрос.Текст = ТекстЗапроса;
			Запрос.УстановитьПараметр("ПравилаРасчета", ПравилаРасчета);
		...
		Если МенеджерВременныхТаблиц.Таблицы.Найти("КэшПараметровСтатей") = Неопределено Тогда
		...
		Для Каждого КлючИЗначение Из СКДДанныеДокумента Цикл
			...
			Иначе
				...
				ФинОтчеты.УстановитьПараметрКомпоновки(Компоновщик, "КонецПериода",         КонецПериода);
				ФинОтчеты.УстановитьПараметрКомпоновки(Компоновщик, "ГраницаФактДанных",    ГраницаФактДанных);
				ФинОтчеты.УстановитьПараметрКомпоновки(Компоновщик, "Статус",               ПараметрыДанных.Статус);
			...
			ДанныеТаблицыДокумента = ФинОтчеты.ВыгрузитьРезультатСКДПоМакету(МакетКомпоновки, , , МенеджерВременныхТаблиц);
	...
	// 1 - Хранимые фактические данные
	// 2 - Хранимые фактические данные привилегированно
	Для НомерИтерации = 1 По 2 Цикл
		
		КлючДанных = ?(НомерИтерации = 1, "ХранимыеФактическиеДанные", "ХранимыеФактическиеДанныеПривилегированно");
		ИмяСКДДанных = ?(НомерИтерации = 1, "СКДФактическиеДанные", "СКДФактическиеДанныеПривилегированно");
		
		СКДФактическиеДанные = Неопределено;
		
		Если КэшДанных.Свойство(КлючДанных) Тогда
			
			ХранимыеФактическиеДанные = КэшДанных[КлючДанных];
			
		ИначеЕсли СКДДанных.Свойство(ИмяСКДДанных, СКДФактическиеДанные)
			...
			Если ПоляПересчета.Количество() Тогда
				ПараметрыТаблицыДанных = БюджетированиеСервер.ПараметрыПересчетаТаблицыДанныхВВалюту(ХранимыеФактическиеДанные);
				БюджетированиеСервер.ПересчитатьТаблицуДанныхВВалюту(ПараметрыТаблицыДанных, ПараметрыДанных.ВалютаОтчета,
					ФункциональнаяВалюта, ПоляПересчета);
				ХранимыеФактическиеДанные = ПараметрыТаблицыДанных.ТаблицаДанных;
			КонецЕсли;
			
			КэшДанных.Вставить(КлючДанных, ХранимыеФактическиеДанные);
		...
		ВнешниеНаборы.Вставить(КлючДанных, ХранимыеФактическиеДанные);
...
//
// Возвращаемое значение:
//   Массив из Дата - массив периодов
//
Функция ПериодыБюджетногоОтчета(Знач НачалоПериода, Знач КонецПериода, Периодичность) Экспорт
	
	Результат = Новый Массив;
	НачалоПериода = РаботаСПериодамиКлиентСервер.ДатаНачалаПериода(НачалоПериода, Периодичность);
	Пока НачалоПериода <= КонецПериода Цикл
		Результат.Добавить(НачалоПериода);
		НачалоПериода = РаботаСПериодамиКлиентСервер.ДобавитьИнтервал(НачалоПериода, Периодичность, 1);
	КонецЦикла;
...
//
// Параметры:
//   Периодичности - Массив из ПеречислениеСсылка.Периодичность - значения периодичностей, которые планируется добавить в таблицу
//   НачалоПериода - Дата - начало периода
...
Функция ТаблицаПериодов(Периодичности, НачалоПериода, КонецПериода) Экспорт
	...
	Для Каждого Период Из МассивПериодов Цикл
		...
		Для Каждого Периодичность Из Периодичности Цикл
			ИмяПериода = ФинансоваяОтчетностьКлиентСервер.СтрокиПериода(Периодичность).Период;
			НоваяСтрока[ИмяПериода] = РаботаСПериодамиКлиентСервер.ДатаНачалаПериода(Период, Периодичность);
		КонецЦикла;
	КонецЦикла;
...
Процедура ВывестиШапкуТаблицыВОтчет(ПараметрыВывода, ДокументРезультат, СтруктураОбластей, СтруктураКолонок, СтрокаТаблицы, НомерТаблицы)
	...
	ВыводитьЗаголовкиПоВертикали = ПараметрыВывода.ПараметрыТаблицы.Свойство("ВыводитьЗаголовкиПоВертикали")
			И ПараметрыВывода.ПараметрыТаблицы.ВыводитьЗаголовкиПоВертикали = Истина;
	...
	Для Сч = 1 По КоличествоУровней Цикл
		...
		Если СоответствиеЭлементовПоУровням.Количество() Тогда // проверка на наличие значений колонок
			...
			Для Каждого ЭлементУровня Из ЭлементыУровня Цикл
				...
				Если ВыводитьЗаголовкиПоВертикали Тогда
					ВыведеннаяОбласть.ШиринаКолонки = 3;
					ВыведеннаяОбласть.ОриентацияТекста = 90;
				КонецЕсли;
		...
		ИначеЕсли ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.ПроизводныйПоказатель
			...
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				...
				Если СтрокиСсылки.Количество() Тогда
					...
					Если ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.ФормулаПоГруппе
						И ДобавленныеПериодичности.Количество() Тогда
						// Необходимо отсортировать массив строк по значениям минимальной периодичности
						Для Счетчик = 0 По СтрокиСсылки.ВГраница() Цикл
							Для ОбратныйСчетчик = 0 По СтрокиСсылки.ВГраница() - Счетчик - 1 Цикл
								Если СтрокиСсылки[ОбратныйСчетчик][ДобавленныеПериодичности[0]] = Неопределено
									Или СтрокиСсылки[ОбратныйСчетчик+1][ДобавленныеПериодичности[0]] = Неопределено Тогда
									Продолжить;
								КонецЕсли;
								Если СтрокиСсылки[ОбратныйСчетчик][ДобавленныеПериодичности[0]]
									> СтрокиСсылки[ОбратныйСчетчик+1][ДобавленныеПериодичности[0]] Тогда
									СдвигаемаяСтрока = СтрокиСсылки[ОбратныйСчетчик];
									СтрокиСсылки[ОбратныйСчетчик] = СтрокиСсылки[ОбратныйСчетчик+1];
									СтрокиСсылки[ОбратныйСчетчик+1] = СдвигаемаяСтрока;
								КонецЕсли;
							КонецЦикла;
						КонецЦикла;
					КонецЕсли;
...
Функция ТекстГруппировки(Значение, СтрокаОтчета, ПараметрыВывода, ЭтоКолонки = Ложь)
	...
	Если ВидЭлемента = ВидыЭлементов.Измерение И СтрНайти(ИмяИзмерения, "Период") Тогда
		...
		Возврат РаботаСПериодамиКлиентСервер.ПредставлениеПериодаПоДате(Значение, Периодичность);
...
Функция РассчитатьСвязанныеСтатьиБюджетов(ПараметрыВыводаВидаБюджета, АдресДанныхЯчеек, ПараметрыДанных,
	...
	Иначе
		...
		Для Каждого СвязаннаяЯчейка Из СвязанныеЯчейки Цикл
			...
			Если Не НайденныеСтроки.Количество() Тогда
				...
				НайденныеСтроки = ДанныеЯчеек.НайтиСтроки(СтруктураПоискаЯчейки);
				Если НайденныеСтроки.Количество() Тогда
					ЗаполнитьЗначенияСвойств(НоваяСтрока, НайденныеСтроки[0], , СвязаннаяЯчейка.ОбщиеИзмерения);
				КонецЕсли;
				СтрокиКРасчету.Добавить(НоваяСтрока);
			Иначе
				Для Каждого СтрокаТаблицы Из НайденныеСтроки Цикл
	...
	Для Каждого СтрокаТаблицы Из СтрокиКРасчету Цикл
		...
		РасшифровкаКоличество = ОбщегоНазначения.СкопироватьРекурсивно(РасшифровкаСумма);
...
Функция СтруктураОбластейМакетаДляВыводаТаблицы(ПараметрыВывода, Уровень)
	
	СтруктураОбластей = ПараметрыВывода.СтруктураОбластей;
	ОтображатьНумерациюСтрок = ПараметрыВывода.ПараметрыТаблицы.ОтображатьНумерациюСтрок;
	ВыводитьПримечания = ПараметрыВывода.ВыводитьПримечания;
	
	ОтрабатываетсяУровней = 8;
	
	УровеньГруппировок = Мин(Уровень, ОтрабатываетсяУровней);
	
	Если ОтображатьНумерациюСтрок Тогда
		ИмяОбластиНачало = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Группировка%1НачалоТаблицыНумерацияСтрок", УровеньГруппировок);
		ИмяОбластиНачалоФинал = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Группировка%1ФиналНачалоТаблицыНумерацияСтрок", УровеньГруппировок);
	Иначе
		ИмяОбластиНачало = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Группировка%1НачалоТаблицы", УровеньГруппировок);
		ИмяОбластиНачалоФинал = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Группировка%1ФиналНачалоТаблицы", УровеньГруппировок);
	КонецЕсли;
	ИмяОбластиЗначение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Группировка%1Значение", УровеньГруппировок);
	ИмяОбластиЗначениеФинал = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Группировка%1ФиналЗначение", УровеньГруппировок);
	ИмяОбластиДополнительнаяСекция = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Группировка%1ДополнительнаяСекция", УровеньГруппировок);
	ИмяОбластиДополнительнаяСекцияФинал = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Группировка%1ФиналДополнительнаяСекция", УровеньГруппировок);
	ИмяОбластиПримечание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Группировка%1Примечание", УровеньГруппировок);
	ИмяОбластиПримечаниеФинал = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Группировка%1ФиналПримечание", УровеньГруппировок);
	
	СтруктураОбластейМакета = Новый Структура;
	
	Если УровеньГруппировок = ОтрабатываетсяУровней Тогда
		СтруктураОбластейМакета.Вставить("Начало", СтруктураОбластей[ИмяОбластиНачалоФинал]);
		СтруктураОбластейМакета.Вставить("Значение", СтруктураОбластей[ИмяОбластиЗначениеФинал]);
		СтруктураОбластейМакета.Вставить("ДополнительнаяСекция", СтруктураОбластей[ИмяОбластиДополнительнаяСекцияФинал]);
		СтруктураОбластейМакета.Вставить("Примечание", ?(ВыводитьПримечания, СтруктураОбластей[ИмяОбластиПримечаниеФинал],
			Неопределено));
	Иначе
		СтруктураОбластейМакета.Вставить("Начало", СтруктураОбластей[ИмяОбластиНачало]);
		СтруктураОбластейМакета.Вставить("Значение", СтруктураОбластей[ИмяОбластиЗначение]);
		СтруктураОбластейМакета.Вставить("ДополнительнаяСекция", СтруктураОбластей[ИмяОбластиДополнительнаяСекция]);
		СтруктураОбластейМакета.Вставить("Примечание", ?(ВыводитьПримечания, СтруктураОбластей[ИмяОбластиПримечание],
			Неопределено));
	КонецЕсли;
	СтруктураОбластейМакета.Вставить("НачалоФинал", СтруктураОбластей[ИмяОбластиНачалоФинал]);
	СтруктураОбластейМакета.Вставить("ЗначениеФинал", СтруктураОбластей[ИмяОбластиЗначениеФинал]);
	СтруктураОбластейМакета.Вставить("ДополнительнаяСекцияФинал", СтруктураОбластей[ИмяОбластиДополнительнаяСекцияФинал]);
	СтруктураОбластейМакета.Вставить("ПримечаниеФинал", ?(ВыводитьПримечания, СтруктураОбластей[ИмяОбластиПримечаниеФинал],
		Неопределено));
...
Процедура ВывестиТаблицуВОтчет(ПараметрыВывода, СтрокиОтчета, СчетчикТаблиц, НакопленныйОтбор = Неопределено, Уровень = 1, СчетчикСтрок = 0, СтрокаИерархииАналитики = Неопределено) Экспорт
	...
	СтруктураОбластей = СтруктураОбластейМакетаДляВыводаТаблицы(ПараметрыВывода, Уровень);
	...
	Для Каждого СтрокаОтчета Из КоллекцияСтрокОтчета Цикл
		...
		Если Не СтрокаОтчета.Строки.Количество() Тогда
			ОбластьНачало = СтруктураОбластей.НачалоФинал; // ТабличныйДокумент
			ОбластьЗначение = СтруктураОбластей.ЗначениеФинал; // ТабличныйДокумент
			ОбластьДопСекция = СтруктураОбластей.ДополнительнаяСекцияФинал; // ТабличныйДокумент
			ОбластьПримечание = СтруктураОбластей.ПримечаниеФинал; // ТабличныйДокумент
		Иначе
			ОбластьНачало = СтруктураОбластей.Начало; // ТабличныйДокумент
			ОбластьЗначение = СтруктураОбластей.Значение; // ТабличныйДокумент
			ОбластьДопСекция = СтруктураОбластей.ДополнительнаяСекция; // ТабличныйДокумент
			ОбластьПримечание = СтруктураОбластей.Примечание; // ТабличныйДокумент
		КонецЕсли;
		...
		Иначе
			...
			Иначе
				...
				Иначе
					...
					КонецЕсли;
					Если ВыводитьЗаголовкиПоВертикали Тогда
						Область.ШиринаКолонки = Мин(16, Область.Отступ + СтрДлина(СокрЛП(Область.Текст)) + 3);
					КонецЕсли;
							...
							Пока СтрокаИерархииАналитикиРодитель <> Неопределено Цикл
								...
								Если СтрокаИерархииАналитики.Строки.Количество() = 0
									...
									Если ЗначениеЗаполнено(Строки[0][ИмяПоказателяОборот]) Тогда
										...
										ОбластьРодитель = ДокументРезультат.Область(СтрокиИерархия[0][ИмяОбласти]); 
										РасшифровкаРодителя = ПараметрыВывода.ДанныеРасшифровки.Получить(ОбластьРодитель.Расшифровка);
										РасшифровкаРодителя.Значение = СтрокиИерархия[0][ИмяПоказателя];
										ОбластьРодитель.Текст = Формат(СтрокиИерархия[0][ИмяПоказателя], ОбластьРодитель.Формат);
									КонецЕсли;
								Иначе
									Прервать;
								КонецЕсли;
...
Функция ПолучитьПредварительнуюКомпоновкуДляРасчетаОформления()
	...
	ЯчейкиПоКлючам = КомпоновкаДанныхСервер.ПустаяСхема();
...
Функция ПолучитьБазовуюКомпоновкуДляРасчетаОформления(ЭлементыОформления, ТипОформляемойОбласти, ДанныеОтчетаДляОформления)
	...
	КомпоновкаПоЯчейкам = КомпоновкаДанныхСервер.ПустаяСхема();
...
Функция РазностьДат(ПараметрыПериодов, НачалоПериода, КонецПериода, Периодичность) Экспорт
	...
	Пока ТекущийКонецПериода <= КонецПериода Цикл
		ТекущийКонецПериода = РаботаСПериодамиКлиентСервер.ДобавитьИнтервал(ТекущийКонецПериода, ПериодичностьИнтервала, 1, КэшДобавленияПериодов);
...
Функция ВычислитьФормулуВБезопасномРежиме(Знач Формула, Знач МассивЗначений, Знач ДополнительныеПараметры = Неопределено)
	...
	ПоискПоВыражению = Истина;
	
	Для Каждого КлючЗначение Из Параметры Цикл
		Если ПоискПоВыражению Тогда
			Если КлючЗначение.Ключ = "МассивЗначений" Тогда
				Выражение = СтрНайтиПоРегулярномуВыражению(Формула, "^" + КлючЗначение.Ключ + "\[\d+\]$", , , ,
					Истина).Значение;
			Иначе
				Выражение = СтрНайтиПоРегулярномуВыражению(Формула, "^" + КлючЗначение.Ключ + "$", , , ,
					Истина).Значение;
			КонецЕсли;
			Если Не ПустаяСтрока(Выражение) Тогда
				Если КлючЗначение.Ключ = "МассивЗначений" Тогда
					ВыражениеИндекса = Число(СтрНайтиПоРегулярномуВыражению(Выражение, "\d+").Значение);
					Возврат МассивЗначений[ВыражениеИндекса];
				Иначе
					Возврат Параметры[Выражение];
				КонецЕсли;
			Иначе
				ДлинаФормулы = СтрДлина(Формула);
				Формула = СтрЗаменить(Формула, Врег(КлючЗначение.Ключ), "Параметры." + КлючЗначение.Ключ);
				Если ДлинаФормулы <> СтрДлина(Формула) Тогда
					ПоискПоВыражению = Ложь;
				КонецЕсли;
			КонецЕсли;
		Иначе
			Формула = СтрЗаменить(Формула, Врег(КлючЗначение.Ключ), "Параметры." + КлючЗначение.Ключ);
	...
	Для Каждого КлючЗначение Из ФункцииФормул Цикл
		Если ПоискПоВыражению Тогда
			Выражение = СтрНайтиПоРегулярномуВыражению(Формула, "^" + КлючЗначение.Ключ + "\(Параметры\.\w+\)$", , , ,
				Истина).Значение;
			Если Не ПустаяСтрока(Выражение) Тогда
				ИмяПараметра = СтрЗаменить(СтрНайтиПоРегулярномуВыражению(Выражение, "Параметры\.\w+").Значение,
					"Параметры.", "");
				Если КлючЗначение.Ключ = "Максимум" Тогда
					Возврат Максимум(Параметры[ИмяПараметра]);
				ИначеЕсли КлючЗначение.Ключ = "Минимум" Тогда
					Возврат Минимум(Параметры[ИмяПараметра]);
				ИначеЕсли КлючЗначение.Ключ = "Сумма" Тогда
					Возврат Сумма(Параметры[ИмяПараметра]);
				ИначеЕсли КлючЗначение.Ключ = "Среднее" Тогда
					Возврат Среднее(Параметры[ИмяПараметра]);
				ИначеЕсли КлючЗначение.Ключ = "Отклонение" Тогда
					Возврат Отклонение(Параметры[ИмяПараметра]);
				ИначеЕсли КлючЗначение.Ключ = "ПроцентОтклонения" Тогда
					Возврат ПроцентОтклонения(Параметры[ИмяПараметра]);
				ИначеЕсли КлючЗначение.Ключ = "ПроцентВыполнения" Тогда
					Возврат ПроцентВыполнения(Параметры[ИмяПараметра]);
				Иначе
...
Процедура ПроверитьФормулу(Знач Формула, Операнды, Отказ) Экспорт
	...
	Попытка
		ВычислитьФормулуВБезопасномРежиме(Формула, МассивЗначений, ПараметрыПериодов);
	Исключение
...
Функция СтруктураОбластейМакета(ПараметрыВывода) Экспорт
	...
	СоответствиеИменОбластей.Вставить("ЗаголовокОтчета", "ЗаголовокОтчета|ОбластьЗаголовков");
	СоответствиеИменОбластей.Вставить("Текст", "Текст|ОбластьЗаголовков");
	СоответствиеИменОбластей.Вставить("ЗаголовокТаблицы", "ЗаголовокТаблицы|ОбластьЗаголовков");
	СоответствиеИменОбластей.Вставить("ПустаяСтрока", "ПустаяСтрока");
	
	СоответствиеИменОбластей.Вставить("ШапкаНачалоТаблицы", "Шапка|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("ШапкаДополнительнаяСекция", "Шапка|ДополнительнаяСекция");
	СоответствиеИменОбластей.Вставить("НумерацияКолонокНачалоТаблицы", "НумерацияКолонок|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("НумерацияКолонокДополнительнаяСекция", "НумерацияКолонок|ДополнительнаяСекция");
	
	Если ПараметрыВывода.ВыводитьПримечания Тогда
		СоответствиеИменОбластей.Вставить("ШапкаГруппировкаКолонок1СПримечанием", "Шапка|ГруппировкаКолонок1СПримечанием");
		СоответствиеИменОбластей.Вставить("ШапкаГруппировкаКолонок2СПримечанием", "Шапка|ГруппировкаКолонок2СПримечанием");
		СоответствиеИменОбластей.Вставить("ШапкаГруппировкаКолонок3СПримечанием", "Шапка|ГруппировкаКолонок3СПримечанием");
		СоответствиеИменОбластей.Вставить("НумерацияКолонокГруппировкаКолонок1СПримечанием",
			"НумерацияКолонок|ГруппировкаКолонок1СПримечанием");
		СоответствиеИменОбластей.Вставить("НумерацияКолонокГруппировкаКолонок2СПримечанием",
			"НумерацияКолонок|ГруппировкаКолонок2СПримечанием");
		СоответствиеИменОбластей.Вставить("НумерацияКолонокГруппировкаКолонок3СПримечанием",
			"НумерацияКолонок|ГруппировкаКолонок3СПримечанием");
	Иначе
		СоответствиеИменОбластей.Вставить("ШапкаГруппировкаКолонок1", "Шапка|ГруппировкаКолонок1");
		СоответствиеИменОбластей.Вставить("ШапкаГруппировкаКолонок2", "Шапка|ГруппировкаКолонок2");
		СоответствиеИменОбластей.Вставить("ШапкаГруппировкаКолонок3", "Шапка|ГруппировкаКолонок3");
		СоответствиеИменОбластей.Вставить("НумерацияКолонокГруппировкаКолонок1", "НумерацияКолонок|ГруппировкаКолонок1");
		СоответствиеИменОбластей.Вставить("НумерацияКолонокГруппировкаКолонок2", "НумерацияКолонок|ГруппировкаКолонок2");
		СоответствиеИменОбластей.Вставить("НумерацияКолонокГруппировкаКолонок3", "НумерацияКолонок|ГруппировкаКолонок3");
	КонецЕсли;
	
	СоответствиеИменОбластей.Вставить("ШапкаНачалоТаблицыНумерацияСтрок", "ШапкаНумерацияСтрок|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("НумерацияКолонокНачалоТаблицыНумерацияСтрок",
		"НумерацияКолонокНумерацияСтрок|НачалоТаблицы");
	
	СоответствиеИменОбластей.Вставить("Группировка1НачалоТаблицы", "Группировка1|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка2НачалоТаблицы", "Группировка2|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка3НачалоТаблицы", "Группировка3|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка4НачалоТаблицы", "Группировка4|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка5НачалоТаблицы", "Группировка5|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка6НачалоТаблицы", "Группировка6|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка7НачалоТаблицы", "Группировка7|НачалоТаблицы");
	
	СоответствиеИменОбластей.Вставить("Группировка1НачалоТаблицыНумерацияСтрок",
		"Группировка1НумерацияСтрок|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка2НачалоТаблицыНумерацияСтрок",
		"Группировка2НумерацияСтрок|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка3НачалоТаблицыНумерацияСтрок",
		"Группировка3НумерацияСтрок|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка4НачалоТаблицыНумерацияСтрок",
		"Группировка4НумерацияСтрок|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка5НачалоТаблицыНумерацияСтрок",
		"Группировка5НумерацияСтрок|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка6НачалоТаблицыНумерацияСтрок",
		"Группировка6НумерацияСтрок|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка7НачалоТаблицыНумерацияСтрок",
		"Группировка7НумерацияСтрок|НачалоТаблицы");
	
	СоответствиеИменОбластей.Вставить("Группировка1ФиналНачалоТаблицы", "Группировка1Финал|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка2ФиналНачалоТаблицы", "Группировка2Финал|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка3ФиналНачалоТаблицы", "Группировка3Финал|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка4ФиналНачалоТаблицы", "Группировка4Финал|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка5ФиналНачалоТаблицы", "Группировка5Финал|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка6ФиналНачалоТаблицы", "Группировка6Финал|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка7ФиналНачалоТаблицы", "Группировка7Финал|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка8ФиналНачалоТаблицы", "Группировка8Финал|НачалоТаблицы");
	
	СоответствиеИменОбластей.Вставить("Группировка1ФиналНачалоТаблицыНумерацияСтрок",
		"Группировка1ФиналНумерацияСтрок|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка2ФиналНачалоТаблицыНумерацияСтрок",
		"Группировка2ФиналНумерацияСтрок|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка3ФиналНачалоТаблицыНумерацияСтрок",
		"Группировка3ФиналНумерацияСтрок|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка4ФиналНачалоТаблицыНумерацияСтрок",
		"Группировка4ФиналНумерацияСтрок|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка5ФиналНачалоТаблицыНумерацияСтрок",
		"Группировка5ФиналНумерацияСтрок|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка6ФиналНачалоТаблицыНумерацияСтрок",
		"Группировка6ФиналНумерацияСтрок|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка7ФиналНачалоТаблицыНумерацияСтрок",
		"Группировка7ФиналНумерацияСтрок|НачалоТаблицы");
	СоответствиеИменОбластей.Вставить("Группировка8ФиналНачалоТаблицыНумерацияСтрок",
		"Группировка8ФиналНумерацияСтрок|НачалоТаблицы");
	
	СоответствиеИменОбластей.Вставить("Группировка1ДополнительнаяСекция", "Группировка1|ДополнительнаяСекция");
	СоответствиеИменОбластей.Вставить("Группировка2ДополнительнаяСекция", "Группировка2|ДополнительнаяСекция");
	СоответствиеИменОбластей.Вставить("Группировка3ДополнительнаяСекция", "Группировка3|ДополнительнаяСекция");
	СоответствиеИменОбластей.Вставить("Группировка4ДополнительнаяСекция", "Группировка4|ДополнительнаяСекция");
	СоответствиеИменОбластей.Вставить("Группировка5ДополнительнаяСекция", "Группировка5|ДополнительнаяСекция");
	СоответствиеИменОбластей.Вставить("Группировка6ДополнительнаяСекция", "Группировка6|ДополнительнаяСекция");
	СоответствиеИменОбластей.Вставить("Группировка7ДополнительнаяСекция", "Группировка7|ДополнительнаяСекция");
	
	СоответствиеИменОбластей.Вставить("Группировка1ФиналДополнительнаяСекция", "Группировка1Финал|ДополнительнаяСекция");
	СоответствиеИменОбластей.Вставить("Группировка2ФиналДополнительнаяСекция", "Группировка2Финал|ДополнительнаяСекция");
	СоответствиеИменОбластей.Вставить("Группировка3ФиналДополнительнаяСекция", "Группировка3Финал|ДополнительнаяСекция");
	СоответствиеИменОбластей.Вставить("Группировка4ФиналДополнительнаяСекция", "Группировка4Финал|ДополнительнаяСекция");
	СоответствиеИменОбластей.Вставить("Группировка5ФиналДополнительнаяСекция", "Группировка5Финал|ДополнительнаяСекция");
	СоответствиеИменОбластей.Вставить("Группировка6ФиналДополнительнаяСекция", "Группировка6Финал|ДополнительнаяСекция");
	СоответствиеИменОбластей.Вставить("Группировка7ФиналДополнительнаяСекция", "Группировка7Финал|ДополнительнаяСекция");
	СоответствиеИменОбластей.Вставить("Группировка8ФиналДополнительнаяСекция", "Группировка8Финал|ДополнительнаяСекция");

	Если ПараметрыВывода.ВыводитьПримечания Тогда
		
		СоответствиеИменОбластей.Вставить("Группировка1Значение", "Группировка1|ГруппировкаКолонок1СПримечанием");
		СоответствиеИменОбластей.Вставить("Группировка2Значение", "Группировка2|ГруппировкаКолонок1СПримечанием");
		СоответствиеИменОбластей.Вставить("Группировка3Значение", "Группировка3|ГруппировкаКолонок1СПримечанием");
		СоответствиеИменОбластей.Вставить("Группировка4Значение", "Группировка4|ГруппировкаКолонок1СПримечанием");
		СоответствиеИменОбластей.Вставить("Группировка5Значение", "Группировка5|ГруппировкаКолонок1СПримечанием");
		СоответствиеИменОбластей.Вставить("Группировка6Значение", "Группировка6|ГруппировкаКолонок1СПримечанием");
		СоответствиеИменОбластей.Вставить("Группировка7Значение", "Группировка7|ГруппировкаКолонок1СПримечанием");
		СоответствиеИменОбластей.Вставить("Группировка1Примечание", "Группировка1|ПримечаниеКолонка");
		СоответствиеИменОбластей.Вставить("Группировка2Примечание", "Группировка2|ПримечаниеКолонка");
		СоответствиеИменОбластей.Вставить("Группировка3Примечание", "Группировка3|ПримечаниеКолонка");
		СоответствиеИменОбластей.Вставить("Группировка4Примечание", "Группировка4|ПримечаниеКолонка");
		СоответствиеИменОбластей.Вставить("Группировка5Примечание", "Группировка5|ПримечаниеКолонка");
		СоответствиеИменОбластей.Вставить("Группировка6Примечание", "Группировка6|ПримечаниеКолонка");
		СоответствиеИменОбластей.Вставить("Группировка7Примечание", "Группировка7|ПримечаниеКолонка");
		
		СоответствиеИменОбластей.Вставить("Группировка1ФиналЗначение", "Группировка1Финал|ГруппировкаКолонок1СПримечанием");
		СоответствиеИменОбластей.Вставить("Группировка2ФиналЗначение", "Группировка2Финал|ГруппировкаКолонок1СПримечанием");
		СоответствиеИменОбластей.Вставить("Группировка3ФиналЗначение", "Группировка3Финал|ГруппировкаКолонок1СПримечанием");
		СоответствиеИменОбластей.Вставить("Группировка4ФиналЗначение", "Группировка4Финал|ГруппировкаКолонок1СПримечанием");
		СоответствиеИменОбластей.Вставить("Группировка5ФиналЗначение", "Группировка5Финал|ГруппировкаКолонок1СПримечанием");
		СоответствиеИменОбластей.Вставить("Группировка6ФиналЗначение", "Группировка6Финал|ГруппировкаКолонок1СПримечанием");
		СоответствиеИменОбластей.Вставить("Группировка7ФиналЗначение", "Группировка7Финал|ГруппировкаКолонок1СПримечанием");
		СоответствиеИменОбластей.Вставить("Группировка8ФиналЗначение", "Группировка8Финал|ГруппировкаКолонок1СПримечанием");
		СоответствиеИменОбластей.Вставить("Группировка1ФиналПримечание", "Группировка1Финал|ПримечаниеКолонка");
		СоответствиеИменОбластей.Вставить("Группировка2ФиналПримечание", "Группировка2Финал|ПримечаниеКолонка");
		СоответствиеИменОбластей.Вставить("Группировка3ФиналПримечание", "Группировка3Финал|ПримечаниеКолонка");
		СоответствиеИменОбластей.Вставить("Группировка4ФиналПримечание", "Группировка4Финал|ПримечаниеКолонка");
		СоответствиеИменОбластей.Вставить("Группировка5ФиналПримечание", "Группировка5Финал|ПримечаниеКолонка");
		СоответствиеИменОбластей.Вставить("Группировка6ФиналПримечание", "Группировка6Финал|ПримечаниеКолонка");
		СоответствиеИменОбластей.Вставить("Группировка7ФиналПримечание", "Группировка7Финал|ПримечаниеКолонка");
		СоответствиеИменОбластей.Вставить("Группировка8ФиналПримечание", "Группировка8Финал|ПримечаниеКолонка");
		
	Иначе
		
		СоответствиеИменОбластей.Вставить("Группировка1Значение", "Группировка1|ГруппировкаКолонок1");
		СоответствиеИменОбластей.Вставить("Группировка2Значение", "Группировка2|ГруппировкаКолонок1");
		СоответствиеИменОбластей.Вставить("Группировка3Значение", "Группировка3|ГруппировкаКолонок1");
		СоответствиеИменОбластей.Вставить("Группировка4Значение", "Группировка4|ГруппировкаКолонок1");
		СоответствиеИменОбластей.Вставить("Группировка5Значение", "Группировка5|ГруппировкаКолонок1");
		СоответствиеИменОбластей.Вставить("Группировка6Значение", "Группировка6|ГруппировкаКолонок1");
		СоответствиеИменОбластей.Вставить("Группировка7Значение", "Группировка7|ГруппировкаКолонок1");
		
		СоответствиеИменОбластей.Вставить("Группировка1ФиналЗначение", "Группировка1Финал|ГруппировкаКолонок1");
		СоответствиеИменОбластей.Вставить("Группировка2ФиналЗначение", "Группировка2Финал|ГруппировкаКолонок1");
		СоответствиеИменОбластей.Вставить("Группировка3ФиналЗначение", "Группировка3Финал|ГруппировкаКолонок1");
		СоответствиеИменОбластей.Вставить("Группировка4ФиналЗначение", "Группировка4Финал|ГруппировкаКолонок1");
		СоответствиеИменОбластей.Вставить("Группировка5ФиналЗначение", "Группировка5Финал|ГруппировкаКолонок1");
		СоответствиеИменОбластей.Вставить("Группировка6ФиналЗначение", "Группировка6Финал|ГруппировкаКолонок1");
		СоответствиеИменОбластей.Вставить("Группировка7ФиналЗначение", "Группировка7Финал|ГруппировкаКолонок1");
		СоответствиеИменОбластей.Вставить("Группировка8ФиналЗначение", "Группировка8Финал|ГруппировкаКолонок1");
		
	КонецЕсли;
	
	Если ПараметрыВывода.ВыводитьПримечания И ПараметрыВывода.ВыводитьНомераТаблиц Тогда
		СоответствиеИменОбластей.Вставить("НомерТаблицы", "НомерТаблицы");
	КонецЕсли;
	
	МакетОформления 	= ПолучитьОбщийМакет("БюджетныйОтчет");
	СтруктураОбластей 	= Новый Структура;
	
	Для Каждого ИмяОбласти Из СоответствиеИменОбластей Цикл
		СтруктураОбластей.Вставить(ИмяОбласти.Ключ, МакетОформления.ПолучитьОбласть(ИмяОбласти.Значение));
	КонецЦикла;
...
// 		** Расшифровка - Число -
// 		** ЭтоОбъединеннаяАналитика - Булево -
// 	* ДанныеРасшифровки - Соответствие из КлючИЗначение - содержит:
