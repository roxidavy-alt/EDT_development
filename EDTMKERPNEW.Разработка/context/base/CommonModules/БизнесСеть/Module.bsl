...
Процедура АктуализироватьКэшДанныхКонтрагентов() Экспорт
	...
	ЗаписьЖурналаРегистрации(
		...
		НСтр(
			...
			|en = 'Completing Business Network counterparty IDs update'", 
			ОбщегоНазначения.КодОсновногоЯзыка()));
КонецПроцедуры
		...
		Если ТекущаяУниверсальнаяДатаВМиллисекундах() > ВремяЗавершения Тогда
			...
			ВызватьИсключение
				НСтр(
					...
					|en = 'An error occurred when receiving a response from the service: the service response timeout is exceeded'",
					ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
		...
		Если ТекущаяУниверсальнаяДатаВМиллисекундах() > ВремяЗавершения Тогда
			...
			ВызватьИсключение
				НСтр(
					...
					|en = 'An error occurred when postprocessing the request result: postprocessing time is exceeded'",
					ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
...
Функция ТекстОшибкиСервисаИзДанныхОтвета(КодСостояния, ОписаниеОшибки, ПараметрыКоманды, ПараметрыСоединения, Данные)
	...
	Если ТипЗнч(Данные) = Тип("Структура") Тогда
		...
		ТекстОшибкиСервиса = НСтр("ru = 'Описание ошибки';
									|en = 'Error details'") + ": " + ТекстОшибкиСервиса;
