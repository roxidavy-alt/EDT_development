...
Процедура ДополнитьОписаниеЭтаповЗакрытияМесяца(ТаблицаЭтапов) Экспорт
	...
	НастройкаСчетовУчетаСервер.ДобавитьЭтап_ПереносСальдоПриИзмененииНастроекСчетовРеглУчета(ТаблицаЭтапов, ТекущийРодитель);
	...
	РеглУчетСервер.ДобавитьЭтап_ПерерасчетНалогаНаПрибыльПрошлыхПериодов(ТаблицаЭтапов, ТекущийРодитель);
...
Процедура ДополнитьОписаниеГруппЭтаповЗакрытияМесяца(ТаблицаГруппЭтапов) Экспорт
	...
	НоваяСтрока.Наименование = НСтр("ru = 'Учет по УСН';
									|en = 'STS accounting'");
	НоваяСтрока.МеждународноеНаименование = "ru = 'Учет по УСН';
											|en = 'STS accounting'";
	//-- НЕ УТ
...
Процедура ДополнитьДанныеВыбораОперацийЗакрытияМесяца(ДанныеВыбора) Экспорт
	...
	//++ НЕ УТ
	ДанныеВыбора.Добавить(Справочники.ОперацииЗакрытияМесяца.НачислениеАмортизацииОС);
	ДанныеВыбора.Добавить(Справочники.ОперацииЗакрытияМесяца.НачислениеАмортизацииНМА);
	ДанныеВыбора.Добавить(Справочники.ОперацииЗакрытияМесяца.ПризнаниеВНалоговомУчетеАрендныхПлатежей);
	ДанныеВыбора.Добавить(Справочники.ОперацииЗакрытияМесяца.ПогашениеСтоимостиТМЦВЭксплуатации);
	ДанныеВыбора.Добавить(Справочники.ОперацииЗакрытияМесяца.РасчетНалогаНаИмущество);
	ДанныеВыбора.Добавить(Справочники.ОперацииЗакрытияМесяца.РасчетТранспортногоНалога);
	ДанныеВыбора.Добавить(Справочники.ОперацииЗакрытияМесяца.РасчетЗемельногоНалога);
	ДанныеВыбора.Добавить(Справочники.ОперацииЗакрытияМесяца.ПереносСальдоПриИзмененииНастроекСчетовРеглУчета);
	ДанныеВыбора.Добавить(Справочники.ОперацииЗакрытияМесяца.СторноДоходовКУДиР);
	ДанныеВыбора.Добавить(Справочники.ОперацииЗакрытияМесяца.ПризнаниеРасходовПриУСН);
	ДанныеВыбора.Добавить(Справочники.ОперацииЗакрытияМесяца.РасчетНалогаУСН);
	ДанныеВыбора.Добавить(Справочники.ОперацииЗакрытияМесяца.РеклассификацияДолгосрочныхАктивовОбязательств);
	ДанныеВыбора.Добавить(Справочники.ОперацииЗакрытияМесяца.ФормированиеФинансовогоРезультата);
	ДанныеВыбора.Добавить(Справочники.ОперацииЗакрытияМесяца.ПерерасчетНалогаНаПрибыльПрошлыхПериодов);
	ДанныеВыбора.Добавить(Справочники.ОперацииЗакрытияМесяца.РасчетЕНС);
...
Функция ПроверитьНаличиеДокументаРегламентнаяОперация(ПараметрыОбработчика, Знач ТипОперации = Неопределено,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.КоличествоДокументов > 1 И Выборка.ТипОперации <> Перечисления.ТипыРегламентныхОпераций.ПерерасчетНалогаНаПрибыльПрошлыхПериодов Тогда
...
Процедура ПроверитьНеобходимостьФормированияЗаданийНаСледующийПериод(ФормироватьЗадания, ПараметрыОбработчика) Экспорт
	...
	//++ НЕ УТ
	Если ПараметрыОбработчика.ДанныеЭтапа.Код = Справочники.ОперацииЗакрытияМесяца.ПерерасчетНалогаНаПрибыльПрошлыхПериодов Тогда
		ФормироватьЗадания = Ложь;
	КонецЕсли;
	...
	Пока НачалоРасчета <= ПараметрыРасчета.КонецПериода Цикл
		...
		ИначеЕсли ДанныеКРасчетуЗаМесяц.Организация.Количество() <> 0 Тогда
			...
			Если Не ЕстьОшибки Тогда
				РеглУчетСервер.СформироватьЗаданияДляПоследующихНаборовКорректировок(ДоступныеОрганизации, КонецМесяца(НачалоРасчета));
			КонецЕсли;
...
Процедура ОбработчикиОбновленияБлокирующиеЗакрытиеМесяца(Обработчики) Экспорт
	...
	//++ НЕ УТ
	|РегистрыНакопления.РасчетыПоФинансовымИнструментамНУ.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|РегистрыСведений.РасчетНалогаНаПрибыль.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|Справочники.КорректировкиПрошлыхПериодовНУ.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|РегистрыСведений.ПорядокОтраженияНаСчетахУчета.ОбработатьДанныеДляПереходаНаНовуюВерсию
	//-- НЕ УТ
...
Процедура ОбработчикиОбновленияНеВлияющиеНаЗакрытиеМесяца(Обработчики) Экспорт
	...
	//++ НЕ УТ
	|РегистрыСведений.СведенияПоКонтрактамГОЗ.ОбработатьДанныеДляПереходаНаНовуюВерсию
	//-- НЕ УТ
...
Функция ДатаЗаданияНаТекущийПериод(Операция, Организация, ДокументЗадания, Знач Период) Экспорт
	...
	//++ НЕ УТ
	Если Операция = Справочники.ОперацииЗакрытияМесяца.РасчетНалогаНаИмущество 
			И НЕ РасчетИмущественныхНалоговУП.УплачиваютсяАвансыПоНалогуНаИмущество(Организация, Период)
		
		ИЛИ Операция = Справочники.ОперацииЗакрытияМесяца.РасчетЗемельногоНалога 
			И НЕ РасчетИмущественныхНалоговУП.УплачиваютсяАвансыПоЗемельномуНалогу(Организация, Период)
			
		ИЛИ Операция = Справочники.ОперацииЗакрытияМесяца.РасчетТранспортногоНалога 
...
Функция ДатаЗаданияНаСледующийПериод(Операция, Организация, ДокументЗадания, Знач СледующийМесяц) Экспорт
	...
	Если Операция = Справочники.ОперацииЗакрытияМесяца.РасчетНалогаНаИмущество Тогда
		
		ДатаЗадания = РасчетИмущественныхНалоговУП.ПериодРасчетаНалогаНаИмущество(Организация, СледующийМесяц);
		
	ИначеЕсли Операция = Справочники.ОперацииЗакрытияМесяца.РасчетЗемельногоНалога Тогда
		
		ДатаЗадания = РасчетИмущественныхНалоговУП.ПериодРасчетаЗемельногоНалога(Организация, СледующийМесяц);
		
	ИначеЕсли Операция = Справочники.ОперацииЗакрытияМесяца.РасчетТранспортногоНалога Тогда
		
		ДатаЗадания = РасчетИмущественныхНалоговУП.ПериодРасчетаТранспортногоНалога(Организация, СледующийМесяц);
		
	ИначеЕсли Операция = Справочники.ОперацииЗакрытияМесяца.ПризнаниеВНалоговомУчетеАрендныхПлатежей Тогда
		
		ДатаЗадания = УчетАрендованныхОСЛокализация.СледующийПериодПризнанияВНУАрендныхПлатежей(Организация, СледующийМесяц);
		
	ИначеЕсли Операция = Справочники.ОперацииЗакрытияМесяца.ФормированиеФинансовогоРезультата Тогда
		
		ДатаЗадания = НачалоМесяца(КонецГода(СледующийМесяц));
		
	ИначеЕсли Операция = Справочники.ОперацииЗакрытияМесяца.РасчетНалогаУСН Тогда
		
		ДатаЗадания = НачалоМесяца(КонецКвартала(СледующийМесяц));
		
	ИначеЕсли Операция = Справочники.ОперацииЗакрытияМесяца.ПереносСальдоПриИзмененииНастроекСчетовРеглУчета Тогда
		
		ДатаЗадания = НастройкаСчетовУчетаСервер.Период_ПереносСальдоПриИзмененииНастроекСчетовРеглУчета(Организация, СледующийМесяц);
...
Функция ИсточникиДанныхДляЗаданийЗакрытияМесяца() Экспорт
	...
	ВходящиеДанные.Вставить(Метаданные.РегистрыНакопления.НДСПредъявленный, Истина);
