...
Функция СформироватьДанныеЗаполненияПередачиСырья(МассивСсылок, ИмяДокумента, РеквизитыШапки, СвойстваЗаказов = Неопределено) Экспорт
	...
	Запрос.Текст = ВТ_РаспоряженияНаОтгрузкуОборотыРазвернутая
		...
		|	ВЫБОР
		//++ НЕ УТКА
		|		КОГДА Заказ.ГруппировкаЗатрат = ЗНАЧЕНИЕ(Перечисление.ГруппировкиЗатратВЗаказеПереработчику.ПоЭтапамПроизводства)
		...
		|	Заказ.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.СиламиПеревозчика) КАК СпособДоставкиСиламиПеревозчика,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_РаспоряженияНаОтгрузкуОборотыРазвернутая КАК ОстаткиЗаказов
		...
		|	ВЫБОР
		|		КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВЫБОР
		|			КОГДА Заказы.БанковскийСчетКонтрагента <> ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаКонтрагентов.ПустаяСсылка)
		...
		|	ВЫБОР
		|		КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВЫБОР
		|			КОГДА Заказы.БанковскийСчетГрузоотправителя <> ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаКонтрагентов.ПустаяСсылка)
		...
		|	ВЫБОР
		|		КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВЫБОР
		|			КОГДА Заказы.БанковскийСчетГрузополучателя <> ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаКонтрагентов.ПустаяСсылка)
		...
		|	ВЫБОР
		|		КОГДА Заказ.Статус В (&МассивСтатусовЗаказа)
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ИСТИНА
	...
	Иначе
		...
		Если Не РезультатЗапроса[5].Пустой() Тогда
			...
			ВыборкаЗаказы = РезультатЗапроса[5].Выбрать();
