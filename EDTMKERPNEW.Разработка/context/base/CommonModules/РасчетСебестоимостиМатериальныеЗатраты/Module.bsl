...
Функция ТекстЗапросаДляРаспределенияНоменклатурыНаПроизводство(ПараметрыРасчета = Неопределено) Экспорт
	...
	ТекстЗапроса = ""
		...
		+ РасчетСебестоимостиЗаполнениеПартий.ТекстИсправленныеДокументы() + ОбщегоНазначения.РазделительПакетаЗапросов() // ИсправленныеДокументы
...
Функция ТекстРаспределенияНоменклатурыНаПроизводствоИнициализация()
	...
	|	ПО ИсправленныеДокументы.СторнируемыйДокумент = ДД.Ссылка
	|ГДЕ
	|	ДД.Проведен
...
Функция ТекстДанныеПоМатериальнымЗатратам(ДляРасшифровки)
	...
	Если ДляРасшифровки Тогда
		...
		|		ПартииПроизводстваСводно.Назначение			КАК Назначение,
		|		ДД.АналитикаФинансовогоУчета				КАК ГруппаПродукции,
		|		ДД.Партия									КАК ПартияПроизводства,
		|		ПартииПроизводстваСводно.ВидДеятельностиНДС	КАК ВидДеятельностиНДС,
		|		ПартииПроизводстваСводно.НаправлениеДеятельности КАК НаправлениеДеятельности,
		|		СУММА(ДД.Количество)						КАК Количество,
		...
		|			ПО ПартииПроизводстваСводно.ПартияПроизводства = ДД.Партия
		|			И ПартииПроизводстваСводно.Организация = ДД.Организация
		|
		...
		|		И ТИПЗНАЧЕНИЯ(ПартииПроизводстваСводно.ДокументПартии) = ТИП(Документ.ПроизводствоБезЗаказа)
		|		И ПартииПроизводстваСводно.РаспределениеЗатратНаВыпуск
		...
		|		ПартииПроизводстваСводно.Назначение,
		|		ДД.АналитикаФинансовогоУчета,
		|		ДД.Партия,
		|		ПартииПроизводстваСводно.ВидДеятельностиНДС,
		|		ПартииПроизводстваСводно.НаправлениеДеятельности
		//++ НЕ УТКА
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
	...
	Иначе
		...
		|		
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПартииПроизводстваСводно КАК ПартииПроизводстваСводно
		|		ПО ПартииПроизводстваСводно.ПартияПроизводства = ДД.КорПартия
		|		И ПартииПроизводстваСводно.Организация = ДД.Организация
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СН
		|		ПО СН.Ссылка = Аналитика.Номенклатура
		...
		|	И ТИПЗНАЧЕНИЯ(ПартииПроизводстваСводно.ДокументПартии) В(
		//++ НЕ УТКА
...
Функция ТекстОписаниеДанныхДляРаспределенияНоменклатурыНаВыпуск()
	...
	|	ДД.КорВидДеятельностиНДС                                             КАК КорВидДеятельностиНДС,
	|	ДД.КорАналитикаФинансовогоУчета                                      КАК КорАналитикаФинансовогоУчета,
	|	ДД.ВидДеятельностиНДСДокумента                                       КАК ВидДеятельностиНДСДокумента,
	|	ДД.ИдентификаторСтроки                                               КАК ИдентификаторСтроки,
...
Функция ТекстРасчетноеРаспределениеНоменклатурыНаВыпуск()
	...
	|	ВЫБОР
	|		КОГДА СУММА(ПланФактМатериалов.Факт) < 0
	|			ТОГДА -1
	|		ИНАЧЕ 1
	|	КОНЕЦ                                    КАК КоэффициентПриведения,
	|	ПланФактМатериалов.ПобочноеИзделие       КАК ПобочноеИзделие,
	|	СУММА(ПланФактМатериалов.План)           КАК План,
	|	ВЫБОР
	|		КОГДА СУММА(ПланФактМатериалов.Факт) < 0
	|			ТОГДА -1 * СУММА(ПланФактМатериалов.Факт)
	|		ИНАЧЕ СУММА(ПланФактМатериалов.Факт)
	|	КОНЕЦ                                                  КАК Факт
	...
	|		0                                                      КАК План,
	|		Материалы.Количество                                   КАК Факт,
	|		ВЫБОР
	|			КОГДА Материалы.АналитикаУчетаНоменклатуры.СтатьяКалькуляции.ТипЗатрат = ЗНАЧЕНИЕ(Перечисление.ТипыЗатрат.ВозвратныеОтходы)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	...
	|		И Материалы.СлужебноеВидДвиженияПриход
	// Исключаем сторно, так как будут получены исходные движения прошлых периодов
	|		И НЕ Материалы.Сторно
	//++ Устарело_Переработка24
	...
	|	ГДЕ
	|		ДД.РаспределениеЗатратНаВыпуск
	|		И Обеспечение.Ссылка.ПроизводствоНаСтороне2_5
	|		И НЕ Обеспечение.Отменено
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	ГДЕ
	|		ДД.РаспределениеЗатратНаВыпуск
	|		И Материалы.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	...
	|	ПланФактМатериалов.ПобочноеИзделие
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|				ГДЕ
	|					Т.ДлительноеПроизводство
	|					И Т.РаспределениеЗатратНаВыпуск)
	|		) КАК Расходы
	|СГРУППИРОВАТЬ ПО
	|	Расходы.Партия,
	...
	|			ТОГДА ДД.План
	|		ИНАЧЕ ДД.Факт
	|	КОНЕЦ * ДолиВыпуска.ДоляВыпускаВсего
	|		- ДД.КоэффициентПриведения * ЕСТЬNULL(Списано.Количество, 0) КАК РасчетноеКоличество,
	...
	|		ИНАЧЕ ДД.КоэффициентПриведения * ДД.РасчетноеКоличество
...
Функция ТекстНоменклатураКРаспределениюНаВыпуск() // использует ОстаткиМатериалов, ДолиВыпуска
	...
	|		* (ДД.Количество
	|			- ЕСТЬNULL(ЗамкнутыеПартии.Количество, 0)) КАК ЧИСЛО(23,10)) КАК Знаменатель,
...
Функция ТекстЗацикленнаяНоменклатураНаВыпуск()
	...
	|	ЕСТЬNULL(Движения.ВидДеятельностиНДС, Продукция.ВидДеятельностиНДС)  КАК ВидДеятельностиНДСДокумента,
...
Функция ТекстРасчетноеКоличествоКРаспределению() // использует Расчетное
	...
	|	НЕОПРЕДЕЛЕНО                                                         КАК ВидДеятельностиНДС,
	|	ВЫРАЗИТЬ(ВЫБОР
	|		КОГДА ДД.РасчетноеКоличество - ЕСТЬNULL(ЗамкнутыеПартии.Количество, 0) < 0
	|			ТОГДА (-1)*(ДД.РасчетноеКоличество - ЕСТЬNULL(ЗамкнутыеПартии.Количество, 0))
	|		ИНАЧЕ ДД.РасчетноеКоличество - ЕСТЬNULL(ЗамкнутыеПартии.Количество, 0)
...
Функция ТекстРаспределениеНоменклатурыНаВыпускПоПродукции() // использует Продукция
	...
	|	КОНЕЦ КАК КорАналитикаФинансовогоУчета,
	|	ЕСТЬNULL(Движения.ВидДеятельностиНДС, ДД.ВидДеятельностиНДС)         КАК ВидДеятельностиНДСДокумента,
	|	ДД.ИдентификаторСтроки                                               КАК ИдентификаторСтроки,
	...
	|	ЕСТЬNULL(Движения.ВидДеятельностиНДС, ДД.ВидДеятельностиНДС), // ВидДеятельностиНДСДокумента
