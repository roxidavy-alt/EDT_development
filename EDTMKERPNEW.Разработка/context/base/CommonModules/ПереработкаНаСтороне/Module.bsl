...
Функция ТекстПроверкиНесколькоИмпортныхИзделий() Экспорт
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	&ИмяСтраницыПродукция               КАК ИмяТаблицы,
	|	""""                                КАК СинонимТЧ,
	|	0                                   КАК НомерСтроки,
	|	НомераГруппЗатрат.НомерГруппыЗатрат КАК НомерГруппыЗатрат,
...
Процедура СообщитьОшибкиЗаполненияГруппЗатрат(Выборка, ПредставлениеГруппЗатрат, Отказ) Экспорт
	...
	ШаблонНеУказанИмпортныйМатериал = НСтр("ru = 'Для группы затрат ""%1"" не указан импортный материал или работа.';
											|en = 'No imported material or work is specified for the ""%1"" cost group.'");
...
Процедура ДобавитьЭтап_ОстаткиПереработкиНаСтороне2_4(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока =  ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(
		ТаблицаЭтапов,
		ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ОформлениеОстатковПередачиВПереработку2_4);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Оформление остатков передачи в переработку (версии 2.4)';
											|en = 'Register subcontracting balance (version 2.4)'"; // @НСтр-1
...
Процедура Использование_ОстаткиПереработкиНаСтороне2_4(ПараметрыОбработчика) Экспорт
	...
	|ИЗ
	|	РегистрНакопления.ЗаказыПоставщикам.Остатки(, ТИПЗНАЧЕНИЯ(ЗаказПоставщику) = ТИП(Документ.ЗаказПереработчику)) КАК
	|		ОстаткиТоваров
	|		ЛЕВОЕ СОЕДИНЕНИЕ Константы КАК Константы
	|		ПО ИСТИНА
	|ГДЕ
	...
	ПараметрыФормированияТаблицы.ТекстУсловия =
		...
		|		ГДЕ
		|			ТИПЗНАЧЕНИЯ(ОткрытыеЗаказы.Распоряжение) = ТИП(Документ.ЗаказПереработчику))";
...
Процедура ДобавитьЭтап_ОстаткиКОформлениюОтчетовПереработчиков(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока =  ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(
		ТаблицаЭтапов,
		ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ОформлениеОтчетовПереработчиков);
	
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Оформление отчетов переработчиков';
											|en = 'Register subcontractor reports'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
	НоваяСтрока.ДействиеОформление =  ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
...
Функция ДобавитьПредупреждениеПодсистема24Устарела(Форма, ГруппаРодитель = Неопределено) Экспорт
	...
	ТекстНадписи =
		НСтр("ru = 'Внимание! Функциональность ""Производство (переработка) на стороне (версия 2.4)"" устарела, и будет удалена в версии 2.6.3. Пожалуйста, перейдите на ""Производство (переработка) на стороне (версия 2.5)"".';
			|en = 'Important. The ""Production (subcontracting) by third party (version 2.4)"" functionality is outdated and will be removed in version 2.6.3. Please switch to ""Production (subcontracting) by third party (version 2.5)"".'");
...
Процедура ВыполнитьКонтрольЗаказаПослеПроведения(Объект, Отказ) Экспорт
	...
	ТекстЗапроса =
		...
		|	И ((РаспоряженияНаОтгрузкуОбороты.ЗаказаноКоличествоОборот -
		|		РаспоряженияНаОтгрузкуОбороты.ОформленоКоличествоОборот) > 0
		|		ИЛИ (РаспоряженияНаОтгрузкуОбороты.КОформлениюКоличествоОборот -
		|			РаспоряженияНаОтгрузкуОбороты.ОформленоКоличествоОборот) > 0)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|ИЗ
		|	РегистрНакопления.УслугиПереработчиковКОформлению.ОстаткиИОбороты(,,,, ЗаказПереработчику = &Ссылка) КАК
		|		УслугиПереработчика
		|ГДЕ
	...
	ПараметрыФормированияТаблицы.ТекстУсловия =
		"Распоряжение = &Ссылка";
	ПараметрыФормированияТаблицы.Группировка = "Распоряжение";
	ПараметрыФормированияТаблицы.Показатели = "Заказано, КОформлению, Оформлено";
	ПараметрыФормированияТаблицы.Ресурсы = "Количество";
