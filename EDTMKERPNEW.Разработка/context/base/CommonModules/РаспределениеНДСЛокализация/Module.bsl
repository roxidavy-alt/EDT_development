...
#Область ПередачаНДСВСтоимостьОС                 // (Дт <08.04.2> :: Кт <19.1, 19.3>)
	...
	|	ПартииНДСКРаспределению.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|	
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Списание НДС по приобретенным ценностям"" КАК Содержание,
	|	""РаспределениеНДСЛокализация_1045"" КАК НомерФрагмента
	|ИЗ
...
#Область ИсключениеНДСИзСтоимостиОС              // (Дт <19.1, 19.3> :: Кт <08.04.2>)
	...
	|	""РаспределениеНДСЛокализация_1046"" КАК НомерФрагмента
	|ИЗ
	|	ПартииНДСКРаспределению КАК ПартииНДСКРаспределению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область СтоимостьОСНаСчетУчета                  // (Дт <СчетУчетаОС/НМА> :: Кт <08.04.2>)
	...
	|	""РаспределениеНДСЛокализация_1047"" КАК НомерФрагмента
	|ИЗ
	|	ПартииНДСКРаспределению КАК ПартииНДСКРаспределению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область СтоимостьОСВРасходыНУ                  // (Дт <СчетРасходов> :: Кт <01>)
	...
	|	""РаспределениеНДСЛокализация_1048"" КАК НомерФрагмента
	|ИЗ
	|	ПартииНДСКРаспределению КАК ПартииНДСКРаспределению
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
...
#Область ИсключениеНДССоСчетаУчетаОС             // (Дт <08.04.2> :: Кт <СчетУчетаОС/НМА>)
	...
	|	""РаспределениеНДСЛокализация_1049"" КАК НомерФрагмента
	|ИЗ
	|	ПартииНДСКРаспределению КАК ПартииНДСКРаспределению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область КорректировкаАмортизационнойПремии      // (Дт <КВ> :: Кт <ПустаяСсылка>)
	...
	|	""РаспределениеНДСЛокализация_1050"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ПартииНДСКРаспределению КАК ПартииНДСКРаспределению
	|
...
#Область СтоимостьПрочегоАктиваНаОСиНМА              // (Дт <СчетУчетаОС/НМА> :: Кт <08.04.2>)
	...
	|	""РаспределениеНДСЛокализация_1051"" КАК НомерФрагмента
	|ИЗ
	|	ПартииНДСКРаспределению КАК ПартииНДСКРаспределению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ПередачаНДСВСтоимостьПрочегоАктива               // (Дт <08.04.2>, ХХ :: Кт <19.1, 19.3>)
	...
	|	""РаспределениеНДСЛокализация_1052"" КАК НомерФрагмента
	|ИЗ
	|	ПартииНДСКРаспределению КАК ПартииНДСКРаспределению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ИсключениеНДСИзСтоимостиПостатейныхРасходов  // (Дт <26> :: Кт <19.1, 19.3>)
	...
	|	""РаспределениеНДСЛокализация_1053"" КАК НомерФрагмента
	|ИЗ
	|	ПартииНДСКРаспределению КАК ПартииНДСКРаспределению
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
...
#Область ВключениеНДСВСтоимостьПостатейныхРасходов  // (Дт <26> :: Кт <19.1, 19.3>)
	...
	|	""РаспределениеНДСЛокализация_1054"" КАК НомерФрагмента
	|ИЗ
	|	ПартииНДСКРаспределению КАК ПартииНДСКРаспределению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Процедура РассчитатьВосстановлениеНДСИСписаниеНаРасходы(ТаблицыРаспределения, ДанныеДляРаспределения, Реквизиты) Экспорт
	...
	ПартииПродажаНеОблагаетсяНДС =
		ТаблицыРаспределения[Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС];
	ОстаткиНДСПредъявленный = ДанныеДляРаспределения.ОстаткиНДСПредъявленный; // ТаблицаЗначений
	ОстаткиНДСПредъявленный.Индексы.Добавить("СчетФактура");
	Для Каждого Строка Из ПартииПродажаНеОблагаетсяНДС Цикл
		Если Строка.ВидДеятельностиНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС 
			И (Строка.ВидЦенности = Перечисления.ВидыЦенностей.ОС 
				ИЛИ Строка.ВидЦенности = Перечисления.ВидыЦенностей.НМА) Тогда
			СтрокиСФ = ОстаткиНДСПредъявленный.НайтиСтроки(Новый Структура("СчетФактура", Строка.ДокументПоступленияРасходов));
			Если СтрокиСФ.Количество() > 0 Тогда
				Если СтрокиСФ[0].СуммаНДС = 0 Тогда
					Строка.СтатьяОтраженияРасходов = НастройкиУчетаНДС.СтатьяРасходовНеНДС;
					Строка.АналитикаОтраженияРасходов = НастройкиУчетаНДС.АналитикаРасходовНеНДС;
				ИначеЕсли Строка.НДСРегл - СтрокиСФ[0].СуммаНДС > 0 Тогда
					СуммаВосстановления = Строка.НДСРегл - СтрокиСФ[0].СуммаНДС;
					СуммаВосстановленияУпр = Окр(Строка.НДСУпр * СуммаВосстановления / Строка.НДСРегл, 2);
					СуммаВосстановленияСтоимость = Окр(Строка.СтоимостьРегл * СуммаВосстановления / Строка.НДСРегл, 2);
					
					СтрокаВосстановления = ПартииПродажаНеОблагаетсяНДС.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаВосстановления, Строка);
					СтрокаВосстановления.НДСРегл = СуммаВосстановления;
					СтрокаВосстановления.НДСУпр = СуммаВосстановленияУпр;
					СтрокаВосстановления.СтоимостьРегл = СуммаВосстановленияСтоимость;
					СтрокаВосстановления.СтатьяОтраженияРасходов = НастройкиУчетаНДС.СтатьяРасходовНеНДС;
					СтрокаВосстановления.АналитикаОтраженияРасходов = НастройкиУчетаНДС.АналитикаРасходовНеНДС;
					
					Строка.НДСРегл = Строка.НДСРегл - СуммаВосстановления;
					Строка.НДСУпр = Строка.НДСРегл - СуммаВосстановленияУпр;
					Строка.СтоимостьРегл = Строка.НДСРегл - СуммаВосстановленияСтоимость;
					
					СтрокиСФ[0].СуммаНДС = СтрокиСФ[0].СуммаНДС - СуммаВосстановления;
				КонецЕсли;
			Иначе
				Строка.СтатьяОтраженияРасходов = НастройкиУчетаНДС.СтатьяРасходовНеНДС;
				Строка.АналитикаОтраженияРасходов = НастройкиУчетаНДС.АналитикаРасходовНеНДС;
...
Процедура ИнициализироватьВтВнеоборотныеАктивы(МенеджерВременныхТаблиц, Реквизиты) Экспорт
	...
	Если ВнеоборотныеАктивы.ИспользуетсяУправлениеВНА_2_4(Реквизиты.Период) Тогда
		ПериодНачалоМесяца = НачалоМесяца(Реквизиты.ПериодНачало);
		НачалоПериода = НачалоМесяца(Реквизиты.ПериодОкончание);
		КонецПериода = Реквизиты.ПериодОкончание;
		НеОтбиратьПоОстаткамПартий = Реквизиты.РаспределятьНДСВМесяцеОсуществленияКапВложений ИЛИ ПериодНачалоМесяца <> НачалоМесяца(КонецПериода);
		ВнеоборотныеАктивыЛокализация.СформироватьТаблицуПринятыхКУчетуВнеоборотныхАктивовДляРаспределенияНДС_2_4(
			Реквизиты.Организация, НачалоПериода, КонецПериода, МенеджерВременныхТаблиц, , НеОтбиратьПоОстаткамПартий);
		Если Реквизиты.РаспределениеЗаПолныйПериод Тогда // Условие конца квартала
			ВнеоборотныеАктивыЛокализация.СформироватьТаблицуПринятыхКУчетуВнеоборотныхАктивовДляРаспределенияНДС_2_4(
				Реквизиты.Организация, ПериодНачалоМесяца, Реквизиты.ПериодОкончание, МенеджерВременныхТаблиц, "ВтВнеоборотныеАктивыКвартал", Истина);
		КонецЕсли;
...
Процедура СформироватьДвижения(ТаблицыДляДвижений, ДанныеДляРаспределения, РезультатРаспределения, Реквизиты, Регистры) Экспорт
	...
	НоваяСтрока.ДатаОтражения = НачалоДня(Реквизиты.ДатаРаспределения);
