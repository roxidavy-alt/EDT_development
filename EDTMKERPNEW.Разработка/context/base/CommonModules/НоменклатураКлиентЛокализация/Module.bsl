...
Процедура РазрешитьРедактированиеРеквизитовОбъектаЗавершение(Форма, Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Найти("Операции0") <> Неопределено И Не Форма.Элементы.Операции0.ТолькоПросмотр Тогда
		Форма.Элементы.ТоварУчаствуетВДеятельностиПоСтавкеНДС0.Доступность = Истина;
		Форма.Элементы.ТоварУчаствуетВДеятельностиПоСтавкеНДС0.ТолькоПросмотр = Ложь;
	КонецЕсли;
	//-- НЕ УТ
