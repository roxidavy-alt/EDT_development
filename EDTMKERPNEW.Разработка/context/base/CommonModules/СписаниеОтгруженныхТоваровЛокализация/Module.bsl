...
#Область ОтражениеПревышенияЦеныНадФактическойСебестоимостью // (Дт ХХ.ХХ :: Кт 91.01)
	...
	|	""Отражение превышения прайсовой цены над фактической себестоимостью товара"" КАК Содержание,
	|	""СписаниеОтгруженныхТоваровЛокализация_1776"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.СписаниеОтгруженныхТоваров КАК Операция
	|	ПО
...
#Область ОтражениеПревышенияФактическойСебестоимостьюНадЦеной // (Дт 91.02 :: Кт ХХ.ХХ)
	...
	|	Стоимости.СуммаВР КАК СуммаВРКт,
	|	""Отражение превышения фактической себестоимости товара над прайсовой ценой"" КАК Содержание,
	|	""СписаниеОтгруженныхТоваровЛокализация_1777"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстСписаниеКомиссионныхТоваров
	...
	|	""СписаниеОтгруженныхТоваровЛокализация_718"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстЗапросаТаблицаПрослеживаемыеТоварыВСоставеОС(Запрос, ТекстыЗапроса, Регистры)
	...
	Если НЕ ПроведениеДокументов.ЕстьТаблицаЗапроса("ВтВидыЗапасов", ТекстыЗапроса) Тогда
		Документы.СписаниеОтгруженныхТоваров.ТекстЗапросаВтВидыЗапасов(Запрос, ТекстыЗапроса);
	КонецЕсли;
	...
	|	И &СтатьяРасходов = ЗНАЧЕНИЕ(Перечисление.ТипыРасходов.ФормированиеСтоимостиВНА)
