...
// Функция проверяет ведется ли учет внеоборотных активов версии 2.2.
// 
// Возвращаемое значение:
//  Булево - Истина, если ведется учет внеоборотных активов версии 2.2.
//
...
// Вовзращает этап закрытия месяца, направленный на переход учета внеоборотных активов версии 2.4.
// 
// Возвращаемое значение:
//  СправочникСсылка.ОперацииЗакрытияМесяца
//
Функция ОперацияПереходаНаУчетВнеоборотныхАктивов2_4() Экспорт
	
	Операция = Неопределено;
	//++ Локализация
	Операция = Справочники.ОперацииЗакрытияМесяца.ПереходНаУчетВнеоборотныхАктивовВерсии24;
	//-- Локализация
...
Процедура ДобавитьЭтап_ПереходНаУчетВнеоборотныхАктивовВерсии24(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		ОперацияПереходаНаУчетВнеоборотныхАктивов2_4(),,,,
		Справочники.ОперацииЗакрытияМесяца.НачисленияПоКредитамДепозитамИВыданнымЗаймам);
		
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Переход на учет внеоборотных активов версии 2.4';
											|en = 'Migration to fixed asset accounting 2.4'"; // @НСтр-1
...
Процедура СформироватьТаблицуПринятыхКУчетуВнеоборотныхАктивовДляРаспределенияНДС_2_2(НачалоПериода, КонецПериода, МенеджерВременныхТаблиц) Экспорт
	...
	|	ИСТИНА КАК ЭтоВводВЭксплуатацию,
...
//
// Параметры:
//  Организация             - СправочникСсылкаОрганизация - организация, по которой формируются принятые к учету ВНА. 
//  НачалоПериода           - Дата - Начало периода.
//  КонецПериода            - Дата  - Конец периода.
//  МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - Хранит сформированную таблицу, согласно наименованию.
//  ИмяТаблицы              - Строка - необязательный параметр наименование сформированной временной таблицы.
//                                     Значение по умолчанию "ВтВнеоборотныеАктивы".
//  НеОтбиратьПоОстаткамПартий - Булево - не требуется отбирать по остаткам регистра партий, вывести все принятия к учету.
//
Процедура СформироватьТаблицуПринятыхКУчетуВнеоборотныхАктивовДляРаспределенияНДС_2_4(Организация, НачалоПериода, КонецПериода,
			МенеджерВременныхТаблиц, ИмяТаблицы = "ВтВнеоборотныеАктивы", НеОтбиратьПоОстаткамПартий = Ложь) Экспорт
	...
	|	ТаблицаОстатков КАК ТаблицаОстатков
	|ГДЕ
	|	НЕ &НеОтбиратьПоОстаткамПартий
	|	И ТаблицаОстатков.Организация = &Организация
	...
	|	ВЫБОР КОГДА СтоимостьОС.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.МодернизацияОС)
	|		ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ                                         КАК ЭтоВводВЭксплуатацию,
	...
	|ГДЕ
	|	СтоимостьОС.Организация = &Организация
	|	И СтоимостьОС.Период МЕЖДУ &НачалоПериода И &КонецПериода
	...
	|
	|	И (&НеОтбиратьПоОстаткамПартий
	|		ИЛИ СтоимостьОС.АналитикаКапитализацииРасходов В
	|			(ВЫБРАТЬ
	|				ОтборОбъектов.Объект
	|			ИЗ
	|				ОтборОбъектов))
	...
	|	ВЫБОР КОГДА СтоимостьНМА.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.УлучшениеНМА)
	|		ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ                                         КАК ЭтоВводВЭксплуатацию,
	// В режиме 2.4 не используются, но нужны для совместимости
	...
	|ГДЕ
	|	СтоимостьНМА.Организация = &Организация 
	|	И СтоимостьНМА.Период МЕЖДУ &НачалоПериода И &КонецПериода
	...
	|
	|	И (&НеОтбиратьПоОстаткамПартий
	|		ИЛИ СтоимостьНМА.НематериальныйАктив В
	|			(ВЫБРАТЬ
	|				ОтборОбъектов.Объект
	|			ИЗ
	|				ОтборОбъектов))
	...
	Если ЗначениеЗаполнено(ИмяТаблицы)
		И ИмяТаблицы <> "ВтВнеоборотныеАктивы" Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВтВнеоборотныеАктивы", ИмяТаблицы);
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("НеОтбиратьПоОстаткамПартий", НеОтбиратьПоОстаткамПартий);
...
Процедура ПолучитьСведенияОбОсновныхСредствахВДокументах(МенеджерВременныхТаблиц, СписокДокументов) Экспорт
	...
	|	МАКСИМУМ(ПорядокОтраженияСтоимостьВНА.Период) КАК ПериодСтоимостьВНА,
	|	МАКСИМУМ(ПорядокОтраженияАмортизацияВНА.Период) КАК ПериодАмортизацияВНА
	|ПОМЕСТИТЬ втПорядокОтраженияНаСчетахУчетаПериод
	|ИЗ
	|	втПорядокУчетаОС КАК втПорядокУчетаОС
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета КАК ПорядокОтраженияСтоимостьВНА
	|		ПО втПорядокУчетаОС.ГруппаФинансовогоУчета = ПорядокОтраженияСтоимостьВНА.АналитикаУчета
	|			И ПорядокОтраженияСтоимостьВНА.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|			И ПорядокОтраженияСтоимостьВНА.МестоУчета = НЕОПРЕДЕЛЕНО
	|			И ПорядокОтраженияСтоимостьВНА.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.СтоимостьВНА)
	|			И втПорядокУчетаОС.Период >= ПорядокОтраженияСтоимостьВНА.Период
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета КАК ПорядокОтраженияАмортизацияВНА
	|		ПО втПорядокУчетаОС.ГруппаФинансовогоУчета = ПорядокОтраженияАмортизацияВНА.АналитикаУчета
	|			И ПорядокОтраженияАмортизацияВНА.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|			И ПорядокОтраженияАмортизацияВНА.МестоУчета = НЕОПРЕДЕЛЕНО
	|			И ПорядокОтраженияАмортизацияВНА.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.АмортизацияВНА)
	|			И втПорядокУчетаОС.Период >= ПорядокОтраженияАмортизацияВНА.Период
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Данные.ГруппаФинансовогоУчета КАК ГруппаФинансовогоУчета,
	|	ПорядокОтраженияСтоимостьВНА.СчетУчета КАК СчетУчета_СтоимостьВНА,
	|	ПорядокОтраженияАмортизацияВНА.СчетУчета КАК СчетУчета_АмортизацияВНА
	|ПОМЕСТИТЬ втПорядокОтраженияНаСчетахУчета
	|ИЗ
	|	втПорядокОтраженияНаСчетахУчетаПериод КАК Данные
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета КАК ПорядокОтраженияСтоимостьВНА
	|		ПО Данные.ГруппаФинансовогоУчета = ПорядокОтраженияСтоимостьВНА.АналитикаУчета
	|			И ПорядокОтраженияСтоимостьВНА.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|			И ПорядокОтраженияСтоимостьВНА.МестоУчета = НЕОПРЕДЕЛЕНО
	|			И ПорядокОтраженияСтоимостьВНА.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.СтоимостьВНА)
	|			И Данные.ПериодСтоимостьВНА = ПорядокОтраженияСтоимостьВНА.Период
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета КАК ПорядокОтраженияАмортизацияВНА
	|		ПО Данные.ГруппаФинансовогоУчета = ПорядокОтраженияАмортизацияВНА.АналитикаУчета
	|			И ПорядокОтраженияАмортизацияВНА.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|			И ПорядокОтраженияАмортизацияВНА.МестоУчета = НЕОПРЕДЕЛЕНО
	|			И ПорядокОтраженияАмортизацияВНА.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.АмортизацияВНА)
	|			И Данные.ПериодАмортизацияВНА = ПорядокОтраженияАмортизацияВНА.Период
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ГруппаФинансовогоУчета
	|;
	...
	|	ЕСТЬNULL(ПорядокОтраженияНаСчетахУчета.СчетУчета_СтоимостьВНА, ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)) КАК СчетУчета,
	|	ЕСТЬNULL(ПорядокОтраженияНаСчетахУчета.СчетУчета_АмортизацияВНА, ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)) КАК СчетАмортизации,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ втПорядокОтраженияНаСчетахУчета КАК ПорядокОтраженияНаСчетахУчета
	|		ПО втПорядокУчетаОС.ГруппаФинансовогоУчета = ПорядокОтраженияНаСчетахУчета.ГруппаФинансовогоУчета
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка,
	|	ОсновноеСредство
...
// 
// Возвращаемое значение:
//  Булево - Истина, если требуется показать статьи отражения расходов по налогам.
//
...
//  Организация	- СправочникСсылка.Организации - Организация документа.
//  Дата - Дата - Дата документа.
//  СписокОС - Массив из СправочникСсылка.ОбъектыЭксплуатации - Список ОС документа.
// 
// Возвращаемое значение:
//  Булево - Истина, если требуется заполнить статью.
Функция МожетНачислятьсяНалогНаИмущество(Организация, Дата, СписокОС) Экспорт
	
	Если НЕ ЗначениеЗаполнено(Организация)
		ИЛИ СписокОС.Количество() = 0 Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	СписокОС.ГруппаОС КАК ГруппаОС,
	|	ПорядокУчетаОСБУ.АмортизационнаяГруппа КАК АмортизационнаяГруппа,
	...
	Пока Выборка.Следующий() Цикл
		...
		ТребуетсяНастройка = ТребуетсяНастройкаОтраженияРасходовПоНалогам(
			Организация, Дата, Неопределено, Выборка.ГруппаОС, Выборка.АмортизационнаяГруппа, Выборка.НедвижимоеИмущество);
...
Функция СуммыНалоговогоУчетаЗаполнены(ТипСсылки) Экспорт
	...
	СуммыНалоговогоУчетаЗаполнены = 
		...
		Или ТипСсылки = Тип("ДокументСсылка.ОтнесениеОтклоненийВСтоимостьОС")
...
Процедура ДобавитьЭтап_ПризнаниеВНалоговомУчетеАрендныхПлатежей(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ПризнаниеВНалоговомУчетеАрендныхПлатежей,,,,
		Справочники.ОперацииЗакрытияМесяца.НачислениеАмортизацииНМА);

	НоваяСтрока.МультиязычноеНаименование = "ru = 'Признание в налоговом учете арендных платежей';
											|en = 'Recognition of rental payments in tax accounting'"; // @НСтр-1
	НоваяСтрока.ПредшествующиеЭтапы.Добавить(ОперацияПереходаНаУчетВнеоборотныхАктивов2_4());
...
Процедура ДополнитьПараметрамиУчетаВнеоборотныхАктивов(Знач ПараметрыРасчета) Экспорт
	...
	Запрос.УстановитьПараметр("Период", ПараметрыРасчета.Период);
	...
	ПараметрыРасчета.Вставить("ПрименяетсяФСБУ6", Истина);
	ПараметрыРасчета.Вставить("НеПрименяетсяФСБУ6", Ложь);
	ПараметрыРасчета.Вставить("ПрименяетсяФСБУ14", Истина);
	ПараметрыРасчета.Вставить("НеПрименяетсяФСБУ14", Ложь);

	Выборка = Результат[Результат.ВГраница()-1].Выбрать();
	Пока Выборка.Следующий() Цикл
		Если Выборка.ПорядокУчетаВНА = Перечисления.ПорядокУчетаВНА.ПоСтандартамМУ Тогда
			ПараметрыРасчета.Вставить("ПрименяетсяПорядокУчетаВНАПоСтандартамМУ", Истина);
		Иначе
			ПараметрыРасчета.Вставить("НеПрименяетсяПорядокУчетаВНАПоСтандартамМУ", Истина);
		КонецЕсли;
	КонецЦикла;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ПрименяетсяФСБУ6 Тогда
			ПараметрыРасчета.Вставить("ПрименяетсяФСБУ6", Истина);
		Иначе
			ПараметрыРасчета.Вставить("НеПрименяетсяФСБУ6", Истина);
		КонецЕсли;
		
		Если Выборка.ПрименяетсяФСБУ14 Тогда
			ПараметрыРасчета.Вставить("ПрименяетсяФСБУ14", Истина);
		Иначе
			ПараметрыРасчета.Вставить("НеПрименяетсяФСБУ14", Истина);
		КонецЕсли;
...
Функция ТекстЗапросаДляФормированияЗаданийКРасчетуАмортизацииНМА(ДанныеТаблиц, ТекстОбъединенияДанных) Экспорт
	...
	|
	|	МИНИМУМ(ВЫБОР
	|				КОГДА ТИПЗНАЧЕНИЯ(ИзмененныеДанные.Документ) = ТИП(Документ.АмортизацияНМА2_4)
	|					ТОГДА ДОБАВИТЬКДАТЕ(ИзмененныеДанные.Период, МЕСЯЦ, 1)
	|				ИНАЧЕ ИзмененныеДанные.Период
	|			КОНЕЦ) КАК Период
	|
...
Функция ТекстЗапросаТаблицаОтражениеНачисленнойАмортизацииВПрочихРасходах(ДопПоля) Экспорт
	...
	ВнеоборотныеАктивыСлужебный.ЗаменитьДополнительныеПоляВЗапросе(ТекстЗапроса, ДопПоля, "Регистратор,ПериодОтклонения");
...
Функция ТекстЗапросаПрочиеРасходыНеПриниматьКУчетуВключитьВРасходы(ЕстьПорядокУчетаУУ, ЕстьПорядокУчетаБУ, ЕстьПорядокУчетаНУ) Экспорт
	...
	#Область НеПриниматьКУчетуВключитьВРасходыУУ // Дт 91 - Кт 08
	...
	#Область НеПриниматьКУчетуВключитьВРасходыБУ // Дт 91 - Кт 08
...
Процедура ЗаменитьТекстКорректировкиСтоимостиИНВ(ТекстЗапроса) Экспорт
	...
	|
	|		ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(ВтРасчетСтоимости.СуммаНУ, 0) * ЕСТЬNULL(ВтНастройкиУчетаНалогаНаПрибыль.ДоляРасходовИНВ, 0) / 100 КАК ЧИСЛО (31,0))
	|
	|	ИНАЧЕ 0
	|
	|КОНЕЦ";
...
Процедура ТекстОтраженияВРеглУчетеПеремещенияСтоимостиОС(СписокЗапросов) Экспорт
	...
	|	""ВнеоборотныеАктивыЛокализация_182"" КАК НомерФрагмента
	|ИЗ
	...
	|	""Перемещение стоимости ОС"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_183"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Процедура ТекстОтраженияВРеглУчетеПеремещенияАмортизацииОС(СписокЗапросов) Экспорт
	...
	|	""ВнеоборотныеАктивыЛокализация_184"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	""Перемещение амортизации ОС"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_185"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	""ВнеоборотныеАктивыЛокализация_186"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.АмортизацияОС КАК Строки
	...
	|
	|	""Перемещение накопленного износа ОС"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_187"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.АмортизацияОС КАК Строки
...
Процедура ТекстОтраженияВРеглУчетеПеремещенияСтоимостиНМА(СписокЗапросов) Экспорт
	...
	|	""ВнеоборотныеАктивыЛокализация_188"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|
	|	""Перемещение стоимости НМА"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_189"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьНМА КАК Строки
...
Процедура ТекстОтраженияВРеглУчетеПеремещенияАмортизацииНМА(СписокЗапросов) Экспорт
	...
	|	""ВнеоборотныеАктивыЛокализация_190"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	Строки.АмортизацияВРЦФ КАК СуммаВРКт,
	|	""Перемещение амортизации НМА"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_191"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.АмортизацияНМА КАК Строки
...
Процедура ТекстОтраженияВРеглУчетеПеремещенияРезерваДооценкиОС(СписокЗапросов) Экспорт
	...
	|	""ВнеоборотныеАктивыЛокализация_192"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|	""ВнеоборотныеАктивыЛокализация_193"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|	""ВнеоборотныеАктивыЛокализация_194"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
Процедура ТекстОтраженияВРеглУчетеПеремещенияРезерваДооценкиНМА(СписокЗапросов) Экспорт
	...
	|
	|	""Перемещение резерва дооценки"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_195"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|
	|	""Передача резерва дооценки в обособленное подразделение"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_196"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|
	|	""Принятие резерва дооценки в обособленное подразделение"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_197"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
Процедура ТекстОтраженияВРеглУчетеНачисленияАмортизацииОС(СписокЗапросов) Экспорт
	...
	|	""ВнеоборотныеАктивыЛокализация_198"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.АмортизацияОС КАК Строки
	...
	|	
	|	""Начисление амортизации"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_199"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|	
	|	""Начисление амортизации"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_200"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.АмортизацияОС КАК Строки
	...
	|	
	|	""Начисление амортизации"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_201"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.АмортизацияОС КАК Строки
	...
	|	
	|	""Начисление амортизации"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_202"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.АмортизацияОС КАК Строки
	...
	|	
	|	""Начисление амортизации"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_203"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.АмортизацияОС КАК Строки
	...
	|	""ВнеоборотныеАктивыЛокализация_204"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.АмортизацияОС КАК Строки
	...
	|	
	|	""Начисление амортизации"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_205"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.АмортизацияОС КАК Строки
	...
	|	""ВнеоборотныеАктивыЛокализация_206"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.АмортизацияОС КАК Строки
	...
	|	""ВнеоборотныеАктивыЛокализация_207"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.АмортизацияОС КАК Строки
	...
	|	
	|	""Корректировка обесценения"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_208"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.АмортизацияОС КАК Строки
	...
	|	""ВнеоборотныеАктивыЛокализация_209"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|	
	|	""Сторно обесценения"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_210"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|	
	|	""Сторно обесценения"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_211"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
Процедура ТекстОтраженияВРеглУчетеАмортизационнойПремии(СписокЗапросов) Экспорт
	...
	|	""ВнеоборотныеАктивыЛокализация_212"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|	
	|	""Амортизационная премия"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_213"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьОС КАК Строки
	...
	|	
	|	""Амортизационная премия"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_214"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьОС КАК Строки
	...
	|	
	|	""Амортизационная премия"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_215"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьОС КАК Строки
	...
	|	
	|	""Амортизационная премия"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_216"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьОС КАК Строки
...
Процедура ТекстОтраженияВРеглУчетеПеремещенияДоходовОСЗаСчетЦФ(СписокЗапросов) Экспорт
	...
	|	""ВнеоборотныеАктивыЛокализация_217"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Процедура ТекстОтраженияВРеглУчетеСписанияОС(СписокЗапросов) Экспорт
	...
	|	""ВнеоборотныеАктивыЛокализация_218"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПрочиеРасходы КАК СтрокиДт
	...
	|	
	|	""Списание первоначальной стоимости ОС"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_219"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПрочиеРасходы КАК СтрокиДт
	...
	|	""ВнеоборотныеАктивыЛокализация_220"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.АмортизацияОС КАК СтрокиДт
	...
	|	
	|	""Списание начисленной амортизации"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_221"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.АмортизацияОС КАК СтрокиДт
	...
	|	""ВнеоборотныеАктивыЛокализация_1688"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.АмортизацияОС КАК Строки
	...
	|	""ВнеоборотныеАктивыЛокализация_222"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|	""ВнеоборотныеАктивыЛокализация_223"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ОбесценениеВНА КАК СтрокиДт
	...
	|	""ВнеоборотныеАктивыЛокализация_224"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|	""ВнеоборотныеАктивыЛокализация_225"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьОС КАК Строки
	...
	|	""ВнеоборотныеАктивыЛокализация_226"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|
	|	""Амортизационная премия"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_227"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьОС КАК Строки
...
Процедура ТекстОтраженияВРеглУчетеЗакрытияРасходовОтВыбытияОС(СписокЗапросов) Экспорт
	...
	|	""ВнеоборотныеАктивыЛокализация_228"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПрочиеРасходы КАК СтрокиДт
	...
	|	
	|	""Закрытие расходов от выбытия ОС"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_229"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПрочиеРасходы КАК Строки
...
Процедура ТекстОтраженияВРеглУчетеЗакрытияРасходовОтВыбытияОССОтложеннымПереходомПрав(СписокЗапросов) Экспорт
	...
	|	""ВнеоборотныеАктивыЛокализация_230"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	""ВнеоборотныеАктивыЛокализация_1689"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	
	|	""Закрытие расходов от выбытия ОС до перехода права собственности"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_231"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	
	|	""Закрытие расходов от выбытия ОС после перехода права собственности"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_232"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Процедура ТекстОтраженияВРеглУчетеУслугПоАренде(СписокЗапросов) Экспорт
	...
	|			ТОГДА ""Списание НДС""
	|	КОНЕЦ КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_1690"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	""ВнеоборотныеАктивыЛокализация_233"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	""ВнеоборотныеАктивыЛокализация_234"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""Поступление НДС"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_235"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	""ВнеоборотныеАктивыЛокализация_236"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПрочиеРасходы КАК СтрокиДт
	...
	|
	|	""Отражение услуги в расходах"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_237"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	""ВнеоборотныеАктивыЛокализация_238"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПрочиеРасходы КАК СтрокиДт
	...
	|
	|	""Балансировка ВР по договору аренды"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_239"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ДвиженияПоПрочимАктивамПассивам КАК СтрокиДт
...
Процедура ТекстОтраженияВРеглУчетеЗачетаАвансовПоАренде(СписокЗапросов) Экспорт
	...
	|	КОНЕЦ КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_1691"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыПоФинансовымИнструментам КАК СтрокиДт
...
Процедура ТекстОтраженияВРеглУчетеЗакрытияСчетовПоАренде(СписокЗапросов) Экспорт
	...
	|			ТОГДА ""Списание НДС""
	|	КОНЕЦ КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_1692"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыПоФинансовымИнструментам КАК СтрокиДт
	...
	|	""ВнеоборотныеАктивыЛокализация_240"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыПоФинансовымИнструментам КАК СтрокиДт
	...
	|	""ВнеоборотныеАктивыЛокализация_241"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыПоФинансовымИнструментам КАК СтрокиДт
	...
	|	""ВнеоборотныеАктивыЛокализация_242"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПрочиеРасходы КАК СтрокиДт
	...
	|	""ВнеоборотныеАктивыЛокализация_243"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПрочиеРасходы КАК СтрокиДт
	...
	|	""ВнеоборотныеАктивыЛокализация_244"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыПоФинансовымИнструментам КАК СтрокиДт
	...
	|	""ВнеоборотныеАктивыЛокализация_245"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыПоФинансовымИнструментам КАК СтрокиДт
...
Процедура ЗаполнитьСуммыЦелевыхСредств(ОС, ЦелевоеФинансирование, ВариантПримененияЦелевогоФинансирования, ПолеСуммаБУ) Экспорт
	...
	ИначеЕсли ВариантПримененияЦелевогоФинансирования = Перечисления.ВариантыПримененияЦелевогоФинансирования.Частичное Тогда
		...
		Для каждого ДанныеСтроки Из ОС Цикл
			СтоимостьРеглКоэффициенты.Добавить(ДанныеСтроки[ПолеСуммаБУ]);
		КонецЦикла; 
...
Функция ЕстьОшибкиЗаполненияРасходовПоАмортизацииНМА(МассивОрганизаций, Период) Экспорт
	...
	РегистрыСведений.ПорядокОтраженияНаСчетахУчета.ПереопределитьВТекстеЗапросаПорядокОтраженияСчетаУчета(Запрос.Текст, "НачалоМесяца");
...
Функция ЕстьОшибкиЗаполненияРасходовПоАмортизацииОС(МассивОрганизаций, Период) Экспорт
	...
	РегистрыСведений.ПорядокОтраженияНаСчетахУчета.ПереопределитьВТекстеЗапросаПорядокОтраженияСчетаУчета(ТекстЗапроса, "НачалоМесяца");
...
//  СтатьяРасходовАмортизационнойПремии - ПланВидовХарактеристикСсылка.СтатьиРасходов - 
//  СписокОС - Массив - 
// 
// Возвращаемое значение:
//  Булево - Истина, если амортизационную премию можно указать в виде процента от суммы.
Функция ДоступенВводАмортизационнойПремииВПроцентах(Дата, Организация, СтатьяРасходовАмортизационнойПремии, СписокОС) Экспорт
	...
	Если НЕ ПараметрыУчетнойПолитики = Неопределено
		...
		ИЛИ НЕ ЗначениеЗаполнено(СтатьяРасходовАмортизационнойПремии)
		ИЛИ СписокОС.Количество() = 0) Тогда
...
// Позволяет добавить в текст запроса формирование таблицы регистра при локализации расчета амортизации.
// 
// Параметры:
...
Процедура ВключитьВСтоимостьВНАРезультатРаспределенияНДС_2_2(ТаблицаВнеоборотныхАктивов, РезультатРаспределения, ТаблицыДляДвижений, Реквизиты)
	...
	|		- ПервоначальныеСведенияНМА.ПервоначальнаяСтоимостьПР КАК ПервоначальнаяСтоимостьВР,
	|
...
// Параметры:
//  СписокЗапросов - Массив из Строка - 
Процедура ТекстОтраженияВРеглУчетеСписаниеНаРасходыНеПринимаяКУчету(СписокЗапросов) Экспорт
	...
	|	""ВнеоборотныеАктивыЛокализация_246"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПрочиеРасходы КАК СтрокиДт
	...
	|		ПО СтатьиСтроительстваДт.Ссылка = СтрокиДт.СтатьяРасходов
	...
	|		ПО СтатьиСтроительстваКт.Ссылка = СтрокиКт.СтатьяРасходов
	...
	|
	|	""Списание на расходы, не принимая к учету"" КАК Содержание,
	|	""ВнеоборотныеАктивыЛокализация_247"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПрочиеРасходы КАК СтрокиДт
		...
		Если СведенияОбУчетеОС.СостояниеНУ = Перечисления.СостоянияОС.ПринятоКУчету
			...
			Если ДатаВыбытия <= ДатаОкончанияСПИ
				...
				ТекстНадписи = НСтр("ru = 'В отношении основных средств применен инвестиционный налоговый вычет.
									...
									|en = 'An investment tax deduction is applied to fixed assets.
									|If a fixed asset for which an investment deduction is applied is disposed from the company before the end of its useful life (unless a different useful life is established by a law of a territorial entity of the Russian Federation), unfavorable tax consequences arise.
									|The amount of income tax (advance payment) that was not paid due to the deduction application must be restored and paid to the budget.
									|Penalties calculated from the day following the day of the payment of the tax (advance payment) established by the Tax Code of the Russian Federation must also be paid. In this case, the income from the fixed asset sale can be reduced by its initial cost.'");
		...
		Если СведенияОбУчетеНМА.СостояниеНУ = Перечисления.СостоянияНМА.ПринятКУчету
			...
			Если ДатаВыбытия <= ДатаОкончанияСПИ
				...
				ТекстНадписи = НСтр("ru = 'В отношении нематериальных активов применен инвестиционный налоговый вычет.
									...
									|en = 'An investment tax deduction is applied to intangible assets.
									|If an intangible asset for which an investment deduction is applied is disposed from the company before the end of its useful life (unless a different useful life is established by a law of a territorial entity of the Russian Federation), unfavorable tax consequences arise. The amount of income tax (advance payment) that was not paid due to the deduction application must be restored and paid to the budget. Penalties calculated from the day following the day of the payment of the tax (advance payment) established by the Tax Code of the Russian Federation must also be paid. In this case, the income from the intangible asset sale can be reduced by its initial cost.'");
