...
Процедура ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	Если ТекстыЗапроса.Количество() = 0 Тогда
		...
		//++ НЕ УТ
		ТаблицаТоваровИсходная.Колонки.Добавить("СуммаАкциза", Новый ОписаниеТипов("Число"));
		ТаблицаТоваровИсходная.Колонки.Добавить("ОблагаетсяАкцизом", Новый ОписаниеТипов("Булево"));
		//-- НЕ УТ
	Иначе
	...
	//++ НЕ УТ
	|	ТаблицаТоваровИсходная.ОблагаетсяАкцизом КАК ОблагаетсяАкцизом,
	|	СУММА(ТаблицаТоваровИсходная.СуммаАкциза) КАК СуммаАкциза,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|	ТаблицаТоваровИсходная.ОблагаетсяАкцизом,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|	ТаблицаТоваровИзменение.СуммаАкциза КАК СуммаАкциза,
	|	ТаблицаТоваровИзменение.ОблагаетсяАкцизом КАК ОблагаетсяАкцизом,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|			И ТаблицаТоваровИзменение.ОблагаетсяАкцизом = ТаблицаТоваровИсходнаяСвернуто.ОблагаетсяАкцизом
	//-- НЕ УТ
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	//++ НЕ УТ
	|	ЕСТЬNULL(ТаблицаТоваровИсходнаяСвернуто.СуммаАкциза, 0),
	|	ТаблицаТоваровИзменение.ОблагаетсяАкцизом,
	//-- НЕ УТ
	|	ИСТИНА
	|ИЗ
	|	ТаблицаТоваровИзменение КАК ТаблицаТоваровИзменение
...
Процедура ПреобразоватьВременнуюТаблицуТоваровДляСчетаФактуры(МенеджерВременныхТаблиц) Экспорт
	...
	//++ НЕ УТ
	|	ЕСТЬNULL(ТаблицаТоваровПредыдущие.СуммаАкциза, 0) - ТаблицаДокумента.СуммаАкциза               КАК СуммаАкциза,
	|	ЕСТЬNULL(ТаблицаТоваровПредыдущие.СуммаАкциза, 0)        КАК СуммаАкцизаДо,
	|	0                                                        КАК СуммаАкцизаУвеличение,
	|	ТаблицаДокумента.СуммаАкциза                             КАК СуммаАкцизаУменьшение,
...
#Область ТекстВозвратОтКомиссионера // (Дт 41.01 :: Кт 45.01)
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_293"" КАК НомерФрагмента
	|ИЗ
...
#Область ТекстВозвратОтКомиссионераСкладПроизводства // (Дт 20 :: Кт 45.01)
	...
	|	ЕСТЬNULL(Стоимости.СуммаВР, Строки.СуммаВР) КАК СуммаВРКт,
	|	""Возврат собственных товаров от комиссионера"" КАК Содержание,
	|	""ВозвратТоваровОтКлиентаЛокализация_294"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстСторноПередачаНаСубкомиссию // (Дт 004.02 :: Кт 004.01)
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_295"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
#Область ТекстСторноНДСПрошлыхЛет // (Дт 91.02 :: Кт 19.03)
	ТекстСторноНДСПрошлыхЛет = "
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_297"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|
	|ГДЕ
	|	(Операция.ОтразитьВПрибылиУбыткахПрошлыхЛет
	|		ИЛИ НАЧАЛОПЕРИОДА(Операция.Дата, ГОД) <> НАЧАЛОПЕРИОДА(ЕСТЬNULL(Строки.ДокументРеализации.Дата, Операция.Дата), ГОД))
...
#Область ТекстУменьшениеНДСПрошлыхЛет // (Дт 91.02 :: Кт 19.09)

	ТекстУменьшениеНДСПрошлыхЛет = "
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_299"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|		ИЛИ Операция.ОтразитьВПрибылиУбыткахПрошлыхЛет)
...
#Область ТекстПереносНДС // (Дт 68.02 :: Кт 19.09)
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_300"" КАК НомерФрагмента
	|	
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
...
#Область ТекстВозвратРеализацииПрошлыхЛет // (Дт 91.02 :: Кт 62.01)
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_307"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
#Область ТекстВозвратСебестоимостиТоваровПрошлыхЛет // (Дт 41.01, 45.01 :: Кт 91.01)
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_308"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
#Область ТекстВозвратСебестоимостиТоваровПрошлыхЛетСкладПроизводства // (Дт 20 :: Кт 91.01)
	...
	|	КОНЕЦ КАК СуммаВРКт,
	|	""Себестоимость возвращенного товара прошлых лет"" КАК Содержание,
	|	""ВозвратТоваровОтКлиентаЛокализация_309"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
#Область ТекстВозвратТоваровДругогоКачества // (Дт 41.01, 45.01 :: Кт 41.01, 45.01)
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_310"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
#Область ТекстВозвратТоваровДругогоКачестваСкладПроизводства // (Дт 20 :: Кт 20)
	...
	|	ЕСТЬNULL(Стоимости.СуммаВР, Строки.СуммаВР) КАК СуммаВРКт,
	|	""Себестоимость возвращенного товара другого качества"" КАК Содержание,
	|	""ВозвратТоваровОтКлиентаЛокализация_311"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
#Область ТекстСторноВыручкиОтРеализацииКомиссионногоТовара // (Дт 62.01 :: Кт 76.ОК)
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_312"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
#Область ТекстСторноПродажаКомиссионногоТовара // (Дт :: Кт 004.01, 004.02)
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_313"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
#Область ТекстВозвратПереданнойТары // (Дт 41.03 :: Кт 76.09)
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_314"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_315"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_316"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстВключениеИсключениеНДСВСтоимость // (Дт 90.02.1 :: Кт 19.03)
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_317"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|	И НЕ Операция.ОтразитьВПрибылиУбыткахПрошлыхЛет
	|	И Операция.ХозяйственнаяОперация В (
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратТоваровОтКлиента),
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОтРозничногоПокупателя),
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратТоваровЧерезКомиссионера))
...
#Область ТекстВключениеИсключениеНДСВСтоимостьПрошлыхЛет // (Дт 91.02 :: Кт 19.03)
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_318"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстВосстановлениеАвансаПереносДолгаОтКлиента // (Дт <62.2> :: Кт <62.1, 76>)
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_1704"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#КонецОбласти
	...
	ТекстыОтражения.Добавить(ТекстСторноНДСПрошлыхЛет);
	ТекстыОтражения.Добавить(ТекстУменьшениеНДС());
	ТекстыОтражения.Добавить(ТекстУменьшениеНДСПрошлыхЛет);
	...
	ТекстыОтражения.Добавить(ТекстСторноСебестоимостиРеализованногоТовараПрошлыхМесяцев()); // Себестоимость реализованного товара прошлых месяцев (сторно) (Дт 90.02 :: Кт 41.01, 20)
	...
	ТекстыОтражения.Добавить(ТекстКорректировкаНачисленияАкцизовПрошлыхПериодов()); // Корректировка начисления акцизов прошлых периодов (Дт 91.02 :: Кт 68.03)
...
Функция ТекстКорректировкаНачисленияАкцизов()
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_1934"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|
	|ГДЕ
	|	НЕ Операция.ОтразитьВПрибылиУбыткахПрошлыхЛет
	|	И (НАЧАЛОПЕРИОДА(СтрокиДт.Период, ГОД) = НАЧАЛОПЕРИОДА(Строки.ПериодПродажи, ГОД)
...
Функция ТекстКорректировкаНачисленияАкцизовПрошлыхПериодов()
	
	ТекстЗапроса = "
	|////////////////////////////////////////////////////////////////////////////////////////////////////
	|// Корректировка начисления акцизов прошлых периодов (Дт 91.02 :: Кт 68.03)
	|ВЫБРАТЬ
	|	СтрокиКт.Регистратор КАК Ссылка,
	|	СтрокиКт.Период КАК Период,
	|	СтрокиКт.Организация КАК Организация,
	...
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПрочиеРасходы) КАК СчетДт,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПрибыльУбытокПрошлыхЛет) КАК СубконтоДт1,
	|	СтрокиДт.АналитикаУчетаНоменклатуры.Номенклатура КАК СубконтоДт2,
	|	ЕСТЬNULL(ТаблицаНалогообложенияПрибыли.ВариантНалогообложенияПрибыли, НЕОПРЕДЕЛЕНО) КАК СубконтоДт3,
	|
	|	0 КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_1935"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|
	|ГДЕ
	|	НАЧАЛОПЕРИОДА(СтрокиДт.Период, ГОД) > НАЧАЛОПЕРИОДА(Строки.ПериодПродажи, ГОД)
	|		И Строки.ПериодПродажи <> ДАТАВРЕМЯ(1,1,1)
	|	ИЛИ Операция.ОтразитьВПрибылиУбыткахПрошлыхЛет
	|";
...
// Возвращает текст запроса проводки по отражению выручки от реализации
// 
// Возвращаемое значение:
//  Строка - текст запроса
Функция ТекстВыручкаОтРеализации()
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_2027"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|		Документ.ВозвратТоваровОтКлиента КАК Операция
	|	ПО
	|		СтрокиДт.Ссылка = Операция.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РаздельныйУчет_НастройкиНалогообложенияПрибыли КАК ТаблицаНалогообложенияПрибыли
	|	ПО 
	...
	|		ВтСтроки КАК Строки
	|	ПО
	|		СтрокиКт.Ссылка = Строки.Ссылка
	|		И СтрокиКт.НастройкаХозОперации = Строки.НастройкаХозяйственнойОперации
	|		И СтрокиКт.ИдентификаторФинЗаписи = Строки.ИдентификаторФинЗаписи
	|		И СтрокиКт.Организация = Строки.Организация
	|		И СтрокиКт.Номенклатура = Строки.Номенклатура
	|		И СтрокиКт.МестоХранения = Строки.Склад
	|
	|ГДЕ
	|	НЕ Операция.ОтразитьВПрибылиУбыткахПрошлыхЛет
	|	И (Строки.Ссылка ЕСТЬ NULL
	|		ИЛИ НАЧАЛОПЕРИОДА(СтрокиКт.Период, ГОД) = НАЧАЛОПЕРИОДА(Строки.ПериодПродажи, ГОД)
	|		ИЛИ Строки.ПериодПродажи = ДАТАВРЕМЯ(1,1,1))
	|";
...
// Возвращает текст запроса проводки сторно входящего НДС (Дт 90.03 :: Кт 19.03)
// 
// Возвращаемое значение:
//  Строка - текст запроса
Функция ТекстСторноНДС()
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_2006"" КАК НомерФрагмента
	|
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	...
	|		И СтрокиДт.ИдентификаторФинЗаписи = Строки.ИдентификаторФинЗаписи
	|		И СтрокиКт.Организация = Строки.Организация
	|		И СтрокиДт.Номенклатура = Строки.Номенклатура
	|		И СтрокиДт.МестоХранения = Строки.Склад
	|
	|ГДЕ
	|	НЕ Операция.ОтразитьВПрибылиУбыткахПрошлыхЛет
	|	И (Строки.Ссылка ЕСТЬ NULL
...
// Возвращает текст запроса проводки уменьшения начисленного НДС (Дт 90.03 :: Кт 19.09)
// 
// Возвращаемое значение:
//  Строка - текст запроса
Функция ТекстУменьшениеНДС()
	...
	|	СтрокиДт.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|
	|	СтрокиКт.НДС КАК Сумма,
	|	СтрокиКт.НДСУпр КАК СуммаУУ,
	|
	|	// Дт ///////////////////////////////////////////////////////////////////////////////////////////
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НДСПриПродаже) КАК ВидСчетаДт,
	...
	|	СтрокиКт.Регистратор КАК СубконтоКт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,
	|
	|	0 КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	...
	|	""ВозвратТоваровОтКлиентаЛокализация_2028"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.ДвиженияПоНДС КАК СтрокиКт
	|	ПО
	|		СтрокиДт.Ссылка = СтрокиКт.Регистратор
	|		И СтрокиДт.Период = СтрокиКт.Период
	|		И СтрокиДт.Организация = СтрокиКт.Организация
	...
	|		И СтрокиДт.НастройкаХозОперации = Строки.НастройкаХозяйственнойОперации
	|		И СтрокиДт.ИдентификаторФинЗаписи = Строки.ИдентификаторФинЗаписи
	|		И СтрокиДт.Организация = Строки.Организация
	|
	|ГДЕ
	|	НЕ Операция.ОтразитьВПрибылиУбыткахПрошлыхЛет
	|	И (Строки.Ссылка ЕСТЬ NULL
...
// Возвращает текст запроса проводки себестоимости реализованного товара прошлых месяцев (сторно)
// 
// Возвращаемое значение:
//  Строка - текст запроса
Функция ТекстСторноСебестоимостиРеализованногоТовараПрошлыхМесяцев()
	
	ТекстЗапроса = "
	|////////////////////////////////////////////////////////////////////////////////////////////////////
	|// Себестоимость реализованного товара прошлых месяцев (сторно) (Дт 90.02 :: Кт 41.01, 20)
	|ВЫБРАТЬ
	|	СтрокиДт.Ссылка КАК Ссылка,
	|	СтрокиДт.Период КАК Период,
	|	СтрокиДт.Организация КАК Организация,
	|	СтрокиДт.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|
	|	СтрокиДт.СебестоимостьРегл КАК Сумма,
	|	СтрокиДт.СебестоимостьУпр КАК СуммаУУ,
	|
	|	// Дт ///////////////////////////////////////////////////////////////////////////////////////////
	|
	|	СтрокиДт.ВидСчетаСебестоимости КАК ВидСчетаДт,
	|	СтрокиДт.ГруппаФинансовогоУчета КАК АналитикаУчетаДт,
	|	СтрокиДт.Склад КАК МестоУчетаДт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаДт,
	...
	|	0 КАК КоличествоДт,
	|
	|	СтрокиДт.СебестоимостьНУ КАК СуммаНУДт,
	|	СтрокиДт.СебестоимостьПР КАК СуммаПРДт,
	|	СтрокиДт.СебестоимостьВР КАК СуммаВРДт,
	|
	|	// Кт ///////////////////////////////////////////////////////////////////////////////////////////
	|
	|	ВЫБОР
	|		КОГДА СтрокиКт.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.МатериалыИРаботыВПодразделении)
	|		КОГДА СтрокиКт.РазделУчета В (ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПереданныеНаКомиссию),
	|									ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.СобственныеТоварыПереданныеПартнерам))
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НоменклатураПереданная)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НаСкладе) 
	|	КОНЕЦ КАК ВидСчетаКт,
	|	ВЫБОР
	|		КОГДА СтрокиКт.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)
	|			ТОГДА СтрокиКт.ГруппаПродукции
	|		ИНАЧЕ СтрокиКт.ГруппаФинансовогоУчета
	|	КОНЕЦ КАК АналитикаУчетаКт,
	|	ВЫБОР
	|		КОГДА СтрокиКт.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)
	|			ТОГДА СтрокиКт.ПодразделениеАналитики
	|		ИНАЧЕ СтрокиКт.Склад
	|	КОНЕЦ КАК МестоУчетаКт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаКт,
	...
	|		КОГДА СтрокиКт.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)
	|			ТОГДА СтрокиКт.ГруппаПродукции
	|		ИНАЧЕ СтрокиКт.Номенклатура
	|	КОНЕЦ КАК СубконтоКт1,
	|	СтрокиКт.Склад КАК СубконтоКт2,
	|	СтрокиКт.Контрагент КАК СубконтоКт3,
	|
	|	0 КАК ВалютнаяСуммаКт,
	|	СтрокиДт.Количество КАК КоличествоКт,
	|
	|	СтрокиДт.СебестоимостьНУ КАК СуммаНУКт,
	|	СтрокиДт.СебестоимостьПР КАК СуммаПРКт,
	|	СтрокиДт.СебестоимостьВР КАК СуммаВРКт,
	|	""Себестоимость реализованного товара прошлых месяцев (сторно)"" КАК Содержание,
	|	""ВозвратТоваровОтКлиентаЛокализация_2029"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|		И СтрокиДт.МестоХранения = СтрокиКт.КорСклад
	|		И СтрокиКт.КорНоменклатура <> НЕОПРЕДЕЛЕНО
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РаздельныйУчет_НастройкиНалогообложенияПрибыли КАК ТаблицаНалогообложенияПрибыли
	|	ПО 
	...
	|
	|ГДЕ
	|	СтрокиКт.ХозяйственнаяОперация В (
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратТоваровОтКлиентаПрошлыхПериодов),
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОприходованиеПоВозврату))
	|	И (НАЧАЛОПЕРИОДА(СтрокиКт.Период, ГОД) = НАЧАЛОПЕРИОДА(СтрокиКт.ПериодПродажи, ГОД)
	|		ИЛИ СтрокиКт.ПериодПродажи = ДАТАВРЕМЯ(1,1,1))
	|	И НЕ Операция.ОтразитьВПрибылиУбыткахПрошлыхЛет
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	СтрокиДт.СебестоимостьВР КАК СуммаВРКт,
	|	""Себестоимость реализованного товара прошлых месяцев (сторно)"" КАК Содержание,
	|	""ВозвратТоваровОтКлиентаЛокализация_2033"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|		И СтрокиДт.НастройкаХозОперации = СтрокиКт.НастройкаХозяйственнойОперации
	|		И СтрокиДт.ИдентификаторФинЗаписи = СтрокиКт.ИдентификаторФинЗаписи
	|		И СтрокиДт.Номенклатура = СтрокиКт.Номенклатура
	|		И СтрокиДт.МестоХранения = СтрокиКт.Склад
	|		И СтрокиКт.КорНоменклатура = НЕОПРЕДЕЛЕНО
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РаздельныйУчет_НастройкиНалогообложенияПрибыли КАК ТаблицаНалогообложенияПрибыли
