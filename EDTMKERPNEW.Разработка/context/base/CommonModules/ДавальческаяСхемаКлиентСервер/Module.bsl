////////////////////////////////////////////////////////////////////////////
// Модуль "ДавальческаяСхемаКлиентСервер" содержит процедуры и функции для 
// работы с механизмом производством из давальческого сырья.
//
////////////////////////////////////////////////////////////////////////////

#Область ПрограммныйИнтерфейс

//++ Устарело_Переработка24
#Область СтруктураДействий

// Добавляет действие "ЗаполнитьПризнакДоступноОформлениеОтчетаДавальцу" в структуру действий
//
// Параметры:
//  Форма             - ФормаКлиентскогоПриложения - форма, в которой формируется структура действий
//  СтруктураДействий - Структура        - структура действий
// 
Процедура ДобавитьДействиеЗаполнитьПризнакДоступноОформлениеОтчетаДавальцу(Форма, СтруктураДействий) Экспорт
	
	Если Не Форма.ИспользуетсяДавальческаяСхема Тогда
		Возврат;
	КонецЕсли;	
	
	СтруктураДействий.Вставить("ЗаполнитьПризнакДоступноОформлениеОтчетаДавальцу");
	
КонецПроцедуры

#КонецОбласти
//-- Устарело_Переработка24

#Область ХозяйственныеОперации

// Возвращает хозяйственную операцию договора.
//
// Возвращаемое значение:
//  ПеречислениеСсылка.ХозяйственныеОперации - 
// 
Функция ХозяйственнаяОперацияДоговора() Экспорт
	
	Возврат ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПроизводствоИзДавальческогоСырья2_5");
	
КонецФункции

#КонецОбласти

#Область ПредставлениеОбъектов

// Возвращает синоним документа ПриемкаТоваровНаХранение в давальческой схеме
//
// Возвращаемое значение:
//  Строка - 
//
Функция СинонимДокументаПриемкаТоваровНаХранение() Экспорт
	
	Возврат НСтр("ru = 'Поступление от давальца';
				|en = 'Receipt from material provider'");
	
КонецФункции

// Возвращает "Передача давальцу" как синоним документа ОтгрузкаТоваровСХранения
// 
// Возвращаемое значение:
//  Строка - 
//
Функция СинонимДокументаОтгрузкаТоваровСХраненияПередачаДавальцу() Экспорт
	
	Возврат НСтр("ru = 'Передача давальцу';
				|en = 'Goods issue — Subcontracting services delivered'");
	
КонецФункции

// Возвращает "Возврат давальцу" как синоним документа ОтгрузкаТоваровСХранения
// 
// Возвращаемое значение:
//  Строка - 
//
Функция СинонимДокументаОтгрузкаТоваровСХраненияВозвратДавальцу() Экспорт
	
	Возврат НСтр("ru = 'Возврат давальцу';
				|en = 'Goods return — Subcontracting services delivered'");
	
КонецФункции

// Возвращает "Выкуп товаров давальца" как синоним документа ВыкупПринятыхНаХранениеТоваров
// 
// Возвращаемое значение:
//  Строка - 
//
Функция СинонимДокументаВыкупПринятыхНаХранениеТоваров() Экспорт
	
	Возврат НСтр("ru = 'Выкуп товаров давальца';
				|en = 'Redemption of provider goods'");
	
КонецФункции

// Возвращает "Выкуп товаров давальца" как синоним документа ОтчетОСписанииТоваровСХранения
// 
// Возвращаемое значение:
//  Строка - 
//
Функция СинонимДокументаОтчетОСписанииТоваровСХранения() Экспорт
	
	Возврат НСтр("ru = 'Отчет о списании товаров давальца';
				|en = 'Report on provider goods adjustment'");
	
КонецФункции

// Возвращает синоним акта о расхождении для передачи давальцу
//
// Возвращаемое значение:
//  Строка - 
//
Функция СинонимДокументаАктОРасхожденияхПередачаДавальцу() Экспорт
	
	Возврат НСтр("ru = 'Акт о расхождениях после передачи давальцу';
				|en = 'Discrepancy report following Goods issue — Subcontracting services delivered'");
	
КонецФункции

// Возвращает синоним акта о расхождении для возврата давальцу
//
// Возвращаемое значение:
//  Строка - 
//
Функция СинонимДокументаАктОРасхожденияхВозвратСырьяДавальцу() Экспорт
	
	Возврат НСтр("ru = 'Акт о расхождениях после возврата давальцу';
				|en = 'Discrepancy report following Goods return — Subcontracting services delivered'");
	
КонецФункции

#КонецОбласти

#Область КлючиНазначения
	

// Ключ назначения документы приема в переработку.
// 
// Возвращаемое значение:
//  Строка - Ключ назначения документы приема в переработку
//
Функция КлючНазначенияДокументыПриемаВПереработку() Экспорт
	
	Возврат "ДокументыПриемаВПереработку";
	
КонецФункции
	
#КонецОбласти

#КонецОбласти