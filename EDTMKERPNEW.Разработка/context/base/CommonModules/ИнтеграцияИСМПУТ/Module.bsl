...
//-- НЕ УТКА
	...
	Тогда
		...
		ИмяТЧРасход = ?(ТипДокумента = Тип("ДокументСсылка.ПроизводствоБезЗаказа"), "МатериалыИРаботы", "РасходМатериаловИРабот");
		...
		+ ?(КакДокументаОснования, 
			"
			|
			|;
			|
			|"
			+	ТекстЗапросаМаркируемойПродукции(Документ, ИмяТЧРасход)
			+
			"
			|
			|ОБЪЕДИНИТЬ ВСЕ
			...
			|	СУММА(ТаблицаТовары.Количество) КАК Количество
			|ИЗ
			|	ТаблицаТоварыПобочныеИзделия КАК ТаблицаТовары
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
			|		ПО ОписаниеНоменклатурыИС.Номенклатура = ТаблицаТовары.Номенклатура
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК Наборы
			|		ПО Наборы.Номенклатура = ТаблицаТовары.Номенклатура.НаборУпаковок
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ТекстОшибкиЗаполнения) Тогда
			ТекстОшибкиЗаполнения = СтрШаблон(НСтр("ru = 'По документу %1 отсутствуют данные для заполнения.';
													|en = 'There is no data to fill in for the ""%1"" document.'"), Основание);
		КонецЕсли;
...
Процедура ЗаполнитьМаркировкаТоваровИСМПНаОснованииЭтапаПроизводства2_2(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	КонецЕсли;
	Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМОТП(ДокументОбъект.ТоварнаяГруппа) Тогда
		ДокументОбъект.Операция = Перечисления.ВидыОперацийИСМП.ОтчетОВерификацииНанесенныхКМ;
	КонецЕсли;
...
Процедура ЗаполнитьВыводИзОборотаПриОбъемноСортовомУчете(ДокументОбъект)
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ВыводИзОборотаИСМП")
		И ВыводИзОборотаКлиентСервер.ДоступноПолеОСУ(ДокументОбъект.ТоварнаяГруппа) Тогда
