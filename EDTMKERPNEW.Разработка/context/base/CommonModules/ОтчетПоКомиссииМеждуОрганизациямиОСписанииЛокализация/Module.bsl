...
#Область ТекстКомитентСписаниеКомиссионногоТовара // (Дт  :: Кт 004.02)
	...
	|	""ОтчетПоКомиссииМеждуОрганизациямиОСписанииЛокализация_758"" КАК НомерФрагмента
	|
	|ИЗ
...
#Область ТекстКомитентВзаиморасчетыПоКомиссионномуТовару // (Дт 76.09 :: Кт 76.ОК)
	...
	|	""ОтчетПоКомиссииМеждуОрганизациямиОСписанииЛокализация_759"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстКомитентПрочихДоходов // (Дт 76.09 :: Кт 91.01.1)
	...
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	Операция.ОбъектРасчетовСКомиссионером.УникальныйИдентификатор КАК ИдентификаторСтроки,
	|
	...
	|	""ОтчетПоКомиссииМеждуОрганизациямиОСписанииЛокализация_760"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстКомитентСписаниеСебестоимостиТовара // (Дт 91.02 :: Кт 45.01)
	...
	|	""ОтчетПоКомиссииМеждуОрганизациямиОСписанииЛокализация_761"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстКомиссионерЗадолженностьПередКомитентом // (Дт 76.ОК :: Кт 60.01)
	...
	|	ЕСТЬNULL(ВидыЗапасовДокумента.Договор.ГруппаФинансовогоУчета, ЗНАЧЕНИЕ(Справочник.ГруппыФинансовогоУчетаРасчетов.ПустаяСсылка)) КАК ГруппаФинансовогоУчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаДт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаДт,
	...
	|	""ОтчетПоКомиссииМеждуОрганизациямиОСписанииЛокализация_762"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
