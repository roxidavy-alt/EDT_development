...
Процедура ЗаписатьФискальнуюОперацию(ДанныеОперации, РезультатВыполнения) Экспорт
	...
	Если ПустаяСтрока(ДанныеОперации.ИдентификаторФискальнойЗаписи) Тогда
		...
		ДанныеОперации.ИдентификаторФискальнойЗаписи = ИдентификаторЗаписи;
	Иначе
	...
	Если РезультатВыполнения.ДанныеXML <> Неопределено Тогда
		НоваяСтрока.ДанныеXML = Новый ХранилищеЗначения(РезультатВыполнения.ДанныеXML);
	КонецЕсли;
