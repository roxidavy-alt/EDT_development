...
Процедура ДобавитьЭтап_ФормированиеДвиженийПоОбесценениюВНА(ТаблицаЭтапов, ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов,
		...
		НоваяСтрока.МультиязычноеНаименование = "ru = 'Обесценение ВНА';
												|en = 'Fixed assets impairment'"; // @НСтр-1
...
Процедура Выполнить_ФормированиеДвиженийПоОбесценениюВНА(ПараметрыОбработчика) Экспорт
	...
	Пока НачалоПериода <= ПараметрыРасчета.КонецПериода Цикл
		...
		ОписаниеЗамера = ОценкаПроизводительности.НачатьЗамерДлительнойОперации("ЗакрытиеМесяца." 
			+ Строка(ПараметрыОбработчика.ДанныеЭтапа.Код));
...
#Область ОбесценениеВОрганизацияхСФилиалами

// Получить организации для обесценения.
// 
// Параметры:
//  Организация - СправочникСсылка.Организации - Организация, для которой требуется получить данные.
// 
// Возвращаемое значение:
//  Массив из СправочникСсылка.Организации -  Для филиала возвращает массив из 1 элемента - переданного филиала,
// 											  для головной оргнаизации возвращает массив из головной организации 
//											  и всех её филиалов.
Функция ПолучитьОрганизацииДляОбесценения(Организация) Экспорт
...
Функция ТекстЗапросаСтоимостьОС(Запрос)
	...
	|	ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.ОбесценениеВНА) КАК Операция,
...
Процедура ДополнитьПараметрыОтложенногоФормированияДвижений(СписокЗапросов, ИспользуемыеВТ) Экспорт
	...
	|		ИЗ
	|			ВтДанныеКРасчету КАК ДанныеКРасчету)
	|	И НЕ ОбесценениеВНА.НастройкаХозяйственнойОперации В (
	|			ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.КорректировкаОбесцененияОС),
	|			ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.КорректировкаОбесцененияНМА)
	|			)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОбъектУчета,
	...
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|// Движения, сделанные регламентным документом амортизации. Могут быть как до, так и после документа, меняющего суммы обесценения.
	|ВЫБРАТЬ
	|	ОбесценениеВНА.Период КАК Период,
	|	ОбесценениеВНА.Регистратор КАК Регистратор,
	|	ОбесценениеВНА.Организация КАК Организация,
	...
	|	КОНЕЦ КАК УбытокОтОбесцененияУпр
	|
	|ПОМЕСТИТЬ ОбесценениеВНА_ДвиженияАмортизации
	|
	|ИЗ
	|	РегистрНакопления.ОбесценениеВНА КАК ОбесценениеВНА
	|
	...
	|				И ОбесценениеВНА.Период <= ДанныеКРасчету.Дата
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|		ВтДанныеКРасчету КАК ДанныеКРасчету
	|
	|			ЛЕВОЕ СОЕДИНЕНИЕ ОбесценениеВНА_ДвиженияАмортизации КАК ОбесценениеВНА
	|			ПО ОбесценениеВНА.ОбъектУчета = ДанныеКРасчету.ОбъектУчета
	|				И ОбесценениеВНА.Организация = ДанныеКРасчету.Организация
	|				И ОбесценениеВНА.Регистратор <> ДанныеКРасчету.Ссылка
	|
...
Процедура ВыполнитьРеклассификациюОбесценения(ДанныеКРасчетуЗаМесяц)
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|// Список внеоборотных активов, по которым имеет смысл вычислять обесценение.
	|ВЫБРАТЬ
	...
	Пока Выборка.Следующий() Цикл
		...
		//++ Локализация	
		Если ТипЗнч(Выборка.Ссылка) = Тип("ДокументСсылка.ОбесценениеВНА") Тогда
			ОбесценениеВНАЛокализация.ДобавитьЗаданияНаДоначислениеНалогаНаИмуществоПослеОбесценения(Выборка.Ссылка);
		КонецЕсли;
		//-- Локализация
...
Процедура ИнициализироватьВтОстаткиДляРеклассификации(Запрос, ТекстыЗапроса)
	...
	Если Запрос.Параметры.Ссылка.Метаданные().Реквизиты.Найти("Организация") <> Неопределено Тогда
		 ОрганизацииДокумента.Добавить(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Запрос.Параметры.Ссылка, "Организация"));
	КонецЕсли;
	Если Запрос.Параметры.Ссылка.Метаданные().Реквизиты.Найти("ОрганизацияПолучатель") <> Неопределено Тогда
		 ОрганизацииДокумента.Добавить(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Запрос.Параметры.Ссылка, "ОрганизацияПолучатель"));
	 КонецЕсли;
	 
	ЗапросОстаткиОбесценения.УстановитьПараметр("ОрганизацииДокумента", ОрганизацииДокумента);
	...
	|	И ОбесценениеВНА.Организация В (&ОрганизацииДокумента)
	...
	|			&ГраницаДокумента,
	|			Организация В (&ОрганизацииДокумента) 
	|			И
	|			ВнеоборотныйАктив В
	|				(ВЫБРАТЬ
	|					Т.ВнеоборотныйАктив
...
Процедура ИнициализироватьВтНачисленияВосстановления(Запрос, ТекстыЗапроса)
	...
	"// Головная организация со всеми её филиалами или только филиал.
	|// В составе активов, входящих в ЕГДС головной организации, могут быть 
	|// активы принадлежащие филиалам.
	|// В ЕГДС филиалов -  активы только филиалов.
	|ВЫБРАТЬ
	|	Организации.Ссылка КАК Организация
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|// Возмещаемая стоимость по документу Обесценение ВНА.
	|ВЫБРАТЬ
	...
	|
	|ПОМЕСТИТЬ втВозмещаемаяСтоимостьПоДокументуОбесценения
	|
	|ИЗ
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|// Список активов, входящих в ЕГДС, указанных в документе Обесценение ВНА.
	|// По списку организаций, на момент времени перед документом Обесценение ВНА.
	|ВЫБРАТЬ
	|	СоставЕГДССрезПоследних.ВнеоборотныйАктив КАК ВнеоборотныйАктив,
	|	СоставЕГДССрезПоследних.Организация КАК Организация,
	|	СоставЕГДССрезПоследних.ЕГДС КАК ЕГДС
	|ПОМЕСТИТЬ СписокАктивовЕГДС
	|ИЗ
	|	РегистрСведений.СоставЕГДС.СрезПоследних(&ГраницаДокумента, ) КАК СоставЕГДССрезПоследних
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СписокОрганизаций КАК СписокОрганизаций
	|		ПО (СписокОрганизаций.Организация = СоставЕГДССрезПоследних.Организация)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОбесценениеВНА.ЕдиницыГенерирующиеДенежныеСредства КАК СписокЕГДСДокумента
	|		ПО (СписокЕГДСДокумента.ЕГДС = СоставЕГДССрезПоследних.ЕГДС
	|			И СписокЕГДСДокумента.Ссылка = &Ссылка)
	|	ГДЕ
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|// Возмещаемая стоимость по активам, входящим в ЕГДС вычисляется по ЕГДС и списку организаций.
	|// Для самостоятельных активов, не включенных в ЕГДС - по указанной организации.
	|ВЫБРАТЬ
	|	СписокАктивовЕГДС.ВнеоборотныйАктив                КАК ВнеоборотныйАктив,
	|	втВозмещаемаяСтоимостьПоДокументуОбесценения.ВнеоборотныйАктив    КАК ЕГДС,
	|	СписокАктивовЕГДС.Организация
	|	
	|ПОМЕСТИТЬ ОбесцениваемыеВНА_Предварительная
	|
	|ИЗ
	|	втВозмещаемаяСтоимостьПоДокументуОбесценения КАК втВозмещаемаяСтоимостьПоДокументуОбесценения
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СписокАктивовЕГДС КАК СписокАктивовЕГДС
	|		ПО (СписокАктивовЕГДС.ЕГДС = втВозмещаемаяСтоимостьПоДокументуОбесценения.ВнеоборотныйАктив)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ИЗ
	|	втВозмещаемаяСтоимостьПоДокументуОбесценения КАК втВозмещаемаяСтоимость
	|ГДЕ
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|// Узлы и компоненты обесцениваемых внеоборотных активов.
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	УзлыКомпоненты.УзелКомпонент КАК УзелКомпонент,
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|// Все объекты ВНА с их узлами и компонентами для расчета обесценения.
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ОбесцениваемыеВНА.ВнеоборотныйАктив КАК ВнеоборотныйАктив,
	|	ОбесцениваемыеВНА.ЕГДС              КАК ЕГДС,
	|	ОбесцениваемыеВНА.Организация
	|	
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|// Расчет балансовой стоимости как Стоимость - Амортизация - Ранее начисленное обесценение (с учетом движений текущего документа Обесценения ВНА).
	|ВЫБРАТЬ
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|// Балансовая и возмещаемая стоимость Объектов ВНА, не входящих в ЕГДС.
	|ВЫБРАТЬ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втВозмещаемаяСтоимостьПоДокументуОбесценения КАК втВозмещаемаяСтоимость
	|		ПО (втВозмещаемаяСтоимость.ВнеоборотныйАктив = втБалансоваяСтоимость.ВнеоборотныйАктив)
	|		ЛЕВОЕ СОЕДИНЕНИЕ СписокАктивовЕГДС КАК СписокАктивовЕГДС
	|		ПО СписокАктивовЕГДС.ВнеоборотныйАктив = втБалансоваяСтоимость.ВнеоборотныйАктив
	|ГДЕ
	|	СписокАктивовЕГДС.ВнеоборотныйАктив ЕСТЬ NULL
	|;//7
	|
	|////////////////////////////////////////////////////////////////////////////////
	|// Ликвидационне обязательства узлов и компонентов амортизации с отрицательной стоимостью.
	|ВЫБРАТЬ
	|	втБалансоваяСтоимость.Организация               КАК Организация,
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|// Вт Балансовая стоимость ВНА, входящих в ЕГДС, с учетом ликвидационных обязательств.
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|//Таблица для расчета обесценения по ВНА, входящим в ЕГДС, с учетом ликвидационных обязательств.
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	втБалансоваяСтоимость.Организация               КАК Организация,
	|	втБалансоваяСтоимость.ВнеоборотныйАктив         КАК ВнеоборотныйАктив,
	|	втБалансоваяСтоимость.Подразделение             КАК Подразделение,
	...
	|
	|ВЫБРАТЬ
	|	ДанныеПоЕГДС.Организация КАК Организация,
	|	ДанныеПоЕГДС.ЕГДС        КАК ЕГДС,
	|	ДанныеПоЕГДС.ВозмещаемаяСтоимостьРегл КАК ВозмещаемаяСтоимостьРегл,
	|	ДанныеПоЕГДС.ВозмещаемаяСтоимостьУпр  КАК ВозмещаемаяСтоимостьУпр,
	|	СУММА(ДанныеПоЕГДС.СтоимостьРегл)     КАК СтоимостьРегл,
	|	СУММА(ДанныеПоЕГДС.СтоимостьУпр)     КАК СтоимостьУпр
	|ИЗ
	|	ДанныеПоЕГДС КАК ДанныеПоЕГДС
	|СГРУППИРОВАТЬ ПО
	|	ДанныеПоЕГДС.Организация,
	|	ДанныеПоЕГДС.ЕГДС,
	|	ДанныеПоЕГДС.ВозмещаемаяСтоимостьРегл,
	|	ДанныеПоЕГДС.ВозмещаемаяСтоимостьУпр
	|;//12
	|
	|ВЫБРАТЬ
	|	ДанныеПоЕГДС.Организация КАК Организация,
	|	ДанныеПоЕГДС.ВнеоборотныйАктив КАК ВнеоборотныйАктив,
	|	ДанныеПоЕГДС.Подразделение КАК Подразделение,
	|	ДанныеПоЕГДС.ГруппаФинансовогоУчета КАК ГруппаФинансовогоУчета,
	|	ДанныеПоЕГДС.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ДанныеПоЕГДС.ВозмещаемаяСтоимостьРегл КАК ВозмещаемаяСтоимостьРегл,
	|	ДанныеПоЕГДС.ВозмещаемаяСтоимостьУпр КАК ВозмещаемаяСтоимостьУпр,
	|	ДанныеПоЕГДС.СтоимостьРегл КАК СтоимостьРегл,
	|	ДанныеПоЕГДС.СтоимостьУпр КАК СтоимостьУпр,
	|	ДанныеПоЕГДС.БазаРаспределенияРегл КАК БазаРаспределенияРегл,
	|	ДанныеПоЕГДС.БазаРаспределенияУпр КАК БазаРаспределенияУпр,
	|	ДанныеПоЕГДС.ОбесценениеРегл КАК ОбесценениеРегл,
	|	ДанныеПоЕГДС.ОбесценениеУпр КАК ОбесценениеУпр,
	|	ДанныеПоЕГДС.РезервПереоценкиСтоимостиУпр         КАК РезервПереоценкиСтоимостиУпр,
	|	ДанныеПоЕГДС.РезервПереоценкиСтоимостиРегл        КАК РезервПереоценкиСтоимостиРегл,
	|	ДанныеПоЕГДС.РезервПереоценкиАмортизацииУпр       КАК РезервПереоценкиАмортизацииУпр,
	|	ДанныеПоЕГДС.РезервПереоценкиАмортизацииРегл      КАК РезервПереоценкиАмортизацииРегл,
	|	ДанныеПоЕГДС.УбытокОтОбесцененияУпр    КАК УбытокОтОбесцененияУпр,
	|	ДанныеПоЕГДС.УбытокОтОбесцененияРегл   КАК УбытокОтОбесцененияРегл,
	|	ДанныеПоЕГДС.ЕГДС КАК ЕГДС
	|ИЗ
	|	ДанныеПоЕГДС КАК ДанныеПоЕГДС
	|//13
	...
	// ВНА, входящие в ЕГДС.
	ДанныеПоЕГДС = Результат[13].Выгрузить(); // ТаблицаЗначений - 
	
	// По организациям с филиалами требуется распределить общую сумму обесценения между филиалами, 
	// и уже в рамках филиалов распределить обесценение по активам.
	
	ДанныеПоФилиалам = Результат[12].Выгрузить(); // ТаблицаЗначений -
	СписокЕГДС = ДанныеПоФилиалам.Скопировать(,"ЕГДС");
	СписокЕГДС.Свернуть("ЕГДС","");
	
	ТаблицаЕГДС = ДанныеПоЕГДС.Скопировать(, "Организация, ЕГДС, ВозмещаемаяСтоимостьРегл, ВозмещаемаяСтоимостьУпр, СтоимостьРегл, СтоимостьУпр");
	ТаблицаЕГДС.Очистить();
	
	Для Каждого СтрокаСпискаЕГДС Из СписокЕГДС Цикл
		
		КРаспределениюПоЕГДС = ДанныеПоФилиалам.Скопировать(Новый Структура("ЕГДС", СтрокаСпискаЕГДС.ЕГДС));
		
		РаспределеннаяВозмещаяемаяСтоимостьРегл = ОбщегоНазначенияУТКлиентСервер.РаспределитьПропорционально(
													КРаспределениюПоЕГДС.Получить(0).ВозмещаемаяСтоимостьРегл,
													КРаспределениюПоЕГДС.ВыгрузитьКолонку("СтоимостьРегл"));
													
		РаспределеннаяВозмещаяемаяСтоимостьУпр = ОбщегоНазначенияУТКлиентСервер.РаспределитьПропорционально(
													КРаспределениюПоЕГДС.Получить(0).ВозмещаемаяСтоимостьУпр,
													КРаспределениюПоЕГДС.ВыгрузитьКолонку("СтоимостьУпр"));
		
		Если РаспределеннаяВозмещаяемаяСтоимостьРегл <> Неопределено Тогда 
			КРаспределениюПоЕГДС.ЗагрузитьКолонку(РаспределеннаяВозмещаяемаяСтоимостьРегл, "ВозмещаемаяСтоимостьРегл");
		КонецЕсли;
		Если РаспределеннаяВозмещаяемаяСтоимостьУпр <> Неопределено Тогда
			КРаспределениюПоЕГДС.ЗагрузитьКолонку(РаспределеннаяВозмещаяемаяСтоимостьУпр, "ВозмещаемаяСтоимостьУпр");
		КонецЕсли;
		
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(КРаспределениюПоЕГДС, ТаблицаЕГДС);
	КонецЦикла;
	
	Для Каждого СтрокаЕГДС Из ТаблицаЕГДС Цикл
		
		СтруктураОтбора = Новый Структура("ЕГДС, Организация", СтрокаЕГДС.ЕГДС, СтрокаЕГДС.Организация);
		СтрокиВНА = ДанныеПоЕГДС.НайтиСтроки(СтруктураОтбора);
		ТаблицаОбесцененияЕГДС = ДанныеПоЕГДС.Скопировать(СтрокиВНА,); // по организации.
...
Процедура ТекстЗапросаПрочиеРасходы(Запрос, ТекстыЗапроса)
	...
	|	ПрочиеРасходы.СчетКт КАК СчетКт,
	|	ПрочиеРасходы.СуммаПрочихАктивов КАК СуммаПрочихАктивов,
	|	ПрочиеРасходы.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
	|	ПрочиеРасходы.ДокументДвижения КАК ДокументДвижения,
	|	ПрочиеРасходы.КорСтатьяРасходов КАК КорСтатьяРасходов,
	|	ПрочиеРасходы.КорОрганизация КАК КорОрганизация,
	|	ПрочиеРасходы.ДоляСтоимости КАК ДоляСтоимости,
	|	ПрочиеРасходы.КорПодразделение КАК КорПодразделение,
	|	ПрочиеРасходы.КорАналитикаРасходов КАК КорАналитикаРасходов,
	|	ПрочиеРасходы.ГруппаПродукции КАК ГруппаПродукции,
	|	ПрочиеРасходы.КорСтатьяКалькуляции КАК КорСтатьяКалькуляции,
	|	ПрочиеРасходы.РасчетСебестоимости КАК РасчетСебестоимости,
	|	ПрочиеРасходы.РасчетПартий КАК РасчетПартий,
	|	ПрочиеРасходы.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ПрочиеРасходы.КорНаправлениеДеятельности КАК КорНаправлениеДеятельности,
	|	ПрочиеРасходы.КорОбъектУчетаРезервов КАК КорОбъектУчетаРезервов,
	|	ПрочиеРасходы.КорВидРезервов КАК КорВидРезервов,
	|	ПрочиеРасходы.ВидИсточника КАК ВидИсточника,
	|	ПрочиеРасходы.Сторно КАК Сторно,
	|	ПрочиеРасходы.ДокументРасчета КАК ДокументРасчета,
	|	ПрочиеРасходы.РежимЗакрытияМесяца КАК РежимЗакрытияМесяца,
	|	ПрочиеРасходы.ПериодОтклонения КАК ПериодОтклонения
	|ИЗ
	|	РегистрНакопления.ПрочиеРасходы КАК ПрочиеРасходы
	|ГДЕ
	|	ПрочиеРасходы.Регистратор = &Ссылка
	|	И НЕ ПрочиеРасходы.НастройкаХозяйственнойОперации В (ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ОбесценениеОС), 
...
Процедура ТекстЗапросаДвиженияДоходыРасходыПрочиеАктивыПассивы(Запрос, ТекстыЗапроса)
	...
	|	ВЫБОР
	|		КОГДА втОстаткиОбесценения.ВнеоборотныйАктив ССЫЛКА Справочник.ОбъектыЭксплуатации
	|			ТОГДА ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансОсновныеСредства)
	|		ИНАЧЕ ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансНематериальныеАктивы)
	|	КОНЕЦ,
	|	НЕОПРЕДЕЛЕНО,
	|	НЕОПРЕДЕЛЕНО,
	|	втОстаткиОбесценения.ВнеоборотныйАктив,
