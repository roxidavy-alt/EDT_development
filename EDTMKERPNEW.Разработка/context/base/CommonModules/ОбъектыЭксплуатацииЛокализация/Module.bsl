...
Функция ПервоначальныеСведения(ОсновноеСредствоИлиСписок, Период) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ОсновноеСредствоИлиСписок)
...
Функция ПечатьОС6_2003(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Пока ВыборкаОбъектов.Следующий() Цикл
		...
		|		ПО МодернизацияОС.Ссылка = СобытияОСОрганизаций.Регистратор
		|
		|ГДЕ
		...
		|			И МодернизацияОС.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.МодернизацияОС)
...
Функция ТекстЗапросаТекущиеСведения(ДатаСведений)
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПоследних(&ДатаСведений,
	|			ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.СтоимостьВНА) И МестоУчета = НЕОПРЕДЕЛЕНО
	|				И Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)) КАК СчетаУчетаСтоимости
	|		ПО СчетаУчетаСтоимости.АналитикаУчета = ПорядокУчетаОС.ГруппаФинансовогоУчета
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПоследних(&ДатаСведений,
	|			ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.АмортизацияВНА) И МестоУчета = НЕОПРЕДЕЛЕНО
	|				И Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)) КАК СчетаУчетаАмортизации
	|		ПО СчетаУчетаАмортизации.АналитикаУчета = ПорядокУчетаОС.ГруппаФинансовогоУчета
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПоследних(&ДатаСведений,
	|			ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.ЗабалансовыйУчетВНА) И МестоУчета = НЕОПРЕДЕЛЕНО
	|				И Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)) КАК СчетаЗабалансовогоУчета
	|		ПО СчетаУчетаАмортизации.АналитикаУчета = ПорядокУчетаОС.ГруппаФинансовогоУчета
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Функция ВыборкаПриемкаПеремещениеВыбытие(ДатаСведений, ВыборкаОбъектов)
	...
	Иначе	
		...
		|	МАКСИМУМ(СчетаУчетаСтоимости.Период) КАК ПоследнийПериодСтоимости,
		|	МАКСИМУМ(СчетаУчетаАмортизации.Период) КАК ПоследнийПериодАмортизации
		|ПОМЕСТИТЬ ПорядокОтраженияСрезПоследнихПериодов
		|ИЗ
		...
		|			И ПорядокУчетаОС.Период >= СчетаУчетаСтоимости.Период
		...
		|			И ПорядокУчетаОС.Период >= СчетаУчетаАмортизации.Период
		|ГДЕ
		...
		|ИЗ
		|	ПорядокОтраженияСрезПоследнихПериодов КАК ПорядокУчетаОС
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета КАК СчетаУчетаСтоимости
		|		ПО СчетаУчетаСтоимости.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.СтоимостьВНА)
		|			И СчетаУчетаСтоимости.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
		|			И СчетаУчетаСтоимости.АналитикаУчета = ПорядокУчетаОС.ГруппаФинансовогоУчета
