		...
		Если Не ЗначениеЗаполнено(ТекущееОписание.КлючВидовПродукции) Тогда
			ВызватьИсключение СтрШаблон(
				НСтр("ru = 'Не удалось определить ключ видов продукции для вида номенклатуры ""%1"" при переходе на конструктор настроек видов продукции';
					|en = 'Не удалось определить ключ видов продукции для вида номенклатуры ""%1"" при переходе на конструктор настроек видов продукции'"),
				СтрокаТаблицы.Ссылка);
		КонецЕсли;
...
Процедура ПереопределитьТекстЗапросаДинамическогоСпискаНоменклатурыДляОтбораПоВидуПродукции(Форма, ВидыПродукцииИС) Экспорт
	...
	Форма.СписокНоменклатура.ТекстЗапроса = Форма.СписокНоменклатура.ТекстЗапроса
		...
		Если ПоискПоСсылкам.Количество() = 0 Тогда
			...
			Исключение
				...
				НСтр("ru = 'При пометке на удаление дубля ключа видов продукции ИС %1 возникла ошибка:
				           |Текст ошибки: %2';
				           |en = 'При пометке на удаление дубля ключа видов продукции ИС %1 возникла ошибка:
				           |Текст ошибки: %2'"),
				СправочникОбъект,
				...
				ОбщегоНазначенияИСВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
					...
					НСтр("ru = 'Обмен данными';
						|en = 'Обмен данными'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПриЗаписиНаСервереШтрихкодаНоменклатуры(Форма, Отказ, ТекущийОбъект, ПараметрыЗаписи) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Ошибка записи описания GTIN';
				|en = 'Ошибка записи описания GTIN'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		ОбщегоНазначения.СообщитьПользователю(
			СтрШаблон(
				НСтр("ru = 'Ошибка записи описания GTIN по причине: %1';
					|en = 'Ошибка записи описания GTIN по причине: %1'"),
...
// Текст запроса движение продукции и материалов.
// 
// Возвращаемое значение:
//  Строка - Текст запроса движение продукции и материалов
...
// Текст запроса производство без заказа.
// 
// Возвращаемое значение:
...
// Текст запроса этап производства2 2.
// 
// Возвращаемое значение:
//  Строка - Текст запроса этап производства2 2
...
Процедура ДобавитьКомандыНастройкиИнтеграцииСФГИСВФормуСпискаНоменклатуры(Форма) Экспорт
	...
	КомандаФормы.Заголовок = НСтр("ru = 'Настроить интеграцию с ФГИС';
									|en = 'Настроить интеграцию с ФГИС'");
...
Процедура ЗаписатьДанныеОписанияНоменклатурыПриПереходеНаКонструкторНастроекВидовПродукции(НаборЗаписей)
	...
	Исключение
		...
		ШаблонСообщения = НСтр("ru = 'Не удалось обработать номенклатуру при переходе на конструктор настроек видов продукции по причине:
							         |%1';
							         |en = 'Не удалось обработать номенклатуру при переходе на конструктор настроек видов продукции по причине:
							         |%1'");
