...
Функция ДанныеПечатнойФормыНМА1(МассивОбъектов, ДатаСведений)
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПоследних(&ДатаСведений, ВидСчета В (&ВидыСчетовВнеоборотныхАктивов)
	|		И Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка) И МестоУчета = НЕОПРЕДЕЛЕНО) КАК НастройкиПоАналитике
	|		ПО НастройкиПоАналитике.АналитикаУчета = ПорядокУчета.ГруппаФинансовогоУчета
	|
	|СГРУППИРОВАТЬ ПО
