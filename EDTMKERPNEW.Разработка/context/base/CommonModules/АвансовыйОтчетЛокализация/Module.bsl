...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	//++ НЕ УТ
	Если Не ЕдиныйАвансовыйОтчетБезусловно И Не ЕстьПечатьАвансовогоОтчетаСтаройВерсии
		И МассивОбъектов.Количество() = 1 И ТипЗнч(МассивОбъектов[0]) = Тип("ДокументСсылка.ПоступлениеДенежныхДокументов") Тогда
			
			ШаблонОшибки = НСтр("ru = 'Дата документа %1 больше даты начала печати единого АО %2.';
								|en = 'The date of the %1 document is greater than the start date of printing the %2 single expense report.'");
			ДатаДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(МассивОбъектов[0], "Дата");
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон(ШаблонОшибки, Формат(ДатаДокумента, "ДЛФ=Д"), Формат(ДатаНачалаПечатиЕдиногоАвансовогоОтчета, "ДЛФ=Д")));
...
#Область ТекстОплатаПоставщикам // (Дт 60.01 :: Кт 71)
	...
	|	""АвансовыйОтчетЛокализация_1"" КАК НомерФрагмента
	|
	|ИЗ
...
#Область ТекстПредоплатаПоставщикам // (Дт 60.02 :: Кт 71)
	...
	|	""АвансовыйОтчетЛокализация_2"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстПрочиеРасходы // (Дт 44 :: Кт 71)
	...
	|	""АвансовыйОтчетЛокализация_1670"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстНаПрочиеАктивы // (Дт 44 :: Кт 71)
	...
	|		ВЫРАЗИТЬ(Строки.Содержание КАК Строка(150))
	|	КОНЕЦ КАК Содержание,
	|	""АвансовыйОтчетЛокализация_1671"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|	""АвансовыйОтчетЛокализация_3"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстПоступлениеУслугНДС // (Дт <19.ХХ> :: Кт <71>)
	...
	|	""АвансовыйОтчетЛокализация_4"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстКонвертацияВалюты // (Дт 57:: Кт 71)
	...
	|	""АвансовыйОтчетЛокализация_5"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстПоступлениеДСотКонвертацииВалюты // (Дт 71:: Кт 57)
	...
	|	""АвансовыйОтчетЛокализация_6"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстКонвертацияВалютыПрочиеДоходы // (Дт 57:: Кт 91.1)
	...
	|	""АвансовыйОтчетЛокализация_7"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстКонвертацияВалютыПрочиеРасходы // (Дт 91.2 :: Кт 57)
	...
	|	""АвансовыйОтчетЛокализация_8"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
...
Функция ТекстЗапросаТаблицаВтТаблицаОбъектовРасчетов(ТекстыЗапроса)
	
	ИмяТаблицы = "ВтТаблицаОбъектовРасчетов";
...
Функция ТекстЗапросаТаблицаОтражениеДокументовВРеглУчете(Запрос, ТекстыЗапроса, Регистры)
	...
	Если Не ПроведениеДокументов.ЕстьТаблицаЗапроса("ВтТаблицаОбъектовРасчетов", ТекстыЗапроса) Тогда
		ТекстЗапросаТаблицаВтТаблицаОбъектовРасчетов(ТекстыЗапроса);
	КонецЕсли;
	...
	|	ОБЪЕДИНИТЬ ВСЕ 
	|"
		+ РеглУчетПроведениеСервер.ТекстЗапросаОтраженияПоОбъектамРасчетов("ВтТаблицаОбъектовРасчетов");
...
Функция ТекстЗапросаПечати(Запрос, ИспользуетсяРеглУчет)
	...
	|	МИНИМУМ(Проводки.Период) КАК Период,
...
Функция ПостфиксНомераДокумента(Ссылка)
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ПриобретениеУслугПрочихАктивов") Тогда
		Возврат НСтр("ru = 'П';
					|en = 'P'", Метаданные.Языки.Русский.КодЯзыка);
	Иначе
		Возврат "";
	КонецЕсли;
