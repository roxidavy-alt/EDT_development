...
Процедура ОбработкаЗаполнения(Объект, ДанныеЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ПоступлениеБезналичныхДенежныхСредств") Тогда
		ВыборИдентификаторГосКонтракта = "ВЫБОР КОГДА Реквизиты.ДокументОснование.БанковскийСчет.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыБанковскихСчетов.Расчетный)
										|		ТОГДА Реквизиты.ДокументОснование.БанковскийСчет.ГосударственныйКонтракт.НомерГОЗ
										|		КОГДА Реквизиты.ДокументОснование.БанковскийСчет.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыБанковскихСчетов.Казначейский)
										|		ТОГДА Реквизиты.ДокументОснование.БанковскийСчет.НаправлениеДеятельности.ГосударственныйКонтракт.НомерИГК
										|	ИНАЧЕ """"
										|КОНЕЦ ";
	КонецЕсли;
	//-- НЕ УТ
...
Процедура СформироватьТекстОтраженияВРеглУчете(ТекстЗапроса) Экспорт
	...
	|	""СчетФактураВыданныйАвансЛокализация_1304"" КАК НомерФрагмента
	|ИЗ
	...
	|	""СчетФактураВыданныйАвансЛокализация_1305"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|	""СчетФактураВыданныйАвансЛокализация_1306"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ПолучитьДанныеДляПечатнойФормыСчетФактура(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	0 КАК Акциз,
	|	0 КАК АкцизПослеИзменения,
	|	0 КАК АкцизДоИзменения,
