...
// Текст запроса раздел 4 декларации по НДС.
// 
// Параметры:
//  ПараметрыОтчета - Структура
// 
// Возвращаемое значение:
//  Строка - Текст запроса раздел 4 декларации по НДС
...
// Текст запроса раздел 5 декларации по НДС.
// 
// Параметры:
//  ПараметрыОтчета - Структура
// 
// Возвращаемое значение:
//  Строка - Текст запроса раздел 5 декларации по НДС
Функция ТекстЗапросаРаздел5(ПараметрыОтчета) Экспорт
	...
	|	НДССостояниеРеализации0.ДатаПодтверждения КАК ДатаНеподтвержденияНулевойСтавки
	...
	|	ПО
	|		НДСЗаписиКнигиПокупок.Организация = НДССостояниеРеализации0.Организация
	|		И НДСЗаписиКнигиПокупок.ДокументОтгрузки = НДССостояниеРеализации0.ДокументРеализации
	...
	|ПОМЕСТИТЬ ВычетыПредварительные
	|ИЗ
	|	РегистрНакопления.НДСЗаписиКнигиПокупок КАК НДСЗаписиКнигиПокупок
	...
	|	НДСЗаписиКнигиПокупок.НДС,
	|	ЛОЖЬ,
	|	НАЧАЛОПЕРИОДА(НДССостояниеРеализации0.ДатаПодтверждения, КВАРТАЛ),
	...
	|	И НАЧАЛОПЕРИОДА(НДССостояниеРеализации0.ДатаПодтверждения, КВАРТАЛ) >= ДАТАВРЕМЯ(2024, 1, 1)
	|	И НДСЗаписиКнигиПокупок.Организация В(&Организация)
	|	И НЕ НДСЗаписиКнигиПокупок.ЗаписьДополнительногоЛиста
	|	И НДСЗаписиКнигиПокупок.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И НДСЗаписиКнигиПокупок.Событие = &СобытиеВычетНеПодтвержденаСтавка0
	...
	|	ВЫБОР
	|		КОГДА ВычетыНеподтвержденныеРанее.ДатаНеподтвержденияНулевойСтавки >= ДАТАВРЕМЯ(2024, 1, 1)
	|			ТОГДА КОНЕЦПЕРИОДА(ВычетыНеподтвержденныеРанее.ДатаНеподтвержденияНулевойСтавки, КВАРТАЛ)
	|		ИНАЧЕ КОНЕЦПЕРИОДА(ЕСТЬNULL(ДанныеПервичныхДокументов.Дата, ДАТАВРЕМЯ(2024, 1, 1)), КВАРТАЛ)
	|	КОНЕЦ
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВычетыПредварительные.КодОперации КАК КодОперации,
	|	ВычетыПредварительные.НДС КАК НДС,
	|	ВычетыПредварительные.ПодтвержденаСтавка0 КАК ПодтвержденаСтавка0,
	|	ВычетыПредварительные.НачалоКвартала КАК НачалоКвартала,
	|	ВычетыПредварительные.КонецКвартала КАК КонецКвартала
	|ПОМЕСТИТЬ Вычеты
	|ИЗ ВычетыПредварительные
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВычетыПредварительные
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|		РегистрСведений.НДССостояниеРеализации0.СрезПоследних(&КонецПериода) КАК НДССостояниеРеализации0
	|	ПО
	|		НДСЗаписиКнигиПродаж.Организация = НДССостояниеРеализации0.Организация
	|		И НДСЗаписиКнигиПродаж.СчетФактура = НДССостояниеРеализации0.ДокументРеализации
	|		И НДССостояниеРеализации0.Состояние = ЗНАЧЕНИЕ(Перечисление.НДССостоянияРеализация0.НеПодтвержденаРеализация0)
	|ГДЕ
	|	НДСЗаписиКнигиПродаж.Организация В(&Организация)
	|	И Периоды.ПодтвержденаСтавка0 = ЛОЖЬ
	|	И НДСЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.СчетФактураНаНеподтвержденнуюРеализацию0
	|	И НДСЗаписиКнигиПродаж.Период МЕЖДУ Периоды.НачалоКвартала И &КонецПериодаУчитываемыхКорректировок
...
// Текст запроса раздел 6 декларации по НДС.
// 
// Параметры:
//  ПараметрыОтчета - Структура
// 
// Возвращаемое значение:
//  Строка - Текст запроса раздел 6 декларации по НДС
...
// Параметры:
//  Запрос -  Запрос.
//  ПараметрыОтчета - см. РегламентированнаяОтчетностьПереопределяемый.ПолучитьСведенияОПоказателяхОтчета
Процедура ТекстЗапросаРеестрНДСПриложение(Запрос, ПараметрыОтчета) Экспорт
	...
	// При выписке исправительного счета-фактуры в измерение СчетФактура регистра НДСЗаписиКнигиПродаж
	// записывается ссылка на ранее выданный счета-фактура на документ отгрузки, а не сам документ отгрузки.
	...
	|	И НЕ НДСЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.СчетФактураВыданный
	|
	|СГРУППИРОВАТЬ ПО
	...
	|
	|ВЫБРАТЬ
	|	ЕСТЬNULL(СчетФактураВыданный.ДокументОснование, НДСЗаписиКнигиПродаж.СчетФактура),
	|	СУММА(НДСЗаписиКнигиПродаж.СуммаБезНДС),
	|	НДСЗаписиКнигиПродаж.КодОперации0
	|ИЗ
	|	РегистрНакопления.НДСЗаписиКнигиПродаж КАК НДСЗаписиКнигиПродаж
	...
	|	ПодтвержденныеОтгрузки.КодОперации КАК КодОперации
	|ПОМЕСТИТЬ ПодтвержденныеОтгрузкиСуммаПоКодамОпераций
	|ИЗ
	|	ПодтвержденныеОтгрузки КАК ПодтвержденныеОтгрузки
	|
	|СГРУППИРОВАТЬ ПО
	|	ПодтвержденныеОтгрузки.ДокументОтгрузки,
	|	ПодтвержденныеОтгрузки.КодОперации
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПодтвержденныеОтгрузкиСуммаПоКодамОпераций.ДокументОтгрузки КАК ДокументОтгрузки,
	|	СведенияТаможенныхДекларацийЭкспорт.НомерТаможеннойДекларации КАК НомерТаможеннойДекларации,
	|	ПодтвержденныеОтгрузкиСуммаПоКодамОпераций.КодОперации КАК КодОперации
	|ПОМЕСТИТЬ ДокументыОтгрузкиИТД
	|ИЗ
	|	ПодтвержденныеОтгрузкиСуммаПоКодамОпераций КАК ПодтвержденныеОтгрузкиСуммаПоКодамОпераций
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СведенияТаможенныхДекларацийЭкспорт КАК СведенияТаможенныхДекларацийЭкспорт
	|		ПО ПодтвержденныеОтгрузкиСуммаПоКодамОпераций.ДокументОтгрузки = СведенияТаможенныхДекларацийЭкспорт.ДокументОтгрузки
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументОтгрузки";
	...
	Если ПараметрыОтчета.ВключатьСведенияОКонтракте Тогда
		...
		|		КОГДА ДокументыОтгрузкиИТД.ДокументОтгрузки ССЫЛКА Документ.ОтгрузкаТоваровКлиенту
		|			ТОГДА ВЫРАЗИТЬ(ДокументыОтгрузкиИТД.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровКлиенту).Договор
	...
	|	ДокументыОтгрузкиИТД.КодОперации КАК КодОперации,
	|	ДокументыОтгрузкиИТД.НомерТаможеннойДекларации КАК НомерТаможеннойДекларации,
	|	ПодтвержденныеОтгрузкиСуммаПоКодамОпераций.ПодтвержденнаяНалоговаяБаза КАК ПодтвержденнаяНалоговаяБаза
	|ИЗ
	|	ПодтвержденныеОтгрузкиСуммаПоКодамОпераций КАК ПодтвержденныеОтгрузкиСуммаПоКодамОпераций
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДокументыОтгрузкиИТД КАК ДокументыОтгрузкиИТД
	|		ПО ПодтвержденныеОтгрузкиСуммаПоКодамОпераций.ДокументОтгрузки = ДокументыОтгрузкиИТД.ДокументОтгрузки
	|			И ПодтвержденныеОтгрузкиСуммаПоКодамОпераций.КодОперации = ДокументыОтгрузкиИТД.КодОперации
	|ИТОГИ
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ПодтвержденныеОтгрузкиСуммаПоКодамОпераций
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
