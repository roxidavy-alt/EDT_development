...
Функция ИнтервалПланированияСтрокой(Период, ИнтервалПланирования) Экспорт
	...
	ИначеЕсли ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Неделя Тогда
		ПериодСтрока = Формат(Период, НСтр("ru = 'ДФ=''dd.MM.yy''';
											|en = 'DF=''MM.dd.yy'''")) + " - " + Формат(КонецНедели(Период), НСтр("ru = 'ДФ=''dd.MM.yy''';
																												|en = 'DF=''MM.dd.yy'''"));
...
#Область ПооперационноеУправление

// Устанавливает объектную блокировку документа Произвосдтвенная операция
// 
// Параметры:
//  ДокументОбъект - ДокументОбъект.ПроизводственнаяОперация2_2
Процедура ЗаблокироватьПроизводственнуюОперацию(ДокументОбъект) Экспорт
	
	ШаблонИсключения = НСтр("ru = 'Не удалось заблокировать производственную операцию %1 от %2 (%3). Возможно документ в настоящий момент редактируется.';
							|en = 'Cannot lock the %1 routing operations dated %2 (%3). The document is probably locked for editing.'");
	
	Попытка
		ДокументОбъект.Заблокировать();
	Исключение
		ТекстСообщения = СтрШаблон(ШаблонИсключения, ДокументОбъект.Номер, ДокументОбъект.Дата, ДокументОбъект.Наименование);
...
// Реквизиты рабочего центра.
// 
// Параметры:
//  РабочийЦентр - СправочникСсылка.РабочиеЦентры,СправочникСсылка.ВидыРабочихЦентров
// 
// Возвращаемое значение:
//  см. ОперативныйУчетПроизводстваКлиентСервер.РеквизитыРабочегоЦентраКонструктор
Функция РеквизитыРабочегоЦентра(РабочийЦентр) Экспорт
	
	Результат = ОперативныйУчетПроизводстваКлиентСервер.РеквизитыРабочегоЦентраКонструктор();
	
	Если ЗначениеЗаполнено(РабочийЦентр) Тогда
	
		Реквизиты = Новый Структура;
		Если ТипЗнч(РабочийЦентр) = Тип("СправочникСсылка.РабочиеЦентры") Тогда
			Реквизиты.Вставить("РабочийЦентр", "Ссылка");
			Реквизиты.Вставить("ВидРабочегоЦентра", "ВидРабочегоЦентра");
			Реквизиты.Вставить("ИспользоватьВариантыНаладки", "ВидРабочегоЦентра.ИспользуютсяВариантыНаладки");
			Реквизиты.Вставить("КоэффициентВремениРаботы", "КоэффициентВремениРаботы");
			Реквизиты.Вставить("ПараллельнаяЗагрузка", "ВидРабочегоЦентра.ПараллельнаяЗагрузка");
			Реквизиты.Вставить("ЕдиницаИзмеренияЗагрузки", "ВидРабочегоЦентра.ЕдиницаИзмеренияЗагрузки");
		Иначе
			Реквизиты.Вставить("ВидРабочегоЦентра", "Ссылка");
			Реквизиты.Вставить("ИспользоватьВариантыНаладки", "ИспользуютсяВариантыНаладки");
		...
		КонецЕсли;
		ЗаполнитьЗначенияСвойств(Результат, ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РабочийЦентр, Реквизиты));
