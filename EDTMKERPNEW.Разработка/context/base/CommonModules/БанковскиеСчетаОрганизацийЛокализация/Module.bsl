...
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.НаправлениеДеятельности) Тогда
		...
		ГосударственныйКонтракт = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.НаправлениеДеятельности, "ГосударственныйКонтракт");
		
		Если Объект.СчетПоГосконтракту И Не ЗначениеЗаполнено(ГосударственныйКонтракт) Тогда
			ТекстСообщения = НСтр("ru = 'Направление деятельности должно быть связано с исполнением государственного контракта.';
									|en = 'Направление деятельности должно быть связано с исполнением государственного контракта.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Объект, "НаправлениеДеятельности", , Отказ);
		ИначеЕсли (Не Объект.СчетПоГосконтракту И Не Объект.ОтдельныйСчетГОЗ)
			И ЗначениеЗаполнено(ГосударственныйКонтракт) Тогда
			ТекстСообщения = НСтр("ru = 'Направление деятельности не должно быть связано с исполнением государственного контракта.';
									|en = 'Направление деятельности не должно быть связано с исполнением государственного контракта.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Объект, "НаправлениеДеятельности", , Отказ);
		КонецЕсли;
...
Функция ПечатьЗаявления(МассивОбъектов, ОбъектыПечати, ВариантЗаявления)
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(ГосударственныеКонтракты.Номер, """") КАК НомерКонтракта,
		|	ЕСТЬNULL(ГосударственныеКонтракты.ДатаЗаключения, """") КАК ДатаКонтракта,
