	...
	Если ОшибкаЧтенияКарточки Тогда
		...
		Если ТипЭлектронногоДокумента <> Неопределено И ДоступныеТипыЭД.Найти(ТипЭлектронногоДокумента) = Неопределено Тогда
			ШаблонСообщения =
				...
				|en = 'The current application version does not support this type of the ""%1"" electronic document.
		...
		Для Каждого Документ Из Документооборот.Documents.Document Цикл
			...
			Если Не ЗначениеЗаполнено(ДвоичныеДанныеФайлаЭД) И ЗначениеЗаполнено(ИмяОсновногоФайла) Тогда
				ВызватьИсключение СтрШаблон(НСтр("ru = 'Ошибка при разборе файла meta.xml транспортного контейнера ЭДО.
					...
					|en = 'An error occurred while parsing the meta.xml file of the EDI transport container.
					|The %1 file is not found in the transport container.'"), ИмяОсновногоФайла);
			КонецЕсли;
