...
Процедура ОбновитьПоддерживаемыеФорматыАбонентовИзСервисаНастроек(ДатаПоследнегоОбновления,
	...
	ВидОперации = НСтр("ru = 'Обновление данных абонентов ЭДО';
						|en = 'Обновление данных абонентов ЭДО'");
...
Процедура СинхронизироватьКлассификаторыОграниченийМЧД(ПараметрыОперации, АдресРезультата = Неопределено) Экспорт
	...
	Исключение
		...
		ИмяСобытия = НСтр("ru = 'Синхронизация классификатора ограничений МЧД из сервиса настроек';
							|en = 'Синхронизация классификатора ограничений МЧД из сервиса настроек'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка, , ,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
	...
	Для Каждого ИдентификаторЭДО Из ИдентификаторыЭДО Цикл
		...
		Если Токен = Неопределено Тогда
			ВидОперации = НСтр("ru = 'Подготовка запроса в сервис ЭДО';
								|en = 'Prepare a request to the EDI service'");
			ТекстОшибки = НСтр("ru = 'Отсутствует токен доступа для авторизации в сервисе ЭДО';
								|en = 'There is no access token for authorization in the EDI service'");
