...
Процедура ПриЗаписиОбъектаНастройкиСчетовУчета(Форма, Отказ, Объект, ПараметрыЗаписи) Экспорт
	...
	Если СтруктураСчетовУчета.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	СтруктураИзмерений = НастройкаСчетовУчетаКлиентСервер.ИнициализироватьСтруктуруИзмеренийРегистра(Ложь);
	СтруктураИзмерений.АналитикаУчета = Объект.Ссылка;
	СтруктураИзмерений.Период = НачалоМесяца(ДанныеНастройкиСчетовУчета.Период);
	ЗаполнитьЗначенияСвойств(СтруктураИзмерений, ДанныеНастройкиСчетовУчета.РеквизитыАналитики);
...
Процедура ПриЧтенииСозданииОбъектаНастройкиСчетовУчета(Форма) Экспорт
	...
	//++ НЕ УТ
	ПараметрыНастройкиСчетовУчета.Вставить("ИзмененныеДанные", "");
	...
	ЗаполнитьЗначенияСвойств(ПараметрыНастройкиСчетовУчета.Блокировка, Блокировка_ДатыЗапрета(ОрганизацияАналитики));
	ПараметрыНастройкиСчетовУчета.Блокировка.РазрешеноРедактирование = ПараметрыНастройкиСчетовУчета.Период > ПараметрыНастройкиСчетовУчета.Блокировка.ПоследняяДатаОтражения;
	
	ДобавитьЭлементыНастройкиСчетовНаФорму(Форма);
	...
	Если ПараметрыНастройкиСчетовУчета.ИспользуетсяНастройкаИсключений Тогда
		ИмяРеквизита = НастройкаСчетовУчетаКлиентСервер.ИмяРеквизитаНаФорме(ПараметрыНастройкиСчетовУчета, "", "Ссылка", "Ссылка");
	...
	ИмяРеквизита = НастройкаСчетовУчетаКлиентСервер.ИмяРеквизитаНаФорме(ПараметрыНастройкиСчетовУчета, "", "ОписаниеИстории", "Ссылка");
	Форма[ИмяРеквизита] = НастройкаСчетовУчетаСлужебныйВызовСервера.СсылкаИзмененияНастроекСчетовУчета(Форма.Объект.Ссылка);
...
Функция ПараметрыНастройкиСчетовУчета(РазделыУчета = Неопределено) Экспорт
	...
	ПараметрыНастройки.Вставить("РазбитьПоРазделам", Ложь); // Реквизит на форме будет называться с учетом раздела к которому он относится
	ПараметрыНастройки.Вставить("РазбитьПоРазделамЭлементы", Ложь); // Элемент на форме будет называться с учетом раздела к которому он относится
	ПараметрыНастройки.Вставить("НастройкиРазделов", Новый Соответствие);
	ПараметрыНастройки.Вставить("ИспользуетсяНастройкаИсключений", Ложь); // Используются настройки исключений по организации и месту учета, необходим вывод гиперссылки на форму для переходу к настройке счетов учета исключений
	...
	ПараметрыНастройки.Вставить("СворачиваемыеГруппы", Ложь);
	...
	ПараметрыНастройки.Вставить("ИспользоватьОтдельныеТаблицы", Ложь); // Для каждой настройки раздела будет создаваться своя таблица значений на форме
	ПараметрыНастройки.Вставить("ДолгосрочныеСчетаОтдельнойСтрокой", Ложь); // Долгосрочные счета выводятся в одной строке вместе с краткосрочными
	
	Цвета = Новый Структура;
	Цвета.Вставить("НеЗаполнено", ЦветаСтиля.НезаполненноеПолеТаблицы);
	Цвета.Вставить("Информация", ЦветаСтиля.ТекстИнформационнойНадписи);
	Цвета.Вставить("Отменено", ЦветаСтиля.ЦветТекстаОтмененнойСтрокиДокумента);
	Цвета.Вставить("АвтоЦвет", ЦветаСтиля.ЦветТекстаПоля);
	ПараметрыНастройки.Вставить("Цвета", Цвета);
	
	ПараметрыНастройки.Вставить("Период", ТекущаяДатаСеанса());
	ПараметрыНастройки.Вставить("СчетПоУмолчанию", Неопределено);
	
	Блокировка = Новый Структура; // Совокупность данных по блокированию реквизитов и периода
	Блокировка.Вставить("Используется", Истина); // Признак использования блокировки
	Блокировка.Вставить("ДатаЗапрета", '00010101'); // Дата запрета изменения на текущий момент, если включено использование дат запрета изменения
	Блокировка.Вставить("ДатаБлокировкиМесяца", '00010101'); // Дата блокировки закрытия месяца (максимальная)
	Блокировка.Вставить("ПоследняяДатаОтражения", '00010101'); // Последняя дата отражения документов в регл. учете
	Блокировка.Вставить("РазрешеноРедактирование", Ложь); // Возможно редактирование указанных счетов учета
	Блокировка.Вставить("ОтключенаПользователем", Ложь); // Блокировка редактирования указанных счетов учета отключена пользователем через соответствующую форму
	ПараметрыНастройки.Вставить("Блокировка", Блокировка);
	...
	РазделыУчетаСоСворачиваемымиГруппами = НастройкаСчетовУчетаКлиентСервер.РазделыУчетаСоСворачиваемымиГруппами();
	...
	Для каждого РазделУчета Из СписокРазделовУчета Цикл
		...
		Если РазделыУчетаСоСворачиваемымиГруппами.Найти(РазделУчета.Значение) <> Неопределено Тогда
			ПараметрыНастройки.СворачиваемыеГруппы = Истина;
		КонецЕсли;
		
		Если СтруктураНастроекРаздела.ДолгосрочныеСчета Тогда
...
Процедура СозданиеРеквизитовФормы(Форма) Экспорт
	...
	Если Не Настройка.СоздаватьВСлучаеОтсутствия Тогда
		Возврат;
	КонецЕсли;
	
	Для каждого НастройкаРаздела Из Настройка.НастройкиРазделов Цикл
		
		ИмяОбщегоРаздела = НастройкаСчетовУчетаКлиентСервер.ОбщийРазделУчетаПоИмениТекущегоРаздела(НастройкаРаздела.Ключ);
		
		Если Настройка.ИспользоватьОтдельныеТаблицы Тогда
			
			ИмяРеквизита = НастройкаСчетовУчетаКлиентСервер.ИмяРеквизитаНаФорме(Настройка, НастройкаРаздела.Ключ, ИмяОбщегоРаздела, "Таблица");
			РеквизитФормы = Новый РеквизитФормы(ИмяРеквизита, Новый ОписаниеТипов("ТаблицаЗначений"),, НастройкаРаздела.Значение.Представление, Истина);
			ДобавитьРеквизитФормы(ДобавляемыеРеквизиты, РеквизитФормы);
		...
		Для каждого ЭлементНастройки Из НастройкаРаздела.Значение.ЭлементыНастройки Цикл
			...
			Если ТипЭлемента = "Долгосрок" И Настройка.ДолгосрочныеСчетаОтдельнойСтрокой Тогда
				Продолжить;
			КонецЕсли;
			...
			ПутьКРеквизиту = "";
			Если Настройка.ИспользоватьОтдельныеТаблицы Тогда
				МассивИменРеквизита = СтрРазделить(ИмяРеквизита, ".");
				ИмяРеквизита = МассивИменРеквизита.Получить(1);
				ПутьКРеквизиту = МассивИменРеквизита.Получить(0);
			КонецЕсли;
			РеквизитФормы = Новый РеквизитФормы(ИмяРеквизита, ТипЭлементаФормы, ПутьКРеквизиту, ЭлементНастройки.Представление, Истина);
			ДобавитьРеквизитФормы(ДобавляемыеРеквизиты, РеквизитФормы);
	...
	Если Форма.ИмяФормы <> "РегистрСведений.ПорядокОтраженияНаСчетахУчета.Форма.ФормаЗаписиДинамически" Тогда
		...
		РеквизитФормы = Новый РеквизитФормы(ИмяРеквизитаПредставлениеПериода, Новый ОписаниеТипов("Строка"));
		ДобавитьРеквизитФормы(ДобавляемыеРеквизиты, РеквизитФормы);
		Если Не Настройка.РедактированиеВСписке Тогда
			ИмяРеквизита = НастройкаСчетовУчетаКлиентСервер.ИмяРеквизитаНаФорме(Настройка, "", "ОписаниеИстории", "Ссылка");
			РеквизитФормы = Новый РеквизитФормы(ИмяРеквизита, Новый ОписаниеТипов("ФорматированнаяСтрока"));
			ДобавитьРеквизитФормы(ДобавляемыеРеквизиты, РеквизитФормы);
		КонецЕсли;
	КонецЕсли;
	
	Если Настройка.СворачиваемыеГруппы И Форма.ИмяФормы <> "РегистрСведений.ПорядокОтраженияНаСчетахУчета.Форма.ФормаЗаписиДинамически" Тогда
		ИмяРеквизитаСворачиваемости = НастройкаСчетовУчетаКлиентСервер.ИмяРеквизитаНаФорме(Настройка, "", "НастройкаСворачиваемостиГрупп", "Команда");
		РеквизитФормы = Новый РеквизитФормы(ИмяРеквизитаСворачиваемости, Новый ОписаниеТипов("Строка"));
		ДобавитьРеквизитФормы(ДобавляемыеРеквизиты, РеквизитФормы);
	КонецЕсли;
	...
	// Создадим массив реквизитов, проверив, что добавляемых реквизитов нет на форме
	ЗаполнитьЗначенияСвойств(ДобавляемыеРеквизиты, Форма);
	МассивДобавляемыхРеквизитов = Новый Массив;
	Для каждого Реквизит Из ДобавляемыеРеквизиты Цикл
		Если Тип("РеквизитФормы") = ТипЗнч(Реквизит.Значение) Тогда
			МассивДобавляемыхРеквизитов.Добавить(Реквизит.Значение);
		ИначеЕсли ТипЗнч(Реквизит.Значение) = Тип("Структура") Тогда
			// Это таблица, проверим наличие реквизитов внутри нее:
			ИмяТаблицы = СтрЗаменить(Реквизит.Ключ, "Таблица" + "_", "");
			Если ДобавляемыеРеквизиты[ИмяТаблицы] <> Тип("РеквизитФормы") Тогда
				// Проверяем наличие реквизитов внутри таблицы только тогда, когда она уже есть на форме.
				// В противном случае она будет добавляться, а значит и все реквизиты для нее должны добавляться безусловно.
				СуществующиеКолонки = Форма[ИмяТаблицы].Выгрузить().Колонки;
			КонецЕсли;
			Для каждого РеквизитТаблицы Из Реквизит.Значение Цикл
				Если СуществующиеКолонки.Найти(РеквизитТаблицы.Ключ) = Неопределено Тогда
					МассивДобавляемыхРеквизитов.Добавить(РеквизитТаблицы.Значение);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	...
	Если Форма.ИмяФормы <> "РегистрСведений.ПорядокОтраженияНаСчетахУчета.Форма.ФормаЗаписиДинамически" Тогда
		// Установим значение сворачиваемости по умолчанию:
		Если Настройка.СворачиваемыеГруппы Тогда
			Форма[ИмяРеквизитаСворачиваемости] = "СвернутьВсе";
		КонецЕсли;
		Форма[ИмяРеквизитаПредставлениеПериода] = Формат(Настройка.Период, НСтр("ru = 'ДФ=''ММММ гггг''; ДП=''С начала учета''';
																				|en = 'DF=''MMMM yyyy''; DE=''From accounting start'''"));
	КонецЕсли;
	
КонецПроцедуры

Процедура ДобавитьЭлементыНастройкиСчетовНаФорму(Форма) Экспорт
	...
	#Область ДобавлениеПериода
	Параметры = ПараметрыДобавленияЭлементаНаФорму(Форма);
	СуществующийЭлемент = Неопределено;
	Если Параметры.ГруппаДляДобавления.ПодчиненныеЭлементы.Количество() Тогда
		СуществующийЭлемент = Параметры.ГруппаДляДобавления.ПодчиненныеЭлементы.Получить(0);
	КонецЕсли;
	ГруппаПериод = ДобавитьЭлементНаФорму("Период", Параметры, "Группа");
	Если СуществующийЭлемент <> Неопределено И ГруппаПериод <> Неопределено Тогда
		Форма.Элементы.Переместить(ГруппаПериод, Параметры.ГруппаДляДобавления, СуществующийЭлемент);
	КонецЕсли;
	ГруппаПериод.ОтображатьЗаголовок = Ложь;
	Параметры.ГруппаДляДобавления = ГруппаПериод;
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, ИмяРеквизита) Тогда
		ПолеПериода = ДобавитьЭлементНаФорму("ПредставлениеПериода", Параметры, "НастройкаПериода");
		ПолеПериода.РедактированиеТекста = Ложь;
		ПолеПериода.Ширина = 10;
		ПолеПериода.РастягиватьПоГоризонтали = Ложь;
		ПолеПериода.ЦветТекста = ?(ЗначениеЗаполнено(Настройка.Период), ЦветаСтиля.ЦветТекстаПоля, ЦветаСтиля.ЗаблокированныйРеквизитЦвет);
		УстановитьДействия(ПолеПериода, "ПриИзменении,НачалоВыбора,Регулирование,Очистка");
	КонецЕсли;
	
	ИмяРеквизита = НастройкаСчетовУчетаКлиентСервер.ИмяРеквизитаНаФорме(Настройка, "", "ОписаниеИстории", "Ссылка");
	Если Не Настройка.РедактированиеВСписке И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, ИмяРеквизита) Тогда
		ДобавитьЭлементНаФорму("ОписаниеИстории", Параметры, "Ссылка");
		Параметры.ГруппаДляДобавления = Форма.Элементы[Настройка.ИмяГруппыНастроекСчетовУчета];
	КонецЕсли;
	#КонецОбласти
	
	ИмяРеквизита = НастройкаСчетовУчетаКлиентСервер.ИмяРеквизитаНаФорме(Настройка, "", "НастройкаСворачиваемостиГрупп", "Команда");
	Если Настройка.СворачиваемыеГруппы И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, ИмяРеквизита) Тогда
		// Добавим на форму команду свертки всех групп:
		ДобавитьЭлементНаФорму("НастройкаСворачиваемостиГрупп", Параметры, "Переключатель");
	КонецЕсли;
	
	МассивДолгосрокКраткосрок = Новый Массив;
	
	Для каждого ЭлементРазделаУчета Из РегистрыСведений.ПорядокОтраженияНаСчетахУчета.СписокРазделовСчетовУчета() Цикл
		
		НастройкаРаздела = Настройка.НастройкиРазделов.Получить(ЭлементРазделаУчета.Значение);
		...
		Если Не Настройка.Свойство("НастройкаПоОдному") Тогда
			...
			Для каждого ЭлементНастройки Из НастройкаРаздела.ЭлементыНастройки Цикл
				...
				ОбщегоНазначенияУТКлиентСервер.ДобавитьНовоеЗначениеВМассив(МассивДолгосрокКраткосрок, ТипЭлемента);
				ИмяЭлемента = СтрЗаменить(ЭлементНастройки.Значение, ТипЭлемента + "_", "");
				ДобавитьГруппуНаФормуДляТекущегоЭлемента(ИмяЭлемента, Параметры, ТипЭлемента);
				ГруппыБылиДобавлены = Параметры.ГруппаДляДобавления <> ГруппаРаздела;
	...
	Параметры.РазделУчета = "";
	
	Параметры.ГруппаДляДобавления = Форма.Элементы[Настройка.ИмяГруппыНастроекСчетовУчета];
	Параметры.ЭлементВыводится = Настройка.ИспользуетсяНастройкаИсключений;
	ПолеНастройкиИсключений = ДобавитьЭлементНаФорму("Ссылка", Параметры, "Ссылка");
	Если ПолеНастройкиИсключений <> Неопределено Тогда
		ПолеНастройкиИсключений.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСверху;
		ТекстПодсказки = НСтр("ru = 'Счет учета используется при отражении документов в учете, если %Детализация% не настроены исключения.';
								|en = 'Ledger account is used while recording documents in accounting if %Детализация% are not configured.'");
		УточнитьДетализациюПоРазделуУчета(ТекстПодсказки, НастройкаСчетовУчетаКлиентСервер.ОбщийРазделУчетаПоИмениТекущегоРаздела(РазделУчета));
		ПолеНастройкиИсключений.РасширеннаяПодсказка.Заголовок = ТекстПодсказки;
		ПолеНастройкиИсключений.РасширеннаяПодсказка.АвтоМаксимальнаяШирина = Ложь;
	КонецЕсли;
	
	Если Не Настройка.РедактированиеВСписке И МассивДолгосрокКраткосрок.Количество() > 1 Тогда
		Параметры.ЭлементВыводится = Настройка.ИспользуютсяДолгосрочныеСчетаУчета;
		Параметры.ГруппаДляДобавления = Форма;
	...
	Параметры.ЭлементВыводится = Настройка.Блокировка.Используется;
	Параметры.ГруппаДляДобавления = Форма;
	Параметры.Представление = НСтр("ru = 'Разрешить редактирование указанных счетов учета';
									|en = 'Allow editing the specified ledger accounts'");
	ЭлементРазрешитьРедактирование = ДобавитьЭлементНаФорму("РазрешитьРедактированиеСчетовУчета", Параметры, "Команда");
	Если ЭлементРазрешитьРедактирование <> Неопределено Тогда
		ЭлементРазрешитьРедактирование.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
		ЭлементРазрешитьРедактирование.Картинка = БиблиотекаКартинок.РазрешитьРедактированиеРеквизитовОбъекта;
		ЭлементРазрешитьРедактирование.Пометка = Настройка.Блокировка.ОтключенаПользователем;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление(Форма, РазделУчета, ИмяЭлементаНастройки, ТипЭлемента = "СчетУчета") Экспорт
	...
	Если ТипЭлемента = "Долгосрок" И Настройка.ДолгосрочныеСчетаОтдельнойСтрокой Тогда
		// Долгосрочные счета настраиваются в отдельной строке, поэтому вместе с основной настройкой не описываем УО.
		Возврат;
	КонецЕсли;
	...
	Если ИмяЭлементаНастройки = "АналитикаУчета" Тогда
		...
		Если НастройкиАналитики.Используется Тогда
			...
			Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", Настройка.Цвета.НеЗаполнено);
	...
	Если Настройка.УчитыватьСчетаПоУмолчанию Тогда
		...
		Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", Настройка.Цвета.Информация);
	...
	Если Настройка.УчитыватьИзмененныеСчетаУчета И Настройка.УчитыватьСчетаТребующиеНастройки Тогда
		...
		Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", Настройка.Цвета.Отменено);
	...
	Если Настройка.ИспользуютсяДолгосрочныеСчетаУчета И (НастройкаРазделаУчета = Неопределено
		ИЛИ НастройкаСчетовУчетаКлиентСервер.ВидыСчетовСНастройкамиДолгосрочнойЗадолженности().Найти(ИмяЭлементаНастройки) = Неопределено) Тогда
		Элемент = Форма.УсловноеОформление.Элементы.Добавить();
...
//			* ДолгосрочныеСчета - Булево - признак наличия в разделе элементов настройки долгосрочных счетов.
//
Функция ИнициализироватьСтруктуруНастроекРаздела(РазделУчета, ТолькоИспользуемые = Ложь) Экспорт
	...
	ЕстьДолгосрочныеСчета = Ложь;
	Для каждого ЭлементНастройки Из СписокЭлементов Цикл
		ИмяЭлемента = ?(ТолькоИспользуемые, ЭлементНастройки, ЭлементНастройки.Значение);
		ЕстьДолгосрочныеСчета = ЕстьДолгосрочныеСчета ИЛИ СтрНачинаетсяС(ИмяЭлемента, "Долгосрок");
	КонецЦикла;
	
	СтруктураНастроек.Вставить("ДолгосрочныеСчета", ЕстьДолгосрочныеСчета);
...
Функция НастройкиДолгосрочныхСчетовПоРазделуУчета(РазделУчета) Экспорт 
	...
	ЗначениеВозврата.Используется = СтруктураНастроекРаздела.ДолгосрочныеСчета;
...
Процедура ЗаполнитьРеквизитыНастройкиСчетовУчетаПоДаннымАналитики(ПараметрыНастройки, АналитикаУчета) Экспорт
	...
	Если АналитикаУчета <> Неопределено И РеквизитыАналитики <> Неопределено Тогда
		Для каждого ЭлементСписка Из ПараметрыНастройки.СписокРазделовУчета Цикл
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(РеквизитыАналитики, ЭлементСписка.Значение) Тогда
				ЭлементСписка.Пометка = РеквизитыАналитики[ЭлементСписка.Значение];
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура УточнитьДетализациюПоРазделуУчета(Строка, РазделУчета) Экспорт
	
	ТекстДетализации = НСтр("ru = 'по организациям';
							|en = 'by companies'");
	Если РазделУчета <> Неопределено Тогда
		НастройкиМестаУчета = НастройкиМестаУчетаПоРазделуУчета(РазделУчета);
	...
	Строка = СтрЗаменить(Строка, "%Детализация%", " " + ТекстДетализации);
	
КонецПроцедуры

// Возвращает данные по блокировке настроек счетов учета
//
//	Параметры:
//		Организация - СправочникСсылка.Организации - организация, по которой необходимо получить данные по блокировкам, если не заполнена
//			будут анализироваться все организации.
//
//	Возвращаемое значение:
//		Структура:
//			* ДатаЗапрета - Дата - Дата запрета изменения на текущий момент, если включено использование дат запрета изменения
//			* ДатаБлокировкиМесяца - Дата - Дата блокировки закрытия месяца (максимальная)
//			* ПоследняяДатаОтражения - Дата - Последняя дата отражения документов в регл. учете.
//
Функция Блокировка_ДатыЗапрета(Организация = Неопределено) Экспорт
	
	Блокировка = Новый Структура;
	Блокировка.Вставить("ДатаЗапрета", '00010101');
	Блокировка.Вставить("ДатаБлокировкиМесяца", '00010101');
	Блокировка.Вставить("ПоследняяДатаОтражения", '00010101');
	
	УстановитьПривилегированныйРежим(Истина);
	
	Организации = ?(ЗначениеЗаполнено(Организация), ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Организация), Справочники.Организации.ДоступныеОрганизации());
	Блокировка.ДатаБлокировкиМесяца = РегистрыСведений.НастройкаБлокировкиОтИзменений.НезаблокированныйПериод(Организации, Ложь);
	
	СтруктураЗапрета = Новый Структура;
	...
	СтрокаДанных.Раздел = "БухгалтерскийУчет";
	СтрокаДанных.Объект = Организация;
	ЕстьЗапрет = ДатыЗапретаИзменения.НайденЗапретИзмененияДанных(ДанныеДляПроверки, ПараметрыСообщенияОЗапрете, СтруктураЗапрета);
	Блокировка.ДатаЗапрета = ?(ЕстьЗапрет, СтруктураЗапрета.Запреты.Получить(0).ДатаЗапрета, Блокировка.ДатаЗапрета);
	
	Блокировка.ПоследняяДатаОтражения = ПоследняяДатаОтражения(Организации, 3);
...
Процедура ДобавитьЭтап_ПереносСальдоПриИзмененииНастроекСчетовРеглУчета(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Перенос сальдо при изменении настроек счетов регл учета';
											|en = 'Transfer balance when changing local accounting account settings'"; // @НСтр-1
	...
	НоваяСтрока.ДействиеИспользование = ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
		"НастройкаСчетовУчетаСервер.Использование_ПереносСальдоПриИзмененииНастроекСчетовРеглУчета");
	НоваяСтрока.ДействиеВыполнить  = ЗакрытиеМесяцаСервер.ОписаниеДействия_ВыполнитьРасчет(
		"НастройкаСчетовУчетаСервер.Выполнить_ПереносСальдоПриИзмененииНастроекСчетовРеглУчета");
	НоваяСтрока.ТипыРегламентныхОпераций.Добавить(Перечисления.ТипыРегламентныхОпераций.ПереносСальдоПриИзмененииНастроекСчетовРеглУчета);
	НоваяСтрока.ДействиеПодробнее = ЗакрытиеМесяцаЛокализация.ОписаниеДействия_ОткрытьСписокДокументовРегламентнаяОперация();
	
КонецПроцедуры

Процедура Использование_ПереносСальдоПриИзмененииНастроекСчетовРеглУчета(ПараметрыОбработчика) Экспорт
	
	ЗакрытиеМесяцаСервер.ПроверитьИспользованиеРегламентированногоУчета(ПараметрыОбработчика);
	
	Данные = ДанныеИзмененияНастроекСчетовУчетаТребующиеПереноса(ПараметрыОбработчика.ПараметрыРасчета);
	
	Если Данные.НастройкиИзменены_ЭтотМесяц
		Или Не Данные.ТекущийМесяцЗакрыт И Данные.ПереносНеВыполнен_СледующийМесяц
		Или Данные.МесяцЗадания <= ПараметрыОбработчика.ПараметрыРасчета.НачалоПериода Тогда
		
		Если Не Данные.ПереносНеВыполнен_ЭтотМесяц И Не (Данные.ПереносНеВыполнен_СледующийМесяц И Не Данные.ТекущийМесяцЗакрыт) Тогда
		...
		Если Данные.МесяцЗадания < ПараметрыОбработчика.ПараметрыРасчета.НачалоПериода Тогда
			ТекстПояснения = НСтр("ru = 'Перенос сальдо нужно выполнить в предшествующих периодах (%1 г.)';
									|en = 'The balance transfer in previous periods (%1) is required.'", ОбщегоНазначения.КодОсновногоЯзыка());
			ТекстПояснения = СтрШаблон(ТекстПояснения, Формат(Данные.МесяцЗадания, НСтр("ru = 'ДФ=''ММММ гггг''';
																						|en = 'DF=''MMMM yyyy'''")));
			ЗакрытиеМесяцаСервер.ДобавитьПоясняющуюИнформациюКЭтапу(ПараметрыОбработчика, ТекстПояснения);
		КонецЕсли;
		
		Если Данные.НастройкиИзменены_ЭтотМесяц Или Данные.МесяцЗадания = ПараметрыОбработчика.ПараметрыРасчета.НачалоПериода Тогда
			ТекстПояснения = НСтр("ru = 'Необходимо выполнить операцию переноса сальдо';
									|en = 'Balance transfer operation is required'", ОбщегоНазначения.КодОсновногоЯзыка());
			Если Не Данные.ПереносНеВыполнен_ЭтотМесяц И Данные.НастройкиИзменены_ЭтотМесяц Тогда
				ТекстПояснения = НСтр("ru = 'В текущем месяце изменились настройки счетов учета и был выполнен перенос сальдо';
										|en = 'The ledger account settings are changed for the current month. The balance was transferred'", ОбщегоНазначения.КодОсновногоЯзыка());
			ИначеЕсли Данные.ПереносНеВыполнен_ЭтотМесяц И Данные.НастройкиИзменены_ЭтотМесяц Тогда
				ТекстПояснения = НСтр("ru = 'В текущем месяце изменились настройки счетов учета, необходимо выполнить перенос сальдо';
										|en = 'The ledger account settings are changed for the current month. Transfer the balance'", ОбщегоНазначения.КодОсновногоЯзыка());
			КонецЕсли;
			ЗакрытиеМесяцаСервер.ДобавитьПоясняющуюИнформациюКЭтапу(ПараметрыОбработчика, ТекстПояснения);
		КонецЕсли;
		
		Если (Данные.ПереносНеВыполнен_СледующийМесяц Или Данные.НастройкиИзменены_СледующийМесяц) И Не Данные.ТекущийМесяцЗакрыт Тогда
			ТекстПояснения = НСтр("ru = 'В следующем месяце необходимо выполнить операцию переноса сальдо';
									|en = 'The balance transfer operation is required next month'", ОбщегоНазначения.КодОсновногоЯзыка());
			Если Данные.НастройкиИзменены_СледующийМесяц И Не Данные.ПереносНеВыполнен_СледующийМесяц Тогда
				ТекстПояснения = НСтр("ru = 'В следующем месяце выполнен перенос сальдо по измененным настройкам счетов учета';
										|en = 'In the next month, the balance was transferred according to the changed ledger account settings.'", ОбщегоНазначения.КодОсновногоЯзыка());
			ИначеЕсли Данные.ПереносНеВыполнен_ЭтотМесяц И Данные.ПереносНеВыполнен_СледующийМесяц Тогда
				ТекстПояснения = НСтр("ru = 'В следующем месяце изменились настройки счетов учета, необходимо выполнить перенос сальдо';
										|en = 'The ledger account settings are changed for the next month. Transfer the balance.'", ОбщегоНазначения.КодОсновногоЯзыка());
			КонецЕсли;
			ЗакрытиеМесяцаСервер.ДобавитьПоясняющуюИнформациюКЭтапу(ПараметрыОбработчика, ТекстПояснения);
		КонецЕсли;
...
Процедура Выполнить_ПереносСальдоПриИзмененииНастроекСчетовРеглУчета(ПараметрыОбработчика) Экспорт
	
	Данные = ДанныеИзмененияНастроекСчетовУчетаТребующиеПереноса(ПараметрыОбработчика.ПараметрыРасчета);
	
	МассивОбрабатываемыхПериодов = Новый Массив;
	Если Данные.ПереносНеВыполнен_ЭтотМесяц Тогда
		МассивОбрабатываемыхПериодов.Добавить(ПараметрыОбработчика.ПараметрыРасчета.НачалоПериода);
	КонецЕсли;
	Если Данные.ПереносНеВыполнен_СледующийМесяц Тогда
		МассивОбрабатываемыхПериодов.Добавить(НачалоМесяца(ПараметрыОбработчика.ПараметрыРасчета.КонецПериода + 1));
	КонецЕсли;
	Параметры = Новый Структура;
	Параметры.Вставить("МассивОрганизаций", ПараметрыОбработчика.ПараметрыРасчета.МассивОрганизаций);
	Для каждого ОбрабатываемыйПериод Из МассивОбрабатываемыхПериодов Цикл
		Параметры.Вставить("Период", ОбрабатываемыйПериод);
		Попытка
			Документы.РегламентнаяОперация.ПереносСальдоПриИзмененииНастроекСчетовРеглУчета(Параметры);
		Исключение
			
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Перенос сальдо за период %1 завершилось с ошибкой:
					|%2';
					|en = 'The transfer of the balance for the %1 period completed with an error:
					|%2'", ОбщегоНазначения.КодОсновногоЯзыка()),
				РасчетСебестоимостиПротоколРасчета.ПредставлениеПериодаРасчета(ОбрабатываемыйПериод),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Перенос сальдо при изменении настроек счетов регл. учета';
											|en = 'Transfer balance when changing local accounting account settings'"),
...
// Возвращает период изменения настроек счетов учета, либо Неопределено в случае,
// если начиная с данного периода настройки не изменялись.
//
// Параметры:
// 	Организация - СправочникСсылка.Организации - организация, по которой нужно получить период изменения настроек
// 	Период - Дата - дата, на которую нужно получить данные
//
// Возвращаемое значение:
// 	Дата, Неопределено - дату начала изменения настроек счетов учета или Неопределено, если они далее не изменялись
//
Функция Период_ПереносСальдоПриИзмененииНастроекСчетовРеглУчета(Организация, Период) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЕСТЬNULL(МИНИМУМ(НастройкиСчетов.Период), ДАТАВРЕМЯ(1,1,1)) КАК Месяц,
	|	Неопределено КАК Документ,
	|	&Организация КАК Организация,
	|	ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.ПереносСальдоПриИзмененииНастроекСчетовРеглУчета) КАК Операция
	|ИЗ
	|	РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПервых(&Период,) КАК НастройкиСчетов";
	...
	Если ЭлементФормы <> Неопределено Тогда
		...
		Если ЭлементФормы.Видимость Тогда
			Если ТипЭлемента <> "Субконто" И ТипЭлемента <> "ДопПоле" Тогда
				ЭлементФормы.ПараметрыВыбора = НастройкаСчетовУчетаКлиентСервер.ПараметрыВыбораСчетаУчета(ИмяЭлементаНастройки, Настройки, ТипЭлемента = "Долгосрок");
			КонецЕсли;
			Если Настройки.ИспользуютсяДолгосрочныеСчетаУчета И Не Настройки.РедактированиеВСписке
				...
				Если ТипЭлемента = "Долгосрок" И Не Настройки.РедактированиеВСписке
				...
				МассивТекстовПодсказки.Добавить(?(ТипЭлемента = "Долгосрок", НСтр("ru = 'Долгосрочный';
		...
		Если Не Настройки.РедактированиеВСписке Тогда
			ИмяКомандыИсторииИзменений = НастройкаСчетовУчетаКлиентСервер.ИмяЭлементаНаФорме(Настройки, Параметры.РазделУчета, ИмяЭлементаНастройки, ТипЭлемента)
				+ "_" + "КомандаКонтекста" + "_" + "ИсторияИзменений";
			КомандаИсторииИзменений = ЭлементФормы.КонтекстноеМеню.ПодчиненныеЭлементы.Найти(ИмяКомандыИсторииИзменений);
			ИмяКоманды = НастройкаСчетовУчетаКлиентСервер.ИмяРеквизитаНаФорме(Настройки,, "ИсторияИзмененийСчета", "Команда");
			Если Параметры.Форма.Команды.Найти(ИмяКоманды) = Неопределено Тогда
				...
				КомандаФормы.Заголовок = НСтр("ru = 'История изменений';
												|en = 'Change history'");
				КомандаФормы.Картинка = БиблиотекаКартинок.ИсторияДанных;
			КонецЕсли;
			Если КомандаИсторииИзменений = Неопределено Тогда
				КомандаИсторииИзменений = Параметры.Форма.Элементы.Добавить(ИмяКомандыИсторииИзменений, Тип("КнопкаФормы"), ЭлементФормы.КонтекстноеМеню);
				КомандаИсторииИзменений.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
				КомандаИсторииИзменений.ИмяКоманды = ИмяКоманды;
			КонецЕсли;
		КонецЕсли;
		
		Если Настройки.Свойство("ИзмененныеДанные") Тогда
			УстановитьДействия(ЭлементФормы, "ПриИзменении");
		КонецЕсли;
...
// 		* РазделУчета - Строка -
// 		* Форма - ФормаКлиентскогоПриложения - 
// 	ТипЭлемента - Строка - название типа элемента формы, возможные значения:
//		Команда,Переключатель,Таблица,Группа,НастройкаПериода,Ссылка,СчетУчета,Долгосрок,Субконто,ДопПоле.
// Возвращаемое значение:
...
Функция ДобавитьЭлементНаФорму(ИмяЭлемента, Параметры, ТипЭлемента = "СчетУчета")
	...
	Если ЭлементФормы <> Неопределено Или Не Параметры.СоздаватьВСлучаеОтсутствия Или Не Параметры.ЭлементВыводится Тогда
		Если ЭлементФормы <> Неопределено И Параметры.СоздаватьВСлучаеОтсутствия
			И Не Настройка.РедактированиеВСписке И ТипЭлемента <> "Команда" И ТипЭлемента <> "Переключатель"
			И (Параметры.ДобавленныеГруппы.Количество() И Параметры.ДобавленныеГруппы.Найти(ИмяЭлемента) = Неопределено ИЛИ ТипЭлемента <> "Группа")
			Тогда
			Параметры.Форма.Элементы.Переместить(ЭлементФормы, Параметры.ГруппаДляДобавления);
		КонецЕсли;
	...
	Если ТипЭлемента = "Таблица" Тогда
		ТипЭлементаФормы = Тип("ТаблицаФормы");
	ИначеЕсли СтрНачинаетсяС(ТипЭлемента, "Группа") Тогда
		ТипЭлементаФормы = Тип("ГруппаФормы");
		Параметры.ДобавленныеГруппы.Добавить(ИмяЭлемента);
	Иначе
	...
	ГруппаДляДобавления = ?(ТипЭлемента = "Команда", Параметры.ГруппаДляДобавления.КоманднаяПанель, Параметры.ГруппаДляДобавления);
	ЭлементФормы = Параметры.Форма.Элементы.Добавить(ИмяЭлементаНаФорме, ТипЭлементаФормы, ГруппаДляДобавления);
	Если СтрНачинаетсяС(ТипЭлемента, "Группа") Тогда
		ВыводГруппуВТабличнуюЧасть = Настройка.РедактированиеВСписке И ИмяЭлемента <> "Период";
		ЭлементФормы.Вид = ?(ВыводГруппуВТабличнуюЧасть, ВидГруппыФормы.ГруппаКолонок, ВидГруппыФормы.ОбычнаяГруппа);
		ЭлементФормы.Группировка = ?(ВыводГруппуВТабличнуюЧасть, ГруппировкаКолонок.Горизонтальная, ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяЕслиВозможно);
		Если Не ВыводГруппуВТабличнуюЧасть Тогда
			ЭлементФормы.Отображение = ОтображениеОбычнойГруппы.Нет;
		КонецЕсли;
		Если Настройка.СворачиваемыеГруппы И ИмяЭлемента <> "Период" Тогда
			...
			ЭлементФормы.Скрыть();
		КонецЕсли;
	ИначеЕсли ТипЭлемента = "Таблица" Тогда
	...
	ИначеЕсли ТипЭлемента = "Ссылка" Тогда
	...
	Иначе
		...
		ЭлементФормы.ПутьКДанным = НастройкаСчетовУчетаКлиентСервер.ИмяРеквизитаНаФорме(Настройка, Параметры.РазделУчета, ИмяЭлемента, ТипЭлемента);
		Если (ТипЭлемента = "СчетУчета" ИЛИ ТипЭлемента = "Долгосрок") И Не Настройка.РедактированиеВСписке Тогда
		...
		Если ТипЭлемента = "Субконто" И Не Настройка.РедактированиеВСписке Тогда
			...
			Если Параметры.ГруппаДляДобавления <> Неопределено И СтрЗаканчиваетсяНа(Параметры.ГруппаДляДобавления.Имя, "Субконто") Тогда
				ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
			КонецЕсли;
			УстановитьДействия(ЭлементФормы, "НачалоВыбора");
		КонецЕсли;
		Если ТипЭлемента = "Долгосрок" Тогда
			ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
		КонецЕсли;
		Если ТипЭлемента = "СчетУчета" И (НастройкаСчетовУчетаКлиентСервер.СчетИспользуетСубконто(ИмяЭлемента) ИЛИ Настройка.СворачиваемыеГруппы) Тогда
			УстановитьДействия(ЭлементФормы, "ПриИзменении");
		КонецЕсли;
		Если ТипЭлемента = "НастройкаПериода" Тогда
			ЭлементФормы.Заголовок = НСтр("ru = 'Действует с';
											|en = 'Valid from'");
		КонецЕсли;
	КонецЕсли;
...
Функция ДобавитьГруппуНаФормуДляТекущегоЭлемента(ИмяЭлемента, Параметры, ТипЭлемента = "СчетУчета")
	
	Настройка = Параметры.Форма.КэшДанныхМеханизмов.НастройкаСчетовУчета;
	ГруппаНаФорме = Неопределено;
	НачальнаяГруппа = ?(Параметры.ГруппаРаздела <> Неопределено, Параметры.ГруппаРаздела, Параметры.Форма.Элементы[Настройка.ИмяГруппыНастроекСчетовУчета]);
	
	НастройкиГруппы = Параметры.СоответствиеЭлементовГруппам.Получить(ТипЭлемента + "_" + ИмяЭлемента);
	
	Если НастройкиГруппы <> Неопределено И (ИмяЭлемента <> НастройкиГруппы.ИмяГруппы ИЛИ ТипЭлемента = "СчетУчета") Тогда
		
		ГруппаРодитель = ДобавитьГруппуНаФормуДляТекущегоЭлемента(НастройкиГруппы.ИмяГруппы, Параметры, "Группа");
		Параметры.ГруппаДляДобавления = ?(ГруппаРодитель <> Неопределено, ГруппаРодитель, НачальнаяГруппа);
		...
		Если ГруппаНаФорме <> Неопределено Тогда
			...
			Если Настройка.СворачиваемыеГруппы И ГруппаНаФорме.ЗаголовокСвернутогоОтображения = "" Тогда
...
Функция ИнициализироватьРазделНаФорме(Форма, РазделУчета, Параметры)
	...
	Если Настройка.РазбитьПоРазделамЭлементы Тогда
		...
		Если ГруппаРаздела <> Неопределено Тогда
			...
			Параметры.ГруппаРаздела = ГруппаРаздела;
		КонецЕсли;
		Если Настройка.ИспользоватьОтдельныеТаблицы Тогда
			// Выведем отдельно таблицу (список) настроек счетов:
			ТаблицаРаздела = ДобавитьЭлементНаФорму("", Параметры, "Таблица");
			Если ТаблицаРаздела <> Неопределено Тогда
				ТаблицаРаздела.Заголовок = РазделУчета.Представление;
			КонецЕсли;
			Параметры.ГруппаДляДобавления = ТаблицаРаздела;
			Параметры.ГруппаРаздела = ТаблицаРаздела;
		КонецЕсли;
	КонецЕсли;
	
	Параметры.СоответствиеЭлементовГруппам = НастройкаСчетовУчетаКлиентСервер.СоответствиеЭлементовНастройкиГруппам(РазделУчета.Значение, Настройка.РедактированиеВСписке);
...
Функция ПараметрыДобавленияЭлементаНаФорму(Форма)
	...
	СтруктураВозврата.Вставить("ГруппаРаздела", Неопределено);
	...
	СтруктураВозврата.Вставить("СоответствиеЭлементовГруппам", Новый Соответствие);
	СтруктураВозврата.Вставить("ДобавленныеГруппы", Новый Массив);
	
	Возврат СтруктураВозврата;
	
КонецФункции

Процедура ДобавитьРеквизитФормы(ДобавляемыеРеквизиты, РеквизитФормы)
	
	УказанПуть = ЗначениеЗаполнено(РеквизитФормы.Путь);
	Если УказанПуть Тогда
		// Добавляем реквизит в отдельную таблицу, нужно проверять данный реквизит конкретно в этой таблице,
		//	поэтому пропишем таблицу и набор реквизитов для нее в отдельной структуре:
		Таблица = Неопределено;
		ДобавляемыеРеквизиты.Свойство("Таблица" + "_" + РеквизитФормы.Путь, Таблица);
		Если Таблица = Неопределено Тогда
			Таблица = Новый Структура;
			ДобавляемыеРеквизиты.Вставить("Таблица" + "_" + РеквизитФормы.Путь, Таблица);
		КонецЕсли;
		Если Не Таблица.Свойство(РеквизитФормы.Имя) Тогда
			Таблица.Вставить(РеквизитФормы.Имя, РеквизитФормы);
		КонецЕсли;
	ИначеЕсли Не ДобавляемыеРеквизиты.Свойство(РеквизитФормы.Имя) Тогда
		ДобавляемыеРеквизиты.Вставить(РеквизитФормы.Имя, РеквизитФормы);
	КонецЕсли;
	
КонецПроцедуры

Процедура УстановитьДействия(ПолеФормы, СписокДействий)
	
	СоответствиеДействийКнопкам = Новый Соответствие;
	СоответствиеДействийКнопкам.Вставить("НачалоВыбора", "КнопкаВыбора");
	СоответствиеДействийКнопкам.Вставить("Регулирование", "КнопкаРегулирования");
	СоответствиеДействийКнопкам.Вставить("Очистка", "КнопкаОчистки");
	
	Действия = СтрРазделить(СписокДействий, ",");
	
	Для каждого Действие Из Действия Цикл
		ИмяКнопки = СоответствиеДействийКнопкам.Получить(Действие);
		ЗначениеКнопки = ?(ИмяКнопки = Неопределено, Неопределено, ПолеФормы[ИмяКнопки]);
		Если ПолеФормы.ПолучитьДействие(Действие) = "" И ЗначениеКнопки <> Ложь Тогда
			ИмяМетода = "Подключаемый" + "_" + Действие + "_" + "НастройкаСчетовУчета";
			ПолеФормы.УстановитьДействие(Действие, ИмяМетода);
		КонецЕсли;
		Если ИмяКнопки <> Неопределено И ИмяКнопки <> "КнопкаОчистки" И ЗначениеКнопки <> Ложь Тогда
			ПолеФормы[ИмяКнопки] = Истина;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьОбщиеРеквизитыВлияющиеНаНастройкуСчетовУчета(РазделыУчета, Реквизиты)
	...
	Для каждого РазделУчета Из РазделыУчета Цикл
		...
		Если РазделУчета = "НоменклатураСобственная" Или РазделУчета = "Номенклатура" Тогда
			...
			Реквизиты.Вставить("ИспользуетсяПСН", УчетнаяПолитикаПереопределяемый.ИспользуетсяПСН());
			...
			Реквизиты.Вставить("ИспользоватьОтгрузкуБезПереходаПраваСобственности",
...
Функция ДанныеИзмененияНастроекСчетовУчетаТребующиеПереноса(Параметры)
	
	СтруктураВозврата = Новый Структура("ПереносНеВыполнен_ЭтотМесяц,ПереносНеВыполнен_СледующийМесяц", Ложь, Ложь);
	СтруктураВозврата.Вставить("ТекущийМесяцЗакрыт", Ложь);
	СтруктураВозврата.Вставить("МесяцЗадания", Параметры.НачалоПериода);
	СтруктураВозврата.Вставить("НастройкиИзменены_ЭтотМесяц", Ложь);
	СтруктураВозврата.Вставить("НастройкиИзменены_СледующийМесяц", Ложь);
	...
	|	МАКСИМУМ(Данные.ЕстьЗадание) КАК ЕстьЗадание
	|ПОМЕСТИТЬ ЗаданияКЗакрытиюМесяца
	|ИЗ
	|	(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		УчетнаяПолитикаБухУчета.Организация КАК Организация,
	|		НЕ ТекущиеЗадания.Организация ЕСТЬ NULL КАК ЕстьЗадание
	|	ИЗ
	|		РегистрСведений.УчетнаяПолитикаБухУчета.СрезПоследних(&Период, Организация В (&Организации)) КАК УчетнаяПолитикаБухУчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗаданияКЗакрытиюМесяца КАК ТекущиеЗадания
	|		ПО ТекущиеЗадания.Месяц <= &КонецПериода
	|			И ТекущиеЗадания.Организация = УчетнаяПолитикаБухУчета.Организация
	|			И ТекущиеЗадания.Операция = ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.ФормированиеФинансовогоРезультата)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		УчетнаяПолитикаБухУчета.Организация,
	|		НЕ ОтражениеДокументовВРеглУчете.Организация ЕСТЬ NULL
	|	ИЗ
	|		РегистрСведений.УчетнаяПолитикаБухУчета.СрезПоследних(&Период, Организация В (&Организации)) КАК УчетнаяПолитикаБухУчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОтражениеДокументовВРеглУчете КАК ОтражениеДокументовВРеглУчете
	|		ПО ОтражениеДокументовВРеглУчете.ДатаОтражения <= &КонецПериода
	|			И ОтражениеДокументовВРеглУчете.Организация = УчетнаяПолитикаБухУчета.Организация
	|			И ОтражениеДокументовВРеглУчете.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтраженияДокументовВРеглУчете.КОтражениюВРеглУчете)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		УчетнаяПолитикаБухУчета.Организация,
	|		НЕ ТекущиеЗадания.Организация ЕСТЬ NULL
	|	ИЗ
	|		РегистрСведений.УчетнаяПолитикаБухУчета.СрезПоследних(&Период, Организация В (&Организации)) КАК УчетнаяПолитикаБухУчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗаданияКРасчетуСебестоимости КАК ТекущиеЗадания
	|		ПО ТекущиеЗадания.Месяц <= &КонецПериода
	|			И ТекущиеЗадания.Организация = УчетнаяПолитикаБухУчета.Организация) КАК Данные
	|
	|СГРУППИРОВАТЬ ПО
	|	Данные.Организация
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЕСТЬNULL(МАКСИМУМ(НЕ ТекущиеНастройки.СчетУчета ЕСТЬ NULL
	|			ИЛИ НЕ СледующиеНастройки.СчетУчета ЕСТЬ NULL), ЛОЖЬ) КАК ЕстьИзмененияНастроек,
	|	ЕСТЬNULL(МАКСИМУМ(НЕ ЗаданияКЗакрытиюМесяца.ЕстьЗадание), ИСТИНА) КАК ТекущийМесяцЗакрыт,
	|	ЕСТЬNULL(МИНИМУМ(ТекущиеЗадания.Месяц), &Период) КАК МесяцЗадания,
	|	ЕСТЬNULL(МАКСИМУМ(НЕ ТекущиеЗадания.Организация ЕСТЬ NULL
	|			ИЛИ ТекущиеДокументыПереноса.Ссылка ЕСТЬ NULL И (НЕ ТекущиеНастройки.СчетУчета ЕСТЬ NULL
	|		ИЛИ Константы.ДатаНачалаРеглУчетаПродукцииИПолуфабрикатовПоПартиямВыпуска = &Период
	|		И Константы.РеглУчетПродукцииИПолуфабрикатовПоПартиямВыпуска
	|		ИЛИ Константы.ДатаНачалаРеглУчетаТоваровИМатериаловПоПартиямПриобретения = &Период
	|		И Константы.РеглУчетТоваровИМатериаловПоПартиямПриобретения)), ЛОЖЬ) КАК ПереносНеВыполнен_ЭтотМесяц,
	|	ЕСТЬNULL(МАКСИМУМ(НЕ ТекущиеНастройки.СчетУчета ЕСТЬ NULL
	|		ИЛИ Константы.ДатаНачалаРеглУчетаПродукцииИПолуфабрикатовПоПартиямВыпуска = &Период
	|		И Константы.РеглУчетПродукцииИПолуфабрикатовПоПартиямВыпуска
	|		ИЛИ Константы.ДатаНачалаРеглУчетаТоваровИМатериаловПоПартиямПриобретения = &Период
	|		И Константы.РеглУчетТоваровИМатериаловПоПартиямПриобретения), ЛОЖЬ) КАК НастройкиИзменены_ЭтотМесяц,
	|	ЕСТЬNULL(МАКСИМУМ(НЕ СледующиеЗадания.Организация ЕСТЬ NULL
	|			ИЛИ СледующиеДокументыПереноса.Ссылка ЕСТЬ NULL И (НЕ СледующиеНастройки.СчетУчета ЕСТЬ NULL
	|		ИЛИ Константы.ДатаНачалаРеглУчетаПродукцииИПолуфабрикатовПоПартиямВыпуска = &СледующийПериод
	|		И Константы.РеглУчетПродукцииИПолуфабрикатовПоПартиямВыпуска
	|		ИЛИ Константы.ДатаНачалаРеглУчетаТоваровИМатериаловПоПартиямПриобретения = &СледующийПериод
	|		И Константы.РеглУчетТоваровИМатериаловПоПартиямПриобретения)), ЛОЖЬ) КАК ПереносНеВыполнен_СледующийМесяц,
	|	ЕСТЬNULL(МАКСИМУМ((НЕ СледующиеНастройки.СчетУчета ЕСТЬ NULL
	|		ИЛИ Константы.ДатаНачалаРеглУчетаПродукцииИПолуфабрикатовПоПартиямВыпуска = &СледующийПериод
	|		И Константы.РеглУчетПродукцииИПолуфабрикатовПоПартиямВыпуска
	|		ИЛИ Константы.ДатаНачалаРеглУчетаТоваровИМатериаловПоПартиямПриобретения = &СледующийПериод
	|		И Константы.РеглУчетТоваровИМатериаловПоПартиямПриобретения)
	|		И Константы.ДатаНачалаРеглУчетаНЗППоПартиямПроизводства <> &СледующийПериод
	|		И Константы.ДатаНачалаРеглУчетаПродажПоПартиямРеализованнойНоменклатуры <> &СледующийПериод), ЛОЖЬ) КАК НастройкиИзменены_СледующийМесяц
	|ИЗ
	|	ЗаданияКЗакрытиюМесяца КАК ЗаданияКЗакрытиюМесяца
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Константы КАК Константы
	|	ПО Истина
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета КАК ТекущиеНастройки
	|	ПО ТекущиеНастройки.Период = &Период
	|		И (ТекущиеНастройки.Организация = ЗаданияКЗакрытиюМесяца.Организация
	|			ИЛИ ТекущиеНастройки.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка))
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета КАК СледующиеНастройки
	|	ПО СледующиеНастройки.Период = &СледующийПериод
	|		И (СледующиеНастройки.Организация = ЗаданияКЗакрытиюМесяца.Организация
	|			ИЛИ СледующиеНастройки.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка))
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗаданияКЗакрытиюМесяца КАК ТекущиеЗадания
	|	ПО ТекущиеЗадания.Месяц <= &Период
	|		И ТекущиеЗадания.Организация = ЗаданияКЗакрытиюМесяца.Организация
	|		И ТекущиеЗадания.Операция = ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.ПереносСальдоПриИзмененииНастроекСчетовРеглУчета)
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗаданияКЗакрытиюМесяца КАК СледующиеЗадания
	|	ПО СледующиеЗадания.Месяц <= &СледующийПериод
	|		И СледующиеЗадания.Организация = ЗаданияКЗакрытиюМесяца.Организация
	|		И СледующиеЗадания.Операция = ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.ПереносСальдоПриИзмененииНастроекСчетовРеглУчета)
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.РегламентнаяОперация КАК ТекущиеДокументыПереноса
	|	ПО НАЧАЛОПЕРИОДА(ТекущиеДокументыПереноса.Дата, МЕСЯЦ) = &Период И ТекущиеДокументыПереноса.Проведен
	|		И ТекущиеДокументыПереноса.Организация = ЗаданияКЗакрытиюМесяца.Организация
	|		И ТекущиеДокументыПереноса.ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипыРегламентныхОпераций.ПереносСальдоПриИзмененииНастроекСчетовРеглУчета)
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.РегламентнаяОперация КАК СледующиеДокументыПереноса
	|	ПО НАЧАЛОПЕРИОДА(СледующиеДокументыПереноса.Дата, МЕСЯЦ) = &СледующийПериод И СледующиеДокументыПереноса.Проведен
	|		И СледующиеДокументыПереноса.Организация = ЗаданияКЗакрытиюМесяца.Организация
	|		И (СледующиеДокументыПереноса.ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипыРегламентныхОпераций.ПереносСальдоПриИзмененииНастроекСчетовРеглУчета))";
	...
	Запрос.УстановитьПараметр("СледующийПериод", НачалоМесяца(ДобавитьМесяц(Параметры.НачалоПериода, 1)));
	Запрос.УстановитьПараметр("Организации", Параметры.МассивОрганизаций);
...
Функция ПоследняяДатаОтражения(Организации, ШагМесяцевПроверки)
	
	ПоследняяДатаОтражения = Дата(1,1,1);
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ЕСТЬNULL(МАКСИМУМ(Данные.ДатаОтражения), ДАТАВРЕМЯ(1,1,1)) КАК ДатаОтражения
	|ИЗ
	|	РегистрСведений.ОтражениеДокументовВРеглУчете КАК Данные
	|ГДЕ
	|	Данные.Организация В (&МассивОрганизаций) И Данные.ДатаОтражения > &Период
	|	И Данные.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтраженияДокументовВРеглУчете.ОтраженоВРеглУчете)";
	Запрос.УстановитьПараметр("МассивОрганизаций", Организации);
	ДатаПроверки = ?(ШагМесяцевПроверки = 0, ПоследняяДатаОтражения, ДобавитьМесяц(ТекущаяДатаСеанса(), -ШагМесяцевПроверки));
	Запрос.УстановитьПараметр("Период", ДатаПроверки); 
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		ПоследняяДатаОтражения = Выборка.ДатаОтражения;
	ИначеЕсли ШагМесяцевПроверки <> 0 Тогда
		ШагМесяцевПроверки = ?(ШагМесяцевПроверки = 3, 12, 0);
		ПоследняяДатаОтражения(Организации, ШагМесяцевПроверки);
	КонецЕсли;
	
	Возврат ПоследняяДатаОтражения;
