...
Функция ОчередиЗаданий() Экспорт
	...
	//++ НЕ УТ
	Параметры = ПараметрыОчередиЗаданий();
	Параметры.Очередь                     = МетаРегистр.ЗаданияКОтражениюДокументовПланированияВБюджетировании.Имя;
	Параметры.ИмяМетода                   = "БюджетированиеСервер.ВыполнитьОтражениеДокументовПланированияВБюджетировании";
	...
	//++ НЕ УТКА
	Параметры = ПараметрыОчередиЗаданий();
	Параметры.Очередь           = МетаРегистр.ЗаданияКРасчетуСвязейНоменклатурыПоРазрешениямНаЗамену.Имя;
	Параметры.ИмяМетода         = "РегистрыСведений.СвязиНоменклатурыПоРазрешениямНаЗамену.РассчитатьСвязиМатериаловИАналоговВПланировании";
	Параметры.КоличествоПотоков = 1;
	Параметры.АналитикаРасчета  = "Разрешение";
	ДобавитьОчередь(ТаблицаОчередей, Параметры);
	//-- НЕ УТКА
...
Процедура ЗаданияКОтражениюДокументовПланированияВБюджетировании(
			ПараметрыОчереди,
			ПараметрыДанных,
			ЗафиксироватьОшибку = Ложь,
			ОбработатьВТекущемСеансе = Ложь,
...
//  ТекстОшибки              - см. ВыполнитьЗадания.ТекстОшибки
//
Процедура ЗаданияКРасчетуСвязейНоменклатурыПоРазрешениямНаЗамену(
			ПараметрыОчереди,
			ПараметрыДанных,
			ЗафиксироватьОшибку = Ложь,
			ОбработатьВТекущемСеансе = Ложь,
