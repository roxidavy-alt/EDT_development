...
Процедура ЗаполнитьМаркировкуТоваровПоВыпускуПродукции(ДанныеЗаполнения, ДокументОбъект)
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|	ПО ВыпускПродукцииТовары.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|	И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииМЕХ)
	|ГДЕ
...
Процедура ЗаполнитьМаркировкуТоваровПоПроизводствуБезЗаказа(ДанныеЗаполнения, ДокументОбъект)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
		...
		|	Документ.ПроизводствоБезЗаказа.ВыходныеИзделия КАК ВыходныеИзделия
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
		|	ПО ВыходныеИзделия.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
		|	И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииМЕХ)
		|ГДЕ
		|	ВыходныеИзделия.Ссылка = &Ссылка
		|
		...
		|	Документ.ПроизводствоБезЗаказа.ПобочныеИзделия КАК ПобочныеИзделия
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
		|	ПО ПобочныеИзделия.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
		|	И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииМЕХ)
		|ГДЕ
		|	ПобочныеИзделия.Ссылка = &Ссылка;
		|
...
Процедура ЗаполнитьМаркировкуТоваровПоМаршрутномуЛистуПроизводства(ДанныеЗаполнения, ДокументОбъект)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
		...
		|	Документ.МаршрутныйЛистПроизводства.ВозвратныеОтходы КАК ВозвратныеОтходы
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
		|	ПО ВозвратныеОтходы.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
		|	И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииМЕХ)
		|ГДЕ
		|	ВозвратныеОтходы.Ссылка = &Ссылка
		|
...
Функция ТекстМаркировкаТоваровГИСМСписаниеТоваровНаРасходы()
	...
	|	""МаркировкаТоваровГИСМУТ_584"" КАК НомерФрагмента
	|ИЗ
...
Функция ТекстМаркировкаТоваровГИСМВключениеИсключениеНДСВСтоимостьТоваров()
	...
	|	""МаркировкаТоваровГИСМУТ_585"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстПеремаркировкаТоваровГИСМСписаниеТоваровНаРасходы()
	...
	|	ЕСТЬNULL(Стоимости.СуммаВР, Строки.СуммаВР) КАК СуммаВРКт,
	|	""Списание товаров на расходы"" КАК Содержание,
	|	""МаркировкаТоваровГИСМУТ_586"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстПеремаркировкаТоваровГИСМВключениеИсключениеНДСВСтоимостьТоваров()
	...
	|	0 КАК СуммаВРКт,
	|	""Включение/исключение НДС в стоимость товара на складе - получателе"" КАК Содержание,
	|	""МаркировкаТоваровГИСМУТ_587"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
