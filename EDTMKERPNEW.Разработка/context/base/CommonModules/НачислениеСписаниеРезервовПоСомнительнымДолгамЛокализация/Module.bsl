...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	|	РезервыПоСомнительнымДолгам.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|	
	...
	|	""НачислениеСписаниеРезервовПоСомнительнымДолгамЛокализация_598"" КАК НомерФрагмента
	|ИЗ
	...
	|	""НачислениеСписаниеРезервовПоСомнительнымДолгамЛокализация_599"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.НачислениеСписаниеРезервовПоСомнительнымДолгам КАК Операция
	|		ПО ДокументыКОтражению.Ссылка = Операция.Ссылка
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	|	РезервыПоСомнительнымДолгам.ИдентификаторФинЗаписи КАК ИдентификаторФинЗаписи,
	...
	|	ЕСТЬNULL(РезервыПоСомнительнымДолгам.ОбъектРасчетов.НаправлениеДеятельности, ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)) КАК НаправлениеДеятельности,
	|	ВЫБОР
	...
	|	РезервыПоСомнительнымДолгам.ИдентификаторФинЗаписи,
	|	ЕСТЬNULL(РезервыПоСомнительнымДолгам.ОбъектРасчетов.Подразделение, ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)),
	|	ЕСТЬNULL(РезервыПоСомнительнымДолгам.ОбъектРасчетов.НаправлениеДеятельности, ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)),
	|	ВЫБОР
	|		КОГДА РезервыПоСомнительнымДолгам.ОбъектРасчетов = НЕОПРЕДЕЛЕНО
	|			ТОГДА ЕСТЬNULL(РезервыПоСомнительнымДолгам.АналитикаУчетаПоПартнерам.Контрагент, ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка))
	|		ИНАЧЕ ЕСТЬNULL(РезервыПоСомнительнымДолгам.ОбъектРасчетов.Контрагент, ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка))
...
Процедура СформироватьПроводкиПереходаНаНовыйМеханизмРасчетаРезервов(Объект, ДополнительныеСвойства, Движения, Отказ)
	...
	|	""НачислениеСписаниеРезервовПоСомнительнымДолгамЛокализация_600"" КАК НомерФрагмента
		|ИЗ
