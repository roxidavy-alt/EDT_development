...
Процедура ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки) Экспорт
	...
	ОписаниеОбработчика.ЗаголовокОшибки = НСтр("ru = 'Обработчик установки параметров сеанса ""%1"".';
												|en = 'Session parameter setting handler ""%1"".'");
	...
	ОписаниеОбработчика.ЗаголовокОшибки = НСтр("ru = 'Обработчик регламентного задания (ручной запуск) ""%1"".';
												|en = 'Scheduled job handler (manual launch) ""%1"".'");
			   ...
			   И ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
				...
				ШаблонОшибки =
					НСтр("ru = 'Не удалось установить монопольный режим для настройки узла РИБ по причине:
					           |%1';
					           |en = 'Cannot enable the exclusive mode to set up the distributed infobase node due to:
					           |%1'");
...
Процедура ПроверитьМетодДляВызоваКакПроизвольныйКод(ПолноеИмяМетода, ВФоне = Ложь, Объект = Неопределено, Текст = Null) Экспорт
	...
	Если ВФоне И ПроцедураОпределена Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Метод ""%1"" не разрешен для запуска в фоне в процедуре ""%2"" модуля ""%3"".';
				|en = 'The ""%1"" method cannot run in the background in the ""%2""  procedure of the ""%3"" module.'"),
	...
	ИначеЕсли Не ВФоне И ПроцедураОпределена Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Метод ""%1"" не разрешен для вызова, как произвольный код, в процедуре ""%2"" модуля ""%3"".';
				|en = 'The ""%1"" method cannot be called as arbitrary code in the ""%2""  procedure of the ""%3"" module.'"),
			ИмяМетода, "ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод", ИмяМодуля);
	ИначеЕсли ВФоне И Не ПроцедураОпределена Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Метод ""%1"" не разрешен для запуска в фоне в процедуре ""%2"" модуля ""%3"" (процедура не определена).';
				|en = 'The ""%1"" method cannot run in the background in the ""%2""  procedure of the ""%3"" module (the procedure is not defined).'"),
			ИмяМетода, "ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод", ИмяМодуля);
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Метод ""%1"" не разрешен для вызова, как произвольный код, в процедуре ""%2"" модуля ""%3"" (процедура не определена).';
				|en = 'The ""%1"" method cannot be called as arbitrary code in the ""%2""  procedure of the ""%3"" module (the procedure is not defined).'"),
			ИмяМетода, "ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод", ИмяМодуля);
	КонецЕсли;
	...
	ИмяСобытия = НСтр("ru = 'Безопасность.Метод не разрешен для вызова, как произвольный код';
						|en = 'Security.The method cannot be called as arbitrary code'",
		ОбщегоНазначения.КодОсновногоЯзыка());
