...
Процедура ДобавитьЭтап_РасчетНалогаНаИмущество(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.РасчетНалогаНаИмущество,,,,
		Справочники.ОперацииЗакрытияМесяца.ФормированиеДвиженийПоВНА);

	НоваяСтрока.МультиязычноеНаименование = "ru = 'Расчет налога на имущество';
											|en = 'Property tax calculation'"; // @НСтр-1
...
Процедура ДобавитьЭтап_РасчетТранспортногоНалога(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.РасчетТранспортногоНалога,,,,
		Справочники.ОперацииЗакрытияМесяца.РасчетНалогаНаИмущество);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Расчет транспортного налога';
											|en = 'Vehicle tax calculation'"; // @НСтр-1
...
Процедура ДобавитьЭтап_РасчетЗемельногоНалога(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.РасчетЗемельногоНалога,,,,
		Справочники.ОперацииЗакрытияМесяца.РасчетТранспортногоНалога);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Расчет земельного налога';
											|en = 'Land value tax calculation'"; // @НСтр-1
	НоваяСтрока.ПредшествующиеЭтапы.Добавить(
		ВнеоборотныеАктивыЛокализация.ОперацияПереходаНаУчетВнеоборотныхАктивов2_4());
	НоваяСтрока.ТекстВыполнить = НСтр("ru = 'Рассчитать';
										|en = 'Calculate'");
...
Процедура ДобавитьЭтап_ДоначислениеНалогаНаИмущество(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ДоначислениеНалогаНаИмущество,,,,
		Справочники.ОперацииЗакрытияМесяца.ОбесценениеВНА);
		
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Доначисление налога на имущество';
											|en = 'Property tax charge'"; // @НСтр-1
...
Процедура СформироватьТаблицуОССостоящиеНаУчете(ПараметрыРасчета, Период, ДополнительныеПараметры) Экспорт
	...
	Если ПараметрыРасчета.Свойство("Организации") И ПараметрыРасчета.Организации.Количество() > 1 Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, " = &Организация", " В (&Организации)");
	КонецЕсли;
	...
	|		И &ИсправлениеПараметровОрганизация
	...
	Если ПараметрыРасчета.Свойство("Организации") И
			ПараметрыРасчета.Организации.Количество() > 1 Тогда 
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, 
									"&ИсправлениеПараметровОрганизация", 
									"ИсправлениеПараметров.Организация В (&Организации)");
	...
	Иначе
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, 
									"&ИсправлениеПараметровОрганизация", 
									"ИсправлениеПараметров.Организация = &Организация");
...
Если ВнеоборотныеАктивы.ИспользуетсяУправлениеВНА_2_4(ДобавитьМесяц(Период, -1)) Тогда
		...
		Если Месяц(ПараметрыРасчета.мДатаКонцаПериодаОтчета) + 1 = Месяц(Период)
			...
			|
			|ВЫБРАТЬ
			|	ОтнесениеОтклоненийВСтоимостьОС.Организация,
			|	СтоимостьОС.СуммаБУ,
			|	СтоимостьОС.ОсновноеСредство
			|ИЗ
			|	Документ.ОтнесениеОтклоненийВСтоимостьОС КАК ОтнесениеОтклоненийВСтоимостьОС
			|
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтнесениеОтклоненийВСтоимостьОС.ОС КАК СтоимостьОС
			|		ПО (СтоимостьОС.Ссылка = ОтнесениеОтклоненийВСтоимостьОС.Ссылка)
			|
			|ГДЕ
			|	ОтнесениеОтклоненийВСтоимостьОС.Организация В (&Организации)
			|	И ОтнесениеОтклоненийВСтоимостьОС.Проведен
			|	И ОтнесениеОтклоненийВСтоимостьОС.ОтражатьВРеглУчете
			|	И ОтнесениеОтклоненийВСтоимостьОС.Дата МЕЖДУ &НачалоПредыдущегоМесяца И &КонецПредыдущегоМесяца
			|
			|	И ИСТИНА В
			|			(ВЫБРАТЬ ПЕРВЫЕ 1
			|				ИСТИНА
			...
			|				СписокОС КАК СписокОС
			|			ГДЕ
			|				СписокОС.Организация = ОтнесениеОтклоненийВСтоимостьОС.Организация
			|				И СписокОС.ОсновноеСредство = СтоимостьОС.ОсновноеСредство)
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	Организация,
...
Процедура СформироватьТаблицуРегистрацияТранспортныхСредствСрезПоследних(ПараметрыРасчета, Период, ДополнительныеПараметры, ИмяТаблицы = "") Экспорт
	...
	// За периоды до 2020 в расчете применялись устаревшие поля. Вернем им прежние имена.
	Если Период < РасчетИмущественныхНалоговКлиентСервер.НачалоПримененияНовогоРасчетаТранспортногоНалога() Тогда
		ВосстановитьИменаУстаревшихПолейРегистраПараметрыНачисленияТранспортногоНалога(ТекстЗапроса);
	КонецЕсли;
	...
	// Получим ее из табицы записей регистра через регистратор. 
...
Функция КодыВидовИКатегорииТранспортныхСредств() Экспорт
	...
	Макет = Документы.ПараметрыНачисленияТранспортногоНалога.ПолучитьМакет("КодыВидовИКатегорииТСДо2020");
...
Процедура СформироватьТаблицуРегистрацияЗемельныхУчастковСрезПоследних(ПараметрыРасчета, Период, ДополнительныеПараметры, ИмяТаблицы = "", УсловияЗапроса = "") Экспорт
	...
	Если ВременнаяТаблицаЗаписейСУчетомИсправлений = Неопределено Тогда
		 ВызватьИсключение НСтр("ru = 'Для формирования среза последних по записям регистра параметров начисления земельного налога
		 							...
		 							|en = 'To form a slice of the latest records of the parameters register for accruing land value tax
		 							|a temporary table of records is required.'");
...
Процедура СформироватьЗаданиеКРасчетуНалогаНаИмущество(Документ, ДанныеТаблиц) Экспорт
	...
	Запрос.УстановитьПараметр("Операция", Справочники.ОперацииЗакрытияМесяца.РасчетНалогаНаИмущество);
...
Процедура СформироватьЗаданиеКРасчетуТранспортногоНалога(Документ, ДанныеТаблиц, ЕстьОсновныеСредства = Неопределено) Экспорт
	...
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.МенеджерВременныхТаблиц = ДанныеТаблиц.МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("Операция", Справочники.ОперацииЗакрытияМесяца.РасчетТранспортногоНалога);
...
Процедура СформироватьЗаданиеКРасчетуЗемельногоНалога(Документ, ДанныеТаблиц) Экспорт
	...
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.МенеджерВременныхТаблиц = ДанныеТаблиц.МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("Операция", Справочники.ОперацииЗакрытияМесяца.РасчетЗемельногоНалога);
...
Процедура ПривестиСуммуНалогаНаИмуществоКЦеломуЧислу(Организация, ДатаОтражения, РасчетПоНалогу)
	...
	// из-за перерасчета общая сумма налога по всем ОС перестает быть целым числом.
...
// Возвращаемое значение:
// 	Строка - Текст запроса.
Функция ТекстЗапросаСрезПоследнихРегистраСУчетомИсправлений(ИмяРегистра, ИмяВременнойТаблицы = "", 
																ДополнительныеПоля = Неопределено, УсловияЗапроса = "",
																ВременнаяТаблицаЗаписей = Неопределено, УникальныйРегистратор = Истина) Экспорт
	...
	|			ДАТАВРЕМЯ(3999,12,31) // Возвращаем назад подмененную дату конца времен
	|		ИНАЧЕ
...
Процедура УточнитьПараметрыРегистрацииОрганизацииВФНС(Параметры, Организация, Период) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	РегистрацииВНалоговомОрганеСрезПоследних.Период КАК Период,
	|	РегистрацииВНалоговомОрганеСрезПоследних.РегистрацияВНалоговомОргане КАК ОсновнойНалоговыйОрган,
	|	ЕСТЬNULL(РегистрацииВНалоговомОрганеСрезПоследних.РегистрацияВНалоговомОргане.КодПоОКТМО, """") КАК КодПоОКТМО
	|ИЗ
	|	РегистрСведений.РегистрацииВНалоговомОргане.СрезПоследних(
	|			&Период,
	|			Организация = &Организация
	|				И Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)) КАК РегистрацииВНалоговомОрганеСрезПоследних";
...
Функция ПустаяСправкаРасчет(ИмяРегистраСведений) Экспорт
	...
	Если ИмяРегистраСведений = "РасчетТранспортногоНалогаДо2020" Тогда
...
Функция ТаблицыРасчетНалога(Реквизиты, ДополнительныеПараметры)
	...
	ИначеЕсли Реквизиты.ВидНалога = Перечисления.ВидыИмущественныхНалогов.ТранспортныйНалог Тогда
		...
		РасчетПоНалогу = РасчетыПоНалогу.РезультатыРасчетаПоНалогу;
		...
		Если РасчетПоНалогу.Свойство("РасчетТранспортногоНалогаДо2020") Тогда
			ПроверитьРасчетТранспортногоНалога(РасчетПоНалогу.РасчетТранспортногоНалогаДо2020, ДополнительныеПараметры.СписокОшибок);
		Иначе
			ПроверитьРасчетТранспортногоНалога(РасчетПоНалогу.РасчетТранспортногоНалога, ДополнительныеПараметры.СписокОшибок);
		КонецЕсли;
	...
	ИначеЕсли Реквизиты.ВидНалога = Перечисления.ВидыИмущественныхНалогов.ТранспортныйНалог Тогда
		Если ТипЗнч(РасчетПоНалогу) = Тип("Структура") Тогда
			Если РасчетПоНалогу.Свойство("РасчетТранспортногоНалогаДо2020") Тогда
				ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(РасчетПоНалогу.РасчетТранспортногоНалогаДо2020, СправкаРасчет);
				ТаблицыРасчетНалога.Вставить("СправкаРасчет", СправкаРасчет);
			Иначе
				ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(РасчетПоНалогу.РасчетТранспортногоНалога, СправкаРасчет);
				ТаблицыРасчетНалога.Вставить("СправкаРасчет", СправкаРасчет);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьРасчетТранспортногоНалога(РасчетНалога, СписокОшибок)

	СтрокиБезСтавки = РасчетНалога.НайтиСтроки(Новый Структура("ОтсутствуетСтавка", Истина));
...
Процедура ВосстановитьИменаУстаревшихПолейРегистраПараметрыНачисленияТранспортногоНалога(ТекстЗапроса)
	СписокУстаревшихПолей  = Новый Соответствие;
	СписокУстаревшихПолей.Вставить("КодВидаТранспортногоСредстваДо2020", "КодВидаТранспортногоСредства");
	СписокУстаревшихПолей.Вставить("КодНалоговойЛьготыДо2020", "КодНалоговойЛьготы");
	СписокУстаревшихПолей.Вставить("КодПоОКАТОДо2014", "КодПоОКАТО");
	СписокУстаревшихПолей.Вставить("НалоговаяЛьготаДо2020", "НалоговаяЛьгота");
	СписокУстаревшихПолей.Вставить("УдалитьНалоговаяСтавкаЗависитОтГодаВыпускаТС",
										"НалоговаяСтавкаЗависитОтГодаВыпускаТС");
	СписокУстаревшихПолей.Вставить("ПроцентУменьшенияДо2020", "ПроцентУменьшения");
	СписокУстаревшихПолей.Вставить("РегиональныйКодЛьготыДо2020", "РегиональныйКодЛьготы");
	
	Для Каждого УстаревшееПоле Из СписокУстаревшихПолей Цикл
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "КАК " + УстаревшееПоле.Ключ, "КАК " + УстаревшееПоле.Значение);
	КонецЦикла;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	РегистрыСведений.ПараметрыНачисленияТранспортногоНалога.ПриДобавленииОбработчиковОбновления(Обработчики);
	РегистрыСведений.РасчетТранспортногоНалога.ПриДобавленииОбработчиковОбновления(Обработчики);
	Документы.СверкаСФНСПоИмущественнымНалогам.ПриДобавленииОбработчиковОбновления(Обработчики);
