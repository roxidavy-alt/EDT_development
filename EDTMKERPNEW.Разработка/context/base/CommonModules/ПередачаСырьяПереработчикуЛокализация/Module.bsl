...
#Область ПередачаКомиссионныхТоваровСписание          // (Дт  :: Кт 004.01)
	...
	|	""ПередачаСырьяПереработчикуЛокализация_770"" КАК НомерФрагмента
	|ИЗ
...
#Область ПередачаТоваров                   // (Дт 10.07 :: Кт 10.01, 76.ОК)
	...
	|	""ПередачаСырьяПереработчикуЛокализация_771"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ПередачаТоваровСоСкладаПроизводства // (Дт 10.07 :: Кт 20, 76.ОК)
	...
	|	ЕСТЬNULL(Стоимости.СуммаВР, Строки.СуммаВР) КАК СуммаВРКт,
	|	""Передача в переработку"" КАК Содержание,
	|	""ПередачаСырьяПереработчикуЛокализация_772"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ЗалоговаяСтоимостьПереданнойТары             // (Дт 76 :: Кт 41.03)
	...
	|	""ПередачаСырьяПереработчикуЛокализация_773"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ЗалоговаяСтоимостьПереданнойТарыСкладПроизводства // (Дт 76 :: Кт 20)
	...
	|	0 КАК СуммаВРКт,
	|	""Себестоимость реализованного товара"" КАК Содержание,
	|	""ПередачаСырьяПереработчикуЛокализация_774"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстВключениеИсключениеНДСВСтоимость        // (Дт 10.07 :: Кт 19.03)
	...
	|	""ПередачаСырьяПереработчикуЛокализация_775"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
