...
// Конструктор параметров назначения производственной операции.
// 
// Параметры:
//  МножественныйРежим - Булево - Множественный режим назначения
// 
// Возвращаемое значение:
//  Структура - Параметры назначения производственной операции конструктор:
...
// * Исполнитель - СправочникСсылка.ФизическиеЛица
// * Количество - Число
Функция ПараметрыНазначенияПроизводственнойОперацииКонструктор(МножественныйРежим = Ложь) Экспорт
	
	Результат = Новый Структура(ПараметрыНазначенияПроизводственнойОперацииСписок(МножественныйРежим));
	
	Возврат Результат;
	
КонецФункции

// Перечень реквизитов, заполняемых при назначении операции
// 
// Параметры:
//  МножественныйРежим - Булево - Множественный режим назначения
// 
// Возвращаемое значение:
//  Строка
Функция ПараметрыНазначенияПроизводственнойОперацииСписок(МножественныйРежим = Ложь) Экспорт
	
	Если МножественныйРежим Тогда
		Результат = "Участок,СменноеЗадание,ВидРабочегоЦентра,РабочийЦентр,ВариантНаладки,Исполнитель";
	Иначе
		Результат = "Участок,СменноеЗадание,ВидРабочегоЦентра,РабочийЦентр,ВариантНаладки,Исполнитель,Количество,ВидыРабот";
	КонецЕсли;
...
// Конструктор результата формирования производственных операций
// 
// Возвращаемое значение:
//  Структура - Результат формирования производственных операций конструктор:
// * КоличествоВсего - Число
// * КоличествоОбработанных - Число
// * ОперацииСозданные - Массив из ДокументСсылка.ПроизводственнаяОперация2_2
// * ОперацииОбновленные - Массив из ДокументСсылка.ПроизводственнаяОперация2_2
// * Индивидуальные - Массив из ДокументСсылка.ПроизводственнаяОперация2_2
Функция РезультатФормированияПроизводственныхОперацийКонструктор() Экспорт
	
	Результат = Новый Структура;
	
	Результат.Вставить("КоличествоВсего", 0);
	Результат.Вставить("КоличествоОбработанных", 0);
	Результат.Вставить("ОперацииСозданные", Новый Массив);
	Результат.Вставить("ОперацииОбновленные", Новый Массив);
	Результат.Вставить("Индивидуальные", Новый Массив);
...
// Статус операции в виде ссылки
// 
// Параметры:
//  Статус - Строка,ПеречислениеСсылка.СтатусыПроизводственныхОпераций
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.СтатусыПроизводственныхОпераций
Функция СтатусОперацииСсылка(Статус) Экспорт
	
	Если ТипЗнч(Статус) = Тип("Строка") Тогда
		Возврат ПредопределенноеЗначение("Перечисление.СтатусыПроизводственныхОпераций."+Статус);
	Иначе
		Возврат Статус;
	КонецЕсли;
...
// Начальный статус индивидуальной операции.
// 
// Параметры:
...
// Возвращаемое значение:
//  ПеречислениеСсылка.СтатусыПроизводственныхОпераций
Функция НачальныйСтатусПроизводственнойОперации(Объект) Экспорт
	
	Возврат ?(Объект.НаОснованииНСИ,
		ПредопределенноеЗначение("Перечисление.СтатусыПроизводственныхОпераций.Создана"),
		ПредопределенноеЗначение("Перечисление.СтатусыПроизводственныхОпераций.Формируется"));
	
КонецФункции

// Подтверждение аннулирование показателей.
// 
// Возвращаемое значение:
//  Строка
Функция Подтверждение_АннулированиеПоказателей() Экспорт
	Возврат "АннулированиеПоказателей";
КонецФункции

// Подтверждение создание индивидуальной операции.
// 
// Возвращаемое значение:
//  Строка
Функция Подтверждение_СозданиеИндивидуальнойОперации() Экспорт
	Возврат "СозданиеИндивидуальнойОперации";
КонецФункции

// Состояния операций, выполнение которых начато либо завершено
// 
// Возвращаемое значение:
//  Массив из ПеречислениеСсылка.СостоянияВыполненияОпераций
Функция АктивныеСостоянияОперации() Экспорт
	
	Результат = Новый Массив;
	
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.СостоянияВыполненияОпераций.Выполнена"));
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.СостоянияВыполненияОпераций.Отменена"));
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.СостоянияВыполненияОпераций.Пропущена"));
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.СостоянияВыполненияОпераций.НаДоработке"));
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.СостоянияВыполненияОпераций.НаКонтроле"));
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.СостоянияВыполненияОпераций.Выполняется"));
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.СостоянияВыполненияОпераций.Назначена"));
	
	Возврат Результат;
	
КонецФункции

#Область РеквизитыРабочегоЦентра

// Реквизиты рабочего центра конструктор.
// 
// Возвращаемое значение:
//  Структура - Реквизиты рабочего центра конструктор:
// * ВидРабочегоЦентра - СправочникСсылка.ВидыРабочихЦентров - 
// * РабочийЦентр - СправочникСсылка.РабочиеЦентры - 
...
Функция РеквизитыРабочегоЦентраКонструктор() Экспорт
	
	Результат = Новый Структура;
	
	Результат.Вставить("ВидРабочегоЦентра", ПредопределенноеЗначение("Справочник.ВидыРабочихЦентров.ПустаяСсылка"));
	Результат.Вставить("РабочийЦентр", ПредопределенноеЗначение("Справочник.РабочиеЦентры.ПустаяСсылка"));
	Результат.Вставить("ИспользоватьВариантыНаладки", Ложь);
	Результат.Вставить("КоэффициентВремениРаботы", 1);
...
Функция ЕдиницаИзмеренияОбщегоВремени(ТекущиеДанные)
	...
	ЕстьВремяШтучное = ЗначениеЗаполнено(ТекущиеДанные.ВремяШтучное) И ЗначениеЗаполнено(ТекущиеДанные.ВремяШтучноеЕдИзм);
	ЕстьВремяПЗ = ЗначениеЗаполнено(ТекущиеДанные.ВремяПЗ) И ЗначениеЗаполнено(ТекущиеДанные.ВремяПЗЕдИзм);
	
	Если ЕстьВремяШтучное И ЕстьВремяПЗ Тогда
...
// Определяет, совершались ли с операцией какие-либо действия, меняющие ее показатели
// 
// Параметры:
...
// Возвращаемое значение:
//  Булево
Функция ЕстьНакопленныеПоказателиВыполнения(Объект) Экспорт
	
	Возврат Объект.Протокол.Количество() > 0
		ИЛИ Объект.КоличествоНаКонтроле > 0
		ИЛИ Объект.КоличествоНаДоработке > 0
		ИЛИ Объект.КоличествоБрак > 0
		ИЛИ Объект.КоличествоОтменено > 0
		ИЛИ Объект.КоличествоФакт > 0;
	
КонецФункции

// Текст варианта заполнения операции по нормативу
// 
// Параметры:
//  Техпроцесс - СправочникСсылка.МаршрутныеКарты,СправочникСсылка.ТехнологическиеПроцессы,СправочникСсылка.РесурсныеСпецификации
// 
// Возвращаемое значение:
//  Строка
Функция ТекстВариантЗаполненияПоНормативу(Техпроцесс) Экспорт
	
	Результат = "";
	
	Если ТипЗнч(Техпроцесс) = Тип("СправочникСсылка.МаршрутныеКарты") Тогда
		Результат = НСтр("ru = 'по маршрутной карте';
						|en = 'by route sheet'");
	ИначеЕсли ТипЗнч(Техпроцесс) = Тип("СправочникСсылка.ТехнологическиеПроцессы") Тогда
		Результат = НСтр("ru = 'по технологическому процессу';
						|en = 'by technological process'");
	ИначеЕсли ТипЗнч(Техпроцесс) = Тип("СправочникСсылка.РесурсныеСпецификации") Тогда
		Результат = НСтр("ru = 'по спецификации';
						|en = 'by bill of materials'");
	Иначе
		Результат = НСтр("ru = 'по НСИ';
						|en = 'by master data'");
	КонецЕсли;
...
Функция ВариантыКонтроляИндивидуальнойОперации(ТекущееЗначение = Неопределено) Экспорт
	
	Результат = Новый Массив;
	
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.ВариантыКонтроляТехнологическихОпераций.НеТребуется"));
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.ВариантыКонтроляТехнологическихОпераций.Требуется"));
	Если ТекущееЗначение = ПредопределенноеЗначение("Перечисление.ВариантыКонтроляТехнологическихОпераций.ТребуетсяНаПоследующих") Тогда
