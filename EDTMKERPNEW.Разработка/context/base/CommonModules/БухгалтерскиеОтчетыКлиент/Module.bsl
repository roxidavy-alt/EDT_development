...
Процедура ТабличноеПолеПоСчетамПредставлениеНачалоВыбора(ФормаОтчета, ИмяЭлемента, Элемент, ДанныеВыбора, СтандартнаяОбработка) Экспорт
	...
	Иначе
		...
		Для Индекс = 1 По КоличествоСубконто Цикл
			...
			Если БухгалтерскиеОтчетыКлиентСерверПереопределяемый.ДополнитьСписокВидовСубконто(СписокВидовСубконто, СтрокаПоСубконто, Индекс) Тогда
				Продолжить;
			КонецЕсли;
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("СписокЗначений") Тогда 
		...
		Для Каждого ЭлементСписка Из ВыбранноеЗначение Цикл
			...
			Если ТипЗнч(ЭлементСписка.Значение) = Тип("ПланВидовХарактеристикСсылка.ВидыСубконтоХозрасчетные") Тогда
				...
				КонецЦикла;
			Иначе
				Если БухгалтерскиеОтчетыКлиентСерверПереопределяемый.ДополнитьСтрокуПоСубконто(СтрокаПоСубконто, ЭлементСписка) Тогда
					Продолжить;
				КонецЕсли;
...
Процедура СброситьВидПериода(Отчет) Экспорт
	...
	Если СбросКОтчетномуПериоду(Отчет.НачалоПериода, Отчет.КонецПериода, Отчет.ВидПериода) Тогда
		Отчет.ВидПериода = БухгалтерскиеОтчетыКлиентСервер.ВидыПериода().ОтчетныйПериод;
	КонецЕсли;
...
Функция СбросКОтчетномуПериоду(НачалоПериода, КонецПериода, ВидПериода)
	
	НуженСброс = Ложь;
	НуженСброс = КонецДня(КонецПериода) <> КонецГода(КонецПериода)
		ИЛИ КонецДня(НачалоПериода) <> КонецГода(НачалоПериода)
		И ВидПериода = БухгалтерскиеОтчетыКлиентСервер.ВидыПериода().МежотчетныйПериод;
		
	НуженСброс = БухгалтерскиеОтчетыКлиентСерверПереопределяемый.СбросКОтчетномуПериоду(НачалоПериода, КонецПериода, ВидПериода);
