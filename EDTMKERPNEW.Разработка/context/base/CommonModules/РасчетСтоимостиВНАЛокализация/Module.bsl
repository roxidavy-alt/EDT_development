...
Процедура КорректировкаСтоимостиАрендованногоИмущества(РасчетРасходов, ТаблицаОбъектов)
	...
	Для Каждого ДанныеОбъекта Из ТаблицаОбъектов Цикл
		...
		СуммаНУ = ОбщегоНазначенияУТКлиентСервер.РаспределитьПропорционально(ДанныеОбъекта.СуммаНУ, СтоимостьНУКоэффициенты);
...
Процедура СформироватьЗаданияКДоначислениюНалогаНаИмущество(МенеджерВременныхТаблиц) Экспорт
	...
	|		0 КАК ФактическаяСтоимостьБУ,
	|
	|		ТаблицаСтоимости.СуммаУУ КАК ПредварительнаяСтоимостьУУ,
	|		ТаблицаСтоимости.СуммаБУ КАК ПредварительнаяСтоимостьБУ
	|
	|	ИЗ
	|		Документ.ОтнесениеОтклоненийВСтоимостьОС.ОС КАК ТаблицаСтоимости
	|
	|	ГДЕ
	|		ТаблицаСтоимости.Ссылка В
	|				(ВЫБРАТЬ
	...
	|			И СУММА(ТаблицаСтоимости.ПредварительнаяСтоимостьУУ) <> 0
	|		ИЛИ СУММА(ТаблицаСтоимости.ФактическаяСтоимостьБУ) <> СУММА(ТаблицаСтоимости.ПредварительнаяСтоимостьБУ)
	|			И СУММА(ТаблицаСтоимости.ПредварительнаяСтоимостьБУ) <> 0)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка,
	|	ОбъектУчета
