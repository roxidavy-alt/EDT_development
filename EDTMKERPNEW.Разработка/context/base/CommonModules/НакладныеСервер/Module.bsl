...
Функция ТекстЗапросаСостоянияПередачВПроизводство(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению, ПараметрыЗамены = Неопределено)
	...
	|		ИНАЧЕ 2
	|	КОНЕЦ                           КАК Состояние,
	|	ВтЕстьРаспоряжения.КОформлению  КАК КОформлению
	|ПОМЕСТИТЬ ВтСостоянияПередачВПроизводство
	|ИЗ
	|	ВтЕстьРаспоряжения КАК ВтЕстьРаспоряжения
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтЕстьНакладные КАК ВтЕстьНакладные
	...
	Если ПараметрыОтбора.Подразделение Тогда
		ДопУсловие = ДопУсловие +
			" И (Получатель В(&Подразделение)
			|	ИЛИ (ТИПЗНАЧЕНИЯ(Получатель) = ТИП(Справочник.Склады)
			|		И ВЫРАЗИТЬ(Получатель КАК Справочник.Склады).ЦеховаяКладовая
			|		И ВЫРАЗИТЬ(Получатель КАК Справочник.Склады).Подразделение В (&Подразделение)))";
	КонецЕсли;
	
	ДополнитьТекстОтборомПоОстаткам(ТекстЗапроса, "Распоряжение", ТипыРаспоряжений, ПараметрыОтбора.Склад, ДопУсловие,, Истина);
...
Функция ТекстЗапросаСостоянияПередач(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению)
	...
	|				ИЛИ РеестрДокументов.Подразделение В (&Подразделение))
	|				И (НЕ &ОтбиратьПоМенеджеру
	|				ИЛИ РеестрДокументов.Ответственный В (&Менеджер)))) КАК Заказы
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ТекстЗапросаСостоянияОтчетовДавальцам2_5(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению)
	...
	Если Не ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат)
	 Или Не ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.ТоварыОрганизаций) Тогда
	...
	|		Остатки.Склад КАК Склад,
	|		Остатки.Серия КАК Серия,
	|		(Остатки.КОформлениюКоличествоОборот -
	|			Остатки.ОформленоКоличествоОборот) КАК КОформлению
	|	ИЗ
	|		#РаспоряженияНаОтгрузкуОборотыРазвернутая КАК Остатки
	|	ГДЕ
	|		(Остатки.КОформлениюКоличествоОборот -
	|			Остатки.ОформленоКоличествоОборот) <> 0
	|		И НЕ Остатки.Распоряжение.Ссылка ЕСТЬ NULL // для отработки RLS
	|		И ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	...
	|				ИЛИ РеестрДокументов.Подразделение В (&Подразделение))
	|				И (НЕ &ОтбиратьПоМенеджеру
	|				ИЛИ РеестрДокументов.Ответственный В (&Менеджер))
	|				И НЕ РеестрДокументов.ДополнительнаяЗапись)
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	ИЗ
	|		&ОстаткиБезЗаказа КАК ОстаткиБезЗаказа) КАК ВложенныйЗапрос
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|			И ВЫРАЗИТЬ(ТоварыОрганизаций.ВидЗапасов.Договор КАК
	|				Справочник.ДоговорыКонтрагентов).ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СДавальцем2_5)
	...
	ПараметрыФормированияТаблицы = РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат.НовыйПараметрыФормированияРазвернутойТаблицы();
	ПараметрыФормированияТаблицы.ТекстУсловия =
		"&УсловиеОтбораОстатков И КодСтроки <> 0";
	ПараметрыФормированияТаблицы.Группировка = "Распоряжение, Номенклатура, Характеристика, КодСтроки, Склад, Серия";
...
Функция ТекстЗапросаСостоянияОтчетовДавальцам2_5ОстаткиБезЗаказа() Экспорт
	...
	ТекстЗапроса =
		...
		|		И ВЫРАЗИТЬ(ВидЗапасов.Договор КАК
		|			Справочник.ДоговорыКонтрагентов).ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СДавальцем2_5)
		|		И АналитикаУчетаНоменклатуры.ТипМестаХранения = ЗНАЧЕНИЕ(Перечисление.ТипыМестХранения.Склад)
		|		И (НЕ &ОтбиратьПоОрганизации
		|		ИЛИ Организация В (&Организация))
		|		И (НЕ &ОтбиратьПоСкладу
		|		ИЛИ АналитикаУчетаНоменклатуры.МестоХранения В (&Склад))
		...
		|	ИЗ
		|		#РаспоряженияНаОтгрузкуОборотыРазвернутая КАК Обороты
	...
	ПараметрыФормированияТаблицы.ТекстУсловия =
		"&УсловиеОтбораОстатков";
	ПараметрыФормированияТаблицы.Группировка = "Распоряжение, Номенклатура, Характеристика, КодСтроки, Склад, Серия";
	ПараметрыФормированияТаблицы.Показатели = "КОформлению";
	ПараметрыФормированияТаблицы.Ресурсы = "Количество";
