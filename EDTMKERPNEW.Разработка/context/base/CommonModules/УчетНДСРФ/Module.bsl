...
Процедура ОписаниеЛимитовПримененияСтавокНДС(Элементы, ДатаИзменения) Экспорт
	...
	Если Элементы.ПрочиеСтавкиНДС.СписокВыбора.Количество() Тогда
		СтрокаШаблон = НСтр("ru = 'Применяются общие ставки НДС (%1), применяются все вычеты';
							|en = 'Применяются общие ставки НДС (%1), применяются все вычеты'") + Символы.ПС;
...
Процедура УстановкаСтавкиНДС22() Экспорт
	...
	ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
		...
		НСтр("ru = 'Начат процесс замены Ставки НДС 20% на 22% с 01.01.2026г.';
			|en = 'The process of replacing the 20% VAT rate with 22% is started since 01/01/2026.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), 
		...
		НСтр("ru = 'Закончено выполнение регламентной операции Замена ставки НДС 20% на 22% с 01.01.2026г.';
			|en = 'The ""Replace 20% VAT rate with 22% since 01/01/2026"" period-end transaction is completed.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция СчетаФактурыВыданныеНаОсновании(ПараметрыРегистрации) Экспорт
	...
	Если ПараметрыРегистрации.ПереходПраваСобственности Тогда
		...
		ТекстСчетФактура = НСтр("ru = 'Выставить ИСФ/КСФ по отгрузкам';
								|en = 'Issue replacement/corrective tax invoices for shipments'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура АктуализироватьСчетаФактурыВыданныеПриУдаленииПроведения(ПараметрыРегистрации) Экспорт
	...
	РегистрыСведений.ТребуетсяОформлениеСчетаФактуры.ОбновитьСостояние(ПараметрыРегистрации.Ссылка, ОснованияСчетаФактуры, Истина);
	//++ НЕ УТ
	РегистрыСведений.ТаможенныеДекларацииЭкспортКРегистрации.ОбновитьСостояние(ОснованияСчетаФактуры, Истина);
	//-- НЕ УТ
...
Процедура ДобавитьЭтап_ОформлениеСчетовФактурИТаможенныхДокументов(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Оформление счетов-фактур и таможенных документов';
											|en = 'Register tax invoices and customs documents'"; // @НСтр-1
	...
	Для Каждого Результат Из РезультатыПоОрганизациям Цикл
		...
		Если Результат.ЕстьОбязательныеОперацииКВыполнению Тогда     
			...
			ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'По организации ""%1"" требуется оформление документов по учету НДС';
					|en = 'Registration of documents for VAT accounting for the %1 company is required'", ОбщегоНазначения.КодОсновногоЯзыка()), 
...
Процедура ДобавитьЭтап_ПеречислениеНДСВБюджет(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ПеречислениеНДСВБюджетПоОтдельнымОперациям,
		Ложь, Истина, Ложь,
		Справочники.ОперацииЗакрытияМесяца.ОформлениеСчетовФактурИТаможенныхДокументов);
		
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Перечисление НДС в бюджет по отдельным операциям';
											|en = 'VAT transfer to the budget by certain transactions'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
...
Процедура ЗафиксироватьРезультат(ПараметрыРасчета) Экспорт
	...
	|		ПО РаспределениеНДС.Организация = Задания.Организация
	|		И РаспределениеНДС.Дата > КОНЕЦПЕРИОДА(&КонецПериода, МЕСЯЦ)
	|		И РаспределениеНДС.Дата = КОНЕЦПЕРИОДА(РаспределениеНДС.Дата, КВАРТАЛ)
	...
	|				И Т.ДокументПоступленияРасходов = Задания.СчетФактура)";
...
#Область ТекстНДСАвансыПолученные
	...
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НДСАвансыПолученныеСторно.Период КАК Период,
	|	НДСАвансыПолученныеСторно.Регистратор КАК Документ,
	|	НДСАвансыПолученныеСторно.Организация КАК Организация,
	|	НАЧАЛОПЕРИОДА(НДСАвансыПолученныеСторно.Период, ДЕНЬ) КАК ДатаОтражения
	|ИЗ
...
#Область ТекстНДСАвансыВыданные
	...
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НДСАвансыВыданныеПереносы.Период КАК Период,
	|	НДСАвансыВыданныеПереносы.Регистратор КАК Документ,
	|	НДСАвансыВыданныеПереносы.Организация КАК Организация,
	|	НАЧАЛОПЕРИОДА(НДСАвансыВыданныеПереносы.Период, ДЕНЬ) КАК ДатаОтражения
	|ИЗ
	|	НДСАвансыВыданныеПереносыРасход КАК НДСАвансыВыданныеПереносы
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	НАЧАЛОПЕРИОДА(НДСАвансыВыданныеПереносы.Период, ДЕНЬ) КАК ДатаОтражения
	|ИЗ
	|	НДСАвансыВыданныеСторноЗачетов КАК НДСАвансыВыданныеПереносы
	|";
...
#КонецОбласти
	...
	Если ПараметрыРасчета.РасчетАвансовВыданных Тогда
		...
		|	НАЧАЛОПЕРИОДА(НДСАвансыВыданныеПереносы.Период, ДЕНЬ) КАК ДатаОтражения
		|ИЗ
		|	НДСАвансыВыданныеПереносыПриход КАК НДСАвансыВыданныеПереносы
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	НДСАвансыВыданныеСторно.Период КАК Период,
		|	НДСАвансыВыданныеСторно.Регистратор КАК Документ,
		|	НДСАвансыВыданныеСторно.Организация КАК Организация,
		|	НАЧАЛОПЕРИОДА(НДСАвансыВыданныеСторно.Период, ДЕНЬ) КАК ДатаОтражения
		|ИЗ
...
Процедура ДобавитьЭтап_ФормированиеСводныхСправокНДС(ТаблицаЭтапов, ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Формирование сводных справок НДС';
											|en = 'Generation of VAT consolidated certificates'"; // @НСтр-1
...
Процедура РассчитатьНДСЗаписиКнигиПокупокПоНДСПредъявленныйПредварительно(ПараметрыРасчета)
	...
	|			ТОГДА """"
	//++ НЕ УТ
	|		КОГДА НДСПредъявленный.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.Космос)
	|			ТОГДА ""1010409""
	//-- НЕ УТ
...
Процедура ЗаполнитьСписокВыбораВидовТранспорта(СписокЗначений) Экспорт
	...
	СписокЗначений.Добавить("32",
		НСтр("ru = '32 - Состав транспортных средств (тягач с прицепом (-ами) и полуприцепом (-ами) )';
			|en = '32 - Состав транспортных средств (тягач с прицепом (-ами) и полуприцепом (-ами) )'"));
...
Процедура ДополнитьИнформациейОРНПТ(Знач ДополнительнаяИнформация, Знач СтруктураПараметров, Запрос, ПоискРНПТПоОснованию = Ложь) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ВТ_СопоставлениеСФОснований.Исправление
	|			ИЛИ ВТ_СопоставлениеСФОснований.Корректировочный
	|			И ВЫРАЗИТЬ(ВТ_СопоставлениеСФОснований.СчетФактура КАК Документ.СчетФактураВыданный).НачислениеНДСПриОтгрузке ТОГДА
