...
Функция ПолучитьДатыПериодаДокумента(Ссылка) Экспорт
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЗаявлениеОВвозеТоваров") Тогда
		...
		Возврат ПериодОтчета;
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ДокументацияПоКонтролируемымСделкам") Тогда
...
Функция ВыгрузитьДокумент(Ссылка, УникальныйИдентификатор = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ДокументацияПоКонтролируемымСделкам") Тогда
		...
		Для каждого ДанныеФайла Из ДанныеФайлов Цикл
			...
			МассивФайловДляОтправки.Добавить(СвойстваФайлаДляОтправки);
		КонецЦикла;
		Возврат МассивФайловДляОтправки;
	Иначе
...
Функция ПолучитьКодИФНСПолучателяПоСсылке(ОбъектСсылка) Экспорт
	...
	ИначеЕсли ТипЗнч(ОбъектСсылка) = Тип("ДокументСсылка.ДокументацияПоКонтролируемымСделкам") Тогда
		Возврат "0023";
	КонецЕсли;
