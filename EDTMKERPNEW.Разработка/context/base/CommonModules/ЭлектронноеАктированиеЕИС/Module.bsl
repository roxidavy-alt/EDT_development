...
Функция НовоеЗначениеОКЕИ() Экспорт
	...
	Описание = НСтр("ru = 'Национальное условное обозначение (поле localSymbol в справочнике ОКЕИ (nsiOKEI)).';
					|en = 'National designation (field localSymbol in RNCMU catalog (nsiOKEI)).'");
...
Функция НовыеСведенияОИныхУполномоченныхЛицахЗаказчика() Экспорт
	...
	ТехническоеОписание =
		НСтр("ru = 'Контролируется, что в УПД (титул покупателя) присутствует подписант с указанным номером';
			|en = 'It is controlled that UTD (customer title) contains the signer with the specified number'");
...
Функция ПолучитьДатуИзвещенияИзОткрытойЧастиЕИСНаСервере(НомерИзвещения)
	...
	Если НЕ РезультатЗагрузки.Статус Тогда
		ОписаниеОшибки = НСтр("ru = 'Электронный документооборот с ЕИС. При загрузке извещения с сервера ЕИС возникла ошибка:';
								|en = 'EDI with the Unified Information System. An error occurred when importing the notification from the Unified Information System server:'", ОбщегоНазначения.КодОсновногоЯзыка())
