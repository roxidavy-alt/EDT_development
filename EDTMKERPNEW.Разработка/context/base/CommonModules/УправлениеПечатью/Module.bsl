...
Функция СписокОператоров(ДополнительныеПоля = Неопределено) Экспорт
	...
	Иначе
		...
		ДобавитьОператорВГруппу(Группа, ИмяМетода, НСтр("ru = 'Если пустое';
														|en = 'If empty'"),, Истина);
		ИмяМетода =  Префикс + "ЧислоПрописьюПоПараметрам";
		ДобавитьОператорВГруппу(Группа, ИмяМетода, НСтр("ru = 'Число прописью';
														|en = 'Amount in words'"), ТипСтрока, Истина);
	...
	Если ЭтоДокумент Тогда
		...
		Если ДокументСодержитНомер Тогда
			...
			Шаблон = НСтр("ru = '[НазваниеПечатнойФормы] № [Номер] от [Дата]';
							|en = '[НазваниеПечатнойФормы] # [Номер] dated [Дата]'");
		Иначе
