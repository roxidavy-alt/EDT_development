...
Функция ЗапроситьПодтверждениеИзмененияОперацииПриЗакрытииКассовойСмены(Форма, ОповещениеОЗавершении) Экспорт
	...
	Если ОперацияДоИзменения = ПредопределенноеЗначение("Перечисление.ОперацииПриЗакрытииКассовойСмены.Нет")
		...
		ТекстВопроса = НСтр("ru = 'Текущий режим работы не является рекомендуемым и более не поддерживается.
						...
						| After the modification of the current mode, a rollback to the previous setting will become unavailable. Continue?'");
...
Процедура СообщитьОбОшибкеПриПечатиЧека(Форма, РезультатВыполнения, ЗаписатьВЖурналРегистрации = Истина) Экспорт
	Если Форма.ИмяФормы = РозничныеПродажиКлиентСервер.ИменаФормРМК().Найти(Форма.ИмяФормы) <> Неопределено Тогда
		...
		ТекстСообщения = НСтр("ru = 'При печати чека произошла ошибка. Чек не напечатан на фискальном устройстве.
									...
									|en = 'An error occurred when printing a receipt. The receipt is not printed on a fiscal device.
		...
		Если ГиперссылкаПоказатьПолностью Тогда
			СтрокаПоказатьПолностью = Новый ФорматированнаяСтрока(НСтр("ru = '(показать полностью)';
																		|en = '(show in full)'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),,,, "ПоказатьПолностьюОшибкуПриПечатиЧека");
