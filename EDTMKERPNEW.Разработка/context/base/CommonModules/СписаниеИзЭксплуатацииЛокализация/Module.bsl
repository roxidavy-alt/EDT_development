...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	|	""СписаниеИзЭксплуатацииЛокализация_1291"" КАК НомерФрагмента
	|	
	|ИЗ
...
Функция ПолучитьДанныеДляПечатнойФормыМБ8(МассивДокументов)
	...
	СписокВидовСчетов = РегистрыСведений.ПорядокОтраженияНаСчетахУчета.СписокСчетовУчетаПоРазделуУчета("ТМЦВЭксплуатации");
	МассивВидовСчетов = ОбщегоНазначенияКлиентСервер.ОтмеченныеЭлементы(СписокВидовСчетов);
	ТекстЗапроса = РегистрыСведений.ПорядокОтраженияНаСчетахУчета.ТекстВременнойТаблицыПорядкаОтраженияПоПериодуДействия(МассивВидовСчетов, "ТМЦВЭксплуатации");
	МассивТекстовЗапроса.Добавить(ТекстЗапроса);
	
	МассивВидовСчетов = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("Расходы");
	ТекстЗапроса = РегистрыСведений.ПорядокОтраженияНаСчетахУчета.ТекстВременнойТаблицыПорядкаОтраженияПоПериодуДействия(МассивВидовСчетов, "СчетаРасходов");
	МассивТекстовЗапроса.Добавить(ТекстЗапроса);
	...
	|	Товары КАК Товары
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТМЦВЭксплуатации КАК НастройкиПоОрганизации
	...
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТМЦВЭксплуатации КАК НастройкиПоАналитике
	...
	|	Товары КАК Товары
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ СчетаРасходов КАК НастройкиПоМестуУчета
	|	ПО НастройкиПоМестуУчета.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.Расходы)
	...
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ СчетаРасходов КАК НастройкиПоОрганизации
	|	ПО НастройкиПоОрганизации.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.Расходы)
	...
	|		И Товары.Период < НастройкиПоОрганизации.ОкончаниеДействия
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ СчетаРасходов КАК НастройкиПоАналитике
	|	ПО НастройкиПоАналитике.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.Расходы)
	|		И НастройкиПоАналитике.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
