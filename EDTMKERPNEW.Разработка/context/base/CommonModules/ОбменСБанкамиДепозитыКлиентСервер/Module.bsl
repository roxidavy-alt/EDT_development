...
Функция ПредставлениеСуммы(Сумма) Экспорт
	...
	Если Сумма >= СуммаМиллиард Тогда
		...
		Возврат СтрШаблон(НСтр("ru = '%1 млрд';
								|en = '%1 млрд'"), ПредставлениеСуммы);
	ИначеЕсли Сумма >= СуммаМиллион Тогда
		ПредставлениеСуммы = Окр(Сумма / СуммаМиллион, Разрядность);
		Возврат СтрШаблон(НСтр("ru = '%1 млн';
								|en = '%1 млн'"), ПредставлениеСуммы);
	ИначеЕсли Сумма >= СуммаТысяча Тогда
		ПредставлениеСуммы = Окр(Сумма / СуммаТысяча, Разрядность);
		Возврат СтрШаблон(НСтр("ru = '%1 тыс.';
								|en = '%1 thousand'"), ПредставлениеСуммы);
	Иначе
...
Функция СформироватьСтрокуИнформацииПоУсловию(ПараметрыПродукта) Экспорт
	...
	Если СуммаМинимум > 0 И (СуммаМаксимум = 0 Или СуммаМаксимум = ПределСуммы) Тогда
		...
		ПодстрокаСумма = СтрШаблон(НСтр("ru = 'от %1 %2';
										|en = 'dated %1 %2'"), СуммаСтрока, Валюта);
	ИначеЕсли СуммаМинимум = 0 Тогда
		СуммаСтрока = ПредставлениеСуммы(СуммаМаксимум);
		ПодстрокаСумма = СтрШаблон(НСтр("ru = 'до %1 %2';
										|en = 'до %1 %2'"), СуммаСтрока, Валюта);
	Иначе
		...
		Если МинСуммаСтрока = МаксСуммаСтрока Тогда
			ПодстрокаСумма = СтрШаблон(НСтр("ru = '%1 %2';
											|en = '%1 %2'"), МинСуммаСтрока, Валюта);
		Иначе
			ПодстрокаСумма = СтрШаблон(НСтр("ru = 'от %1 - до %2 %3';
											|en = 'от %1 - до %2 %3'"), МинСуммаСтрока, МаксСуммаСтрока, 
				Валюта);
		КонецЕсли;
	...
	Если СрокМинимум = СрокМаксимум Тогда
		ПодстрокаСрок = СтрШаблон(НСтр("ru = '%1 дн.';
										|en = '%1 days'"), СрокМинимум);
	ИначеЕсли СрокМинимум > 0 И (СрокМаксимум = 0 Или СрокМаксимум = ПределСрока) Тогда
		ПодстрокаСрок = СтрШаблон(НСтр("ru = 'от %1 дн.';
										|en = 'от %1 дн.'"), СрокМинимум);
	...
	ИначеЕсли СрокМинимум = 0 Тогда
		ПодстрокаСрок = СтрШаблон(НСтр("ru = 'до %1 дн.';
										|en = 'до %1 дн.'"), СрокМаксимум);
	Иначе
		ПодстрокаСрок = СтрШаблон(НСтр("ru = 'от %1 - %2 дн.';
										|en = 'от %1 - %2 дн.'"), СрокМинимум, СрокМаксимум);
	КонецЕсли;
...
Функция ТекстСообщенияОбновлениеПредложений() Экспорт
	Возврат НСтр("ru = 'Пожалуйста, подождите, выполняется запрос предложений банков.';
				|en = 'Пожалуйста, подождите, выполняется запрос предложений банков.'");
КонецФункции
...
Функция ТекстСообщенияНетПредложенийБанковДляОрганизации() Экспорт
	Возврат НСтр("ru = 'Для организации нет доступных предложений банков';
				|en = 'Для организации нет доступных предложений банков'");
КонецФункции
...
Функция ТекстСообщенияНеЗаполненаОрганизация() Экспорт
	Возврат НСтр("ru = 'Укажите организацию для подбора предложений банков';
				|en = 'Укажите организацию для подбора предложений банков'");
КонецФункции
