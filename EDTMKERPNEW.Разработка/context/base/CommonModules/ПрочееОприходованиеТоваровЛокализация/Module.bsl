...
#Область ВозвратТоваровДавальцаИзПроизводства // (Дт 003.01/002 :: Кт 003.02)
	...
	|	""ПрочееОприходованиеТоваровЛокализация_1002"" КАК НомерФрагмента
	|
	|ИЗ
...
#Область ВозвратТоваровДавальцаИзПроизводстваСкладПроизводства // (Дт 003.02 :: Кт 003.02)
	...
	|	0 КАК СуммаВРКт,
	|	""Возврат товаров давальца из производства"" КАК Содержание,
	|	""ПрочееОприходованиеТоваровЛокализация_1003"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстОприходованиеЗаСчетДоходов()
	...
	|	""ПрочееОприходованиеТоваровЛокализация_1004"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	ПО
	|		СтатьиДоходов.Ссылка = Строки.СтатьяРасходовДоходов
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	...
	|	0 КАК СуммаВРКт,
	|	""Оприходование за счет доходов"" КАК Содержание,
	|	""ПрочееОприходованиеТоваровЛокализация_1005"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ОприходованиеЗаСчетРасходов // (Дт 10.01 :: Кт СтатьяРасходов)
	...
	|	""ПрочееОприходованиеТоваровЛокализация_1006"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ОприходованиеЗаСчетРасходовСкладПроизводства // (Дт 20 :: Кт СтатьяРасходов)
	...
	|	0 КАК СуммаВРКт,
	|	""Оприходование за счет расходов"" КАК Содержание,
	|	""ПрочееОприходованиеТоваровЛокализация_1007"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ОприходованиеЗаСчетДоходовПоСтатьямАктивовПассивов // (Дт 10.01 :: Кт ХХ.ХХ)
	...
	|	""ПрочееОприходованиеТоваровЛокализация_1008"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ОприходованиеЗаСчетДоходовПоСтатьямАктивовПассивовСкладПроизводства // (Дт 20 :: Кт ХХ.ХХ)
	...
	|	0 КАК СуммаВРКт,
	|	""Прочее поступление товаров по статьям активов/пассивов"" КАК Содержание,
	|	""ПрочееОприходованиеТоваровЛокализация_1009"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	ПО
	|		Операция.Ссылка = ДвиженияПоПрочимАктивамПассивам.Регистратор
	|		И Операция.Организация = ДвиженияПоПрочимАктивамПассивам.Организация
	|		И Операция.Дата = ДвиженияПоПрочимАктивамПассивам.Период
...
#Область СторноСписанияКомиссионныхДавальческихТоваров // (Дт  :: Кт 002.01, 003.01, 003.02, 004.01)
	...
	|	""ПрочееОприходованиеТоваровЛокализация_1821"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстСписаниеТоваровНаРасходыСторно()
	...
	|	""ПрочееОприходованиеТоваровЛокализация_1010"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ВключениеИсключениеНДСВСтоимостьТоваровСторно // (Дт 25, 26, 44 :: Кт 19.03)
	...
	|	""ПрочееОприходованиеТоваровЛокализация_1011"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ВключениеИсключениеНДСВСтоимостьПрочихАктивовСторно // (Дт 08 :: Кт 19.03)
	...
	|	""ПрочееОприходованиеТоваровЛокализация_1012"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ВозвратТоваровНаСкладИзПроизводства // (Дт 20.01 :: Кт 10.01 сторно)
	...
	|	""ПрочееОприходованиеТоваровЛокализация_1013"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ВозвратТоваровНаСкладИзПроизводстваСкладПроизводства // (Дт 20.01 :: Кт 20 сторно)
	...
	|	-ЕСТЬNULL(Стоимости.СуммаВР, Строки.СуммаВР) КАК СуммаВРКт,
	|	""Возврат товаров на склад из производства"" КАК Содержание,
	|	""ПрочееОприходованиеТоваровЛокализация_1014"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция СписаниеИзЭксплуатацииЗабалансовыйУчет()
	...
	|	""ПрочееОприходованиеТоваровЛокализация_1015"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстОприходованиеПриВыбытииОС()
	...
	|	""ПрочееОприходованиеТоваровЛокализация_1016"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|
	|	""Оприходование (при выбытии ОС)"" КАК Содержание,
	|	""ПрочееОприходованиеТоваровЛокализация_1017"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция КорректировкаСтоимостиТМЦОприходованныхПриВыбытииОС()
	...
	|	""ПрочееОприходованиеТоваровЛокализация_1018"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|
	|	""Корректировка стоимости"" КАК Содержание,
	|	""ПрочееОприходованиеТоваровЛокализация_1019"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция КорректировкаРасходовОтВыбытияОС()
	...
	|	""ПрочееОприходованиеТоваровЛокализация_1020"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
