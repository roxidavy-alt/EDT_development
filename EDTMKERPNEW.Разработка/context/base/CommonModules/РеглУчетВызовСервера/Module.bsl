...
Функция ПараметрыРасшифровки(АналитикаСтроки) Экспорт
	...
	ПоляОтбора.Вставить(Новый ПолеКомпоновкиДанных("Аналитика1")	, АналитикаСтроки.Аналитика1);
	ПоляОтбора.Вставить(Новый ПолеКомпоновкиДанных("Аналитика2")	, АналитикаСтроки.Аналитика2);
...
// Получает аналитику строки отчета СверкаДанныхОУиБУ.
// 
// Параметры:
//  ДанныеРасшифровки - ДанныеРасшифровкиКомпоновкиДанных
//  ИндексТекущегоЭлемента - ИдентификаторРасшифровкиКомпоновкиДанных
// 
// Возвращаемое значение:
//  Структура
Функция ПолучитьАналитикуСтрокиСверки(ДанныеРасшифровки, ИндексТекущегоЭлемента)
	
	НаименованияПолей = "НачалоПериода, КонецПериода,
	| РазделУчета, СчетБУ, Организация, Подразделение, Аналитика1, Аналитика2";
	АналитикаСтрокиСверки = Новый Структура(НаименованияПолей);
	
	ОтчетыУТВызовСервераПереопределяемый.ПолучитьЗначенияПолей(
		АналитикаСтрокиСверки, ДанныеРасшифровки.Элементы, ИндексТекущегоЭлемента);
