...
#Область СписаниеВПроизводство // (Дт 08, 20, 2Х, 44, 91.02 :: Кт 10.07, 20)
	...
	|	""ОтчетПереработчика25Локализация_719"" КАК НомерФрагмента
	|
	|ИЗ
...
#Область ВыпускПродукцииКоличество // (Дт 10.07, 2Х :: Кт 2Х)
	...
	|
	|	0 КАК ВалютнаяСуммаДт,
	|	МАКСИМУМ(Строки.Количество) КАК КоличествоДт,
	|	0 КАК СуммаНУДт,
	|	0 КАК СуммаПРДт,
	|	0 КАК СуммаВРДт,
	|
	...
	|	МАКСИМУМ(ЕСТЬNULL(КорСтроки.Номенклатура, Строки.Номенклатура)) КАК СубконтоКт2,
	...
	|	""ОтчетПереработчика25Локализация_720"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|		И КорСтроки.Организация = Строки.Организация
	...
	|		И КорСтроки.КорРазделУчета = Строки.РазделУчета
	|		И КорСтроки.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|		И КорСтроки.ТипЗатрат = ЗНАЧЕНИЕ(Перечисление.ТипыЗатратРегл.МатериальныеЗатраты)
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		УчетнаяПолитикаОрганизаций КАК УчетнаяПолитикаОрганизаций
	|	ПО
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.МатериалыИРаботыВПодразделении)
	|	КОНЕЦ,
	|	ЕСТЬNULL(ГруппыПродукции.ГруппаПродукции, Строки.Номенклатура.ГруппаАналитическогоУчета),
	|	ВЫБОР
	|		КОГДА Строки.Склад ССЫЛКА Справочник.ДоговорыКонтрагентов
	|			ТОГДА Строки.Склад.Партнер
	|		ИНАЧЕ Строки.ПодразделениеАналитики
...
#Область ВыпускПродукцииПлан // (Дт 10.07, 2Х :: Кт 2Х)
	...
	|	""Выпуск продукции"" КАК Содержание,
	|	""ОтчетПереработчика25Локализация_721"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
#Область ВыпускПродукцииСтоимость // (Дт 10.07, 2Х :: Кт 2Х)
	...
	|	Стоимости.СуммаВР КАК СуммаВРКт,
	|	""Выпуск продукции"" КАК Содержание,
	|	""ОтчетПереработчика25Локализация_722"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
#Область ВыпускПродукцииНематериальныеЗатраты // (Дт 10.07, 2Х :: Кт 2Х)
	...
	|	""Выпуск продукции"" КАК Содержание,
	|	""ОтчетПереработчика25Локализация_723"" КАК НомерФрагмента
	|ИЗ
...
#Область ЗакупкаУслуги // (Дт 08, 2Х, 44, 91 :: Кт 60, 71, 76)
	...
	|	""ОтчетПереработчика25Локализация_724"" КАК НомерФрагмента
	|
	|ИЗ 
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ЗакупкаНДС // (Дт 19 :: Кт 60, 71, 76)
	...
	|	""ОтчетПереработчика25Локализация_725"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ВключениеИсключениеНДСВСтоимостьТоваров // (Дт 20, 25, 26, 44 :: Кт 19.03)
	...
	|		КОГДА Партии.КорРазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.МатериалыИРаботыВПодразделении)
	|		КОГДА Партии.КорРазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НезавершенноеПроизводство)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НезавершенноеПроизводство)
	...
	|	""ОтчетПереработчика25Локализация_726"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстВыпускВозвратныхОтходов // (Дт 10.07, 20 :: Кт 2Х)
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(КорСтроки.РазделУчета, ЛОЖЬ) = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)
	|			ИЛИ ЕСТЬNULL(КорСтроки.РазделУчета, НЕОПРЕДЕЛЕНО) В (&ЗабалансовыеРазделыУчета)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.МатериалыИРаботыВПодразделении)
	|		КОГДА ЕСТЬNULL(КорСтроки.РазделУчета, ЛОЖЬ) = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НезавершенноеПроизводство)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НезавершенноеПроизводство)
	|		КОГДА УчетнаяПолитикаОрганизаций.УчетПродукцииПереработчикаНа20Счете
	|			И Строки.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПродукцияПереработчика)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.МатериалыИРаботыВПодразделении)
	...
	|	""ОтчетПереработчика25Локализация_727"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
