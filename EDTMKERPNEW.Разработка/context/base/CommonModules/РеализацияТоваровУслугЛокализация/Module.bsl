...
Функция СформироватьПечатнуюФормуРеестрНомеровГТД(МассивОбъектов, ОбъектыПечати)
	...
	Пока ВыборкаПоДокументамПереходПраваСобственности.Следующий() Цикл
		...
		ТекстСообщения = НСтр("ru = 'В документе %1 хозяйственная операция %2. Печать реестра номеров ГТД не требуется.';
								|en = 'The ""%1"" document has the ""%2"" business transaction. Printing a CCD number registry is not required.'");
...
Функция ПолучитьДанныеДляПечатнойФормыМ15(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	Пока ВыборкаОшибки.Следующий() Цикл
		...
		ТекстСообщения = НСтр("ru = 'В документе %1 хозяйственная операция %2. Печать М-15 не требуется.';
								|en = 'The ""%1"" document has the ""%2"" business transaction. Printing M-15 is not required.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстСообщения,
...
Функция ПолучитьДанныеДляПечатнойФормыУПД(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	Пока ВыборкаОшибки.Следующий() Цикл
		...
		ТекстСообщения = НСтр("ru = 'В документе %1 хозяйственная операция %2. Печать универсального передаточного документа (УПД) не требуется.';
								|en = 'The ""%1"" document has the ""%2"" business transaction. Printing a universal transfer document (UTD) is not required.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстСообщения,
				ВыборкаОшибки.ПредставлениеСсылки,
				ВыборкаОшибки.ПредставлениеХозяйственнойОперации);
...
Процедура ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	|	СУММА(ТаблицаДокумента.СуммаАкциза)                            КАК СуммаАкциза,
	//-- НЕ УТ
	|	
	|	ВЫБОР
...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	ТекстыОтражения.Добавить(ТекстВыручкаОтРеализацииСобственногоТовараТолькоРеглУчет());
	ТекстыОтражения.Добавить(ТекстВыручкаОтРеализацииРаботУслугТолькоРеглУчет());
	...
	ТекстыОтражения.Добавить(ТекстСебестоимостьОтгруженногоКомиссионногоТовара());
	ТекстыОтражения.Добавить(ТекстСебестоимостьРеализованногоТовараТолькоРеглУчет());
	ТекстыОтражения.Добавить(ТекстСебестоимостьРеализованныхРаботТолькоРеглУчет());
	ТекстыОтражения.Добавить(ТекстНДССРеализации());
	ТекстыОтражения.Добавить(ТекстНДССРеализацииТолькоРеглУчет());
	ТекстыОтражения.Добавить(ТекстНДССРеализацииРаботыУслугиТолькоРеглУчет());
	...
	ТекстыОтражения.Добавить(ТекстНачислениеАкцизов());
	ТекстыОтражения.Добавить(ТекстНачислениеАкцизовТолькоРеглУчет());
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	|			И Операция.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.Отгружено)
	|		ИЛИ Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности))
	|	И (
...
Функция ТекстПередачаНаСубкомиссию()
	...
	|	""РеализацияТоваровУслугЛокализация_1153"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область РеализацияБезПереходаПраваСобственности // (Дт 45.01 :: Кт 41.01, 45.01)
	...
	|	""РеализацияТоваровУслугЛокализация_1154"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область РеализацияБезПереходаПраваСобственностиСкладПроизводства // (Дт 45.01 :: Кт 20)
	...
	|	ЕСТЬNULL(Стоимости.СуммаВР, 0) КАК СуммаВРКт,
	|	""Отгрузка без перехода права собственности"" КАК Содержание,
	|	""РеализацияТоваровУслугЛокализация_1155"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстВыручкаОтРеализацииКомиссионногоТовара()
	...
	|	""РеализацияТоваровУслугЛокализация_1156"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстПродажаКомиссионногоТовара()
	...
	|
	|	ВЫБОР
	|		КОГДА Строки.КорРазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПринятыеНаКомиссиюВПути)
	...
	|	""РеализацияТоваровУслугЛокализация_1157"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстВыручкаОтРеализации()
	...
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаДт,
	|
	|	ЕСТЬNULL(РасчетыПоЗаказам.Валюта, Расчеты.Валюта) КАК ВалютаДт,
	|	ЕСТЬNULL(РасчетыПоЗаказам.Подразделение, Расчеты.Подразделение) КАК ПодразделениеДт,
	|	ЕСТЬNULL(РасчетыПоЗаказам.НаправлениеДеятельности, Расчеты.НаправлениеДеятельности) КАК НаправлениеДеятельностиДт,
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетДт,
	...
	|	""РеализацияТоваровУслугЛокализация_2032"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|		ВТРасчетыСКлиентамиПоЗаказам КАК РасчетыПоЗаказам
	|	ПО 
	|		СтрокиКт.Ссылка = РасчетыПоЗаказам.Ссылка
	|		И СтрокиКт.ЗаказКлиента = РасчетыПоЗаказам.Заказ
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ВТРасчетыСКлиентамиПоЗаказам КАК Расчеты
	|	ПО 
	|		СтрокиКт.Ссылка = Расчеты.Ссылка
	|		И Расчеты.Заказ = НЕОПРЕДЕЛЕНО
	|		И Расчеты.СуммаРегл <> 0
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
...
Функция ТекстВыручкаОтРеализацииСобственногоТовараТолькоРеглУчет()
	
	ТекстВыручкаОтРеализацииСобственногоТовараТолькоРеглУчет = "
	...
	|		КОГДА НЕ ТаблицаНалогообложенияПрибыли.Организация ЕСТЬ NULL
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.ВыручкаРаздельныйНУ)
	|		КОГДА Операция.НалогообложениеНДС В (ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД),
	...
	|	0 КАК ВалютнаяСуммаКт,
	|	Строки.Количество КАК КоличествоКт,
	|	ЕСТЬNULL(Суммы.СуммаБезНДСРегл - Строки.СуммаАкциза, Строки.СуммаСНДС - Строки.СуммаНДС - Строки.СуммаАкциза) КАК СуммаНУКт,
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Выручка от реализации собственного товара"" КАК Содержание,
	|	""РеализацияТоваровУслугЛокализация_1158"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|ГДЕ
	|	Строки.ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
	|	И Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиентуРеглУчет)
	|	И Операция.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.КПредоплате)
	|	И (
	|		ВЫБОР КОГДА Операция.ВернутьМногооборотнуюТару ТОГДА
	|			Строки.АналитикаУчетаНоменклатуры.Номенклатура.ТипНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
	...
	|		КОНЕЦ)
	|";
	Возврат ТекстВыручкаОтРеализацииСобственногоТовараТолькоРеглУчет;
	
КонецФункции

Функция ТекстВыручкаОтРеализацииРаботУслугТолькоРеглУчет()
	
	ТекстВыручкаОтРеализацииРаботУслугТолькоРеглУчет = "
	...
	|	""РеализацияТоваровУслугЛокализация_1159"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|
	|ГДЕ
	|	Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиентуРеглУчет)
	|	И Операция.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.КПредоплате)
	|	И Строки.Номенклатура.ТипНоменклатуры НЕ В (
	|		ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
	...
	|	0 КАК СуммаВРКт,
	|	""Выручка от реализации работ и услуг"" КАК Содержание,
	|	""РеализацияТоваровУслугЛокализация_1160"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|";
	Возврат ТекстВыручкаОтРеализацииРаботУслугТолькоРеглУчет;
	
КонецФункции

Функция ТекстСебестоимостьОтгруженногоКомиссионногоТовара()
	
	ТекстСебестоимостьОтгруженногоКомиссионногоТовара = "
	|ВЫБРАТЬ //// Себестоимость реализованного комиссионного товара (Дт  :: Кт 004)
	|
	...
	|	0 КАК СуммаВРДт,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.ТоварыПереданныеНаКомиссиюВПути) КАК ВидСчетаКт,
	|	Строки.ГруппаФинансовогоУчета АналитикаУчетаКт,
	|	Строки.Склад КАК МестоУчетаКт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаКт,
	...
	|	0 КАК ВалютнаяСуммаКт,
	|	Строки.Количество КАК КоличествоКт,
	|	ЕСТЬNULL(Стоимости.СуммаЗабалансовая, Строки.СуммаЗабалансовая) КАК СуммаНУКт,
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Себестоимость реализованного комиссионного товара"" КАК Содержание,
	|	""РеализацияТоваровУслугЛокализация_1910"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|		И Строки.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|		И Строки.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПринятыеНаКомиссиюВПути)
	|
	...
	Возврат ТекстСебестоимостьОтгруженногоКомиссионногоТовара;
	
КонецФункции

Функция ТекстСебестоимостьРеализованногоТовараТолькоРеглУчет()
...
#Область СебестоимостьРеализованногоТовара // (Дт 90.02.1 :: Кт 41.01, 45.01)
	...
	|	""РеализацияТоваровУслугЛокализация_1162"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ВтВыручкаИСебестоимостьПродаж КАК Выручка
	|	ПО
	|		Стоимости.Ссылка = Выручка.Ссылка
	|		И Стоимости.НастройкаХозяйственнойОперации = Выручка.НастройкаХозОперации
	|		И Стоимости.ИдентификаторФинЗаписи = Выручка.ИдентификаторФинЗаписи
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|		РаздельныйУчет_НастройкиНалогообложенияПрибыли КАК ТаблицаНалогообложенияПрибыли
	|	ПО 
	...
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиенту),
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияЧерезКомиссионера))
	|			И НЕ Стоимости.Ссылка ЕСТЬ NULL И Выручка.Ссылка ЕСТЬ NULL)
	|	И (
	|		ВЫБОР КОГДА Операция.ВернутьМногооборотнуюТару ТОГДА
	|			Строки.Номенклатура.ТипНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
	|		ИНАЧЕ
...
#Область СебестоимостьРеализованногоТовараСкладПроизводства // (Дт 90.02.1 :: Кт 20)
	...
	|	ЕСТЬNULL(Стоимости.СуммаВР, Строки.СуммаВР) КАК СуммаВРКт,
	|	""Себестоимость реализованного товара"" КАК Содержание,
	|	""РеализацияТоваровУслугЛокализация_1163"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|	И Строки.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)
	|	И (Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиентуРеглУчет)
	|		ИЛИ Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиенту)
	|			И НЕ Стоимости.Ссылка ЕСТЬ NULL И Выручка.Ссылка ЕСТЬ NULL)
	|	И (
	|		ВЫБОР КОГДА Операция.ВернутьМногооборотнуюТару ТОГДА
	|			Строки.Номенклатура.ТипНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
...
Функция ТекстСебестоимостьРеализованныхРаботТолькоРеглУчет()
	
	ТекстСебестоимостьРеализованныхРаботТолькоРеглУчет = "
	...
	|	""РеализацияТоваровУслугЛокализация_1164"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|			И НЕ Стоимости.Ссылка ЕСТЬ NULL И Выручка.Ссылка ЕСТЬ NULL)
	|";
	Возврат ТекстСебестоимостьРеализованныхРаботТолькоРеглУчет;
...
Функция ТекстНДССРеализации()
	...
	|	0 КАК СуммаВРКт,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(СтрокиДт.Ссылка) = ТИП(Документ.РеализацияТоваровУслуг) И СтрокиДт.ТипЗапасов В (
	|			ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Услуга), ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Работа))
	|			ТОГДА ""НДС с реализации услуг""
	|		ИНАЧЕ ""НДС с реализации""
	|	КОНЕЦ КАК Содержание,
	|	""РеализацияТоваровУслугЛокализация_2021"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|
	|	ГДЕ НЕ СтрокиДт.НастройкаХозОперации В (ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.РеализацияБезПереходаПраваСобственности),
	|										ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.РеализацияЧерезКомиссионераБезПереходаПраваСобственности),
	|										ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ПереходПраваСобственности))
	|";
...
Функция ТекстНДССРеализацииТолькоРеглУчет()
	
	ТекстНДССРеализацииТолькоРеглУчет = "
	...
	|	""РеализацияТоваровУслугЛокализация_1165"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|		КОНЕЦ)
	|";
	Возврат ТекстНДССРеализацииТолькоРеглУчет;
	
КонецФункции

Функция ТекстНДССРеализацииРаботыУслугиТолькоРеглУчет()
	
	ТекстНДССРеализацииРаботыУслугиТолькоРеглУчет = "
	|ВЫБРАТЬ //// НДС с реализации (Дт 90.03 :: Кт 68.02)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	""РеализацияТоваровУслугЛокализация_1166"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""НДС с реализации услуг"" КАК Содержание,
	|	""РеализацияТоваровУслугЛокализация_1167"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	И Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиентуРеглУчет)
	|";
	Возврат ТекстНДССРеализацииРаботыУслугиТолькоРеглУчет;
...
Функция ТекстНДССОтгрузкиБезПереходаПраваСобственности()
	...
	|	""РеализацияТоваровУслугЛокализация_1168"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстНДССРеализацииПриПереходеПраваСобственности()
	...
	ТекстЗапроса = "
	|////////////////////////////////////////////////////////////////////////////////////////////////////
	|// НДС с реализации при переходе права собственности (Дт 90.03 :: Кт 76.ОТ)
	|ВЫБРАТЬ
	|	СтрокиДт.Ссылка КАК Ссылка,
	|	СтрокиДт.Период КАК Период,
	|	СтрокиДт.Организация КАК Организация,
	...
	|	СтрокиДт.Склад КАК МестоУчетаКт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаКт,
	|	ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) КАК ПодразделениеКт,
	|	Операция.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетКт,
	|	СтрокиДт.Контрагент КАК СубконтоКт1,
	|	ВЫБОР
	|		КОГДА СтрокиДт.НастройкаХозОперации = ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ПереходПраваСобственности) 
	|			ТОГДА СтрокиДт.ЗаказКлиента
	|		ИНАЧЕ СтрокиДт.Ссылка
	...
	|			ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Услуга), ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Работа))
	|			ТОГДА ""НДС с реализации услуг при переходе права собственности""
	|		ИНАЧЕ ""НДС с реализации при переходе права собственности""
	|	КОНЕЦ КАК Содержание,
	|	""РеализацияТоваровУслугЛокализация_2012"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.ДвиженияПоПрочимАктивамПассивам КАК СтрокиКт
	|	ПО
	...
	|		И ТаблицаНалогообложенияПрибыли.НаправлениеДеятельности = СтрокиДт.НДВыручки
	|
	|	ГДЕ СтрокиДт.НастройкаХозОперации В (ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.РеализацияБезПереходаПраваСобственности),
	|										ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.РеализацияЧерезКомиссионераБезПереходаПраваСобственности),
	|										ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ПереходПраваСобственности))
	|		И (СтрокиДт.СуммаНДСРегл <> 0 ИЛИ СтрокиДт.СуммаНДСУпр <> 0)
	|
	|";
...
Функция ТекстНДССОтгрузкиБезПереходаПраваСобственностиРаботыУслуги()
	...
	|	0 КАК СуммаВРКт,
	|	""НДС с отгрузки без перехода права собственности"" КАК Содержание,
	|	""РеализацияТоваровУслугЛокализация_1170"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""НДС с отгрузки без перехода права собственности"" КАК Содержание,
	|	""РеализацияТоваровУслугЛокализация_1171"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстНулеваяСтавкаНДСОтклонена()
	...
	|	""РеализацияТоваровУслугЛокализация_1174"" КАК НомерФрагмента
	|
	|ИЗ 
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ЗалоговаяСтоимостьПереданнойТары // (Дт 76 :: Кт 41.03)
	...
	|	""РеализацияТоваровУслугЛокализация_1175"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ЗалоговаяСтоимостьПереданнойТарыСкладПроизводства // (Дт 76 :: Кт 20)
	...
	|	0 КАК СуммаВРКт,
	|	""Передача возвратной тары"" КАК Содержание,
	|	""РеализацияТоваровУслугЛокализация_1176"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстВключениеИсключениеНДСВСтоимость()
	...
	|	""РеализацияТоваровУслугЛокализация_1177"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстПродажаАгентскихУслуг()
	...
	|	""РеализацияТоваровУслугЛокализация_1178"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ПередачаНаКомиссию // (Дт 45.01 :: Кт 41.01)
	...
	|	""РеализацияТоваровУслугЛокализация_1179"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ПередачаНаКомиссиюСкладПроизводства // (Дт 45.01 :: Кт 20)
	...
	|	ЕСТЬNULL(Стоимости.СуммаВР, 0) КАК СуммаВРКт,
	|	""Передача на комиссию"" КАК Содержание,
	|	""РеализацияТоваровУслугЛокализация_1180"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
// Функция возвращает текст фрагмента начисления акцизов.
//
// Возвращаемое значение:
//	Строка - сформированный текст запроса.
//
Функция ТекстНачислениеАкцизов() Экспорт
	
	ТекстЗапроса = "
	|////////////////////////////////////////////////////////////////////////////////////////////////////
	|// Начисление акцизов (Дт 90.04 :: Кт 68.03)
	|ВЫБРАТЬ
	...
	|	0 КАК СуммаВРКт,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДокументыКОтражению.Ссылка) В (
	|				ТИП(Документ.РеализацияТоваровУслуг),
	|				ТИП(Документ.ПередачаТоваровМеждуОрганизациями))
	|			ТОГДА ""Начисление акцизов""
	|		ИНАЧЕ ""Корректировка начисления акцизов""
	|	КОНЕЦ КАК Содержание,
	|	""РеализацияТоваровУслугЛокализация_1933"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|		И ТаблицаНалогообложенияПрибыли.НаправлениеДеятельности = СтрокиДт.НаправлениеДеятельностиКонтрагента
	|
	|ГДЕ
	|	СтрокиКт.НастройкаХозяйственнойОперации <> ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.РеализацияКлиентуРеглУчет)
	|";
...
Функция ТекстНачислениеАкцизовТолькоРеглУчет()
	...
	|		ИНАЧЕ ЕСТЬNULL(СтрокиКт.Номенклатура.ГруппаФинансовогоУчета, ЗНАЧЕНИЕ(Справочник.ГруппыФинансовогоУчетаНоменклатуры.ПустаяСсылка))
	|	КОНЕЦ КАК АналитикаУчетаДт,
	|	ЕСТЬNULL(СтрокиДт.АналитикаУчетаНоменклатуры.МестоХранения, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)) КАК МестоУчетаДт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаДт,
	|	Операция.Подразделение КАК ПодразделениеДт,
	|	Операция.НаправлениеДеятельности КАК НаправлениеДеятельностиДт,
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Начисление акцизов"" КАК Содержание,
	|	""РеализацияТоваровУслугЛокализация_1932"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.РеализацияТоваровУслуг.ВидыЗапасов КАК СтрокиДт
	|	ПО
	|		ДокументыКОтражению.Ссылка = СтрокиДт.Ссылка
	|		И (СтрокиДт.СуммаАкциза <> 0)
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.ДвиженияПоАкцизам КАК СтрокиКт
	|	ПО
	|		СтрокиДт.Ссылка = СтрокиКт.Регистратор
	|		И СтрокиДт.ИдентификаторСтроки = СтрокиКт.ИдентификаторФинЗаписи
	|		И СтрокиКт.Активность
	|		И НЕ СтрокиКт.Сторно
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|		И ТаблицаНалогообложенияПрибыли.Подразделение = Операция.Подразделение
	|		И ТаблицаНалогообложенияПрибыли.ОбъектУчета = СтрокиКт.Номенклатура
	|		И ТаблицаНалогообложенияПрибыли.НаправлениеДеятельности = СтрокиКт.НаправлениеДеятельности
	|
	|ГДЕ
	|	Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиентуРеглУчет)
	|";
