...
Функция ВыполнениеКомандыОтменено(Форма, НаименованиеОтчета, Организация, НаименованиеКоманды = "") Экспорт
	...
	ИначеЕсли НаименованиеОтчета = "Форма 2-ТП (отходы)" Тогда 
		Предупреждение = НСтр("ru = 'Формат выгрузки не опубликован, для сдачи отчета следует воспользоваться личным кабинетом природопользователя';
								|en = 'The export format is not published, use the environmental user''s personal account to submit the report'");
		ПоказатьИВернутьОшибку(Форма, Предупреждение);
		Возврат Истина;
...
Процедура ПередЗакрытиемРегламентированногоОтчета(Форма, Отказ, СтандартнаяОбработка, ЗавершениеРаботы, ТекстПредупреждения) Экспорт
	...
	Если Форма.Модифицированность Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("ru = 'Данные были изменены. Сохранить изменения?';
										|en = 'Data has changed. Save the changes?'"), РежимДиалогаВопрос.ДаНетОтмена, , КодВозвратаДиалога.Да);
...
Процедура ПредложитьОформитьЗаявлениеНаПодключениеПриЗаполнении(Форма, Отказ) Экспорт
	...
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ПоказатьФормуПредложениеОформитьЗаявлениеНаПодключениеПриЗаполнении(ПараметрыФормы, Отказ);
КонецПроцедуры
