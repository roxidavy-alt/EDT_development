...
//  
Процедура ЗаполнитьПризнакЦеховаяКладоваяВТаблице(Объект) Экспорт
...
Функция ТекстЗапросаОстаткиТоваровКОформлению(ИмяВременнойТаблицы, ОтборПоИзмерениям = Неопределено) Экспорт
	...
	ТекстыЗапросов.Добавить(
		РегистрыНакопления.ЗаказыПоставщикам.ТекстЗапросаОстатки(
			"ОстаткиЗаказовПоставщиков",
			ОтборПоИзмерениям,
			"КПоступлению > 0"));
	
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ДанныеУчета.ЗаказПоставщику КАК Распоряжение,
	...
	|	ДанныеУчета.Заказано        КАК Заказано,
	|	ДанныеУчета.КПоступлению    КАК КОформлению,
	|	ДанныеУчета.Сумма           КАК Сумма
	|ПОМЕСТИТЬ ИмяТаблицы
	|ИЗ
	|	ОстаткиЗаказовПоставщиков КАК ДанныеУчета
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПереработчику2_5.ВыходныеИзделия КАК ТаблицаПродукции
	|	ПО ТаблицаПродукции.Ссылка = ДанныеУчета.ЗаказПоставщику
	|	И ТаблицаПродукции.КодСтроки = ДанныеУчета.КодСтроки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ОстаткиЗаказовПоставщиков КАК ДанныеУчета
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПереработчику2_5.ПобочныеИзделия КАК ТаблицаПродукции
	|	ПО ТаблицаПродукции.Ссылка = ДанныеУчета.ЗаказПоставщику
	|	И ТаблицаПродукции.КодСтроки = ДанныеУчета.КодСтроки
	//++ НЕ УТКА
	|
	...
	|	ПО РеквизитыЭтапа.ЗаказПереработчику = ДанныеУчета.ЗаказПоставщику
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ВыходныеИзделия КАК ТаблицаПродукции
	|	ПО ТаблицаПродукции.Ссылка = РеквизитыЭтапа.Ссылка
	|	И ТаблицаПродукции.КодСтроки = ДанныеУчета.КодСтроки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Функция ТекстЗапросаТоварыПереданные() Экспорт
	...
	|				ВидыЗапасов.Ссылка КАК Ссылка
	|			ИЗ
	|				ВидыЗапасов КАК ВидыЗапасов)) КАК ТоварыПереданныеОстатки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
