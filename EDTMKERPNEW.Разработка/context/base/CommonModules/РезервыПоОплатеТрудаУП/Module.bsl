...
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты, НепроверяемыеРеквизиты) Экспорт
	...
	Запрос.УстановитьПараметр("Начисления",    Объект.ОценочныеОбязательства);
	...
	|	Начисления.МестоВСтруктуреПредприятия КАК Подразделение,
	|	Начисления.СпособОтраженияЗарплатыВБухучете КАК СпособОтражения,
	|	Начисления.СуммаРезерва КАК СуммаРезерва,
	|	Начисления.СуммаРезерваНУ КАК СуммаРезерваНУ,
	|	Начисления.СуммаРезерваСтраховыхВзносов КАК СуммаРезерваСтраховыхВзносов,
	|	Начисления.СуммаРезерваСтраховыхВзносовНУ КАК СуммаРезерваСтраховыхВзносовНУ,
	|	Начисления.СуммаРезерваФССНесчастныеСлучаи КАК СуммаРезерваФССНесчастныеСлучаи,
	|	Начисления.СуммаРезерваФССНесчастныеСлучаиНУ КАК СуммаРезерваФССНесчастныеСлучаиНУ
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Не указана статья в способе отражения ""%2"" в строке %1 списка ""Обязательства и резервы (сводно)"")';
				|en = 'The item is not specified in the ""%2"" record method in line %1 of the ""Provisions and payroll funds (summary)"" list)'"),
			Выборка.НомерСтроки, Выборка.СпособОтражения);
		ПутьКСтрокеТабличнойЧасти =
			ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ОценочныеОбязательства", Выборка.НомерСтроки, "СпособОтраженияЗарплатыВБухучете");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Объект, ПутьКСтрокеТабличнойЧасти, , Отказ);
	...
	ШаблоныСообщенияОбОшибке.Добавить(НСтр("ru = 'В подразделении ""%2"" не регистрировалась выработка сотрудников (строка %1 списка ""Обязательства и резервы (сводно)"")';
											|en = 'In the ""%2"" business unit, employee output was not registered (line %1 of the ""Provisions and payroll funds (summary)"" list)'"));
	ШаблоныСообщенияОбОшибке.Добавить(НСтр("ru = 'Не задана трудоемкость работ выполненных в подразделении ""%2"" (строка %1 списка ""Обязательства и резервы (сводно)"")';
											|en = 'Labor input of performed works is not specified in the ""%2"" business unit (line %1 of the ""Provisions and payroll funds (summary)"" list)'"));
	...
	|	И (Начисления.СуммаРезерва < 0
	|		ИЛИ Начисления.СуммаРезерваНУ < 0
	|		ИЛИ Начисления.СуммаРезерваСтраховыхВзносов < 0
	|		ИЛИ Начисления.СуммаРезерваСтраховыхВзносовНУ < 0
	|		ИЛИ Начисления.СуммаРезерваФССНесчастныеСлучаи < 0
	|		ИЛИ Начисления.СуммаРезерваФССНесчастныеСлучаиНУ < 0)";
	
	Если Не ЗначениеЗаполнено(Объект.СтатьяДоходов) И Не Запрос.Выполнить().Пустой() Тогда
...
Функция ТекстОтраженияВРеглУчете(ИмяДокумента) Экспорт
	...
	|	РасходыДокумента.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|
	...
	|		ИНАЧЕ РасходыДокумента.СуммаНУ
	|	КОНЕЦ КАК СуммаНУДт,
	|	ВЫБОР
	...
	|	""РезервыПоОплатеТрудаУП_1254"" КАК НомерФрагмента
	|
	|ИЗ
	...
	|	И РасходыДокумента.ВидОперации <> ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРезервовПоОплатеТруда.Списание)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|
	|	СодержаниеПроводок.Содержание КАК Содержание,
	|	""РезервыПоОплатеТрудаУП_1255"" КАК НомерФрагмента
	|
	|ИЗ
	|	ВТРасходыДокумента КАК РасходыДокумента
	|
	...
	|
	|	СодержаниеПроводок.Содержание КАК Содержание,
	|	""РезервыПоОплатеТрудаУП_1256"" КАК НомерФрагмента
	|
	|ИЗ
	|	ВТРасходыДокумента КАК РасходыДокумента
	|
	...
	|	ДоходыДокумента.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|
	...
	|	СодержаниеПроводок.Содержание КАК Содержание,
	|	""РезервыПоОплатеТрудаУП_1257"" КАК НомерФрагмента
	|
	|ИЗ
	...
	|	РасходыСдельные.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|
	...
	|	""РезервыПоОплатеТрудаУП_1258"" КАК НомерФрагмента
	|
	|ИЗ
	|	ВТРасходыСдельные КАК РасходыСдельные
	|
	|ГДЕ
	|	РасходыСдельные.ВидОперацииДокумента <> ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРезервовПоОплатеТруда.Списание)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	РасходыДокумента.Ссылка,
	|	РасходыДокумента.ПериодРегистрации,
	|	РасходыДокумента.Организация,
	|	РасходыДокумента.ИдентификаторСтроки,
	|
	|	-РасходыДокумента.СуммаРегл,
	|	-ВЫРАЗИТЬ(РасходыДокумента.СуммаРегл / КурсВалютыУпрУчета.Курс КАК ЧИСЛО(31, 2)),
	|
	|	НЕОПРЕДЕЛЕНО,
	|	НЕОПРЕДЕЛЕНО,
	|	НЕОПРЕДЕЛЕНО,
	|
	|	&ВалютаРеглУчета,
	|	РасходыДокумента.Подразделение,
	|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка),
	|
	|	РасходыДокумента.СчетУчета,
	|	РасходыДокумента.ВидРезервов,
	|	НЕОПРЕДЕЛЕНО,
	|	НЕОПРЕДЕЛЕНО,
	|	
	|	-РасходыДокумента.СуммаРегл,
	|	0,
	|	-РасходыДокумента.СуммаНУ,
	|	0,
	|	-РасходыДокумента.СуммаРегл + РасходыДокумента.СуммаНУ,
	|
	...
	|	СодержаниеПроводок.Содержание,
	|	""РезервыПоОплатеТрудаУП_2005""
	|
	|ИЗ
	|	ВТРасходыДокумента КАК РасходыДокумента
	|
	...
	|	-РасходыДокумента.СуммаРегл + РасходыДокумента.СуммаНУ,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.ПрочиеАктивыПассивы),
	|	НЕОПРЕДЕЛЕНО,
	|	НЕОПРЕДЕЛЕНО,
	|	
	|	НЕОПРЕДЕЛЕНО,
	...
	|
	|ВЫБРАТЬ
	|	РасходыСдельные.Ссылка,
	|	РасходыСдельные.ПериодРегистрации,
	|	РасходыСдельные.Организация,
	|	РасходыСдельные.ИдентификаторФинЗаписи,
	|
	|	РасходыСдельные.СуммаРегл,
	|	РасходыСдельные.СуммаУУ,
	|
	|	НЕОПРЕДЕЛЕНО,
	|	НЕОПРЕДЕЛЕНО,
	|	НЕОПРЕДЕЛЕНО,
	|
	|	&ВалютаРеглУчета,
	|	РасходыСдельные.Подразделение,
	|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка),
	|
	|	РасходыСдельные.СчетУчета,
	|	РасходыСдельные.ВидРезервов,
	|	НЕОПРЕДЕЛЕНО,
	|	НЕОПРЕДЕЛЕНО,
	|	
	|	РасходыСдельные.СуммаРегл,
	|	0,
	|	РасходыСдельные.СуммаРегл - РасходыСдельные.ПостояннаяРазница - РасходыСдельные.ВременнаяРазница,
	|	РасходыСдельные.ПостояннаяРазница,
	|	РасходыСдельные.ВременнаяРазница,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НезавершенноеПроизводство),
	|	РасходыСдельные.ГруппаПродукции,
	|	РасходыСдельные.Подразделение,
	|	
	|	НЕОПРЕДЕЛЕНО,
	|	РасходыСдельные.Подразделение,
	|	РасходыСдельные.НаправлениеДеятельности,
	|	
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка),
	|	РасходыСдельные.ВидРабот,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыЗатратРегл.СдельнаяОплатаТруда),
	|	РасходыСдельные.ГруппаПродукции,
	|
	|	0,
	|	0,
	...
	|	""Распределение оценочных обязательств по выработке"",
	|	""РезервыПоОплатеТрудаУП_2009""
	|
	|ИЗ
	|	ВТРасходыСдельные КАК РасходыСдельные
	|
...
Функция ТекстЗапросаВТОтраженияВРеглУчете(ИмяДокумента) Экспорт
	...
	|	СдельныеНачисления.ИдентификаторФинЗаписи КАК ИдентификаторФинЗаписи,
	|
	...
	|	СдельныеНачисления.ИдентификаторФинЗаписи,
...
Функция ТекстЗапросаПрочиеДоходы(ТекстыЗапроса, Регистры)
	...
	|	СУММА(ВЫБОР
	|		КОГДА &ИспользоватьУчетПрочихДоходовРасходовРегл
	|			И НЕ ПрочиеДоходы.СтатьяДоходов.ПринятиеКНалоговомуУчету
	|			ТОГДА ПрочиеДоходы.СуммаНУ
	|		ИНАЧЕ 0
	|	КОНЕЦ)                                 КАК ПостояннаяРазница,
	|	СУММА(ВЫБОР
	|		КОГДА &ИспользоватьУчетПрочихДоходовРасходовРегл
