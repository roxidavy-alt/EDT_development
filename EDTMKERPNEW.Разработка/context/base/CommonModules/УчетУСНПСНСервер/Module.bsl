...
Функция НачалоРасчетаОперацийПоУСН(КонецРасчета, СписокОрганизаций = Неопределено) Экспорт
	...
	|			И (ЗаданияКЗакрытиюМесяца.Операция В (ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.ПризнаниеРасходовПриУСН), ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.СторноДоходовКУДиР), ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.РасчетНалогаУСН)))
	|ГДЕ
	|	(ЗаданияКЗакрытиюМесяца.Операция = ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.ПризнаниеРасходовПриУСН)
	|				И УчетныеПолитикиОрганизаций.ПрименяетсяУСНДоходыМинусРасходы
	|			ИЛИ ЗаданияКЗакрытиюМесяца.Операция = ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.СторноДоходовКУДиР)
	|			ИЛИ ЗаданияКЗакрытиюМесяца.Операция = ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.РасчетНалогаУСН)
	...
	|ВЫБРАТЬ
	|	МИНИМУМ(ВЫБОР
	|			КОГДА НеЗаполненныеДатыРасчета.Операция = ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.ПризнаниеРасходовПриУСН)
	...
	|	МИНИМУМ(ВЫБОР
	|			КОГДА НеЗаполненныеДатыРасчета.Операция = ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.СторноДоходовКУДиР)
	|				ТОГДА НеЗаполненныеДатыРасчета.Месяц
	|			ИНАЧЕ ДОБАВИТЬКДАТЕ(&КонецРасчета, СЕКУНДА, 1)
	|		КОНЕЦ) КАК НачалоРасчетаСторноДоходовКУДиР,
	|	МИНИМУМ(ВЫБОР
	|			КОГДА НеЗаполненныеДатыРасчета.Операция = ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.РасчетНалогаУСН)
	|				ТОГДА НеЗаполненныеДатыРасчета.Месяц
	|			ИНАЧЕ ДОБАВИТЬКДАТЕ(&КонецРасчета, СЕКУНДА, 1)
...
// Процедура формирует движения по регистру "КУДиР", при выполнении регламентной операции "РасчетТранспортногоНалогаДо2020",
...
Процедура ДобавитьЭтап_ПризнаниеРасходовПриУСН(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ПризнаниеРасходовПриУСН,
		Ложь, Истина, Ложь,
		Справочники.ОперацииЗакрытияМесяца.ФормированиеФинансовогоРезультата);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Признание расходов';
											|en = 'Expense recognition'"; // @НСтр-1
...
Процедура ДобавитьЭтап_СторноДоходовКУДиР(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Сторнирование доходов КУДиР';
											|en = 'LIE income reversal'"; // @НСтр-1
	НоваяСтрока.ТекстВыполнить = НСтр("ru = 'Выполнить';
										|en = 'Execute'");
	НоваяСтрока.ДействиеИспользование = ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
...
Процедура ДобавитьЭтап_РасчетНалогаУСН(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.РасчетНалогаУСН,
		Ложь, Истина, Ложь,
		Справочники.ОперацииЗакрытияМесяца.СторноДоходовКУДиР);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Расчет налога УСН';
											|en = 'STS tax calculation'"; // @НСтр-1
...
Процедура ПризнатьИОтразитьРасходыКУДиР(Параметры, АдресХранилища) Экспорт
	...
	ПараметрыРасчета = ЗакрытиеМесяцаСервер.ИнициализироватьСвойстваРаботыСЗаданиями(Истина);
	ПараметрыРасчета.Операция = Справочники.ОперацииЗакрытияМесяца.ПризнаниеРасходовПриУСН;
	ПараметрыРасчета.Организации = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Параметры.Организация);
	ПараметрыРасчета.НачалоПериода = НачалоМесяца(ПериодРегламентнойОперации);
	ПараметрыРасчета.КонецПериода = ПериодРегламентнойОперации;
	...
	Пока ПараметрыРасчета.КонецПериода <= Параметры.КонДата И Не ПараметрыРасчета.Отказ Цикл
		
		ДанныеКПризнаниюРасходовЗаМесяц = ЗакрытиеМесяцаСервер.ДанныеКРасчету(ПараметрыРасчета);
			
		Если Не ПараметрыРасчета.Отказ Тогда
			Документы.РегламентнаяОперация.РассчитатьРегламентныеОперации(
				ПараметрыРасчета.КонецПериода, ОперацииДляВыполнения, Параметры.Организация, ПараметрыРасчета.Отказ);
		КонецЕсли;
		
		ЗакрытиеМесяцаСервер.ЗафиксироватьРасчетТранзакция(ПараметрыРасчета, ДанныеКПризнаниюРасходовЗаМесяц);
		
		ПараметрыРасчета.КонецПериода = КонецМесяца(ПараметрыРасчета.КонецПериода + 1);
		
	КонецЦикла;
	
	УстановитьПривилегированныйРежим(Ложь);
	
	Если ПараметрыРасчета.Отказ Тогда
...
Процедура ПризнатьРасходы(ПараметрыРасчета, Движения) Экспорт
	...
	ВходящиеДанные = ВходящиеДанныеПоОперации(Справочники.ОперацииЗакрытияМесяца.ПризнаниеРасходовПриУСН);
	...
	ЗаполнитьРегистрыРасходовУСН(ВременныеТаблицыДляПризнанияРасходов, КоэффициентОтнесенияРасходовКПрочейДеятельности, Движения, ПараметрыРасчета, КоличествоОбработанныхДанных);
...
// Параметры:
//	Параметры - Структура:
//					НачДата - Дата - дата, с которой выполняется сторнирование доходов;
//					НачДата - Дата - дата, по которую выполняется сторнирование доходов;
//					Организация - СправочникСсылка.Организация - организация, для которой необходимо выполнить сторнирование доходов.
//	АдресХранилища - строка.
//
Процедура СторноДоходовКУДиР(Параметры, АдресХранилища = "") Экспорт
	...
	ПараметрыРасчета = ЗакрытиеМесяцаСервер.ИнициализироватьСвойстваРаботыСЗаданиями(Истина);
	ПараметрыРасчета.Операция = Справочники.ОперацииЗакрытияМесяца.СторноДоходовКУДиР;
	ПараметрыРасчета.Организации = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Параметры.Организация);
	ПараметрыРасчета.НачалоПериода = Параметры.НачДата;
	ПараметрыРасчета.КонецПериода = Параметры.КонДата;
	
	ВходящиеДанные = ВходящиеДанныеПоОперации(ПараметрыРасчета.Операция);
	...
	ДанныеКСторнированиюДоходовЗаМесяц = ЗакрытиеМесяцаСервер.ДанныеКРасчету(ПараметрыРасчета);
	...
	ЗакрытиеМесяцаСервер.ЗафиксироватьРасчетТранзакция(ПараметрыРасчета, ДанныеКСторнированиюДоходовЗаМесяц);
...
// Параметры:
//	Параметры - Структура:
//					КонДата - дата, по которую необходимо начислить налог УСН;
...
Процедура НачислитьНалогУСН(Параметры, АдресХранилища) Экспорт
	...
	ПараметрыРасчета = ЗакрытиеМесяцаСервер.ИнициализироватьСвойстваРаботыСЗаданиями(Истина);
	ПараметрыРасчета.Операция = Справочники.ОперацииЗакрытияМесяца.РасчетНалогаУСН;
	ПараметрыРасчета.Организации = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Параметры.Организация);
	ПараметрыРасчета.НачалоПериода = НачалоКвартала(Параметры.КонДата);
	ПараметрыРасчета.КонецПериода = КонецКвартала(Параметры.КонДата);
	...
	ДанныеКРасчетуНалогаЗаМесяц = ЗакрытиеМесяцаСервер.ДанныеКРасчету(ПараметрыРасчета);
		
	Если Не ПараметрыРасчета.Отказ Тогда
		Документы.РегламентнаяОперация.РассчитатьРегламентныеОперации(Параметры.КонДата, ОперацииДляВыполнения, Параметры.Организация, ПараметрыРасчета.Отказ);
	КонецЕсли;
	
	ЗакрытиеМесяцаСервер.ЗафиксироватьРасчетТранзакция(ПараметрыРасчета, ДанныеКРасчетуНалогаЗаМесяц);
...
Процедура ЗаполнитьРегистрыРасходовУСН(ИсходныеДанные, КоэффициентОтнесенияРасходовКПрочейДеятельности, Движения, ПараметрыРасчета, КоличествоОбработанныхДанных = 0)
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		СуммаПоПлатежу = ?(СуммаОплатыПоДокументу = Неопределено, 0, СуммаОплатыПоДокументу);
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ВидОперации = "Аванс" ИЛИ Выборка.ВидОперации = "Оплата" ИЛИ Выборка.ВидОперации = "АвансБезРазбивкиПоДокументам"
				...
				Если ЕНП Тогда 
					...
					КонецЕсли;
				Иначе
					Если ДокументВозникновенияРасхода <> Выборка.ДокументВозникновенияРасходов 
						Или Выборка.ВидРасходов = Перечисления.ВидыРасходовУСН.Лизинг Тогда
						ДокументВозникновенияРасхода = Выборка.ДокументВозникновенияРасходов;
						Если РасходыПоАренднымПлатежам().Найти(Выборка.СтатьяРасходов) <> Неопределено Тогда
							СуммаОплатыПоДокументу = Выборка.СуммаОплаты;
						Иначе
				...
				МассивРезультатов = РезультатОплатыРасходов(ДанныеДляОбработки, ТаблицаОстатков, ПараметрыРасчета);
			...
			Для каждого ЗаписьРасхода Из МассивРезультатов Цикл
				...
				РесурсыРегистра = НаборЗаписей.метаданные().Ресурсы;
				Для Каждого Ресурс Из РесурсыРегистра Цикл
					Если ЗаписьРасхода[Ресурс.Имя] <> 0 Тогда
...
Функция РезультатОплатыРасходов(Данные, ТаблицаОстатков, ПараметрыРасчета, СтруктураПоискаОстатка = Неопределено)
	...
	Для каждого СтрокаОстатка Из СтрокиОстатка Цикл
		...
		Если (ОсталосьОплатитьПоТекущейВыборке > 0 ИЛИ ОплатаВнеЗависимостиОтРасходов) И СуммаОплаты > 0 
			ИЛИ (Данные.Сторно И РасходыПоАренднымПлатежам().Найти(Данные.СтатьяРасходов) <> Неопределено) Тогда
			...
			Если Не Перечисления.ВидыРасходовУСН.ПризнаетсяКакОСНМА(СтрокаОстатка.ВидРасходов)
				...
				Если Данные.Сторно И РасходыПоАренднымПлатежам().Найти(Данные.СтатьяРасходов) <> Неопределено Тогда
					ПризнаваемаяСумма = 0;
				Иначе
			...
			Если Данные.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыкупАрендованныхОС 
				И Данные.Период < ПараметрыРасчета.НачДата Тогда
				РезультатДоп = ИнициализироватьДанныеРезультата(Данные);
				ЗаполнитьЗначенияСвойств(РезультатДоп, Результат);
				РезультатДоп.ЭлементРасходов = Неопределено;
				РезультатДоп.СуммаОплаты = - Результат.СуммаОплаты;
				МассивРезультатов.Добавить(РезультатДоп);
			КонецЕсли;
	...
	Если Данные.СуммаОплатыПоДокументу > 0 И Данные.СуммаОплаты > 0 И СтруктураПоискаОстатка.Количество() 
		И НЕ (СтруктураПоискаОстатка.Количество() = 3 И Данные.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыкупАрендованныхОС) Тогда
		
		МассивРезультатовРеклассификации = РезультатОплатыРасходов(Данные, ТаблицаОстатков, ПараметрыРасчета, СтруктураПоискаОстатка);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивРезультатов, МассивРезультатовРеклассификации);
		
	КонецЕсли;
	
	Если Данные.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыкупАрендованныхОС И Данные.СуммаОплатыПоДокументу > 0 
		И Данные.Период >= ПараметрыРасчета.НачДата Тогда 
		Результат = ИнициализироватьДанныеРезультата(Данные);
		Результат.ЭлементРасходов = Неопределено;
		Результат.СуммаОплаты = Данные.СуммаОплатыПоДокументу; 
		Данные.СуммаОплатыПоДокументу = 0;
		Результат.ДатаОплаты = Данные.Период;
		МассивРезультатов.Добавить(Результат);
	КонецЕсли;
...
Функция РезультатВозвратаРасходов(Данные, ТаблицаОстатков, СтруктураПоискаОстатка = Неопределено)
	...
	ОсталосьОплатитьПоТекущейВыборке = СтрокаОстатка.СуммаРасходов - СтрокаОстатка.СуммаОплаты;
	Если ОсталосьОплатитьПоТекущейВыборке < СуммаРасходовВозврата 
		И Не (Данные.Сторно 
			И Перечисления.ВидыРасходовУСН.ВидыРасходовНеКорректируетсяОплатаПриСторно().Найти(Данные.ВидРасходов) <> Неопределено) Тогда
...
Процедура ЗаполнитьКУДиР2ПризнаннымиРасходами(ВременнныеТаблицыРасходовНаОС, Движения)
	...
	Для Каждого СтрокаРасходов Из ТаблицаРасходовКПризнанию Цикл
		...
		Если СтрокаРасходов.ПорядокУчета <> ПорядокУчета.СтоимостьНеВключаетсяВРасходы
			...
			СуммаПризнания = ?(СтрокаРасходов.ВидДеятельности <> ВидыДеятельности.ОсобыйПорядокНалогообложения, СуммаПризнания, 0);
			ЗаписьРасхода.Признано = СуммаПризнания;
			ЗаписьРасхода.НеПризнано = ЗаписьКУДиР.РасходЕНВД;
			ЗаписьПризнания.Признано = СуммаПризнания;
...
Функция ТекстЗапросаОпределенияМатериалов()
	...
	|ИЗ
	|	РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПоследних(&НачДата, Организация = &Организация
	|		И МестоУчета <> НЕОПРЕДЕЛЕНО И ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НаСкладе)) КАК ПорядокОтражения
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|ИЗ
	|	РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПоследних(&НачДата, МестоУчета = НЕОПРЕДЕЛЕНО
	|		И Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|		И ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НаСкладе)) КАК ПорядокОтраженияПоАналитикеУчета
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|		РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПоследних(&НачДата, Организация = &Организация
	|		И МестоУчета = НЕОПРЕДЕЛЕНО И ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НаСкладе)) КАК ПорядокОтраженияПоОрганизации
	|	ПО 
	|		ПорядокОтраженияПоАналитикеУчета.АналитикаУчета = ПорядокОтраженияПоОрганизации.АналитикаУчета
	|ГДЕ
	|	ЕСТЬNULL(ПорядокОтраженияПоОрганизации.СчетУчета, ПорядокОтраженияПоАналитикеУчета.СчетУчета) В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Материалы), ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ВложенияВоВнеоборотныеАктивы))
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ГруппаФинансовогоУчета";
...
Функция ТекстЗапросаПартийРеализацийБезПереходаПраваСобственности()
	...
	|ИЗ
	|	РегистрНакопления.ДетализацияПартийТоваровДляНДСиУСН.ОстаткиИОбороты(&НачГраница, &КонГраница, Регистратор,,
	|		Организация = &Организация И Партия ССЫЛКА Документ.РеализацияТоваровУслуг) КАК Партии
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	Партии.Партия
	|ИЗ
	|	РегистрНакопления.ДетализацияПартийТоваровДляНДСиУСН2_4.ОстаткиИОбороты(&НачГраница, &КонГраница, Регистратор,,
	|		Организация = &Организация И Партия ССЫЛКА Документ.РеализацияТоваровУслуг) КАК Партии
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	Партии.Партия
	|ИЗ
	|	РегистрНакопления.СебестоимостьТоваров.ОстаткиИОбороты(&НачГраница, &КонГраница, Регистратор,,
	|		Организация = &Организация И Партия ССЫЛКА Документ.РеализацияТоваровУслуг) КАК Партии
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументРеализации
...
Функция ТекстЗапросаРасходовПоКредитамИЛизингу()
	...
	|	СУММА(ВЫБОР
	|		КОГДА РасчетыПоФинансовымИнструментам.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) 
	|			И РасчетыПоФинансовымИнструментам.ХозяйственнаяОперация = Значение(Перечисление.ХозяйственныеОперации.ИзменениеУсловийДоговораАренды)
	|			ТОГДА - (РасчетыПоФинансовымИнструментам.СуммаРегл + ЕСТЬNULL(СуммыНДС.СуммаНДСРегл, 0))
	|	ИНАЧЕ РасчетыПоФинансовымИнструментам.СуммаРегл + ЕСТЬNULL(СуммыНДС.СуммаНДСРегл, 0) КОНЕЦ),
	|	РасчетыПоФинансовымИнструментам.Договор,
	...
	|			И РасчетыПоФинансовымИнструментам.ХозяйственнаяОперация = Значение(Перечисление.ХозяйственныеОперации.ИзменениеУсловийДоговораАренды)  
	|			ТОГДА Истина
	|		ИНАЧЕ РасчетыПоФинансовымИнструментам.Сторно 
	|	КОНЕЦ КАК Сторно
	|ИЗ
	|	РегистрНакопления.РасчетыПоФинансовымИнструментам КАК РасчетыПоФинансовымИнструментам
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК Аналитика
	...
	|	ИЛИ РасчетыПоФинансовымИнструментам.ХозяйственнаяОперация = Значение(Перечисление.ХозяйственныеОперации.ИзменениеУсловийДоговораАренды) И РасчетыПоФинансовымИнструментам.ТипСуммы В 
	...
	|	РасчетыПоФинансовымИнструментам.Регистратор,
	|	Аналитика.Организация,  
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыкупАрендованныхОС),
	|	РасчетыПоФинансовымИнструментам.ТипСуммы,
	|	СУММА(РасчетыПоФинансовымИнструментам.СуммаРегл),
	|	РасчетыПоФинансовымИнструментам.Договор,
	|	0,
	|	""Оплата"",
	|	ЗНАЧЕНИЕ(Перечисление.ВидыРасходовУСН.ОС),
	|	Аналитика.Контрагент КАК Контрагент,
	|	РасчетыПоФинансовымИнструментам.Договор КАК Договор,
	|	РасчетыПоФинансовымИнструментам.Сторно КАК Сторно
	|ИЗ
	...
	|	И Аналитика.Организация = &Организация
	|	И РасчетыПоФинансовымИнструментам.Договор ССЫЛКА Справочник.ДоговорыАренды
	|	И РасчетыПоФинансовымИнструментам.ТипСуммы = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.ВыкупнаяСтоимость)
	|	И РасчетыПоФинансовымИнструментам.Сумма <> 0
	|	И РасчетыПоФинансовымИнструментам.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|
	|СГРУППИРОВАТЬ ПО
	|	РасчетыПоФинансовымИнструментам.Регистратор,
	|	РасчетыПоФинансовымИнструментам.Период,
	...
	|
	|ВЫБРАТЬ
	|	РасходыПриУСНОСиНМАОстаткиРегистратор.Период КАК Период,
	|	РасходыПриУСНОСиНМАОстаткиРегистратор.Регистратор КАК Регистратор,
	|	РасходыПриУСНОСиНМАОстаткиРегистратор.Организация КАК Организация,
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыкупАрендованныхОС),
	|	ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.ВыкупнаяСтоимость),
	|	СУММА(РасходыПриУСНОСиНМАОстаткиРегистратор.СуммаОплаты),
	|	РасходыПриУСНОСиНМАОстаткиРегистратор.ДокументВозникновенияРасходов,
	|	(0) КАК НДС,
	|	""Оплата"" КАК ВидОперации,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыРасходовУСН.ОС),
	|	"""" КАК Контрагент,
	|	РасходыПриУСНОСиНМАОстаткиРегистратор.ДокументВозникновенияРасходов,
	|	РасходыПриУСНОСиНМАОстаткиРегистратор.Сторно КАК Сторно
	|
	|ИЗ
	|	РегистрНакопления.РасходыПриУСНОСиНМА.Остатки(&НачДата, Организация = &Организация И ЭлементРасходов = НЕОПРЕДЕЛЕНО) КАК РасходыПриУСНОСиНМАОстатки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасходыПриУСНОСиНМА КАК РасходыПриУСНОСиНМАОстаткиРегистратор
	|		ПО РасходыПриУСНОСиНМАОстатки.Организация = РасходыПриУСНОСиНМАОстаткиРегистратор.Организация
	|			И РасходыПриУСНОСиНМАОстатки.ДокументВозникновенияРасходов = РасходыПриУСНОСиНМАОстаткиРегистратор.ДокументВозникновенияРасходов 
	|			И РасходыПриУСНОСиНМАОстатки.ЭлементРасходов = РасходыПриУСНОСиНМАОстаткиРегистратор.ЭлементРасходов
	|ГДЕ
	|	РасходыПриУСНОСиНМАОстаткиРегистратор.Период <= &КонДата
	|	И РасходыПриУСНОСиНМАОстаткиРегистратор.ДокументВозникновенияРасходов Ссылка Справочник.ДоговорыАренды
	|	И РасходыПриУСНОСиНМАОстаткиРегистратор.СуммаОплаты > 0
	|
	|СГРУППИРОВАТЬ ПО
	|	РасходыПриУСНОСиНМАОстаткиРегистратор.Период,
	|	РасходыПриУСНОСиНМАОстаткиРегистратор.Регистратор,
	|	РасходыПриУСНОСиНМАОстаткиРегистратор.Организация,
	|	РасходыПриУСНОСиНМАОстаткиРегистратор.ДокументВозникновенияРасходов,
	|	РасходыПриУСНОСиНМАОстаткиРегистратор.Сторно
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	ДокументВозникновенияРасходов";
...
Функция ТекстЗапросаКДокументамВозникновенияРасходов()
	...
	ТекстЗапроса =
	"ВЫБРАТЬ   
	|	ДокументВыкупа.Организация                КАК Организация,
	|	ДокументВыкупа.Ссылка                     КАК Регистратор,
	|	ДокументВыкупа.Дата                       КАК Период, 
	|	СУММА(СуммыДокументов.СуммаНДСРегл)       КАК НДС,
	|	СУММА(СуммыДокументов.СуммаБезНДСРегл + СуммыДокументов.СуммаНДСРегл) КАК СуммаРасходов,
	|	0 КАК СуммаОплаты, 
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыкупАрендованныхОС),
	|	""Приход"" КАК ВидОперации,  
	|	ЗНАЧЕНИЕ(Перечисление.ВидыРасходовУСН.ОС) КАК ВидРасходов,
	|	ТаблицаОС.ОсновноеСредство                КАК ЭлементРасходов,
	|	ДокументВыкупа.Договор                    КАК ДокументВозникновенияРасходов,
	|	ДокументВыкупа.Договор                    КАК Партия,
	|	ДокументВыкупа.Договор                    КАК СтатьяРасходов,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиДляНалоговогоУчетаЗатрат.ОсновнаяСистемаНалогообложения),
	|	ИСТИНА,
	|	ДокументВыкупа.Контрагент                 КАК Контрагент,
	|	ДокументВыкупа.Договор                    КАК Договор,
	|	ЛОЖЬ                                      КАК Сторно
	|ИЗ 
	|	Документ.ВыкупАрендованныхОС КАК ДокументВыкупа
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВыкупАрендованныхОС.ОС КАК ТаблицаОС
	|		ПО ДокументВыкупа.Ссылка = ТаблицаОС.Ссылка
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СуммыДокументовВВалютахУчета КАК СуммыДокументов
	|		ПО СуммыДокументов.Регистратор = ТаблицаОС.Ссылка
	|		И СуммыДокументов.ИдентификаторСтроки = ТаблицаОС.ИдентификаторСтроки
	|ГДЕ
	|	ДокументВыкупа.Дата >= &НачДата
	|	И ДокументВыкупа.Дата <= &КонДата 
	|	И ДокументВыкупа.Организация = &Организация
	|	И ДокументВыкупа.Проведен
	|
	|СГРУППИРОВАТЬ ПО 
	|	ДокументВыкупа.Ссылка,
	|	ДокументВыкупа.Организация,
	|	ДокументВыкупа.Контрагент,
	|	ДокументВыкупа.Договор,
	|	ДокументВыкупа.Дата,
...
Функция ТекстЗапросаКДокументамОплатыРасходов()
	...
	#Область ОплатыВыкупнойСтоимостиЗаОСН
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ОстаткиДетально.Организация,
	|	РасходыПоКредитамЛизингу.Регистратор,
	...
	|	РасходыПоКредитамЛизингу КАК РасходыПоКредитамЛизингу
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Остатки КАК ОстаткиДетально
	|		ПО РасходыПоКредитамЛизингу.Организация = ОстаткиДетально.Организация
	|			И РасходыПоКредитамЛизингу.ДокументВозникновенияРасходов = ОстаткиДетально.ДокументВозникновенияРасходов
	|			И ОстаткиДетально.ВидРасходов В (ЗНАЧЕНИЕ(Перечисление.ВидыРасходовУСН.ОС))
	|		ЛЕВОЕ СОЕДИНЕНИЕ Остатки КАК ОстаткиИтого
	|		ПО РасходыПоКредитамЛизингу.Организация = ОстаткиИтого.Организация
	|			И РасходыПоКредитамЛизингу.ДокументВозникновенияРасходов = ОстаткиИтого.ДокументВозникновенияРасходов
	|			И ОстаткиИтого.ВидРасходов В (ЗНАЧЕНИЕ(Перечисление.ВидыРасходовУСН.ОС))
	|
	|СГРУППИРОВАТЬ ПО
	|	ОстаткиДетально.Организация,
	|	РасходыПоКредитамЛизингу.Регистратор,
...
Процедура ОбновитьСтруктуруПоиска(СтруктураПоиска, Выборка)
	...
	Если СтруктураПоиска.Свойство("ВидРасходов") И ВидыОперацийДляКоторыхИщемРеклассификацию.Найти(Выборка.ВидОперации) <> Неопределено
		...
		Если Выборка.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ВыкупАрендованныхОС Тогда
...
Функция ИсключающиесяХозОперацииВСебестоимости()
	...
	ИсключающиесяХозОперации.Добавить(Перечисления.ХозяйственныеОперации.ОтгрузкаТоваровКлиенту);
...
Функция ИсключающиесяРегистраторыСебестоимости()
	...
	ИсключающиесяТипыРегистраторов.Добавить(Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту"));
...
Функция ИсключающиесяДокументыПоступленияСебестоимости()
	...
	ИсключающиесяТипыДокументыПартий.Добавить(Тип("ДокументСсылка.ОприходованиеИзлишковОтгруженныхТоваров"));
...
Функция ХозОперацииСписанияТоваров()
	...
	ХозОперации.Добавить(Перечисления.ХозяйственныеОперации.ПереходПраваСобственности);
	//++ НЕ УТКА
...
Функция ВходящиеДанныеПоОперации(Операция)
	...
	Если Операция = Справочники.ОперацииЗакрытияМесяца.РасчетНалогаУСН Тогда
	...
	Если Операция = Справочники.ОперацииЗакрытияМесяца.СторноДоходовКУДиР Тогда
	...
	Если Операция = Справочники.ОперацииЗакрытияМесяца.ПризнаниеРасходовПриУСН Тогда
...
Функция ТекстЗапросаОбновленияЗаданийКЗакрытиюМесяцаПриЗаписиРегистра(ИмяВременнойТаблицыДляЗаписиВРегистрЗаданий = "")
	...
	|	ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.ПризнаниеРасходовПриУСН) КАК Операция,
...
Функция ТекстЗапросаВыручкаИСебестоимостьПродаж(Запрос)
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НАЧАЛОПЕРИОДА(ТаблицаИзменений.Период, МЕСЯЦ) КАК Месяц,
	|	ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.СторноДоходовКУДиР) КАК Операция,
...
Функция ТекстЗапросаКнигаУчетаДоходовИРасходов(Запрос)
	...
	|	ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.РасчетНалогаУСН) КАК Операция,
...
Функция РасходыПоАренднымПлатежам() Экспорт
	
	РасходыПоАренднымПлатежам = Новый Массив;
	РасходыПоАренднымПлатежам.Добавить(Перечисления.ТипыПлатежейПоАренде.УслугаПоАренде);
	РасходыПоАренднымПлатежам.Добавить(Перечисления.ТипыПлатежейПоАренде.УслугаПоАрендеАванс);
	РасходыПоАренднымПлатежам.Добавить(Перечисления.ТипыПлатежейПоАренде.ОбеспечительныйПлатеж);
	
	Возврат РасходыПоАренднымПлатежам;
