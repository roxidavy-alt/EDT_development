...
Процедура ДобавитьЭтап(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ФормированиеДвиженийПоВНА,,,,
		Справочники.ОперацииЗакрытияМесяца.НачислениеПроцентовПоАренде);
		
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Формирование движений по ОС и НМА';
											|en = 'Generate fixed asset and intangible asset records'"; // @НСтр-1
	
	НоваяСтрока.ПредшествующиеЭтапы.Добавить(ВнеоборотныеАктивыЛокализация.ОперацияПереходаНаУчетВнеоборотныхАктивов2_4());
	НоваяСтрока.ПредшествующиеЭтапы.Добавить(Справочники.ОперацииЗакрытияМесяца.НачислениеПроцентовПоАренде);
...
// 	ТаблицаЭтапов - (См. Обработки.ОперацииЗакрытияМесяца.ЗаполнитьОписаниеЭтаповЗакрытияМесяца)
// 	ТекущийРодитель - Строка - идентификатор группы.
Процедура ДобавитьЭтап_ФормированиеДвиженийПоВНАПередРасчетомАмортизации(ТаблицаЭтапов,ТекущийРодитель) Экспорт

	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ФормированиеДвиженийПоВНАПередРасчетомАмортизации);
	...
	НоваяСтрока.ДействиеИспользование = ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
		"ОтложенноеФормированиеДвиженийВНА.ОпределитьСтатусОперацииФормированиеДвиженийПоВНАПередРасчетомАмортизации");
...
// Опредяет статус операций закрытия месяца "ФормированиеДвиженийПоВНА", "ФормированиеДвиженийПоВНАПередРасчетомАмортизации".
// 
// Параметры:
...
// Параметры:
//  ПараметрыОбработчика - Структура - Параметры обработчика
Процедура ОпределитьСтатусОперацииФормированиеДвиженийПоВНАПередРасчетомАмортизации(ПараметрыОбработчика) Экспорт
	...
	Результат = ТребуетсяАктуализацияДвиженийПередРасчетомАмортизации(ПараметрыОбработчика);
	
	Если Результат.ТребуетсяАктуализацияДвижений Тогда
		Возврат;
	КонецЕсли;
	
	// Операция выполнена, если расчет амортизации отображается выполненным и есть документы по которым формируются движения.
	Если НЕ Результат.ЕстьЗаданияКЗаданияКРасчетуАмортизацииОС 
		И НЕ Результат.ЕстьЗаданияКЗаданияКРасчетуАмортизацииНМА Тогда
		
		КоличествоДокументовОС = ЗакрытиеМесяцаСервер.КоличествоРегламентныхДокументовЭтапаЗакрытияМесяца(
			ПараметрыОбработчика, Метаданные.Документы.АмортизацияОС2_4.Имя);
	
		КоличествоДокументовНМА = ЗакрытиеМесяцаСервер.КоличествоРегламентныхДокументовЭтапаЗакрытияМесяца(
			ПараметрыОбработчика, Метаданные.Документы.АмортизацияНМА2_4.Имя);

		Если (КоличествоДокументовОС <> 0 ИЛИ КоличествоДокументовНМА <> 0) 
			И ЕстьДокументыПоКоторымФормируютсяДвижения(ПараметрыОбработчика) Тогда
	...
	ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
		ПараметрыОбработчика,
		НСтр("ru = 'Выполнение операции не требуется.';
			|en = 'Operation is not required.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
//  ОперацияЗакрытияМесяца - СправочникСсылка.ОперацииЗакрытияМесяца - Операция
// 
// Возвращаемое значение:
...
//
Функция ВыполнитьОперацию(НачалоПериода, КонецПериода, СписокОрганизаций, НомерДоРасчета, МенеджерВременныхТаблиц, ИдентификаторРасчета, ОперацияЗакрытияМесяца = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ОперацияЗакрытияМесяца) Тогда
		ОперацияЗакрытияМесяца = Справочники.ОперацииЗакрытияМесяца.ФормированиеДвиженийПоВНА;
	КонецЕсли;
	
	Если ОперацияЗакрытияМесяца = Справочники.ОперацииЗакрытияМесяца.АктуализацияДвиженийПоВнеоборотнымАктивам Тогда
		ИмяРегистраЗаданий = Метаданные.РегистрыСведений.ЗаданияКАктуализацииДвиженийПоВНА.Имя;
		УдалитьЗадания = Ложь;
	ИначеЕсли ОперацияЗакрытияМесяца = Справочники.ОперацииЗакрытияМесяца.ФормированиеДвиженийПоВНАПередРасчетомАмортизации Тогда
		ИмяРегистраЗаданий = Метаданные.РегистрыСведений.ЗаданияКФормированиюДвиженийПоВНА.Имя;
	...
	ОчередьЗаданийКРасчету = СоздатьОчередьЗаданий(
		НачалоПериода, КонецПериода, СписокОрганизаций, НомерДоРасчета, МенеджерВременныхТаблиц, ОперацияЗакрытияМесяца, УдалитьЗадания);
...
Функция ПараметрыВыполнения() Экспорт
	...
	ПараметрыВыполнения.Вставить("ОперацияЗакрытияМесяца", Справочники.ОперацииЗакрытияМесяца.ФормированиеДвиженийПоВНА);
...
Процедура СформироватьЗадания(Документ, ДанныеТаблиц) Экспорт
	...
	Если Документ <> Неопределено
		...
		ЗарегистрироватьДокумент = 
			...
			ИЛИ ТипДокумента = Тип("ДокументСсылка.ОтнесениеОтклоненийВСтоимостьОС")
...
Процедура ПодготовитьПараметры(Период, МенеджерВременныхТаблиц)
	...
	ИспользуемыеВТ.Добавить("ВТ_АмортизацияЗаПериод");
	...
	#Область ВТ_АмортизацияЗаПериод
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ДанныеКРасчету.Ссылка КАК Ссылка,
	|	ДанныеКРасчету.ОбъектУчета КАК ОбъектУчета,
	|	МИНИМУМ(НАЧАЛОПЕРИОДА(КорректировкаАмортизацииОС.НачалоПериода, МЕСЯЦ)) КАК НачалоПериода,
	|	МАКСИМУМ(НАЧАЛОПЕРИОДА(ДанныеКРасчету.Дата, МЕСЯЦ)) КАК КонецПериода
	|
	|ПОМЕСТИТЬ ВтПериодКорректировки
	|
	|ИЗ
	|	ВтДанныеКРасчету КАК ДанныеКРасчету
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.КорректировкаСтоимостиИАмортизацииОС КАК КорректировкаАмортизацииОС
	|		ПО КорректировкаАмортизацииОС.Ссылка = ДанныеКРасчету.Ссылка
	|
	|ГДЕ
	|	КорректировкаАмортизацииОС.РежимИсправленияСПрошлойДаты
	|	И КорректировкаАмортизацииОС.КорректировкаАмортизацииЗаПериод
	|
	|СГРУППИРОВАТЬ ПО
	|	ДанныеКРасчету.Ссылка,
	|	ДанныеКРасчету.ОбъектУчета
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	АмортизацияЗаПериод.Ссылка КАК Ссылка,
	|	АмортизацияЗаПериод.КонецМесяца КАК КонецМесяца,
	|	АмортизацияЗаПериод.ОбъектУчета КАК ОбъектУчета,
	|
	|	СУММА(АмортизацияЗаПериод.Амортизация) КАК Амортизация,
	|	СУММА(АмортизацияЗаПериод.АмортизацияРегл) КАК АмортизацияРегл,
	|	СУММА(АмортизацияЗаПериод.АмортизацияНУ) КАК АмортизацияНУ,
	|	СУММА(АмортизацияЗаПериод.АмортизацияПР) КАК АмортизацияПР,
	|	СУММА(АмортизацияЗаПериод.АмортизацияЦФ) КАК АмортизацияЦФ
	|
	|ПОМЕСТИТЬ ВТ_АмортизацияЗаПериод
	|
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВтПериодКорректировки.Ссылка КАК Ссылка,
	|		КОНЕЦПЕРИОДА(АмортизацияВНА.Период, МЕСЯЦ) КАК КонецМесяца,
	|		АмортизацияВНА.ОсновноеСредство КАК ОбъектУчета,
	|
	|		ВЫБОР КОГДА АмортизацияВНА.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) 
	|			ТОГДА АмортизацияВНА.Амортизация 
	...
	|
	|	ИЗ
	|		ВтПериодКорректировки КАК ВтПериодКорректировки
	|
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.АмортизацияОС КАК АмортизацияВНА
	|			ПО АмортизацияВНА.ОсновноеСредство = ВтПериодКорректировки.ОбъектУчета
	|				И АмортизацияВНА.Период >= ВтПериодКорректировки.НачалоПериода
	|				И АмортизацияВНА.Период < ВтПериодКорректировки.КонецПериода
	|				И АмортизацияВНА.Регистратор <> ВтПериодКорректировки.Ссылка
	|				И АмортизацияВНА.Активность
	|
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.КорректировкаСтоимостиИАмортизацииОС КАК Корректировка
	|			ПО Корректировка.Ссылка = АмортизацияВНА.Регистратор
	|
	|		ГДЕ
	|			Корректировка.Ссылка ЕСТЬ NULL
	|				ИЛИ Корректировка.ДокументОснование = НЕОПРЕДЕЛЕНО
	...
	|					И НЕ Корректировка.ДокументОснование ССЫЛКА Документ.ПринятиеКУчетуОС2_4
	|
	|	) КАК АмортизацияЗаПериод
	|
	|СГРУППИРОВАТЬ ПО
	|	АмортизацияЗаПериод.Ссылка,
	|	АмортизацияЗаПериод.КонецМесяца,
	|	АмортизацияЗаПериод.ОбъектУчета
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка,
	|	ОбъектУчета,
...
Процедура ЗарегистрироватьОшибки(Период, ПараметрыВыполнения)
	...
	|	ДанныеДокумента.СписатьУзлы КАК СписатьУзлы
	|
	...
	|
	|ГДЕ
	|	ДанныеДокумента.Ссылка В (ВЫБРАТЬ Т.Ссылка ИЗ ВТ_РазукомплектацияОС КАК Т)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство
	|;
	...
	|	ТаблицаВНА.ОсновноеСредство КАК ОсновноеСредство,
	...
	|		ПО ДанныеРегистра.ОсновноеСредство = ТаблицаВНА.ОсновноеСредство
	|			И ДанныеРегистра.Период <= ТаблицаВНА.Дата
	|			И ДанныеРегистра.Регистратор <> ТаблицаВНА.Ссылка
	|			И ДанныеРегистра.Активность
	|
	|ГДЕ
	|	ТаблицаВНА.СписатьУзлы
	|		ИЛИ ДанныеРегистра.УзелКомпонент.ТипОС = ЗНАЧЕНИЕ(Перечисление.ТипыОС.КомпонентАмортизации)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	УзелКомпонент
	|;
	...
	|	ТаблицаВсеУзлыКомпонентыАмортизации.ОсновноеСредство КАК ОсновноеСредство,
	...
	|	ТаблицаВсеУзлыКомпонентыАмортизации.ОсновноеСредство,
	...
	|			И ДанныеРегистра.ОсновноеСредство = ДанныеРегистра_Регистратор.ОсновноеСредство
	|			И ДанныеРегистра.Активность
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_РазукомплектацияОС.Ссылка КАК Ссылка,
	|	СУММА(ТаблицаОС.СтоимостьБУ) КАК СтоимостьБУ,
	|	СУММА(ТаблицаОС.СтоимостьНУ) КАК СтоимостьНУ,
	|	СУММА(ТаблицаОС.СтоимостьПР) КАК СтоимостьПР,
	|	СУММА(ТаблицаОС.СтоимостьУУ) КАК СтоимостьУУ
	|
	|ПОМЕСТИТЬ ВТ_ОбщаяСтоимостьНовыхОС
	|
	|ИЗ
	|
	...
	|		ПО ТаблицаОС.Ссылка = ВТ_РазукомплектацияОС.Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_РазукомплектацияОС.Ссылка
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СтоимостьОС.Ссылка КАК Ссылка,
	|	СУММА(СтоимостьОС.СтоимостьБУ) КАК СтоимостьБУ,
	|	СУММА(СтоимостьОС.СтоимостьНУ) КАК СтоимостьНУ,
	|	СУММА(СтоимостьОС.СтоимостьПР) КАК СтоимостьПР,
	|	СУММА(СтоимостьОС.СтоимостьУУ) КАК СтоимостьУУ
	|
	|ПОМЕСТИТЬ ВТ_ОбщаяСтоимостьИсходногоОС
	|
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВТ_РазукомплектацияОС.Ссылка КАК Ссылка,
	|		СтоимостьОС.СтоимостьРегл + СтоимостьОС.СтоимостьЦФ КАК СтоимостьБУ,
	|		СтоимостьОС.СтоимостьНУ + СтоимостьОС.СтоимостьНУЦФ КАК СтоимостьНУ,
	|		СтоимостьОС.СтоимостьПР + СтоимостьОС.СтоимостьПРЦФ КАК СтоимостьПР,
	|		СтоимостьОС.Стоимость КАК СтоимостьУУ
	|	ИЗ
	|
	|		ВТ_РазукомплектацияОС КАК ВТ_РазукомплектацияОС
	|
	...
	|			ПО СтоимостьОС.Ссылка = ТаблицаДокумента.Ссылка
	|				И СтоимостьОС.ОбъектУчета = ТаблицаДокумента.ОсновноеСредство
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	|		УзлыКомпонентыАмортизации.Ссылка КАК Ссылка,
	|		СтоимостьОС.СтоимостьРегл + СтоимостьОС.СтоимостьЦФ КАК СтоимостьБУ,
	|		СтоимостьОС.СтоимостьНУ + СтоимостьОС.СтоимостьНУЦФ КАК СтоимостьНУ,
	|		СтоимостьОС.СтоимостьПР + СтоимостьОС.СтоимостьПРЦФ КАК СтоимостьПР,
	|		СтоимостьОС.Стоимость КАК СтоимостьУУ
	...
	|
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_СтоимостьВНА КАК СтоимостьОС
	|			ПО СтоимостьОС.Ссылка = УзлыКомпонентыАмортизации.Ссылка
	|				И СтоимостьОС.ОбъектУчета = УзлыКомпонентыАмортизации.УзелКомпонент
	|
	|	) КАК СтоимостьОС
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ТаблицаДокумента.Номер КАК НомерДокумента,
	|	ТаблицаДокумента.Дата КАК ДатаДокумента,
	|	ТаблицаДокумента.ОтражатьВРеглУчете КАК ОтражатьВРеглУчете,
	|	ТаблицаДокумента.ОтражатьВУпрУчете КАК ОтражатьВУпрУчете,
	|	ТаблицаДокумента.СписатьУзлы КАК СписатьУзлы,
	|
	|	ЕСТЬNULL(ОбщаяСтоимостьНовыхОС.СтоимостьБУ, 0) КАК ОбщаяСтоимостьБУ,
	|	ЕСТЬNULL(ОбщаяСтоимостьНовыхОС.СтоимостьНУ, 0) КАК ОбщаяСтоимостьНУ,
	|	ЕСТЬNULL(ОбщаяСтоимостьНовыхОС.СтоимостьПР, 0) КАК ОбщаяСтоимостьПР,
	|	ЕСТЬNULL(ОбщаяСтоимостьНовыхОС.СтоимостьУУ, 0) КАК ОбщаяСтоимостьУУ,
	|
	|	ЕСТЬNULL(ОбщаяСтоимостьИсходногоОС.СтоимостьБУ, 0) КАК ИсходнаяСтоимостьБУ,
	|	ЕСТЬNULL(ОбщаяСтоимостьИсходногоОС.СтоимостьНУ, 0) КАК ИсходнаяСтоимостьНУ,
	|	ЕСТЬNULL(ОбщаяСтоимостьИсходногоОС.СтоимостьПР, 0) КАК ИсходнаяСтоимостьПР,
	|	ЕСТЬNULL(ОбщаяСтоимостьИсходногоОС.СтоимостьУУ, 0) КАК ИсходнаяСтоимостьУУ
	|
	|ИЗ
	|	ВТ_РазукомплектацияОС КАК ВТ_РазукомплектацияОС
	|
	...
	|		ПО (ТаблицаДокумента.Ссылка = ВТ_РазукомплектацияОС.Ссылка)
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ОбщаяСтоимостьНовыхОС КАК ОбщаяСтоимостьНовыхОС
	|		ПО (ОбщаяСтоимостьНовыхОС.Ссылка = ТаблицаДокумента.Ссылка)
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ОбщаяСтоимостьИсходногоОС КАК ОбщаяСтоимостьИсходногоОС
	|		ПО (ОбщаяСтоимостьИсходногоОС.Ссылка = ТаблицаДокумента.Ссылка)
	|
	|ГДЕ
	...
	|		И (ЕСТЬNULL(ОбщаяСтоимостьНовыхОС.СтоимостьУУ, 0) >= ЕСТЬNULL(ОбщаяСтоимостьИсходногоОС.СтоимостьУУ, 0)
	|					И ТаблицаДокумента.ОтражатьВУпрУчете
	|			ИЛИ ЕСТЬNULL(ОбщаяСтоимостьНовыхОС.СтоимостьБУ, 0) >= ЕСТЬNULL(ОбщаяСтоимостьИсходногоОС.СтоимостьБУ, 0)
	|					И ТаблицаДокумента.ОтражатьВРеглУчете
	|			ИЛИ ЕСТЬNULL(ОбщаяСтоимостьНовыхОС.СтоимостьНУ, 0) >= ЕСТЬNULL(ОбщаяСтоимостьИсходногоОС.СтоимостьНУ, 0)
	|					И ТаблицаДокумента.ОтражатьВРеглУчете)
	|
	...
	|			ИЛИ ЕСТЬNULL(ОбщаяСтоимостьНовыхОС.СтоимостьНУ, 0) <> ЕСТЬNULL(ОбщаяСтоимостьИсходногоОС.СтоимостьНУ, 0)
	|					И ТаблицаДокумента.ОтражатьВРеглУчете
	|			ИЛИ ЕСТЬNULL(ОбщаяСтоимостьНовыхОС.СтоимостьПР, 0) <> ЕСТЬNULL(ОбщаяСтоимостьИсходногоОС.СтоимостьПР, 0)
	|					И ТаблицаДокумента.ОтражатьВРеглУчете)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Организация,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если (Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РазукомплектацияОСЧастичная
			...
			ОписаниеПроблемы = НСтр("ru = 'В документе Разукомплектация ОС №%1 от %2 общая стоимость, принимаемых к учету ОС в управленческом учете (%3 %4) должна быть меньше стоимости исходного ОС (%5 %6).';
									|en = 'In the ""Split fixed assets"" document No.%1 dated %2, the total value of recognized fixed assets in management accounting (%3 %4) must be less than the original fixed asset value (%5 %6).'");
			...
			ВнеоборотныеАктивыСлужебный.ЗарегистрироватьПроблемуВыполненияРасчета(
				ПараметрыВыполнения.ОперацияЗакрытияМесяца, 
				Период,
				Выборка.Организация,
				ОписаниеПроблемы,,
				Выборка.Ссылка);
		...
		Если Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РазукомплектацияОСПолная
			...
			Если Выборка.СписатьУзлы Тогда
				ОписаниеПроблемы = НСтр("ru = 'В документе Разукомплектация ОС №%1 от %2 общая стоимость, принимаемых к учету ОС в управленческом учете (%3 %4) должна равняться стоимости исходного ОС (%5 %6) с учетом стоимости узлов и компонент амортизации.';
										|en = 'In the ""Split fixed assets"" document No.%1 dated %2, the total value of recognized fixed assets in management accounting (%3 %4) must be equal to the original fixed asset value (%5 %6) including the cost of sub-assets and depreciation components.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В документе Разукомплектация ОС №%1 от %2 общая стоимость, принимаемых к учету ОС в управленческом учете (%3 %4) должна равняться стоимости исходного ОС (%5 %6) с учетом стоимости компонент амортизации.';
										|en = 'In the ""Split fixed assets"" document No.%1 dated %2, the total value of recognized fixed assets in management accounting (%3 %4) must be equal to the original fixed asset value (%5 %6) including the cost of depreciation components.'");
			КонецЕсли;
		...
		Если (Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РазукомплектацияОСЧастичная
			...
			И Выборка.ОбщаяСтоимостьБУ <> 0
			И Выборка.ОтражатьВРеглУчете Тогда
			...
			ОписаниеПроблемы = НСтр("ru = 'В документе Разукомплектация ОС №%1 от %2 общая стоимость, принимаемых к учету ОС в бухгалтерском учете (%3 %4) должна быть меньше стоимости исходного ОС (%5 %6).';
									|en = 'In the ""Split fixed assets"" document No. %1 dated %2, the total cost of recognized fixed assets in local accounting (%3 %4) must be less than the original fixed asset cost (%5 %6).'");
		...
		Если Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РазукомплектацияОСПолная
			...
			Если Выборка.СписатьУзлы Тогда
				ОписаниеПроблемы = НСтр("ru = 'В документе Разукомплектация ОС №%1 от %2 общая стоимость, принимаемых к учету ОС в бухгалтерском учете (%3 %4) должна равняться стоимости исходного ОС (%5 %6) с учетом стоимости узлов и компонент амортизации.';
										|en = 'In the ""Split fixed assets"" document No. %1 dated %2, the total cost of recognized fixed assets in local accounting (%3 %4) must be equal to the original fixed asset cost (%5 %6) including the cost of sub-assets and depreciation components.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В документе Разукомплектация ОС №%1 от %2 общая стоимость, принимаемых к учету ОС в бухгалтерском учете (%3 %4) должна равняться стоимости исходного ОС (%5 %6) с учетом стоимости компонент амортизации.';
										|en = 'In the ""Split fixed assets"" document No.%1 dated %2, the total value of recognized fixed assets in local accounting (%3 %4) must be equal to the original fixed asset value (%5 %6) including the cost of depreciation components.'");
			КонецЕсли;
		...
		Если (Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РазукомплектацияОСЧастичная
			...
			ОписаниеПроблемы = НСтр("ru = 'В документе Разукомплектация ОС №%1 от %2 общая стоимость, принимаемых к учету ОС в налоговом учете (%3 %4) должна быть меньше стоимости исходного ОС (%5 %6).';
									|en = 'In the ""Split fixed assets"" document No.%1 dated %2, the total value of recognized fixed assets in tax accounting (%3 %4) must be less than the original fixed asset value (%5 %6).'");
		...
		Если Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РазукомплектацияОСПолная
			...
			Если Выборка.СписатьУзлы Тогда
				ОписаниеПроблемы = НСтр("ru = 'В документе Разукомплектация ОС №%1 от %2 общая стоимость, принимаемых к учету ОС в налоговом учете (%3 %4) должна равняться стоимости исходного ОС (%5 %6) с учетом стоимости узлов и компонент амортизации.';
										|en = 'In the ""Split fixed assets"" document No. %1 dated %2, the total cost of recognized fixed assets in tax accounting (%3 %4) must be equal to the original fixed asset cost (%5 %6) including the cost of sub-assets and depreciation components.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В документе Разукомплектация ОС №%1 от %2 общая стоимость, принимаемых к учету ОС в налоговом учете (%3 %4) должна равняться стоимости исходного ОС (%5 %6) с учетом стоимости компонент амортизации.';
										|en = 'In the ""Split fixed assets"" document No.%1 dated %2, the total value of recognized fixed assets in tax accounting (%3 %4) must be equal to the original fixed asset value (%5 %6) including the cost of depreciation components.'");
			КонецЕсли;
		...
		Если Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РазукомплектацияОСПолная
			И Выборка.ОбщаяСтоимостьПР <> Выборка.ИсходнаяСтоимостьПР
			И Выборка.ОтражатьВРеглУчете Тогда
			
			ДокументыСОшибками.Добавить(Выборка.Ссылка);
			
			Если Выборка.СписатьУзлы Тогда
				ОписаниеПроблемы = НСтр("ru = 'В документе Разукомплектация ОС №%1 от %2 постоянная разница в общей стоимости, принимаемых к учету ОС (%3 %4) должна равняться постоянной разнице в стоимости исходного ОС (%5 %6) с учетом стоимости узлов и компонент амортизации.';
										|en = 'In the ""Split fixed assets"" document No.%1 dated %2, the permanent difference in the total value of recognized fixed assets (%3 %4) must be equal to the permanent difference in the original fixed asset value (%5 %6) including the cost of sub-assets and depreciation components.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В документе Разукомплектация ОС №%1 от %2 постоянная разница в общей стоимости, принимаемых к учету ОС (%3 %4) должна равняться постоянной разнице в стоимости исходного ОС (%5 %6) с учетом стоимости компонент амортизации.';
										|en = 'In the ""Split fixed assets"" document No.%1 dated %2, the permanent difference in the total value of recognized fixed assets (%3 %4) must be equal to the permanent difference in the original fixed asset value (%5 %6) including the cost of depreciation components.'");
			КонецЕсли;
			
			ОписаниеПроблемы = СтрШаблон(ОписаниеПроблемы, 
										Выборка.НомерДокумента, 
										Формат(Выборка.ДатаДокумента, "ДЛФ=D"), 
										Выборка.ОбщаяСтоимостьПР,
										ЗначениеНастроекПовтИсп.ВалютаРегламентированногоУчетаОрганизации(Выборка.Организация),
										Выборка.ИсходнаяСтоимостьПР,
										ЗначениеНастроекПовтИсп.ВалютаРегламентированногоУчетаОрганизации(Выборка.Организация));
	...
	Пока Выборка.Следующий() Цикл
		...
		ОписаниеПроблемы = НСтр("ru = 'Реализация услуг и прочих активов №%1 от %2 для внеоборотного актива %3 оформлена раньше чем подготовка к передаче.';
								|en = 'Customer invoice — Services and Assets No. %1 dated %2 for the %3 fixed asset is registered earlier than the preparation for transfer.'");
	...
	|	ЕСТЬNULL(КорректировкаОС.КорректировкаАмортизацииЗаПериод, ЛОЖЬ) КАК КорректировкаАмортизацииЗаПериод,
	|
		...
		Если Выборка.ОтражениеУвеличенияСтоимостиУУ Тогда
			...
			Если ЭтоНМА Тогда
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только увеличения стоимости, но для нематериального актива ""%1"" стоимость (УУ), указанная в документе, меньше фактической.';
										|en = 'Only cost increase recording is selected in the document parameters, but for the ""%1"" intangible asset, the cost (MA) specified in the document is less than the actual cost.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только увеличения стоимости, но для основного средства ""%1"" стоимость (УУ), указанная в документе, меньше фактической.';
										|en = 'Only cost increase recording is selected in the document parameters, but for the ""%1"" fixed asset, the cost (MA) specified in the document is less than the actual cost.'");
			КонецЕсли;
		...
		ИначеЕсли Выборка.ОтражениеУменьшенияСтоимостиУУ Тогда
			...
			Если ЭтоНМА Тогда
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только уменьшения стоимости, но для нематериального актива ""%1"" стоимость (УУ), указанная в документе, больше фактической.';
										|en = 'Only cost decrease recording is selected in the document parameters, but for the ""%1"" intangible asset, the cost (MA) specified in the document is greater than the actual cost.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только уменьшения стоимости, но для основного средства ""%1"" стоимость (УУ), указанная в документе, больше фактической.';
										|en = 'Only cost decrease recording is selected in the document parameters, but for the ""%1"" fixed asset, the cost (MA) specified in the document is greater than the actual cost.'");
			КонецЕсли;
		...
		Если Выборка.ОтражениеУвеличенияСтоимостиБУ Тогда
			...
			Если ЭтоНМА Тогда
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только увеличения стоимости, но для нематериального актива ""%1"" стоимость (БУ), указанная в документе, меньше фактической.';
										|en = 'Only cost increase recording is selected in the document parameters, but for the ""%1"" intangible asset, the cost (LA) specified in the document is less than the actual cost.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только увеличения стоимости, но для основного средства ""%1"" стоимость (БУ), указанная в документе, меньше фактической.';
										|en = 'Only cost increase recording is selected in the document parameters, but for the ""%1"" fixed asset, the cost (LA) specified in the document is less than the actual cost.'");
			КонецЕсли;
		...
		ИначеЕсли Выборка.ОтражениеУменьшенияСтоимостиБУ Тогда
			...
			Если ЭтоНМА Тогда
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только уменьшения стоимости, но для нематериального актива ""%1"" стоимость (БУ), указанная в документе, больше фактической.';
										|en = 'Only cost decrease recording is selected in the document parameters, but for the ""%1"" intangible asset, the cost (LA) specified in the document is greater than the actual cost.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только уменьшения стоимости, но для основного средства ""%1"" стоимость (БУ), указанная в документе, больше фактической.';
										|en = 'Only cost decrease recording is selected in the document parameters, but for the ""%1"" fixed asset, the cost (LA) specified in the document is greater than the actual cost.'");
			КонецЕсли;
		...
		Если Выборка.ОтражениеУвеличенияСтоимостиНУ Тогда
			...
			Если ЭтоНМА Тогда
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только увеличения стоимости, но для нематериального актива ""%1"" стоимость (НУ), указанная в документе, меньше фактической.';
										|en = 'Only cost increase recording is selected in the document parameters, but for the ""%1"" intangible asset, the cost (TA) specified in the document is less than the actual cost.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только увеличения стоимости, но для основного средства ""%1"" стоимость (НУ), указанная в документе, меньше фактической.';
										|en = 'Only cost increase recording is selected in the document parameters, but for the ""%1"" fixed asset, the cost (TA) specified in the document is less than the actual cost.'");
			КонецЕсли;
		...
		ИначеЕсли Выборка.ОтражениеУменьшенияСтоимостиНУ Тогда
			...
			Если ЭтоНМА Тогда
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только уменьшения стоимости, но для нематериального актива ""%1"" стоимость (НУ), указанная в документе, больше фактической.';
										|en = 'Only cost decrease recording is selected in the document parameters, but for the ""%1"" intangible asset, the cost (TA) specified in the document is greater than the actual cost.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только уменьшения стоимости, но для основного средства ""%1"" стоимость (НУ), указанная в документе, больше фактической.';
										|en = 'Only cost decrease recording is selected in the document parameters, but for the ""%1"" fixed asset, the cost (TA) specified in the document is greater than the actual cost.'");
			КонецЕсли;
		...
		Если Выборка.ОтражениеУвеличенияСтоимостиЦФ Тогда
			...
			Если ЭтоНМА Тогда
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только увеличения стоимости, но для нематериального актива ""%1"" стоимость (ЦФ), указанная в документе, меньше фактической.';
										|en = 'Only cost increase recording is selected in the document parameters, but for the ""%1"" intangible asset, the cost (TF) specified in the document is less than the actual cost.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только увеличения стоимости, но для основного средства ""%1"" стоимость (ЦФ), указанная в документе, меньше фактической.';
										|en = 'Only cost increase recording is selected in the document parameters, but for the ""%1"" fixed asset, the cost (TF) specified in the document is less than the actual cost.'");
			КонецЕсли;
		...
		ИначеЕсли Выборка.ОтражениеУменьшенияСтоимостиЦФ Тогда
			...
			Если ЭтоНМА Тогда
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только уменьшения стоимости, но для нематериального актива ""%1"" стоимость (ЦФ), указанная в документе, больше фактической.';
										|en = 'Only cost decrease recording is selected in the document parameters, but for the ""%1"" intangible asset, the cost (TF) specified in the document is greater than the actual cost.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только уменьшения стоимости, но для основного средства ""%1"" стоимость (ЦФ), указанная в документе, больше фактической.';
										|en = 'Only cost decrease recording is selected in the document parameters, but for the ""%1"" fixed asset, the cost (TF) specified in the document is greater than the actual cost.'");
			КонецЕсли;
		...
		Если Выборка.ОтражениеУвеличенияАмортизацииУУ Тогда
			...
			Если ЭтоНМА Тогда
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только увеличения амортизации, но для нематериального актива ""%1"" амортизация (УУ), указанная в документе, меньше фактической.';
										|en = 'Only amortization increase recording is selected in the document parameters, but for the ""%1"" intangible asset, the amortization (MA) specified in the document is less than the actual amortization.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только увеличения амортизации, но для основного средства ""%1"" амортизация (УУ), указанная в документе, меньше фактической.';
										|en = 'Only depreciation increase recording is selected in the document parameters, but for the ""%1"" fixed asset, the depreciation (MA) specified in the document is less than the actual depreciation.'");
			КонецЕсли;
		...
		ИначеЕсли Выборка.ОтражениеУменьшенияАмортизацииУУ Тогда
			...
			Если ЭтоНМА Тогда
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано только отражение уменьшения амортизации, но для нематериального актива ""%1"" амортизация (УУ), указанная в документе, больше фактической.';
										|en = 'Only amortization decrease recording is selected in the document parameters, but for the ""%1"" intangible asset, the amortization (MA) specified in the document is greater than the actual amortization.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано только отражение уменьшения амортизации, но для основного средства ""%1"" амортизация (УУ), указанная в документе, больше фактической.';
										|en = 'Only depreciation decrease recording is selected in the document parameters, but for the ""%1"" fixed asset, the depreciation (MA) specified in the document is greater than the actual depreciation.'");
			КонецЕсли;
		...
		Если Выборка.ОтражениеУвеличенияАмортизацииБУ Тогда
			...
			Если ЭтоНМА Тогда
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только увеличения амортизации, но для нематериального актива ""%1"" амортизация (БУ), указанная в документе, меньше фактической.';
										|en = 'Only amortization increase recording is selected in the document parameters, but for the ""%1"" intangible asset, the amortization (LA) specified in the document is less than the actual amortization.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только увеличения амортизации, но для основного средства ""%1"" амортизация (БУ), указанная в документе, меньше фактической.';
										|en = 'Only depreciation increase recording is selected in the document parameters, but for the ""%1"" fixed asset, the depreciation (LA) specified in the document is less than the actual depreciation.'");
			КонецЕсли;
		...
		ИначеЕсли Выборка.ОтражениеУменьшенияАмортизацииБУ Тогда
			...
			Если ЭтоНМА Тогда
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано только отражение уменьшения амортизации, но для нематериального актива ""%1"" амортизация (БУ), указанная в документе, больше фактической.';
										|en = 'Only amortization decrease recording is selected in the document parameters, but for the ""%1"" intangible asset, the amortization (LA) specified in the document is greater than the actual amortization.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано только отражение уменьшения амортизации, но для основного средства ""%1"" амортизация (БУ), указанная в документе, больше фактической.';
										|en = 'Only depreciation decrease recording is selected in the document parameters, but for the ""%1"" fixed asset, the depreciation (LA) specified in the document is greater than the actual depreciation.'");
			КонецЕсли;
		...
		Если Выборка.ОтражениеУвеличенияАмортизацииНУ Тогда
			...
			Если ЭтоНМА Тогда
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только увеличения амортизации, но для нематериального актива ""%1"" амортизация (НУ), указанная в документе, меньше фактической.';
										|en = 'Only amortization increase recording is selected in the document parameters, but for the ""%1"" intangible asset, the amortization (TA) specified in the document is less than the actual amortization.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только увеличения амортизации, но для основного средства ""%1"" амортизация (НУ), указанная в документе, меньше фактической.';
										|en = 'Only depreciation increase recording is selected in the document parameters, but for the ""%1"" fixed asset, the depreciation (TA) specified in the document is less than the actual depreciation.'");
			КонецЕсли;
		...
		ИначеЕсли Выборка.ОтражениеУменьшенияАмортизацииНУ Тогда
			...
			Если ЭтоНМА Тогда
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано только отражение уменьшения амортизации, но для нематериального актива ""%1"" амортизация (НУ), указанная в документе, больше фактической.';
										|en = 'Only amortization decrease recording is selected in the document parameters, but for the ""%1"" intangible asset, the amortization (TA) specified in the document is greater than the actual amortization.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано только отражение уменьшения амортизации, но для основного средства ""%1"" амортизация (НУ), указанная в документе, больше фактической.';
										|en = 'Only depreciation decrease recording is selected in the document parameters, but for the ""%1"" fixed asset, the depreciation (TA) specified in the document is greater than the actual depreciation.'");
			КонецЕсли;
		...
		Если Выборка.ОтражениеУвеличенияАмортизацииЦФ Тогда
			...
			Если ЭтоНМА Тогда
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только увеличения амортизации, но для нематериального актива ""%1"" амортизация (ЦФ), указанная в документе, меньше фактической.';
										|en = 'Only amortization increase recording is selected in the document parameters, but for the ""%1"" intangible asset, the amortization (TF) specified in the document is less than the actual amortization.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано отражение только увеличения амортизации, но для основного средства ""%1"" амортизация (ЦФ), указанная в документе, меньше фактической.';
										|en = 'Only depreciation increase recording is selected in the document parameters, but for the ""%1"" fixed asset, the depreciation (TF) specified in the document is less than the actual depreciation.'");
			КонецЕсли;
		...
		ИначеЕсли Выборка.ОтражениеУменьшенияАмортизацииЦФ Тогда
			...
			Если ЭтоНМА Тогда
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано только отражение уменьшения амортизации, но для нематериального актива ""%1"" амортизация (ЦФ), указанная в документе, больше фактической.';
										|en = 'Only amortization decrease recording is selected in the document parameters, but for the ""%1"" intangible asset, the amortization (TF) specified in the document is greater than the actual amortization.'");
			Иначе
				ОписаниеПроблемы = НСтр("ru = 'В параметрах документа выбрано только отражение уменьшения амортизации, но для основного средства ""%1"" амортизация (ЦФ), указанная в документе, больше фактической.';
										|en = 'Only depreciation decrease recording is selected in the document parameters, but for the ""%1"" fixed asset, the depreciation (TF) specified in the document is greater than the actual depreciation.'");
			КонецЕсли;
	...
	ОтложенноеФормированиеДвиженийВНАЛокализация.ПроверитьОтложенноеФормированиеДвижений(Период, ПараметрыВыполнения);
...
Функция СоздатьОчередьЗаданий(НачалоПериода, КонецПериода, СписокОрганизаций, НомерДоРасчета, МенеджерВременныхТаблиц, ОперацияЗакрытияМесяца, УдалитьЗадания)
		...
		Если ОчередьЗаданийКРасчету.Количество() < Выборка.НомерОчереди Тогда
			...
			Иначе
				ДанныеЗадания.НаименованиеФоновогоЗадания = НСтр("ru = 'Формирование движений по ОС и НМА';
																|en = 'Generate fixed asset and intangible asset records'"); 
			КонецЕсли;
			...
			ПараметрыВыполнения.ОперацияЗакрытияМесяца = ОперацияЗакрытияМесяца;
...
Функция ТребуетсяАктуализацияДвиженийПередРасчетомАмортизации(ПараметрыОбработчика)
	
	Результат = Новый Структура;
	Результат.Вставить("ТребуетсяАктуализацияДвижений", Ложь);
	Результат.Вставить("ЕстьЗаданияКЗаданияКРасчетуАмортизацииОС", Ложь);
	Результат.Вставить("ЕстьЗаданияКЗаданияКРасчетуАмортизацииНМА", Ложь);
	
	ДанныеЗаданий = ВнеоборотныеАктивы.ДанныеЗаданийКЗакрытиюМесяца(
		ПараметрыОбработчика.ПараметрыРасчета.КонецПериода, 
		ПараметрыОбработчика.ПараметрыРасчета.МассивОрганизаций, 
		Метаданные.РегистрыСведений.ЗаданияКФормированиюДвиженийПоВНА.Имя);
	
	Если ДанныеЗаданий.КоличествоОбъектов = 0
		ИЛИ ДанныеЗаданий.ДатаНачалаРасчета = '00010101' Тогда
		Возврат Результат;
	КонецЕсли;
	
	ДанныеЗаданийАмортизацияОС = ВнеоборотныеАктивы.ДанныеЗаданийКЗакрытиюМесяца(
		ПараметрыОбработчика.ПараметрыРасчета.КонецПериода, 
		ПараметрыОбработчика.ПараметрыРасчета.МассивОрганизаций, 
		Метаданные.РегистрыСведений.ЗаданияКРасчетуАмортизацииОС.Имя);
		
	ДанныеЗаданийАмортизацияНМА = ВнеоборотныеАктивы.ДанныеЗаданийКЗакрытиюМесяца(
		ПараметрыОбработчика.ПараметрыРасчета.КонецПериода, 
		ПараметрыОбработчика.ПараметрыРасчета.МассивОрганизаций,
		Метаданные.РегистрыСведений.ЗаданияКРасчетуАмортизацииНМА.Имя);
	
	Результат.ЕстьЗаданияКЗаданияКРасчетуАмортизацииОС = 
		ДанныеЗаданийАмортизацияОС.КоличествоОбъектов <> 0
		И ДанныеЗаданийАмортизацияОС.ДатаНачалаРасчета <> '00010101';
			
	Результат.ЕстьЗаданияКЗаданияКРасчетуАмортизацииНМА = 
		ДанныеЗаданийАмортизацияНМА.КоличествоОбъектов <> 0
		И ДанныеЗаданийАмортизацияНМА.ДатаНачалаРасчета <> '00010101';
	...
	ЕстьРасчетСДатыПринятияКУчетуОС = Ложь;
	ЕстьРасчетСДатыПринятияКУчетуНМА = Ложь;
	
	ВедетсяРегламентированныйУчетВНА = ВнеоборотныеАктивыСлужебный.ВедетсяРегламентированныйУчетВНА();
		
	НастройкиУчетаУУ = НастройкиНалоговУчетныхПолитик.ДействующиеПараметрыНалоговУчетныхПолитикПоОрганизациям(
		Метаданные.РегистрыСведений.УчетнаяПолитикаФинансовогоУчета.Имя, 
		ПараметрыОбработчика.ПараметрыРасчета.МассивОрганизаций, 
		ПараметрыОбработчика.ПараметрыРасчета.КонецПериода);
	
	Для Каждого Элемент Из НастройкиУчетаУУ Цикл
		
		Если Результат.ЕстьЗаданияКЗаданияКРасчетуАмортизацииОС
				И (Элемент.ПорядокНачисленияАмортизацииОС = Перечисления.ПорядокНачисленияАмортизации.СДатыПринятияКУчету
					ИЛИ Элемент.ПорядокНачисленияАмортизацииАренда = Перечисления.ПорядокНачисленияАмортизации.СДатыПринятияКУчету
					ИЛИ Элемент.ПорядокНачисленияАмортизацииОСРегл = Перечисления.ПорядокНачисленияАмортизации.СДатыПринятияКУчету
						И НЕ ВедетсяРегламентированныйУчетВНА
					ИЛИ Элемент.ПорядокНачисленияАмортизацииАрендаРегл = Перечисления.ПорядокНачисленияАмортизации.СДатыПринятияКУчету
						И НЕ ВедетсяРегламентированныйУчетВНА) Тогда
						
			ЕстьРасчетСДатыПринятияКУчетуОС = Истина;
			Прервать;
		КонецЕсли;
		
		Если Результат.ЕстьЗаданияКЗаданияКРасчетуАмортизацииНМА
				И (Элемент.ПорядокНачисленияАмортизацииНМА = Перечисления.ПорядокНачисленияАмортизации.СДатыПринятияКУчету
					ИЛИ Элемент.ПорядокНачисленияАмортизацииНМАРегл = Перечисления.ПорядокНачисленияАмортизации.СДатыПринятияКУчету
						И НЕ ВедетсяРегламентированныйУчетВНА) Тогда
						
			ЕстьРасчетСДатыПринятияКУчетуНМА = Истина;
			Прервать;
		КонецЕсли;
	...
	Если НЕ ЕстьРасчетСДатыПринятияКУчетуОС ИЛИ НЕ ЕстьРасчетСДатыПринятияКУчетуНМА Тогда
		
		НастройкиУчетаБУ = НастройкиНалоговУчетныхПолитик.ДействующиеПараметрыНалоговУчетныхПолитикПоОрганизациям(
			Метаданные.РегистрыСведений.УчетнаяПолитикаБухУчета.Имя, 
			ПараметрыОбработчика.ПараметрыРасчета.МассивОрганизаций, 
			ПараметрыОбработчика.ПараметрыРасчета.КонецПериода);
			
		Для Каждого Элемент Из НастройкиУчетаБУ Цикл
			
			Если Результат.ЕстьЗаданияКЗаданияКРасчетуАмортизацииОС
					И (Элемент.ПорядокНачисленияАмортизацииОС = Перечисления.ПорядокНачисленияАмортизации.СДатыПринятияКУчету
						ИЛИ Элемент.ПорядокНачисленияАмортизацииАренда = Перечисления.ПорядокНачисленияАмортизации.СДатыПринятияКУчету) Тогда
			...
			Если Результат.ЕстьЗаданияКЗаданияКРасчетуАмортизацииНМА
				И Элемент.ПорядокНачисленияАмортизацииНМА = Перечисления.ПорядокНачисленияАмортизации.СДатыПринятияКУчету Тогда
	...
	Если НЕ (НачалоМесяца(ДанныеЗаданий.ДатаНачалаРасчета) < НачалоМесяца(ДанныеЗаданийАмортизацияОС.ДатаНачалаРасчета)
				ИЛИ НачалоМесяца(ДанныеЗаданий.ДатаНачалаРасчета) = НачалоМесяца(ДанныеЗаданийАмортизацияОС.ДатаНачалаРасчета)
					И ЕстьРасчетСДатыПринятияКУчетуОС
				ИЛИ НачалоМесяца(ДанныеЗаданий.ДатаНачалаРасчета) < НачалоМесяца(ДанныеЗаданийАмортизацияНМА.ДатаНачалаРасчета)
				ИЛИ НачалоМесяца(ДанныеЗаданий.ДатаНачалаРасчета) = НачалоМесяца(ДанныеЗаданийАмортизацияНМА.ДатаНачалаРасчета)
					И ЕстьРасчетСДатыПринятияКУчетуНМА) Тогда
	
		Возврат Результат;
	КонецЕсли;
	
	ПараметрыОбработчика.ДанныеЭтапа.ДатаНачалаРасчета = ДанныеЗаданий.ДатаНачалаРасчета;

	ШаблонСостояния = ?(ДанныеЗаданий.КоличествоОбъектов = 1,
	...
	ЗакрытиеМесяцаСервер.УстановитьСостояниеНеВыполнен(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонСостояния,
			РасчетСебестоимостиПротоколРасчета.ПредставлениеПериодаРасчета(ДанныеЗаданий.ДатаНачалаРасчета),
			Формат(ДанныеЗаданий.КоличествоОбъектов, "ЧГ=3,0;")));
	
	Результат.ТребуетсяАктуализацияДвижений = Истина;
...
Функция МаксимальноеКоличествоЗаданий()
	...
	ЗначенияПараметров = Константы.НастройкиЗакрытияМесяца.СоздатьМенеджерЗначения().УстановленныеЗначенияПараметровОперации(
  							Справочники.ОперацииЗакрытияМесяца.ФормированиеДвиженийПоВНА); // Число
...
Функция ТипыДокументовКоторыеМогутНачислитьАмортизацию() Экспорт
...
Функция ТипыДокументовКоторыеИзменяютСтоимостьВКонцеМесяца() Экспорт
