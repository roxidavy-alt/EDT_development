...
// 	НаборКорректировок - СправочникСсылка.КорректировкиПрошлыхПериодовНУ - Карточка расчета налога на прибыль
//
...
//		* ДатаЗакрытияНабора - Дата -
//		* ЭтоПерерасчет - Булево -
//		* НачалоМесяца - Дата -
//		* НалоговыйПериод - Дата - начало стандартного налогового периода.
//		* КонецПериода - Дата -
//		* НачалоНалоговогоПериода - Дата - фактическое начало налогового периода (может совпадать со стандартным началом или датой регистрации организации.
//		* РеквизитыРеглОперации - см. Документы.РегламентнаяОперация.ПодготовитьДанныеРасчетаНалогаНаПрибыль
// * РасчетТорговогоСбора - ТаблицаЗначений - См. ТаблицаРасчетаТорговогоСбора()
Функция НовыйКонтекстРасчета(Организация, КонецПериода, РеквизитыРеглОперации = Неопределено, НаборКорректировок = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(НаборКорректировок) Тогда
		КонтекстРасчета.Вставить("НаборКорректировок",  НаборКорректировок);
		КонтекстРасчета.Вставить("ДатаЗакрытияНабора",  ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НаборКорректировок,"ДатаНачалаПодготовкиДекларации"));
		КонтекстРасчета.Вставить("ЭтоПерерасчет",       Истина);
	Иначе
		КонтекстРасчета.Вставить("НаборКорректировок",  Справочники.КорректировкиПрошлыхПериодовНУ.ПустаяСсылка());
		КонтекстРасчета.Вставить("ДатаЗакрытияНабора",  Дата(1,1,1));
		КонтекстРасчета.Вставить("ЭтоПерерасчет",       Ложь);
	КонецЕсли;
	...
	КонтекстРасчета.Вставить("ПараметрыУчетаОтложенногоНалогаВДобавочномКапитале",
		НалогНаПрибыльБухгалтерскийУчет.ПараметрыУчетаОтложенногоНалогаВДобавочномКапитале(НачалоМесяца));
...
Функция ПараметрыРасчета() Экспорт
	...
	СтруктураПараметров.Вставить("ЭтоПерерасчет", Ложь);
	СтруктураПараметров.Вставить("ДатаЗакрытияНабора", Дата(1,1,1));
...
Функция СуммыНалоговойБазы(НачалоПериода, КонецПериода, Организация, ОписаниеСумм = Неопределено, СчетНалоговойБазы = Неопределено) Экспорт
	...
	ТаблицаПроводок = Документы.РегламентнаяОперация.ТаблицаПроводокНовый();
	
	СтруктураПараметров = ПараметрыРасчета();
...
//  ТаблицаПроводок - ТаблицаЗначений - см. Документы.РегламентнаяОперация.ТаблицаПроводокНовый()
// 
// Возвращаемое значение:
...
Функция СуммыНалоговойБазыВРазрезеВариантовНалогообложения(
	СтруктураПараметров, Знач ИспользуетсяРаздельныйУчет = Неопределено, Знач ТаблицаПроводок = Неопределено) Экспорт
...
Процедура ПодготовитьВТ_СуммыНалоговойБазыВРазрезеВариантовНалогообложения(МенеджерВременныхТаблиц,
	СтруктураПараметров, Знач ИспользуетсяРаздельныйУчет = Неопределено) Экспорт
...
//  ТаблицаРеквизитыРасчетНалога - ТаблицаЗначений - см. Документы.РегламентнаяОперация.ПодготовитьДанныеРасчетаНалогаНаПрибыль() -
//  							содержит данные регистратора движений в РС РасчетНалогаНаПрибыль.	
...
//
Функция НачислитьНалогНаПрибыль(Движения, КонтекстРасчета, РасчетРаспределенияНалоговойБазыОП, ТаблицаБазаНалогаЗаГод, ТаблицаРеквизитыРасчетНалога, МенеджерВременныхТаблиц, ТаблицаПроводок) Экспорт
	...
	Если НЕ КонтекстРасчета.ЭтоПерерасчет Тогда
	...
	Если КонтекстРасчета.ЭтоПерерасчет И КонтекстРасчета.НачалоМесяца = КонтекстРасчета.НачалоНалоговогоПериода Тогда
		ДополнитьУжеСформированнымиПроводкамиПоНачислениюНалога(КонтекстРасчета, ТаблицаПроводок);
	КонецЕсли;
	...
	Для Каждого ДанныеПроводки Из НачислениеНалога Цикл
		...
		Если КонтекстРасчета.ЭтоПерерасчет Тогда
			...
			Если ДанныеПроводки.СуммаНалога = 0 И ДанныеПроводки.НалоговаяБаза = 0 Тогда
				Продолжить;
			КонецЕсли;
			
			СтрокаПроводки = ТаблицаПроводок.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаПроводки, КонтекстРасчета, "Организация, ПрименяетсяПБУ18, НаборКорректировок");
			ЗаполнитьЗначенияСвойств(СтрокаПроводки,ДанныеПроводки,,"РегистрацияВНалоговомОргане");
			СтрокаПроводки.СчетДт = СчетДт;
			СтрокаПроводки.СчетКт = ПланыСчетов.Хозрасчетный.РасчетыСБюджетом;
			Если СубконтоРегистрацияВНалоговомОргане <> Неопределено Тогда
				СтрокаПроводки.РегистрацияВНалоговомОргане = ДанныеПроводки.РегистрацияВНалоговомОргане;
			КонецЕсли;
			СтрокаПроводки.ПериодРасчета = НачалоДня(КонтекстРасчета.КонецПериода);
			СтрокаПроводки.Сумма = ДанныеПроводки.СуммаПроводки;
			СтрокаПроводки.ВидПлатежейВГосБюджет = Перечисления.ВидыПлатежейВГосБюджет.Налог;
...
Функция НовыйРаспределениеНалогаНаПрибыль() Экспорт
	...
	РаспределениеНалогаНаПрибыль.Колонки.Добавить("СтавкаНалога",                Новый ОписаниеТипов("Число"));
	Возврат РаспределениеНалогаНаПрибыль;
	
КонецФункции

// Создаем временную таблицу с данными Проводок, по которые нужно учесть при перерасчете налога на прибыль
// 
// Параметры:
//  МенеджерВременныхТаблиц - МенеджерВременныхТаблиц.
//  ТаблицаПроводок - Неопределено - ТаблицаЗначений - см. Документы.РегламентнаяОперация.ТаблицаПроводокНовый
Процедура СоздатьВТ_ТаблицаПроводок(МенеджерВременныхТаблиц, ТаблицаПроводок = Неопределено) Экспорт
	...
	|	ТаблицаПроводок.ПериодРасчета КАК Период,
	|	ТаблицаПроводок.УбыткиПрошлыхЛет КАК УбыткиПрошлыхЛет,
	|	ТаблицаПроводок.СтатьяРасходов КАК СтатьяРасходов,
	|	ТаблицаПроводок.ВариантНалогообложенияПрибыли КАК ВариантНалогообложенияПрибыли,
	|	ТаблицаПроводок.Организация КАК Организация,
	|	ТаблицаПроводок.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ТаблицаПроводок.Подразделение КАК Подразделение,
	|	ТаблицаПроводок.Сумма КАК СуммаНУ
	|ПОМЕСТИТЬ ДанныеПроводок
	|ИЗ
...
Функция НачислениеНалогаНаПрибыль(КонтекстРасчета,
	...
	ИсключитьРанееНачисленныеАвансы(НачислениеНалога, КонтекстРасчета, СоответствиеПрежнихИТекущихНалоговыхОрганов, ТаблицаПроводок);
	
	Возврат НачислениеНалога;
	
КонецФункции

Процедура ИсключитьРанееНачисленныеАвансы(НачислениеНалога, КонтекстРасчета, СоответствиеПрежнихИТекущихНалоговыхОрганов = Неопределено, ТаблицаПроводок = Неопределено)
	...
	Если КонтекстРасчета.НачалоМесяца > КонтекстРасчета.НачалоНалоговогоПериода Тогда
		...
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("КонецПредыдущегоПериода", НачалоДня(КонтекстРасчета.НачалоМесяца - 1));
		Запрос.УстановитьПараметр("Организация",             КонтекстРасчета.Организация);
		Запрос.УстановитьПараметр("ДатаЗакрытияНабора",      ?(ЗначениеЗаполнено(КонтекстРасчета.ДатаЗакрытияНабора),
																КонтекстРасчета.ДатаЗакрытияНабора, КонтекстРасчета.КонецПериода));
		...
		Если ТипЗнч(СоответствиеПрежнихИТекущихНалоговыхОрганов) = Тип("МенеджерВременныхТаблиц")
			...
			Запрос.Текст = 
			"ВЫБРАТЬ
			|	ТаблицаПроводок.ПериодРасчета КАК ПериодРасчета,
			|	ТаблицаПроводок.УровеньБюджета КАК УровеньБюджета,
			|	ТаблицаПроводок.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
			|	ТаблицаПроводок.ВидПлатежейВГосБюджет КАК ВидПлатежейВГосБюджет,
			|	ТаблицаПроводок.ВариантНалогообложенияПрибыли КАК ВариантНалогообложенияПрибыли,
			|	ТаблицаПроводок.СуммаНалога КАК СуммаНалога
			|ПОМЕСТИТЬ ДанныеПроводок
			|ИЗ
			|	&ТаблицаПроводок КАК ТаблицаПроводок
			|;
			...
			|	МАКСИМУМ(ВложенныйЗапрос.ДатаНачалаПодготовкиДекларации) КАК ДатаНачалаПодготовкиДекларации
			|ПОМЕСТИТЬ ПромежуточнаяТаблица
			|ИЗ
			|(ВЫБРАТЬ
			|	РасчетНалогаНаПрибыль.ПериодРасчета КАК ПериодРасчета,
			|	РасчетНалогаНаПрибыль.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
			|	РасчетНалогаНаПрибыль.ВариантНалогообложенияПрибыли КАК ВариантНалогообложенияПрибыли,
			|	ЕСТЬNULL(НаборыКорректировок.ДатаНачалаПодготовкиДекларации, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаНачалаПодготовкиДекларации
			|	ИЗ
			|		РегистрСведений.РасчетНалогаНаПрибыль КАК РасчетНалогаНаПрибыль
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КорректировкиПрошлыхПериодовНУ КАК НаборыКорректировок
			|		ПО РасчетНалогаНаПрибыль.НаборКорректировок = НаборыКорректировок.Ссылка
			|			И (НаборыКорректировок.СборКорректировокЗакрыт)
			|			И (НаборыКорректировок.ДатаНачалаПодготовкиДекларации <= &ДатаЗакрытияНабора)
			|	ГДЕ
			|		РасчетНалогаНаПрибыль.ПериодРасчета = &КонецПредыдущегоПериода
			|		И РасчетНалогаНаПрибыль.Организация= &Организация
			|		И (НЕ НаборыКорректировок.Ссылка ЕСТЬ NULL
			|			ИЛИ РасчетНалогаНаПрибыль.НаборКорректировок = ЗНАЧЕНИЕ(Справочник.КорректировкиПрошлыхПериодовНУ.ПустаяССылка))
			|	
			|	ОБЪЕДИНИТЬ ВСЕ
			|	
			|	ВЫБРАТЬ
			|		ДанныеПроводок.ПериодРасчета КАК ПериодРасчета,
			|		ДанныеПроводок.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
			|		ДанныеПроводок.ВариантНалогообложенияПрибыли КАК ВариантНалогообложенияПрибыли,
			|		&ДатаЗакрытияНабора КАК ДатаНачалаПодготовкиДекларации
			|	ИЗ
			|		ДанныеПроводок КАК ДанныеПроводок
			|	ГДЕ
			|		ДанныеПроводок.ПериодРасчета = &КонецПредыдущегоПериода) КАК ВложенныйЗапрос
			|
			|СГРУППИРОВАТЬ ПО
			...
			|
			|ВЫБРАТЬ
			|	РасчетНалогаНаПрибыль.УровеньБюджета КАК УровеньБюджета,
			|	РасчетНалогаНаПрибыль.ВариантНалогообложенияПрибыли КАК ВариантНалогообложенияПрибыли,
			|	ВЫБОР
			|		КОГДА РасчетНалогаНаПрибыль.УровеньБюджета = ЗНАЧЕНИЕ(Перечисление.УровниБюджетов.РегиональныйБюджет)
			|			ТОГДА РасчетНалогаНаПрибыль.РегистрацияВНалоговомОргане
			|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.РегистрацииВНалоговомОргане.ПустаяСсылка)
			|	КОНЕЦ КАК РегистрацияВНалоговомОргане,
			|	РасчетНалогаНаПрибыль.СуммаНалога КАК СуммаАвансовыхПлатежей
			|ПОМЕСТИТЬ ПрежниеНачисления
			|ИЗ
			|	РегистрСведений.РасчетНалогаНаПрибыль КАК РасчетНалогаНаПрибыль
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПромежуточнаяТаблица КАК ПромежуточнаяТаблица
			|		ПО РасчетНалогаНаПрибыль.ПериодРасчета = ПромежуточнаяТаблица.ПериодРасчета
			|			И РасчетНалогаНаПрибыль.РегистрацияВНалоговомОргане = ПромежуточнаяТаблица.РегистрацияВНалоговомОргане
			|			И РасчетНалогаНаПрибыль.ВариантНалогообложенияПрибыли = ПромежуточнаяТаблица.ВариантНалогообложенияПрибыли
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КорректировкиПрошлыхПериодовНУ КАК КорректировкиПрошлыхПериодовНУ
			|		ПО РасчетНалогаНаПрибыль.НаборКорректировок = КорректировкиПрошлыхПериодовНУ.Ссылка
			|ГДЕ
			|	РасчетНалогаНаПрибыль.Организация= &Организация
			...
			|			
			|ВЫБРАТЬ
			|	ДанныеПроводок.УровеньБюджета КАК УровеньБюджета,
			|	ДанныеПроводок.ВариантНалогообложенияПрибыли КАК ВариантНалогообложенияПрибыли,
			|	ВЫБОР
			|		КОГДА ДанныеПроводок.УровеньБюджета = ЗНАЧЕНИЕ(Перечисление.УровниБюджетов.РегиональныйБюджет)
			|			ТОГДА ДанныеПроводок.РегистрацияВНалоговомОргане
			|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.РегистрацииВНалоговомОргане.ПустаяСсылка)
			|	КОНЕЦ КАК РегистрацияВНалоговомОргане,
			|	ДанныеПроводок.СуммаНалога КАК СуммаАвансовыхПлатежей
			|	ИЗ
			|		ДанныеПроводок КАК ДанныеПроводок
			|	ГДЕ
			|		ДанныеПроводок.ПериодРасчета = &КонецПредыдущегоПериода
			|	
			|ИНДЕКСИРОВАТЬ ПО
			|	РегистрацияВНалоговомОргане
			|;
	...
	Для Каждого Начисление Из НачислениеНалога Цикл
		...
		Начисление.СуммаНалогаПрошлыхМесяцев = Начисление.СуммаАвансовыхПлатежей;
	КонецЦикла;
...
Функция НовыйНачислениеНалогаНаПрибыль(ТочностьРасчета = 0)
	...
	НачислениеНалога.Колонки.Добавить("НалоговаяБаза",                 ТипСуммаНалога);
	НачислениеНалога.Колонки.Добавить("СуммаНалогаПрошлыхМесяцев",     ТипСуммаНалога);
	НачислениеНалога.Колонки.Добавить("СтавкаНалога",                  ОбщегоНазначения.ОписаниеТипаЧисло(5, 2));
...
Процедура СвернутьНачислениеНалогаНаПрибыль(НачислениеНалога, БезУчетаВариантаНалогообложения = Ложь)
	...
	мКолонкиГруппировки.Добавить("СтавкаНалога");
	
	КолонкиГруппировки  = СтрСоединить(мКолонкиГруппировки, ", ");
	
	КолонкиСуммирования = "СуммаНалога, СуммаАвансовыхПлатежей, СуммаПроводки, СуммаНалогаПрошлыхМесяцев, НалоговаяБаза";
...
Процедура ДобавитьНачислениеНалогаНаПрибыль(НачислениеНалога,
	...
	Начисление.ВариантНалогообложенияПрибыли = Строка.ВариантНалогообложенияПрибыли;
	Начисление.СуммаНалога                   = СуммаНалога;
	Начисление.НалоговаяБаза                   = Строка.НалоговаяБаза;
	Если УровеньБюджета = Перечисления.УровниБюджетов.ФедеральныйБюджет Тогда
		Начисление.СтавкаНалога              = Строка.СтавкаФедеральныйБюджет * 100;
	Иначе
		Начисление.СтавкаНалога              = Строка.СтавкаРегиональныйБюджет * 100;
	КонецЕсли;
...
//  	*КонецПериода  - Дата - конец периода расчета.
//  	*Организация   - СправочникСсылка.Организации - налогоплательщик (головная организация).
//  ТаблицаПроводок - см. Документы.РегламентнаяОперация.ТаблицаПроводокНовый().
//  МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - 
//  СчетНалоговойБазы - ПланСчетовСсылка.Хозрасчетный - субсчет счета прибылей и убытков, соответствующий части налоговой базы,
//                      облагаемой по отдельной ставке налога (в частном случае - не облагаемой).
//                      Если не передан, подразумевается счет 99.01.1
...
//   * СуммаВР - Число.
//
Функция РезультатЗапроса_СуммыНалоговойБазы(СтруктураПараметров,
											ТаблицаПроводок,
	...
	Запрос.УстановитьПараметр("ВидыСубконтоНУ",   ВидыСубконтоНУ);
	Запрос.УстановитьПараметр("НеПерерасчетНалога",   НЕ СтруктураПараметров.ЭтоПерерасчет);
	Запрос.УстановитьПараметр("ДатаЗакрытияНабора",   СтруктураПараметров.ДатаЗакрытияНабора);
	...
	|ГДЕ
	|	КорректировкиПрошлыхПериодовНУ.Владелец = &Организация
	|	И КорректировкиПрошлыхПериодовНУ.СборКорректировокЗакрыт
	...
	|		ЕСТЬNULL(-ПрибылиУбытки.СуммаОборот, 0) КАК Сумма,
	|		ЕСТЬNULL(-ПрибылиУбытки.СуммаНУОборот, 0) КАК СуммаНУ,
	|		ЕСТЬNULL(-ПрибылиУбытки.СуммаПРОборот, 0) КАК СуммаПР,
	|		ЕСТЬNULL(-ПрибылиУбытки.СуммаВРОборот, 0) КАК СуммаВР
	|	ИЗ
	...
	|		0,
	|		ЕСТЬNULL(-ПрибылиУбытки.СуммаНУОборот, 0),
	|		0,
	|		0
	|	ИЗ
	...
	|				&КонецПериода,
	|				,
	|				Счет В (&СчетаПрибылиУбытки),     // Может быть Списание убытков прошлых лет
	|				&ВидыСубконто,
	|				Организация = &Организация
	|				И (НаборКорректировок В (ВЫБРАТЬ Т.НаборКорректировок ИЗ ВТКарточки КАК Т))
	|				И Субконто1 В (&СоставБазы),
	|				,
	|				) КАК ПрибылиУбытки
	|	
	...
	|	
	|	ВЫБРАТЬ
	|		&ПолеНУВариантНалогообложенияПрибыли,
	|		0,
	|		ЕСТЬNULL(-ПрибылиУбытки.СуммаНУОборот, 0),
	|		0,
	|		0
	...
	|	
	|	ВЫБРАТЬ
	|		&ПолеПроводкиВариантНалогообложенияПрибыли,
	|		0,
	|		ВЫБОР
	|			КОГДА ДанныеПроводок.СчетКТ В (&СчетаПрибылиУбытки)
	|				ТОГДА ДанныеПроводок.Сумма
	|			ИНАЧЕ - ДанныеПроводок.Сумма
	|		КОНЕЦ,
	|		0,
	|		0
	|	ИЗ
	...
	|		И (ДанныеПроводок.СчетДТ В (&СчетаПрибылиУбытки, &СчетаНУПрибылиУбытки)
	|			ИЛИ ДанныеПроводок.СчетКТ В (&СчетаПрибылиУбытки))
	|) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.ВариантНалогообложенияПрибыли";
	...
	Если НЕ ВРазрезеВариантовНалогообложения Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ВидыСубконтоНУ", "");
	КонецЕсли;
...
// Создаем временную таблицу с Наборами корректировок, движения по которым нужно учесть при перерасчете налога на прибыль.

// Создать в т наборы корректировок.
// 
// Параметры:
//  МенеджерВременныхТаблиц - МенеджерВременныхТаблиц.
//  ДатаЗакрытияНабора - Дата - дата закрытия набора.
//  Организация - СправочникСсылка.Организации.
Процедура СоздатьВТ_НаборыКорректировок(МенеджерВременныхТаблиц, ДатаЗакрытияНабора, Организация = Неопределено) Экспорт
	...
	|	ВЫБРАТЬ
	|	КорректировкиПрошлыхПериодовНУ.Ссылка КАК НаборКорректировок
	|ПОМЕСТИТЬ ВТНаборыКорректировок
	|ИЗ
	|	Справочник.КорректировкиПрошлыхПериодовНУ КАК КорректировкиПрошлыхПериодовНУ
	|ГДЕ
	|	КорректировкиПрошлыхПериодовНУ.СборКорректировокЗакрыт
	|	И КорректировкиПрошлыхПериодовНУ.Владелец В (&Организация)
	|	И КорректировкиПрошлыхПериодовНУ.ДатаНачалаПодготовкиДекларации <= &ДатаЗакрытияНабора
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	НаборКорректировок";
...
Процедура ДобавитьПроводкиУменьшениеНаСуммуТорговогоСбора(Движения, НачислениеНалога, КонтекстРасчета)
	...
	Для Каждого ЗаписьРасходы Из Расходы Цикл
		...
		СтруктураИзмеренийНастройки.АналитикаУчета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтатьяТорговогоСбора, "ГруппаФинансовогоУчетаРегл");
		СтруктураИзмеренийНастройки.МестоУчета = ЗаписьРасходы.Подразделение;
		НастройкиОтраженияТорговогоСбора = РегистрыСведений.ПорядокОтраженияНаСчетахУчета.НастройкаОтраженияРаздела("Расходы", СтруктураИзмеренийНастройки, "РасходыТекущегоПериода");
		// Деятельность, облагаемая торговым сбором, не облагается ЕНВД:
		СчетСписанияОСНО = НастройкиОтраженияТорговогоСбора.РасходыТекущегоПериода;
		
		ЗаполнитьАналитику90Счета(Период, Организация, СчетСписанияОСНО, СтатьяТорговогоСбора, ПроводкиРасходы.Дебет90Кредит44);
...
Процедура ЗаполнитьАналитику90Счета(Знач Период, Знач Организация, Знач СчетСписанияОСНО, Знач СтатьяТорговогоСбора, Проводка)

	Проводка.СчетДт = ПланыСчетов.Хозрасчетный.Продажи_РасходыНаПродажуНеЕНВД;
	
	ВидыДетализацииУчета = РеглУчетСервер.ВидыДетализацииУчета();
	ПериодыДетализацииУчета = РеглУчетСервер.ПериодыДетализацииУчета(НачалоМесяца(Период), 
		Период, ВидыДетализацииУчета.РеглУчетПродажПоПартиямРеализованнойНоменклатуры);

	Если ЗначениеЗаполнено(СчетСписанияОСНО) Тогда
		Проводка.СчетДт = СчетСписанияОСНО;
	ИначеЕсли ПериодыДетализацииУчета.ДанныеСДетализацией Тогда
		СчетаРасходыНаПродажу = РеглУчетВыборкиСерверПовтИсп.СчетаРасходыНаПродажу();
		
		Если СчетаРасходыНаПродажу.Количество() > 0 Тогда
			Проводка.СчетДт = СчетаРасходыНаПродажу[0];
		КонецЕсли;	
	КонецЕсли;
	
	ОсновнойВариантНалогообложенияПрибыли = РаздельныйУчетПоНалогуНаПрибыль.ОсновнойВариантНалогообложенияПрибыли(Организация, Период);
	
	Если ПериодыДетализацииУчета.ДанныеСДетализацией Тогда
		БухгалтерскийУчет.УстановитьСубконто(
			Проводка.СчетДт,
			Проводка.СубконтоДт,
			ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат,
			СтатьяТорговогоСбора);
		
		Если ЗначениеЗаполнено(ОсновнойВариантНалогообложенияПрибыли) Тогда
			ПараметрыАналитики = РеглУчетВыборкиСерверПовтИсп.ПараметрыАналитикиНалогообложенияРеализаций();
			ПараметрыАналитики.ВариантНалогообложенияПрибыли = ОсновнойВариантНалогообложенияПрибыли;
			
			НалогообложениеРеализаций = РеглУчетВыборкиСерверПовтИсп.НалогообложениеРеализаций(ПараметрыАналитики);
			
			БухгалтерскийУчет.УстановитьСубконто(
				Проводка.СчетДт,
				Проводка.СубконтоДт,
				ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НалогообложенияРеализаций,
				НалогообложениеРеализаций);
		КонецЕсли;
	Иначе
		Если ЗначениеЗаполнено(ОсновнойВариантНалогообложенияПрибыли) Тогда
			БухгалтерскийУчет.УстановитьСубконто(
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Проводка.СуммаНУКт > 0 Тогда
			Проводка.Содержание  = НСтр("ru = 'Налог на прибыль уменьшен на сумму инвестиционного вычета';
										|en = 'Income tax is reduced by the investment deduction amount'");
		Иначе	
			Проводка.Содержание  = НСтр("ru = 'Налог на прибыль восстановлен на сумму ранее примененного инвестиционного налогового вычета';
										|en = 'Income tax is restored for the amount of the previously applied investment tax deduction'");
		КонецЕсли;
...
Процедура СформироватьПроводкиУменьшенияНалогаНаСуммуВычета(КонтекстРасчета, Движения, ОстаткиИнвестиционногоВычета)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Проводка.Сумма > 0 Тогда
			Проводка.Содержание  = НСтр("ru = 'Налог на прибыль уменьшен на сумму инвестиционного вычета';
										|en = 'Income tax is reduced by the amount of the investment deduction amount'");
		Иначе	
			Проводка.Содержание  = НСтр("ru = 'Налог на прибыль восстановлен на сумму ранее примененного инвестиционного налогового вычета';
										|en = 'Income tax is restored by the amount of the previously applied investment tax deduction'");
		КонецЕсли;
...
Процедура ДополнитьУжеСформированнымиПроводкамиПоНачислениюНалога(КонтекстРасчета, ТаблицаПроводок)
	
	Запрос = Новый Запрос();
	Запрос.Текст ="ВЫБРАТЬ
	|	ХозрасчетныйОбороты.Организация КАК Организация,
	|	ХозрасчетныйОбороты.Субконто2 КАК УровеньБюджета,
	|	ХозрасчетныйОбороты.Субконто1 КАК ВидПлатежейВГосБюджет,
	|	ХозрасчетныйОбороты.Субконто3 КАК РегистрацияВНалоговомОргане,
	|	ХозрасчетныйОбороты.КорСубконто1 КАК КорСубконто1,
	...
	|			Организация = &Организация
	|				И Субконто1 = ЗНАЧЕНИЕ(Перечисление.ВидыПлатежейВГосБюджет.Налог),
	|			КорСчет = &СчетДт,) КАК ХозрасчетныйОбороты";
	...
	Запрос.УстановитьПараметр("СчетКт", ПланыСчетов.Хозрасчетный.РасчетыСБюджетом);
	Запрос.УстановитьПараметр("Организация", КонтекстРасчета.Организация);
	...
	Пока РезультатЗапроса.Следующий() Цикл
		...
		Строка.СчетКт = ПланыСчетов.Хозрасчетный.РасчетыСБюджетом;
		Строка.ПериодРасчета = НачалоДня(КонтекстРасчета.КонецПериода);
		Строка.ПрименяетсяПБУ18 = КонтекстРасчета.ПрименяетсяПБУ18;
		Строка.НаборКорректировок = КонтекстРасчета.НаборКорректировок;
		
		Если КонтекстРасчета.ВариантУчетаОтложенногоНалога = Перечисления.ВариантыУчетаОтложенногоНалога.Нет Тогда
			Строка.ВариантНалогообложенияПрибыли = РезультатЗапроса.КорСубконто2;  // 99.01.1 
			Строка.ВидПлатежейВГосБюджет = РезультатЗапроса.КорСубконто1; 
		ИначеЕсли КонтекстРасчета.ВариантУчетаОтложенногоНалога = Перечисления.ВариантыУчетаОтложенногоНалога.БалансовыйМетод Тогда
			Строка.ВариантНалогообложенияПрибыли = РезультатЗапроса.КорСубконто1; // 99.02.Т
		КонецЕсли;
