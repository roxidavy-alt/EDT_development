...
Функция СоответствиеЗапросовКонтрольнымРегистрам(Запрос) Экспорт
	...
	СоответствиеТекстовЗапросов.Вставить("НДСПредъявленныйИзменение", ТекстЗапросаНДСПредъявленный(Запрос));
	//-- НЕ УТ
...
Функция ТекстЗапросаНДСПредъявленный(Запрос)
	...
	|ИЗ
	|	НДСПредъявленныйИзменение КАК Таблица
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РаспределениеНДС КАК РаспределениеНДС
	|		ПО РаспределениеНДС.Организация = Таблица.Организация
	|		И РаспределениеНДС.Дата > КОНЕЦПЕРИОДА(Таблица.Период, МЕСЯЦ)
	|		И РаспределениеНДС.Дата = КОНЕЦПЕРИОДА(РаспределениеНДС.Дата, КВАРТАЛ)
	|		И РаспределениеНДС.Проведен
	|ГДЕ
	|	&ИспользоватьРаздельныйУчетВНАПоНалогообложению
	|	И Таблица.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияНДСПредъявленный.НДСПринятКВычету)
	|	И ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	...
	|			ГДЕ
	|				Т.Период > КОНЕЦПЕРИОДА(Таблица.Период, МЕСЯЦ)
	|				И Т.ДокументПоступленияРасходов = Таблица.СчетФактура)";
...
#Область ТекстНДСАвансыПолученные
	...
	|	НАЧАЛОПЕРИОДА(НДСАвансыПолученные.Период, ДЕНЬ) КАК ДатаОтражения
	|ИЗ
	|	НДСАвансыПолученныеСторноЗачетов КАК НДСАвансыПолученные
	|ГДЕ
	|	НДСАвансыПолученные.НДС <> 0";
...
#Область ТекстНДСАвансыВыданные
	...
	|	НАЧАЛОПЕРИОДА(НДСАвансыПолученные.Период, ДЕНЬ) КАК ДатаОтражения
	|ИЗ
	|	НДСАвансыВыданныеСторноЗачетов КАК НДСАвансыПолученные
	|ГДЕ
	|	НДСАвансыПолученные.НДС <> 0";
...
#КонецОбласти
	...
	Если ПараметрыРасчета.РасчетАвансовВыданных Тогда
		...
		|	НАЧАЛОПЕРИОДА(НДСАвансыВыданные.Период, ДЕНЬ) КАК ДатаОтражения
		|ИЗ
		|	НДСАвансыВыданныеСторноЗачетов КАК НДСАвансыВыданные
		|;
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ТекущиеРегистраторыАвансы.Документ КАК Документ,
...
Процедура ПодготовитьВТ_АвансыВыданные_Возникновения(МенеджерВременныхТаблиц, ПараметрыРасчета, ТаблицаНастроек = Неопределено) Экспорт
	...
	|	РасчетыПоЛизингу.Договор.НаправлениеДеятельности                   КАК НаправлениеДеятельности,
	...
	|	РасчетыПоЛизингу.Договор.НаправлениеДеятельности,
...
Процедура ПодготовитьВТ_ПереносыАвансовВыданных(МенеджерВременныхТаблиц, ПараметрыРасчета, ТаблицаНастроек = Неопределено) Экспорт
	...
	|	РасчетыПоЛизингу_Расход.Договор.НаправлениеДеятельности                   КАК КорНаправлениеДеятельности,
	...
	|	РасчетыПоЛизингу_Расход.Договор.НаправлениеДеятельности,
