	...
	Если ПравоДоступа("Изменение", Метаданные.Документы.ВзаимозачетЗадолженности) Тогда
		...
		Если Метаданные.Документы.ВзаимозачетЗадолженности.Макеты.Найти(ИмяМакета) <> Неопределено Тогда
			...
			КомандаПечати.Представление = НСтр("ru = 'Акт переуступки долга между организациями (Microsoft Word)';
												|en = 'Debt assignment protocol intercompany (Microsoft Word)'");
			КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктВзаимозачетаMicrosoftWord") Тогда 
		...
		ПараметрыПечати.Вставить("ШаблонИмениДокумента", 
					НСтр("ru = '[Организация]-[КонтрагентДебитор]-[КонтрагентКредитор] Акт взаимозачета задолженности №[Номер] от [Дата]';
						|en = '[Организация]-[КонтрагентДебитор]-[КонтрагентКредитор] AR/AP offset certificate No.[Номер] dated [Дата]'"));
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктПереуступкиДолгаMicrosoftWord") Тогда
		...
		ПараметрыПечати.Вставить("ШаблонИмениДокумента", 
					НСтр("ru = '[Организация]-[КонтрагентДебитор]-[КонтрагентКредитор] Акт переуступки долга №[Номер] от [Дата]';
						|en = '[Организация]-[КонтрагентДебитор]-[КонтрагентКредитор] Debt assignment protocol No.[Номер] dated [Дата]'"));
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктПереуступкиДолгаМеждуОрганизациямиMicrosoftWord") Тогда
        ...
        ПараметрыПечати.Вставить("СинонимМакета",НСтр("ru = 'Акт переуступки долга между организациями';
														|en = 'Debt assignment protocol intercompany'"));
		...
		ПараметрыПечати.Вставить("ШаблонИмениДокумента", 
					НСтр("ru = '[Организация]-[ОрганизацияКредитор] Акт переуступки долга между организациями №[Номер] от [Дата]';
						|en = '[Организация]-[ОрганизацияКредитор] Debt assignment protocol intercompany No.[Номер] dated [Дата]'"));
...
#Область ВзаимозачетЧерез76    // (Дт 6Х.ХХ :: Кт 76.Х9), (Дт 76.Х9 :: Кт 6Х.ХХ)
	...
	|	""ВзаимозачетЗадолженностиЛокализация_178"" КАК НомерФрагмента
	|	
	|ИЗ
	...
	|	0 КАК СуммаВРКт,
	|	""Взаимозачет задолженности через транзитный счет"" КАК Содержание,
	|	""ВзаимозачетЗадолженностиЛокализация_179"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ВзаимозачетНапрямую   // (Дт 6Х.ХХ :: Кт 6Х.ХХ)
	...
	|	""ВзаимозачетЗадолженностиЛокализация_180"" КАК НомерФрагмента
	|ИЗ  
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстВзаимозачетаСФилиалами(ИмяДокумента = "") Экспорт
	...
	|	""ВзаимозачетЗадолженностиЛокализация_181"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ИмяДокумента", "Документ.ВзаимозачетЗадолженности");		
	КонецЕсли;
