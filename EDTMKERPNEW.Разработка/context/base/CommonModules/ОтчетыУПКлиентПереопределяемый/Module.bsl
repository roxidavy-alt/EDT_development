...
Процедура ОбработкаРасшифровки(ЭтаФорма, Элемент, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ПолноеИмяОтчета = "Отчет.ЗагрузкаВидовРабочихЦентров" Тогда
		...
		Если КлючТекущегоВарианта = "ЗагрузкаВидовРабочихЦентровТаблица" 
			...
			ПараметрыДействия.Вставить("Заголовок", НСтр("ru = 'Изменить доступность вида рабочих центров';
														|en = 'Change capacity of work center type'"));
	...
	ИначеЕсли ПолноеИмяОтчета = "Отчет.КонтрольЗаказаНаПроизводство" Тогда
		...
		ЗаменаПараметров = Новый Структура;
		ЗаменаПараметров.Вставить("ЗаказНаПроизводство", Неопределено);
		ПараметрыОтчета.Вставить("ЗаменаПараметров", ЗаменаПараметров);
		
		МенюОтчетов.Добавить(ПараметрыОтчета);
		
		Если КлючТекущегоВарианта = "КонтрольПроизводства" Тогда
			...
			ДанныеОтчета = КомпоновкаДанныхВызовСервера.ПараметрыФормыРасшифровки(
				Расшифровка,
				ЭтаФорма.ОтчетДанныеРасшифровки,
				СписокПараметров,
				ПоляРасшифровки);
			
			Если Не ДанныеОтчета.Свойство("Назначение") Тогда
				
				ПараметрыОтчета = Новый Структура;
				ПараметрыОтчета.Вставить("Имя", "ПартииЗапуска");
				ПараметрыОтчета.Вставить("Заголовок", НСтр("ru = 'Партии запуска';
															|en = 'Release lots'"));
				ПараметрыОтчета.Вставить("ИмяОтчета", "Отчет.РасшифровкаКонтроляЗаказаНаПроизводстваПоПартиямЗапуска");
				
				СписокПараметров = Новый Массив;
				СписокПараметров.Добавить("ЗаказНаПроизводство");
				ПараметрыОтчета.Вставить("СписокПараметров", СписокПараметров);
				
				НастройкиОтчета = ЭтаФорма.НастройкиОтчета.СвойстваРезультата.ИтоговыеНастройки;
				ПоказыватьПобочныйВыпуск = КомпоновкаДанныхКлиентСервер.ПолучитьПараметр(НастройкиОтчета, "ПоказыватьПобочныйВыпуск");
				ФиксированныеПараметры = Новый Структура;
				ФиксированныеПараметры.Вставить("ПоказыватьПобочныйВыпуск", ПоказыватьПобочныйВыпуск.Значение);
				ПараметрыОтчета.Вставить("ФиксированныеПараметры", ФиксированныеПараметры);
	...
	ИначеЕсли ПолноеИмяОтчета = "Отчет.РасшифровкаКонтроляЗаказаНаПроизводстваПоПартиямЗапуска" Тогда
		
		СтандартнаяОбработка = Ложь;
		ИсточникНастроек = Новый ИсточникДоступныхНастроекКомпоновкиДанных(ЭтаФорма.Отчет);
		ОбработкаРасшифровки = Новый ОбработкаРасшифровкиКомпоновкиДанных(ЭтаФорма.ОтчетДанныеРасшифровки, ИсточникНастроек);
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ОбработкаРасшифровкиЗавершение", ОтчетыУТКлиентПереопределяемый, ПараметрыДействия);
