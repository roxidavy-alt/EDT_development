...
Процедура ЗаполнитьПродукциюДокумента(Контекст, ТаблицаМаркируемойПродукции, Сценарий = 0) Экспорт
	...
	ИначеЕсли ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(Контекст, "ПроизводствоБезЗаказа") Тогда
		РезультатЗапроса = ЗапросМаркируемойПродукцииПроизводствоБезЗаказа(Контекст, Сценарий);
	ИначеЕсли ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(Контекст, "ДвижениеПродукцииИМатериалов") Тогда
		РезультатЗапроса = ЗапросМаркируемойПродукцииДвижениеПродукцииИМатериалов(Контекст);
	//-- НЕ УТ

	//++ НЕ УТКА
	ИначеЕсли ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(Контекст, "ЭтапПроизводства2_2") Тогда
		РезультатЗапроса = ЗапросМаркируемойПродукцииЭтапПроизводства(Контекст, Сценарий);
	//-- НЕ УТКА
	Иначе
		Возврат;
	КонецЕсли;
...
Функция ЗапросМаркируемойПродукцииПроизводствоБезЗаказа(Документ, Сценарий)
	...
	|		И Товары.Характеристика = ШтрихкодыНоменклатуры.Характеристика
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК Наборы
	|			ПО Товары.Номенклатура.НаборУпаковок = Наборы.Номенклатура
	|ГДЕ
	|	Товары.Ссылка = &ДокументСсылка
	|	И &УсловиеНефасованнаяПродукция
	|
...
Функция ЗапросМаркируемойПродукцииДвижениеПродукцииИМатериалов(Документ)
...
//++ НЕ УТКА
Функция ЗапросМаркируемойПродукцииЭтапПроизводства(Документ, Сценарий)
...
Функция ИмяТабличнойЧасти(ПараметрыСканирования) Экспорт
	...
	ИначеЕсли ИмяДокумента = "ПроизводствоБезЗаказа" Тогда
		Возврат ?(ПараметрыСканирования.Сценарий = 0, "ВыходныеИзделия", "МатериалыИРаботы");

	//-- НЕ УТ

	//++ НЕ УТКА

	ИначеЕсли ИмяДокумента = "ЭтапПроизводства2_2" Тогда
		Возврат ?(ПараметрыСканирования.Сценарий = 0, "ВыходныеИзделия", "РасходМатериаловИРабот");
	ИначеЕсли ИмяДокумента = "ПроизводственнаяОперация2_2" Тогда
		Возврат ?(ПараметрыСканирования.Сценарий = 0, "ВыходныеИзделия", "МатериалыИРаботы");
