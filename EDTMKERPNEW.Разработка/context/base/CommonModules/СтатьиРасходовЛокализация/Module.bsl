...
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если НЕ ВидРасходовИспользуется 
		Или Объект.ВидДеятельностиРасходов = Перечисления.ВидыДеятельностиРасходов.ПрочаяДеятельность Тогда
		МассивНепроверяемыхРеквизитов.Добавить("ВидРасходов"); 
	КонецЕсли;
	
	Если НЕ (Объект.ПринятиеКНалоговомуУчету 
			И ВидПрочихРасходовИспользуется(Объект)
			И Объект.ВидДеятельностиРасходов = Перечисления.ВидыДеятельностиРасходов.ПрочаяДеятельность) Тогда
...
Процедура ПередЗаписью(Объект, Отказ) Экспорт
	...
	Если Не Объект.ЭтоГруппа Тогда
		...
		Если Объект.ВариантРаспределенияРасходовРегл <> Перечисления.ВариантыРаспределенияРасходов.НаНаправленияДеятельности
			И Объект.ВариантРаспределенияРасходовРегл <> Перечисления.ВариантыРаспределенияРасходов.НаСебестоимостьПродаж Тогда
			Объект.СтатьяРКМ = Неопределено;
			Объект.УчитыватьПриФормированииЦеныПродукцииГОЗ = Ложь;
		КонецЕсли;
...
Процедура ВидДеятельностиРасходовПриИзменении(Форма) Экспорт
	...
	Объект = Форма.Объект;
	Элементы.ВидРасходов.ОтметкаНезаполненного = Объект.ВидДеятельностиРасходов = Перечисления.ВидыДеятельностиРасходов.ОсновнаяДеятельность И НЕ ЗначениеЗаполнено(Объект.ВидРасходов);
	Элементы.ВидПрочихРасходов.ОтметкаНезаполненного = Объект.ВидДеятельностиРасходов = Перечисления.ВидыДеятельностиРасходов.ПрочаяДеятельность И НЕ ЗначениеЗаполнено(Объект.ВидПрочихДоходовИРасходов)
													И Объект.ПринятиеКНалоговомуУчету;
...
//
// Параметры:
// 	СписокВыбора - СписокЗначений Из Строка - Список типов аналитик:
// 		* Значение - Строка - Строковое описание типа (Например, "ПеречислениеСсылка.ВидыОперацийПоЗарплате")
// 		* Представление - Строка - Представление типа для отображения на форме.
//
...
Процедура УправлениеЭлементамиФормы(Форма) Экспорт
	...
	Элементы.ГруппаВидовРасходовОсновнаяДеятельность.Видимость = ОсновнаяДеятельность;
	Элементы.ГруппаВидовРасходовОсновнаяДеятельность.Доступность = НастройкиРегл.ПринимаетсяКНУ;
	Элементы.ГруппаВидовРасходовПрочаяДеятельность.Видимость = ПрочаяДеятельность;
	...
	Элементы.СтатьяРКМ.Видимость = НаНаправленияДеятельностиРегл ИЛИ НаПродажиРегл;
	Элементы.СтатьяРКМ.АвтоОтметкаНезаполненного = Объект.УчитыватьПриФормированииЦеныПродукцииГОЗ;
	Если Не Объект.УчитыватьПриФормированииЦеныПродукцииГОЗ Тогда
		Элементы.СтатьяРКМ.ОтметкаНезаполненного = Ложь;
	КонецЕсли;
	Элементы.УчитыватьПриФормированииЦеныПродукцииГОЗ.Видимость = НаНаправленияДеятельностиРегл ИЛИ НаПродажиРегл;
	//-- НЕ УТ
...
Функция ПолучитьНастройкиРегл(Объект)
	...
	НастройкиРегл.НаФинансовыйРезультат = (ВариантРаспределенияРегл = Перечисления.ВариантыРаспределенияРасходов.НаНаправленияДеятельности ИЛИ ВариантРаспределенияНУ = Перечисления.ВариантыРаспределенияРасходов.НаНаправленияДеятельности
											ИЛИ ВариантРаспределенияРегл = Перечисления.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода ИЛИ ВариантРаспределенияНУ = Перечисления.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода);
	...
	Если НастройкиРегл.УпрУчетНаСчетахРегл Тогда
		...
		НастройкиРегл.НаФинансовыйРезультат = НастройкиРегл.НаФинансовыйРезультат ИЛИ (ВариантРаспределенияУпр = Перечисления.ВариантыРаспределенияРасходов.НаНаправленияДеятельности
												ИЛИ ВариантРаспределенияУпр = Перечисления.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода);
...
Функция ВидРасходовИспользуется(Объект)
	...
	НаФинансовыйРезультат = (Объект.ВариантРаспределенияРасходовРегл = Перечисления.ВариантыРаспределенияРасходов.НаНаправленияДеятельности
	...
	Используется = (НаФинансовыйРезультат ИЛИ НеРаспределять ИЛИ НаПроизводственныеЗатраты ИЛИ НаВнеоборотныеАктивы 
	...
	НаФинансовыйРезультатНУ = (Объект.ВариантРаспределенияРасходовНУ = Перечисления.ВариантыРаспределенияРасходов.НаНаправленияДеятельности
								ИЛИ Объект.ВариантРаспределенияРасходовНУ = Перечисления.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода);
	...
	Используется = Используется ИЛИ НаФинансовыйРезультатНУ ИЛИ НеРаспределятьНУ ИЛИ НаПроизводственныеЗатратыНУ
						ИЛИ НаВнеоборотныеАктивыНУ ИЛИ НаСебестоимостьПроизводстваНУ;
...
Функция ВидПрочихРасходовИспользуется(Объект)
	...
	Используется = (НаФинансовыйРезультат ИЛИ НеРаспределять ИЛИ НаОВЗ)
	...
	НаРасходыТекущегоПериода = (Объект.ВариантРаспределенияРасходовРегл = Перечисления.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода
							ИЛИ Объект.ВариантРаспределенияРасходовНУ = Перечисления.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода);
							
	Используется = Используется ИЛИ НаФинансовыйРезультатНУ ИЛИ НеРаспределятьНУ ИЛИ НаРасходыТекущегоПериода;
