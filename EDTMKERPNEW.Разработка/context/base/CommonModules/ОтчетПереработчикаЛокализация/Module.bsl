...
#Область СписаниеВПроизводство // (Дт 08, 20, 2Х, 44, 91.02 :: Кт 10.07)
	...
	|	""ОтчетПереработчикаЛокализация_728"" КАК НомерФрагмента
	|
	|ИЗ
...
#Область ВыпускПродукции // (Дт 2Х :: Кт 2Х)
	...
	|	""ОтчетПереработчикаЛокализация_729"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ВыпускПродукцииНематериальныеЗатраты // (Дт 2Х :: Кт 2Х)
	...
	|	""Выпуск продукции"" КАК Содержание,
	|	""ОтчетПереработчикаЛокализация_730"" КАК НомерФрагмента
	|ИЗ
...
#Область ЗакупкаУслуги // (Дт 08, 2Х, 44, 91 :: Кт 60, 71, 76)
	...
	|	""ОтчетПереработчикаЛокализация_731"" КАК НомерФрагмента
	|
	|ИЗ 
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ЗакупкаУслугиВалютнаяСумма // (Дт 08, 2Х, 44, 91 :: Кт 60, 71, 76)
	...
	|	0 КАК СуммаВРКт,
	|	""Поступление услуг"" КАК Содержание,
	|	""ОтчетПереработчикаЛокализация_732"" КАК НомерФрагмента
	|
	|ИЗ 
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ЗакупкаНДС // (Дт 19 :: Кт 60, 71, 76)
	...
	|	""ОтчетПереработчикаЛокализация_733"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область СписаниеВПроизводствоМатериаловДавальца // (Дт  :: Кт 003.02)
	...
	|	""ОтчетПереработчикаЛокализация_734"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ВключениеИсключениеНДСВСтоимостьТоваров // (Дт 20 :: Кт 19.03)
	...
	|	""ОтчетПереработчикаЛокализация_735"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#КонецОбласти
	...
	|	""ОтчетПереработчикаЛокализация_736"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|	""ОтчетПереработчикаЛокализация_1778"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика КАК Операция
	...
	|	""Списание продукции на расходы""                                           КАК Содержание,
	|	""ОтчетПереработчикаЛокализация_1779"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика КАК Операция
	...
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|			РегистрНакопления.ДвиженияПоПрочимАктивамПассивам КАК ТаблицаПродукции
	|		ПО
	|			Строки.Ссылка = ТаблицаПродукции.Регистратор
	|			И Строки.Организация = ТаблицаПродукции.Организация
	|			И Строки.Период = ТаблицаПродукции.Период
	|			И Строки.ИдентификаторФинЗаписи = ТаблицаПродукции.ИдентификаторФинЗаписи
	|			И ТаблицаПродукции.Активность
	|			И НЕ ТаблицаПродукции.Сторно
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтСтоимости КАК Стоимости
	|		ПО Строки.Ссылка                   = Стоимости.Ссылка
	|		 И Строки.Номенклатура             = Стоимости.Номенклатура
