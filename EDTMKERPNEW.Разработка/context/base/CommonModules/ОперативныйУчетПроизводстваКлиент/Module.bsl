...
//
// Параметры:
//  РезультатФормирования - см. ОперативныйУчетПроизводстваКлиентСервер.РезультатФормированияПроизводственныхОперацийКонструктор
//
Процедура ОповеститьПользователяОФормированииПроизводственныхОпераций(РезультатФормирования) Экспорт
	
	Если РезультатФормирования.КоличествоОбработанных > 0 Тогда
		
		КоличествоСоздано = РезультатФормирования.ОперацииСозданные.Количество();
		КоличествоОбновлено = РезультатФормирования.ОперацииОбновленные.Количество();
		
		Если КоличествоОбновлено = 0 Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Сформировано %1 из %2 документов';
											|en = 'Generated %1 out of %2 documents'"),
				КоличествоСоздано,
				РезультатФормирования.КоличествоВсего);
			ТекстЗаголовка = НСтр("ru = 'Производственные операции сформированы';
									|en = 'Routing operations are generated'");
		ИначеЕсли КоличествоСоздано = 0 Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Изменено %1 из %2 документов';
											|en = 'Changed %1 out of %2 documents'"),
				КоличествоОбновлено,
				РезультатФормирования.КоличествоВсего);
			ТекстЗаголовка = НСтр("ru = 'Производственные операции изменены';
									|en = 'Routing operations are changed'")
		Иначе
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Обработано %1 из %2 документов (создано: %3, изменено: %4)';
											|en = 'Processed %1 of %2 documents (created: %3, changed: %4)'"),
				КоличествоСоздано+КоличествоОбновлено,
				РезультатФормирования.КоличествоВсего,
				КоличествоСоздано,
				КоличествоОбновлено);
			ТекстЗаголовка = НСтр("ru = 'Производственные операции обработаны';
									|en = 'Routing operations are processed'");
		КонецЕсли;
...
// Параметры:
//  Подразделение			 - СправочникСсылка.СтруктураПредприятия							 - производственное подразделение
//  КлючиОпераций			 - Массив из РегистрСведенийКлючЗаписи.ОчередьПроизводственныхОпераций - ключ записи
//  ПараметрыНазначения		 - см. ОперативныйУчетПроизводстваКлиентСервер.ПараметрыНазначенияПроизводственнойОперацииКонструктор
//  УстанавливаемыйСтатус	- ПеречислениеСсылка.СтатусыПроизводственныхОпераций				 - статус нового документа
//
// Возвращаемое значение:
//  см. ОперативныйУчетПроизводстваКлиентСервер.РезультатФормированияПроизводственныхОперацийКонструктор
//
Функция СформироватьПроизводственныеОперации(Подразделение, КлючиОпераций, ПараметрыНазначения, УстанавливаемыйСтатус = Неопределено) Экспорт
	...
	Если Не (Результат.КоличествоВсего = Результат.КоличествоОбработанных)
		И Результат.КоличествоВсего = 1
		И Результат.Индивидуальные.Количество() = 0 Тогда
		...
		ЗначенияЗаполнения = СписокДанныхЗаполнения[0];
		
		Если ПараметрыНазначения.Свойство("ВидыРабот") Тогда
			ЗначенияЗаполнения.Вставить("ВидыРабот", ПараметрыНазначения.ВидыРабот);
		КонецЕсли;
	...
	Иначе
		...
		ОповеститьПользователяОФормированииПроизводственныхОпераций(Результат);
				
		ИзмененныеДокументы = Новый Массив;
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ИзмененныеДокументы, Результат.ОперацииСозданные);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ИзмененныеДокументы, Результат.ОперацииОбновленные);
		ОповеститьОЗаписиПроизводственныхОпераций(Подразделение,, ИзмененныеДокументы);
...
// Параметры выдачи подтверждения при установке статуса операции.
// 
// Параметры:
//  ДанныеОбъекта - ДанныеФормыСтруктура
//  НовыйСтатус - Строка,ПеречислениеСсылка.СтатусыПроизводственныхОпераций
//  Список - Булево - Необходимость сформировать вопрос во множественном числе
// 
// Возвращаемое значение:
//  Неопределено,Структура - из:
// * НовыйСтатус -  ПеречислениеСсылка.СтатусыПроизводственныхОпераций,Строка
// * ПодтверждаемоеСобытие - Строка
// * ТекстВопроса - Строка
// * РежимДиалогаВопрос - РежимДиалогаВопрос
Функция ПодтверждениеПриУстановкеСтатусаОперации(ДанныеОбъекта, НовыйСтатус, Список = Ложь) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("НовыйСтатус", НовыйСтатус);
	Результат.Вставить("ПодтверждаемоеСобытие", "");
	Результат.Вставить("ТекстВопроса", "");
	Результат.Вставить("РежимДиалогаВопрос", РежимДиалогаВопрос.ДаНет);
	
	СсылкаСтатус = ОперативныйУчетПроизводстваКлиентСервер.СтатусОперацииСсылка(НовыйСтатус);
	
	Если СсылкаСтатус = ПредопределенноеЗначение("Перечисление.СтатусыПроизводственныхОпераций.Пропущена")
		И ДанныеОбъекта.МожноПропустить Тогда
		
		Показатели = "КоличествоНаКонтроле,КоличествоНаДоработке,КоличествоБрак,КоличествоФакт,КоличествоГотово";
		Для каждого Показатель Из СтрРазделить(Показатели, ",") Цикл
			Значение = Неопределено;
			Если ДанныеОбъекта.Свойство(Показатель, Значение) И Значение > 0 Тогда
				Результат.ПодтверждаемоеСобытие = ОперативныйУчетПроизводстваКлиентСервер.Подтверждение_АннулированиеПоказателей();
				Результат.ТекстВопроса = ?(Список,
					НСтр("ru = 'При установке статуса ""Пропущена"" все накопленные показатели выполнения операций будут аннулированы. Продолжить?';
						|en = 'If you set the ""Skipped"" status, all accumulated performance indicators for the operations will be canceled. Continue?'"),
					НСтр("ru = 'При установке статуса ""Пропущена"" все накопленные показатели выполнения операции будут аннулированы. Продолжить?';
						|en = 'If you set the ""Skipped"" status, all accumulated performance indicators for the operation will be canceled. Continue?'"));
				Возврат Результат;
			КонецЕсли;
		КонецЦикла;
		
	ИначеЕсли СсылкаСтатус = ПредопределенноеЗначение("Перечисление.СтатусыПроизводственныхОпераций.НеВыполнена")
		И РедакторПроизводственногоПроцессаКлиентСервер.ЭтоОперацияИндивидуальная(ДанныеОбъекта) Тогда
			
		Результат.ПодтверждаемоеСобытие = ОперативныйУчетПроизводстваКлиентСервер.Подтверждение_СозданиеИндивидуальнойОперации();
		Результат.ТекстВопроса = ?(Список,
			НСтр("ru = 'Создать новые индивидуальные операции взамен невыполненных?';
				|en = 'Create new individual operations to replace the uncompleted ones?'"),
			НСтр("ru = 'Создать новую индивидуальную операцию взамен невыполненной?';
				|en = 'Create a new individual operation to replace the uncompleted one?'"));
		Результат.РежимДиалогаВопрос = РежимДиалогаВопрос.ДаНетОтмена;
		Возврат Результат;
