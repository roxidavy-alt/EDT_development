...
Процедура ОтразитьДвижения(ТаблицыДляДвижений, Движения, Отказ) Экспорт
	...
	ДвиженияКорректировкиНУ = Движения.КорректировкиНалоговойБазы;
	ДвиженияКорректировкиНУ.Записывать = Истина;
...
// 		* ВыполнитьОффлайновыеРасчетыПриПовторнойПопыткеОтражения - Булево - Выполняет оффлайн расчеты при повторной попытке отражения.
// 
Функция ПараметрыОтраженияВРеглУчете() Экспорт
	...
	Параметры.Вставить("ВыполнитьОффлайновыеРасчетыПриПовторнойПопыткеОтражения", Ложь);	
...
Процедура ОтразитьВсеРегламент() Экспорт
	...
	ПараметрыОтражения.ВыполнитьОффлайновыеРасчетыПриПовторнойПопыткеОтражения = Истина;
...
Процедура ВернутьДокументыКОтражению(ДокументыКОтражению, КоличествоОбработанных = Неопределено) Экспорт
	...
	Иначе
		...
		Если ДанныеКОтражению.Количество() = 0 Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	НепроведенныеДокументы = ОбщегоНазначения.ПроверитьПроведенностьДокументов(
		ДанныеКОтражению.ВыгрузитьКолонку("Документ"));
	...
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(ДокументыКОтражению.Документ) В (&ТипыДокументовКОтражению)
	|	И НЕ (ДокументыКОтражению.Документ) В (&НепроведенныеДокументы)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|			И ТИПЗНАЧЕНИЯ(ОтражениеДокументовВРеглУчете.Регистратор) В (&ТипыДокументовКОтражению)
	...
	|ГДЕ
	|	НЕ (ДокументыКОтражению.Документ) В (&НепроведенныеДокументы)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Регистратор,
	|	Организация,
	...
	|	КОНЕЦ КАК Комментарий,
	|	(ОтражениеДокументовВРеглУчете.ДатаОтражения < &ДатаНачалаВеденияРеглУчета) КАК ЗаписьДоНачалаВеденияРеглУчета,
	...
	|	"""" КАК Комментарий,
	|	(ДокументыКОтражению.ДатаОтражения < &ДатаНачалаВеденияРеглУчета) КАК ЗаписьДоНачалаВеденияРеглУчета,
	|	ИСТИНА
	|ИЗ
	|	ПериодыРегистрации КАК ДокументыКОтражению
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	...
	|	
	|ИТОГИ
	|	МАКСИМУМ(ЗаписьДоНачалаВеденияРеглУчета),
	...
	Запрос.УстановитьПараметр("ТипыДокументовКОтражению", ТипыДокументовОтражаемыхВРеглУчете().Типы());
	Запрос.УстановитьПараметр("ДатаНачалаВеденияРеглУчета", Константы.ДатаНачалаВеденияРеглУчета.Получить());
	Запрос.УстановитьПараметр("НепроведенныеДокументы", НепроведенныеДокументы);
	Результат = Запрос.Выполнить();
	
	РегистраторыНачальныхОстатков = РазрешенныеТипыРегистраторовНачальныхОстатков();
	РегистраторыНачальныхОстатковПоОснованию = РазрешенныеТипыРегистраторовНачальныхОстатковПоОснованию();
	
	ВыборкаПоДокументам = Результат.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Пока ВыборкаПоДокументам.Следующий() Цикл
		
		Если Не ВыборкаПоДокументам.ЕстьИзменения Тогда
			Продолжить;
		КонецЕсли;
		
		ЗаписиДоНачалаВеденияРеглУчетаРазрешены = Ложь;
		Если ВыборкаПоДокументам.ЗаписьДоНачалаВеденияРеглУчета Тогда
			ТипРегистратора = ТипЗнч(ВыборкаПоДокументам.Регистратор);
			Если РегистраторыНачальныхОстатковПоОснованию.Получить(ТипРегистратора) <> Неопределено Тогда
				ИмяРеквизитаОснования = РегистраторыНачальныхОстатковПоОснованию.Получить(ТипРегистратора);
				ПервичныйДокумент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВыборкаПоДокументам.Регистратор, ИмяРеквизитаОснования);
			Иначе
				ПервичныйДокумент = ВыборкаПоДокументам.Регистратор;
			КонецЕсли;
			ЗаписиДоНачалаВеденияРеглУчетаРазрешены = РегистраторыНачальныхОстатков.СодержитТип(ТипЗнч(ПервичныйДокумент));
		КонецЕсли;
		...
		Пока Выборка.Следующий() Цикл
			Если Выборка.ЗаписьДоНачалаВеденияРеглУчета
				И Не ЗаписиДоНачалаВеденияРеглУчетаРазрешены Тогда
				Продолжить;
			КонецЕсли;
	...
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	МИНИМУМ(НастройкиСчетов.Период) КАК Месяц,
	|	Неопределено КАК Документ,
	|	ДокументыКОтражению.Организация КАК Организация,
	|	ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.ПереносСальдоПриИзмененииНастроекСчетовРеглУчета) КАК Операция
	|ИЗ
	|	ПериодыРегистрации КАК ДокументыКОтражению
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета КАК НастройкиСчетов
	|	ПО НастройкиСчетов.Период >= ДокументыКОтражению.ДатаОтражения
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗаданияКЗакрытиюМесяца КАК Задания
	|	ПО ДокументыКОтражению.Организация = Задания.Организация
	|		И НастройкиСчетов.Период = НАЧАЛОПЕРИОДА(Задания.Месяц, МЕСЯЦ)
	|		И Задания.Операция = ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.ПереносСальдоПриИзмененииНастроекСчетовРеглУчета)
	|ГДЕ
	|	ДокументыКОтражению.Организация <> ЗНАЧЕНИЕ(Справочник.Организации.УправленческаяОрганизация)
...
Процедура ВернутьРезультатОтраженияДокумента(ПараметрыОтражения, АдресРезультата) Экспорт
	...
	|	ЕСТЬNULL(РеестрДокументов.СторнируемыйДокумент, НЕОПРЕДЕЛЕНО) КАК СторнируемыйДокумент,
	...
	СтруктураВозврата =
		Новый Структура("ТаблицаПроводок, ТаблицаПроводокКорректировкиНУ, КомментарийОшибки, СтатусОтражения");
	...
	Если Результат.ВыборкаСтатусов.Следующий() Тогда
		...
		СтруктураВозврата.ТаблицаПроводокКорректировкиНУ = ТаблицаПроводокКорректировкиНУ(
				Результат.ВыборкаСтатусов,
				СтруктураВозврата.ТаблицаПроводок,
				Результат.ВыборкаКорректировкиНалоговойБазыДополнение,
				Результат.ВыборкаКорректировкиНалоговойБазыСторно);
...
Процедура ЗарегистрироватьОтражениеДокумента(Документ, СтатусОтражения, Комментарий = Неопределено, ТаблицаОтражения = Неопределено) Экспорт
	...
	Попытка
		...
		Если ТипЗнч(Документ) = Тип("ДокументСсылка.КорректировкаРеализации")
			И ОтражениеДокументовВРегламентированномУчете.Количество() > 1 Тогда
			// Корректировка реализации прошлого периода может сформировать запись в заблокированном от изменений периоде.
			ЗакрытиеМесяцаСервер.ОтключитьПроверкуБлокировкуПериодаДляНабораЗаписей(ОтражениеДокументовВРегламентированномУчете);
		КонецЕсли;
...
// 		3 - Алгоритм с обогащением проводок по ГОЗ.
// 		4 - Алгоритм по проекту Две даты.
// 	
...
Процедура ВыполнитьОффлайновыеРасчеты(Документы = Неопределено)
	...
	Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") И ПланыОбмена.ГлавныйУзел() = Неопределено Тогда
		НеоперативнаяОчередьЗаданий.ВыполнитьЗаданияНеоперативнойОчередиПоСпискуДокументов(МассивДокументов,
			Справочники.ВидыНеоперативныхЗаданий.ОтражениеВРегламентированномУчете);
	Иначе
...
Функция ОтразитьДокументыПакетно(ПараметрыОтражения, Знач НомерИтерации = 0)
	...
	Если НомерИтерации < ПараметрыОтражения.КоличествоПопыток Тогда
		...
		Если ПараметрыОтражения.ПрогнозныеПараметры.КоличествоДокументов > 0 Тогда
			...
			Если ПараметрыОтражения.ВыполнитьОффлайновыеРасчетыПриПовторнойПопыткеОтражения Тогда
				УчетНДСУП.ВыполнитьФормированиеДвиженийПоНДС(КонецМесяца(ПараметрыОтражения.ПериодРасчета));
				ВыполнитьОффлайновыеРасчеты();         				
			КонецЕсли;                                 
...
Функция ВыполнитьОтражениеПорцииТекущимПотоком(ТипДокумента, ВременныеТаблицы, ПараметрыОтраженияВРеглУчете)
	...
	Для НомерПопытки = 1 По ПараметрыОтраженияВРеглУчете.КоличествоПопыток Цикл
		Попытка
			...
			Прервать;
		Исключение
			УничтожитьВспомогательныеВТ(ВременныеТаблицы);
...
Функция ВыполнитьОтражение(ТипДокумента, ВременныеТаблицы, ПараметрыОтраженияВРеглУчете)
	...
	ВыборкаКорректировкиНалоговойБазыДополнение = Результат.ВыборкаКорректировкиНалоговойБазыДополнение;
	ВыборкаКорректировкиНалоговойБазыДополнение.Следующий();
	
	ВыборкаКорректировкиНалоговойБазыСторно = Результат.ВыборкаКорректировкиНалоговойБазыСторно;
	ВыборкаКорректировкиНалоговойБазыСторно.Следующий();
	...
	Пока ВыборкаСтатусов.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировки = Блокировка.Добавить("РегистрБухгалтерии.Хозрасчетный.НаборЗаписей");
			ЭлементБлокировки.УстановитьЗначение("Регистратор", ВыборкаСтатусов.Ссылка);
			ЭлементБлокировки = Блокировка.Добавить("РегистрБухгалтерии.КорректировкиНалоговойБазы.НаборЗаписей");
			ЭлементБлокировки.УстановитьЗначение("Регистратор", ВыборкаСтатусов.Ссылка);
			...
			//-- НЕ УТКА
			ТаблицаПроводокХозрасчетный = Новый ТаблицаЗначений;
			НаборЗаписей = СформироватьХозрасчетный(ВыборкаСтатусов, 
				ВыборкаХозрасчетный,
				ВыборкаХозрасчетныйДополнение,
				ВыборкаХозрасчетныйСторно,
				ПараметрыОтраженияВРеглУчете,
				ТаблицаПроводокХозрасчетный);
			
			СформироватьКорректировкиНалоговойБазы(
				ВыборкаСтатусов,
				ТаблицаПроводокХозрасчетный,
				ВыборкаКорректировкиНалоговойБазыДополнение,
				ВыборкаКорректировкиНалоговойБазыСторно);
	...
	ФормируемыеЗадания = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Справочники.ОперацииЗакрытияМесяца.ФормированиеФинансовогоРезультата);
	Если Не ПараметрыОтраженияВРеглУчете.ПакетноеОтражение Тогда
		ФормируемыеЗадания.Добавить(Справочники.ОперацииЗакрытияМесяца.ПереносСальдоПриИзмененииНастроекСчетовРеглУчета);
	КонецЕсли;
	ЗапросЗаданийКФормированиюФинансовогоРезультата.УстановитьПараметр("ФормируемыеЗадания", ФормируемыеЗадания);
...
Функция ВыборкиОтраженияДокументов(ИмяДокумента, ВременныеТаблицы, ПараметрыОтраженияВРеглУчете)
	...
	ЗапросУсловийОтражения = РеглУчетВыборкиСерверПовтИсп.ЗапросУсловийОтражения();
	ЗапросУсловийОтражения.МенеджерВременныхТаблиц = ВременныеТаблицы;
	ЗапросУсловийОтражения.Выполнить();
	
	ЕстьВключениеНДСВСтоимость = ОбщегоНазначенияУТ.РазмерВременнойТаблицы(ВременныеТаблицы, "ЕстьВключениеНДСВСтоимость") > 0;
	ЕстьСторно = ОбщегоНазначенияУТ.РазмерВременнойТаблицы(ВременныеТаблицы, "ДокументыСторно") > 0;
	ЕстьДополнение = ОбщегоНазначенияУТ.РазмерВременнойТаблицы(ВременныеТаблицы, "ЕстьДополнение") > 0;
	
	// Выборка контекстных данных (данных из документов)
	ЭтоДокументСебестоимости = РеглУчетВыборкиСерверПовтИсп.ЭтоДокументУчетаСебестоимости(ИмяДокумента) И Не ИмяДокумента = "РегистраторРасчетов";
	Если ЕстьВключениеНДСВСтоимость Тогда
		ЗапросДанныеНДСВключениеВСтоимость = РеглУчетВыборкиСерверПовтИсп.ЗапросДанныеНДСВключениеВСтоимость(
			ЭтоДокументСебестоимости,
			ПараметрыОтраженияВРеглУчете.ДетализацияПроводок);
	Иначе
		ЗапросДанныеНДСВключениеВСтоимость = РеглУчетВыборкиСерверПовтИсп.ЗапросДанныеНДСВключениеВСтоимостьПустые(
			ПараметрыОтраженияВРеглУчете.ДетализацияПроводок);
	КонецЕсли;
	ЗапросДанныеНДСВключениеВСтоимость.МенеджерВременныхТаблиц = ВременныеТаблицы;
	ЗаполнениеПараметровЗапросаИПрочихФункциональныхОпций(
		ЗапросДанныеНДСВключениеВСтоимость,
		РеглУчетВыборкиСерверПовтИсп.ПараметрыОтраженияРеглУчетаПоУмолчанию(ПараметрыОтраженияВРеглУчете.ДетализацияПроводок));
	ЗапросДанныеНДСВключениеВСтоимость.Выполнить();
	...
	ЗапросДанных = РеглУчетВыборкиСерверПовтИсп.ЗапросДанных(ИмяДокумента, ЭтоОбъектРасчетов,
										ПараметрыОтраженияВРеглУчете.ВременныеТаблицыПереданы, ЕстьВключениеНДСВСтоимость);
	ЗапросДанных.МенеджерВременныхТаблиц = ВременныеТаблицы;
	УстановитьПараметрыЗапросаДанных(ЗапросДанных, ИмяДокумента, ВыборкаСтатусов, ПараметрыОтраженияВРеглУчете.ДетализацияПроводок, ЕстьВключениеНДСВСтоимость);
	
	ЗапросДанных.Выполнить();
	
	// Выборка периодов корректировки налоговой базы
	Если ИмяДокумента = "ОтнесениеОтклоненийВСтоимостьОС" Тогда
		ЗапросПериодыКорректировкиНалоговойБазы = РеглУчетВыборкиСерверПовтИсп.ЗапросПериодыКорректировкиНалоговойБазыКВ();
	Иначе
		ЗапросПериодыКорректировкиНалоговойБазы = РеглУчетВыборкиСерверПовтИсп.ЗапросПериодыКорректировкиНалоговойБазы();
	КонецЕсли;
	ЗапросПериодыКорректировкиНалоговойБазы.УстановитьПараметр(
		"ВидыСчетовКорректировкиНалоговойБазы", РеглУчетВыборкиСерверПовтИсп.ВидыСчетовКорректировкиНалоговойБазы());
	ЗапросПериодыКорректировкиНалоговойБазы.МенеджерВременныхТаблиц = ВременныеТаблицы;
	ЗапросПериодыКорректировкиНалоговойБазы.Выполнить();
	...
	ПовышениеДетальностиРеглУчета = РеглУчетВыборкиСерверПовтИсп.ИспользуетсяИнвестиционныйНалоговыйВычет(ИмяДокумента)
		Или ТребуетсяДетализацияРеглУчета(ВременныеТаблицы);
	
	Если ПовышениеДетальностиРеглУчета Тогда
		СформироватьПартииПродукцииСоСтатьямиКалькуляции(ВременныеТаблицы);
		...
		ПараметрыОтраженияВРеглУчете.ДетализацияПроводок = ПовышениеДетальностиРеглУчета
			И (РеглУчетСервер.ИспользуетсяРеглУчетНЗППоПартиямПроизводства()
				Или РеглУчетВыборкиСерверПовтИсп.ИспользуетсяИнвестиционныйНалоговыйВычет(ИмяДокумента));
	КонецЕсли;
	...
	Если ЕстьДополнение Или ЕстьСторно Тогда
		...
		ЗапросКорректировкиНалоговойБазыДополнение = РеглУчетВыборкиСерверПовтИсп.ЗапросКорректировкиНалоговойБазыДополнение();
		ЗапросКорректировкиНалоговойБазыДополнение.МенеджерВременныхТаблиц = ВременныеТаблицы;
		ЗапросКорректировкиНалоговойБазыДополнение.УстановитьПараметр(
			"ДокументыОтражаемыеПоследовательно", РеглУчетВыборкиСерверПовтИсп.ТипыДокументовКПоследовательномуОтражению());
		РезультатЗапросКорректировкиНалоговойБазыДополнение = ЗапросКорректировкиНалоговойБазыДополнение.Выполнить();
		ВыборкаКорректировкиНалоговойБазыДополнение = РезультатЗапросКорректировкиНалоговойБазыДополнение.Выбрать(
			ОбходРезультатаЗапроса.ПоГруппировкам);
	Иначе
		ВыборкаХозрасчетныйДополнение = ПустаяВыборка();
		ВыборкаКорректировкиНалоговойБазыДополнение = ПустаяВыборка();
	КонецЕсли;
	...
	Если ЕстьСторно Тогда
		...
		ЗапросКорректировкиНалоговойБазыСторно = РеглУчетВыборкиСерверПовтИсп.ЗапросКорректировкиНалоговойБазыСторно();
		ЗапросКорректировкиНалоговойБазыСторно.МенеджерВременныхТаблиц = ВременныеТаблицы;
		РезультатКорректировкиНалоговойБазыСторно = ЗапросКорректировкиНалоговойБазыСторно.Выполнить();
		ВыборкаКорректировкиНалоговойБазыСторно = РезультатКорректировкиНалоговойБазыСторно.Выбрать(
			ОбходРезультатаЗапроса.ПоГруппировкам);
	Иначе
		ВыборкаХозрасчетныйСторно = ПустаяВыборка();
		ВыборкаКорректировкиНалоговойБазыСторно = ПустаяВыборка();
	КонецЕсли;
	...
	ЗапросСвязанныеДокументыКОтражению.УстановитьПараметр("ТипСсылки", ОбщегоНазначения.ИдентификаторОбъектаМетаданных("Документ." + ИмяДокумента));
	...
	СтруктураВозврата.Вставить("ВыборкаКорректировкиНалоговойБазыДополнение", ВыборкаКорректировкиНалоговойБазыДополнение);
	СтруктураВозврата.Вставить("ВыборкаКорректировкиНалоговойБазыСторно", ВыборкаКорректировкиНалоговойБазыСторно);
	
	Возврат СтруктураВозврата;
	
КонецФункции

Функция ПустаяВыборка()
...
Процедура УстановитьПараметрыЗапросаДанных(ЗапросДанных, ИмяДокумента, ВыборкаДокументов, ДетализацияПроводок, ЕстьВключениеНДСВСтоимость)
	...
	ЭтоОбъектРасчетов = РеглУчетВыборкиСерверПовтИсп.ЭтоОбъектРасчетов(ИмяДокумента); 
	ПараметрыДанных = РеглУчетВыборкиСерверПовтИсп.ЗапросДанныхПараметры(ИмяДокумента, ЭтоОбъектРасчетов, ЕстьВключениеНДСВСтоимость);
...
Функция СформироватьТаблицыОтбораДанных(ТипДокумента, ВременныеТаблицы, ПараметрыОтражения, ПараметрыПорции = Неопределено)
	...
	Если ФормированиеПроводок Тогда
		...
		|		РегистрСведений.РеестрДокументов КАК РеестрДокументов
		|	ПО
		|		ДанныеРегистра.Ссылка = РеестрДокументов.Ссылка
	...
	Если ТипДокумента = Тип("ДокументСсылка.Сторно") Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И РеестрДокументов.ТипСсылки = &ТипСсылки", "");
	Иначе
		Запрос.УстановитьПараметр("ТипСсылки", ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ТипДокумента));
	КонецЕсли;
...
Процедура СдвинутьВыборки(Выборки)
	...
	КонецЕсли;
	Если Выборки.ВыборкаКорректировкиНалоговойБазыДополнение.Ссылка = ТекущаяСсылка Тогда
		Выборки.ВыборкаСвязанныеДокументыКОтражению.Следующий();
	КонецЕсли;
	Если Выборки.ВыборкаКорректировкиНалоговойБазыСторно.Ссылка = ТекущаяСсылка Тогда
		Выборки.ВыборкаСвязанныеДокументыКОтражению.Следующий();
	КонецЕсли;
...
Функция СформироватьХозрасчетный(ВыборкаСтатусов, ВыборкаХозрасчетный, ВыборкаХозрасчетныйДополнение, ВыборкаХозрасчетныйСторно, ПараметрыОтраженияВРеглУчете, ТаблицаПроводокХозрасчетный)
	...
	ТаблицаПроводокХозрасчетный = ТаблицаПроводок.Скопировать();
	...
	Если ВыборкаСтатусов.Ссылка = ВыборкаХозрасчетныйСторно.Ссылка Тогда
		...
		ДобавитьСтрокиВТаблицуПроводок(ТаблицаПроводокХозрасчетный, ВыборкаХозрасчетныйСторно);
		ВыборкаХозрасчетныйСторно.Следующий();
	КонецЕсли;
...
Функция СформироватьКорректировкиНалоговойБазы(ВыборкаСтатусов, ТаблицаПроводок, ВыборкаДополнение, ВыборкаСторно)
	
	ТаблицаПроводокКорректировкиНУ = ТаблицаПроводокКорректировкиНУ(ВыборкаСтатусов, ТаблицаПроводок, ВыборкаДополнение, ВыборкаСторно);
	
	НаборЗаписей = РегистрыБухгалтерии.КорректировкиНалоговойБазы.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Регистратор.Установить(ВыборкаСтатусов.Ссылка);
	
	НаборЗаписей.ДополнительныеСвойства.Вставить("ПропуститьПроверкуЗапретаИзменения", Истина);
	
	НаборЗаписей.Загрузить(ТаблицаПроводокКорректировкиНУ);
	НаборЗаписей.Записать();
...
Функция ТаблицаПроводокКорректировкиНУ(ВыборкаСтатусов, ТаблицаПроводок, ВыборкаДополнение, ВыборкаСторно)
	
	ТаблицаПроводокКорректировкиНУ = ТаблицаПроводок.СкопироватьКолонки();
	ТаблицаПроводокКорректировкиНУ.Колонки.Добавить("НаборКорректировок", Новый ОписаниеТипов("СправочникСсылка.КорректировкиПрошлыхПериодовНУ"));
	ТаблицаПроводокКорректировкиНУ.Колонки.Добавить("ДатаОтражения", Новый ОписаниеТипов("Дата"));
	ТаблицаПроводокКорректировкиНУ.Колонки.Добавить("АннулирующаяЗапись", Новый ОписаниеТипов("Булево"));
	
	Для Каждого Проводка Из ТаблицаПроводок Цикл
		Если ЗначениеЗаполнено(Проводка.ПериодКорректировкиНУ) Тогда
			
			Если Проводка.СуммаНУКт = 0
				И Проводка.СуммаНУДт = 0 
				И Проводка.ВалютнаяСуммаКт = 0
				И Проводка.ВалютнаяСуммаДт = 0
				И Проводка.КоличествоКт = 0
				И Проводка.КоличествоДт = 0 Тогда
				Продолжить;
			КонецЕсли;
			
			ПроводкаКорректировки = ТаблицаПроводокКорректировкиНУ.Добавить();
			ЗаполнитьЗначенияСвойств(ПроводкаКорректировки, Проводка);
			ПроводкаКорректировки.Период = Проводка.ПериодКорректировкиНУ;
			ПроводкаКорректировки.ДатаОтражения = Проводка.Период;
			ПроводкаКорректировки.Сторно = Ложь;
			
			ПроводкаКорректировки = ТаблицаПроводокКорректировкиНУ.Добавить();
			ЗаполнитьЗначенияСвойств(ПроводкаКорректировки, Проводка);
			ПроводкаКорректировки.СуммаНУКт = -Проводка.СуммаНУКт;
			ПроводкаКорректировки.СуммаНУДт = -Проводка.СуммаНУДт;
			ПроводкаКорректировки.ВалютнаяСуммаКт = -Проводка.ВалютнаяСуммаКт;
			ПроводкаКорректировки.ВалютнаяСуммаДт = -Проводка.ВалютнаяСуммаДт;
			ПроводкаКорректировки.КоличествоКт = -Проводка.КоличествоКт;
			ПроводкаКорректировки.КоличествоДт = -Проводка.КоличествоДт;
			ПроводкаКорректировки.ДатаОтражения = Проводка.Период;
			ПроводкаКорректировки.АннулирующаяЗапись = Истина;
			ПроводкаКорректировки.Сторно = Ложь;
			
		КонецЕсли;
	КонецЦикла;
	
	Если ВыборкаСтатусов.Ссылка = ВыборкаСторно.Ссылка Тогда
		ДобавитьСтрокиВТаблицуПроводок(ТаблицаПроводокКорректировкиНУ, ВыборкаСторно);
		ВыборкаСторно.Следующий();
	КонецЕсли;
	
	Если ВыборкаСтатусов.Ссылка = ВыборкаДополнение.Ссылка Тогда
		ДобавитьСтрокиВТаблицуПроводок(ТаблицаПроводокКорректировкиНУ, ВыборкаДополнение);
		ВыборкаДополнение.Следующий();
	КонецЕсли;
	
	ТаблицаПроводокКорректировкиНУ.Сортировать("Организация, Сторно Убыв, Период");
	
	ТекущаяОрганизация = Неопределено;
	НаборКорректировок = Справочники.КорректировкиПрошлыхПериодовНУ.ПустаяСсылка();
	Для Каждого ПроводкаКорректировки Из ТаблицаПроводокКорректировкиНУ Цикл
		Если ТекущаяОрганизация <> ПроводкаКорректировки.Организация Тогда 
			ТекущаяОрганизация = ПроводкаКорректировки.Организация;
			НаборКорректировок = 
			Справочники.КорректировкиПрошлыхПериодовНУ.ОткрытыйНаборКорректировок(ТекущаяОрганизация);
		КонецЕсли;
		ПроводкаКорректировки.НаборКорректировок = НаборКорректировок;
	КонецЦикла;
	
	Возврат ТаблицаПроводокКорректировкиНУ;
...
Функция ТаблицаПроводок(НаборЗаписей = Неопределено)
	...
	ТаблицаПроводок.Колонки.Добавить("ПериодКорректировкиНУ", Новый ОписаниеТипов("Дата",,, Новый КвалификаторыДаты(ЧастиДаты.ДатаВремя)));
...
Процедура ЗаписатьТребующиеНастройкиСчетаРеглУчета(ТаблицаОшибокОтражения, ОтражаемыйДокумент)
	...
	Попытка
		...
		Для каждого СтрокаОшибки Из ТаблицаОшибокОтражения Цикл
			...
			Для каждого ЭлементИзмерения Из ИзмеренияРегистра Цикл
				СтруктураОтбора.Вставить(ЭлементИзмерения, СтрокаОшибки[ЭлементИзмерения]);
			КонецЦикла;
			УдаляемыеСтроки = ТаблицаОшибокОтражения.НайтиСтроки(СтруктураОтбора);
			Для Каждого УдаляемаяСтрока Из УдаляемыеСтроки Цикл
				Если УдаляемаяСтрока = СтрокаОшибки Тогда
					Продолжить;
				КонецЕсли;
				ТаблицаОшибокОтражения.Удалить(УдаляемаяСтрока);
			КонецЦикла;
...
Функция ТипыДокументовОтражаемыхВРеглУчете()
	
	Регистраторы = Метаданные.РегистрыСведений.ОтражениеДокументовВРеглУчете.СтандартныеРеквизиты.Регистратор.Тип;
	Возврат Новый ОписаниеТипов(Регистраторы, , "ДокументСсылка.КорректировкаРегистров");
	
КонецФункции

Функция РазрешенныеТипыРегистраторовНачальныхОстатков()
	
	ДоступныеТипы = Новый Массив;
	Для Каждого ИмяДокумента Из ИменаДокументовОтражаемыеВРеглУчетеВнеЗависимостиОтДаты() Цикл
		ДоступныеТипы.Добавить(Тип("ДокументСсылка." + ИмяДокумента));
	КонецЦикла;
...
Функция РазрешенныеТипыРегистраторовНачальныхОстатковПоОснованию()
	
	ТипыДокументов = Новый Соответствие;
	Для Каждого Элемент Из ДокументыОтражаемыеВРеглУчетеВнеЗависимостиОтДатыЗависящиеОтОснования() Цикл
		ТипыДокументов.Вставить(Тип("ДокументСсылка." + Элемент.Ключ), Элемент.Значение.Имя);
	КонецЦикла;
...
// Формирует запрос, использующийся при включении настроек детализации регламентированного учета.
// 
// Параметры:
//  ВременныеТаблицы - МенеджерВременныхТаблиц, Неопределено - Временные таблицы
//  СоздаватьСубконто - Булево - Создавать субконто при детализации продаж.
//  ЗамещатьНастройкиНалогообложения - Булево - Замещать ВТ настройки налогообложения (используется при отладке).
// 
// Возвращаемое значение:
//  Запрос - Запрос детальных данных.
Функция ЗапросДетальныхДанных(ВременныеТаблицы, СоздаватьСубконто, ЗамещатьНастройкиНалогообложения = Ложь, ИмяДокумента = "") Экспорт
	...
	ВидыСчетовДетализации = Новый Массив;
	...
	Если РеглУчетСервер.ИспользуетсяРеглУчетТоваровИМатериаловПоПартиямПриобретения()
		ИЛИ РеглУчетСервер.ИспользуетсяРеглУчетПродукцииИПолуфабрикатовПоПартиямВыпуска() Тогда
		
		ТекстыЗапроса.Добавить(РеглУчетВыборкиСерверПовтИсп.ТекстДетальныеДанныеНоменклатуры(РежимОтладки));
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(
			СтруктураПараметров, ПараметрыДетализацииНоменклатуры(), Истина);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ВидыСчетовДетализации,
			РеглУчетВыборкиСерверПовтИсп.ВидыСчетовДетализацииТМЦ());
		ТекстыЗапросаДетальныхДанных.Добавить("ВЫБРАТЬ * ИЗ ДетальныеДанныеНоменклатуры");
		ТекстыУничтоженияТаблиц.Добавить("УНИЧТОЖИТЬ ДетальныеДанныеНоменклатуры");
		
	КонецЕсли;
	
	Если РеглУчетСервер.ИспользуетсяРеглУчетНЗППоПартиямПроизводства() Тогда
		
		ТекстыЗапроса.Добавить(РеглУчетВыборкиСерверПовтИсп.ТекстДетальныеДанныеПроизводства(РежимОтладки));
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(
			СтруктураПараметров, ПараметрыДетализацииПроизводства(), Истина);
		ВидыСчетовДетализации.Добавить(Перечисления.ВидыСчетовРеглУчета.МатериалыИРаботыВПодразделении);
		ВидыСчетовДетализации.Добавить(Перечисления.ВидыСчетовРеглУчета.НезавершенноеПроизводство);
		ТекстыЗапросаДетальныхДанных.Добавить("ВЫБРАТЬ * ИЗ ДетальныеДанныеПроизводства");
		ТекстыУничтоженияТаблиц.Добавить("УНИЧТОЖИТЬ ДетальныеДанныеПроизводства");
		
	КонецЕсли;
	
	Если РеглУчетСервер.ИспользуетсяРеглУчетПродажПоПартиямРеализованнойНоменклатуры() Тогда
		
		Если СоздаватьСубконто Тогда
			
			ЗапросДанныеПродаж = Новый Запрос;
			
			СтруктураПараметровЗапросаДанныхПродаж = ПараметрыДетализацииПродаж();
			
			Для Каждого ЭлементСтруктуры Из СтруктураПараметровЗапросаДанныхПродаж Цикл
				ЗапросДанныеПродаж.УстановитьПараметр(ЭлементСтруктуры.Ключ, ЭлементСтруктуры.Значение);
			КонецЦикла;
			...
			ЗапросДанныеПродаж.Текст = РеглУчетВыборкиСерверПовтИсп.ТекстВтДанныеПродаж(ЗамещатьНастройкиНалогообложения);
			ЗапросДанныеПродаж.МенеджерВременныхТаблиц = ВременныеТаблицы;
			ЗапросДанныеПродаж.Выполнить();
			
			Справочники.ПартииРеализованнойНоменклатуры.СоздатьПартииРеализованнойНоменклатуры(ВременныеТаблицы);
			Справочники.ВариантыНалогообложенияРеализации.СоздатьНалогообложенияРеализаций(ВременныеТаблицы);
			
		КонецЕсли;
		
		ТекстыЗапроса.Добавить(РеглУчетВыборкиСерверПовтИсп.ТекстДетальныеДанныеПродаж(
			СоздаватьСубконто, ЗамещатьНастройкиНалогообложения, РежимОтладки));
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(
			СтруктураПараметров, ПараметрыДетализацииПродаж(), Истина);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ВидыСчетовДетализации,
			РеглУчетВыборкиСерверПовтИсп.ВидыСчетовДетализацииПродаж());
		ТекстыЗапросаДетальныхДанных.Добавить("ВЫБРАТЬ * ИЗ ДетальныеДанныеПродаж");
		ТекстыУничтоженияТаблиц.Добавить("УНИЧТОЖИТЬ ДетальныеДанныеПродаж");
	...
	КонецЦикла;
	Запрос.УстановитьПараметр("ВидыСчетовДетализации", ВидыСчетовДетализации);
	
	Возврат Запрос;
	
КонецФункции

// Параметры детализации номенклатуры.
// 
// Возвращаемое значение:
//  Структура - Параметры детализации номенклатуры:
// * ВидыСчетовДетализацииТМЦ - Массив Из ПеречислениеСсылка.ВидыСчетовРеглУчета
// * ХозОперацииВозвратаНоменклатуры - Массив Из ПеречислениеСсылка.ХозяйственныеОперации
// * ТипыЗапасовВнешнийВладелец - Массив Из ПеречислениеСсылка.ТипыЗапасов
// * ФормироватьВидыЗапасовПоГруппамФинансовогоУчета - Булево
// * КорВидыСчетовДетализации - Массив Из ПеречислениеСсылка.ВидыСчетовРеглУчета
Функция ПараметрыДетализацииНоменклатуры() Экспорт
	
	СтруктураПараметров = Новый Структура;
	СтруктураПараметров.Вставить("ВидыСчетовДетализацииТМЦ",
		РеглУчетВыборкиСерверПовтИсп.ВидыСчетовДетализацииТМЦ());
	СтруктураПараметров.Вставить("ХозОперацииВозвратаНоменклатуры",
		РеглУчетВыборкиСерверПовтИсп.ХозОперацииВозвратаНоменклатуры());
	СтруктураПараметров.Вставить("ТипыЗапасовВнешнийВладелец",
		РеглУчетВыборкиСерверПовтИсп.ТипыЗапасовВнешнийВладелец());
	СтруктураПараметров.Вставить("ФормироватьВидыЗапасовПоГруппамФинансовогоУчета",
		ПолучитьФункциональнуюОпцию("ФормироватьВидыЗапасовПоГруппамФинансовогоУчета"));
	СтруктураПараметров.Вставить("КорВидыСчетовДетализации",
		РеглУчетВыборкиСерверПовтИсп.КорВидыСчетовДетализации());
	Возврат СтруктураПараметров;
	
КонецФункции

// Параметры детализации производства.
// 
// Возвращаемое значение:
//  Структура - Параметры детализации производства:
// * ВидыСчетовДетализацииПроизводства - Массив Из ПеречислениеСсылка.ВидыСчетовРеглУчета
// * ЗабалансовыеРазделыУчета - Массив Из ПеречислениеСсылка.РазделыУчетаСебестоимостиТоваров
// * СчетМатериалыПодрядныеРаботыОсновногоПроизводстваКИспользованию - ПланСчетовСсылка.Хозрасчетный
// * ХозОперацииВозвратаИзПроизводства - Массив Из ПеречислениеСсылка.ХозяйственныеОперации
// * АналитическийУчетПоГруппамПродукции - Булево
// * ПартионныйУчетВерсии22 - Булево
// * ДатаПереходаНаПартионныйУчетВерсии22 - Дата
// * КорВидыСчетовДетализации - Массив Из ПеречислениеСсылка.ВидыСчетовРеглУчета
Функция ПараметрыДетализацииПроизводства() Экспорт
	
	СтруктураПараметров = Новый Структура;
	СтруктураПараметров.Вставить("ВидыСчетовДетализацииПроизводства",
		РеглУчетВыборкиСерверПовтИсп.ВидыСчетовДетализацииПроизводства());
	СтруктураПараметров.Вставить("ЗабалансовыеРазделыУчета",
		Перечисления.РазделыУчетаСебестоимостиТоваров.ЗабалансовыеРазделыУчета());
	СтруктураПараметров.Вставить("СчетМатериалыПодрядныеРаботыОсновногоПроизводстваКИспользованию",
		РеглУчетВыборкиСерверПовтИсп.СчетМатериалыПодрядныеРаботыОсновногоПроизводстваКИспользованию());
	СтруктураПараметров.Вставить("ХозОперацииВозвратаИзПроизводства",
		РеглУчетВыборкиСерверПовтИсп.ХозОперацииВозвратаИзПроизводства());
	СтруктураПараметров.Вставить("АналитическийУчетПоГруппамПродукции",
...
// Параметры детализации продаж.
// 
// Возвращаемое значение:
//  Структура - Параметры детализации продаж:
// * ВидыСчетовДетализацииПродаж - Массив Из ПеречислениеСсылка.ВидыСчетовРеглУчета
// * ВидыСчетовДетализацииПродажВыручка - Массив Из ПеречислениеСсылка.ВидыСчетовРеглУчета
// * ВидыСчетовДетализацииПродажРасходыНаПродажу - Массив Из ПеречислениеСсылка.ВидыСчетовРеглУчета
// * ВидыСчетовДетализацииПродажСебестоимость - Массив Из ПеречислениеСсылка.ВидыСчетовРеглУчета
// * ВидыСчетовДетализацииПродажСписаниеРасходов- Массив Из ПеречислениеСсылка.ВидыСчетовРеглУчета
// * ТипыДокументовРеализации - Массив Из Тип
// * СчетПрибыльУбытокОтПродаж - ПланСчетовСсылка.Хозрасчетный
// * ФормироватьВидыЗапасовПоГруппамФинансовогоУчета - Булево
// * КорВидыСчетовДетализации - Массив Из ПеречислениеСсылка.ВидыСчетовРеглУчета
Функция ПараметрыДетализацииПродаж() Экспорт
	
	СтруктураПараметров = Новый Структура;
	СтруктураПараметров.Вставить("ВидыСчетовДетализацииПродаж",
		РеглУчетВыборкиСерверПовтИсп. ВидыСчетовДетализацииПродаж());
	СтруктураПараметров.Вставить("ВидыСчетовДетализацииПродажВыручка",
		РеглУчетВыборкиСерверПовтИсп.ВидыСчетовДетализацииПродажВыручка());
	СтруктураПараметров.Вставить("ВидыСчетовДетализацииПродажРасходыНаПродажу",
		РеглУчетВыборкиСерверПовтИсп.ВидыСчетовДетализацииПродажРасходыНаПродажу());
	СтруктураПараметров.Вставить("ВидыСчетовДетализацииПродажСебестоимость",
		РеглУчетВыборкиСерверПовтИсп.ВидыСчетовДетализацииПродажСебестоимость());
	СтруктураПараметров.Вставить("ВидыСчетовДетализацииПродажСписаниеРасходов",
		РеглУчетВыборкиСерверПовтИсп.ВидыСчетовДетализацииПродажСписаниеРасходов());
	СтруктураПараметров.Вставить("ТипыДокументовРеализации",
				Метаданные.Справочники.ПартииРеализованнойНоменклатуры.Реквизиты.ДокументРеализации.Тип.Типы());
	СтруктураПараметров.Вставить("СчетПрибыльУбытокОтПродаж", РеглУчетВыборкиСерверПовтИсп.СчетПрибыльУбытокОтПродаж());
	СтруктураПараметров.Вставить("ФормироватьВидыЗапасовПоГруппамФинансовогоУчета",
		ПолучитьФункциональнуюОпцию("ФормироватьВидыЗапасовПоГруппамФинансовогоУчета"));
	СтруктураПараметров.Вставить("КорВидыСчетовДетализации",
		РеглУчетВыборкиСерверПовтИсп.КорВидыСчетовДетализации());
...
// Виды счетов, для которых выполняется детализация регламентированного учета.
// 
// Возвращаемое значение:
//  Массив Из ПеречислениеСсылка.ВидыСчетовРеглУчета
Функция ВидыСчетовДетализации() Экспорт
	
	ВидыСчетовДетализации = Новый Массив;
	Если РеглУчетСервер.ИспользуетсяРеглУчетТоваровИМатериаловПоПартиямПриобретения()
		ИЛИ РеглУчетСервер.ИспользуетсяРеглУчетПродукцииИПолуфабрикатовПоПартиямВыпуска() Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			ВидыСчетовДетализации, РеглУчетВыборкиСерверПовтИсп.ВидыСчетовДетализацииТМЦ());
	КонецЕсли;
	
	Если РеглУчетСервер.ИспользуетсяРеглУчетНЗППоПартиямПроизводства() Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			ВидыСчетовДетализации, РеглУчетВыборкиСерверПовтИсп.ВидыСчетовДетализацииПроизводства());
	КонецЕсли;
	
	Если РеглУчетСервер.ИспользуетсяРеглУчетПродажПоПартиямРеализованнойНоменклатуры() Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			ВидыСчетовДетализации, РеглУчетВыборкиСерверПовтИсп.ВидыСчетовДетализацииПродаж());
	КонецЕсли;
	
	Возврат ВидыСчетовДетализации;
	
КонецФункции

Функция ТребуетсяДетализацияРеглУчета(ВременныеТаблицы)
	
	ТребуетсяДетализация = РеглУчетСервер.ИспользуетсяРеглУчетНЗППоПартиямПроизводства()
		ИЛИ РеглУчетСервер.ИспользуетсяРеглУчетТоваровИМатериаловПоПартиямПриобретения()
		ИЛИ РеглУчетСервер.ИспользуетсяРеглУчетПродукцииИПолуфабрикатовПоПартиямВыпуска()
		ИЛИ РеглУчетСервер.ИспользуетсяРеглУчетПродажПоПартиямРеализованнойНоменклатуры();
		
	Если ТребуетсяДетализация Тогда
		...
		|	Данные КАК Данные
		|ГДЕ
		|	Данные.ВидСчетаДт В(&ВидыСчетовДетализации)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|	Данные КАК Данные
		|ГДЕ
		|	Данные.ВидСчетаКт В(&ВидыСчетовДетализации)";
		Результат = Запрос.Выполнить();
		ТребуетсяДетализация = НЕ Результат.Пустой();
		
	КонецЕсли;
	
	Возврат ТребуетсяДетализация;
	
КонецФункции

Процедура СформироватьПартииПродукцииСоСтатьямиКалькуляции(ВременныеТаблицы)
	...
	КонецЕсли;
	Если РеглУчетСервер.ИспользуетсяРеглУчетПродажПоПартиямРеализованнойНоменклатуры() Тогда
		ВидыСчетовДетализации.Добавить(Перечисления.ВидыСчетовРеглУчета.СебестоимостьПродаж);
	КонецЕсли;
	...
	|	Данные.Ссылка КАК Ссылка
	|ПОМЕСТИТЬ ДанныеДляДетализацииПоСтатьямКалькуляции
	|ИЗ
	|	(ВЫБРАТЬ
	|		Данные.Ссылка КАК Ссылка
	|	ИЗ
	...
	|		Данные КАК Данные
	|	ГДЕ
	|		Данные.ВидСчетаКт В(&ВидыСчетовДетализации)) КАК Данные
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка";
	Запрос.Выполнить();
	РасчетДетализацииПродукции.СформироватьДетализациюПродукции(ВременныеТаблицы,
		"ДанныеДляДетализацииПоСтатьямКалькуляции", "Ссылка");
	
КонецПроцедуры

#КонецОбласти

Процедура УничтожитьВспомогательныеВТ(ВременныеТаблицы)
	
	УничтожаемыеВТ = Новый Массив;
	
	НачальнаяВТ = ВременныеТаблицы.Таблицы.Найти("ДокументыСторно");
	Если НачальнаяВТ = Неопределено Тогда
		Возврат;
	КонецЕсли;
	НачальныйИндекс = ВременныеТаблицы.Таблицы.Индекс(НачальнаяВТ);
	
	Для Индекс = НачальныйИндекс По ВременныеТаблицы.Таблицы.Количество() - 1 Цикл
		УничтожаемыеВТ.Добавить(ВременныеТаблицы.Таблицы[Индекс].ПолноеИмя);
	КонецЦикла;
	
	ОбщегоНазначенияУТ.УничтожитьВременныеТаблицы(ВременныеТаблицы, УничтожаемыеВТ);
