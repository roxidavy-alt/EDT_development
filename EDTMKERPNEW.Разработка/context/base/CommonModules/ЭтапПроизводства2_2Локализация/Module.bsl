...
#Область ТекстДвижениеМатериальныхЗатрат // (Дт 2Х :: Кт 2Х)
	...
	|
	|	ВЫБОР
	|		КОГДА Стоимости.КорРазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НезавершенноеПроизводство)
	...
	|	
	|	ВЫБОР
	|		КОГДА Стоимости.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НезавершенноеПроизводство)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НезавершенноеПроизводство)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.МатериалыИРаботыВПодразделении)
	...
	|	""ЭтапПроизводства22Локализация_1358"" КАК НомерФрагмента
	|
	|ИЗ
...
#Область ТекстДвижениеМатериальныхЗатратСИспользованиемСчета40 // (Дт 40 :: Кт 2Х)
	...
	|	""ЭтапПроизводства22Локализация_1359"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстВыпускПродукцииПоПлановойСтоимости // (Дт 20 :: Кт 40)
	...
	|	""ЭтапПроизводства22Локализация_1360"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстВыпускПродукцииПоПлановойСтоимостиОтклонение // (Дт 20 :: Кт 40)
	...
	|	""ЭтапПроизводства22Локализация_1361"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстСписаниеНаПрочиеАктивыИзделий // (Дт ХХ.ХХ :: Кт 20)
	ТекстСписаниеНаПрочиеАктивыИзделий = "
	...
	|	""Списание на прочие активы изделий"" КАК Содержание,
	|	""ЭтапПроизводства22Локализация_1362"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.ДвиженияПоПрочимАктивамПассивам КАК Таблица
	|	ПО
	|		Строки.Ссылка = Таблица.Регистратор
	|		И Строки.Организация = Таблица.Организация
	|		И Строки.Период = Таблица.Период
	|		И Строки.ИдентификаторФинЗаписи = Таблица.ИдентификаторФинЗаписи
...
#Область ПеремещениеТрудозатратНаПолучателяПродукции // (Дт 20 :: Кт 20)
	...
	|	""ЭтапПроизводства22Локализация_1365"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ПеремещениеТрудозатратСИспользованиемСчета40 // (Дт 40 :: Кт 20)
	...
	|	Строки.ВременнаяРазница КАК СуммаВРКт,
	|	""Перемещение трудозатрат на получателя продукции"" КАК Содержание,
	|	""ЭтапПроизводства22Локализация_1366"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ПеремещениеПостатейныхНаПолучателяПродукции // (Дт 20 :: Кт 20)
	...
	|	""ЭтапПроизводства22Локализация_1367"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ПеремещениеПостатейныхСИспользованиемСчета40 // (Дт 40 :: Кт 20)
	...
	|	Строки.ВременнаяРазница КАК СуммаВРКт,
	|	""Перемещение постатейных на получателя продукции"" КАК Содержание,
	|	""ЭтапПроизводства22Локализация_1368"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстВыпускВозвратныхОтходов // (Дт 2Х :: Кт 2Х)
	...
	|	ВЫБОР
	|		КОГДА КорСтроки.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НезавершенноеПроизводство)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НезавершенноеПроизводство)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.МатериалыИРаботыВПодразделении)
	|	КОНЕЦ КАК ВидСчетаДт,
	...
	|	""ЭтапПроизводства22Локализация_1369"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|		ИЛИ Константы.РеглУчетНЗППоПартиямПроизводства И Строки.Период >= Константы.ДатаНачалаРеглУчетаНЗППоПартиямПроизводства)
	|";
#КонецОбласти
...
#Область ТекстСписаниеКомиссионныхТоваровНаПроизводство // (Дт 20 :: 76.ОК)
	...
	|
	|	ВЫБОР
	|		КОГДА Строки.КорРазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НезавершенноеПроизводство)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НезавершенноеПроизводство)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.МатериалыИРаботыВПодразделении)
	|	КОНЕЦ КАК ВидСчетаДт,
	...
	|	""ЭтапПроизводства22Локализация_1370"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстВключениеИсключениеНДСВСтоимость // (Дт 20 :: Кт 19.03)
	...
	|		ВЫБОР
	|			КОГДА Партии.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НезавершенноеПроизводство)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НезавершенноеПроизводство)
	|			ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.МатериалыИРаботыВПодразделении)
	|		КОНЕЦ
	...
	|	""ЭтапПроизводства22Локализация_1371"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#КонецОбласти
	...
	ТекстыОтражения.Добавить(ТекстСписаниеНаПрочиеАктивыИзделий);
	...
	// Детализация регл. учета
	ТекстыОтражения.Добавить(ТекстСписаниеПроизводственныхРасходовНаПартииПроизводства()); // Списание производственных расходов на партии производства (Дт 20.03 :: Кт 20.01)
...
// Параметры:
//  Объект - ДокументОбъект - Обрабатываемый документ.
//  РезультатПроверки - см. РасчетСебестоимостиПрикладныеАлгоритмы.ПериодыИзмененияТабличныеЧастиДокумента
//
Процедура ЗаполнитьДополнительныеСвойства(Объект, РезультатПроверки) Экспорт
	...
	Для Каждого КлючЗначение Из РезультатПроверки Цикл
		...
		Если КлючЗначение.Ключ = "Трудозатраты"
		 ИЛИ КлючЗначение.Ключ = "ОбщийМинимальныйПериод" Тогда
			Продолжить;
		КонецЕсли;
		
		ДатаТаблицы = КлючЗначение.Значение;
		Если ДатаТаблицы = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ОбщийМинимальныйПериод) Тогда
			ОбщийМинимальныйПериод = Мин(ОбщийМинимальныйПериод, ДатаТаблицы);
		Иначе
			ОбщийМинимальныйПериод = ДатаТаблицы;
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ОбщийМинимальныйПериод) Тогда
		...
		ПериодПерезаписиДвижений = НачалоМесяца(ОбщийМинимальныйПериод);
	...
	Иначе
		...
		Если ДополнительныеСвойства.Свойство("ПериодПерезаписиДвиженийРегламентированныйУчет") Тогда
			ПериодПерезаписиДвижений = ДополнительныеСвойства.ПериодПерезаписиДвиженийРегламентированныйУчет;
		Иначе
	...
	ДополнительныеСвойства.Вставить("ПериодПерезаписиДвиженийРегламентированныйУчет", ПериодПерезаписиДвижений);
...
//
Процедура ДобавитьТекстПроверкиПоРНПТ(ТекстыЗапроса) Экспорт
...
// 	Отказ - Булево
//
Процедура СообщитьОшибкиЗаполненияГруппЗатрат(Выборка, КлючДанных, ПутьКДанным, ПредставлениеОбъекта, Отказ) Экспорт
	
	//++ Локализация
	
	ШаблонНеУказанПрослеживаемыйМатериал =
		НСтр("ru = 'Для ""%1"" должен быть указан материал, прослеживаемый по РНПТ, так как в нем указано изделие, прослеживаемое по РНПТ.';
			|en = 'For ""%1"", the material must be traceable by goods batch registration number, as it contains a finished product which is traceable by goods batch registration number.'");
	
	ШаблонНеУказаноПрослеживаемоеИзделие =
		НСтр("ru = 'Для ""%1"" должно быть указано изделие, прослеживаемое по РНПТ, так как в нем указан материал, прослеживаемый по РНПТ.';
			|en = 'For ""%1"", the finished product must be traceable by goods batch registration number, as it contains a material which is traceable by goods batch registration number.'");
	
	Если Выборка.ТипПроверки = "НеУказаноПрослеживаемоеИзделие" Тогда
		
		ТекстСообщения = СтрШаблон(ШаблонНеУказаноПрослеживаемоеИзделие, ПредставлениеОбъекта);
	...
	Если Выборка.ТипПроверки = "НеУказанПрослеживаемыйМатериал" Тогда
		...
		ТекстСообщения = СтрШаблон(ШаблонНеУказанПрослеживаемыйМатериал, ПредставлениеОбъекта);
...
Процедура УстановитьПараметрыВыборочнойРегистрацииКОтражениюВРеглУчете(Объект)
	...
	Объект.ДополнительныеСвойства.Свойство("ПериодПерезаписиДвиженийРегламентированныйУчет", ПериодЗаполнения);
	...
	Если ТекстыЗапроса.Количество() > 0 Тогда
		...
		Запрос = Новый Запрос;
		Документы.ЭтапПроизводства2_2.ЗаполнитьПараметрыИнициализации(Запрос);
...
Функция ТекстПроверкиНеУказаноПрослеживаемоеИзделие()
	...
	|	ИмпортныеМатериалыДокумента.Ссылка КАК Ссылка,
	...
	|	ПО ИмпортныеМатериалыДокумента.Ссылка = ИмпортныеИзделияДокумента.Ссылка
	|	И ИмпортныеИзделияДокумента.Прослеживаемый
	|
	|ГДЕ
	|	ИмпортныеМатериалыДокумента.Прослеживаемый
	|	И ИмпортныеИзделияДокумента.Ссылка ЕСТЬ NULL
	|
	|СГРУППИРОВАТЬ ПО
	|	ИмпортныеМатериалыДокумента.Ссылка
	|";
...
Функция ТекстПроверкиНеУказанПрослеживаемыйМатериал()
	...
	|	ИмпортныеИзделияДокумента.Ссылка   КАК Ссылка,
	...
	|	И ИмпортныеМатериалыДокумента.Ссылка ЕСТЬ NULL
	|
	|СГРУППИРОВАТЬ ПО
	|	ИмпортныеИзделияДокумента.Ссылка
	|";
...
Функция ЕстьМаркируемаяПродукцияГИСМ(Источник, ПутиКДанным)
	...
	|	ПО Таблица.Ссылка = ОписаниеНоменклатурыИС.Номенклатура
...
Процедура ЗаполнитьПризнакЕстьМаркируемаяПродукция(ДанныеСпецификаций) Экспорт
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|	ПО Таблица.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|	И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииМЕХ)";
...
// Текст фрагмента отражения списания производственных затрат на партии производства
//
// Возвращаемое значение:
//	Строка - сформированный текст запроса.
//
Функция ТекстСписаниеПроизводственныхРасходовНаПартииПроизводства()
	
	ТекстЗапроса = "
	|////////////////////////////////////////////////////////////////////////////////////////////////////
	|// Списание производственных расходов на партии производства (Дт 20.03 :: Кт 20.01)
	|ВЫБРАТЬ
	|	СтрокиДт.Ссылка КАК Ссылка,
	|	СтрокиДт.Период КАК Период,
	|	СтрокиДт.Организация КАК Организация,
	|	СтрокиДт.ИдентификаторФинЗаписи КАК ИдентификаторСтрокиДт,
	|
	|	ЕСТЬNULL(Стоимости.Сумма, СтрокиДт.Сумма) КАК Сумма,
	|	ЕСТЬNULL(Стоимости.СуммаУУ, СтрокиДт.СуммаУУ) КАК СуммаУУ,
	|
	|	// Дт ///////////////////////////////////////////////////////////////////////////////////////////
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НезавершенноеПроизводство) КАК ВидСчетаДт,
	...
	|	НЕОПРЕДЕЛЕНО КАК СчетКт,
	|	ВЫБОР
	|		КОГДА СтрокиКт.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыЗатратРегл.Работы)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыЗатратРегл.МатериальныеЗатраты)
	|	КОНЕЦ КАК СубконтоКт1,
	|	СтрокиКт.Номенклатура КАК СубконтоКт2,
	|	СтрокиКт.ГруппаПродукции КАК СубконтоКт3,
	|
	|	0 КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	|	0 КАК СуммаНУКт,
	...
	|	0 КАК СуммаВРКт,
	|
	|	""Списание производственных расходов на партии производства"" КАК Содержание,
	|	""ЭтапПроизводства22Локализация_1944"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|		И СтрокиДт.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НезавершенноеПроизводство)
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|		И СтрокиДт.ВидДвижения <> СтрокиКт.ВидДвижения
	|		И СтрокиДт.Номенклатура = СтрокиКт.КорНоменклатура
	|		И СтрокиДт.ГруппаФинансовогоУчета = СтрокиКт.КорГруппаФинансовогоУчета
	|		И СтрокиДт.ГруппаПродукции = СтрокиКт.КорГруппаПродукции
	|		И СтрокиДт.Склад = СтрокиКт.КорСклад
	|		И СтрокиДт.ПодразделениеАналитики = СтрокиКт.КорПодразделениеАналитики
	|		И СтрокиДт.РазделУчета = СтрокиКт.КорРазделУчета
	...
	|		И СтрокиКт.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Константы КАК Константы
	|	ПО
	...
	|		ВтСтоимости КАК Стоимости
	|	ПО
	|		СтрокиДт.Ссылка = Стоимости.Ссылка
	|		И СтрокиДт.Период = Стоимости.Период
	|		И СтрокиДт.ВидДвижения = Стоимости.ВидДвижения
	|		И СтрокиДт.РазделУчета = Стоимости.РазделУчета
	|		И СтрокиДт.Номенклатура = Стоимости.Номенклатура
	|		И СтрокиДт.ГруппаФинансовогоУчета = Стоимости.ГруппаФинансовогоУчета
	|		И СтрокиДт.ГруппаПродукции = Стоимости.ГруппаПродукции
	|		И СтрокиДт.Склад = Стоимости.Склад
	|		И СтрокиДт.ИдентификаторФинЗаписи = Стоимости.ИдентификаторФинЗаписи
	|		И СтрокиДт.НастройкаХозяйственнойОперации = Стоимости.НастройкаХозяйственнойОперации
	|		И Стоимости.Сумма <> 0
	|ГДЕ
	|	СтрокиДт.НастройкаХозяйственнойОперации = ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.СписаниеРасходовНаПартииПроизводства)
	|		И Константы.РеглУчетНЗППоПартиямПроизводства
	|		И СтрокиДт.Период >= Константы.ДатаНачалаРеглУчетаНЗППоПартиямПроизводства
	|";
