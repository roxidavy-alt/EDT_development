...
Функция ПравилаЗаполненияПоляТипЗаписи(ИмяДокументаДляОтбора = "") Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ИмяДокументаДляОтбора) ИЛИ НРег(ИмяДокументаДляОтбора) = НРег(ТекущийДокумент) Тогда
		...
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПроизводствоПродукцииИзДавальческогоСырья2_5;
		НовоеПравило.ТипЗаписиРасход = Перечисления.ТипыЗаписейПартий.Потребление;
		НовоеПравило.ТипЗаписиПриход = Перечисления.ТипыЗаписейПартий.Перемещение;
		
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РемонтПродукцииИзДавальческогоСырья2_5;
		НовоеПравило.ТипЗаписиРасход = Перечисления.ТипыЗаписейПартий.Потребление;
		НовоеПравило.ТипЗаписиПриход = Перечисления.ТипыЗаписейПартий.Перемещение;
		
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыполнениеРаботСИспользованиемДавальческогоСырья2_5;
		НовоеПравило.ТипЗаписиРасход = Перечисления.ТипыЗаписейПартий.Потребление;
		НовоеПравило.ТипЗаписиПриход = Перечисления.ТипыЗаписейПартий.Перемещение;
		
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыполнениеРемонтныхРаботСИспользованиемДавальческогоСырья2_5;
		НовоеПравило.ТипЗаписиРасход = Перечисления.ТипыЗаписейПартий.Потребление;
		НовоеПравило.ТипЗаписиПриход = Перечисления.ТипыЗаписейПартий.Перемещение;
...
//++ НЕ УТ
#Область ДетализацияПродукцииПоСтатьямКалькуляции

// Определяет, используется ли детализация продукции по статьям калькуляции на указанную дату.
//
// Параметры:
//	Период - Дата - дата, для которой надо определить использование детализации.
//
// Возвращаемое значение:
//	Булево - признак использования детализация продукции на указанную дату
//
Функция ИспользуетсяДетализацияПродукции(Период) Экспорт

	Если НЕ РасчетСебестоимостиПовтИсп.ПартионныйУчетВерсии22(Период)
	 ИЛИ НЕ ПолучитьФункциональнуюОпцию("РеглУчетНЗППоПартиямПроизводства")
	 ИЛИ НЕ ПолучитьФункциональнуюОпцию("РеглУчетПродажПоПартиямРеализованнойНоменклатуры")
	 ИЛИ НачалоМесяца(Период) < Константы.ДатаНачалаРеглУчетаНЗППоПартиямПроизводства.Получить() 
	 ИЛИ НачалоМесяца(Период) < Константы.ДатаНачалаРеглУчетаПродажПоПартиямРеализованнойНоменклатуры.Получить() Тогда
		Возврат Ложь;
 	КонецЕсли;
