...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	|	""ВводОстатковВнеоборотныхАктивовЛокализация_56"" КАК НомерФрагмента
	|ИЗ
	...
	|	""ВводОстатковВнеоборотныхАктивовЛокализация_57"" КАК НомерФрагмента
	|ИЗ
	|	втСтоимостьОС КАК Строки
	|
	|ГДЕ
	...
	|	""ВводОстатковВнеоборотныхАктивовЛокализация_58"" КАК НомерФрагмента
	|ИЗ
	...
	|	""ВводОстатковВнеоборотныхАктивовЛокализация_59"" КАК НомерФрагмента
	|ИЗ
	|	втАмортизацияОС КАК Строки
	|ГДЕ
	...
	|	""ВводОстатковВнеоборотныхАктивовЛокализация_60"" КАК НомерФрагмента
	|ИЗ
	|	втАмортизацияОС КАК Строки
	|ГДЕ
	...
	|	""ВводОстатковВнеоборотныхАктивовЛокализация_61"" КАК НомерФрагмента
	|ИЗ
	|	втСтоимостьОС КАК Строки
	|ГДЕ
	...
	|	0 КАК СуммаВРКт,
	|	""Поступление на забалансовый учет арендуемых ОС"" КАК Содержание,
	|	""ВводОстатковВнеоборотныхАктивовЛокализация_62"" КАК НомерФрагмента
	|ИЗ
	|	втСтоимостьОС КАК Строки
	|ГДЕ
	|	Строки.ЗалоговаяСтоимость <> 0
	...
	|	""ВводОстатковВнеоборотныхАктивовЛокализация_63"" КАК НомерФрагмента
	|ИЗ
	|	втСтоимостьОС КАК Строки
	|ГДЕ
	...
	|	""ВводОстатковВнеоборотныхАктивовЛокализация_64"" КАК НомерФрагмента
	|	
	|ИЗ
	...
	|	""ВводОстатковВнеоборотныхАктивовЛокализация_65"" КАК НомерФрагмента
	|ИЗ
	...
	|	""ВводОстатковВнеоборотныхАктивовЛокализация_66"" КАК НомерФрагмента
	|ИЗ
	|	втСтоимостьНМА КАК Строки
	|ГДЕ
	|	Строки.СтоимостьЦФ <> 0
	...
	|	""ВводОстатковВнеоборотныхАктивовЛокализация_67"" КАК НомерФрагмента
	|ИЗ
	...
	|	""ВводОстатковВнеоборотныхАктивовЛокализация_68"" КАК НомерФрагмента
	|ИЗ
	|	втАмортизацияНМА КАК Строки
	|ГДЕ
	|	Строки.АмортизацияЦФ <> 0
	...
	|	""ВводОстатковВнеоборотныхАктивовЛокализация_69"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|	""ВводОстатковВнеоборотныхАктивовЛокализация_70"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВводОстатковВнеоборотныхАктивов2_4 КАК Операция
	...
	|	""ВводОстатковВнеоборотныхАктивовЛокализация_71"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВводОстатковВнеоборотныхАктивов2_4 КАК Операция
	...
	|	""ВводОстатковВнеоборотныхАктивовЛокализация_1677"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВводОстатковВнеоборотныхАктивов2_4 КАК Операция
...
Процедура ФормаРедактированияСтрокиОС_ПриЧтенииСозданииНаСервере(Форма) Экспорт
	...
	Если Форма.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковПереданныхВАрендуОС
		...
		Элементы.МОЛ.Подсказка = НСтр("ru = 'Материально ответственное лицо за основное средство на время аренды.';
										|en = 'Employee responsible for the fixed asset for the duration of rental.'")
	Иначе
		...
		Элементы.МОЛ.Подсказка = НСтр("ru = 'Материально ответственное лицо за основное средство.';
										|en = 'Employee responsible for fixed asset.'")
	КонецЕсли;
...
Процедура ФормаРедактированияСтрокиОС_НастроитьЗависимыеЭлементыФормы(Форма, СтруктураИзмененныхРеквизитов) Экспорт
	...
	Если ОбновитьВсе Тогда
		...
		Если ВспомогательныеРеквизиты.ВариантУчетаОтложенногоНалога = ПредопределенноеЗначение("Перечисление.ВариантыУчетаОтложенногоНалога.БалансовыйМетод")
			И Форма.ОтражатьВРеглУчете Тогда
			
			Элементы.ФормаПоказатьПР.Видимость = Истина;
			
			Если Форма.ТекущаяСтоимостьПР <> 0
				ИЛИ Форма.ТекущаяСтоимостьПРЦФ <> 0
				ИЛИ Форма.ПервоначальнаяСтоимостьПР <> 0
				ИЛИ Форма.НакопленнаяАмортизацияПР <> 0
				ИЛИ Форма.НакопленнаяАмортизацияПРЦФ <> 0 Тогда
		...
		Иначе
			...
			Элементы.ФормаПоказатьПР.Видимость = Ложь;
		...
		Если Форма.ОтражатьВРеглУчете Тогда
			...
			Иначе
				...
				ТекстСтоимостьЦФ   = НСтр("ru = 'Для бухгалтерского и налогового учета определяется как остаток на счете учета средств целевого финансирования.';
											|en = 'Determined as balance on ledger account of target funds for local accounting and tax accounting.'");
				...
				ТекстАмортизацияЦФ = НСтр("ru = 'Для бухгалтерского и налогового учета определяется как остаток на счете учета средств целевого финансирование в накопленной амортизации.';
											|en = 'Determined as balance on ledger account of target funds in accumulated depreciation for local accounting and tax accounting.'");
			КонецЕсли; 
...
Процедура ДополнитьВспомогательныеРеквизиты(Объект, ЭтоДанныеСтроки, ВспомогательныеРеквизиты) Экспорт
	...
	ВспомогательныеРеквизиты.Вставить(
		...
		НалоговыйУчет.ВариантУчетаОтложенногоНалога(ДатаУчетнойПолитики, Объект.Организация));
