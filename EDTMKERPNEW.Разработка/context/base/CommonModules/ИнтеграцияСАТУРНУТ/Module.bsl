...
Процедура ЗаполнитьТабличнуюЧастьСырьеПроизводственнойОперации(ДокументОбъект, ДанныеЗаполнения, ТекстТоварыДокумента, ДополнительныеПараметры = Неопределено)
	...
	//++ НЕ УТ
	УстановитьПараметрЗапросаТаблицаСырье(Запрос, ДанныеЗаполнения, ДетализацияДоНомераГруппыЗатрат);
	//-- НЕ УТ
...
Процедура ЗаполнитьПроизводственнуюОперациюСАТУРННаОснованииЭтапаПроизводства2_2(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС1
	|		ПО Товары1.Номенклатура = ОписаниеНоменклатурыИС1.Номенклатура
	|		И ЕСТЬNULL(ОписаниеНоменклатурыИС1.КлючВидовПродукции.ВидПродукцииСАТУРН, ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)) В (&ВидыПродукции)
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ПобочныеИзделия КАК Товары2
	|	ПО Товары2.Ссылка = &ДокументОснование
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС2
	|		ПО Товары2.Номенклатура = ОписаниеНоменклатурыИС2.Номенклатура
	|		И ЕСТЬNULL(ОписаниеНоменклатурыИС2.КлючВидовПродукции.ВидПродукцииСАТУРН, ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)) В (&ВидыПродукции)
	|ГДЕ
	|	ДокументСсылка.Ссылка = &ДокументОснование");
...
Процедура ЗаполнитьНакладнуюСАТУРННаОснованииПроизводстваБезЗаказа(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|		ПО Товары.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|		И ЕСТЬNULL(ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииСАТУРН, ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)) В (&ВидыПродукции)
	|ГДЕ
	|	ДокументСсылка.Ссылка = &ДокументОснование");
...
Функция ТекстЗапросаСырьеСНомеромГруппыЗатрат(ИмяВТ)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
	|		ПО ТаблицаСырье.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
	|		И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииСАТУРН В (&ВидыПродукции)
	|СГРУППИРОВАТЬ ПО
