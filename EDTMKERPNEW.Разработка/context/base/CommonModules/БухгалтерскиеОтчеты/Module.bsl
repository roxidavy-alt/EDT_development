...
Процедура ДобавитьСторноДляПоказателяКонтроль(ПараметрыОтчета, ОписаниеТекстаЗапроса) Экспорт
	...
	Если ТипЗнч(СторнирующийОператор.Источники[0].Источник) = Тип("ВложенныйЗапросСхемыЗапроса") Тогда
		Для Каждого Оператор Из СторнирующийОператор.Источники[0].Источник.Запрос.Операторы Цикл
			ВыражениеПараметр = Оператор.Источники[0].Источник.Параметры[1];
		...
		Если ЗапросПакета.ТаблицаДляПомещения = "ОстаткиНаНачало"
		 ...
		 Или ЗапросПакета.ТаблицаДляПомещения = "ХозрасчетныйОстаткиИОбороты" Тогда
			...
			Если ПозицияСимвола = 0 Тогда // добавляем секцию {ГДЕ
				...
				Если СтрНайти(ТекстЗапроса, "КорректировкиНБ") <> 0 Тогда
					
					ПозицияСимвола = СтрНайти(ТекстЗапроса, ") КАК " + ЗапросПакета.ТаблицаДляПомещения, НаправлениеПоиска.СКонца);
					
					ТаблицаДляПомещения = СтрЗаменить(ЗапросПакета.ТаблицаДляПомещения, "Хозрасчетный", "");
					...
					"#ТаблицаОстатков",
					"КорректировкиНБ" + ТаблицаДляПомещения)
					+ Сред(ТекстЗапроса, ПозицияСимвола);
					
					ПозицияСимвола = СтрНайти(ТекстЗапроса, "ОБЪЕДИНИТЬ ВСЕ", НаправлениеПоиска.СКонца);
			...
			Иначе // добавляем поля в секцию {ГДЕ
				...
				ПозицияСимвола = СтрНайти(ТекстЗапроса, "{ГДЕ", НаправлениеПоиска.СНачала);
				ТекстЗапроса = Лев(ТекстЗапроса, ПозицияСимвола + 4)
					+ СтрЗаменить("
					|	#ТаблицаОстатков.Субконто1.* КАК Субконто1,
					|	#ТаблицаОстатков.Субконто2.* КАК Субконто2,
					|	#ТаблицаОстатков.Субконто3.* КАК Субконто3,",
					"#ТаблицаОстатков",
					ЗапросПакета.ТаблицаДляПомещения)
					+ Сред(ТекстЗапроса, ПозицияСимвола + 4);
...
Функция НачалоПериодаСУчетомВидаПериода(НачалоПериода, ВидПериода) Экспорт
	...
	НачалоПериода = БухгалтерскиеОтчетыПереопределяемый.НачалоПериодаСУчетомВидаПериода(НачалоПериода, ВидПериода);
...
Функция КонецПериодаСУчетомВидаПериода(КонецПериода, ВидПериода) Экспорт
	...
	КонецПериода = БухгалтерскиеОтчетыПереопределяемый.КонецПериодаСУчетомВидаПериода(КонецПериода, ВидПериода);
...
Функция ЗаменитьНомераСубконтоВТексте(Знач ТекстЗапроса, СтрокаКартыСубконто, МаксНомерЗаполненногоСубконто)
	...
	Для НомерСубконто = 1 По БухгалтерскийУчет.МаксимальноеКоличествоСубконто() Цикл
		...
		Иначе
			...
			НомерСимвола = СтрНайти(ТекстЗапроса, "РегистрБухгалтерии.КорректировкиНалоговойБазы.");
			Если НЕ НомерСимвола = 0 Тогда
				
				ТаблицаВТексте = "КорректировкиНалоговойБазы" + Сред(ТекстЗапроса, НомерСимвола + 46,
					СтрНайти(ТекстЗапроса, "(", , НомерСимвола + 46) - НомерСимвола - 46);
