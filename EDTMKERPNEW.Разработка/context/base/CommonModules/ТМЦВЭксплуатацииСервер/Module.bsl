...
Процедура ПроверитьИнвентарныйУчет(Объект, НепроверяемыеРеквизиты, Отказ, ПараметрыПроверки = Неопределено) Экспорт
	...
	ШаблонСообщенияКоличество = НСтр("ru = 'По партии ведется инвентарный учет, количество в единицах хранения должно быть установлено равным 1 в строке %1 списка ""%2""';
									|en = 'Inventory accounting is kept for the lot, the quantity in storage units must be set equal to 1 in the line %1 of the ""%2"" list'");
...
Процедура ПроверитьПередачуВЭксплуатацию(Объект, НепроверяемыеРеквизиты, Отказ) Экспорт
	...
	|
	|ПОМЕСТИТЬ ДублиИнвентарныхНомеров
	|
	|ИЗ
	|	ТаблицаДокумента КАК ТаблицаДокумента
	|
	...
	|
	|		НЕ ДублиИнвентарныхНомеров.НомерСтроки ЕСТЬ NULL КАК ОшибкаДубль,
	|
	...
	|		ТаблицаДокумента КАК ТаблицаДокумента
	|
	|			ЛЕВОЕ СОЕДИНЕНИЕ ДублиИнвентарныхНомеров КАК ДублиИнвентарныхНомеров
	|			ПО ДублиИнвентарныхНомеров.НомерСтроки = ТаблицаДокумента.НомерСтроки
	|
