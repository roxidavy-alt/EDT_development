...
// Открыть форму выбора операции по классификатору.
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
//  ТекущийОбъект - СправочникОбъект.ТехнологическиеОперации
//  ЗакрыватьПриВыборе - Булево
Процедура ОткрытьФормуВыбораОперацииПоКлассификатору(Форма, ТекущийОбъект = Неопределено, ЗакрыватьПриВыборе = Истина) Экспорт
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("РежимВыбора",        Истина);
	ПараметрыФормы.Вставить("ЗакрыватьПриВыборе", ЗакрыватьПриВыборе);
	Если ТекущийОбъект <> Неопределено Тогда
		ПараметрыФормы.Вставить("ТекущаяСсылка",       ТекущийОбъект.КТО);
		ПараметрыФормы.Вставить("ТекущееНаименование", ТекущийОбъект.Наименование);
	КонецЕсли;
	
	ОткрытьФорму("Справочник.КлассификаторТехнологическихОпераций.ФормаВыбора",
		ПараметрыФормы,
		Форма,,,,,
...
Процедура ОперацияПоКлассификаторуАвтоПодбор(Текст, ДанныеВыбора, СтандартнаяОбработка, ПолеПоиска) Экспорт
	
	Если СтрДлина(Текст) < 2 Тогда
		Возврат;
	Иначе
		ДанныеВыбора = РедакторПроизводственногоПроцессаВызовСервера.ДанныеВыбораОперацииПоКлассификатору(ПолеПоиска, Текст);
...
Процедура НачатьЗаписьДанныхТехпроцесса(Форма, ДанныеДляЗавершенияРедактирования, Оповещение) Экспорт
	
	Если Форма.ДлительныеОперацииСтруктура.ЗаписьДанных <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Форма.ДлительныеОперацииСтруктура.ЗаписьДанных = РедакторПроизводственногоПроцессаВызовСервера.НачатьЗаписьДанныхТехпроцесса(
		ДанныеДляЗавершенияРедактирования, Форма.УникальныйИдентификатор);
	
	Если Форма.ДлительныеОперацииСтруктура.ЗаписьДанных <> Неопределено Тогда
		
		ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(Форма);
		ПараметрыОжидания.ВыводитьОкноОжидания = Истина;
		ПараметрыОжидания.ТекстСообщения = РедакторПроизводственногоПроцессаКлиентСервер.ФоноваяОперацияЗаписьДанных();
		
		ДлительныеОперацииКлиент.ОжидатьЗавершение(
			Форма.ДлительныеОперацииСтруктура.ЗаписьДанных,
			Оповещение,
			ПараметрыОжидания);
...
Функция ЗаписьДанныхТехпроцессаЗавершение(Форма, ДлительнаяОперация, ДополнительныеПараметры) Экспорт
	
	Форма.ДлительныеОперацииСтруктура.ЗаписьДанных = Неопределено;
	...
	Если ДлительнаяОперация <> Неопределено Тогда
		...
		ЗаполнитьЗначенияСвойств(Результат, РезультатЗадания);
		
		Если ДлительнаяОперация.Статус = "Ошибка" ИЛИ РезультатЗадания = Неопределено Тогда
			ТекстСообщения = НСтр("ru = 'Ошибка выполнения задания записи. Подробности см. в журнале регистрации.';
									|en = 'An error occurred when executing the writing job. For details, see the Event log.'");
			Картинка = БиблиотекаКартинок.Ошибка32;
		ИначеЕсли РезультатЗадания.ЕстьОшибкиЗаписиЭтапа Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось обновить этап по причине: %1';
											|en = 'Cannot refresh the stage. Reason: %1'"), РезультатЗадания.КраткоеПредставлениеОшибки);
			Картинка = БиблиотекаКартинок.Ошибка32;
		ИначеЕсли РезультатЗадания.ЕстьОшибкиЗаписиТехпроцесса Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось записать технологический процесс этапа по причине: %1';
											|en = 'Cannot save the technological process of the stage. Reason: %1'"), РезультатЗадания.КраткоеПредставлениеОшибки);
			Картинка = БиблиотекаКартинок.Ошибка32;
		Иначе
			ТекстСообщения = НСтр("ru = 'Технологический процесс этапа сохранен.';
									|en = 'Technological process of the stage is saved.'");
			Картинка = БиблиотекаКартинок.Информация32;
			ДополнительныеПараметры.Свойство("ЗакрытьПриЗавершении", Результат.ЗакрытьПриЗавершении);
		КонецЕсли;
		
		ПоказатьОповещениеПользователя(РедакторПроизводственногоПроцессаКлиентСервер.ФоноваяОперацияЗаписьДанных(),,ТекстСообщения, Картинка);
		
		Оповестить("ИзменениеТехпроцесса", ДополнительныеПараметры.Этап);
		Оповестить("Запись_ОчередьПроизводственныхОпераций", Новый Структура("Подразделение", ДополнительныеПараметры.Подразделение));
...
Процедура НачатьСозданиеТиповогоТехпроцесса(Форма, ДанныеДляСозданияТехпроцесса, Оповещение) Экспорт
	
	Если Форма.ДлительныеОперацииСтруктура.СозданиеТиповогоТехпроцесса <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Форма.ДлительныеОперацииСтруктура.СозданиеТиповогоТехпроцесса = РедакторПроизводственногоПроцессаВызовСервера.НачатьСозданиеТиповогоТехпроцесса(
		ДанныеДляСозданияТехпроцесса, Форма.УникальныйИдентификатор);
	
	Если Форма.ДлительныеОперацииСтруктура.СозданиеТиповогоТехпроцесса <> Неопределено Тогда
		
		ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(Форма);
		ПараметрыОжидания.ВыводитьОкноОжидания = Истина;
		ПараметрыОжидания.ТекстСообщения = РедакторПроизводственногоПроцессаКлиентСервер.ФоноваяОперацияСозданиеТехпроцесса();
		
		ДлительныеОперацииКлиент.ОжидатьЗавершение(
			Форма.ДлительныеОперацииСтруктура.СозданиеТиповогоТехпроцесса,
			Оповещение,
			ПараметрыОжидания);
...
Процедура СозданиеТиповогоТехпроцессаЗавершение(Форма, ДлительнаяОперация) Экспорт
	
	Форма.ДлительныеОперацииСтруктура.СозданиеТиповогоТехпроцесса = Неопределено;
	...
	Если ДлительнаяОперация <> Неопределено Тогда
		...
		Если ДлительнаяОперация.Статус = "Ошибка"
			...
			ТекстСообщения = НСтр("ru = 'Не удалось создать новый технологический процесс. Подробности см. в журнале регистрации.';
									|en = 'Cannot create a new technological process. For more information, see the Event log.'");
			Картинка = БиблиотекаКартинок.Ошибка32;
		Иначе
			ТекстСообщения = НСтр("ru = 'Технологический процесс создан.';
									|en = 'Technological process is created.'");
			Картинка = БиблиотекаКартинок.Информация32;
			ПоказатьЗначение(, РезультатЗадания.ТехнологическийПроцесс);
		КонецЕсли;
		
		ПоказатьОповещениеПользователя(РедакторПроизводственногоПроцессаКлиентСервер.ФоноваяОперацияСозданиеТехпроцесса(),,ТекстСообщения, Картинка);
