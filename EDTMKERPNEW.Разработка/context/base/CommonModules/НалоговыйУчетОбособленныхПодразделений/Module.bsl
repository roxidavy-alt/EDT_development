...
Процедура ПодготовитьРасчетРаспределенияНалоговойБазы(РасчетРаспределенияНалоговойБазы, КонтекстРасчета, ПараметрТаблицаНалоговаяБаза, МенеджерВременныхТаблиц) Экспорт
	...
	Если МенеджерВременныхТаблиц.Таблицы.Найти("СтатьиОплатыТруда") = Неопределено Тогда
		СоздатьВТ_СтатьиОплатыТруда(МенеджерВременныхТаблиц);
	КонецЕсли;

	Если КонтекстРасчета.ЭтоПерерасчет Тогда
		ДатаЗакрытияНабора = КонтекстРасчета.ДатаЗакрытияНабора;
	Иначе
		ДатаЗакрытияНабора = КонтекстРасчета.КонецПериода;
	КонецЕсли;

	Если МенеджерВременныхТаблиц.Таблицы.Найти("ВТНаборыКорректировок") = Неопределено Тогда
		РасчетНалогаНаПрибыль.СоздатьВТ_НаборыКорректировок(МенеджерВременныхТаблиц, ДатаЗакрытияНабора, КонтекстРасчета.Организации);
	КонецЕсли;

	СоздатьВТ_ДанныеРасходовПоОплатеТруда(МенеджерВременныхТаблиц, КонтекстРасчета, ЕстьРасчетДолейЗаПрошлыйМесяц);
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВТ_ДанныеРасходовПоОплатеТруда.Организация КАК Организация,
	|		ВТ_ДанныеРасходовПоОплатеТруда.Подразделение КАК Подразделение,
	|		ВТ_ДанныеРасходовПоОплатеТруда.Сумма КАК Сумма
	|	ИЗ
	|		ВТ_ДанныеРасходовПоОплатеТруда КАК ВТ_ДанныеРасходовПоОплатеТруда) КАК РасходыПоОплатеТруда
	|
	|СГРУППИРОВАТЬ ПО
	...
	Если МенеджерВременныхТаблиц.Таблицы.Найти("АрендованноеИмущество") = Неопределено Тогда

		ЗапросИмущество = Новый Запрос;
		ЗапросИмущество.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
		
		ЗапросИмущество.Текст = 
		"ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("НеПерерасчетНалога",            НЕ КонтекстРасчета.ЭтоПерерасчет);
	...
	ТекстЗапросаПерерасчет = 
	"ВЫБРАТЬ
	|	КорректировкиНалоговойБазыОстатки.Организация КАК Организация,
	|	КорректировкиНалоговойБазыОстатки.Подразделение КАК Подразделение,
	|	0 КАК СтоимостьОСПрошлыхМесяцев,
	|	ЕСТЬNULL(КорректировкиНалоговойБазыОстатки.СуммаНУОстаток, 0) КАК СтоимостьОССледующегоМесяца
	|
	|ИЗ
	...
	|		ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.ОсновныеСредства),
	|		Организация В (&СписокОрганизаций)
	|		И (&НеПерерасчетНалога ИЛИ НаборКорректировок В (ВЫБРАТЬ Т.НаборКорректировок ИЗ ВТНаборыКорректировок КАК Т))
	|) КАК КорректировкиНалоговойБазыОстатки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	КорректировкиНалоговойБазыОстатки.Организация,
	|	КорректировкиНалоговойБазыОстатки.Подразделение,
	|	0,
	|	-ЕСТЬNULL(КорректировкиНалоговойБазыОстатки.СуммаНУОстаток, 0)
	|ИЗ
	|	РегистрБухгалтерии.КорректировкиНалоговойБазы.Остатки(
	|			&НачалоСледующегоМесяца,
	|			Счет В (&СчетаАмортизируемогоИмущества),
	...
	|					ВТ_ИсключаемыеОбъекты.Объект КАК Объект
	|					ИЗ
	|					ВТ_ИсключаемыеОбъекты КАК ВТ_ИсключаемыеОбъекты)) КАК КорректировкиНалоговойБазыОстатки"; 
	ТекстыЗапроса.Добавить(ТекстЗапросаПерерасчет);
	
	ТекстЗапроса = СтрСоединить(ТекстыЗапроса, ОбщегоНазначения.ТекстОбъединитьВсе());
	...
	Запрос.УстановитьПараметр("ДатаЗакрытияНабора",                    ДатаЗакрытияНабора);
	...
	|	МАКСИМУМ(ЕСТЬNULL(НаборыКорректировок.ДатаНачалаПодготовкиДекларации, ДАТАВРЕМЯ(1, 1, 1))) КАК ДатаНачалаПодготовкиДекларации
	|ПОМЕСТИТЬ ПромежуточнаяТаблица
	|ИЗ
	|	РегистрСведений.РасчетДолейБазыНалогаНаПрибыль КАК РасчетДолейБазыНалогаНаПрибыль
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КорректировкиПрошлыхПериодовНУ КАК НаборыКорректировок
	|		ПО РасчетДолейБазыНалогаНаПрибыль.НаборКорректировок = НаборыКорректировок.Ссылка
	|			И (НаборыКорректировок.СборКорректировокЗакрыт)
	|			И (НаборыКорректировок.ДатаНачалаПодготовкиДекларации <= &ДатаЗакрытияНабора)
	|ГДЕ
	...
	|	И (НЕ НаборыКорректировок.Ссылка ЕСТЬ NULL
	|			ИЛИ РасчетДолейБазыНалогаНаПрибыль.НаборКорректировок = ЗНАЧЕНИЕ(Справочник.КорректировкиПрошлыхПериодовНУ.ПУстаяССылка))
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ДанныеНалоговыхОрганов.НалоговыйОрганТекущий, РасчетДолейБазыНалогаНаПрибыль.РегистрацияВНалоговомОргане) КАК РегистрацияВНалоговомОргане,
	|	РасчетДолейБазыНалогаНаПрибыль.РасходыПоОплатеТруда КАК РасходыПоОплатеТруда,
	|	РасчетДолейБазыНалогаНаПрибыль.СтоимостьОСПрошлыхМесяцев КАК СтоимостьОСПрошлыхМесяцев,
	|	0 КАК СтоимостьОССледующегоМесяца
	|ПОМЕСТИТЬ ДанныеДляРасчетаПредварительная
	|ИЗ
	|	РегистрСведений.РасчетДолейБазыНалогаНаПрибыль КАК РасчетДолейБазыНалогаНаПрибыль
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПромежуточнаяТаблица КАК ПромежуточнаяТаблица
	|		ПО РасчетДолейБазыНалогаНаПрибыль.РегистрацияВНалоговомОргане = ПромежуточнаяТаблица.РегистрацияВНалоговомОргане
	|			И РасчетДолейБазыНалогаНаПрибыль.Организация = ПромежуточнаяТаблица.Организация
	|		ЛЕВОЕ СОЕДИНЕНИЕ СоответствиеПрежнихИТекущихНалоговыхОрганов КАК ДанныеНалоговыхОрганов
	|		ПО РасчетДолейБазыНалогаНаПрибыль.РегистрацияВНалоговомОргане = ДанныеНалоговыхОрганов.НалоговыйОрганПрежний
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КорректировкиПрошлыхПериодовНУ КАК КорректировкиПрошлыхПериодовНУ
	|		ПО РасчетДолейБазыНалогаНаПрибыль.НаборКорректировок = КорректировкиПрошлыхПериодовНУ.Ссылка
	|ГДЕ
	|	РасчетДолейБазыНалогаНаПрибыль.ПериодРасчета МЕЖДУ &НачалоПрошлогоМесяца И &КонецПрошлогоМесяца
	|	И РасчетДолейБазыНалогаНаПрибыль.ПериодРасчета >= &НачалоНалоговогоПериода 
	|	И ЕСТЬNULL(КорректировкиПрошлыхПериодовНУ.ДатаНачалаПодготовкиДекларации, ДАТАВРЕМЯ(1, 1, 1))
	|		= ПромежуточнаяТаблица.ДатаНачалаПодготовкиДекларации
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ДанныеНалоговыхОрганов.НалоговыйОрганТекущий, РасчетДолейБазыНалогаНаПрибыль.РегистрацияВНалоговомОргане),
	|	0,
	...
	|		ПО РасчетДолейБазыНалогаНаПрибыль.НаборКорректировок = КорректировкиПрошлыхПериодовНУ.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПромежуточнаяТаблица КАК ПромежуточнаяТаблица
	|		ПО РасчетДолейБазыНалогаНаПрибыль.Организация = ПромежуточнаяТаблица.Организация
	|			И РасчетДолейБазыНалогаНаПрибыль.РегистрацияВНалоговомОргане = ПромежуточнаяТаблица.РегистрацияВНалоговомОргане
	|ГДЕ
	|	РасчетДолейБазыНалогаНаПрибыль.ПериодРасчета МЕЖДУ &НачалоПрошлогоМесяца И &КонецПрошлогоМесяца
	|	И ЕСТЬNULL(КорректировкиПрошлыхПериодовНУ.ДатаНачалаПодготовкиДекларации, ДАТАВРЕМЯ(1, 1, 1))
	|		= ПромежуточнаяТаблица.ДатаНачалаПодготовкиДекларации
	|;
	|
	|УНИЧТОЖИТЬ ПромежуточнаяТаблица
	|";
	...
	|	
	|	ВЫБРАТЬ
	|		ДанныеДляРасчетаПредварительная.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
	|		ДанныеДляРасчетаПредварительная.РасходыПоОплатеТруда КАК РасходыПоОплатеТруда,
	|		ДанныеДляРасчетаПредварительная.СтоимостьОСПрошлыхМесяцев КАК СтоимостьОСПрошлыхМесяцев,
	|		ДанныеДляРасчетаПредварительная.СтоимостьОССледующегоМесяца КАК СтоимостьОССледующегоМесяца
	|	ИЗ
	|		ДанныеДляРасчетаПредварительная КАК ДанныеДляРасчетаПредварительная
	|	) КАК ДанныеДляРасчета
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(СоответствиеПрежнихИТекущихНалоговыхОрганов.НалоговыйОрганТекущий, ТаблицаПоказателей.РегистрацияВНалоговомОргане) КАК РегистрацияВНалоговомОргане,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ СоответствиеПрежнихИТекущихНалоговыхОрганов КАК СоответствиеПрежнихИТекущихНалоговыхОрганов
	|		ПО ТаблицаПоказателей.РегистрацияВНалоговомОргане = СоответствиеПрежнихИТекущихНалоговыхОрганов.НалоговыйОрганПрежний
	|ГДЕ
	...
	|СГРУППИРОВАТЬ ПО
	|	ЕСТЬNULL(СоответствиеПрежнихИТекущихНалоговыхОрганов.НалоговыйОрганТекущий, ТаблицаПоказателей.РегистрацияВНалоговомОргане)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	РегистрацияВНалоговомОргане
	|;
	...
	|ПОМЕСТИТЬ КлючиДанныхЗакрытыхПодразделенийПредварительная
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КлючиДанных.ПоследнийОтчетныйПериод КАК ПоследнийОтчетныйПериод,
	|	КлючиДанных.ПериодРасчета КАК ПериодРасчета,
	|	КлючиДанных.Организация КАК Организация,
	|	КлючиДанных.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
	|	МАКСИМУМ(ЕСТЬNULL(НаборыКорректировок.ДатаНачалаПодготовкиДекларации, ДАТАВРЕМЯ(1, 1, 1))) КАК ДатаНачалаПодготовкиДекларации
	|ПОМЕСТИТЬ КлючиДанныхЗакрытыхПодразделений
	|ИЗ
	|	РегистрСведений.РасчетДолейБазыНалогаНаПрибыль КАК РасчетПоследнегоОтчетногоПериода
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ КлючиДанныхЗакрытыхПодразделенийПредварительная КАК КлючиДанных
	|		ПО РасчетПоследнегоОтчетногоПериода.РегистрацияВНалоговомОргане = КлючиДанных.РегистрацияВНалоговомОргане
	|			И РасчетПоследнегоОтчетногоПериода.Организация = КлючиДанных.Организация
	|			И РасчетПоследнегоОтчетногоПериода.ПериодРасчета = КлючиДанных.ПериодРасчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КорректировкиПрошлыхПериодовНУ КАК НаборыКорректировок
	|		ПО РасчетПоследнегоОтчетногоПериода.НаборКорректировок = НаборыКорректировок.Ссылка
	|			И (НаборыКорректировок.СборКорректировокЗакрыт)
	|			И (НаборыКорректировок.ДатаНачалаПодготовкиДекларации <= &ДатаЗакрытияНабора)
	|ГДЕ
	|	НЕ НаборыКорректировок.Ссылка ЕСТЬ NULL
	|	ИЛИ РасчетПоследнегоОтчетногоПериода.НаборКорректировок = ЗНАЧЕНИЕ(Справочник.КорректировкиПрошлыхПериодовНУ.ПустаяССылка)
	|
	|СГРУППИРОВАТЬ ПО
	|	КлючиДанных.ПоследнийОтчетныйПериод,
	|	КлючиДанных.Организация,
	|	КлючиДанных.РегистрацияВНалоговомОргане,
	|	КлючиДанных.ПериодРасчета
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ПериодРасчета,
	|	Организация,
	...
	|	РасчетПоследнегоОтчетногоПериода.НаборКорректировок КАК НаборКорректировок,
	...
	|		ПО РасчетПоследнегоОтчетногоПериода.НаборКорректировок = НаборыКорректировок.Ссылка
	|ГДЕ
	|	КлючиДанныхЗакрытыхПодразделений.ДатаНачалаПодготовкиДекларации
	|			= ЕСТЬNULL(НаборыКорректировок.ДатаНачалаПодготовкиДекларации, ДАТАВРЕМЯ(1, 1, 1))
	|ИНДЕКСИРОВАТЬ ПО
	|	РегистрацияВНалоговомОргане";
	...
	|				И РасчетПоследнегоОтчетногоПериода.НаборКорректировок = РасчетПоВариантамНалогообложения.НаборКорректировок
	...
	Пока ВыборкаОбщийРасчет.Следующий() Цикл
		...
		НоваяСтрокаЗакрытыхПодразделений.НаборКорректировок = КонтекстРасчета.НаборКорректировок;
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	&НаборКорректировок КАК НаборКорректировок,
	|	ПоказателиПоНалоговымОрганам.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
	|	ПоказателиПоНалоговымОрганам.РасходыПоОплатеТруда КАК РасходыПоОплатеТруда,
	|	ПоказателиПоНалоговымОрганам.СтоимостьОСПрошлыхМесяцев КАК СтоимостьОСПрошлыхМесяцев,
	|	ПоказателиПоНалоговымОрганам.СтоимостьОССледующегоМесяца КАК СтоимостьОССледующегоМесяца,
	...
	Запрос.УстановитьПараметр("НаборКорректировок", КонтекстРасчета.НаборКорректировок);
...
Функция РазвернутьРасчетРаспределенияНалоговойБазыПоВариантамНалогообложения(ИсходныйРасчет) Экспорт
	...
	Для каждого СтрокаИсходныйРасчет Из ИсходныйРасчет Цикл
		...
		Для каждого КлючИЗначение Из СтрокаИсходныйРасчет.НалоговаяБазаПоВариантамНалогообложения Цикл
			...
			СтрокаРазвернутыйРасчет.НаборКорректировок            = СтрокаИсходныйРасчет.НаборКорректировок;
	...
	Если НалоговаяБаза = Неопределено Тогда
		...
		Иначе
			...
			СуммыНалоговойБазы = РасчетНалогаНаПрибыль.СуммыНалоговойБазыВРазрезеВариантовНалогообложения(
				СтруктураПараметров, ИспользуетсяРаздельныйУчет);
...
Процедура ПолучитьРаспределениеНалогаДляЗаполненияДекларации(Расчет, КонтекстРасчета, ВариантыНО) Экспорт
	...
	|	И Распределение.НаборКорректировок = &НаборКорректировок
	...
	ПараметрыПолученияНабора = НалогНаПрибыльКорректировки.ПараметрыПоследнегоНабораРасчетаПоДатеПодписи();
	ПараметрыПолученияНабора.Организация = КонтекстРасчета.Организация;
	ПараметрыПолученияНабора.ДатаНачалаПодготовкиДекларации = КонецДня(КонтекстРасчета.ДатаПодписи);
	ПараметрыПолученияНабора.МесяцРасчета = КонтекстРасчета.КонецПериода;
	ПараметрыПолученияНабора.ПолноеИмяРегистраРасчета = Метаданные.РегистрыСведений.РасчетДолейБазыНалогаНаПрибыль.ПолноеИмя();
	
	НаборКорректировок = НалогНаПрибыльКорректировки.ПоследнийНаборРасчетаПоДатеПодписиДекларации(ПараметрыПолученияНабора);
...
Функция ВыполнитьЗапросРасчетНалогаНаПрибыль(Организация, КонецПериода, ВариантНО = Неопределено) Экспорт
	...
	ПараметрыЗапросаРасчетНалогаНаПрибыль = ПараметрыЗапросаРасчетНалогаНаПрибыль(Организация, КонецПериода);
	
	МенеджерВТ = Новый МенеджерВременныхТаблиц;
	
	СоздатьВТ_НалогГод(МенеджерВТ, ПараметрыЗапросаРасчетНалогаНаПрибыль);

	ДанныеСтавокНалога = РасчетНалогаНаПрибыль.НовыйРаспределениеНалогаНаПрибыль();
	ЗаполнитьПараметрыПолученияСтавокНалога(МенеджерВТ, ДанныеСтавокНалога); 
	...
	УстановитьПараметрыЗапроса(Запрос, ПараметрыЗапросаРасчетНалогаНаПрибыль); 
	Запрос.Текст = ТекстЗапроса_РасчетНалогаНаПрибыль();
	Запрос.УстановитьПараметр("ДанныеСтавокНалога", ДанныеСтавокНалога);
...
Функция НовыйРасчетРаспределенияНалоговойБазы()
	...
	ЗаписьРаспределенияБазыНалогаНаПрибыль.Колонки.Добавить("НаборКорректировок",
		Новый ОписаниеТипов("СправочникСсылка.КорректировкиПрошлыхПериодовНУ"));
...
Функция НовыйРасчетРаспределенияНалоговойБазыПоВариантамНалогообложения()
	...
	ТаблицаРасчета.Колонки.Добавить("РегистрацияВНалоговомОргане",
		Новый ОписаниеТипов("СправочникСсылка.РегистрацииВНалоговомОргане")); 
	ТаблицаРасчета.Колонки.Добавить("НаборКорректировок",
		Новый ОписаниеТипов("СправочникСсылка.КорректировкиПрошлыхПериодовНУ"));
...
Функция ПараметрыЗапросаРасчетНалогаНаПрибыль(Знач Организация, Знач КонецПериода)
	...
	ПараметрыЗапросаРасчетНалогаНаПрибыль = Новый Структура;
	ПараметрыЗапросаРасчетНалогаНаПрибыль.Вставить("СписокОрганизаций", СписокДоступныхОрганизаций(Организация));
	ПараметрыЗапросаРасчетНалогаНаПрибыль.Вставить("Субконто68_04_1",          Субконто68_04_1);
	ПараметрыЗапросаРасчетНалогаНаПрибыль.Вставить("НачалоПериода",            НачалоПериода);
	ПараметрыЗапросаРасчетНалогаНаПрибыль.Вставить("КонецПериода",             КонецПериода);
	ПараметрыЗапросаРасчетНалогаНаПрибыль.Вставить("УровниБюджетов",           УровниБюджетов);
	ПараметрыЗапросаРасчетНалогаНаПрибыль.Вставить("СчетаРасчетыСБюджетом",    БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.РасчетыСБюджетом));
	ПараметрыЗапросаРасчетНалогаНаПрибыль.Вставить("ПрименяютсяРазныеСтавкиНалогаНаПрибыль",    НалоговыйУчет.ПрименяютсяРазныеСтавкиНалогаНаПрибыль());
	
    Возврат ПараметрыЗапросаРасчетНалогаНаПрибыль;
	
КонецФункции 

Процедура СоздатьВТ_НалогГод(МенеджерВТ, ПараметрыЗапросаРасчетНалогаНаПрибыль)
	...
	|	ЕСТЬNULL(НалогГод.СубконтоДт1, ЗНАЧЕНИЕ(Справочник.ВариантыНалогообложенияПрибыли.ПустаяСсылка)) КАК ВариантНалогообложенияПрибыли
	|ПОМЕСТИТЬ НалогГод
	|ИЗ
...
Процедура УстановитьПараметрыЗапроса(Запрос, ПараметрыЗапросаРасчетНалогаНаПрибыль)

	Для каждого Параметр Из ПараметрыЗапросаРасчетНалогаНаПрибыль Цикл
		Запрос.УстановитьПараметр(Параметр.Ключ, Параметр.Значение);
	КонецЦикла;	

КонецПроцедуры

Процедура ЗаполнитьПараметрыПолученияСтавокНалога(МенеджерВТ, ПараметрыПолученияСтавокНалога)

	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВТ; 
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НалогГод.ВариантНалогообложенияПрибыли КАК ВариантНалогообложенияПрибыли,
	|	НалогГод.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане
	|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		НоваяСтрока = ПараметрыПолученияСтавокНалога.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
	КонецЦикла;

КонецПроцедуры

Функция ТекстЗапроса_РасчетНалогаНаПрибыль()

	Текст = 
	"ВЫБРАТЬ
	|	НалогМесяц.СуммаОборот КАК СуммаНалогМесяц,
	|	НалогМесяц.Организация КАК Организация,
	...
	|	ЕСТЬNULL(НалогМесяц.СубконтоДт1, ЗНАЧЕНИЕ(Справочник.ВариантыНалогообложенияПрибыли.ПустаяСсылка)) КАК ВариантНалогообложенияПрибыли
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДанныеСтавокНалога.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
	|	ДанныеСтавокНалога.ВариантНалогообложенияПрибыли КАК ВариантНалогообложенияПрибыли,
	|	ДанныеСтавокНалога.СтавкаРегиональныйБюджет КАК СтавкаРегиональныйБюджет,
	|	ДанныеСтавокНалога.СтавкаФедеральныйБюджет КАК СтавкаФедеральныйБюджет
	|ПОМЕСТИТЬ ДанныеСтавокНалога
	|ИЗ
	|	&ДанныеСтавокНалога КАК ДанныеСтавокНалога
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	РегистрацияВНалоговомОргане,
	|	ВариантНалогообложенияПрибыли
	...
	|	ЕСТЬNULL(НалогГод.ВариантНалогообложенияПрибыли, ЗНАЧЕНИЕ(Справочник.ВариантыНалогообложенияПрибыли.ПустаяСсылка)) КАК ВариантНалогообложения,
	|	ВЫБОР
	...
	|	ВЫБОР
	|		КОГДА НалогГод.Бюджет = ЗНАЧЕНИЕ(Перечисление.УровниБюджетов.ФедеральныйБюджет)
	|			ТОГДА ЕСТЬNULL(ДанныеСтавокНалога.СтавкаФедеральныйБюджет, 0)
	|		ИНАЧЕ ЕСТЬNULL(ДанныеСтавокНалога.СтавкаРегиональныйБюджет, 0)
	|	КОНЕЦ * 100 КАК СтавкаСубъектРФ,
	|	НалогГод.СуммаНалогГод КАК СуммаНалогГод,
	|	НалогМесяц.СуммаНалогМесяц КАК СуммаНалогМесяц,
	|	НалогГод.СуммаНалогГод - ЕСТЬNULL(НалогМесяц.СуммаНалогМесяц, 0) КАК СуммаНалогОстальныеМесяцы,
	|	НалогГод.Бюджет КАК Бюджет,
	|	НалогГод.Организация КАК Организация,
	|	НалогГод.Организация.ГоловнаяОрганизация КАК ОрганизацияГоловнаяОрганизация
	|ИЗ
	|	НалогГод КАК НалогГод
	...
	|			И НалогГод.ВариантНалогообложенияПрибыли = НалогМесяц.ВариантНалогообложенияПрибыли
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеСтавокНалога КАК ДанныеСтавокНалога
	|		ПО НалогГод.РегистрацияВНалоговомОргане = ДанныеСтавокНалога.РегистрацияВНалоговомОргане
	|			И НалогГод.ВариантНалогообложенияПрибыли = ДанныеСтавокНалога.ВариантНалогообложенияПрибыли";
	
	Возврат Текст;
	
КонецФункции

Процедура СоздатьВТ_СтатьиОплатыТруда(МенеджерВременныхТаблиц)
...
Процедура СоздатьВТ_ДанныеРасходовПоОплатеТруда(МенеджерВременныхТаблиц, КонтекстРасчета, ЕстьРасчетДолейЗаПрошлыйМесяц)
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	
	НачалоПериодаРасходовПоОплатеТруда = ?(ЕстьРасчетДолейЗаПрошлыйМесяц, КонтекстРасчета.НачалоМесяца, КонтекстРасчета.НачалоНалоговогоПериода);
	КонецПериодаРасходовПоОплатеТруда = КонтекстРасчета.КонецПериода;
	
	ВидыДетализацииУчета = РеглУчетСервер.ВидыДетализацииУчета();
	ПериодыДетализацииУчета = РеглУчетСервер.ПериодыДетализацииУчета(НачалоПериодаРасходовПоОплатеТруда, 
		КонецПериодаРасходовПоОплатеТруда, ВидыДетализацииУчета.РеглУчетНЗППоПартиямПроизводства);

	Запрос.УстановитьПараметр("НачалоПериодаБезДетализации",        ПериодыДетализацииУчета.НачалоПериодаБезДетализации);
	Запрос.УстановитьПараметр("КонецПериодаБезДетализации",         ПериодыДетализацииУчета.КонецПериодаБезДетализации);
	Запрос.УстановитьПараметр("НачалоПериодаДанныеСДетализацией",   ПериодыДетализацииУчета.НачалоПериодаДанныеСДетализацией);
	Запрос.УстановитьПараметр("КонецПериодаДанныеСДетализацией",    ПериодыДетализацииУчета.КонецПериодаДанныеСДетализацией);
	Запрос.УстановитьПараметр("СписокОрганизаций",                  КонтекстРасчета.Организации);
	Запрос.УстановитьПараметр("ДатаЗакрытияНабора",                 КонтекстРасчета.ДатаЗакрытияНабора);
	Запрос.УстановитьПараметр("СчетаРасходовБезДетализации",        БухгалтерскийУчет.СформироватьМассивСубсчетов(НалоговыйУчет.ПолучитьМассивСчетовУчетаРасходов()));
	Запрос.УстановитьПараметр("СчетаРасходовСДетализацией",         БухгалтерскийУчет.СформироватьМассивСубсчетов(НалоговыйУчет.СчетаУчетаРасходовСДетализацией()));
	Запрос.УстановитьПараметр("НеЭтоПерерасчет",                    Не КонтекстРасчета.ЭтоПерерасчет);
						
	Запрос.Текст = ТекстЗапроса_СоздатьВТ_ДанныеРасходовПоОплатеТруда(ПериодыДетализацииУчета.ДанныеСДетализацией, 
		ПериодыДетализацииУчета.ДанныеБезДетализации);
	
	Запрос.Выполнить();
		
КонецПроцедуры

Функция ТекстЗапроса_СоздатьВТ_ДанныеРасходовПоОплатеТруда(Знач ДанныеСДетализацией, Знач ДанныеБезДетализации)
	...
	|	0 КАК Сумма
	|ПОМЕСТИТЬ ВТ_ДанныеРасходовПоОплатеТруда
	|ГДЕ
	|	ЛОЖЬ";
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериодаБезДетализации, &КонецПериодаБезДетализации,, Счет В (&СчетаРасходовБезДетализации),
	|		ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат), Организация В (&СписокОрганизаций)
	|	И Субконто1 В
	|		(ВЫБРАТЬ
	|			СтатьиОплатыТруда.Статья
	|		ИЗ
	|			СтатьиОплатыТруда),
	|	НЕ КорСчет В (&СчетаРасходовБезДетализации),) КАК ХозрасчетныйОбороты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериодаБезДетализации, &КонецПериодаБезДетализации,, Счет В (&СчетаРасходовБезДетализации),
	|		ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.ТипыЗатрат), Организация В (&СписокОрганизаций)
	|	И Субконто1 = ЗНАЧЕНИЕ(Перечисление.ТипыЗатратРегл.СдельнаяОплатаТруда),
	|	НЕ КорСчет В (&СчетаРасходовБезДетализации)
	|	И НЕ КорСчет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыПоСоциальномуСтрахованию)),) КАК ХозрасчетныйОбороты";
	...
	|	ЕСТЬNULL(ХозрасчетныйОбороты.СуммаНУОборотДт, 0) КАК Сумма
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериодаДанныеСДетализацией, &КонецПериодаДанныеСДетализацией,, Счет В (&СчетаРасходовСДетализацией),
	|		ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат), Организация В (&СписокОрганизаций)
	|	И Субконто1 В
	|		(ВЫБРАТЬ
	|			СтатьиОплатыТруда.Статья
	...
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериодаДанныеСДетализацией, &КонецПериодаДанныеСДетализацией,, Счет В (&СчетаРасходовСДетализацией),
	|		ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат), Организация В (&СписокОрганизаций)
	|	И ТИПЗНАЧЕНИЯ(Субконто1) = ТИП(Справочник.ВидыРаботСотрудников),
	|	НЕ КорСчет В (&СчетаРасходовБезДетализации)
	|	И НЕ КорСчет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыПоСоциальномуСтрахованию)),) КАК ХозрасчетныйОбороты";

	ТекстыЗапроса = Новый Массив;
	
	ТекстыЗапроса.Добавить(ОбщийТекст);
	
	Если ДанныеБезДетализации Тогда
		ТекстыЗапроса.Добавить(ТекстБезДетализации);
	КонецЕсли;

	Если ДанныеСДетализацией Тогда
		ТекстыЗапроса.Добавить(ТекстСДетализацией);
	КонецЕсли;
	
	ТекстБезДетализацииКорректировки = 	
	"ВЫБРАТЬ
	|	КорректировкиНалоговойБазыОбороты.Организация КАК Организация,
	|	КорректировкиНалоговойБазыОбороты.Подразделение КАК Подразделение,
	|	ЕСТЬNULL(КорректировкиНалоговойБазыОбороты.СуммаНУОборотДт, 0) КАК Сумма
	|ИЗ
	|	РегистрБухгалтерии.КорректировкиНалоговойБазы.Обороты(&НачалоПериодаБезДетализации, &КонецПериодаБезДетализации,, Счет В (&СчетаРасходовБезДетализации),
	|		ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат), Организация В (&СписокОрганизаций)
	...
	|		ИЗ
	|			СтатьиОплатыТруда),
	|	НЕ КорСчет В (&СчетаРасходовБезДетализации),) КАК КорректировкиНалоговойБазыОбороты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И Субконто1 = ЗНАЧЕНИЕ(Перечисление.ТипыЗатратРегл.СдельнаяОплатаТруда),
	|	НЕ КорСчет В (&СчетаРасходовБезДетализации)
	|	И НЕ КорСчет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыПоСоциальномуСтрахованию)),) КАК КорректировкиНалоговойБазыОбороты";

	ТекстСДетализациейКорректировки = 	
	"ВЫБРАТЬ
	|	КорректировкиНалоговойБазыОбороты.Организация КАК Организация,
	|	КорректировкиНалоговойБазыОбороты.Подразделение КАК Подразделение,
	|	ЕСТЬNULL(КорректировкиНалоговойБазыОбороты.СуммаНУОборотДт, 0) КАК Сумма
	|ИЗ
	|	РегистрБухгалтерии.КорректировкиНалоговойБазы.Обороты(&НачалоПериодаДанныеСДетализацией, &КонецПериодаДанныеСДетализацией,, Счет В (&СчетаРасходовСДетализацией),
	|		ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат), Организация В (&СписокОрганизаций)
	|	И (&НеЭтоПерерасчет ИЛИ НаборКорректировок В (ВЫБРАТЬ Т.НаборКорректировок ИЗ ВТНаборыКорректировок КАК Т))
	|	И Субконто1 В
	|		(ВЫБРАТЬ
	...
	Если ДанныеБезДетализации Тогда
		ТекстыЗапроса.Добавить(ТекстБезДетализацииКорректировки);
	КонецЕсли;
		
	Если ДанныеСДетализацией Тогда
		ТекстыЗапроса.Добавить(ТекстСДетализациейКорректировки);
	КонецЕсли;		
