...
Функция НовыйНастройкиОтчета(ПостроительОписанияОтчетов, СоставШагаРасчета, Процессор)
	...
	Если ПостроительОписанияОтчетов.ВидОтчета = "ОборотыСчета" И ПостроительОписанияОтчетов.Субконто.Свойство("НоменклатурнаяГруппа") Тогда
		ВидСубконтоСчета = СчетОтчета.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПартииРеализованнойНоменклатуры);
		Если Не ВидСубконтоСчета = Неопределено Тогда
			ОписаниеСубконто = Новый Структура;
			ОписаниеСубконто.Вставить("Имя", "Субконто" + XMLСтрока(ВидСубконтоСчета.НомерСтроки) + ".Номенклатура.ГруппаФинансовогоУчета");
			ОписаниеСубконто.Вставить("Номер", ВидСубконтоСчета.НомерСтроки);
			ОписаниеСубконто.Вставить("Представление", "Номенклатурная группа");

			ОписаниеВсехСубконтоСчета.Вставить(ПостроительОписанияОтчетов.Субконто.НоменклатурнаяГруппа, ОписаниеСубконто);
...
Процедура ДобавитьПоясняющийЗаголовокРасшифровкиПоказателей(РасчетГруппы)
	...
	ЗаписьРасшифровки.Описание  = НСтр("ru = 'Ниже приведен подробный расчет каждого слагаемого';
										|en = 'Below is a detailed calculation of each addend'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура СократитьЗначенияЗаполненияУпрощенногоОтчета(Процессор)
	...
	Для Каждого ПланОбобщеннойГруппыПоказателей Из ПланСокращения Цикл
		...
		Если РасшифровкаСоставаОбобщеннойГруппыПоказателей.Количество() > 1 Тогда
			...
			ЗаписьРасшифровки.Описание  = СтрШаблон(
				НСтр("ru = '%1 - за %2';
					|en = '%1 - for %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
			...
			ЗаписьРасшифровки.Описание  = НСтр(
				...
				|en = 'This is a summary group of indicators. It is calculated by summing up the indicators (groups of indicators) described below.
				|Among such indicators, you can define an indicator group that has the greatest share in this summary group of indicators.'",
			...
			ЗаписьРасшифровки.Описание = НСтр("ru = 'Итого по обобщенной группе показателей:';
												|en = 'Total by summary indicator group:'", ОбщегоНазначения.КодОсновногоЯзыка());
