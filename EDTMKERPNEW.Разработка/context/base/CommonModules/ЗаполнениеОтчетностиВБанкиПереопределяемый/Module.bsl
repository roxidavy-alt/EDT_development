...
Функция ОСВ_ПоЛизингу(ПараметрыОСВ)
	...
	МассивТекстовЗапроса = Новый Массив;
	МассивВидовСчетов = ОбщегоНазначенияКлиентСервер.ОтмеченныеЭлементы(РегистрыСведений.ПорядокОтраженияНаСчетахУчета.СписокСчетовУчетаПоРазделуУчета("РасчетыСАрендодателями"));
	...
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		ПорядокОтраженияНаСчетахУчетаПоПериодуДействия КАК ПорядокОтраженияНаСчетахУчета
	|	ПО
	...
	|		И ХозрасчетныйОбороты.Период >= ПорядокОтраженияНаСчетахУчета.НачалоДействия
	|		И ХозрасчетныйОбороты.Период < ПорядокОтраженияНаСчетахУчета.ОкончаниеДействия
...
Процедура Заполнить2011Кв4_ДополнительныеПоказателиПоГрафе_Строка5040(КонтекстВыполнения)
	...
	ЗаполнениеБухгалтерскойОтчетностиПереопределяемый.ДобавитьСчета(СчетаАвансов, "АвансыВыданные");
	СчетаНДСАвансов = "76.ВА";
	СчетаПрочие   = "68.01, 68.02, 68.03, 68.04.1, 68.06, 68.07, 68.08, 68.09, 68.10, 68.11, 68.12, 68.13, 68.15,  68.22, 68.90, 68.91,
...
Функция СпособыОбработкиСчетовПояснений5()
	...
	СпособыОбработкиСчетовПояснений5.Вставить("68.91", "СчетаБезОбработки");
...
Функция СчетаУчета2017Кв3_ДебКср(ВидЗадолженности) Экспорт
	...
	ИначеЕсли ВидЗадолженности = "Платежи" Тогда
		...
		СчетаЗапроса.Вставить("СчетаУчетаБезАналитики", "68.04.2, 68.90, 68.91");
...
Функция СчетаУчета2017Кв3_КредКср(ВидЗадолженности) Экспорт
	...
	ИначеЕсли ВидЗадолженности = "Платежи" Тогда
		...
		СчетаЗапроса.Вставить("СчетаУчетаВидыПлатежейУровни", "68.04.1, 68.10");
		СчетаЗапроса.Вставить("СчетаУчетаБезАналитики", "68.04.2, 68.32, 68.42, 68.90, 68.91");
		СчетаЗапроса.Вставить("СчетаУчетаБезАналитикиРазвернуто", "69.13");
...
Функция ТекстЗапросаВТ_БанковскиеСчета()
	...
	|	ЛОЖЬ КАК ЭтоЦифровойСчет,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторЦифровогоСчета
