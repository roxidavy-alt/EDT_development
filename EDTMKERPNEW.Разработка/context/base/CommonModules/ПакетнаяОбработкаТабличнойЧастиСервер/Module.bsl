...
Процедура ПересчитатьКоличествоУпаковок(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Иначе
		...
		ТекстИсключения = НСтр("ru = 'При попытке пересчета количества в %ЕдИзмерения% превышена допустимая разрядность.';
								|en = 'While trying to recalculate the amount in %ЕдИзмерения%, the allowable capacity is exceeded.'");
		...
		Если ОписаниеЗапроса.ТекстыЗапросов.Количество() > 1 Тогда
			...
			Исключение
				...
				ТекстСообщения = НСтр("ru = 'Не удалось выполнить пакетную обработку строк таблицы.
					...
					|en = 'Cannot perform batch processing of table rows.
	...
	Если ЗначениеЗаполнено(СтруктураВидаЦен.СтавкаНДС) Тогда
		...
		Если (НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС
			...
			ТекстОшибки = НСтр("ru = 'Ставка НДС установленная для вида цены, не соответствует указанному в документе налогообложению.';
								|en = 'The VAT rate set for the price type does not match the taxation specified in the document.'");
			...
			Если КэшированныеЗначения.ИспользоватьМногострановойУчет Тогда
				...
				Если ЗначениеЗаполнено(СтруктураПараметровДействия.СтранаРегистрацииОрганизации)
					...
					ТекстОшибки = НСтр("ru = 'Страна в виде цен не соответствует стране регистрации организации.';
										|en = 'The country in the price type does not match the company''s country of registration.'");
				КонецЕсли;
		...
		Если СтруктураВидаЦен.УказыватьСтавкуНДС Тогда
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'На %1 не установлена Ставка НДС для выбранного вида цен.';
					|en = 'The VAT rate for the selected price type is not set as of %1.'"),
	...
	Если ВывестиСообщение И ЗначениеЗаполнено(ТекстОшибки)  Тогда
		...
		ТекстШаблона = СтрШаблон(
				НСтр("ru = 'Номенклатуру ""%1"" по виду цен ""%2"" продавать нельзя.';
					|en = 'Cannot sell the ""%1"" item by the ""%2"" price type.'") + " ",
				ТекущаяСтрока.Номенклатура,
				ВидЦены);
		...
		Если ПакетнаяОбработкаТабличнойЧастиСервер.ЕстьКолонкаВИсточникеДанных("НомерСтроки", КэшированныеЗначения) Тогда
			...
			ТекстШаблона = СтрШаблон(
				НСтр("ru = 'В строке %1 - номенклатуру ""%2"" по виду цен ""%3"" продавать нельзя.';
					|en = 'Cannot sell the ""%2"" item in line %1 by the ""%3"" price type.'") + " ",
				ТекущаяСтрока.НомерСтроки,
				ТекущаяСтрока.Номенклатура,
				ВидЦены);
