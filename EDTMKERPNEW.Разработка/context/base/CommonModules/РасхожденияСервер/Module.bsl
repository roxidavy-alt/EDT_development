	...
	Если ЭтоАктыОРасхожденияПослеОтгрузки Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		      ...
		      НСтр("ru = 'Оформление корректировки (исправление)';
					|en = 'Register an adjustment (correction)'"));
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		      Форма.Список, 
		      "СтатусКорректировкаКакИсправление", 
...
Процедура ДополнитьСтруктуруДействийПоТЧТоварыПриемкиТоваровНаХранение(СтруктураДействий, Объект)
	...
	//++ НЕ УТКА
	ХозяйственнаяОперация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "ХозяйственнаяОперация");
