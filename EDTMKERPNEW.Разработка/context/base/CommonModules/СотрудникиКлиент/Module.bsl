		...
		Если НЕ СотрудникиВызовСервера.ЗаблокироватьФизическоеЛицоПриРедактированииНаСервере(Форма.ФизическоеЛицоСсылка, Форма.ФизическоеЛицоВерсияДанных, Форма.УникальныйИдентификатор) Тогда
			...
			Если СообщатьОНевозможностиБлокировки Тогда
				...
				Иначе
					...
					ПоказатьПредупреждение(, СтрШаблон(НСтр("ru = 'Не удается внести изменения в личные данные сотрудника. Уже открыта форма с личными данными или личные данные редактируются другим пользователем %1';
															|en = 'Не удается внести изменения в личные данные сотрудника. Уже открыта форма с личными данными или личные данные редактируются другим пользователем %1'"), СведенияОБлокировке));
				КонецЕсли;
