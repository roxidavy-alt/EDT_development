					...
					Если ОшибкаПоШаблону = Неопределено Тогда
						...
						ОшибкаПоТипуШтрихкода.Вставить(СтрокаТЧ.Шаблон, СтрШаблон(
							НСтр("ru = 'При печати кодов маркировки для продукции %1 с видом упаковки %2 не может быть использован шаблон этикетки с типом штрихкода %3, укажите шаблон этикетки с типом штрихкода %4';
								|en = 'Cannot use the label template with the ""%3"" barcode type when printing marking codes for the ""%1"" item with the ""%2"" packaging unit kind. Specify the label template with the ""%4"" barcode type'"),
							КлючНоменклатура,
