...
Процедура ЗаполнитьГруппыЗапасов(ГруппыЗапасов, СчетаУчетаЗапасов, КонтекстЗаполненияОтчета)
	...
	Если РеглУчетСервер.ИспользуетсяРеглУчетПродажПоПартиямРеализованнойНоменклатуры() Тогда
		ПериодыДетализацииУчета = РеглУчетСервер.ПериодыДетализацииУчета(КонтекстЗаполненияОтчета.Период.Начало, КонтекстЗаполненияОтчета.Период.КонецДата, "РеглУчетПродажПоПартиямРеализованнойНоменклатуры");
		Если ПериодыДетализацииУчета.ДанныеСДетализацией Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Группа.СчетаОбесценения, РеглУчетВыборкиСерверПовтИсп.СчетаРезервыПодСнижениеСтоимостиНезавершенногоПроизводства());
		КонецЕсли;
		Если ПериодыДетализацииУчета.ДанныеБезДетализации Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Группа.СчетаОбесценения, РеглУчетВыборкиСерверПовтИсп.УстаревшиеСчетаРезервыПодСнижениеСтоимостиНезавершенногоПроизводства());
		КонецЕсли;
	Иначе
