...
Процедура ДобавитьЭтап_ОстаткиПриемаВПереработку2_4(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока =  ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(
		ТаблицаЭтапов,
		ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ОформлениеОстатковПриемаВПереработку2_4);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Оформление остатков приема в переработку (версии 2.4)';
											|en = 'Register remaining subcontracting services delivered (version 2.4)'"; // @НСтр-1
...
Процедура Использование_ОстаткиПриемаВПереработку2_4(ПараметрыОбработчика) Экспорт
	...
	|	ОстаткиТоваров.Номенклатура КАК Номенклатура,
	|	ОстаткиТоваров.Характеристика КАК Характеристика,
	|	(ОстаткиТоваров.ЗаказаноКоличествоОборот
	|		- ОстаткиТоваров.ОформленоКоличествоОборот) КАК Остаток
	|ИЗ
	|	#РаспоряженияНаОтгрузкуОборотыРазвернутая КАК ОстаткиТоваров
	|	ЛЕВОЕ СОЕДИНЕНИЕ Константы КАК Константы
	|	ПО ИСТИНА
	|ГДЕ
	|	НЕ Константы.ИспользоватьПроизводствоИзДавальческогоСырья
	|	И ВЫРАЗИТЬ(ОстаткиТоваров.Распоряжение КАК Документ.ЗаказДавальца).Организация В (&МассивОрганизаций)
	|	И (ОстаткиТоваров.ЗаказаноКоличествоОборот
	|		- ОстаткиТоваров.ОформленоКоличествоОборот) <> 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ПО ОстаткиТоваров.ЗаказПоставщику = РеестрДокументов.Ссылка
	...
	ПараметрыФормированияТаблицы.ТекстУсловия =
		...
		|		ИЗ
		|			РегистрСведений.РаспоряженияНаОтгрузкуИВозвратКВыполнению КАК ОтборЗаказа
		|		ГДЕ
		|			ТИПЗНАЧЕНИЯ(ОтборЗаказа.Распоряжение) = ТИП(Документ.ЗаказДавальца))";
...
Процедура ДобавитьЭтап_ОстаткиКОформлениюОтчетовДавальцам(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока =  ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(
		ТаблицаЭтапов,
		ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ОформлениеОтчетовДавальцам);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Оформление отчетов давальцам';
											|en = 'Register Consumption reports — Subcontracting services delivered'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
	НоваяСтрока.ДействиеОформление =  ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
...
// Предупреждение о скором ограничении функциональности подсистемы 2.4
// 
// Параметры:
...
// Возвращаемое значение:
//  ГруппаФормы -
Функция ДобавитьПредупреждениеПодсистема24Устарела(Форма, ГруппаРодитель = Неопределено) Экспорт
	
	ПрефиксЭлементов = "Подсистема24Устарела";
	...
	ТекстНадписи =
		НСтр("ru = 'Внимание! Функциональность ""Производство из давальческого сырья (версия 2.4)"" устарела, и будет удалена в версии 2.6.3. Пожалуйста, перейдите на ""Производство из давальческого сырья (версия 2.5)"".';
			|en = 'Внимание! Функциональность ""Производство из давальческого сырья (версия 2.4)"" устарела, и будет удалена в версии 2.6.3. Пожалуйста, перейдите на ""Производство из давальческого сырья (версия 2.5)"".'");
	...
	Иначе
		...
		ТекстСообщения =
			НСтр("ru = 'В строке %1 списка ""%2"" выбрано давальческое назначение.
					   ...
					   |You can select a provider assignment only when you group costs by assignments.'");
...
// Тип производственного процесса по хозяйственной операции.
// 
// Параметры:
//  ХозяйственнаяОперация - ПеречислениеСсылка.ХозяйственныеОперации - Хозяйственная операция
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.ТипыПроизводственныхПроцессов - Тип производственного процесса по хозяйственной операции
//
Функция ТипПроизводственногоПроцессаПоХозяйственнойОперации(ХозяйственнаяОперация) Экспорт
	
	Результат = Перечисления.ТипыПроизводственныхПроцессов.Сборка;
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыполнениеРемонтныхРаботСИспользованиемДавальческогоСырья2_5 Тогда
		Результат = Перечисления.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот;
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РемонтПродукцииИзДавальческогоСырья2_5 Тогда
			Результат = Перечисления.ТипыПроизводственныхПроцессов.Ремонт;
	КонецЕсли;
