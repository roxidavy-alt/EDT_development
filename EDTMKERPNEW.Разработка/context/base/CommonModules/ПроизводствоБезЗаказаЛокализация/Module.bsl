...
Функция ПолучитьДанныеДляПечатнойФормыМХ18(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|			КОГДА ГруппыЗатрат.ОрганизацияДавалец ЕСТЬ НЕ NULL
	...
	//++ НЕ УТКА
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ ГруппыЗатрат КАК ГруппыЗатрат
	|		ПО ГруппыЗатрат.Ссылка = Товары.Ссылка
	|		И ГруппыЗатрат.НомерГруппыЗатрат = Товары.НомерГруппыЗатрат
	|		И ГруппыЗатрат.ВнутренняяПереработка
	//-- НЕ УТКА
	|	
	|	ГДЕ
	|		Товары.Ссылка В
		...
		Текст =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В документе ""%1"" все изделия выпускаются в цеховые кладовые. Печать накладной не доступна.';
					|en = 'All the items are released to the shop floor stockrooms in document ""%1"". Invoice printing is not available.'"),
...
#Область ТекстВыпускСобственнойПродукцииКоличество // (Дт 10, 2Х, 41 :: Кт 10, 2Х, 41)
	...
	|	""ПроизводствоБезЗаказаЛокализация_1810"" КАК НомерФрагмента
	|ИЗ
...
#Область ТекстВыпускСобственнойПродукцииСтоимости // (Дт 10, 2Х, 41 :: Кт 10, 2Х, 41)
	...
	|		ИНАЧЕ ""Выпуск собственной продукции""
	|	КОНЕЦ КАК Содержание,
	|	""ПроизводствоБезЗаказаЛокализация_1811"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК Операция
...
#Область ТекстВыпускСобственнойПродукцииПлановаяСтоимостьСИспользованиемСчета40 // (Дт 10, 2Х, 41 :: Кт 40)
	...
	|	""ПроизводствоБезЗаказаЛокализация_978"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК Операция
...
#Область ТекстВыпускСобственнойПродукцииФактическаяСтоимостьСИспользованиемСчета40 // (Дт 40 :: Кт 10, 2Х, 41)
	...
	|	""ПроизводствоБезЗаказаЛокализация_979"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК Операция
...
#Область ТекстВыпускСобственнойПродукцииОтклонениеСИспользованиемСчета40 // (Дт 10, 2Х, 43 :: Кт 40)
	...
	|	""ПроизводствоБезЗаказаЛокализация_980"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК Операция
	...
	|	Стоимости.СуммаВР КАК СуммаВРКт,
	|	""Выпуск продукции (отклонение)"" КАК Содержание,
	|	""ПроизводствоБезЗаказаЛокализация_981"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК Операция
...
#Область ТекстВыпускСобственнойПродукцииНематериальныеЗатраты // (Дт 2Х :: Кт 2Х)
	...
	|	НематериальныеЗатраты.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	...
	|	""ПроизводствоБезЗаказаЛокализация_982"" КАК НомерФрагмента
	|ИЗ
...
#Область ТекстВыпускСобственнойПродукцииНематериальныеЗатратыФактическиеСИспользованиемСчета40 // (Дт 40 :: Кт 2Х)
	...
	|	НематериальныеЗатраты.СуммаВР КАК СуммаВРКт,
	|	""Выпуск собственной продукции (фактическая стоимость)"" КАК Содержание,
	|	""ПроизводствоБезЗаказаЛокализация_983"" КАК НомерФрагмента
	|ИЗ
	|	НематериальныеЗатраты КАК НематериальныеЗатраты
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК Операция
...
#Область ТекстСписаниеПродукцииНаРасходы // (Дт СтатьяРасходов.СчетУчета :: Кт Счет учета затрат подразделения)
	...
	|	""ПроизводствоБезЗаказаЛокализация_984"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК Операция
	|		ПО ДокументыКОтражению.Ссылка = Операция.Ссылка
...
#Область ТекстСписаниеНаРасходыПлановая // (Дт 20, 2Х, 44, 91.02 :: Кт 40)
	...
	|	""ПроизводствоБезЗаказаЛокализация_985"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстСписаниеНаРасходыОтклонение // (Дт 20, 2Х, 44, 91.02 :: Кт 40)
	...
	|	""Выпуск продукции (отклонение)"" КАК Содержание,
	|	""ПроизводствоБезЗаказаЛокализация_986"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстСписаниеНаРасходыАктивыФактическая // (Дт 40 :: Кт 20)
	...
	|	""ПроизводствоБезЗаказаЛокализация_987"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстСписаниеПродукцииНаАктивыПассивы // (Дт Документ.СчетУчета :: Кт Счет учета затрат подразделения)
	...
	|	""Списание продукции на расходы"" КАК Содержание,
	|	""ПроизводствоБезЗаказаЛокализация_988"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК Операция
	|		ПО ДокументыКОтражению.Ссылка = Операция.Ссылка
...
#Область ТекстСписаниеНаАктивыПлановая // (Дт Документ.СчетУчета :: Кт 40)
	...
	|	Строки.СуммаВР КАК СуммаВРКт,
	|	""Выпуск продукции (плановая)"" КАК Содержание,
	|	""ПроизводствоБезЗаказаЛокализация_989"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстСписаниеНаАктивыОтклонение // (Дт Документ.СчетУчета :: Кт 40)
	...
	|	Стоимости.СуммаВР - Строки.СуммаВР КАК СуммаВРКт,
	|	""Выпуск продукции (отклонение)"" КАК Содержание,
	|	""ПроизводствоБезЗаказаЛокализация_990"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстВыпускВозвратныхОтходов // (Дт 2Х :: Кт 2Х)
	...
	|	""ПроизводствоБезЗаказаЛокализация_991"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК Операция
...
#Область ТекстСписаниеКомиссионныхТоваровНаПроизводство // (Дт 20 :: 76.ОК)
	...
	|	""ПроизводствоБезЗаказаЛокализация_992"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстВключениеИсключениеНДСВСтоимость // (Дт 20 :: Кт 19.03)
	...
	|	""ПроизводствоБезЗаказаЛокализация_993"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	|	ПрочиеРасходыНезавершенногоПроизводства.ИдентификаторФинЗаписи КАК ИдентификаторФинЗаписи,
	...
	|	ПрочиеРасходыНезавершенногоПроизводства.ИдентификаторФинЗаписи,
	...
	|	ТрудозатратыНезавершенногоПроизводства.ИдентификаторФинЗаписи КАК ИдентификаторФинЗаписи,
	|	АналитикаПродукции.Номенклатура КАК Затрата,
	|	ЕСТЬNULL(СпрПартииПроизводства.НаправлениеДеятельности,
	|		ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)) КАК НаправлениеДеятельности,
	|	ЕСТЬNULL(АналитикаПродукции.Назначение.НаправлениеДеятельности,
	...
	|	ТрудозатратыНезавершенногоПроизводства.Организация,
	|	ТрудозатратыНезавершенногоПроизводства.ИдентификаторФинЗаписи,
	|	АналитикаПродукции.Номенклатура,
	|	ЕСТЬNULL(СпрПартииПроизводства.НаправлениеДеятельности,
	|		ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)),
	|	ЕСТЬNULL(АналитикаПродукции.Назначение.НаправлениеДеятельности,
...
Функция ТекстВыпускДавальческойПродукцииБаланс() Экспорт
	...
	|	""ПроизводствоБезЗаказаЛокализация_994"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
Функция ТекстОприходованиеТоваровПоСебестоимости() Экспорт
	...
	|
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(КорСтроки.РазделУчета, Строки.КорРазделУчета) = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)
	...
	|		КОГДА ЕСТЬNULL(КорСтроки.РазделУчета, Строки.КорРазделУчета) = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НезавершенноеПроизводство)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НезавершенноеПроизводство)
	...
	|	КОНЕЦ КАК Содержание,
	|	""ПроизводствоБезЗаказаЛокализация_1812"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстОприходованиеТоваровПоСебестоимостиНалоговыйУчет() Экспорт
	...
	|	КОНЕЦ КАК Содержание,
	|	""ПроизводствоБезЗаказаЛокализация_1813"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстОприходованиеТоваровОтклонениеВСтоимостиРасходы() Экспорт
	...
	|
	|	ВЫБОР
	|		КОГДА Стоимости.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)
	|			ИЛИ Стоимости.РазделУчета В (&ЗабалансовыеРазделыУчета)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.МатериалыИРаботыВПодразделении)
	|		КОГДА Стоимости.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НезавершенноеПроизводство)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НезавершенноеПроизводство)
	...
	|	""ПроизводствоБезЗаказаЛокализация_995"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстОприходованиеТоваровОтклонениеВСтоимостиДоходы() Экспорт
	...
	|	""ПроизводствоБезЗаказаЛокализация_996"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстСписаниеТоваровНаРасходы(Производство = Ложь, СодержаниеПоУмолчанию = Истина, ПереопределениеОтбора = Ложь) Экспорт
	...
	|	""ПроизводствоБезЗаказаЛокализация_1814"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтСтроки КАК Строки
	...
	|		И Расходы.ИдентификаторФинЗаписи <> """"
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтСтоимости КАК Стоимости
	|	ПО Строки.Ссылка = Стоимости.Ссылка
	...
	|	ПО Статьи.Ссылка = Строки.СтатьяРасходов
	|
	...
	Если Производство Тогда
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			"Строки.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)", "ИСТИНА");
	КонецЕсли;
...
Функция ТекстПриходТоваровЗаБалансом(Производство = Ложь, СодержаниеПоУмолчанию = Истина, ПереопределениеОтбора = Ложь) Экспорт
	...
	|				ИЛИ Строки.Склад ССЫЛКА Справочник.ДоговорыКонтрагентов)
	...
	|	0 КАК СуммаВРКт,
	|	&Содержание КАК Содержание,
	|	""ПроизводствоБезЗаказаЛокализация_1815"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстПередачаТоваровЗаБалансом(Производство = Ложь, СодержаниеПоУмолчанию = Истина, ПереопределениеОтбора = Ложь) Экспорт
	...
	|				ИЛИ Строки.КорСклад ССЫЛКА Справочник.ДоговорыКонтрагентов)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.ТоварыПереданныеНаКомиссию)
	|		КОГДА Строки.КорРазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПринятыеНаКомиссию)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.ТоварыНаСкладе)
	...
	|	0 КАК СуммаВРКт,
	|	&Содержание КАК Содержание,
	|	""ПроизводствоБезЗаказаЛокализация_1816"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстСписаниеТоваровЗаБалансом(Производство = Ложь, СодержаниеПоУмолчанию = Истина, ПереопределениеОтбора = Ложь) Экспорт
	...
	|	0 КАК СуммаВРКт,
	|	&Содержание КАК Содержание,
	|	""ПроизводствоБезЗаказаЛокализация_1817"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
Функция ТекстСписаниеТоваровНаСтатьиАктивовПассивов(СодержаниеПоУмолчанию = Истина, ПереопределениеОтбора = Ложь) Экспорт
	...
	|	ЕСТЬNULL(Стоимости.СуммаВР, Строки.СуммаВР) КАК СуммаВРКт,
	|	&Содержание КАК Содержание,
	|	""ПроизводствоБезЗаказаЛокализация_1818"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтСтроки КАК Строки
...
Функция ТекстВыпускПродукцииНематериальныеЗатратыПереопределяемый(СодержаниеПоУмолчанию = Истина, ПереопределениеОтбора = Ложь) Экспорт
	...
	|	&Содержание КАК Содержание,
	|	""ПроизводствоБезЗаказаЛокализация_1819"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПрочиеРасходыНезавершенногоПроизводства КАК ДанныеРегистра
...
Функция ТекстВыпускПродукцииТрудозатратыПереопределяемый(СодержаниеПоУмолчанию = Истина, ПереопределениеОтбора = Ложь) Экспорт
	...
	|	ДанныеРегистра.ВременнаяРазница КАК СуммаВРКт,
	|	&Содержание КАК Содержание,
	|	""ПроизводствоБезЗаказаЛокализация_1820"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ТрудозатратыНезавершенногоПроизводства КАК ДанныеРегистра
...
Функция ТекстВыпускСобственнойПродукцииПлановаяСтоимость() Экспорт
	...
	|		КОГДА СтрокиДт.ВидСчета В (
	|				ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.МатериалыИРаботыВПодразделении),
	|				ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НезавершенноеПроизводство))
	|			ТОГДА СтрокиДт.ГруппаПродукции
	...
	|	""ПроизводствоБезЗаказаЛокализация_997"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстВыпускПродукцииСоСписаниемНаРасходыПлановаяСтоимость() Экспорт
	...
	|	""ПроизводствоБезЗаказаЛокализация_998"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстВыпускПродукцииСоСписаниемНаАктивыПассивыПлановаяСтоимость() Экспорт
	...
	|	""ПроизводствоБезЗаказаЛокализация_999"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстВыпускСобственнойПродукцииНаСкладОтклонение() Экспорт
	...
	|	""ПроизводствоБезЗаказаЛокализация_1000"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстЗапросаТаблицаРНПТМатериаловВПроизводстве(Запрос, ТекстыЗапроса, Регистры)
	...
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПартииПроизводства КАК СпрПартииПроизводства
	|	ПО СпрПартииПроизводства.Документ = &ДокументПартии
...
Процедура ЗаполнитьПризнакЕстьМаркируемаяПродукция(Объект)
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	ТаблицаНоменклатура.Ссылка В(&ВыходныеИзделия)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|	И ОписаниеНоменклатурыИС.КлючВидовПродукции.ВидПродукцииГИСМТ В (&ВидыПродукцииМЕХ)
		|ГДЕ
		|	ТаблицаНоменклатура.Ссылка В(&ПобочныеИзделия)";
...
Функция ТекстСторноПроизводственныхЗатрат()
	...
	|
	|	ВЫБОР
	|		КОГДА СторноПроизводственныхЗатрат.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.НезавершенноеПроизводство)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НезавершенноеПроизводство)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.МатериалыИРаботыВПодразделении)
	|	КОНЕЦ КАК ВидСчетаКт,
	...
	|	""ПроизводствоБезЗаказаЛокализация_1001"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
