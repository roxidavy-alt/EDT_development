...
Функция ПредставлениеВводаОстатков(Объект) Экспорт
	...
	ИначеЕсли РеквизитыОбъекта.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковПредметовЛизингаЗаБалансом Тогда
		Представление = НСтр("ru = 'Ввод начальных остатков предметов лизинга за балансом %1';
							|en = 'OB entry: financial lease items off the balance sheet %1'");
	ИначеЕсли РеквизитыОбъекта.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковПереданныхВАрендуПредметовЛизингаНаБалансе Тогда
		Представление = НСтр("ru = 'Ввод начальных остатков переданных в аренду предметов лизинга на балансе %1';
							|en = 'OB entry: financial lease items on the balance sheet %1'");
	ИначеЕсли РеквизитыОбъекта.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковВложенийВоВнеоборотныеАктивы Тогда
		Представление = НСтр("ru = 'Ввод начальных остатков вложений во внеоборотные активы %1';
							|en = 'OB entry: investments in fixed and intangible assets %1'");
	КонецЕсли;
...
Функция ДанныеВыбораНематериальныхАктивов2_4(Параметры)
	...
	НастройкиПрав.Вставить(
		"ЕстьПравоЧтение_МестоУчетаНМА", 
		ПравоДоступа("Чтение", Метаданные.РегистрыСведений.МестоУчетаНМА));

	НастройкиПрав.Вставить(
		"ЕстьПравоЧтение_ПорядокУчетаНМАУУ", 
		ПравоДоступа("Чтение", Метаданные.РегистрыСведений.ПорядокУчетаНМАУУ));

	НастройкиПрав.Вставить(
		"ЕстьПравоЧтение_ПорядокУчетаНМАБУ", 
		ПравоДоступа("Чтение", Метаданные.РегистрыСведений.ПорядокУчетаНМАБУ));

	НастройкиПрав.Вставить(
		"ЕстьПравоЧтение_ПорядокУчетаНМА", 
		ПравоДоступа("Чтение", Метаданные.РегистрыСведений.ПорядокУчетаНМА));

	НастройкиПрав.Вставить(
		"ЕстьПравоЧтение_ПараметрыАмортизацииНМАУУ", 
		ПравоДоступа("Чтение", Метаданные.РегистрыСведений.ПараметрыАмортизацииНМАУУ));

	НастройкиПрав.Вставить(
		"ЕстьПравоЧтение_ПараметрыАмортизацииНМАБУ", 
		ПравоДоступа("Чтение", Метаданные.РегистрыСведений.ПараметрыАмортизацииНМАБУ));

	НастройкиПрав.Вставить(
		"ЕстьПравоЧтение_ПервоначальныеСведенияНМА", 
		ПравоДоступа("Чтение", Метаданные.РегистрыСведений.ПервоначальныеСведенияНМА));

	ПараметрыВыбора = Справочники.НематериальныеАктивы.ПолучитьПараметрыВыбора(Параметры, НастройкиПрав);
	ОписаниеЗапросаДляВыбора = Справочники.НематериальныеАктивы.ОписаниеЗапросаВыбораЭлементов(ПараметрыВыбора, НастройкиПрав, Ложь, Истина);
	...
	Если ОписаниеЗапросаДляВыбора.НеобходимыеТаблицы.Количество() <> 0 Тогда
		...
		|	ДанныеСправочника.ГруппаНМА КАК ГруппаНМА,
		|	ДанныеСправочника.ВидНМА КАК ВидНМА,
		|	ДанныеСправочника.ВидОбъектаУчета КАК ВидОбъектаУчета,
		|	ДанныеСправочника.АмортизационнаяГруппа КАК АмортизационнаяГруппа
		|
		|ПОМЕСТИТЬ ВтСписокНМА
		|
		|ИЗ
		|	Справочник.НематериальныеАктивы КАК ДанныеСправочника
		|
		|ГДЕ
		|	ДанныеСправочника.Наименование ПОДОБНО &ПодстрокаПоиска
		|	И НЕ ДанныеСправочника.ЭтоГруппа
		...
		Если ОписаниеЗапросаДляВыбора.НеобходимыеТаблицы.Свойство("МестоУчетаНМА") Тогда
			...
			|						ВтСписокНМА.Ссылка 
			|					ИЗ 
			|						ВтСписокНМА КАК ВтСписокНМА)) КАК ДанныеРегистра
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	НематериальныйАктив,
			|	Организация
			...
			|		ПО МестоУчетаНМА.НематериальныйАктив = ДанныеСправочникаПереопределяемый.Ссылка
			|"; // @query-part-1
			
		КонецЕсли;
		
		Если ОписаниеЗапросаДляВыбора.НеобходимыеТаблицы.Свойство("ПервоначальныеСведенияНМА") Тогда
			
			ТекстЗапроса =
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	ДанныеРегистра.Организация КАК Организация,
			|	ДанныеРегистра.НематериальныйАктив КАК НематериальныйАктив,
			|	ДанныеРегистра.ДатаПринятияКУчетуБУ КАК ДатаПринятияКУчетуБУ,
			|	ДанныеРегистра.ДатаПринятияКУчетуНУ КАК ДатаПринятияКУчетуНУ,
			|	ДанныеРегистра.ДатаПринятияКУчетуУУ КАК ДатаПринятияКУчетуУУ,
			|	ДанныеРегистра.МетодНачисленияАмортизацииБУ КАК МетодНачисленияАмортизацииБУ,
			|	ДанныеРегистра.МетодНачисленияАмортизацииНУ КАК МетодНачисленияАмортизацииНУ
			|
			...
			|					ИЗ 
			|						МестоУчетаНМА КАК МестоУчетаНМА)) КАК ДанныеРегистра
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	НематериальныйАктив,
			|	Организация
		...
		Если ОписаниеЗапросаДляВыбора.НеобходимыеТаблицы.Свойство("ПорядокУчетаНМАУУ") Тогда
			...
			|		ПО ПорядокУчетаНМАУУ.Организация = МестоУчетаНМА.Организация
			|			И ПорядокУчетаНМАУУ.НематериальныйАктив = МестоУчетаНМА.НематериальныйАктив
			|"; // @query-part-1
			
		КонецЕсли;
		
		Если ОписаниеЗапросаДляВыбора.НеобходимыеТаблицы.Свойство("ПорядокУчетаНМАБУ") Тогда
			...
			|		ПО ПорядокУчетаНМАБУ.Организация = МестоУчетаНМА.Организация
			|			И ПорядокУчетаНМАБУ.НематериальныйАктив = МестоУчетаНМА.НематериальныйАктив
			|"; // @query-part-1
			
		КонецЕсли;
	
		Если ОписаниеЗапросаДляВыбора.НеобходимыеТаблицы.Свойство("ПараметрыАмортизацииНМАУУ") Тогда
			...
			|		ПО ПараметрыАмортизацииНМАУУ.Организация = МестоУчетаНМА.Организация
			|			И ПараметрыАмортизацииНМАУУ.НематериальныйАктив = МестоУчетаНМА.НематериальныйАктив
			|"; // @query-part-1
			
		КонецЕсли;
		
		Если ОписаниеЗапросаДляВыбора.НеобходимыеТаблицы.Свойство("ПараметрыАмортизацииНМАБУ") Тогда
			...
			|		ПО ПараметрыАмортизацииНМАБУ.Организация = МестоУчетаНМА.Организация
			|			И ПараметрыАмортизацииНМАБУ.НематериальныйАктив = МестоУчетаНМА.НематериальныйАктив
			|"; // @query-part-1
			
		КонецЕсли;
		
		Если ОписаниеЗапросаДляВыбора.НеобходимыеТаблицы.Свойство("ПорядокУчетаНМА") Тогда
			...
			|		ПО ПорядокУчетаНМА.Организация = МестоУчетаНМА.Организация
			|			И ПорядокУчетаНМА.НематериальныйАктив = МестоУчетаНМА.НематериальныйАктив
			|"; // @query-part-1
		...
		|
		|ИЗ
		|	ВтСписокНМА КАК ДанныеСправочникаПереопределяемый
		|";
	...
	Иначе
		...
		|
		|ИЗ	
		|	Справочник.НематериальныеАктивы КАК ДанныеСправочникаПереопределяемый
		|";
