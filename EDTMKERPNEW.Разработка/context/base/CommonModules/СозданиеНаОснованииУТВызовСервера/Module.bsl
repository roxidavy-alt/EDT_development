...
Функция ПараметрыОткрытияФормыНакладнойНаОснованииЗаказа(Распоряжения, ИмяНакладной, ХозяйственнаяОперация = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДанныеДляПроверки.РаспоряженияКОформлению) Тогда
		ВызватьИсключение НСтр("ru = 'Оформление накладных не требуется: распоряжения на оформление уже исполнены, либо отсутствуют.';
								|en = 'Invoice registration is not required: the registration references are already fulfilled or missing.'");
	КонецЕсли;
		...
		Если ЗапрещенныеХозяйственныеОперации.Найти(ХозяйственнаяОперация) <> Неопределено Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ввод отгрузки товаров клиенту на основании документа с операцией ""%1"" невозможен.';
					|en = 'Cannot generate a goods shipment to customer from a base document with the ""%1"" business transaction.'"),
				ХозяйственнаяОперация);
...
Функция ПриемкаРемонтируемогоИзделияПараметрыОткрытияФормы(ПараметрКоманды) Экспорт
	...
	ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтДавальца2_5");
	
	Если ОбщегоНазначенияУТ.ПроверитьОперациюРаспоряжения(ДокументОснование, "ПриемкаТоваровНаХранение", ХозяйственнаяОперация) Тогда
		ПараметрыОснования.Вставить("ДокументОснованиеРемонтируемогоИзделия", ДокументОснование);
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Функция ОтгрузкаТоваровСХраненияВозвратРемонтируемогоИзделияПараметрыОткрытияФормы(ПараметрКоманды) Экспорт
	...
	ПараметрыОснования.Вставить("ДокументОснование",                      ДокументОснование);
	ПараметрыОснования.Вставить("ДокументОснованиеРемонтируемогоИзделия", Истина);
