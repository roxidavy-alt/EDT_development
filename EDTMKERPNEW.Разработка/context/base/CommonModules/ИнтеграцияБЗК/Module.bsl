...
Функция ПараметрыДляПроведенияДокумента(Документ, Свойства) Экспорт
	...
	Если Свойства.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		МетаданныеРегистра = Метаданные.РегистрыНакопления.ЗначенияОперативныхПоказателейРасчетаЗарплатыСотрудников;
		Если Параметры.ПодчиненныеРегистры.Найти(МетаданныеРегистра) <> Неопределено Тогда
			Документ.ДополнительныеСвойства.Вставить("ЗаполнитьПерерасчетыЗарплаты", Истина);
...
Функция ТекстыЗапросовСторнирования(МетаданныеДокумента) Экспорт
	...
	Если ДвиженияДокумента.Содержит(МетаданныеРегистра) Тогда
		...
		|	ИСТИНА КАК Сторно
		|ИЗ
		|	РегистрНакопления.ЗначенияОперативныхПоказателейРасчетаЗарплатыСотрудников КАК ДанныеРегистра
		...
		Исключения.Добавить(МетаданныеРегистра.Измерения.Показатель.Имя);
...
Процедура ОтразитьДвижения(ТаблицыДляДвижений, Движения, Отказ) Экспорт
	...
	Если ТаблицыДляДвижений.Свойство("ТаблицаДивиденды")
		...
		ДатаПеречисленияНДФЛ = '00010101';
		Если Реквизиты.ДатаВыплаты < ЕдиныйНалоговыйСчет.ДатаВступленияВСилу539ФЗ() Тогда
			ДатаПеречисленияНДФЛ = Реквизиты.ДатаВыплаты;
		КонецЕсли;
		
		УчетНДФЛРасширенный.СформироватьДоходыИНДФЛСДивидендов(Реквизиты.Ссылка, Движения, Отказ, Реквизиты.Организация,
			Реквизиты.ДатаВыплаты, ТаблицаДивиденды, Реквизиты.ВключатьВДекларациюПоНалогуНаПрибыль, Истина, ДатаПеречисленияНДФЛ);
	...
	Если ТаблицыДляДвижений.Свойство("ТаблицаЗначенияОперативныхПоказателейРасчетаЗарплатыСотрудников")
		И ЗначениеЗаполнено(ТаблицыДляДвижений.ТаблицаЗначенияОперативныхПоказателейРасчетаЗарплатыСотрудников)
		И ТаблицыДляДвижений.ТаблицаЗначенияОперативныхПоказателейРасчетаЗарплатыСотрудников.Количество() Тогда
		
		Реквизиты = ТаблицыДляДвижений.ТаблицаЗначенияОперативныхПоказателейРасчетаЗарплатыСотрудников[0];
...
Процедура ПослеЗаписиДвиженийДокумента(Документ, МенеджерВременныхТаблиц, Отказ) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Документ.ДополнительныеСвойства, "ЗаполнитьПерерасчетыЗарплаты", Ложь) = Истина Тогда
		ПерерасчетЗарплаты.РегистрацияПерерасчетовПоПредварительнымДанным(Документ.Ссылка);
	КонецЕсли;
...
Процедура ЗаполнитьКадровыеДанныеСотрудников(Объект, ИмяТЧ, ЗаполняемыеПоля, МассивСтрок = Неопределено) Экспорт
	...
	ПараметрыЗапроса.НачалоПериода = ?(ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "Месяц"), НачалоМесяца(Объект.Месяц), ?(ЗначениеЗаполнено(Объект.Дата), НачалоМесяца(Объект.Дата), НачалоМесяца(Объект.НачалоПериода)));
	ПараметрыЗапроса.ОкончаниеПериода = ?(ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "Месяц"), КонецМесяца(Объект.Месяц), ?(ЗначениеЗаполнено(Объект.Дата), КонецМесяца(Объект.Дата), КонецМесяца(Объект.КонецПериода)));
...
Процедура ДобавитьПрофилиЗарплатаИКадры(ОписанияПрофилей, ПараметрыОбновления) Экспорт
	...
	ПапкаПрофилей.Наименование  = НСтр("ru = 'Зарплата и кадры';
										|en = 'HR and payroll'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ОперацияОтражениеЗарплатыВФинансовомУчете() Экспорт
	
	Операция = Неопределено;
	//++ Локализация
	Операция = Справочники.ОперацииЗакрытияМесяца.ОтражениеЗарплатыВФинансовомУчете;
	//-- Локализация
	Возврат Операция;
...
Процедура ДобавитьЭтап_НачислениеРезервовПоОплатеТруда(ТаблицаЭтапов, ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.НачислениеРезервовПоОплатеТруда,,,,
		Справочники.ОперацииЗакрытияМесяца.ОтражениеЗарплатыВФинансовомУчете);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Начисление резервов по оплате труда';
											|en = 'Accrue payroll funds'"; // @НСтр-1
	НоваяСтрока.ПредшествующиеЭтапы.Добавить(Справочники.ОперацииЗакрытияМесяца.ОтражениеЗарплатыВФинансовомУчете);
...
Процедура ДобавитьЭтап_ОтражениеЗарплатыВФинансовомУчете(ТаблицаЭтапов, ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Отражение зарплаты в финансовом учете';
											|en = 'Post payroll expenses to accounting'"; // @НСтр-1
...
Функция ТекстЗапросаВидыСтраховыхВзносов(ИмяВременнойТаблицы = "ВТВидыСтраховыхВзносов") Экспорт
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Справочник.ВидыНалоговВзносов.ПФРНакопительнаяЧасть) КАК ВидНалога,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудников.ПФРНакопительная) КАК ВидСтраховыхВзносов
	|ПОМЕСТИТЬ ВТВидыСтраховыхВзносов
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	Если Не ПустаяСтрока(ИмяВременнойТаблицы) И ИмяВременнойТаблицы <> "ВТВидыСтраховыхВзносов" Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПОМЕСТИТЬ ВТВидыСтраховыхВзносов", "ПОМЕСТИТЬ " + ИмяВременнойТаблицы);
	КонецЕсли;
