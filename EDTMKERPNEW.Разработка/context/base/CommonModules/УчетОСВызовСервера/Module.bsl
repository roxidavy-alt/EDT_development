...
Процедура СоздатьВременнуюТаблицуСостоянияОС(МенеджерВТ) Экспорт
	...
	|	ЕСТЬNULL(ПорядокУчетаОС.СчетУчета, ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)) КАК СчетУчета,
	|	ЕСТЬNULL(ПорядокУчетаОС.СчетНачисленияАмортизации, ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)) КАК СчетАмортизации,
	|	ЕСТЬNULL(СчетаУчетаЦФ.СчетУчета, ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)) КАК СчетУчетаЦФ,
	|	ЕСТЬNULL(СчетаУчетаЦФ.СчетАмортизации, ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)) КАК СчетАмортизацииЦФ,
	|	МАКСИМУМ(ЕСТЬNULL(СчетУчетаСтоимости.Период, ДАТАВРЕМЯ(1,1,1))) КАК Период_СтоимостьВНА,
	|	МАКСИМУМ(ЕСТЬNULL(СчетУчетаАмортизации.Период, ДАТАВРЕМЯ(1,1,1))) КАК Период_АмортизацияВНА,
	|	МАКСИМУМ(ЕСТЬNULL(СчетУчетаСтоимостиЦФ.Период, ДАТАВРЕМЯ(1,1,1))) КАК Период_СтоимостьВНА_ЦФ,
	|	МАКСИМУМ(ЕСТЬNULL(СчетУчетаАмортизацииЦФ.Период, ДАТАВРЕМЯ(1,1,1))) КАК Период_АмортизацияВНА_ЦФ
	|ПОМЕСТИТЬ втПорядокУчетаОССрезПоследних
	|ИЗ
	...
	|			И СчетУчетаСтоимости.Период <= ПорядокУчетаОС.Период
	...
	|			И СчетУчетаСтоимостиЦФ.Период <= ПорядокУчетаОС.Период
	...
	|			И СчетУчетаАмортизации.Период <= ПорядокУчетаОС.Период
	...
	|			И СчетУчетаАмортизацииЦФ.Период <= ПорядокУчетаОС.Период
	|СГРУППИРОВАТЬ ПО
	|	втПорядокУчетаОСПериод.Ссылка,
	|	втПорядокУчетаОСПериод.ОсновноеСредство,
	|	ПорядокУчетаОС.НаправлениеДеятельности,
	|	ПорядокУчетаОС.ГруппаФинансовогоУчета,
	|	ЕСТЬNULL(ПорядокУчетаОС.СчетУчета, ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)),
	|	ЕСТЬNULL(ПорядокУчетаОС.СчетНачисленияАмортизации, ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)),
	|	ЕСТЬNULL(СчетаУчетаЦФ.СчетУчета, ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)),
	|	ЕСТЬNULL(СчетаУчетаЦФ.СчетАмортизации, ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	втПорядокУчетаОСПериод.Ссылка,
	|	втПорядокУчетаОСПериод.ОсновноеСредство,
	|	втПорядокУчетаОСПериод.НаправлениеДеятельности,
	|	ЕСТЬNULL(СчетУчетаСтоимости.СчетУчета, ЕСТЬNULL(втПорядокУчетаОСПериод.СчетУчета, ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка))) КАК СчетУчета,
	|	ЕСТЬNULL(СчетУчетаАмортизации.СчетУчета, ЕСТЬNULL(втПорядокУчетаОСПериод.СчетАмортизации, ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка))) КАК СчетАмортизации,
	|	ЕСТЬNULL(СчетУчетаСтоимостиЦФ.СчетУчета, ЕСТЬNULL(втПорядокУчетаОСПериод.СчетУчетаЦФ, ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка))) КАК СчетУчетаЦФ,
	|	ЕСТЬNULL(СчетУчетаАмортизацииЦФ.СчетУчета, ЕСТЬNULL(втПорядокУчетаОСПериод.СчетАмортизацииЦФ, ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка))) КАК СчетАмортизацииЦФ
	|ПОМЕСТИТЬ втПорядокУчетаОС
	|ИЗ
	|	втПорядокУчетаОССрезПоследних КАК втПорядокУчетаОСПериод
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета КАК СчетУчетаСтоимости
	|		ПО СчетУчетаСтоимости.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.СтоимостьВНА)
	|			И СчетУчетаСтоимости.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|			И СчетУчетаСтоимости.АналитикаУчета = втПорядокУчетаОСПериод.ГруппаФинансовогоУчета
	|			И СчетУчетаСтоимости.МестоУчета = НЕОПРЕДЕЛЕНО
	|			И СчетУчетаСтоимости.Период = втПорядокУчетаОСПериод.Период_СтоимостьВНА
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета КАК СчетУчетаСтоимостиЦФ
	|		ПО СчетУчетаСтоимостиЦФ.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.СтоимостьВНА_ЦФ)
	|			И СчетУчетаСтоимостиЦФ.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|			И СчетУчетаСтоимостиЦФ.АналитикаУчета = втПорядокУчетаОСПериод.ГруппаФинансовогоУчета
	|			И СчетУчетаСтоимостиЦФ.МестоУчета = НЕОПРЕДЕЛЕНО
	|			И СчетУчетаСтоимостиЦФ.Период = втПорядокУчетаОСПериод.Период_СтоимостьВНА_ЦФ
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета КАК СчетУчетаАмортизации
	|		ПО СчетУчетаАмортизации.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.АмортизацияВНА)
	|			И СчетУчетаАмортизации.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|			И СчетУчетаАмортизации.АналитикаУчета = втПорядокУчетаОСПериод.ГруппаФинансовогоУчета
	|			И СчетУчетаАмортизации.МестоУчета = НЕОПРЕДЕЛЕНО
	|			И СчетУчетаАмортизации.Период = втПорядокУчетаОСПериод.Период_АмортизацияВНА
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета КАК СчетУчетаАмортизацииЦФ
	|		ПО СчетУчетаАмортизацииЦФ.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.АмортизацияВНА_ЦФ)
	|			И СчетУчетаАмортизацииЦФ.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|			И СчетУчетаАмортизацииЦФ.АналитикаУчета = втПорядокУчетаОСПериод.ГруппаФинансовогоУчета
	|			И СчетУчетаАмортизацииЦФ.МестоУчета = НЕОПРЕДЕЛЕНО
	|			И СчетУчетаАмортизацииЦФ.Период = втПорядокУчетаОСПериод.Период_АмортизацияВНА_ЦФ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ТекстЗапросаВТСчетаОтражения(ИмяДокумента)
	...
	Иначе	
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПоследних(&Дата, МестоУчета = НЕОПРЕДЕЛЕНО
		|			И Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
		|			И ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.СтоимостьВНА)) КАК СчетУчетаСтоимости
		|		ПО СчетУчетаСтоимости.АналитикаУчета = СчетаОтражения.ГруппаФинансовогоУчета
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПоследних(&Дата, МестоУчета = НЕОПРЕДЕЛЕНО
		|			И Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
		|			И ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.СтоимостьВНА_ЦФ)) КАК СчетУчетаСтоимостиЦФ
		|		ПО СчетУчетаСтоимостиЦФ.АналитикаУчета = СчетаОтражения.ГруппаФинансовогоУчета
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПоследних(&Дата, МестоУчета = НЕОПРЕДЕЛЕНО
		|			И Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
		|			И ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.АмортизацияВНА)) КАК СчетУчетаАмортизации
		|		ПО СчетУчетаАмортизации.АналитикаУчета = СчетаОтражения.ГруппаФинансовогоУчета
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПоследних(&Дата, МестоУчета = НЕОПРЕДЕЛЕНО
		|			И Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
		|			И ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.АмортизацияВНА_ЦФ)) КАК СчетУчетаАмортизацииЦФ
		|		ПО СчетУчетаАмортизацииЦФ.АналитикаУчета = СчетаОтражения.ГруппаФинансовогоУчета";
...
Функция ТекстОтраженияАмортизационнаяПремияКВ()
	...
	|	""УчетОСВызовСервера_1338"" КАК НомерФрагмента
	|ИЗ
...
Функция ТекстОтраженияИзнос()
	...
	|	""УчетОСВызовСервера_1339"" КАК НомерФрагмента
	|ИЗ
...
Функция ТекстОтраженияДоходыОтЦелевогоФинансирования()
	...
	|	""УчетОСВызовСервера_1340"" КАК НомерФрагмента
	|ИЗ
...
Функция ТекстОтраженияАмортизация()
	...
	|	""УчетОСВызовСервера_1341"" КАК НомерФрагмента
	|ИЗ
	|	втНачисленнаяАмортизация КАК Строки
	|	
...
Функция ТекстОтраженияАмортизацияБезНУ()
	...
	|	
	|	""Начисление амортизации"" КАК Содержание,
	|	""УчетОСВызовСервера_1342"" КАК НомерФрагмента
	|ИЗ
	|	втНачисленнаяАмортизация КАК Строки
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.СтатьиРасходов КАК СтатьиСтроительства
...
Функция ТекстОтраженияАмортизационнаяПремия()
	...
	|	
	|	""Амортизационная премия"" КАК Содержание,
	|	""УчетОСВызовСервера_1343"" КАК НомерФрагмента
	|ИЗ
	|	втНачисленнаяАмортизация КАК Строки
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.СтатьиРасходов КАК СтатьиСтроительства
...
Функция ТекстОтраженияАмортизационнаяПремияБезНУ()
	...
	|	
	|	""Амортизационная премия"" КАК Содержание,
	|	""УчетОСВызовСервера_1344"" КАК НомерФрагмента
	|ИЗ
	|	втНачисленнаяАмортизация КАК Строки
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.СтатьиРасходов КАК СтатьиСтроительства
...
Функция ТекстОтраженияАмортизацияОтправитель()
	...
	|	
	|	""Начисление амортизации"" КАК Содержание,
	|	""УчетОСВызовСервера_1345"" КАК НомерФрагмента
	|ИЗ
	|	втНачисленнаяАмортизация КАК Строки
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ втМестонахожденияОС КАК МестонахождениеОС
...
Функция ТекстОтраженияАмортизацияБезНУОтправитель()
	...
	|	
	|	""Начисление амортизации"" КАК Содержание,
	|	""УчетОСВызовСервера_1346"" КАК НомерФрагмента
	|ИЗ
	|	втНачисленнаяАмортизация КАК Строки
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ втМестонахожденияОС КАК МестонахождениеОС
...
Функция ТекстОтраженияАмортизационнаяПремияОтправитель()
	...
	|	
	|	""Амортизационная премия"" КАК Содержание,
	|	""УчетОСВызовСервера_1347"" КАК НомерФрагмента
	|ИЗ
	|	втНачисленнаяАмортизация КАК Строки
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ втМестонахожденияОС КАК МестонахождениеОС
...
Функция ТекстОтраженияАмортизационнаяПремияБезНУОтправитель()
	...
	|	
	|	""Амортизационная премия"" КАК Содержание,
	|	""УчетОСВызовСервера_1348"" КАК НомерФрагмента
	|ИЗ
	|	втНачисленнаяАмортизация КАК Строки
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ втМестонахожденияОС КАК МестонахождениеОС
...
Функция ТекстОтраженияАмортизацияПолучатель()
	...
	|	
	|	""Начисление амортизации"" КАК Содержание,
	|	""УчетОСВызовСервера_1349"" КАК НомерФрагмента
	|ИЗ
	|	втНачисленнаяАмортизация КАК Строки
	|	
...
Функция ТекстОтраженияАмортизацияБезНУПолучатель()
	...
	|	
	|	""Начисление амортизации"" КАК Содержание,
	|	""УчетОСВызовСервера_1350"" КАК НомерФрагмента
	|ИЗ
	|	втНачисленнаяАмортизация КАК Строки
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Организации КАК ОрганизацияПолучатель
...
Функция ТекстОтраженияАмортизационнаяПремияПолучатель()
	...
	|	
	|	""Амортизационная премия"" КАК Содержание,
	|	""УчетОСВызовСервера_1351"" КАК НомерФрагмента
	|ИЗ
	|	втНачисленнаяАмортизация КАК Строки
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Организации КАК ОрганизацияПолучатель
...
Функция ТекстОтраженияАмортизационнаяПремияБезНУПолучатель()
	...
	|	
	|	""Амортизационная премия"" КАК Содержание,
	|	""УчетОСВызовСервера_1352"" КАК НомерФрагмента
	|ИЗ
	|	втНачисленнаяАмортизация КАК Строки
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Организации КАК ОрганизацияПолучатель
...
Функция ТекстОтраженияАмортизацияНаПрочиеОперации()
	...
	|	
	|	""Начисление амортизации"" КАК Содержание,
	|	""УчетОСВызовСервера_1353"" КАК НомерФрагмента
	|ИЗ
	|	втНачисленнаяАмортизация КАК Строки
	|	
...
Функция ТекстОтраженияАмортизацияБезНУНаПрочиеОперации()
	...
	|	
	|	""Начисление амортизации"" КАК Содержание,
	|	""УчетОСВызовСервера_1354"" КАК НомерФрагмента
	|ИЗ
	|	втНачисленнаяАмортизация КАК Строки
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.СтатьиРасходов КАК СтатьиНаПрочиеАктивы
...
Функция ТекстОтраженияАмортизационнаяПремияНаПрочиеОперации()
	...
	|	
	|	""Амортизационная премия"" КАК Содержание,
	|	""УчетОСВызовСервера_1355"" КАК НомерФрагмента
	|ИЗ
	|	втНачисленнаяАмортизация КАК Строки
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.СтатьиРасходов КАК СтатьиНаПрочиеАктивы
...
Функция ТекстОтраженияАмортизационнаяПремияБезНУНаПрочиеОперации()
	...
	|	
	|	""Амортизационная премия"" КАК Содержание,
	|	""УчетОСВызовСервера_1356"" КАК НомерФрагмента
	|ИЗ
	|	втНачисленнаяАмортизация КАК Строки
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.СтатьиРасходов КАК СтатьиНаПрочиеАктивы
...
Процедура ВременнаяТаблицаСтоимостиУпрПрочиеАктивыДанныеДляГруппировки(ТекстыЗапроса)
	...
	|			Организация = &Организация
	|				И Статья В (
	|					ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.УпрБалансНематериальныеАктивы),
...
Функция ПолучитьТекстЗапросаАмортизацияОСПараметрыОтраженияВУчете()
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПоследних(&НачалоПериода, МестоУчета = НЕОПРЕДЕЛЕНО
	|			И ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.СтоимостьВНА)
	|			И Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)) КАК СчетУчетаСтоимости
	|		ПО СчетУчетаСтоимости.АналитикаУчета = ПорядокУчетаОС.ГруппаФинансовогоУчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПоследних(&НачалоПериода, МестоУчета = НЕОПРЕДЕЛЕНО
	|			И ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.АмортизацияВНА)
	|			И Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)) КАК СчетУчетаАмортизации
	|		ПО СчетУчетаАмортизации.АналитикаУчета = ПорядокУчетаОС.ГруппаФинансовогоУчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПоследних(&НачалоПериода, МестоУчета = НЕОПРЕДЕЛЕНО
	|			И ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.СтоимостьВНА_ЦФ)
	|			И Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)) КАК СчетУчетаСтоимостиЦФ
	|		ПО СчетУчетаСтоимостиЦФ.АналитикаУчета = ПорядокУчетаОС.ГруппаФинансовогоУчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПоследних(&НачалоПериода, МестоУчета = НЕОПРЕДЕЛЕНО
	|			И ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.АмортизацияВНА_ЦФ)
	|			И Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)) КАК СчетУчетаАмортизацииЦФ
	|		ПО СчетУчетаАмортизацииЦФ.АналитикаУчета = ПорядокУчетаОС.ГруппаФинансовогоУчета
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство
	|;
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ОСПоКоторымНачисляетсяИзнос КАК ОСПоКоторымНачисляетсяИзнос
	|		ПО ОСПоКоторымНачисляетсяИзнос.ОсновноеСредство = ПорядокУчетаОС.ОсновноеСредство
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПоследних(&НачалоГода, МестоУчета = НЕОПРЕДЕЛЕНО
	|			И Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|			И ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.СтоимостьВНА)) КАК СчетУчетаСтоимости
	|		ПО СчетУчетаСтоимости.АналитикаУчета = ПорядокУчетаОС.ГруппаФинансовогоУчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета.СрезПоследних(&НачалоГода, МестоУчета = НЕОПРЕДЕЛЕНО
