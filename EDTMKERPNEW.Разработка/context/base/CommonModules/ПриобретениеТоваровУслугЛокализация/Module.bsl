...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	|	""ПриобретениеТоваровУслугЛокализация_946"" КАК НомерФрагмента
	|
	|ИЗ
...
#Область ФактуровкаПоставкиРаботыУслуги                 // (Дт 60.НП :: Кт 60, 76)
	...
	|	""ПриобретениеТоваровУслугЛокализация_947"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|ГДЕ
	|	НЕ ЕстьNULL(Склады.ЦеховаяКладовая, Ложь)
	|	И Расчеты.Договор.ВариантОформленияДокументовПоступленияПоУслугам = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется)
...
#Область ОтклоненияПриФактуровке            // (Дт 10, 20, 41 :: Кт 60.НП)
	...
	|	""ПриобретениеТоваровУслугЛокализация_948"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|	""ПриобретениеТоваровУслугЛокализация_949"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТоварыВПути	                        // (Дт 15 :: Кт 60, 76)
	...
	|	""ПриобретениеТоваровУслугЛокализация_950"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ЗакупкаЦенности                    // (Дт 10, 41, 76 :: Кт 60, 71, 76)
	...
	|	""ПриобретениеТоваровУслугЛокализация_951"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ЗакупкаЦенностиСоСписаниемНаРасходы    // (Дт 2Х, 44, 91 :: Кт 60, 71, 76)
	...
	|	Операция.Организация КАК Организация,
	|	ВЫБОР
	|		КОГДА Строки.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
	|			И Операция.ВернутьМногооборотнуюТару И НЕ Операция.ТребуетсяЗалогЗаТару
	|			ТОГДА Строки.ИдентификаторСтроки
	|		КОГДА Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаЧерезПодотчетноеЛицо)
	|			ТОГДА Строки.ИдентификаторСтроки
	|		ИНАЧЕ Строки.ОбъектРасчетов.УникальныйИдентификатор
	...
	|					И ЕСТЬNULL(УчетнаяПолитикаОрганизаций.СписыватьНДСПоРасходамНеПринимаемымВНУ, ЛОЖЬ)
	|			ТОГДА Суммы.СуммаПриведеннаяРегл
	|		КОГДА Операция.ЗакупкаПодДеятельность В (&ТипыНеОблагаемыеНДС)
	...
	|	""ПриобретениеТоваровУслугЛокализация_952"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ЗакупкаУслуги                      // (Дт 2Х, 44, 91 :: Кт 60, 71, 76)
	...
	|				ИЛИ НЕ СтатьиРасходов.ПринятиеКНалоговомуУчету
	|					И ЕСТЬNULL(УчетнаяПолитикаОрганизаций.СписыватьНДСПоРасходамНеПринимаемымВНУ, ЛОЖЬ)
	|			ТОГДА Суммы.СуммаПриведеннаяРегл
	|		КОГДА ВЫБОР КОГДА ЕСТЬNULL(Строки.Назначение.ВидДеятельностиНДС,&ПустойТипНалогообложения) <> &ПустойТипНалогообложения
	|				ТОГДА Строки.Назначение.ВидДеятельностиНДС
	...
	|	""ПриобретениеТоваровУслугЛокализация_1799"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|		(ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар),ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
	|	И НЕ (Расчеты.Договор.ВариантОформленияДокументовПоступленияПоУслугам = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется)
	|		И Операция.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки),
	|										ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСФактуровкаПоставки)))
	|	И (Операция.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщика)
	|		, ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаРеглУчет)
...
#Область ЗакупкаНДС                         // (Дт 19 :: Кт 60, 71, 76)
	...
	|	""ПриобретениеТоваровУслугЛокализация_953"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""НДС при приобретении"" КАК Содержание,
	|	""ПриобретениеТоваровУслугЛокализация_954"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстПроцентыПоДисконтированию()
	...
	|	""ПриобретениеТоваровУслугЛокализация_955"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстПодотчетникиЗакупкаЦенности()
	...
	|	КОНЕЦ КАК Содержание,
	|	""ПриобретениеТоваровУслугЛокализация_1800"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстПодотчетникиСписаниеНаРасходы()
	...
	|	РегистрПрочиеРасходы.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|
	...
	|	""ПриобретениеТоваровУслугЛокализация_956"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстПодотчетникиЗакупкаНДС()
	...
	|	0 КАК СуммаВРКт,
	|	""НДС при приобретении"" КАК Содержание,
	|	""ПриобретениеТоваровУслугЛокализация_957"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
