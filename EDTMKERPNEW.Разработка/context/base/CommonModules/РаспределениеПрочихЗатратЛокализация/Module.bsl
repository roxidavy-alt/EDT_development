...
#Область ТекстРаспределениеРасходовНаПартии //(Дт 20 :: Кт 2х.хх)
	...
	|	""РаспределениеПрочихЗатратЛокализация_1056"" КАК НомерФрагмента
	|ИЗ
...
#Область ТекстРаспределениеРасходов //(Дт 2х.хх :: Кт 2х.хх)
	...
	|	""Распределение расходов"" КАК Содержание,
	|	""РаспределениеПрочихЗатратЛокализация_1057"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РаспределениеПрочихЗатрат КАК Операция
	|		ПО ДокументыКОтражению.Ссылка = Операция.Ссылка
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПрочиеРасходы_Предварительная КАК ПрочиеРасходы
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПрочиеРасходы_Предварительная КАК КорПрочиеРасходы
...
#Область ТекстСписаниеНаСтатьиАктивовПассивов //(Дт ХХ.ХХ :: Кт 20)
	...
	|	""РаспределениеПрочихЗатратЛокализация_1058"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		ПрочиеРасходы_Предварительная КАК Строки
	|	ПО
...
#Область ТекстСписаниеКосвенныхРасходовОСНО
	...
	|	""РаспределениеПрочихЗатратЛокализация_1059"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
#Область ТекстСписаниеКосвенныхРасходовЕНВД
	...
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.РасходыТекущегоПериодаПСН) КАК ВидСчетаДт,
	|	КосвенныеРасходы.СтатьяРасходов КАК АналитикаУчетаДт,
	|	КосвенныеРасходы.Подразделение КАК МестоУчетаДт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаДт,
	...
	|	""РаспределениеПрочихЗатратЛокализация_1060"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
#Область ТекстВключениеИсключениеНДС // Дт 20/41/26,44,91 :: Кт 19.03/19.11
	...
	|	""РаспределениеПрочихЗатратЛокализация_1823"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстВключениеИсключениеНДС_НУ // Дт 20/41/26,44,91 :: Кт 19.03/19.11
	...
	|		ИНАЧЕ ""Включение/исключение НДС в расходы""
	|	КОНЕЦ КАК Содержание,
	|	""РаспределениеПрочихЗатратЛокализация_1824"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область РаспределениеНормируемыхРасходовПоНУ
	...
	|	""РаспределениеПрочихЗатратЛокализация_1061"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
#Область ТекстОтклонениеВСтоимости
	...
	|	""РаспределениеПрочихЗатратЛокализация_1062"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		ПрочиеРасходы_Предварительная КАК СтрокиКт
	|	ПО
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	|	Движения.НастройкаХозяйственнойОперации КАК НастройкаХозяйственнойОперации,
	...
	|		ПО ДокументыКОтражению.Ссылка = Движения.Регистратор
	|			И (Движения.Активность)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка,
