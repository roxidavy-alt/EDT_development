	...
	ИначеЕсли Форма.ДокументыОснования.Количество() > 1 Тогда
		...
		ИначеЕсли РасхожденияКлиентСервер.ТипОснованияОтгрузкаТоваровКлиенту(Форма.Объект.ТипОснованияАктаОРасхождении) Тогда
			ЗаголовокФормыПросмотра = НСтр("ru = 'Отгрузки товаров клиенту (%КоличествоДокументов%)';
											|en = 'Goods shipments to customer (%КоличествоДокументов%)'");
		...
		ИначеЕсли Форма.Объект.ТипОснованияАктаОРасхождении =
			...
			ЗаголовокФормыПросмотра = НСтр("ru = 'Возвраты давальцу (%КоличествоДокументов%)';
											|en = 'Goods return — Subcontracting services delivered (%КоличествоДокументов%)'");
		//-- НЕ УТКА
...
Процедура ПоказатьПредупреждениеДляЗаполненныхНаОснованииСтрок() Экспорт
	...
	ТекстСообщения = НСтр("ru = 'Для строк, заполненных по основанию, возможно редактирование колонок ""Фактическое количество"", ""Номер паспорта"".
		...
		|en = 'For lines filled from the base document, you can edit only the ""Actual quantity"" and ""Certificate number"" columns.
		|The ""Amount (actual)"", ""VAT (actual)"" and ""Discrepancy treatment"" columns can be edited after discrepancies in quantity are recorded.'");
