...
Функция ИсточникиСуммыПравила(СхемаКомпоновкиДанных, Правило) Экспорт
	...
	Если РеквизитыПравила.РазделИсточникаДанных = Перечисления.РазделыИсточниковДанныхБюджетирования.ОперативныйУчет Тогда
		Если ПолучитьФункциональнуюОпцию("НастройкаПравилПолученияФактаПоСтатьямАктивовПассивов") Тогда
			ПравилоПоСтатьеАктивовПассивов = ЭтоСтатьяАктивовПассивов(РеквизитыПравила.ИсточникДанных);
		Иначе
			ПравилоПоСтатьеАктивовПассивов = Ложь;
		КонецЕсли;
		Если ПравилоПоСтатьеАктивовПассивов Тогда
			ПоказателиРегистра = ИсточникиДанныхПовтИсп.ПоказателиСуммыСтатьиАктивовПассивов(РеквизитыПравила.ИсточникДанных);
		Иначе
		...
		Для каждого Ресурс Из РесурсыИсточникаСуммы Цикл
			Если ПравилоПоСтатьеАктивовПассивов Тогда
				ПолеСуммы = БюджетированиеСервер.ПолеРесурсаБухгалтерии(Ресурс["Имя"], РеквизитыПравила.ТипИтога);
			Иначе
...
Функция СхемаКомпоновкиДанныхПравила(ОписаниеПравила, ОчищаемыеКоллекцииНастроек = Неопределено, РежимПолученияДанных = Неопределено, ПолучатьКопиюДляИзменения = Ложь, ТаблицаКорреспонденций = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ЭтоСсылка(ТипОписанияПравила) Тогда
		Если ТипОписанияПравила = Тип("СправочникСсылка.ПравилаПолученияФактаПоСтатьямБюджетов") Тогда
			РеквизитыПравила = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОписаниеПравила,
				"Ссылка, РазделИсточникаДанных, ИсточникДанных, КорИсточникДанных, ХешСхемыКомпоновкиДанных, ИсточникСуммыОперации, ТипИтога, ТипПравила");
		Иначе
			РеквизитыПравила = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОписаниеПравила,
				"Ссылка, РазделИсточникаДанных, ИсточникДанных, ХешСхемыКомпоновкиДанных, ИсточникСуммыОперации");
		КонецЕсли;
	...
	ИначеЕсли ТипОписанияПравила = Тип("ДанныеФормыСтруктура") 
		...
		РеквизитыПравила = Новый Структура(
			"Ссылка, РазделИсточникаДанных, ИсточникДанных, КорИсточникДанных, ХешСхемыКомпоновкиДанных, ИсточникСуммыОперации, ТипИтога, ТипПравила");
	...
	Если РеквизитыПравила.РазделИсточникаДанных = Перечисления.РазделыИсточниковДанныхБюджетирования.ОперативныйУчет Тогда
		...
		Если ИмяСправочникаИсточника = "ПравилаПолученияФактаПоСтатьямБюджетов"
			И (Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(РеквизитыПравила, "ТипДанныхУчетаПриход")
			Или Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(РеквизитыПравила, "ТипДанныхУчетаРасход")) Тогда
			Если ЭтоНастройкаХозяйственнойОперации(РеквизитыПравила.ИсточникДанных) Тогда
			...
			Иначе
				...
				Если ЗначениеЗаполнено(РеквизитыПравила.ИсточникДанных) Тогда
					ИсточникиДанных.Добавить(РеквизитыПравила.ИсточникДанных);
				КонецЕсли;
				Если ЗначениеЗаполнено(РеквизитыПравила.КорИсточникДанных) Тогда
					ИсточникиДанных.Добавить(РеквизитыПравила.КорИсточникДанных);
				КонецЕсли;
				Если ИсточникиДанных.Количество() > 0 Тогда
					ТипыДанныхУчета = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ИсточникиДанных, "ТипДанныхУчета");
				КонецЕсли;
				Если РеквизитыПравила.ТипИтога = Перечисления.ТипыИтогов.ОборотКт Тогда
					ТипДанныхУчетаРасход = ?(
						ЗначениеЗаполнено(РеквизитыПравила.ИсточникДанных),
						ТипыДанныхУчета[РеквизитыПравила.ИсточникДанных],
						Перечисления.ТипыДанныхУчета.ПустаяСсылка());
					ТипДанныхУчетаПриход = ?(
						ЗначениеЗаполнено(РеквизитыПравила.КорИсточникДанных),
						ТипыДанныхУчета[РеквизитыПравила.КорИсточникДанных],
						Перечисления.ТипыДанныхУчета.ПустаяСсылка());
				Иначе
					ТипДанныхУчетаПриход = ?(
						ЗначениеЗаполнено(РеквизитыПравила.ИсточникДанных),
					...
					ТипДанныхУчетаРасход = ?(
						...
						Перечисления.ТипыДанныхУчета.ПустаяСсылка());
				КонецЕсли;
				РеквизитыПравила.Вставить("ТипДанныхУчетаПриход", ТипДанныхУчетаПриход);
				РеквизитыПравила.Вставить("ТипДанныхУчетаРасход", ТипДанныхУчетаРасход);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	Если ТаблицаКорреспонденций <> Неопределено Тогда
		Если ЗначениеЗаполнено(ОписаниеПравила.ИсточникДанных) Тогда
			СхемаКомпоновкиДанныхПравила = СхемаКомпоновкиДанныхПоТаблицеКорреспонденцийРегистров(
	...
	ИначеЕсли Не ПроизвольныеДанныеВИсточнике Тогда
		...
		Если ИмяСправочникаИсточника = "ПравилаПолученияФактаПоСтатьямБюджетов" Тогда
			Если РеквизитыПравила.РазделИсточникаДанных = Перечисления.РазделыИсточниковДанныхБюджетирования.ОперативныйУчет
				И РеквизитыПравила.ТипПравила = Перечисления.ТипПравилаПолученияФактическихДанныхБюджетирования.ИсполнениеБюджета Тогда
		...
		ИначеЕсли Справочники[ИмяСправочникаИсточника].ПравилоПоСтатьеАктивовПассивов(РеквизитыПравила) Тогда
			Если НЕ ЗначениеЗаполнено(ОписаниеПравила.ИсточникДанных) Тогда
				СхемаКомпоновкиДанныхПравила = Неопределено;
			ИначеЕсли ИмяСправочникаИсточника = "ПравилаПолученияФактаПоПоказателямБюджетов" Тогда
				Если ПланыВидовХарактеристик.СтатьиАктивовПассивов.ЭтоПользовательскаяСтатья(РеквизитыПравила.ИсточникДанных) Тогда
				...
				Иначе
					СхемаКомпоновкиДанныхПравила = СхемаКомпоновкиДанныхФактаПоказателяПоСтатьеАктивовПассивов(РеквизитыПравила);
				КонецЕсли;
			ИначеЕсли НЕ ЗначениеЗаполнено(РеквизитыПравила.КорИсточникДанных) Тогда
				СхемаКомпоновкиДанныхПравила = СхемаКомпоновкиДанныхФактаСтатьиПоСтатьеАктивовПассивов(РеквизитыПравила);
			Иначе
				СхемаКомпоновкиДанныхПравила = СхемаКомпоновкиДанныхФактаСтатьиПоКорреспонденцииСтатейАктивовПассивов(РеквизитыПравила);
			КонецЕсли;
		Иначе
			СхемаКомпоновкиДанныхПравила = ИсточникиДанныхПовтИсп.СхемаКомпоновкиДанныхПравила(ИмяСправочникаИсточника,
				РеквизитыПравила.РазделИсточникаДанных, РеквизитыПравила.ИсточникДанных);
	...
	Если СхемаКомпоновкиДанныхПравила = Неопределено Тогда
		Если РеквизитыПравила.РазделИсточникаДанных = Перечисления.РазделыИсточниковДанныхБюджетирования.ОперативныйУчет
			...
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				?(ТипОписанияПравила = Тип("СтрокаТаблицыЗначений"), РеквизитыПравила.Правило, РеквизитыПравила.Ссылка),
...
//    * ДополнительныеДанные             - Структура - дополнительные данные, которые будут присоединяться к фактическим данным:
...
//    * ДополнительныеФильтрыПоАналитикам- Структура - поля глобального отбора на аналитики 1..6, на основании которого формируются настройки:
//      ** Ключ     - Строка - имя поля, по которому требуется применить доп. отбор
//      ** Значение - Произвольный - значение доп. отбора
//    * ДополнительныеФильтрыПоИзмерениям- Структура - поля глобального отбора на измерения, на основании которого формируются настройки:
//      ** Ключ     - Строка - имя поля, по которому требуется применить доп. отбор
//      ** Значение - Произвольный - значение доп. отбора
//    * ИндексыТаблиц                    - Соответствие - имена таблиц значений параметров получения факта и массивы включенных в них индексах:
...
// Возвращает массив видов документов по источнику данных
//
// Параметры:
...
//
Функция МассивВидовДокументовПоИсточникуДанных(ИсточникДанных) Экспорт
	
	ТипыДанныхУчета = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ИсточникДанных, "Приход, Расход");
	
	МассивВидовДокументовПриход = ОбщегоНазначения.СкопироватьРекурсивно(
		ИсточникиДанныхПовтИсп.МассивВидовДокументовПоТипуДанныхУчета(ТипыДанныхУчета.Приход));
	МассивВидовДокументовРасход = ОбщегоНазначения.СкопироватьРекурсивно(
		ИсточникиДанныхПовтИсп.МассивВидовДокументовПоТипуДанныхУчета(ТипыДанныхУчета.Расход));
	Если Не ЗначениеЗаполнено(ТипыДанныхУчета.Приход) Тогда
		Возврат МассивВидовДокументовРасход;
	ИначеЕсли Не ЗначениеЗаполнено(ТипыДанныхУчета.Расход) Тогда
		Возврат МассивВидовДокументовПриход;
	КонецЕсли;
	
	НесовпадающиеВидыДокументов = ОбщегоНазначенияКлиентСервер.РазностьМассивов(МассивВидовДокументовПриход,
		МассивВидовДокументовРасход);
	
	Счетчик = 0;
	Пока Счетчик < МассивВидовДокументовПриход.Количество() Цикл
		Если НесовпадающиеВидыДокументов.Найти(МассивВидовДокументовПриход[Счетчик]) <> Неопределено Тогда
			МассивВидовДокументовПриход.Удалить(Счетчик);
		Иначе
			Счетчик = Счетчик + 1;
		КонецЕсли;
	КонецЦикла;
	...
	Если ТипыДанныхУчета.Приход = Перечисления.ТипыДанныхУчета.ПрочиеАктивыПассивы
		...
		МассивВидовДокументовПриход.Добавить(ОбщегоНазначения.ИдентификаторОбъектаМетаданных(
			Метаданные.Документы.ДвижениеПрочихАктивовПассивов));
	КонецЕсли;
...
Функция ДанныеДляЗапросаКИсточникуФактаПоПравилу(ОписаниеПравила, НастройкиСКДИсточника) Экспорт
	...
	// Отбор
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ОписаниеПравила, "КомпоновщикНастроек") Тогда
...
// Возвращает данные для формирования текста запроса к остаткам (оборотам) показателя (статьи) бюджетов
//
// Параметры:
//  ОписаниеПравила - Структура - параметры правила расчета остатков (оборотов) бюджетов:
//   *НастройкиЗаполненияАналитик - ТаблицаЗначений - настройки заполнения аналитик показателя (статьи) приемника
//   *СтатьяБюджетов - СправочникСсылка.СтатьиБюджетов - статья бюджета правила
//  НастройкиСКДИсточника - НастройкиКомпоновкиДанных - настройки по умолчанию СКД источника фактических данных
//  ВыражениеПериодСмещения - Строка - текст выражения для определения количества периодов смещения
//
// Возвращаемое значение:
//  Структура - структура данных запроса:
//   *ФрагментыТекстаЗапроса - Структура - фрагменты текста запроса:
...
//    **ПараметрGUID - Произвольный - значение параметра запроса
//
Функция ДанныеДляЗапросаКИсточникуПоНастройкамРасчетаДанных(ОписаниеПравила, НастройкиСКДИсточника,
	ВыражениеПериодСмещения) Экспорт
	
	ФрагментыТекстаЗапроса = Новый Структура("ТекстПолейАналитик, ТекстУсловия, ТекстСоединений", "", "", "");
	Если Не ПустаяСтрока(ВыражениеПериодСмещения) Тогда
		ФрагментыТекстаЗапроса.Вставить("ВыражениеПериодСмещения", "");
	КонецЕсли;
	ДанныеДляЗапроса = Новый Структура("ФрагментыТекстаЗапроса, ПараметрыЗапроса", ФрагментыТекстаЗапроса, Новый Структура);
	...
	ОписанияТиповАналитик = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОписаниеПравила.Приемник, СтрСоединить(
		РеквизитыОписанияТиповАналитик, ","));
	
	ДеревоРеквизитовПолейИсточника = РаботаСФормулами.ПолучитьПустоеДеревоОперандов();
	
	// Выражение заполнения аналитик
	ШаблонВыраженияЗначенияАналитики = "ВЫБОР
		|	КОГДА НЕ ТИПЗНАЧЕНИЯ(%1) В (%2)
		|		ТОГДА %3
		|	ИНАЧЕ %1
	...
	// Выражения соответствия значений аналитик
	ШаблонВыраженияСоответствияЗначенийАналитик = "ВЫБОР ПсевдонимТаблицыИсточника.Аналитика%1
		|	%2
		|	ИНАЧЕ %3
		|КОНЕЦ"; // @query-part
	ШаблонВыраженияЭлементаСоответствия = "КОГДА %1 ТОГДА %2"; // @query-part
	...
	Для НомерАналитики = 1 По МаксимальноеКоличествоАналитик Цикл
		...
		Если Не ЗначениеЗаполнено(ВидАналитики) Тогда
			Выражение = БюджетированиеСервер.ВыражениеПустогоЗначенияАналитики();
		Иначе
			СтрокаВидАналитики = ОписаниеПравила.НастройкиЗаполненияАналитик.Найти(ВидАналитики, "ВидАналитики");
			Если СтрокаВидАналитики = Неопределено Тогда
				ТекстОшибки = НСтр(
					"ru = 'В настройках заполнения аналитик правила %1 не найден вид аналитики %2. Необходимо перевыбрать приемник в правиле.';
					|en = 'The %2 dimension kind is not found in the dimension population settings of the %1 rule. Reselect the destination in the rule.'");
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
					ОписаниеПравила.Наименование, ВидАналитики);
			КонецЕсли;
			Если СтрокаВидАналитики.СпособЗаполнения
				= Перечисления.СпособыЗаполненияАналитикСтатейБюджетов.ИзИсточникаДанных Тогда
				Выражение = СтрокаВидАналитики.ВыражениеЗаполненияАналитики;
				Если ПустаяСтрока(Выражение) Тогда
					Выражение = БюджетированиеСервер.ВыражениеПустогоЗначенияАналитики();
				Иначе
					Выражение = ВыражениеПоляДляПомещенияВТекстЗапроса(Выражение, ДанныеДляЗапроса,
					...
					КонецЕсли;
				КонецЕсли;
			ИначеЕсли СтрокаВидАналитики.СпособЗаполнения
				= Перечисления.СпособыЗаполненияАналитикСтатейБюджетов.ПоЗаданномуСоответствию Тогда
				МассивВыраженийСоответствия = Новый Массив;
				СтрокиСоответствияЗначенийАналитик = ОписаниеПравила.СоответствиеЗначенийАналитик.НайтиСтроки(
					Новый Структура("ВидАналитики", СтрокаВидАналитики.ВидАналитики));
				Для Каждого СтрокаСоответствия Из СтрокиСоответствияЗначенийАналитик Цикл
					СтрокаЗначениеАналитикиИсточника = ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(
						СтрокаСоответствия.ЗначениеАналитикиИсточника);
					Если СтрНачинаетсяС(СтрокаЗначениеАналитикиИсточника, "Параметр") Тогда
						НайтиИДобавитьПараметрЗапроса(СтрокаЗначениеАналитикиИсточника,
							СтрокаСоответствия.ЗначениеАналитикиИсточника, ДанныеДляЗапроса.ПараметрыЗапроса);
						СтрокаЗначениеАналитикиИсточника = "&" + СтрокаЗначениеАналитикиИсточника;
					КонецЕсли;
					СтрокаЗначениеАналитикиПриемника = ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(
						СтрокаСоответствия.ЗначениеАналитики);
					Если СтрНачинаетсяС(СтрокаЗначениеАналитикиПриемника, "Параметр") Тогда
						НайтиИДобавитьПараметрЗапроса(СтрокаЗначениеАналитикиПриемника,
							СтрокаСоответствия.ЗначениеАналитики, ДанныеДляЗапроса.ПараметрыЗапроса);
						СтрокаЗначениеАналитикиПриемника = "&" + СтрокаЗначениеАналитикиПриемника;
					КонецЕсли;
					МассивВыраженийСоответствия.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						ШаблонВыраженияЭлементаСоответствия, СтрокаЗначениеАналитикиИсточника,
						СтрокаЗначениеАналитикиПриемника));
				КонецЦикла;
				Если МассивВыраженийСоответствия.Количество() = 0 Тогда
					МассивВыраженийСоответствия.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						ШаблонВыраженияЭлементаСоответствия, БюджетированиеСервер.ВыражениеПустогоЗначенияАналитики(),
						БюджетированиеСервер.ВыражениеПустогоЗначенияАналитики()));
				КонецЕсли;
				Если ЗначениеЗаполнено(ОписаниеПравила.Источник) Тогда
					НомерАналитикиИсточника = БюджетированиеПовтИсп.НомерВидаАналитикиСтатьиПоказателяБюджетов(
						ОписаниеПравила.Источник, СтрокаВидАналитики.ВидАналитикиИсточника);
					Выражение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						ШаблонВыраженияСоответствияЗначенийАналитик, НомерАналитикиИсточника, СтрСоединить(
						МассивВыраженийСоответствия, Символы.ПС + Символы.Таб),
						БюджетированиеСервер.ВыражениеПустогоЗначенияАналитики());
				Иначе
					ДоступныеТипыАналитик = БюджетированиеПовтИсп.ДоступныеТипыАналитикДляТипаИсточникаПланирования(
						ОписаниеПравила.ТипИсточника);
					ТипВидаАналитики = БюджетированиеПовтИсп.ТипыВидаАналитики(
						СтрокаВидАналитики.ВидАналитикиИсточника)[0];
					ИмяПоляАналитики = СтрЗаменить(ДоступныеТипыАналитик.Получить(ТипВидаАналитики), "ИмяТаблицы.", "");
					Выражение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрЗаменить(
						ШаблонВыраженияСоответствияЗначенийАналитик, "Аналитика%1", ИмяПоляАналитики), , СтрСоединить(
						МассивВыраженийСоответствия, Символы.ПС + Символы.Таб),
						БюджетированиеСервер.ВыражениеПустогоЗначенияАналитики());
				КонецЕсли;
			Иначе
				ЗначениеАналитики = СтрокаВидАналитики.ЗначениеАналитики;
				Выражение = ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(ЗначениеАналитики);
				Если СтрНачинаетсяС(Выражение, "Параметр") Тогда
					НайтиИДобавитьПараметрЗапроса(Выражение, ЗначениеАналитики, ДанныеДляЗапроса.ПараметрыЗапроса);
					Выражение = "&"+Выражение;
	...
	// Отбор
	Отбор = НастройкиСКДИсточника.Отбор;

	ТекстУсловия = ПреобразоватьОтборКомпоновкиДанныхВТекстУсловияЗапроса(Отбор, ДанныеДляЗапроса,
		НастройкиСКДИсточника.ДоступныеПоляОтбора, ДеревоРеквизитовПолейИсточника);
	ФрагментыТекстаЗапроса.ТекстУсловия = ?(ПустаяСтрока(ТекстУсловия), "ИСТИНА", ТекстУсловия);
	
	// Выражение смещения периода
	Если Не ПустаяСтрока(ВыражениеПериодСмещения) Тогда
		ФрагментыТекстаЗапроса.ВыражениеПериодСмещения = ВыражениеПоляДляПомещенияВТекстЗапроса(
			ВыражениеПериодСмещения, ДанныеДляЗапроса, НастройкиСКДИсточника.ДоступныеПоляВыбора,
			ДеревоРеквизитовПолейИсточника);
	КонецЕсли;
...
// Возвращает принадлежность переданного источника данных к статьям управленческого баланса.
//
// Параметры:
//  ИсточникДанных - Структура,ДанныеФормыСтруктура,
//  					ПланВидовХарактеристикОбъект.СтатьиАктивовПассивов,
//  					ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов.
//
// Возвращаемое значение:
//  Булево
//
Функция ЭтоСтатьяУправленческогоБаланса(ИсточникДанных) Экспорт
	
	ТипИсточникаДанных = ТипЗнч(ИсточникДанных);
	
	Если ТипИсточникаДанных = Тип("ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов") Тогда
		ЗначенияРеквизитовИсточника = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			ИсточникДанных, "ИмяИсточникаДанныхБюджетов,Предопределенный");
	ИначеЕсли ТипИсточникаДанных = Тип("ПланВидовХарактеристикОбъект.СтатьиАктивовПассивов") Тогда
		ЗначенияРеквизитовИсточника = ИсточникДанных;
	ИначеЕсли (ТипИсточникаДанных = Тип("Структура") ИЛИ ТипИсточникаДанных = Тип("ДанныеФормыСтруктура"))
		И ИсточникДанных.Свойство("ИмяИсточникаДанныхБюджетов") И ИсточникДанных.Свойство("Предопределенный") Тогда
		ЗначенияРеквизитовИсточника = ИсточникДанных;
	ИначеЕсли ТипИсточникаДанных = Тип("Строка") И НЕ ПустаяСтрока(ИсточникДанных) Тогда
		Запрос = Новый Запрос("
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	СтатьиАктивовПассивов.ИмяИсточникаДанныхБюджетов КАК ИмяИсточникаДанныхБюджетов,
		|	СтатьиАктивовПассивов.Предопределенный КАК Предопределенный
		|ИЗ
		...
		|	И СтатьиАктивовПассивов.ИмяИсточникаДанныхБюджетов = &ИсточникДанных
		|");
		Запрос.УстановитьПараметр("ИсточникДанных", ИсточникДанных);
		Результат = Запрос.Выполнить();
		Если НЕ Результат.Пустой() Тогда
			ЗначенияРеквизитовИсточника = Результат.Выгрузить()[0];
		Иначе
			Возврат Ложь;
		КонецЕсли;
	Иначе
	...
	Если НЕ ЗначенияРеквизитовИсточника.Предопределенный Тогда
		Возврат НЕ ПолучитьФункциональнуюОпцию("НастройкаПравилПолученияФактаПоСтатьямАктивовПассивов");
	Иначе
		Возврат ЗначениеЗаполнено(ЗначенияРеквизитовИсточника.ИмяИсточникаДанныхБюджетов);
	КонецЕсли;
	
КонецФункции

// Возвращает принадлежность переданного источника данных к статьям активов и пассивов,
// описывающих данные балансовых регистров оперативного учета.
//
// Параметры:
//  ИсточникДанных - Структура,ДанныеФормыСтруктура,
//  					ПланВидовХарактеристикОбъект.СтатьиАктивовПассивов,
...
//  Булево
//
Функция ЭтоСтатьяАктивовПассивов(ИсточникДанных) Экспорт
	
	ТипИсточникаДанных = ТипЗнч(ИсточникДанных);
	
	Если ТипИсточникаДанных = Тип("ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов") Тогда
		ЗначенияРеквизитовИсточника = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			ИсточникДанных, "ИсточникДанных,Предопределенный");
	ИначеЕсли ТипИсточникаДанных = Тип("ПланВидовХарактеристикОбъект.СтатьиАктивовПассивов") Тогда
		ЗначенияРеквизитовИсточника = ИсточникДанных;
	ИначеЕсли (ТипИсточникаДанных = Тип("Структура") ИЛИ ТипИсточникаДанных = Тип("ДанныеФормыСтруктура"))
		И ИсточникДанных.Свойство("ИсточникДанных") И ИсточникДанных.Свойство("Предопределенный") Тогда
		ЗначенияРеквизитовИсточника = ИсточникДанных;
	ИначеЕсли ТипИсточникаДанных = Тип("Строка") И НЕ ПустаяСтрока(ИсточникДанных) Тогда
		ПолноеИмяИсточника = "ПланВидовХарактеристик.СтатьиАктивовПассивов." + ИсточникДанных;
		ИсточникДанныхСсылка = ОбщегоНазначения.ПредопределенныйЭлемент(ПолноеИмяИсточника);
		Если ИсточникДанныхСсылка = Неопределено Тогда
			Возврат Ложь;
		Иначе
			Возврат ЭтоСтатьяАктивовПассивов(ИсточникДанныхСсылка);
		КонецЕсли;
	Иначе
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ ЗначенияРеквизитовИсточника.Предопределенный Тогда
		Возврат ПолучитьФункциональнуюОпцию("НастройкаПравилПолученияФактаПоСтатьямАктивовПассивов");
	Иначе
		Возврат ЗначениеЗаполнено(ЗначенияРеквизитовИсточника.ИсточникДанных);
	КонецЕсли;
	
КонецФункции

// Возвращает принадлежность переданного источника данных к справочнику хозяйственных операций.
//
// Параметры:
//  ИсточникДанных - СправочникСсылка.НастройкиХозяйственныхОпераций,
//  					СправочникОбъект.НастройкиХозяйственныхОпераций.
//
// Возвращаемое значение:
//  Булево
//
Функция ЭтоНастройкаХозяйственнойОперации(ИсточникДанных) Экспорт
	
	ТипИсточникаДанных = ТипЗнч(ИсточникДанных);
	
	Если ТипИсточникаДанных = Тип("СправочникСсылка.НастройкиХозяйственныхОпераций")
	 ИЛИ ТипИсточникаДанных = Тип("СправочникОбъект.НастройкиХозяйственныхОпераций") Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
// Возвращает имя папки полей источника данных правила получения факта по статьям бюджетов.
//
// Возвращаемое значение:
//  Строка
//
Функция ИмяПапкиПолейИсточникаДанных() Экспорт
	
	Возврат "ГруппаИсточникДанных";
	
КонецФункции

// Возвращает имя папки полей кор. источника данных правила получения факта по статьям бюджетов.
//
// Возвращаемое значение:
//  Строка
//
Функция ИмяПапкиПолейКорИсточникаДанных() Экспорт
	
	Возврат "ГруппаКорИсточникДанных";
	
КонецФункции

// Возвращает префикс имен полей кор. источника данных правила получения факта по статьям бюджетов.
//
// Возвращаемое значение:
//  Строка
//
Функция ПрефиксИменПолейКорИсточникаДанных() Экспорт
...
Функция ТаблицаКорреспонденцийРегистров(ИсточникДанных, АналитикаКорреспонденций = Неопределено) Экспорт
	...
	Иначе
		...
		Если АналитикаРасход.Найти("Подразделение") <> Неопределено Тогда
			Если ТипыДанныхУчета.Приход = Перечисления.ТипыДанныхУчета.Контрагенты
				Или ТипыДанныхУчета.Приход = Перечисления.ТипыДанныхУчета.ДенежныеСредства
				И ТипыДанныхУчета.Расход <> Перечисления.ТипыДанныхУчета.Контрагенты
				Или ТипыДанныхУчета.Приход = Перечисления.ТипыДанныхУчета.Номенклатура
				И ТипыДанныхУчета.Расход <> Перечисления.ТипыДанныхУчета.Контрагенты
				И ТипыДанныхУчета.Расход <> Перечисления.ТипыДанныхУчета.ДенежныеСредства Тогда
				АналитикаРасход.Удалить(АналитикаРасход.Найти("Подразделение"));
				АналитикаПриход.Добавить("Подразделение");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ВыражениеПоляДляПомещенияВТекстЗапроса(ИсходноеВыражение, ДанныеДляЗапроса, ДоступныеПоля, ДеревоРеквизитов)
	...
	ШаблонЭлементаСоставногоВыраженияХарактеристики = "КОГДА ТИПЗНАЧЕНИЯ(%1) = ТИП(%2)
		|	ТОГДА %3"; // @query-part
	...
	Если ЧастиВыражения.Количество() > 1 Тогда
		...
		Пока ЧастиВыражения.Количество() > 0 Цикл
			...
			Если СтрокаДерева = Неопределено Тогда
				...
				Поле = ДоступныеПоля.НайтиПоле(Новый ПолеКомпоновкиДанных(Представление));
				Если Поле <> Неопределено Тогда
					СтрокаДерева.ТипЗначения = Поле.ТипЗначения;
				КонецЕсли;
...
Функция ПреобразоватьОтборКомпоновкиДанныхВТекстУсловияЗапроса(Отбор, ДанныеДляЗапроса, ДоступныеПоля, ДеревоРеквизитов,
	...
	Для Каждого Элемент Из Отбор.Элементы Цикл
		...
		Иначе
			...
			КонецЕсли;
			Если СтрНачинаетсяС(СтрокаЗначениеПараметра, "&Параметр") Тогда
				ИмяПараметра = СтрЗаменить(СтрокаЗначениеПараметра, "&", "");
				НайтиИДобавитьПараметрЗапроса(ИмяПараметра, ЗначениеПараметра,
					ДанныеДляЗапроса.ПараметрыЗапроса);
				Если "&"+ИмяПараметра <> СтрокаЗначениеПараметра Тогда
					УсловиеПоОтбору = СтрЗаменить(УсловиеПоОтбору, СтрокаЗначениеПараметра, "&"+ИмяПараметра);
				КонецЕсли;
			КонецЕсли;
...
#Область ФормированиеСхемКомпоновкиДанныхПравилПолученияФактаПоСтатьямАктивовПассивов

Функция СхемаКомпоновкиДанныхФактаСтатьиПоКорреспонденцииСтатейАктивовПассивов(РеквизитыПравила)
	
	Если ТипЗнч(РеквизитыПравила.ИсточникДанных) <> Тип("ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов") Тогда
		ВызватьИсключение НСтр("ru = 'Недопустимый тип источника получения данных.';
								|en = 'Invalid type of data receipt source.'");
	КонецЕсли;
	
	Если ТипЗнч(РеквизитыПравила.КорИсточникДанных) <> Тип("ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов") Тогда
		ВызватьИсключение НСтр("ru = 'Недопустимый тип корреспондирующего источника получения данных.';
								|en = 'Invalid type of corresponding data source.'");
	КонецЕсли;
	
	ОписаниеИсточникаДанных = ПланыВидовХарактеристик.СтатьиАктивовПассивов.ОписаниеОбъектаФинансовогоУчета(РеквизитыПравила.ИсточникДанных);
	ОписаниеКорИсточникаДанных = ПланыВидовХарактеристик.СтатьиАктивовПассивов.ОписаниеОбъектаФинансовогоУчета(РеквизитыПравила.КорИсточникДанных);
	
	Если ОписаниеИсточникаДанных = Неопределено ИЛИ ПустаяСтрока(ОписаниеИсточникаДанных.ИсточникДанных) Тогда
		ВызватьИсключение НСтр("ru = 'Недопустимый источник получения данных.';
								|en = 'Invalid data source.'");
	КонецЕсли;
	
	Если ОписаниеКорИсточникаДанных = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Недопустимый корреспондирующий источник получения данных.';
								|en = 'Invalid corresponding data source.'");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(РеквизитыПравила.ИсточникСуммыОперации)
		И ОписаниеИсточникаДанных.Показатели[РеквизитыПравила.ИсточникСуммыОперации] = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Недопустимый источник получения суммы.';
								|en = 'Invalid amount source.'");
	КонецЕсли;
	
	Если ПустаяСтрока(ОписаниеКорИсточникаДанных.ИсточникДанных) Тогда
		СхемаКомпоновкиДанных = СхемаКомпоновкиДанныхФактаСтатьиПоСтатьеАктивовПассивов(РеквизитыПравила);
		
		КорПараметрыОтраженияВУчете = ФинансовыйУчетПоДаннымБалансовыхРегистров.ПараметрыОтраженияДвиженийВФинансовомУчете();
		КорПараметрыОтраженияВУчете.ПутьКДаннымПодразделение =
			ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(Справочники.СтруктураПредприятия.ПустаяСсылка());
		КорПараметрыОтраженияВУчете.ПутьКДаннымНаправлениеДеятельности =
			ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(Справочники.НаправленияДеятельности.ПустаяСсылка());
		КорПараметрыОтраженияВУчете.ПутьКДаннымВалюта =
			ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(Справочники.Валюты.ПустаяСсылка());
		
		КорПоляНабораДанных = Новый Структура();
		ОписаниеПолейЗапросаКорИсточника = ОписаниеПолейЗапросаИсточникаДанныхСтатьиАктивовПассивов();
		ДополнитьПоляНабораПолямиСтандартныхИзмерений(ОписаниеПолейЗапросаКорИсточника, КорПоляНабораДанных, ОписаниеКорИсточникаДанных, КорПараметрыОтраженияВУчете);
		
		ИмяПапкиПолейИсточника = ИмяПапкиПолейИсточникаДанных();
		ИмяПапкиПолейКорИсточника = ИмяПапкиПолейКорИсточникаДанных();
		
		НаборДанных = СхемаКомпоновкиДанных.НаборыДанных[0];
		ФинансоваяОтчетностьСервер.НоваяГруппаНабора(НаборДанных, ИмяПапкиПолейКорИсточника, ОписаниеКорИсточникаДанных.Представление);
		
		Для Каждого КлючИЗначение Из КорПоляНабораДанных Цикл
			ИмяПоля = КлючИЗначение.Ключ;
			ОписаниеПоля = КлючИЗначение.Значение; // см. ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных
			
			Если ОписаниеПолейЗапросаКорИсточника.БалансныеПоля.Найти(ИмяПоля) <> Неопределено
			 ИЛИ НаборДанных.Поля.Найти(ИмяПапкиПолейИсточника + "." + ИмяПоля) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			ПостфиксЗаголовковПолей = " " + НСтр("ru = '(кор.)';
												|en = '(cor.)'");
			ЗаголовокПоля = ОписаниеПоля.Заголовок + ПостфиксЗаголовковПолей;
			ПутьКДанным = ИмяПапкиПолейКорИсточника + "." + ИмяПоля;
			ТипПоля = ОписаниеПоля.Тип;
			ФинансоваяОтчетностьСервер.НовоеПолеНабора(НаборДанных, ИмяПоля, ПутьКДанным, ЗаголовокПоля, ТипПоля);
		КонецЦикла;
		
		Возврат СхемаКомпоновкиДанных;
	КонецЕсли;
	
	МенеджерИсточникаДанных = РегистрыНакопления[ОписаниеИсточникаДанных.ИсточникДанных]; // РегистрНакопленияМенеджер -
	МетаданныеИсточникаДанных = МенеджерИсточникаДанных.СоздатьНаборЗаписей().Метаданные();
	ПараметрыОтраженияВУчете = МенеджерИсточникаДанных.ПараметрыОтраженияДвиженийВФинансовомУчете(МетаданныеИсточникаДанных);
	
	МенеджерКорИсточникаДанных = РегистрыНакопления[ОписаниеКорИсточникаДанных.ИсточникДанных]; // РегистрНакопленияМенеджер -
	МетаданныеКорИсточникаДанных = МенеджерКорИсточникаДанных.СоздатьНаборЗаписей().Метаданные();
	КорПараметрыОтраженияВУчете = МенеджерКорИсточникаДанных.ПараметрыОтраженияДвиженийВФинансовомУчете(МетаданныеКорИсточникаДанных);
	
	ТекстЗапросаВременнойТаблицы = "
	|ВЫБРАТЬ
	|	ПсевдонимИсточникаДанных.Период КАК Период,
	|	&ПоляАналитики КАК ПсевдонимыПолейАналитики,
	|	&ПоляРесурсов КАК ПсевдонимыПолейРесурсов
	|ПОМЕСТИТЬ ВременнаяТаблицаИсточникаДанных
	|ИЗ
	|	&ТаблицаИсточникаДанных КАК ПсевдонимИсточникаДанных
	|ГДЕ
	|	ПсевдонимИсточникаДанных.Активность
	|	И ПсевдонимИсточникаДанных.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И &ПривилегированныйРежимИсточника = &ПривилегированныйРежим
	|{ГДЕ
	|	(""ИдентификаторИсточникаДанных"") КАК ИдентификаторИсточникаДанных,
	|	&ПоляОтбора.* КАК ПсевдонимыПолейОтбора
	|}
	|ИНДЕКСИРОВАТЬ ПО
	|	ПсевдонимыПолейАналитики
	|";
	
	ПоляВременнойТаблицыИсточника = Новый Структура();
	ОписаниеПолейЗапросаИсточника = ОписаниеПолейЗапросаИсточникаДанныхСтатьиАктивовПассивов();
	ОписаниеПолейЗапросаИсточника.ВременнаяТаблица = Истина;
	ОписаниеПолейЗапросаИсточника.Показатель = РеквизитыПравила.ИсточникСуммыОперации;
	
	ДополнитьПоляНабораПолямиСтандартныхИзмерений(ОписаниеПолейЗапросаИсточника, ПоляВременнойТаблицыИсточника, ОписаниеИсточникаДанных, ПараметрыОтраженияВУчете);
	ДополнитьПоляНабораБаланснымиПолямиИсточникаДанных(ОписаниеПолейЗапросаИсточника, ПоляВременнойТаблицыИсточника, ОписаниеИсточникаДанных, ПараметрыОтраженияВУчете);
	ДополнитьПоляНабораПолямиАналитикиИсточникаДанных(ОписаниеПолейЗапросаИсточника, ПоляВременнойТаблицыИсточника, ПараметрыОтраженияВУчете, Истина);
	ДополнитьПоляНабораРесурсамиИсточникаДанных(ОписаниеПолейЗапросаИсточника, ПоляВременнойТаблицыИсточника, ОписаниеИсточникаДанных, ПараметрыОтраженияВУчете);
	
	ПоляВременнойТаблицыКорИсточника = Новый Структура();
	ОписаниеПолейЗапросаКорИсточника = ОписаниеПолейЗапросаИсточникаДанныхСтатьиАктивовПассивов();
	ОписаниеПолейЗапросаКорИсточника.Корреспонденция = Истина;
	ОписаниеПолейЗапросаКорИсточника.ВременнаяТаблица = Истина;
	
	ДополнитьПоляНабораПолямиСтандартныхИзмерений(ОписаниеПолейЗапросаКорИсточника, ПоляВременнойТаблицыКорИсточника, ОписаниеКорИсточникаДанных, КорПараметрыОтраженияВУчете);
	ДополнитьПоляНабораБаланснымиПолямиИсточникаДанных(ОписаниеПолейЗапросаКорИсточника, ПоляВременнойТаблицыКорИсточника, ОписаниеКорИсточникаДанных, КорПараметрыОтраженияВУчете);
	ДополнитьПоляНабораПолямиАналитикиИсточникаДанных(ОписаниеПолейЗапросаКорИсточника, ПоляВременнойТаблицыКорИсточника, КорПараметрыОтраженияВУчете, Истина);
	ДополнитьПоляНабораРесурсамиИсточникаДанных(ОписаниеПолейЗапросаКорИсточника, ПоляВременнойТаблицыКорИсточника, ОписаниеКорИсточникаДанных, КорПараметрыОтраженияВУчете);
	
	ТаблицаИсточникаДанных = "РегистрНакопления." + ОписаниеИсточникаДанных.ИсточникДанных; //@query-part
	ТаблицаКорИсточникаДанных = "РегистрНакопления." + ОписаниеКорИсточникаДанных.ИсточникДанных; //@query-part
	
	ТекстыЗапросов = Новый Массив();
	Разделитель = "," + Символы.ПС +Символы.Таб;
	
	ТекстЗапросаВременнойТаблицыИсточника = СтрЗаменить(
		ТекстЗапросаВременнойТаблицы,
		"&ТаблицаИсточникаДанных",
		ТаблицаИсточникаДанных);
	ТекстЗапросаВременнойТаблицыИсточника = СтрЗаменить(
		ТекстЗапросаВременнойТаблицыИсточника,
		"&ПоляАналитики КАК ПсевдонимыПолейАналитики",
		СтрСоединить(ОписаниеПолейЗапросаИсточника.ПоляАналитики, Разделитель));
	ТекстЗапросаВременнойТаблицыИсточника = СтрЗаменить(
		ТекстЗапросаВременнойТаблицыИсточника,
		"&ПоляРесурсов КАК ПсевдонимыПолейРесурсов",
		СтрСоединить(ОписаниеПолейЗапросаИсточника.ПоляРесурсов, Разделитель));
	ТекстЗапросаВременнойТаблицыИсточника = СтрЗаменить(
		ТекстЗапросаВременнойТаблицыИсточника,
		"&ПоляОтбора.* КАК ПсевдонимыПолейОтбора",
		СтрСоединить(ОписаниеПолейЗапросаИсточника.ПоляОтбора, Разделитель));
	ТекстЗапросаВременнойТаблицыИсточника = СтрЗаменить(
		ТекстЗапросаВременнойТаблицыИсточника,
		"ПсевдонимыПолейАналитики",
		СтрСоединить(ОписаниеПолейЗапросаИсточника.БалансныеПоля, Разделитель));
	ТекстЗапросаВременнойТаблицыИсточника = СтрЗаменить(
		ТекстЗапросаВременнойТаблицыИсточника,
		"ПсевдонимИсточникаДанных",
		ОписаниеПолейЗапросаИсточника.ПсевдонимИсточникаДанных);
	
	ТекстыЗапросов.Добавить(ТекстЗапросаВременнойТаблицыИсточника);
	
	ТекстЗапросаВременнойТаблицыКорИсточника = СтрЗаменить(
		ТекстЗапросаВременнойТаблицы,
		"&ТаблицаИсточникаДанных",
		ТаблицаКорИсточникаДанных);
	ТекстЗапросаВременнойТаблицыКорИсточника = СтрЗаменить(
		ТекстЗапросаВременнойТаблицыКорИсточника,
		"ВременнаяТаблицаИсточникаДанных",
		"ВременнаяТаблицаКорИсточникаДанных");
	ТекстЗапросаВременнойТаблицыКорИсточника = СтрЗаменить(
		ТекстЗапросаВременнойТаблицыКорИсточника,
		"&ПоляАналитики КАК ПсевдонимыПолейАналитики",
		СтрСоединить(ОписаниеПолейЗапросаКорИсточника.ПоляАналитики, Разделитель));
	ТекстЗапросаВременнойТаблицыКорИсточника = СтрЗаменить(
		ТекстЗапросаВременнойТаблицыКорИсточника,
		"&ПоляРесурсов КАК ПсевдонимыПолейРесурсов",
		СтрСоединить(ОписаниеПолейЗапросаКорИсточника.ПоляРесурсов, Разделитель));
	ТекстЗапросаВременнойТаблицыКорИсточника = СтрЗаменить(
		ТекстЗапросаВременнойТаблицыКорИсточника,
		"&ПоляОтбора.* КАК ПсевдонимыПолейОтбора",
		СтрСоединить(ОписаниеПолейЗапросаКорИсточника.ПоляОтбора, Разделитель));
	ТекстЗапросаВременнойТаблицыКорИсточника = СтрЗаменить(
		ТекстЗапросаВременнойТаблицыКорИсточника,
		"ПсевдонимыПолейАналитики",
		СтрСоединить(ОписаниеПолейЗапросаКорИсточника.БалансныеПоля, Разделитель));
	ТекстЗапросаВременнойТаблицыКорИсточника = СтрЗаменить(
		ТекстЗапросаВременнойТаблицыКорИсточника,
		"ПсевдонимИсточникаДанных",
		ОписаниеПолейЗапросаКорИсточника.ПсевдонимИсточникаДанных);
	
	ТекстыЗапросов.Добавить(ТекстЗапросаВременнойТаблицыКорИсточника);
	
	ТекстЗапросаКорреспонденцииИсточников = "
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	&СтатьяАктивовПассивов КАК ИсточникДанных,
	|	&КорСтатьяАктивовПассивов КАК КорИсточникДанных,
	|	&ПоляПериодов КАК ПсевдонимыПолейПериодов,
	|	&ПоляАналитики КАК ПсевдонимыПолейАналитики,
	|	&ПоляРесурсов КАК ПсевдонимыПолейРесурсов
	|{ВЫБРАТЬ
	|	ИсточникДанных,
	|	КорИсточникДанных,
	|	ПсевдонимыПолейПериодов.*,
	|	ПсевдонимыПолейАналитики.*,
	|	ПсевдонимыПолейРесурсов.*
	|}
	|ИЗ
	|	ВременнаяТаблицаИсточникаДанных КАК ТаблицаИсточникаДанных
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаКорИсточникаДанных КАК ТаблицаКорИсточникДанных
	|	ПО ТаблицаИсточникаДанных.Период = ТаблицаКорИсточникДанных.Период
	|	 И ТаблицаИсточникаДанных.Регистратор = ТаблицаКорИсточникДанных.Регистратор
	|	 И ТаблицаИсточникаДанных.Организация = ТаблицаКорИсточникДанных.Организация
	|	 И ТаблицаИсточникаДанных.ИдентификаторФинЗаписи = ТаблицаКорИсточникДанных.ИдентификаторФинЗаписи
	|	 И ВЫБОР
	|		КОГДА ТаблицаИсточникаДанных.НастройкаХозяйственнойОперации <> ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ПустаяСсылка)
	|			И ТаблицаКорИсточникДанных.НастройкаХозяйственнойОперации <> ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ПустаяСсылка)
	|			ТОГДА ТаблицаИсточникаДанных.НастройкаХозяйственнойОперации = ТаблицаКорИсточникДанных.НастройкаХозяйственнойОперации
	|		ИНАЧЕ ИСТИНА
	|	 КОНЕЦ
	|	 И ТаблицаИсточникаДанных.Сторно = ТаблицаКорИсточникДанных.Сторно
	|ГДЕ
	...
	|}
	|СГРУППИРОВАТЬ ПО
	|	&ПоляГруппировкиПериодов,
	|	&ПоляГруппировкиАналитики
	|";
	
	ОписаниеПолейЗапроса = ОписаниеПолейЗапросаИсточникаДанныхСтатьиАктивовПассивов();
	ОписаниеПолейЗапроса.ПсевдонимИсточникаДанных = "ТаблицаИсточникаДанных";
	ПоляНабораДанных = ПоляНабораСхемыКомпоновкиДанныхПоСтатьеАктивовПассивов(Истина);
	
	Для Каждого ИмяПоля Из ОписаниеПолейЗапросаИсточника.ПсевдонимыПолейРесурсов Цикл
		ПутьКДанным = ФинансовыйУчетПоДаннымБалансовыхРегистров.ПутьКДаннымСПсевдонимомИсточника(
			ИмяПоля, ОписаниеПолейЗапроса.ПсевдонимИсточникаДанных);
		ПолеИсточникаДанных = "СУММА(" + ПутьКДанным + ")"; //@query-part
		ОписаниеПолейЗапроса.ПоляРесурсов.Добавить(ПолеИсточникаДанных + " КАК " + ИмяПоля); //@query-part
		ОписаниеПолейЗапроса.ПсевдонимыПолейРесурсов.Добавить(ИмяПоля);
		ПоляНабораДанных.Вставить(ИмяПоля, ОбщегоНазначения.СкопироватьРекурсивно(ПоляВременнойТаблицыИсточника[ИмяПоля]));
		ПоляВременнойТаблицыИсточника.Удалить(ИмяПоля);
	КонецЦикла;
	
	Для Каждого КлючИЗначение Из ПоляВременнойТаблицыИсточника Цикл
		ИмяПоля = КлючИЗначение.Ключ;
		ОписаниеПоля = КлючИЗначение.Значение; // см. ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных
		ОписаниеПоля.ПутьКДанным = ИмяПоля;
	КонецЦикла;
	
	ДополнитьПоляНабораПолямиИсточникаДанных(ОписаниеПолейЗапроса, ПоляНабораДанных, ПоляВременнойТаблицыИсточника);
	ДополнитьПоляНабораПолямиПериодов(ОписаниеПолейЗапроса, ПоляНабораДанных, ОписаниеИсточникаДанных, ПараметрыОтраженияВУчете);
	
	Для Каждого ИмяПоля Из ОписаниеПолейЗапросаКорИсточника.ПсевдонимыПолейРесурсов Цикл
		ПоляВременнойТаблицыКорИсточника.Удалить(ИмяПоля);
	КонецЦикла;
	
	Для Каждого БалансноеПоле Из ОписаниеПолейЗапроса.БалансныеПоля Цикл
		Если ПоляВременнойТаблицыКорИсточника.Свойство(БалансноеПоле) Тогда
			ПоляВременнойТаблицыКорИсточника.Удалить(БалансноеПоле);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого КлючИЗначение Из ПоляВременнойТаблицыКорИсточника Цикл
		ИмяПоля = КлючИЗначение.Ключ;
		ОписаниеПоля = КлючИЗначение.Значение; // см. ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных
		ОписаниеПоля.ПутьКДанным = ИмяПоля;
	КонецЦикла;
	
	ОписаниеПолейЗапроса.ПсевдонимИсточникаДанных = "ТаблицаКорИсточникДанных";
	ДополнитьПоляНабораПолямиИсточникаДанных(ОписаниеПолейЗапроса, ПоляНабораДанных, ПоляВременнойТаблицыКорИсточника);
	
	СтрокаВыборкиИсточникаДанных = ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(РеквизитыПравила.ИсточникДанных);
	ВыборкаИсточникаЧерезПараметр = СтрНачинаетсяС(СтрокаВыборкиИсточникаДанных, "Параметр");
	
	СтрокаВыборкиКорИсточникаДанных = ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(РеквизитыПравила.КорИсточникДанных);
	ВыборкаКорИсточникаЧерезПараметр = СтрНачинаетсяС(СтрокаВыборкиКорИсточникаДанных, "Параметр");
	
	ТекстЗапросаКорреспонденцииИсточников = СтрЗаменить(
		ТекстЗапросаКорреспонденцииИсточников,
		"&ПоляПериодов КАК ПсевдонимыПолейПериодов",
		СтрСоединить(ОписаниеПолейЗапроса.ПоляПериодов, Разделитель));
	ТекстЗапросаКорреспонденцииИсточников = СтрЗаменить(
		ТекстЗапросаКорреспонденцииИсточников,
		"&ПоляАналитики КАК ПсевдонимыПолейАналитики",
		СтрСоединить(ОписаниеПолейЗапроса.ПоляАналитики, Разделитель));
	ТекстЗапросаКорреспонденцииИсточников = СтрЗаменить(
		ТекстЗапросаКорреспонденцииИсточников,
		"&ПоляРесурсов КАК ПсевдонимыПолейРесурсов",
		СтрСоединить(ОписаниеПолейЗапроса.ПоляРесурсов, Разделитель));
	ТекстЗапросаКорреспонденцииИсточников = СтрЗаменить(
		ТекстЗапросаКорреспонденцииИсточников,
		"ПсевдонимыПолейПериодов.*",
		СтрСоединить(ОписаниеПолейЗапроса.ПсевдонимыПолейПериодов, Разделитель));
	ТекстЗапросаКорреспонденцииИсточников = СтрЗаменить(
		ТекстЗапросаКорреспонденцииИсточников,
		"ПсевдонимыПолейАналитики.*",
		СтрСоединить(ОписаниеПолейЗапроса.ПсевдонимыПолейАналитики, Разделитель));
	ТекстЗапросаКорреспонденцииИсточников = СтрЗаменить(
		ТекстЗапросаКорреспонденцииИсточников,
		"ПсевдонимыПолейРесурсов.*",
		СтрСоединить(ОписаниеПолейЗапроса.ПсевдонимыПолейРесурсов, Разделитель));
	ТекстЗапросаКорреспонденцииИсточников = СтрЗаменить(
		ТекстЗапросаКорреспонденцииИсточников,
		"&ПоляГруппировкиПериодов",
		СтрСоединить(ОписаниеПолейЗапроса.ПоляГруппировкиПериодов, Разделитель));
	ТекстЗапросаКорреспонденцииИсточников = СтрЗаменить(
		ТекстЗапросаКорреспонденцииИсточников,
		"&ПоляГруппировкиАналитики",
		СтрСоединить(ОписаниеПолейЗапроса.ПоляГруппировкиАналитики, Разделитель));
	ТекстЗапросаКорреспонденцииИсточников = СтрЗаменить(
		ТекстЗапросаКорреспонденцииИсточников,
		"&СтатьяАктивовПассивов",
		?(ВыборкаИсточникаЧерезПараметр, "&", "") + СтрокаВыборкиИсточникаДанных);
	ТекстЗапросаКорреспонденцииИсточников = СтрЗаменить(
		ТекстЗапросаКорреспонденцииИсточников,
		"&КорСтатьяАктивовПассивов",
		?(ВыборкаКорИсточникаЧерезПараметр, "&", "") + СтрокаВыборкиКорИсточникаДанных);
	
	ТекстыЗапросов.Добавить(ТекстЗапросаКорреспонденцииИсточников);
	
	СхемаКомпоновкиДанных = КомпоновкаДанныхСервер.ПустаяСхема();
	ИмяНабораДанных = "ИсточникДанных";
	НаборДанных = КомпоновкаДанныхСервер.ДобавитьПустойНаборДанных(СхемаКомпоновкиДанных,, ИмяНабораДанных);
	НаборДанных.АвтоЗаполнениеДоступныхПолей = Ложь;
	НаборДанных.Запрос = СтрСоединить(ТекстыЗапросов, ОбщегоНазначения.РазделительПакетаЗапросов());
	
	ИмяПапкиПолейИсточника = ИмяПапкиПолейИсточникаДанных();
	ИмяПапкиПолейКорИсточника = ИмяПапкиПолейКорИсточникаДанных();
	
	ФинансоваяОтчетностьСервер.НоваяГруппаНабора(НаборДанных, ИмяПапкиПолейИсточника, ОписаниеИсточникаДанных.Представление);
	ФинансоваяОтчетностьСервер.НоваяГруппаНабора(НаборДанных, ИмяПапкиПолейКорИсточника, ОписаниеКорИсточникаДанных.Представление);
	
	ПоляИсточникаВнеПапки = ПоляИсточникаДанныхВнеПапкиПолей(ОписаниеПолейЗапроса, Истина);
	
	Для Каждого КлючИЗначение Из ПоляНабораДанных Цикл
		ИмяПоля = КлючИЗначение.Ключ;
		ОписаниеПоля = КлючИЗначение.Значение; // см. ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных
		ЗаголовокПоля = ОписаниеПоля.Заголовок;
		ТипПоля = ОписаниеПоля.Тип;
		
		Если ПоляИсточникаВнеПапки.Найти(ИмяПоля) <> Неопределено Тогда
			ПутьКДанным = ИмяПоля;
		ИначеЕсли ПоляВременнойТаблицыКорИсточника.Свойство(ИмяПоля) Тогда
			ПутьКДанным = ИмяПапкиПолейКорИсточника + "." + ИмяПоля;
		Иначе
			ПутьКДанным = ИмяПапкиПолейИсточника + "." + ИмяПоля;
		КонецЕсли;
		
		ПолеНабора = ФинансоваяОтчетностьСервер.НовоеПолеНабора(НаборДанных, ИмяПоля, ПутьКДанным, ЗаголовокПоля, ТипПоля);
		
		Если ИмяПоля = "ИдентификаторИсточникаДанных" Тогда
			ПолеНабора.ОграничениеИспользования.Условие = Ложь;
		КонецЕсли;
	КонецЦикла;
	
	КомпоновкаДанныхСервер.ДобавитьПараметр(СхемаКомпоновкиДанных, "НачалоПериода",,, Ложь);
	КомпоновкаДанныхСервер.ДобавитьПараметр(СхемаКомпоновкиДанных, "КонецПериода",,, Ложь);
	КомпоновкаДанныхСервер.ДобавитьПараметр(СхемаКомпоновкиДанных, "ПривилегированныйРежим",,, Ложь);
	КомпоновкаДанныхСервер.ДобавитьПараметр(СхемаКомпоновкиДанных, "ПривилегированныйРежимИсточника",,, Ложь);
	
	Если ВыборкаИсточникаЧерезПараметр Тогда
		КомпоновкаДанныхСервер.ДобавитьПараметр(СхемаКомпоновкиДанных, СтрокаВыборкиИсточникаДанных,
			Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов"), РеквизитыПравила.ИсточникДанных, Ложь);
	КонецЕсли;
	
	Если ВыборкаКорИсточникаЧерезПараметр Тогда
		КомпоновкаДанныхСервер.ДобавитьПараметр(СхемаКомпоновкиДанных, СтрокаВыборкиКорИсточникаДанных,
			Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов"), РеквизитыПравила.КорИсточникДанных, Ложь);
	КонецЕсли;
	
	Для Каждого ПараметрСхемыКомпоновкиДанных Из СхемаКомпоновкиДанных.Параметры Цикл
		ПараметрСхемыКомпоновкиДанных.ВключатьВДоступныеПоля = Ложь;
		ПараметрСхемыКомпоновкиДанных.ОграничениеИспользования = Истина;
	КонецЦикла;
	
	Если ОписаниеИсточникаДанных.ОтборДанных.Элементы.Количество() > 0 Тогда
		КомпоновкаДанныхКлиентСервер.СкопироватьЭлементы(
			СхемаКомпоновкиДанных.НастройкиПоУмолчанию.Отбор,
			ОписаниеИсточникаДанных.ОтборДанных);
	КонецЕсли;
	
	Если ОписаниеКорИсточникаДанных.ОтборДанных.Элементы.Количество() > 0 Тогда
		ФинансовыйУчетПоДаннымБалансовыхРегистров.СкопироватьОтборКомпоновкиДанных(
			СхемаКомпоновкиДанных.НастройкиПоУмолчанию.Отбор,
			ОписаниеКорИсточникаДанных.ОтборДанных,
			ПрефиксИменПолейКорИсточникаДанных());
	КонецЕсли;
...
Функция СхемаКомпоновкиДанныхФактаСтатьиПоСтатьеАктивовПассивов(РеквизитыПравила)
	...
	ПоляНабораДанных = ПоляНабораСхемыКомпоновкиДанныхПоСтатьеАктивовПассивов();
	ОписаниеПолейЗапроса = ОписаниеПолейЗапросаИсточникаДанныхСтатьиАктивовПассивов();
	ОписаниеПолейЗапроса.Показатель = РеквизитыПравила.ИсточникСуммыОперации;
	
	ДополнитьПоляНабораПолямиСтандартныхИзмерений(ОписаниеПолейЗапроса, ПоляНабораДанных, ОписаниеИсточникаДанных, ПараметрыОтраженияВУчете);
	ДополнитьПоляНабораПолямиПериодов(ОписаниеПолейЗапроса, ПоляНабораДанных, ОписаниеИсточникаДанных, ПараметрыОтраженияВУчете);
	ДополнитьПоляНабораПолямиАналитикиИсточникаДанных(ОписаниеПолейЗапроса, ПоляНабораДанных, ПараметрыОтраженияВУчете, Истина);
	ДополнитьПоляНабораРесурсамиИсточникаДанных(ОписаниеПолейЗапроса, ПоляНабораДанных, ОписаниеИсточникаДанных, ПараметрыОтраженияВУчете);
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&ПоляОтбора.* КАК ПсевдонимыПолейОтбора",
		СтрСоединить(ОписаниеПолейЗапроса.ПоляОтбора, Разделитель));
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"ПсевдонимыПолейПериодов.*",
		СтрСоединить(ОписаниеПолейЗапроса.ПсевдонимыПолейПериодов, Разделитель));
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"ПсевдонимИсточникаДанных",
		ОписаниеПолейЗапроса.ПсевдонимИсточникаДанных);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&СтатьяАктивовПассивов",
		?(ВыборкаИсточникаЧерезПараметр, "&", "") + СтрокаВыборкиИсточникаДанных);
	...
	ИмяПапкиПолейИсточника = ИмяПапкиПолейИсточникаДанных();
	ФинансоваяОтчетностьСервер.НоваяГруппаНабора(НаборДанных, ИмяПапкиПолейИсточника, ОписаниеИсточникаДанных.Представление);
	ПоляИсточникаВнеПапки = ПоляИсточникаДанныхВнеПапкиПолей(ОписаниеПолейЗапроса);
...
Функция СхемаКомпоновкиДанныхФактаПоказателяПоСтатьеАктивовПассивов(РеквизитыПравила)
	...
	ОписаниеРесурсов = ОписаниеРесурсовСтатьиАктивовПассивов();
	ПоляНабораДанных = ПоляНабораСхемыКомпоновкиДанныхПоСтатьеАктивовПассивов();
	ОписаниеПолейЗапроса = ОписаниеПолейЗапросаИсточникаДанныхСтатьиАктивовПассивов();
	
	ДополнитьПоляНабораПолямиСтандартныхИзмерений(ОписаниеПолейЗапроса, ПоляНабораДанных, ОписаниеИсточникаДанных, ПараметрыОтраженияВУчете);
	ДополнитьПоляНабораПолямиАналитикиИсточникаДанных(ОписаниеПолейЗапроса, ПоляНабораДанных, ПараметрыОтраженияВУчете);
	
	Если МетаданныеИсточникаДанных.ВидРегистра = ВидРегистраНакопления.Обороты Тогда
		ТаблицаИсточникаДанных = 
			"РегистрНакопления.%1.Обороты(,
			|	&ПериодГраница,
			|	ПЕРИОД,
			|	&ПривилегированныйРежимИсточника = &ПривилегированныйРежим
			|	{(""ИдентификаторИсточникаДанных"") КАК ИдентификаторИсточникаДанных,
			|	(&СтатьяАктивовПассивов) КАК ИсточникДанных,
			|	&ПоляТаблицы.* КАК ПсевдонимыПолейТаблицы
			|	})"; //@query-part
		
		КоэффициентИтога = "ВЫБОР
			|		КОГДА &УсловиеДебет ТОГДА 1
			|		КОГДА &УсловиеКредит ТОГДА -1
			|		ИНАЧЕ 0
			|	КОНЕЦ"; //@query-part
		
		УсловиеДебет = ОписаниеИсточникаДанных.УсловиеДебет;
		УсловиеКредит = ОписаниеИсточникаДанных.УсловиеКредит;
		
		Если ПустаяСтрока(УсловиеДебет) Тогда
			УсловиеДебет = ПараметрыОтраженияВУчете.УсловиеДебет;
		КонецЕсли;
		
		Если ПустаяСтрока(УсловиеКредит) Тогда
			УсловиеКредит = ПараметрыОтраженияВУчете.УсловиеКредит;
		КонецЕсли;
		
		Если ПустаяСтрока(УсловиеДебет) ИЛИ ПустаяСтрока(УсловиеКредит) Тогда
			ВызватьИсключение НСтр("ru = 'Не указаны правила отнесения оборотов к дебету и кредиту.';
									|en = 'The rules for allocating turnover to debit and credit are not specified.'");
		КонецЕсли;
		
		КоэффициентИтога = СтрЗаменить(КоэффициентИтога, "&УсловиеДебет", УсловиеДебет);
		КоэффициентИтога = СтрЗаменить(КоэффициентИтога, "&УсловиеКредит", УсловиеКредит);
		ВидИтогаРесурса = "Оборот"; //@query-part
	Иначе
		ТаблицаИсточникаДанных = 
			"РегистрНакопления.%1.Остатки(
			|	&ПериодГраница,
			|	&ПривилегированныйРежимИсточника = &ПривилегированныйРежим
			|	{(""ИдентификаторИсточникаДанных"") КАК ИдентификаторИсточникаДанных,
			|	(&СтатьяАктивовПассивов) КАК ИсточникДанных,
		...
		ИнверсияДвижений = ПланыВидовХарактеристик.СтатьиАктивовПассивов.ИнверсияДвиженийСтатьиПоПоказателю(
			ОписаниеИсточникаДанных, РеквизитыПравила.ИсточникСуммыОперации);
		
		Если НЕ ИнверсияДвижений Тогда
			ИнверсияДвижений = ПараметрыОтраженияВУчете.ИнверсияДвижений;
		КонецЕсли;
		
		КоэффициентИтога = ?(ИнверсияДвижений, "-1", "1"); //@query-part
		ВидИтогаРесурса = "Остаток"; //@query-part
	КонецЕсли;
	
	Для Каждого КлючИЗначение Из ОписаниеРесурсов Цикл
		ИмяПоля = КлючИЗначение.Ключ;
		ОписаниеПоля = КлючИЗначение.Значение; // см. ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных
		
		Если ЗначениеЗаполнено(РеквизитыПравила.ИсточникСуммыОперации) Тогда
			СоставРесурса = ОписаниеИсточникаДанных.Показатели[РеквизитыПравила.ИсточникСуммыОперации][ОписаниеПоля.ПутьКДанным];
		Иначе
			СоставРесурса = ОписаниеИсточникаДанных[ОписаниеПоля.ПутьКДанным];
		КонецЕсли;
		...
		ПоляИсточникаДанных = Новый Массив();
		
		Для Каждого ПолеРесурса Из СоставРесурса Цикл
			ПутьКДанным = ФинансовыйУчетПоДаннымБалансовыхРегистров.ПутьКДаннымСПсевдонимомИсточника(
				ПолеРесурса.ИмяРесурса, ОписаниеПолейЗапроса.ПсевдонимИсточникаДанных);
			Коэффициент = ПолеРесурса.Коэффициент;
			Если Коэффициент <> 0 Тогда
				ПоляИсточникаДанных.Добавить(?(Коэффициент < 0, "-", "") + ПутьКДанным + ВидИтогаРесурса); //@query-part
			КонецЕсли;
		КонецЦикла;
		
		Если ПоляИсточникаДанных.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		ПолеИсточникаДанных = КоэффициентИтога + " * (" + СтрСоединить(ПоляИсточникаДанных, " + ") + ")"; //@query-part
		ОписаниеПолейЗапроса.ПоляРесурсов.Добавить(ПолеИсточникаДанных + " КАК " + ИмяПоля); //@query-part
		ОписаниеПолейЗапроса.ПсевдонимыПолейРесурсов.Добавить(ИмяПоля);
		ПоляНабораДанных.Вставить(ИмяПоля, ОписаниеПоля);
	КонецЦикла;
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&ТаблицаИсточникаДанных",
		СтрШаблон(ТаблицаИсточникаДанных, ОписаниеИсточникаДанных.ИсточникДанных));
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&ПоляАналитики КАК ПсевдонимыПолейАналитики",
		СтрСоединить(ОписаниеПолейЗапроса.ПоляАналитики, Разделитель));
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&ПоляТаблицы.* КАК ПсевдонимыПолейТаблицы",
		СтрСоединить(ОписаниеПолейЗапроса.ПоляТаблицы, Разделитель));
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&ПоляОтбора.* КАК ПсевдонимыПолейОтбора",
		СтрСоединить(ОписаниеПолейЗапроса.ПоляОтбора, Разделитель));
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
	...
	КомпоновкаДанныхСервер.ДобавитьПараметр(СхемаКомпоновкиДанных, "ПериодГраница",,, Ложь);
	КомпоновкаДанныхСервер.ДобавитьПараметр(СхемаКомпоновкиДанных, "ПривилегированныйРежим",,, Ложь);
	КомпоновкаДанныхСервер.ДобавитьПараметр(СхемаКомпоновкиДанных, "ПривилегированныйРежимИсточника",,, Ложь);
...
Функция ОписаниеПолейЗапросаИсточникаДанныхСтатьиАктивовПассивов()
	
	ОписаниеПолейЗапроса = Новый Структура();
	
	ОписаниеПолейЗапроса.Вставить("ПоляПериодов", Новый Массив());
	ОписаниеПолейЗапроса.Вставить("ПоляАналитики", Новый Массив());
	ОписаниеПолейЗапроса.Вставить("ПоляРесурсов", Новый Массив());
	ОписаниеПолейЗапроса.Вставить("ПоляТаблицы", Новый Массив());
	ОписаниеПолейЗапроса.Вставить("ПоляОтбора", Новый Массив());
	ОписаниеПолейЗапроса.Вставить("ПсевдонимыПолейПериодов", Новый Массив());
	ОписаниеПолейЗапроса.Вставить("ПсевдонимыПолейАналитики", Новый Массив());
	ОписаниеПолейЗапроса.Вставить("ПсевдонимыПолейРесурсов", Новый Массив());
	ОписаниеПолейЗапроса.Вставить("ПоляГруппировкиПериодов", Новый Массив());
	ОписаниеПолейЗапроса.Вставить("ПоляГруппировкиАналитики", Новый Массив());
	ОписаниеПолейЗапроса.Вставить("БалансныеПоля", ФинансовыйУчетПоДаннымБалансовыхРегистров.БалансныеПоляИсточниковДанных());
	ОписаниеПолейЗапроса.Вставить("Корреспонденция", Ложь);
	ОписаниеПолейЗапроса.Вставить("ВременнаяТаблица", Ложь);
	ОписаниеПолейЗапроса.Вставить("Показатель", Перечисления.ПоказателиАналитическихРегистров.ПустаяСсылка());
	ОписаниеПолейЗапроса.Вставить("ПсевдонимИсточникаДанных", ФинансовыйУчетПоДаннымБалансовыхРегистров.ПсевдонимИсточникаДанныхПоУмолчанию());
	
	Возврат ОписаниеПолейЗапроса;
	
КонецФункции

Функция ОписаниеРесурсовСтатьиАктивовПассивов()
	
	ОписаниеРесурсов = Новый Структура();
	МенеджерСтатьиАктивовПассивов = ПланыВидовХарактеристик.СтатьиАктивовПассивов;
	
	ОписаниеРесурсов.Вставить("СуммаУпр",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			МенеджерСтатьиАктивовПассивов.ГруппаРесурсовУправленческогоУчета(),
			НСтр("ru = 'Сумма (упр.)';
				|en = 'Amount, mgmt'"),
			РаботаСКурсамиВалют.ОписаниеТипаДенежногоПоля()));
			
	ОписаниеРесурсов.Вставить("СуммаРегл",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			МенеджерСтатьиАктивовПассивов.ГруппаРесурсовРегламентированногоУчета(),
			НСтр("ru = 'Сумма (регл.)';
				|en = 'Amount (local)'"),
			РаботаСКурсамиВалют.ОписаниеТипаДенежногоПоля()));
			
	ОписаниеРесурсов.Вставить("СуммаВВалюте",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			МенеджерСтатьиАктивовПассивов.ГруппаРесурсовВалютногоУчета(),
			НСтр("ru = 'Сумма в валюте';
				|en = 'Amount in currency'"),
			РаботаСКурсамиВалют.ОписаниеТипаДенежногоПоля()));
			
	ОписаниеРесурсов.Вставить("Количество",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			МенеджерСтатьиАктивовПассивов.ГруппаРесурсовКоличественногоУчета(),
			НСтр("ru = 'Количество';
				|en = 'Quantity'"),
...
Функция ПоляНабораСхемыКомпоновкиДанныхПоСтатьеАктивовПассивов(Корреспонденция = Ложь)
	
	ПоляНабораДанных = Новый Структура();
	
	ПоляНабораДанных.Вставить("ИсточникДанных",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			"ИсточникДанных",
	...
	Если Корреспонденция Тогда
		ПоляНабораДанных.Вставить("КорИсточникДанных",
			ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
				"КорИсточникДанных",
				НСтр("ru = 'Источник данных (кор.)';
					|en = 'Data source (cor.)'"),
				Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов")));
	КонецЕсли;
	
	ПоляНабораДанных.Вставить("ИдентификаторИсточникаДанных",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			"ИдентификаторИсточникаДанных",
			НСтр("ru = 'Идентификатор источника данных';
				|en = 'Data source ID'"),
			Метаданные.ОпределяемыеТипы.ИдентификаторИсточникаДанных.Тип));
	
	Возврат ПоляНабораДанных;
	
КонецФункции

Процедура ДополнитьПоляНабораПолямиПериодов(ОписаниеПолейЗапроса, ПоляНабораДанных, ОписаниеИсточникаДанных, ПараметрыОтраженияВУчете)
	
	ОписаниеПериодов = Новый Структура();
	
	ОписаниеПериодов.Вставить("ПериодСекунда",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			"", //@query-part
			НСтр("ru = 'Период, секунда';
				|en = 'Period, second'"),
			ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.ДатаВремя)));
	
	ОписаниеПериодов.Вставить("ПериодДень",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			"ДЕНЬ", //@query-part
			НСтр("ru = 'Период, день';
				|en = 'Period, day'"),
	...
	ОписаниеПериодов.Вставить("ПериодНеделя",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			"НЕДЕЛЯ", //@query-part
			НСтр("ru = 'Период, неделя';
				|en = 'Period, week'"),
			ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.ДатаВремя)));
	
	ОписаниеПериодов.Вставить("ПериодДекада",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			"ДЕКАДА", //@query-part
			НСтр("ru = 'Период, декада';
				|en = 'Period, ten-day period'"),
			ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.ДатаВремя)));
	
	ОписаниеПериодов.Вставить("ПериодМесяц",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			"МЕСЯЦ", //@query-part
			НСтр("ru = 'Период, месяц';
				|en = 'Period, month'"),
			ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.ДатаВремя)));
	
	ОписаниеПериодов.Вставить("ПериодКвартал",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			"КВАРТАЛ", //@query-part
			НСтр("ru = 'Период, квартал';
				|en = 'Period, quarter'"),
			ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.ДатаВремя)));
	
	ОписаниеПериодов.Вставить("ПериодПолугодие",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			"ПОЛУГОДИЕ", //@query-part
			НСтр("ru = 'Период, полугодие';
				|en = 'Period, half-year'"),
			ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.ДатаВремя)));
	
	ОписаниеПериодов.Вставить("ПериодГод",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			"ГОД", //@query-part
			НСтр("ru = 'Период, год';
				|en = 'Period, year'"),
	...
	Если ПоляНабораДанных.Свойство("Валюта") Тогда
		ОписаниеПериодов.Вставить("ПериодКурса",
			ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
				"ДЕНЬ", //@query-part
				НСтр("ru = 'Период курса';
					|en = 'Rate period'"),
				ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.ДатаВремя)));
	КонецЕсли;
	
	Для Каждого КлючИЗначение Из ОписаниеПериодов Цикл
		ИмяПоля = КлючИЗначение.Ключ;
		ОписаниеПоля = КлючИЗначение.Значение; // см. ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных
		ПутьКДанным = "Период"; //@query-part
		ТипПериода = ОписаниеПоля.ПутьКДанным;
		
		ПолеИсточникаДанных = ФинансовыйУчетПоДаннымБалансовыхРегистров.ПутьКДаннымСПсевдонимомИсточника(
			ПутьКДанным, ОписаниеПолейЗапроса.ПсевдонимИсточникаДанных);
			
		Если НЕ ПустаяСтрока(ТипПериода) Тогда
			ПолеИсточникаДанных = "НАЧАЛОПЕРИОДА(" + ПолеИсточникаДанных + ", " + ТипПериода + ")"; //@query-part
		КонецЕсли;
		
		ОписаниеПолейЗапроса.ПоляПериодов.Добавить(ПолеИсточникаДанных + " КАК " + ИмяПоля); //@query-part
		ОписаниеПолейЗапроса.ПсевдонимыПолейПериодов.Добавить(ИмяПоля); //@query-part
		ОписаниеПолейЗапроса.ПоляГруппировкиПериодов.Добавить(ПолеИсточникаДанных); //@query-part
...
Процедура ДополнитьПоляНабораПолямиСтандартныхИзмерений(ОписаниеПолейЗапроса, ПоляНабораДанных, ОписаниеИсточникаДанных, ПараметрыОтраженияВУчете)
	
	ОписаниеИзмерений = Новый Структура();
	
	ОписаниеИзмерений.Вставить("Организация",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			ПараметрыОтраженияВУчете.ПутьКДаннымОрганизация,
			НСтр("ru = 'Организация';
				|en = 'Company'"),
			Новый ОписаниеТипов("СправочникСсылка.Организации")));
	
	ОписаниеИзмерений.Вставить("Подразделение",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			ПараметрыОтраженияВУчете.ПутьКДаннымПодразделение,
			НСтр("ru = 'Подразделение';
				|en = 'Department'"),
			Новый ОписаниеТипов("СправочникСсылка.СтруктураПредприятия")));
	
	ОписаниеИзмерений.Вставить("НаправлениеДеятельности",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			?(ПустаяСтрока(ОписаниеИсточникаДанных.ПутьКДаннымНаправлениеДеятельности),
				ПараметрыОтраженияВУчете.ПутьКДаннымНаправлениеДеятельности,
				ОписаниеИсточникаДанных.ПутьКДаннымНаправлениеДеятельности),
			НСтр("ru = 'Направление деятельности';
				|en = 'Line of business'"),
			Новый ОписаниеТипов("СправочникСсылка.НаправленияДеятельности")));
	
	ОписаниеИзмерений.Вставить("Валюта",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			ПараметрыОтраженияВУчете.ПутьКДаннымВалюта,
			НСтр("ru = 'Валюта';
				|en = 'Currency'"),
			Новый ОписаниеТипов("СправочникСсылка.Валюты")));
	
	ДополнитьПоляНабораПолямиИсточникаДанных(ОписаниеПолейЗапроса, ПоляНабораДанных, ОписаниеИзмерений);
	
КонецПроцедуры

Процедура ДополнитьПоляНабораБаланснымиПолямиИсточникаДанных(ОписаниеПолейЗапроса, ПоляНабораДанных, ОписаниеИсточникаДанных, ПараметрыОтраженияВУчете)
	
	ОписаниеБалансныхПолей = Новый Структура();
	
	ОписаниеБалансныхПолей.Вставить("ИдентификаторФинЗаписи",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			?(ПустаяСтрока(ОписаниеИсточникаДанных.ПутьКДаннымИдентификаторФинЗаписи),
				ПараметрыОтраженияВУчете.ПутьКДаннымИдентификаторФинЗаписи,
				ОписаниеИсточникаДанных.ПутьКДаннымИдентификаторФинЗаписи),
			НСтр("ru = 'Идентификатор фин. записи';
				|en = 'Fin. record ID'"),
			ОбщегоНазначения.ОписаниеТипаСтрока(36)));
	
	ОписаниеБалансныхПолей.Вставить("НастройкаХозяйственнойОперации",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			?(ПустаяСтрока(ОписаниеИсточникаДанных.ПутьКДаннымНастройкаХозОперации),
				ПараметрыОтраженияВУчете.ПутьКДаннымНастройкаХозОперации,
				ОписаниеИсточникаДанных.ПутьКДаннымНастройкаХозОперации),
			НСтр("ru = 'Настройка хозяйственной операции';
				|en = 'Business transactions setting'"),
			Новый ОписаниеТипов("СправочникСсылка.НастройкиХозяйственныхОпераций")));
	
	ОписаниеБалансныхПолей.Вставить("Сторно",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
			?(ПустаяСтрока(ПараметрыОтраженияВУчете.ПутьКДаннымСторно),
				"ЛОЖЬ",
				ПараметрыОтраженияВУчете.ПутьКДаннымСторно),
			НСтр("ru = 'Сторно';
				|en = 'Storno'"),
			Новый ОписаниеТипов("Булево")));
	
	ДополнитьПоляНабораПолямиИсточникаДанных(ОписаниеПолейЗапроса, ПоляНабораДанных, ОписаниеБалансныхПолей);
	
КонецПроцедуры

Процедура ДополнитьПоляНабораПолямиАналитикиИсточникаДанных(ОписаниеПолейЗапроса, ПоляНабораДанных, ПараметрыОтраженияВУчете, ИспользоватьРеквизиты = Ложь)
	
	ДополнитьПоляНабораПолямиИсточникаДанных(ОписаниеПолейЗапроса, ПоляНабораДанных, ПараметрыОтраженияВУчете.ОписаниеИзмерений);
	
	Если ИспользоватьРеквизиты Тогда
		ДополнитьПоляНабораПолямиИсточникаДанных(ОписаниеПолейЗапроса, ПоляНабораДанных, ПараметрыОтраженияВУчете.ОписаниеРеквизитов);
	КонецЕсли;
	
КонецПроцедуры

Процедура ДополнитьПоляНабораПолямиИсточникаДанных(ОписаниеПолейЗапроса, ПоляНабораДанных, ОписаниеПолейИсточникаДанных)
	
	ОписаниеПримитивныхТипов = ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПримитивныхТипов();
	
	Для Каждого КлючИЗначение Из ОписаниеПолейИсточникаДанных Цикл
		Если ОписаниеПолейЗапроса.Корреспонденция
			И ОписаниеПолейЗапроса.БалансныеПоля.Найти(КлючИЗначение.Ключ) = Неопределено Тогда
			ПрефиксИменПолей = ПрефиксИменПолейКорИсточникаДанных();
			ПостфиксЗаголовковПолей = " " + НСтр("ru = '(кор.)';
												|en = '(cor.)'");
		Иначе
			ПрефиксИменПолей = "";
			ПостфиксЗаголовковПолей = "";
		КонецЕсли;
		
		ИмяПоля = ПрефиксИменПолей + КлючИЗначение.Ключ;
		ОписаниеПоля = КлючИЗначение.Значение; // см. ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных
		ОписаниеПоля.Заголовок = ОписаниеПоля.Заголовок + ПостфиксЗаголовковПолей;
		
		Если ПоляНабораДанных.Свойство(ИмяПоля) Тогда
			Продолжить;
		ИначеЕсли ПустаяСтрока(ОписаниеПоля.ПутьКДанным) Тогда
			Продолжить;
		Иначе
			ПутьКДаннымУжеСуществует = Ложь;
			Для Каждого ПолеНабораДанных Из ПоляНабораДанных Цикл
				Если ПолеНабораДанных.Значение.ПутьКДанным = ОписаниеПоля.ПутьКДанным Тогда
					ПутьКДаннымУжеСуществует = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Если ПутьКДаннымУжеСуществует Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		
		ТипПримитивный = ОписаниеПримитивныхТипов.СодержитТип(ОписаниеПоля.Тип.Типы()[0]);
		ПолеТаблицы = ФинансовыйУчетПоДаннымБалансовыхРегистров.ПутьКДаннымБезПсевдонимаИсточника(
			ОписаниеПоля.ПутьКДанным, ОписаниеПолейЗапроса.ПсевдонимИсточникаДанных);
		ПолеИсточникаДанных = ФинансовыйУчетПоДаннымБалансовыхРегистров.ПутьКДаннымСПсевдонимомИсточника(
			ОписаниеПоля.ПутьКДанным, ОписаниеПолейЗапроса.ПсевдонимИсточникаДанных);
		
		ОписаниеПолейЗапроса.ПоляАналитики.Добавить(ПолеИсточникаДанных + " КАК " + ИмяПоля); //@query-part
		ОписаниеПолейЗапроса.ПоляТаблицы.Добавить("(" + ПолеТаблицы + ?(ТипПримитивный, ")", ").*") + " КАК " + ИмяПоля); //@query-part
		ОписаниеПолейЗапроса.ПоляОтбора.Добавить("(" + ПолеИсточникаДанных + ?(ТипПримитивный, ")", ").*") + " КАК " + ИмяПоля); //@query-part
		ОписаниеПолейЗапроса.ПсевдонимыПолейАналитики.Добавить(ИмяПоля + ?(ТипПримитивный, "", ".*")); //@query-part
		ОписаниеПолейЗапроса.ПоляГруппировкиАналитики.Добавить(ПолеИсточникаДанных); //@query-part
...
Процедура ДополнитьПоляНабораРесурсамиИсточникаДанных(ОписаниеПолейЗапроса, ПоляНабораДанных, ОписаниеИсточникаДанных, ПараметрыОтраженияВУчете)
	...
	Если ОписаниеПолейЗапроса.Корреспонденция Тогда
		ПрефиксИменПолей = ПрефиксИменПолейКорИсточникаДанных();
		ПостфиксЗаголовковПолей = " " + НСтр("ru = '(кор.)';
											|en = '(cor.)'");
	Иначе
	...
	Иначе
		ИнверсияДвижений = ПланыВидовХарактеристик.СтатьиАктивовПассивов.ИнверсияДвиженийСтатьиПоПоказателю(
			ОписаниеИсточникаДанных, ОписаниеПолейЗапроса.Показатель);
		...
		Если ПустаяСтрока(УсловиеДебет) Тогда
			Если ИнверсияДвижений Тогда
				УсловиеДебет = "ПсевдонимИсточникаДанных.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)"; //@query-part;
			Иначе
				УсловиеДебет = "ПсевдонимИсточникаДанных.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)"; //@query-part;
			КонецЕсли;
		КонецЕсли;
		
		Если ПустаяСтрока(УсловиеКредит) Тогда
			Если ИнверсияДвижений Тогда
				УсловиеКредит = "ПсевдонимИсточникаДанных.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)"; //@query-part;
			Иначе
				УсловиеКредит = "ПсевдонимИсточникаДанных.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)"; //@query-part;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	КоэффициентыОборотов = Новый Структура();
	КоэффициентыОборотов.Вставить("ОборотДт", СтрЗаменить(
		"ВЫБОР
		|		КОГДА &УсловиеДебет ТОГДА 1
		|		ИНАЧЕ 0
		|	КОНЕЦ",
		"&УсловиеДебет",
		УсловиеДебет)); //@query-part;
	КоэффициентыОборотов.Вставить("ОборотКт", СтрЗаменить(
		"ВЫБОР
		|		КОГДА &УсловиеКредит ТОГДА 1
		|		ИНАЧЕ 0
		|	КОНЕЦ",
		"&УсловиеКредит",
		 УсловиеКредит)); //@query-part;
	КоэффициентыОборотов.Вставить("Оборот", СтрЗаменить(
		СтрЗаменить(
		"ВЫБОР
		|		КОГДА &УсловиеДебет ТОГДА 1
		|		КОГДА &УсловиеКредит ТОГДА -1
	...
	Для Каждого КлючИЗначение Из ОписаниеРесурсов Цикл
		...
		Если ЗначениеЗаполнено(ОписаниеПолейЗапроса.Показатель) Тогда
			СоставРесурса = ОписаниеИсточникаДанных.Показатели[ОписаниеПолейЗапроса.Показатель][ОписаниеПоля.ПутьКДанным];
		Иначе
			СоставРесурса = ОписаниеИсточникаДанных[ОписаниеПоля.ПутьКДанным];
		КонецЕсли;
		...
		Для Каждого ПолеРесурса Из СоставРесурса Цикл
			...
			Коэффициент = ПолеРесурса.Коэффициент;
			Если Коэффициент <> 0 Тогда
				ПоляИсточникаДанных.Добавить(?(Коэффициент < 0, "-", "") + ПутьКДанным); //@query-part
			КонецЕсли;
		КонецЦикла;
		...
		Для Каждого КоэффициентОборота Из КоэффициентыОборотов Цикл
			ВидОборота = КоэффициентОборота.Ключ;
			Коэффициент = КоэффициентОборота.Значение;
			ИмяОборота = ПрефиксИменПолей + ИмяПоля + ВидОборота;
			
			Если ВидОборота = "ОборотДт" Тогда
				ПредставлениеОборота = НСтр("ru = 'оборот Дт';
											|en = 'Dr turnover'");
			ИначеЕсли ВидОборота = "ОборотКт" Тогда
				ПредставлениеОборота = НСтр("ru = 'оборот Кт';
											|en = 'Cr turnover'");
			Иначе
				ПредставлениеОборота = НСтр("ru = 'оборот';
											|en = 'turnover'");
			КонецЕсли;
			
			ПредставлениеОборота = ПредставлениеОборота + ПостфиксЗаголовковПолей;
			
			Если ОписаниеПолейЗапроса.ВременнаяТаблица Тогда
				ПолеИсточникаДанных = Коэффициент + " * (" + СтрСоединить(ПоляИсточникаДанных, " + ") + ")"; //@query-part
			Иначе
				ПолеИсточникаДанных = "СУММА(" + Коэффициент + " * (" + СтрСоединить(ПоляИсточникаДанных, " + ") + "))"; //@query-part
			КонецЕсли;
			
			ОписаниеПолейЗапроса.ПоляРесурсов.Добавить(ПолеИсточникаДанных + " КАК " + ИмяОборота); //@query-part
			ОписаниеПолейЗапроса.ПсевдонимыПолейРесурсов.Добавить(ИмяОборота);
			
			ПоляНабораДанных.Вставить(ИмяОборота,
				ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляИсточникаДанных(
					ИмяОборота,
					ОписаниеПоля.Заголовок + ", " + ПредставлениеОборота,
					ОписаниеПоля.Тип));
		КонецЦикла;
	КонецЦикла;
...
Функция ПоляИсточникаДанныхВнеПапкиПолей(ОписаниеПолейЗапроса, Корреспонденция = Ложь)
	
	ПоляИсточникаВнеПапки = Новый Массив();
	
	Для Каждого КлючИЗначение Из ПоляНабораСхемыКомпоновкиДанныхПоСтатьеАктивовПассивов(Корреспонденция) Цикл
		ПоляИсточникаВнеПапки.Добавить(КлючИЗначение.Ключ);
	КонецЦикла;
	
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ПоляИсточникаВнеПапки, ОписаниеПолейЗапроса.БалансныеПоля);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ПоляИсточникаВнеПапки, ОписаниеПолейЗапроса.ПсевдонимыПолейПериодов);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ПоляИсточникаВнеПапки, ОписаниеПолейЗапроса.ПсевдонимыПолейРесурсов);
	
	Возврат ПоляИсточникаВнеПапки;
