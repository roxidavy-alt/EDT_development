	...
	Если ЗначениеЗаполнено(Параметры.СтрокаПоиска) Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ПодборПоКоду Тогда
				Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПредставления, Выборка.Код, Выборка.Наименование);
			Иначе
				Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПредставления, Выборка.Наименование, Выборка.Код);
			КонецЕсли;
...
Функция СоздатьАктВыполненныхВнутреннихРаботПроверкаОснований(ОбъектыОснований) Экспорт
	...
	|	ПО ГруппыЗатрат.Ссылка = ТаблицаТоваров.Ссылка
	|	И ГруппыЗатрат.НомерГруппыЗатрат = ТаблицаТоваров.НомерГруппыЗатрат
