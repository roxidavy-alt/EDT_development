...
Функция ЗаменитьСпецификации(Замены, КонтролироватьЗначения = Ложь) Экспорт
	...
	Для каждого Замена Из Замены Цикл
		...
		Если КонтролироватьЗначения И СтруктураЗаказаСлужебный.ЗаменаСпецификацииВСтруктуреЗаказаДопускается(Замена) Тогда
			...
			Шаблон = НСтр("ru = 'Не поддерживается замена спецификации для полуфабрикатов (работ) производимых в процессе (%1, %2).';
							|en = 'Replacement of the bill of materials for semi-finished products (works) manufactured throughout the process is not supported (%1, %2).'");
		...
		Если Задания.Количество() <> 1 Тогда
			ЗаписьЖурналаРегистрации(
				...
				НСтр("ru = 'Не найдено регламентное задание: Свертка сторно записей в регистрах структуры заказов на производство';
					|en = 'A scheduled job is not found: The storno record rollup in production order structure registers'"));
			Возврат;
		КонецЕсли;
...
Процедура КонтрольРасчета() Экспорт
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(Изделия.КОбеспечению - Изделия.Заменено) <> СУММА(Изделия.Запланировано)
...
Процедура ВыполнитьПересчетРасчетВнутриЗамераВремени(ПараметрыДействия)
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(Изделия.Заказано + Изделия.КОбеспечению - Изделия.Заменено) > 0
	|";
...
Процедура УдалитьПартииИзСтруктурыПоКлючамНаборов(Список) Экспорт
...
Функция ТабличнаяЧастьПеремещениеИзделийПоНоменклатурномуПлану(ИдентификаторыСтрок, ТолькоВНаличии = Ложь) Экспорт
	...
	|
	|	СУММА(План.КОбеспечению - План.Заменено) КАК Требуется,
	|
...
Процедура ПроверитьЗапуститьФоновоеЗаданиеРасчетаСтруктурыЗаказа() Экспорт
	
	Если ВыполняетсяРасчет() Тогда
		Возврат;
	КонецЕсли;
