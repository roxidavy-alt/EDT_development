...
Функция ПоказатьВопросДаНетАсинх(ТекстВопроса, ПараметрыВопроса = Неопределено) Экспорт
	...
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Недопустимое значение кнопки по умолчанию: %1';
										|en = 'Invalid value for the default button: %1'"),
...
Асинх Процедура ЗакончитьРедактированиеФайла(Знач ID, Знач Наименование, Знач Расширение,
	...
	Если Не СведенияОРедактировании.Сохранен Или СведенияОРедактировании.НаЧтение Тогда
		Ответ = Ждать ПоказатьВопросДаНетАсинх(НСтр("ru = 'Файл не найден в рабочем каталоге. Освободить его?';
													|en = 'File is not found in the working directory. Unlock the file?'"));
		Если Ответ = КодВозвратаДиалога.Да Тогда
...
Асинх Процедура ОткрытьКаталогФайла(Знач ID) Экспорт
	...
	Если Не Ждать РасширениеРаботыСФайламиПодключеноАсинх() Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Открывать каталог файла можно только после подключения
			...
			|en = 'You can open the file directory only after
...
Асинх Процедура ОтменитьРедактированиеФайла(Знач ID, Знач ОписаниеОповещенияОЗавершении = Неопределено,
	...
	Ответ = Ждать ПоказатьВопросДаНетАсинх(
		НСтр("ru = 'Отмена редактирования файла может привести к потере изменений. Продолжить?';
			|en = 'If you cancel file editing, the changes will be lost. Continue?'"));
	...
	ИначеЕсли ВыделенныеФайлыКоличество = 1 Тогда
		Если ТекущаяПометкаУдаления = Истина Тогда
			ТекстШаблона = НСтр("ru = 'Снять с ""%1"" пометку на удаление?';
								|en = 'Do you want to clear the deletion mark for ""%1""?'");
		Иначе
...
Процедура НачатьФормированиеПодписиОбъекта(ОбъектДО, Файлы, АдресДвоичныхДанныхОбъекта,
	...
	Если Не ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентПовтИсп.СоздаватьЭПДляОбъектов1СДокументооборотаСредствамиИС() Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Возможность создания подписей выключена в настройках интеграции с 1С:Документооборотом.
			...
			|en = 'The ability to create signatures is disabled in the 1C:Document Management integration settings.
			|
			|You need to enable this setting or sign the object in 1C:Document Management.'"));
		Возврат;
	КонецЕсли;
...
Асинх Процедура УдалитьПодписи(Знач УдаленныеСтроки, Знач ТаблицаПодписей, Знач ВладелецФайла,
	...
	Для Каждого ЭлементСоответствия Из ФайлыДляОповещения Цикл
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Удалена подпись файла ""%1""';
										|en = 'The ""%1"" file signature is deleted.'"), ЭлементСоответствия.Значение);
		ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
...
Процедура НачатьДобавлениеЭПИзФайлаПослеПодключенияРасширения(Подключено, Параметры) Экспорт
	...
	Если Не Подключено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Для выполнения операции нужно установить расширение работы с файлами.';
										|en = 'To perform the operation, install 1C:Enterprise Extension.'"));
		Возврат;
	КонецЕсли;
	...
	Если ДанныеФайла.Зашифрован Тогда
		ВызватьИсключение(
			НСтр("ru = 'Файл зашифрован. Расшифровка файла доступна только на стороне 1С:Документооборот.';
				|en = 'The file is encrypted. Decryption is only available in 1C:Document Management.'"));
	КонецЕсли;
