...
Функция ПараметрыРаспределенияЗатратНаВыходныеИзделия(ИмяТЧ, СпособРаспределенияЗатратНаВыходныеИзделия) Экспорт
	...
	ПараметрыРаспределенияЗатрат.Вставить("УстановитьДоляСтоимостиОбязательнаПоУмолчанию", Ложь);
...
// Настраивает элемент характирстики номенклатуры
// 
// Параметры:
//  Элемент - ПолеФормы
//  ХарактеристикиИспользуются - Булево
Процедура НастроитьЭлементХарактеристикаНоменклатуры(Элемент, ХарактеристикиИспользуются) Экспорт
	...
	Иначе
		Элемент.ПодсказкаВвода = НСтр("ru = '<характеристики не используются>';
...
// Настраивает элементы серий ремонтируемого изделия
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
//  ТипПроизводственногоПроцесса - ПеречислениеСсылка.ТипыПроизводственныхПроцессов
//  РемонтируемоеИзделиеСтатусУказанияСерий - Число
Процедура УстановитьДоступностьСерийРемонтируемогоИзделия(Форма, ТипПроизводственногоПроцесса, РемонтируемоеИзделиеСтатусУказанияСерий) Экспорт

	ВидимостьСерийРемонтируемогоИзделия =
		ТипПроизводственногоПроцесса = ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот")
		И Форма.ПараметрыУказанияСерий.РемонтируемоеИзделие.ИспользоватьСерииНоменклатуры;
	
	Форма.Элементы.РемонтируемоеИзделиеСтатусУказанияСерий.Видимость = ВидимостьСерийРемонтируемогоИзделия;
	Форма.Элементы.РемонтируемоеИзделиеСерия.Видимость               = ВидимостьСерийРемонтируемогоИзделия;
	
	Если ВидимостьСерийРемонтируемогоИзделия Тогда
		
		Если Не РемонтируемоеИзделиеСтатусУказанияСерий = 0 Тогда
			Форма.Элементы.РемонтируемоеИзделиеСерия.Доступность = Истина;
			Форма.Элементы.РемонтируемоеИзделиеСерия.ПодсказкаВвода = "";
		Иначе
			Форма.Элементы.РемонтируемоеИзделиеСерия.Доступность = Ложь;
			Форма.Элементы.РемонтируемоеИзделиеСерия.ПодсказкаВвода = НСтр("ru = '<серия не указывается>';
																			|en = '<batch is not specified>'");
		КонецЕсли;
		
		Если РемонтируемоеИзделиеСтатусУказанияСерий = 1 Тогда
			Форма.Элементы.РемонтируемоеИзделиеСерия.АвтоОтметкаНезаполненного = Истина;
		Иначе
			Форма.Элементы.РемонтируемоеИзделиеСерия.АвтоОтметкаНезаполненного = Ложь;
			Форма.Элементы.РемонтируемоеИзделиеСерия.ОтметкаНезаполненного = Ложь;
		КонецЕсли;
...
Функция ЗаголовокТабличнойЧастиПоТипуПроцесса(ТипПроизводственногоПроцесса, КодЯзыка = Неопределено) Экспорт
	...
	ИначеЕсли ТипПроизводственногоПроцесса = ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот") Тогда
		Заголовок = НСтр("ru = 'Выполняемые работы';
						|en = 'Works to perform'", КодЯзыка);
