...
Функция ПереопределитьСообщениеПользователю(
	...
	Иначе
		Результат = НСтр("ru = 'Не удалось отправить обращение, повторите попытку позже.';
						|en = 'Cannot send a request, please try again later.'");
	КонецЕсли;
	...
	Если Получатель = "webIts"
		...
		ДополнительныйТекст = НСтр("ru = 'Альтернативный способ отправки обращения: на электронную почту <a href=""mailto:webits-info@1c.ru"">webits-info@1c.ru</a>';
									|en = 'An alternative way to send a request: by e-mail (<a href=""mailto:webits-info@1c.ru"">webits-info@1c.ru</a>)'");
	КонецЕсли;
	...
	Если НедопустимоеРасширение.Количество() > 0 Тогда
		Ошибки.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|%1';
				|en = 'Следующие вложения имеют недопустимое расширение:
				|%1'"),
	...
	Если ОшибкаВременныйКаталог.Количество() > 0 Тогда
		Ошибки.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|%1';
				|en = 'Отправка вложений по имени файла доступна только из каталога временных файлов.
				|Следующие вложения расположены в другом каталоге и не могут быть отправлены:
				|%1'"),
