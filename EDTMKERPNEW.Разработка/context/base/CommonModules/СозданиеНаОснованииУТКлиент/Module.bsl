...
//  ПараметрыВыполнения - см. ПодключаемыеКомандыКлиент.ПараметрыВыполненияКоманды
//
Процедура ОтгрузкаТоваровСХраненияВозвратРемонтируемогоИзделияСоздатьНаОснованииЗаказа(МассивСсылок, ПараметрыВыполнения) Экспорт
	
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.ЗамерВремени(
		"ОбщийМодуль.СозданиеНаОснованииУТКлиент.ОтгрузкаТоваровСХраненияВозвратРемонтируемогоИзделияСоздатьНаОснованииЗаказа");
	
	ПараметрыВыполненияКоманды = Новый Структура("Источник,Уникальность,Окно,НавигационнаяСсылка");
	ЗаполнитьЗначенияСвойств(ПараметрыВыполненияКоманды, ПараметрыВыполнения.ОписаниеКоманды.ДополнительныеПараметры);
	
	ПараметрыОткрытия =
		СозданиеНаОснованииУТВызовСервера.ОтгрузкаТоваровСХраненияВозвратРемонтируемогоИзделияПараметрыОткрытияФормы(МассивСсылок);
		...
		Для Каждого Строка Из Массив Цикл
			...
			Если ЕстьОшибкиСтатус Тогда
				...
				ТекстОшибкиСтатус = НСтр("ru = 'Документ %Документ% находится в статусе ""%Статус%"". Ввод на основании разрешен в статусе ""Закрыто"".';
										|en = '%Документ% document is in the ""%Статус%"" status. Generation is allowed in ""Closed"" status.'");
...
// Создать заказ на производство на разборку изделий
// 
// Параметры:
//  СсылкаНаДокумент - ДокументСсылка.ЭтапПроизводства2_2
//  ПараметрыВыполнения - см. ПодключаемыеКомандыКлиент.ПараметрыВыполненияКоманды
Процедура СоздатьЗаказНаПроизводствоТипаРазборка(СсылкаНаДокумент, ПараметрыВыполнения) Экспорт
	...
	РеквизитыШапки.Вставить("ТипПроизводственногоПроцесса", ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.Разборка"));
	
	ПараметрыОснования = Новый Структура;
	ПараметрыОснования.Вставить("Основание", СсылкаНаДокумент);
	ПараметрыОснования.Вставить("РеквизитыШапки", РеквизитыШапки); 
	
	ОткрытьФорму("Документ.ЗаказНаПроизводство2_2.ФормаОбъекта",
		Новый Структура("Основание", ПараметрыОснования),
		ПараметрыВыполненияКоманды.Источник,
		ПараметрыВыполненияКоманды.Уникальность,
		ПараметрыВыполненияКоманды.Окно,
...
// Создать заказ на выполнение ремонтных работ
// 
// Параметры:
//  СсылкаНаДокумент - ДокументСсылка.ЭтапПроизводства2_2
//  ПараметрыВыполнения - см. ПодключаемыеКомандыКлиент.ПараметрыВыполненияКоманды
Процедура СоздатьЗаказНаПроизводствоТипаВыполнениеРемонтныхРабот(СсылкаНаДокумент, ПараметрыВыполнения) Экспорт
	...
	Оповещение = Новый ОписаниеОповещения(
		"СоздатьЗаказНаПроизводствоТипаВыполнениеРемонтныхРаботЗавершение",
		СозданиеНаОснованииУТКлиент,
		ПараметрыВыполнения);
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("СсылкаНаДокумент", СсылкаНаДокумент);
	
	ОткрытьФорму("Документ.ЗаказНаПроизводство2_2.Форма.СозданиеНаОснованииЗаказаНаВыполнениеРемонтныхРабот",
		ПараметрыФормы,
		ПараметрыВыполненияКоманды.Источник,
		ПараметрыВыполненияКоманды.Уникальность,
		ПараметрыВыполненияКоманды.Окно,
...
// Обработчик создания заказа на выполнение ремонтных работ
// 
// Параметры:
//  Результат - Неопределено, Структура -
//  ДополнительныеПараметры - Структура -
Процедура СоздатьЗаказНаПроизводствоТипаВыполнениеРемонтныхРаботЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Для каждого ДанныеЗаполнения Из Результат.ДанныеЗаполнения Цикл
		
		Основание = Новый Структура();
		Основание.Вставить("Основание", Результат.Основание);
		Основание.Вставить("РеквизитыШапки", ДанныеЗаполнения.РеквизитыШапки);
		Основание.Вставить("АдресТовары", ДанныеЗаполнения.АдресТовары);
...
//   ПараметрыВыполнения - см. ПодключаемыеКомандыКлиент.ПараметрыВыполненияКоманды
//
Процедура ПриемкаРемонтируемогоИзделияСоздатьНаОснованииЗаказа(МассивСсылок, ПараметрыВыполнения) Экспорт
	...
	ОчиститьСообщения();
	ПараметрыОткрытия = СозданиеНаОснованииУТВызовСервера.ПриемкаРемонтируемогоИзделияПараметрыОткрытияФормы(ПараметрКоманды);
	Если ПараметрыОткрытия = Неопределено Тогда
		Возврат;
	КонецЕсли;
