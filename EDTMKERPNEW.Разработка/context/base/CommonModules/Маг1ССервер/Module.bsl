...
Функция ПроверитьУстановкуРасширенияИВзвестиКонстанту() Экспорт
	...
	ИмяФО = Маг1СКлиентСерверПереопределяемый.ИмяПроверямойФункциональнойОпции();
	
	Если Не УжеУстановлено 
		И Не ПустаяСтрока(ИмяФО) Тогда
		ФОВключена = ПолучитьФункциональнуюОпцию(ИмяФО);
	Иначе
		ФОВключена = Истина;
	КонецЕсли;
	
	Если Не ФОВключена Тогда
		Если Константы.ПоказыватьКомандуУстановитьРасширениеМаг1с.Получить() Тогда
...
Функция ПолучитьТокенАвторизацииМенеджераВитрин(ПараметрыБазы)
	...
	Если СведенияАутентификации.КодВозврата = 200 Тогда
		...
		Возврат ПолучитьТокенАвторизацииВСервисе(
			СтрШаблон("%1://%2/%3", СтруктураАдреса.Схема, СтруктураАдреса.ИмяСервера, СтруктураАдреса.ПутьНаСервере),
...
Функция ПолучитьТокенАвторизацииВСервисе(URIСервиса, СведенияАутентификации)
	...
	ИначеЕсли Ответ.КодСостояния = 302 Тогда
		НовыйURIСервиса = Ответ.Заголовки.Получить("Location");
		Если НовыйURIСервиса <> Неопределено Тогда 
			Результат = ПолучитьТокенАвторизацииВСервисе(НовыйURIСервиса, СведенияАутентификации);
		КонецЕсли;
	Иначе
		Результат.КодВозврата       = Ответ.КодСостояния;
		Результат.СообщениеОбОшибке = ОписаниеОшибкиСервиса(
