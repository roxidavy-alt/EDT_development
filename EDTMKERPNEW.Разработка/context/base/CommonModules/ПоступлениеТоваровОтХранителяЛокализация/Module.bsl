...
Функция ПолучитьДанныеДляПечатнойФормыМХ18(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	КолонкаКодов = ФормированиеПечатныхФорм.ДополнительнаяКолонкаПечатныхФормДокументов().ИмяКолонки;
	ТекстПоляКодНоменклатуры = СтрШаблон("ВыпускПродукцииТовары.Номенклатура.%1",
		?(ЗначениеЗаполнено(КолонкаКодов),
			КолонкаКодов,
			"Код"));
	...
	|	&ТекстПоляКодНоменклатуры КАК НоменклатураКод,
	...
	|	ВЫБОР
	|		КОГДА ВыпускПродукцииТовары.Упаковка.Вес ЕСТЬ НЕ NULL
	|			ТОГДА ВыпускПродукцииТовары.КоличествоУпаковок
	|		ИНАЧЕ ВыпускПродукцииТовары.Количество
...
Функция ТекстПоступлениеТоваровОтХранителя()
	...
	|	""ПоступлениеТоваровОтХранителяЛокализация_1796"" КАК НомерФрагмента
	|
	|ИЗ
...
Функция ТекстПеремещениеТоваровЗаБалансом()
	...
	|	КОНЕЦ КАК Содержание,
	|	""ПоступлениеТоваровОтХранителяЛокализация_1797"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|	""ПоступлениеТоваровОтХранителяЛокализация_924"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
