...
Процедура ДобавитьВТекстЗапросаРеквизитыДокумента(ТекстЗапроса, ИмяТаблицы) Экспорт
	...
	|
	|	,ВЫБОР
	|		КОГДА ДанныеДокумента.СписатьЗатратыНаМодернизацию
...
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты, ПараметрыРеквизитовОбъекта) Экспорт
	...
	ДатаДок = ?(Объект.Дата <> '00010101', Объект.Дата, ТекущаяДатаСеанса());
	
	ПроверитьВозможностьВводаАмортизационнойПремииВПроцентах(Объект, Отказ);
	
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВключениеРасходовВСтоимостьОС
		И ЗначениеЗаполнено(Объект.Организация)
		И НЕ НастройкиНалоговУчетныхПолитикПовтИсп.ПрименяетсяФСБУ6(Объект.Организация, КонецМесяца(ДатаДок)) Тогда
			
		ТекстСообщения = НСтр("ru = 'Операция ""Включение доп. расходов в стоимость ОС"" доступна, если в организации применяется ФСБУ 6/2020.';
								|en = 'The ""Include additional expenses in fixed asset cost"" transaction is available if the company uses FAS 6/2020.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Объект, "ХозяйственнаяОперация",, Отказ); 
...
Процедура ПередЗаписью(Объект, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	ВнеоборотныеАктивыЛокализация.ЗаполнитьСуммыЦелевыхСредств(
		Объект.ОС, Объект.ЦелевоеФинансирование, Объект.ВариантПримененияЦелевогоФинансирования, "СтоимостьБУ");
...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	|	""МодернизацияОСЛокализация_588"" КАК НомерФрагмента
	|ИЗ
	...
	|	
	|	""Увеличение стоимости ОС"" КАК Содержание,
	|	""МодернизацияОСЛокализация_589"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьОС КАК Строки
	...
	|	""МодернизацияОСЛокализация_590"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьОС КАК Строки
	...
	|
	|	""Амортизационная премия"" КАК Содержание,
	|	""МодернизацияОСЛокализация_591"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьОС КАК Строки
	...
	|
	|	""Амортизационная премия"" КАК Содержание,
	|	""МодернизацияОСЛокализация_592"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьОС КАК Строки
	...
	|	""МодернизацияОСЛокализация_593"" КАК НомерФрагмента
	|ИЗ
	...
	|	""МодернизацияОСЛокализация_594"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьОС КАК Строки
	...
	|	""МодернизацияОСЛокализация_1995"" КАК НомерФрагмента
	|ИЗ
	...
	|
	|	""Применение инвестиционного налогового вычета"" КАК Содержание,
	|	""МодернизацияОСЛокализация_1996"" КАК НомерФрагмента
	|ИЗ
	|	РазрезыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьОС КАК Строки
	...
	|
	|	""Применение инвестиционного налогового вычета"" КАК Содержание,
	|	""МодернизацияОСЛокализация_1997"" КАК НомерФрагмента
	|ИЗ
	|	РазрезыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьОС КАК Строки
...
Процедура ДополнитьТекстЗапросаТаблицаВтИсходныеПрочиеРасходы(СписокЗапросовОбъединение) Экспорт
	...
	|
	|ГДЕ
	|	ДанныеДокумента.СписатьЗатратыНаМодернизацию
	|	И ДанныеДокумента.ОтражатьВНУ
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|			ТОГДА ДанныеДокумента.Подразделение
	|		ИНАЧЕ ДанныеДокумента.ПодразделениеСписанияЗатрат
	...
	|		ИНАЧЕ ДанныеДокумента.НаправлениеДеятельностиСписанияЗатрат
...
Функция ТекстЗапросаТаблицаПараметрыАмортизацииОСБУ() Экспорт
	...
	|
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.МодернизацияОС)
	|			ТОГДА ДанныеДокумента.Период
	|		ИНАЧЕ ПараметрыАмортизацииОСБУ.ДатаПоследнегоИзменения
	...
	|				И ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.МодернизацияОС)
	...
	|		КОГДА ДанныеДокумента.ОтражатьВБУ
	|				И ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.МодернизацияОС)
	|			ТОГДА ЕСТЬNULL(СтоимостьОС.СтоимостьЦФ, 0) 
	|					- ЕСТЬNULL(АмортизацияОС.АмортизацияЦФ, 0)
	|					+ ЕСТЬNULL(СуммаМодернизации.СтоимостьЦФ, 0)
	|
	...
	|		КОГДА ДанныеДокумента.ОтражатьВБУ
	|				И ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.МодернизацияОС)
	|			ТОГДА ЕСТЬNULL(ВтПорядокУчетаОС.ОбъемНаработки, 0)
	|					- ЕСТЬNULL(НаработкиОбъектовЭксплуатации.Значение, 0)
	|
	...
	|					- РАЗНОСТЬДАТ(ЕСТЬNULL(ПараметрыАмортизацииОСБУ.ДатаПоследнегоИзменения, ДанныеДокумента.Период), ДанныеДокумента.Период, МЕСЯЦ)
	|
	...
	|
	|ГДЕ
	|	НЕ ДанныеДокумента.СписатьЗатратыНаМодернизацию
	...
	|ВЫБРАТЬ
	|	ДанныеДокумента.Ссылка       КАК Регистратор,
	|	НАЧАЛОПЕРИОДА(ДанныеДокумента.ДатаМодернизации, ДЕНЬ) КАК Период,
	|	ДанныеДокумента.Период       КАК ДатаИсправления,
	|	ДанныеДокумента.Организация  КАК Организация,
	|	ТаблицаОС.ОсновноеСредство   КАК ОсновноеСредство,
	|	ДанныеДокумента.ДатаМодернизации КАК ДатаПоследнегоИзменения,
	|
	|	ПараметрыАмортизацииОСБУ.СрокПолезногоИспользованияБУ КАК СрокПолезногоИспользованияБУ,
	|	ПараметрыАмортизацииОСБУ.КоэффициентАмортизацииБУ     КАК КоэффициентАмортизацииБУ,
	|	ПараметрыАмортизацииОСБУ.КоэффициентУскорения         КАК КоэффициентУскорения,
	...
	|				И ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.МодернизацияОС)
	|			ТОГДА ПараметрыАмортизацииОСБУ.СрокИспользованияДляВычисленияАмортизации
	|					- РАЗНОСТЬДАТ(ЕСТЬNULL(ПараметрыАмортизацииОСБУ.ДатаПоследнегоИзменения, ДанныеДокумента.ДатаМодернизации), ДанныеДокумента.ДатаМодернизации, МЕСЯЦ)
	|
	|		ИНАЧЕ ПараметрыАмортизацииОСБУ.СрокИспользованияДляВычисленияАмортизации
	|	КОНЕЦ КАК СрокИспользованияДляВычисленияАмортизации
	|
	...
	|			И АмортизацияОС.ОбъектУчета = ТаблицаОС.ОсновноеСредство
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтПараметрыАмортизацииОСБУ_НаПрошлуюДату КАК ПараметрыАмортизацииОСБУ
	|		ПО ПараметрыАмортизацииОСБУ.Ссылка = ТаблицаОС.Ссылка
	|			И ПараметрыАмортизацииОСБУ.ОсновноеСредство = ТаблицаОС.ОсновноеСредство
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтПорядокУчетаОС КАК ВтПорядокУчетаОС
	...
	|				И ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.МодернизацияОС)
	|		ИЛИ ДанныеДокумента.ПорядокУчетаНУ = ЗНАЧЕНИЕ(Перечисление.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.НачислениеАмортизации)
	|				И ДанныеДокумента.ОтражатьВНУ)
	|";
...
Процедура ТекстЗапросаТаблицаПараметрыЦелевогоФинансированияОС(ТекстыЗапроса, Регистры)
	...
	|ГДЕ
	|	ДанныеДокумента.ОтражатьВБУ
	|	И ДанныеДокумента.ВариантПримененияЦелевогоФинансирования В (
	|			ЗНАЧЕНИЕ(Перечисление.ВариантыПримененияЦелевогоФинансирования.Полное),
	|			ЗНАЧЕНИЕ(Перечисление.ВариантыПримененияЦелевогоФинансирования.Частичное))
	|";
