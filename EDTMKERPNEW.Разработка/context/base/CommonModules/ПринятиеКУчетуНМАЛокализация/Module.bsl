...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	|	""ПринятиеКУчетуНМАЛокализация_931"" КАК НомерФрагмента
	|ИЗ
	...
	|	""ПринятиеКУчетуНМАЛокализация_932"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	""ПринятиеКУчетуНМАЛокализация_933"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьНМА КАК Строки
	...
	|	""ПринятиеКУчетуНМАЛокализация_934"" КАК НомерФрагмента
	|
	|ИЗ
	...
	|	Операция.ПервоначальнаяСтоимостьБУ - Операция.ПервоначальнаяСтоимостьНУ - Операция.ПервоначальнаяСтоимостьПР КАК СуммаВРДт,
	|	
	|	// Кт ///////////////////////////////////////////////////////////////////////////////////////////
	|	
	...
	|	Операция.ПервоначальнаяСтоимостьБУ - Операция.ПервоначальнаяСтоимостьНУ - Операция.ПервоначальнаяСтоимостьПР КАК СуммаВРКт,
	|
	|	""Принятие к учету по результатам инвентаризации"" КАК Содержание,
	|	""ПринятиеКУчетуНМАЛокализация_935"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПринятиеКУчетуНМА2_4 КАК Операция
	...
	|	""ПринятиеКУчетуНМАЛокализация_1999"" КАК НомерФрагмента
	|ИЗ
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СтруктураИзмененныхРеквизитов) Экспорт
	...
	Если СтруктураИзмененныхРеквизитов.Свойство("Организация")
		...
		ВнеоборотныеАктивыСлужебный.УстановитьСвойствоСтруктуры(
			"ПоддержкаПБУ18",
			ПараметрыПБУ18.ПрименяетсяПБУ18,
			Форма.СлужебныеПараметрыФормы);
		
		ВнеоборотныеАктивыСлужебный.УстановитьСвойствоСтруктуры(
			"ВедетсяУчетПостоянныхИВременныхРазниц",
			ПараметрыПБУ18.ВедетсяУчетПостоянныхИВременныхРазниц,
			Форма.СлужебныеПараметрыФормы);
		...
		Если Форма.СлужебныеПараметрыФормы.ВариантУчетаОтложенногоНалога = ПредопределенноеЗначение("Перечисление.ВариантыУчетаОтложенногоНалога.БалансовыйМетод") Тогда
			...
			Если Объект.ПервоначальнаяСтоимостьПР <> 0 Тогда
				Элементы.ФормаПоказатьПР.Пометка = Истина;
			КонецЕсли;
...
Функция ТекстЗапросаТаблицаСтоимостьНМА(Запрос, ТекстыЗапроса) Экспорт
	...
	|	ДанныеДокумента.ПервоначальнаяСтоимостьБУ - ДанныеДокумента.ПервоначальнаяСтоимостьНУ - ДанныеДокумента.ПервоначальнаяСтоимостьПР КАК СтоимостьВР,
	|
	|	0 КАК СтоимостьЦФ,
	|	0 КАК СтоимостьПРЦФ,
	|	
