...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	|	МАКСИМУМ(ЕСТЬNULL(Расчеты.ИдентификаторФинЗаписи, """")) КАК ИдентификаторФинЗаписи,
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ЕСТЬNULL(ОбъектыРасчетов.Подразделение, КорректировкаПриобретения.Подразделение),
	|	КорректировкаПриобретения.Ссылка,
	|	ЕСТЬNULL(ДоговорыНеотфактуровки.ГруппаФинансовогоУчета, ЕСТЬNULL(ОбъектыРасчетов.ГруппаФинансовогоУчета, ЕСТЬNULL(Договоры.ГруппаФинансовогоУчета, ПриобретениеТоваровУслуг.ГруппаФинансовогоУчета))),
	|	ЕСТЬNULL(ДоговорыНеотфактуровки.НаправлениеДеятельности, ЕСТЬNULL(ОбъектыРасчетов.НаправлениеДеятельности, ЕСТЬNULL(Договоры.НаправлениеДеятельности, ПриобретениеТоваровУслуг.НаправлениеДеятельности))),
	|	ЕСТЬNULL(ОбъектыРасчетов.Контрагент, КорректировкаПриобретения.Контрагент),
	|	ЕСТЬNULL(ОбъектыРасчетов.Договор, КорректировкаПриобретения.Договор),
	|	ЕСТЬNULL(ПриобретениеТоваровУслуг.ХозяйственнаяОперация, НЕОПРЕДЕЛЕНО) В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки), ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСФактуровкаПоставки)),
	|	ЕСТЬNULL(ПриобретениеТоваровУслуг.ХозяйственнаяОперация, НЕОПРЕДЕЛЕНО) В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаТоварыВПути), ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСТоварыВПути), ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаПоИмпортуТоварыВПути)),
	|	ЕСТЬNULL(ПриобретениеТоваровУслуг.ВернутьМногооборотнуюТару, ЛОЖЬ),
	|	НЕ Расчеты.Регистратор ЕСТЬ NULL,
	|	КорректировкаПриобретения.ВалютаВзаиморасчетов,
...
Функция ТекстЗапросаТаблицаОперацииСПрослеживаемымиТоварамиРасширенный(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР
	|		КОГДА &ХозОперацияОснования В (&ХозОперацииЗакупкаВСтранахЕАЭС)
...
Функция КорректировкаТоваров()
	...
	|	""КорректировкаПриобретенияЛокализация_444"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция КорректировкаНаДоходыПоДокументу()
	...
	|	""КорректировкаПриобретенияЛокализация_1725"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция КорректировкаТоваровДвухходовка()
	...
	|		КОНЕЦ КАК Содержание,
	|	""КорректировкаПриобретенияЛокализация_1726"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция КорректировкаРаботУслугДвухходовка()
	...
	|	""КорректировкаПриобретенияЛокализация_445"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	И (Расчеты.Договор.ВариантОформленияЗакупок = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияЗакупок.НеотфактурованныеПоставкиТоваровИУслуг) ИЛИ
	|		Расчеты.Договор.ВариантОформленияДокументовПоступленияПоУслугам = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется))
...
Функция КорректировкаТоваровДвухходовкаДоходы()
	...
	|	""КорректировкаПриобретенияЛокализация_446"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ОтклоненияВСтоимостиПриФактуровке()
	...
	|	""КорректировкаПриобретенияЛокализация_447"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|	Операция.Дата			КАК Период,
	|	Операция.Организация	КАК Организация,
	|	ДвиженияПрочиеРасходы.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|
	...
	|	""КорректировкаПриобретенияЛокализация_448"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция КорректировкаУслугНаРасходы()
	...
	|		ИНАЧЕ Расчеты.ИдентификаторФинЗаписи
	...
	|	""КорректировкаПриобретенияЛокализация_449"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|				ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар),
	|				ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)))
	|	И НЕ ((Операция.Договор.ВариантОформленияЗакупок = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияЗакупок.НеотфактурованныеПоставкиТоваровИУслуг) ИЛИ
	|			Операция.Договор.ВариантОформленияДокументовПоступленияПоУслугам = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется))
	|			И Расчеты.КорректировкаФактуровкиПоставки
	|			И НЕ Строки.ВариантОтражения В (
	|				ЗНАЧЕНИЕ(Перечисление.ВариантыОтраженияКорректировокПоступлений.ОтразитьНаПрочихДоходах),
	|				ЗНАЧЕНИЕ(Перечисление.ВариантыОтраженияКорректировокПоступлений.СписатьНаРасходы)))
...
Функция КорректировкаУслугНаРасходыНоваяПереработка()
	...
	|	Суммы.СуммаПриведеннаяРегл - Суммы.СуммаБезНДСРегл КАК СуммаВРКт,
	|	""Корректировка приобретения услуг"" КАК Содержание,
	|	""КорректировкаПриобретенияЛокализация_450"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция КорректировкаНаДоходыПоСтрокам()
	...
	|	КОНЕЦ КАК Содержание,
	|	""КорректировкаПриобретенияЛокализация_1727"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ЗакупкаНДС()
	...
	|	""КорректировкаПриобретенияЛокализация_451"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция КорректировкаНДС()
	...
	|	0 КАК СуммаВРКт,
	|	""Корректировка НДС"" КАК Содержание,
	|	""КорректировкаПриобретенияЛокализация_452"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ЗачетАванса()
	...
	|	""КорректировкаПриобретенияЛокализация_453"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ВыделениеАванса()
	...
	|	""КорректировкаПриобретенияЛокализация_454"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстПроцентыПоДисконтированию()
	...
	|	""КорректировкаПриобретенияЛокализация_455"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстВключениеНДСпоПриобретеннымЦенностям() 
	...
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	Доходы.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|
	|	-НДСПредъявленный.НДС    КАК Сумма,
	|	-НДСПредъявленный.НДСУпр КАК СуммаУУ,
	|
	...
	|		КОГДА СтатьиДоходов.ПринятиеКНалоговомуУчету
	|		ТОГДА 0
	|		ИНАЧЕ Доходы.ПостояннаяРазница
	|	КОНЕЦ КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|
	|	""Включение НДС в стоимость"" КАК Содержание,
	|	""КорректировкаПриобретенияЛокализация_456"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстРегистрацияОтклоненийПлановойСтоимости() Экспорт
	...
	|	КОНЕЦ КАК Содержание,
	|	""КорректировкаПриобретенияЛокализация_1728"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|		И Строки.НастройкаХозяйственнойОперации <> ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.НачислениеАкцизов)
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Расчеты КАК Расчеты
	|	ПО 
