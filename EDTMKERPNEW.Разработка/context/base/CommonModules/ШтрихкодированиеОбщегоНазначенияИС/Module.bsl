...
Функция ТекстЗапросаСвойстваМаркируемойПродукции(ТаблицаИсточник = "ДанныеШтрихкодовУпаковок", ПараметрыСканирования = Неопределено) Экспорт
	...
	ИначеЕсли Ограничения.ИСМП Тогда
		...
		|		И КлючиВидовПродукцииИС.ВидПродукцииГИСМТ <> ЗНАЧЕНИЕ(Справочник.ВидыПродукцииИС.ПустаяСсылка)) КАК МаркируемаяПродукция,
		|	МАКСИМУМ(ЕСТЬNULL(КлючиВидовПродукцииИС.Ссылка, ЗНАЧЕНИЕ(Справочник.КлючиВидовПродукцииИС.ПустаяСсылка))) КАК ВидПродукции
		|ПОМЕСТИТЬ СвойстваМаркируемойПродукции
		|ИЗ
		|	ТаблицаИсточник КАК ДанныеШтрихкодовУпаковок
			...
			Если НужноИзменитьТипКолонки Тогда
				...
				Иначе
					...
					ВызватьИсключение НСтр("ru = 'Изменение типа заполненной коллекции';
											|en = 'Изменение типа заполненной коллекции'");
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		ИначеЕсли НеОпределенВидПродукции Тогда
			...
			СтрокаДанных.ТекстОшибки = НСтр("ru = 'Не удалось определить вид продукции';
											|en = 'Не удалось определить вид продукции'");
