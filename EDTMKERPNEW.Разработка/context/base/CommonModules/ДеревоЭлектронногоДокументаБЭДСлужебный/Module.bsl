...
Процедура ПроверитьСтроковыеПараметрыНаПревышениеДлины(ДеревоДанных, ПараметрыПроверки, Ошибки)
	...
	Пояснение = НСтр("ru = 'Некоторые символы ("", &, <, >) занимают больше места.';
					|en = 'Некоторые символы ("", &, <, >) занимают больше места.'");
	...
	Для Каждого СтрокаТаблицы Из ПараметрыПроверки.ТаблицаСтрок Цикл
		...
		Если СтрДлина(СтрокаДанных) > СтрокаТаблицы.Длина Тогда
			...
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Длина строки превышает допустимый размер %1 %2. %3';
										|en = 'Длина строки превышает допустимый размер %1 %2. %3'"),
	...
	ИначеЕсли ЗначениеСтрокиДереваЗаполнено И СтрокаДерева.ТипЗначения <> Неопределено Тогда
		...
		Если ДатаПоФорматуФНС И Не ЗначениеДатыВСтрокеДереваСоответствуетТипуДатыФНС(СтрокаДерева) Тогда
			...
			ШаблонОшибки = НСтр("ru = 'неверное значение даты ""%1"" (интервал значений: 01.01.1900 - 31.12.2099)';
								|en = 'invalid ""%1"" date value (range of values: 01/01/1900 - 12/31/2099)'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, СтрокаДерева.Значение);
			Возврат;
