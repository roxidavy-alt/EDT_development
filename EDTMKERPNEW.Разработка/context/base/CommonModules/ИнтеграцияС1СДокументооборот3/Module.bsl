	...
	Если ОтветНаЗапросыПакета.responses.Количество() = 2 Тогда
		...
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьТип(Прокси, ДанныеДокумента, "DMError") Тогда
			...
			ИмяСобытия = НСтр("ru = 'Получение настроек доступности по состоянию объекта';
								|en = 'Get availability settings based on object state'");
			...
			ТекстОшибки.Добавить(СтрШаблон(
				НСтр("ru = 'Не удалось получить настройки доступности по состоянию объекта ДО (ID = ""%1"", тип = ""%2"") по причине:';
					|en = 'Cannot get the availability settings based on the DI object state (ID = ""%1"", type = ""%2"") due to:'"),
...
Процедура ЗаполнитьЗакладкуОбзорВФорме(Форма, ОбъектДОПредставлениеXDTO, ОбъектXDTO) Экспорт
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ОбъектXDTO, "EDIDocuments") Тогда
		Для Каждого ДокументЭДО Из ОбъектXDTO.EDIDocuments Цикл
			...
			НоваяСтрока.Наименование = СтрШаблон(НСтр("ru = '%1 (Документ ЭДО)';
														|en = '%1 (EDI document)'"), ДокументЭДО.objectID.presentation);
	...
	Если КоличествоФайлов = 0 Тогда
		Элементы.НаименованиеФайла.Заголовок = СтрШаблон(НСтр("ru = '%1 и файлы';
																|en = '%1 and files'"), ПредставлениеОбъекта);
	Иначе
		Элементы.НаименованиеФайла.Заголовок =
			СтрШаблон(НСтр("ru = '%1 и файлы (%2)';
							|en = '%1 and files (%2)'"), ПредставлениеОбъекта, КоличествоФайлов);
	КонецЕсли;
		...
		Если ПравилоИнтеграции.ВидДокументаДОID <> ВидДокументаДОИзНастройки Тогда
			ЗначенияКВыгрузке[ЗначениеРеквизитаИС].Добавить(СтрШаблон(
				НСтр("ru = 'Создание связи ""%1"". Вид документа ""%2"", указанный в правиле интеграции, не совпадает с видом документа ""%3"", указанным в настройке автоматического создания связей.';
					|en = 'Create the ""%1"" link. The ""%2"" document type specified in the integration rule does not match the ""%3"" document type specified in the automatic link creation setting.'"),
				ПредставлениеСвязи,
				...
				Иначе
					ТекстОшибки = СтрШаблон(
						НСтр("ru = 'Не удалось выгрузить объект ""%1"" в 1С:Документооборот:
							|%2';
							|en = 'Cannot export the ""%1"" object to 1C:Document Management:
							|%2'"),
						ПодходящийОбъект.Ссылка,
...
Функция ПреобразоватьРеквизитОбъектаИСВОбъектXDTO(Прокси, ИмяРеквизита, ЗначениеРеквизита, Выражение = "")
	...
	Иначе
		...
		ТекстОшибки.Добавить(НСтр("ru = 'Ошибка формирования структуры загружаемых в 1С:Документооборот данных.';
									|en = 'An error occurred while generating the structure of data imported to 1C:Document Management.'"));
		Если ЗначениеЗаполнено(Выражение) Тогда
			ТекстОшибки.Добавить(СтрШаблон(
				НСтр("ru = 'Невозможно выгрузить данные из результата выражения на встроенном языке.
					...
					|%2';
					|en = 'Cannot export data from the result of an expression in the 1C:Enterprise language.
					|Result type: ""%1"".
					|Expression text in the 1C:Enterprise language:
					|%2'"),
				ТипРеквизита,
				Выражение));
		Иначе
