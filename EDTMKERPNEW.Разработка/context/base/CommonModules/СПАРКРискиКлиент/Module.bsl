...
Процедура ПриОткрытии(Форма, КонтрагентОбъект = Неопределено) Экспорт
	...
	Если СвойстваКонтрагента.ПодлежитПроверке = Истина Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ИндексыСПАРКРиски")
			И (ТипЗнч(Форма.ИндексыСПАРКРиски) = ТипСтруктура)
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ФакторыРискаСПАРК")
			И (ТипЗнч(Форма.ФакторыРискаСПАРК) = ТипСтруктура)
			И (Форма.ФакторыРискаСПАРК.ОжиданиеОтвета) Тогда
			
			СПАРКФакторыРискаКлиент.ПриОткрытии(Форма, КонтрагентОбъект);
...
Процедура ОбработкаНавигационнойСсылки(
	...
	Если СтандартнаяОбработкаБиблиотекой = Истина Тогда
		...
		ИначеЕсли ВРег(НавигационнаяСсылка) = ВРег(Настройки.ПодключитьНавигационнаяСсылкаПодсказки) Тогда // Подключение тестового периода.
			...
			ДополнительныеПараметры.Вставить("ИдентификаторВызова", "ФормаДанныхСервиса");
		...
		ИначеЕсли ВРег(НавигационнаяСсылка) = ВРег(Настройки.ПоказатьРасширеннуюИнформацию) Тогда
			
			ПараметрыФормы = Новый Структура("ИНН, Контрагент");
			
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ИндексыСПАРКРиски")
				И (ТипЗнч(Форма.ИндексыСПАРКРиски) = Тип("Структура")) Тогда
				
				ЗаполнитьЗначенияСвойств(ПараметрыФормы, Форма.ИндексыСПАРКРиски);
				
			КонецЕсли;
			
			Если ПустаяСтрока(ПараметрыФормы.ИНН)
				И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ФакторыРискаСПАРК")
				И (ТипЗнч(Форма.ФакторыРискаСПАРК) = Тип("Структура")) Тогда
				
				ЗаполнитьЗначенияСвойств(ПараметрыФормы, Форма.ФакторыРискаСПАРК);
				
			КонецЕсли;
			
			Если ПустаяСтрока(ПараметрыФормы.ИНН) Тогда
				Возврат;
			КонецЕсли;
			
			ПоказатьРасширенныеДанныеСПАРК(ПараметрыФормы);
...
Процедура ОбработкаОповещения(Форма, КонтрагентОбъект, ИмяСобытия, Параметр, Источник) Экспорт
	...
	Если ВРег(ИмяСобытия) = ВРег("СПАРКРиски:ИзмененоСостояниеФоновогоЗадания")
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ИндексыСПАРКРиски")
		И Форма.ИндексыСПАРКРиски = Неопределено Тогда
	...
	Если Не СвойстваКонтрагента.ПодлежитПроверке Тогда
		Возврат;
	КонецЕсли;
	...
	Если Не СтандартнаяОбработкаБиблиотекой Тогда
		Возврат;
	КонецЕсли;
	
	Если ВРег(ИмяСобытия) = ВРег("СПАРКРиски:ИзмененоСостояниеФоновогоЗадания") Тогда
		ОбработатьИзменениеСостоянияФоновогоЗадания(Форма, Параметр, Источник);
	ИначеЕсли ВРег(ИмяСобытия) = ВРег("СПАРКРиски:ЗагруженыФакторыРиска") Тогда
		СПАРКФакторыРискаКлиент.ОбработатьЗагрузкуФакторовРиска(Форма, Параметр, Источник);
	КонецЕсли;
...
Процедура ПерейтиВВебВерсиюСПАРК(
	...
	ИначеЕсли Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.НекорректныйЗапрос") Тогда
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<b>Переход в веб-версию СПАРК недоступен </b>
				...
				|en = '<b>Cannot go to SPARK online</b> 
				|
				|Errors occurred while receiving a URL. Contact the administrator.'"));
		
	ИначеЕсли Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.НеизвестныйИНН") Тогда
		
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<b>Переход в веб-версию СПАРК недоступен </b>
				|
				|Нет информации о контрагенте, переход в веб-версию невозможен.';
				|en = '<b>Navigation to SPARK online is unavailable</b> 
				|
				|There is no information about the counterparty, navigation to online version is not allowed.'"));
		...
		Для Каждого ТекущееПроверяемоеФоновоеЗадание Из ПроверяемыеФоновыеЗадания Цикл
			Если ТекущееПроверяемоеФоновоеЗадание.Статус = "Выполняется" Тогда
				...
				Иначе
					...
					Оповестить(
						"СПАРКРиски:ИзмененоСостояниеФоновогоЗадания",
						ТекущееПроверяемоеФоновоеЗадание,
						ТекущееПроверяемоеФоновоеЗадание.ИдентификаторФормы);
				КонецЕсли;
			Иначе
				// Оповестить форму о статусе задания.
				Оповестить(
...
Процедура ОбработатьИзменениеСостоянияФоновогоЗадания(Форма, Параметр, Источник)
...
Процедура ПоказатьРасширенныеДанныеСПАРК(ПараметрыФормы)
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПроверкаКонтрагента") Тогда
		МодульПроверкаКонтрагентаКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ПроверкаКонтрагентаКлиент");
		МодульПроверкаКонтрагентаКлиент.ПоказатьРасширенныеДанныеСПАРК(
			ПараметрыФормы);
	Иначе
		ОткрытьФорму("Обработка.СправкаСПАРКРиски.Форма.РасширеннаяИнформацияСПАРКРиски", ПараметрыФормы);
	КонецЕсли;
...
Процедура ПодключитьТестовыйПериодЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	Если Результат = ПредопределенноеЗначение("Перечисление.СостоянияПодключенияСервисов.Подключен")
		...
		ИначеЕсли ДополнительныеПараметры.ИдентификаторВызова = "ФормаДанныхСервиса" Тогда
			
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДополнительныеПараметры.Форма, "ИндексыСПАРКРиски") Тогда
			...
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДополнительныеПараметры.Форма, "ФакторыРискаСПАРК") Тогда
				ДополнительныеПараметры.Форма.ФакторыРискаСПАРК = Неопределено;
				ДополнительныеПараметры.Форма.ПодключитьОбработчикОжидания(
					"Подключаемый_ОбновитьОтображениеФакторовРискаСПАРК",
					0.1,
					Истина);
			КонецЕсли;
...
Процедура ПерейтиКПокупкеСервиса()
	...
	ПоказатьВопрос(
		ОписаниеОповещения,
		НСтр("ru = 'Доступен заказ сервиса у партнера, либо покупка сервиса непосредственно в фирме 1С';
