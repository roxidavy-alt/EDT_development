...
Функция СформироватьСтруктуруТовараПриЗагрузке(ПоказателиТовар, ДиапазонКурсов) Экспорт
	Результат = Новый Структура;
	...
	Результат.Вставить("ЕстьОшибка", ПроверитьЗаполнениеПоказателейТовара(ПоказателиТовар, ДиапазонКурсов));
...
Функция ДиапазонКурсовДоллараВПериоде(ДатаВМесяце) Экспорт 
	Результат = Новый Структура("Минимум, Максимум", 0, 0);
	...
	Если Не ЗначениеЗаполнено(ВалютаДоллар) Тогда 
		Возврат Результат;
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	ЕСТЬNULL(МАКСИМУМ(КурсыВалют.Курс), 0) КАК Максимум,
	|	ЕСТЬNULL(МИНИМУМ(КурсыВалют.Курс), 0) КАК Минимум
	|ИЗ
	|	РегистрСведений.КурсыВалют КАК КурсыВалют
	|ГДЕ
	|	КурсыВалют.Валюта = &ВалютаДоллар
	|	И КурсыВалют.Период >= &НачалоМесяца
	|	И КурсыВалют.Период <= &КонецМесяца";
	
	Запрос.УстановитьПараметр("ВалютаДоллар", ВалютаДоллар);
	Запрос.УстановитьПараметр("НачалоМесяца", НачалоМесяца(ДатаВМесяце));
	Запрос.УстановитьПараметр("КонецМесяца", КонецМесяца(ДатаВМесяце));
...
Функция ПроверитьЗаполнениеПоказателейТовара(Показатели, ДиапазонКурсов) Экспорт 
	...
	Если ЗначениеЗаполнено(ДиапазонКурсов.Минимум) И ЗначениеЗаполнено(ДиапазонКурсов.Максимум) Тогда 
		Если (((1.0 * Показатели.StatisticalCostRUB) / Показатели.StatisticalCostUSD) <= 0.9 * ДиапазонКурсов.Минимум)
			Или (((1.0 * Показатели.StatisticalCostRUB) / Показатели.StatisticalCostUSD) >= 1.1 * ДиапазонКурсов.Максимум) Тогда 
			
			Возврат Истина;
		КонецЕсли;
	ИначеЕсли Не СоотношениеКурсаДолларРубльКорректно(Показатели.StatisticalCostUSD, Показатели.StatisticalCostRUB) Тогда 
		Возврат Истина;
	КонецЕсли;
