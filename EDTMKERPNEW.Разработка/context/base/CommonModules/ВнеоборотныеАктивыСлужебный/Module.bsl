...
Процедура ПроверитьВозможностьПринятияКУчетуОС(Объект, Отказ) Экспорт
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПринятиеКУчетуОС2_4")
		ИЛИ ТипЗнч(Объект) = Тип("ДокументОбъект.ВводОстатковВнеоборотныхАктивов2_4") Тогда
...
Процедура ПроверитьЧтоОСПринятыКУчету(Объект, Отказ) Экспорт
	...
	ЕстьВыборНУ = 
		...
		ИЛИ ТипДокумента = Тип("ДокументОбъект.МодернизацияОС2_4")
		ИЛИ ТипДокумента = Тип("ДокументОбъект.ПодготовкаКПередачеОС2_4")
		ИЛИ ТипДокумента = Тип("ДокументОбъект.МодернизацияОС2_4")
		ИЛИ ТипДокумента = Тип("ДокументОбъект.ОтнесениеОтклоненийВСтоимостьОС")
		ИЛИ ТипДокумента = Тип("ДокументОбъект.КорректировкаСтоимостиИАмортизацииОС")
		ИЛИ ТипДокумента = Тип("ДокументОбъект.СписаниеОС2_4");
	//-- Локализация
...
Процедура ПроверитьЧтоНМАПринятыКУчету(Объект, Отказ) Экспорт
	...
	ЕстьВыборНУ = 
		...
		ИЛИ ТипДокумента = Тип("ДокументОбъект.СписаниеНМА2_4");
	//-- Локализация
...
Процедура ПроверитьЧтоВыбраныОС(Объект, ИмяТЧ, ИмяПоля, Отказ) Экспорт
	...
	ПроверитьТипОС(ДоступныеТипыОС, Объект, ИмяТЧ, ИмяПоля, Отказ);
	
КонецПроцедуры

Процедура ПроверитьЧтоВыбраныУзлыКомпоненты(Объект, ИмяТЧ, ИмяПоля, Отказ) Экспорт
	
	ДоступныеТипыОС = Новый Массив;
	ДоступныеТипыОС.Добавить(Перечисления.ТипыОС.Узел);
	ДоступныеТипыОС.Добавить(Перечисления.ТипыОС.КомпонентАмортизации);

	ПроверитьТипОС(ДоступныеТипыОС, Объект, ИмяТЧ, ИмяПоля, Отказ);
	
КонецПроцедуры

Процедура ПроверитьЧтоВыбраныУзлы(Объект, ИмяТЧ, ИмяПоля, Отказ) Экспорт
...
Процедура ПроверитьСозданиеДокументаНаОснованииГруппыОС(СписокОС) Экспорт
	...
	|ГДЕ
	|	ОбъектыЭксплуатации.Ссылка В (&СписокОС)
	|	И ОбъектыЭксплуатации.ЭтоГруппа";
	...
	Если НЕ Результат.Пустой() Тогда
		...
		ТекстСообщения = НСтр("ru = 'Создание документа на основании группы ОС не поддерживается';
								|en = 'Cannot create the document based on a fixed asset group'");
...
Процедура ДополнитьПараметрамиУчетаВнеоборотныхАктивов(Знач ПараметрыРасчета) Экспорт
	...
	ПараметрыРасчета.Вставить("ПрименяетсяПорядокУчетаВНАПоСтандартамМУ", Истина);
	ПараметрыРасчета.Вставить("НеПрименяетсяПорядокУчетаВНАПоСтандартамМУ", Ложь);
...
//  ИмяТаблицыСписокОбъектовУчета - Строка - Имя таблицы, содержащей список объектов
//  ВычислениеПолей - Структура, Неопределено - Вычисление полей
Процедура ТекстЗапросаПараметрыРасчетаАмортизации(ИмяРегистра, ИмяТаблицы, ОбъектУчета, СписокЗапросов, ИмяТаблицыСписокОбъектовУчета = "", ВычислениеПолей = Неопределено) Экспорт

	Если ВычислениеПолей = Неопределено Тогда
		ВычислениеПолей = Новый Структура;
	КонецЕсли;
	
	ВнеоборотныеАктивыЛокализация.ТекстЗапросаПараметрыРасчетаАмортизации(ИмяРегистра, ИмяТаблицы, ОбъектУчета, СписокЗапросов, ВычислениеПолей);
	...
	Если РегистрПоддерживаетИсправление Тогда
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МАКСИМУМ(ЕСТЬNULL(ТаблицаРегистра.ДатаИсправления, ДАТАВРЕМЯ(1,1,1))) КАК ДатаИсправления,
		|
		|	ВЫБОР
		|		КОГДА МАКСИМУМ(ЕСТЬNULL(ТаблицаРегистра.ДатаИсправления, ДАТАВРЕМЯ(1,1,1))) <> ДАТАВРЕМЯ(1,1,1)
	...
	|				СписокОбъектовУчета.ПолеОбъектУчета
	|			ИЗ
	|				СписокОбъектовУчетаСДатыПринятияКУчету КАК СписокОбъектовУчета)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|		И ТаблицаРегистра.Активность
	|
	|		И ТИПЗНАЧЕНИЯ(ТаблицаРегистра.Регистратор) В (&ТипДокументыДляДоначисления)
	|
	|		И (ТаблицаРегистра.ПолеОбъектУчета) В
	|				(ВЫБРАТЬ
	|					СписокОбъектовУчета.ПолеОбъектУчета
	...
	|		ТаблицаРегистра.ПолеОбъектУчета
	|
	|	) КАК ТаблицаРегистра_Срез
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаРегистраИсточник КАК ТаблицаРегистра
	|		ПО ТаблицаРегистра.Регистратор = ТаблицаРегистра_Срез.Регистратор
	|			И НАЧАЛОПЕРИОДА(ТаблицаРегистра.Период, ДЕНЬ) = ТаблицаРегистра_Срез.Период
	|			И ТаблицаРегистра.ПолеОрганизация = ТаблицаРегистра_Срез.Организация
	|			И ТаблицаРегистра.ПолеОбъектУчета = ТаблицаРегистра_Срез.ПолеОбъектУчета
	|			И ТаблицаРегистра.ДатаИсправления = ДАТАВРЕМЯ(1,1,1)
	|			И ТаблицаРегистра.Активность
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	Если ЗначениеЗаполнено(ИмяТаблицыСписокОбъектовУчета) Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "СписокОбъектовУчета", ИмяТаблицыСписокОбъектовУчета);
	КонецЕсли;
...
Функция ТекстЗапросаТаблицаОтражениеНачисленнойАмортизацииВПрочихРасходах(ДопПоля = "") Экспорт
	...
	Если ТекстЗапроса = Неопределено Тогда
		...
		| 
		|	ВЫБОР
		|		КОГДА ТаблицаРасходы.ЭтоКорректировкаАмортизацииЗаПрошлыйПериод
		|			ТОГДА ТаблицаРасходы.ПериодНачисления
		...
		ЗаменитьДополнительныеПоляВЗапросе(ТекстЗапроса, ДопПоля, "Регистратор,ПериодОтклонения");
...
Функция ТекстЗапросаТаблицаОтражениеОбесцененияВПрочихРасходах(ДополнительныеПоля = "") Экспорт
	...
	ЗаменитьДополнительныеПоляВЗапросе(ТекстЗапроса, ДополнительныеПоля, "Регистратор,ПериодОтклонения");	
...
// Возвращаемое значение:
//	Строка -  
Функция ТекстЗапросаСписанияСтоимостиОС(ТекстыЗапроса, Запрос = Неопределено) Экспорт
	...
	|	втСписаниеСтоимости.Местонахождение    КАК Подразделение,
	|	втСписаниеСтоимости.ОсновноеСредство   КАК ОсновноеСредство,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВУпрУчете 
	|			ТОГДА втСписаниеСтоимости.Стоимость 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК Стоимость,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВБУ 
	|			ТОГДА втСписаниеСтоимости.СтоимостьРегл 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК СтоимостьРегл,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВНУ 
	|			ТОГДА втСписаниеСтоимости.СтоимостьНУ 
	|		ИНАЧЕ 0 
	...
	|					И втСписаниеСтоимости.СтоимостьРегл = 0
	|
	|			ТОГДА втСписаниеСтоимости.СтоимостьПР
	|	
	|		ИНАЧЕ 0 
	...
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВБУ 
	|			ТОГДА втСписаниеСтоимости.СтоимостьЦФ 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК СтоимостьЦФ,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВБУ 
	|			ТОГДА втСписаниеСтоимости.СтоимостьНУЦФ 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК СтоимостьНУЦФ,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВБУ 
	|			ТОГДА втСписаниеСтоимости.СтоимостьПРЦФ 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК СтоимостьПРЦФ,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВБУ 
	|			ТОГДА втСписаниеСтоимости.СтоимостьЦФ - втСписаниеСтоимости.СтоимостьНУЦФ - втСписаниеСтоимости.СтоимостьПРЦФ
	|		ИНАЧЕ 0 
	...
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВНУ 
	|			ТОГДА втСписаниеСтоимости.СуммаРасходовНУПриИНВ 
	|		ИНАЧЕ 0 
	|	КОНЕЦ                                  КАК СуммаРасходовНУПриИНВ,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВНУ 
	|			ТОГДА втСписаниеСтоимости.АмортизационнаяПремия 
	|		ИНАЧЕ 0 
	...
	|	
	|	втСписаниеСтоимости.ГруппаФинансовогоУчета  КАК ГруппаФинансовогоУчета,
	|	втСписаниеСтоимости.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	втСписаниеСтоимости.Арендатор               КАК Арендатор,
	...
	|	
	|	втСписаниеСтоимости.ИдентификаторСтроки КАК ИдентификаторФинЗаписи,
	|	&НастройкаХО_СписаниеСтоимостиОС       КАК НастройкаХозяйственнойОперации
	|
	|ИЗ
	|	ДанныеДокументаРеквизиты КАК ДанныеДокумента
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втСписаниеСтоимостиОС КАК втСписаниеСтоимости
	|		ПО втСписаниеСтоимости.Ссылка = ДанныеДокумента.Ссылка
	|";
	СписокЗапросов.Добавить(ТекстЗапроса);  
	#КонецОбласти
	
	#Область СписаниеРезерваПереоценки
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	|	ДанныеДокумента.Ссылка                 КАК Регистратор,
	...
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВУпрУчете 
	|			ТОГДА втСписаниеСтоимости.РезервПереоценкиСтоимости 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК РезервПереоценкиСтоимости,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВБУ 
	|			ТОГДА втСписаниеСтоимости.РезервПереоценкиСтоимостиРегл 
	|		ИНАЧЕ 0 
	...
	|
	|ГДЕ
	|	(ДанныеДокумента.ОтражатьВУпрУчете И втСписаниеСтоимости.РезервПереоценкиСтоимости <> 0
	|		ИЛИ ДанныеДокумента.ОтражатьВБУ И втСписаниеСтоимости.РезервПереоценкиСтоимостиРегл <> 0)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВУпрУчете 
	|			ТОГДА втСписаниеСтоимости.ПриростСтоимости 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК ПриростСтоимости,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВБУ 
	|			ТОГДА втСписаниеСтоимости.ПриростСтоимостиРегл 
	|		ИНАЧЕ 0 
	...
	|
	|ГДЕ
	|	(ДанныеДокумента.ОтражатьВУпрУчете И втСписаниеСтоимости.ПриростСтоимости <> 0
	|		ИЛИ ДанныеДокумента.ОтражатьВБУ И втСписаниеСтоимости.ПриростСтоимостиРегл <> 0)
	|";
	СписокЗапросов.Добавить(ТекстЗапроса);  
	#КонецОбласти
	
	#Область СписаниеПредварительнойСтоимости
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	|	ДанныеДокумента.Ссылка                 КАК Регистратор,
	...
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВУпрУчете 
	|			ТОГДА втСписаниеСтоимости.ПредварительнаяСтоимость 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК ПредварительнаяСтоимость,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВБУ 
	|			ТОГДА втСписаниеСтоимости.ПредварительнаяСтоимостьРегл 
	|		ИНАЧЕ 0 
	...
	|			И втСписаниеСтоимости.ПредварительнаяСтоимость <> 0
	|		ИЛИ ДанныеДокумента.ОтражатьВБУ
	|			И втСписаниеСтоимости.ПредварительнаяСтоимостьРегл <> 0)
	|";
	СписокЗапросов.Добавить(ТекстЗапроса);
	#КонецОбласти
...
Функция ТекстЗапросаСписанияАмортизацииОС(ТекстыЗапроса) Экспорт
	...
	|	ДанныеДокумента.Период                       КАК Период,
	|	ДанныеДокумента.Организация                  КАК Организация,
	|	втСписаниеАмортизации.Местонахождение        КАК Подразделение,
	|	втСписаниеАмортизации.ОсновноеСредство       КАК ОсновноеСредство,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВУпрУчете 
	|			ТОГДА втСписаниеАмортизации.Амортизация 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК Амортизация,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВБУ 
	|			ТОГДА втСписаниеАмортизации.АмортизацияРегл 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК АмортизацияРегл,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВНУ 
	|			ТОГДА втСписаниеАмортизации.АмортизацияНУ 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК АмортизацияНУ,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВБУ 
	|			ТОГДА втСписаниеАмортизации.АмортизацияПР 
	|		ИНАЧЕ 0 
	...
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВБУ 
	|			ТОГДА втСписаниеАмортизации.АмортизацияЦФ
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК АмортизацияЦФ,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВБУ 
	|			ТОГДА втСписаниеАмортизации.АмортизацияНУЦФ 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК АмортизацияНУЦФ,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВБУ 
	|			ТОГДА втСписаниеАмортизации.АмортизацияПРЦФ 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК АмортизацияПРЦФ,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВБУ 
	|			ТОГДА втСписаниеАмортизации.АмортизацияЦФ - втСписаниеАмортизации.АмортизацияНУЦФ - втСписаниеАмортизации.АмортизацияПРЦФ
	|		ИНАЧЕ 0
	...
	|
	|	втСписаниеАмортизации.ГруппаФинансовогоУчета КАК ГруппаФинансовогоУчета,
	|	втСписаниеАмортизации.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	&ХО_СписаниеАмортизацииОС                    КАК ХозяйственнаяОперация,
	|	втСписаниеАмортизации.НачислятьИзнос         КАК НачислятьИзнос,
	|
	|	втСписаниеАмортизации.ИдентификаторСтроки    КАК ИдентификаторФинЗаписи,
	|	&НастройкаХО_СписаниеАмортизацииОС           КАК НастройкаХозяйственнойОперации
	|	
	|ИЗ
	|	ДанныеДокументаРеквизиты КАК ДанныеДокумента
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втСписаниеАмортизацииОС КАК втСписаниеАмортизации
	|		ПО втСписаниеАмортизации.Ссылка = ДанныеДокумента.Ссылка  
	|
	|ГДЕ
	|	(ДанныеДокумента.ОтражатьВУпрУчете И втСписаниеАмортизации.Амортизация <> 0
	|		ИЛИ ДанныеДокумента.ОтражатьВБУ И (втСписаниеАмортизации.АмортизацияРегл + втСписаниеАмортизации.АмортизацияЦФ) <> 0
	|		ИЛИ ДанныеДокумента.ОтражатьВНУ И втСписаниеАмортизации.АмортизацияНУ <> 0)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВУпрУчете 
	|			ТОГДА втСписаниеАмортизации.РезервПереоценкиАмортизации 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК РезервПереоценкиАмортизации,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВБУ 
	|			ТОГДА втСписаниеАмортизации.РезервПереоценкиАмортизацииРегл 
	|		ИНАЧЕ 0 
	...
	|  
	|ГДЕ
	|	(ДанныеДокумента.ОтражатьВУпрУчете И втСписаниеАмортизации.РезервПереоценкиАмортизации <> 0
	|		ИЛИ ДанныеДокумента.ОтражатьВБУ И втСписаниеАмортизации.РезервПереоценкиАмортизацииРегл <> 0)
	|";
...
Функция ТекстЗапросаСписанияОбесцененияОС(ТекстыЗапроса) Экспорт
	...
	|	ДанныеДокумента.Период                       КАК Период,
	|	ДанныеДокумента.Организация                  КАК Организация,
	|	ВтСписаниеОбесценения.Местонахождение        КАК Подразделение,
	|	ВтСписаниеОбесценения.ОсновноеСредство       КАК ВнеоборотныйАктив,
	|	ВтСписаниеОбесценения.ЕГДС                   КАК ЕГДС,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВУпрУчете 
	|			ТОГДА ВтСписаниеОбесценения.ОбесценениеУпр 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК ОбесценениеУпр,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВБУ 
	|			ТОГДА ВтСписаниеОбесценения.ОбесценениеРегл 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК ОбесценениеРегл,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВУпрУчете 
	|			ТОГДА ВтСписаниеОбесценения.УбытокОтОбесцененияУпр 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК УбытокОтОбесцененияУпр,
	|
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВБУ 
	|			ТОГДА ВтСписаниеОбесценения.УбытокОтОбесцененияРегл 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК УбытокОтОбесцененияРегл,
	|
	|	ВтСписаниеОбесценения.ГруппаФинансовогоУчета  КАК ГруппаФинансовогоУчета,
	|	ВтСписаниеОбесценения.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	&ХО_СписаниеОбесцененияОС                    КАК ХозяйственнаяОперация,
	|	
	|	ВтСписаниеОбесценения.ИдентификаторСтроки    КАК ИдентификаторФинЗаписи,
	|	&НастройкаХО_СписаниеОбесцененияОС           КАК НастройкаХозяйственнойОперации
	|	
	|ИЗ
	|	ДанныеДокументаРеквизиты КАК ДанныеДокумента
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтСписаниеОбесцененияОС КАК ВтСписаниеОбесценения
	|		ПО ВтСписаниеОбесценения.Ссылка = ДанныеДокумента.Ссылка  
	|
	|ГДЕ
	|	(ДанныеДокумента.ОтражатьВУпрУчете И ВтСписаниеОбесценения.ОбесценениеУпр <> 0
	|		ИЛИ ДанныеДокумента.ОтражатьВБУ И ВтСписаниеОбесценения.ОбесценениеРегл <> 0)
	|";
...
Функция ТекстЗапросаТаблицаВтИсходныеПрочиеРасходыПриВыбытииОС(ТекстыЗапроса) Экспорт
	...
	|			ТОГДА втСписаниеСтоимости.ОсновноеСредство
	...
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВБУ 
	|			ТОГДА втСписаниеСтоимости.СтоимостьРегл + втСписаниеСтоимости.СтоимостьЦФ
	|		ИНАЧЕ 0
	...
	|				ИЛИ ДанныеДокумента.ОтражатьВНУ 
	|					И втСписаниеСтоимости.СтоимостьРегл = 0
	|					И втСписаниеСтоимости.СтоимостьЦФ = 0
	| 
	|			ТОГДА втСписаниеСтоимости.СтоимостьПР + втСписаниеСтоимости.СтоимостьПРЦФ
	|
	|		ИНАЧЕ 0
	...
	|		ПО втСписаниеСтоимости.Ссылка = СтрокаВладельца.Ссылка
	|			И втСписаниеСтоимости.ОсновноеСредство = СтрокаВладельца.ОсновноеСредство
	|";
	СписокЗапросовОбъединение.Добавить(ТекстЗапроса);
	#КонецОбласти
	...
	|	ВЫБОР
	|		КОГДА СтрокаВладельца.ОСВладелец = ЗНАЧЕНИЕ(Справочник.ОбъектыЭксплуатации.ПустаяСсылка)
	|			ТОГДА втСписаниеАмортизации.ОсновноеСредство
	|		ИНАЧЕ СтрокаВладельца.ОСВладелец
	|	КОНЕЦ                                           КАК АналитикаРасходов,
	|
	|	втСписаниеАмортизации.НаправлениеДеятельности   КАК НаправлениеДеятельности,
	...
	|	ВЫБОР 
	|		КОГДА ДанныеДокумента.ОтражатьВБУ 
	|			ТОГДА втСписаниеАмортизации.АмортизацияРегл + втСписаниеАмортизации.АмортизацияЦФ
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаСНДСРегл,
	|
	|	ВЫБОР 
	...
	|		КОГДА ДанныеДокумента.ОтражатьВБУ
	|				ИЛИ ДанныеДокумента.ОтражатьВНУ 
	|					И ЕСТЬNULL(втСписаниеСтоимости.СтоимостьРегл, 0) = 0
	|					И ЕСТЬNULL(втСписаниеСтоимости.СтоимостьЦФ, 0) = 0
	|
	|			ТОГДА втСписаниеАмортизации.АмортизацияПР + втСписаниеАмортизации.АмортизацияПРЦФ
	|
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ПостояннаяРазница,
	|
	...
	|		ПО втСписаниеАмортизации.Ссылка = ДанныеДокумента.Ссылка  
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ втСписаниеСтоимостиОС КАК втСписаниеСтоимости
	|		ПО втСписаниеСтоимости.Ссылка = втСписаниеАмортизации.Ссылка  
	|			И втСписаниеСтоимости.ОсновноеСредство = втСписаниеАмортизации.ОсновноеСредство
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ втТаблицаОС КАК СтрокаВладельца
	|		ПО втСписаниеАмортизации.Ссылка = СтрокаВладельца.Ссылка
	|			И втСписаниеАмортизации.ОсновноеСредство = СтрокаВладельца.ОсновноеСредство
	|";
	СписокЗапросовОбъединение.Добавить(ТекстЗапроса);
	#КонецОбласти
...
Процедура ТекстЗапросаТаблицаВтСписаниеСтоимостиОС(ТекстыЗапроса) Экспорт
	...
	|	ТаблицаОС.СуммаСписанияБУ - ТаблицаОС.СуммаСписанияБУ * ЕСТЬNULL(СтоимостьОС.ДоляЦФБУ, 0) КАК СтоимостьРегл,
	|	ТаблицаОС.СуммаСписанияНУ - ТаблицаОС.СуммаСписанияНУ * ЕСТЬNULL(СтоимостьОС.ДоляЦФНУ, 0) КАК СтоимостьНУ,
	|	ТаблицаОС.СуммаСписанияПР - ТаблицаОС.СуммаСписанияПР * ЕСТЬNULL(СтоимостьОС.ДоляЦФПР, 0) КАК СтоимостьПР,
	|
	|	ВЫРАЗИТЬ((ТаблицаОС.СуммаСписанияБУ - ТаблицаОС.СуммаСписанияБУ * ЕСТЬNULL(СтоимостьОС.ДоляЦФБУ, 0)) КАК ЧИСЛО(31,2))
	|		- ВЫРАЗИТЬ((ТаблицаОС.СуммаСписанияНУ - ТаблицаОС.СуммаСписанияНУ * ЕСТЬNULL(СтоимостьОС.ДоляЦФНУ, 0)) КАК ЧИСЛО(31,2))
	|		- ВЫРАЗИТЬ((ТаблицаОС.СуммаСписанияПР - ТаблицаОС.СуммаСписанияПР * ЕСТЬNULL(СтоимостьОС.ДоляЦФПР, 0)) КАК ЧИСЛО(31,2)) КАК СтоимостьВР,
	|
	|	ТаблицаОС.СуммаСписанияБУ * ЕСТЬNULL(СтоимостьОС.ДоляЦФБУ, 0) КАК СтоимостьЦФ,
	|	ТаблицаОС.СуммаСписанияНУ * ЕСТЬNULL(СтоимостьОС.ДоляЦФНУ, 0) КАК СтоимостьНУЦФ,
	|	ТаблицаОС.СуммаСписанияПР * ЕСТЬNULL(СтоимостьОС.ДоляЦФПР, 0) КАК СтоимостьПРЦФ,
	|
	|	ВЫРАЗИТЬ((ТаблицаОС.СуммаСписанияБУ * ЕСТЬNULL(СтоимостьОС.ДоляЦФБУ, 0)) КАК ЧИСЛО(31,2))
	|		- ВЫРАЗИТЬ((ТаблицаОС.СуммаСписанияНУ * ЕСТЬNULL(СтоимостьОС.ДоляЦФНУ, 0)) КАК ЧИСЛО(31,2))
	|		- ВЫРАЗИТЬ((ТаблицаОС.СуммаСписанияПР * ЕСТЬNULL(СтоимостьОС.ДоляЦФПР, 0)) КАК ЧИСЛО(31,2)) КАК СтоимостьВРЦФ,
	|
	//++ Локализация
	|	ВЫБОР
	...
	|			ТОГДА 1
	|			ИНАЧЕ ВЫРАЗИТЬ(ВЫРАЗИТЬ(ТаблицаОС.СуммаСписанияБУ - ТаблицаОС.СуммаСписанияБУ * ЕСТЬNULL(СтоимостьОС.ДоляЦФБУ, 0) КАК ЧИСЛО(31, 10)) 
	...
	|			ТОГДА 1
	|		ИНАЧЕ ВЫРАЗИТЬ(ВЫРАЗИТЬ(ТаблицаОС.СуммаСписанияБУ * ЕСТЬNULL(СтоимостьОС.ДоляЦФБУ, 0) КАК ЧИСЛО(31, 10)) 
	|						/ СтоимостьОС.СтоимостьЦФ КАК ЧИСЛО(31, 10))
	|	КОНЕЦ КАК ДоляСписанияЦФ,
	|
	|	ВЫБОР 
	|		КОГДА ЕСТЬNULL(СтоимостьОС.СтоимостьНУ, 0) = 0
	|			ТОГДА 1
	|		ИНАЧЕ ВЫРАЗИТЬ(ВЫРАЗИТЬ(ТаблицаОС.СуммаСписанияНУ - ТаблицаОС.СуммаСписанияНУ * ЕСТЬNULL(СтоимостьОС.ДоляЦФНУ, 0) КАК ЧИСЛО(31, 10)) 
	...
	|			ТОГДА 1
	|		ИНАЧЕ ВЫРАЗИТЬ(ВЫРАЗИТЬ(ТаблицаОС.СуммаСписанияНУ * ЕСТЬNULL(СтоимостьОС.ДоляЦФНУ, 0) КАК ЧИСЛО(31, 10)) 
	|						/ СтоимостьОС.СтоимостьНУЦФ КАК ЧИСЛО(31, 10))
	|	КОНЕЦ КАК ДоляСписанияНУЦФ,
	|
	|	ВЫБОР 
	|		КОГДА ЕСТЬNULL(СтоимостьОС.СтоимостьПР, 0) = 0
	|			ТОГДА 1
	|		ИНАЧЕ ВЫРАЗИТЬ(ВЫРАЗИТЬ(ТаблицаОС.СуммаСписанияПР - ТаблицаОС.СуммаСписанияПР * ЕСТЬNULL(СтоимостьОС.ДоляЦФПР, 0) КАК ЧИСЛО(31, 10)) 
	...
	|			ТОГДА 1
	|		ИНАЧЕ ВЫРАЗИТЬ(ВЫРАЗИТЬ(ТаблицаОС.СуммаСписанияПР * ЕСТЬNULL(СтоимостьОС.ДоляЦФПР, 0) КАК ЧИСЛО(31, 10)) 
...
//	УчестьИсправления - Булево -
Процедура ТекстЗапросаТаблицаВтПараметрыАмортизацииОСБУ(ТекстыЗапроса, ИмяТаблицыОбъектовУчета = "ДанныеДокументаТаблицаОС", ИмяТаблицы = "", СоответствиеИзмерений = Неопределено, УчестьИсправления = Ложь) Экспорт
	...
	ВнеоборотныеАктивы.ТекстЗапросаСрезПоследнихРегистра(
		"ПараметрыАмортизацииОСБУ", ИмяТаблицы, "ВтСписокОС", ТекстыЗапроса, СоответствиеИзмерений, УчестьИсправления);
...
//  СоответствиеИзмерений - Структура - Соответствие измерений основной таблицы и регистра
//	УчестьИсправления - Булево -
Процедура ТекстЗапросаТаблицаВтПараметрыАмортизацииОСУУ(ТекстыЗапроса, ИмяТаблицыОбъектовУчета = "ДанныеДокументаТаблицаОС", ИмяТаблицы = "", СоответствиеИзмерений = Неопределено, УчестьИсправления = Ложь) Экспорт
	...
	ВнеоборотныеАктивы.ТекстЗапросаСрезПоследнихРегистра(
		"ПараметрыАмортизацииОСУУ", ИмяТаблицы, "ВтСписокОС", ТекстыЗапроса, СоответствиеИзмерений, УчестьИсправления);
...
Процедура УстановитьПараметрыНабораСвойствВНА24() Экспорт
	...
	УправлениеСвойствами.УстановитьПараметрыНабораСвойств("Документ_ОтнесениеОтклоненийВСтоимостьОС", ПараметрыНабора);
...
Функция ДоступенВыборОбъектовЭксплуатации2_4(Параметры) Экспорт
	...
	Если Параметры.Свойство("Отбор") Тогда
		Если Параметры.Отбор.Свойство("БУСостояние") Тогда
			Параметры.Вставить("Состояние", Параметры.Отбор.БУСостояние);
		КонецЕсли; 
...
// Параметры:
//  ЗапросыРегистра - Массив из Строка -
//  ЕстьПорядокУчетаУУ - Булево -
//  ЕстьПорядокУчетаБУ - Булево -
//  ЕстьПорядокУчетаНУ - Булево -
//
Процедура ТекстЗапросаПрочиеРасходыНеПриниматьКУчетуВключитьВРасходы(ЗапросыРегистра, ЕстьПорядокУчетаУУ = Истина, ЕстьПорядокУчетаБУ = Истина, ЕстьПорядокУчетаНУ = Истина) Экспорт
	
	ТекстЗапроса = ВнеоборотныеАктивыЛокализация.ТекстЗапросаПрочиеРасходыНеПриниматьКУчетуВключитьВРасходы(ЕстьПорядокУчетаУУ, ЕстьПорядокУчетаБУ, ЕстьПорядокУчетаНУ);
...
Процедура ЗначенияПараметровПроведенияПринятияКУчетуОС(ЗначенияПараметровПроведения) Экспорт
	...
	СписокРазрешенныхВариантовРаспределенияРасходов.Добавить(Перечисления.ВариантыРаспределенияРасходов.НаРасходыТекущегоПериода);
...
Процедура ТекстЗапросаДвиженияДоходыРасходыПрочиеАктивыПассивыПринятияКУчету(ЗапросыРегистра) Экспорт
	...
	|				ИЛИ ДанныеДокумента.ПорядокУчетаУУ = ЗНАЧЕНИЕ(Перечисление.ПорядокПогашенияСтоимостиОС.НеПриниматьКУчетуВключитьВРасходы)
	|			ТОГДА 0
	|		ИНАЧЕ ВтРасчетСтоимости.Сумма
	|	КОНЕЦ КАК Сумма,
	|
...
//  Исключения - Строка - Поля запроса, которые не нужно заменять
Процедура ЗаменитьДополнительныеПоляВЗапросе(ТекстЗапроса, ДополнительныеПоля, Исключения = "") Экспорт
	...
	Если ЗначениеЗаполнено(ДополнительныеПоля) Тогда
		...
		Для каждого ИмяПоля Из СписокПолей Цикл
			Если СписокИсключений.Найти(ИмяПоля) = Неопределено Тогда
				ТекстДополнительныеПоля = ТекстДополнительныеПоля + "
				|	, 0 КАК " + ИмяПоля;
			КонецЕсли;
		КонецЦикла; 
...
Процедура ОтразитьВУправленческомБалансе(ТаблицыДвижений, МенеджерВременныхТаблиц) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		// При новой архитектуре взаиморасчетов должны формироваться задания неоперативной очереди.
		Возврат;
	КонецЕсли;
...
// 
// Параметры:
//  Объект - ДокументОбъект.ПринятиеКУчетуОС2_4, ДокументОбъект.МодернизацияОС2_4, ДокументОбъект.ПринятиеКУчетуУзловКомпонентовАмортизации - Документ
//  Отказ - Булево - Отказ
Процедура ПроверитьПорядокФормированияСтоимостиОС(Объект, Отказ) Экспорт
	
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПринятиеКУчетуОС2_4") Тогда
		ОтражатьВБУ = Объект.ОтражатьВБУ;
		ОтражатьВНУ = Объект.ОтражатьВНУ;
		ОтражатьВУУ = Объект.ОтражатьВУпрУчете;
	Иначе
		ОтражатьВБУ = Объект.ОтражатьВРеглУчете;
		ОтражатьВНУ = Объект.ОтражатьВРеглУчете;
		ОтражатьВУУ = Объект.ОтражатьВУпрУчете;
	КонецЕсли;
	
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПринятиеКУчетуУзловКомпонентовАмортизации") Тогда
		ПутьКТабличнойЧасти = "КомпонентыАмортизации";
		СписокОС = Объект.КомпонентыАмортизации.ВыгрузитьКолонку("КомпонентАмортизации");
	Иначе
		ПутьКТабличнойЧасти = "ОС";
		СписокОС = Объект.ОС.ВыгрузитьКолонку("ОсновноеСредство");
	КонецЕсли;
	...
	|
	|	И (&ПорядокФормированияСтоимости = ЗНАЧЕНИЕ(Перечисление.ПорядокФормированияСтоимостиВНА.ВключатьВСтоимостьРасходыПоступившиеДоКонцаМесяца)
	|		ИЛИ ДанныеДокумента.ПорядокФормированияСтоимости = ЗНАЧЕНИЕ(Перечисление.ПорядокФормированияСтоимостиВНА.ВключатьВСтоимостьРасходыПоступившиеДоКонцаМесяца))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|	ТаблицаОС.КомпонентАмортизации.Представление КАК ОсновноеСредствоПредставление
	|
	|ИЗ
	...
	|			И ДанныеДокумента.ОтражатьВУпрУчете)
	|
	|	И ТаблицаОС.КомпонентАмортизации В (&СписокОС)
	|
	|	И (&ПорядокФормированияСтоимости = ЗНАЧЕНИЕ(Перечисление.ПорядокФормированияСтоимостиВНА.ВключатьВСтоимостьРасходыПоступившиеДоКонцаМесяца)
	|		ИЛИ ДанныеДокумента.ПорядокФормированияСтоимости = ЗНАЧЕНИЕ(Перечисление.ПорядокФормированияСтоимостиВНА.ВключатьВСтоимостьРасходыПоступившиеДоКонцаМесяца))
	|
	...
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("Ссылка", Объект.Ссылка);
	Запрос.УстановитьПараметр("НачалоМесяца", ?(Объект.Дата <> '00010101', НачалоМесяца(Объект.Дата), НачалоМесяца(ТекущаяДатаСеанса())));
	Запрос.УстановитьПараметр("КонецМесяца", ?(Объект.Дата <> '00010101', КонецМесяца(Объект.Дата), КонецМесяца(ТекущаяДатаСеанса())));
	Запрос.УстановитьПараметр("ПорядокФормированияСтоимости", Объект.ПорядокФормированияСтоимости);
	Запрос.УстановитьПараметр("СписокОС", СписокОС);
	...
	ТекстСообщения = НСтр("ru = 'При порядке формирования стоимости ""Включить в стоимость расходы, поступившие до конца месяца, в котором оформлен документ"" допускается оформление только одного документа в том же месяце (для основного средства %1 уже оформлен документ %2)';
							|en = 'If the ""Include expenses received before end of month in which document is created in cost"" cost generation procedure is used, only one document can be issued in the same month (for the %1 fixed asset, the %2 document is already issued)'");
	ТекстСообщения = СтрШаблон(ТекстСообщения, Выборка.ОсновноеСредствоПредставление, Строка(Выборка.Ссылка));
	
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПринятиеКУчетуУзловКомпонентовАмортизации") Тогда
		ДанныеСтроки = Объект.КомпонентыАмортизации.Найти(Выборка.ОсновноеСредство, "КомпонентАмортизации");
		Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(ПутьКТабличнойЧасти, ДанныеСтроки.НомерСтроки, "КомпонентАмортизации");
	Иначе
		ДанныеСтроки = Объект.ОС.Найти(Выборка.ОсновноеСредство, "ОсновноеСредство");
		Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(ПутьКТабличнойЧасти, ДанныеСтроки.НомерСтроки, "ОсновноеСредство");
	КонецЕсли;
...
// 
// Параметры:
//  Объект - ДокументОбъект.ПринятиеКУчетуНМА2_4, ДокументОбъект.УлучшениеНМА - Документ
//  Отказ - Булево - Отказ
Процедура ПроверитьПорядокФормированияСтоимостиНМА(Объект, Отказ) Экспорт
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПринятиеКУчетуНМА2_4") Тогда
		СписокНМА = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Объект.НематериальныйАктив);
	Иначе
		СписокНМА = Объект.НМА.ВыгрузитьКолонку("НематериальныйАктив");
	КонецЕсли;
	...
	|	ДанныеДокумента.НематериальныйАктив.Представление КАК НематериальныйАктивПредставление
	|
	|ИЗ
	...
	|			И ДанныеДокумента.ОтражатьВУпрУчете)
	|
	|	И ДанныеДокумента.НематериальныйАктив В (&СписокНМА)
	|
	|	И (&ПорядокФормированияСтоимости = ЗНАЧЕНИЕ(Перечисление.ПорядокФормированияСтоимостиВНА.ВключатьВСтоимостьРасходыПоступившиеДоКонцаМесяца)
	|		ИЛИ ДанныеДокумента.ПорядокФормированияСтоимости = ЗНАЧЕНИЕ(Перечисление.ПорядокФормированияСтоимостиВНА.ВключатьВСтоимостьРасходыПоступившиеДоКонцаМесяца))
	|
	...
	|	ТаблицаНМА.Ссылка КАК Ссылка,
	|	ТаблицаНМА.НематериальныйАктив КАК НематериальныйАктив,
	|	ТаблицаНМА.НематериальныйАктив.Представление КАК НематериальныйАктивПредставление
	|
	|ИЗ
	...
	|			И ДанныеДокумента.ОтражатьВУпрУчете)
	|
	|	И ТаблицаНМА.НематериальныйАктив В (&СписокНМА)
	|
	|	И (&ПорядокФормированияСтоимости = ЗНАЧЕНИЕ(Перечисление.ПорядокФормированияСтоимостиВНА.ВключатьВСтоимостьРасходыПоступившиеДоКонцаМесяца)
	|		ИЛИ ДанныеДокумента.ПорядокФормированияСтоимости = ЗНАЧЕНИЕ(Перечисление.ПорядокФормированияСтоимостиВНА.ВключатьВСтоимостьРасходыПоступившиеДоКонцаМесяца))
	|";
	...
	ТекстСообщения = НСтр("ru = 'При порядке формирования стоимости ""Включить в стоимость расходы, поступившие до конца месяца, в котором оформлен документ"" допускается оформление только одного документа в том же месяце (для нематериального актива %1 уже оформлен документ %2)';
							|en = 'If the ""Include expenses received before end of month in which document is created in cost"" cost generation procedure is used, only one document can be issued in the same month (for the %1 intangible asset, the %2 document is already issued)'");
	ТекстСообщения = СтрШаблон(ТекстСообщения, Выборка.НематериальныйАктивПредставление, Строка(Выборка.Ссылка));
	
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПринятиеКУчетуНМА2_4") Тогда
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Объект, "НематериальныйАктив", "", Отказ);
	Иначе
		ДанныеСтроки = Объект.НМА.Найти(Выборка.НематериальныйАктив, "НематериальныйАктив");
...
Процедура ПредставлениеСведенийОДатеПринятияКУчету(СведенияОбУчете, ПредставлениеСведений) Экспорт
	...
	Иначе
		...
		ПараметрыСведений.Заголовок3 = НСтр("ru = 'Дата принятия в упр. учете:';
											|en = 'Date of recognition in mgmt accounting:'");
	КонецЕсли;
...
Процедура ПредставлениеСведенийОДатеСписания(СведенияОбУчете, ПредставлениеСведений) Экспорт
	...
	ПараметрыСведений.Заголовок1 = НСтр("ru = 'Дата снятия в бух. учете:';
										|en = 'Derecognition in local accounting date:'");
	...
	ПараметрыСведений.Заголовок3 = НСтр("ru = 'Дата снятия в упр. учете:';
										|en = 'Derecognition in mgmt accounting date:'");
...
Процедура ДобавитьТекстЗапросаТаблицыСправкиРасчетАмортизации(ИмяРегистра, ИмяТаблицы, ИмяТаблицыПериодыПараметров, ОбъектУчета, СписокЗапросов) Экспорт
	...
	|
	|	МАКСИМУМ(ЕСТЬNULL(ТаблицаРегистра.ДатаИсправления, ДАТАВРЕМЯ(1,1,1))) КАК ДатаИсправленияСрезаПоследних
	|
...
// Возвращаемое значение:
//  Строка -
Функция ТекстПолейРегистра(ИмяРегистра, ВычислениеПолей = Неопределено, НеДобавлятьПоля = Неопределено) Экспорт
...
// 
// Возвращаемое значение:
//  Булево - Истина, если в регистре есть организация
Функция ВРегистреЕстьОрганизация(ИмяРегистра) Экспорт
...
Процедура ПроверитьТипОС(ДоступныеТипыОС, Объект, ИмяТЧ, ИмяПоля, Отказ) Экспорт
	...
	|ГДЕ
	|	ОбъектыЭксплуатации.Ссылка В (&СписокОС)
	|	И НЕ ОбъектыЭксплуатации.ТипОС В (&ДоступныеТипыОС)
	|";
	
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("СписокОС", ?(ИмяТЧ <> Неопределено, Объект[ИмяТЧ].ВыгрузитьКолонку(ИмяПоля), Объект[ИмяПоля]));
	Запрос.УстановитьПараметр("ДоступныеТипыОС", ДоступныеТипыОС);
	...
	Выборка = Результат.Выбрать();
	Пока Выборка.Следующий() Цикл
		РезультатПроверки.Вставить(Выборка.Ссылка, Выборка.Представление);
	КонецЦикла;
	...
	Если ИмяТЧ <> Неопределено Тогда
		...
		Для Каждого ДанныеСтроки Из Объект[ИмяТЧ] Цикл
			...
			Представление = РезультатПроверки.Получить(ДанныеСтроки[ИмяПоля]);

			Если Представление <> Неопределено Тогда
				
				ТекстСообщения = НСтр("ru = 'Тип основного средства %1 не поддерживается в документе, необходимо выбрать другое основное средство или исправить тип ОС.';
										|en = '%1 fixed asset type is not supported in the document. Select another fixed asset or correct the fixed asset type.'");
				ТекстСообщения = СтрШаблон(ТекстСообщения, Представление);
				Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(ИмяТЧ, ДанныеСтроки.НомерСтроки, ИмяПоля);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Объект, Поле, "", Отказ);
	...
	Иначе
		...
		Представление = РезультатПроверки.Получить(Объект[ИмяПоля]);
		
		Если Представление <> Неопределено Тогда
			ТекстСообщения = НСтр("ru = 'Тип основного средства %1 не поддерживается в документе, необходимо выбрать другое основное средство или исправить тип ОС.';
									|en = '%1 fixed asset type is not supported in the document. Select another fixed asset or correct the fixed asset type.'");
			ТекстСообщения = СтрШаблон(ТекстСообщения, Представление);
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Объект, ИмяПоля,, Отказ);
		КонецЕсли;
...
Функция ЕстьОбъектыСписанныеПриПринятииКУчету(Ссылка, Дата, Организация, СписокОС) Экспорт
...
Процедура УдалитьПустойОтбор(ИмяОтбора, Параметры) Экспорт

	Если Параметры = Неопределено
		ИЛИ НЕ Параметры.Свойство("Отбор") Тогда
		Возврат;
	КонецЕсли;

	СписокОтборов = СтрРазделить(ИмяОтбора, ",");
	
	Для Каждого Элемент Из СписокОтборов Цикл
		
		Если Параметры.Отбор.Свойство(Элемент)
			И НЕ ЗначениеЗаполнено(Параметры.Отбор[Элемент]) Тогда
			Параметры.Отбор.Удалить(Элемент);
		КонецЕсли;
...
Процедура УстановитьОтборВБыстромДоступе(ИмяОтбора, Параметры, Список) Экспорт
	...
	Для Каждого Элемент Из СписокОтборов Цикл
		...
		Если НЕ Параметры.Отбор.Свойство(Элемент)
			ИЛИ НЕ ЗначениеЗаполнено(Параметры.Отбор[Элемент]) Тогда
			Продолжить;
		КонецЕсли;
...
// 
// Параметры:
//  НадписьФормы - ДекорацияФормы - 
//
Процедура УстановитьНадписьУстаревшийФункционал(НадписьФормы) Экспорт
	
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьВнеоборотныеАктивы2_2") Тогда
		Возврат;
	КонецЕсли;
	
	НадписьФормы.ЦветТекста = ЦветаСтиля.ЦветОсобогоТекста;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВнеоборотныеАктивы2_4") Тогда
		НадписьФормы.Заголовок = НСтр("ru = 'Внимание! Функциональность учета внеоборотных активов версии 2.2 будет удалена в версии 2.6.3.';
										|en = 'Important. Fixed and intangible assets accounting version 2.2 will be removed in version 2.6.3.'");
	Иначе
		НадписьФормы.Заголовок = НСтр("ru = 'Внимание! Функциональность учета внеоборотных активов версии 2.2 будет удалена в версии 2.6.3. Пожалуйста, перейдите на версию учета 2.4.';
										|en = 'Important. Fixed and intangible assets accounting version 2.2 will be removed in version 2.6.3. Please upgrade to version 2.4.'");
	КонецЕсли;
	
КонецПроцедуры

// Выполняет запрос и выполняет замер при необходимости.
// 
// Параметры:
//  Запрос - Запрос
//  СписокЗапросов - СписокЗначений, Неопределено - Список запросов
//  ВыполнитьЗамер - Булево - Выполнить замер
// 
// Возвращаемое значение:
//  РезультатЗапроса - Результат запроса
Функция ВыполнитьЗапрос(Запрос, СписокЗапросов = Неопределено, ВыполнитьЗамер = Ложь) Экспорт
	
	Если ВыполнитьЗамер Тогда
		
		Если СписокЗапросов <> Неопределено Тогда
			
			Для Сч = 0 по СписокЗапросов.ВГраница() Цикл
	
				КлючеваяОперация = "ВыполнитьЗапрос.Запрос" + Формат(Сч, "ЧЦ=3; ЧН=000; ЧВН=; ЧГ=;");
				
				Запрос.Текст = СписокЗапросов[Сч];
		...
		Иначе
			...
			КлючеваяОперация = "ВыполнитьЗапрос.Запрос";
