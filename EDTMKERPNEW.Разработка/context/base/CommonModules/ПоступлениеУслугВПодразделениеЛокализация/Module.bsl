...
#Область ОтражениеПроизводственныхЗатрат  // (Дт 20 :: Кт 60.НП)
	...
	|	ПроизводственныеЗатраты.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|
	// Сумма
	...
	|   (ВЫБОР КОГДА Операция.Договор.ВариантОформленияДокументовПоступленияПоУслугам 
  	|      В (ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется))
  	|    ТОГДА (НеотфактурованныеПоставки.ВременнаяРазница)
  	|    ИНАЧЕ НеотфактурованныеПоставки.СтоимостьРегл
  	|    КОНЕЦ) 	
	...
	|	""ПоступлениеУслугВПодразделениеЛокализация_925"" КАК НомерФрагмента
	|
	|ИЗ
...
#Область ОтражениеПрочихРасходов  // (Дт 25, 26, 20 :: Кт 60.НП)
    ...
    |	- (ВЫБОР КОГДА Операция.Договор.ВариантОформленияДокументовПоступленияПоУслугам 
	|			В (ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется))
	|		ТОГДА (НеотфактурованныеПоставки.СтоимостьРегл - НеотфактурованныеПоставки.ВременнаяРазница - НеотфактурованныеПоставки.ПостояннаяРазница)
	|		ИНАЧЕ 0
	|		КОНЕЦ)
	|		+ НеотфактурованныеПоставки.ДопРасходыРегл 
	|		- НеотфактурованныеПоставки.ПостояннаяРазница 
	...
	|	""ПоступлениеУслугВПодразделениеЛокализация_926"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область НеотфактурованныйНДС  // (Дт 19.НП :: Кт 60.НП)

	ОтражениеНеотфактурованныйНДС = "
	|ВЫБРАТЬ // Отражение прочих расходов (Дт 19.НП :: Кт 60.НП)
	|
	|	Операция.Ссылка КАК Ссылка,
	// Период, Организация, ИдентификаторСтроки
	|	Операция.Дата			КАК Период,
	...
	|
	// Сумма
	|	ДвиженияНДСПредъявленный.НДС КАК Сумма,
	|	ДвиженияНДСПредъявленный.НДСУпр КАК СуммаУУ,
	|
	// ДТ - Вид счета, Аналитика учета, Место учета
	|	
	...
	|	ДвиженияНДСПредъявленный.НаправлениеДеятельности КАК НаправлениеДеятельностиДт,
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСпоНеотфактурованнымПоставкам) КАК СчетДт,
	|	
	// ДТ - Субконто
	|	ДвиженияНДСПредъявленный.Поставщик КАК СубконтоДт1,
	...
	|	0 КАК КоличествоДт,
	|	ВЫБОР
	|		КОГДА ДоговорКонтрагента.ВариантОформленияДокументовПоступленияПоУслугам = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется)
	|			ТОГДА 0
	|		ИНАЧЕ ДвиженияНДСПредъявленный.НДС
	|	КОНЕЦ КАК СуммаНУДт,
	|	0 КАК СуммаПРДт,
	|	ВЫБОР
	|		КОГДА ДоговорКонтрагента.ВариантОформленияДокументовПоступленияПоУслугам = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугСторнируется)
	|			ТОГДА ДвиженияНДСПредъявленный.НДС
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаВРДт,
	|	
	// КТ - Вид счета, Аналитика учета, Место учета
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НеотфактурованныеПоставки) КАК ВидСчетаКт,
	|	ДоговорКонтрагента.ГруппаФинансовогоУчета КАК АналитикаУчетаКт,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаКт,
	|
	// КТ - Валюта, Подразделение, Счет
	|	&ВалютаРеглУчета КАК ВалютаКт,
	|	ДоговорКонтрагента.Подразделение КАК ПодразделениеКт,
	|	ДоговорКонтрагента.НаправлениеДеятельности КАК НаправлениеДеятельностиКт,
	|
	|	НЕОПРЕДЕЛЕНО КАК СчетКт,
	|
	// КТ - Субконто
	...
	|	КОНЕЦ КАК СуммаВРКт,
	|
	|	""Отражение НДС"" КАК Содержание,
	|	""ПоступлениеУслугВПодразделениеЛокализация_2004"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Справочник.ДоговорыКонтрагентов КАК ДоговорКонтрагента
	|	ПО
	|		Операция.Договор = ДоговорКонтрагента.Ссылка
	|	
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ 
	|		РегистрНакопления.НДСПредъявленный КАК ДвиженияНДСПредъявленный
	|	ПО 
	|		ДвиженияНДСПредъявленный.Регистратор = Операция.Ссылка
	|		И ДвиженияНДСПредъявленный.ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.НеотфактурованныйНДС)
	|	    И НЕ ДвиженияНДСПредъявленный.Сторно
	|	    И ДвиженияНДСПредъявленный.Активность
	|";     
#КонецОбласти
	...
	ТекстыОтражения.Добавить(ОтражениеНеотфактурованныйНДС);
