...
Функция ТекстОНовыхСообщениях(ВсегоКоличество, НазваниеБанка)
	...
	СтруктураСообщения.Вставить("Пояснение", СтрШаблон(НСтр("ru = 'от %1';
															|en = ', %1'"), НазваниеБанка));
...
Функция ТекстОбОтклоненныхДокументах(ДанныеСобытия)
	...
	Пояснение = Нстр("ru = 'Операция отклонена банком.';
					|en = 'Операция отклонена банком.'");
	
	ДеталиОперации = Новый Массив;
	
	Если ЗначениеЗаполнено(ДанныеСобытия.СуммаДокумента) Тогда
		ДеталиОперации.Добавить(СтрШаблон(НСтр("ru = '%1 руб.';
												|en = '%1 rub.'"), Формат(ДанныеСобытия.СуммаДокумента, "ЧДЦ=2; ЧГ=")));
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ДанныеСобытия.НаименованиеКонтрагента) Тогда
		ДеталиОперации.Добавить(СтрШаблон(НСтр("ru = '%1';
												|en = '%1'"), ДанныеСобытия.НаименованиеКонтрагента));
	КонецЕсли;
