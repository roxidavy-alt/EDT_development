...
Процедура УправлениеФормойНаСервере(Форма) Экспорт
	...
	Если ВидОрганизации = "ИндивидуальныйПредприниматель"
		ИЛИ Не ПолучитьФункциональнуюОпцию("ИспользоватьРезервыПоОплатеТруда") Тогда
		ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(
				Элементы,
	...
	КонецЕсли;
	Если ВидОрганизации = "ИндивидуальныйПредприниматель"
		ИЛИ (ПолучитьФункциональнуюОпцию("ИспользоватьРезервыПоОплатеТруда")
...
Процедура ПередЗаписью(Объект, Отказ) Экспорт
	...
	РеглУчетСервер.Организации_ПередЗаписью(Объект, Отказ);
	//-- НЕ УТ
...
Процедура ЗаполнитьОписаниеСФР(Форма, ИмяОписания = "")
	...
	Если Форма.ВидОрганизации = "ИндивидуальныйПредприниматель" Тогда
		СтрокаОписания = НСтр("ru = 'Регистрационный номер: %РегистрационныйНомерСФР%. Категория страхователя - физического лица: %КатегорияСтрахователяФизическогоЛица%.';
								|en = 'Registration number: %РегистрационныйНомерСФР%. Category of insurant who is individual: %КатегорияСтрахователяФизическогоЛица%.'");
	Иначе
...
Процедура ЗаполнитьОписаниеКлассификаторов(Форма, ИмяОписания = "")
	...
	СтрокаОписанияКлассификаторов = НСтр("ru = 'ОКОПФ: %КодОКОПФ%. Организационно-правовая форма: %НаименованиеОКОПФ%. ОКФС: %КодОКФС%. Форма собственности: %НаименованиеОКФС%. ОКВЭД: %КодОКВЭД% (вид деятельности: %НаименованиеОКВЭД%). ОКВЭД 2: %КодОКВЭД2% (вид деятельности: %НаименованиеОКВЭД2%). ОКОНХ: %КодОКОНХ%.';
										|en = 'RNCFI: %КодОКОПФ%. Legal form: %НаименованиеОКОПФ%. RNCO: %КодОКФС%. Ownership form: %НаименованиеОКФС%. RNCEA: %КодОКВЭД% (activity category: %НаименованиеОКВЭД%). RNCEA 2: %КодОКВЭД2% (activity category: %НаименованиеОКВЭД2%). RNCEB: %КодОКОНХ%.'");
