...
Функция ТекстЗапросаДляРаспределенияТрудозатратНаВыпуск(ПараметрыРасчета = Неопределено) Экспорт
	...
	ТекстЗапроса = ""
		...
		+ ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении() + ТекстЗависшиеТрудозатратыКРаспределению2_2()
		+ ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении() + ТекстРаспределениеТрудозатратПоПродукции2_2()
		+ ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении() + ТекстСторноТрудозатратИсправленныхДокументовПрошлогоПериода2_2()
		+ "";
...
Функция ТекстТрудозатратыНЗПИнициализация()
	Возврат
		...
		|	ТрудозатратыНЗП.ВидФондаВзносов,
		...
		|	СУММА(ТрудозатратыНЗП.СтоимостьНДД)
		...
		|			(Организация, ПартияПроизводства) В(
		|				ВЫБРАТЬ
		|					ПартииПроизводстваСводно.Организация,
		|					ПартииПроизводстваСводно.ПартияПроизводства
		|				ИЗ
		|					ПартииПроизводстваСводно КАК ПартииПроизводстваСводно
		|				ГДЕ
		|					ПартииПроизводстваСводно.РаспределениеЗатратНаВыпуск)) КАК Т
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		// В расчет так же берем сторно и исправления в текущем периоде.
		|	ВЫБРАТЬ
		|		ДД.Организация          КАК Организация,
		|		ДД.Подразделение        КАК Подразделение,
		|		ДД.ПартияПроизводства   КАК ПартияПроизводства,
		...
		|		И ДД.РасчетПартий
		|		И НЕ ДД.Сторно
		|		И Партии.РаспределениеЗатратНаВыпуск
		|	) КАК ТрудозатратыНЗП
		|
		|СГРУППИРОВАТЬ ПО
		|	ТрудозатратыНЗП.ПартияПроизводства,
		...
		|		Остатки.НормативнаяСтоимость КАК НормативнаяСтоимость,
		...
		|		И ДД.РаспределениеЗатратНаВыпуск
		//++ НЕ УТКА
		|
		|	ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|ИЗ
		|	РегистрНакопления.ТрудозатратыНезавершенногоПроизводства.Обороты(, ДОБАВИТЬКДАТЕ(&НачалоПериода, СЕКУНДА, -1),,
		...
		|				ПартииПроизводстваСводно КАК ПартииПроизводстваСводно
		|			ГДЕ
		|				ПартииПроизводстваСводно.ДлительноеПроизводство
...
Функция ТекстЗависшиеТрудозатратыКРаспределению2_2() // использует ТрудозатратыПоЭтапам, ДолиВыпуска, РасчетноеСводно
	
	Возврат
		// Отражение заработной платы может не закрывать суммовые показатели по исправленным трудозатратам партий производства,
		// по которым отсутствует количество к распределению. Такие затраты относятся на продукцию в полном объеме.
		"ВЫБРАТЬ
		|	""ТекстЗависшиеТрудозатратыКРаспределению2_2"" КАК ЗапросИсточник,
		|	0 КАК Приоритет,
		|	ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Расчетное) КАК ТипЗаписи,
		|	ИСТИНА КАК РасчетЗавершен,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
		...
		|	И ДД.Организация = Итог.Организация
		|	
		|	ЛЕВОЕ СОЕДИНЕНИЕ РасчетноеСводно КАК РасчетноеСводно
		|	ПО ДД.ПартияПроизводства = РасчетноеСводно.ПартияПроизводства
		|	И ДД.Подразделение = РасчетноеСводно.Подразделение
		|	И ДД.ВидРабот = РасчетноеСводно.ВидРабот
		|	И ДД.ГруппаПродукции = РасчетноеСводно.ГруппаПродукции
		|ГДЕ
		|	ДД.Количество = 0
		|	И РасчетноеСводно.ПартияПроизводства ЕСТЬ NULL
		|";
...
Функция ТекстСторноТрудозатратИсправленныхДокументовПрошлогоПериода2_2() // использует Продукция
	
	Возврат
	"ВЫБРАТЬ
	|	""ТекстСторноТрудозатратИсправленныхДокументовПрошлогоПериода2_2"" КАК ЗапросИсточник,
	|	0                                                         КАК Приоритет,
	...
	|	ИЛИ НЕ СУММА(ДД.НормативнаяСтоимость) = 0
	|	ИЛИ НЕ СУММА(ДД.Стоимость) = 0
	|	ИЛИ НЕ СУММА(ДД.СтоимостьРегл) = 0
	|	ИЛИ НЕ СУММА(ДД.ПостояннаяРазница) = 0
	|	ИЛИ НЕ СУММА(ДД.ВременнаяРазница) = 0
	|	ИЛИ НЕ СУММА(ДД.СтоимостьНДД) = 0
	|";
