	...
	Если Не ЗначениеЗаполнено(Наименование) Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			...
			НСтр("ru = 'Получение данных';
				|en = 'Receiving data'", ОбщегоНазначения.КодОсновногоЯзыка()));
		Возврат Результат;
	КонецЕсли;
...
Процедура ОбработатьОшибкуСервиса(ОбъектXDTO, ОписаниеОшибки, Сервис, ИмяМетода, ИдентификаторУслуги, Результат)
	...
	ИначеЕсли СтрНайти(ОписаниеОшибкиВРег, "SERVER-1:") > 0 Тогда
		...
		ЗаписатьОшибкуВЖурналРегистрации(
			...
			Сервис,
			НСтр("ru = 'Получение данных';
				|en = 'Receiving data'", КодОсновногоЯзыка));
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Некорректное значение параметра ""Имя метода"" (%1).';
				|en = 'Incorrect value of the ""Method name"" parameter (%1).'"),
			ИмяМетода);
	КонецЕсли;
...
Процедура ПриДобавленииОписанийСервисовСопровождения(СервисыСопровождения) Экспорт
	...
	ОписательСервиса.Описание        = НСтр("ru = 'Быстрая проверка информации о контрагентах, автоматическое заполнение реквизитов контрагентов в различных документах, поиск контрагентов и пр.';
											|en = 'Quick counterparty information check, automatic filling of counterparty details in documents, counterparty search, and other functions.'");
