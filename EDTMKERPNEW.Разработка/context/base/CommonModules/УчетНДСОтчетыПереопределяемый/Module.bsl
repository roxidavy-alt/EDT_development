...
Процедура ЗапросПоДаннымЗаполнения7РазделаДекларацииПоНДС(СтруктураПараметров, ДанныеДляЗаполнения, АдресХранилища) Экспорт
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(НДСЗаписиКнигиПродажОбороты.Регистратор) = ТИП(Документ.СчетФактураВыданный)
	|			И ТИПЗНАЧЕНИЯ(НДСЗаписиКнигиПродажОбороты.СчетФактура) = ТИП(Документ.ОтгрузкаТоваровКлиенту)
	|			ТОГДА НДСЗаписиКнигиПродажОбороты.СчетФактура
	|		ИНАЧЕ
	...
	|					ПродажиПоКодам КАК Т)
	|		И ДетализацияПартийТоваровДляНДСиУСН.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|		И ДетализацияПартийТоваровДляНДСиУСН.КорРазделУчета <> ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыВПути)
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ДетализацияПартийТоваровДляНДСиУСН.Регистратор
	|) КАК ВложенныйЗапрос
