...
Функция ИспользуетсяНастраиваемаяОтчетность(ПараметрыОтчета) Экспорт
	...
	ИначеЕсли ПараметрыОтчета.Свойство("Период") Тогда
		ДатаКонцаПериодаОтчета = ПараметрыОтчета.Период;
	Иначе
...
Процедура ЗаполнитьОтчетБухОтчетностьФормаОтчета2019Кв1(ПараметрыОтчета, Контейнер) Экспорт
	...
	НастраиваемаяБухгалтерскаяОтчетностьФорма2019.ЗаполнитьОтчетБухОтчетностьФормаОтчета2019Кв1(ПараметрыОтчета, Контейнер);
...
Процедура ЗаполнитьОтчетБухОтчетностьФормаОтчета2025Кв1(ПараметрыОтчета, Контейнер) Экспорт
	...
	ЭтоРасчетСтоимостиЧистыхАктивов = ПараметрыОтчета.Свойство("ЭтоРасчетСтоимостиЧистыхАктивов");
	...
	Для Каждого Организация Из ПолныйСписокОрганизаций Цикл
		...
		ПараметрыЗаполнения = ПараметрыЗаполненияОтчета();
		
		ПараметрыЗаполнения.Организация = Организация;
		ПараметрыЗаполнения.ЗаполнениеФормОтчетности = ЗаполнениеФормОтчетности;
		...
		ПараметрыЗаполнения.ЭтоРасчетСтоимостиЧистыхАктивов = ЭтоРасчетСтоимостиЧистыхАктивов;
		...
		Если ЗаполнениеФормОтчетности.Баланс Тогда
			...
			Если ЭтоРасчетСтоимостиЧистыхАктивов = Истина Тогда
				ПараметрыЗаполнения.КонтейнерРаздела = Контейнер;
			Иначе
	...
	Если Не ЭтоРасчетСтоимостиЧистыхАктивов Тогда
...
//
// Параметры:
//  НастраиваемыеРазделыОтчета - Структура - см. БухгалтерскаяОтчетностьБРОПереопределяемый.ПриЗаполненииПеречняНастраиваемыхРазделов - (возвращаемый)
...
Процедура УстановитьНастраиваемыеРазделы(НастраиваемыеРазделыОтчета, Контекст) Экспорт
	...
	ИмяФормы = Метаданные.Справочники.НастройкиЗаполненияБухОтчетности.Формы.НастройкиЗаполненияОтчетности.ПолноеИмя();
	НастраиваемыеФормыОтчетности = НастраиваемаяБухгалтерскаяОтчетностьВызовСервера.СоставФормБухотчетности(Контекст.мВыбраннаяФорма);
	Для Каждого Форма Из НастраиваемыеФормыОтчетности Цикл
		
		ИмяПеречисления = Форма.Значение.Метаданные().Имя;
		ИндексЗначенияПеречисления = Перечисления[ИмяПеречисления].Индекс(Форма.Значение);
		ИмяЗначенияПеречисления = Метаданные.Перечисления[ИмяПеречисления].ЗначенияПеречисления[ИндексЗначенияПеречисления].Имя;
		
		Если НастраиваемыеРазделыОтчета.Свойство(ИмяЗначенияПеречисления) Тогда
			НастраиваемыеРазделыОтчета[ИмяЗначенияПеречисления] = ИмяФормы;
		КонецЕсли;
...
Процедура ЗаполнитьПоказательПоАлгоритмам(КонтекстВыполнения, Данные, Алгоритмы, ПараметрыПоказателя)
	...
	УстановитьЗначениеПоказателя(КонтекстВыполнения, ПараметрыПоказателя);
...
Процедура УстановитьЗначениеПоказателя(КонтекстВыполнения, ПараметрыПоказателя)
	
	Раздел = КонтекстВыполнения.КонтейнерРаздела; // ТаблицаЗначений
	
	Если НЕ КонтекстВыполнения.ЭтоРасчетСтоимостиЧистыхАктивов Тогда
		...
		КонецЕсли;
	Иначе
		Если Не ЗначениеЗаполнено(ПараметрыПоказателя.КодОсновнойСтроки) Тогда
			Строка = Раздел.Добавить();
			Строка.КодПоказателя = ПараметрыПоказателя.КодСтроки;
			Строка.ЗначениеПоказателя = ПараметрыПоказателя.ЗначениеПоказателя;
			Строка.КодГрафы = ПараметрыПоказателя.КодГрафы;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОпределитьНомераСтрок(Строка, ПараметрыПоказателя, ПараметрыСтрок)
...
//
// Параметры:
//  ЗаполняемыеПоказатели - Структура - см. БухгалтерскаяОтчетностьБРО.НовыйЗаполняемыеПоказателиРаздела - описание структуры отчета (раздела отчета) и правил его заполнения
// 
// Возвращаемое значение:
//  Соответствие - бакет:
...
Функция ПолучитьТаблицуДанныхПоясненийЗадолженности()
...
Функция ПолучитьДанныеЗаполненияЗадолженности(Организация, НачалоПериода, КонецПериода, МенеджерВременныхТаблиц)
	...
	ДанныеЗаполнения = ПолучитьТаблицуДанныхПоясненийЗадолженности();
	...
	|	ХозрасчетныйОстаткиИОбороты.Счет КАК Счет,
	|	ВЫБОР
	|		КОГДА НЕ ХозрасчетныйВидыСубконто1.ТолькоОбороты
	|			ТОГДА ХозрасчетныйОстаткиИОбороты.Субконто1
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Субконто1,
	|	ВЫБОР
	|		КОГДА НЕ ХозрасчетныйВидыСубконто2.ТолькоОбороты
	|			ТОГДА ХозрасчетныйОстаткиИОбороты.Субконто2
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Субконто2,
	|	ВЫБОР
	|		КОГДА НЕ ХозрасчетныйВидыСубконто3.ТолькоОбороты
	...
	|		ПО ХозрасчетныйОстаткиИОбороты.Счет = ХозрасчетныйВидыСубконто1.Ссылка
	...
	|		ПО ХозрасчетныйОстаткиИОбороты.Счет = ХозрасчетныйВидыСубконто2.Ссылка
	...
	|		ПО ХозрасчетныйОстаткиИОбороты.Счет = ХозрасчетныйВидыСубконто3.Ссылка
			...
			Если Не СтрокаСчета.СчетРезервов Тогда
				...
				Расшифровка.НаименованиеСлагаемого = СтрШаблон(
					НСтр("ru = 'Оборот с кредита счета 91.01 по статьям субконто ""Прочие доходы и расходы"" с видом ""Штрафы, пени и неустойки к получению (уплате)"" и ""Проценты к получению (уплате)"" в дебет счета %1 развернутое по всем субконто';
						|en = 'The turnover from account 91.01 credit by items of the ""Incomes and expenses"" extra dimension with the ""Receivable (payable) fees and penalties"" and ""Interest receivable (payable)"" types to the account %1 debit expanded by all extra dimensions'"),
					СтрокаСчета.Счет);
				...
				Расшифровка.НаименованиеСлагаемого = СтрШаблон(
					НСтр("ru = 'Оборот по дебету счета %1 по статьям субконто ""Виды платежей в бюджет (фонды)"" с видами ""Пени: доначислено/уплачено(самостоятельно)"", ""Пени: начислено/уплачено(по акту проверки)"" и ""Штраф: начислено/уплачено""';
						|en = 'The turnover for account %1 debit by items of the ""Kinds of payments to the budget (funds)"" extra dimension with the ""Penalties: accrued additionally / paid (independently)"" and ""Penalties: accrued / paid (according to inspection certificate)"" and ""Penalty: accrued / paid"" types'"),
					СтрокаСчета.Счет);
				...
				Расшифровка.НаименованиеСлагаемого = СтрШаблон(
					НСтр("ru = 'Оборот с кредита счета %1 в дебет счета 91.02 по статьям субконто ""Прочие доходы и расходы"" с видом ""Списание дебеторской(кредиторской) задолженности""';
						|en = 'The turnover from account %1 credit to account 91.02 debit by items of the ""Incomes and expenses"" extra dimension with the ""Accounts payable crediting (receivable debiting)"" type'"),
					СтрокаСчета.Счет);
		...
		ИначеЕсли СтрокаСчета.ВидЗадолженности = "К" Тогда
			...
			Расшифровка.НаименованиеСлагаемого = СтрШаблон(
				НСтр("ru = 'Оборот с кредита счета %1 в дебет счета 91.02 по статьям субконто ""Прочие доходы и расходы"" с видом ""Штрафы, пени и неустойки к получению (уплате)"" развернутое по всем субконто';
					|en = 'The turnover from account %1 credit to account 91.02 debit by items of the ""Incomes and expenses"" extra dimension with the ""Receivable (payable) fees and penalties"" type expanded by all extra dimensions'"),
				СтрокаСчета.Счет);
			Расшифровка.Сумма = Мин(СчетШтраф, -СчетШтраф);
			Расшифровка.ИмяРаздела = КонтекстВыполнения.ИмяРаздела;
			Расшифровка.Организация = КонтекстВыполнения.Организация;
			...
			Расшифровка.НаименованиеСлагаемого = СтрШаблон(
				НСтр("ru = 'Оборот с кредита счета %1 по статьям субконто ""Виды платежей в бюджет (фонды)"" с видами ""Пени: доначислено/уплачено(самостоятельно)"", ""Пени: начислено/уплачено(по акту проверки)"" и ""Штраф: начислено/уплачено""';
					|en = 'The turnover from account %1 credit by items of the ""Kinds of payments to the budget (funds)"" extra dimension with the ""Penalties: accrued additionally / paid (independently)"" and ""Penalties: accrued / paid (according to inspection certificate)"" and ""Penalty: accrued / paid"" types'"),
				СтрокаСчета.Счет);
			Расшифровка.Сумма = Мин(СчетПени, -СчетПени);
			Расшифровка.ИмяРаздела = КонтекстВыполнения.ИмяРаздела;
			Расшифровка.Организация = КонтекстВыполнения.Организация;
...
Функция ПараметрыЗаполненияОтчета()
	...
	Параметры.Вставить("ЭтоРасчетСтоимостиЧистыхАктивов", Ложь);
	Параметры.Вставить("ЗаполнениеФормОтчетности", Неопределено);
...
Процедура ЗаполнитьДанныеМногострочнойЧасти(Строка, ПараметрыСтроки, ПараметрыПоказателя, КонтекстВыполнения)
	...
	Если КонтекстВыполнения.ЭтоРасчетСтоимостиЧистыхАктивов Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПараметрыПоСтроке(Строка, ПараметрыПоказателя, ПараметрыСтроки, КонтекстВыполнения)
	...
	ОпределитьНомераСтрок(Строка, ПараметрыПоказателя, ПараметрыСтроки);
