...
Процедура ПриСозданииНаСервереФормаЭлемента(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	//++ НЕ УТ
	КонтролируемыеСделки.ИнициализироватьДопСведенияДоговоровУчастниковКонтролируемыхСделок(
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи, Форма) Экспорт
	...
	//++ НЕ УТ
	КонтролируемыеСделки.СохранитьДопСведенияДоговоровУчастниковКонтролируемыхСделок(
		...
		Если СправочникОбъект.ДоговорСУчастникомГОЗ Тогда
			Если ЗначениеЗаполнено(СправочникОбъект.БанковскийСчет) И ЗначениеЗаполнено(СправочникОбъект.ГосударственныйКонтракт)
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					...
					Отказ);
			КонецЕсли;
			Если ЗначениеЗаполнено(СправочникОбъект.БанковскийСчетКонтрагента) И ЗначениеЗаполнено(СправочникОбъект.ГосударственныйКонтракт) Тогда
				
				Реквизиты = Новый Структура();
				Реквизиты.Вставить("ГосударственныйКонтрактСчета", "ГосударственныйКонтракт");
				Реквизиты.Вставить("ТипСопровожденияКонтрактаСчета", "ГосударственныйКонтракт.ТипСопровождения");
				ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СправочникОбъект.БанковскийСчетКонтрагента, Реквизиты);
				
				Если СправочникОбъект.ГосударственныйКонтракт <> ЗначенияРеквизитов.ГосударственныйКонтрактСчета
					И ЗначенияРеквизитов.ТипСопровожденияКонтрактаСчета = Перечисления.ТипыСопровожденияГосударственныхКонтрактов.Банковское Тогда
...
Процедура БанковскийСчетПриИзменении(Форма)
	...
	Иначе
		Форма.Объект.ГосударственныйКонтракт = Справочники.ГосударственныеКонтракты.ПустаяСсылка();
		Форма.Объект.ДоговорСУчастникомГОЗ	 = Ложь;
		Форма.Объект.ПлатежиПо275ФЗ			 = Ложь;
		Форма.Объект.ТипПлатежаФЗ275		 = Справочники.ТипыПлатежейФЗ275.ПустаяСсылка();
		Форма.ВариантПлатежаГОЗ				 = 0;
	КонецЕсли;
...
Процедура УстановитьДоступностьПорядкаОтраженияНеотфактурованныхПоставокНУ(Форма)
	...
	Если СистемаНалогообложения = Перечисления.СистемыНалогообложения.Общая 
		И ((Форма.ИспользоватьОформлениеДокументовРаздельнойЗакупкиТоваров
		И (Форма.Объект.ВариантОформленияЗакупок = Перечисления.ВариантыОформленияЗакупок.НеотфактурованныеПоставкиТоваров
		Или Форма.Объект.ВариантОформленияЗакупок = Перечисления.ВариантыОформленияЗакупок.НеотфактурованныеПоставкиТоваровИУслуг))
		Или (Форма.ИспользоватьОформлениеДокументовРаздельнойЗакупкиУслуг
		И Форма.Объект.ВариантОформленияДокументовПоступленияПоУслугам = Перечисления.ВариантыОформленияДокументовПоУслугам.ПоступлениеУслугФактурируется)) Тогда
