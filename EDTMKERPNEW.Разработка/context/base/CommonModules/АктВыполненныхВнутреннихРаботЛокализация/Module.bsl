...
#Область ВыполнениеРабот // (Дт 20, 23 :: Кт 20, 23)
	...
	|	""АктВыполненныхВнутреннихРаботЛокализация_9"" КАК НомерФрагмента
	|ИЗ
...
#Область ВыполнениеРаботСОбособленногоПодразделения // (Дт 79.02 :: Кт 20, 23)
	...
	|	""АктВыполненныхВнутреннихРаботЛокализация_10"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ВыполнениеРаботНаОбособленноеПодразделение // (Дт 20, 23 :: Кт 79.02)
	...
	|	0 КАК СуммаВРКт,
	|	""Выполнение работ между филиалами"" КАК Содержание,
	|	""АктВыполненныхВнутреннихРаботЛокализация_11"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
