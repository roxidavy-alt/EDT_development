...
Функция ДанныеОперацийИзОчереди(КлючиОпераций) Экспорт
	...
	|	ИД.ПроизводственнаяОперация КАК ПроизводственнаяОперация,
	|	ИД.Состояние                КАК Состояние
	|ИЗ
	...
	|			И ИД.ПроизводственнаяОперация = Очередь.ПроизводственнаяОперация
	|			И ИД.Состояние = Очередь.Состояние
	|";
	...
	Для каждого Идентификатор Из Идентификаторы Цикл
		...
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Идентификатор, "Подразделение,Этап,Операция,ИдентификаторОперации,ПроизводственнаяОперация,Состояние");
	КонецЦикла;
...
Функция ДанныеЗаполненияНовыхПроизводственныхОпераций(КлючиОпераций, ПараметрыНазначения, УстанавливаемыйСтатус = Неопределено, ПроверятьПараметры = Истина) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|	
		|	ВЫБОР
		|		КОГДА ИД.Операция <> ЗНАЧЕНИЕ(Справочник.ТехнологическиеОперации.ПустаяСсылка)
		|			ТОГДА &СпособЗаполнения
		|		ИНАЧЕ """"
		...
		|	ИД.ПроизводственнаяОперация КАК ПроизводственнаяОперация,
		|
		|	Очередь.Операция.Контроль                КАК Контроль,
		...
		|	КОНЕЦ             КАК ВидРабочегоЦентра,
		|	ВЫБОР
		|		КОГДА ИД.Участок <> ЗНАЧЕНИЕ(Справочник.ПроизводственныеУчастки.ПустаяСсылка)
		|			ТОГДА ИД.Участок
		|		КОГДА ИД.РабочийЦентр <> ЗНАЧЕНИЕ(Справочник.РабочиеЦентры.ПустаяСсылка)
		|			ТОГДА ВЫРАЗИТЬ(ИД.РабочийЦентр КАК СПравочник.РабочиеЦентры).Участок
		|		ИНАЧЕ Очередь.Операция.Участок
		|	КОНЕЦ             КАК Участок,
		|	ИД.СменноеЗадание КАК СменноеЗадание,
		|	ИД.РабочийЦентр   КАК РабочийЦентр,
		|	ВЫБОР
		...
		|							ТОГДА ИСТИНА
		|						КОГДА ИД.Участок В (Очередь.Операция.РабочийЦентр.Участок,
		|												Очередь.Операция.Участок,
		|												ЗНАЧЕНИЕ(Справочник.ПроизводственныеУчастки.ПустаяСсылка))
		|							ТОГДА ИСТИНА
		|						ИНАЧЕ ЛОЖЬ
		|					КОНЕЦ
		|					И ВЫБОР
		...
		|								ИЛИ НЕ ТИПЗНАЧЕНИЯ(Очередь.Операция.РабочийЦентр) = ТИП(Справочник.РабочиеЦентры)
		|							ТОГДА ИСТИНА
		|						КОГДА ИД.РабочийЦентр В (Очередь.Операция.РабочийЦентр,
		|													ЗНАЧЕНИЕ(Справочник.РабочиеЦентры.ПустаяСсылка))
		|							ТОГДА ИСТИНА
		|						ИНАЧЕ ЛОЖЬ
		|					КОНЕЦ
		...
		|						ИНАЧЕ ИСТИНА
		|					КОНЕЦ
		|			ИЛИ ИД.ПроизводственнаяОперация <> ЗНАЧЕНИЕ(Документ.ПроизводственнаяОперация2_2.ПустаяСсылка))");
	...
	ИсточникДанных.Колонки.Добавить("ПроизводственнаяОперация", Новый ОписаниеТипов("ДокументСсылка.ПроизводственнаяОперация2_2"));
	ИсточникДанных.Колонки.Добавить("Состояние", Новый ОписаниеТипов("ПеречислениеСсылка.СостоянияВыполненияОпераций"));
...
Функция СформироватьПроизводственныеОперацииВнутриЗамераВремени(КлючиОпераций, ПараметрыНазначения, УстанавливаемыйСтатус)
	...
	Результат = ОперативныйУчетПроизводстваКлиентСервер.РезультатФормированияПроизводственныхОперацийКонструктор();
	...
	Попытка
		...
		Если СписокДанныхЗаполнения.Количество() > 0 Тогда
			...
			Для каждого ТекущиеДанные Из СписокДанныхЗаполнения Цикл
				
				ДанныеЗаполнения = ТекущиеДанные;
				
				Если ЗначениеЗаполнено(ДанныеЗаполнения.ПроизводственнаяОперация) Тогда
					Результат.Индивидуальные.Добавить(ДанныеЗаполнения.ПроизводственнаяОперация);
					ОперацияОбъект = ДанныеЗаполнения.ПроизводственнаяОперация.ПолучитьОбъект();
					ОперативныйУчетПроизводства.ЗаблокироватьПроизводственнуюОперацию(ОперацияОбъект);
					ДанныеЗаполнения = ОперативныйУчетПроизводстваКлиентСервер.ПараметрыНазначенияПроизводственнойОперацииКонструктор(Истина);
					ДанныеЗаполнения.Вставить("Статус");
					ЗаполнитьЗначенияСвойств(ДанныеЗаполнения, ТекущиеДанные);
					Счетчик = Результат.ОперацииОбновленные;
				Иначе
					ОперацияОбъект = Документы.ПроизводственнаяОперация2_2.СоздатьДокумент();
					Счетчик = Результат.ОперацииСозданные;
				КонецЕсли;
				
				ОперацияОбъект.Заполнить(ДанныеЗаполнения);
				
				Если ПараметрыНазначения.Свойство("ВидыРабот") Тогда
					ВидыРабот = ПараметрыНазначения.ВидыРабот;// ДанныеФормыКоллекция
					Отбор = Новый Структура("ВидРабот");
					
					Для Каждого Строка Из ОперацияОбъект.Трудозатраты Цикл
						
						Отбор.ВидРабот = Строка.ВидРабот;
						СтрокаВидРабот = ВидыРабот.НайтиСтроки(Отбор);
						
						Если СтрокаВидРабот.Количество() <> 0 Тогда
							Строка.Исполнитель = СтрокаВидРабот[0].Исполнитель;
						КонецЕсли;
				...
				Если ОперацияОбъект.ПроверитьЗаполнение() Тогда
					...
					Счетчик.Добавить(ОперацияОбъект.Ссылка);
	...
	Исключение
		...
		Результат.ОперацииСозданные.Очистить();
		Результат.ОперацииОбновленные.Очистить();
		
	КонецПопытки;
	
	Результат.КоличествоВсего = ИсточникДанных.Количество();
	Результат.КоличествоОбработанных = Результат.ОперацииОбновленные.Количество() + Результат.ОперацииСозданные.Количество();
...
Функция ПараметрыОформленияВыработкиСотрудников(Отбор = Неопределено, Идентификаторы = Неопределено, Коллекция = Неопределено) Экспорт
	...
	Для Каждого Итератор Из Строки Цикл
		...
		Если ТекущийКонецПериода = Неопределено Тогда
			ТекущийКонецПериода = КонецДня(Строка.КонецПериода);
...
Процедура ЗафиксироватьРезультатКонтроляОперации(ЗаписиПротокола, ПересчитатьНормативы, ДокументСсылка) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	ДокументОбъект = ДокументСсылка.ПолучитьОбъект(); // ДокументОбъект.ПроизводственнаяОперация2_2
	
	ИтогиДоИзменения = ОперативныйУчетПроизводстваКлиентСервер.СтруктураИтоговКонструктор(ДокументОбъект);
	
	Для каждого ЗаписьПротокола Из ЗаписиПротокола Цикл // см. ОперативныйУчетПроизводстваКлиентСервер.ЗаписьПротоколаКонструктор
	...
	Если ПересчитатьНормативы Тогда
		Если ДокументОбъект.НаОснованииНСИ Тогда
