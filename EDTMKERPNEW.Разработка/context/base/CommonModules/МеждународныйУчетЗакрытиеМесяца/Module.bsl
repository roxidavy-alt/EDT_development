...
Процедура ДобавитьЭтап_ОтражениеДокументовВМеждународномУчете(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ОтражениеДокументовВМеждународномУчете,
		Ложь, Ложь, Истина);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Отражение документов в международном учете';
											|en = 'Record documents in financial accounting'"; // @НСтр-1
...
Процедура ДобавитьЭтап_РеклассификацияДолгосрочныхАктивовОбязательств(ТаблицаЭтапов, ТекущийРодитель) Экспорт
	...
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.РеклассификацияДолгосрочныхАктивовОбязательствМУ,
		Ложь, Ложь, Истина);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Реклассификация долгосрочных активов и обязательств (международный учет)';
											|en = 'Reclassification of long-term assets and liabilities (financial accounting)'"; // @НСтр-1
...
Процедура ДобавитьЭтап_РасчетКурсовыхРазницВФункциональнойВалюте(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.РасчетКурсовыхРазницВФункциональнойВалюте,
		Ложь, Ложь, Истина);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Расчет курсовых разниц в функциональной валюте';
											|en = 'Exchange difference calculation in the functional currency'"; // @НСтр-1
...
Процедура ДобавитьЭтап_ЗакрытиеСчетовУчетаДоходовИРасходовМУ(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ЗакрытиеСчетовУчетаДоходовИРасходовМУ,
		Ложь, Ложь, Истина);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Закрытие счетов учета доходов и расходов';
											|en = 'Closing of ledger accounts of income and expenses'"; // @НСтр-1
	НоваяСтрока.ТекстВыполнить = НСтр("ru = 'Выполнить';
										|en = 'Run'");
	НоваяСтрока.ДействиеИспользование = ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
...
Процедура ДобавитьЭтап_РасчетКурсовыхРазницВВалютеПредставления(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.РасчетКурсовыхРазницВВалютеПредставления,
		Ложь, Ложь, Истина);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Расчет курсовых разниц в валюте представления';
											|en = 'Exchange difference calculation in the presentation currency'"; // @НСтр-1
	НоваяСтрока.ТекстВыполнить = НСтр("ru = 'Рассчитать';
										|en = 'Calculate'");
	НоваяСтрока.ДействиеИспользование = ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
	...
	Если РезультатыЗапроса[0].Пустой() Тогда
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Функциональная валюта совпадает с валютой представления.';
				|en = 'Functional currency matches the presentation currency.'", ОбщегоНазначения.КодОсновногоЯзыка()));
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьЭтап_УстановкаДатыЗапретаФормированияПроводокМУ(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.УстановкаДатыЗапретаФормированияПроводокМУ,
		Ложь, Ложь, Истина);
	
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Дата запрета формирования проводок';
											|en = 'Closing date of posting generation'"; // @НСтр-1
...
Функция НевыполненныеОтложенныеЗадания(СтатусОтражения, ДатаОтражения, Организация, Документ, Подробно = Ложь) Экспорт
	...
	|		 И ЗаданияКЗакрытиюМесяца.Операция = ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.ПризнаниеРасходовПоИсследованиямИРазработкам)
	|ГДЕ
	|	&ОтборПоСтатусуОтражения
	|	И &ОтборПоДатеОтражения
	|	И &ОтборПоОрганизации
	...
	НевыполненныеЗадания.Добавить(НСтр("ru = 'Признание расходов по исследованиям и разработкам не выполнено';
										|en = 'R&D expense recognition is not performed'"));
...
Функция ОбщийТекстЗапросаЗаданияПоРеклассификации(ИсточникДанных)
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТаблицаИзменений.Месяц КАК Месяц,
	|	ЗНАЧЕНИЕ(Справочник.ОперацииЗакрытияМесяца.РеклассификацияДолгосрочныхАктивовОбязательствМУ) КАК Операция,
	|	ТаблицаИзменений.Организация КАК Организация,
	|	ТаблицаИзменений.Документ КАК Документ
	|ИЗ 
