...
//   * НомерКорректировки - Число - номер корректировки декларации
Функция ПараметрыКонтекстаРасчета() Экспорт
	...
	ПараметрыКонтекстаРасчета.Вставить("НомерКорректировки", 0);
...
//   * ЭтоПервичнаяДекларация - Булево -  
//   * НаборыКорректировок - Массив из СправочникСсылка.КорректировкиПрошлыхПериодовНУ - 
Функция НовыйКонтекстРасчета(ПараметрыКонтекстаРасчета) Экспорт
	...
	КППОбособленногоПодразделения = ПараметрыКонтекстаРасчета.КППОбособленногоПодразделения;
	Периодичность = ПараметрыКонтекстаРасчета.Периодичность;
	НомерКорректировки = ПараметрыКонтекстаРасчета.НомерКорректировки;
	...
	ПериодыДетализацииУчета = РеглУчетСервер.ПериодыДетализацииУчета(НачалоНалоговогоПериода, 
		КонецМесяца(КонецПериода), ВидыДетализацииУчета.РеглУчетПродажПоПартиямРеализованнойНоменклатуры); 
		
	КонтекстРасчета.Вставить("ДанныеБезДетализации", ПериодыДетализацииУчета.ДанныеБезДетализации);
	КонтекстРасчета.Вставить("ДанныеСДетализацией", ПериодыДетализацииУчета.ДанныеСДетализацией);
	КонтекстРасчета.Вставить("НачалоПериодаБезДетализации", ПериодыДетализацииУчета.НачалоПериодаБезДетализации);
	КонтекстРасчета.Вставить("КонецПериодаБезДетализации", ПериодыДетализацииУчета.КонецПериодаБезДетализации);
	КонтекстРасчета.Вставить("НачалоПериодаДанныеСДетализацией", ПериодыДетализацииУчета.НачалоПериодаДанныеСДетализацией);
	КонтекстРасчета.Вставить("КонецПериодаДанныеСДетализацией", ПериодыДетализацииУчета.КонецПериодаДанныеСДетализацией);	
	КонтекстРасчета.Вставить("ЭтоПервичнаяДекларация", НомерКорректировки = 0);
	
	НаборыКорректировок = Новый Массив;
	
	Если НЕ КонтекстРасчета.ЭтоПервичнаяДекларация Тогда
		НаборыКорректировок = НалогНаПрибыльКорректировки.НаборыКорректировокПоДатеПодписиДекларации(ДатаПодписи, Организация);
	КонецЕсли;
	
	КонтекстРасчета.Вставить("НаборыКорректировок", НаборыКорректировок);
	...
	Если НалоговыйУчетОбособленныхПодразделений.УчетВРазрезеНалоговыхОрганов() Тогда
		...
		Если ЗначениеЗаполнено(РасчетРаспределенияНалога) Тогда
			...
			Для Каждого СтрокаСоответствия Из ТаблицаСтраницВариантовНО Цикл
				...
				Если ЗначениеЗаполнено(СтрокаСоответствия.НомерДокумента) Тогда
					РасчетРаспределенияПоВариантуНО.ЗаполнитьЗначения(
						СтрокаСоответствия.НомерДокумента,
						"НомерДокумента");     
				КонецЕсли;
...
Функция СхемаРасчета(КонтекстРасчета, ОписаниеСумм)
	...
	Если КонтекстРасчета.ДанныеСДетализацией Тогда
		ПерезаполнитьПоказателиПоНастройкамДетализации(СхемаРасчета, КонтекстРасчета); 
	КонецЕсли;
...
Процедура ДополнитьСхемуРасчета_РегиональныйИнвестиционныйВычет(СхемаРасчета, КонтекстРасчета, ОписаниеСумм)
	...
	ДобавитьОписание(СхемаРасчета, "02", "7А", "", НСтр("ru = 'Расчет составлен';
														|en = 'Calculation is performed'"), "Идентификатор", "00100");			
	ИсточникДанных = ДобавитьИсточникДанных(СхемаРасчета, "РегиональныйИнвестиционныйВычет");
	...
	ДобавитьОписание(СхемаРасчета, "02", "7А", "", НСтр("ru = 'Дата закона субъекта Российской Федерации, которым установлено право на налоговый вычет';
														|en = 'Date of the law of the territorial entity of the Russian Federation, which established the right to a tax deduction'"), 
		"Идентификатор", "00300");			
	ИсточникДанных = ДобавитьИсточникДанных(СхемаРасчета, "РегиональныйИнвестиционныйВычет");
	...
	ДобавитьОписание(СхемаРасчета, "02", "7А", "", НСтр("ru = 'Номер закона субъекта Российской Федерации, которым установлено право на налоговый вычет';
														|en = 'Number of the law of the territorial entity of the Russian Federation, which established the right to a tax deduction'"), 
		"Идентификатор", "00400");			
	ИсточникДанных = ДобавитьИсточникДанных(СхемаРасчета, "РегиональныйИнвестиционныйВычет");
	...
	ДобавитьОписание(СхемаРасчета, "02", "7А","010",
		НСтр("ru = 'Максимальный размер инвестиционного налогового вычета';
			|en = 'Maximum investment tax deduction'"));
	...
	ДобавитьОписание(СхемаРасчета, "02", "7А","011",
		НСтр("ru = 'Сумма расходов, формирующих ИНВ отчетного (налогового) периода, составляющих первоначальную стоимость основных средств';
			|en = 'Amount of expenses forming the investment tax deduction of the accounting (fiscal) period that make up the initial cost of fixed assets'"));
	...
	ДобавитьОписание(СхемаРасчета, "02", "7А","012",
		НСтр("ru = 'Сумма расходов, формирующих ИНВ отчетного (налогового) периода, составляющих изменение первоначальной стоимости основных средств';
			|en = 'Amount of expenses forming the investment tax deduction of the accounting (fiscal) period that make up the change in the initial cost of fixed assets'"));
	...
	ПредставлениеКолонкиСумма = НСтр("ru = 'Остаток инвестиционного налогового вычета, не учтенного при исчислении налога за предыдущие налоговые периоды';
									|en = 'Balance of the investment tax deduction that was not recorded when calculating taxes for previous fiscal periods'");
	...
	ПредставлениеКолонкиКод = НСтр("ru = 'Код расходов, формирующих инвестиционный вычет';
									|en = 'Expense code that forms the investment deduction'");
	...
	ПредставлениеКолонкиСумма = НСтр("ru = 'Сумма инвестиционного налогового вычета, учитываемая в текущем налоговом (отчетном) периоде';
									|en = 'Amount of the investment tax deduction recorded in the current fiscal (accounting) period'");
	...
	Если КонтекстРасчета.КонецПериода = КонецГода(КонтекстРасчета.КонецПериода) Тогда
		...
		ПредставлениеКолонкиСумма = НСтр("ru = 'Остаток неучтенного инвестиционного налогового вычета на конец налогового периода';
										|en = 'Balance of unaccounted investment tax deduction at the end of the fiscal period'");
	...
	ДобавитьОписание(СхемаРасчета, "02", "7Б","20", 
		НСтр("ru = 'Ставка налога, применяемая для расчета предельной величины инвестиционного налогового вычета';
			|en = 'Tax rate used to calculate the maximum investment tax deduction'"));			
	ИсточникДанных = ДобавитьИсточникДанных(СхемаРасчета, "РегиональныйИнвестиционныйВычет");
	...
	ДобавитьОписание(СхемаРасчета, "02", "7Г","020",
		НСтр("ru = 'Расходы, учитываемые при расчете уменьшения налога в федеральный бюджет';
			|en = 'Expenses recorded when calculating the tax reduction to the federal budget'"));
	...
	ДобавитьОписание(СхемаРасчета, "02", "7Г","021",
		НСтр("ru = 'Сумма расходов, формирующих ИНВ в федеральный бюджет отчетного (налогового) периода, составляющих первоначальную стоимость основных средств';
			|en = 'Amount of expenses forming the investment tax deduction to the federal budget of the accounting (fiscal) period that make up the initial cost of fixed assets'"));
	...
	ДобавитьОписание(СхемаРасчета, "02", "7Г","022",
		НСтр("ru = 'Сумма расходов, формирующих ИНВ в федеральный бюджет отчетного (налогового) периода, составляющих изменение первоначальной стоимости основных средств';
			|en = 'Amount of expenses forming the investment tax deduction to the federal budget of the accounting (fiscal) period that make up the change in the initial cost of fixed assets'"));
	...
	ПредставлениеКолонкиСумма = НСтр("ru = 'Остаток суммы уменьшения налога, подлежащей зачислению в федеральный бюджет, не учтенной при исчислении налога за предыдущие налоговые периоды';
									|en = 'Balance of the tax reduction amount to be credited to the federal budget, which was not recorded when calculating taxes for previous fiscal periods'");
	...
	ДобавитьОписание(СхемаРасчета, "02", "7Г","050",
		НСтр("ru = 'Сумма расходов, на которую уменьшены платежи текущего налогового (отчетного) периода в федеральный бюджет';
			|en = 'Amount of expenses by which payments to the federal budget of the current fiscal (accounting) period are reduced'"));
	...
	ДобавитьОписание(СхемаРасчета, "02", "7Г","051",
		НСтр("ru = 'Сумма расходов, на которую уменьшены платежи текущего налогового (отчетного) периода в федеральный бюджет, составляющих первоначальную стоимость основных средств';
			|en = 'Amount of expenses by which payments to the federal budget of the current fiscal (accounting) period, which make up the initial cost of fixed assets, are reduced'"));
	...
	ДобавитьОписание(СхемаРасчета, "02", "7Г","052",
		НСтр("ru = 'Сумма расходов, на которую уменьшены платежи текущего налогового (отчетного) периода в федеральный бюджет, составляющих изменение первоначальной стоимости основных средств';
			|en = 'Amount of expenses by which payments to the federal budget of the current fiscal (accounting) period, which make up the change in the initial cost of fixed assets, are reduced'"));
...
Процедура ДобавитьКолонкиСпискаРИНВ(Список, ПредставлениеКолонкиСумма)
	...
	ДобавитьКолонкуСписка(
		...
		НСтр("ru = 'Вариант налогообложения прибыли, к которому относится инвестиционный вычет';
			|en = 'Profit taxation option that includes an investment deduction'"),
	...
	ДобавитьКолонкуСписка(
		...
		НСтр("ru = 'Вид расходов, формирующих инвестиционный вычет';
			|en = 'Expense kind that forms the investment deduction'"),
...
Процедура ПолучитьДанныеИнформационнойБазы(ПроцессорПолученияДанных)
	...
	ИначеЕсли Алгоритм = ИмяСДетализацией("ВыручкаОбычныеВидыДеятельности") Тогда
		
		ПолучитьВыручкаОбычныеВидыДеятельностиСДетализацией(ПроцессорПолученияДанных);
		
	ИначеЕсли Алгоритм = "ПрямыеРасходы" 
		ИЛИ Алгоритм = ИмяСДетализацией("ПрямыеРасходы") Тогда
		
		ПоказательСДетализацией = Алгоритм = ИмяСДетализацией("ПрямыеРасходы");
		ПолучитьПрямыеРасходы(ПроцессорПолученияДанных, ПоказательСДетализацией);
		
	ИначеЕсли Алгоритм = "КосвенныеРасходы" 
		ИЛИ Алгоритм = ИмяСДетализацией("КосвенныеРасходы") Тогда
		
		ПоказательСДетализацией = Алгоритм = ИмяСДетализацией("КосвенныеРасходы");
		ПолучитьКосвенныеРасходы(ПроцессорПолученияДанных, ПоказательСДетализацией);
...
Функция НовыйОтборДанных(ИмяАлгоритма)
	...
	ИначеЕсли ИмяАлгоритма = "ВыручкаОбычныеВидыДеятельности" ИЛИ ИмяАлгоритма = ИмяСДетализацией("ВыручкаОбычныеВидыДеятельности") Тогда
		ОтборДанных.Вставить("Раздел", ""); // "ПродукцияПоНастройке", "УслугиБезНастройки"; пустая строка означает "все разделы"
	ИначеЕсли ИмяАлгоритма = "ПрямыеРасходы" ИЛИ ИмяАлгоритма = ИмяСДетализацией("ПрямыеРасходы") Тогда
		ОтборДанных.Вставить("КорСчет", ПланыСчетов.Хозрасчетный.ПустаяСсылка()); // пустая ссылка означает "по всем корсчетам"
	ИначеЕсли ИмяАлгоритма = "КосвенныеРасходы" ИЛИ ИмяАлгоритма = ИмяСДетализацией("КосвенныеРасходы") Тогда
...
Функция НовыйУсловиеПолученияДанныхИнформационнойБазы(ИмяАлгоритма) Экспорт
	...
	ИначеЕсли ИмяАлгоритма = ИмяСДетализацией("ВыручкаОбычныеВидыДеятельности") Тогда
		Условие.Вставить("Счет",      ПланыСчетов.Хозрасчетный.ПустаяСсылка());
		Условие.Вставить("ВариантНалогообложенияПрибыли", Справочники.ВариантыНалогообложенияПрибыли.ПустаяСсылка());		
	ИначеЕсли ИмяАлгоритма = "ПрямыеРасходы" ИЛИ ИмяАлгоритма = ИмяСДетализацией("ПрямыеРасходы") Тогда
		Условие.Вставить("Счет",        ПланыСчетов.Хозрасчетный.ПустаяСсылка());
...
Функция НастройкиОтчетаРасшифровкаЗначения(ЗаписьПротокола, СхемаРасчета)
	...
	ИначеЕсли ЗаписьПротокола.ТипЗаписи = ИмяСДетализацией("ВыручкаОбычныеВидыДеятельности") Тогда

		Настройки = НастройкиОтчетаВыручкаОбычныеВидыДеятельностиСДетализацией(
			ЗаписьПротокола.Условие,
			СхемаРасчета);
	...
	ИначеЕсли ЗаписьПротокола.ТипЗаписи = "ПрямыеРасходы" Тогда
		...
		Настройки = НастройкиОтчетаПрямыеРасходы(
			...
			СхемаРасчета,
			ЗаписьПротокола.ВариантНО);
			
	ИначеЕсли ЗаписьПротокола.ТипЗаписи = ИмяСДетализацией("ПрямыеРасходы") Тогда

		Настройки = НастройкиОтчетаПрямыеРасходыСДетализацией(
			ЗаписьПротокола.ЭтапРасчета,
			ЗаписьПротокола.Условие.Счет,
			ЗаписьПротокола.Условие.КорСчет,
			СхемаРасчета,
	...
	ИначеЕсли ЗаписьПротокола.ТипЗаписи = ИмяСДетализацией("КосвенныеРасходы") Тогда
		
		Настройки = НастройкиОтчетаКосвенныеРасходыСДетализацией(
			ЗаписьПротокола.ЭтапРасчета,
			ЗаписьПротокола.Условие.Счет,
			ЗаписьПротокола.Условие.ВидРасходов,
			СхемаРасчета,
...
Процедура ПолучитьВыручкаОбычныеВидыДеятельности(ПроцессорПолученияДанных)
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериодаБезДетализации, &КонецПериодаБезДетализации, Регистратор, Счет В (&СчетаВыручки), &СубконтоВыручки, Организация В (&Организации), , ) КАК ХозрасчетныйОбороты
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	&ПолеВариантНО КАК ВариантНО
	|ИЗ
	|	РегистрБухгалтерии.КорректировкиНалоговойБазы.Обороты(&НачалоПериодаБезДетализации, &КонецПериодаБезДетализации, Регистратор, Счет В (&СчетаВыручки), &СубконтоВыручки, (&ЭтоПервичнаяДекларация ИЛИ НаборКорректировок В (&НаборыКорректировок)) И Организация В (&Организации), , ) КАК ХозрасчетныйОбороты
	|
	|СГРУППИРОВАТЬ ПО
	|	ХозрасчетныйОбороты.Регистратор,
	|	ХозрасчетныйОбороты.Счет,
	...
	|			&НачалоПериодаБезДетализации,
	|			&КонецПериодаБезДетализации,
	...
	|			&НачалоПериодаБезДетализации,
	|			&КонецПериодаБезДетализации,
	|			(&ЭтоПервичнаяДекларация ИЛИ НаборКорректировок В (&НаборыКорректировок))
	...
	ДобавитьКолонкиСумм(СхемаЗапроса, "ХозрасчетныйОбороты.%ОборотКт", ПроцессорПолученияДанных.ОписаниеСумм, Истина, 0, Истина);
...
Процедура ПолучитьВыручкаОбычныеВидыДеятельностиСДетализацией(ПроцессорПолученияДанных)

	СхемаЗапроса = Новый СхемаЗапроса;
	СхемаЗапроса.УстановитьТекстЗапроса(ТекстЗапросаВыручкаОбычныеВидыДеятельностиСДетализацией());
	
	ДобавитьКолонкиСумм(СхемаЗапроса, "ХозрасчетныйОбороты.%ОборотКт", ПроцессорПолученияДанных.ОписаниеСумм, Ложь, 0, Истина);
	ДобавитьКолонкиСумм(СхемаЗапроса, "ДанныеВыручки.%", ПроцессорПолученияДанных.ОписаниеСумм, , 1);
	
	Запрос = НовыйЗапрос(ПроцессорПолученияДанных.КонтекстРасчета);
	
	СчетаВыручки = РеглУчетВыборкиСерверПовтИсп.СчетаВыручка();
	Запрос.УстановитьПараметр("СчетаВыручки", СчетаВыручки);
	
	ВидыСубконтоВыручки = Новый Массив;
	ВидыСубконтоВыручки.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПартииРеализованнойНоменклатуры);
	ВидыСубконтоВыручки.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НалогообложенияРеализаций);
	Запрос.УстановитьПараметр("ВидыСубконтоВыручки", ВидыСубконтоВыручки);
	...
	Для Каждого Задача Из ПроцессорПолученияДанных.Задачи Цикл
		...
		Для Каждого Счет Из СчетаВыручки Цикл
			...
			Если ПустаяСтрока(Задача.ОтборДанных.Раздел) Тогда
				...
				|ГДЕ
				|	Выручка.Счет = &Счет
				|	И Выручка.ВариантНалогообложенияПрибыли = &ВариантНО";
			...
			ИначеЕсли Задача.ОтборДанных.Раздел = "ПродукцияПоНастройке" Тогда
				...
				|ГДЕ
				|	Выручка.Счет = &Счет
				|	И Выручка.ТоварыРаботыУслугиСобственногоПроизводства
				|	И НЕ Выручка.ТипНоменклатуры В (&ТипыНоменклатурыРаботУслуг)
				|	И Выручка.ВариантНалогообложенияПрибыли = &ВариантНО";
			...
			ИначеЕсли Задача.ОтборДанных.Раздел = "УслугиБезНастройки" Тогда
				...
				|	Выручка.Счет = &Счет
				|	И Выручка.ТоварыРаботыУслугиСобственногоПроизводства
				|	И Выручка.ТипНоменклатуры В (&ТипыНоменклатурыРаботУслуг)
				|	И Выручка.ВариантНалогообложенияПрибыли = &ВариантНО";
			...
			ТипыНоменклатурыРаботУслуг = Новый Массив;
			ТипыНоменклатурыРаботУслуг.Добавить(Перечисления.ТипыНоменклатуры.Работа);
			ТипыНоменклатурыРаботУслуг.Добавить(Перечисления.ТипыНоменклатуры.Услуга);
			Запрос.УстановитьПараметр("ТипыНоменклатурыРаботУслуг", ТипыНоменклатурыРаботУслуг);   
...
Функция ТекстЗапросаВыручкаОбычныеВидыДеятельностиСДетализацией()

	Текст = "ВЫБРАТЬ
			|	ХозрасчетныйОбороты.Счет КАК Счет,
	        |	ЕСТЬNULL(ХозрасчетныйОбороты.Субконто1.ТоварыРаботыУслугиСобственногоПроизводства, ЛОЖЬ) КАК ТоварыРаботыУслугиСобственногоПроизводства,
	        |	ЕСТЬNULL(ХозрасчетныйОбороты.Субконто1.Номенклатура.ТипНоменклатуры, ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.ПустаяСсылка)) КАК ТипНоменклатуры,
	        |	ЕСТЬNULL(ХозрасчетныйОбороты.Субконто2.ВариантНалогообложенияПрибыли, ЗНАЧЕНИЕ(Справочник.ВариантыНалогообложенияПрибыли.ПустаяСсылка)) КАК ВариантНалогообложенияПрибыли
	        |ПОМЕСТИТЬ ВТ_ДанныеВыручки
	        |ИЗ
	        |	РегистрБухгалтерии.Хозрасчетный.Обороты(
			|		&НачалоПериодаДанныеСДетализацией, 
			|		&КонецПериодаДанныеСДетализацией, 
			|		, 
			...
			|		Счет В ИЕРАРХИИ (&СчетаВыручки), 
			|		&ВидыСубконтоВыручки, 
			|		(&ЭтоПервичнаяДекларация
			|			ИЛИ НаборКорректировок В (&НаборыКорректировок))
			|		И Организация В (&Организации), 
			|		, 
			|		) КАК ХозрасчетныйОбороты
			|
	        ...
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        |	ДанныеВыручки.Счет КАК Счет,
	        |	ДанныеВыручки.ТоварыРаботыУслугиСобственногоПроизводства КАК ТоварыРаботыУслугиСобственногоПроизводства,
	        |	ДанныеВыручки.ТипНоменклатуры КАК ТипНоменклатуры,
	        |	ДанныеВыручки.ВариантНалогообложенияПрибыли КАК ВариантНалогообложенияПрибыли
	        |ПОМЕСТИТЬ ВТ_Выручка
	        |ИЗ
	        |	ВТ_ДанныеВыручки КАК ДанныеВыручки
	        |
	        |СГРУППИРОВАТЬ ПО
	        |	ДанныеВыручки.Счет,
	        |	ДанныеВыручки.ТоварыРаботыУслугиСобственногоПроизводства,
	        |	ДанныеВыручки.ТипНоменклатуры,
	        |	ДанныеВыручки.ВариантНалогообложенияПрибыли";
...
Процедура ДобавитьВТекстЗапросаТаблицуРегистраторовСебестоимости(ШаблонТекстаЗапроса)
	...
	|	&НачалоПериодаБезДетализации, &КонецПериодаБезДетализации, Регистратор, СчетДт В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.СебестоимостьПродажНеЕНВД)), 
	...
	|	ХозрасчетныйОборотыДтКт.СубконтоДт2 КАК ВариантНО
	|ИЗ
	|	РегистрБухгалтерии.КорректировкиНалоговойБазы.ОборотыДтКт(
	|	&НачалоПериодаБезДетализации, &КонецПериодаБезДетализации, Регистратор, СчетДт В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.СебестоимостьПродажНеЕНВД)), 
	|	&СубконтоСебестоимости, НЕ СчетКт В ИЕРАРХИИ (&СчетаУчетаСебестоимостиРабот), , (&ЭтоПервичнаяДекларация ИЛИ НаборКорректировок В (&НаборыКорректировок)) И Организация В (&Организации)) КАК ХозрасчетныйОборотыДтКт
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДокументыРеализации КАК ДокументыРеализации
	|		ПО ХозрасчетныйОборотыДтКт.Регистратор = ДокументыРеализации.ДокументРеализации
	|
	|ИНДЕКСИРОВАТЬ ПО
...
Функция НастройкиОтчетаВыручкаОбычныеВидыДеятельности(ЭтапРасчета, Счет, Раздел, ВариантНалогообложенияПрибыли, СхемаРасчета)
	...
	Настройки = ЗаполнениеРасшифровкаРегламентированнойОтчетности.НовыйНастройкиОтчетаПоРегиструБухгалтерии(
		...
		СхемаРасчета.КонтекстРасчета.НачалоПериодаБезДетализации,
		СхемаРасчета.КонтекстРасчета.КонецПериодаБезДетализации,
		Счет,
	...
	Если НЕ СхемаРасчета.КонтекстРасчета.ЭтоПервичнаяДекларация Тогда
		ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьОтборВНастройкиОтчета(Настройки, 
			"КорректировкиНалоговойБазыПо", КонецДня(СхемаРасчета.КонтекстРасчета.ДатаПодписи), ВидСравненияКомпоновкиДанных.МеньшеИлиРавно); 
	КонецЕсли;		
...
Функция НастройкиОтчетаВыручкаОбычныеВидыДеятельностиСОтбором(ЭтапРасчета, Условие, СхемаРасчета)
	...
	Настройки.Отчет.ИмяОтчета = СтрЗаменить(Настройки.Отчет.ИмяОтчета, ".ФормаОтчета", "");
		
	Настройки.Отчет.ПараметрыФормы.Вставить("КлючВарианта", Условие.КлючВарианта);
	Настройки.Отчет.ПараметрыФормы.Вставить("Организация", СхемаРасчета.КонтекстРасчета.Организация);
	
	ПериодОтчета = Новый СтандартныйПериод;
	ПериодОтчета.ДатаНачала = СхемаРасчета.КонтекстРасчета.НачалоПериодаБезДетализации;
	ПериодОтчета.ДатаОкончания = СхемаРасчета.КонтекстРасчета.КонецПериодаБезДетализации;
	Настройки.Отчет.ПараметрыФормы.Вставить("ПериодОтчета", ПериодОтчета);
	
	Если ЗначениеЗаполнено(Условие.ВариантНалогообложенияПрибыли) Тогда
		Настройки.Отчет.ПараметрыФормы.Вставить("ВариантНалогообложенияПрибыли", Условие.ВариантНалогообложенияПрибыли);
	КонецЕсли;
	
	Настройки.Отчет.ПараметрыФормы.Вставить("ВидДоходов", Условие.ВидДоходов);

	Если НЕ СхемаРасчета.КонтекстРасчета.ЭтоПервичнаяДекларация Тогда
		Настройки.Отчет.ПараметрыФормы.Вставить("ДатаНачалаПодготовкиДекларации", КонецДня(СхемаРасчета.КонтекстРасчета.ДатаПодписи));
	КонецЕсли;		
...
Функция НастройкиОтчетаВыручкаОбычныеВидыДеятельностиСДетализацией(Условие, СхемаРасчета)
	
	Счет = Условие.Счет;
	ВариантНО = Условие.ВариантНалогообложенияПрибыли;
	Раздел = Условие.Раздел;
	
	Настройки = ЗаполнениеРасшифровкаРегламентированнойОтчетности.НовыйНастройкиОтчетаПоРегиструБухгалтерии(
		"ОборотноСальдоваяВедомостьПоСчету",
		СхемаРасчета.КонтекстРасчета.Организация,
		СхемаРасчета.КонтекстРасчета.НачалоПериодаДанныеСДетализацией,
		СхемаРасчета.КонтекстРасчета.КонецПериодаДанныеСДетализацией,
		Счет,
		СхемаРасчета.ОписаниеСумм);
	...
	ОписаниеПоляПартииРеализованнойНоменклатуры = ЗаполнениеРасшифровкаРегламентированнойОтчетности.ОписаниеСубконтоСчета(
		"Субконто",
		Счет,
		ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПартииРеализованнойНоменклатуры);
	
	ТипыНоменклатурыРаботУслуг = Новый Массив;
	ТипыНоменклатурыРаботУслуг.Добавить(Перечисления.ТипыНоменклатуры.Работа);
	ТипыНоменклатурыРаботУслуг.Добавить(Перечисления.ТипыНоменклатуры.Услуга);
	
	Если Раздел = "ПродукцияПоНастройке" 
		И ОписаниеПоляПартииРеализованнойНоменклатуры <> Неопределено Тогда
		ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьОтборВНастройкиОтчета(Настройки, 
			ОписаниеПоляПартииРеализованнойНоменклатуры.Имя + ".Номенклатура.ТипНоменклатуры", ТипыНоменклатурыРаботУслуг, ВидСравненияКомпоновкиДанных.НеВСписке); 
		ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьОтборВНастройкиОтчета(Настройки, 
			ОписаниеПоляПартииРеализованнойНоменклатуры.Имя + ".ТоварыРаботыУслугиСобственногоПроизводства", Истина); 		
	ИначеЕсли Раздел = "УслугиБезНастройки" 
		И ОписаниеПоляПартииРеализованнойНоменклатуры <> Неопределено Тогда
		ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьОтборВНастройкиОтчета(Настройки, 
			ОписаниеПоляПартииРеализованнойНоменклатуры.Имя + ".Номенклатура.ТипНоменклатуры", ТипыНоменклатурыРаботУслуг, ВидСравненияКомпоновкиДанных.ВСписке); 
		ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьОтборВНастройкиОтчета(Настройки, 
			ОписаниеПоляПартииРеализованнойНоменклатуры.Имя + ".ТоварыРаботыУслугиСобственногоПроизводства", Истина); 		
	КонецЕсли; 
	
	Если ЗначениеЗаполнено(ВариантНО) Тогда
		ОписаниеПоляНалогообложенияРеализаций = ЗаполнениеРасшифровкаРегламентированнойОтчетности.ОписаниеСубконтоСчета(
			"Субконто",
			Счет,
			ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НалогообложенияРеализаций);
			
		Если ОписаниеПоляНалогообложенияРеализаций <> Неопределено Тогда
			ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьОтборВНастройкиОтчета(Настройки, 
				"ВариантНалогообложенияПрибыли", ВариантНО); 
		КонецЕсли;
	КонецЕсли;        
...
Процедура ПолучитьИзлишкиПриИнвентаризацииДляСовместимости(ПроцессорПолученияДанных)
	...
	ДобавитьКолонкиСумм(СхемаЗапроса, ШаблонСумм, ПроцессорПолученияДанных.ОписаниеСумм, , , Истина);
...
Процедура ПолучитьПрямыеРасходы(ПроцессорПолученияДанных, Знач ПоказательСДетализацией)
	...
	Если ПроцессорПолученияДанных.КонтекстРасчета.ДанныеСДетализацией 
		И НЕ ПоказательСДетализацией Тогда
		Вариант.ШаблонОписания = НСтр("ru = 'Оборот по дебету счета [Счет] (устаревший) [ЭтапРасчета]';
										|en = '[Счет] debit turnover (obsolete) [ЭтапРасчета]'");			
	Иначе
	...
	Если ПроцессорПолученияДанных.КонтекстРасчета.ДанныеСДетализацией 
		И НЕ ПоказательСДетализацией Тогда
		Вариант.ШаблонОписания = НСтр("ru = 'Оборот по дебету счета [Счет] (устаревший) в корреспонденции со счетом [КорСчет][ЭтапРасчета]';
										|en = '[Счет] debit turnover (obsolete) in correspondence with account [КорСчет][ЭтапРасчета]'");
	Иначе
	...
	Для Каждого Вариант Из Варианты Цикл
		...
		Если ПоказательСДетализацией Тогда
			ТекстОборотыПредварительно = // модифицируется при помощи СхемаЗапроса с небезопасным обращением к элементам запроса по индексу
			"ВЫБРАТЬ
			|	ХозрасчетныйОбороты.Счет КАК Счет,
			|	ХозрасчетныйОбороты.КорСчет КАК КорСчет,
			|	ЕСТЬNULL(ХозрасчетныйОбороты.Субконто1.ВариантНалогообложенияПрибыли,
			|		ЗНАЧЕНИЕ(Справочник.ВариантыНалогообложенияПрибыли.ПустаяСсылка)) КАК ВариантНО
			|ПОМЕСТИТЬ ВТ_ОборотыПредварительно
			|ИЗ
			|	РегистрБухгалтерии.Хозрасчетный.Обороты(
			|			&НачалоПериодаДанныеСДетализацией,
			|			&КонецПериодаДанныеСДетализацией,
			...
			|			ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.НалогообложенияРеализаций),
			|			Организация В (&Организации),
			|			КорСчет В
			|				(ВЫБРАТЬ
			|					СчетаУчета.Ссылка
		...
		ТекстыЗапросов.Добавить(ТекстОборотыПредварительно);
	
		ТекстГруппировки = "
		|ВЫБРАТЬ
		|	ОборотыПредварительно.Счет КАК Счет,
		|	ОборотыПредварительно.КорСчет КАК КорСчет,
		|	ОборотыПредварительно.ВариантНО КАК ВариантНО
		|ПОМЕСТИТЬ ВТ_Обороты
		|ИЗ
		|	ВТ_ОборотыПредварительно КАК ОборотыПредварительно
		|
		|СГРУППИРОВАТЬ ПО
		|	ОборотыПредварительно.Счет,
		|	ОборотыПредварительно.КорСчет,
		|	ОборотыПредварительно.ВариантНО
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	КорСчет";
		
		ТекстыЗапросов.Добавить(ТекстГруппировки);
		
		ШаблонТекстаЗапроса = СтрСоединить(ТекстыЗапросов, ОбщегоНазначения.РазделительПакетаЗапросов());
		...
		Иначе
			...
			ОператорВыбрать = ПакетЗапросов.Операторы[1];
			ОператорВыбрать.Источники[0].Источник.Параметры[6].Выражение = Новый ВыражениеСхемыЗапроса("");// УсловиеКорСчета
			ОператорВыбрать.ВыбираемыеПоля.Удалить(1); // КорСчет. Автоматически будет удален индекс
			ПакетЗапросов.Индекс.Добавить("Счет");
		КонецЕсли;
	
		ДобавитьКолонкиСумм(СхемаЗапроса, "ХозрасчетныйОбороты.%ОборотДт", ПроцессорПолученияДанных.ОписаниеСумм, , , Истина);
		ДобавитьКолонкиСумм(СхемаЗапроса, "ОборотыПредварительно.%", ПроцессорПолученияДанных.ОписаниеСумм, , 1);
...
Функция НастройкиОтчетаПрямыеРасходы(ЭтапРасчета, Счет, КорСчет, СхемаРасчета, ВариантНО)
	...
	Если ЗначениеЗаполнено(ВариантНО) Тогда
		...
		Если ОписаниеПоляВариантыНалогообложенияПрибыли <> Неопределено Тогда
			ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьОтборВНастройкиОтчета(Настройки, 
				ОписаниеПоляВариантыНалогообложенияПрибыли.Имя, ВариантНО); 
		Иначе
			ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьОтборВНастройкиОтчета(Настройки, 
				"Субконто2", ВариантНО); 
		КонецЕсли;
...
Функция НастройкиОтчетаПрямыеРасходыСДетализацией(ЭтапРасчета, Счет, КорСчет, СхемаРасчета, ВариантНО)
	...
	Если ЗначениеЗаполнено(ВариантНО) Тогда
		...
		Если ОписаниеПоляНалогообложенияРеализаций <> Неопределено Тогда
			ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьОтборВНастройкиОтчета(Настройки, 
				ОписаниеПоляНалогообложенияРеализаций.Имя + ".ВариантНалогообложенияПрибыли", ВариантНО); 
		КонецЕсли;
	КонецЕсли;        
...
Процедура ПолучитьКосвенныеРасходы(ПроцессорПолученияДанных, Знач ПоказательСДетализацией)
	...
	Если ПроцессорПолученияДанных.КонтекстРасчета.ДанныеСДетализацией 
		И НЕ ПоказательСДетализацией Тогда
		Вариант.ШаблонОписания = НСтр("ru = 'Оборот по дебету счета [Счет] (устаревший) в корреспонденции со счетами затрат по статьям вида ""[ВидРасходов]""[ЭтапРасчета]';
										|en = 'Account debit turnover [Счет] (obsolete) in correspondence with cost accounts by items of the ""[ВидРасходов]""[ЭтапРасчета] kind'");
	Иначе
		Вариант.ШаблонОписания = НСтр("ru = 'Оборот по дебету счета [Счет] по статьям вида ""[ВидРасходов]""[ЭтапРасчета]';
										|en = 'Account debit turnover [Счет] by items of the ""[ВидРасходов]""[ЭтапРасчета] kind'");
	КонецЕсли;
	...
	Если ПоказательСДетализацией Тогда
		ТекстОборотыПредварительно = 	
		"ВЫБРАТЬ
		|	ОборотыДтКт.СчетДт КАК Счет,
		|	ЕСТЬNULL(ОборотыДтКт.СубконтоДт1.ВариантНалогообложенияПрибыли,
		|		ЗНАЧЕНИЕ(Справочник.ВариантыНалогообложенияПрибыли.ПустаяСсылка)) КАК ВариантНО,
		|	ОборотыДтКт.СубконтоДт2 КАК СтатьяЗатрат,
		|	СУММА(ОборотыДтКт.СуммаОборот) КАК Сумма,
		|	СУММА(ОборотыДтКт.СуммаНУОборотДт) КАК СуммаНУ,
		|	СУММА(ОборотыДтКт.СуммаПРОборотДт) КАК СуммаПР,
		|	СУММА(ОборотыДтКт.СуммаВРОборотДт) КАК СуммаВР
		|ПОМЕСТИТЬ ВТ_ОборотыПредварительно
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(
		|			&НачалоПериодаДанныеСДетализацией,
		...
		|					ИЗ
		|						СчетаУчета),
		|			&ВидыСубконтоНалогообложениеРеализацийСтатьяЗатрат,
		|			,
		|			,
		|			Организация В (&Организации)
		|			) КАК ОборотыДтКт
	...
	|	ОборотыПредварительно.Счет КАК Счет,
	|	ОборотыПредварительно.ВариантНО КАК ВариантНО,
	|	ОборотыПредварительно.СтатьяЗатрат КАК СтатьяЗатрат,
	|	СУММА(ОборотыПредварительно.Сумма) КАК Сумма,
	|	СУММА(ОборотыПредварительно.СуммаНУ) КАК СуммаНУ,
	|	СУММА(ОборотыПредварительно.СуммаПР) КАК СуммаПР,
	|	СУММА(ОборотыПредварительно.СуммаВР) КАК СуммаВР
	|ПОМЕСТИТЬ ВТ_Обороты
	|ИЗ
	|	ВТ_ОборотыПредварительно КАК ОборотыПредварительно
	|
	...
	|	ОборотыПредварительно.Счет,
	|	ОборотыПредварительно.ВариантНО,
	|	ОборотыПредварительно.СтатьяЗатрат
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СтатьяЗатрат";
...
Функция НастройкиОтчетаКосвенныеРасходы(ЭтапРасчета, Счет, ВидРасходов, СхемаРасчета, ВариантНО)
...
Функция НастройкиОтчетаКосвенныеРасходыСДетализацией(ЭтапРасчета, Счет, ВидРасходов, СхемаРасчета, ВариантНО)
	...
	Если ОписаниеПоляПартииРеализованнойНоменклатуры <> Неопределено Тогда
		ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьГруппировкуВНастройкиОтчета(
			Настройки,
			ОписаниеПоляПартииРеализованнойНоменклатуры.Имя,
			ОписаниеПоляПартииРеализованнойНоменклатуры.Представление);
	КонецЕсли;        
	...
	ОписаниеПоляСтатьиЗатрат = ЗаполнениеРасшифровкаРегламентированнойОтчетности.ОписаниеСубконтоСчета(
		"Субконто",
		Счет,
		ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат);
	
	Если ЗначениеЗаполнено(ВидРасходов) И ОписаниеПоляСтатьиЗатрат <> Неопределено Тогда
		ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьОтборВНастройкиОтчета(Настройки, 
			ОписаниеПоляСтатьиЗатрат.Имя + ".ВидРасходов", ВидРасходов); 
	КонецЕсли;
	
	Если ЭтапРасчета = "НеУчитываемые" И ОписаниеПоляСтатьиЗатрат <> Неопределено Тогда
		ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьОтборВНастройкиОтчета(Настройки, 
			ОписаниеПоляСтатьиЗатрат.Имя + ".ВидРасходов", 
			Перечисления.ВидыРасходовНУ.НеУчитываемыеВЦеляхНалогообложения); 		
	ИначеЕсли ЭтапРасчета = "РаспределенныеНаСпециальныеРежимы" И ОписаниеПоляСтатьиЗатрат <> Неопределено  Тогда
		ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьОтборВНастройкиОтчета(Настройки, 
			ОписаниеПоляСтатьиЗатрат.Имя + ".ВидДеятельностиДляНалоговогоУчетаЗатрат", 
			Перечисления.ВидыДеятельностиДляНалоговогоУчетаЗатрат.РаспределяемыеЗатраты); 		
	ИначеЕсли ЭтапРасчета = "ОтнесенныеНаСпециальныеРежимы" И ОписаниеПоляСтатьиЗатрат <> Неопределено  Тогда
		...
		ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьОтборВНастройкиОтчета(Настройки, 
			ОписаниеПоляСтатьиЗатрат.Имя + ".ВидДеятельностиДляНалоговогоУчетаЗатрат", 
			Перечисления.ВидыДеятельностиДляНалоговогоУчетаЗатрат.РаспределяемыеЗатраты, ВидСравненияКомпоновкиДанных.НеРавно); 		
	КонецЕсли;
...
Функция РезультатЗапросаПрочиеДоходыРасходы(ХарактеристикиДоходовРасходов, ПроцессорПолученияДанных)
	...
	ТекстГруппировки = "
	|ВЫБРАТЬ
	|	ОборотыПредварительно.СтатьяДоходов КАК СтатьяДоходов,
	|	ОборотыПредварительно.ВариантНО КАК ВариантНО
	|ПОМЕСТИТЬ ВТ_Обороты
	|ИЗ
	|	ВТ_ОборотыПредварительно КАК ОборотыПредварительно
	|
	|СГРУППИРОВАТЬ ПО
	|	ОборотыПредварительно.СтатьяДоходов,
	|	ОборотыПредварительно.ВариантНО
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СтатьяДоходов";
	...
	Если ХарактеристикиДоходовРасходов.УчитыватьДолиСпециальныеРежимы Тогда
		...
		СхемаЗапроса.ПакетЗапросов[0].Операторы[1].Источники[0].Источник.Параметры[2].Выражение = Новый ВыражениеСхемыЗапроса("Месяц");
		ДобавитьПолеСхемыЗапроса(СхемаЗапроса, "Период", "Месяц", Истина, , Истина);
		ДобавитьПолеСхемыЗапроса(СхемаЗапроса, "Месяц", "Месяц", Истина, 1, Истина);
	КонецЕсли;
	
	Если ХарактеристикиДоходовРасходов.ДетализироватьПоРеализованнымОбъектам Тогда
		ДобавитьПолеСхемыЗапроса(СхемаЗапроса, "ХозрасчетныйОбороты.Субконто3", "РеализованныйОбъект", Истина, , Истина);
		ДобавитьПолеСхемыЗапроса(СхемаЗапроса, "ОборотыПредварительно.РеализованныйОбъект", "РеализованныйОбъект", Истина, 1);
	КонецЕсли;
...
Функция РезультатЗапросаРасходыПризнанныеПослеПолученияУбытка(ХарактеристикиРасходов, ПроцессорПолученияДанных)
	...
	ТекстОборотыПредварительно = // модифицируется при помощи СхемаЗапроса
	"ВЫБРАТЬ
	|	ХозрасчетныйОбороты.Субконто1 КАК СтатьяДоходов,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ХозрасчетныйОбороты.Субконто2 КАК Справочник.ВариантыНалогообложенияПрибыли),
	|		ЗНАЧЕНИЕ(Справочник.ВариантыНалогообложенияПрибыли.ПустаяСсылка)) КАК ВариантНО
	...
	|	Движения.ВариантНО КАК ВариантНО
	|ПОМЕСТИТЬ ВТ_Обороты
	|ИЗ
	|	ВТ_ОборотыПредварительно КАК Движения
	|
	|СГРУППИРОВАТЬ ПО
	...
	ДобавитьКолонкиСумм(СхемаЗапроса, "ХозрасчетныйОбороты.%ОборотДт", ПроцессорПолученияДанных.ОписаниеСумм, , , Истина);
	ДобавитьКолонкиСумм(СхемаЗапроса, "Движения.%", ПроцессорПолученияДанных.ОписаниеСумм, , 1);
	
	Если ХарактеристикиРасходов.ДетализироватьПоРеализованнымОбъектам Тогда
		ДобавитьПолеСхемыЗапроса(СхемаЗапроса, "ХозрасчетныйОбороты.Субконто3", "РеализованныйОбъект", Истина, , Истина);
		ДобавитьПолеСхемыЗапроса(СхемаЗапроса, "Движения.РеализованныйОбъект", "РеализованныйОбъект", Истина, 1);
	КонецЕсли;
			...
			ИначеЕсли Показатель = ПоказателиРИНВ.Расходы 
				...
				Если Показатель = ПоказателиРИНВ.Расходы Тогда
					ШаблонОписания = НСтр("ru = 'Оборот по дебету счета %1 с видом расходов ""%2""';
											|en = '%1 debit account turnover with the ""%2"" expense kind'");
				ИначеЕсли Показатель = ПоказателиРИНВ.УменьшенНалог Тогда
					ШаблонОписания = НСтр("ru = 'Оборот по кредиту счета %1 с видом расходов ""%2""';
											|en = '%1 credit account turnover with the ""%2"" expense kind'");
				КонецЕсли;
			...
			Для Каждого СтрокаВычета Из СтрокиВычета Цикл
				...
				Если Алгоритм = "РегиональныйИнвестиционныйВычет.НачальноеСальдо.КодыРасходов"
					...
					ШаблонОписания = НСтр("ru = 'Начальное сальдо по дебету счета %1 по виду расходов с кодом %2';
											|en = 'Opening debit balance of account %1 by expense kind with code %2'", ОбщегоНазначения.КодОсновногоЯзыка());
				ИначеЕсли Алгоритм = "РегиональныйИнвестиционныйВычет.УменьшенНалог.КодыРасходов" Тогда
					ШаблонОписания = НСтр("ru = 'Оборот по кредиту счета %1 с видом расходов ""%2""';
											|en = '%1 credit account turnover with the ""%2"" expense kind'", ОбщегоНазначения.КодОсновногоЯзыка());
				ИначеЕсли Алгоритм = "РегиональныйИнвестиционныйВычет.КонечноеСальдо.КодыРасходов"
					Или Алгоритм = "РегиональныйИнвестиционныйВычет.КонечноеСальдоФБ.КодыРасходов" Тогда
					ШаблонОписания = НСтр("ru = 'Конечное сальдо по дебету счета %1 по виду расходов с кодом %2';
											|en = 'Closing debit balance of account %1 by expense kind with code %2'", ОбщегоНазначения.КодОсновногоЯзыка());
				КонецЕсли;
...
Функция ДанныеРегиональногоИнвестиционногоВычета(КонтекстРасчета, РегистрацииВНО)
	...
	НастройкиПримененияРИНВ = НалоговыйУчет.ДействующиеНастройкиПримененияИНВПоОрганизации(КонтекстРасчета.Организация, 
		КонтекстРасчета.КонецПериода);
...
Процедура ПолучитьФедеральныйИнвестиционныйВычетСальдо(ПроцессорПолученияДанных, Алгоритм)
	...
	Если Алгоритм = "ФедеральныйИнвестиционныйВычет.НачальноеСальдо.ПериодыОбразования" Тогда
		ШаблонОписания = НСтр("ru = 'Начальное сальдо по дебету счета %1 за %2 год';
								|en = 'Opening debit balance of account %1 for %2'", ОбщегоНазначения.КодОсновногоЯзыка());
	Иначе
		ШаблонОписания = НСтр("ru = 'Конечное сальдо по дебету счета %1 за %2 год';
								|en = 'Closing debit balance of account %1 for %2'", ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
	...
	Для Каждого Задача Из МассивЗадач Цикл
		...
		Если Задача.ОтборДанных.Показатель = ПоказателиФИНВ.Расходы Тогда
			...
			ШаблонОписания = НСтр("ru = 'Оборот по дебету счета %1 с видом расходов ""%2"" %3';
									|en = '%1 debit account turnover with the ""%2"" %3 expense kind'");
...
Процедура ПолучитьКапитальныеВложения(ПроцессорПолученияДанных)
	...
	|ПОМЕСТИТЬ ВТ_РасходыНаКапитальныеВложенияПредварительно
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(
	|			&НачалоПериода, 
	|			&КонецПериода,,,, 
	...
	|
	|ВЫБРАТЬ
	|	КорректировкиНалоговойБазыДтКт.ПодразделениеКт КАК Подразделение,
	|	КорректировкиНалоговойБазыДтКт.НаправлениеДеятельностиКт КАК НаправлениеДеятельности,
	|	КорректировкиНалоговойБазыДтКт.СубконтоКт1 КАК ОсновноеСредство,
	|	КорректировкиНалоговойБазыДтКт.СубконтоКт2 КАК ДокументАмортизационнойПремии,
	|	СУММА(КорректировкиНалоговойБазыДтКт.СуммаНУОборотКт) КАК Сумма
	|ИЗ
	|	РегистрБухгалтерии.КорректировкиНалоговойБазы.ОборотыДтКт(
	|			&НачалоПериода, 
	|			&КонецПериода,,,, 
	...
	|			(&ЭтоПервичнаяДекларация
	|				ИЛИ НаборКорректировок В (&НаборыКорректировок))
	|				И Организация В (&Организации)) КАК КорректировкиНалоговойБазыДтКт
	|
	|СГРУППИРОВАТЬ ПО
	|	КорректировкиНалоговойБазыДтКт.СубконтоКт1,
	|	КорректировкиНалоговойБазыДтКт.СубконтоКт2,
	|	КорректировкиНалоговойБазыДтКт.ПодразделениеКт,
	|	КорректировкиНалоговойБазыДтКт.НаправлениеДеятельностиКт
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РасходыНаКапитальныеВложенияПредварительно.Подразделение КАК Подразделение,
	|	РасходыНаКапитальныеВложенияПредварительно.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	РасходыНаКапитальныеВложенияПредварительно.ОсновноеСредство КАК ОсновноеСредство,
	|	РасходыНаКапитальныеВложенияПредварительно.ДокументАмортизационнойПремии КАК ДокументАмортизационнойПремии,
	|	СУММА(РасходыНаКапитальныеВложенияПредварительно.Сумма) КАК Сумма
	|ПОМЕСТИТЬ РасходыНаКапитальныеВложения
	|ИЗ
	|	ВТ_РасходыНаКапитальныеВложенияПредварительно КАК РасходыНаКапитальныеВложенияПредварительно
	|
	|СГРУППИРОВАТЬ ПО
	|	РасходыНаКапитальныеВложенияПредварительно.ОсновноеСредство,
	|	РасходыНаКапитальныеВложенияПредварительно.ДокументАмортизационнойПремии,
	|	РасходыНаКапитальныеВложенияПредварительно.Подразделение,
	|	РасходыНаКапитальныеВложенияПредварительно.НаправлениеДеятельности
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|		ИЛИ ТИПЗНАЧЕНИЯ(РасходыНаКапитальныеВложения.ДокументАмортизационнойПремии) = ТИП(Документ.ОтнесениеОтклоненийВСтоимостьОС)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство
	|;
	...
	|	ВЫБРАТЬ
	|		КорректировкиНалоговойБазыДтКт.ПодразделениеКт КАК Подразделение,
	|		КорректировкиНалоговойБазыДтКт.Регистратор КАК Регистратор,
	|		КорректировкиНалоговойБазыДтКт.СубконтоКт1 КАК ОсновноеСредство,
	|		КорректировкиНалоговойБазыДтКт.СуммаНУОборотКт КАК СуммаМодернизации
	|	ИЗ
	|		РегистрБухгалтерии.КорректировкиНалоговойБазы.ОборотыДтКт(
	|				&НачалоПериода,
	|				&КонецПериода,
	...
	|							ДокументыМодернизации.ОсновноеСредство
	|						ИЗ
	|							ДокументыМодернизации КАК ДокументыМодернизации)) КАК КорректировкиНалоговойБазыДтКт
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	|		КорректировкиНалоговойБазыДтКт.ПодразделениеКт КАК Подразделение,
	|		КорректировкиНалоговойБазыДтКт.Регистратор,
	|		КорректировкиНалоговойБазыДтКт.СубконтоДт1,
	|		КорректировкиНалоговойБазыДтКт.СуммаНУОборотКт
	|	ИЗ
	|		РегистрБухгалтерии.КорректировкиНалоговойБазы.ОборотыДтКт(
	|				&НачалоПериода,
	|				&КонецПериода,
	...
	|
	|			КОГДА НЕ ОтнесениеОтклоненийВСтоимостьОС.Ссылка ЕСТЬ NULL
	|				ТОГДА ОтнесениеОтклоненийВСтоимостьОС.ПроцентКапитальныхВложенийВключаемыхВРасходыНУ
	|
	...
	|
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтнесениеОтклоненийВСтоимостьОС.ОС КАК ОтнесениеОтклоненийВСтоимостьОС
	|			ПО (РасходыНаКапитальныеВложения.ДокументАмортизационнойПремии = ОтнесениеОтклоненийВСтоимостьОС.Ссылка)
	|				И РасходыНаКапитальныеВложения.ОсновноеСредство = ОтнесениеОтклоненийВСтоимостьОС.ОсновноеСредство
	|				И ОтнесениеОтклоненийВСтоимостьОС.ПроцентКапитальныхВложенийВключаемыхВРасходыНУ <> 0
	|				И ОтнесениеОтклоненийВСтоимостьОС.Ссылка.АмортизационнаяПремияУказанаВПроцентах
	|
	...
	|				И ОтнесениеОтклоненийВСтоимостьОС.Ссылка ЕСТЬ NULL
	...
	|			ПО РасходыНаКапитальныеВложения.ДокументАмортизационнойПремии = ОтнесениеОтклоненийВСтоимостьОС.Ссылка
	|				И РасходыНаКапитальныеВложения.ОсновноеСредство = ОтнесениеОтклоненийВСтоимостьОС.ОсновноеСредство
	|				И НЕ ОтнесениеОтклоненийВСтоимостьОС.Ссылка.АмортизационнаяПремияУказанаВПроцентах
	|	ГДЕ
...
Процедура ПолучитьАмортизация(ПроцессорПолученияДанных)
	...
	|ПОМЕСТИТЬ ПодготовленныеДанныеПредварительно
	|ИЗ
	...
	|	ЕСТЬNULL(ОсновныеСредства.СуммаНУОборотКт, 0) - ЕСТЬNULL(ОсновныеСредства.СуммаНУОборотДт, 0) КАК НеУчитываемые
	|ИЗ
	|	РегистрБухгалтерии.КорректировкиНалоговойБазы.ОборотыДтКт(&НачалоПериода, &КонецПериода, , НЕ СчетДт В (&СчетаОсновныеСредства), , СчетКт В (&СчетаОсновныеСредства), ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.ОсновныеСредства), (&ЭтоПервичнаяДекларация ИЛИ НаборКорректировок В (&НаборыКорректировок)) И Организация В (&Организации)) КАК ОсновныеСредства
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ЕСТЬNULL(НематериальныеАктивы.СуммаНУОборотКт, 0) - ЕСТЬNULL(НематериальныеАктивы.СуммаНУОборотДт, 0)
	|ИЗ
	|	РегистрБухгалтерии.КорректировкиНалоговойБазы.ОборотыДтКт(&НачалоПериода, &КонецПериода, , НЕ СчетДт В (&СчетаНематериальныеАктивы), , СчетКт В (&СчетаНематериальныеАктивы), ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.НематериальныеАктивы), (&ЭтоПервичнаяДекларация ИЛИ НаборКорректировок В (&НаборыКорректировок)) И Организация В (&Организации)) КАК НематериальныеАктивы";
		
	ТекстыЗапросов.Добавить(ТекстЗапроса);
	
	ТекстГруппировки = "
	|ВЫБРАТЬ
	|	ПодготовленныеДанныеПредварительно.Организация КАК Организация,
	|	ПодготовленныеДанныеПредварительно.Подразделение КАК Подразделение,
	|	ПодготовленныеДанныеПредварительно.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ПодготовленныеДанныеПредварительно.ВидАктива КАК ВидАктива,
	|	СУММА(ПодготовленныеДанныеПредварительно.Все) КАК Все,
	|	СУММА(ПодготовленныеДанныеПредварительно.НеУчитываемые) КАК НеУчитываемые
	|ПОМЕСТИТЬ ПодготовленныеДанные
	|ИЗ
	|	ПодготовленныеДанныеПредварительно КАК ПодготовленныеДанныеПредварительно
	|
	|СГРУППИРОВАТЬ ПО
	|	ПодготовленныеДанныеПредварительно.Организация,
	|	ПодготовленныеДанныеПредварительно.Подразделение,
	|	ПодготовленныеДанныеПредварительно.НаправлениеДеятельности,
	|	ПодготовленныеДанныеПредварительно.ВидАктива
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	Подразделение,
...
Функция СуммыАмортизацииСпециальныеНалоговыеРежимы(ШаблонЗапроса, ЕстьИсключенияНелинейногоМетода, СчетаУчета, ПроцессорПолученияДанных)
	...
	ОператорЗапроса.ВыбираемыеПоля.Добавить("ПодготовленныеДанныеПредварительно.Месяц");
	КолонкаМесяц = ОписаниеЗапроса.Колонки[ОписаниеЗапроса.Колонки.Количество()-1];
	КолонкаМесяц.Псевдоним = "Месяц";
	ОператорЗапроса.Группировка.Добавить("ПодготовленныеДанныеПредварительно.Месяц");
...
Процедура ПолучитьУбыткиПрошлыхЛет(ПроцессорПолученияДанных)
	...
	|ПОМЕСТИТЬ ВТ_СуммыУбыткаПредварительно
	|ИЗ
	...
	|	СУММА(Убытки.СуммаНУОстатокДт) КАК СуммаНУ
	|ИЗ
	|	РегистрБухгалтерии.КорректировкиНалоговойБазы.Остатки(
	|			&НачалоПериода,
	|			Счет В (&СчетаУчетаУбытков),
	|			&МассивСубконто,
	|			(&ЭтоПервичнаяДекларация
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_СуммыУбыткаПредварительно.СтатьяУбытка КАК СтатьяУбытка,
	|	ВТ_СуммыУбыткаПредварительно.ВариантНО КАК ВариантНО,
	|	СУММА(ВТ_СуммыУбыткаПредварительно.СуммаНУ) КАК СуммаНУ
	|ПОМЕСТИТЬ ВТ_СуммыУбытка
	|ИЗ
	|	ВТ_СуммыУбыткаПредварительно КАК ВТ_СуммыУбыткаПредварительно
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_СуммыУбыткаПредварительно.СтатьяУбытка,
	|	ВТ_СуммыУбыткаПредварительно.ВариантНО
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура ПолучитьПереносУбытковПрошлыхЛет(ПроцессорПолученияДанных)
	...
	|ПОМЕСТИТЬ СуммыУбыткаПредварительно
	|ИЗ
	...
	|	СУММА(УменьшениеПрибыли.СуммаНУОборотДт) КАК СуммаНУ
	|ИЗ
	|	РегистрБухгалтерии.КорректировкиНалоговойБазы.ОборотыДтКт(&НачалоПериода, &КонецПериода,, СчетДт = &СчетУчетаПрибыли,, СчетКт В
	|	(&СчетаУчетаУбытков), &МассивСубконто, (&ЭтоПервичнаяДекларация ИЛИ НаборКорректировок В (&НаборыКорректировок)) И Организация В (&Организации)) КАК УменьшениеПрибыли
	|СГРУППИРОВАТЬ ПО
	|	УменьшениеПрибыли.СубконтоКт1,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(УменьшениеПрибыли.СубконтоКт2 КАК Справочник.ВариантыНалогообложенияПрибыли),
	|		ЗНАЧЕНИЕ(Справочник.ВариантыНалогообложенияПрибыли.ПустаяСсылка))
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СуммыУбыткаПредварительно.СтатьяУбытка КАК СтатьяУбытка,
	|	СуммыУбыткаПредварительно.ВариантНО КАК ВариантНО,
	|	СУММА(СуммыУбыткаПредварительно.СуммаНУ) КАК СуммаНУ
	|ПОМЕСТИТЬ СуммыУбытка
	|ИЗ
	|	СуммыУбыткаПредварительно
	|СГРУППИРОВАТЬ ПО
	|	СуммыУбыткаПредварительно.СтатьяУбытка,
	|	СуммыУбыткаПредварительно.ВариантНО
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ОписаниеФормыРасчетаРаспределенияНалога(КонтекстРасчета, ИмяРедакции)
	...
	Показатель = ОписаниеФормыОтчета.Добавить();
	Показатель.Имя                  = "НомерДокумента";
	Показатель.Представление        = "Номер документа";
	Показатель.СлужебныйНомерСтроки = РегламентированныйОтчетПрибыль.КодМножественногоЭлемента(
		КодСпискаФНС,
		"НомерДокумента",
		ИмяРедакции);
...
Функция НовыйРасчетРаспределенияНалога()
	...
	Расчет.Колонки.Добавить("НомерДокумента",                   ОбщегоНазначения.ОписаниеТипаСтрока(21));
...
Процедура ПерезаполнитьПоказателиПоНастройкамДетализации(СхемаРасчета, КонтекстРасчета)
	
	ИсходныеПоказатели = СхемаРасчета.Показатели;
	СхемаРасчета.Показатели = ИсходныеПоказатели.СкопироватьКолонки();
	
	ИсходныеФормулы = СхемаРасчета.Формулы;
	СхемаРасчета.Формулы = ИсходныеФормулы.СкопироватьКолонки();
	
	ИменаПоказателейСБезДетализацией = Новый Соответствие;
	
	Для каждого СтрокаБезДетализации Из ИсходныеПоказатели Цикл
		Алгоритм = СтрокаБезДетализации.АлгоритмПолученияДанных;
		
		Если ЭтоАлгоритмСДетализацией(Алгоритм) Тогда
			Если КонтекстРасчета.ДанныеБезДетализации Тогда
				ЗаполнитьЗначенияСвойств(СхемаРасчета.Показатели.Добавить(), СтрокаБезДетализации);
			КонецЕсли;
			
			СтрокаСДетализацией = СхемаРасчета.Показатели.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаСДетализацией, СтрокаБезДетализации);
		    СтрокаСДетализацией.АлгоритмПолученияДанных = ИмяСДетализацией(Алгоритм);  
			СтрокаСДетализацией.ОтборДанных = ОбщегоНазначения.СкопироватьРекурсивно(СтрокаБезДетализации.ОтборДанных);    
			СтрокаСДетализацией.Имя = ИмяСДетализацией(СтрокаБезДетализации.Имя);
			
			ИменаПоказателейСБезДетализацией.Вставить(СтрокаБезДетализации.Имя, СтрокаСДетализацией.Имя);
	...
	Для каждого СтрокаБезДетализации Из ИсходныеФормулы Цикл
		ИмяСДетализацией = ИменаПоказателейСБезДетализацией.Получить(СтрокаБезДетализации.ЭлементФормулы);
		
		Если ИмяСДетализацией = Неопределено Тогда
			ЗаполнитьЗначенияСвойств(СхемаРасчета.Формулы.Добавить(), СтрокаБезДетализации);		
		Иначе	 
			...
			СтрокаСДетализацией = СхемаРасчета.Формулы.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаСДетализацией, СтрокаБезДетализации);
			СтрокаСДетализацией.ЭлементФормулы = ИмяСДетализацией;
		КонецЕсли;
	КонецЦикла;

КонецПроцедуры

Функция ЭтоАлгоритмСДетализацией(Знач ИмяАлгоритма)
	
	ЭтоАлгоритмСДетализацией = Ложь;
	
	Если ЗначениеЗаполнено(ИмяАлгоритма) Тогда
		АлгоритмыСДетализацией = АлгоритмыСДетализацией();
		ЭтоАлгоритмСДетализацией = АлгоритмыСДетализацией.Получить(ИмяАлгоритма) <> Неопределено;
	КонецЕсли; 

	Возврат ЭтоАлгоритмСДетализацией; 

КонецФункции 

Функция АлгоритмыСДетализацией()

	АлгоритмыСДетализацией = Новый Соответствие;
	АлгоритмыСДетализацией.Вставить("КосвенныеРасходы", Истина);	
	АлгоритмыСДетализацией.Вставить("ПрямыеРасходы", Истина);	
	АлгоритмыСДетализацией.Вставить("ВыручкаОбычныеВидыДеятельности", Истина);	
		
	Возврат АлгоритмыСДетализацией;
	
КонецФункции

Функция ИмяСДетализацией(Знач Имя)

	ИмяСДетализацией = Имя + "СДетализацией";
	
	Возврат ИмяСДетализацией;	
...
Функция НовыйЗапрос(КонтекстРасчета, ВременныеТаблицы = Неопределено)
	...
	Запрос.УстановитьПараметр("НачалоПериодаБезДетализации", КонтекстРасчета.НачалоПериодаБезДетализации);
	Запрос.УстановитьПараметр("КонецПериодаБезДетализации",  КонтекстРасчета.КонецПериодаБезДетализации);
	Запрос.УстановитьПараметр("НачалоПериодаДанныеСДетализацией", КонтекстРасчета.НачалоПериодаДанныеСДетализацией);
	Запрос.УстановитьПараметр("КонецПериодаДанныеСДетализацией",  КонтекстРасчета.КонецПериодаДанныеСДетализацией);
	
	ВидыСубконтоНалогообложениеРеализацийСтатьяЗатрат = Новый Массив;
	ВидыСубконтоНалогообложениеРеализацийСтатьяЗатрат.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НалогообложенияРеализаций);
	ВидыСубконтоНалогообложениеРеализацийСтатьяЗатрат.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат);
	
	Запрос.УстановитьПараметр("ВидыСубконтоНалогообложениеРеализацийСтатьяЗатрат", ВидыСубконтоНалогообложениеРеализацийСтатьяЗатрат);

	Запрос.УстановитьПараметр("ЭтоПервичнаяДекларация", КонтекстРасчета.ЭтоПервичнаяДекларация);
	Запрос.УстановитьПараметр("НаборыКорректировок",  КонтекстРасчета.НаборыКорректировок);
...
Процедура ДобавитьКолонкиСумм(СхемаЗапроса, ШаблонСумм, ОписаниеСумм, ПолеСГруппировкой = Истина, НомерПакета = 0, УчитыватьОбъединение = Ложь)
	
	Если ПолеСГруппировкой Тогда
		ШаблонВыражения = "ЕСТЬNULL(СУММА(%), 0)";
	Иначе
		ШаблонВыражения = "ЕСТЬNULL(%, 0)";
	КонецЕсли;    
	...
	ДобавитьПоляСхемыЗапроса(СхемаЗапроса, Выражения, , НомерПакета, УчитыватьОбъединение);
...
Процедура ДобавитьПолеСхемыЗапроса(СхемаЗапроса, Выражение, Псевдоним, Группировать = Ложь, НомерПакета = 0, УчитыватьОбъединение = Ложь)
	
	Выражения = Новый Структура;
	Выражения.Вставить(Псевдоним, Выражение);
	
	ДобавитьПоляСхемыЗапроса(СхемаЗапроса, Выражения, Группировать, НомерПакета, УчитыватьОбъединение);
...
Процедура ДобавитьПоляСхемыЗапроса(СхемаЗапроса, Выражения, Группировать = Ложь, НомерПакета = 0, УчитыватьОбъединение = Ложь)
	
	ЗапросВыбора         = СхемаЗапроса.ПакетЗапросов[НомерПакета];
	...
	Для Каждого Выражение Из Выражения Цикл
		...
		Если УчитыватьОбъединение Тогда
			Для Каждого ОператорСхемыЗапроса Из ЗапросВыбора.Операторы Цикл
				ДобавитьВременныйПараметрВСхему(ВременныеПараметры, Выражение, ОператорСхемыЗапроса,
					ЗапросВыбора, Группировать, СлужебноеПолеГруппировок, ВременныйПараметр);				
			КонецЦикла;
		Иначе
			ДобавитьВременныйПараметрВСхему(ВременныеПараметры, Выражение, ОператорСхемыЗапроса,
				ЗапросВыбора, Группировать, СлужебноеПолеГруппировок, ВременныйПараметр);
...
Процедура ДобавитьВременныйПараметрВСхему(ВременныеПараметры, Выражение, ОператорСхемыЗапроса, 
	ЗапросВыбора, Группировать, СлужебноеПолеГруппировок, Знач ВременныйПараметр)
...
Функция ПолучитьСоответствиеСтраницДекларацииИВариантовНО(Контейнер, ВариантыНО, КонтекстРасчета, ИмяРедакции)
	...
	ТаблицаСоответствия.Колонки.Добавить("НомерДокумента",           ОбщегоНазначения.ОписаниеТипаСтрока(21));
