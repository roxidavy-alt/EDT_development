...
Функция ПроверитьЗаполнениеМаршрутаПодписанияПредварительногоПакета(Форма) Экспорт
	...
	ШаблонСообщения = НСтр("ru = 'Поле ""Маршрут подписания"" не заполнено для документа ""%1"". Пакет не записан.
		...
		|en = 'The ""Signing route"" field is blank for the ""%1"" document. The package is not saved.
	...
	Для Каждого РеквизитыПоДокументуПакета Из РеквизитыДокументовПакета Цикл
		...
		Если ЗначениеЗаполнено(РеквизитыДокумента.ДоговорКонтрагента) Тогда
			ТекстСообщенияДляДоговора = СтрШаблон(НСтр("ru = 'Договор: %1';
														|en = 'Contract: %1'"), РеквизитыДокумента.ДоговорКонтрагента);
		КонецЕсли;
		...
		Результат.ТекстСообщения = СтрШаблон(
			...
			|Контрагент: %3
			|%4';
			|en = 'The package contains the ""%1"" formalized document. Cannot change the key attributes.
			|
			|Return the previous attribute values and try again, or delete the document from the package.
			|
			|Company: %2
			|Counterparty: %3
			|%4'"),
...
Функция ПредставлениеДокументаПакета(ДанныеДокументаПакета)
	...
	Если ЗначениеЗаполнено(ДанныеДокументаПакета.ФорматОсновногоТитула)
		...
		ПредставлениеДокумента = СтрШаблон(НСтр("ru = '№%1 от %2';
												|en = 'No. %1 from %2'"), ДанныеДокументаПакета.НомерДокумента,
	...
	ИначеЕсли МногоОбъектовУчета Тогда
		Форма.Элементы.КнопкаПоказатьВсеДокументыУчета.Заголовок =
			СтрШаблон(НСтр("ru = 'и еще (%1)';
							|en = 'и еще (%1)'"), КоличествоОбъектовУчета - 2);
	Иначе
