	...
	Иначе
		...
		Для каждого ДанныеСпецификации Из СтруктураДанных.ДанныеСпецификаций Цикл
			...
			Если ДанныеСпецификации.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот Тогда
				Справочники.РесурсныеСпецификации.ИсключитьПромежуточныйВыпуск(ДанныеСпецификации);
				ИсключитьУзлыПодлежащиеРемонту(ДанныеСпецификации);
			КонецЕсли;
			//-- НЕ УТКА
...
//++ НЕ УТКА
Процедура ИсключитьУзлыПодлежащиеРемонту(ДанныеСпецификации)
	
	Для индекс = -ДанныеСпецификации.ВозвратныеОтходы.Количество() + 1 По 0 Цикл
		Строка = ДанныеСпецификации.ВозвратныеОтходы[-индекс];
		Если Строка.ПодлежитРемонту Тогда
			ДанныеСпецификации.ВозвратныеОтходы.Удалить(Строка);
		КонецЕсли;
	КонецЦикла;
...
Процедура ДобавитьСтрокиКРасчетуДатыЗапускаВыпуска(СтруктураРасчетаДатЗапускаВыпуска, Таблица)
	...
	Для каждого Строка Из Таблица Цикл
		...
		Если Строка.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Сборка
			ИЛИ Строка.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот
	...
	Для каждого Строка Из Таблица Цикл // план производства
		...
		Если НоваяСтрока.ДатаЗапускаВыпуска = '00010101' Тогда
			...
			Если НоваяСтрока.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Сборка
				ИЛИ НоваяСтрока.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот
	...
	Для каждого Строка Из Таблица Цикл //СтрокаТаблицыЗначений
		...
		Если Строка.СпособАвтовыбораНоменклатуры <> Перечисления.СпособыАвтовыбораНоменклатуры.УказываетсяВНСИ
			...
			Шаблон = НСтр("ru = 'При разузловании изделия по спецификации %1 в строке №%2 списка ""Материалы и работы"" не удалось выполнить автовыбор номенклатуры по причине: %3.';
							|en = 'Cannot autoselect products while exploding the product by BOM %1 in line %2 of the list ""Materials and works"". Reason: %3.'");
			ТекстОшибки = СтрШаблон(Шаблон, СпецификацияПредставление(Данные.Спецификация), Строка.НомерСтроки, Строка.СведенияАвтовыбора);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
...
Процедура УстановитьСтандартныеНастройкиРасчетаПланаПроизводства(Инициализация) Экспорт
	...
	Если Инициализация Тогда
		...
		Если НЕ ЗначениеЗаполнено(Хранилище.Получить()) Тогда
			
			ОбщиеНастройки = Новый Структура(СоставПараметров());
			
			ЗаполнитьЗначенияСвойств(ОбщиеНастройки, СтандартныеНастройкиРасчетаПланаПроизводства());
...
Процедура ДобавитьТаблицыПодбораСпецификаций(Запрос, ТекстыЗапросов)
	...
	ПараметрыЗапросаСпецификаций.УсловияОтбораВходящихДанных.Добавить("Дельта.Спецификация = ЗНАЧЕНИЕ(Справочник.РесурсныеСпецификации.ПустаяСсылка)");
