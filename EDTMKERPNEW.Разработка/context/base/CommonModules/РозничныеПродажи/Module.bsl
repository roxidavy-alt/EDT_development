...
Процедура ВыполнитьУдалениеНеиспользуемыхДанныхОПринадлежностиЗапасовТорговыхТочек() Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Удаление неиспользуемых данных о принадлежности запасов торговых точек';
	                         ...
	                         НСтр("ru = 'Начато регламентное удаление неиспользуемых данных о принадлежности запасов торговых точек.';
									|en = 'Scheduled deletion of unused inventory affiliation data of retail outlets is started.'"));
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Удаление неиспользуемых данных о принадлежности запасов торговых точек';
	                         ...
	                         НСтр("ru = 'Закончено регламентное удаление неиспользуемых данных о принадлежности запасов торговых точек.';
									|en = 'Scheduled deletion of unused inventory affiliation data of retail outlets is completed.'"));
		...
		Если ОтчетОРозничныхПродажах <> Неопределено
			...
			Исключение
				...
				ОписаниеОшибки = НСтр("ru = 'При дозаполнении в ОРВ и ОРП данными возврата в смену с измененным видом оплаты по кассовой смене %КассоваяСмена% произошла ошибка.
		                                     |Дополнительное описание:
		                                     |%ДополнительноеОписание%';
		                                     |en = 'An error occurred when completing the retail return report and retail sales report with return data for a shift with a changed payment kind for the %КассоваяСмена% POS shift.
		                                     |Additional description:
		                                     |%ДополнительноеОписание%'");
				ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%КассоваяСмена%", СостояниеКассовойСмены.КассоваяСмена);
				ОписаниеОшибки = СтрЗаменить(
	...
	Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ЧекККМКассоваяСмена) 
		...
		Если НайденныеДокументы.Количество() = 0 Тогда
			...
			ОписаниеОшибки = НСтр("ru = 'При создании отчета о розничных продажах по кассовой смене %1 в строке %2 по чеку коррекции не удалось подобрать документ реализации по кассовой смене %3.';
									|en = 'Cannot pick a sales document by the %3 POS shift in line %2 for the adjustment voucher when creating a retail sales report for the %1 POS shift.'");
			Если ТипЗнч(СтрокаТабличнойЧастиТовары) = Тип("ДокументТабличнаяЧастьСтрока.ОтчетОРозничныхВозвратах.Товары") Тогда
				...
				ОписаниеОшибки = НСтр("ru = 'При создании отчета о розничных возвратах по кассовой смене %1 в строке %2 не удалось подобрать документ реализации по кассовой смене %3. Способ определения себестоимости в строке изменен на %4.';
										|en = 'Cannot pick a sales document by the %3 POS shift in line %2 when creating a retail sales report for the %1 POS shift. The cost defining method in the line is changed to %4.'");
