...
Функция ПодготовитьТаблицыРасходовПоАренднымПлатежамНУ(ТаблицаОС, ТаблицаРеквизиты, ТаблицаЗатрат, КоличествоОбработанныхДанных, Отказ)
	...
	МассивВидовСчетов = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("АмортизацияВНА");
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПорядокОтраженияНаСчетахУчетаПоПериодуДействия КАК СчетУчетаАмортизации
	...
	|			И ПорядокУчетаОС.Период >= СчетУчетаАмортизации.НачалоДействия
	|			И ПорядокУчетаОС.Период < СчетУчетаАмортизации.ОкончаниеДействия
	|ГДЕ
	...
	Если ИспользуетсяУправлениеВНА_2_4 Тогда
		...
		|			И СчетУчетаАмортизации.МестоУчета = НЕОПРЕДЕЛЕНО
		|			И РанееНачисленнаяАмортизация.Период >= СчетУчетаАмортизации.НачалоДействия
		|			И РанееНачисленнаяАмортизация.Период < СчетУчетаАмортизации.ОкончаниеДействия
		|ГДЕ
