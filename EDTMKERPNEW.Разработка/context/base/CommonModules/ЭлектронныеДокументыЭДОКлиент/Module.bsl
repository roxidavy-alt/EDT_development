...
Процедура ВывестиПредупреждениеОДокументеВНеПоддерживаемомФормате(КонтекстДиагностики, ПараметрыОбработкиОшибок) Экспорт
	...
	ТекстЗаголовка = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Ошибка.ВидОшибки, "ЗаголовокПроблемы", 
		НСтр("ru = 'Формат документа не поддерживается.';
			|en = 'Формат документа не поддерживается.'")); // Строка
...
Процедура ОтказатьсяОтПодписания(ОбъектыДействий, ОписаниеОповещения) Экспорт
	...
	ДополнительныеПараметры.ЗаголовокФормы = НСтр("ru = 'Укажите причину отказа в подписи документа';
													|en = 'Укажите причину отказа в подписи документа'");
