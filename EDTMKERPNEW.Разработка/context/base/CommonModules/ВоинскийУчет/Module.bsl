...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение регистра Состав документов зарплата кадры.';
									|en = 'Fill in the ""Document content salary human resources"" register.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение справочника Воинские звания.';
									|en = 'Заполнение справочника Воинские звания.'");
		...
		Если НайденныеСтроки.Количество() > 0 Тогда 
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 - наименование для обмена не соответствует классификатору';
					|en = '%1 - description for exchange does not match the classifier'"), Выборка.Наименование);
...
Процедура ДобавитьПриемыУвольненияВТекущиеДела(ТекущиеДела, Раздел)
	...
	Для Каждого КлючИЗначение Из ДанныеОрганизаций Цикл 
		...
		Дело.Представление = НСтр("ru = 'Сформировать извещения о приемах и увольнениях';
									|en = 'Generate notices of hiring or termination of employment'") + ", " + Строка(Организация);
		Дело.Количество = Сотрудники.Количество();
		Дело.Подсказка = НСтр("ru = 'Организации обязаны сообщать в военкоматы сведения о гражданах, подлежащих воинскому учету, и принятию или увольнению их с работы в течение 5 дней.';
								|en = 'Companies must report to military registration and enlistment offices information about citizens subject to military registration and their hiring or dismissal from work within 5 days.'");
...
Процедура ДобавитьСотрудниковНеСостоящихНаВоинскомУчетеВТекущиеДела(ТекущиеДела, Раздел)
	...
	Для Каждого КлючИЗначение Из ДанныеОрганизаций Цикл 
		...
		Дело.Представление = НСтр("ru = 'Сформировать сообщения о не состоящих на воинском учете';
									|en = 'Generate messages about citizens not registered with military authorities'") + ", " + Строка(Организация);
		Дело.Количество = Сотрудники.Количество();
		Дело.Подсказка = НСтр("ru = 'Организации обязаны сообщать в военкоматы сведения о случаях выявления граждан, не состоящих на воинском учете, но обязанных состоять на воинском учете, в течение 3 рабочих дней.';
								|en = 'Companies must report to military registration and enlistment offices information about  cases of identification of citizens not registered with military authorities but subject to military registration within 3 days.'");
...
Процедура ДобавитьСотрудниковДостигшихПредельногоВозрастаПребыванияВЗапасеВТекущиеДела(ТекущиеДела, Раздел)
	...
	Для Каждого КлючИЗначение Из ДанныеОрганизаций Цикл 
		...
		Дело.Важное         = Истина;
		Дело.Владелец       = Раздел;
		Дело.Представление  = НСтр("ru = 'Снять с воинского учета';
									|en = 'Снять с воинского учета'") + ", " + Строка(Организация);
		Дело.Количество     = Сотрудники.Количество();
		Дело.Подсказка      =
			НСтр("ru = 'Требуется снять сотрудников с воинского учета в связи с наступлением предельного возраста пребывания в запасе.';
				|en = 'Требуется снять сотрудников с воинского учета в связи с наступлением предельного возраста пребывания в запасе.'");
