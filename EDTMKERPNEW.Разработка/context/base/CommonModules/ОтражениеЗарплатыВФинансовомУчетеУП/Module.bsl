...
Функция РаспределитьНачисленияПоЭтапамПроизводства(Параметры) Экспорт
	...
	ЗапросВыработка.Текст = 
		БазаРаспределенияТрудозатрат() + ОбщегоНазначения.РазделительПакетаЗапросов() +
	...
	БазаРаспределения             = МассивРезультатов[МассивРезультатов.ВГраница() - 2].Выгрузить();
	СтрокиКРаспределению          = МассивРезультатов[МассивРезультатов.ВГраница() - 1].Выгрузить();
	ПараметрыСпособовОтражения    = МассивРезультатов[МассивРезультатов.ВГраница()].Выбрать();
	...
	Для Каждого Строка Из СтрокиКРаспределению Цикл
		...
		Для Каждого ВидНачисления Из ВидыРаспределяемыхНачислений Цикл
			...
			РезультатРаспределения = ОбщегоНазначенияУТ.РаспределитьСуммуПропорциональноКоэффициентамРазныхЗнаков(Строка[ВидНачисления.Представление], МассивКоэффициентов);
			Если Строка.РаспределятьНУ Тогда
				РезультатРаспределенияНУ = ОбщегоНазначенияУТ.РаспределитьСуммуПропорциональноКоэффициентамРазныхЗнаков(Строка[ВидНачисления.Представление + "НУ"], МассивКоэффициентов);
			Иначе
...
Функция РаспределитьРасходыНаОплатуТрудаПоВыработке(ПараметрыЗапроса, ТекстыЗапроса) Экспорт
	...
	Для Каждого СтрокаНачисления Из РаспределяемыеСтроки Цикл
		...
		РезультатРаспределенияУпр = ОбщегоНазначенияУТ.РаспределитьСуммуПропорциональноКоэффициентамРазныхЗнаков(СтрокаНачисления.СуммаУпр, Коэффициенты);
		Если РезультатРаспределенияУпр = Неопределено Тогда
			РезультатРаспределенияУпр = ПустоеРаспределение;
		КонецЕсли;
		
		РезультатРаспределенияРегл = ОбщегоНазначенияУТ.РаспределитьСуммуПропорциональноКоэффициентамРазныхЗнаков(СтрокаНачисления.СуммаРегл, Коэффициенты);
		Если РезультатРаспределенияРегл = Неопределено Тогда
			РезультатРаспределенияРегл = ПустоеРаспределение;
		КонецЕсли;
		
		РезультатРаспределенияНУ = ОбщегоНазначенияУТ.РаспределитьСуммуПропорциональноКоэффициентамРазныхЗнаков(СтрокаНачисления.СуммаНУ, Коэффициенты);
		Если РезультатРаспределенияНУ = Неопределено Тогда
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплатыУТ") Тогда
		ЗакрытиеМесяцаСервер.УстановитьСостояниеОтключено(
			...
			НСтр("ru = 'Учет зарплаты не ведется.';
				|en = 'Учет зарплаты не ведется.'", ОбщегоНазначения.КодОсновногоЯзыка()));
		Возврат;
	КонецЕсли;
	...
	Если РазмерыВременныхТаблиц.ОрганизацииСОборотами = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Нет оборотов по регистру накопления ""Начисления удержания по сотрудникам"".';
				|en = 'Нет оборотов по регистру накопления ""Начисления удержания по сотрудникам"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Иначе
		...
		ЗакрытиеМесяцаСервер.ЗафиксироватьРезультатыОбработчикаИспользованияПоОрганизациям(
			...
			НСтр("ru = 'По организации ""%1"" за период %2 не сформирован регламентный документ ""Отражение зарплаты в финансовом учете"".';
				|en = 'По организации ""%1"" за период %2 не сформирован регламентный документ ""Отражение зарплаты в финансовом учете"".'", ОбщегоНазначения.КодОсновногоЯзыка()),
			Запрос,
...
Функция ТекстЗапросаВТВыработкаСотрудников() Экспорт
	
	ТекстЗапроса =
	"
	// Получим периоды баз распределения по партиям производства,
	// по которым было сторнирование отражения з/п в текущем периоде.
	|ВЫБРАТЬ
	|	СторнируемыеТрудозатратыНЗП.Организация         КАК Организация,
	|	СторнируемыеТрудозатратыНЗП.ПартияПроизводства  КАК ПартияПроизводства,
	|	НАЧАЛОПЕРИОДА(ВыработкаПоПартиям.Период, МЕСЯЦ) КАК ДатаНачала,
	|	КОНЕЦПЕРИОДА(ВыработкаПоПартиям.Период, МЕСЯЦ)  КАК ДатаОкончания
	|ПОМЕСТИТЬ ПериодыБазыРаспределенияПартийСоСторнированием
	|ИЗ
	|	РегистрСведений.РеестрДокументов КАК СторнирующиеДокументы
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтражениеЗарплатыВФинансовомУчете КАК СторнируемыеДокументы
	|	ПО СторнируемыеДокументы.Ссылка = СторнирующиеДокументы.СторнируемыйДокумент
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ТрудозатратыНезавершенногоПроизводства КАК СторнируемыеТрудозатратыНЗП
	|	ПО СторнируемыеТрудозатратыНЗП.Регистратор = СторнируемыеДокументы.Ссылка
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ТрудозатратыНезавершенногоПроизводства КАК ВыработкаПоПартиям
	|	ПО ВыработкаПоПартиям.Организация = СторнируемыеТрудозатратыНЗП.Организация
	|	И ВыработкаПоПартиям.ПартияПроизводства = СторнируемыеТрудозатратыНЗП.ПартияПроизводства
	|	И ВыработкаПоПартиям.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|	И НЕ ВыработкаПоПартиям.НормативнаяСтоимость = 0
	|	И ВыработкаПоПартиям.Период < &НачалоПериода
	|
	|ГДЕ
	|	СторнирующиеДокументы.Организация = &Организация
	|	И СторнирующиеДокументы.ДатаДокументаИБ МЕЖДУ &НачалоПериода И &КонецПериода
	|	И СторнирующиеДокументы.СторноИсправление
	|	И НЕ СторнирующиеДокументы.ДополнительнаяЗапись
	|	И СторнирующиеДокументы.Проведен
	|
	|СГРУППИРОВАТь ПО
	|	СторнируемыеТрудозатратыНЗП.Организация,
	|	СторнируемыеТрудозатратыНЗП.ПартияПроизводства,
	|	НАЧАЛОПЕРИОДА(ВыработкаПоПартиям.Период, МЕСЯЦ),
	|	КОНЕЦПЕРИОДА(ВыработкаПоПартиям.Период, МЕСЯЦ)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	ПартияПроизводства
	...
	|	СУММА(Трудозатраты.НормативнаяСтоимость) КАК НормативнаяСтоимость,
	|	СУММА(Трудозатраты.Длительность)         КАК Длительность
	...
	|			ИНАЧЕ Трудозатраты.Сотрудник
	|		КОНЕЦ КАК ФизическоеЛицо,
	|		Трудозатраты.НормативнаяСтоимость КАК НормативнаяСтоимость,
	|		ВЫБОР
	|			КОГДА ЕСТЬNULL(ВидыРабот.КратностьТрудоемкости, 0) > 0
	|				ТОГДА ВЫРАЗИТЬ(Трудозатраты.Количество * ВидыРабот.Трудоемкость / ВидыРабот.КратностьТрудоемкости КАК ЧИСЛО(15,3))
	|			ИНАЧЕ 0
	|		КОНЕЦ КАК Длительность
	|	ИЗ
	|		РегистрНакопления.ТрудозатратыНезавершенногоПроизводства КАК Трудозатраты
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСдельныеНачисления КАК Отбор
	|		ПО Трудозатраты.Организация = Отбор.Организация
	|		И Трудозатраты.Подразделение = Отбор.Подразделение
	|		И (&Сводно ИЛИ Трудозатраты.Сотрудник = Отбор.ФизическоеЛицо)
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыРаботСотрудников КАК ВидыРабот
	|		ПО Трудозатраты.ВидРабот = ВидыРабот.Ссылка
	|	
	|	ГДЕ
	|		Трудозатраты.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		И ТИПЗНАЧЕНИЯ(Трудозатраты.Регистратор) В(
	|			ТИП(Документ.ВыработкаСотрудников),
	|			ТИП(Документ.Сторно))
	|		И Трудозатраты.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|		И НЕ Трудозатраты.НормативнаяСтоимость = 0
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// Добавим данные прошлых периодов, отсторнированные в текущем.
	|	ВЫБРАТЬ
	|		Трудозатраты.Подразделение        КАК Подразделение,
	|		ВЫБОР
	|			КОГДА &Сводно
	...
	|		Трудозатраты.НормативнаяСтоимость КАК НормативнаяСтоимость,
	|		ВЫБОР
	|			КОГДА НЕ ВидыРабот.КратностьТрудоемкости = 0
	|				ТОГДА ВЫРАЗИТЬ(Трудозатраты.Количество * ВидыРабот.Трудоемкость / ВидыРабот.КратностьТрудоемкости КАК ЧИСЛО(15,3))
	|			ИНАЧЕ 0
	|		КОНЕЦ                             КАК Длительность
	|	ИЗ
	|		ПериодыБазыРаспределенияПартийСоСторнированием КАК ПериодыБазыРаспределения
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ТрудозатратыНезавершенногоПроизводства КАК Трудозатраты
	|		ПО Трудозатраты.Организация = ПериодыБазыРаспределения.Организация
	|		И Трудозатраты.ПартияПроизводства = ПериодыБазыРаспределения.ПартияПроизводства
	|		И Трудозатраты.Период МЕЖДУ ПериодыБазыРаспределения.ДатаНачала И ПериодыБазыРаспределения.ДатаОкончания
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСдельныеНачисления КАК Отбор
	|		ПО Трудозатраты.Организация = Отбор.Организация
	|		И Трудозатраты.Подразделение = Отбор.Подразделение
...
Процедура РаспределитьПоПодразделениямВыработки(Организация, ПериодРегистрации, ТаблицаРезервов)
	...
	|
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(ТрудозатратыНЗП.Регистратор) В(
	|		ТИП(Документ.ВыработкаСотрудников),
	|		ТИП(Документ.Сторно))
	...
	Для Каждого СтрокаТаблицы Из СтрокиКРаспределению Цикл
		...
		Для Каждого Колонка Из ЗначенияРаспределения Цикл
			РезультатыРаспределения[Колонка.Ключ] = 
				ОбщегоНазначенияУТ.РаспределитьСуммуПропорциональноКоэффициентамРазныхЗнаков(Колонка.Значение, МассивКоэффициентов);
		КонецЦикла;
...
Процедура ДобавитьТекстЗапросаВТТрудозатратыНЗП(ТекстыЗапроса)
	...
	|	И ТИПЗНАЧЕНИЯ(ТрудозатратыНЗП.Регистратор) В(
	|		ТИП(Документ.ВыработкаСотрудников),
	|		ТИП(Документ.Сторно))
	|	И ТрудозатратыНЗП.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|	И НЕ ТрудозатратыНЗП.НормативнаяСтоимость = 0
	|
	|СГРУППИРОВАТЬ ПО
	|	РасходыРаспределяемые.Организация,
	|	РасходыРаспределяемые.Подразделение,
...
Функция ТекстЗапросаБазаРаспределения()
	...
	|
	|ГДЕ
	|	НЕ ВЫБОР Трудозатраты.БазаРаспределения
	|		КОГДА ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.НормативыОплатыТруда)
	|			ТОГДА Трудозатраты.НормативнаяСтоимость
	|		КОГДА ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.ДлительностьВыполненияРабот)
	|			ТОГДА Трудозатраты.Длительность
...
Функция БазаРаспределенияТрудозатрат()
	...
	|ВЫБРАТЬ
	|	СторнируемыеТрудозатратыНЗП.Организация                       КАК Организация,
	|	НАЧАЛОПЕРИОДА(СторнируемыеДокументы.ПериодРегистрации, МЕСЯЦ) КАК МесяцОтражения,
	|	НАЧАЛОПЕРИОДА(ВыработкаПоПартиям.Период, МЕСЯЦ)               КАК ДатаНачала,
	|	КОНЕЦПЕРИОДА(ВыработкаПоПартиям.Период, МЕСЯЦ)                КАК ДатаОкончания
	|ПОМЕСТИТЬ ПериодыБазыРаспределенияПартийСоСторнированием
	|ИЗ
	...
	|	И ВыработкаПоПартиям.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|	И НЕ ВыработкаПоПартиям.НормативнаяСтоимость = 0
	|	И ВыработкаПоПартиям.Период < &ДатаНачала
	|
	|ГДЕ
	|	СторнирующиеДокументы.Организация = &Организация
	|	И СторнирующиеДокументы.ДатаДокументаИБ МЕЖДУ &ДатаНачала И &ДатаОкончания
	|	И СторнирующиеДокументы.СторноИсправление
	|	И НЕ СторнирующиеДокументы.ДополнительнаяЗапись
	|	И СторнирующиеДокументы.Проведен
	|
	|СГРУППИРОВАТь ПО
	|	СторнируемыеТрудозатратыНЗП.Организация,
	|	НАЧАЛОПЕРИОДА(СторнируемыеДокументы.ПериодРегистрации, МЕСЯЦ),
	|	НАЧАЛОПЕРИОДА(ВыработкаПоПартиям.Период, МЕСЯЦ),
	|	КОНЕЦПЕРИОДА(ВыработкаПоПартиям.Период, МЕСЯЦ)
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СотрудникиСдельная.Подразделение   КАК Подразделение,
	|	СотрудникиСдельная.СпособОтражения КАК СпособОтражения,
	|	СотрудникиСдельная.Сотрудник       КАК Сотрудник,
	|	&Организация                       КАК Организация,
	|	СотрудникиСдельная.ОблагаетсяЕНВД  КАК ОблагаетсяЕНВД,
	|	СотрудникиСдельная.ДатаНачала      КАК ДатаНачала,
	|	СотрудникиСдельная.ДатаОкончания   КАК ДатаОкончания
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// Добавим в расчет данные прошлых периодов, отсторнированные в текущем.
	|	ВЫБРАТЬ
	|		ДД.ПодразделениеПредприятия            КАК Подразделение,
	|		ДД.СпособОтраженияЗарплатыВБухучете    КАК СпособОтражения,
	|		ДД.ФизическоеЛицо                      КАК Сотрудник,
	|		ДД.ОблагаетсяЕНВД                      КАК ОблагаетсяЕНВД,
	|		ПериодыБазыРаспределения.ДатаНачала    КАК ДатаНачала,
	|		ПериодыБазыРаспределения.ДатаОкончания КАК ДатаОкончания
	|	ИЗ
	|		ПериодыБазыРаспределенияПартийСоСторнированием КАК ПериодыБазыРаспределения
	|	
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносыПоФизлицам КАК ДД
	|		ПО ДД.Ссылка.Организация = ПериодыБазыРаспределения.Организация
	|		И НАЧАЛОПЕРИОДА(ДД.Ссылка.ПериодРегистрации, МЕСЯЦ) = ПериодыБазыРаспределения.МесяцОтражения
	|	
	|	ГДЕ
	|		ДД.СпособОтраженияЗарплатыВБухучете.ОплатаСдельныхРабот
	|		И НЕ &Сводно
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	|		ПериодыБазыРаспределенияПартийСоСторнированием КАК ПериодыБазыРаспределения
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносы КАК ДД
	|		ПО ДД.Ссылка.Организация = ПериодыБазыРаспределения.Организация
	|		И НАЧАЛОПЕРИОДА(ДД.Ссылка.ПериодРегистрации, МЕСЯЦ) = ПериодыБазыРаспределения.МесяцОтражения
	|		
	|	ГДЕ
	...
	|		ПериодыБазыРаспределенияПартийСоСторнированием КАК ПериодыБазыРаспределения
	|	
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.НачислениеОценочныхОбязательствПоОтпускам.ОценочныеОбязательстваПоСотрудникам КАК ДД
	|		ПО ДД.Ссылка.Организация = ПериодыБазыРаспределения.Организация
	|		И НАЧАЛОПЕРИОДА(ДД.Ссылка.ПериодРегистрации, МЕСЯЦ) = ПериодыБазыРаспределения.МесяцОтражения
	|	
	|	ГДЕ
	...
	|		ПериодыБазыРаспределенияПартийСоСторнированием КАК ПериодыБазыРаспределения
	|	
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.НачислениеОценочныхОбязательствПоОтпускам.ОценочныеОбязательства КАК ДД
	|		ПО ДД.Ссылка.Организация = ПериодыБазыРаспределения.Организация
	|		И НАЧАЛОПЕРИОДА(ДД.Ссылка.ПериодРегистрации, МЕСЯЦ) = ПериодыБазыРаспределения.МесяцОтражения
	|	
	|	ГДЕ
	...
	|	Трудозатраты.ВидРабот КАК ВидРабот,
	|	Трудозатраты.ГруппаПродукции КАК ГруппаПродукции,
	|	Трудозатраты.СпособОтражения КАК СпособОтражения,
	|	Трудозатраты.СпособОтраженияПрочихРабот КАК СпособОтраженияПрочихРабот,
	|	Трудозатраты.Организация КАК Организация,
	|	Трудозатраты.ИдентификаторНабора КАК ИдентификаторНабора,
	|	Трудозатраты.АналитикаУчетаПродукции КАК АналитикаУчетаПродукции,
	|	Трудозатраты.ОблагаетсяЕНВД КАК ОблагаетсяЕНВД,
	|	Трудозатраты.ОбъектЭксплуатации КАК ОбъектЭксплуатации,
	|	Трудозатраты.ДоступноПереопределениеСтатьиКалькуляции КАК ДоступноПереопределениеСтатьиКалькуляции,
	...
	|		ВыработкаСотрудниковСотрудники.Сумма
	|			* ВыработкаСотрудниковВидыРабот.Сумма
	|			/ ВыработкаСотрудников.СуммаДокумента
	...
	|		И ВыработкаСотрудников.Дата МЕЖДУ ВТСотрудникиСдельная.ДатаНачала И ВТСотрудникиСдельная.ДатаОкончания
	...
	|	
	|	ГДЕ
	|		ТрудозатратыНЗП.Период МЕЖДУ ВТСотрудникиСдельная.ДатаНачала И ВТСотрудникиСдельная.ДатаОкончания
	|		И ТрудозатратыНЗП.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		И НЕ ТрудозатратыНЗП.НормативнаяСтоимость = 0
	|		И ТрудозатратыНЗП.ПартияПроизводства = ЗНАЧЕНИЕ(Справочник.ПартииПроизводства.ПустаяСсылка)
	|		И ТИПЗНАЧЕНИЯ(ТрудозатратыНЗП.Регистратор) В(
	|			ТИП(Документ.ВыработкаСотрудников),
	|			ТИП(Документ.Сторно))
	|	
	...
	|		ВТСотрудникиСдельная.ОблагаетсяЕНВД,
	|		НЕОПРЕДЕЛЕНО,
	|		ИСТИНА КАК ДоступноПереопределениеСтатьиКалькуляции
	|	ИЗ
	|		РегистрНакопления.ТрудозатратыНезавершенногоПроизводства КАК ТрудозатратыНЗП
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСотрудникиСдельная КАК ВТСотрудникиСдельная
	|		ПО ВТСотрудникиСдельная.Организация = ТрудозатратыНЗП.Организация
	|		И ВТСотрудникиСдельная.Подразделение = ТрудозатратыНЗП.Подразделение
	|		И (&Сводно ИЛИ ВТСотрудникиСдельная.Сотрудник = ТрудозатратыНЗП.Сотрудник)
	|		
	...
	|	Трудозатраты.ПартияПроизводства,
	|	Трудозатраты.ЗаказНаПроизводство,
	|	Трудозатраты.НаправлениеДеятельности,
	|	Трудозатраты.КодСтрокиПродукция,
	|	Трудозатраты.Этап,
	|	Трудозатраты.СтатьяКалькуляции,
	|	Трудозатраты.ВидРабот,
	|	Трудозатраты.ГруппаПродукции,
	|	Трудозатраты.СпособОтражения,
	|	Трудозатраты.СпособОтраженияПрочихРабот,
	|	Трудозатраты.Организация,
	|	Трудозатраты.ИдентификаторНабора,
	|	Трудозатраты.АналитикаУчетаПродукции,
	|	Трудозатраты.ОблагаетсяЕНВД,
	|	Трудозатраты.ОбъектЭксплуатации,
	|	Трудозатраты.ДоступноПереопределениеСтатьиКалькуляции
	|
	|ИНДЕКСИРОВАТЬ ПО
