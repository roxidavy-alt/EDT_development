...
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5
		...
		ИЛИ Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РемонтПродукцииИзДавальческогоСырья2_5
		ИЛИ Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыполнениеРаботСИспользованиемДавальческогоСырья2_5
...
Процедура ОбработкаЗаполнения(Объект, ДанныеЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		...
		НастройкиУчетаНалогаНаПрибыль = НастройкиНалоговУчетныхПолитик.ДействующиеПараметрыНалоговУчетныхПолитикНаДату(
			"НастройкиУчетаНалогаНаПрибыль", Объект.Организация, ДатаДокумента);
		Если НастройкиУчетаНалогаНаПрибыль <> Неопределено Тогда
			...
			ИначеЕсли НачалоГода(ДатаДокумента) = НачалоГода(ДатаОснования)
				И Не НастройкиУчетаНалогаНаПрибыль.ПодаватьУточненныеДекларацииТолькоПоНалоговомуПериоду Тогда
				
				Если НастройкиУчетаНалогаНаПрибыль.ПорядокУплатыАвансовПоНалогуНаПрибыль = Перечисления.ПорядокУплатыАвансовПоНалогуНаПрибыль.Ежемесячно 
						И НачалоКвартала(ДатаДокумента) > НачалоКвартала(ДатаОснования)
					Или НастройкиУчетаНалогаНаПрибыль.ПорядокУплатыАвансовПоНалогуНаПрибыль = Перечисления.ПорядокУплатыАвансовПоНалогуНаПрибыль.ПоФактическойПрибыли
						И НачалоМесяца(ДатаДокумента) > НачалоМесяца(ДатаОснования)
					Или НастройкиУчетаНалогаНаПрибыль.ПорядокУплатыАвансовПоНалогуНаПрибыль = Перечисления.ПорядокУплатыАвансовПоНалогуНаПрибыль.Ежеквартально
						И НачалоКвартала(ДатаДокумента) > НачалоКвартала(ДатаОснования) Тогда
...
Процедура ПередЗаписью(Объект, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности 
		И ТипыНалогообложенияНДСНеоблагаемые.Найти(Объект.НалогообложениеНДС) = Неопределено Тогда
		
		ПрослеживаемостьУП.ДобавитьСвойствоЗаписыватьВРасширенныйРегистр(Объект, Ложь);
		УчетПрослеживаемыхТоваровЛокализация.НеОтражатьОперацииСПрослеживаемымиТоварамиПоДвижениям(Объект.Движения);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	//++ НЕ УТКА
	СписокОперацийДавальца = Новый СписокЗначений;
	СписокОперацийДавальца.Добавить(Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5);
	СписокОперацийДавальца.Добавить(Перечисления.ХозяйственныеОперации.ПроизводствоПродукцииИзДавальческогоСырья2_5);
	СписокОперацийДавальца.Добавить(Перечисления.ХозяйственныеОперации.РемонтПродукцииИзДавальческогоСырья2_5);
	СписокОперацийДавальца.Добавить(Перечисления.ХозяйственныеОперации.ВыполнениеРаботСИспользованиемДавальческогоСырья2_5);
	СписокОперацийДавальца.Добавить(Перечисления.ХозяйственныеОперации.ВыполнениеРемонтныхРаботСИспользованиемДавальческогоСырья2_5);
	//-- НЕ УТКА
...
Функция ПолучитьДанныеДляПечатнойФормыУПД(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	//++ НЕ УТ
	|	ТаблицаДокумента.ОблагаетсяАкцизом КАК ОблагаетсяАкцизом,
	|	ТаблицаДокумента.СуммаАкциза КАК Акциз,
	//-- НЕ УТ
...
Функция ПолучитьДанныеДляПечатнойФормыУКД(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	//++ НЕ УТ
	|	ТаблицаДокумента.СуммаАкциза КАК АкцизПослеИзменения,
	|	ЕСТЬNULL(ТаблицаТоваровПредыдущие.СуммаАкциза, 0) КАК АкцизДоИзменения,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.СуммаАкциза >= ЕСТЬNULL(ТаблицаТоваровПредыдущие.СуммаАкциза, 0)
	|			ТОГДА ТаблицаДокумента.СуммаАкциза - ЕСТЬNULL(ТаблицаТоваровПредыдущие.СуммаАкциза, 0)
	|		ИНАЧЕ 0
	...
	|	ТаблицаТоваровПредыдущие.СуммаБезНДС + ТаблицаТоваровПредыдущие.СуммаНДС,
	//++ НЕ УТ
	|	ТаблицаТоваровПредыдущие.СуммаАкциза,
	|	0,
	|	ТаблицаТоваровПредыдущие.СуммаАкциза,
	|	0,
	//-- НЕ УТ
...
Процедура ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	//++ НЕ УТ
	|	ВЫБОР
	|		КОГДА НЕ &ИспользоватьПодакцизныеТовары
	|			ТОГДА 0
	|		КОГДА СУММА(ТаблицаДокумента.СуммаАкциза) > 0
	|			ТОГДА СУММА(ТаблицаДокумента.СуммаАкциза)
	|		ИНАЧЕ -СУММА(ТаблицаДокумента.СуммаАкциза)
	|	КОНЕЦ КАК СуммаАкциза,
	|	ВЫБОР КОГДА &ИспользоватьПодакцизныеТовары ТОГДА
	|		ТаблицаДокумента.ВидЗапасов.ОблагаетсяАкцизом
	|	ИНАЧЕ
	|		ЛОЖЬ
	...
	Если ПараметрыЗаполнения.ОбработатьНастройкиПечатиНаборов Тогда
		...
		//++ НЕ УТ
		|	МАКСИМУМ(ТаблицаТоваров.ОблагаетсяАкцизом) КАК ОблагаетсяАкцизом,
		|	СУММА(ТаблицаТоваров.СуммаАкциза) КАК СуммаАкциза,
		//-- НЕ УТ
		...
		//++ НЕ УТ
		|	Таблица.ОблагаетсяАкцизом КАК ОблагаетсяАкцизом,
		|	Таблица.СуммаАкциза КАК СуммаАкциза,
		//-- НЕ УТ
		...
		|	ТаблицаТоваров.СуммаНДС КАК СуммаНДС,
		//++ НЕ УТ
		|	ТаблицаТоваров.ОблагаетсяАкцизом КАК ОблагаетсяАкцизом,
		|	ТаблицаТоваров.СуммаАкциза КАК СуммаАкциза,
		//-- НЕ УТ
		|	ТаблицаТоваров.Характеристика КАК Характеристика,
		...
		//++ НЕ УТ
		|	ВременнаяТаблицаНаборы.ОблагаетсяАкцизом,
		|	ВременнаяТаблицаНаборы.СуммаАкциза,
		//-- НЕ УТ
		|	ВременнаяТаблицаНаборы.ХарактеристикаНабора,
...
Процедура ПолучитьИсходныеДокументы(МенеджерВременныхТаблиц)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтДанныеКорректировки КАК ТаблицаДанныхДокументов
	|		ПО ВтПредыдущиеДокументы.Основной = ТаблицаДанныхДокументов.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетДавальцу2_5 КАК ШапкаОснования
	|		ПО ШапкаОснования.Ссылка = ВтПредыдущиеДокументы.Подчиненный
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетДавальцу2_5.Продукция КАК ДанныеОснования
	|		ПО ШапкаОснования.Ссылка = ДанныеОснования.Ссылка
	|			И ТаблицаДанныхДокументов.Номенклатура = ДанныеОснования.Номенклатура
	|			И ТаблицаДанныхДокументов.Характеристика = ДанныеОснования.Характеристика
	|			И ТаблицаДанныхДокументов.Назначение = ДанныеОснования.Назначение
...
Процедура ПреобразоватьВременнуюТаблицуТоваровДляСчетаФактуры(МенеджерВременныхТаблиц) Экспорт
	...
	|	ЕСТЬNULL(ТаблицаТоваровПредыдущие.СуммаАкциза, 0)        КАК СуммаАкцизаДо,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.СуммаАкциза >= ЕСТЬNULL(ТаблицаТоваровПредыдущие.СуммаАкциза, 0)
	|			ТОГДА ТаблицаДокумента.СуммаАкциза - ЕСТЬNULL(ТаблицаТоваровПредыдущие.СуммаАкциза, 0)
	|		ИНАЧЕ 0
	|	КОНЕЦ                                                    КАК СуммаАкцизаУвеличение,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.СуммаАкциза < ЕСТЬNULL(ТаблицаТоваровПредыдущие.СуммаАкциза, 0)
	|			ТОГДА -(ТаблицаДокумента.СуммаАкциза - ЕСТЬNULL(ТаблицаТоваровПредыдущие.СуммаАкциза, 0))
	|		ИНАЧЕ 0
	|	КОНЕЦ                                                    КАК СуммаАкцизаУменьшение,
	//-- НЕ УТ
	|	ТаблицаДокумента.НомерСтроки                             КАК НомерСтроки,
	...
	|			И ТаблицаДокумента.НомерГТД = ТаблицаТоваровПредыдущие.НомерГТД
	//++ НЕ УТ
	|			И ТаблицаДокумента.ОблагаетсяАкцизом = ТаблицаТоваровПредыдущие.ОблагаетсяАкцизом
	//-- НЕ УТ
	|			И ТаблицаДокумента.СтавкаНДС = ТаблицаТоваровПредыдущие.СтавкаНДС
	|			И ТаблицаДокумента.Содержание = ТаблицаТоваровПредыдущие.Содержание
	|		ЛЕВОЕ СОЕДИНЕНИЕ СчетаФактурыОснования КАК СчетаФактуры
	...
	//++ НЕ УТ
	|	ТаблицаТоваровПредыдущие.ОблагаетсяАкцизом                               КАК ОблагаетсяАкцизом,
	|	0                                                                        КАК СуммаАкциза,
	|	ТаблицаТоваровПредыдущие.СуммаАкциза                                     КАК СуммаАкцизаДо,
	|	0                                                                        КАК СуммаАкцизаУвеличение,
	|	ТаблицаТоваровПредыдущие.СуммаАкциза                                     КАК СуммаАкцизаУменьшение,
	//-- НЕ УТ
	...
	|			И ТаблицаТоваровПредыдущие.НомерГТД = ТаблицаТоваровПослеКорректировки.НомерГТД
	//++ НЕ УТ
	|			И ТаблицаТоваровПредыдущие.ОблагаетсяАкцизом = ТаблицаТоваровПослеКорректировки.ОблагаетсяАкцизом
	//-- НЕ УТ
	|			И ТаблицаТоваровПредыдущие.СтавкаНДС = ТаблицаТоваровПослеКорректировки.СтавкаНДС
	|			И ТаблицаТоваровПредыдущие.Содержание = ТаблицаТоваровПослеКорректировки.Содержание
	|		ЛЕВОЕ СОЕДИНЕНИЕ СчетаФактурыОснования КАК СчетаФактуры
...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	ТекстыОтражения.Добавить(ТекстВыручкаОтРеализацииСобственногоКомиссионногоТовараДоходы());
	ТекстыОтражения.Добавить(ТекстВыручкаОтРеализацииСобственногоКомиссионногоТовараПрошлыеПериодыДоходы());
	ТекстыОтражения.Добавить(ТекстВыручкаОтРеализации()); // Выручка от реализации Дт 62.01 :: Кт 90.01.1
	ТекстыОтражения.Добавить(ТекстВыручкаОтРеализацииНУ()); // Выручка от реализации товара (налоговый учет) (Дт 76.К :: Кт 90.01.1)
	...
	ТекстыОтражения.Добавить(ТекстВыручкаОтРеализацииРаботУслугСписаниеНаРасходы());
	ТекстыОтражения.Добавить(ТекстВыручкаОтРеализацииРаботУслугНаДоходы());
	...
	ТекстыОтражения.Добавить(ТекстВыручкаОтРеализацииДоходы2_5()); // Выручка от реализации (Дт <62.1, 76> :: Кт 91.01)
	ТекстыОтражения.Добавить(ТекстВыручкаОтРеализацииРасходы2_5()); // Выручка от реализации (Дт 91.2 :: Кт <62.1, 76>)  
	ТекстыОтражения.Добавить(ТекстВыручкаОтРеализацииПрочаяВыручка2_5()); // Выручка от реализации (Дт <62.1, 76> :: Кт 91.01)
	
	// НДС
	ТекстыОтражения.Добавить(ТекстНДССРеализацииПрочаяВыручка2_5());// НДС с реализации (Дт 91.02 :: Кт 68.02)
	ТекстыОтражения.Добавить(ТекстНДССРеализацииПрочиеДоходы2_5());// НДС с реализации (Дт 68.02 :: Кт 91.01) 
	ТекстыОтражения.Добавить(ТекстНДССРеализацииПрочиеРасходы2_5());// НДС с реализации (Дт 91.02 :: Кт 68.02)
	ТекстыОтражения.Добавить(ТекстПереносУменьшенияНДС2_5());
	
	ТекстыОтражения.Добавить(ТекстНДССРеализации()); // НДС с реализации собственных товаров (Дт 90.03 :: Кт 68.02)
	ТекстыОтражения.Добавить(ТекстНДССРеализацииНаДоходы()); // НДС с реализации собственных товаров (Дт 90.03 :: Кт 68.02)
	...
	ТекстыОтражения.Добавить(ТекстНДССРеализацииРаботУслугНаДоходы()); // Корректировка НДС с реализации работ и услуг (Дт 90.03 :: Кт 68.02 / 19.09)
	...
	// Акцизы
	ТекстыОтражения.Добавить(ТекстКорректировкаНачисленияАкцизов()); // Корректировка начисления акцизов (Дт 90.04 :: Кт 68.03)
	ТекстыОтражения.Добавить(ТекстКорректировкаНачисленияАкцизовПрошлыхПериодовНаРасходы()); // Корректировка начисления акцизов прошлых периодов на расходы (Дт 91.02 :: Кт 68.03)
	ТекстыОтражения.Добавить(ТекстКорректировкаНачисленияАкцизовПрошлыхПериодовНаДоходы()); // Корректировка начисления акцизов прошлых периодов на доходы (Дт 68.03 :: Кт 91.01)
	ТекстыОтражения.Добавить(ТекстКорректировкаНачисленияАкцизовПрошлыхПериодовНаРасходы2_5()); // Корректировка начисления акцизов прошлых периодов на расходы (Дт 91.02 :: Кт 68.03)
	ТекстыОтражения.Добавить(ТекстКорректировкаНачисленияАкцизовПрошлыхПериодовНаДоходы2_5()); // Корректировка начисления акцизов прошлых периодов на доходы (Дт 68.03 :: Кт 91.01)
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	|	Операция.КорректироватьНалоговуюБазуПрошлогоПериода КАК КорректироватьНалоговуюБазуПрошлогоПериода,
	|	Операция.ОтразитьВПрибылиУбыткахПрошлыхЛет КАК ОтразитьВПрибылиУбыткахПрошлыхЛет,
	|	Операция.КорректировкаРеализации2_5 КАК КорректировкаРеализации2_5,
	...
	//++ НЕ УТКА
	|			ИЛИ Операция.ХозяйственнаяОперация В(
...
Функция НеКорректироватьНалоговуюБазуПрошлогоПериода(Организация, Дата) Экспорт
	...
	НеКорректироватьНалоговуюБазуПрошлогоПериода = УчетнаяПолитика.ПрименяетсяУСН(Организация, Дата);
...
Процедура ЗаполнитьОтражениеВУчетеПоУмолчанию(Форма) Экспорт
	...
	ДатаДокумента = ?(ЗначениеЗаполнено(Форма.Объект.Дата), Форма.Объект.Дата, ТекущаяДатаСеанса());
	
	Форма.Объект.КорректироватьНалоговуюБазуПрошлогоПериода = Ложь;
	НастройкиУчетаНалогаНаПрибыль = НастройкиНалоговУчетныхПолитик.ДействующиеПараметрыНалоговУчетныхПолитикНаДату(
		"НастройкиУчетаНалогаНаПрибыль", Форма.Объект.Организация, ДатаДокумента);
	Если НастройкиУчетаНалогаНаПрибыль <> Неопределено Тогда
		Если Форма.КорректировкаПрошлогоПериода Тогда
			
			Форма.Объект.КорректироватьНалоговуюБазуПрошлогоПериода = Истина;
			
		ИначеЕсли НачалоГода(ДатаДокумента) = НачалоГода(Форма.ДатаОснования)
			И Не НастройкиУчетаНалогаНаПрибыль.ПодаватьУточненныеДекларацииТолькоПоНалоговомуПериоду Тогда
			
			Если НастройкиУчетаНалогаНаПрибыль.ПорядокУплатыАвансовПоНалогуНаПрибыль = Перечисления.ПорядокУплатыАвансовПоНалогуНаПрибыль.Ежемесячно 
					И НачалоКвартала(ДатаДокумента) > НачалоКвартала(Форма.ДатаОснования)
				Или НастройкиУчетаНалогаНаПрибыль.ПорядокУплатыАвансовПоНалогуНаПрибыль = Перечисления.ПорядокУплатыАвансовПоНалогуНаПрибыль.ПоФактическойПрибыли
					И НачалоМесяца(ДатаДокумента) > НачалоМесяца(Форма.ДатаОснования)
				Или НастройкиУчетаНалогаНаПрибыль.ПорядокУплатыАвансовПоНалогуНаПрибыль = Перечисления.ПорядокУплатыАвансовПоНалогуНаПрибыль.Ежеквартально
					И НачалоКвартала(ДатаДокумента) > НачалоКвартала(Форма.ДатаОснования) Тогда
...
Функция ТекстЗапросаТаблицаОтражениеДокументовВРеглУчете(Запрос, ТекстыЗапроса, Регистры)
	...
	|ГДЕ
	|	НАЧАЛОПЕРИОДА(&Период, ГОД) <> НАЧАЛОПЕРИОДА(&ПериодПродажи, ГОД) И НЕ &КорректировкаРеализации2_5
	|";
...
Функция ТекстСторноВыручкиОтРеализацииКомиссионногоТовара()
	...
	|	""КорректировкаРеализацииЛокализация_457"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстКорректировкаВыручкиОтРеализацииКомиссионногоТовара()
	...
	|	""КорректировкаРеализацииЛокализация_458"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстПриходТоваровЗаБалансом()
	...
	|		КОГДА Строки.Знак = -1 И Строки.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПринятыеНаКомиссиюВПути)
	|		ТОГДА ""Уменьшение себестоимости реализованных комиссионных товаров""
	|		КОГДА Строки.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПринятыеНаКомиссиюВПути)
	|		ТОГДА ""Увеличение себестоимости реализованных комиссионных товаров""
	|		КОГДА Строки.Знак = -1
	...
	|	""КорректировкаРеализацииЛокализация_1729"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
Функция ТекстПродажаАгентскихУслуг()
	...
	|	""КорректировкаРеализацииЛокализация_459"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстВыручкаОтРеализацииСобственногоКомиссионногоТовараДоходы()
	
	ТекстВыручкаОтРеализацииСобственногоКомиссионногоТовараДоходы = "
	|ВЫБРАТЬ //// Выручка от реализации собственного/комиссионного товара (Дт 62.01 :: Кт 91.01)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	Расчеты.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|
	|	ЕСТЬNULL(Суммы.СуммаБезНДСРегл + Суммы.СуммаНДСРегл, Строки.СуммаСНДС) КАК Сумма,
	|	ЕСТЬNULL(Суммы.СуммаБезНДСУпр + Суммы.СуммаНДСУпр, Строки.СуммаСНДС / КурсВалютыУпрУчета.Курс) КАК СуммаУУ,
	|
	...
	|		ТОГДА 0
	|		ИНАЧЕ ЕСТЬNULL(Суммы.СуммаБезНДСРегл, Строки.СуммаСНДС - Строки.СуммаНДС) - Строки.СуммаАкциза
	|	КОНЕЦ) СуммаНУКт,
	|	(ВЫБОР КОГДА НЕ Операция.СтатьяДоходов.ПринятиеКНалоговомуУчету
	|		ТОГДА ЕСТЬNULL(Суммы.СуммаБезНДСРегл, Строки.СуммаСНДС - Строки.СуммаНДС) - Строки.СуммаАкциза
	|		ИНАЧЕ 0
	|	КОНЕЦ) КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	ВЫБОР
	...
	|	КОНЕЦ КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1730"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	
	|ГДЕ
	|	НЕ ДанныеДокументаОснования.КорректировкаРеализации2_5
	|	И НЕ ДанныеДокументаОснования.ПередачаНаКомиссию
	|	И НЕ ДанныеДокументаОснования.КорректировкаПрошлыхПериодов
	...
	Возврат ТекстВыручкаОтРеализацииСобственногоКомиссионногоТовараДоходы;
	
КонецФункции

Функция ТекстВыручкаОтРеализацииСобственногоКомиссионногоТовараПрошлыеПериодыДоходы()
	...
	|	КОНЕЦ КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1731"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	""КорректировкаРеализацииЛокализация_460"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	#Область ТекстВыручкаОтРеализацииКомиссионногоТовараНалоговыйУчет
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Увеличение выручки прошлых нал. периодов (Дт 76.К :: Кт 91.01)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	КОНЕЦПЕРИОДА(ДанныеДокументаОснования.ДатаОтгрузки, МЕСЯЦ) КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	КОНЕЦ КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1732"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстСторноВыручкиОтРеализацииСобственногоКомиссионногоТовара()
	...
	|	КОНЕЦ КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1733"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстСторноВыручкиОтРеализацииСобственногоКомиссионногоТовараПрошлыеПериоды()
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ФинансовыеРезультатыКорректировокПрошлогоГода.РезультатНУ, 0) <= 0
	|			ТОГДА ЕСТЬNULL(-СуммыДокументовВВалютахУчета.СуммаБезНДСРегл, Строки.СуммаСНДС - Строки.СуммаНДС) - Строки.СуммаАкциза 
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаНУДт,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ФинансовыеРезультатыКорректировокПрошлогоГода.РезультатНУ, 0) > 0
	...
	|	""КорректировкаРеализацииЛокализация_462"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстКорректировкаВыручкиОтРеализацииСобственногоТовараПрошлыеПериоды()
	...
	|	""КорректировкаРеализацииЛокализация_465"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|
	|	""Корректировка выручки от реализации собственного товара прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_466"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстВыручкаОтРеализацииРаботУслугСписаниеНаРасходы()
	...
	|	""КорректировкаРеализацииЛокализация_475"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""Уменьшение реализации работ и услуг на расходы"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_476"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстВыручкаОтРеализацииРаботУслугНаДоходы()
	
	ТекстыЗапроса = Новый Массив;
	
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Выручка от реализации работ и услуг (Дт 62.01 :: Кт 91.01)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	""КорректировкаРеализацииЛокализация_468"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Выручка от реализации работ (Дт 62.01 :: Кт 91.01)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	0 КАК СуммаВРКт,
	|	""Выручка от реализации работ и услуг "" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_473"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""Выручка от реализации работ и услуг "" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_474"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстВыручкаОтРеализацииРаботУслугПрошлыйНалоговыеПериоды()
	...
	|	""КорректировкаРеализацииЛокализация_477"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""Выручка от реализации работ и услуг прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_478"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""Выручка от реализации работ и услуг прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_479"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""Выручка от реализации работ и услуг прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_480"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""Выручка от реализации работ и услуг прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_481"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""Выручка от реализации работ и услуг прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_482"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|
	|	""Выручка от реализации работ и услуг прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_483"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|
	|	""Выручка от реализации работ и услуг прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_484"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|
	|	""Выручка от реализации работ и услуг прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_485"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|
	|	""Выручка от реализации работ и услуг прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_486"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|
	|	""Выручка от реализации работ и услуг прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_487"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	""КорректировкаРеализацииЛокализация_488"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстВыручкаОтРеализацииРасходы2_5()
	...
	|	ДанныеРегистра.Период КАК Период,
	|	ДанныеРегистра.Организация КАК Организация,
	|	ДанныеРегистра.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|
	...
	|	
	|	ВЫБОР
	|		КОГДА ДанныеДокументаОснования.ОтразитьВПрибылиУбыткахПрошлыхЛет
	...
	|	0 КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	|	ДанныеРегистра.СуммаРегл - ДанныеРегистра.ПостояннаяРазница КАК СуммаНУДт,
	...
	|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,
	|
	|	ДанныеРегистра.Сумма КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	|	ДанныеРегистра.СуммаРегл КАК СуммаНУКт,
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	ВЫБОР
	|		КОГДА ДанныеДокументаОснования.ОтразитьВПрибылиУбыткахПрошлыхЛет
	...
	|	КОНЕЦ КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1951"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	
	|ГДЕ
	|	ДанныеДокументаОснования.КорректировкаРеализации2_5
	|";
...
Функция ТекстВыручкаОтРеализацииДоходы2_5()
	
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Выручка от реализации (Дт <62.1, 76> :: Кт 91.1)
	|
	|	ДанныеРегистра.Регистратор КАК Ссылка,
	|	ДанныеРегистра.Период КАК Период,
	|	ДанныеРегистра.Организация КАК Организация,
	...
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|
	|	ДанныеРегистра.Сумма КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	|	ДанныеРегистра.СуммаРегл КАК СуммаНУДт,
	|	0 КАК СуммаПРДт,
	|	0 КАК СуммаВРДт,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.Доходы) КАК ВидСчетаКт,
	|	ДанныеРегистра.АналитикаДоходов КАК АналитикаУчетаКт,
	|	ДанныеРегистра.СтатьяДоходов КАК МестоУчетаКт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаКт,
	...
	|	0 КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	|	ДанныеРегистра.СуммаРегл - ДанныеРегистра.ПостояннаяРазница КАК СуммаНУКт,
	|	ДанныеРегистра.ПостояннаяРазница КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	ВЫБОР
	|		КОГДА ДанныеДокументаОснования.ОтразитьВПрибылиУбыткахПрошлыхЛет
	|		ТОГДА ""Корректировка выручки прошлых периодов""
	|		ИНАЧЕ ""Корректировка выручки""
	|	КОНЕЦ КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1952"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстВыручкаОтРеализацииПрочаяВыручка2_5()
	
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Выручка от реализации (Дт <62.1, 76> :: Кт 91.01)
	|
	|	ДанныеРегистра.Регистратор КАК Ссылка,
	|	ДанныеРегистра.Период КАК Период,
	|	ДанныеРегистра.Организация КАК Организация,
	|	ДанныеРегистра.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|
	|	ДанныеРегистра.ВыручкаБезНДСРегл + ДанныеРегистра.НДСРегл КАК Сумма,
	|	ДанныеРегистра.ВыручкаБезНДСУпр + ДанныеРегистра.НДСУпр КАК СуммаУУ,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.РасчетыСКлиентами) КАК ВидСчетаДт,
	|	Расчеты.ГруппаФинансовогоУчета КАК АналитикаУчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаДт,
	...
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|
	|	Суммы.СуммаВзаиморасчетов КАК ВалютнаяСуммаДт, //Протестировать валютные операции
	|	0 КАК КоличествоДт,
	|	ДанныеРегистра.ВыручкаБезНДСРегл КАК СуммаНУДт,
	|	0 КАК СуммаПРДт,
	|	0 КАК СуммаВРДт,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.Доходы) КАК ВидСчетаКт,
	...
	|
	|	0 КАК ВалютнаяСуммаКт,
	|	ДанныеРегистра.Количество КАК КоличествоКт,
	|	ДанныеРегистра.ВыручкаБезНДСРегл - ДанныеРегистра.ВыручкаПостояннаяРазница КАК СуммаНУКт,
	|	ДанныеРегистра.ВыручкаПостояннаяРазница КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Выручка от реализации работ и услуг"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1953"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|		ДокументыКОтражению.Ссылка = ДанныеДокументаОснования.Ссылка
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПрочаяВыручка КАК ДанныеРегистра
	|	ПО ДокументыКОтражению.Ссылка = ДанныеРегистра.Регистратор
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СуммыДокументовВВалютахУчета КАК Суммы
	|	ПО Суммы.Регистратор = ДанныеРегистра.Регистратор
	...
	|		И Расчеты.ОбъектРасчетов = ДанныеРегистра.ОбъектРасчетов
	|	
	|ГДЕ
	|	ДанныеДокументаОснования.КорректировкаРеализации2_5
	|";
...
Функция ТекстСебестоимостьРеализованногоТовараРабот()
	...
	|	ВЫБОР
	|		КОГДА НЕ ДанныеДокументаОснования.КорректировкаРеализации2_5 ИЛИ НЕ ДанныеДокументаОснования.ОтразитьВПрибылиУбыткахПрошлыхЛет
	|		ТОГДА ВыручкаИСебестоимость.СебестоимостьРегл
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР
	|		КОГДА НЕ ДанныеДокументаОснования.КорректировкаРеализации2_5 ИЛИ НЕ ДанныеДокументаОснования.ОтразитьВПрибылиУбыткахПрошлыхЛет
	|		ТОГДА ВыручкаИСебестоимость.СебестоимостьУпр
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаУУ,
	|
	...
	|	ВЫБОР
	|		КОГДА НЕ ДанныеДокументаОснования.КорректировкаРеализации2_5 ИЛИ НЕ ДанныеДокументаОснования.ОтразитьВПрибылиУбыткахПрошлыхЛет
	|		ТОГДА ВыручкаИСебестоимость.Количество
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК КоличествоДт,
	|	ВЫБОР
	|		КОГДА НЕ ДанныеДокументаОснования.КорректировкаРеализации2_5 ИЛИ ДанныеДокументаОснования.КорректироватьНалоговуюБазуПрошлогоПериода ИЛИ НЕ ДанныеДокументаОснования.КорректировкаПрошлыхПериодов
	|		ТОГДА ВыручкаИСебестоимость.СебестоимостьНУ
	|	ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаНУДт,
	|	Строки.Знак * ВЫБОР
	|		КОГДА ДанныеДокументаОснования.КорректировкаРеализации2_5 И ДанныеДокументаОснования.ОтразитьВПрибылиУбыткахПрошлыхЛет И ДанныеДокументаОснования.КорректироватьНалоговуюБазуПрошлогоПериода
	|			ТОГДА - ВыручкаИСебестоимость.СебестоимостьНУ
	|		КОГДА НЕ ДанныеДокументаОснования.КорректировкаРеализации2_5 ИЛИ ДанныеДокументаОснования.КорректироватьНалоговуюБазуПрошлогоПериода ИЛИ НЕ ДанныеДокументаОснования.КорректировкаПрошлыхПериодов
	|			ТОГДА ВыручкаИСебестоимость.СебестоимостьПР
	|		ИНАЧЕ ВыручкаИСебестоимость.СебестоимостьНУ + ВыручкаИСебестоимость.СебестоимостьПР
	|	КОНЕЦ КАК СуммаПРДт,
	|	ВЫБОР
	|		КОГДА НЕ ДанныеДокументаОснования.КорректировкаРеализации2_5 ИЛИ НЕ ДанныеДокументаОснования.ОтразитьВПрибылиУбыткахПрошлыхЛет
	|		ТОГДА ВыручкаИСебестоимость.СебестоимостьВР
	|	ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаВРДт,
	|
	|	ВЫБОР
	...
	|	КОНЕЦ КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1734"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|ГДЕ
	|	НЕ ДанныеДокументаОснования.ПередачаНаКомиссию
	|	И (НЕ ДанныеДокументаОснования.КорректировкаПрошлыхПериодов И НЕ ДанныеДокументаОснования.КорректировкаРеализации2_5
	|		ИЛИ ДанныеДокументаОснования.КорректировкаРеализации2_5
	|			И (НЕ ДанныеДокументаОснования.ОтразитьВПрибылиУбыткахПрошлыхЛет
	|				ИЛИ ДанныеДокументаОснования.КорректироватьНалоговуюБазуПрошлогоПериода
	|				ИЛИ НЕ ДанныеДокументаОснования.КорректировкаПрошлыхПериодов))
	|	И (НЕ ДанныеДокументаОснования.ВернутьМногооборотнуюТару
	|		ИЛИ ЕСТЬNULL(Строки.ТипНоменклатуры, ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
	|	И ВыручкаИСебестоимость.ТипЗапасов <> ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар)
	|";
...
Функция ТекстСебестоимостьРеализованногоТовараРаботПрошлыхПериодовРасходы()
	...
	|	Строки.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|	ВЫБОР
	|		КОГДА НЕ ДанныеДокументаОснования.КорректировкаРеализации2_5 ИЛИ ДанныеДокументаОснования.ОтразитьВПрибылиУбыткахПрошлыхЛет
	|		ТОГДА Строки.Знак * ЕСТЬNULL(Стоимости.СуммаБалансовая, Строки.СуммаБалансовая) 
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР
	|		КОГДА НЕ ДанныеДокументаОснования.КорректировкаРеализации2_5 ИЛИ ДанныеДокументаОснования.ОтразитьВПрибылиУбыткахПрошлыхЛет
	|		ТОГДА Строки.Знак * ЕСТЬNULL(Стоимости.СуммаБалансоваяУУ, Строки.СуммаБалансоваяУУ)
	|	ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаУУ,
	|
	|	НЕОПРЕДЕЛЕНО КАК ВидСчетаДт,
	...
	|	ВЫБОР
	|		КОГДА НЕ ДанныеДокументаОснования.КорректировкаРеализации2_5 ИЛИ ДанныеДокументаОснования.ОтразитьВПрибылиУбыткахПрошлыхЛет
	|		ТОГДА Строки.Знак * Строки.Количество
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК КоличествоДт,
	|	Строки.Знак * ВЫБОР
	|		КОГДА ДанныеДокументаОснования.КорректировкаРеализации2_5 И НЕ ДанныеДокументаОснования.КорректироватьНалоговуюБазуПрошлогоПериода
	|				ИЛИ НЕ ДанныеДокументаОснования.КорректировкаРеализации2_5 И ЕСТЬNULL(ФинансовыеРезультатыКорректировокПрошлогоГода.РезультатНУ, 0) <= 0
	|		ТОГДА ЕСТЬNULL(Стоимости.СуммаНУ, Строки.СуммаНУ)
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаНУДт,
	|	Строки.Знак * ВЫБОР
	|		КОГДА ДанныеДокументаОснования.КорректировкаРеализации2_5 И НЕ ДанныеДокументаОснования.ОтразитьВПрибылиУбыткахПрошлыхЛет
	|			ТОГДА - ЕСТЬNULL(Стоимости.СуммаНУ, Строки.СуммаНУ)
	|		КОГДА ДанныеДокументаОснования.КорректировкаРеализации2_5 И ДанныеДокументаОснования.КорректироватьНалоговуюБазуПрошлогоПериода
	|			ТОГДА ЕСТЬNULL(Стоимости.СуммаПР, Строки.СуммаПР) + ЕСТЬNULL(Стоимости.СуммаНУ, Строки.СуммаНУ)
	|		КОГДА ДанныеДокументаОснования.КорректировкаРеализации2_5 И НЕ ДанныеДокументаОснования.КорректироватьНалоговуюБазуПрошлогоПериода
	|			ИЛИ НЕ ДанныеДокументаОснования.КорректировкаРеализации2_5 И ЕСТЬNULL(ФинансовыеРезультатыКорректировокПрошлогоГода.РезультатНУ, 0) <= 0
	...
	|	КОНЕЦ КАК СуммаПРДт,
	|	ВЫБОР
	|		КОГДА ДанныеДокументаОснования.КорректировкаРеализации2_5 И ДанныеДокументаОснования.ОтразитьВПрибылиУбыткахПрошлыхЛет
	|		ТОГДА Строки.Знак * ЕСТЬNULL(Стоимости.СуммаВР, Строки.СуммаВР) 
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаВРДт,
	|
	|	ВЫБОР
	...
	|	КОНЕЦ КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1735"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|	НЕ ДанныеДокументаОснования.ПередачаНаКомиссию
	|	И ДанныеДокументаОснования.КорректировкаПрошлыхПериодов
	|	И (НЕ ДанныеДокументаОснования.КорректировкаРеализации2_5
	|		ИЛИ ДанныеДокументаОснования.КорректировкаРеализации2_5 
	|			И (ДанныеДокументаОснования.ОтразитьВПрибылиУбыткахПрошлыхЛет 
	|				ИЛИ НЕ ДанныеДокументаОснования.КорректироватьНалоговуюБазуПрошлогоПериода))
	|	И ВЫБОР
	|		КОГДА Строки.РазделУчета В (ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаСкладах),
...
Функция ТекстСебестоимостьРеализованногоТовараРаботПрошлыхПериодовДоходы()
	...
	|	-Строки.Знак * ВЫБОР
	|		КОГДА ЕСТЬNULL(ФинансовыеРезультатыКорректировокПрошлогоГода.РезультатНУ, 0) <= 0
	|			ИЛИ ДанныеДокументаОснования.КорректировкаРеализации2_5 И ДанныеДокументаОснования.ОтразитьВПрибылиУбыткахПрошлыхЛет
	...
	|		ИНАЧЕ ""Себестоимость реализованного товара прошлых периодов""
	|	КОНЕЦ КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1736"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстСебестоимостьРеализованногоТовараРаботПрошлыеПериодыНалоговыйУчет()
	...
	|	КОНЕЦ КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1737"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстПеренос76К()
	...
	|	""КорректировкаРеализацииЛокализация_493"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""Перенос сальдо 76.К"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_494"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""Перенос сальдо 76.К"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_495"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""Перенос сальдо 76.К"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_496"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстПеренос41К()
	...
	|	""КорректировкаРеализацииЛокализация_497"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстКорректировкаФинансовогоРезультатаПрошлыеПериоды()
	...
	|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка) КАК НаправлениеДеятельностиДт,
	|
	|	&СчетПрибыльУбытокОтПродаж КАК СчетДт,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт1,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|
	...
	|	""КорректировкаРеализацииЛокализация_498"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|	0 КАК СуммаВРКт,
	|	""Корректировка финансового результата прошлого нал. периода"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_499"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстНДССРеализации()
	...
	|
	|	ВЫБОР
	|		КОГДА Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|			И НЕ Операция.КорректировкаРеализации2_5
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НДСПриОтгрузкеБезПереходаПраваСобственности)
	|		ИНАЧЕ
	|			НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ВидСчетаКт,
	|	НЕОПРЕДЕЛЕНО КАК АналитикаУчетаКт,
	...
	|
	|	ВЫБОР
	|		КОГДА (КорректировкиПоСогласованиюСторон.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон)
	|				ИЛИ Операция.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон))
	|				И СтрокиДт.СуммаНДСРегл < 0
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСПоУменьшениюСтоимостиРеализации)
	|		КОГДА Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|			И Операция.КорректировкаРеализации2_5
	...
	|				ИЛИ Операция.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон))
	|				И СтрокиДт.СуммаНДСРегл < 0
	|			ТОГДА СтрокиДт.Ссылка
	|		КОГДА Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|			ТОГДА СтрокиДт.ЗаказКлиента
	|		ИНАЧЕ
	|			НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК СубконтоКт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,
	...
	|	0 КАК СуммаВРКт,
	|	ВЫБОР
	|		КОГДА Операция.КорректировкаРеализации2_5
	|			ТОГДА ""Корректировка НДС с реализации""
	|		КОГДА СтрокиДт.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
	|			И СтрокиДт.СуммаНДСРегл < 0
	|			ТОГДА ""Уменьшение НДС с реализации собственных товаров""
	|		КОГДА СтрокиДт.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
	|			И СтрокиДт.Количество = 0
	|			ТОГДА ""Корректировка НДС с реализации собственных товаров""
	|		КОГДА СтрокиДт.ТипЗапасов В (
	|				ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Услуга),
	|				ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Работа))
	|			ТОГДА ""Корректировка НДС с реализации работ и услуг""
	|		ИНАЧЕ ""НДС с реализации собственных товаров""
	|	КОНЕЦ КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_2020"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|		ВТДанныеДокументаОснования КАК ДанныеДокументаОснования
	|	ПО
	|		СтрокиДт.Ссылка = ДанныеДокументаОснования.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	...
	|		И СтрокиДт.НастройкаХозОперации = Строки.НастройкаХозяйственнойОперации
	|		И СтрокиДт.ИдентификаторФинЗаписи = Строки.ИдентификаторФинЗаписи
	|
	|ГДЕ
	|	НЕ ДанныеДокументаОснования.ПередачаНаКомиссию
	|	И (НЕ ДанныеДокументаОснования.КорректировкаПрошлыхПериодов ИЛИ ДанныеДокументаОснования.КорректировкаРеализации2_5)
	|";
...
Функция ТекстНДССРеализацииНаДоходы()
	
	ТекстНДССРеализации = "
	|ВЫБРАТЬ  //// НДС с реализации собственных товаров (Дт 91.02 :: Кт 68.02)  
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|
	|	Операция.СтатьяДоходов КАК АналитикаУчетаДт,
	|
	|	Операция.ПодразделениеДоходы КАК МестоУчетаДт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаДт,
	|	Операция.ПодразделениеДоходы КАК ПодразделениеДт,
	|	Операция.НаправлениеДеятельности КАК НаправлениеДеятельностиДт,
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетДт,
	...
	|	""КорректировкаРеализацииЛокализация_500"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстНДССРеализацииПрочаяВыручка2_5()
	
	ТекстНДССРеализации = "
	|ВЫБРАТЬ  //// НДС с реализации (Дт 91.02 :: Кт 68.02/19.09)
	|
	|	ДанныеРегистра.Регистратор КАК Ссылка,
	|	ДанныеРегистра.Период КАК Период,
	|	ДанныеРегистра.Организация КАК Организация,
	|	ДанныеРегистра.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|
	|	ДвиженияПоНДС.НДС КАК Сумма,
	|	ДвиженияПоНДС.НДСУпр КАК СуммаУУ,
	|
	|	НЕОПРЕДЕЛЕНО КАК ВидСчетаДт,
	|
	|	НЕОПРЕДЕЛЕНО КАК АналитикаУчетаДт,
	...
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПрочиеРасходы) КАК СчетДт,
	|	ДанныеРегистра.СтатьяДоходов КАК СубконтоДт1,
	|	ДанныеРегистра.АналитикаДоходов КАК СубконтоДт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|
	|	0 КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	...
	|			ИЛИ ЕСТЬNULL(КорректировкиПоСогласованиюСторон.ВидКорректировки, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон))
	|				И ДвиженияПоНДС.НДС < 0 ТОГДА
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Корректировка НДС с реализации"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1954"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	ПО ДвиженияПоНДС.Регистратор = ДанныеРегистра.Регистратор
	|		И ДвиженияПоНДС.ИдентификаторФинЗаписи = ДанныеРегистра.ИдентификаторФинЗаписи
	|ГДЕ
	|	Операция.КорректировкаРеализации2_5
	|";
...
Функция ТекстНДССРеализацииПрочиеДоходы2_5()
	
	ТекстНДССРеализации = "
	|ВЫБРАТЬ  //// НДС с реализации прошлых периодов увеличение (Дт 68.02 :: Кт 91.01)
	|
	|	ДанныеРегистра.Регистратор КАК Ссылка,
	|	ДанныеРегистра.Период КАК Период,
	|	ДанныеРегистра.Организация КАК Организация,
	|	ДанныеРегистра.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|
	|	ВЫБОР КОГДА ДанныеРегистра.СуммаРегл = 0
	|		ТОГДА 0
	|		ИНАЧЕ -ДвиженияПоНДС.НДС
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР КОГДА ДанныеРегистра.СуммаУпр = 0
	|		ТОГДА 0
	|		ИНАЧЕ -ДвиженияПоНДС.НДСУпр
	|	КОНЕЦ КАК СуммаУУ,
	|
	|	НЕОПРЕДЕЛЕНО КАК ВидСчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК АналитикаУчетаДт,
	...
	|
	|	""Корректировка НДС с реализации"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1981"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ДвиженияПоНДС КАК ДвиженияПоНДС
	|	ПО ДвиженияПоНДС.Регистратор = ДанныеРегистра.Регистратор
	|		И ДвиженияПоНДС.НастройкаХозяйственнойОперации = ДанныеРегистра.НастройкаХозяйственнойОперации 
	|		И ДвиженияПоНДС.ИдентификаторФинЗаписи = ДанныеРегистра.ИдентификаторФинЗаписи 
	|	
	|ГДЕ
	|	Операция.КорректировкаРеализации2_5
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ  //// НДС по переходу права собственности прошлых периодов увеличение (Дт 76.ОТ :: Кт 91.01)
	|
	|	ДанныеРегистра.Регистратор КАК Ссылка,
	|	ДанныеРегистра.Период КАК Период,
	|	ДанныеРегистра.Организация КАК Организация,
	...
	|	ВЫБОР КОГДА ДанныеРегистра.СуммаРегл = 0
	|		ТОГДА 0
	|		ИНАЧЕ ДвиженияПо76ОТ.СуммаРегл
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР КОГДА ДанныеРегистра.СуммаУпр = 0
	|		ТОГДА 0
	|		ИНАЧЕ ДвиженияПо76ОТ.СуммаУпр
	|	КОНЕЦ КАК СуммаУУ,
	|
	|	НЕОПРЕДЕЛЕНО КАК ВидСчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК АналитикаУчетаДт,
	...
	|	0 КАК КоличествоКт,
	|	ВЫБОР КОГДА ДанныеРегистра.СуммаРегл = 0
	|		ТОГДА ДвиженияПо76ОТ.СуммаРегл
	|	КОГДА ДанныеРегистра.ПостояннаяРазница <> 0
	|		ТОГДА 0
	|		ИНАЧЕ ДвиженияПо76ОТ.СуммаРегл
	|	КОНЕЦ КАК СуммаНУКт,
	...
	|
	|	""Корректировка НДС с реализации"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1983"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	ПО ДокументыКОтражению.Ссылка = ДанныеРегистра.Регистратор
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ДвиженияПоПрочимАктивамПассивам КАК ДвиженияПо76ОТ
	|	ПО ДвиженияПо76ОТ.Регистратор = ДанныеРегистра.Регистратор
	|		И ДвиженияПо76ОТ.ИдентификаторФинЗаписи = ДанныеРегистра.ИдентификаторФинЗаписи
	|		И ДвиженияПо76ОТ.НастройкаХозяйственнойОперации = ДанныеРегистра.НастройкаХозяйственнойОперации 
	|		И ДвиженияПо76ОТ.Статья = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.НДСПоОтгрузкамБезПереходаПраваСобственности)
	|	
	|ГДЕ
	|	Операция.КорректировкаРеализации2_5
	|";
...
Функция ТекстНДССРеализацииПрочиеРасходы2_5()
	
	ТекстНДССРеализации = "
	|ВЫБРАТЬ  //// НДС с реализации прошлых периодов уменьшение (Дт 91.02 :: Кт 68.02/19.09)
	|
	|	ДанныеРегистра.Регистратор КАК Ссылка,
	|	ДанныеРегистра.Период КАК Период,
	|	ДанныеРегистра.Организация КАК Организация,
	...
	|
	|	ВЫБОР
	|		КОГДА (Операция.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон)
	|			ИЛИ ЕСТЬNULL(КорректировкиПоСогласованиюСторон.ВидКорректировки, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон))
	|				И ДвиженияПоНДС.НДС < 0
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСПоУменьшениюСтоимостиРеализации)
	|		ИНАЧЕ ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДС)
	|	КОНЕЦ КАК СчетКт,
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""Корректировка НДС с реализации"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1982"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ  //// НДС по переходу права собственности прошлых периодов уменьшение (Дт 91.02 :: Кт 76.ОТ/19.09)
	|
	|	ДанныеРегистра.Регистратор КАК Ссылка,
	|	ДанныеРегистра.Период КАК Период,
	|	ДанныеРегистра.Организация КАК Организация,
	...
	|		КОГДА (Операция.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон)
	|			ИЛИ ЕСТЬNULL(КорректировкиПоСогласованиюСторон.ВидКорректировки, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон))
	|				И ДвиженияПо76ОТ.СуммаРегл < 0
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСПоУменьшениюСтоимостиРеализации)
	|		ИНАЧЕ ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСНачисленныйПоОтгрузке)
	|	КОНЕЦ КАК СчетКт,
	|	
	|	Операция.Контрагент КАК СубконтоКт1,
	|	ВЫБОР
	...
	|	0 КАК СуммаВРКт,
	|	""Корректировка НДС с реализации"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1980"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстНДССРеализацииПрошлыеПериоды()
	...
	|	""КорректировкаРеализацииЛокализация_501"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстНДССРеализацииРаботУслугНаДоходы()
	
	ТекстНДСКорректировка = "
	|ВЫБРАТЬ //// Корректировка НДС с реализации работ и услуг (Дт 26, 91.02 :: Кт 68.02 / 19.09)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	""КорректировкаРеализацииЛокализация_502"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстНДССРеализацииРаботУслугПрошлыхНалоговыхПериодов()
	...
	|	""КорректировкаРеализацииЛокализация_506"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|
	|	""Корректировка НДС с реализации работ и услуг прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_507"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|
	|	""Корректировка НДС с реализации работ и услуг прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_508"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|
	|	""Корректировка НДС с реализации работ и услуг прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_509"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|
	|	""Корректировка НДС с реализации работ и услуг прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_510"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|
	|	""Корректировка НДС с реализации работ и услуг прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_511"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|
	|	""Корректировка НДС с реализации работ и услуг прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_512"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область НДСПредъявлен // (Дт 90.03 :: Кт 19.03)
	...
	|////////////////////////////////////////////////////////////////////////////////////////////////////
	|// Входящий НДС (сторно) по рег. НДСПредъявленный (Дт 90.03 :: Кт 19.03) @НДСПредъявленный
	|ВЫБРАТЬ
	|	СтрокиДт.Ссылка КАК Ссылка,
	|	СтрокиДт.Период КАК Период,
	|	СтрокиДт.Организация КАК Организация,
	...
	|	СтрокиКт.ВидДеятельностиНДС КАК АналитикаУчетаКт,
	|	СтрокиКт.ВидЦенности КАК МестоУчетаКт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаКт,
	|	ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) КАК ПодразделениеКт,
	|	ЕСТЬNULL(ВводСобственныхСредств.НаправлениеДеятельности, СтрокиКт.НаправлениеДеятельности) КАК НаправлениеДеятельностиКт,
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетКт,
	|	СтрокиКт.Поставщик КАК СубконтоКт1,
	|	СтрокиКт.СчетФактура КАК СубконтоКт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,
	|
	|	0 КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	...
	|	""КорректировкаРеализацииЛокализация_513"" КАК НомерФрагмента
	|
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	...
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.НДСПредъявленный КАК СтрокиКт
	|	ПО
	|		СтрокиДт.Ссылка = СтрокиКт.Регистратор
	|		И СтрокиДт.Период = СтрокиКт.Период
	|		И СтрокиДт.Организация = СтрокиКт.Организация
	|		И СтрокиДт.НастройкаХозОперацииВыручки = СтрокиКт.НастройкаХозяйственнойОперации
	|		И СтрокиДт.ИдентификаторФинЗаписиВыручки = СтрокиКт.ИдентификаторФинЗаписи
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.КорректировкаРеализации КАК Операция
	|	ПО
	...
	|		И СтрокиКт.СчетФактура = ВводСобственныхСредств.СчетФактура
	|		И СтрокиКт.НаправлениеДеятельности = ВводСобственныхСредств.ИсходноеНаправлениеДеятельности
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|		РаздельныйУчет_НастройкиНалогообложенияПрибыли КАК ТаблицаНалогообложенияПрибыли
	|	ПО 
	...
	|ГДЕ
	|	НЕ ДанныеДокументаОснования.КорректировкаРеализации2_5
	|	И Операция.ВидКорректировки <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратНедопоставленногоТовара)
	|	И НЕ ДанныеДокументаОснования.ПередачаНаКомиссию
	|	И НЕ ДанныеДокументаОснования.КорректировкаПрошлыхПериодов
	|";
	...
	|
	|	ВЫБОР
	|		КОГДА НЕ (ЕСТЬNULL(КорректировкиПоСогласованиюСторон.ВидКорректировки, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон)
	|			ИЛИ Операция.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон))
	|			И Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НДСПриОтгрузкеБезПереходаПраваСобственности)
	|		ИНАЧЕ
	|			НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ВидСчетаКт,
	...
	|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка) КАК НаправлениеДеятельностиКт,
	|
	|	ВЫБОР КОГДА (ЕСТЬNULL(КорректировкиПоСогласованиюСторон.ВидКорректировки, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон))
	...
	|
	|	ВЫБОР 
	|		КОГДА (ЕСТЬNULL(КорректировкиПоСогласованиюСторон.ВидКорректировки, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон))
	|		ИЛИ Операция.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон) ТОГДА
	|		ДанныеДокументаОснования.Контрагент
	|		КОГДА Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|			ТОГДА ДанныеДокументаОснования.Контрагент
	...
	|	""КорректировкаРеализацииЛокализация_514"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	Строки.ОбъектРасчетов.УникальныйИдентификатор,
	|	Строки.АналитикаУчетаНоменклатуры.Номенклатура,
	|	Строки.ИдентификаторСтроки
	|";
	ТекстыЗапроса.Добавить(ТекстСторноНДСНаРасходы);
	#КонецОбласти
...
Функция ТекстСторноНДСПрошлыеПериоды()
	...
	|	""КорректировкаРеализацииЛокализация_515"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстНДСКорректировкаВыручкиПрошлыхНалоговыхПериодов()
	...
	|	""КорректировкаРеализацииЛокализация_517"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	
	|	ВЫБОР
	|		КОГДА НЕ (Операция.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон)
	|			ИЛИ ЕСТЬNULL(КорректировкиПоСогласованиюСторон.ВидКорректировки, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон))
	|			И Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НДСПриОтгрузкеБезПереходаПраваСобственности)
	|		ИНАЧЕ
	...
	|
	|	ВЫБОР КОГДА (Операция.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон)
	|			ИЛИ ЕСТЬNULL(КорректировкиПоСогласованиюСторон.ВидКорректировки, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон)) ТОГДА
	|		ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСПоУменьшениюСтоимостиРеализации)
	|	КОГДА Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|		ТОГДА НЕОПРЕДЕЛЕНО
	|	ИНАЧЕ
	...
	|	0 КАК СуммаВРКт,
	|	""Корректировка НДС с реализации собственных товаров прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_518"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстПереносУменьшенияНДС()
	...
	|	""КорректировкаРеализацииЛокализация_519"" КАК НомерФрагмента
	|	
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	...
	|	""КорректировкаРеализацииЛокализация_520"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	""КорректировкаРеализацииЛокализация_521"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""Сторно НДС с реализации работ и услуг"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_522"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""Сторно НДС с реализации работ и услуг"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_523"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""Сторно НДС с реализации работ и услуг"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_524"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстПереносУменьшенияНДС2_5()
	...
	|	0 КАК СуммаВРКт,
	|	""Перенос НДС"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1926"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	ПО
	|		ДвиженияПоНДС.Регистратор = Операция.Ссылка
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ВТКорректировкиПоСогласованиюСторон КАК КорректировкиПоСогласованиюСторон
	|	ПО
	...
	|ГДЕ 
	|	ДанныеДокументаОснования.КорректировкаРеализации2_5
	|	И(Операция.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон)
	|		ИЛИ ЕСТЬNULL(КорректировкиПоСогласованиюСторон.ВидКорректировки, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон))
	|			И  ДвиженияПоНДС.НДС < 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ  //// Перенос НДС (Дт 76.ОТ :: Кт 19.09)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	|
	|	- ДвиженияПо76ОТ.СуммаРегл КАК Сумма,
	|	- ДвиженияПо76ОТ.СуммаУпр КАК СуммаУУ,
	|
	|	НЕОПРЕДЕЛЕНО КАК ВидСчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК ГруппаФинансовогоУчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаДт,
	...
	|	
	|	Операция.Контрагент КАК СубконтоДт1,
	|	ЕСТЬNULL(ВыручкаИСебестоимость.ЗаказКлиента, НЕОПРЕДЕЛЕНО) КАК СубконтоДт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|
	|	0 КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	...
	|	0 КАК СуммаВРКт,
	|	""Перенос НДС"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1927"" КАК НомерФрагмента
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ДвиженияПоПрочимАктивамПассивам КАК ДвиженияПо76ОТ
	|	ПО ДвиженияПо76ОТ.Регистратор = Операция.Ссылка
	|		И ДвиженияПо76ОТ.Статья = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.НДСПоОтгрузкамБезПереходаПраваСобственности)
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТВыручкаИСебестоимостьПродаж КАК ВыручкаИСебестоимость
	|	ПО ДокументыКОтражению.Ссылка = ВыручкаИСебестоимость.Ссылка
	|		И ДвиженияПо76ОТ.ИдентификаторФинЗаписи = ВыручкаИСебестоимость.ИдентификаторФинЗаписи
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ВТКорректировкиПоСогласованиюСторон КАК КорректировкиПоСогласованиюСторон
	|	ПО
...
Функция ТекстВключениеИсключениеНДСВСтоимость()
	...
	|	""КорректировкаРеализацииЛокализация_525"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстЗачетАванса()
	...
	|	""КорректировкаРеализацииЛокализация_526"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ 
...
Функция ТекстВыделенияАванса()
	...
	|	""КорректировкаРеализацииЛокализация_527"" КАК НомерФрагмента
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ 
...
Функция ТекстПередачаТары()
	...
	|	""КорректировкаРеализацииЛокализация_528"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""Передача тары"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_529"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстСторноПередачиТары()
	...
	|	""КорректировкаРеализацииЛокализация_530"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""Сторно передачи тары"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_531"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстКорректировкаПередачиТары()
	...
	|	""КорректировкаРеализацииЛокализация_532"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	0 КАК СуммаВРКт,
	|	""Корректировка передачи тары"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_533"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстСписаниеТоваровНаРасходы()
	...
	ТекстЗапроса = ПроизводствоБезЗаказаЛокализация.ТекстСписаниеТоваровНаРасходы(, , Истина);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборПоУмолчанию", "Строки.ТипНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)");
	Возврат ТекстЗапроса;
...
Функция ТекстОтклонениеВСтоимостиДоходы()
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Содержание", """Отклонение в стоимости возвращенной тары на доходы""");
	УсловиеОтбора = "ГДЕ
	|	&ОтборПоУмолчанию";
	НовоеУсловиеОтбора = "
	|ГДЕ
...
Функция ТекстОтклонениеВСтоимостиРасходы()
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Содержание", """Отклонение в стоимости возвращенной тары в расходы""");
	УсловиеОтбора = "ГДЕ
	|	&ОтборПоУмолчанию";
	НовоеУсловиеОтбора = "
	|ГДЕ
...
#Область ТекущийГод
	...
	|	0 КАК СуммаВРКт,
	|	""Выручка от реализации комиссионного товара"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_536"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстВыручкаОтРеализацииДавальцы()
	...
	|	""КорректировкаРеализацииЛокализация_537"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстВыручкаОтРеализацииДавальцыПрошлыеПериоды()
	...
	|	""КорректировкаРеализацииЛокализация_538"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстВыручкаОтРеализацииДавальцыПрошлыеПериодыНалоговыйУчет()
	...
	|	""КорректировкаРеализацииЛокализация_539"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстВыручкаОтРеализацииДавальцыРасходы()
	...
	|		ИНАЧЕ ""Корректировка выручки в расходы""
	|	КОНЕЦ КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1738"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстВыручкаОтРеализацииДавальцыРасходыПрошлыеПериоды()
	...
	|	""КорректировкаРеализацииЛокализация_540"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстВыручкаОтРеализацииДавальцыАктивыПассивы()
	...
	|	КОНЕЦ КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1739"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстВыручкаОтРеализацииДавальцыАктивыПассивыПрошлыеПериоды()
	...
	|	""КорректировкаРеализацииЛокализация_541"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстВыручкаОтРеализацииДавальцыДоходы()
	...
	|			И ЕСТЬNULL(ФинРезКорПрошлогоГода.РезультатНУ, 0) <= 0
	|		ТОГДА 0
	|		ИНАЧЕ ДанныеРегистра.ПостояннаяРазница
	|	КОНЕЦ КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	ВЫБОР
	|		КОГДА ДанныеДокументаОснования.КорректировкаПрошлыхПериодов
	...
	|	КОНЕЦ КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1740"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстВыручкаОтРеализацииДавальцыДоходыПрошлыеПериодыНалоговыйУчет()
	...
	|	""КорректировкаРеализацииЛокализация_542"" КАК НомерФрагмента
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Функция ТекстКорректировкаНачисленияАкцизов()
	
	ТекстЗапроса = "
	|////////////////////////////////////////////////////////////////////////////////////////////////////
	|// Корректировка начисления акцизов (Дт 90.04 :: Кт 68.03)
	|ВЫБРАТЬ
	...
	|
	|	СтрокиДт.СуммаАкцизаРегл КАК Сумма,
	|	СтрокиДт.СуммаАкцизаУпр КАК СуммаУУ,
	|
	|	// Дт ///////////////////////////////////////////////////////////////////////////////////////////
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.АкцизыПриПродаже) КАК ВидСчетаДт,
	|	ВЫБОР
	...
	|		ИНАЧЕ ЕСТЬNULL(СтрокиКт.Номенклатура.ГруппаФинансовогоУчета, ЗНАЧЕНИЕ(Справочник.ГруппыФинансовогоУчетаНоменклатуры.ПустаяСсылка))
	|	КОНЕЦ КАК АналитикаУчетаДт,
	|	ВЫБОР
	|		КОГДА СтрокиДт.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияЧерезКомиссионера)
	|			ТОГДА ЕСТЬNULL(СтрокиДт.АналитикаУчетаНоменклатуры.МестоХранения, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка))
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	|	КОНЕЦ КАК МестоУчетаДт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаДт,
	|	СтрокиДт.Подразделение КАК ПодразделениеДт,
	|	СтрокиДт.НаправлениеДеятельностиКонтрагента КАК НаправлениеДеятельностиДт,
	|
	|	НЕОПРЕДЕЛЕНО КАК СчетДт,
	|	СтрокиКт.Номенклатура КАК СубконтоДт1,
	|	ЕСТЬNULL(ТаблицаНалогообложенияПрибыли.ВариантНалогообложенияПрибыли, НЕОПРЕДЕЛЕНО) КАК СубконтоДт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|
	|	0 КАК ВалютнаяСуммаДт,
	...
	|	ЗНАЧЕНИЕ(Перечисление.ВидыПлатежейВГосБюджет.Налог) КАК СубконтоКт1,
	|	ЕСТЬNULL(РегистрацииПоПодразделению.Регистрация, ЕСТЬNULL(РегистрацииПоОрганизации.Регистрация, СтрокиКт.Организация.РегистрацияВНалоговомОргане)) КАК СубконтоКт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,
	|
	|	0 КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Корректировка начисления акцизов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1937"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.ВыручкаИСебестоимостьПродаж КАК СтрокиДт
	|	ПО
	|		ДокументыКОтражению.Ссылка = СтрокиДт.Регистратор
	|		И (СтрокиДт.СуммаАкцизаРегл <> 0 ИЛИ СтрокиДт.СуммаАкцизаУпр <> 0)
	|		И СтрокиДт.Активность
	|		И НЕ СтрокиДт.Сторно
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.ДвиженияПоАкцизам КАК СтрокиКт
	|	ПО
	...
	|		ВТДанныеДокументаОснования КАК ДанныеДокументаОснования
	|	ПО
	|		СтрокиДт.Регистратор = ДанныеДокументаОснования.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрацииВНалоговомОргане КАК РегистрацииПоПодразделению
	|	ПО
	|		ДокументыКОтражению.Ссылка = РегистрацииПоПодразделению.Ссылка
	|		И НАЧАЛОПЕРИОДА(СтрокиКт.Период, ДЕНЬ) = РегистрацииПоПодразделению.Период
	|		И СтрокиКт.Организация = РегистрацииПоПодразделению.Организация
	|		И СтрокиКт.ПодразделениеУплатыАкциза = РегистрацииПоПодразделению.Подразделение
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрацииВНалоговомОргане КАК РегистрацииПоОрганизации
	|	ПО
	|		ДокументыКОтражению.Ссылка = РегистрацииПоОрганизации.Ссылка
	|		И НАЧАЛОПЕРИОДА(СтрокиКт.Период, ДЕНЬ) = РегистрацииПоОрганизации.Период
	|		И СтрокиКт.Организация = РегистрацииПоОрганизации.Организация
	|		И РегистрацииПоОрганизации.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|		РаздельныйУчет_НастройкиНалогообложенияПрибыли КАК ТаблицаНалогообложенияПрибыли
	|	ПО 
	|		ТаблицаНалогообложенияПрибыли.Период = СтрокиДт.Период
	|		И ТаблицаНалогообложенияПрибыли.Организация = СтрокиДт.АналитикаУчетаПоПартнерам.Организация
	|		И ТаблицаНалогообложенияПрибыли.Подразделение = СтрокиДт.Подразделение
	|		И ТаблицаНалогообложенияПрибыли.ОбъектУчета = СтрокиДт.АналитикаУчетаНоменклатуры.Номенклатура
	|		И ТаблицаНалогообложенияПрибыли.НаправлениеДеятельности = СтрокиДт.НаправлениеДеятельностиНоменклатуры
	|
	|ГДЕ
	|	НЕ ДанныеДокументаОснования.КорректировкаПрошлыхПериодов
	|	ИЛИ ДанныеДокументаОснования.КорректировкаРеализации2_5
...
Функция ТекстКорректировкаНачисленияАкцизовПрошлыхПериодовНаРасходы()
	
	ТекстЗапроса = "
	|////////////////////////////////////////////////////////////////////////////////////////////////////
	|// Корректировка начисления акцизов прошлых периодов на расходы (Дт 91.02 :: Кт 68.03)
	|ВЫБРАТЬ
	|	СтрокиКт.Регистратор КАК Ссылка,
	|	СтрокиКт.Период КАК Период,
	|	СтрокиКт.Организация КАК Организация,
	...
	|	0 КАК КоличествоДт,
	|
	|	СтрокиКт.СуммаАкцизаРегл КАК СуммаНУДт,
	|	0 КАК СуммаПРДт,
	|	0 КАК СуммаВРДт,
	|
	|	// Кт ///////////////////////////////////////////////////////////////////////////////////////////
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Корректировка начисления акцизов прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1938"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
Функция ТекстКорректировкаНачисленияАкцизовПрошлыхПериодовНаДоходы()
	
	ТекстЗапроса = "
	|////////////////////////////////////////////////////////////////////////////////////////////////////
	|// Корректировка начисления акцизов прошлых периодов на доходы (Дт 68.03 :: Кт 91.01)
	|ВЫБРАТЬ
	...
	|	СтрокиДт.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|
	|	-СтрокиКт.СуммаАкцизаРегл КАК Сумма,
	|	-СтрокиКт.СуммаАкцизаУпр КАК СуммаУУ,
	|
	|	// Дт ///////////////////////////////////////////////////////////////////////////////////////////
	|
	|	НЕОПРЕДЕЛЕНО КАК ВидСчетаДт,
	...
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Акцизы) КАК СчетДт,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыПлатежейВГосБюджет.Налог) КАК СубконтоДт1,
	|	ЕСТЬNULL(РегистрацииПоПодразделению.Регистрация, ЕСТЬNULL(РегистрацииПоОрганизации.Регистрация, СтрокиДт.Организация.РегистрацияВНалоговомОргане)) КАК СубконтоДт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|
	|	0 КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	...
	|	СтрокиКт.НаправлениеДеятельностиКонтрагента КАК НаправлениеДеятельностиКт,
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПрочиеДоходы) КАК СчетКт,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПрибыльУбытокПрошлыхЛет) КАК СубконтоКт1,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоКт2,
	...
	|	0 КАК КоличествоКт,
	|
	|	-СтрокиДт.СуммаАкцизаРегл КАК СуммаНУКт,
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|
	|	""Корректировка начисления акцизов прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1939"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.ДвиженияПоАкцизам КАК СтрокиДт
	|	ПО
	|		ДокументыКОтражению.Ссылка = СтрокиДт.Регистратор
	|		И СтрокиДт.Активность
	|		И НЕ СтрокиДт.Сторно
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.ВыручкаИСебестоимостьПродаж КАК СтрокиКт
	|	ПО
	|		СтрокиДт.Регистратор = СтрокиКт.Регистратор
	|		И СтрокиДт.Период = СтрокиКт.Период
	|		И СтрокиДт.НастройкаХозяйственнойОперации = СтрокиКт.НастройкаХозяйственнойОперации
	|		И СтрокиДт.ИдентификаторФинЗаписи = СтрокиКт.ИдентификаторФинЗаписи
	|		И (СтрокиКт.СуммаАкцизаРегл <> 0 ИЛИ СтрокиКт.СуммаАкцизаУпр <> 0)
	|		И СтрокиКт.Активность
	|		И НЕ СтрокиКт.Сторно
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|	ПО
	|		ДокументыКОтражению.Ссылка = РегистрацииПоПодразделению.Ссылка
	|		И НАЧАЛОПЕРИОДА(СтрокиДт.Период, ДЕНЬ) = РегистрацииПоПодразделению.Период
	|		И СтрокиДт.Организация = РегистрацииПоПодразделению.Организация
	|		И СтрокиДт.ПодразделениеУплатыАкциза = РегистрацииПоПодразделению.Подразделение
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрацииВНалоговомОргане КАК РегистрацииПоОрганизации
	|	ПО
	|		ДокументыКОтражению.Ссылка = РегистрацииПоОрганизации.Ссылка
	|		И НАЧАЛОПЕРИОДА(СтрокиДт.Период, ДЕНЬ) = РегистрацииПоОрганизации.Период
	|		И СтрокиДт.Организация = РегистрацииПоОрганизации.Организация
	|		И РегистрацииПоОрганизации.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|
	|ГДЕ
	|	НЕ ДанныеДокументаОснования.КорректировкаРеализации2_5
...
Функция ТекстКорректировкаНачисленияАкцизовПрошлыхПериодовНаРасходы2_5()
	...
	|	СтрокиКт.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|
	|	ВЫБОР КОГДА СтрокиДт.СуммаРегл = 0
	|		ТОГДА 0
	|		ИНАЧЕ СтрокиКт.СуммаАкцизаРегл
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР КОГДА СтрокиДт.СуммаУпр = 0
	|		ТОГДА 0
	|		ИНАЧЕ СтрокиКт.СуммаАкцизаУпр
	|	КОНЕЦ КАК СуммаУУ,
	|
	|	// Дт ///////////////////////////////////////////////////////////////////////////////////////////
	|
	...
	|
	|	ВЫБОР КОГДА СтрокиДт.СуммаРегл = 0
	|		ТОГДА СтрокиКт.СуммаАкцизаРегл
	|	КОГДА СтрокиДт.ПостояннаяРазница <> 0
	|		ТОГДА 0
	|		ИНАЧЕ СтрокиКт.СуммаАкцизаРегл
	|	КОНЕЦ КАК СуммаНУДт,
	|	ВЫБОР КОГДА СтрокиДт.СуммаРегл = 0
	...
	|	0 КАК СуммаВРКт,
	|	""Корректировка начисления акцизов прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1955"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
Функция ТекстКорректировкаНачисленияАкцизовПрошлыхПериодовНаДоходы2_5()
	...
	|	СтрокиДт.ИдентификаторФинЗаписи КАК ИдентификаторСтроки,
	|
	|	ВЫБОР КОГДА СтрокиКт.СуммаРегл = 0
	|		ТОГДА 0
	|		ИНАЧЕ -СтрокиДт.СуммаАкцизаРегл
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР КОГДА СтрокиКт.СуммаУпр = 0
	|		ТОГДА 0
	|		ИНАЧЕ -СтрокиДт.СуммаАкцизаУпр
	|	КОНЕЦ КАК СуммаУУ,
	|
	|	// Дт ///////////////////////////////////////////////////////////////////////////////////////////
	|
	...
	|
	|	ВЫБОР КОГДА СтрокиКт.СуммаРегл = 0
	|		ТОГДА -СтрокиДт.СуммаАкцизаРегл
	|	КОГДА СтрокиКт.ПостояннаяРазница <> 0
	|		ТОГДА 0
	|		ИНАЧЕ -СтрокиДт.СуммаАкцизаРегл
	|	КОНЕЦ КАК СуммаНУКт,
	|	ВЫБОР КОГДА СтрокиКт.СуммаРегл = 0
	...
	|
	|	""Корректировка начисления акцизов прошлых периодов"" КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_1956"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
Функция ТекстВыручкаОтРеализации()
	...
	|
	|	СтрокиКт.СуммаБезНДСРегл - СтрокиКт.СуммаПР - СтрокиКт.СуммаАкцизаРегл КАК СуммаНУДКт,
	|	СтрокиКт.СуммаПР КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	ВЫБОР
	|		КОГДА ДанныеДокументаОснования.КорректировкаРеализации2_5
	|			ТОГДА ""Выручка от реализации""
	|		КОГДА СтрокиКт.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
	|			И СтрокиКт.Количество < 0
	|			ТОГДА ""Сторно выручки от реализации собственного товара""
	|		КОГДА СтрокиКт.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
	|			И СтрокиКт.Количество = 0
	|			ТОГДА ""Корректировка выручки от реализации собственного товара""
	|		КОГДА СтрокиКт.ТипЗапасов В (
	|				ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Услуга),
	|				ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Работа))
	|			ТОГДА ""Выручка от реализации работ и услуг""
	|		ИНАЧЕ ""Выручка от реализации собственного товара""
	|	КОНЕЦ КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_2030"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
Функция ТекстВыручкаОтРеализацииНУ()
	
	ТекстЗапроса = "
	|////////////////////////////////////////////////////////////////////////////////////////////////////
	|// Выручка от реализации товара (налоговый учет) (Дт 76.К :: Кт 90.01.1)
	|ВЫБРАТЬ
	|	СтрокиКт.Ссылка КАК Ссылка,
	|	КОНЕЦПЕРИОДА(ДанныеДокументаОснования.ДатаОтгрузки, МЕСЯЦ) КАК Период,
	|	СтрокиКт.Организация КАК Организация,
	...
	|		КОГДА СтрокиКт.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
	|			И СтрокиКт.Количество < 0
	|			ТОГДА ""Уменьшение реализации собственных товаров прошлых периодов (НУ)""
	|		КОГДА СтрокиКт.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
	|			И СтрокиКт.Количество = 0
	|			ТОГДА ""Корректировка выручки от реализации собственного товара прошлых периодов (НУ)""
	|		КОГДА СтрокиКт.ТипЗапасов В (
	|				ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Услуга),
	|				ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Работа))
	|			ТОГДА ""Выручка от реализации работ и услуг прошлых периодов (НУ)""
	|		ИНАЧЕ ""Выручка от реализации собственного товара прошлых периодов (НУ)""
	|	КОНЕЦ КАК Содержание,
	|	""КорректировкаРеализацииЛокализация_2031"" КАК НомерФрагмента
	|ИЗ
	|
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
