			...
			Если Строка.Спецификация.Пустая() Тогда
				...
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось подобрать спецификацию для материала в строке %1 (спецификация ""%2"")';
						|en = 'Cannot pick the bill of materials for the material in the line %1 (bill of materials ""%2"")'"),
					Строка.НомерСтроки,
				...
				Если ДанныеСпецификации.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот Тогда
					ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Не удалось подобрать спецификацию для ""%1"" (спецификация ""%2"")';
							|en = 'Cannot pick the bill of materials for %1 (""%2"" bill of materials)'"),
						Строка.Номенклатура,
						ДанныеСпецификации.Спецификация);
				КонецЕсли;
...
Процедура ПодобратьОстаткиПолуфабрикатов(СтрокиПотребности, Материалы, Резервы)
	...
	ТекстЗапроса =
		...
		|			ИЗ
		|				ФильтрМатериаловДляПолученияОстатков КАК Фильтр)) КАК Запасы";
	...
	Тексты.Добавить(
		...
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ОстаткиОбособленно.КоличествоДляДействияОтгрузить, 0) - ЕСТЬNULL(РезервыОбособленно.Количество, 0) > 0
		|			ТОГДА
		|				ЕСТЬNULL(ОстаткиОбособленно.КоличествоДляДействияОтгрузить, 0) - ЕСТЬNULL(РезервыОбособленно.Количество, 0)
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК ОстатокОбособленно,
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ОстаткиНеобособленно.КоличествоДляДействияОтгрузить, 0) - ЕСТЬNULL(РезервыНеобособленно.Количество, 0) > 0
		|			ТОГДА
		|				ЕСТЬNULL(ОстаткиНеобособленно.КоличествоДляДействияОтгрузить, 0) - ЕСТЬNULL(РезервыНеобособленно.Количество, 0)
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК ОстатокНеобособленно
		|ИЗ
		|	ВТМатериалы КАК ВТМатериалы
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ РасчетОстатковДляУстановкиДействий КАК ОстаткиОбособленно
		...
		|			И ОстаткиОбособленно.КоличествоДляДействияОтгрузить > 0
		...
		|			И ВТМатериалы.Обособленно
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ РасчетОстатковДляУстановкиДействий КАК ОстаткиНеобособленно
		...
		|			И ОстаткиНеобособленно.КоличествоДляДействияОтгрузить > 0
...
Процедура ПодобратьАналогиПолуфабрикатов(СтрокиПотребности, ТаблицаМатериалов, РезервыПФ)
	...
	ТаблицаТоваров.Колонки.Добавить("КоличествоВсего", ОбщегоНазначения.ОписаниеТипаЧисло(15, 3));
	...
	ПараметрыОтбораРазрешений = Новый Соответствие();
	
	Если ПолучитьФункциональнуюОпцию("ДинамическаяСтруктураЗаказовНаПроизводство") Тогда
	
		ОбщегоНазначенияУТ.ДобавитьЭлементОтбораВКоллекцию(Отбор, "АналогиМатериалов.ИспользоватьПриАвтозаменеВЗаказах", "&ИспользоватьПриАвтозаменеВЗаказах");
		ПараметрыОтбораРазрешений.Вставить("ИспользоватьПриАвтозаменеВЗаказах", ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Истина));	
	
	КонецЕсли;
	
	ОтборРазрешений = Новый Структура();
	ОтборРазрешений.Вставить("Отбор", Отбор);
	ОтборРазрешений.Вставить("ПараметрыОтбораРазрешений", ПараметрыОтбораРазрешений);
	
	Аналоги = АналогиМатериалов.ПолучитьТаблицуЗаменыДляФормированияЭтапов(ТаблицаТоваров, ОтборРазрешений, РезервыПФ, Ложь, ТипИсточника);
