...
Функция ПолучитьПараметрыОтчета(ПараметрыОтчета, ДанныеЯчеек, СвязаннаяСтрокаДанных, ВидРасшифровки, ИмяПоказателя)
	...
	Если МинимальнаяПериодичность <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ЗначениеПериода) Тогда
			...
			КонецПериода  = РаботаСПериодамиКлиентСервер.ДатаКонцаПериода(НачалоПериода, МинимальнаяПериодичность);
...
Функция ТаблицаКурсов(ОбъектИПараметрыРасчета, СтруктураПараметров, Валюта, Отказ, ТекстСообщения) Экспорт
	...
	МассивВалютДокумента = ОбщегоНазначенияУТ.ВыгрузитьТаблицуЗначений(Объект.АналитикаСтатейБюджетов, ,
		"Валюта").ВыгрузитьКолонку("Валюта"); // Массив
