			...
			Иначе
				Контекст.ОписаниеОшибкиНаКлиенте = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось получить менеджер криптографии для алгоритма %1: %2.';
						|en = 'Cannot get the cryptography manager for the algorithm %1: %2.'"),
		...
		Если Не Результат.УстановкаВыполнена Тогда
			...
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
				НСтр("ru = 'Электронная подпись.Обновление списков отзыва';
					|en = 'Digital signature.Revocation list update'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
...
Процедура ПродолжитьОткрытиеПослеПолученияОтпечатковНаТокенах(ОтпечаткиСертификатовНаТокенах, ДополнительныеПараметры) Экспорт
	...
	ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ru = 'Требуется приложение электронной подписи и шифрования';
			|en = 'A digital signing and encryption app is required'"), "", ДополнительныеПараметры.Результат,
		Контекст.ОшибкаНаСервере);
...
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
		ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки", НСтр("ru = 'Не найден токен для шифрования данных.';
																|en = 'Token for data encryption not found.'"));
#Иначе
...
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
		ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки", НСтр("ru = 'Не найден токен с указанным сертификатом.';
																|en = 'Token with the specified certificate not found.'"));
#Иначе
		ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки", ДополнительныеПараметры.ОписаниеОшибки);
#КонецЕсли
			...
			ИначеЕсли ТипЗнч(МенеджерКриптографии) = Тип("Структура") Тогда // Токен
				...
				ВыполнитьОбработкуОповещения(ОповещениеПослеОперацииНаТокене, 
							НСтр("ru = 'Не найдена процедура подписания для токена.';
								|en = 'Signing procedure for the token not found.'"));
			Иначе
				ОповещениеУспех = Новый ОписаниеОповещения(
					"ВыполнитьНаСторонеЦиклПослеОперацииНаКлиентеXMLDSig", ЭтотОбъект, Контекст);
				...
				ИначеЕсли ТипЗнч(МенеджерКриптографии) = Тип("Структура") Тогда // Токен
					...
					ВыполнитьОбработкуОповещения(ОповещениеПослеОперацииНаТокене, 
						НСтр("ru = 'Не найдена процедура шифрования для токена.';
							|en = 'Encryption procedure for the token not found.'"));
				...
				ИначеЕсли ТипЗнч(МенеджерКриптографии) = Тип("Структура") Тогда // Токен
					...
					ВыполнитьОбработкуОповещения(ОповещениеПослеОперацииНаТокене, 
						НСтр("ru = 'Не найдена процедура расшифровки для токена.';
							|en = 'Decryption procedure for the token not found.'"));
...
Процедура ПослеРасшифровкиНаТокене(РасшифрованныеДанные, Контекст) Экспорт
	...
	Если Не ЗначениеЗаполнено(РасшифрованныеДанные) Тогда
		ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки", НСтр("ru = 'Получены пустые расшифрованные данные.';
																|en = 'Empty decrypted data retrieved.'"));
		ВыполнитьНаСторонеПослеЦикла(ОшибкаНаКлиенте, Контекст);
		Возврат;
	КонецЕсли;
