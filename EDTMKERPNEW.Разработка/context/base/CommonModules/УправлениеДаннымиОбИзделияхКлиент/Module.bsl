...
// Оформить разборочную спецификацию на основании сборочной
// 
// Параметры:
//  Источник - СправочникСсылка.РесурсныеСпецификации
//  ПараметрыВыполнения - см. ПодключаемыеКомандыКлиент.ПараметрыВыполненияКоманды
//
Процедура СоздатьРазборкуНаОснованииСборки(Источник, ПараметрыВыполнения) Экспорт
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Основание", УправлениеДаннымиОбИзделияхВызовСервера.ДанныеЗаполненияРазборкиНаОснованииСборки(Источник));
	
	ОткрытьФорму("Справочник.РесурсныеСпецификации.Форма.ФормаЭлемента", ПараметрыФормы);
...
// * ВидСтроки - ПеречислениеСсылка.ВидыСтрокСпецификации - вид строки дерева
...
// Дерево спецификаций перед началом изменения.
// 
// Параметры:
//  Элемент - ТаблицаФормы
//  Отказ - Булево
Процедура ДеревоСпецификацийПередНачаломИзменения(Элемент, Отказ) Экспорт
	...
	Если Элемент.ТекущийЭлемент.Имя = "ДеревоСпецификацийСпецификация"
		И УправлениеДаннымиОбИзделияхКлиентСервер.ТипПроизводственногоПроцессаСтрокиДерева(ТекущиеДанные)
					= ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот")
...
// Начало выбора спецификации в дереве.
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
//  Элемент - ТаблицаФормы
//  ДанныеВыбора - Массив из  Строка
//  СтандартнаяОбработка - Булево
Процедура ДеревоСпецификацийСпецификацияНачалоВыбора(Форма, Элемент, ДанныеВыбора, СтандартнаяОбработка) Экспорт
	
	СтандартнаяОбработка = Ложь;
	
	ТекущиеДанные = Форма.Элементы.ДеревоСпецификаций.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Ключ = УправлениеДаннымиОбИзделияхКлиентСервер.КлючПараметровВыбораСпецификацийДерева(ТекущиеДанные);
	
	ДанныеОбИзделии = УправлениеДаннымиОбИзделияхКлиентСервер.СтруктураДанныхОбИзделииДляВыбораСпецификации();
	ЗаполнитьЗначенияСвойств(ДанныеОбИзделии, ТекущиеДанные);
	ДанныеОбИзделии.НачалоПроизводства      = Форма.ДеревоДата;
	ДанныеОбИзделии.ПодразделениеДиспетчер  = Форма.ПараметрНазначенияПодразделениеДиспетчер;
	ДанныеОбИзделии.НаправлениеДеятельности = Форма.ПараметрНазначенияНаправлениеДеятельности;
	
	УправлениеДаннымиОбИзделияхКлиент.ОткрытьФормуВыбораСпецификацийПоНоменклатуре(
		ДанныеОбИзделии,
		Форма.ПараметрыВыбораСпецификаций[Ключ],
		Форма);
