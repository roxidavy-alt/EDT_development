...
Функция ДанныеЭлементаОтчета(ЭлементОтчета, ЗначенияРеквизитов = Неопределено) Экспорт
	...
	СтруктураТаблицЭлемента = Новый Структура;
	СтруктураТаблицЭлемента.Вставить("ОперандыФормулы", СтрРазделить("Идентификатор,Операнд", ","));
	СтруктураТаблицЭлемента.Вставить("РеквизитыВидаЭлемента", СтрРазделить("Реквизит,Значение", ","));
	СтруктураТаблицЭлемента.Вставить("ЭлементыТаблицы", СтрРазделить("Строка,Колонка,Элемент", ","));
	СтруктураТаблицЭлемента.Вставить("ДополнительныеПоля", СтрРазделить("Реквизит,Наименование,ВыводитьЗаголовок", ","));
	СтруктураТаблицЭлемента.Вставить("ЭлементыОформления", СтрРазделить("Оформление,Условие,ТипОформляемойОбласти,КлючЭлементаОформления", ","));
	СтруктураТаблицЭлемента.Вставить("ОформляемыеСтроки", СтрРазделить("ЭлементОтчета,КлючЭлементаОформления", ","));
	СтруктураТаблицЭлемента.Вставить("ОформляемыеКолонки", СтрРазделить("ЭлементОтчета,КлючЭлементаОформления", ","));
	СтруктураТаблицЭлемента.Вставить("РасшифровкаПолейОтбораЭО", СтрРазделить("КлючЭлементаОформления,ЭлементОтчета,ИмяПоляОтбора,ИмяРесурса", ","));
	СтруктураТаблицЭлемента.Вставить("ИсточникиЗначений", СтрРазделить("Источник,ДобавляемыеЗначенияДокумента", ","));
	СтруктураТаблицЭлемента.Вставить("ПравилаРасчета", СтрРазделить("Правило", ","));
	Для Каждого КлючИЗначение Из СтруктураТаблицЭлемента Цикл
		Если ТипЗнч(СтруктураЭлемента[КлючИЗначение.Ключ]) <> Тип("ТаблицаЗначений") Тогда
			ТаблицаЭлемента = Новый ТаблицаЗначений;
			Для Каждого ИмяКолонки Из КлючИЗначение.Значение Цикл
				ТаблицаЭлемента.Колонки.Добавить(ИмяКолонки);
			КонецЦикла;
			СтруктураЭлемента[КлючИЗначение.Ключ] = ТаблицаЭлемента;
		КонецЕсли;
	КонецЦикла;
...
Функция ПараметрыИспользованияФормы(ВидЭлемента, Элемент, ДополнительныйРежим = Неопределено, ОткрытиеФормыЭлементаБюджетногоОтчета = Ложь) Экспорт
	...
	ИначеЕсли ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.ТаблицаСложная
		...
		МассивДополнительныхРеквизитов.Добавить(ДополнительныеРеквизиты.ВыводитьЗаголовкиПоВертикали);
	...
	ИначеЕсли ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.НастройкаПравилРасчета Тогда
		ИмяФормыЭлемента = "Форма.НастройкаПравилРасчета";
		МассивДополнительныхРеквизитов.Добавить(ДополнительныеРеквизиты.ПривилегированныйРежим);
		//фильтры
		МассивДополнительныхРеквизитов.Добавить(ДополнительныеРеквизиты.ИспользоватьФильтрПоОрганизации);
		МассивДополнительныхРеквизитов.Добавить(ДополнительныеРеквизиты.ИспользоватьФильтрПоПодразделению);
...
Процедура КопироватьТаблицыЭлемента(Источник, Приемник)
	...
	ИменаТаблиц = "ОперандыФормулы, РеквизитыВидаЭлемента, ЭлементыТаблицы, ПравилаРасчета, ЭлементыОформления, ОформляемыеСтроки, ОформляемыеКолонки, РасшифровкаПолейОтбораЭО, ИсточникиЗначений, ДополнительныеПоля, ЭлементыОформления";
	СтруктураТаблиц = Новый Структура(ИменаТаблиц);
	
	Для Каждого КлючИЗначение Из СтруктураТаблиц Цикл
		Приемник.Вставить(КлючИЗначение.Ключ, Источник[КлючИЗначение.Ключ].Выгрузить());
	КонецЦикла;
	ПереопределитьТипКолонокТаблицКэша(Приемник, СтруктураТаблиц);

КонецПроцедуры

Процедура ПереопределитьТипКолонокТаблицКэша(СтруктураЭлемента, СтруктураТаблиц)
