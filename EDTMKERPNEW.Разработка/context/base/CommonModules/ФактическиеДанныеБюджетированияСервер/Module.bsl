...
Процедура ВыполнитьОффлайновыеРасчеты(ДокументыКРасчету, Знач ДатаОкончания = Неопределено) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодакцизныеТовары")
		И ДокументыКРасчету.Количество() > 0 Тогда
			НеоперативнаяОчередьЗаданий.ВыполнитьЗаданияНеоперативнойОчередиПоСпискуДокументов(
				ДокументыКРасчету, Справочники.ВидыНеоперативныхЗаданий.ОтражениеДвиженийПоАкцизам);
	КонецЕсли;
...
Функция НужноСтавитьДокументВОчередьЗаданий(Источник)
	...
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.ЭкземплярБюджета") Тогда
		Возврат Ложь;
	ИначеЕсли ТипЗнч(Источник) = Тип("ДокументОбъект.Сторно")
		И (ТипЗнч(Источник.СторнируемыйДокумент) = Тип("ДокументСсылка.ВводОстатков")
		Или ТипЗнч(Источник.СторнируемыйДокумент) = Тип("ДокументСсылка.ВводОстатковВзаиморасчетов")) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	МетаданныеОбъекта = Источник.Метаданные(); // ОбъектМетаданныхРегистрНакопления, ОбъектМетаданныхДокумент -
	ЭтоРегистрНакопления = Метаданные.РегистрыНакопления.Содержит(МетаданныеОбъекта);
	ЭтоРегистрБухгалтерии = Метаданные.РегистрыБухгалтерии.Содержит(МетаданныеОбъекта);
	Если ЭтоРегистрНакопления Или ЭтоРегистрБухгалтерии Тогда
		...
		Если Не Источник.Количество() Тогда
			...
			Иначе
				Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Таблица", "РегистрБухгалтерии." + МетаданныеОбъекта.Имя);
			КонецЕсли;
		...
		Если ТипЗнч(Регистратор) = Тип("ДокументСсылка.Сторно") Тогда
			СторнируемыйДокумент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Регистратор, "СторнируемыйДокумент");
			Если ТипЗнч(СторнируемыйДокумент) = Тип("ДокументСсылка.ВводОстатков")
				Или ТипЗнч(СторнируемыйДокумент) = Тип("ДокументСсылка.ВводОстатковВзаиморасчетов") Тогда
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
// Возвращает массив видов документов, по которым собираются фактические данные по правилу
//
// Параметры:
...
//   * ВидДокумента - СправочникСсылка.ИдентификаторыОбъектовМетаданных
// Возвращаемое значение:
//  Массив из СправочникСсылка.ИдентификаторыОбъектовМетаданных
//
Функция ОпределитьВидыДокументовПоСКД(ОписаниеПравила, КэшИменДокументов = Неопределено) Экспорт
	
	Если ТипЗнч(ОписаниеПравила) = Тип("СправочникОбъект.ПравилаПолученияФактаПоСтатьямБюджетов") Тогда
		Запрос = Новый Запрос;
		Запрос.Текст =
		"ВЫБРАТЬ
			|	КэшИменДокументовДляОбработкиПоПравилу.ВидДокумента КАК ВидДокумента
			|ИЗ
		...
		МассивВидовДокументов = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ВидДокумента");
		Правило = ОписаниеПравила.Ссылка;
		ИсточникДанных = ОписаниеПравила.ИсточникДанных;
	Иначе
		МассивВидовДокументов = КэшИменДокументов.Скопировать(Новый Структура("Правило", ОписаниеПравила.Правило), "ВидДокумента").ВыгрузитьКолонку("ВидДокумента");
		Правило = ОписаниеПравила.Правило;
		ИсточникДанных = ОписаниеПравила.ИсточникДанныхХО;
	КонецЕсли;
	
	Если МассивВидовДокументов.Количество() Тогда
	...
	Если ОписаниеПравила.РазделИсточникаДанных = Перечисления.РазделыИсточниковДанныхБюджетирования.ОперативныйУчет
		...
		И (НЕ ПолучитьФункциональнуюОпцию("НастройкаПравилПолученияФактаПоСтатьямАктивовПассивов")
		ИЛИ ОписаниеПравила.ТипПравила <> Перечисления.ТипПравилаПолученияФактическихДанныхБюджетирования.ФактическиеДанные) Тогда
		МассивВидовДокументов = ИсточникиДанныхСервер.МассивВидовДокументовПоИсточникуДанных(ИсточникДанных);
		Если ОписаниеПравила.ИсточникСуммыОперации = Перечисления.ПоказателиАналитическихРегистров.СуммаКВыплате
			...
			МассивВидовДокументов.Добавить(ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Метаданные.Документы.ЗаявкаНаКомандировку));
			МассивВидовДокументов.Добавить(ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Метаданные.Документы.ЗаявкаНаРасходованиеДенежныхСредств));
		КонецЕсли;
	Иначе
		...
		Для Каждого Набор Из СКД.НаборыДанных Цикл
			Если ТипЗнч(Набор) <> Тип("НаборДанныхЗапросСхемыКомпоновкиДанных") Тогда
				...
				Прервать;
			Иначе
				МетаданныеТиповДокументов = Новый Массив;
					...
					Для Каждого Колонка Из ПакетЗапроса.Колонки Цикл
						...
						Для Каждого Тип Из Колонка.ТипЗначения.Типы() Цикл
							...
							Если ОбщегоНазначения.ЭтоДокумент(МетаданныеТипа) Тогда
								МетаданныеТиповДокументов.Добавить(МетаданныеТипа);
							КонецЕсли;
						КонецЦикла;
					КонецЦикла;
				КонецЦикла;
				Если МетаданныеТиповДокументов.Количество() > 0 Тогда
					МассивВидовДокументов = ОбщегоНазначенияУТКлиентСервер.ПреобразоватьСоответствиеИлиСтруктуруВМассив(
						ОбщегоНазначения.ИдентификаторыОбъектовМетаданных(МетаданныеТиповДокументов), Ложь);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		
		Если Не МассивВидовДокументов.Количество() Тогда
			МассивВидовДокументов.Добавить(Справочники.ИдентификаторыОбъектовМетаданных.ПустаяСсылка());
		КонецЕсли;
	...
	ВидыДокументов.Колонки.Добавить("Правило");
	ВидыДокументов.Колонки.Добавить("ВидДокумента");
	...
	ВидыДокументов.Свернуть("Правило, ВидДокумента", "");
	...
	НаборЗаписей.Загрузить(ВидыДокументов);
	НаборЗаписей.Записать();
	
	Возврат ВидыДокументов.ВыгрузитьКолонку("ВидДокумента");
...
//  ВидыДокументов - Массив из СправочникСсылка.ИдентификаторыОбъектовМетаданных, Неопределено
//  Правило - СправочникСсылка.ПравилаПолученияФактическихДанныхПоСтатьямБюджетов, Неопределено
//
Процедура СформироватьДвиженияПоРегиструФактаБюджетирования(ДокументыКОбработке, КоличествоОбработанныхДокументов, Знач ВидыДокументов = Неопределено, Правило = Неопределено)
	...
	ДокументыКОбработке.Индексы.Добавить("ВидДокумента");
	...
	КонецЕсли;
	Если (ПолучитьФункциональнуюОпцию("НастройкаПравилПолученияФактаПоСтатьямАктивовПассивов")
		Или ПолучитьФункциональнуюОпцию("НастройкаПравилПолученияФактаПоКорреспондирующимРегистрам"))
		И Не ПолучитьФункциональнуюОпцию("КонтрольЛимитовПоДаннымБюджетирования2_4") Тогда
		ФинансоваяОтчетностьСервер.УстановитьОтбор(Настройки.Отбор, "Правило.ТипПравила",
	...
	ОпределятьВидыДокументов = (ВидыДокументов = Неопределено);
	ДокументыКОбработкеПоПравилу = Неопределено;
	ДокументыКОбработкеПоПравилам = Неопределено;
	ТаблицаКорреспонденций = Неопределено;
	ПараметрыПредыдущейИтерации = Новый Структура("ИсточникДанныхХО, ДействуетС, ДействуетПо, ВидыДокументов");
	...
	Если ПолучитьФункциональнуюОпцию("НастройкаПравилПолученияФактаПоСтатьямАктивовПассивов") Тогда
		...
		ДокументыКОбработкеПоПравилам = ДокументыКОбработке.Скопировать();
		ДокументыКОбработкеПоПравилам.Колонки.Добавить("Период", ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.ДатаВремя));
		ДокументыКОбработкеПоПравилам.Колонки.Добавить("Организация", Новый ОписаниеТипов("СправочникСсылка.Организации"));
		ДокументыКОбработкеПоПравилам.Колонки.Добавить("СтатьяБюджетов", Новый ОписаниеТипов("СправочникСсылка.СтатьиБюджетов"));
		ДокументыКОбработкеПоПравилам.Колонки.Добавить("Правило", Новый ОписаниеТипов("СправочникСсылка.ПравилаПолученияФактаПоСтатьямБюджетов"));
		
		ФактПоСтатьямАктивовПассивов = ФактическиеДанныеБюджетированияПоСтатьямАктивовПассивов.ФактическиеДанныеОперативногоУчета(
			ДокументыКОбработкеПоПравилам, ПараметрыПолученияФакта, ТаблицаПравил);
			
	ИначеЕсли ПолучитьФункциональнуюОпцию("НастройкаПравилПолученияФактаПоКорреспондирующимРегистрам") Тогда
	...
	Для Каждого ОписаниеПравила Из ТаблицаПравил Цикл
		...
		Если ОпределятьВидыДокументов Тогда
			ВидыДокументов = ОпределитьВидыДокументовПоСКД(ОписаниеПравила, КэшИменДокументов);
		КонецЕсли;
		...
		Если ПараметрыПредыдущейИтерации.ВидыДокументов = Неопределено
			Или Не ОбщегоНазначенияУТКлиентСервер.МассивыРавны(ВидыДокументов, ПараметрыПредыдущейИтерации.ВидыДокументов, Ложь) Тогда
			СтруктураПоиска = Новый Структура("Ошибка", Ложь);
			Если ВидыДокументов.Найти(Справочники.ИдентификаторыОбъектовМетаданных.ПустаяСсылка()) <> Неопределено Тогда
			...
			Иначе
				...
				Для Каждого ВидДокумента Из ВидыДокументов Цикл
					СтруктураПоиска.Вставить("ВидДокумента", ВидДокумента);
				...
				КонецЦикла;
			КонецЕсли;
			ПараметрыПредыдущейИтерации.ВидыДокументов = ОбщегоНазначения.СкопироватьРекурсивно(ВидыДокументов);
		КонецЕсли;
		...
		Если ОписаниеПравила.РазделИсточникаДанных = Перечисления.РазделыИсточниковДанныхБюджетирования.ОперативныйУчет
			И ОписаниеПравила.ТипПравила = Перечисления.ТипПравилаПолученияФактическихДанныхБюджетирования.ФактическиеДанные
			И ПолучитьФункциональнуюОпцию("НастройкаПравилПолученияФактаПоСтатьямАктивовПассивов") Тогда
			Продолжить;
		ИначеЕсли ОписаниеПравила.РазделИсточникаДанных = Перечисления.РазделыИсточниковДанныхБюджетирования.ОперативныйУчет
			...
			Иначе
				...
				Если ТаблицаКорреспонденций = Неопределено Тогда
					Если ДокументыКОбработкеПоПравилу.Найти(ОбщегоНазначения.ИдентификаторОбъектаМетаданных(
						Метаданные.Документы.ДвижениеПрочихАктивовПассивов), "ВидДокумента") <> Неопределено
	...
	Если ПолучитьФункциональнуюОпцию("НастройкаПравилПолученияФактаПоСтатьямАктивовПассивов") Тогда
		Если ФактПоСтатьямАктивовПассивов.Количество() > 0 Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ФактПоСтатьямАктивовПассивов, ТаблицаНаборовКЗаписи);
		КонецЕсли;
		
		// Возможно были ошибки корреспонденций - их необходимо зафиксировать для расследования
		СтруктураПоиска = Новый Структура("Ошибка", Истина);
		ДокументыСОшибками = ДокументыКОбработкеПоПравилам.НайтиСтроки(СтруктураПоиска);
		
		Для Каждого СтрокаДокумент Из ДокументыСОшибками Цикл
			Если Не ПустаяСтрока(СтрокаДокумент.Комментарий) Тогда
				НоваяСтрока = ТаблицаНаборовКЗаписи.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаДокумент);
				НоваяСтрока.Регистратор = СтрокаДокумент.Документ;
			...
			СтруктураПоиска = Новый Структура("Документ", СтрокаДокумент.Документ);
			НеобработанныеДокументы = ДокументыКОбработке.НайтиСтроки(СтруктураПоиска);
			
			Для Каждого НеобработанныйДокумент Из НеобработанныеДокументы Цикл
				НеобработанныйДокумент.Ошибка = Истина;
				Если Не ПустаяСтрока(СтрокаДокумент.Комментарий) Тогда
					НеобработанныйДокумент.Комментарий = НеобработанныйДокумент.Комментарий
						+ ?(ПустаяСтрока(НеобработанныйДокумент.Комментарий), "", Символы.ПС)
						+ СтрокаДокумент.Комментарий;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
...
Процедура ОтразитьДокументыВБюджетировании(НачалоПериода = Неопределено, КонецПериода = Неопределено,
	ВидыДокументов = Неопределено, Правило = Неопределено) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("НастройкаПравилПолученияФактаПоКорреспондирующимРегистрам")
	 ИЛИ ПолучитьФункциональнуюОпцию("НастройкаПравилПолученияФактаПоСтатьямАктивовПассивов") Тогда
	...
	Попытка
		...
		Если ЧастичноеОтражениеДокументов Тогда
			...
			ПоместитьДокументыДляОтраженияВоВременнуюТаблицу(НачалоПериода, КонецПериода, МенеджерВременныхТаблиц, ВидыДокументов);
		...
		"ВЫБРАТЬ
			...
			|	ЗаданияКОтражениюВБюджетировании.НомерЗадания,
			|	ИдентификаторыОбъектовМетаданных.Ссылка КАК ВидДокумента
			|ИЗ
			...
			|		ПО ЗаданияКОтражениюВБюджетировании.ИмяВидаДокумента = ИдентификаторыОбъектовМетаданных.Имя
			|ГДЕ
			|	НЕ ЗаданияКОтражениюВБюджетировании.НомерЗадания ЕСТЬ NULL
			|	И ИдентификаторыОбъектовМетаданных.Родитель В
			|		(ВЫБРАТЬ ПЕРВЫЕ 1
			|			РодителиОбъектовМетаданных.Ссылка
			|		ИЗ
			|			Справочник.ИдентификаторыОбъектовМетаданных КАК РодителиОбъектовМетаданных
			|		ГДЕ
			|			РодителиОбъектовМетаданных.Родитель = ЗНАЧЕНИЕ(Справочник.ИдентификаторыОбъектовМетаданных.ПустаяСсылка)
			|			И РодителиОбъектовМетаданных.Имя = &Документы
			|		)
			|	И &ФильтрНаДокументы";
		...
		Если ПолучитьФункциональнуюОпцию("НастройкаПравилПолученияФактаПоСтатьямАктивовПассивов") Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПЕРВЫЕ 10001", "ПЕРВЫЕ 1001");
			КоличествоДокументовВПорции = 1001;
		Иначе
			КоличествоДокументовВПорции = 10001;
		КонецЕсли;
	...
	Пока ЕстьЧтоОбрабатывать Цикл
		...
		ОбрабатываетсяПорция = ДокументыКОбработке.Количество() = КоличествоДокументовВПорции;
		
		КоличествоОбработанныхДокументов = 0;
		СформироватьДвиженияПоРегиструФактаБюджетирования(ДокументыКОбработке, КоличествоОбработанныхДокументов, ВидыДокументов, Правило);
...
Функция ЕстьХранимыеПравилаСтатейДляВидаБюджета(ВидБюджета)
	...
	|			И СтатьиИПоказателиБюджета.СтатьяПоказатель = ПравилаРасчета.Приемник
	|			И (НЕ ПравилаРасчета.ПометкаУдаления)
	|			И (ПравилаРасчета.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПравилБюджетирования.Действует))
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПравилаПолученияФактаПоСтатьямБюджетов КАК ПравилаПолученияФактаПоСтатьямБюджетов
	|		ПО (ПравилаРасчета.Источник = ПравилаПолученияФактаПоСтатьямБюджетов.СтатьяБюджетов)
	|ГДЕ
	|	ПравилаПолученияФактаПоСтатьямБюджетов.ПромежуточноеКэшированиеРезультатовРаботыПравил";
	...
	Если НужнаНадпись Тогда
		...
		Иначе
			ЧастиТекста.Добавить(НСтр("ru = 'Необходимо запустить задание по отражению за период с %2 по %3.';
										|en = 'Start the job of recording from %2 to %3.'"));
		КонецЕсли;
...
Процедура ПоместитьДокументыДляОтраженияВоВременнуюТаблицу(Знач НачалоПериода, Знач КонецПериода, МенеджерВременныхТаблиц, ВидыДокументов = Неопределено) Экспорт
	...
	Если ВидыДокументов <> Неопределено Тогда
		Запрос.УстановитьПараметр("ИменаВидовДокументов",
			ОбщегоНазначенияУТКлиентСервер.ПреобразоватьСоответствиеИлиСтруктуруВМассив(
			ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ВидыДокументов, "Имя"), Ложь));
	Иначе
...
Функция ПолучитьТаблицуКорреспонденцийБалансовыхРегистров(ДокументыКОбработке, ПараметрыПолученияФакта, ОписаниеПравила, ПараметрыФормированияКорреспонденций)
	...
	Для Каждого СтрокаДокумент Из ВидыДокументов Цикл
		ОстаточныеФинансовыеРегистрыПоВидуДокумента = БюджетированиеПовтИсп.ОстаточныеФинансовыеРегистрыПоВидуДокумента(
	...
	ИначеЕсли Запрос.МенеджерВременныхТаблиц.Таблицы.Найти("КэшДвижений") = Неопределено
		Или ПараметрыЗапроса.НастройкиХозОперацийДляЗамены.Найти(ОписаниеПравила.ИсточникДанныхХО) <> Неопределено Тогда
		...
		Пока ВыборкаКорреспонденций.Следующий() Цикл
			...
			Если Счетчик = 2 Тогда
				...
				Если ТаблицаКорреспонденций.Индексы.Количество() = 0 Тогда
					ТаблицаКорреспонденций.Индексы.Добавить(ПоляПоискаКорреспонденцииРегистров());
				КонецЕсли;
			КонецЕсли;
	...
	ПоляГруппировок = СтрРазделить(ПоляПоискаКорреспонденцииРегистров(), ",", );
	...
	Если НЕ ВыборкаДвижения.УчитыватьХозОперацию Тогда
		...
		Для ОбратныйИндекс = 1 По КоличествоСтрок Цикл
			...
			Если ЗначениеЗаполнено(СтрокаАналитики.НастройкаХозОперации)
				...
				// При формировании корреспонденций с регистрами учета НДС настройка хоз операций может быть пустой
				// и отличаться от настройки хоз операции в строках аналитик
				Если Не (СтрокаАналитики.Показатель = Перечисления.ПоказателиАналитическихРегистров.СуммаНДС
					И Не ЗначениеЗаполнено(ВыборкаДвижения.НастройкаХозОперации)) Тогда
...
Функция ПараметрыЗаполненияКорреспонденций()
	...
	ПараметрыЗаполнения.Вставить("ПоляПоиска", ПоляПоискаКорреспонденцииРегистров());
...
Процедура ДобавитьСтрокиТаблицыКорреспонденций(ТаблицаКорреспонденций, ВыборкаДвижения, ДанныеДляЗаполнения,
	...
	Для Счетчик = 1 По МногоАналитик.СтрокиАналитики.Количество() Цикл
		...
		Если ОсновнаяТаблица Тогда
			...
			КонецЕсли;
			Если ТаблицаКорреспонденций.Колонки.Найти("ПодразделениеПлатежа") <> Неопределено
				И Не ЗначениеЗаполнено(НоваяСтрока.ПодразделениеПлатежа) Тогда
				Если ТаблицаКорреспонденций.Колонки.Найти("КорПодразделениеПлатежа") <> Неопределено Тогда
					НоваяСтрока.ПодразделениеПлатежа = НоваяСтрока.КорПодразделениеПлатежа;
				Иначе
					НоваяСтрока.ПодразделениеПлатежа = НоваяСтрока.Подразделение;
				КонецЕсли;
			КонецЕсли;
		Иначе
...
Функция ПоляПоискаКорреспонденцииРегистров()
...
Процедура ОпределитьСложныеПроводкиСУказаниемКорИсточников(Запрос, ТекстыЗапросов)
	...
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ДвиженияДебет.ИмяРегистра) КАК КоличествоРегистров
	...
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ДвиженияКредит.ИмяРегистра) КАК КоличествоРегистров
	...
	|	И ИдентификаторыОперацийДебет.КоличествоРегистров > 1
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|	И ИдентификаторыОперацийКредит.КоличествоРегистров > 1
	|");
	...
	Если НЕ ЕстьСложныеПроводки Тогда
		...
		|	И КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ДвиженияДебет.ИмяРегистра) > 1
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|	И КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ДвиженияКредит.ИмяРегистра) > 1
		|";
...
Процедура СоединитьДвиженияПоКлючевымПолям(Запрос, ТекстыЗапросов)
	...
	|	КОНЕЦ
	|	И ВЫБОР
	|			КОГДА ДвиженияДебет.КоличествоЗаписейОперации > 1
	|					И ДвиженияКредит.КоличествоЗаписейОперации > 1
	|					И ЕСТЬNULL(ДвиженияДебет.СуммаВал, 0) <> 0
	|					И ЕСТЬNULL(ДвиженияКредит.СуммаВал, 0) <> 0
	|					И ЕСТЬNULL(ДвиженияДебет.СуммаВал, 0) = ЕСТЬNULL(ДвиженияКредит.СуммаВал, 0)
	|				ТОГДА ЕСТЬNULL(ДвиженияДебет.СуммаУпр, 0) = ЕСТЬNULL(ДвиженияКредит.СуммаУпр, 0)
	|						ИЛИ ЕСТЬNULL(ДвиженияДебет.СуммаРегл, 0) = ЕСТЬNULL(ДвиженияКредит.СуммаРегл, 0)
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	...
	|	КОНЕЦ
	|	И ВЫБОР
	|			КОГДА ДвиженияДебет.Показатель В (ЗНАЧЕНИЕ(Перечисление.ПоказателиАналитическихРегистров.Стоимость), ЗНАЧЕНИЕ(Перечисление.ПоказателиАналитическихРегистров.СтоимостьБезНДС))
	|					И ДвиженияКредит.Показатель В (ЗНАЧЕНИЕ(Перечисление.ПоказателиАналитическихРегистров.СуммаДопРасходов), ЗНАЧЕНИЕ(Перечисление.ПоказателиАналитическихРегистров.СуммаДопРасходовБезНДС))
	|				ТОГДА ЕСТЬNULL(ДвиженияДебет.СуммаУпр, 0) = ЕСТЬNULL(ДвиженияКредит.СуммаУпр, 0)
	|						ИЛИ ЕСТЬNULL(ДвиженияДебет.СуммаРегл, 0) = ЕСТЬNULL(ДвиженияКредит.СуммаРегл, 0)
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|	И ВЫБОР
	|			КОГДА ДвиженияДебет.Показатель В (ЗНАЧЕНИЕ(Перечисление.ПоказателиАналитическихРегистров.СуммаДопРасходов), ЗНАЧЕНИЕ(Перечисление.ПоказателиАналитическихРегистров.СуммаДопРасходовБезНДС))
	|					И ДвиженияКредит.Показатель В (ЗНАЧЕНИЕ(Перечисление.ПоказателиАналитическихРегистров.Стоимость), ЗНАЧЕНИЕ(Перечисление.ПоказателиАналитическихРегистров.СтоимостьБезНДС))
	|				ТОГДА ЕСТЬNULL(ДвиженияДебет.СуммаУпр, 0) = ЕСТЬNULL(ДвиженияКредит.СуммаУпр, 0)
	|						ИЛИ ЕСТЬNULL(ДвиженияДебет.СуммаРегл, 0) = ЕСТЬNULL(ДвиженияКредит.СуммаРегл, 0)
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
...
Функция ТекстыЗапросовЗаписейРегистров(Запрос, ОстаточныеФинансовыеРегистры)
	...
	Для Каждого КлючИЗначение Из ОстаточныеФинансовыеРегистры Цикл
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ВидДеятельностиНДС", СтрШаблон("МАКСИМУМ(%1)", ПутиКДаннымПолей.ВидДеятельностиНДС));
...
Процедура СформироватьКорреспонденцииЗаписейРегистров(Запрос, ТекстыЗапросов)
	...
	Если Запрос.Параметры.ВосстановитьРасчетыПоСторно Тогда
		...
		|	РасчетыСКлиентамиПоСторно.ПоказательДт КАК ПоказательДт,
		|	РасчетыСКлиентамиПоСторно.ПоказательКт КАК ПоказательКт,
		...
		|	РасчетыСКлиентамиПоСторно.ДопНастройкаХозОперацииДт КАК ДопНастройкаХозОперацииДт,
		|	РасчетыСКлиентамиПоСторно.ДопНастройкаХозОперацииКт КАК ДопНастройкаХозОперацииКт,
		...
		|	РасчетыСПоставщикамиПоСторно.ПоказательДт КАК ПоказательДт,
		|	РасчетыСПоставщикамиПоСторно.ПоказательКт КАК ПоказательКт,
		...
		|	РасчетыСПоставщикамиПоСторно.ДопНастройкаХозОперацииДт КАК ДопНастройкаХозОперацииДт,
		|	РасчетыСПоставщикамиПоСторно.ДопНастройкаХозОперацииКт КАК ДопНастройкаХозОперацииКт,
...
Процедура ПроверитьОшибкиКорреспонденций(ДокументыКОбработке, Запрос, ТекстыЗапросов)
	...
	|			И Движения.ДопНастройкаХозОперацииКт = ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ПустаяСсылка)
	|			И (Движения.ПоказательДт = ЗНАЧЕНИЕ(Перечисление.ПоказателиАналитическихРегистров.СуммаНДС)
	|			ИЛИ Движения.ПоказательКт = ЗНАЧЕНИЕ(Перечисление.ПоказателиАналитическихРегистров.СуммаНДС))
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ПустаяСсылка)
	|		ИНАЧЕ Движения.НастройкаХозОперации
	|	КОНЕЦ КАК НастройкаХозОперации,
	|	ВЫБОР
	|		КОГДА Движения.ДопНастройкаХозОперацииДт = ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ПустаяСсылка)
	...
	|			ТОГДА ВЫБОР
	|				КОГДА Движения.ПоказательДт = ЗНАЧЕНИЕ(Перечисление.ПоказателиАналитическихРегистров.СуммаНДС)
	|					ТОГДА Движения.ДопНастройкаХозОперацииКт
	|				ИНАЧЕ Движения.ДопНастройкаХозОперацииДт
	|			КОНЕЦ
...
Процедура СформироватьЗапросыАналитикиКорреспонденцийИПоместитьВКэш(Запрос, ТекстыЗапросов, ТекстыЗапросовРегистров, СмещениеИндекса)
	...
	Если ТекстыЗапросовРегистров <> Неопределено Тогда
		...
		ПоляИндексаРеверса = СтрРазделить("Период,Регистратор,ИдентификаторФинЗаписи,Организация", ",");
	...
	Если ТекстыЗапросовРегистров <> Неопределено Тогда
		...
		Пока ВыборкаИмяРегистра.Следующий() Цикл
			...
			Если Запрос.МенеджерВременныхТаблиц.Таблицы.Найти(ИмяВременнойТаблицыРегистра) = Неопределено Тогда
				...
				Если Запрос.Параметры.ВосстановитьРасчетыПоСторно
					И Запрос.Параметры.РегистрыВосстановленияРасчетовПоСторно.Найти(ИмяТаблицыРегистра) <> Неопределено Тогда
...
Функция ПолучитьТаблицуКорреспонденцийПрочихАктивовПассивов(ДокументыКОбработке, ПараметрыПолученияФакта, ОписаниеПравила)
	...
	ДокументыКОбработкеПоПравилу = ДокументыКОбработке.Скопировать(Новый Структура("ВидДокумента",
		ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Метаданные.Документы.ДвижениеПрочихАктивовПассивов)));
