...
Процедура ЗаполнитьОписаниеГруппЭтаповЗакрытияМесяца(ТаблицаГруппЭтапов) Экспорт
	...
	НоваяСтрока.Наименование = НСтр("ru = 'Допроведение документов';
									|en = 'Additional document posting'");
	...
	НоваяСтрока.МеждународноеНаименование = "ru = 'Международный учет';
											|en = 'Financial accounting'";
	//-- НЕ УТКА
...
Процедура ЗаполнитьОписаниеЭтаповЗакрытияМесяца(ТаблицаЭтапов) Экспорт
	...
	ПроизводствоСервер.ДобавитьЭтап_ПриемкаВнутреннихРабот(ТаблицаЭтапов, ТекущийРодитель);
	...
	ОтложенноеФормированиеДвиженийВНА.ДобавитьЭтап_ФормированиеДвиженийПоВНАПередРасчетомАмортизации(ТаблицаЭтапов,ТекущийРодитель);
	...
	//++ НЕ УТ
	РасчетДетализацииПродукции.ДобавитьЭтап_РасчетДетализацииПродукции(ТаблицаЭтапов, ТекущийРодитель);
	//-- НЕ УТ
...
Процедура ДобавитьЭтап_ПереходНаНовыеРежимыРаботы(ТаблицаЭтапов, ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Требуется осуществить переход на новые режимы учета';
											|en = 'Migrate to new accounting modes'"; // @НСтр-1
...
Процедура Использование_ПереходНаНовыеРежимыРаботы(ПараметрыОбработчика) Экспорт
	...
	Иначе
		...
		ПараметрыОбработчика.ДанныеЭтапа.Наименование = НСтр("ru = 'Рекомендуется осуществить переход на новые режимы учета';
															|en = 'Рекомендуется осуществить переход на новые режимы учета'");
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоНаСтороне")
	 И НЕ ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоНаСтороне2_5") Тогда
		...
		УстановитьСостояниеВыполненСОшибками(
			ПараметрыОбработчика,
			НСтр("ru = 'Производство на стороне (версия 2.5)';
				|en = 'Production by third party (version 2.5)'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ТекстГиперссылкиПоУмолчанию,
			ОписаниеДействия_ОткрытьФорму(Метаданные.Обработки.ПанельАдминистрированияКА.Формы.Производство.ПолноеИмя()),
			СтатусПоУмолчанию);
...
Процедура ДобавитьЭтап_ИсправлениеОшибокВИсходныхДанных(ТаблицаЭтапов, ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Исправление ошибок в исходных данных';
											|en = 'Correct errors in the initial data'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
	НоваяСтрока.ТекстПодробнее     = ТекстПодробнееПоУмолчанию();
...
Процедура ОписаниеПроверок_ИсправлениеОшибокВИсходныхДанных(ТаблицаПроверок)
	...
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		...
		НСтр("ru = 'Указаны документы производства без заказов для которых введено сторно или исправление.';
			|en = 'Specifies backflush production documents with storno or correction.'", ОбщегоНазначения.КодОсновногоЯзыка())); 	
...
Процедура ДобавитьЭтап_ЗаблокироватьОтИзменений(ТаблицаЭтапов, ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Заблокировать от изменений';
											|en = 'Lock from changes'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
	НоваяСтрока.Информационный = Истина;
...
Функция ДатаЗаданияНаСледующийПериод(Операция, Организация, ДокументЗадания, Знач СледующийМесяц) Экспорт
	...
	//++ НЕ УТ
	Если Операция = Справочники.ОперацииЗакрытияМесяца.ФормированиеРезервовПоСомнительнымДолгам Тогда
	...
	ИначеЕсли Операция = Справочники.ОперацииЗакрытияМесяца.НачислениеПроцентовПоАренде Тогда
		
		ДатаЗадания = Документы.НачислениеПроцентовПоАренде.ПериодНачисленияПроцентовПоАренде(Организация, СледующийМесяц);
				
	ИначеЕсли Операция = Справочники.ОперацииЗакрытияМесяца.НачислениеПроцентовПоДоходнойАренде Тогда
		
		ДатаЗадания = Документы.НачислениеПроцентовПоАренде.ПериодНачисленияПроцентовПоДоходнойАренде(Организация, СледующийМесяц);
				
	ИначеЕсли Операция = Справочники.ОперацииЗакрытияМесяца.ЗакрытиеРасходовОтВыбытияОС Тогда
		
		ДатаЗадания = ЗакрытиеРасходовОтВыбытияОС.СледующийПериодОперации(Организация, СледующийМесяц);
				
	ИначеЕсли Операция = Справочники.ОперацииЗакрытияМесяца.РасчетСтоимостиИнвестицииВАренду Тогда
...
Функция ОбработчикиОбновленияБлокирующиеЗакрытиеМесяца() Экспорт
	...
	Обработчики = "
	//++ НЕ УТ
	|Документы.ПроизводствоБезЗаказа.ОбработатьДанныеДляПереходаНаНовуюВерсию
	//-- НЕ УТ
...
Функция ОбработчикиОбновленияНеВлияющиеНаЗакрытиеМесяца() Экспорт
	...
	|РегистрыСведений.СтруктураЗаказа.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|РегистрыСведений.ЗаказыНаПроизводство.ОбработатьДанныеДляПереходаНаНовуюВерсию
	//-- НЕ УТКА
	...
	|ОбновлениеИнформационнойБазыКА.УстановитьКонстантуИспользоватьЗаказыМатериаловВПроизводство
	|РегистрыСведений.ДокументыПоНМА.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|РегистрыСведений.ДокументыПоОС.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|Справочники.ПоказателиБюджетов.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|Справочники.ПравилаРасчетаСвязанныхОстатковИОборотовБюджетов.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|Справочники.СтатьиБюджетов.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|Документы.ПринятиеКУчетуОС2_4.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|Документы.МодернизацияОС2_4.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|Документы.КорректировкаСтоимостиИАмортизацииОС.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|Документы.СписаниеОС2_4.ОбработатьДанныеДляПереходаНаНовуюВерсию
	//-- НЕ УТ
	|
	//++ НЕ УТКА
	|Документы.РазрешениеНаЗаменуМатериалов.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|РегистрыСведений.СвязиНоменклатурыПоРазрешениямНаЗамену.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|РегистрыСведений.АналогиМатериалов.ОбработатьДанныеДляПереходаНаНовуюВерсию
	//-- НЕ УТКА
...
Процедура ЗапуститьОбновлениеСостоянияЗакрытияМесяца(Период)
	...
	НаименованиеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Обновление состояния операций закрытия месяца %1';
			|en = 'Update month-end closing operation state %1'"),
...
Процедура ЗаданияКЗакрытиюМесяцаПередЗаписью(Источник, Отказ, Замещение) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяРегистра) Тогда
		...
		Если ТипЗнч(Регистратор) = Тип("ДокументСсылка.РасчетСебестоимостиТоваров")
		 И ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Регистратор, "ТипЗаписи"))
		 И ЗначениеЗаполнено(ИмяРегистра) Тогда
...
Процедура ЗаполнитьПроверкиДляРегистрации(ТаблицаПроверок) Экспорт
	...
	//++ НЕ УТКА
	РасчетДетализацииПродукции.ОписаниеПроверок_РасчетДетализацииПродукции(ТаблицаПроверок);
	//-- НЕ УТКА
	...
	ПроизводствоСервер.ОписаниеПроверок_ПриемкаВнутреннихРабот(ТаблицаПроверок);
	//++ Устарело_Переработка24
