...
Процедура ЗаполнитьПоОперациям(ДанныеДляЗаполнения = Неопределено) Экспорт
	...
	ПараметрыТехпроцесса = РедакторПроизводственногоПроцессаВызовСервера.ПараметрыТехпроцессаЭтапа(Ссылка);
	НастройкиУчета = Документы.ПроизводственнаяОперация2_2.НастройкиУчета(Подразделение, ПараметрыТехпроцесса.ТехнологическийПроцесс);
...
// Реквизиты хранятся в регистрах РеквизитыДеленияЭтаповПроизводства и ТехнологическиеПроцессыЭтаповПроизводства
// Запись в регистр происходит в транзакции записи документа.
// 
// Параметры:
//  РеквизитыДеления - см. Обработки.ДелениеПартииПроизводства.РеквизитыДеленияКонструктор
//
...
// Позволяет изменить реквизиты, связанные с параметрами техпроцесса
// Реквизиты хранятся в регистре ТехнологическиеПроцессыЭтаповПроизводства
// Запись в регистр происходит в транзакции записи документа.
//
// Параметры:
//  ПараметрыТехпроцесса - см. РедакторПроизводственногоПроцессаКлиентСервер.ПараметрыТехпроцессаКонструктор
//
Процедура ИзменитьПараметрыТехпроцесса(ПараметрыТехпроцесса) Экспорт
	
	ДополнительныеСвойства.Вставить("ПараметрыТехпроцесса", ПараметрыТехпроцесса);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ЭтоНовый() Тогда
		Если ВыполняетсяПакетноеФормирование()
	...
	Если РежимЗаписи <> РежимЗаписиДокумента.Запись Тогда
		...
		ЭтапПроизводства2_2Локализация.ЗаполнитьДополнительныеСвойства(ЭтотОбъект, РезультатПроверки);
...
Процедура ПриЗаписи(Отказ)
	...
	Если ДополнительныеСвойства.Свойство("РеквизитыДеления") Тогда
		Обработки.ДелениеПартииПроизводства.ИзменитьРеквизитыДеленияЭтапа(Ссылка, ДополнительныеСвойства.РеквизитыДеления);
	КонецЕсли;
	
	Если ДополнительныеСвойства.Свойство("ПараметрыТехпроцесса") Тогда
		РегистрыСведений.ТехнологическиеПроцессыЭтаповПроизводства.ЗаписатьПараметрыТехпроцесса(Ссылка, ДополнительныеСвойства.ПараметрыТехпроцесса);
	КонецЕсли;
		...
		Исключение
			Событие = НСтр("ru = 'Этап производства';
				...
				+ НСтр("ru = 'Изменение партии производства';
						|en = 'Change production lot'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		Если ДанныеЗаполнения.Свойство("ПараметрыТехпроцесса") Тогда
			ИзменитьПараметрыТехпроцесса(ДанныеЗаполнения.ПараметрыТехпроцесса);
		КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ВыполняетсяПакетноеФормирование() Тогда
		...
		ПроверитьЗаполнениеСерий(МассивНепроверяемыхРеквизитов, Отказ);
...
Процедура ПроверитьЗаполнениеСерий(МассивНепроверяемыхРеквизитов, Отказ)
	...
	Если ПланированиеРабот Тогда
		ПараметрыПроверки = НоменклатураСервер.ПараметрыПроверкиЗаполненияСерий();
		ПараметрыПроверки.ПредставлениеТЧ = Документы.ЭтапПроизводства2_2.ПредставлениеТЧОбеспечение(ЭтотОбъект);
...
Процедура ВыполнитьКонтрольПриЗакрытииЭтапа(Отказ)
	...
	Если Статус <> Перечисления.СтатусыЭтаповПроизводства2_2.Завершен
		ИЛИ (ДополнительныеСвойства.ДанныеДоИзменения <> Неопределено
				И ДополнительныеСвойства.ДанныеДоИзменения.Реквизиты.Статус = Перечисления.СтатусыЭтаповПроизводства2_2.Завершен) Тогда
		Возврат;
	КонецЕсли;
	...
	Если Константы.КонтролироватьЗавершениеОперацийПриЗакрытииЭтаповПроизводства.Получить() Тогда
		...
		ТекстЗапроса = "
			...
			|ИМЕЮЩИЕ
			|	СУММА(Очередь.Запланировано) > СУММА(Очередь.Выполнено) + СУММА(Очередь.Пропущено) + СУММА(Очередь.Брак)
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ ПЕРВЫЕ 1
			...
			|		ЗНАЧЕНИЕ(Перечисление.СтатусыПроизводственныхОпераций.Формируется),
			...
			|	) КАК ВложенныйЗапрос
			|ГДЕ
			|	ВложенныйЗапрос.ЕстьТехнологическиеОперации
			|	И НЕ ИСТИНА В (
			|		ВЫБРАТЬ ПЕРВЫЕ 1
			|			ИСТИНА
			|		ИЗ
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			"&ПараметрыТехпроцесса",
			РедакторПроизводственногоПроцесса.ТекстЗапросаПараметрыТехпроцессаЭтапа("ДокЭтап"));
...
Процедура ЗаполнитьКэшОсновнойВРЦ()
	
	Если ВидыРабочихЦентров.Количество() > 0 И ВидыРабочихЦентров[0].Использовать Тогда
		ОсновнойВРЦ = ВидыРабочихЦентров[0].ВидРабочегоЦентра;
	ИначеЕсли АльтернативныеВидыРабочихЦентров.Количество() > 0 И АльтернативныеВидыРабочихЦентров[0].Использовать Тогда
		ОсновнойВРЦ = АльтернативныеВидыРабочихЦентров[0].ВидРабочегоЦентра;
	КонецЕсли;
...
Процедура ЗаполнитьМатериалыПоОперациям(МатериалыОпераций)
	...
	РасходМатериаловИРаботДобавить = РасходМатериаловИРабот.ВыгрузитьКолонки();
	
	Для каждого СтрокаОперация Из МатериалыОпераций Цикл

		НоваяСтрока = РасходМатериаловИРаботДобавить.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаОперация);
		
		Если НЕ ЗначениеЗаполнено(НоваяСтрока.СтатьяКалькуляции) Тогда
	...
	Если РасходМатериаловИРаботДобавить.Количество() > 0 Тогда
		ПроизводствоСервер.СвернутьТаблицуЗначений(РасходМатериаловИРаботДобавить,,"КоличествоУпаковок,Количество", Истина);
		ОбщегоНазначенияУТ.ДобавитьСтрокиВТаблицу(РасходМатериаловИРабот, РасходМатериаловИРаботДобавить);
	КонецЕсли;
...
Процедура ЗагрузитьДанныеОперацийВИзделия(ИмяТЧ, ИзделияОпераций)
	...
	УправлениеПроизводствомКлиентСервер.ДобавитьВСтруктуруДействияКонтроляКачества(ИмяТЧ, СтруктураДействий, КонтрольКачества.КэшДанныхМеханизма(ЭтотОбъект));
...
Функция ТаблицаДляЗаполненияВидыЗапасовИзделия(МенеджерВременныхТаблиц)
	...
	ЧастьТекстаЗапроса = 
	"ВЫБРАТЬ
	|	ТаблицаТоваров.ДатаПроизводства	КАК Период,
	|	ВЫБОР
	...
	ЧастьТекстаЗапроса = УчетПодакцизныхТоваров.ТекстЗапросаПроизводителиПодакцизныхТоваров(
	...
	|	ПроизводителиПодакцизныхТоваров.Страна							КАК Страна,
	|	&Валюта															КАК Валюта,
	...
	|		И НЕ ТаблицаТоваров.Отменено
	|	) КАК ВложенныйЗапрос
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПроизводителиПодакцизныхТоваров КАК ПроизводителиПодакцизныхТоваров
	|		ПО ВложенныйЗапрос.Организация = ПроизводителиПодакцизныхТоваров.Организация
	|			И ВложенныйЗапрос.ДатаОперации = ПроизводителиПодакцизныхТоваров.Период
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.Организация,
	|	ПроизводителиПодакцизныхТоваров.Страна,
	...
	ПараметрыТекстаЗапроса.СтранаВТЧ			= Истина;
	ПараметрыТекстаЗапроса.ИмяПоляПериод		= "ДатаОперации";
	...
	|	ТаблицаТоваров.Период								КАК Период,
	|	ТаблицаТоваров.ДатаОперации							КАК ДатаОперации,
	|	ТаблицаТоваров.Номенклатура							КАК Номенклатура,
	|	ТаблицаТоваров.Характеристика						КАК Характеристика,
	|	ТаблицаТоваров.Назначение							КАК Назначение,
	...
	|	ЕСТЬNULL(ДанныеКатегорий.ПодакцизныйТовар, ЛОЖЬ)	КАК ОблагаетсяАкцизом,
	|	&Подразделение										КАК ПодразделениеПроизводства
	...
	|			И ТаблицаТоваров.Страна = ДанныеКатегорий.Страна
	|			И ТаблицаТоваров.ДатаОперации = ДанныеКатегорий.Период
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТДанныеОрганизаций";
	ТекстыЗапроса.Добавить(ЧастьТекстаЗапроса);
	
	НалогообложениеОрганизации	=
		УчетНДСУП.ПараметрыУчетаПоОрганизации(Организация, Дата).ОсновноеНалогообложениеНДСПродажи;
	НалогообложениеОрганизацииДавальца = УчетНДСУП.ПараметрыУчетаПоОрганизации(ОрганизацияДавалец, Дата).ОсновноеНалогообложениеНДСПродажи;
	...
	Запрос.УстановитьПараметр("НалогообложениеОрганизацииДавальца", НалогообложениеОрганизацииДавальца);
	
	ЗапасыСервер.ДополнитьВременныеТаблицыОбязательнымиКолонками(Запрос);
	
	Результат = Запрос.ВыполнитьПакетСПромежуточнымиДанными();
	
	Возврат Результат[Результат.ВГраница() - 2].Выгрузить();
...
#Область ЗаполнениеКлючейАналитик
	...
	Для Каждого Строка Из ТаблицыДокумента.ОбеспечениеМатериаламиИРаботами Цикл
		...
		Если Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.ПереданРанее
			Или Строка.Отменено Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого Строка Из ТаблицыДокумента.ОбеспечениеМатериаламиИРаботами Цикл
		...
		Если Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.ПереданРанее
			Или ТипыНоменклатуры[Строка.Номенклатура] = Перечисления.ТипыНоменклатуры.Работа Тогда
			СтруктураАналитки.Склад = Строка.Подразделение;
		КонецЕсли;
		
		Если Не Строка.Обособленно Тогда
			СтруктураАналитки.Назначение = Справочники.Назначения.ПустаяСсылка();
		КонецЕсли;
		
		Если Не Строка.СтатусУказанияСерийОтправитель = 14 Тогда
			СтруктураАналитки.Серия = Справочники.СерииНоменклатуры.ПустаяСсылка();
		КонецЕсли;
		...
		СтруктураАналитки.Склад = Строка.Подразделение;
		СтруктураАналитки.Назначение = Назначение;
		Если Не Строка.СтатусУказанияСерийПолучатель = 14 Тогда
			СтруктураАналитки.Серия = Справочники.СерииНоменклатуры.ПустаяСсылка();
		КонецЕсли;
...
Процедура ЗарегистрироватьРасчетОчередиПроизводственныхОпераций(ДанныеДоИзменения)
	...
	Если СвойстваДокумента.ЭтоНовый Тогда
		...
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ЭтапыПроизводства КАК ЭтапСпецификации
		|	ПО ЭтапСпецификации.Ссылка = &ЭтапСпецификации
		|
		|ГДЕ
		|	НЕ Т.ПометкаУдаления
		|	И Т.Владелец = ВЫБОР
		|		КОГДА &ХранитьОперацииВРесурсныхСпецификациях
		|			ТОГДА ЭтапСпецификации.Ссылка
		|		ИНАЧЕ
		|			ЭтапСпецификации.МаршрутнаяКарта
		|		КОНЕЦ
		|";
		
		Запрос = Новый Запрос(ТекстЗапроса);
		Запрос.УстановитьПараметр("ЭтапСпецификации", Этап);
		Запрос.УстановитьПараметр("ХранитьОперацииВРесурсныхСпецификациях", ПолучитьФункциональнуюОпцию("ХранитьОперацииВРесурсныхСпецификациях"));
		
		ЕстьТехнологическиеОперации = НЕ Запрос.Выполнить().Пустой();
		
	Иначе
		
		ЕстьТехнологическиеОперации = РедакторПроизводственногоПроцессаВызовСервера.ПараметрыТехпроцессаЭтапа(Ссылка).ЕстьТехнологическиеОперации;
		
	КонецЕсли;
	
	Если НЕ ЕстьТехнологическиеОперации Тогда
		Возврат;
	КонецЕсли;
	...
	ИзмененыПараметрыТехпроцесса = ДополнительныеСвойства.Свойство("ПараметрыТехпроцесса");
	...
	Если (ПроверяемыеСтатусы.Найти(Статус) <> Неопределено
		...
		И (ИзменилсяСтатусИлиСостояниеДокумента(ДанныеДоИзменения, ПереходыИсключения)
			...
			ИЛИ ДобавленоОграничениеОчереди
			ИЛИ ИзмененыПараметрыТехпроцесса) Тогда
...
Процедура ОбновитьСвязанныеДанные(Отказ, УдалениеПроведения = Ложь)
	...
	Если НЕ (ПроведениеДокументов.СвойстваДокумента(ЭтотОбъект).ЭтоНовый
		...
		Если ОтменаПроведения
			ИЛИ НЕ МожетБытьЗапланирован
			ИЛИ РедактированиеРучногоРазмещения Тогда
			
			Если ДинамическаяСтруктура Тогда
				ОтборЗаписей = Новый Структура("ЗаказНаПроизводство, ЭтапПроизводства", Распоряжение, Ссылка);
				РегистрыСведений.ЗаданияКРасчетуСтруктурыЗаказаГрафикПроизводства.УдалитьЗаписиПоОтбору(ОтборЗаписей);
			Иначе
				ОтборЗаписей = Новый Структура("Распоряжение, ЭтапПроизводства", Распоряжение, Ссылка);
				РегистрыСведений.ЗаданияКРасчетуГрафикаПроизводства.УдалитьЗаписиПоОтбору(ОтборЗаписей);
			КонецЕсли;
	...
	Исключение
		Событие = НСтр("ru = 'Этап производства';
			...
			+ НСтр("ru = 'Заполнение кодов строк';
					|en = 'Fill in line codes'", ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(Событие,
			УровеньЖурналаРегистрации.Ошибка,
			Метаданные(),
			Ссылка,
		...
		ТекстСообщения = НСтр("ru = 'Не удалось заблокировать этапы, включенные в заказ переработчику.';
								|en = 'Cannot lock the stages included in the subcontracting purchase order.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка,,, Отказ);
		Возврат;
	КонецПопытки;
