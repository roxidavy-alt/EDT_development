...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПередачаКВыполнению = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ПередачаКВыполнению", Ложь);
	
	Если ПередачаКВыполнению Тогда
		
		КодДействияПоУмолчанию = "ДЕЙСТВИЕ_РЕЗЕРВИРОВАТЬ_ПРИОРИТЕТНО_ОБОСОБЛЕННО";
		Элементы.ГиперссылкаТекущееДействие.Доступность = Ложь;
	...
	Если ПустаяСтрока(КодДействия) Тогда
