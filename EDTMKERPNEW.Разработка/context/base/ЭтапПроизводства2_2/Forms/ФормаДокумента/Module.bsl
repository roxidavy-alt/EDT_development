...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ВозможнаОбработкаШтрихкодов = Не ОтражениеВыполненного
	...
	Если ВводДоступен()
		...
		Если ИмяСобытия = "ScanData"
			...
			Если ТипЗнч(ТекущийЭлемент) = Тип("ТаблицаФормы")
				И Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаРасходМатериаловИРабот Тогда
	...
	Если ВозможнаОбработкаШтрихкодов
		...
		ОбработатьШтрихкоды(ДанныеШтрихкодов, Параметр.ИмяТЧ);
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	ПараметрыЗаписи.Вставить("Подразделение", Объект.Подразделение);
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Если ПравоЧтенияАналогов И ПоказатьАналоги И Не Объект.ПланированиеРабот Тогда
...
&НаКлиенте
Процедура ЗаписатьИОбработатьНажатиеГиперссылкиОбеспечения(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура СпецификацияЭтапОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "#РедактироватьТехпроцесс" Тогда
		
		Если Модифицированность Тогда
			ПоказатьВопрос(
				Новый ОписаниеОповещения("НавигационнаяСсылкаРедактироватьТехпроцессЗавершение", ЭтотОбъект),
		...
		Иначе
			ОткрытьФормуРедактированияТехпроцесса();
		КонецЕсли;
...
Процедура ВыходныеИзделияДатаПроизводстваПриИзменении(Элемент)
	...
	СтруктураДействий = Новый Структура;
	ДобавитьВСтруктуруДействияКонтроляКачества("ВыходныеИзделия", СтруктураДействий, КонтрольКачестваКэш);
	ПакетнаяОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ТекущиеДанные, СтруктураДействий, КэшированныеЗначения);
...
&НаКлиенте
Процедура ВыходныеИзделияПроконтролироватьКачествоПриИзменении(Элемент)
...
&НаКлиенте
Процедура ВыходныеИзделияДатаПодтвержденияКачестваПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПобочныеИзделияДатаПроизводстваПриИзменении(Элемент)
	
	ТекущиеДанные = Элементы.ПобочныеИзделия.ТекущиеДанные;
	
	СтруктураДействий = Новый Структура;
	ДобавитьВСтруктуруДействияКонтроляКачества("ПобочныеИзделия", СтруктураДействий, КонтрольКачестваКэш);
	ПакетнаяОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ТекущиеДанные, СтруктураДействий, КэшированныеЗначения);
...
&НаКлиенте
Процедура ПобочныеИзделияПроконтролироватьКачествоПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПобочныеИзделияДатаПодтвержденияКачестваПриИзменении(Элемент)
...
Процедура ОбеспечениеМатериаламиИРаботамиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		Если Копирование
		   И СтруктураДействий.Свойство("ПроверитьЗаполнитьОбеспечениеВДокументеПродажи") Тогда
			...
			Если ТипЗнч(ПараметрыДействия) = Тип("Структура") 
			   И ПараметрыДействия.Свойство("ВариантОбеспеченияИФлагОбособленно") Тогда
				ПараметрыДействия.ВариантОбеспеченияИФлагОбособленно = ВариантОбеспеченияИФлагОбособленно;
			КонецЕсли;
...
Процедура ОбеспечениеМатериаламиИРаботамиПроизводитсяПриИзменении(Элемент)
	...
	ДобавитьДействиеПроверитьЗаполнитьСпецификацию(ЭтотОбъект, СтруктураДействий, ТекущиеДанные);
...
Процедура ОбеспечениеМатериаламиИРаботамиОтмененоПриИзменении(Элемент)
	...
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ПроверитьСериюРассчитатьСтатус", Новый Структура("Склад, ПараметрыУказанияСерий", ТекущиеДанные.Склад, ПараметрыУказанияСерий.ОбеспечениеМатериаламиИРаботами));
...
&НаКлиенте
Процедура ОбеспечениеМатериаламиИРаботамиРемонтируемоеИзделиеНоменклатураПриИзменении(Элемент)
	
	ТекущиеДанные = Элементы.ОбеспечениеМатериаламиИРаботами.ТекущиеДанные;
	ТекущиеДанныеСтруктура = УправлениеПроизводствомКлиентСервер.ДанныеСтрокиОбеспечениеМатериаламиИРаботамиСтруктурой(ТекущиеДанные);

	// переопределяем для использования стандартных действий
	ТекущиеДанныеСтруктура.Вставить("Номенклатура", ТекущиеДанные.РемонтируемоеИзделиеНоменклатура);
	ТекущиеДанныеСтруктура.Вставить("Характеристика", ТекущиеДанные.РемонтируемоеИзделиеХарактеристика);
	ТекущиеДанныеСтруктура.Вставить("ХарактеристикиИспользуются", ТекущиеДанные.РемонтируемоеИзделиеХарактеристикиИспользуются);
	ТекущиеДанныеСтруктура.Вставить("ТипНоменклатуры", ТекущиеДанные.РемонтируемоеИзделиеТипНоменклатуры);
	
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ПроверитьХарактеристикуПоВладельцу", ТекущиеДанные.РемонтируемоеИзделиеХарактеристика);
	СтруктураДействий.Вставить("ЗаполнитьПризнакТипНоменклатуры", Новый Структура("РемонтируемоеИзделиеНоменклатура", "РемонтируемоеИзделиеТипНоменклатуры"));
	СтруктураДействий.Вставить("ПроверитьСериюРассчитатьСтатус", Новый Структура("Склад, ПараметрыУказанияСерий", Неопределено, ПараметрыУказанияСерий.ОбеспечениеМатериаламиИРаботами));
	
	ПакетнаяОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ТекущиеДанныеСтруктура, СтруктураДействий, КэшированныеЗначения);
	
	ТекущиеДанные.РемонтируемоеИзделиеХарактеристика             = ТекущиеДанныеСтруктура.Характеристика;
	ТекущиеДанные.РемонтируемоеИзделиеХарактеристикиИспользуются = ТекущиеДанныеСтруктура.ХарактеристикиИспользуются;
	ТекущиеДанные.РемонтируемоеИзделиеСерия                      = ТекущиеДанныеСтруктура.РемонтируемоеИзделиеСерия;
	ТекущиеДанные.РемонтируемоеИзделиеСтатусУказанияСерий        = ТекущиеДанныеСтруктура.РемонтируемоеИзделиеСтатусУказанияСерий;
	ТекущиеДанные.РемонтируемоеИзделиеТипНоменклатуры            = ТекущиеДанныеСтруктура.РемонтируемоеИзделиеТипНоменклатуры;
...
&НаКлиенте
Процедура ОбеспечениеМатериаламиИРаботамиРемонтируемоеИзделиеХарактеристикаПриИзменении(Элемент)
...
&НаКлиенте
Процедура ОбеспечениеМатериаламиИРаботамиРемонтируемоеИзделиеСерияПриИзменении(Элемент)
	
	ВыбранноеЗначение = НоменклатураКлиентСервер.ВыбраннаяСерия();
	ТекущиеДанные = Элементы.ОбеспечениеМатериаламиИРаботами.ТекущиеДанные;
	
	ВыбранноеЗначение.Значение                   = ТекущиеДанные.РемонтируемоеИзделиеСерия;
	ВыбранноеЗначение.ИдентификаторТекущейСтроки = ТекущиеДанные.ПолучитьИдентификатор();
	
	ТекущаяСтрока = Новый Структура;
	ТекущаяСтрока.Вставить("ВариантОбеспечения", ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.НеТребуется"));
	ТекущаяСтрока.Вставить("Отменено", ТекущиеДанные.Отменено);
	ТекущаяСтрока.Вставить("Серия", ТекущиеДанные.РемонтируемоеИзделиеСерия);
	ТекущаяСтрока.Вставить("СтатусУказанияСерий", ТекущиеДанные.РемонтируемоеИзделиеСтатусУказанияСерий);
	
	НоменклатураКлиент.ОбработатьУказаниеСерии(ЭтотОбъект, ПараметрыУказанияСерий.ОбеспечениеМатериаламиИРаботами, ВыбранноеЗначение, ТекущаяСтрока);
	
	ТекущиеДанные.РемонтируемоеИзделиеСерия               = ТекущаяСтрока.Серия;
	ТекущиеДанные.РемонтируемоеИзделиеСтатусУказанияСерий = ТекущаяСтрока.СтатусУказанияСерий;
...
&НаКлиенте
Процедура Производство_РедактироватьТехпроцесс(Команда)
...
Процедура Производство_ВыполнениеОпераций(Команда)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ЗаписатьИОткрытьВыполнениеОперацийЗавершение", ЭтотОбъект),
			НСтр("ru = 'Необходимо записать документ для выполнения операции. Записать документ?';
				|en = 'You need to save the document before running the operation. Do you want to save it?'"),
			РежимДиалогаВопрос.ДаНет);
...
&НаКлиенте
Процедура ЗаписатьИОткрытьВыполнениеОперацийЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура Производство_ПроизводственныеОперации(Команда)
	
	ОткрытьФорму(
		"Документ.ПроизводственнаяОперация2_2.ФормаСписка",
		Новый Структура("ЭтапОтбор", Объект.Ссылка),
		ЭтотОбъект,
		УникальныйИдентификатор);
...
Процедура Производство_ЗаполнитьПоСпецификации(Команда)
	...
	Если Объект.ПланированиеРабот Тогда
		ЗаполнитьПоСпецификацииНаСервере(Новый Структура("ОбеспечениеМатериаламиИРаботами"));
	Иначе
...
Процедура Обеспечение_ПодобратьПоСпецификации(Команда)
	...
	ПараметрыФормы.Вставить("ПромежуточныеПолуфабрикаты", УправлениеДаннымиОбИзделияхКлиентСервер.ПромежуточныеПолуфабрикатыПолучить());
...
Процедура ВыходныеИзделия_ЗаполнитьЦеныПоКалькуляции(Команда)
	...
	СтруктураОтбора.Вставить("ПодразделениеДиспетчер", Объект.Подразделение);
	СтруктураОтбора.Вставить("Изделие", ТекущиеДанные.Номенклатура);
	ОбъектыКалькуляции = Новый Массив();
	ОбъектыКалькуляции.Добавить(Объект.Распоряжение);
	Если ЗначениеЗаполнено(Объект.Спецификация) Тогда
		ОбъектыКалькуляции.Добавить(Объект.Спецификация);
	КонецЕсли;
	СтруктураОтбора.Вставить("ОбъектыКалькуляции", ОбъектыКалькуляции);
	СтруктураОтбора.Вставить("Характеристика", ТекущиеДанные.Характеристика);
	СтруктураОтбора.Вставить("Назначение", ТекущиеДанные.Назначение);
...
Процедура ЗаполнитьПолучателяНаСервере(ПараметрыТЧ, ВыделенныеСтроки, Получатель)
	...
	СтруктураДействий = Новый Структура;
	ДобавитьВСтруктуруДействияКонтроляКачества(ИмяТЧОбъект, СтруктураДействий, КонтрольКачестваКэш);
...
Процедура УстановитьУсловноеОформлениеОбеспечение()
	...
	ПутиКДанным.Вставить("ПланированиеРабот", "Объект.ПланированиеРабот");
	...
	ИменаЭлементов.ТЧРемонтируемоеИзделиеНоменклатура = Элементы.ОбеспечениеМатериаламиИРаботамиРемонтируемоеИзделиеНоменклатура.Имя;
	ИменаЭлементов.ТЧРемонтируемоеИзделиеХарактеристика = Элементы.ОбеспечениеМатериаламиИРаботамиРемонтируемоеИзделиеХарактеристика.Имя;
	ИменаЭлементов.ТЧРемонтируемоеИзделиеСерия = Элементы.ОбеспечениеМатериаламиИРаботамиРемонтируемоеИзделиеСерия.Имя;
	
	ПроизводствоУправлениеФормами.УстановитьУсловноеОформлениеОбеспеченияМатериаламиИРаботами(
		ЭтотОбъект, "Объект", "ОбеспечениеМатериаламиИРаботами", ИменаЭлементов, ПутиКДанным);
	...
	// Отметка незаполненного у спецификации
	...
	Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ПланированиеРабот");
	Отбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	Отбор.ПравоеЗначение = Ложь;
	
	Отбор = ГруппаОтбораИЛИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ОбеспечениеМатериаламиИРаботами.Производится");
	Отбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	Отбор.ПравоеЗначение = Ложь;
	
	Отбор = ГруппаОтбораИЛИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ОбеспечениеМатериаламиИРаботами.Отменено");
	Отбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	Отбор.ПравоеЗначение = Истина;
	
	Элемент.Оформление.УстановитьЗначениеПараметра("ОтметкаНезаполненного", Ложь);
	
	// Запрет редактирования у спецификации
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<указывается для производимых работ>';
																|en = '<specified for works performed>'"));
...
Процедура УстановитьУсловноеОформлениеВыходныеИзделия()
	...
	ИменаЭлементовПобочныеИзделия.Вставить("ВладелецИзделия", Элементы.ПобочныеИзделияВладелецИзделия.Имя);
	...
	ВнутренняяПереработкаСервер.УстановитьУсловноеОформлениеПоляВладелецИзделия(
		ЭтотОбъект,
		"Объект",
		"ПобочныеИзделия",
		ИменаЭлементовПобочныеИзделия);
...
Процедура НастроитьЭлементыФормы()
	...
	// Видимость ремонтируемого изделия
	Элементы.ОбеспечениеМатериаламиИРаботамиГруппаРемонтируемоеИзделие.Видимость =
		Объект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот
			И Объект.ПланированиеРабот;
	...
	Элементы.ВыходныеИзделияГруппаСпецификация.Видимость = ЭтоРазборка
		И (Объект.ВыходныеИзделия.НайтиСтроки(СтруктураПоиска).Количество() ИЛИ НЕ Объект.ДинамическаяСтруктура);
	Элементы.ПобочныеИзделияГруппаСпецификация.Видимость = ЭтоРазборка
		И (Объект.ПобочныеИзделия.НайтиСтроки(СтруктураПоиска).Количество() ИЛИ НЕ Объект.ДинамическаяСтруктура);
	
	Элементы.СтраницаОбеспечениеМатериаламиИРаботами.Заголовок = Документы.ЭтапПроизводства2_2.ПредставлениеТЧОбеспечение(
		Объект, ОтражениеВыполненного);
	...
	Если ОтражениеВыполненного Тогда
		...
		Элементы.УказатьСерииИзрасходованныхМатериалов.Видимость = Истина;
	...
	Если Объект.ПланированиеРабот Тогда
		...
		ДоступныеТипыНоменклатуры = ПроизводствоСервер.МассивЗначений(Перечисления.ТипыНоменклатуры.Работа);
		
		УправлениеДаннымиОбИзделияхКлиентСервер.УстановитьОтборПоТипуНоменклатуры(
			Элементы.ОбеспечениеМатериаламиИРаботамиНоменклатура,
			ДоступныеТипыНоменклатуры);
		
		Элементы.Обеспечение_ПодобратьТовары.Заголовок = НСтр("ru = 'Подобрать работы';
																|en = 'Pick works'");
		Элементы.ОбеспечениеМатериаламиИРаботамиПроизводится.Заголовок = НСтр("ru = 'Произвести';
																				|en = 'Produce'");
		Элементы.ОбеспечениеМатериаламиИРаботамиСклад.Видимость = Ложь;
		Элементы.Обеспечение_ЗаполнитьСкладВВыделенныхСтроках.Видимость = Ложь;
		
	КонецЕсли;
	
	Если Не ПравоДоступа("Добавление", Метаданные.Документы.КорректировкаНазначенияТоваров)
		Или Объект.ПланированиеРабот Тогда
	...
	Элементы.ОбеспечениеМатериаламиИРаботамиПроизводится.Видимость = 
		Объект.ТипПроизводственногоПроцесса <> Перечисления.ТипыПроизводственныхПроцессов.БезСпецификаций
		И Не Объект.ДинамическаяСтруктура
		ИЛИ Объект.ПланированиеРабот;
	
	Элементы.ОбеспечениеМатериаламиИРаботамиГруппаАналоги.Видимость = ПравоЧтенияАналогов
		И Не Объект.ПланированиеРабот;
	
	Элементы.ОбеспечениеМатериаламиИРаботамиВходитВНабор.Видимость =  ДоступноИспользованиеНаборовВПроизводстве;
	
	Элементы.ОбеспечениеМатериаламиИРаботамиСпецификация.Видимость = 
		Объект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Сборка
		ИЛИ Объект.ПланированиеРабот
		ИЛИ Объект.ТипПроизводственногоПроцесса <> Перечисления.ТипыПроизводственныхПроцессов.БезСпецификаций
			И Не Объект.ДинамическаяСтруктура;
	...
	ЭлементыКОбработке.Добавить("ВнутренняяПереработка");
	ЭлементыКОбработке.Добавить("ДекорацияОтступВнутренняяПереработка");
	ЭлементыКОбработке.Добавить("ОрганизацияДавалец");
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если Инициализация
		ИЛИ СтруктураРеквизитов.Свойство("НаименованиеЭтапа") Тогда
	...
	Если Инициализация
		...
		Если Объект.ТипПроизводственногоПроцесса = ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.Сборка")
			...
			Элементы.ГруппаСпособПроизводства.Видимость =
				(Форма.ИспользуетсяПроизводствоНаСтороне Или Форма.ИспользоватьЗаказыПереработчикам2_5)
				И Не Объект.ПланированиеРабот
	...
	Если Инициализация
		ИЛИ СтруктураРеквизитов.Свойство("СпособПроизводства")  Тогда
	...
	Если СтруктураРеквизитов.Свойство("СпособПроизводства") 
		...
		ВидимостьЭлементов = НЕ Форма.Объект.ПроизводствоНаСтороне
							И НЕ Форма.ОтражениеВыполненного
							И НЕ Форма.Объект.ПланированиеРабот
	...
	Если СтруктураРеквизитов.Свойство("ЭтоВыпускающийЭтап")
		...
		Элементы.СтраницаВыходныеИзделия.Видимость = НЕ Форма.Объект.ПланированиеРабот;
...
//		ИЛИ СтруктураРеквизитов.Свойство("ПобочныеИзделия")
	...
	// Количество строк в ТЧ
	Если Инициализация ИЛИ СтруктураРеквизитов.Свойство("Трудозатраты") Тогда
	...
	Если Инициализация
		ИЛИ СтруктураРеквизитов.Свойство("СпособПроизводства")
		ИЛИ СтруктураРеквизитов.Свойство("Подразделение") Тогда
		
		Элементы.СтраницаТрудозатраты.Видимость = НЕ Форма.Объект.ПроизводствоНаСтороне И НЕ Форма.Объект.ПланированиеРабот;
	...
	Если СтруктураРеквизитов.Свойство("Статус")
		...
		Элементы.Производство_ОтразитьБрак.Видимость = Форма.ЕстьПравоНаДелениеПартии
			И Объект.Статус <> СтатусЗавершен();
	...
	Если Инициализация ИЛИ СтруктураРеквизитов.Свойство("НачатоВыполнениеЭтапа") Тогда
	...
	Если Инициализация ИЛИ СтруктураРеквизитов.Свойство("ЗавершеноВыполнениеЭтапа") Тогда
	...
	Если Инициализация ИЛИ СтруктураРеквизитов.Свойство("СпособПроизводства") Тогда
		
		Элементы.ГруппаВыполнение.Видимость = 
			НЕ Объект.ТипПроизводственногоПроцесса = ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.БезСпецификаций")
			И НЕ Объект.ПланированиеРабот;
	...
	Если СтруктураРеквизитов.Свойство("Спецификация")
		...
		ЕстьДанныеСпецификации = НЕ Форма.Объект.Спецификация.Пустая()
			И НЕ Форма.Объект.Этап.Пустая();
			
		Элементы.ФормаЗаполнитьПоСпецификации.Видимость        = ЕстьДанныеСпецификации;
		Элементы.Обеспечение_ПоказатьНормативы.Видимость       = ЕстьДанныеСпецификации;
		Элементы.Трудозатраты_ПоказатьНормативы.Видимость      = ЕстьДанныеСпецификации;
		Элементы.Обеспечение_ПодобратьПоСпецификации.Видимость = ЕстьДанныеСпецификации И НЕ Форма.Объект.ПланированиеРабот;
	...
	Если СтруктураРеквизитов.Свойство("Подразделение")
		...
		// видимость элементов
		ВидимостьЭлементов = ВидимостьЭлементовПооперационногоУправления(Форма);
		...
		Элементы.ФормаПроизводственныеОперации.Видимость = ВидимостьЭлементов;
		Элементы.РедактироватьТехпроцесс.Видимость       = ВидимостьЭлементов;
		
		// доступность элементов
		ДоступностьЭлементов = ДоступностьЭлементовПооперационногоУправления(Форма);
		
		Элементы.ФормаЗаполнитьПоОперациям.Доступность     = ДоступностьЭлементов;
		Элементы.ФормаВыполнениеОпераций.Доступность       = ДоступностьЭлементов И НЕ Форма.Объект.Статус = СтатусЗавершен();
		Элементы.ФормаСменныеЗадания.Доступность           = ДоступностьЭлементов;
		Элементы.ФормаПроизводственныеОперации.Доступность = ДоступностьЭлементов;
		Элементы.РедактироватьТехпроцесс.Доступность       = ДоступностьЭлементов;
	...
	Если СтруктураРеквизитов.Свойство("ПоказатьАналоги") ИЛИ Инициализация Тогда
		...
		Элементы.ОбеспечениеМатериаламиИРаботамиЕстьАналогиМатериала.Видимость = Форма.ПравоЧтенияАналогов
			И НЕ Объект.ПланированиеРабот
	...
	Если СтруктураРеквизитов.Свойство("КоличествоУпаковокПлан")
		...
		Элементы.Производство_СократитьУвеличить.Видимость = Форма.ЕстьПравоНаДелениеПартии
		...
		Элементы.Производство_Отменить.Видимость = Форма.ЕстьПравоНаДелениеПартии
			И Объект.НомерСледующегоЭтапа <> 0
	...
	Если СтруктураРеквизитов.Свойство("КоличествоУпаковокОтменено")
		...
		Элементы.Производство_СоздатьНовыйЭтап.Видимость = Форма.ЕстьПравоНаДобавление
...
Процедура НастроитьПравоРедактированияРеквизитовГруппыЗапланировано(Форма)
	...
	ПравоРедактироватьЗапланировано = НЕ Форма.ОперацииНазначены
		И НЕ Форма.ТипТехнологическогоПроцесса = ПредопределенноеЗначение("Перечисление.ТипыТехнологическихПроцессовЭтапа.Индивидуальный");
...
Процедура УстановитьВидимостьЭлементовСерий()
	...
	Элементы.ОбеспечениеМатериаламиИРаботамиСерия.Видимость = ПараметрыУказанияСерий.ОбеспечениеМатериаламиИРаботами.ИспользоватьСерииНоменклатуры
		И НЕ Объект.ПланированиеРабот; // в ТЧ только работы
	...
	Элементы.УказатьСерииИзрасходованныхМатериалов.Видимость = ПараметрыУказанияСерий.ОбеспечениеМатериаламиИРаботами.ИспользоватьСерииНоменклатуры
	...
	Элементы.ОбеспечениеМатериаламиИРаботамиРемонтируемоеИзделиеСерия.Видимость = ПараметрыУказанияСерий.ОбеспечениеМатериаламиИРаботами.ИспользоватьСерииНоменклатуры
		И Объект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот
			И Объект.ПланированиеРабот;
...
Процедура ОбработатьИзменениеСпособаПроизводстваНаСервере()
	...
	Если КонтрольКачества.ОбновитьПараметрыДействий(Документы.ЭтапПроизводства2_2, Объект, КонтрольКачестваКэш) Тогда
		СтруктураДействий = Новый Структура();
		СтруктураДействий.Вставить("ЗаполнитьПроконтролироватьКачествоДатаПодтвержденияКачества", КонтрольКачестваКэш);
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Объект.ВыходныеИзделия, СтруктураДействий);
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Объект.ПобочныеИзделия, СтруктураДействий);
	КонецЕсли;
...
#Область ИзменитьТехпроцесс

&НаСервереБезКонтекста
Процедура РазблокироватьДанныеДляРедактированияНаСервере(Ссылка, УникальныйИдентификатор)
...
&НаКлиенте
Процедура НавигационнаяСсылкаРедактироватьТехпроцессЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ОткрытьФормуРедактированияТехпроцесса()
	
	Если НЕ Модифицированность Тогда
		РазблокироватьДанныеДляРедактированияНаСервере(Объект.Ссылка, УникальныйИдентификатор);
	КонецЕсли;
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Этап", Объект.Ссылка);
	ПараметрыФормы.Вставить("Режим", РедакторПроизводственногоПроцессаКлиентСервер.РежимИзФормыЭтапа());
...
Процедура ОткрытьФормуРедактированияТехпроцессаЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗакрытия <> Неопределено Тогда
		...
		ОткрытьФормуРедактированияТехпроцессаЗавершениеНаСервере(РезультатЗакрытия);
...
&НаСервере
Процедура ОткрытьФормуРедактированияТехпроцессаЗавершениеНаСервере(РезультатЗакрытия)
	
	СписокРеквизитов = Новый Массив;
	СписокРеквизитов.Добавить("ТехнологическийПроцесс");
	
	Если РезультатЗакрытия.Свойство("АдресДанныхЭтапаДляОбновления")
		И ЭтоАдресВременногоХранилища(РезультатЗакрытия.АдресДанныхЭтапаДляОбновления) Тогда
		
		ДанныеДляОбновления = ПолучитьИзВременногоХранилища(РезультатЗакрытия.АдресДанныхЭтапаДляОбновления); // Структура
		
		Если ЗначениеЗаполнено(ДанныеДляОбновления) Тогда
			
			Для каждого КлючИЗначение Из ДанныеДляОбновления Цикл
				
				ИмяТЧ = КлючИЗначение.Ключ;
				
				Объект[ИмяТЧ].Загрузить(КлючИЗначение.Значение);
				
				ПослеИзмененияСтрокТЧПрограммноНаСервере(ИмяТЧ);
				
				Модифицированность = Истина;
				
				СписокРеквизитов.Добавить(ИмяТЧ);
	...
	НастроитьЗависимыеЭлементыФормы(ЭтотОбъект, СтрСоединить(СписокРеквизитов, ","));
...
&НаКлиентеНаСервереБезКонтекста
Функция ВидимостьЭлементовПооперационногоУправления(Форма)
	
	Возврат Форма.ИспользоватьПооперационноеУправление
		И НЕ Форма.Объект.ПроизводствоНаСтороне
		И НЕ Форма.Объект.ПланированиеРабот
		И НЕ Форма.ОтражениеВыполненного
		И НЕ Форма.Объект.ТипПроизводственногоПроцесса = ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.БезСпецификаций");
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ДоступностьЭлементовПооперационногоУправления(Форма)
	
	ПроверяемыеСтатусы = УправлениеПроизводствомКлиентСервер.СтатусыДоступноПооперационноеУправление(Форма);
	Возврат ПроверяемыеСтатусы.Найти(Форма.Объект.Статус) <> Неопределено;
...
Функция ДоступноПланированиеПоПроизводственнымРесурсам(Форма)
	...
	Возврат (Форма.ИспользуетсяПланированиеПоПроизводственнымРесурсам Или Форма.Объект.ДинамическаяСтруктура)
			И НЕ Форма.Объект.ПланированиеРабот
			И НЕ Форма.Объект.ПроизводствоНаСтороне;
...
Процедура Подключаемый_ПроверитьВыполнениеЗаданияРаспределенияЗапасов()
	...
	Если ОбеспечениеВДокументахКлиент.НужноОбновитьДоступностьПослеЗаписи(ЭтотОбъект,
		...
		ОтразитьАналогиВГиперссылкеСостоянияОбеспечения(ЭтотОбъект);
...
Функция ОбеспечениеУстановитьДействие(КодДействия, УстановитьДействиеДляВыделенныхСтрок = Истина)
	...
	Если КодДействия = "ФЛАГ_ОБОСОБЛЕННО" Или КодДействия = "ФЛАГ_НЕОБОСОБЛЕННО" Тогда
		...
		Для Каждого Изменение Из Изменения.ТаблицаИзменений Цикл
			ИзмененныеСтроки.Добавить(Изменение.Строка);
		КонецЦикла;
...
Процедура ГиперссылкаСостояниеОбеспеченияНажатиеОбщий()
	...
	Если ЕстьАналоги И ПравоЧтенияАналогов И Не Объект.ПланированиеРабот Тогда
		
		ПараметрыОткрытия = ПараметрыФормыПомощникаВыполненияЗамен();
		
		Оповещение = Новый ОписаниеОповещения("ПослеЗаменыЧерезПомощник", ЭтотОбъект);
		
		ОткрытьФорму("Документ.РазрешениеНаЗаменуМатериалов.Форма.ПомощникВыполненияЗамен", ПараметрыОткрытия, ЭтотОбъект, УникальныйИдентификатор,,,
			Оповещение,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		
	Иначе	
		
		ОткрытьСостояниеОбеспеченияЗаказов();
...
Процедура УстановитьВидимостьОбеспечения()
	...
	Иначе
		...
		Элементы.ОбеспечениеМатериаламиИРаботамиОбеспечениеРезервировать.Видимость =
			Не Объект.ПланированиеРабот;
		Элементы.ОбеспечениеМатериаламиИРаботамиКонтекстноеМенюОбеспечениеРезервировать.Видимость =
			Не Объект.ПланированиеРабот;
		...
		Элементы.ОбеспечениеМатериаламиКонтекстноеМенюОбеспечениеРезервироватьПоМереПоступления.Видимость =
			Не Объект.ДинамическаяСтруктура;
...
Процедура ПослеИзмененияСтрокТЧПрограммноНаСервере(ИмяТЧ, МассивИдентификаторовСтрок = Неопределено, СтруктураРанееВыполненныхДействий = Неопределено)
	...
	ИначеЕсли ИмяТЧ = "ЭкономияМатериалов" Тогда
		...
		Если ПодразделениеИзменено Тогда
			Если Объект.ЭкономияМатериаловСерии.Количество() > 0 Тогда
				ОтборСтрок = Новый Структура("Подразделение", Справочники.СтруктураПредприятия.ПустаяСсылка());
				ЗаполняемыеСтроки = Объект.ЭкономияМатериаловСерии.НайтиСтроки(ОтборСтрок);
				
				Для Каждого СтрокаТЧ Из ЗаполняемыеСтроки Цикл
	...
	ДобавитьВСтруктуруДействияКонтроляКачества(ИмяТЧ, СтруктураДействий, КонтрольКачестваКэш);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ДобавитьДействиеПроверитьЗаполнитьСпецификацию(Форма, СтруктураДействий, СтрокаТабличнойЧасти)
	
	Если НЕ СтрокаТабличнойЧасти.Производится Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыЗаполнения = УправлениеДаннымиОбИзделияхКлиентСервер.ПараметрыДействияПроверитьЗаполнитьСпецификацию(
		Форма.Объект, СтрокаТабличнойЧасти, Форма.ПараметрыВыбораСпецификаций.ОбеспечениеМатериаламиИРаботами, Форма);
...
Процедура ПолучитьСтрокиИзБуфераОбменаНаСервере(ИмяТЧ)
	...
	Если ПодборТоваров Тогда
		...
		Если НЕ Объект.ПланированиеРабот Тогда
...
Процедура ОтметитьНаличиеАналогов()
	...
	АналогиМатериалов.ОтметитьНаличиеАналогов(Объект[ИмяТаблицы], ПараметрыВыбораАналогов(ИмяТаблицы, 0), ЭтотОбъект);
	ОтразитьАналогиВГиперссылкеСостоянияОбеспечения(ЭтотОбъект);
...
Процедура ОбработатьШтрихкодыСервер(СтруктураПараметровДействия, КэшированныеЗначения)
	...
	Если СтруктураПараметровДействия.ТекущаяСтрока <> Неопределено Тогда
		МассивСтрок = ОбщегоНазначенияУТКлиентСервер.Массив(СтруктураПараметровДействия.ТекущаяСтрока, Ложь);
		
		ПослеИзмененияСтрокТЧПрограммноНаСервере(
			СтруктураПараметровДействия.ИмяТЧ,
			МассивСтрок,
...
Процедура ОткрытьПодборТоваров(ИмяТЧ)
	...
	Если Объект.ПланированиеРабот Тогда
		ПараметрЗаголовок = НСтр("ru = 'Подбор работ в %1';
								|en = 'Pick works in %1'");
	ИначеЕсли ИмяТЧ = "ОбеспечениеМатериаламиИРаботами" ИЛИ ИмяТЧ = "РасходМатериаловИРабот" Тогда
	...
	Если Объект.ПланированиеРабот Тогда
		ТипыНоменклатуры = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
			ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Работа"));
	Иначе
		ДоступныНаборы = ДоступноИспользованиеНаборовВПроизводстве
			И ИмяТЧ = "ОбеспечениеМатериаламиИРаботами";
		ТипыНоменклатуры = НоменклатураКлиентСервер.ОтборПоТоваруМногооборотнойТареРаботе(ДоступныНаборы);
	КонецЕсли;
...
Процедура НачатьДобавлениеСоставаНаборовВРезультатПодбора(ТаблицаТоваров, ВариантПодбора, ИмяТЧ = "ОбеспечениеМатериаламиИРаботами")
	...
	ПараметрыПолученияСостава.Изделие                  = ДанныеПартииПроизводства.ОсновноеИзделиеНоменклатура;
	ПараметрыПолученияСостава.ХарактеристикаИзделия    = ДанныеПартииПроизводства.ОсновноеИзделиеХарактеристика;
	ПараметрыПолученияСостава.СпецификацияИзделия      = Объект.Спецификация;
	ПараметрыПолученияСостава.ЭтапСпецификацииИзделия  = Объект.Этап;
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	УправлениеДаннымиОбИзделиях.УстановитьПараметрыВыбораСпецификаций(Элементы.ОбеспечениеМатериаламиИРаботамиСпецификация, ПараметрыВыбораСпецификаций.ОбеспечениеМатериаламиИРаботами);
	УправлениеДаннымиОбИзделиях.УстановитьПараметрыВыбораСпецификаций(Элементы.ВыходныеИзделияСпецификация, ПараметрыВыбораСпецификаций.ВыходныеИзделия);
	УправлениеДаннымиОбИзделиях.УстановитьПараметрыВыбораСпецификаций(Элементы.ПобочныеИзделияСпецификация, ПараметрыВыбораСпецификаций.ВыходныеИзделия);
	...
	ИспользоватьВнутреннююПереработку =
		ПолучитьФункциональнуюОпцию("ИспользоватьВнутреннююПереработку")
		И Не Объект.ПланированиеРабот;
...
Процедура ПрочитатьВнешниеРеквизиты()
	...
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	Техпроцесс.ТипТехнологическогоПроцесса                                                                     КАК ТипТехнологическогоПроцесса,
		|	Техпроцесс.ТехнологическийПроцесс                                                                          КАК ТехнологическийПроцесс,
		|	Техпроцесс.КоэффициентТехнологическогоПроцесса                                                             КАК КоэффициентТехнологическогоПроцесса,
		|	Техпроцесс.ИдентификаторПервойОперации                                                                     КАК ИдентификаторПервойОперации,
		|	Техпроцесс.ИдентификаторПоследнейОперации                                                                  КАК ИдентификаторПоследнейОперации,
		|	ЕСТЬNULL(ПерваяОперация.Операция.Ссылка, ПерваяОперация.ПроизводственнаяОперация)                          КАК ПерваяОперация,
		|	ЕСТЬNULL(ПерваяОперация.Операция.Наименование, ПерваяОперация.ПроизводственнаяОперация.Наименование)       КАК ПерваяОперацияПредставление,
		|	ЕСТЬNULL(ПоследняяОперация.Операция.Ссылка, ПоследняяОперация.ПроизводственнаяОперация)                    КАК ПоследняяОперация,
		|	ЕСТЬNULL(ПоследняяОперация.Операция.Наименование, ПоследняяОперация.ПроизводственнаяОперация.Наименование) КАК ПоследняяОперацияПредставление
		|ИЗ
		...
		|	ПО ДокЭтап.Ссылка = Техпроцесс.Этап
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОчередьПроизводственныхОпераций КАК ПерваяОперация
		|	ПО ДокЭтап.Ссылка = ПерваяОперация.Этап
		|		И Техпроцесс.ИдентификаторПервойОперации = ПерваяОперация.ИдентификаторОперации
		|		И ПерваяОперация.ОсновнаяЗапись
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОчередьПроизводственныхОпераций КАК ПоследняяОперация
		|	ПО ДокЭтап.Ссылка = ПоследняяОперация.Этап
		|		И Техпроцесс.ИдентификаторПоследнейОперации = ПоследняяОперация.ИдентификаторОперации
		|		И ПоследняяОперация.ОсновнаяЗапись
		|
		|ГДЕ
		|	ДокЭтап.Ссылка = &Этап
		|
	...
	ВыборкаДанныеТехпроцесса = РезультатыЗапроса[0].Выбрать();
	Если ВыборкаДанныеТехпроцесса.Следующий() Тогда
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, ВыборкаДанныеТехпроцесса);
	КонецЕсли;
	
	Если (ЗначениеЗаполнено(ИдентификаторПервойОперации)
			И НЕ ЗначениеЗаполнено(ПерваяОперация))
		ИЛИ (ЗначениеЗаполнено(ИдентификаторПоследнейОперации)
			И НЕ ЗначениеЗаполнено(ПоследняяОперация)) Тогда
		
		Операции = РегистрыСведений.ОчередьПроизводственныхОпераций.СписокОперацийПоДаннымЭтапа(
			Документы.ЭтапПроизводства2_2.ДанныеДляРасчетаОчередиОпераций(Объект.Ссылка, Ложь)[Объект.Ссылка]);
		
		СтрокаОперация = Операции.Найти(ИдентификаторПервойОперации, "ИдентификаторОперации");
		Если СтрокаОперация <> Неопределено Тогда
			ПерваяОперация = СтрокаОперация.Операция;
			ПерваяОперацияПредставление = СтрокаОперация.ОперацияПредставление;
		КонецЕсли;
		
		СтрокаОперация = Операции.Найти(ИдентификаторПоследнейОперации, "ИдентификаторОперации");
		Если СтрокаОперация <> Неопределено Тогда
			ПоследняяОперация = СтрокаОперация.Операция;
			ПоследняяОперацияПредставление = СтрокаОперация.ОперацияПредставление;
		КонецЕсли;
...
Процедура ЗаполнитьСлужебныеРеквизиты()
	...
	ЕстьПравоНаДелениеПартии = ПравоДоступа("Использование", Метаданные.Обработки.ДелениеПартииПроизводства);
	ЕстьПравоНаДобавление = ПравоДоступа("Добавление", Метаданные.Документы.ЭтапПроизводства2_2);
...
Процедура ЗаполнитьСлужебныеРеквизитыТабличныхЧастей(СписокТЧ = "", СтрокиЗаполнения = Неопределено)
	...
	Если СтруктураТЧ.Свойство("ОбеспечениеМатериаламиИРаботами") ИЛИ ВсеТЧ Тогда
		...
		Если Объект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.ВыполнениеРемонтныхРабот
			И Объект.ПланированиеРабот Тогда
			СтруктураДействий.ЗаполнитьПризнакХарактеристикиИспользуются.Вставить("РемонтируемоеИзделиеНоменклатура", "РемонтируемоеИзделиеХарактеристикиИспользуются");
			СтруктураДействий.ЗаполнитьПризнакТипНоменклатуры.Вставить("РемонтируемоеИзделиеНоменклатура", "РемонтируемоеИзделиеТипНоменклатуры");
		КонецЕсли;
		...
		ПроизводствоСервер.ЗаполнитьПризнакЦеховаяКладоваяВТаблице(Объект.ОбеспечениеМатериаламиИРаботами, "Склад",, СтрокиЗаполнения);
		
		Если ПравоЧтенияАналогов И ПоказатьАналоги И Не Объект.ПланированиеРабот Тогда
			ОтметитьНаличиеАналогов();
		КонецЕсли;
		
		ОбеспечениеВДокументахСервер.ЗаполнитьСлужебныеРеквизитыФормыДокумента(
			ПараметрыДокументаДляДействийОбеспечения(ЭтотОбъект));
...
Функция ФорматированнаяСтрокаНадписьСпецификация()
	...
	Если Не Объект.Спецификация.Пустая() Тогда
		...
		Если ЗначениеЗаполнено(ПерваяОперация) Тогда
			НоваяСтрока = Таблица.Добавить();
		...
		Если ЗначениеЗаполнено(ПоследняяОперация) Тогда
			НоваяСтрока = Таблица.Добавить();
	...
	Если ВидимостьЭлементовПооперационногоУправления(ЭтотОбъект)
			И ПравоДоступа("Использование", Метаданные.Обработки.РедакторПроизводственногоПроцесса)
			И Объект.Проведен Тогда
		МассивСтрок.Добавить("  ");
		Если Объект.Статус = СтатусЗавершен() Тогда
			ТекстГиперссылки = НСтр("ru = 'показать техпроцесс';
									|en = 'show technological process'");
		Иначе
			ТекстГиперссылки = НСтр("ru = 'изменить техпроцесс';
									|en = 'edit technological process'");
		КонецЕсли;
		Если ТипТехнологическогоПроцесса = Перечисления.ТипыТехнологическихПроцессовЭтапа.Альтернативный Тогда
			ТекстГиперссылки = СтрШаблон("%1 (%2)", ТекстГиперссылки, ТехнологическийПроцесс);
		ИначеЕсли ТипТехнологическогоПроцесса = Перечисления.ТипыТехнологическихПроцессовЭтапа.Индивидуальный Тогда
			ТекстГиперссылки = СтрШаблон("%1 (%2)", ТекстГиперссылки, НСтр("ru = 'индивидуальный';
																			|en = 'individual'"));
		КонецЕсли;
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(ТекстГиперссылки,,,, "#РедактироватьТехпроцесс"));
	КонецЕсли;
...
Процедура ПриИзмененииНоменклатуры(СтрокаТабличнойЧасти, ИмяТЧ)
	...
	Если ИмяТЧ = "ОбеспечениеМатериаламиИРаботами" Тогда
		...
		ДобавитьДействиеПроверитьЗаполнитьСпецификацию(ЭтотОбъект, СтруктураДействий, СтрокаТабличнойЧасти);
		ОбеспечениеВДокументахКлиентСервер.ДобавитьДействияОбеспечения(
			СтруктураДействий,
			"ДоступноВДругихСтроках",
			ПараметрыДокументаДляДействийОбеспечения(ЭтотОбъект));
	...
	УправлениеПроизводствомКлиентСервер.ДобавитьВСтруктуруДействияКонтроляКачества(
		ИмяТЧ,
		СтруктураДействий,
		КонтрольКачестваКэш);
...
Процедура ПриИзмененииФлагаПроизведено(Знач ИмяТЧ)
	...
	Если ИмяТЧ = "ПобочныеИзделия" Или ИспользоватьПлановуюСтоимость Тогда
		Если ИмяТЧ = "ВыходныеИзделия" И ТекущиеДанные.Цена = 0 Тогда
			// Если цена установлена, изменение флага Произведено не должно ее менять.
			СтруктураДействий.Вставить("ПроверитьЗаполнитьЦенуВыпуска", ПараметрыЗаполненияЦенВыпуска(ЭтотОбъект));
		КонецЕсли;
		СтруктураДействий.Вставить("ПересчитатьСумму");
	КонецЕсли;
...
Процедура ДобавитьВСтруктуруДействияПриИзмененииНоменклатуры(ИмяТЧ, Форма, СтруктураДействий, ОбработкаПодбора = Ложь)
	...
	УправлениеПроизводствомКлиентСервер.ДобавитьВСтруктуруДействияКонтроляКачества(
		ИмяТЧ,
		СтруктураДействий,
		Форма.КонтрольКачестваКэш);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ДобавитьВСтруктуруДействияКонтроляКачества(ИмяТЧ, СтруктураДействий, КонтрольКачестваКэш)
...
Процедура ЗаполнитьСтруктурыДействийДляОбработкиШтрихкодов(ИмяТЧ, Форма, СтруктураДействийСДобавленнымиСтроками, СтруктураДействийСИзмененнымиСтроками)
	...
	ДобавитьВСтруктуруДействияКонтроляКачества(ИмяТЧ, СтруктураДействийСДобавленнымиСтроками, Форма.КонтрольКачестваКэш);
...
Процедура ЗаполнитьПлановыеКалькуляцииИЦеныНаСервере(ИдентификаторыСтрок = Неопределено, Калькуляция = Неопределено)
	...
	ВыделенныеСтроки = Элементы.ВыходныеИзделия.ВыделенныеСтроки;
	МассивСтрок = Новый Массив;
	Для Каждого ИдВыделеннойСтроки Из ВыделенныеСтроки Цикл
		СтрокаТовара = Объект.ВыходныеИзделия.НайтиПоИдентификатору(ИдВыделеннойСтроки);
...
&НаСервере
Функция ПараметрыФормыПомощникаВыполненияЗамен()

	ПараметрыОткрытия = Новый Структура;
	
	Строки = Новый Массив;
	Колонки = "НомерСтроки, Номенклатура, Характеристика, ХарактеристикиИспользуются, Назначение, НазначениеОбеспечения, Обособленно, ВариантОбеспечения, Склад, Подразделение, Спецификация, Количество";
	Для Каждого СтрокаТЧ Из Объект.ОбеспечениеМатериаламиИРаботами Цикл
			
		Если СтрокаТЧ.ЕстьАналогиМатериала
		   И Не СтрокаТЧ.Отменено
		   И Не СтрокаТЧ.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить Тогда
	...
	ТЗ = Объект.ОбеспечениеМатериаламиИРаботами.Выгрузить(Строки, Колонки);
	ТЗ.Колонки.Добавить("Ссылка", Новый ОписаниеТипов("ДокументСсылка.ЭтапПроизводства2_2"));
	ТЗ.Колонки.Добавить("Этап", Новый ОписаниеТипов("ДокументСсылка.ЭтапПроизводства2_2"));
	ТЗ.Колонки.Добавить("ЭтапРесурснойСпецификации", Новый ОписаниеТипов("СправочникСсылка.ЭтапыПроизводства"));
	ТЗ.Колонки.Добавить("НаправлениеДеятельности", Новый ОписаниеТипов("СправочникСсылка.НаправленияДеятельности"));
	ТЗ.Колонки.Добавить("ЗаказНаПроизводство", Новый ОписаниеТипов("ДокументСсылка.ЗаказНаПроизводство2_2"));
	ТЗ.Колонки.Добавить("Изделие", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));	
	ТЗ.Колонки.Добавить("ХарактеристикаИзделия", Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
	
	ТЗ.Колонки.Добавить("НомерСтрокиДокумент", ОбщегоНазначения.ОписаниеТипаЧисло(5));
	ТЗ.Колонки.Добавить("Отгружено", ОбщегоНазначения.ОписаниеТипаЧисло(15, 3));
	ТЗ.Колонки.Добавить("Обеспечено", ОбщегоНазначения.ОписаниеТипаЧисло(15, 3));
	ТЗ.Колонки.Добавить("Всего", ОбщегоНазначения.ОписаниеТипаЧисло(15, 3));
		
	Для каждого СтрокаТЗ Из ТЗ Цикл
		
		СтрокаТЗ.Ссылка = Объект.Ссылка;
		СтрокаТЗ.Этап = Объект.Ссылка;
		СтрокаТЗ.ЗаказНаПроизводство = ДанныеПартииПроизводства.Распоряжение;
		СтрокаТЗ.Спецификация = ДанныеПартииПроизводства.Спецификация;
		СтрокаТЗ.Изделие = ДанныеПартииПроизводства.Номенклатура;
		СтрокаТЗ.ХарактеристикаИзделия = ДанныеПартииПроизводства.Характеристика;
		СтрокаТЗ.ЭтапРесурснойСпецификации = Объект.Этап;
		СтрокаТЗ.НаправлениеДеятельности = Объект.НаправлениеДеятельности;
		СтрокаТЗ.Назначение = СтрокаТЗ.НазначениеОбеспечения;
		СтрокаТЗ.НомерСтрокиДокумент = СтрокаТЗ.НомерСтроки;
		СтрокаТЗ.Всего = СтрокаТЗ.Количество;
		
		Если СтрокаТЗ.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить Тогда
		
			СтрокаТЗ.Отгружено = СтрокаТЗ.Количество;
			СтрокаТЗ.Обеспечено = СтрокаТЗ.Количество;
			
		ИначеЕсли СтрокаТЗ.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.РезервироватьПоМереПоступления
			  ИЛИ СтрокаТЗ.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.СоСклада Тогда
	...
	ДополнительныеПараметрыАналогиМатериалов.ОтборСтрок = Новый Структура("Отменено", ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Ложь));
	...
	ДанныеТоваров = АналогиМатериалов.ПолучитьДанныеТоваров(
		ЭтотОбъект, "Объект", "ОбеспечениеМатериаламиИРаботами", 1, Истина, СтруктураПолей, ДополнительныеПараметрыАналогиМатериалов);
		
	ПараметрыОткрытия.Вставить("РаспределениеЗапасов", ДанныеТоваров.РаспределениеЗапасов);
	ПараметрыОткрытия.Вставить("Этап", Объект.Ссылка);
	ПараметрыОткрытия.Вставить("АдресТаблицыТоваров", ПоместитьВоВременноеХранилище(ТЗ));
...
&НаКлиенте
Процедура ПослеЗаменыЧерезПомощник(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат <> Неопределено Тогда
	
		ОбновитьДанныеПослеЗаменыЧерезПомощник(Результат);
...
&НаСервере
Процедура ОбновитьДанныеПослеЗаменыЧерезПомощник(Адрес)

	ТаблицаВыполненныхЗамен = ПолучитьИзВременногоХранилища(Адрес);
	УдалитьИзВременногоХранилища(Адрес);
	
	МассивВыполненныхЗамен = ТаблицаВыполненныхЗамен.НайтиСтроки(Новый Структура("ИзменилосьКоличество", Истина));
	ЕстьИзменения = МассивВыполненныхЗамен.Количество() > 0;
	
	Если НЕ ЕстьИзменения Тогда
		Возврат;
	КонецЕсли;
	
	СтруктураИзмененныхСтрок = Новый Структура;
	СтруктураИзмененныхСтрок.Вставить("ИндексыИзмененныхСтрок", Новый Массив);
	СтруктураИзмененныхСтрок.Вставить("ИндексыНовыхСтрок", Новый Массив);
	
	ПустаяУпаковка = Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка();
	
	СтрокиКУдалению = Новый Массив;
	
	ДанныеДоИзменения = Объект.ОбеспечениеМатериаламиИРаботами.Выгрузить(,"НомерСтроки, СтатьяКалькуляции");
	ДанныеДоИзменения.Индексы.Добавить("НомерСтроки");
	ПоискСтарыхДанных = Новый Структура("НомерСтроки");
	
	Для каждого СтрокаТаблицыЗамен Из МассивВыполненныхЗамен Цикл
	
		Если СтрокаТаблицыЗамен.ЭтоАналог Тогда
			
			НоваяСтрока = Объект.ОбеспечениеМатериаламиИРаботами.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицыЗамен);
			НоваяСтрока.КоличествоУпаковок = НоваяСтрока.Количество;
			НоваяСтрока.Обособленно = ЗначениеЗаполнено(СтрокаТаблицыЗамен.Назначение);
			НоваяСтрока.НазначениеОбеспечения = НоваяСтрока.Назначение;
			СтруктураИзмененныхСтрок.ИндексыНовыхСтрок.Добавить(НоваяСтрока.НомерСтроки - 1);
			
			Если СтрокаТаблицыЗамен.НомерСтрокиДокумент >= 0 Тогда
			
				ПоискСтарыхДанных.НомерСтроки = СтрокаТаблицыЗамен.НомерСтрокиДокумент;
				МассивСтарыхДанных = ДанныеДоИзменения.НайтиСтроки(ПоискСтарыхДанных);
				Для Каждого СтрокаСтарыхДанных Из МассивСтарыхДанных Цикл
					НоваяСтрока.СтатьяКалькуляции = СтрокаСтарыхДанных.СтатьяКалькуляции;
				КонецЦикла;
		...
		Иначе
			...
			СтрокаДокумента = Объект.ОбеспечениеМатериаламиИРаботами[СтрокаТаблицыЗамен.НомерСтрокиДокумент - 1];
			СтрокаДокумента.Количество = СтрокаТаблицыЗамен.Количество;
			СтрокаДокумента.КоличествоУпаковок = СтрокаТаблицыЗамен.Количество;
			СтрокаДокумента.Упаковка = ПустаяУпаковка;
			
			Если СтрокаДокумента.Количество = 0 Тогда
				СтрокиКУдалению.Добавить(СтрокаДокумента);
			КонецЕсли;
			
			СтруктураИзмененныхСтрок.ИндексыИзмененныхСтрок.Добавить(СтрокаТаблицыЗамен.НомерСтрокиДокумент - 1);
	...
	Документы.ЭтапПроизводства2_2.ПослеЗаменыНаАналоги(
		ЭтотОбъект,
		ИмяТаблицы,
		СтруктураИзмененныхСтрок,
		СтруктураДействий,
		ИдентификаторыСтрок);
	
	ПослеИзмененияСтрокТЧПрограммноНаСервере(ИмяТаблицы, ИдентификаторыСтрок, СтруктураДействий);
	
	ОбщегоНазначенияУТ.УдалитьСтрокиТаблицы(Объект.ОбеспечениеМатериаламиИРаботами, СтрокиКУдалению);

КонецПроцедуры

&НаКлиенте
Процедура ОткрытьСостояниеОбеспеченияЗаказов()
	...
	ПараметрыФормы.ЭтапДинамическойСтруктурыЗаказа = Объект.ДинамическаяСтруктура;
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОтразитьАналогиВГиперссылкеСостоянияОбеспечения(Форма)
	
	Если Форма.ПравоЧтенияАналогов Тогда
	
		Объект = Форма.Объект;
		Элементы = Форма.Элементы;
		ЕстьАналоги = Форма.ЕстьАналоги;
		ПодсказкаДоступностьАналогов = НСтр("ru = ' (доступна замена на аналоги)';
											|en = ' (replacement with substitutes is available)'");
		
		Если Объект.ДинамическаяСтруктура Тогда
			
			ЭлементФормы = Элементы.СостояниеОбеспеченияДинамическаяСтруктура;
		...
		Иначе
			...
			ЭлементФормы = Элементы[ИменаЭлементов[СвойстваГиперссылки.ИмяГиперссылки]];
		
		КонецЕсли;
		
		ЗаголовокГиперссылки = ЭлементФормы.Заголовок;
		ЗаголовокГиперссылки = СтрЗаменить(ЗаголовокГиперссылки, ПодсказкаДоступностьАналогов, "");
		
		Если ЕстьАналоги Тогда
			МассивТекстовПодсказки = Новый Массив;
			МассивТекстовПодсказки.Добавить(ЗаголовокГиперссылки);
			МассивТекстовПодсказки.Добавить(ПодсказкаДоступностьАналогов);
			ЗаголовокГиперссылки = СтрСоединить(МассивТекстовПодсказки);
		КонецЕсли;
		
		ЭлементФормы.Заголовок = ЗаголовокГиперссылки;
...
&НаСервере
Процедура НастроитьИнформациюПоЗаказу()

	СтатусЗаказа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Распоряжение, "Статус");
	
	Если СтатусЗаказа = Перечисления.СтатусыЗаказовНаПроизводство2_2.Закрыт Тогда
		Элементы.ТекстИнформация.Заголовок = НСтр("ru = 'Заказ на производство закрыт.';
													|en = 'The production order is closed.'");
