	...
	АвансовыеСчетаФактуры =
		...
		И ЗначениеЗаполнено(ДанныеПечати.ПредставлениеАвансовыеСчетаФактуры),
			...
			НСтр("ru = '№ -- от --, исправление № -- от --';
				|en = 'No. -- dated --, correction No. -- dated --'"));
...
Процедура ЗаполнитьТабличныйДокументСчетФактура(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ОпцииПечатиСчетаФактуры) Экспорт
	...
	//++ НЕ УТ
	ЕстьАкцизы = Ложь;
	ЕстьКолонкаАкциз = Ложь;
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодакцизныеТовары")
		И ДанныеДляПечати.РезультатПоТабличнойЧасти.Колонки.Найти("Акциз") <> Неопределено Тогда
		ЕстьКолонкаАкциз = Истина;
	КонецЕсли;
	//-- НЕ УТ
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Иначе
			...
			//++ НЕ УТ
			Если ЕстьКолонкаАкциз И ЗначениеЗаполнено(СтрокаТовары.Акциз) Тогда
				ЕстьАкцизы = Истина;
			КонецЕсли;
			//-- НЕ УТ
	...
	//++ НЕ УТ
	Если ЕстьАкцизы И ВерсияПостановления1137 >= 7 И Не ИспользоватьПечатныеФормыСФ2_5 Тогда
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщенияЕстьАкцизы());
	КонецЕсли;
	//-- НЕ УТ
...
Процедура ЗаполнитьРеквизитыСтрокиТовара2_5(СтрокаТовары, ПрослеживаемыеТовары, ОбластьМакета, НомерСтроки, ПараметрыПечати, ВыводитьКодНоменклатуры = Истина, ВыводитьКодТНВД = Неопределено, ВыводитьСерии = Ложь)
	...
	Если ПараметрыПечати.ПечатьСчетаФактуры Тогда
		ТекстБезАкциза = НСтр("ru = 'без акциза';
								|en = 'no excise'", Метаданные.Языки.Русский.КодЯзыка);
	Иначе
		ТекстБезАкциза = НСтр("ru = 'без
							|акциза';
							|en = 'without
							|excise'", Метаданные.Языки.Русский.КодЯзыка);
	КонецЕсли;
...
Процедура ЗаполнитьТабличныйДокументУПД(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати) Экспорт
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		ИначеЕсли (ЕстьПостановление1137_625 Или ЕстьПостановление1137 Или ЕстьПостановление981 Или ЕстьПостановление534 Или ЕстьПостановление1096)
			...
			|en = 'Cannot simultaneously print universal transfer documents
			|generated both before and after the commencement date of the Decree of the Russian Federation Government No. -- dated 01/01/2026.'");
		...
		ИначеЕсли (ЕстьПостановление1137_625 Или ЕстьПостановление1137 Или ЕстьПостановление981 Или ЕстьПостановление534 Или ЕстьПостановление1096 Или ЕстьПостановление2026)
			...
			|en = 'Cannot simultaneously print universal transfer documents
			|generated both before and after the commencement date of the Decree of the Russian Federation Government No. 26 dated 01/23/2026.'");
...
Функция ТекстСообщенияЕстьАкцизы()
	
	Возврат НСтр("ru = 'Для заполнения суммы акциза включите опцию ""Использовать печатные формы СФ, УПД, УКД 2.5"" (раздел ""НСИ и администрирование"" - ""Учет НДС, импорт и экспорт"").';
				|en = 'To fill in the excise amount, enable the ""Use print forms of the tax invoice, UTD, and UAD 2.5"" checkbox in Master data and settings > VAT accounting, import and export.'");
...
Функция ТекстСообщенияПостановление2026()
	...
	Возврат НСтр("ru = 'Для заполнения строки 5б включите опцию ""Использовать печатные формы СФ, УПД, УКД 2.5"" (раздел ""НСИ и администрирование"" - ""Учет НДС, импорт и экспорт"").';
				|en = 'To fill in line 5b, enable the ""Use print forms of the tax invoice, UTD, and UAD 2.5"" checkbox in Master data and settings > VAT accounting, import and export.'");
