...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ПодразделениеОтбор", Подразделение)  Тогда
		НастроитьПоПодразделению(ЭтотОбъект);
	КонецЕсли;
	
	Этап = Неопределено;
	Если Параметры.Свойство("ЭтапОтбор", Этап) Тогда
		НастроитьОтборПоЭтапу(Этап);
	КонецЕсли;
...
Процедура ПередЗагрузкойДанныхИзНастроекНаСервере(Настройки)
	Если ЗначениеЗаполнено(Подразделение)
		И Настройки.Получить("Подразделение") <> Неопределено Тогда
...
Процедура НастроитьОтборПоЭтапу(Этап)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		Список, "Этап", Этап, ВидСравненияКомпоновкиДанных.Равно,, ЗначениеЗаполнено(Этап));
